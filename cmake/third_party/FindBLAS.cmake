...
# ::
#
# BLAS_FOUND - set to true if a library implementing the BLAS interface is found BLAS_LINKER_FLAGS -
# uncached list of required linker flags (excluding -l and -L). BLAS_LIBRARIES - uncached list of
# libraries (using full path name) to link against to use BLAS BLAS95_LIBRARIES - uncached list of
# libraries (using full path name) to link against to use BLAS95 interface BLAS95_FOUND - set to
# true if a library implementing the BLAS f95 interface is found BLA_STATIC  if set on this
# determines what kind of linkage we do (static) BLA_VENDOR  if set checks only the specified
# vendor, if not set checks all the possibilities BLA_F95     if set on tries to find the f95
# interfaces for BLAS/LAPACK
#
# ######### ## List of vendors (BLA_VENDOR) valid in this module # Goto,OpenBLAS,ATLAS
# PhiPACK,CXML,DXML,SunPerf,SCSL,SGIMATH,IBMESSL, Intel10_32 (intel mkl v10 32 bit),Intel10_64lp
# (intel mkl v10 64 bit, lp thread model, lp64 model), # Intel10_64lp_seq (intel mkl v10 64
# bit,sequential code, lp64 model), # Intel( older versions of mkl 32 and 64 bit),
...
macro(
  ...
  _list
  _thread)
  # This macro checks for the existence of the combination of fortran libraries given by _list.  If
  # the combination is found, this macro checks (using the Check_Fortran_Function_Exists macro)
  # whether can link against that library combination using the name of a routine given by _name
  # using the linker flags given by _flags.  If the combination of libraries is found and passes the
  # link test, LIBRARIES is set to the list of complete library paths that have been found.
  # Otherwise, LIBRARIES is set to FALSE.

  # N.B. _prefix is the prefix applied to the names of all cached variables that are generated
  ...
  if(((BLA_VENDOR STREQUAL "ACML") AND (NOT BLAS_ACML_LIB_DIRS))
    ...
    if(_ACML_ROOT)
      ...
      elseif(CMAKE_Fortran_COMPILER_ID STREQUAL "Open64")
        # 32 bit builds not supported on Open64 but for code simplicity We'll just use the same
        # directory twice
