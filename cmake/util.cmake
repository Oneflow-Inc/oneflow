...
macro(write_file_if_different file_path content)
  if(EXISTS ${file_path})
    ...
    # NOTE: it seems a cmake bug that "content" in this macro is not treated as a variable
...
function(target_try_compile_option target flag)
  # We cannot check for -Wno-foo as this won't throw a warning so we must check for the -Wfoo option
  # directly
...
function(target_treat_warnings_as_errors target)
  if(TREAT_WARNINGS_AS_ERRORS)
    ...
    # disable unused-* for different compile mode (maybe unused in cpu.cmake, but used in
    # cuda.cmake)
    ...
    # there is some strict-overflow warnings in oneflow/user/kernels/ctc_loss_kernel_util.cpp for
    # unknown reason, disable them for now
...
function(set_compile_options_to_oneflow_target target)
  ...
  # the mangled name between `struct X` and `class X` is different in MSVC ABI, remove it while
  # windows is supported (in MSVC/cl or clang-cl)
  ...
  if(BUILD_CUDA)
    ...
    elseif("${CMAKE_CUDA_COMPILER_ID}" STREQUAL "Clang")
      target_compile_options(
        ...
        PRIVATE $<$<COMPILE_LANGUAGE:CUDA>:
                -Werror=return-type;
                # Suppress warning from cub library -- marking as system header seems not working
                # for .cuh files
                -Wno-pass-failed;
                >)
    else()
