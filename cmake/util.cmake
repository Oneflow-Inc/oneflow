...
macro(copy_all_files_in_dir source_dir dest_dir target)
  ...
  if (rsync)
    add_custom_command(TARGET ${target} POST_BUILD
      ...
      ARGS --omit-dir-times --no-perms --no-owner --no-group -a --inplace ${source_dir}/ ${dest_dir})
  else()
    add_custom_command(TARGET ${target} POST_BUILD
...
macro(copy_files file_paths source_dir dest_dir target)
  ...
  if (rsync)
    ...
    add_custom_command(TARGET ${target} POST_BUILD
      COMMAND ${rsync}
      ARGS --omit-dir-times --no-perms --no-owner --no-group -a --inplace --files-from=${CACHE_FILELIST} ${source_dir} ${dest_dir})
  else()
    foreach(file ${file_paths})
      file(RELATIVE_PATH rel_path "${source_dir}" ${file})
      add_custom_command(TARGET ${target} POST_BUILD
