...
macro(write_file_if_different file_path content)
  if(EXISTS ${file_path})
    ...
    # NOTE: it seems a cmake bug that "content" in this macro is not treated as
    # a variable
...
macro(copy_all_files_in_dir source_dir dest_dir target)
  ...
  if(rsync)
    add_custom_command(
      ...
      COMMAND
        ${rsync}
        # NOTE: the trailing slash of source_dir is needed. Reference:
        # https://stackoverflow.com/a/56627246
        ARGS -a --omit-dir-times --no-perms --no-owner --no-group --inplace
        ${source_dir}/ ${dest_dir})
  else()
    add_custom_command(
      TARGET ${target}
...
macro(copy_files file_paths source_dir dest_dir target)
  ...
  if(rsync)
    set(CACHE_FILELIST
        ${PROJECT_BINARY_DIR}/cached_filename_lists/cache_${_COUNTER})
    ...
    add_custom_command(
      ...
      POST_BUILD
      COMMAND
        ${rsync} ARGS -a --omit-dir-times --no-perms --no-owner --no-group
        --inplace --files-from=${CACHE_FILELIST} ${source_dir} ${dest_dir})
  else()
    foreach(file ${file_paths})
      file(RELATIVE_PATH rel_path "${source_dir}" ${file})
      add_custom_command(
...
function(add_copy_headers_target)
  ...
  add_custom_target(
    "${PARSED_ARGS_NAME}_create_header_dir"
  ...
  foreach(header_file ${headers})
    ...
    add_custom_command(
      TARGET "${PARSED_ARGS_NAME}_copy_headers_to_destination"
      PRE_BUILD
  ...
  foreach(header_file ${inventory_headers})
    add_custom_command(
      ...
      COMMAND
        ...
        "${PARSED_ARGS_SRC}/${header_file}" "${PARSED_ARGS_DST}/${header_file}")
  endforeach()
endfunction()
...
function(use_mirror)
  ...
  if(DEFINED THIRD_PARTY_MIRROR)
    if(THIRD_PARTY_MIRROR STREQUAL "aliyun")
      ...
      string(REPLACE "https://" ${ALIYUN_URL_PREFIX} MIRRORED_URL
                     ${PARSED_ARGS_URL})
      set(${PARSED_ARGS_VARIABLE}
          ${MIRRORED_URL}
          PARENT_SCOPE)
      message(
        NOTICE
        "-- fetch ${PARSED_ARGS_VARIABLE} using aliyun mirror ${MIRRORED_URL}")
...
function(check_cxx11_abi OUTPUT_VAR)
  ...
  if(NOT COMPILE_SUCCESS)
    message(
      FATAL_ERROR
        "Detecting cxx11 availability failed. Please report to OneFlow developers."
    )
  endif()
  execute_process(
  ...
  execute_process(COMMAND rm ${CMAKE_CURRENT_BINARY_DIR}/temp
                          ${CMAKE_CURRENT_BINARY_DIR}/temp.cpp)
  set(${OUTPUT_VAR}
      ${CXX11_ABI_AVAILABLE}
      PARENT_SCOPE)
...
function(target_try_compile_option target flag)
  # We cannot check for -Wno-foo as this won't throw a warning so we must check
  # for the -Wfoo option directly
  ...
  # Avoid double checks. A compiler will not magically support a flag it did not
  # before
    ...
    if(BUILD_CUDA)
      if("${CMAKE_CXX_COMPILER_ID}" STREQUAL "Clang"
        ...
        target_compile_options(${target}
                               PRIVATE $<$<COMPILE_LANGUAGE:CUDA>:${flag}>)
      endif()
    endif()
  endif()
endfunction()
...
function(target_treat_warnings_as_errors target)
  if(TREAT_WARNINGS_AS_ERRORS)
    ...
    if(BUILD_CUDA)
      # Only pass flags when cuda compiler is Clang because cmake handles
      # -Xcompiler incorrectly
    ...
    # disable unused-* for different compile mode (maybe unused in cpu.cmake,
    # but used in cuda.cmake)
    ...
    # there is some strict-overflow warnings in
    # oneflow/user/kernels/ctc_loss_kernel_util.cpp for unknown reason, disable
    # them for now
    ...
    # disable visibility warnings related to
    # https://github.com/Oneflow-Inc/oneflow/pull/3676.
...
function(set_compile_options_to_oneflow_target target)
  ...
  target_compile_options(${target}
                         PRIVATE $<$<COMPILE_LANGUAGE:CXX>:-Werror=return-type>)
  # the mangled name between `struct X` and `class X` is different in MSVC ABI,
  # remove it while windows is supported (in MSVC/cl or clang-cl)
  ...
  if(BUILD_CUDA)
    ...
    elseif("${CMAKE_CUDA_COMPILER_ID}" STREQUAL "Clang")
      target_compile_options(
        ...
        PRIVATE $<$<COMPILE_LANGUAGE:CUDA>:
                -Werror=return-type;
                # Suppress warning from cub library -- marking as system header
                # seems not working for .cuh files
                -Wno-pass-failed;
                >)
    else()
    ...
    target_compile_definitions(
      ${target} PRIVATE $<$<COMPILE_LANGUAGE:CUDA>:
...
function(checkDirAndAppendSlash)
  ...
  set(prefix ARG)
  cmake_parse_arguments(PARSE_ARGV 0 ${prefix} "${noValues}" "${singleValues}"
                        "${multiValues}")
