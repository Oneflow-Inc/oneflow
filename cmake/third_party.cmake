...
set_mirror_url_with_hash(
  INJA_URL https://github.com/pantor/inja/archive/refs/tags/v3.3.0.zip
...
if(BUILD_CUDA)
  ...
  if(DEFINED CUDA_TOOLKIT_ROOT_DIR)
    message(
      WARNING
        "CUDA_TOOLKIT_ROOT_DIR is deprecated, use CUDAToolkit_ROOT instead")
  ...
  if(CUDA_NVCC_GENCODES)
    message(
      FATAL_ERROR
        "CUDA_NVCC_GENCODES is deprecated, use CMAKE_CUDA_ARCHITECTURES instead"
    )
  endif()
  add_definitions(-DWITH_CUDA)
  # NOTE: For some unknown reason, CUDAToolkit_VERSION may become empty when
  # running cmake again
  set(CUDA_VERSION
      ${CUDAToolkit_VERSION}
      CACHE STRING "")
...
if(NOT WIN32)
  ...
  set(BLAS_LIBRARIES
      ${MKL_LIB_PATH}/mkl_core_dll.lib ${MKL_LIB_PATH}/mkl_sequential_dll.lib
...
if(WITH_ONEDNN)
  set(oneflow_third_party_libs ${oneflow_third_party_libs}
...
if(BUILD_CUDA)
  ...
  list(APPEND ONEFLOW_THIRD_PARTY_INCLUDE_DIRS ${CUDNN_INCLUDE_DIRS}
       ${CUB_INCLUDE_DIR} ${NCCL_INCLUDE_DIR})
endif()
...
foreach(oneflow_third_party_lib IN LISTS oneflow_third_party_libs)
  if(NOT "${oneflow_third_party_lib}" MATCHES "^-l.+"
    ...
    message(
      STATUS
        "Creating imported lib: ${oneflow_third_party_lib} => ${IMPORTED_LIB_NAME}"
    )
    add_library(${IMPORTED_LIB_NAME} UNKNOWN IMPORTED)
    set_property(TARGET ${IMPORTED_LIB_NAME}
                 PROPERTY IMPORTED_LOCATION "${oneflow_third_party_lib}")
...
if(THIRD_PARTY)
  ...
  if(BUILD_PYTHON)
    ...
    get_filename_component(ONEFLOW_INCLUDE_DIR_PARENT "${ONEFLOW_INCLUDE_DIR}"
                           DIRECTORY)
    foreach(of_include_src_dir ${ONEFLOW_THIRD_PARTY_INCLUDE_DIRS})
      if(of_include_src_dir MATCHES "/include$")
        # it requires two slashes, but in CMake doc it states only one slash is
        # needed
