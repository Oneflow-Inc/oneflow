...
file(GLOB_RECURSE oneflow_all_hdr_to_be_expanded
     "${PROJECT_SOURCE_DIR}/oneflow/core/*.e.h"
     "${PROJECT_SOURCE_DIR}/oneflow/python/*.e.h")
foreach(oneflow_hdr_to_be_expanded ${oneflow_all_hdr_to_be_expanded})
  file(RELATIVE_PATH of_ehdr_rel_path ${PROJECT_SOURCE_DIR}
       ${oneflow_hdr_to_be_expanded})
  ...
  if(WIN32)
    ...
    add_custom_command(
      ...
      COMMAND
        ...
        -I"${PROJECT_BINARY_DIR}" -o "${of_e_h_expanded}"
        "${oneflow_hdr_to_be_expanded}"
  ...
  set_source_files_properties(${oneflow_all_hdr_expanded} PROPERTIES GENERATED
                                                                     TRUE)
endforeach()
...
if(WITH_XLA
  ...
  if(NOT WITH_XLA)
    file(GLOB_RECURSE xla_removing_src
         "${PROJECT_SOURCE_DIR}/oneflow/xrt/xla/*.*")
  endif()
  if(NOT WITH_TENSORRT)
    file(GLOB_RECURSE trt_removing_src
         "${PROJECT_SOURCE_DIR}/oneflow/xrt/tensorrt/*.*")
  endif()
  if(NOT WITH_OPENVINO)
    file(GLOB_RECURSE openvino_removing_src
         "${PROJECT_SOURCE_DIR}/oneflow/xrt/openvino/*.*")
  endif()
...
foreach(oneflow_single_file ${oneflow_all_src})
  ...
  foreach(oneflow_platform_exclude ${oneflow_platform_excludes})
    string(FIND ${oneflow_single_file} ${oneflow_platform_exclude}
                platform_found)
    if(NOT ${platform_found} EQUAL -1) # the ${oneflow_single_file} is for other
                                       # platforms
      set(exclude_this ON)
    endif()
  endforeach()
  ...
  if("${oneflow_single_file}" MATCHES
    ...
    if((NOT RPC_BACKEND MATCHES "GRPC")
       AND "${oneflow_single_file}" MATCHES
  ...
  if("${oneflow_single_file}" MATCHES
     "^${PROJECT_SOURCE_DIR}/oneflow/(core|user|xrt)/.*\\.proto$")
    list(APPEND of_all_proto ${oneflow_single_file})
    # list(APPEND of_all_obj_cc ${oneflow_single_file})   # include the proto
    # file in the project
    set(group_this ON)
  endif()
  ...
  if(BUILD_PYTHON)
    ...
    if("${oneflow_single_file}" MATCHES
       "^${PROJECT_SOURCE_DIR}/oneflow/api/python/.*\\.(h|cpp)$")
  ...
  if("${oneflow_single_file}" MATCHES
    ...
    elseif(APPLE AND "${oneflow_single_file}" MATCHES
      ...
      # skip if macOS
    elseif((NOT RPC_BACKEND MATCHES "GRPC")
           AND "${oneflow_single_file}" MATCHES
               "^${PROJECT_SOURCE_DIR}/oneflow/core/control.*")
      # skip if GRPC not enabled
    else()
  ...
  if(group_this)
    file(RELATIVE_PATH oneflow_relative_file
         ${PROJECT_SOURCE_DIR}/oneflow/core/ ${oneflow_single_file})
...
add_custom_target(
  ...
  COMMAND
    ${Python_EXECUTABLE}
    ${CMAKE_CURRENT_SOURCE_DIR}/ci/check/run_license_format.py -i
    ${CMAKE_CURRENT_SOURCE_DIR}/oneflow --fix
  COMMAND
    ${Python_EXECUTABLE}
    ${CMAKE_CURRENT_SOURCE_DIR}/ci/check/run_license_format.py -i
    ${ONEFLOW_PYTHON_DIR} --fix --exclude="oneflow/include"
    --exclude="oneflow/core"
  COMMAND
    ${Python_EXECUTABLE}
    ${CMAKE_CURRENT_SOURCE_DIR}/ci/check/run_clang_format.py --source_dir
    ${CMAKE_CURRENT_SOURCE_DIR}/oneflow --fix --quiet
  COMMAND
    ${Python_EXECUTABLE} ${CMAKE_CURRENT_SOURCE_DIR}/ci/check/run_py_format.py
    --source_dir ${CMAKE_CURRENT_SOURCE_DIR}/python --fix
  COMMAND
    ${Python_EXECUTABLE}
    ${CMAKE_CURRENT_SOURCE_DIR}/ci/check/run_clang_format.py --source_dir
    ${CMAKE_CURRENT_SOURCE_DIR}/tools/oneflow-tblgen --fix --quiet)
# clang tidy
add_custom_target(
  of_tidy
  COMMAND ${Python_EXECUTABLE} ${CMAKE_SOURCE_DIR}/ci/check/run_clang_tidy.py
...
add_custom_target(
  of_git_version_create_dir COMMAND ${CMAKE_COMMAND} -E make_directory
                                    ${OF_GIT_VERSION_DIR})
add_custom_command(
  OUTPUT ${OF_GIT_VERSION_DUMMY_FILE}
  COMMAND
    ${CMAKE_COMMAND} -DOF_GIT_VERSION_FILE=${OF_GIT_VERSION_FILE}
    -DOF_GIT_VERSION_ROOT=${PROJECT_SOURCE_DIR}
    -DBUILD_GIT_VERSION=${BUILD_GIT_VERSION} -P
...
# proto obj lib
add_custom_target(
  make_pyproto_dir ALL COMMAND ${CMAKE_COMMAND} -E make_directory
                               ${of_proto_python_dir})
...
relative_protobuf_generate_cpp(PROTO_SRCS PROTO_HDRS ${PROJECT_SOURCE_DIR}
                               ${of_all_rel_protos})
...
generate_cfg_and_pybind11_cpp(CFG_SRCS CFG_HRCS CFG_PYBIND11_SRCS
                              ${PROJECT_SOURCE_DIR})
...
if(BUILD_SHARED_LIBS)
  ...
  # For some unknown reasons, when building static libraries, we have to link
  # of_protoobj and of_cfgobj with oneflow_third_party_libs
...
generate_functional_api_and_pybind11_cpp(
  FUNCTIONAL_GENERATED_SRCS FUNCTIONAL_GENERATED_HRCS FUNCTIONAL_PYBIND11_SRCS
  ${PROJECT_SOURCE_DIR})
...
if(BUILD_PYTHON)
  ...
  target_include_directories(
    of_functional_tensor_obj PRIVATE ${Python_INCLUDE_DIRS}
                                     ${Python_NumPy_INCLUDE_DIRS})

  set(PYBIND11_SRCS
      ${CFG_PYBIND11_SRCS} ${FUNCTIONAL_PYBIND11_SRCS}
...
set(LLVM_MONO_REPO_MD5
    "9bda804e5cc61899085fb0f0dce1089f"
    CACHE STRING "")
...
if(BUILD_PYTHON)
  ...
  pybind11_add_module(oneflow_internal ${PYBIND11_SRCS} ${of_pybind_obj_cc}
                      ${PYBIND_REGISTRY_CC})
  ...
  add_dependencies(oneflow_internal of_cfgobj of_functional_obj
                   of_functional_tensor_obj of_op_schema)
  set_target_properties(oneflow_internal PROPERTIES PREFIX "_")
  set_target_properties(
    oneflow_internal PROPERTIES LIBRARY_OUTPUT_DIRECTORY
                                "${ONEFLOW_PYTHON_DIR}/oneflow")
  target_link_libraries(
    oneflow_internal
    PRIVATE ${of_libs} of_functional_tensor_obj of_api_common
  ...
  add_custom_target(
    ...
    COMMAND ${CMAKE_COMMAND} -E touch
            "${of_proto_python_dir}/oneflow/core/__init__.py"
    COMMAND
      ${CMAKE_COMMAND} -E create_symlink "${of_proto_python_dir}/oneflow/core"
      "${ONEFLOW_PYTHON_DIR}/oneflow/core"
    COMMAND
      ${Python_EXECUTABLE} ${PROJECT_SOURCE_DIR}/tools/generate_pip_version.py
...
if(BUILD_CPP_API)
  ...
  set_target_properties(
    oneflow_cpp PROPERTIES ARCHIVE_OUTPUT_DIRECTORY "${LIBONEFLOW_LIBRARY_DIR}"
                           LIBRARY_OUTPUT_DIRECTORY "${LIBONEFLOW_LIBRARY_DIR}")
  target_link_libraries(oneflow_cpp PRIVATE ${of_libs} of_api_common
                                            ${oneflow_third_party_libs})
endif()

file(RELATIVE_PATH PROJECT_BINARY_DIR_RELATIVE ${PROJECT_SOURCE_DIR}
     ${PROJECT_BINARY_DIR})
...
if(BUILD_TESTING)
  if(of_all_test_cc)
    oneflow_add_test(oneflow_testexe SRCS ${of_all_test_cc} TEST_NAME
                     oneflow_test)
    target_link_libraries(
      oneflow_testexe ${of_libs} ${oneflow_third_party_libs} glog::glog
      ${oneflow_test_libs})
  endif()

  if(BUILD_CPP_API)
    file(GLOB_RECURSE cpp_api_test_files
         ${PROJECT_SOURCE_DIR}/oneflow/api/cpp/tests/*.cpp)
    oneflow_add_test(
    ...
    target_link_libraries(oneflow_cpp_api_testexe oneflow_cpp
...
if(BUILD_CPP_API)
  ...
  checkdirandappendslash(DIR ${PROTOBUF_LIBRARY_DIR} OUTPUT
                         PROTOBUF_LIBRARY_DIR_APPENDED)
  list(APPEND LIBONEFLOW_THIRD_PARTY_DIRS ${PROTOBUF_LIBRARY_DIR_APPENDED})
  if(BUILD_CUDA)
    checkdirandappendslash(DIR ${NCCL_LIBRARY_DIR} OUTPUT
                           NCCL_LIBRARY_DIR_APPENDED)
  ...
  install(
    FILES ${PROJECT_SOURCE_DIR}/cmake/oneflow-config.cmake
    COMPONENT oneflow_cpp_all
    DESTINATION share)
  ...
  add_custom_target(
    ...
    COMMAND
      ...
      -DCMAKE_INSTALL_PREFIX="${LIBONEFLOW_DIR}" -P
      "${CMAKE_BINARY_DIR}/cmake_install.cmake"
    DEPENDS oneflow_cpp)
  if(BUILD_TESTING)
    add_dependencies(install_oneflow_cpp oneflow_cpp_api_testexe
                     oneflow_testexe)
  endif(BUILD_TESTING)
  ...
  set(CPACK_INSTALL_CMAKE_PROJECTS
      ${PROJECT_BINARY_DIR};oneflow;oneflow_cpp_all;/)
  include(CPack)
endif(BUILD_CPP_API)
