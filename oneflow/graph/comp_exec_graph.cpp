...
void CompExecGraph::FwBuildGraph() {
...
void CompExecGraph::FwBuildFromUserOps(
    Lbn2NodeMap* lbn2producer,
    Lbn2NodeVecMap* extern_in_lbn2consumers) {
  for (std::shared_ptr<const Operator> op : task_node()->chain_node()->op_vec()) {
    ExecNode* cur_node = NewFinalNode();
    cur_node->mut_op() = op;
  ...
  for (const std::unique_ptr<ExecNode>& cur_node : nodes()) {
...
void CompExecGraph::FwAddCopyInOp(Lbn2NodeVecMap* extern_in_lbn2consumers) {
  // If only DataOp
  ...
  ExecNode* copy_node = NewFinalNode();
  ...
  for (const auto& pair : *extern_in_lbn2consumers) {
    ...
    for (ExecNode* old_consumer : old_consumers) {
      Connect(copy_node, NewExecEdge(lbn), old_consumer);
    }
    extern_in_lbn2consumers->at(lbn) = {copy_node};
  }
}

void CompExecGraph::FwAddCloneOp() {
  struct CloneInfo {
    std::string lbn;
  ...
  for (const CloneInfo& clone_info : clone_info_vec) {
    ExecNode* clone_node = NewFinalNode();
    clone_node->mut_op() = clone_info.clone_op;
    // Update Edge
    Connect(clone_info.pred_node, NewExecEdge(clone_info.lbn), clone_node);
...
void CompExecGraph::FwSetRelatedTaskEdges(
...
void CompExecGraph::BpBuildGraph() {
  const ExecGraph* fw_graph = task_node()->GetFwNode()->exec_graph();
  ...
  for (const std::unique_ptr<ExecEdge>& fw_edge : fw_graph->edges()) {
    ...
    Connect(fw_node2bp_node.at(fw_edge->dst_node()),
            NewExecEdge(fw_edge->lbn()),
            fw_node2bp_node.at(fw_edge->src_node()));
  }
  // Set in TaskEdge
  for (const std::unique_ptr<ExecNode>& bp_node : nodes()) {
...
void CompExecGraph::SetupProducedRegisterDesc() {
...
void CompExecGraph::FwSetupProducedRegisterDesc() {
  std::unique_ptr<RegisterDesc> data_register(new DisContigRegistDesc);
  data_register->Init();
  // blobs not used by succ_task
  for (const std::unique_ptr<ExecEdge>& cur_edge : edges()) {
...
void CompExecGraph::BpSetupProducedRegisterDesc() {
