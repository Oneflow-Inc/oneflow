...
class MluReduce final : public Reduce {
 public:
  OF_DISALLOW_COPY_AND_MOVE(MluReduce);
  MluReduce() : cncl_datatype_(), cncl_reduce_op_() {}
  ~MluReduce() = default;
  ...
  void Launch(ep::Stream* stream, const void* in, void* out, size_t elem_cnt, int64_t root,
    ...
    OF_CNCL_CHECK(cnclReduce(in, out, elem_cnt, cncl_datatype_, cncl_reduce_op_,
...
REGISTER_COLLECTIVE_COMMUNICATION(DeviceType::kMLU, Reduce, MluReduce);
