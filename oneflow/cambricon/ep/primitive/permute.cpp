...
class PermuteImpl : public Permute {
 public:
  ...
  void Launch(Stream* stream, DataType data_type, size_t num_dims, const int64_t* src_dims,
    ...
    cnnlTransposeDescriptor_t tran_desc = nullptr;
    OF_CNNL_CHECK(cnnlCreateTransposeDescriptor(&tran_desc));
    OF_CNNL_CHECK(cnnlSetTransposeDescriptor(tran_desc, num_dims, permutation));
