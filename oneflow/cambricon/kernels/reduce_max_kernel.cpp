...
class ReduceKernel final : public user_op::OpKernel {
 ...
 private:
  void Compute(user_op::KernelComputeContext* ctx) const override {
    ...
    auto reduce_mode = CNNL_REDUCE_MAX;
...
#define REGISTER_REDUCE_MAX_KERNELS(device, dtype) \
  REGISTER_REDUCE_MLU_KERNEL("reduce_max", device, dtype)

#define REGISTER_REDUCE_MAX_KERNELS_BY_DEVICE(device) \
  REGISTER_REDUCE_MAX_KERNELS(device, float)          \
  REGISTER_REDUCE_MAX_KERNELS(device, float16)        \
  REGISTER_REDUCE_MAX_KERNELS(device, int32_t)
