 ...
 private:
  ...
  void Compute(user_op::KernelComputeContext* ctx) const override {
    ...
    if constexpr (std::is_same<T, float16>::value) {   
        bang_scalar_pow_gradient_half_kernel(handle, dx->shape_view().elem_cnt(), x->dptr<float16>(), dy->dptr<float16>(), value_t, dx->mut_dptr<float16>());
    }
    else{
        bang_scalar_pow_gradient_kernel(handle, dx->shape_view().elem_cnt(), x->dptr<float>(), dy->dptr<float>(), value_t, dx->mut_dptr<float>());
    }
  }
