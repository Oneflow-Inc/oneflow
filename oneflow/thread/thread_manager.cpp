...
void ThreadMgr::InitFromProto(const PbRpf<TaskProto>& tasks) {
  for (auto task : tasks) {
    ...
    if (thrd_loc_id2thread_.find(task.thrd_local_id()) == thrd_loc_id2thread_.end()) {
      auto thrd = of_make_unique<Thread>();
      thrd->set_thrd_loc_id(task.thrd_local_id());
      CHECK(thrd_loc_id2thread_.emplace(task.thrd_local_id(), std::move(thrd)).second);
    }
    auto& thrd = thrd_loc_id2thread_.at(task.thrd_local_id());
    thrd->AddActor(task);
  }
}
