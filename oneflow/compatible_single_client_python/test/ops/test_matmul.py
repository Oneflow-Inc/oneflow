...
def filter_args_v2(arg_dict_list):
    ...
    for arg_dict in arg_dict_list:
        ...
        # a_shape and b_shape don't match (m, k) * (k * n) -> (m, n)
        if a_shape[-1] != b_shape[-2]:
            continue

        # float16 and tf32 only be supported on gpu
        if arg_dict["device_type"] == "cpu" and (
            arg_dict["data_type"] == "float16" or arg_dict["enable_tf32"] is True
        ):
            continue

        # tf32 only work on data type float32
...
def get_lr_scheduler():
    return flow.optimizer.PiecewiseConstantScheduler([], [1e-4])
...
def compare_with_np(
    ...
    def print_dbg_info(lhs=None, rhs=None):
        print(
            f"a_shape: {a_shape}, b_shape: {b_shape}, transpose_a: {transpose_a}, transpose_b: {transpose_b}, alpha: {alpha}"
            f", data_type: {data_type}, device_type: {device_type}"
            f", test_add_to_output: {test_add_to_output}, fuse_add_to_output: {fuse_add_to_output}"
