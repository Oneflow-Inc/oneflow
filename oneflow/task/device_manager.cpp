...
namespace oneflow {
DeviceManager::DeviceManager(int32_t thread_local_id)
  : thread_local_id_(thread_local_id) {
...
void DeviceManager::AddTask(int32_t task_id,
  std::shared_ptr<Task> task) {
  CHECK_EQ(task_dict_.count(task_id), 0);
  task_dict_.insert({task_id, task});
}

std::shared_ptr<Task> DeviceManager::GetTask(
...
//DeviceManager::device_context() const {
//  CHECK(is_gpu_);
