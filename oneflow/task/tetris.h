...
class TetrisColumn {
 public:
  ...
  virtual void Push(int64_t element, int64_t element_id) = 0;
  virtual std::vector<std::pair<int64_t, int64_t>> Pop() = 0;
  int32_t col_id() const { return col_id_; }
 protected:
  int32_t col_id_;
};

class TetrisFIFOColumn :public TetrisColumn {
 public:
  ...
  void Push(int64_t element, int64_t element_id = -1) override;
  std::vector<std::pair<int64_t, int64_t>> Pop() override;
 private:
  std::queue<std::pair<int64_t, int64_t>> column_;
};

class TetrisIDColumn :public TetrisColumn {
 public:
  ...
  void Push(int64_t element, int64_t element_id) override;
  std::vector<std::pair<int64_t, int64_t>> Pop() override;
 ...
 private:
  ...
  std::unordered_map<int64_t, std::vector<int64_t>> column_;
  std::queue<int64_t> ready_ids_;
};

class TetrisSSPColumn :public TetrisColumn {
 public:
 ...
 private:
  // Data
  using DataPair = std::pair<int64_t, int64_t>;
  struct CmpGreater {
  ...
  using ModelCount = std::pair<int64_t, int32_t>;
  std::unordered_map<int64_t, ModelCount> model_;
  std::unordered_map<int64_t, int64_t> model_to_version_;
  int64_t newest_;

  int64_t task_id_;
  void erase_model(int64_t model);
};

class TetrisRefCountColumn :public TetrisColumn {
 public:
 ...
 private:
  uint32_t ref_num_;
  std::unordered_map<int64_t, uint32_t> ref_count_;
  std::queue<int64_t> avaliable_;
};
...
class Tetris {
 public:
  ...
  void Add(std::shared_ptr<TetrisColumn> child);
  void Push(int32_t col_id, int64_t element, int64_t element_id);
  bool Ready() const;
  std::unordered_map<int32_t, std::vector<std::pair<int64_t, int64_t>>> Pop();

 private:
  std::unordered_map<int32_t, std::shared_ptr<TetrisColumn>> columns_;
};
