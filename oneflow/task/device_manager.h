#ifndef ONEFLOW_TASK_DEVICE_MANAGER_H_
#define ONEFLOW_TASK_DEVICE_MANAGER_H_
#include <memory>
...
class DeviceManager {
 public:
  ...
  void Setup();
  void AddTask(int32_t task_id, std::shared_ptr<Task> task);
 ...
 private:
  ...
  bool is_gpu_{ false };
  // std::shared_ptr<DeviceContext> device_context_;  // For GPU device

  // task_id -> task
  std::unordered_map<int32_t, std::shared_ptr<Task>> task_dict_;

  DeviceManager(const DeviceManager& other) = delete;
  DeviceManager& operator=(const DeviceManager& other) = delete;
};
}  // namespace oneflow
#endif  // ONEFLOW_TASK_DEVICE_MANAGER_H_
