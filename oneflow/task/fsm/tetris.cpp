...
bool TetrisSSPColumn<T, Dtype>::Ready() const {
...
template <typename T, typename Dtype>
void TetrisSSPColumn<T, Dtype>::erase_model(T model) {
  ...
  auto& comm_bus = oneflow::TheOne<Dtype>::comm_bus();
  comm_bus->SendMessage(model_ack_msg);
}

template <typename T, typename Dtype>
void TetrisSSPColumn<T, Dtype>::Push(T element, int64_t element_id) {
  auto& node_manager = oneflow::TheOne<Dtype>::node_manager();
  bool is_model = node_manager->is_in_model_path(element);
  if (!is_model) {
...
template <typename T, typename Dtype>
std::vector<std::pair<T, int64_t>> TetrisSSPColumn<T, Dtype>::Pop() {
  CHECK(Ready());
  std::vector<std::pair<T, int64_t>> items;
  // Data
  auto data = data_.top();
...
template class TetrisSSPColumn<int64_t, float>;
template class TetrisSSPColumn<int64_t, double>;
