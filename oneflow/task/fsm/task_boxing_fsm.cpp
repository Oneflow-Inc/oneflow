...
TaskBoxingFSM<Dtype>::TaskBoxingFSM(Task<Dtype>* task) : TaskFSM<Dtype>(task) {
  /* src side uses TetrisIDColumn, dst side uses multiple
   * TetrisFIFOColumn (assume each dst register is consumed by
   * single consumer).
   */
  // config Tetris
  ...
  std::shared_ptr<TetrisIDColumn<int64_t>> source(new TetrisIDColumn<int64_t>(
    source_col_id_, consumed_group_ids_.size()));
  tetris_.Add(source);
...
template <typename Dtype>
TaskBoxingFSM<Dtype>::~TaskBoxingFSM() {}

template <typename Dtype>
TaskItem* TaskBoxingFSM<Dtype>::GetTaskItem() {
  // CHECK(HasAction());
  ...
  for (auto source : source_ids) {
    ...
    // Consumed msgs
    auto to_task_id = id_map->task_id_from_register_id(source);
    MsgPtr msg(new EventMessage(
      task_id, to_task_id, data_id, source, MessageType::kConsumed));
...
INSTANTIATE_CLASS(TaskBoxingFSM);
}  // namespace caffe
