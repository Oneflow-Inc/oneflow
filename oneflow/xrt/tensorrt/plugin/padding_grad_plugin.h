...
*/
#ifndef ONEFLOW_XRT_TENSORRT_PLUGIN_PADDING_GRAD_PLUGIN_H_
#define ONEFLOW_XRT_TENSORRT_PLUGIN_PADDING_GRAD_PLUGIN_H_
...
class PaddingGradPlugin : public TrtPlugin {
 public:
  PaddingGradPlugin(const std::string& name, const std::vector<int64_t>& padding_before,
                    const std::vector<int64_t>& padding_after)
      : name_(name), padding_before_(padding_before), padding_after_(padding_after) {}

  const char* getPluginType() const TRT_NOEXCEPT override { return "PaddingGrad"; }
 ...
 private:
  std::string name_;
  std::vector<int64_t> padding_before_;
  std::vector<int64_t> padding_after_;
};
...
#endif  // ONEFLOW_XRT_TENSORRT_PLUGIN_PADDING_GRAD_PLUGIN_H_
