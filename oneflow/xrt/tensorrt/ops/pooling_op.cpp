...
class PoolingOp : public TrtOpKernel {
 public:
  void Compile(TrtOpContext *ctx) override {
    ...
    auto *layer = ctx->builder()->addPooling(*in, pooling_type,
        nvinfer1::DimsHW(pool_size[0], pool_size[1]));
    layer->setStride(nvinfer1::DimsHW(strides[0], strides[1]));
    const std::string valid_str = "valid";
    if(strcasecmp(padding.c_str(), valid_str.c_str()) == 0)
      layer->setPaddingMode(nvinfer1::PaddingMode::kEXPLICIT_ROUND_DOWN);
    const std::string same_str = "same";
    if(strcasecmp(padding.c_str(), same_str.c_str()) == 0)
      layer->setPaddingMode(nvinfer1::PaddingMode::kSAME_LOWER);
    ctx->SetOutput("out", layer->getOutput(0));
  }
};

REGISTER_TRT_OP_KERNEL(MaxPooling2D, PoolingOp<nvinfer1::PoolingType::kMAX>).EnableTrainPhase().Finalize();
REGISTER_TRT_OP_KERNEL(AveragePooling2D, PoolingOp<nvinfer1::PoolingType::kAVERAGE>).EnableTrainPhase().Finalize();
