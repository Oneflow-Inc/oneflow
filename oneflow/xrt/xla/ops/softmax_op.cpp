...
void SoftmaxOp::Compile(XlaOpContext *ctx) {
  ...
  xla::XlaOp logits_max =
      xla::Reduce(input, MinValue(builder, data_type), max_func, {input_shape.NumAxes() - 1});
  // y = exp(x - max)
  ...
  xla::XlaOp sum = xla::Reduce(y, Zero(builder, data_type), add_func, {input_shape.NumAxes() - 1});
...
void SoftmaxGradOp::Compile(XlaOpContext *ctx) {
  ...
  xla::XlaComputation add_func = CreateAddFunc(data_type);
  xla::XlaOp sum = xla::Reduce(y * dy, Zero(builder, data_type), add_func, {y_shape.NumAxes() - 1});
