...
void SoftmaxOp::Compile(XlaOpContext *ctx) {
  ...
  for (int i = axis; i < input_shape.NumAxes() - 1; ++i) {
    batch_dims[i] = i + 1;
  }
  ...
  xla::XlaOp logits_max =
      xla::Reduce(input, MinValue(builder, data_type), max_func, {axis});
  // y = exp(x - max)
...
void SoftmaxGradOp::Compile(XlaOpContext *ctx) {
  ...
  for (int i = axis; i < y_shape.NumAxes() - 1; ++i) {
    batch_dims[i] = i + 1;
  }
  ...
  xla::XlaComputation add_func = CreateAddFunc(data_type);
  xla::XlaOp sum =
      xla::Reduce(y * dy, Zero(builder, data_type), add_func, {axis});
