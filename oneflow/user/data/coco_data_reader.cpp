...
COCODataReader::COCODataReader(user_op::KernelInitContext *ctx)
    : DataReader<COCOImage>(ctx) {
  std::shared_ptr<const COCOMeta> meta(
      new COCOMeta(ctx->Attr<std::string>("annotation_file"),
                   ctx->Attr<std::string>("image_dir"),
                   ctx->Attr<bool>("remove_images_without_annotations")));

  std::unique_ptr<RandomAccessDataset<COCOImage>> coco_dataset_ptr(
      new COCODataset(ctx, meta));
  loader_.reset(new DistributedTrainingDataset<COCOImage>(
      ctx->parallel_ctx().parallel_num(), ctx->parallel_ctx().parallel_id(),
      ctx->Attr<bool>("stride_partition"),
  ...
  if (ctx->Attr<bool>("group_by_ratio")) {
    ...
    loader_.reset(new GroupBatchDataset<COCOImage>(batch_size, GetGroupId,
                                                   std::move(loader_)));
  } else {
...
COCOMeta::COCOMeta(const std::string &annotation_file,
  ...
  if (remove_images_without_annotations) {
    ...
    image_ids_.erase(std::remove_if(image_ids_.begin(), image_ids_.end(),
                                    [&to_remove_image_ids](int64_t image_id) {
                                      return to_remove_image_ids.find(
                                                 image_id) !=
  ...
  for (int32_t category_id : category_ids) {
    CHECK(category_id2contiguous_id_.emplace(category_id, contiguous_id++)
              .second);
  }
}
...
bool COCOMeta::ImageHasValidAnnotations(int64_t image_id) const {
  ...
  for (int64_t anno_id : anno_id_vec) {
    ...
    if (anno["bbox"][2] > 1 && anno["bbox"][3] > 1) {
      bbox_area_all_close_to_zero = false;
    }
  ...
  if (bbox_area_all_close_to_zero) {
    return false;
  }
  // keypoints task have a slight different critera for considering
  // if an annotation is valid
  if (!anno_id2anno_.at(anno_id_vec.at(0)).contains("keypoints")) {
    return true;
  }
  // for keypoint detection tasks, only consider valid images those
  // containing at least min_keypoints_per_image
  if (visible_keypoints_count >= kMinKeypointsPerImage) {
    return true;
  }
  return false;
}
