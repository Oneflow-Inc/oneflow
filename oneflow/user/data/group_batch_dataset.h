...
public:
  ...
  GroupBatchDataset(
      ...
      : base_(std::move(dataset)), batch_size_(batch_size),
        group_fn_(GroupId4Sample), order_count_(0) {}
  ...
  LoadTargetShdPtrVec Next() override {
    ...
    while (ret.size() < batch_size_) {
      ...
      if (group_id == next_group_id) {
        ...
        if (group_it == group_id2buffered_samples_.end()) {
          group_it = group_id2buffered_samples_
                         .emplace(next_group_id, std::list<BatchSample>())
                         .first;
        }
        auto &batch_sample_list = group_it->second;
        if (batch_sample_list.empty() ||
            batch_sample_list.back().data.size() == batch_size_) {
          ...
          batch_sample_list.back().data.push_back(
              std::move(next_sample_vec[0]));
        }
      }
    }
    return ret;
