...
void COCOParser::Parse(std::shared_ptr<LoadTargetShdPtrVec> batch_data,
  ...
  user_op::Tensor *image_size_tensor =
      ctx->Tensor4ArgNameAndIndex("image_size", 0);
  ...
  user_op::Tensor *segm_index_tensor =
      ctx->Tensor4ArgNameAndIndex("gt_segm_index", 0);
  ...
  MultiThreadLoop(batch_data->size(), [&](size_t i) {
    ...
    if (bbox_tensor) {
      ...
      std::copy(bbox_vec.begin(), bbox_vec.end(),
                bbox_buffer->mut_data<float>());
    }
    if (label_tensor) {
      TensorBuffer *label_buffer = label_tensor->mut_dptr<TensorBuffer>() + i;
      const auto &label_vec = meta_->GetLabelVec<int32_t>(image->index);
      label_buffer->Resize(Shape({static_cast<int64_t>(label_vec.size())}),
                           DataType::kInt32);
      std::copy(label_vec.begin(), label_vec.end(),
                label_buffer->mut_data<int32_t>());
    }
    if (segm_tensor && segm_index_tensor) {
      TensorBuffer *segm_buffer = segm_tensor->mut_dptr<TensorBuffer>() + i;
      TensorBuffer *segm_index_buffer =
          segm_index_tensor->mut_dptr<TensorBuffer>() + i;
      meta_->ReadSegmentationsToTensorBuffer<float>(image->index, segm_buffer,
                                                    segm_index_buffer);
    }
  });
  // dynamic batch size
  ...
  if (image_size_tensor &&
      image_size_tensor->shape().At(0) != batch_data->size()) {
  ...
  if (segm_tensor && segm_index_tensor &&
      segm_tensor->shape().elem_cnt() != batch_data->size()) {
    CHECK_EQ(segm_tensor->shape().NumAxes(), 1);
    CHECK_EQ(segm_index_tensor->shape().NumAxes(), 1);
    CHECK_EQ(segm_tensor->shape().elem_cnt(),
             segm_index_tensor->shape().elem_cnt());
