...
public:
  ...
  OFRecordDataset(user_op::KernelInitContext *ctx) {
    ...
    in_stream_.reset(new PersistentInStream(
        DataFS(), local_file_paths, !shuffle_after_epoch_, save_to_local_));
  }
...
private:
  ...
  void ShuffleAfterEpoch() {
    ...
    std::vector<std::string> local_file_paths = GetLocalFilePaths();
    in_stream_.reset(new PersistentInStream(DataFS(), local_file_paths, false,
                                            save_to_local_));
  }
  ...
  std::vector<std::string> GetLocalFilePaths() {
    ...
    for (int i = range_.begin(); i < range_.end(); ++i) {
      ret.push_back(data_file_paths_.at(i));
    }
    return ret;
  }
