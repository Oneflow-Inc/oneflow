...
void DecodeImageFromOFRecord(const OFRecord &record,
  ...
  cv::Mat image = cv::imdecode(
      cv::Mat(1, src_data.size(), CV_8UC1, (void *)(src_data.data())),
      cv::IMREAD_COLOR);
  int W = image.cols;
  int H = image.rows;
...
void DecodeLabelFromFromOFRecord(const OFRecord &record,
                                 const std::string &feature_name,
                                 TensorBuffer *out) {
...
void LoadWorker(BaseDataset *record_dataset,
                std::vector<std::unique_ptr<Buffer<BaseLoadTargetPtr>>>
                    *decode_in_buffers) {
...
void DecodeWorker(
  ...
  while (true) {
    ...
    if (receive_status == kBufferStatusErrorClosed) {
      break;
    }
    ...
    DecodeImageFromOFRecord(record, image_feature_name, color_space,
                            instance->image.get());
    instance->label.reset(new TensorBuffer());
    DecodeLabelFromFromOFRecord(record, label_feature_name,
                                instance->label.get());
    auto send_status = out_buffer->Send(instance);
    if (send_status == kBufferStatusErrorClosed) {
      break;
    }
...
int32_t GetNumLocalDecodeThreads(int32_t num_decode_threads_per_machine,
  ...
  const int64_t machine_id =
      parallel_desc.MachineIdForParallelId(parallel_ctx.parallel_id());
  const int64_t parallel_num_on_this_machine =
      parallel_desc.sorted_dev_phy_ids(machine_id).size();
  return std::max<int32_t>(
      num_decode_threads_per_machine / parallel_num_on_this_machine, 1);
}

} // namespace

class OFRecordImageClassificationDataset final
    : public Dataset<ImageClassificationDataInstance> {
public:
  ...
  OFRecordImageClassificationDataset(user_op::KernelInitContext *ctx,
    ...
    const std::string &image_feature_name =
        ctx->Attr<std::string>("image_feature_name");
    const std::string &label_feature_name =
    ...
    const auto decode_buffer_size_per_thread =
        ctx->Attr<int32_t>("decode_buffer_size_per_thread");
    const int32_t num_local_decode_threads =
        GetNumLocalDecodeThreads(num_decode_threads_per_machine,
    ...
    for (int64_t i = 0; i < num_local_decode_threads; ++i) {
      decode_in_buffers_.at(i).reset(
          new Buffer<BaseLoadTargetPtr>(decode_buffer_size_per_thread));
      decode_out_buffers_.at(i).reset(
          new Buffer<LoadTargetPtr>(decode_buffer_size_per_thread));
      decode_threads_.emplace_back(std::thread(
          &DecodeWorker, image_feature_name, label_feature_name, color_space,
  ...
  ~OFRecordImageClassificationDataset() override {
    for (auto &out_buffer : decode_out_buffers_) {
      out_buffer->Close();
    }
    for (auto &in_buffer : decode_in_buffers_) {
      in_buffer->Close();
    }
    load_thread_.join();
    for (auto &decode_thread : decode_threads_) {
      decode_thread.join();
    }
  }
  ...
  LoadTargetPtrList Next() override {
    ...
    size_t thread_idx =
        out_thread_idx_.fetch_add(1, std::memory_order_relaxed) %
        decode_out_buffers_.size();
