...
*/
#ifndef ONEFLOW_USER_DATA_TFRECORD_DATASET_H_
#define ONEFLOW_USER_DATA_TFRECORD_DATASET_H_
...
class TFRecordDataset final : public Dataset<TensorBuffer> {
 public:
  using LoadTargetPtr = std::shared_ptr<TensorBuffer>;
  using LoadTargetPtrList = std::vector<LoadTargetPtr>;
  OF_DISALLOW_COPY_AND_MOVE(TFRecordDataset);
  TFRecordDataset(user_op::KernelInitContext* ctx) {
    current_epoch_ = 0;
 ...
 private:
  ...
  void ReadSample(TensorBuffer& tensor) {
    /*
    Each record of dataset is stored in the following formats:
      uint64 length
      uint32 masked_crc32_of_length
      byte   data[length]
      uint32 masked_crc32_of_data
    */
    int64_t tfrecordLen = -1;
    uint32_t maskedCrc32OfLen;
    uint32_t maskedCrc32OfData;

    char* pTfRecordLen = reinterpret_cast<char*>(&tfrecordLen);
    char* pMaskedCrc32OfLen = reinterpret_cast<char*>(&maskedCrc32OfLen);
...
#endif  // ONEFLOW_USER_DATA_TFRECORD_DATASET_H_
