...
class ParamsUnfold3D {
 public:
  ParamsUnfold3D(const int32_t dim, const ShapeView& x_shape, const std::string& data_format,
  ...
  const std::vector<int32_t>& kernel_size_3d() const { return kernel_size_3d_; }
  const std::vector<int32_t>& strides_3d() const { return strides_3d_; }
  const std::vector<int32_t>& dilation_rate_3d() const { return dilation_rate_3d_; }
...
#endif  // ONEFLOW_USER_UTILS_UNFOLD_UTIL_H_
