...
void PlanToPhysicalGraphFile(const Plan &plan) {
  ...
  for (const TaskProto &task : plan.task()) {
    if (task_id2op_name.find(task.task_id()) == task_id2op_name.end()) {
      continue;
    }
    NodeDef *node = physical_graph.add_node();
    node->set_name(task_id2op_name.at(task.task_id()));
    const OperatorConf &op_conf = task.exec_sequence()
                                      .exec_node(0)
                                      .kernel_conf()
                                      .op_attribute()
                                      .op_conf();
    ...
    if (op_conf.has_user_conf()) {
      ...
      node->mutable_attr()->insert(user_op.attr().begin(),
                                   user_op.attr().end());
    } else {
      ...
      for (int64_t regst_desc_id : pair.second.regst_desc_id()) {
        if (regst_desc_id2produce_op_name.find(regst_desc_id) !=
          ...
          std::string input_name =
              regst_desc_id2produce_op_name.at(regst_desc_id);
          if (ctrl_regst_desc_id_set.find(regst_desc_id) !=
              ctrl_regst_desc_id_set.end()) {
