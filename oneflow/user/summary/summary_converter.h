...
static void ConvertProtobufMsg2Json(nlohmann::json &json_value,
                                    const PbMessage &pb_msg);

static void
ConvertRepeatedField2Json(nlohmann::json &json_value, const PbMessage &pb_msg,
                          const PbFd *pb_field,
                          const google::protobuf::Reflection *pb_reflection) {
  if (NULL == pb_field || NULL == pb_reflection) {
    ConvertProtobufMsg2Json(json_value, pb_msg);
  }
  ...
  for (int i = 0; i < pb_reflection->FieldSize(pb_msg, pb_field); ++i) {
    ...
    case PbFd::TYPE_MESSAGE: {
      const PbMessage &msg =
          pb_reflection->GetRepeatedMessage(pb_msg, pb_field, i);
      if (0 != msg.ByteSize()) {
        ConvertProtobufMsg2Json(tmp_json_value, msg);
      }
    } break;
    ...
    case PbFd::TYPE_UINT64: {
      ...
      snprintf(uint64str, sizeof(uint64str), "%llu",
               (unsigned long long)pb_reflection->GetRepeatedUInt64(
                   pb_msg, pb_field, i));
    ...
    case PbFd::TYPE_ENUM:
      tmp_json_value =
          pb_reflection->GetRepeatedEnum(pb_msg, pb_field, i)->name();
      break;
  ...
  for (int i = 0; i < count; ++i) {
    ...
    if (pb_field->is_repeated()) {
      if (pb_reflection->FieldSize(pb_msg, pb_field) > 0) {
        ConvertRepeatedField2Json(json_value[pb_field->name()], pb_msg,
                                  pb_field, pb_reflection);
      }
      continue;
    }

    if (!pb_reflection->HasField(pb_msg, pb_field)) {
      continue;
    }
    ...
    case PbFd::TYPE_MESSAGE: {
      ...
      if (0 != msg.ByteSize()) {
        ConvertProtobufMsg2Json(json_value[pb_field->name()], msg);
      }
    } break;
    case PbFd::TYPE_INT32:
    ...
    case PbFd::TYPE_ENUM: {
      json_value[pb_field->name()] =
          pb_reflection->GetEnum(pb_msg, pb_field)->name();
    } break;
    case PbFd::TYPE_FLOAT: {
