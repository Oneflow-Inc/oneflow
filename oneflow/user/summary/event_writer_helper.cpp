...
Maybe<void> FillScalarInSummary(const float &value, const std::string &tag,
                                Summary *s) {
...
template <typename T>
Maybe<void> FillHistogramInSummary(const user_op::Tensor &value,
...
bool WriteImageToBuffer(const uint8_t *image, int width, int height, int depth,
  ...
  png_structp png_ptr =
      png_create_write_struct(USER_LIBPNG_VER_STRING, 0, 0, 0);
  if (png_ptr == nullptr)
    return false;
...
Maybe<void> FillImageInSummary(const user_op::Tensor &tensor,
                               const std::string &tag, Summary *s) {
  SummaryMetadata metadata;
  ...
  if (!(tensor.shape().At(0) < (1LL << 31) &&
        tensor.shape().At(1) < (1LL << 31) &&
  ...
  if (tensor.data_type() == DataType::kUInt8) {
    ...
    for (int i = 0; i < batch_size; ++i) {
      ...
      *v->mutable_metadata() = metadata;
      ...
      if (!WriteImageToBuffer(image, w, h, depth,
...
template <typename T> struct EventWriterHelper<DeviceType::kCPU, T> {
  ...
  static void WriteScalarToFile(int64_t step, float value,
