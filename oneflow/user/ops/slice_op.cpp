  ...
  if (ctx->parallel_ctx().parallel_num() != 1 && out_sbp.has_split_parallel()) {
    FOR_RANGE(int, i, 0, ndim) {
      if (out_sbp.split_parallel().axis() == i) {
        CHECK_OR_RETURN(IsFullSlice(start_vec.at(i), stop_vec.at(i),
                                    step_vec.at(i), x_shape->At(i)));
      }
    }
  }
  ...
  FOR_RANGE(size_t, i, 0, dim_vec.size()) {
    ...
    if (step > 0) {
      CHECK_LT_OR_RETURN(start, stop)
          << "slice start must be less than stop when step > 0"
             ", otherwise empty result will be outputted.";
    } else {
      CHECK_GT_OR_RETURN(start, stop)
          << "slice start must be more than stop when step < 0"
             ", otherwise empty result will be outputted.";
    }
...
Maybe<void> GetSliceOpSbpSignature(user_op::SbpContext *ctx) {
  ...
  FOR_RANGE(int, i, 0, ndim) {
    if (IsFullSlice(start_vec.at(i), stop_vec.at(i), step_vec.at(i),
                    x_shape.At(i))) {
      ctx->NewBuilder()
          .Split(ctx->inputs(), i)
          .Split(ctx->outputs(), i)
...
void InferSliceGradInputArgModifier(
