...
void CalcOutAndPadding(int64_t input_size, int32_t filter_size,
...
void CalcSamePadding(int64_t input_size, int32_t filter_size,
                     int32_t dilation_rate, int32_t stride,
  ...
  const int32_t padding_needed =
      std::max(0, static_cast<int32_t>((tmp_output_size - 1) * stride +
                                       effective_filter_size - input_size));
  if (padding_small) {
    *padding_small = padding_needed / 2;
  }
  if (padding_large) {
    *padding_large = padding_needed - padding_needed / 2;
  }
}

void CalcConvOut(int64_t input_size, int32_t filter_size, int32_t dilation_rate,
  ...
  if (output_size) {
    *output_size =
        (input_size + 2 * padding_before - effective_filter_size + stride) /
        stride;
    CHECK_GE((*output_size), 0);
  }
}
