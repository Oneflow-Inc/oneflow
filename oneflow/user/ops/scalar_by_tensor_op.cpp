...
Maybe<void> TensorDescInferFn(user_op::InferContext *ctx) {
  ...
  const user_op::TensorDesc *scalar =
      ctx->TensorDesc4ArgNameAndIndex("scalar", 0);
...
REGISTER_USER_OP_GRAD("scalar_add_by_tensor")
    .SetGenBackwardOpConfFn([](const user_op::UserOpWrapper &op,
      ...
      if (op.NeedGenGradTensor4OpInput("x", 0)) {
        op.BindGradTensorWithOpInput(op.GetGradTensorWithOpOutput("y", 0), "x",
                                     0);
      }
      if (op.NeedGenGradTensor4OpInput("scalar", 0)) {
        std::vector<int32_t> axes_vec(
            op.TensorDesc4ArgNameAndIndex("y", 0).shape().NumAxes());
        ...
        user_op::UserOpConfWrapper grad_op =
            builder.Op("reduce_sum")
                ...
                .Attr("keepdims", false)
                .Build();
        op.BindGradTensorWithOpInput(grad_op.output("output_tensor", 0),
                                     "scalar", 0);
        AddOp(grad_op);
      }
    });
    ...
    .SetGenBackwardOpConfFn([](const user_op::UserOpWrapper &op,
      ...
      if (op.NeedGenGradTensor4OpInput("scalar", 0)) {
        std::vector<int32_t> axes_vec(
            op.TensorDesc4ArgNameAndIndex("y", 0).shape().NumAxes());
        std::iota(axes_vec.begin(), axes_vec.end(), 0);
        user_op::UserOpConfWrapperBuilder builder0(op.op_name() +
                                                   "scalar_grad_reduce_sum");
        user_op::UserOpConfWrapper scalar_grad_reduce_sum_op =
            builder0.Op("reduce_sum")
                ...
                .Attr("keepdims", false)
                .Build();
        user_op::UserOpConfWrapperBuilder builder1(op.op_name() +
                                                   "scalar_grad_scalar_mul");
        user_op::UserOpConfWrapper scalar_grad_scalar_mul_op =
            builder1.Op("scalar_mul")
                .Input("in",
                       scalar_grad_reduce_sum_op.output("output_tensor", 0))
                .Output("out")
                ...
                .Build();
        op.BindGradTensorWithOpInput(scalar_grad_scalar_mul_op.output("out", 0),
                                     "scalar", 0);
...
REGISTER_USER_OP_GRAD("scalar_mul_by_tensor")
    .SetGenBackwardOpConfFn([](const user_op::UserOpWrapper &op,
      ...
      if (op.NeedGenGradTensor4OpInput("x", 0)) {
        ...
        user_op::UserOpConfWrapper grad_op =
            builder.Op("scalar_mul_by_tensor")
      ...
      if (op.NeedGenGradTensor4OpInput("scalar", 0)) {
        ...
        op.BindGradTensorWithOpInput(
            scalar_grad_reduce_sum_op.output("output_tensor", 0), "scalar", 0);
...
REGISTER_USER_OP_GRAD("scalar_div_by_tensor")
    .SetGenBackwardOpConfFn([](const user_op::UserOpWrapper &op,
      ...
      if (op.NeedGenGradTensor4OpInput("x", 0)) {
        user_op::UserOpConfWrapperBuilder builder(op.op_name() + "_grad");
        user_op::UserOpConfWrapper grad_op =
            builder.Op("scalar_div_by_tensor")
                .Input("x", op.GetGradTensorWithOpOutput("y", 0))
                .Input("scalar", op.input("scalar", 0))
                .Output("y")
                .Build();
