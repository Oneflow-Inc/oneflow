...
REGISTER_USER_OP("batch_gather")
    ...
    .SetTensorDescInferFn([](user_op::InferContext *ctx) -> Maybe<void> {
      ...
      CHECK_LE_OR_RETURN(indices->shape().dim_vec().size(),
                         in->shape().dim_vec().size());
      FOR_RANGE(int64_t, i, 0, indices->shape().dim_vec().size() - 1) {
        if (in->is_dynamic() && indices->is_dynamic() == false) {
          CHECK_GE_OR_RETURN(indices->shape().dim_vec().at(i),
                             in->shape().dim_vec().at(i));
        } else if (in->is_dynamic() == false && indices->is_dynamic()) {
          UNIMPLEMENTED();
        } else {
          CHECK_EQ_OR_RETURN(indices->shape().dim_vec().at(i),
                             in->shape().dim_vec().at(i));
        }
      }

      DimVector dim_vec(in->shape().dim_vec());
      dim_vec.at(indices->shape().NumAxes() - 1) =
          indices->shape().dim_vec().back();
    ...
    .SetGetSbpFn([](user_op::SbpContext *ctx) -> Maybe<void> {
      ...
      if (indices_num_axes > 1) {
        ...
        err->set_msg("BatchGatherOp: indices_num_axes equals " +
                     std::to_string(indices_num_axes) +
    ...
    .SetGenBackwardOpConfFn([](const user_op::UserOpWrapper &op,
      ...
      if (need_grad_in) {
        ...
        const Shape indices_shape =
            op.TensorDesc4ArgNameAndIndex("indices", 0).shape();
