...
/* static */ auto AsStridedGradOp::InferLogicalTensorDesc(user_op::InferContext* ctx) -> Maybe<void> {
  const Shape& input_shape = ctx->InputShape("input", 0);
  user_op::TensorDesc* dx_desc = ctx->OutputTensorDesc("dx", 0);
  *dx_desc->mut_shape() = input_shape;
  return Maybe<void>::Ok();
}
/*static*/ auto AsStridedGradOp::InferPhysicalTensorDesc(user_op::InferContext* ctx) -> Maybe<void> {
...
REGISTER_USER_OP_GRAD("as_strided").SetGenBackwardOpConfFn([](const user_op::UserOpWrapper& op,
