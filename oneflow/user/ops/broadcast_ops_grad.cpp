...
std::string
CreateReduceSumLikeBlob(const std::string &in_lbn, const Shape &in_shape,
  ...
  if (in_shape == like_shape) {
    ...
    const AxisVector &broadcast_axis_vec =
        left_extended_shape.Axes4BroadcastTo(in_shape);
    user_op::UserOpConfWrapperBuilder builder(op_name +
                                              "_grad_reduce_sum_like");
    user_op::UserOpConfWrapper grad_op =
        builder.Op("reduce_sum_like")
            .Input("x", in_lbn)
            .Input("like", like_lbn)
            .Attr<std::vector<int32_t>>(
                "axis", {broadcast_axis_vec.begin(), broadcast_axis_vec.end()})
            .Output("y")
            .Build();
    AddOp(grad_op);
    ...
    .SetGenBackwardOpConfFn([](const user_op::UserOpWrapper &op,
      ...
      if (op.NeedGenGradTensor4OpInput("x", 0)) {
        ...
        const std::string &out_lbn = CreateReduceSumLikeBlob(
            dz_lbn, z_shape, x_lbn, x_shape, op.op_name() + "_x", AddOp);
      ...
      if (op.NeedGenGradTensor4OpInput("y", 0)) {
        ...
        const std::string &out_lbn = CreateReduceSumLikeBlob(
            dz_lbn, z_shape, y_lbn, y_shape, op.op_name() + "_y", AddOp);
...
REGISTER_USER_OP_GRAD("broadcast_sub")
    .SetGenBackwardOpConfFn([](const user_op::UserOpWrapper &op,
      ...
      if (op.NeedGenGradTensor4OpInput("y", 0)) {
        user_op::UserOpConfWrapperBuilder scalar_mul_builder(op.op_name() +
                                                             "_grad_y_mul");
        ...
        const std::string &out_lbn =
            CreateReduceSumLikeBlob(scalar_mul_op.output("out", 0), z_shape,
                                    y_lbn, y_shape, op.op_name() + "_y", AddOp);
        op.BindGradTensorWithOpInput(out_lbn, "y", 0);
      }
    });
...
REGISTER_USER_OP_GRAD("broadcast_mul")
    .SetGenBackwardOpConfFn([](const user_op::UserOpWrapper &op,
      ...
      const std::string &dz_lbn = op.GetGradTensorWithOpOutput("z", 0);
      if (op.NeedGenGradTensor4OpInput("x", 0)) {
        user_op::UserOpConfWrapperBuilder broadcast_mul_builder(op.op_name() +
                                                                "_grad_x_mul");
        ...
        const std::string &x_lbn = op.input("x", 0);
        const std::string &out_lbn =
            CreateReduceSumLikeBlob(broadcast_mul_op.output("z", 0), z_shape,
                                    x_lbn, x_shape, op.op_name() + "_x", AddOp);
        op.BindGradTensorWithOpInput(out_lbn, "x", 0);
      }
      if (op.NeedGenGradTensor4OpInput("y", 0)) {
        user_op::UserOpConfWrapperBuilder broadcast_mul_builder(op.op_name() +
...
REGISTER_USER_OP_GRAD("broadcast_div")
    .SetGenBackwardOpConfFn([](const user_op::UserOpWrapper &op,
      ...
      if (op.NeedGenGradTensor4OpInput("x", 0)) {
        const Shape &z_shape = op.TensorDesc4ArgNameAndIndex("z", 0).shape();
        user_op::UserOpConfWrapperBuilder broadcast_div_builder(op.op_name() +
                                                                "_grad_x_div");
        ...
        const std::string &x_lbn = op.input("x", 0);
        const std::string &out_lbn =
            CreateReduceSumLikeBlob(broadcast_div_op.output("z", 0), z_shape,
                                    x_lbn, x_shape, op.op_name() + "_x", AddOp);
        op.BindGradTensorWithOpInput(out_lbn, "x", 0);
      }
      if (op.NeedGenGradTensor4OpInput("y", 0)) {
