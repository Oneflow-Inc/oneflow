...
/* static */ Maybe<void> AddNpuOp::InferLogicalTensorDesc(user_op::InferContext* ctx) {
...
/*static*/ Maybe<void> AddNpuOp::InferPhysicalTensorDesc(user_op::InferContext* ctx) {
  return InferLogicalTensorDesc(ctx);
}

/* static */ Maybe<void> AddNpuOp::GetSbp(user_op::SbpContext* ctx) {
...
/* static */ Maybe<void> AddNpuOp::InferDataType(user_op::InferContext* ctx) {
  const auto& in_0 = ctx->InputTensorDesc("in", 0);
  auto* out = ctx->MutOutputTensorDesc("out", 0);
  CHECK_NOTNULL_OR_RETURN(out);  // NOLINT(maybe-need-error-msg)
  for (const auto& pair : ctx->inputs()) {
...
/*static*/ Maybe<void> AddNpuOp::CheckAttr(const user_op::UserOpDefWrapper&,
...
// dck_caution_here
// REGISTER_USER_OP_GRAD("add_npu").SetGenBackwardOpConfFn([](const user_op::UserOpWrapper& op,
