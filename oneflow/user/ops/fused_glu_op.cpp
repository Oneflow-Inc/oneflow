...
/* static */ auto FusedGluOp::InferLogicalTensorDesc(user_op::InferContext* ctx) -> Maybe<void> {
  ...
  if (ctx->has_input("v", 0)) { CHECK(ctx->has_input("c", 0)); }
  if (ctx->has_input("c", 0)) { CHECK(ctx->has_input("v", 0)); }
  ...
  if (!is_split_mode) CHECK_EQ_OR_RETURN(w_in_shape.At(1) % 2, 0);
...
/* static */ auto FusedGluOp::InferDataType(user_op::InferContext* ctx) -> Maybe<void> {
  ...
  if (is_split_mode) ctx->SetOutputDType("matmul_vx", 0, in_x_dtype);
