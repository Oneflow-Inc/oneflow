...
Maybe<void> GetSbpFn(user_op::SbpContext *ctx) {
  ...
  const auto &outshape =
      JUST(ReshapeUserOpUtil::GetLogicalOutBlobShape(in_shape, shape_proto));
  return ReshapeUserOpUtil::GetReshapeUserOpSbpSignatures(in_shape, *outshape,
                                                          ctx);
}
...
Maybe<void> TensorDescInferFn(user_op::InferContext *ctx) {
  ...
  *out_tensor_desc = *in_tensor_desc;
  ...
  if (sbp_parallel.has_split_parallel()) {
    ...
    BalancedSplitter spliter(shape.dim_vec().at(split_axis),
                             parallel_ctx.parallel_num());
    CHECK_GE_OR_RETURN(shape.dim_vec().at(split_axis),
                       parallel_ctx.parallel_num());
