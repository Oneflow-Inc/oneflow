...
Maybe<void> NormalizationAddReluTensorDescInfer(user_op::InferContext* ctx) {
#ifdef WITH_CUDA
...
REGISTER_USER_OP("normalization_add_relu")
    ...
    .SetTensorDescInferFn(NormalizationAddReluTensorDescInfer)
...
Maybe<void> NormalizationAddReluGradTensorDescInfer(user_op::InferContext* ctx) {
#ifdef WITH_CUDA
  // assume cudnn is enabled
  CHECK_GE_OR_RETURN(ctx->Attr<float>("epsilon"), CUDNN_BN_MIN_EPSILON);
#endif
...
REGISTER_USER_OP_GRAD("normalization_add_relu")
    .SetBackwardOpConfGenFn([](user_op::BackwardOpConfContext* ctx) {
      ...
      const std::string mul_beta_name = ctx->FwOp().op_name() + "_out_grad_mul_beta";
      const auto dy_mul_beta_op_name = "System-AutoGrad-" + mul_beta_name + "-BroadcastMul";
      BroadcastMulAtAxisOpDefine([&ctx]() { return ctx->FwOp().input("beta", 0); },
                                 [&ctx, &is_fp16, &dy_h2f_cast_op_name]() {
                                   if (is_fp16) {
                                     return ctx->GetOp(dy_h2f_cast_op_name).output("out", 0);
                                   } else {
