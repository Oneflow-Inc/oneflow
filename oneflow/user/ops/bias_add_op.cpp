...
REGISTER_USER_OP("bias_add")
    ...
    .SetTensorDescInferFn([](user_op::InferContext *ctx) -> Maybe<void> {
      ...
      CHECK_EQ_OR_RETURN(a_tensor_desc->shape().At(bias_add_axis),
                         b_tensor_desc->shape().At(0));
    ...
    .SetGetSbpFn([](user_op::SbpContext *ctx) -> Maybe<void> {
      ...
      for (int64_t i = 0;
           i < ctx->LogicalTensorDesc4InputArgNameAndIndex("a", 0)
                   .shape()
                   .NumAxes();
           ++i) {
        if (i == axis) {
      ...
      if (op.NeedGenGradTensor4OpInput("b", 0)) {
        ...
        auto grad_op =
            builder.Op("reduce_sum")
                ...
                .Build();
        AddOp(grad_op);
        op.BindGradTensorWithOpInput(grad_op.output("output_tensor", 0), "b",
                                     0);
      }
    });
