...
/* static */ Maybe<void> ConcatOp::InferLogicalTensorDesc(user_op::InferContext* ctx) {
  ...
  for (const auto& in_arg_pair : ctx->inputs()) {
    ...
    //   if ((!in_desc.shape().all_dims_known() or in_desc.shape().elem_cnt() != 0) or in_desc.shape().NumAxes() != 1) {
    ...
    FOR_RANGE(int64_t, i, 0, in_desc.shape().NumAxes()) {
      if (in_desc.shape().all_dims_known() and in_desc.shape().elem_cnt() == 0 and in_desc.shape().NumAxes() == 1) { continue; }
      if (i == axis) {
