...
REGISTER_CPU_ONLY_USER_OP("COCOReader")
    ...
    .Attr<bool>("remove_images_without_annotations", UserOpAttrType::kAtBool,
                true)
    ...
    .SetTensorDescInferFn([](user_op::InferContext *ctx) -> Maybe<void> {
      ...
      CHECK_OR_RETURN(sbp ==
                      ctx->SbpParallel4ArgNameAndIndex("gt_segm_index", 0));
    ...
    .SetBatchAxisInferFn([](user_op::BatchAxisContext *ctx) -> Maybe<void> {
      for (const auto &out_arg_pair : ctx->outputs()) {
        ctx->BatchAxis4ArgNameAndIndex(out_arg_pair.first, out_arg_pair.second)
            ->set_value(0);
      }
      return Maybe<void>::Ok();
    })
    .SetOutputArgModifyFn(
        [](user_op::GetOutputArgModifier GetOutputArgModifierFn,
          ...
          user_op::OutputArgModifier *image_id_modifier =
              GetOutputArgModifierFn("image_id", 0);
          CHECK(image_id_modifier != nullptr);
          image_id_modifier->set_header_infered_before_compute(false);

          user_op::OutputArgModifier *image_size_modifier =
              GetOutputArgModifierFn("image_size", 0);
          CHECK(image_size_modifier != nullptr);
          image_size_modifier->set_header_infered_before_compute(false);

          user_op::OutputArgModifier *gt_bbox_modifier =
              GetOutputArgModifierFn("gt_bbox", 0);
          CHECK(gt_bbox_modifier != nullptr);
          gt_bbox_modifier->set_header_infered_before_compute(false);

          user_op::OutputArgModifier *gt_label_modifier =
              GetOutputArgModifierFn("gt_label", 0);
          CHECK(gt_label_modifier != nullptr);
          gt_label_modifier->set_header_infered_before_compute(false);

          user_op::OutputArgModifier *gt_segm_modifier =
              GetOutputArgModifierFn("gt_segm", 0);
