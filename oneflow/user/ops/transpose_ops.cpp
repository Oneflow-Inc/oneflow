...
REGISTER_USER_OP("transpose")
    ...
    .SetTensorDescInferFn([](user_op::InferContext *ctx) -> Maybe<void> {
      const user_op::TensorDesc *in_tensor_desc =
          ctx->TensorDesc4ArgNameAndIndex("input", 0);
      user_op::TensorDesc *out_tensor_desc =
          ctx->TensorDesc4ArgNameAndIndex("output", 0);
      ...
      CheckIsPerm(perm);
      if (perm.at(0) != 0) {
        CHECK_OR_RETURN(!in_tensor_desc->is_dynamic());
      }
      *out_tensor_desc = *in_tensor_desc;
      FOR_RANGE(size_t, i, 0, perm.size()) {
        out_shape->Set(i, in_shape.At(perm[i]));
      }
    ...
    .SetBatchAxisInferFn([](user_op::BatchAxisContext *ctx) -> Maybe<void> {
      if (ctx->BatchAxis4ArgNameAndIndex("input", 0)->has_value()) {
        ...
        ctx->BatchAxis4ArgNameAndIndex("output", 0)
            ->set_value(
                perm.at(ctx->BatchAxis4ArgNameAndIndex("input", 0)->value()));
      } else {
      ...
      if (op.NeedGenGradTensor4OpInput("input", 0)) {
        ...
        user_op::UserOpConfWrapper transpose_grad_op =
            builder.Op("transpose")
                ...
                .Build();
        op.BindGradTensorWithOpInput(transpose_grad_op.output("output", 0),
                                     "input", 0);
