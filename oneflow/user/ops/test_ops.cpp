      ...
      if (op.NeedGenGradTensor4OpInput("in", 0)) {
        ...
        user_op::UserOpConfWrapper test_reshape_like_op =
            builder.Op("TestReshapeLike4KeepHeaderOnly")
                ...
                .Output("out")
                .Build();
        op.BindGradTensorWithOpInput(test_reshape_like_op.output("out", 0),
                                     "in", 0);
...
REGISTER_USER_OP("TestMultiInput")
    ...
    .SetGetSbpFn([](user_op::SbpContext *ctx) -> Maybe<void> {
      const user_op::TensorDesc &x1_tensor =
          ctx->LogicalTensorDesc4InputArgNameAndIndex("x1", 0);
...
REGISTER_USER_OP_GRAD("TestMultiInput")
    .SetGenBackwardOpConfFn([](const user_op::UserOpWrapper &op,
                               user_op::AddOpFn AddOp) {
      if (op.NeedGenGradTensor4OpInput("x1", 0) ||
          op.NeedGenGradTensor4OpInput("x2", 0)) {
        user_op::UserOpConfWrapperBuilder builder(op.op_name() + "_grad");
        ...
        op.BindGradTensorWithOpInput(
            test_multi_input_grad_op.output("x1_diff", 0), "x1", 0);
        op.BindGradTensorWithOpInput(
            test_multi_input_grad_op.output("x2_diff", 0), "x2", 0);
...
REGISTER_USER_OP("TestListDataTypeAndListShapeAndListStringAttr")
    ...
    .SetTensorDescInferFn([](user_op::InferContext *ctx) -> Maybe<void> {
      ...
      const auto &string_list =
          ctx->Attr<std::vector<std::string>>("string_list");
