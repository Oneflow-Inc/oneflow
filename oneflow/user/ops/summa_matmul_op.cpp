...
REGISTER_USER_OP("summa_broadcast_matmul")
    .Input("a")
    .Input("b")
    .Output("out")
    ...
    .SetLogicalTensorDescInferFn([](user_op::InferContext* ctx) -> Maybe<void> {
      ...
      CHECK_EQ_OR_RETURN(a->shape().NumAxes(), 3);
      CHECK_EQ_OR_RETURN(b->shape().NumAxes(), 2);
      DimVector out_dim_vec = a->shape().dim_vec();
      out_dim_vec.at(2) = b->shape().At(1);
      *ctx->Shape4ArgNameAndIndex("out", 0) = Shape(out_dim_vec);
    ...
    .SetParallelDistributionInferFn(
        [](user_op::InferParallelDistributionFnContext* ctx) -> Maybe<void> {
          ParallelDistribution a_parallel_distribution;
          a_parallel_distribution.add_sbp_parallel()->mutable_split_parallel()->set_axis(0);
          a_parallel_distribution.add_sbp_parallel()->mutable_split_parallel()->set_axis(2);
          ParallelDistribution b_parallel_distribution;
          b_parallel_distribution.add_sbp_parallel()->mutable_split_parallel()->set_axis(0);
          b_parallel_distribution.add_sbp_parallel()->mutable_split_parallel()->set_axis(1);
          *ctx->ParallelDistribution4ArgNameAndIndex("a", 0) = a_parallel_distribution;
          *ctx->ParallelDistribution4ArgNameAndIndex("b", 0) = b_parallel_distribution;
          *ctx->ParallelDistribution4ArgNameAndIndex("out", 0) = a_parallel_distribution;
          return Maybe<void>::Ok();
        });

REGISTER_USER_OP("summa_broadcast_matmul_grad_a")  // abt
    .Input("a")
    .Input("b")
    .Output("out")
    .Attr<double>("alpha", 1.0)
...
REGISTER_USER_OP("summa_broadcast_matmul_grad_b")  // atb
    .Input("a")
    .Input("b")
    .Output("out")
    .Attr<double>("alpha", 1.0)
    .SetLogicalTensorDescInferFn([](user_op::InferContext* ctx) -> Maybe<void> {
      ...
      const user_op::TensorDesc* b = ctx->TensorDesc4ArgNameAndIndex("b", 0);
      CHECK_EQ_OR_RETURN(a->shape().NumAxes(), 3);
      CHECK_EQ_OR_RETURN(b->shape().NumAxes(), 3);
      *ctx->Shape4ArgNameAndIndex("out", 0) = Shape({a->shape().At(2), b->shape().At(2)});
      *ctx->IsDynamic4ArgNameAndIndex("out", 0) = *ctx->IsDynamic4ArgNameAndIndex("a", 0);
      return Maybe<void>::Ok();
    })
    .SetDataTypeInferFn([](user_op::InferContext* ctx) -> Maybe<void> {
    ...
    .SetParallelDistributionInferFn(
        [](user_op::InferParallelDistributionFnContext* ctx) -> Maybe<void> {
          ...
          a_parallel_distribution.add_sbp_parallel()->mutable_split_parallel()->set_axis(0);
          a_parallel_distribution.add_sbp_parallel()->mutable_split_parallel()->set_axis(2);
          ParallelDistribution c_parallel_distribution;
          c_parallel_distribution.add_sbp_parallel()->mutable_split_parallel()->set_axis(0);
          c_parallel_distribution.add_sbp_parallel()->mutable_split_parallel()->set_axis(1);
          *ctx->ParallelDistribution4ArgNameAndIndex("a", 0) = a_parallel_distribution;
          *ctx->ParallelDistribution4ArgNameAndIndex("b", 0) = a_parallel_distribution;
          *ctx->ParallelDistribution4ArgNameAndIndex("out", 0) = c_parallel_distribution;
          return Maybe<void>::Ok();
        });
...
REGISTER_USER_OP_GRAD("summa_broadcast_matmul")
    .SetBackwardOpConfGenFn([](user_op::BackwardOpConfContext* ctx) -> void {
      double alpha = ctx->FwOp().attr<double>("alpha");
      std::string a_grad_op_name = ctx->FwOp().op_name() + "_a_grad";
      ctx->DefineOp(a_grad_op_name,
                    [&](user_op::BackwardOpBuilder& builder) -> user_op::UserOpConfWrapper {
                      return builder.OpTypeName("summa_broadcast_matmul_grad_a")
                          .InputBind("a", ctx->FwOp().output_grad("out", 0))
                          .InputBind("b", ctx->FwOp().input("b", 0))
                          .Attr<double>("alpha", alpha)
                          .Output("out")
      ...
      ctx->DefineOp(b_grad_op_name,
                    [&](user_op::BackwardOpBuilder& builder) -> user_op::UserOpConfWrapper {
                      return builder.OpTypeName("summa_broadcast_matmul_grad_b")
                          .InputBind("a", ctx->FwOp().input("a", 0))
                          .InputBind("b", ctx->FwOp().output_grad("out", 0))
                          .Attr<double>("alpha", alpha)
                          .Output("out")
