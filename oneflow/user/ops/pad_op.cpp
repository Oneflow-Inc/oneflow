...
REGISTER_USER_OP("pad")
    ...
    .SetTensorDescInferFn([](user_op::InferContext *ctx) -> Maybe<void> {
      ...
      const auto &padding_after =
          ctx->Attr<std::vector<int64_t>>("padding_after");
...
REGISTER_USER_OP("pad_grad")
    ...
    .SetTensorDescInferFn([](user_op::InferContext *ctx) -> Maybe<void> {
      ...
      *ctx->Dtype4ArgNameAndIndex("dx", 0) =
          *ctx->Dtype4ArgNameAndIndex("dy", 0);
      return Maybe<void>::Ok();
    })
    .SetGetSbpFn([](user_op::SbpContext *ctx) -> Maybe<void> {
      const user_op::TensorDesc &dy_tensor =
      ...
      if (op.NeedGenGradTensor4OpInput("x", 0)) {
        ...
        user_op::UserOpConfWrapper grad_op =
            builder.Op("pad_grad")
                ...
                .Output("dx")
                .Attr("floating_constant_value",
                      op.attr<double>("floating_constant_value"))
                .Attr("integral_constant_value",
                      op.attr<int64_t>("integral_constant_value"))
                .Attr("padding_before",
                      op.attr<std::vector<int64_t>>("padding_before"))
                .Attr("padding_after",
                      op.attr<std::vector<int64_t>>("padding_after"))
                .Build();
