...
std::function<Maybe<void>(const std::string &)>
MakeCheckParamTensorDescFn(user_op::InferContext *ctx, DataType data_type,
                           const Shape &shape) {
...
std::function<Maybe<void>(const std::string &)>
MakeSetParamTensorDescFn(user_op::InferContext *ctx, DataType data_type,
                         const Shape &shape) {
  return [=](const std::string &bn) -> Maybe<void> {
...
void FwInputArgModifyFn(
  ...
  user_op::InputArgModifier *moving_mean_modifier =
      GetInputArgModifierFn("moving_mean", 0);
...
Maybe<void> FwGetSbpFn(user_op::SbpContext *ctx) {
  ...
  if (ctx->user_op_conf().has_input("addend", 0)) {
    split_args.emplace_back("addend", 0);
  }
  ...
  if (ctx->user_op_conf().has_output("mean", 0)) {
...
user_op::TensorDescInferFn MakeFwTensorDescInferFn(
    const std::function<Maybe<void>(
        user_op::InferContext *ctx, const user_op::TensorDesc *x,
  ...
  return [reserve_space_infer_fn](user_op::InferContext *ctx) -> Maybe<void> {
    ...
    *ctx->TensorDesc4ArgNameAndIndex("y", 0) = *x;
    ...
    const auto CheckParamTensorDesc =
        MakeCheckParamTensorDescFn(ctx, param_data_type, param_shape);
    const auto SetParamTensorDesc =
        MakeSetParamTensorDescFn(ctx, param_data_type, param_shape);
    ...
    if (ctx->user_op_conf().has_output("reserve_space", 0)) {
      ...
      reserve_space_infer_fn(
          ctx, x, ctx->TensorDesc4ArgNameAndIndex("reserve_space", 0));
    }
    return Maybe<void>::Ok();
  };
}
...
user_op::TensorDescInferFn MakeFwTensorDescInferFn() {
  return MakeFwTensorDescInferFn(
      std::function<Maybe<void>(user_op::InferContext * ctx,
...
REGISTER_USER_OP("cudnn_fused_normalization_add_relu")
    ...
    .SetTensorDescInferFn(MakeFwTensorDescInferFn(
        [](user_op::InferContext *ctx, const user_op::TensorDesc *x,
          ...
          CudnnTensorDesc xy_desc(CUDNN_TENSOR_NHWC, x->data_type(), n, c, h,
                                  w);
          ...
          OF_CUDNN_CHECK(cudnnGetBatchNormalizationTrainingExReserveSpaceSize(
              cudnn_handle, CUDNN_BATCHNORM_SPATIAL_PERSISTENT, ops,
              activation_desc.Get(), xy_desc.Get(), &reserve_space_size));
          reserve_space_size =
              std::max(reserve_space_size, GetOneVal<size_t>());
          *reserve_space->mut_data_type() = DataType::kChar;
          *reserve_space->mut_shape() =
              Shape({static_cast<int64_t>(reserve_space_size)});
...
Maybe<void> BwTensorDescInferFn(user_op::InferContext *ctx) {
  ...
  *ctx->TensorDesc4ArgNameAndIndex("dx", 0) = *x;
  ...
  const DataType param_data_type =
      x_type == DataType::kFloat16 ? DataType::kFloat : x_type;
  const auto CheckParamTensorDesc =
      MakeCheckParamTensorDescFn(ctx, param_data_type, param_shape);
  const auto SetParamTensorDesc =
      MakeSetParamTensorDescFn(ctx, param_data_type, param_shape);
...
Maybe<void> BwGetSbpFn(user_op::SbpContext *ctx) {
  ...
  broadcast_args.emplace_back("inv_variance", 0);
  broadcast_args.emplace_back("gamma", 0);
  if (ctx->user_op_conf().has_input("beta", 0)) {
    broadcast_args.emplace_back("beta", 0);
  }
...
REGISTER_USER_OP_GRAD("normalization")
    .SetBackwardOpConfGenFn([](user_op::BackwardOpConfContext *ctx) {
      ...
      ctx->DefineOp(
          var_add_eps_op_name, [&ctx](user_op::BackwardOpBuilder &builder) {
            return builder.OpTypeName("scalar_add")
                ...
                .Attr("float_operand",
                      static_cast<double>(ctx->FwOp().attr<float>("epsilon")))
                .Output("out")
                .Build();
          });
      ...
      ctx->DefineOp(
          ...
          [&ctx, &var_add_eps_op_name](user_op::BackwardOpBuilder &builder) {
            return builder.OpTypeName("rsqrt")
                .InputBind("x",
                           ctx->GetOp(var_add_eps_op_name).output("out", 0))
                .Output("y")
                .Build();
          });

      const auto grad_op_name = ctx->FwOp().op_name() + "_grad";
      ctx->DefineOp(grad_op_name, [&ctx, &is_training, &var_rsqrt_op_name](
        ...
        if (is_training) {
          ...
          builder.InputBind("mean", ctx->FwOp().input("moving_mean", 0))
              .InputBind("inv_variance",
                         ctx->GetOp(var_rsqrt_op_name).output("y", 0));
        }
      ...
      const auto BroadcastMulAtAxisOpDefine = [&ctx, &axis](
          std::function<std::string()> scale_bn_func,
        ...
        ctx->DefineOp(
            ...
            [&ctx, &axis, scale_bn_func](user_op::BackwardOpBuilder &builder) {
              ...
              const auto &in_shape =
                  ctx->FwOp().arg_tensor_desc("x", 0).shape();
        ...
        // local variable(reshape_op_name/input_bn_func) need to be captured by
        // value
      ...
      const auto dy_h2f_cast_op_name =
          "System-AutoGrad-" + ctx->FwOp().op_name() + "-Cast-dy-h2f";
      ctx->DefineOp(
          dy_h2f_cast_op_name, [&ctx](user_op::BackwardOpBuilder &builder) {
            return builder.OpTypeName("cast")
                .Input("in", ctx->FwOp().output_grad("y", 0))
                .Output("out")
                .Attr("dtype",
                      ctx->FwOp().arg_tensor_desc("gamma", 0).data_type())
                .Build();
          });

      const std::string mul_gamma_name =
          ctx->FwOp().op_name() + "_out_grad_mul_gamma";
      const auto dy_mul_gamma_op_name =
          "System-AutoGrad-" + mul_gamma_name + "-BroadcastMul";
      BroadcastMulAtAxisOpDefine(
          [&ctx]() { return ctx->FwOp().input("gamma", 0); },
      ...
      const std::string mul_inv_var_name =
          ctx->FwOp().op_name() + "_out_grad_mul_inv_var";
      const auto dy_mul_inv_var_op_name =
          "System-AutoGrad-" + mul_inv_var_name + "-BroadcastMul";
      BroadcastMulAtAxisOpDefine(
          [&ctx, &var_rsqrt_op_name]() {
            return ctx->GetOp(var_rsqrt_op_name).output("y", 0);
          },
      ...
      const auto dx_f2h_cast_op_name =
          "System-AutoGrad-" + ctx->FwOp().op_name() + "-Cast-dx-f2h";
      ctx->DefineOp(
          dx_f2h_cast_op_name,
          [&ctx, &dy_mul_inv_var_op_name](user_op::BackwardOpBuilder &builder) {
            return builder.OpTypeName("cast")
                .InputBind("in",
                           ctx->GetOp(dy_mul_inv_var_op_name).output("z", 0))
                .Output("out")
                .Attr("dtype", DataType::kFloat16)
                .Build();
          });

      // TODO(liujuncheng): delete identity op when boxing support separated
      // regsts
      const auto gamma_identity_op_name =
          ctx->FwOp().op_name() + "_grad_gamma_diff_identity";
      ctx->DefineOp(gamma_identity_op_name,
                    [&ctx, &grad_op_name](user_op::BackwardOpBuilder &builder) {
                      return builder.OpTypeName("identity")
                          .InputBind(
                              "in",
                              ctx->GetOp(grad_op_name).output("gamma_diff", 0))
                          .Output("out")
                          .Build();
                    });
      ...
      // TODO(liujuncheng): delete identity op when boxing support separated
      // regsts
      const auto beta_identity_op_name =
          ctx->FwOp().op_name() + "_grad_beta_diff_identity";
      ctx->DefineOp(beta_identity_op_name,
                    [&ctx, &grad_op_name](user_op::BackwardOpBuilder &builder) {
                      return builder.OpTypeName("identity")
                          .InputBind(
                              "in",
                              ctx->GetOp(grad_op_name).output("beta_diff", 0))
                          .Output("out")
                          .Build();
                    });
    ...
    .SetBackwardOpConfGenFn([](user_op::BackwardOpConfContext *ctx) {
      ...
      ctx->DefineOp(grad_op_name, [&ctx](user_op::BackwardOpBuilder &builder) {
        builder.OpTypeName("normalization_add_relu_grad")
            ...
            .Output("beta_diff")
            .Output("dx");
        if (ctx->FwOp().input_size("addend") != 0) {
          builder.Output("addend_diff");
        }
        return builder.Build();
      });
