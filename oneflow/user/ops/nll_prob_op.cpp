...
/* static */ Maybe<void> NLLProbOp::InferDataType(user_op::InferContext* ctx) {
  CHECK_OR_RETURN(IsFloatingDataType(ctx->InputDType("target", 0)))
      << ctx->op_name() << ": expected target being float type";
...
/* static */ Maybe<void> NLLProbOp::InferLogicalTensorDesc(user_op::InferContext* ctx) {
...
/* static */ Maybe<void> NLLProbOp::GetSbp(user_op::SbpContext* ctx) {
  // split batch dim
...
/* static */ Maybe<void> NLLProbGradOp::InferDataType(user_op::InferContext* ctx) {
  CHECK_OR_RETURN(IsFloatingDataType(ctx->InputDType("target", 0)))
      << ctx->op_name() << ": expected target being float type";
...
/* static */ Maybe<void> NLLProbGradOp::InferLogicalTensorDesc(user_op::InferContext* ctx) {
  const auto& input_desc = ctx->InputTensorDesc("input", 0);
  const auto& target_desc = ctx->InputTensorDesc("target", 0);
...
/* static */ Maybe<void> NLLProbGradOp::GetSbp(user_op::SbpContext* ctx) {
  // split batch dim
  auto builder1 = ctx->NewBuilder()
                      .Split(user_op::OpArg("input", 0), 0)
                      .Split(user_op::OpArg("target", 0), 0)
