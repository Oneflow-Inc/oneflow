...
Maybe<Shape> GetBroadcastShape(const Shape& cond_shape, const Shape& x_shape,
  ...
  for (size_t i = 0; i < ndim; ++i) {
    ...
    int64_t cond_dim = (i < cond_lpad) ? 1 : static_cast<int>(cond_shape[i - cond_lpad]);
    int64_t x_dim = (i < x_lpad) ? 1 : static_cast<int>(x_shape[i - x_lpad]);
    int64_t y_dim = (i < y_lpad) ? 1 : static_cast<int>(y_shape[i - y_lpad]);
...
/*static*/ Maybe<void> WhereOp::GetSbp(user_op::SbpContext* ctx) {
  ...
  auto CheckArgCanSplit = [&](std::string&& arg_name, const int dim, const Shape& shape) {
    ...
    int dim_size = (dim >= ddiff) ? static_cast<int>(shape[dim - ddiff]) : 1;
    if (dim_size == 1) {
