...
REGISTER_USER_OP("unfold")
    ...
    .SetTensorDescInferFn([](user_op::InferContext* ctx) -> Maybe<void> {
      ...
      const int32_t in_channels = x_shape.At(1); 
      const int32_t x_height = x_shape.At(2); 
      const int32_t x_width = x_shape.At(3); 

      const int32_t ndim = x_shape.NumAxes(); 
      CHECK_EQ_OR_RETURN(ndim, 4); // only support 4-D Tensor Input
