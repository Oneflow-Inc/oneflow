...
Maybe<void>
TransformNegativeAxesToPositive(const std::vector<int32_t> &axes_vec,
  ...
  FOR_RANGE(size_t, i, 0, fixed_axes_vec->size()) {
    ...
    fixed_axes_vec->at(i) =
        axes_vec[i] >= 0 ? axes_vec[i] : axes_vec[i] + num_axes;
  }
  return Maybe<void>::Ok();
}

Maybe<void> CheckAndLabelAxesToSqueezeMinusOne(const AxisVector &axes,
                                               DimVector *dim_vec) {
...
REGISTER_USER_OP("squeeze")
    ...
    .SetTensorDescInferFn([](user_op::InferContext *ctx) -> Maybe<void> {
      ...
      dim_vec.erase(std::remove(dim_vec.begin(), dim_vec.end(), -1),
                    dim_vec.end());
      if (dim_vec.empty()) {
        *out_shape = Shape({1});
      } else {
    ...
    .SetBatchAxisInferFn([](user_op::BatchAxisContext *ctx) -> Maybe<void> {
      ...
      const int32_t in_batch_axis_value =
          static_cast<int32_t>(in_batch_axis->value());
      if (in_batch_axis->has_value() &&
          std::find(fixed_axes_vec.begin(), fixed_axes_vec.end(),
                    in_batch_axis_value) == fixed_axes_vec.end()) {
        DimVector dim_vec = in_desc.shape().dim_vec();
        CheckAndLabelAxesToSqueezeMinusOne(fixed_axes_vec, &dim_vec);
        const int32_t cnt = std::count(
            dim_vec.begin(), dim_vec.begin() + in_batch_axis_value, -1);
