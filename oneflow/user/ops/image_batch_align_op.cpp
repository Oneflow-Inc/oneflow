...
template <typename T> bool PowerOfTwo(T x) {
...
REGISTER_CPU_ONLY_USER_OP("image_batch_align")
    ...
    .SetCheckAttrFn([](const user_op::UserOpDefWrapper &def,
      ...
      if (shape.NumAxes() != 3) {
        err << ", shape: " << shape.ToString()
            << " (image shape must has 3 axes)";
        check_failed = true;
      }
    ...
    .SetTensorDescInferFn([](user_op::InferContext *ctx) -> Maybe<void> {
      ...
      FOR_RANGE(int64_t, i, 0, shape_attr.NumAxes()) {
        dim_vec.at(i + 1) = shape_attr.At(i);
      }
