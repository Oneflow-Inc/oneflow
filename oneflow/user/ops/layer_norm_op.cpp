...
Shape InferBnParamShape(const Shape &x_shape, const int64_t begin_norm_axis) {
  ...
  bn_param_shape_dim_vec.insert(bn_param_shape_dim_vec.end(),
                                x_shape.dim_vec().cbegin(),
...
REGISTER_USER_OP("layer_norm")
    ...
    .SetTensorDescInferFn([](user_op::InferContext *ctx) -> Maybe<void> {
      ...
      user_op::TensorDesc *inv_variance =
          ctx->TensorDesc4ArgNameAndIndex("inv_variance", 0);
      const bool center = ctx->Attr<bool>("center");
      const bool scale = ctx->Attr<bool>("scale");
      const int64_t begin_params_axis = ShiftNegativeAxisIfNeed(
          x->shape(), ctx->Attr<int64_t>("begin_params_axis"));
      *y = *x;
      DimVector param_shape_dim_vec;
      param_shape_dim_vec.insert(param_shape_dim_vec.end(),
                                 x->shape().dim_vec().cbegin() +
                                     begin_params_axis,
                                 x->shape().dim_vec().cend());
      if (param_shape_dim_vec.empty()) {
        param_shape_dim_vec.push_back(1);
      }
      const Shape param_shape(param_shape_dim_vec);
      if (center) {
        const user_op::TensorDesc *beta =
            ctx->TensorDesc4ArgNameAndIndex("beta", 0);
        CHECK_OR_RETURN(ctx->parallel_ctx().parallel_num() == 1 ||
                        ctx->SbpParallel4ArgNameAndIndex("beta", 0)
                            .has_broadcast_parallel());
        CHECK_EQ_OR_RETURN(beta->shape(), param_shape);
        CHECK_EQ_OR_RETURN(beta->data_type(), x->data_type());
      }
      if (scale) {
        user_op::TensorDesc *normalized =
            ctx->TensorDesc4ArgNameAndIndex("normalized", 0);
        const user_op::TensorDesc *gamma =
            ctx->TensorDesc4ArgNameAndIndex("gamma", 0);
        CHECK_OR_RETURN(ctx->parallel_ctx().parallel_num() == 1 ||
                        ctx->SbpParallel4ArgNameAndIndex("gamma", 0)
                            .has_broadcast_parallel());
      ...
      const int64_t begin_norm_axis = ShiftNegativeAxisIfNeed(
          x->shape(), ctx->Attr<int64_t>("begin_norm_axis"));
...
REGISTER_USER_OP("layer_norm_grad")
    ...
    .SetTensorDescInferFn([](user_op::InferContext *ctx) -> Maybe<void> {
      ...
      const user_op::TensorDesc *x = ctx->TensorDesc4ArgNameAndIndex("x", 0);
      const user_op::TensorDesc *mean =
          ctx->TensorDesc4ArgNameAndIndex("mean", 0);
      const user_op::TensorDesc *inv_variance =
          ctx->TensorDesc4ArgNameAndIndex("inv_variance", 0);
      ...
      const Shape &bn_param_shape =
          InferBnParamShape(x->shape(), begin_norm_axis);
...
REGISTER_USER_OP("layer_norm_param_grad")
    ...
    .SetTensorDescInferFn([](user_op::InferContext *ctx) -> Maybe<void> {
      ...
      if (has_beta_diff || has_gamma_diff) {
        user_op::TensorDesc *reduce_buf =
            ctx->TensorDesc4ArgNameAndIndex("reduce_buf", 0);
        *reduce_buf = *dy;
      }
      ...
      const Shape param_shape(param_shape_dim_vec);
      if (has_beta_diff) {
        user_op::TensorDesc *beta_diff =
            ctx->TensorDesc4ArgNameAndIndex("beta_diff", 0);
        *beta_diff->mut_data_type() = dy->data_type();
        *beta_diff->mut_shape() = param_shape;
      }
      if (has_gamma_diff) {
        user_op::TensorDesc *gamma_diff =
            ctx->TensorDesc4ArgNameAndIndex("gamma_diff", 0);
        const user_op::TensorDesc *normalized =
            ctx->TensorDesc4ArgNameAndIndex("normalized", 0);
      ...
      if (has_gamma) {
        ...
        CHECK_OR_RETURN(ctx->parallel_ctx().parallel_num() == 1 ||
            ...
            << "gamma sbp:"
            << ctx->SbpParallel4ArgNameAndIndex("gamma", 0).DebugString();
...
REGISTER_USER_OP_GRAD("layer_norm")
    .SetGenBackwardOpConfFn([](const user_op::UserOpWrapper &op,
      ...
      const bool has_beta_diff =
          has_beta && op.NeedGenGradTensor4OpInput("beta", 0);
      const bool has_gamma_diff =
          has_gamma && op.NeedGenGradTensor4OpInput("gamma", 0);
      const bool need_scale_out_diff =
          has_gamma && op.NeedGenGradTensor4OpInput("x", 0);
      ...
      const int64_t begin_params_axis = ShiftNegativeAxisIfNeed(
          x_shape, op.attr<int64_t>("begin_params_axis"));
      ...
      if (has_beta_diff || has_gamma_diff || need_scale_out_diff) {
        ...
        auto grad_op_builder =
            builder.Op("layer_norm_param_grad")
        ...
        if (has_beta_diff || has_gamma_diff) {
          grad_op_builder.Output("reduce_buf");
        }
        auto grad_op = grad_op_builder.Build();
        if (has_beta_diff) {
          op.BindGradTensorWithOpInput(grad_op.output("beta_diff", 0), "beta",
                                       0);
        }
        if (has_gamma_diff) {
          op.BindGradTensorWithOpInput(grad_op.output("gamma_diff", 0), "gamma",
                                       0);
        }
        if (need_scale_out_diff) {
          dy = grad_op.output("normalized_diff", 0);
        }
        AddOp(grad_op);
      }
