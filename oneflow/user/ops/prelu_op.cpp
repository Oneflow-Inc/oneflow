...
REGISTER_USER_OP("prelu_alpha_grad")
    ...
    .SetTensorDescInferFn([](user_op::InferContext *ctx) -> Maybe<void> {
      ...
      const user_op::TensorDesc *dy_desc =
          ctx->TensorDesc4ArgNameAndIndex("dy", 0);
      const user_op::TensorDesc *alpha_desc =
          ctx->TensorDesc4ArgNameAndIndex("alpha", 0);
      CHECK_EQ_OR_RETURN(x_desc->shape().NumAxes(),
                         alpha_desc->shape().NumAxes() + 1);
      FOR_RANGE(int64_t, i, 1, x_desc->shape().NumAxes()) {
        CHECK_OR_RETURN(
            (alpha_desc->shape().At(i - 1) == x_desc->shape().At(i)) ||
      ...
      if (op.NeedGenGradTensor4OpInput("alpha", 0)) {
        ...
        user_op::UserOpConfWrapper alpha_grad_op =
            builder.Op("prelu_alpha_grad")
                ...
                .Output("alpha_diff")
                .Build();
        op.BindGradTensorWithOpInput(alpha_grad_op.output("alpha_diff", 0),
                                     "alpha", 0);
        AddOp(alpha_grad_op);
      }
    });
