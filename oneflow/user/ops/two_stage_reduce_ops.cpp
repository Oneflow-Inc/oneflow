...
Maybe<void> InferReduceDeviceStageGradTensorDescFn(user_op::InferContext *ctx) {
  ...
  *ctx->Shape4ArgNameAndIndex("in_diff", 0) =
      *ctx->Shape4ArgNameAndIndex("mask", 0);
  *ctx->Dtype4ArgNameAndIndex("in_diff", 0) =
      *ctx->Dtype4ArgNameAndIndex("out_diff", 0);
...
Maybe<void> InferReduceGlobalStageTensorDescFn(user_op::InferContext *ctx) {
  ...
  CHECK_EQ_OR_RETURN(*ctx->Dtype4ArgNameAndIndex("device_count", 0),
                     DataType::kInt32);
  const auto &axis = ctx->Attr<std::vector<int32_t>>("axis");
...
Maybe<void>
InferReduceDeviceStageGradBatchAxisFn(user_op::BatchAxisContext *ctx) {
  const auto &reduced_axes = ctx->Attr<std::vector<int32_t>>("axis");
  HashSet<int32_t> conf_axes = {reduced_axes.begin(), reduced_axes.end()};
  const auto *out_diff_batch_axis =
      ctx->BatchAxis4ArgNameAndIndex("out_diff", 0);
...
#define REGISTER_REDUCE_DEVICE_STAGE_USER_OP_GRAD(op_type_name,                \
                                                  grad_op_type_name)           \
  REGISTER_USER_OP_GRAD(op_type_name)                                          \
      .SetGenBackwardOpConfFn(                                                 \
          [](const user_op::UserOpWrapper &op, user_op::AddOpFn AddOp) {       \
            return GenBackwardOpConf4ReduceDeviceStage(grad_op_type_name, op,  \
                                                       AddOp);                 \
          });
REGISTER_REDUCE_DEVICE_STAGE_USER_OP_GRAD("reduce_min_device_stage",
                                          "reduce_min_device_stage_grad")
REGISTER_REDUCE_DEVICE_STAGE_USER_OP_GRAD("reduce_max_device_stage",
...
#define REGISTER_REDUCE_GLOBAL_STAGE_USER_OP_GRAD(op_type_name,                \
                                                  grad_op_type_name)           \
  REGISTER_USER_OP_GRAD(op_type_name)                                          \
      .SetGenBackwardOpConfFn(                                                 \
          [](const user_op::UserOpWrapper &op, user_op::AddOpFn AddOp) {       \
            return GenBackwardOpConf4ReduceGlobalStage(grad_op_type_name, op,  \
                                                       AddOp);                 \
          });
REGISTER_REDUCE_GLOBAL_STAGE_USER_OP_GRAD("reduce_min_global_stage",
                                          "reduce_min_global_stage_grad")
REGISTER_REDUCE_GLOBAL_STAGE_USER_OP_GRAD("reduce_max_global_stage",
