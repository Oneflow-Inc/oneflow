...
#define REGISTER_MATH_BINARY_ELEMENTWISE_OP_AND_GRAD(                          \
    math_binary_elementwise_type, func_prefix)                                 \
  REGISTER_USER_OP(math_binary_elementwise_type)                               \
      ...
      .MATH_ELEMENTWISE_DEFAULT_SET_FUNC();                                    \
                                                                               \
  REGISTER_USER_OP(                                                            \
      (std::string("") + math_binary_elementwise_type + "_x_grad"))            \
      .Input("x")                                                              \
      .Input("y")                                                              \
      .Input("dz")                                                             \
      .Output("dx")                                                            \
      .MATH_ELEMENTWISE_DEFAULT_SET_FUNC();                                    \
                                                                               \
  REGISTER_USER_OP(                                                            \
      (std::string("") + math_binary_elementwise_type + "_y_grad"))            \
      .Input("x")                                                              \
      .Input("y")                                                              \
      .Input("dz")                                                             \
      .Output("dy")                                                            \
      ...
      .SetGenBackwardOpConfFn([](const user_op::UserOpWrapper &op,             \
        ...
        if (op.NeedGenGradTensor4OpInput("x", 0)) {                            \
          ...
          user_op::UserOpConfWrapper binary_grad_op =                          \
              builder                                                          \
                  .Op(std::string("") + math_binary_elementwise_type +         \
                      "_x_grad")                                               \
                  ...
                  .Output("dx")                                                \
                  .Build();                                                    \
          op.BindGradTensorWithOpInput(binary_grad_op.output("dx", 0), "x",    \
                                       0);                                     \
        ...
        if (op.NeedGenGradTensor4OpInput("y", 0)) {                            \
          ...
          user_op::UserOpConfWrapper binary_grad_op =                          \
              builder                                                          \
                  ...
                  .Output("dy")                                                \
                  .Build();                                                    \
          op.BindGradTensorWithOpInput(binary_grad_op.output("dy", 0), "y",    \
                                       0);                                     \
          AddOp(binary_grad_op);                                               \
        }                                                                      \
      });
