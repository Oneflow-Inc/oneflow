...
Maybe<void> AddGradMsSignature(user_op::SbpContext *ctx) {
  ...
  ctx->NewBuilder()
      ...
      .Split(user_op::OpArg("prediction_diff", 0),
             prediction.shape().NumAxes() - 1)
      .Build();
  return Maybe<void>::Ok();
}
...
REGISTER_SPAESE_CROSS_ENTROPY_USER_OP("sparse_cross_entropy_ms",
                                      AddMsSignature);
REGISTER_SPAESE_CROSS_ENTROPY_GRAD_USER_OP("sparse_cross_entropy_grad",
                                           AddGradSignature);
...
REGISTER_USER_OP_GRAD("sparse_cross_entropy")
    .SetGenBackwardOpConfFn([](const user_op::UserOpWrapper &op,
                               user_op::AddOpFn AddOp) {
      return GenBackwardOpConf4SparseCrossEntropy("sparse_cross_entropy_grad",
                                                  op, AddOp);
    });
...
REGISTER_USER_OP_GRAD("sparse_cross_entropy_ms")
    .SetGenBackwardOpConfFn([](const user_op::UserOpWrapper &op,
                               user_op::AddOpFn AddOp) {
      return GenBackwardOpConf4SparseCrossEntropy(
          "sparse_cross_entropy_ms_grad", op, AddOp);
    });
