...
Maybe<void> InferTensorDescFn(user_op::InferContext *ctx) {
  const user_op::TensorDesc *prediction_desc =
  ...
  FOR_RANGE(int64_t, i, 0, num_out_axes) {
    CHECK_EQ_OR_RETURN(prediction_desc->shape().At(i),
                       label_desc->shape().At(i));
  }
  *ctx->TensorDesc4ArgNameAndIndex("prob", 0) =
      *prediction_desc; //'prob' is just for compute prediction's grad, prob's
                        //grad will be ignored
...
Maybe<void> InferGradTensorDescFn(user_op::InferContext *ctx) {
  const user_op::TensorDesc *prob_desc =
      ctx->TensorDesc4ArgNameAndIndex("prob", 0);
  const user_op::TensorDesc *label_desc =
      ctx->TensorDesc4ArgNameAndIndex("label", 0);
...
Maybe<void> AddGradMsSignature(user_op::SbpContext *ctx) {
  const user_op::TensorDesc &prob =
      ctx->LogicalTensorDesc4InputArgNameAndIndex("prob", 0);
  ctx->NewBuilder()
...
void GenBackwardOpConf4SparseSoftmaxCrossEntropy(
    const std::string &op_type_name, const user_op::UserOpWrapper &op,
  ...
  if (op.NeedGenGradTensor4OpInput("prediction", 0)) {
    ...
    user_op::UserOpConfWrapper grad_op =
        builder.Op(op_type_name)
            ...
            .Build();
    op.BindGradTensorWithOpInput(grad_op.output("prediction_diff", 0),
                                 "prediction", 0);
    AddOp(grad_op);
  }
}
  ...
  REGISTER_USER_OP(op_name)                                                    \
      ...
      .SetInputArgModifyFn(                                                    \
          [](user_op::GetInputArgModifier GetInputArgModifierFn,               \
            ...
            user_op::InputArgModifier *label_modifier =                        \
                GetInputArgModifierFn("label", 0);                             \
...
REGISTER_SPAESE_SOFTMAX_CROSS_ENTROPY_USER_OP("sparse_softmax_cross_entropy",
                                              AddSignature);
REGISTER_SPAESE_SOFTMAX_CROSS_ENTROPY_USER_OP("sparse_softmax_cross_entropy_ms",
                                              AddMsSignature);
REGISTER_SPAESE_SOFTMAX_CROSS_ENTROPY_GRAD_USER_OP(
    "sparse_softmax_cross_entropy_grad", AddGradSignature);
REGISTER_SPAESE_SOFTMAX_CROSS_ENTROPY_GRAD_USER_OP(
    "sparse_softmax_cross_entropy_ms_grad", AddGradMsSignature);

REGISTER_USER_OP_GRAD("sparse_softmax_cross_entropy")
    .SetGenBackwardOpConfFn([](const user_op::UserOpWrapper &op,
                               user_op::AddOpFn AddOp) {
      return GenBackwardOpConf4SparseSoftmaxCrossEntropy(
          "sparse_softmax_cross_entropy_grad", op, AddOp);
    });
...
REGISTER_USER_OP_GRAD("sparse_softmax_cross_entropy_ms")
    .SetGenBackwardOpConfFn([](const user_op::UserOpWrapper &op,
                               user_op::AddOpFn AddOp) {
      return GenBackwardOpConf4SparseSoftmaxCrossEntropy(
          "sparse_softmax_cross_entropy_ms_grad", op, AddOp);
    });
