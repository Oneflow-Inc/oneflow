...
REGISTER_USER_OP("nll")
    ...
    .SetGetSbpFn([](user_op::SbpContext* ctx) -> Maybe<void> {
      const auto& out_tensor = ctx->LogicalTensorDesc4InputArgNameAndIndex("out", 0);
...
REGISTER_USER_OP("nll_grad")
    ...
    .SetGetSbpFn([](user_op::SbpContext* ctx) -> Maybe<void> {
      const auto& dy_tensor = ctx->LogicalTensorDesc4InputArgNameAndIndex("dy", 0);
      auto builder = ctx->NewBuilder()
                         .Split(user_op::OpArg("input", 0), 0)
                         .Split(user_op::OpArg("target", 0), 0)
                         .Broadcast(user_op::OpArg("weight", 0))
