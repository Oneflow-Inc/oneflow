...
REGISTER_USER_OP("diagonal")
    ...
    .SetTensorDescInferFn([](user_op::InferContext* ctx) -> Maybe<void> {
      ...
      CHECK_EQ_OR_RETURN(in_dim,3);
      
      DimVector out_dim_vec = {in_shape.At(2),0};
      if (offset >= 0) {
          out_dim_vec[1] = std::min(in_shape.At(0), in_shape.At(1) - offset);
      } else {
          out_dim_vec[1] = std::min(in_shape.At(0) + offset, in_shape.At(1));
      }
...
REGISTER_USER_OP_GRAD("diagonal").SetBackwardOpConfGenFn([](user_op::BackwardOpConfContext* ctx)
                                                         -> Maybe<void> {
