...
private: 
    void Compute(user_op::KernelComputeContext* ctx) const override{
        ...
        user_op::Tensor* relu_mask = ctx->Tensor4ArgNameAndIndex("relu_mask", 0); 
        ...
        const cudaDataType_t cublas_dtype = GetComputeType(data_type); 
        ...
        OF_CUBLAS_CHECK(cublasLtMatmulDescCreate(&operationDesc, cublas_dtype));
        ...
        OF_CUBLAS_CHECK(cublasLtMatmulDescSetAttribute(operationDesc, CUBLASLT_MATMUL_DESC_EPILOGUE, &CUBLASLT_EPILOGUE_RELU_AUX_BIAS, sizeof(CUBLASLT_EPILOGUE_RELU_AUX_BIAS))); 
        ...
        // Set bias ptr
        const T* bias = reinterpret_cast<const T*>(bias->dptr()); 
        ...
        // OF_CUBLAS_CHECK(cublasLtMatrixLayoutCreate(&Adesc, cublas_dtype, m, k, cublas_lda)); 
        // OF_CUBLAS_CHECK(cublasLtMatrixLayoutCreate(&Bdesc, cublas_dtype, k, n, cublas_ldb)); 
        // OF_CUBLAS_CHECK(cublasLtMatrixLayoutCreate(&Cdesc, cublas_dtype, m, n, cublas_ldc));
        // todo! 
        OF_CUBLAS_CHECK(cublasLtMatrixLayoutCreate(&Adesc, cublas_dtype, k, m, cublas_lda)); 
        OF_CUBLAS_CHECK(cublasLtMatrixLayoutCreate(&Bdesc, cublas_dtype, n, k, cublas_ldb)); 
        OF_CUBLAS_CHECK(cublasLtMatrixLayoutCreate(&Cdesc, cublas_dtype, n, m, cublas_ldc)); 


        OF_CUBLAS_CHECK(cublasLtMatmul(ctx->stream()->As<CudaStream>()->cublas_lt_handle(),
                                       operationDesc,
                                       alpha,
...
#define REGISTER_MATMUL_BIAS_ADD_RELU_KERNEL_GPU(cpp_type, data_type)                                     \
  REGISTER_USER_KERNEL("fused_matmul_bias_add_relu").SetCreateFn<FusedMatmulBiasAddReluKernel<cpp_type>>().SetIsMatchedHob( \
    ...
    && (user_op::HobDataType("relu_mask", 0) == GetDataType<int8_t>::value))
