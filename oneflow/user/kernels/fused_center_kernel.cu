...
template<typename T>
__global__ void FusedCenterForward(
  const int n,
  const T* b1_x1, const T* b1_x2, const T* b2_x1, const T* b2_x2,
  const T* b1_y1, const T* b1_y2, const T* b2_y1, const T* b2_y2,
  T* rho) {
  CUDA_1D_KERNEL_LOOP(i, n) {
    rho[i] = (static_cast<T>(pow(b2_x1[i] + b2_x2[i] - b1_x1[i] - b1_x2[i], 2)) + \
              static_cast<T>(pow(b2_y1[i] + b2_y2[i] - b1_y1[i] - b1_y2[i], 2))) / static_cast<T>(4.0);
  }
}

template<typename T>
__global__ void FusedCenterBackward(
    const int n, 
    const T* b1_x1, const T* b1_x2, const T* b2_x1, const T* b2_x2,
    const T* b1_y1, const T* b1_y2, const T* b2_y1, const T* b2_y2,
    const T* rho2_diff,
  ...
  CUDA_1D_KERNEL_LOOP(i, n) {
    ...
    b1_x1_diff[i] = rho2_diff_i / static_cast<T>((b1_x1_i + b1_x2_i - b2_x1_i - b2_x2_i) / 2);
    b1_x2_diff[i] = rho2_diff_i / static_cast<T>((b1_x1_i + b1_x2_i - b2_x1_i - b2_x2_i) / 2);
    b2_x1_diff[i] = rho2_diff_i / static_cast<T>((b2_x1_i + b2_x2_i - b1_x1_i - b1_x2_i) / 2);
    b2_x2_diff[i] = rho2_diff_i / static_cast<T>((b2_x1_i + b2_x2_i - b1_x1_i - b1_x2_i) / 2);

    b1_y1_diff[i] = rho2_diff_i / static_cast<T>((b1_y1_i + b1_y2_i - b2_y1_i - b2_y2_i) / 2);
    b1_y2_diff[i] = rho2_diff_i / static_cast<T>((b1_y1_i + b1_y2_i - b2_y1_i - b2_y2_i) / 2);
    b2_y1_diff[i] = rho2_diff_i / static_cast<T>((b2_y1_i + b2_y2_i - b1_y1_i - b1_y2_i) / 2);
    b2_y2_diff[i] = rho2_diff_i / static_cast<T>((b2_y1_i + b2_y2_i - b1_y1_i - b1_y2_i) / 2);
  }
}
...
class FusedCenterKernel final : public user_op::OpKernel {
 ...
 private:
  ...
  void Compute(user_op::KernelComputeContext* ctx) const override {
    ...
    RUN_CUDA_KERNEL((FusedCenterForward<T>), ctx->stream(), elem_cnt, elem_cnt,
      b1_x1->dptr<T>(),
      b1_x2->dptr<T>(),
      b2_x1->dptr<T>(),
      b2_x2->dptr<T>(),
...
class FusedCenterGradKernel final : public user_op::OpKernel {
 ...
 private:
  ...
  void Compute(user_op::KernelComputeContext* ctx) const override {
    ...
    RUN_CUDA_KERNEL((FusedCenterBackward<T>), ctx->stream(), elem_cnt, elem_cnt, 
      b1_x1->dptr<T>(), b1_x2->dptr<T>(), b2_x1->dptr<T>(), b2_x2->dptr<T>(),
      b1_y1->dptr<T>(), b1_y2->dptr<T>(), b2_y1->dptr<T>(), b2_y2->dptr<T>(),
      rho2_diff->dptr<T>(),
      b1_x1_diff->mut_dptr<T>(), b1_x2_diff->mut_dptr<T>(), b2_x1_diff->mut_dptr<T>(), b2_x2_diff->mut_dptr<T>(),
      b1_y1_diff->mut_dptr<T>(), b1_y2_diff->mut_dptr<T>(), b2_y1_diff->mut_dptr<T>(), b2_y2_diff->mut_dptr<T>()
    ); 
  }
  bool AlwaysComputeWhenAllOutputsEmpty() const override { return false; }
};
...
#define REGISTER_FUSED_CENTER_GRAD_CUDA_KERNEL(dtype)                    \
  REGISTER_USER_KERNEL("fused_center_grad")                              \
      ...
      .SetIsMatchedHob((user_op::HobDeviceType() == DeviceType::kCUDA)   \
                       && (user_op::HobDataType("rho2_diff", 0) == GetDataType<dtype>::value));
