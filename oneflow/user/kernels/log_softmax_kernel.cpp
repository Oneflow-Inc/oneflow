...
user_op::InferTmpSizeFn GenFwInferTmpSizeFn() {
  return [](user_op::InferContext* ctx) {
    ...
    return SoftmaxComputeProbTempStorageSize<T, SoftmaxType::kLogSoftmax>(num_instances,
                                                                          num_classes);
  };
}
...
class LogSoftmaxGradKernel final : public user_op::OpKernel {
 ...
 private:
  void Compute(user_op::KernelComputeContext* ctx) const override {
    ...
    SoftmaxKernelUtil<device_type, SoftmaxType::kLogSoftmax, T>::ComputeDiff(
...
user_op::InferTmpSizeFn GenBwInferTmpSizeFn() {
  return [](user_op::InferContext* ctx) {
    ...
    return SoftmaxComputeDiffTempStorageSize<T, SoftmaxType::kLogSoftmax>(num_instances,
                                                                          num_classes);
  };
}
