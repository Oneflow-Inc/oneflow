...
template<typename T>
__global__ void IndexToOffsetForwardGpuKernel(int32_t in_num, int32_t ndim, const T* index,
...
struct NdIndexToOffsetFunctor<DeviceType::kGPU, T> final {
  void operator()(DeviceCtx* ctx, int32_t in_num, int32_t ndim, const T* index,
                  const T* dims_tensor, T* out) {
    RUN_CUDA_KERNEL((IndexToOffsetForwardGpuKernel<T>), ctx, in_num, in_num, ndim, index,
                    dims_tensor, out);
  }
};
