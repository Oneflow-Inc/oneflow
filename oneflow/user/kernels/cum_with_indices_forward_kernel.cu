...
template<typename T>
struct MaxFunctor {
  __device__ __forceinline__ bool operator()(const T a, const T b) const { return a > b; }
};
template<typename T>
struct MinFunctor {
  __device__ __forceinline__ bool operator()(const T a, const T b) const { return a < b; }
...
__device__ typename std::enable_if<!std::is_integral<T>::value, void>::type BinaryOpUpdate(
...
template<typename T, template<typename> class BinaryFunc>
__global__ void CumForwardGpu(const T* in_ptr, T* out_ptr, int64_t* indices_ptr,
                              int64_t cs_up_space, int64_t cs_space, int64_t cs_down_space) {
  CUDA_1D_KERNEL_LOOP(i, cs_up_space * cs_down_space) {
    ...
    auto* indices_ptr_base =
        indices_ptr + cs_up_space_id * cs_space * cs_down_space + cs_down_space_id;

    // calculate cs_space data in one thread
    for (auto j = 0; j < cs_space; j++) {
      auto idx = j * cs_down_space;
      out_ptr_base[idx] = in_ptr_base[idx];
      indices_ptr_base[idx] = j;
      if (j != 0) {
        BinaryOpUpdate(out_ptr_base[idx - cs_down_space], out_ptr_base[idx],
                       indices_ptr_base[idx - cs_down_space], indices_ptr_base[idx],
                       BinaryFunc<T>());
      }
    }
  }
...
void ScanOuterDim(ep::Stream* ep_stream, const ShapeView& in_shape, int64_t dim, const T* in_ptr,
  ...
  RUN_CUDA_KERNEL((CumForwardGpu<T, BinaryFunc>), ep_stream, thread_num, in_ptr, out_ptr,
                  indices_ptr, up_space, space, down_space);
}
...
__device__ void ScanInnerMostDimKernelWithIndicesImpl(T* row_buf, int64_t* row_idx_buf, T* src_,
                                                      T* tgt_, int64_t* indices_,
  ...
  for (uint32_t block_row = blockIdx.x * blockDim.y; block_row < num_rows;
    ...
    for (uint32_t block_col = 0; block_col < row_size; block_col += 2 * num_threads_x) {
      ...
      if (row < num_rows) {
        ...
        if (threadIdx.x == 0) {
          BinaryOpUpdate(block_total, row_buf[0], block_idx_final, row_idx_buf[0], BinaryFunc<T>());
        }
      }
      __syncthreads();
      ...
      for (uint32_t s = num_threads_x, d = 1; s >= 1; s >>= 1, d <<= 1) {
        if (row < num_rows && threadIdx.x < s) {
          ...
          BinaryOpUpdate(row_buf[offset], row_buf[offset + d], row_idx_buf[offset],
                         row_idx_buf[offset + d], BinaryFunc<T>());
        }
        __syncthreads();
      }
      ...
      if (row < num_rows) {
        if (col1 < row_size) {
          row_tgt[col1] = row_buf[threadIdx.x];
...
template<typename T, int num_threads_x, int num_threads_y, template<typename> class BinaryFunc>
__global__ void ScanInnerMostDimKernel(const T* in_ptr, T* out_ptr, int64_t* indices_ptr,
                                       const int64_t num_rows, const int64_t row_size, T init) {
  __shared__ T sbuf[num_threads_y][2 * num_threads_x];
  __shared__ int64_t idx_sbuf[num_threads_y][2 * num_threads_x];
  T* row_buf = sbuf[threadIdx.y];
  int64_t* row_idx_buf = idx_sbuf[threadIdx.y];
  ScanInnerMostDimKernelWithIndicesImpl<T, num_threads_x, num_threads_y, BinaryFunc>(
      row_buf, row_idx_buf, const_cast<T*>(in_ptr), out_ptr, indices_ptr, num_rows, row_size, init);
}

template<typename T, template<typename> class BinaryFunctor>
void ScanInnerMostDim(const T* in_ptr, T* out_ptr, int64_t* indices_ptr, const int64_t num_rows,
                      const int64_t row_size, const ep::CudaStream* cuda_stream) {
  dim3 block(16, 32);
  ...
  if (std::is_same<BinaryFunctor<T>, MaxFunctor<T>>::value) {
    ScanInnerMostDimKernel<T, 16, 32, MaxFunctor><<<grid, block, 0, cuda_stream->cuda_stream()>>>(
        in_ptr, out_ptr, indices_ptr, num_rows, row_size,
        /*init*/ GetMinVal<T>());
  } else if (std::is_same<BinaryFunctor<T>, MinFunctor<T>>::value) {
    ScanInnerMostDimKernel<T, 16, 32, MinFunctor><<<grid, block, 0, cuda_stream->cuda_stream()>>>(
        in_ptr, out_ptr, indices_ptr, num_rows, row_size,
        /*init*/ GetMaxVal<T>());
  } else {
    UNIMPLEMENTED() << "Only Support cummax and cummin for now.";
  }
}
...
class GpuCumKernel : public user_op::OpKernel {
 ...
 private:
  ...
  void Compute(user_op::KernelComputeContext* ctx) const override {
    ...
    auto* indices_ptr = indices->mut_dptr<int64_t>();

    const auto* cuda_stream = ctx->stream()->As<ep::CudaStream>();

    if (dim == in_shape.NumAxes() - 1) {
      // Treat all outer dimension as a single dimension.
      const int64_t num_rows = elem_cnt / dim_size;
      ScanInnerMostDim<T, BinaryFunc>(in_ptr, out_ptr, indices_ptr, num_rows, dim_size,
                                      cuda_stream);
    } else {
      ScanOuterDim<T, BinaryFunc>(ctx->stream(), in_shape, dim, in_ptr, out_ptr, indices_ptr);
    }
  }
