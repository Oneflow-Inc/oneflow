...
struct PoolOpKernelState final : public user_op::OpKernelState {
  ...
  void Update(const ShapeView &x_shape) {
    if (is_dynamic) {
      params_3d.Reset(x_shape);
    }
  }
};

template <typename T> struct PoolCpuKernelUtil {
public:
  ...
  typedef std::function<void(const int64_t size, const int64_t col,
                             EigenMatrixMap<T> &out_mat)>
      CLastFinalize;
  typedef std::function<void(const T &in, const T &out, const T &out_diff,
                             const int64_t size, T &in_diff)>
      CFirstProcessGrad;
  ...
  static void
  CFirstForward(const Params3D &params_3d, const user_op::Tensor *in_blob,
        ...
        FOR_RANGE(int64_t, pd, 0, out.At(2)) {
          ...
          FOR_RANGE(int64_t, ph, 0, out.At(3)) {
            ...
            FOR_RANGE(int64_t, pw, 0, out.At(4)) {
              ...
              finalize((dend - dstart) * (hend - hstart) * (wend - wstart),
                       res);
  ...
  static void CFirstBackward(const Params3D &params_3d,
              ...
              FOR_RANGE(int64_t, d, dstart, dend) {
                FOR_RANGE(int64_t, h, hstart, hend) {
                  FOR_RANGE(int64_t, w, wstart, wend) {
                    ...
                    process(input[index], output[pool_index],
                            output_diff[pool_index], size, input_diff[index]);
                  }
                }
              }
            }
  ...
  static void CLastForward(const Params3D &params_3d,
                           const user_op::Tensor *in_blob,
                           user_op::Tensor *out_blob,
                           const ForwardInitialize &forward_initialize,
                           const CLastProcess &process,
                           const CLastFinalize &finalize) {
    const Shape &in = params_3d.GetXShape5D();
    const Shape &out = params_3d.GetYShape5D();
    const std::vector<int32_t> &pool_size = params_3d.pool_size_3d();
    const std::vector<int32_t> &strides = params_3d.strides_3d();
      ...
      FOR_RANGE(int64_t, pd, 0, out.At(2)) {
        ...
        FOR_RANGE(int64_t, ph, 0, out.At(3)) {
          ...
          FOR_RANGE(int64_t, pw, 0, out.At(4)) {
            ...
            finalize((hend - hstart) * (wend - wstart) * (dend - dstart),
                     out_col, out_mat);
          }
        }
      }
  ...
  static void CLastBackward(const Params3D &params_3d,
                            const user_op::Tensor *out_diff_blob,
                            const user_op::Tensor *out_blob,
                            const user_op::Tensor *in_blob,
                            user_op::Tensor *in_diff_blob,
            ...
            FOR_RANGE(int64_t, d, dstart, dend) {
              FOR_RANGE(int64_t, h, hstart, hend) {
                FOR_RANGE(int64_t, w, wstart, wend) {
                  const int64_t input_index =
                      ((n * in.At(2) + d) * in.At(3) + h) * in.At(4) + w;
                  process(pool_index, input_index, size, out_mat, in_mat,
    ...
    if (data_format == "channels_first") {
      ...
      CLastForward(
          ...
          [](const int64_t size, const int64_t col,
             EigenMatrixMap<T> &out_mat) { out_mat.col(col) /= size; });
    } else {
      UNIMPLEMENTED();
    }
  }
    ...
    if (data_format == "channels_first") {
      ...
      CLastForward(pool_state->GetParams3D(), x, y, GetMinVal<T>,
                   [](const int64_t in_col, const int64_t out_col,
                      ...
                      EigenMatrixMap<T> &out_mat) {
                     out_mat.col(out_col) =
                         out_mat.col(out_col).cwiseMax(in_mat.col(in_col));
                   },
                   [](const int64_t size, const int64_t col,
                      EigenMatrixMap<T> &out_mat) {});
    } else {
  ...
  static void MaxBWCompute(user_op::KernelComputeContext *ctx,
    ...
    if (data_format == "channels_first") {
      CFirstBackward(pool_state->GetParams3D(), dy, y, x, dx,
                     [](const T &in, const T &out, const T &out_diff,
                        const int64_t size, T &in_diff) {
                       if (in == out) {
                         in_diff += out_diff;
                       }
...
DoCreateOpKernelState(user_op::KernelInitContext *ctx, const int32_t &dim) {
  ...
  Params3D params_3d =
      Params3D(dim, x_shape, data_format, padding, padding_before,
               padding_after, pool_size, strides, ceil_mode);
  std::shared_ptr<PoolOpKernelState> state(
      new PoolOpKernelState(params_3d, is_dynamic));
