...
template <typename T, int32_t N> struct GpuAddCaller {
  static void call(user_op::KernelComputeContext *ctx) {
    ...
    gpu_add<T, N><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock, 0,
                    ctx->device_ctx()->cuda_stream()>>>(n, para);
  }
};
...
template <typename T> const AddNKernelRegistry &SingletonRegistry() {
...
template <typename T> const CallFn *LookUpInRegistry(int32_t in_num) {
  auto it = SingletonRegistry<T>().find(in_num);
  if (it == SingletonRegistry<T>().end()) {
    return nullptr;
  }
  return &(it->second);
}
...
template <typename T> class GpuAddNKernel : public user_op::OpKernel {
public:
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    CHECK(caller != nullptr)
        << "GpuAddNKernel: Cannot find registered funtion for in_num: "
...
template <int32_t N> struct GpuAddCaller<float16, N> {
  static void call(user_op::KernelComputeContext *ctx) {
    CHECK_EQ(N, ctx->inputs().size());
    ...
    for (int32_t i = 0; i < N; ++i) {
      para.in[i] = reinterpret_cast<const half *>(
          ctx->Tensor4ArgNameAndIndex("in", i)->dptr<float16>());
    }

    gpu_half_add<N><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock, 0,
                      ctx->device_ctx()->cuda_stream()>>>(n, para);
  }
};
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    CHECK(caller != nullptr)
        << "GpuAddNHalfKernel: Cannot find registered funtion for in_num: "
