...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    const user_op::Tensor *model_diff =
        ctx->Tensor4ArgNameAndIndex("model_diff", 0);
    ...
    L1L2RegularizeGradientKernelUtil<device_type, T>::RegularizeGradient(
        ctx->device_ctx(), out->shape().elem_cnt(), model->dptr<T>(),
  ...
  REGISTER_USER_KERNEL("l1_l2_regularize_gradient")                            \
      ...
      .SetInplaceProposalFn(                                                   \
          [](const user_op::InferContext &,                                    \
            ...
            OF_RETURN_IF_ERROR(                                                \
                AddInplaceArgPairFn("out", 0, "model_diff", 0, true));         \
