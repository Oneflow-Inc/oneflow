...
template <typename T>
__global__ void ComputeEntropyGpu(const int64_t num_instances,
...
__global__ void ComputeEntropyGpuHalf(const int64_t num_instances,
                                      const int64_t num_classes, const half *x,
                                      const half *labels, half *y) {
...
#else
  printf(
      "use half softmax cross entropy need nvcc arch >= 700 and cuda >= 10.0");
  assert(false);
#endif /* defined(__CUDA_ARCH__) && __CUDA_ARCH__ >= 700 && CUDA_VERSION >=    \
          10000 */
}

template <typename T>
__global__ void
ComputeDiffWithSoftmaxGpu(const int64_t elem_cnt, const int64_t num_classes,
...
__global__ void ComputeDiffWithSoftmaxGpuHalf(const int64_t elem_cnt,
...
template <typename T> struct CrossEntropyKernelUtil<DeviceType::kGPU, T> {
  ...
  static void ComputeDiffWithSoftmax(DeviceCtx *ctx, const int64_t elem_cnt,
    ...
    ComputeDiffWithSoftmaxGpu<<<BlocksNum4ThreadsNum(elem_cnt),
                                kCudaThreadsNumPerBlock, 0,
                                ctx->cuda_stream()>>>(elem_cnt, num_classes,
                                                      prob, labels, dy, dx);
  }
};

template <> struct CrossEntropyKernelUtil<DeviceType::kGPU, float16> {
  static void ComputeEntropy(DeviceCtx *ctx, const int64_t num_instances,
                             const int64_t num_classes, const float16 *x,
                             const float16 *labels, float16 *y) {
    cudaMemset(y, 0, sizeof(float16) * num_instances);
    ComputeEntropyGpuHalf<<<BlocksNum4ThreadsNum(num_instances),
                            kCudaThreadsNumPerBlock, 0, ctx->cuda_stream()>>>(
  ...
  static void ComputeDiffWithSoftmax(DeviceCtx *ctx, const int64_t elem_cnt,
                                     const int64_t num_classes,
                                     const float16 *prob, const float16 *labels,
                                     const float16 *dy, float16 *dx) {
    ComputeDiffWithSoftmaxGpuHalf<<<BlocksNum4ThreadsNum(elem_cnt),
                                    kCudaThreadsNumPerBlock, 0,
                                    ctx->cuda_stream()>>>(
