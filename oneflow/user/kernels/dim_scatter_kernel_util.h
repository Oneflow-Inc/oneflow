...
OF_DEVICE_FUNC void DoDimScatterBinOp(const DimOpIndexNdHelper<IDX_T>& src_nd_helper, 
                                      ...
                                      const int64_t elem_cnt, const int32_t dim, const int64_t upper_bound, const IDX_T* index,
  ...
  XPU_1D_KERNEL_LOOP(idx_offset, elem_cnt) {
    ...
    if(idx_offset>upper_bound){
    #if __CUDA_ARCH__
      __trap(); 
    #else
      std::cout<<"The index element "<<idx_offset<<" is out of bounds for dimension "<<dim<<" with size "<<upper_bound<<std::endl;
