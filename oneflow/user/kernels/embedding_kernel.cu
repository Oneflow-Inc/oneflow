...
class GpuEmbeddingRenormKernel final : public user_op::OpKernel {
 ...
 private:
  void Compute(user_op::KernelComputeContext* ctx) const override {
    ...
    Memset<DeviceType::kCUDA>(ctx->stream(), tmp_buf, 0, GetCudaAlignedSize(sizeof(double) * emb_size * 2));
    EmbeddingRenormFunctor<DeviceType::kCUDA, T, index_T>()(ctx->stream(), in_buf, indices_buf, out_buf, 
                                                          max_norm, norm_type, num_indices, emb_size, emb_dim, tmp_buf);
  }
...
class GpuEmbeddingKernel final : public user_op::OpKernel {
 ...
 private:
  void Compute(user_op::KernelComputeContext* ctx) const override {
    ...
    EmbeddingFunctor<DeviceType::kCUDA, T, index_T>()(ctx->stream(), weight_buf, indices_buf, out_buf, 
...
class GpuEmbeddingGradKernel final : public user_op::OpKernel {
 ...
 private:
  void Compute(user_op::KernelComputeContext* ctx) const override {
    ...
    Memset<DeviceType::kCUDA>(ctx->stream(), tmp_buf, 0, GetCudaAlignedSize(sizeof(int32_t) * emb_size));
    EmbeddingGradFunctor<DeviceType::kCUDA,T, index_T>()(ctx->stream(), dy_buf, indices_buf, dx_buf, 
                                              padding_idx, scale_grad_by_freq, num_indices, emb_size, emb_dim, tmp_buf);
  }
  bool AlwaysComputeWhenAllOutputsEmpty() const override { return false; }
};
