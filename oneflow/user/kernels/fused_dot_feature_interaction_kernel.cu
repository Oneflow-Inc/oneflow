...
template<typename T>
__global__ void GatherConcatGpu(int32_t elem_cnt, int32_t out_dim, int32_t interaction_dim,
  ...
  CUDA_1D_KERNEL_LOOP(i, elem_cnt) {
    ...
    } else if (col < output_concat_end_dim + interaction_dim) {
      int32_t gather_col_idx = gather_indices[col - output_concat_end_dim];
      int32_t gather_idx = row * concated_padded_dim * concated_padded_dim + gather_col_idx;
...
__global__ void ScatterSplitGpu(int32_t elem_cnt, int32_t stride_dim, int32_t out_dim,
                                int32_t concated_padded_dim, int32_t output_concat_end_dim,
                                const T* dy, const int32_t* scatter_indices, T* output_concat_grad,
                                T* matmul_out_grad) {
  CUDA_1D_KERNEL_LOOP(i, elem_cnt) {
    ...
    if (col < output_concat_end_dim) {
      ...
      int32_t matmul_idx = row * concated_padded_dim * concated_padded_dim + matmul_col_id;
...
void ConcatFeatures(user_op::KernelComputeContext* ctx, const T* pad_tensor_ptr,
                    const int32_t pad_dim, const int32_t embedding_size) {
  const int32_t feature_input_size = ctx->input_size("features");
  ...
  int32_t out_col_offset = 0;
  for (int32_t i = 0; i < feature_input_size; ++i) {
    const user_op::Tensor* feature = ctx->Tensor4ArgNameAndIndex("features", i);
    const int32_t feature_rows = feature->shape().At(0);
    const int32_t feature_cols = feature->shape().Count(1);
...
void GatherConcatKernel(ep::Stream* stream, int32_t elem_cnt, int32_t out_dim,
  ...
  GatherConcatGpu<<<BlocksNum4ThreadsNum(elem_cnt), kCudaThreadsNumPerBlock, 0,
      ...
      elem_cnt, out_dim, interaction_dim, concated_padded_dim, output_concat_end_dim,
      gather_indices, matmul_out, output_concat_ptr, out_ptr);
}
...
void ScatterSplit(ep::Stream* stream, int32_t batch_size, int32_t out_dim,
  ...
  ScatterSplitGpu<<<BlocksNum4ThreadsNum(elem_cnt), kCudaThreadsNumPerBlock, 0,
                    stream->As<ep::CudaStream>()->cuda_stream()>>>(
      elem_cnt, stride_dim, out_dim, concated_padded_dim, output_concat_end_dim, dy,
      scatter_indices, output_concat_grad, matmul_out_grad_ptr);
}

template<typename T>
void ConcatFeaturesGrad(user_op::KernelComputeContext* ctx, const int32_t batch_size,
                        const int32_t concated_padded_dim, const int32_t embedding_size,
                        const T* concated_features_grad) {
  auto primitive = ep::primitive::NewPrimitive<ep::primitive::CopyNdFactory>(DeviceType::kCUDA, 2);
  DimVector src_shape = {batch_size, concated_padded_dim * embedding_size};
  int32_t in_col_offset = 0;
  for (int32_t i = 0; i < ctx->output_size("features_grad"); ++i) {
    user_op::Tensor* feature_grad = ctx->Tensor4ArgNameAndIndex("features_grad", i);
    const int32_t feature_grad_rows = feature_grad->shape().At(0);
    const int32_t feature_grad_cols = feature_grad->shape().Count(1);
...
class FusedDotFeatureInteractionKernel final : public user_op::OpKernel {
 ...
 private:
  ...
  void Compute(user_op::KernelComputeContext* ctx) const override {
    ...
    const int32_t batch_size = padded_concated_features->shape().At(0);
    int32_t features_concated_dim = 0;
    for (int32_t i = 0; i < ctx->input_size("features"); ++i) {
      features_concated_dim += ctx->TensorDesc4ArgNameAndIndex("features", i)->shape().At(1);
    }
    const int32_t concated_padded_dim = padded_concated_features->shape().At(1);
    const int32_t embedding_size = padded_concated_features->shape().At(2);
    const int32_t out_dim = out->shape().At(1);
    ...
    const int32_t interaction_dim = self_interaction
    ...
    const int32_t pad_dim = concated_padded_dim - features_concated_dim;
    ...
    BatchMatmul(ctx->stream(), padded_concated_features->data_type(), true, batch_size,
                ...
                matmul_out);
    int32_t output_concat_end_dim = 0;
...
class FusedDotFeatureInteractionGradKernel final : public user_op::OpKernel {
 ...
 private:
  ...
  void Compute(user_op::KernelComputeContext* ctx) const override {
    ...
    const int32_t concated_padded_dim = padded_concated_features->shape().At(1);
    const int32_t embedding_size = padded_concated_features->shape().At(2);
    const int32_t out_dim = dy->shape().At(1);
    const bool self_interaction = ctx->Attr<bool>("self_interaction");
    ...
    const int32_t count = batch_size * concated_padded_dim * concated_padded_dim;
...
user_op::InferTmpSizeFn GenFusedDotFeatureInteractionGradInferTmpSizeFn() {
  return [](user_op::InferContext* ctx) {
    ...
    const int32_t batch_size = padded_concated_features_shape.At(0);
    const int32_t concated_padded_dim = padded_concated_features_shape.At(1);
    const int32_t embedding_size = padded_concated_features_shape.At(2);
    size_t matmul_out_grad_size =
        GetCudaAlignedSize(batch_size * concated_padded_dim * concated_padded_dim * sizeof(T));
    size_t transposed_matmul_out_grad_size = matmul_out_grad_size;
    size_t padded_concated_features_grad_size =
