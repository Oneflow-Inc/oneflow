...
public:
  ...
  TmpBufferManager(int32_t capacity, void *ptr, int32_t instance_num)
    ...
    const int32_t key_value_out_aligned_bytes = GetCudaAlignedSize(
    ...
    temp_storage_ptr_ =
        reinterpret_cast<void *>(reinterpret_cast<char *>(key_value_out_ptr_) +
...
size_t InferTempStorageForArgMax(int32_t num_row, int32_t num_col) {
  ...
  auto err =
      cub::DeviceSegmentedReduce::ArgMax<T *, cub::KeyValuePair<int32_t, T> *,
                                         SegmentOffsetIter>(
...
template <typename T>
void ArgMax(const T *in_ptr, int32_t num_row, int32_t num_col,
            void *temp_storage_ptr, int32_t temp_storage_bytes,
            cub::KeyValuePair<int32_t, T> *out_ptr, cudaStream_t stream) {
...
template <typename T>
__global__ void
WriteKeysToOutput(const int32_t instance_num,
                  const cub::KeyValuePair<int32_t, T> *key_value_out_ptr,
                  int32_t *out_ptr) {
  CUDA_1D_KERNEL_LOOP(i, instance_num) {
    out_ptr[i] = key_value_out_ptr[i].key;
  }
}

} // namespace

template <typename T> class GpuArgMaxKernel final : public user_op::OpKernel {
public:
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    TmpBufferManager<T> buffer_manager(tmp_buffer->shape().elem_cnt(),
                                       tmp_buffer->mut_dptr<void>(),
                                       instance_num);

    ArgMax(in->dptr<T>(), instance_num, instance_size,
           buffer_manager.TempStoragePtr(), buffer_manager.TempStorageBytes(),
           buffer_manager.KeyValueOutPtr(), ctx->device_ctx()->cuda_stream());
    WriteKeysToOutput<
        T><<<BlocksNum4ThreadsNum(instance_num), kCudaThreadsNumPerBlock, 0,
             ctx->device_ctx()->cuda_stream()>>>(
        instance_num, buffer_manager.KeyValueOutPtr(),
        out->mut_dptr<int32_t>());
  }
  ...
  REGISTER_USER_KERNEL("argmax")                                               \
      ...
      .SetInferTmpSizeFn([](user_op::InferContext *ctx) {                      \
        ...
        const int32_t instance_num = in_shape->elem_cnt() / instance_size;     \
                                                                               \
        /* Key-Value Out */                                                    \
        int32_t key_value_out_bytes = GetCudaAlignedSize(                      \
            instance_num * sizeof(cub::KeyValuePair<int32_t, dtype>));         \
                                                                               \
        /* CUB Temp Storage */                                                 \
        size_t temp_storage_bytes =                                            \
            InferTempStorageForArgMax<dtype>(instance_num, instance_size);     \
                                                                               \
