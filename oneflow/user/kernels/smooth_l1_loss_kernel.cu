...
template <typename T>
__global__ void SmoothL1LossForward(const int64_t elem_cnt, const T *prediction,
                                    const T *label, const T beta, T *loss) {
...
__global__ void SmoothL1LossBackward(const int64_t elem_cnt, const T *loss_grad,
                                     const T *prediction, const T *label,
                                     const T beta, T *prediction_grad) {
  CUDA_1D_KERNEL_LOOP(i, elem_cnt) {
    ...
    if (abs_diff < beta) {
      ...
      prediction_grad[i] =
          ((diff > GetZeroVal<T>()) - (diff < GetZeroVal<T>())) * loss_grad[i];
    }
  }
}
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    const user_op::Tensor *prediction_blob =
    ...
    SmoothL1LossForward<
        T><<<BlocksNum4ThreadsNum(elem_cnt), kCudaThreadsNumPerBlock, 0,
             ctx->device_ctx()->cuda_stream()>>>(elem_cnt, prediction, label,
                                                 beta, loss);
  }
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    const T *label = ctx->Tensor4ArgNameAndIndex("label", 0)->dptr<T>();
    T *prediction_grad =
        ctx->Tensor4ArgNameAndIndex("prediction_grad", 0)->mut_dptr<T>();
    SmoothL1LossBackward<
        T><<<BlocksNum4ThreadsNum(elem_cnt), kCudaThreadsNumPerBlock, 0,
             ctx->device_ctx()->cuda_stream()>>>(
        elem_cnt, loss_grad, prediction, label, beta, prediction_grad);
  }
  bool AlwaysComputeWhenAllOutputsEmpty() const override { return false; }
};
...
#define REGISTER_SMOOTH_L1_LOSS_GRAD_GPU_KERNEL(dtype)                         \
  REGISTER_USER_KERNEL("smooth_l1_loss_grad")                                  \
      ...
      .SetIsMatchedHob((user_op::HobDeviceTag() == "gpu") &                    \
                       (user_op::HobDataType("prediction_grad", 0) ==          \
