...
public:
  ...
  CudnnPoolDesc(cudnnPoolingMode_t pooling_mode, int dims, const int *window,
                const int *padding, const int *stride) {
    OF_CUDNN_CHECK(cudnnCreatePoolingDescriptor(&val_));
    OF_CUDNN_CHECK(cudnnSetPoolingNdDescriptor(val_, pooling_mode,
                                               CUDNN_NOT_PROPAGATE_NAN, dims,
...
public:
  ...
  void ResetIfDynamic(user_op::KernelComputeContext *ctx) {
    if (this->is_dynamic_) {
      ...
      const Params3D params_3d(dim_, x_shape, data_format, padding,
                               padding_before, padding_after, pool_size,
                               strides, ceil_mode);
      ...
      Reset(dim_, pooling_type_, x_shape, y_shape, data_format, dtype,
            params_3d);
    }
  }

  void Reset(const int32_t dim, const std::string &pooling_type,
    ...
    pooling_desc_.reset(new CudnnPoolDesc(pooling_mode, dim, pool_size.data(),
                                          padding.data(), strides.data()));
  }

  static std::shared_ptr<user_op::OpKernelState>
  FromKernelInitContext(const int32_t &dim, const std::string &pooling_type,
    ...
    if (x_desc->is_dynamic()) {
      ...
      state.reset(new GPUPoolOpKernelState(dim, pooling_type, x_shape, y_shape,
                                           data_format, dtype, params_3d));
    }
...
template <typename T> struct PoolGpuKernelUtil {
  static void FWCompute(user_op::KernelComputeContext *ctx,
    ...
    OF_CUDNN_CHECK(cudnnPoolingForward(
        ...
        gpu_pool_op_kernel_state->cudnn_x_tensor_desc(), x->dptr(),
        CudnnSPZeroPtr<T>(), gpu_pool_op_kernel_state->cudnn_y_tensor_desc(),
        y->mut_dptr()));
  }
  ...
  static void BWCompute(user_op::KernelComputeContext *ctx,
    ...
    OF_CUDNN_CHECK(cudnnPoolingBackward(
        ...
        gpu_pool_op_kernel_state->cudnn_x_tensor_desc(), x->dptr(),
        CudnnSPZeroPtr<T>(), gpu_pool_op_kernel_state->cudnn_x_tensor_desc(),
        dx->mut_dptr()));
  }
};
