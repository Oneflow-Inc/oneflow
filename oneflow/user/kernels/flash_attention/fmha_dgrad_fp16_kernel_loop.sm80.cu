...
void run_fmha_dgrad_fp16_sm80_loop_(const FMHA_dgrad_params& params, cudaStream_t stream) {
  ...
  static_assert(smem_size_dq
                == 16 * Kernel_traits::Cta_tile_p::K * 4 * Kernel_traits::Cta_tile_p::WARPS_N);

  constexpr int smem_size_dq_dk_dv = smem_size_q * 2
                                     + smem_size_v * (Kernel_traits::V_IN_REGS ? 1 : 2)
  ...
  BOOL_SWITCH(is_dropout, IsDropoutConst, [&] {
    ...
    if (params.seqlen_k == blocksize_c) {
      kernel = params.is_causal
                   ? &fmha_dgrad_fp16_sm80_dq_dk_dv_loop_kernel<Kernel_traits, IsDropoutConst, true,
                                                                /*loop_steps=*/1>
                   : &fmha_dgrad_fp16_sm80_dq_dk_dv_loop_kernel<Kernel_traits, IsDropoutConst,
                                                                false, /*loop_steps=*/1>;
    ...
    if (smem_size_dq_dk_dv >= 48 * 1024) {
      FMHA_CHECK_CUDA(cudaFuncSetAttribute(kernel, cudaFuncAttributeMaxDynamicSharedMemorySize,
                                           smem_size_dq_dk_dv));
    }
...
void run_fmha_dgrad_fp16_sm80(Launch_params<FMHA_dgrad_params>& launch_params,
                              cudaStream_t stream) {
  ...
  FP16_SWITCH(params.is_bf16, [&] {
    ...
    //                 elem_type>; run_fmha_dgrad_fp16_sm80_loop_<Kernel_traits>(params, stream);
    //             } else if (dprops->major == 8 && dprops->minor > 0) {
