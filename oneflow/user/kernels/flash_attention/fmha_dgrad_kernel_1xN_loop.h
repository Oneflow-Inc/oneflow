...
template<int ROWS, int THREADS_PER_ROW, typename elem_type = __half, int M,
         typename Gmem_softmax_sum>
...
#pragma unroll
  for (int mi = 0; mi < M; ++mi) {
    sum[mi] =
        fmha::Allreduce<THREADS_PER_ROW>::run(fmha::hmulsum8<elem_type>(do_[mi], o[mi]), sum_op)
        * scale;
  }
...
template<typename Kernel_traits, bool Is_dropout, bool Is_causal, bool Is_first, bool Is_last,
         typename Params, typename Prng>
inline __device__ void compute_dq_dk_dv_1xN_one_iter(const Params& params, Prng& ph,
  ...
  using Cta_tile_dkv = fmha::Cta_tile_extd<Cta_tile_p::N, Cta_tile_p::K, Cta_tile_p::M,
  ...
  Gmem_tile_q gmem_q(params.q_ptr, params.q_row_stride_in_elts, params.q_head_stride_in_elts, binfo,
                     tidx, true);
  ...
  Gmem_tile_dq gmem_dq(params.dq_ptr, params.dq_row_stride_in_elts, params.dq_head_stride_in_elts,
                       binfo, tidx);
  Gmem_tile_dq_tmp gmem_dq_tmp(params.o_tmp_ptr, params.o_row_stride_in_elts,
                               params.o_head_stride_in_elts, binfo, tidx);
  ...
  Gmem_tile_k gmem_k(params.k_ptr, params.k_row_stride_in_elts, params.k_head_stride_in_elts, binfo,
                     tidx, false);
  // Allocate the global memory tile loader for V.
  Gmem_tile_v gmem_v(params.v_ptr, params.v_row_stride_in_elts, params.v_head_stride_in_elts, binfo,
                     tidx, false);
  ...
  Smem_tile_kt smem_kt(&smem_[Smem_tile_do::BYTES_PER_TILE + Gemm1::Smem_tile_q::BYTES_PER_TILE],
                       tidx);
  ...
  Smem_tile_st smem_s(
      ...
      tidx);
  Smem_tile_st smem_dp(&smem_[Smem_tile_do::BYTES_PER_TILE + Gemm1::SMEM_OFFSET_O
                              + Smem_tile_dq::BYTES_PER_TILE + Smem_tile_st::BYTES_PER_TILE],
                       tidx);

  // Allocate the global memory tile loader for O.
  Gmem_tile_o gmem_o(params.o_ptr, params.o_row_stride_in_elts, params.o_head_stride_in_elts, binfo,
                     tidx, true);
  ...
  if (!Kernel_traits::SHARE_SMEM_FOR_K_AND_V) { gmem_k.commit(gemm_q_k.smem_k); }
  ...
  typename Smem_tile_v::Fragment frag_v[Kernel_traits::V_IN_REGS ? Mma_tile_p::MMAS_K : 2]
                                       [Mma_tile_p::MMAS_N];
  if (Kernel_traits::V_IN_REGS) {
#pragma unroll
    for (int ki = 0; ki < Mma_tile_p::MMAS_K; ++ki) { smem_v.load(frag_v[ki], ki); }
  }
  ...
  for (int l = 0; l < steps; l++) {
    ...
    if (loop >= binfo.actual_seqlen_q) break;
    ...
    if (Is_dropout) {
      ...
      // softmax.template apply_dropout</*encode_dropout_in_sign_bit=*/true>(ph,
      // params.p_dropout_in_uint);
      softmax.template apply_dropout_16bits</*encode_dropout_in_sign_bit=*/true>(
          ph, params.p_dropout_in_uint16_t);
    }
...
#pragma unroll
      for (int ni = 0; ni < Mma_tile_p::MMAS_N; ni++) {
        softmax.elt_[2 * mi + 0][4 * ni + 0] = pointwise_mult(
            softmax.elt_[2 * mi + 0][4 * ni + 0], acc_dp[mi][ni].elt(0), dp_sum[2 * mi + 0]);
        softmax.elt_[2 * mi + 0][4 * ni + 1] = pointwise_mult(
            softmax.elt_[2 * mi + 0][4 * ni + 1], acc_dp[mi][ni].elt(1), dp_sum[2 * mi + 0]);
        softmax.elt_[2 * mi + 0][4 * ni + 2] = pointwise_mult(
            softmax.elt_[2 * mi + 0][4 * ni + 2], acc_dp[mi][ni].elt(4), dp_sum[2 * mi + 0]);
        softmax.elt_[2 * mi + 0][4 * ni + 3] = pointwise_mult(
            softmax.elt_[2 * mi + 0][4 * ni + 3], acc_dp[mi][ni].elt(5), dp_sum[2 * mi + 0]);
        softmax.elt_[2 * mi + 1][4 * ni + 0] = pointwise_mult(
            softmax.elt_[2 * mi + 1][4 * ni + 0], acc_dp[mi][ni].elt(2), dp_sum[2 * mi + 1]);
        softmax.elt_[2 * mi + 1][4 * ni + 1] = pointwise_mult(
            softmax.elt_[2 * mi + 1][4 * ni + 1], acc_dp[mi][ni].elt(3), dp_sum[2 * mi + 1]);
        softmax.elt_[2 * mi + 1][4 * ni + 2] = pointwise_mult(
            softmax.elt_[2 * mi + 1][4 * ni + 2], acc_dp[mi][ni].elt(6), dp_sum[2 * mi + 1]);
        softmax.elt_[2 * mi + 1][4 * ni + 3] = pointwise_mult(
            softmax.elt_[2 * mi + 1][4 * ni + 3], acc_dp[mi][ni].elt(7), dp_sum[2 * mi + 1]);
      }
    }
...
#pragma unroll
    ...
    if (l < steps - 1) { gmem_q.commit(gemm_q_k.smem_q); }
...
#pragma unroll
  ...
  if (Is_dropout) {
    for (int mi = 0; mi < Mma_tile_dkv::MMAS_M; mi++) {
      for (int ni = 0; ni < Mma_tile_dkv::MMAS_N; ni++) { acc_dv[mi][ni].mul_(params.rp_dropout); }
    }
  }
  // if ((threadIdx.x == 0) && (blockIdx.x == 0) && (blockIdx.y == 0))  {
  ...
  for (int mi = 0; mi < Mma_tile_dkv::MMAS_M; mi++) {
    for (int ni = 0; ni < Mma_tile_dkv::MMAS_N; ni++) {
      // acc_dk[mi][ni].mul_(Is_dropout ? params.rp_dropout * params.scale_bmm1f :
      // params.scale_bmm1f); acc_dk[mi][ni].mul_(params.scale_bmm1f);
...
inline __device__ void compute_dq_dk_dv_1xN(const Params& params) {
  ...
  if (loop_steps == 1) {
    ...
    if (params.seqlen_k == blocksize_c) {
      compute_dq_dk_dv_1xN_one_iter<Kernel_traits, Is_dropout, Is_causal, true, true>(params, ph,
                                                                                      0);
    } else {
      const int max_loop_steps = (params.seqlen_k + blocksize_c - 1) / blocksize_c;
      compute_dq_dk_dv_1xN_one_iter<Kernel_traits, Is_dropout, Is_causal, true, false>(params, ph,
                                                                                       0);
      for (int loop_step_idx = 1; loop_step_idx < max_loop_steps - 1; loop_step_idx++) {
        compute_dq_dk_dv_1xN_one_iter<Kernel_traits, Is_dropout, Is_causal, false, false>(
            params, ph, loop_step_idx);
      }
      compute_dq_dk_dv_1xN_one_iter<Kernel_traits, Is_dropout, Is_causal, false, true>(
          params, ph, max_loop_steps - 1);
    }
  }
}
