...
template<typename T>
__global__ void FusedGegluForwardGpu(
        const int in_size,
        const int out_size, 
...
private:
    ...
    void Compute(user_op::KernelComputeContext* ctx, 
        ...
        matmul->Launch(ctx->stream(), num_samples, out_size*2, in_size, 
            1.0, in->dptr(), w->dptr(), 0.0, matmul_out->mut_dptr());
