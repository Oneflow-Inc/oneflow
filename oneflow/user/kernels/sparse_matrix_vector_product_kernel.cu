...
template<typename T, typename IndexType>
__device__ void CsrStream(
        const IndexType row_ptrs_begin_idx, const IndexType row_ptrs_end_idx, const IndexType col_ids_begin_idx, const IndexType col_ids_end_idx, const IndexType block_nnz, const IndexType* row_ptrs, const IndexType* col_ids, const T* mat_values, const T* in_vec, T* out_vec, T* cache){  
    /* ========== step 1: product  ========== */
    ...
    if (reduction_group_size > 1){
        ...
        if(target_row_idx < row_ptrs_end_idx){
            ...
            // merge
            for(IndexType j = cache_begin_idx+thread_in_reduction_group; j < cache_end_idx; j+=reduction_group_size){
        ...
        while(target_row_idx < row_ptrs_end_idx){
            ...
            // reduction loop
            for(IndexType j=cache_begin_idx; j<cache_end_idx; j++){
                reduction_result += cache[j];
            }
...
template<typename T, typename IndexType>
__device__ void CsrVector(
        const IndexType num_rows, const IndexType target_row_idx, const IndexType col_ids_begin_idx, const IndexType col_ids_end_idx, const IndexType block_nnz, const IndexType* row_ptrs, const IndexType* col_ids, const T* mat_values, const T* in_vec, T* out_vec){
    ...
    if(lane == 0 && warp_id == 0 && target_row_idx < num_rows){ // TODO: might no need the last judgement here
        out_vec[target_row_idx] = reduction_result;
    }
}

template<typename T, typename IndexType>
__device__ void CsrVectorL(
        const IndexType num_rows, const IndexType target_row_idx, const IndexType col_ids_begin_idx, const IndexType col_ids_end_idx, const IndexType block_nnz, const IndexType* row_ptrs, const IndexType* col_ids, const T* mat_values, const T* in_vec, T* out_vec, T* cache){
    // obtain metadata of current warp
    const IndexType warp_id = threadIdx.x / 32;
    const IndexType lane = threadIdx.x % 32;
    ...
    if(lane == 0){
        cache[warp_id] = reduction_result;
    }
    __syncthreads();
    ...
    if(warp_id == 0){
        ...
        for(IndexType j=lane; j<BLOCK_SIZE/32; j+=32){
            reduction_result += cache[j];
        }
        reduction_result = warpReduce<T, IndexType>(reduction_result);
...
// ref: https://github.com/senior-zero/matrix_format_performance/blob/master/gpu/csr_adaptive_spmv.cu
template<typename T, typename IndexType>
__global__ void CsrSpMVGpu(
        const IndexType* row_assignment, const IndexType num_rows, const IndexType* row_ptrs, const IndexType* col_ids, const T* mat_values, const T* in_vec, T* out_vec){
    ...
    const IndexType row_ptrs_end_idx = row_assignment[blockIdx.x+1];    // end index of row_ptrs (not included)
    ...
    const IndexType col_ids_end_idx = row_ptrs[row_ptrs_end_idx];       // end index of col_ids (not included)
    ...
    if (row_ptrs_end_idx - row_ptrs_begin_idx > 1){ 
        ...
        CsrStream<T, IndexType>(row_ptrs_begin_idx, row_ptrs_end_idx, col_ids_begin_idx, col_ids_end_idx, block_nnz, row_ptrs, col_ids, mat_values, in_vec, out_vec, cache);
    } else { 
        ...
        if(block_nnz <= 64 || BLOCK_SIZE <= 32){
            ...
            CsrVector<T, IndexType>(num_rows, row_ptrs_begin_idx, col_ids_begin_idx, col_ids_end_idx, block_nnz, row_ptrs, col_ids, mat_values, in_vec, out_vec);
        } else {
            CsrVectorL<T, IndexType>(num_rows, row_ptrs_begin_idx, col_ids_begin_idx, col_ids_end_idx, block_nnz, row_ptrs, col_ids, mat_values, in_vec, out_vec, cache);
        }
    }
}

template<typename IndexType>
std::vector<IndexType>* InferRowAssignment(const IndexType num_ptrs, const IndexType* ptrs, IndexType* num_block){
    IndexType last_row_id = 0;
    IndexType nnz_sum = 0;
    std::vector<IndexType>* row_assignment = new std::vector<IndexType>;

#define UPDATE_ASSIGNMENT_META(row_id) \
    row_assignment->push_back(row_id); last_row_id = row_id; nnz_sum = 0;
...
template<typename T, typename IndexType>
void LaunchCsrSpMVGpuKernel(
        ep::Stream* stream, const IndexType num_rows, const IndexType nnz, const IndexType* mat_row, const IndexType* mat_col, const T* mat_values, const T* in_vec, T* out_vec){
    // conduct row assignment and infer kernel shape
    IndexType num_block = 0;
    std::vector<IndexType>* row_assignment = InferRowAssignment<IndexType>(num_rows, mat_row, &num_block);
    ...
    OF_CUDA_CHECK(cudaMemcpyAsync(d_row_assignment, row_assignment->data(), (num_block+1)*sizeof(IndexType), cudaMemcpyDefault, stream->As<ep::CudaStream>()->cuda_stream()));
...
template<typename T, typename IndexType>
void DispatchFormat(
        ep::Stream* stream, const IndexType num_ptrs, const IndexType nnz, const IndexType* mat_row, const IndexType* mat_col, const T* mat_value, const T* vec, T* out, const std::string& format){
    if(format == "csr"){
        LaunchCsrSpMVGpuKernel<T, IndexType>(stream, num_ptrs, nnz, mat_row, mat_col, mat_value, vec, out);
    } else if(format == "csc"){
        UNIMPLEMENTED();
    } else if(format == "coo"){
        UNIMPLEMENTED();
...
void DispatchIndexType(
        ep::Stream* stream, const int64_t num_ptrs, const int64_t nnz, const user_op::Tensor* mat_row_tensor, const user_op::Tensor* mat_col_tensor, const user_op::Tensor* mat_value_tensor, const user_op::Tensor* in_vec_tensor, user_op::Tensor* out_vec_tensor, const std::string& format){
    if(num_ptrs < (1 << 30) && nnz < (1 << 30)){
        DispatchFormat<T, int32_t>(stream, static_cast<int32_t>(num_ptrs), static_cast<int32_t>(nnz), mat_row_tensor->dptr<int32_t>(), mat_col_tensor->dptr<int32_t>(), mat_value_tensor->dptr<T>(), in_vec_tensor->dptr<T>(), out_vec_tensor->mut_dptr<T>(), format);
    } else {
        DispatchFormat<T, int64_t>(stream, static_cast<int64_t>(num_ptrs), static_cast<int64_t>(nnz), mat_row_tensor->dptr<int64_t>(), mat_col_tensor->dptr<int64_t>(), mat_value_tensor->dptr<T>(), in_vec_tensor->dptr<T>(), out_vec_tensor->mut_dptr<T>(), format);
    }
}
 ...
 private:
    ...
    void Compute(user_op::KernelComputeContext* ctx) const override {
        ...
        CHECK(IsIndexDataType(mat_rows_dtype))
            << "The dtype of mat_rows must be integer, but found "
            << DataType_Name(mat_rows_dtype);
        CHECK(IsIndexDataType(mat_cols_dtype))
            << "The dtype of mat_cols must be integer, but found "
