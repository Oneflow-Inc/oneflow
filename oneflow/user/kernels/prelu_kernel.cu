...
template <typename T>
__global__ void PReluForwardGpu(const int64_t elem_cnt, const T *x,
                                const T *alpha, T *y) {
  CUDA_1D_KERNEL_LOOP(i, elem_cnt) { y[i] = x[i] > 0 ? x[i] : x[i] * alpha[i]; }
}

template <typename T>
__global__ void PReluXBackwardGpu(const int64_t elem_cnt, const T *x,
                                  const T *alpha, const T *dy, T *dx) {
  CUDA_1D_KERNEL_LOOP(i, elem_cnt) {
    dx[i] = x[i] > 0 ? dy[i] : dy[i] * alpha[i];
  }
}

template <typename T>
__global__ void PReluAlphaBackwardGpu(const int64_t elem_cnt, const T *x,
                                      const T *dy, T *alpha_diff) {
  CUDA_1D_KERNEL_LOOP(i, elem_cnt) {
    alpha_diff[i] = x[i] > 0 ? 0 : dy[i] * x[i];
  }
}

} // namespace

template <typename T> class GpuPReluKernel final : public user_op::OpKernel {
public:
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    user_op::Tensor *broadcasted_alpha =
        ctx->Tensor4ArgNameAndIndex("tmp_buffer", 0);
    user_op::Tensor *y = ctx->Tensor4ArgNameAndIndex("y", 0);
    const int32_t elem_cnt = x->shape().elem_cnt();
    const Shape &left_extended_shape = CreateLeftExtendedShape(
        ShapeView(alpha->shape()), x->shape().NumAxes());
    NdarrayUtil<DeviceType::kGPU, T>::BroadcastTo(
        ctx->device_ctx(),
        XpuVarNdarray<T>(x->shape(), broadcasted_alpha->mut_dptr<T>()),
        XpuVarNdarray<const T>(left_extended_shape, alpha->dptr<T>()));

    RUN_CUDA_KERNEL((PReluForwardGpu<T>), ctx->device_ctx(), elem_cnt, elem_cnt,
                    x->dptr<T>(), broadcasted_alpha->dptr<T>(),
                    y->mut_dptr<T>());
  }
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    RUN_CUDA_KERNEL((PReluXBackwardGpu<T>), ctx->device_ctx(), elem_cnt,
                    elem_cnt, x->dptr<T>(), broadcasted_alpha->dptr<T>(),
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    RUN_CUDA_KERNEL((PReluAlphaBackwardGpu<T>), ctx->device_ctx(), elem_cnt,
                    elem_cnt, x->dptr<T>(), dy->dptr<T>(),
                    broadcasted_alpha_diff);
    const Shape &left_extended_shape = CreateLeftExtendedShape(
        ShapeView(alpha->shape()), x->shape().NumAxes());
    NdarrayUtil<DeviceType::kGPU, T>::ReduceSum(
        ctx->device_ctx(),
        XpuVarNdarray<T>(left_extended_shape, alpha_diff->mut_dptr<T>()),
...
#define REGISTER_GPU_PRELU_ALPHA_GRAD_KERNEL(dtype)                            \
  REGISTER_USER_KERNEL("prelu_alpha_grad")                                     \
      ...
      .SetIsMatchedHob((user_op::HobDeviceTag() == "gpu") &                    \
                       (user_op::HobDataType("alpha_diff", 0) ==               \
