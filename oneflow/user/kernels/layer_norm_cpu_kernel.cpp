...
class LayerNormMiopenBnCtx final {
 public:
  LayerNormMiopenBnCtx(const ShapeView& data_shape, const ShapeView& param_shape,
                      DataType data_type) {
    ...
    data_tensor_desc_.reset(new MiopenTensorDesc(data_type, 1,
    ...
    param_tensor_desc_.reset(new MiopenTensorDesc(param_dtype, 1,
                                                 static_cast<int32_t>(cudnn_c), 1, 1));
    mode_ = miopenBNSpatial;
  }
  ~LayerNormMiopenBnCtx() = default;

  const miopenTensorDescriptor_t& data_tensor_desc() const { return data_tensor_desc_->Get(); }
  const miopenTensorDescriptor_t& param_tensor_desc() const { return param_tensor_desc_->Get(); }
  miopenBatchNormMode_t mode() const { return mode_; };

 private:
  std::unique_ptr<MiopenTensorDesc> data_tensor_desc_;
  std::unique_ptr<MiopenTensorDesc> param_tensor_desc_;
...
class LayerNormGpuKernel final : public user_op::OpKernel {
 ...
 private:
  ...
  void Compute(user_op::KernelComputeContext* ctx) const override {
    ...
    CHECK_GE(epsilon, MIOPEN_BN_MIN_EPSILON);
    ...
    if (IsForwardFusedKernelSupported<T>(norm_size, instance_size)) {
      ...
      LayerNormMiopenBnCtx bn_ctx(x->shape(), mean->shape(), x->data_type());
      ...
      OF_MIOPEN_CHECK(miopenBatchNormalizationForwardTraining(
          ctx->device_ctx()->miopen_handle(), bn_ctx.mode(), MiopenSPOnePtr<T>(), MiopenSPZeroPtr<T>(),
 ...
 private:
  ...
  void Compute(user_op::KernelComputeContext* ctx) const override {
    ...
    NewKernelUtil<DeviceType::kGPU>::Fill(ctx->device_ctx(), mean->shape().elem_cnt(),
                                          static_cast<BNParamT>(1),
                                          reinterpret_cast<BNParamT*>(cudnn_bn_scale_ones_dptr));
    const void* sp_alpha = MiopenSPOnePtr<T>();
    const void* sp_beta;
    if (ctx->has_input("_add_to_output", 0)) {
      ...
      sp_beta = MiopenSPOnePtr<T>();
    } else {
      sp_beta = MiopenSPZeroPtr<T>();
    }
    const double epsilon = ctx->Attr<double>("epsilon");
    CHECK_GE(epsilon, MIOPEN_BN_MIN_EPSILON);
    LayerNormMiopenBnCtx bn_ctx(x->shape(), mean->shape(), x->data_type());
    OF_MIOPEN_CHECK(miopenBatchNormalizationBackward(
        ctx->device_ctx()->miopen_handle(), bn_ctx.mode(), sp_alpha, sp_beta, MiopenSPOnePtr<T>(),
        MiopenSPZeroPtr<T>(), bn_ctx.data_tensor_desc(), x->dptr<T>(), bn_ctx.data_tensor_desc(),
