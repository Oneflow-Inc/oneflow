...
class CudnnPoolingDesc final {
 public:
  ...
  ~CudnnPoolingDesc() { OF_CUDNN_CHECK(cudnnDestroyPoolingDescriptor(val_)); }
...
class GPUPoolKernelState final : public user_op::OpKernelState {
 public:
  GPUPoolKernelState(const int32_t dim, const std::string& pooling_type, const Shape& x_shape,
  ...
  GPUPoolKernelState(const int32_t dim, const std::string& pooling_type)
      : is_dynamic_(true), dim_(dim), pooling_type_(pooling_type) {}
  ~GPUPoolKernelState() = default;
  ...
  void ResetIfDynamic(user_op::KernelComputeContext* ctx) {
    if (this->is_dynamic_) {
      ...
      const PoolingParams3D params_3d = PoolingParams3D(
        dim_, x_shape, data_format, padding, padding_before, padding_after, 
  ...
  void Reset(const int32_t dim, const std::string& pooling_type, const ShapeView& x_shape,
             ...
             const PoolingParams3D& params_3d) {
    FixedVector kernel_size(dim);
    FixedVector padding(dim);
    FixedVector stride(dim);
    FOR_RANGE(int, i, 0, dim) {
      int32_t index_in_3d = i + 3 - dim;
      kernel_size[i] = params_3d.pool_size_3d().at(index_in_3d);
      padding[i] = params_3d.padding_before_3d().at(index_in_3d);
      stride[i] = params_3d.strides_3d().at(index_in_3d);
    }
    ...
    pooling_desc_.reset(
        new CudnnPoolingDesc(pooling_mode, dim, kernel_size.data(), padding.data(), stride.data()));
  }
  ...
  static std::shared_ptr<user_op::OpKernelState> FromKernelInitContext(
    ...
    std::shared_ptr<GPUPoolKernelState> state;
    const user_op::TensorDesc* x_desc = ctx->TensorDesc4ArgNameAndIndex("x", 0);
    if (x_desc->is_dynamic()) {
      state.reset(new GPUPoolKernelState(dim, pooling_type));
    } else {
      ...
      state.reset(new GPUPoolKernelState(dim, pooling_type, x_shape, y_shape, data_format, dtype,
                                           params_3d));
    }
...
template<typename T>
struct PoolingGpuKernelUtil {
  static void FWCompute(user_op::KernelComputeContext* ctx, user_op::OpKernelState* state) {
    const user_op::Tensor* x = ctx->Tensor4ArgNameAndIndex("x", 0);
    user_op::Tensor* y = ctx->Tensor4ArgNameAndIndex("y", 0);
    GPUPoolKernelState* gpu_pool_op_kernel_state = dynamic_cast<GPUPoolKernelState*>(state);
...
class MaxPool2dGpuKernel final : public user_op::OpKernel {
 public:
  ...
  std::shared_ptr<user_op::OpKernelState> CreateOpKernelState(
    ...
    return GPUPoolKernelState::FromKernelInitContext(2, "MAX", ctx);
  }

 private:
  bool AlwaysComputeWhenAllOutputsEmpty() const override { return false; }
  void Compute(user_op::KernelComputeContext* ctx, user_op::OpKernelState* state) const override {
    PoolingGpuKernelUtil<T>::FWCompute(ctx, state);
  };
};
...
class MaxPool2dGradGpuKernel final : public user_op::OpKernel {
 ...
 private:
  bool AlwaysComputeWhenAllOutputsEmpty() const override { return false; }
  void Compute(user_op::KernelComputeContext* ctx, user_op::OpKernelState* state) const override {
    PoolingGpuKernelUtil<T>::BWCompute(ctx, state);
  };
};



#define REGISTER_POOLING_GPU_KERNEL(dtype)                                             \
...
REGISTER_POOLING_GPU_KERNEL(float)
REGISTER_POOLING_GPU_KERNEL(double)
REGISTER_POOLING_GPU_KERNEL(float16)
