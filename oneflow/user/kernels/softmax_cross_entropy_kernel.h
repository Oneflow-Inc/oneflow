...
template <DeviceType device_type, typename T> struct CrossEntropyKernelUtil {
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    CrossEntropyKernelUtil<device_type, T>::ComputeEntropy(
        ctx->device_ctx(), num_instances, num_classes, prob->dptr<T>(),
...
#define REGISTER_SOFTMAX_CROSS_ENTROPY_KERNEL(device_type_v, dtype_pair)       \
  REGISTER_USER_KERNEL("softmax_cross_entropy")                                \
      .SetCreateFn<SoftmaxCrossEntropyKernel<device_type_v,                    \
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    CrossEntropyKernelUtil<device_type, T>::ComputeDiffWithSoftmax(
        ...
        prob->dptr<T>(), label->dptr<T>(), dy->dptr<T>(),
...
#define REGISTER_SOFTMAX_CROSS_ENTROPY_GRAD_KERNEL(device_type_v, dtype_pair)  \
  REGISTER_USER_KERNEL("softmax_cross_entropy_grad")                           \
      .SetCreateFn<SoftmaxCrossEntropyGradKernel<                              \
