...
template<typename T>
void convert_to_doublesized(std::complex<T>* in, size_t len, size_t n) {
  size_t fact_len = 2 * len - 2;
  std::complex<T> tempbuf[n];
  for (int i = 0; i < n; i++) {
    int index_x = i / fact_len;
    int index_y = i % fact_len;
    if (index_y == 0) {
      tempbuf[i] = in[index_x * len];
    } else if (index_y == len - 1) {
      tempbuf[i] = in[(index_x + 1) * len - 1];
    } else if (index_y < len - 1 && index_y > 0) {
      tempbuf[i] = in[index_x * len + index_y];
    } else {
      auto index = (index_x + 2) * len - index_y - 2;
      auto realvalue = in[index].real();
      tempbuf[i].real(realvalue);
      auto imagvalue = -in[index].imag();
      tempbuf[i].imag(imagvalue);
    }
  }
  for (int i = 0; i < n; i++) { in[i] = tempbuf[i]; }
}
 ...
 private:
  ...
  void Compute(user_op::KernelComputeContext* ctx) const override {
    ...
    if (!onesized) {
      ...
      convert_to_doublesized<IN>(out_tmp_buffer, last_dim_length, elem_conut);
    }
