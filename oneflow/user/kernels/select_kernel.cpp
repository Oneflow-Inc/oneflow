...
Maybe<void> operator()(ep::Stream* stream, const T* input_buf, T* output_buf, const int32_t dim, const int32_t index,
    ...
    FOR_RANGE(int64_t, i, 0, output_num) {
        ...
        outputIndexOffsetHelper.OffsetToNdIndex(i, dst_index, output_dims_num);
        FOR_RANGE(int64_t, j, 0, input_dims_num){
            if(j<dim){
        ...
        int64_t index_in_input = inputIndexOffsetHelper.NdIndexToOffset(src_index, input_dims_num);
...
Maybe<void> operator()(ep::Stream* stream, const T* dy_buf, T* dx_buf, const int32_t dim, const int32_t index,
    ...
    FOR_RANGE(int64_t, i, 0, dy_num) {
        int64_t src_index[NUM_DIM];
        int64_t dst_index[NUM_DIM];
        dyIndexOffsetHelper.OffsetToNdIndex(i, dst_index, dy_dims_num);
        FOR_RANGE(int64_t, j, 0, dx_dims_num){
            if(j<dim){
               src_index[j] = dst_index[j];
            }else if(j==dim){
               src_index[j] = index;
        ...
        int64_t index_in_dx = dxIndexOffsetHelper.NdIndexToOffset(src_index, dx_dims_num);
...
template<typename T>
class CpuSelectKernel final : public user_op::OpKernel {
 public:
  CpuSelectKernel() = default;
  ~CpuSelectKernel() = default;
...
template<typename T>
class CpuSelectGradKernel final : public user_op::OpKernel {
 public:
  CpuSelectGradKernel() = default;
  ~CpuSelectGradKernel() = default;
...
#define REGISTER_CPUSELECT_KERNEL(in_type)                                     \
    REGISTER_USER_KERNEL("select")                         \
      .SetCreateFn<                                            \
          CpuSelectKernel<in_type>>()                       \
      .SetIsMatchedHob(                                        \
          (user_op::HobDeviceType() == DeviceType::kCPU)       \
          && (user_op::HobDataType("input", 0) == GetDataType<in_type>::value));   \
    REGISTER_USER_KERNEL("select_grad")                    \
      .SetCreateFn<                                            \
          CpuSelectGradKernel<in_type>>()                   \
      .SetIsMatchedHob(                                        \
          (user_op::HobDeviceType() == DeviceType::kCPU)       \
          && (user_op::HobDataType("input", 0) == GetDataType<in_type>::value));            

REGISTER_CPUSELECT_KERNEL(float);
REGISTER_CPUSELECT_KERNEL(double);
REGISTER_CPUSELECT_KERNEL(int8_t);
REGISTER_CPUSELECT_KERNEL(int32_t);
REGISTER_CPUSELECT_KERNEL(int64_t);

#undef  REGISTER_CPUSELECT_KERNEL
