...
template <typename T>
__global__ void L2NormalizeForward(const int32_t n, const int32_t c,
                                   const int32_t d, const T epsilon,
...
template <typename T>
__global__ void L2NormalizeBackward(const int32_t n, const int32_t c,
                                    const int32_t d, const float epsilon,
  ...
  for (int32_t i = blockIdx.x; i < n; i += gridDim.x) {
    ...
    if (square_x_sum[i] >= epsilon) {
      ...
      const T reduce_y_dy_prod_sum =
          BlockReduce(temp_storage_prod_sum).Sum(y_dy_prod_sum);
      __shared__ T y_dy_inner_prod;
      if (threadIdx.x == 0) {
        y_dy_inner_prod = reduce_y_dy_prod_sum;
      }
      __syncthreads();
      ...
      for (int32_t j = threadIdx.x; j < c; j += blockDim.x) {
        const int32_t index = offset + j * d;
        in_diff[index] =
            inv_norm * (out_diff[index] - y_dy_inner_prod * out[index]);
      }
    } else {
      for (int32_t j = threadIdx.x; j < c; j += blockDim.x) {
        const int32_t index = offset + j * d;
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    RUN_CUDA_KERNEL((L2NormalizeForward<T>), ctx->device_ctx(), n, n, c, d,
                    static_cast<T>(epsilon), x->dptr<T>(),
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    RUN_CUDA_KERNEL((L2NormalizeBackward<T>), ctx->device_ctx(), n, n, c, d,
                    static_cast<T>(epsilon), y->dptr<T>(), dy->dptr<T>(),
