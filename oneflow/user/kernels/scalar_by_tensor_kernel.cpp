...
struct ComputeScalarByTensor {
  static void DoCompute(DeviceCtx *ctx, const T *x_ptr, const T *scalar_ptr,
                        T *y_ptr, const int64_t n);
};
...
struct ComputeScalarByTensor<ByScalarFunc::add, device_type, T> {
  static void DoCompute(DeviceCtx *ctx, const T *x_ptr, const T *scalar_ptr,
                        T *y_ptr, const int64_t n) {
    NewKernelUtil<device_type>::AddByScalarPtr(ctx, n, x_ptr, scalar_ptr,
                                               y_ptr);
  }
};
...
struct ComputeScalarByTensor<ByScalarFunc::sub, device_type, T> {
  static void DoCompute(DeviceCtx *ctx, const T *x_ptr, const T *scalar_ptr,
                        T *y_ptr, const int64_t n) {
    NewKernelUtil<device_type>::SubByScalarPtr(ctx, n, x_ptr, scalar_ptr,
                                               y_ptr);
  }
};
...
struct ComputeScalarByTensor<ByScalarFunc::mul, device_type, T> {
  static void DoCompute(DeviceCtx *ctx, const T *x_ptr, const T *scalar_ptr,
                        T *y_ptr, const int64_t n) {
    NewKernelUtil<device_type>::MulByScalarPtr(ctx, n, x_ptr, scalar_ptr,
                                               y_ptr);
  }
};
...
struct ComputeScalarByTensor<ByScalarFunc::div, device_type, T> {
  static void DoCompute(DeviceCtx *ctx, const T *x_ptr, const T *scalar_ptr,
                        T *y_ptr, const int64_t n) {
    NewKernelUtil<device_type>::DivByScalarPtr(ctx, n, x_ptr, scalar_ptr,
                                               y_ptr);
  }
};
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    ComputeScalarByTensor<by_scalar_func, device, T>::DoCompute(
        ctx->device_ctx(), x->dptr<T>(), scalar->dptr<T>(), y->mut_dptr<T>(),
        x->shape().elem_cnt());
  };
...
#define REGISTER_SCALAR_ADD_BY_TENSOR_KERNEL(scalar_by_tensor_pair, device,    \
                                             dtype_pair)                       \
  REGISTER_USER_KERNEL(OF_PP_PAIR_FIRST(scalar_by_tensor_pair))                \
      .SetCreateFn<                                                            \
          ScalarAddByTensorKernel<OF_PP_PAIR_SECOND(scalar_by_tensor_pair),    \
...
OF_PP_SEQ_PRODUCT_FOR_EACH_TUPLE(REGISTER_SCALAR_ADD_BY_TENSOR_KERNEL,
                                 SCALAR_BY_TENSOR_SEQ, DEVICE_TYPE_SEQ,
                                 ARITHMETIC_DATA_TYPE_SEQ)
#ifdef WITH_CUDA
OF_PP_SEQ_PRODUCT_FOR_EACH_TUPLE(REGISTER_SCALAR_ADD_BY_TENSOR_KERNEL,
                                 SCALAR_BY_TENSOR_SEQ, (DeviceType::kGPU),
