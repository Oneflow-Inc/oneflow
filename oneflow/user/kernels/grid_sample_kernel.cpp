...
void GridSampler4DKernel(const index_type count, const data_type* input_ptr,
...
template<typename data_type, typename index_type>
void GridSampler5DKernel(const index_type count, const data_type* input_ptr,
                         const data_type* grid_ptr, data_type* output_ptr, index_type N,
...
template<typename data_type, typename index_type>
void GridSampler4DBackwardKernel(const index_type count, const data_type* grad_output_ptr,
                                 const data_type* input_ptr, const data_type* grid_ptr,
                                 data_type* grad_input_ptr, data_type* grad_grid_ptr, index_type N,
                                 index_type C, index_type inp_H, index_type inp_W, index_type out_H,
                                 index_type out_W,
                                 const GridSamplerInterpolation interpolation_mode,
                                 const GridSamplerPadding padding_mode, const bool align_corners,
...
template<typename data_type, typename index_type>
void GridSampler5DBackwardKernel(const index_type count, const data_type* grad_output_ptr,
                                 const data_type* input_ptr, const data_type* grid_ptr,
                                 data_type* grad_input_ptr, data_type* grad_grid_ptr, index_type N,
                                 index_type C, index_type inp_D, index_type inp_H, index_type inp_W,
                                 index_type out_D, index_type out_H, index_type out_W,
...
class GridSampleGradKernel final : public user_op::OpKernel, public user_op::CudaGraphSupport {
 ...
 private:
  void Compute(user_op::KernelComputeContext* ctx) const override {
    ...
    memset(dinput->mut_dptr<data_type>(), 0, input_shape.elem_cnt() * sizeof(data_type));
...
#define REGISTER_GRID_SAMPLE_KERNEL(data_type)                       \
  REGISTER_USER_KERNEL("grid_sample_grad")                           \
      .SetCreateFn<GridSampleGradKernel<data_type>>()                \
      .SetIsMatchedHob((user_op::HobDeviceTag() == DeviceType::kCPU) \
                       & (user_op::HobDataType("doutput", 0) == GetDataType<data_type>::value));

REGISTER_GRID_SAMPLE_KERNEL(float);
REGISTER_GRID_SAMPLE_KERNEL(double);
