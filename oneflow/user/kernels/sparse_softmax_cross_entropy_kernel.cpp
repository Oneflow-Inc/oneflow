...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    const user_op::Tensor *prediction =
        ctx->Tensor4ArgNameAndIndex("prediction", 0);
    ...
    SoftmaxKernelUtil<device_type, T>::ComputeProb(
        ctx->device_ctx(), num_instances, num_classes, prediction->dptr<T>(),
        prob->mut_dptr<T>(), tmp_buffer->mut_dptr(),
        tmp_buffer->shape().elem_cnt());
    SparseCrossEntropyKernelUtil<device_type, T, K>::ComputeEntropy(
        ctx->device_ctx(), num_instances, num_classes, depth, lower_bound,
        prob->dptr<T>(), label->dptr<K>(), out->mut_dptr<T>());
  }
...
#define REGISTER_SPARSE_SOFTMAX_CROSS_ENTROPY_KERNEL(                          \
    kernel_class, kernel_name, device_type_v, dtype_pair, ltype_pair)          \
  REGISTER_USER_KERNEL(kernel_name)                                            \
      ...
      .SetIsMatchedHob(                                                        \
          ...
          (user_op::HobDataType("label", 0) ==                                 \
           OF_PP_PAIR_SECOND(ltype_pair)) &                                    \
          (user_op::HobDataType("out", 0) == OF_PP_PAIR_SECOND(dtype_pair)))   \
      .SetInferTmpSizeFn([](user_op::InferContext *ctx) {                      \
        const Shape *prediction_shape =                                        \
            ctx->Shape4ArgNameAndIndex("prediction", 0);                       \
        const int64_t num_classes =                                            \
            prediction_shape->At(prediction_shape->NumAxes() - 1);             \
        const int64_t num_instances =                                          \
            prediction_shape->Count(0, prediction_shape->NumAxes() - 1);       \
        return SoftmaxKernelUtil<device_type_v,                                \
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    user_op::Tensor *prediction_diff =
        ctx->Tensor4ArgNameAndIndex("prediction_diff", 0);
    const int64_t num_instances = label->shape().elem_cnt();
    ...
    SparseCrossEntropyKernelUtil<device_type, T, K>::ComputeDiffWithSoftmax(
        ctx->device_ctx(), prediction_diff->shape().elem_cnt(), num_classes,
        depth, lower_bound, prob->dptr<T>(), label->dptr<K>(), dy->dptr<T>(),
...
#define REGISTER_SPARSE_SOFTMAX_CROSS_ENTROPY_GRAD_KERNEL(                     \
    kernel_class, kernel_name, device_type_v, dtype_pair, ltype_pair)          \
  REGISTER_USER_KERNEL(kernel_name)                                            \
      .SetCreateFn<kernel_class<device_type_v, OF_PP_PAIR_FIRST(dtype_pair),   \
                                OF_PP_PAIR_FIRST(ltype_pair)>>()               \
      ...
      .SetInplaceProposalFn(                                                   \
          [](const user_op::InferContext &,                                    \
            ...
            OF_RETURN_IF_ERROR(                                                \
                AddInplaceArgPairFn("prediction_diff", 0, "prob", 0, true));   \
            return Maybe<void>::Ok();                                          \
          });

OF_PP_SEQ_PRODUCT_FOR_EACH_TUPLE(
    REGISTER_SPARSE_SOFTMAX_CROSS_ENTROPY_GRAD_KERNEL,
