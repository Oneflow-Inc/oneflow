...
#define INSTANTIATE_SCALAR_MATH_FUNCTORS(device_type, binary_op)                \
  ...
  template struct ScalarMathWithStrideFunctor<device_type, binary_op, uint8_t>; \
  template struct ScalarMathWithStrideFunctor<device_type, binary_op, int8_t>;  \
  template struct ScalarMathWithStrideFunctor<device_type, binary_op, int32_t>; \
  template struct ScalarMathWithStrideFunctor<device_type, binary_op, int64_t>; \
  template struct ScalarMathWithStrideFunctor<device_type, binary_op, float>;   \
  template struct ScalarMathWithStrideFunctor<device_type, binary_op, double>;  \
  template struct ScalarMathWithStrideFunctor<device_type, binary_op, float16>;
...
template<DeviceType device_type, template<typename> class BIN_OP, typename T>
struct ScalarMathWithStrideFunctor final {
...
template<template<typename> class UnaryFunctor, typename T>
OF_DEVICE_FUNC void DoScalarMathWithStride(const int64_t elem_cnt, const StrideParam& in_stride,
