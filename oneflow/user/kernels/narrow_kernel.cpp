...
class NarrowKernel final : public user_op::OpKernel {
 ...
 private:
  void Compute(user_op::KernelComputeContext* ctx) const override {
    ...
    copy_nd_primitive->Launch(ctx->stream(), out->data_type(), 3, out->mut_dptr(), dst_shape.int64_ptr(),
                              dst_pos_vec.data(), in->dptr(), src_shape.int64_ptr(), src_pos_vec.data(),
                              extent_vec.data());
  }
 ...
 private:
  void Compute(user_op::KernelComputeContext* ctx) const override {
    ...
    copy_nd_primitive->Launch(ctx->stream(), dx->data_type(), 3, dst, dst_shape.int64_ptr(),
                              dst_pos_vec.data(), dy->dptr(), src_shape.int64_ptr(), src_pos_vec.data(),
                              extent_vec.data());
  }
  bool AlwaysComputeWhenAllOutputsEmpty() const override { return false; }
};
