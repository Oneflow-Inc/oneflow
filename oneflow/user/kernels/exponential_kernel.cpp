...
class ExponentialKernel final : public user_op::OpKernel {
 public:
  ...
  ~ExponentialKernel() override = default;
 ...
 private:
  void Compute(user_op::KernelComputeContext* ctx, user_op::OpKernelState* state,
    ...
    double lambda = ctx->Attr<double>("lambda");
    for (int32_t i = 0; i < out_blob->shape_view().elem_cnt(); ++i) {
      std::exponential_distribution<K> dis(lambda);
      *(out_dptr + i) = dis(cpu_generator->engine());
    }
  }
  bool AlwaysComputeWhenAllOutputsEmpty() const override { return false; }
};

#define REGISTER_EXPONENTIAL_KERNEL(in_dtype_pair, out_dtype_pair)                                \
  REGISTER_USER_KERNEL("exponential")                                                             \
      .SetCreateFn<                                                                               \
          ExponentialKernel<OF_PP_PAIR_FIRST(in_dtype_pair), OF_PP_PAIR_FIRST(out_dtype_pair)>>() \
...
OF_PP_SEQ_PRODUCT_FOR_EACH_TUPLE(REGISTER_EXPONENTIAL_KERNEL, FLOATING_DATA_TYPE_SEQ,
