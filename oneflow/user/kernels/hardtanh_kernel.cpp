...
class CpuHardtanhKernel final : public OpKernel {
 ...
 private:
  void Compute(KernelComputeContext* ctx) const override {
    ...
    const float min_val = ctx->Attr<float>("min_val");
    const float max_val = ctx->Attr<float>("max_val");
    ...
    FOR_RANGE(int32_t, i, 0, elem_cnt) {
      if (in_ptr[i] > max_val)
        out_ptr[i] = static_cast<T>(max_val);
      else if (in_ptr[i] < min_val)
        out_ptr[i] = static_cast<T>(min_val);
      else
        out_ptr[i] = in_ptr[i];
    }
  }
...
class CpuHardtanhGradKernel final : public OpKernel {
 ...
 private:
  void Compute(KernelComputeContext* ctx) const override {
    ...
    FOR_RANGE(int32_t, i, 0, elem_cnt) {
      dx_ptr[i] = (y_ptr[i] > static_cast<T>(min_val) && y_ptr[i] < static_cast<T>(max_val))
                      ? dy_ptr[i]
                      : static_cast<T>(0);
    }
  }
