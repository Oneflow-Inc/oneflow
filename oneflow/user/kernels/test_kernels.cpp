...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    NewKernelUtil<DeviceType::kGPU>::ReluBackward(
        ...
        y_blob->dptr<float>(), dy_blob->dptr<float>(),
        dx_blob->mut_dptr<float>());
  }
  bool AlwaysComputeWhenAllOutputsEmpty() const override { return false; }
};
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    Memcpy<DeviceType::kGPU>(ctx->device_ctx(), out1_blob->mut_dptr<char>(),
                             in_blob->dptr<char>(),
                             in_blob->shape().elem_cnt() * sizeof(float));
    NewKernelUtil<DeviceType::kGPU>::Fill(ctx->device_ctx(),
                                          out2_blob->shape().elem_cnt(), 0.0,
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    Memcpy<DeviceType::kGPU>(ctx->device_ctx(), y_blob->mut_dptr<char>(),
                             x1_blob->dptr<char>(),
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    NewKernelUtil<DeviceType::kGPU>::Fill(ctx->device_ctx(),
                                          x1_diff_blob->shape().elem_cnt(), 1.0,
                                          x1_diff_blob->mut_dptr<float>());
    NewKernelUtil<DeviceType::kGPU>::Fill(ctx->device_ctx(),
                                          x2_diff_blob->shape().elem_cnt(), 2.0,
...
template <typename T> class ReluCpuKernel final : public user_op::OpKernel {
public:
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    NewKernelUtil<DeviceType::kCPU>::Relu(ctx->device_ctx(),
                                          in->shape().elem_cnt(), in->dptr<T>(),
                                          out->mut_dptr<T>());
  }
  bool AlwaysComputeWhenAllOutputsEmpty() const override { return false; }
};
...
public:
  ...
  CreateOpKernelState(user_op::KernelInitContext *ctx) const override {
    ...
    return std::make_shared<
        OpKernelStateWrapper<RandomGenerator<DeviceType::kCPU>>>(
        seed, ctx->device_ctx());
  }
...
private:
  void Compute(user_op::KernelComputeContext *ctx,
    ...
    auto *random_generator =
        dynamic_cast<OpKernelStateWrapper<RandomGenerator<DeviceType::kCPU>> *>(
            state);
    user_op::Tensor *out_blob = ctx->Tensor4ArgNameAndIndex("out", 0);
    random_generator->Mutable()->Uniform<float>(
        out_blob->shape().elem_cnt(), 0.0, 1.0, out_blob->mut_dptr<float>());
  }
  bool AlwaysComputeWhenAllOutputsEmpty() const override { return false; }
};
...
class TestListDataTypeAndShapeAttrAndStringAttrKernel final
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    FOR_RANGE(int32_t, i, 0, ctx->outputs().size()) {
      ...
      CHECK_EQ(out_types.at(i),
               ctx->Tensor4ArgNameAndIndex("out", i)->data_type());
    }
