...
__global__ void OneHotEncodeGpu(int64_t elem_cnt, const int64_t depth,
                                const T on_value, const T off_value,
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    RUN_CUDA_KERNEL((OneHotEncodeGpu<T, K>), ctx->device_ctx(),
                    num_indices * depth, num_indices * depth, depth, on_value,
                    off_value, indices->dptr<K>(), out->mut_dptr<T>());
  }
...
#define REGISTER_GPU_ONE_HOT_KERNEL(dtype, itype)                              \
  REGISTER_USER_KERNEL("one_hot")                                              \
      .SetCreateFn<GpuOneHotKernel<dtype, itype>>()                            \
      .SetIsMatchedHob(                                                        \
