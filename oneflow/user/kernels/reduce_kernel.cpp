...
template <template <typename> class BinaryFunc, DeviceType device_type,
          typename T>
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    const user_op::Tensor *input_tensor =
        ctx->Tensor4ArgNameAndIndex("input_tensor", 0);
    user_op::Tensor *output_tensor =
        ctx->Tensor4ArgNameAndIndex("output_tensor", 0);
    ...
    NdarrayReduce<device_type, T, BinaryFunc>::Reduce(
        ctx->device_ctx(),
        XpuVarNdarray<T>(reduced_shape, output_tensor->mut_dptr<T>()),
...
#define REGISTER_REDUCE_XPU_KERNEL(op_name, binary_func, device, dtype)        \
  REGISTER_USER_KERNEL(op_name)                                                \
      ...
      .SetIsMatchedHob((user_op::HobDeviceTag() == device) &                   \
                       (user_op::HobDataType("output_tensor", 0) ==            \
...
REGISTER_REDUCE_XPU_KERNEL("reduce_sum", BinaryFuncSum, DeviceType::kGPU,
                           float16)
#endif
