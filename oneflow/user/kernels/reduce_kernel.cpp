...
class ReduceSumFloatKernel final : public user_op::OpKernel, public user_op::CudaGraphSupport {
 public:
  ReduceSumFloatKernel() = default;
  ~ReduceSumFloatKernel() = default;
 ...
 private:
  void Compute(user_op::KernelComputeContext* ctx) const override {
    ...
    if ((!axis.empty()) && in_shape.NumAxes() > 0 && is_axis_contiguous
      ...
      CHECK(fill);
      fill->Launch(ctx->stream(), tmp_buffer->mut_dptr(), 1.0, reduce_size);
      NewKernelUtil<DeviceType::kCUDA>::OFGemm(
          ctx->stream(), trans_a, trans_b, m, n, k, GetOneVal<float>(), input_tensor->dptr<float>(),
          tmp_buffer->dptr<float>(), GetZeroVal<float>(), output_tensor->mut_dptr<float>());
    } else {
      const Shape& reduced_shape = CreateReducedShape(in_shape, {axis.begin(), axis.end()});
      NdarrayReduce<DeviceType::kCUDA, float, BinaryFuncSum>::Reduce(
...
REGISTER_USER_KERNEL("reduce_sum")
    .SetCreateFn<ReduceSumFloatKernel>()
    .SetIsMatchedHob((user_op::HobDeviceType() == DeviceType::kCUDA)
