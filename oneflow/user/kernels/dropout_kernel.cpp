...
template <typename T> class DropoutKernelCPU final : public user_op::OpKernel {
public:
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    MaskAndScale<T>(ctx->device_ctx(), in->shape().elem_cnt(), scale,
                    in->dptr<T>(), mask->dptr<int8_t>(), out->mut_dptr<T>());
  }
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    const float scale = ctx->Attr<float>("scale");
    MaskAndScale<T>(ctx->device_ctx(), dy->shape().elem_cnt(), scale,
                    dy->dptr<T>(), mask->dptr<int8_t>(), dx->mut_dptr<T>());
  }
  bool AlwaysComputeWhenAllOutputsEmpty() const override { return false; }
};
...
public:
  ...
  CreateOpKernelState(user_op::KernelInitContext *ctx) const override {
    ...
    return std::make_shared<
        OpKernelStateWrapper<RandomMaskGenerator<device_type>>>(seed);
  }
...
private:
  void Compute(user_op::KernelComputeContext *ctx,
    ...
    auto *random_mask_generator =
        dynamic_cast<OpKernelStateWrapper<RandomMaskGenerator<device_type>> *>(
            state);
