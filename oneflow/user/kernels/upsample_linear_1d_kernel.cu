...
__global__ void UpsampleLinear1DBackward(const int64_t elem_cnt, const T* dy_dptr,
  ...
  CUDA_1D_KERNEL_LOOP(index, elem_cnt) {
    ...
    cuda::atomic::FastAdd(dx_dptr, static_cast<int>(dx_helper.NdIndexToOffset(n, c, h1)),
                          static_cast<int>(elem_cnt), static_cast<T>(h0lambda * dy_dptr[index]));
    cuda::atomic::FastAdd(dx_dptr, static_cast<int>(dx_helper.NdIndexToOffset(n, c, h1 + h1p)),
                          static_cast<int>(elem_cnt), static_cast<T>(h1lambda * dy_dptr[index]));
  }
}
