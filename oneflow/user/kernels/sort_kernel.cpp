...
template <typename T> class CpuSortKernel final : public user_op::OpKernel {
public:
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    Memcpy<DeviceType::kCPU>(ctx->device_ctx(), out->mut_dptr<T>(),
                             in->dptr<T>(), in->shape().elem_cnt() * sizeof(T));
...
#define REGISTER_CPU_SORT_KERNEL(dtype)                                        \
  REGISTER_USER_KERNEL("sort")                                                 \
      .SetCreateFn<CpuSortKernel<dtype>>()                                     \
      .SetIsMatchedHob(                                                        \
