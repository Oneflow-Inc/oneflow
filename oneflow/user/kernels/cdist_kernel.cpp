...
struct TwoDist {
  ...
  static inline T backward(const T& diff, const T grad, const T dist, const T& p) {
    return dist == 0.0 ? T(0) : T(grad) * diff / T(dist);
  }
};
...
void CpuCdistForward(ep::CpuStream* stream, const T* x1, const T* x2, T* out, int64_t size_out,
                     int64_t d, int64_t r1, int64_t r2, int64_t c, double p) {
  ...
  stream->ParallelFor(
      0, size_out,
      [x1, x2, out, d, r1, r2, c, p](int64_t begin, int64_t end) {
...
class CpuCdistKernel final : public user_op::OpKernel {
 ...
 private:
  void Compute(user_op::KernelComputeContext* ctx) const override {
    ...
    int64_t d = x1->shape_view().Count(0, ndim - 2);
    ...
    if (p == 0) {
      CpuCdistForward<T, ZeroDist<T>>(ctx->stream()->As<ep::CpuStream>(), x1_ptr, x2_ptr, out_ptr,
                                      out->shape_view().elem_cnt(), d, r1, r2, c, p);
    } else if (p == 1) {
