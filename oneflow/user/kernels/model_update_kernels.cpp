...
template<DeviceType device_type, typename T, typename G>
class AdamaxUpdateKernel final : public user_op::OpKernel, public user_op::CudaGraphSupport {
 public:
  AdamaxUpdateKernel() = default;
  ~AdamaxUpdateKernel() override = default;
 ...
 private:
  void Compute(user_op::KernelComputeContext* ctx) const override {
    ...
    user_op::Tensor* model = ctx->Tensor4ArgNameAndIndex("model", 0);
    user_op::Tensor* m = ctx->Tensor4ArgNameAndIndex("m", 0);
    user_op::Tensor* norm = ctx->Tensor4ArgNameAndIndex("norm", 0);
    const auto scale = ctx->Attr<double>("scale");
    const auto l1 = ctx->Attr<float>("l1");
    const auto l2 = ctx->Attr<float>("l2");
    const auto beta1 = ctx->Attr<float>("beta1");
    ...
    AdamaxUpdateKernelUtil<device_type, T, G>::Update(
        ctx->stream(), model->shape_view().elem_cnt(), static_cast<T>(scale), l1, l2, beta1, beta2,
...
#define REGISTER_ADAMAX_UPDATE_KERNEL(device, dtype, gtype)                               \
  REGISTER_USER_KERNEL("adamax_update")                                                   \
      .SetCreateFn<AdamaxUpdateKernel<device, dtype, gtype>>()                            \
      .SetIsMatchedHob((user_op::HobDeviceType() == device)                               \
                       && (user_op::HobDataType("model", 0) == GetDataType<dtype>::value) \
                       && (user_op::HobDataType("model_diff", 0) == GetDataType<gtype>::value));

REGISTER_ADAMAX_UPDATE_KERNEL(DeviceType::kCPU, float, float);
REGISTER_ADAMAX_UPDATE_KERNEL(DeviceType::kCPU, double, double);
#ifdef WITH_CUDA
REGISTER_ADAMAX_UPDATE_KERNEL(DeviceType::kCUDA, float, float16);
REGISTER_ADAMAX_UPDATE_KERNEL(DeviceType::kCUDA, float, float);
