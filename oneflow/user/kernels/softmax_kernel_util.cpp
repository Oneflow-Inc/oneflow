...
template <typename T> size_t GetProbTmpSize(int64_t n, int64_t w) {
  return GetCudaAlignedSize(n * sizeof(T));
}

template <typename T> size_t GetDiffTmpSize(int64_t n, int64_t w) {
  return GetCudaAlignedSize(n * sizeof(T));
}

template <typename T> size_t GetReduceTempStorageSize(int64_t n, int64_t w) {
...
size_t SoftmaxKernelUtil<device_type, T>::GetComputeProbTempStorageSizeInBytes(
    int64_t n, int64_t w) {
  return GetProbTmpSize<T>(n, w) + GetReduceTempStorageSize<T>(n, w);
}

template <DeviceType device_type, typename T>
size_t SoftmaxKernelUtil<device_type, T>::GetComputeDiffTempStorageSizeInBytes(
    int64_t n, int64_t w) {
  return GetDiffTmpSize<T>(n, w) + GetReduceTempStorageSize<T>(n, w);
}

template <DeviceType device_type, typename T>
void SoftmaxKernelUtil<device_type, T>::ComputeProb(
    DeviceCtx *ctx, const int64_t n, const int64_t w, const T *in, T *prob,
  ...
  const size_t min_temp_storage_bytes =
      SoftmaxKernelUtil<device_type, T>::GetComputeProbTempStorageSizeInBytes(
          n, w);
  ...
  auto reduce_storage_var =
      Var({static_cast<int64_t>(reduce_temp_storage_bytes / sizeof(T))},
          reduce_storage);
  T *tmp =
  ...
  NdarrayUtil<device_type, T>::BroadcastSub(ctx, Var({n, w}, prob),
                                            Val({n, w}, in), Val({n, 1}, tmp));
  // exp | prob[i][j] = exp(prob[i][j])
  NdarrayUtil<device_type, T>::InplaceExp(ctx, Var({n, w}, prob));
  // sum | tmp[i] = Sum_j(prob[i][j])
  NdarrayUtil<device_type, T>::ReduceSum(ctx, Var({n, 1}, tmp),
                                         Val({n, w}, prob), reduce_storage_var);
  // div | prob[i][j] /= tmp[i]
  NdarrayUtil<device_type, T>::InplaceBroadcastDiv(ctx, Var({n, w}, prob),
                                                   Val({n, 1}, tmp));
}

template <DeviceType device_type, typename T>
void SoftmaxKernelUtil<device_type, T>::ComputeDiff(
    DeviceCtx *ctx, const int64_t n, const int64_t w, const T *dy, const T *out,
    T *dx, void *temp_storage, const size_t temp_storage_bytes) {
  auto Val = NdarrayUtil<device_type, T>::GetValNdarrayBuilder();
  auto Var = NdarrayUtil<device_type, T>::GetVarNdarrayBuilder();
  const size_t min_temp_storage_bytes =
      SoftmaxKernelUtil<device_type, T>::GetComputeProbTempStorageSizeInBytes(
  ...
  T *tmp = dx;
  NdarrayUtil<device_type, T>::Mul(ctx, Var({n * w}, tmp), Val({n * w}, out),
                                   Val({n * w}, dy));
  NdarrayUtil<device_type, T>::ReduceSum(ctx, Var({n, 1}, sum_vec),
                                         Val({n, w}, tmp), reduce_storage_var);
  // sub | dx[i][j] = dy[i][j] - sum_vec[i]
  NdarrayUtil<device_type, T>::BroadcastSub(
      ctx, Var({n, w}, dx), Val({n, w}, dy), Val({n, 1}, sum_vec));
  // elementwise multiplication | dx[i][j] *= out[i][j]
  NdarrayUtil<device_type, T>::InplaceMul(ctx, Var({n * w}, dx),
                                          Val({n * w}, out));
}
