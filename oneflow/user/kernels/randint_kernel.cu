...
namespace oneflow {
__global__ void GenValues(int64_t* a,const int64_t low,const int64_t high,int32_t n,curandState* state) {
  XPU_1D_KERNEL_LOOP(i, n) {
    a[i] = curand(state+i)%(high-low)+low;//@TODO:curandState only generates 32-bit random number 
  }
}
