...
class DiagonalKernel final : public user_op::OpKernel {
 ...
 private:
  void Compute(user_op::KernelComputeContext* ctx) const override {
    ...
    int32_t size = 0;
    if (offset >= 0) {
        size = std::min(in_shape.At(0), in_shape.At(1) - offset);
    } else {
        size = std::min(in_shape.At(0) + offset, in_shape.At(1));
    }   

    int32_t dim0 = in_shape.At(0);
    int32_t dim1 = in_shape.At(1);
    int32_t dim2 = in_shape.At(2);
...
class DiagonalBackwardKernel final : public user_op::OpKernel {
 ...
 private:
  void Compute(user_op::KernelComputeContext* ctx) const override {
    ...
    int32_t dim0 = dx_shape.At(0);
    int32_t dim1 = dx_shape.At(1);
    int32_t dim2 = dx_shape.At(2);
    int32_t size = dy_shape.At(1);
    int32_t offset_in_bufer = (offset >= 0 ? offset * dim2 : -offset * dim1 * dim2);
...
#define REGISTER_DIAGONAL_KERNELS(device, dtype)                                                 \
  REGISTER_USER_KERNEL("diagonal").SetCreateFn<DiagonalKernel<device, dtype>>().SetIsMatchedHob( \
