...
struct TrilScaleLoad {
  ...
  __device__ void load(DST* dst, int64_t row, int64_t col) {
    ...
    hip::softmax::Pack<SRC, N> pack;
    if (need_load) {
      const int64_t offset = row * row_size + col;
      pack.storage = *reinterpret_cast<const hip::softmax::PackType<SRC, N>*>(src + offset);
    }
#pragma unroll
    for (int i = 0; i < N; ++i) {
...
struct MaskAndScaleStore {
  ...
  __device__ void store(const SRC* src, int64_t row, int64_t col) {
    hip::softmax::Pack<DST, N> softmax_y_pack;
    hip::softmax::Pack<DST, N> dst_pack;
    const int64_t offset = row * row_size + col;
    hip::softmax::Pack<int8_t, N> mask_pack;
    mask_pack.storage = *reinterpret_cast<const hip::softmax::PackType<int8_t, N>*>(mask + offset);
#pragma unroll
    for (int i = 0; i < N; ++i) {
    ...
    *reinterpret_cast<hip::softmax::PackType<DST, N>*>(softmax_y + offset) =
        softmax_y_pack.storage;
    *reinterpret_cast<hip::softmax::PackType<DST, N>*>(dst + offset) = dst_pack.storage;
  }
  DST* dst;
  DST* softmax_y;
  const int8_t* mask;
...
#pragma unroll
    ...
    *reinterpret_cast<hip::softmax::PackType<DST, N>*>(dst + offset) = pack.storage;
  }
  DST* dst;
  int64_t tril_num_rows;
  int64_t row_size;
...
class FusedTrilScaleSoftmaxMaskScaleGradKernel final : public user_op::OpKernel {
 ...
 private:
  void Compute(user_op::KernelComputeContext* ctx) const override {
    ...
    using ComputeType = typename hip::softmax::DefaultComputeType<T>::type;
    hip::softmax::DirectLoad<T, ComputeType> load_softmax_y(softmax_y->dptr<T>(), cols);
    ...
    hip::softmax::DispatchSoftmaxGrad<decltype(load_softmax_y), decltype(load_dy), decltype(store),
                                       ComputeType>(ctx->device_ctx()->hip_stream(),
