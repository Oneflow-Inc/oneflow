...
__device__ int64_t GetNearestInputIndex(const int64_t out_dim_idx,
                                        const float scale,
                                        const int64_t in_dim_size) {
  return max(min(static_cast<int64_t>(
                     floorf((static_cast<float>(out_dim_idx) + 0.5f) * scale)),
                 in_dim_size - 1),
...
template <typename T>
__global__ void
UpsampleNearestForward(const int64_t elem_cnt, const T *in_dptr,
...
template <typename T>
__global__ void
UpsampleNearestBackward(const int64_t elem_cnt, const T *dy_dptr,
  ...
  CUDA_1D_KERNEL_LOOP(index, elem_cnt) {
    ...
    atomicAdd(dx_dptr + dx_helper.NdIndexToOffset(n, c, dx_h, dx_w),
              dy_dptr[index]);
  }
}
...
__device__ void GetBilinearParam(const int64_t index, const int64_t h,
                                 const int64_t w, const int64_t in_height,
                                 const int64_t in_width, const float scale_h,
                                 const float scale_w, BilinearParam *params) {
...
template <typename T>
__global__ void
UpsampleBilinearForward(const int64_t elem_cnt, const T *in_dptr,
                        NdIndexOffsetHelper<int64_t, 4> in_helper,
                        NdIndexOffsetHelper<int64_t, 4> out_helper,
                        const int64_t in_height, const int64_t in_width,
                        const float scale_h, const float scale_w, T *out_dptr) {
  CUDA_1D_KERNEL_LOOP(index, elem_cnt) {
    ...
    GetBilinearParam(index, h, w, in_height, in_width, scale_h, scale_w,
                     &params);
    const int64_t top_offset =
        in_helper.NdIndexToOffset(n, c, params.top_h_index, 0);
    const int64_t bottom_offset =
        in_helper.NdIndexToOffset(n, c, params.bottom_h_index, 0);
    ...
    const float bottom =
        bottom_left + (bottom_right - bottom_left) * params.w_lerp;
...
template <typename T>
__global__ void
UpsampleBilinearBackward(const int64_t elem_cnt, const T *dy_dptr,
                         NdIndexOffsetHelper<int64_t, 4> dy_helper,
                         NdIndexOffsetHelper<int64_t, 4> dx_helper,
                         const int64_t dx_height, const int64_t dx_width,
                         const float scale_h, const float scale_w, T *dx_dptr) {
  CUDA_1D_KERNEL_LOOP(index, elem_cnt) {
    ...
    dy_helper.OffsetToNdIndex(index, n, c, h, w);
    BilinearParam params;
    GetBilinearParam(index, h, w, dx_height, dx_width, scale_h, scale_w,
                     &params);
    const int64_t top_offset =
        dx_helper.NdIndexToOffset(n, c, params.top_h_index, 0);
    const int64_t bottom_offset =
        dx_helper.NdIndexToOffset(n, c, params.bottom_h_index, 0);
    ...
    atomicAdd(dx_dptr_top_offset + params.left_w_index,
              static_cast<T>((1 - params.w_lerp) * dtop));
    atomicAdd(dx_dptr_top_offset + params.right_w_index,
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    NdIndexOffsetHelper<int64_t, 4> in_helper(
        x_blob->shape().At(0), x_blob->shape().At(1), x_blob->shape().At(2),
        x_blob->shape().At(3));
    NdIndexOffsetHelper<int64_t, 4> out_helper(
        y_blob->shape().At(0), y_blob->shape().At(1), y_blob->shape().At(2),
        y_blob->shape().At(3));

    RUN_CUDA_KERNEL((UpsampleNearestForward<T>), ctx->device_ctx(), elem_cnt,
                    elem_cnt, x_blob->dptr<T>(), in_helper, out_helper,
                    x_blob->shape().At(2), x_blob->shape().At(3),
                    1.f / height_scale, 1.f / width_scale,
                    y_blob->mut_dptr<T>());
  }
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    NdIndexOffsetHelper<int64_t, 4> dy_helper(
        dy_blob->shape().At(0), dy_blob->shape().At(1), dy_blob->shape().At(2),
        dy_blob->shape().At(3));
    NdIndexOffsetHelper<int64_t, 4> dx_helper(
        dx_blob->shape().At(0), dx_blob->shape().At(1), dx_blob->shape().At(2),
        dx_blob->shape().At(3));
    RUN_CUDA_KERNEL((UpsampleNearestBackward<T>), ctx->device_ctx(), elem_cnt,
                    elem_cnt, dy_blob->dptr<T>(), dy_helper, dx_helper,
...
#define REGISTER_UPSAMPLE_NEAREST_GPU_KERNEL(dtype)                            \
  REGISTER_USER_KERNEL("upsample")                                             \
      ...
      .SetIsMatchedHob(                                                        \
          ...
          (user_op::HobAttr<std::string>("interpolation") ==                   \
           std::string("nearest")));                                           \
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    RUN_CUDA_KERNEL((UpsampleBilinearForward<T>), ctx->device_ctx(), elem_cnt,
                    elem_cnt, x_blob->dptr<T>(), in_helper, out_helper,
                    x_blob->shape().At(2), x_blob->shape().At(3),
                    1.f / height_scale, 1.f / width_scale,
                    y_blob->mut_dptr<T>());
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    RUN_CUDA_KERNEL((UpsampleBilinearBackward<T>), ctx->device_ctx(), elem_cnt,
                    elem_cnt, dy_blob->dptr<T>(), dy_helper, dx_helper,
                    dx_blob->shape().At(2), dx_blob->shape().At(3),
                    1.f / height_scale, 1.f / width_scale,
                    dx_blob->mut_dptr<T>());
