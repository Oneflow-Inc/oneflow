...
template <typename T, int NDIM>
void LaunchSliceForward(DeviceCtx *ctx, const SliceParams &params,
                        const T *entire, T *sliced) {
  ...
  SliceForwardGpu<T, NDIM><<<BlocksNum4ThreadsNum(elem_cnt),
                             kCudaThreadsNumPerBlock, 0, ctx->cuda_stream()>>>(
      elem_cnt, params, entire_idx_cvtr, sliced_idx_cvtr, entire, sliced);
}

template <typename T, int NDIM>
void LaunchSliceBackward(DeviceCtx *ctx, const SliceParams &params,
                         const T *sliced, T *entire) {
  CHECK_EQ(params.ndim, NDIM);
  int64_t elem_cnt = params.elem_cnt();
  SliceIndexHelper<NDIM> entire_idx_cvtr(params.dims);
  SliceIndexHelper<NDIM> sliced_idx_cvtr(params.size);
  SliceBackwardGpu<T, NDIM><<<BlocksNum4ThreadsNum(elem_cnt),
                              kCudaThreadsNumPerBlock, 0, ctx->cuda_stream()>>>(
      elem_cnt, params, entire_idx_cvtr, sliced_idx_cvtr, entire, sliced);
}
...
void LaunchSliceForward(DeviceCtx *ctx, const SliceParams &params,
  ...
  SliceIndexHelper<NDIM> entire_idx_cvtr(params.dims);
  SliceIndexHelper<NDIM> sliced_idx_cvtr(params.size);
  SliceForwardGpuHalf<NDIM><<<BlocksNum4ThreadsNum(elem_cnt),
                              kCudaThreadsNumPerBlock, 0, ctx->cuda_stream()>>>(
      elem_cnt, params, entire_idx_cvtr, sliced_idx_cvtr,
      reinterpret_cast<const half *>(entire), reinterpret_cast<half *>(sliced));
}
...
void LaunchSliceBackward(DeviceCtx *ctx, const SliceParams &params,
  ...
  SliceIndexHelper<NDIM> entire_idx_cvtr(params.dims);
  SliceIndexHelper<NDIM> sliced_idx_cvtr(params.size);
  SliceBackwardGpuHalf<
      NDIM><<<BlocksNum4ThreadsNum(elem_cnt), kCudaThreadsNumPerBlock, 0,
              ctx->cuda_stream()>>>(
      elem_cnt, params, entire_idx_cvtr, sliced_idx_cvtr,
      reinterpret_cast<half *>(entire), reinterpret_cast<const half *>(sliced));
}

template <typename T> struct SliceSwitchUtil final {
#define MAKE_SLICE_SWITCH_ENTRY(func_name, N) func_name<T, N>
#define DEFINE_SLICE_SWITCH_UTIL_STATIC_METHOD(func_name)                      \
  DEFINE_STATIC_SWITCH_FUNC(void, func_name, MAKE_SLICE_SWITCH_ENTRY,          \
...
template <> struct SliceSwitchUtil<float16> {
...
template <typename T> struct SliceKernelUtil<DeviceType::kGPU, T> {
  static void Forward(DeviceCtx *ctx, const SliceParams &params,
    ...
    SliceSwitchUtil<T>::SwitchLaunchSliceForward(
        SwitchCase(fold_slice_params.ndim), ctx, fold_slice_params, entire,
        sliced);
  }
  ...
  static void Backward(DeviceCtx *ctx, const SliceParams &params,
    ...
    SliceSwitchUtil<T>::SwitchLaunchSliceBackward(
        SwitchCase(fold_slice_params.ndim), ctx, fold_slice_params, sliced,
        entire);
  }
};
