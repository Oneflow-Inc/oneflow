...
public:
  UnsortedSegmentSumOpKernelState(int64_t lower, int64_t upper)
...
public:
  ...
  CreateOpKernelState(user_op::KernelInitContext *ctx) const override {
    ...
    if (out_sbp.has_split_parallel() &&
        out_sbp.split_parallel().axis() == axis &&
        ctx->parallel_ctx().parallel_num() > 1) {
      CHECK(ctx->SbpParallel4ArgNameAndIndex("segment_ids", 0)
                .has_broadcast_parallel());
      CHECK(
          ctx->SbpParallel4ArgNameAndIndex("data", 0).has_broadcast_parallel());
      const TensorDesc *out_logical_desc =
          ctx->LogicalTensorDesc4ArgNameAndIndex("out", 0);
...
private:
  void Compute(user_op::KernelComputeContext *ctx,
    ...
    UnsortedSegmentSumKernelUtil<device_type, T, K>::UnsortedSegmentSum(
        ctx->device_ctx(), segment_ids->dptr<K>(), data->dptr<T>(),
        num_segment_ids, num_segments, outer_dim_size, inner_dim_size, offset,
        out->mut_dptr<T>());
  }
  bool AlwaysComputeWhenAllOutputsEmpty() const override { return true; }
};

#define REGISTER_UNSORTED_SEGMENT_SUM_KERNEL(device, out_type,                 \
                                             segment_ids_type, kernel_type)    \
  REGISTER_USER_KERNEL(kernel_type)                                            \
      .SetCreateFn<                                                            \
          UnsortedSegmentSumKernel<device, OF_PP_PAIR_FIRST(out_type),         \
...
#define REGISTER_UNSORTED_SEGMENT_SUM_KERNEL_CASE(device_type, out_type,       \
                                                  segment_ids_type)            \
  REGISTER_UNSORTED_SEGMENT_SUM_KERNEL(                                        \
      device_type, out_type, segment_ids_type, ("unsorted_segment_sum"))

#define REGISTER_UNSORTED_SEGMENT_SUM_LIKE_KERNEL_CASE(device_type, out_type,  \
                                                       segment_ids_type)       \
  REGISTER_UNSORTED_SEGMENT_SUM_KERNEL(                                        \
      device_type, out_type, segment_ids_type, ("unsorted_segment_sum_like"))
