...
class TransposeKernel final : public OpKernel, public user_op::CudaGraphSupport {
 ...
 private:
  void Compute(KernelComputeContext* ctx) const override {
    ...
    primitive->Launch(ctx->stream_ctx(), dtype, num_dims, src_dims.data(), tensor_in->dptr<T>(), perm.data(), tensor_out->mut_dptr<T>()); 
  }
  bool AlwaysComputeWhenAllOutputsEmpty() const override { return false; }
};

// hob::HobContextGetter<KernelRegContext, bool> PermutePrimitiveExists() {
//   return HobCtxGetter<bool>("PermutePrimitiveExists", [](const KernelRegContext& ctx) {
