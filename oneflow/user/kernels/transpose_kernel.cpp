...
private:
  void Compute(KernelComputeContext *ctx) const override {
    ...
    if (num_elem_per_pack != 1 && perm.back() == perm.size() - 1 &&
      ...
      CHECK_EQ(in_shape.At(in_shape.NumAxes() - 1),
      ...
      NewKernelUtil<device_type>::Transpose(
          ctx->device_ctx(), packed_in_shape.NumAxes(), packed_in_shape,
          packed_out_shape, perm, packed_in_shape.elem_cnt(),
          reinterpret_cast<const PackType *>(tensor_in->dptr<T>()),
          reinterpret_cast<PackType *>(tensor_out->mut_dptr<T>()));
    } else {
      NewKernelUtil<device_type>::Transpose(
          ctx->device_ctx(), in_shape.NumAxes(), in_shape, tensor_out->shape(),
          perm, in_shape.elem_cnt(), tensor_in->dptr<T>(),
          tensor_out->mut_dptr<T>());
    }
  }
