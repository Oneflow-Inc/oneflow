...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    for (int32_t i = 0; i < n; ++i) {
      y[i] = UnaryFunctor<T>::Forward(x[i]);
    }
  }
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    CHECK_LE(n, GetMaxVal<int32_t>() / 2);
    for (int32_t i = 0; i < n; ++i) {
      dx[i] = UnaryFunctor<T>::Backward(x[i], dy[i]);
    }
  }
  bool AlwaysComputeWhenAllOutputsEmpty() const override { return false; }
};
...
#define REGISTER_MATH_UNARY_ELEMENTWISE_CPU_KERNEL_AND_GRAD(math_type_pair,    \
  ...
  REGISTER_USER_KERNEL(OF_PP_PAIR_FIRST(math_type_pair))                       \
      .SetCreateFn<MathUnaryElementwiseCpuKernel<                              \
          OF_PP_CAT(OF_PP_PAIR_SECOND(math_type_pair), Functor),               \
          OF_PP_PAIR_FIRST(data_type_pair)>>()                                 \
      .SetIsMatchedHob((user_op::HobDeviceTag() == "cpu") &                    \
                       (user_op::HobDataType("x", 0) ==                        \
                        OF_PP_PAIR_SECOND(data_type_pair)) &                   \
                       (user_op::HobDataType("y", 0) ==                        \
                        OF_PP_PAIR_SECOND(data_type_pair)));                   \
                                                                               \
  REGISTER_USER_KERNEL(                                                        \
      (std::string("") + OF_PP_PAIR_FIRST(math_type_pair) + "_grad"))          \
      .SetCreateFn<MathUnaryElementwiseGradCpuKernel<                          \
          OF_PP_CAT(OF_PP_PAIR_SECOND(math_type_pair), Functor),               \
          OF_PP_PAIR_FIRST(data_type_pair)>>()                                 \
      .SetIsMatchedHob((user_op::HobDeviceTag() == "cpu") &                    \
                       (user_op::HobDataType("x", 0) ==                        \
...
OF_PP_SEQ_PRODUCT_FOR_EACH_TUPLE(
    REGISTER_MATH_UNARY_ELEMENTWISE_CPU_KERNEL_AND_GRAD,
