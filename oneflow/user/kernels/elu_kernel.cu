...
//   dy) {
//     OF_CUDA_CHECK(oneflow::cuda::elementwise::Binary(EluGradFunctor<T>(alpha), elem_cnt, dx, x,
//     dy,
//                                                      ctx->cuda_stream()));
//   }
// };
...
#include "oneflow/user/kernels/elementwise_gpu_kernel.h"
#include "oneflow/user/kernels/elu_kernel.h"
...
#define REGISTER_ELU_GPU_KERNEL(device, dtype)                                             \
  REGISTER_USER_KERNEL("elu")                                                           \
      .SetCreateFn([](user_op::KernelCreateContext* ctx) {                              \
        return new UnaryElemwiseGpuKernel<device, EluFunctor<dtype>, dtype>(               \
...
REGISTER_ELU_GPU_KERNEL(DeviceType::kGPU, half);
REGISTER_ELU_GPU_KERNEL(DeviceType::kGPU, float);
REGISTER_ELU_GPU_KERNEL(DeviceType::kGPU, double);
