...
void DecodeImage(const TensorBuffer &raw_bytes, TensorBuffer *image_buffer,
  ...
  CHECK(raw_bytes.data_type() == DataType::kChar ||
        raw_bytes.data_type() == DataType::kInt8 ||
  ...
  cv::Mat image_mat = cv::imdecode(
      raw_bytes_arr, (ImageUtil::IsColor(color_space) ? cv::IMREAD_COLOR
                                                      : cv::IMREAD_GRAYSCALE) |
                         cv::IMREAD_ANYDEPTH);
  ...
  FOR_RANGE(int64_t, i, 0, h) {
    memcpy(image_ptr + i * w, image_mat.ptr(i),
           w * GetSizeOfDataType(data_type));
  }
}
