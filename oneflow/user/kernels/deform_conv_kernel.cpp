...
scalar_t get_coordinate_weight(const scalar_t* im_data, int height, int width, scalar_t y,
                               scalar_t x, bool is_y_direction) {
  int y_l = floor(y);
  int x_l = floor(x);
  int y_h = y_l + 1;
  int x_h = x_l + 1;
...
template<typename scalar_t>
scalar_t bilinear_interpolate(const scalar_t* in, int height, int width, scalar_t h, scalar_t w) {
...
template<typename scalar_t>
void DeformableIm2Col(int n, const scalar_t* input, const scalar_t* offset, const scalar_t* mask,
                      int height, int width, int weight_h, int weight_w, int pad_h, int pad_w,
...
template<typename scalar_t>
void DeformableCol2Im(int n, const scalar_t* col, const scalar_t* offset_data,
                      const scalar_t* mask_data, int channels, int height, int width, int kernel_h,
                      int kernel_w, int pad_h, int pad_w, int stride_h, int stride_w,
                      int dilation_h, int dilation_w, int batch_sz, int n_offset_grps, int out_h,
...
template<typename scalar_t>
void DeformableCol2ImCoord(int n, const scalar_t* col_data, const scalar_t* im_data,
                           const scalar_t* offset_data, const scalar_t* mask_data, int channels,
                           int height, int width, int weight_h, int weight_w, int pad_h, int pad_w,
                           int stride_h, int stride_w, int dilation_h, int dilation_w, int batch_sz,
                           int offset_channels, int n_offset_grps, int out_h, int out_w,
                           const bool use_mask, scalar_t* grad_offset, scalar_t* grad_mask) {
  for (int index = 0; index != n; ++index) {
