...
template <typename T> T GetDtypeMatchedValue(double floating, int64_t integral);

template <> float GetDtypeMatchedValue(double floating, int64_t integral) {
  return static_cast<float>(floating);
}

template <> double GetDtypeMatchedValue(double floating, int64_t integral) {
  return floating;
}

template <> int8_t GetDtypeMatchedValue(double floating, int64_t integral) {
  return static_cast<int8_t>(integral);
}

template <> int32_t GetDtypeMatchedValue(double floating, int64_t integral) {
  return static_cast<int32_t>(integral);
}

template <> int64_t GetDtypeMatchedValue(double floating, int64_t integral) {
  return integral;
}

} // namespace

template <typename T> struct ClipKernelUtil<DeviceType::kCPU, T> {
  template <typename F>
  static void Forward(DeviceCtx *ctx, F clip_func, const int64_t n, const T *x,
                      T *y) {
    FOR_RANGE(int64_t, i, 0, n) { y[i] = clip_func(x[i]); }
  }

  template <typename F>
  static void Backward(DeviceCtx *ctx, F clip_func, const int64_t n, const T *x,
                       const T *dy, T *dx) {
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    ClipByMinMaxFunctor<T> clip_func(
        GetDtypeMatchedValue<T>(floating_min, integral_min),
        GetDtypeMatchedValue<T>(floating_max, integral_max));
    ClipKernelUtil<device_type, T>::Forward(ctx->device_ctx(), clip_func,
                                            y->shape().elem_cnt(), x->dptr<T>(),
                                            y->mut_dptr<T>());
  }
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    double floating_max = ctx->Attr<double>("floating_max");
    int64_t integral_max = ctx->Attr<int64_t>("integral_max");
    ClipByMinMaxGradFunctor<T> clip_func(
        GetDtypeMatchedValue<T>(floating_min, integral_min),
        GetDtypeMatchedValue<T>(floating_max, integral_max));
    ClipKernelUtil<device_type, T>::Backward(
        ctx->device_ctx(), clip_func, dx->shape().elem_cnt(), x->dptr<T>(),
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    double floating_min = ctx->Attr<double>("floating_min");
    int64_t integral_min = ctx->Attr<int64_t>("integral_min");
    ClipByMinGradFunctor<T> clip_func(
        GetDtypeMatchedValue<T>(floating_min, integral_min));
    ClipKernelUtil<device_type, T>::Backward(
        ctx->device_ctx(), clip_func, dx->shape().elem_cnt(), x->dptr<T>(),
        dy->dptr<T>(), dx->mut_dptr<T>());
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    double floating_max = ctx->Attr<double>("floating_max");
    int64_t integral_max = ctx->Attr<int64_t>("integral_max");
    ClipByMaxGradFunctor<T> clip_func(
        GetDtypeMatchedValue<T>(floating_max, integral_max));
    ClipKernelUtil<device_type, T>::Backward(
        ctx->device_ctx(), clip_func, dx->shape().elem_cnt(), x->dptr<T>(),
        dy->dptr<T>(), dx->mut_dptr<T>());
...
#define REGISTER_CLIP_GRAD_KERNEL(op_type_name, kernel_name, device_type_v,    \
                                  dtype)                                       \
  REGISTER_USER_KERNEL(#op_type_name)                                          \
...
#define REGISTER_CLIP_KERNELS(device_type_v, dtype_pair)                       \
  REGISTER_CLIP_KERNEL(clip_by_scalar, ClipByScalar, device_type_v,            \
  ...
  REGISTER_CLIP_GRAD_KERNEL(clip_by_scalar_grad, ClipByScalar, device_type_v,  \
                            OF_PP_PAIR_FIRST(dtype_pair))                      \
  REGISTER_CLIP_GRAD_KERNEL(clip_by_scalar_min_grad, ClipByScalarMin,          \
                            device_type_v, OF_PP_PAIR_FIRST(dtype_pair))       \
  REGISTER_CLIP_GRAD_KERNEL(clip_by_scalar_max_grad, ClipByScalarMax,          \
                            device_type_v, OF_PP_PAIR_FIRST(dtype_pair))

OF_PP_SEQ_PRODUCT_FOR_EACH_TUPLE(REGISTER_CLIP_KERNELS, DEVICE_TYPE_SEQ,
