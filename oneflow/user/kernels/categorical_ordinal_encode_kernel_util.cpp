  ...
  static void Encode(DeviceCtx *ctx, int64_t capacity, T *table, T *size,
    ...
    for (int64_t i = 0; i < n; ++i) {
      ...
      for (int64_t count = 0; count < capacity; ++count) {
        size_t idx = (static_cast<size_t>(h) + static_cast<size_t>(count)) %
...
#define INSTANTIATE_CATEGORICAL_ORDINAL_ENCODE_KERNEL_UTIL_CPU(type_cpp,       \
                                                               type_proto)     \
  template struct CategoricalOrdinalEncodeKernelUtil<DeviceType::kCPU,         \
                                                     type_cpp>;
OF_PP_FOR_EACH_TUPLE(INSTANTIATE_CATEGORICAL_ORDINAL_ENCODE_KERNEL_UTIL_CPU,
                     INDEX_DATA_TYPE_SEQ);
