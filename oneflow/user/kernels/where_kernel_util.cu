...
__global__ void CudaWhere(const int64_t elem_cnt, const CondT *cond,
                          const T *lhs, const T *rhs, T *out) {
...
struct WhereKernelUtil<DeviceType::kGPU, T, CondT> {
  static void Where(DeviceCtx *ctx, const int64_t elem_cnt, const CondT *cond,
                    const T *lhs, const T *rhs, T *out) {
    RUN_CUDA_KERNEL((CudaWhere<T, CondT>), ctx, elem_cnt, elem_cnt, cond, lhs,
                    rhs, out);
  }
};
