...
template <template <typename> class UnaryFunctor, typename T>
__global__ void MathUnaryElementwiseBackwardGpu(const int n, const T *x,
                                                const T *dy, T *dx) {
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    MathUnaryElementwiseForwardGpu<
        UnaryFunctor, T><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock, 0,
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    int64_t n = tensor_x->shape().elem_cnt();
    CHECK_LE(n, GetMaxVal<int32_t>() / 2);
    MathUnaryElementwiseBackwardGpu<
        UnaryFunctor, T><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock, 0,
                           ctx->device_ctx()->cuda_stream()>>>(n, x, dy, dx);
  }
  bool AlwaysComputeWhenAllOutputsEmpty() const override { return false; }
};

#define REGISTER_MATH_UNARY_ELEMENTWISE_GPU_KERNEL_AND_GRAD(math_type_pair,    \
                                                            data_type_pair)    \
  REGISTER_USER_KERNEL(OF_PP_PAIR_FIRST(math_type_pair))                       \
      .SetCreateFn<MathUnaryElementwiseGpuKernel<                              \
  ...
  REGISTER_USER_KERNEL(                                                        \
      ...
      .SetCreateFn<MathUnaryElementwiseGradGpuKernel<                          \
          OF_PP_CAT(OF_PP_PAIR_SECOND(math_type_pair), Functor),               \
          OF_PP_PAIR_FIRST(data_type_pair)>>()                                 \
      .SetIsMatchedHob((user_op::HobDeviceTag() == "gpu") &                    \
                       (user_op::HobDataType("x", 0) ==                        \
...
OF_PP_SEQ_PRODUCT_FOR_EACH_TUPLE(
    REGISTER_MATH_UNARY_ELEMENTWISE_GPU_KERNEL_AND_GRAD,
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    CHECK_LE(n, GetMaxVal<int32_t>() / 2);
    MathUnaryElementwiseForwardGpu<
        UnaryFunctor, half><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock,
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    MathUnaryElementwiseBackwardGpu<
        UnaryFunctor, half><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock,
                              0, ctx->device_ctx()->cuda_stream()>>>(n, x, dy,
                                                                     dx);
  }
  bool AlwaysComputeWhenAllOutputsEmpty() const override { return false; }
};

#define REGISTER_MATH_UNARY_ELEMENTWISE_GPU_HALF_KERNEL_AND_GRAD(              \
    math_type_str, math_func_prefix)                                           \
  REGISTER_USER_KERNEL(math_type_str)                                          \
      .SetCreateFn<MathUnaryElementwiseGpuHalfKernel<OF_PP_CAT(                \
  ...
  REGISTER_USER_KERNEL((std::string("") + math_type_str + "_grad"))            \
      .SetCreateFn<MathUnaryElementwiseGradGpuHalfKernel<OF_PP_CAT(            \
          math_func_prefix, Functor)>>()                                       \
      .SetIsMatchedHob((user_op::HobDeviceTag() == "gpu") &                    \
                       (user_op::HobDataType("x", 0) == DataType::kFloat16));
