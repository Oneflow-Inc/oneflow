...
__global__ void MathBinaryElementwiseForwardGpu(const int n, const T *x,
                                                const T *y, T *z) {
  CUDA_1D_KERNEL_LOOP(i, n) { z[i] = BinaryFunctor<T>::Forward(x[i], y[i]); }
}

template <template <typename> class BinaryFunctor, typename T>
__global__ void MathBinaryElementwiseBackwardXGradGpu(const int n, const T *x,
                                                      const T *y, const T *dz,
                                                      T *dx) {
...
template <template <typename> class BinaryFunctor, typename T>
__global__ void MathBinaryElementwiseBackwardYGradGpu(const int n, const T *x,
                                                      const T *y, const T *dz,
                                                      T *dy) {
  CUDA_1D_KERNEL_LOOP(i, n) {
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    MathBinaryElementwiseForwardGpu<
        BinaryFunctor, T><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock, 0,
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    int64_t n = tensor_x->shape().elem_cnt();
    CHECK_LE(n, GetMaxVal<int32_t>() / 2);
    MathBinaryElementwiseBackwardXGradGpu<
        BinaryFunctor, T><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock, 0,
                            ctx->device_ctx()->cuda_stream()>>>(
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    int64_t n = tensor_x->shape().elem_cnt();
    CHECK_LE(n, GetMaxVal<int32_t>() / 2);
    MathBinaryElementwiseBackwardYGradGpu<
        BinaryFunctor, T><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock, 0,
                            ctx->device_ctx()->cuda_stream()>>>(
        n, tensor_x->dptr<T>(), tensor_y->dptr<T>(), tensor_dz->dptr<T>(),
...
#define REGISTER_MATH_BINARY_ELEMENTWISE_GPU_KERNEL_AND_GRAD(math_type_pair,   \
                                                             data_type_pair)   \
  REGISTER_USER_KERNEL(OF_PP_PAIR_FIRST(math_type_pair))                       \
      .SetCreateFn<MathBinaryElementwiseGpuKernel<                             \
  ...
  REGISTER_USER_KERNEL(                                                        \
      (std::string("") + OF_PP_PAIR_FIRST(math_type_pair) + "_x_grad"))        \
      ...
      .SetIsMatchedHob((user_op::HobDeviceTag() == "gpu") &                    \
                       (user_op::HobDataType("x", 0) ==                        \
                        OF_PP_PAIR_SECOND(data_type_pair)));                   \
  REGISTER_USER_KERNEL(                                                        \
      (std::string("") + OF_PP_PAIR_FIRST(math_type_pair) + "_y_grad"))        \
...
OF_PP_SEQ_PRODUCT_FOR_EACH_TUPLE(
    REGISTER_MATH_BINARY_ELEMENTWISE_GPU_KERNEL_AND_GRAD,
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    CHECK_LE(n, GetMaxVal<int32_t>() / 2);
    MathBinaryElementwiseForwardGpu<
        BinaryFunctor, half><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock,
                               0, ctx->device_ctx()->cuda_stream()>>>(n, x, y,
                                                                      z);
  }
  bool AlwaysComputeWhenAllOutputsEmpty() const override { return false; }
};
...
#define REGISTER_MATH_BINARY_ELEMENTWISE_GPU_HALF_KERNEL_AND_GRAD(             \
    math_type_str, math_func_prefix)                                           \
  REGISTER_USER_KERNEL(math_type_str)                                          \
      .SetCreateFn<MathBinaryElementwiseGpuHalfKernel<OF_PP_CAT(               \
          math_func_prefix, Functor)>>()                                       \
      .SetIsMatchedHob((user_op::HobDeviceTag() == "gpu") &                    \
                       (user_op::HobDataType("x", 0) == DataType::kFloat16));  \
                                                                               \
  REGISTER_USER_KERNEL((std::string("") + math_type_str + "_x_grad"))          \
      .SetCreateFn<MathBinaryElementwiseXGradGpuHalfKernel<OF_PP_CAT(          \
          math_func_prefix, Functor)>>()                                       \
      .SetIsMatchedHob((user_op::HobDeviceTag() == "gpu") &                    \
                       (user_op::HobDataType("x", 0) == DataType::kFloat16));  \
  REGISTER_USER_KERNEL((std::string("") + math_type_str + "_y_grad"))          \
      .SetCreateFn<MathBinaryElementwiseYGradGpuHalfKernel<OF_PP_CAT(          \
          math_func_prefix, Functor)>>()                                       \
      .SetIsMatchedHob((user_op::HobDeviceTag() == "gpu") &                    \
                       (user_op::HobDataType("x", 0) == DataType::kFloat16));
