...
public:
  GatherOpKernelState(int64_t lower, int64_t upper)
...
public:
  ...
  CreateOpKernelState(user_op::KernelInitContext *ctx) const override {
    ...
    if (in_sbp.has_split_parallel() && in_sbp.split_parallel().axis() == axis &&
      ...
      CHECK(ctx->SbpParallel4ArgNameAndIndex("indices", 0)
                .has_broadcast_parallel());
      CHECK(ctx->SbpParallel4ArgNameAndIndex("out", 0)
                .has_partial_sum_parallel());
      const TensorDesc *in_logical_desc =
          ctx->LogicalTensorDesc4ArgNameAndIndex("in", 0);
      ...
      return std::make_shared<GatherOpKernelState>(
...
private:
  void Compute(user_op::KernelComputeContext *ctx,
    ...
    if (state != nullptr) {
      ...
      CHECK_EQ(in->shape().At(axis),
               gather_state->upper() - gather_state->lower());
...
#define REGISTER_GATHER_KERNEL(device, in_type, indices_type)                  \
  REGISTER_USER_KERNEL("gather")                                               \
      .SetCreateFn<GatherKernel<device, OF_PP_PAIR_FIRST(in_type),             \
                                OF_PP_PAIR_FIRST(indices_type)>>()             \
      .SetIsMatchedHob(                                                        \
          (user_op::HobDeviceTag() == device) &                                \
          (user_op::HobDataType("in", 0) == OF_PP_PAIR_SECOND(in_type)) &      \
          (user_op::HobDataType("indices", 0) ==                               \
           OF_PP_PAIR_SECOND(indices_type)));

OF_PP_SEQ_PRODUCT_FOR_EACH_TUPLE(REGISTER_GATHER_KERNEL, DEVICE_TYPE_SEQ,
