...
#include "oneflow/user/kernels/cutlass_conv2d_operation_cache_key.h"
...
const cutlass::library::Operation* FindFastestOperation(
    const Singleton* singleton, const cutlass::library::ConvFunctionalKey& functional_key,
  ...
  const auto& operations_map = [&]() {
    const auto& it = singleton->operation_table.conv2d_operations.find(functional_key);
    CHECK(it != singleton->operation_table.conv2d_operations.cend());
    return it->second;
  }();
...
const cutlass::library::Operation* GetOperation(
    ...
    int cuda_arch) {
  const auto& it = singleton->operation_table.conv2d_operations.find(functional_key);
  if (it == singleton->operation_table.conv2d_operations.cend()) { return nullptr; }
  const cutlass::library::ConvOperationVectorMap& operations_map = it->second;
  for (const auto& pair : operations_map) {
    for (auto operation : pair.second) {
...
template<>
class CutlassConvTunerImpl<cutlass::library::Conv2dConfiguration, cutlass::library::ConvArguments> {
 public:
  using CacheMap = std::unordered_map<Conv2dOperationCacheKey, const cutlass::library::Operation*,
                                      Conv2dOperationCacheKeyHasher>;

  CutlassConvTunerImpl() { singleton = &cutlass::library::Singleton::get(); }
 ...
 private:
  ...
  const cutlass::library::Singleton* singleton;
};

const cutlass::library::Operation*
CutlassConvTunerImpl<cutlass::library::Conv2dConfiguration, cutlass::library::ConvArguments>::Find(
...
const cutlass::library::Operation*
CutlassConvTunerImpl<cutlass::library::Conv2dConfiguration, cutlass::library::ConvArguments>::Get(
...
#ifdef WITH_CUTLASS_EXTENSION
template<>
class CutlassConvTunerImpl<cutlass::library::Conv2dScaleBiasFusionConfiguration,
                           cutlass::library::ConvScaleBiasFusionArguments> {
 public:
  using CacheMap = std::unordered_map<Conv2dOperationCacheKey, const cutlass::library::Operation*,
                                      Conv2dOperationCacheKeyHasher>;
  ...
  const cutlass::library::Operation* Find(
      ep::CudaStream* stream, cutlass::library::ConvFunctionalKey functional_key,
      const cutlass::library::Conv2dScaleBiasFusionConfiguration& configuraion,
      const cutlass::library::ConvScaleBiasFusionArguments& arguments, void* workspace,
...
const cutlass::library::Operation*
CutlassConvTunerImpl<cutlass::library::Conv2dScaleBiasFusionConfiguration,
                     cutlass::library::ConvScaleBiasFusionArguments>::
    Find(ep::CudaStream* stream, cutlass::library::ConvFunctionalKey functional_key,
         const cutlass::library::Conv2dScaleBiasFusionConfiguration& configuraion,
         const cutlass::library::ConvScaleBiasFusionArguments& arguments, void* workspace,
         size_t workspace_size) {
  int dev = 0;
  ...
  cutlass::library::ConvScaleBiasFusionArguments benchmark_arguments = arguments;
  void* benchmark_workspace = workspace;
  cudaStream_t benchmark_stream = stream->cuda_stream();
#ifdef WITH_CUDA_GRAPHS
  cudaStreamCaptureMode mode = cudaStreamCaptureModeRelaxed;
  if (stream->IsGraphCapturing()) {
    ...
    if (benchmark_arguments.P != nullptr) {
      const size_t p_size = cutlass::library::sizeof_bits(functional_key.element_A) / 8;
      OF_CUDA_CHECK(cudaMalloc(&benchmark_arguments.P, p_size));
    }
    if (benchmark_arguments.Scale != nullptr) {
      const size_t scale_size =
...
CutlassConvTunerImpl<Configuration, Arguments>* GetCutlassConvTunerImpl() {
  static CutlassConvTunerImpl<Configuration, Arguments> impl;
  return &impl;
}

template CutlassConvTunerImpl<cutlass::library::Conv2dConfiguration,
                              cutlass::library::ConvArguments>*
GetCutlassConvTunerImpl<cutlass::library::Conv2dConfiguration, cutlass::library::ConvArguments>();

#ifdef WITH_CUTLASS_EXTENSION
template CutlassConvTunerImpl<cutlass::library::Conv2dScaleBiasFusionConfiguration,
                              cutlass::library::ConvScaleBiasFusionArguments>*
GetCutlassConvTunerImpl<cutlass::library::Conv2dScaleBiasFusionConfiguration,
                        cutlass::library::ConvScaleBiasFusionArguments>();
#endif  // WITH_CUTLASS_EXTENSION
