...
template <typename PerfT> struct CudnnDeConvArgsAndAlgo final {
  ...
  CudnnDeConvArgsAndAlgo(const user_op::Tensor *x, const user_op::Tensor *w,
                         ...
                         DeviceCtx *device_ctx, bool has_forced_algo,
                         int32_t forced_algo)
      : args(user_op_conf, x->data_type(), x->shape(), w->data_type(),
             w->shape(), y->data_type(), y->shape(),
             user_op_conf.attr<std::string>("data_format"),
             buf->shape().elem_cnt(),
    ...
    AllocatedCudnnConvResource res(
        device_ctx->cudnn_handle(), const_cast<void *>(x->dptr()),
    ...
    CHECK_LE(algo_perf.memory, byte_size_of_buf)
        << "op (" << user_op_conf.op_name() << ") find algorithm "
        << algo_perf.algo << ", need memory " << algo_perf.memory
        << ", but cudnn_buf_limit_byte is " << byte_size_of_buf;
  }
...
size_t InferTmpSizeWithCudnn(const user_op::TensorDesc *x,
                             const user_op::TensorDesc *w,
  ...
  if (!x->is_dynamic()) {
    CudnnConvArgs args(
        user_op_conf, x->data_type(), ShapeView(x->shape()), w->data_type(),
    ...
    if (has_forced_algo) {
      algo_perf = GetCudnnConvAlgorithmPerference<PerfT>(
          &args, static_cast<AlgoT>(forced_algo));
    } else {
    ...
    CHECK_LE(algo_perf.memory, workspace_size)
        << "op (" << user_op_conf.op_name() << ") find algorithm "
        << algo_perf.algo << ", need memory " << algo_perf.memory
        << ", but cudnn_buf_limit_byte is " << workspace_size;
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    const cudnnConvolutionBwdDataAlgoPerf_t &algo_perf =
        args_and_algo.algo_perf;

    OF_CUDNN_CHECK(cudnnConvolutionBackwardData(
        ctx->device_ctx()->cudnn_handle(), CudnnSPOnePtr<T>(), args.wdesc.Get(),
        weight->dptr(), args.ydesc.Get(), in->dptr(), args.cdesc.Get(),
        algo_perf.algo, buf->mut_dptr(), args.params.max_ws_size,
        CudnnSPZeroPtr<T>(), args.xdesc.Get(), out->mut_dptr()));
  }
};
