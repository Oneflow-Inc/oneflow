...
__global__ void MaskAndScaleGpu(uint64_t seed, int32_t* counter, uint64_t* offset, uint64_t counter_offset, const int64_t elem_cnt, float rate, float scale, const T* x, int8_t* mask,
                                T* y) {
    ...
    float4 rand_uniform; 
    for(int64_t linear_index=thread_id*pack_size; linear_index < elem_cnt; linear_index += gridDim.x * blockDim.x * pack_size) {
...
template<>
__global__ void MaskAndScaleGpu<half, 4>(uint64_t seed, int32_t* counter, uint64_t* offset, uint64_t counter_offset, const int64_t elem_cnt, float rate, float scale, const half* x, int8_t* mask,
                                half* y) {
    int32_t thread_id = blockIdx.x * blockDim.x + threadIdx.x; 
    curandStatePhilox4_32_10_t state; 
    curand_init(seed, thread_id, *offset, &state); 
    ...
    for(int64_t linear_index=thread_id*4; linear_index < elem_cnt; linear_index += gridDim.x * blockDim.x * 4) {
      rand_uniform = curand_uniform4(&state);
...
template<>
__global__ void MaskAndScaleGpu<double, 2>(uint64_t seed, int32_t* counter, uint64_t* offset, uint64_t counter_offset, 
                                           const int64_t elem_cnt, 
                                           float rate, float scale, const double* x, 
                                           int8_t* mask, double* y) {
    ...
    float4 rand_uniform; 
    for(int64_t linear_index=thread_id*2; linear_index < elem_cnt; linear_index += gridDim.x * blockDim.x * 2) {
      rand_uniform = curand_uniform4(&state);
...
__global__ void MaskAndScaleAddGpu<double, 2>(uint64_t seed, int32_t* counter, uint64_t* offset, 
                                              uint64_t counter_offset, const int64_t elem_cnt, 
                                              float rate, float scale, 
                                              const double* x, int8_t* mask,
                                              const double* addend, double* y) {
  int32_t thread_id = blockIdx.x * blockDim.x + threadIdx.x; 
  curandStatePhilox4_32_10_t state; 
  curand_init(seed, thread_id, *offset, &state); 
  using LoadT = typename std::aligned_storage<sizeof(double)*2, sizeof(double)*2>::type; 
...
template<typename T>
void MaskAndScale(DeviceCtx* ctx, uint64_t seed, int32_t* counter, uint64_t* offset, const int64_t elem_cnt, float rate, float scale, const T* x, int8_t* mask,
                  T* y) {
  int32_t UNROLL = 4; 
...
template<typename T>
void MaskAndScaleAdd(DeviceCtx* ctx, uint64_t seed, int32_t* counter, uint64_t* offset, const int64_t elem_cnt, float rate, float scale, const T* x, int8_t* mask,
                     const T* addend, T* y) {
  int32_t UNROLL = 4; 
  int32_t block_size = 256; 
  unsigned int grid_size = ComputeGridSize<4>(block_size, elem_cnt); 
  uint64_t counter_offset = ((elem_cnt - 1)/(block_size*grid_size*UNROLL)+1)*UNROLL;
  MaskAndScaleAddGpu<T, 4><<<grid_size, block_size, 0, ctx->cuda_stream()>>>(seed, counter, offset, counter_offset, elem_cnt, rate, scale, x, mask, addend, y);
}
 ...
 private:
  ...
  void Compute(user_op::KernelComputeContext* ctx, user_op::OpKernelState* state) const override {
    ...
    if (ctx->has_input("_add_to_output", 0)) {
      ...
      MaskAndScale<T>(ctx->device_ctx(), seed, counter, offset, in->shape().elem_cnt(), rate, scale, in->dptr<T>(),
