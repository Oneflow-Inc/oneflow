...
void DispatchPackSize(const T* x, const T* cos, const T* sin, const PositionType* position_ids, T* out, const int64_t* position_shape, const std::string& x_layout, 
                      const std::string& output_layout, const std::string& mode, const T theta, const int64_t pass_ndims, const int rotary_emb_dim, const IndexType b, 
  ...
  if (CheckPackSize(8)) {
    ...
    LaunchKernel<T, PositionType, IndexType, 8, num_dims>(x, cos, sin, position_ids, out, position_shape, x_layout, output_layout, 
                                          mode, theta, pass_ndims, rotary_emb_dim, b, m, h, k, 
...
void DispatchIndex(const T* x, const T* cos, const T* sin, const PositionType* position_ids, T* out, const int64_t* position_shape, const std::string& x_layout, 
  const std::string& output_layout, const std::string& mode, const T theta, const int64_t pass_ndims, const int rotary_emb_dim, const int64_t b, const int64_t m, const int64_t h, const int64_t k, const int64_t b_stride, 
  const int64_t m_stride, const int64_t h_stride, const int64_t offset) {
  int64_t num_elements = b * m * h * k;
  if (num_elements < (1 << 30)) {
    DispatchPackSize<T, PositionType, int32_t, num_dims>(x, cos, sin, position_ids, out, position_shape, x_layout, output_layout, mode,
    ...
    DispatchPackSize<T, PositionType, int64_t, num_dims>(x, cos, sin, position_ids, out, position_shape, x_layout, output_layout, mode,
  ...
  void Compute(user_op::KernelComputeContext* ctx) const override {
    ...
    DispatchIndex<T, PositionType, ndims>(
        ...
        sin ? reinterpret_cast<const T*>(sin->dptr()) : nullptr, position_ids ? reinterpret_cast<const PositionType*>(position_ids->dptr()) :
          ...
          x_layout, output_layout, mode, static_cast<T>(theta), pass_ndims, rotary_emb_dim, b, m, h, k, b_stride, m_stride, h_stride, offset);
  }
