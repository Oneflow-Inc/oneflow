...
private:
  void Compute(user_op::KernelComputeContext *ctx,
    ...
    auto *random_generator =
        dynamic_cast<OpKernelStateWrapper<std::mt19937> *>(state);
    ...
    for (int32_t i = 0; i < out_blob->shape().elem_cnt(); ++i) {
      ...
      *(out_dptr + i) =
          dis(*random_generator->Mutable()) ? GetOneVal<K>() : GetZeroVal<K>();
    }
  }
...
#define REGISTER_BERNOULLI_KERNEL(in_dtype_pair, out_dtype_pair)               \
  REGISTER_USER_KERNEL("bernoulli")                                            \
      .SetCreateFn<BernoulliKerenl<OF_PP_PAIR_FIRST(in_dtype_pair),            \
                                   OF_PP_PAIR_FIRST(out_dtype_pair)>>()        \
      .SetIsMatchedHob((user_op::HobDeviceTag() == "cpu") &                    \
                       (user_op::HobDataType("in", 0) ==                       \
                        OF_PP_PAIR_SECOND(in_dtype_pair)) &                    \
                       (user_op::HobDataType("out", 0) ==                      \
                        OF_PP_PAIR_SECOND(out_dtype_pair)));

OF_PP_SEQ_PRODUCT_FOR_EACH_TUPLE(REGISTER_BERNOULLI_KERNEL,
