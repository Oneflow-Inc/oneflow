...
template<template<typename> class Op, typename T>
struct UnaryByScalarTensorFunctor {
  __host__ __device__ explicit UnaryByScalarTensorFunctor(T scalar) : scalar(scalar) {}
...
template<template<typename> class BIN_OP>
struct ScalarTensorMathFunctor<DeviceType::kCUDA, BIN_OP, float16> final {
  void operator()(ep::Stream* stream, const int64_t elem_cnt, float16 scalar, const float16* in,
                  float16* out) {
    OF_CUDA_CHECK(cuda::elementwise::Unary(
        UnaryByScalarTensorFunctor<BIN_OP, float16>(float16_2half(scalar)), elem_cnt,
        reinterpret_cast<half*>(out), reinterpret_cast<const half*>(in),
        stream->As<ep::CudaStream>()->cuda_stream()));
  }
};
