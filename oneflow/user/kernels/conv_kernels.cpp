...
template <typename T>
using GemmFunc = void (*)(enum CBLAS_TRANSPOSE trans_a,
                          enum CBLAS_TRANSPOSE trans_b, const int m,
                          const int n, const int k, const T alpha, const T *a,
                          const T *b, const T beta, T *c);

template <typename T>
void Gemm4ChannelFirst(enum CBLAS_TRANSPOSE trans_a,
                       enum CBLAS_TRANSPOSE trans_b, const int m, const int n,
                       const int k, const T alpha, const T *a, const T *b,
                       const T beta, T *c) {
  NewKernelUtil<DeviceType::kCPU>::OFGemm(nullptr, trans_a, trans_b, m, n, k,
                                          alpha, a, b, beta, c);
}

template <typename T>
void Gemm4ChannelLast(enum CBLAS_TRANSPOSE trans_a,
                      enum CBLAS_TRANSPOSE trans_b, const int m, const int n,
                      const int k, const T alpha, const T *a, const T *b,
                      const T beta, T *c) {
  trans_a = (trans_a == CblasNoTrans) ? CblasTrans : CblasNoTrans;
  trans_b = (trans_b == CblasNoTrans) ? CblasTrans : CblasNoTrans;
  NewKernelUtil<DeviceType::kCPU>::OFGemm(nullptr, trans_b, trans_a, n, m, k,
                                          alpha, b, a, beta, c);
}

template <typename T> T *GetImgMutDptr(user_op::Tensor *tensor, int64_t idx) {
...
size_t CalcElemNumOfColBuf(const ShapeView &out_shape,
                           const ShapeView &weight_shape,
                           const int32_t idx_offset) {
  int64_t col_buf_elem_cnt = 1;
  int64_t ndims = out_shape.NumAxes() - 2;
  for (size_t i = 0; i != ndims + 1; ++i) {
    col_buf_elem_cnt *= weight_shape.At(i + 1);
  }
  for (size_t i = 0; i != ndims; ++i) {
    col_buf_elem_cnt *= out_shape.At(idx_offset + i);
  }
  return col_buf_elem_cnt;
}

template <typename T> class ColBufWriter {
public:
  ColBufWriter(const T *src_ptr, T *dst_ptr, int64_t c_size, int64_t id_size,
      ...
      : src_ptr_(src_ptr), dst_ptr_(dst_ptr), c_size_(c_size),
        id_size_(id_size), ih_size_(ih_size), iw_size_(iw_size),
        od_size_(od_size), oh_size_(oh_size), ow_size_(ow_size) {}
...
template <typename T> class Im2ColWriter final : public ColBufWriter<T> {
public:
  Im2ColWriter(const T *src_ptr, T *dst_ptr, int64_t c_size, int64_t id_size,
               int64_t ih_size, int64_t iw_size, int64_t od_size,
               int64_t oh_size, int64_t ow_size)
      : ColBufWriter<T>::ColBufWriter(src_ptr, dst_ptr, c_size, id_size,
                                      ih_size, iw_size, od_size, oh_size,
                                      ow_size) {}
  ~Im2ColWriter() = default;
  void DHWCWrite(int64_t c, int64_t id, int64_t ih, int64_t iw) override {
    *(this->dst_ptr_++) =
        this->src_ptr_[id * this->id_size_ + ih * this->ih_size_ +
                       iw * this->iw_size_ + c];
  }
...
template <typename T> class Col2ImWriter final : public ColBufWriter<T> {
public:
  ...
  void DHWCWrite(int64_t c, int64_t id, int64_t ih, int64_t iw) override {
    this->dst_ptr_[id * this->id_size_ + ih * this->ih_size_ +
                   iw * this->iw_size_ + c] += *(this->src_ptr_++);
  }
  void CDHWWrite(int64_t c, int64_t id, int64_t ih, int64_t iw) override {
...
template <typename T> class ColBufUtil final {
public:
  ColBufUtil(const ShapeView &in_shape, const ShapeView &out_shape,
...
template <typename T> struct ConvKernelUtil final {
public:
...
private:
  static void DoNCDWHFunc(const ShapeView &weight_shape,
  ...
  static void DoNDWHCFunc(const ShapeView &weight_shape,
                          ColBufUtil<T> &col_buf_util,
                          ColBufWriter<T> *col_buf_writer) {
...
CreateConvOpKernelState(user_op::KernelInitContext *ctx,
  ...
  state->in_5d_shape_ = Gen5DShape(
      ctx->TensorDesc4ArgNameAndIndex(in_name, 0)->shape(), state->idx_offset_);
  state->out_5d_shape_ =
      Gen5DShape(ctx->TensorDesc4ArgNameAndIndex(out_name, 0)->shape(),
                 state->idx_offset_);
  state->weight_5d_shape_ =
      Gen5DShape(ctx->TensorDesc4ArgNameAndIndex(weight_name, 0)->shape(),
                 state->idx_offset_);

  auto Gen3DVec =
      [](const std::vector<int32_t> &origin_vec) -> std::vector<int32_t> {
  ...
  state->dilation_rate_3d_ =
      Gen3DVec(ctx->Attr<std::vector<int32_t>>("dilation_rate"));
  state->is_dynamic_ =
      ctx->TensorDesc4ArgNameAndIndex(in_name, 0)->is_dynamic();
...
template <typename T> void InitBiasMulBuf(T *dptr, int64_t num) {
...
public:
  ...
  std::shared_ptr<user_op::OpKernelState>
  CreateOpKernelState(user_op::KernelInitContext *ctx) const {
  ...
  void Compute(user_op::KernelComputeContext *ctx,
    ...
    for (int64_t i = 0; i < in->shape().At(0); ++i) {
      conv_state->im2col_func_(
          GetImgDptr<T>(in, i), ShapeView(conv_state->in_5d_shape_),
      ...
      conv_state->forward_func_(
          ...
          conv_state->out_5d_shape_.Count(idx_offset,
                                          idx_offset + 3), // od * oh * ow
          conv_state->weight_5d_shape_.Count(1),           // ci * kd * kh * kw
          static_cast<T>(1),
          weight->dptr<T>(), col_buf_dptr, static_cast<T>(0),
          GetImgMutDptr<T>(out, i));

      const user_op::Tensor *bias = ctx->Tensor4ArgNameAndIndex("bias", 0);
      if (bias != nullptr) {
        int64_t num_of_col_buf =
            CalcElemNumOfColBuf(out->shape(), weight->shape(), idx_offset);
        int64_t num_of_bias_mul =
            tmp_buffer->shape().elem_cnt() - num_of_col_buf;
        ...
        conv_state->forward_func_(
            ...
            1,                                               // 1
            static_cast<T>(1),
            bias->dptr<T>(), bias_mul_dptr, static_cast<T>(1),
            GetImgMutDptr<T>(out, i));
      }
    }
  }
  ...
  REGISTER_USER_KERNEL(#op_name)                                               \
      ...
      .SetInferTmpSizeFn([](user_op::InferContext *ctx) -> size_t {            \
        ...
        const auto &weight_shape =                                             \
            ctx->TensorDesc4ArgNameAndIndex("weight", 0)->shape();             \
                                                                               \
        int64_t idx_offset = IdxOffset(ctx->Attr<std::string>("data_format")); \
        tmp_buffer_size +=                                                     \
            CalcElemNumOfColBuf(out_shape, weight_shape, idx_offset) *         \
            sizeof(dtype);                                                     \
                                                                               \
        ...
        if (bias != nullptr) {                                                 \
          ...
          for (int i = 0; i < ndims; ++i) {                                    \
            bias_mul_cnt *= out_shape.At(idx_offset + i);                      \
          }                                                                    \
  ...
  void Compute(user_op::KernelComputeContext *ctx,
    ...
    FOR_RANGE(int64_t, i, 0, dy->shape().At(0)) {
      ...
      NewKernelUtil<DeviceType::kCPU>::OFGemm(
          ...
          conv_state->weight_5d_shape_.At(0),              //  filter
          static_cast<T>(1),
          filter->dptr<T>(), GetImgDptr<T>(dy, i), static_cast<T>(0),
          col_buf->mut_dptr<T>());
    ...
    if (ctx->user_op_conf().has_input("_add_to_output", 0)) {
      ...
      KernelUtil<DeviceType::kCPU, T>::Addition(
          ctx->device_ctx(), add_to_output->shape().elem_cnt(),
          dx->mut_dptr<T>(), dx->dptr<T>(), add_to_output->dptr<T>());
    }
  }
};
...
#define REGISTER_CONV_DATA_GRAD_KERNEL(op_name, dtype)                         \
  REGISTER_USER_KERNEL(#op_name)                                               \
      ...
      .SetInferTmpSizeFn([](user_op::InferContext *ctx) -> size_t {            \
        size_t tmp_buffer_size = 0;                                            \
        const auto &out_diff_shape =                                           \
            ctx->TensorDesc4ArgNameAndIndex("dy", 0)->shape();                 \
        const auto &weight_shape =                                             \
            ctx->TensorDesc4ArgNameAndIndex("filter", 0)->shape();             \
                                                                               \
        int64_t idx_offset = IdxOffset(ctx->Attr<std::string>("data_format")); \
        tmp_buffer_size +=                                                     \
            CalcElemNumOfColBuf(out_diff_shape, weight_shape, idx_offset) *    \
            sizeof(dtype);                                                     \
        return tmp_buffer_size;                                                \
      })
...
private:
  void Compute(user_op::KernelComputeContext *ctx,
    ...
    user_op::Tensor *filter_diff =
        ctx->Tensor4ArgNameAndIndex("filter_diff", 0);
    user_op::Tensor *col_buf = ctx->Tensor4ArgNameAndIndex("tmp_buffer", 0);
    ...
    FOR_RANGE(int64_t, i, 0, dy->shape().At(0)) {
      conv_state->im2col_func_(
          GetImgDptr<T>(x, i), ShapeView(conv_state->in_5d_shape_),
          ShapeView(conv_state->weight_5d_shape_),
          ShapeView(conv_state->out_5d_shape_), conv_state->strides_3d_.data(),
          conv_state->dilation_rate_3d_.data(),
      ...
      NewKernelUtil<DeviceType::kCPU>::OFGemm(
          ...
          conv_state->out_5d_shape_.Count(idx_offset,
                                          idx_offset + 3), //  od * oh * ow
          static_cast<T>(1),
          GetImgDptr<T>(dy, i), col_buf->dptr<T>(), static_cast<T>(1),
...
#define REGISTER_CONV_FILTER_GRAD_KERNEL(op_name, dtype)                       \
  REGISTER_USER_KERNEL(#op_name)                                               \
      ...
      .SetInferTmpSizeFn([](user_op::InferContext *ctx) -> size_t {            \
        ...
        int64_t idx_offset = IdxOffset(ctx->Attr<std::string>("data_format")); \
        tmp_buffer_size +=                                                     \
            CalcElemNumOfColBuf(out_diff_shape, weight_diff_shape,             \
                                idx_offset) *                                  \
            sizeof(dtype);                                                     \
        return tmp_buffer_size;                                                \
      })
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    user_op::Tensor *bias_mul_buf =
        ctx->Tensor4ArgNameAndIndex("tmp_buffer", 0);

    InitBiasMulBuf(bias_mul_buf->mut_dptr<T>(),
                   bias_mul_buf->shape().elem_cnt());
...
#define REGISTER_CONV_BIAS_GRAD_KERNEL(op_name, dtype)                         \
  REGISTER_USER_KERNEL(#op_name)                                               \
      ...
      .SetInferTmpSizeFn([](user_op::InferContext *ctx) -> size_t {            \
        ...
        int64_t bias_mul_cnt = 1;                                              \
        for (int i = 0; i < ndims; ++i) {                                      \
          bias_mul_cnt *= out_diff_shape.At(idx_offset + i);                   \
        }                                                                      \
