...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    const std::string &parallel_conf_txt =
        ctx->Attr<std::string>("parallel_conf");
    ...
    FOR_RANGE(int64_t, parallel_id, 0, parallel_desc.parallel_num()) {
      device_set.emplace(
          std::make_pair(parallel_desc.MachineIdForParallelId(parallel_id),
                         parallel_desc.DeviceIdForParallelId(parallel_id)));
    }
    ncclComm_t comm = CHECK_NOTNULL(Global<EagerNcclCommMgr>::Get())
                          ->GetCommForDevice(device_set);
    OF_NCCL_CHECK(ncclAllReduce(in->dptr(), out->mut_dptr(),
                                in->shape().elem_cnt(),
