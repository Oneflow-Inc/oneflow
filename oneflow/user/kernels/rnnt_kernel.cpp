...
#include "oneflow/user/kernels/rnnt_kernel_cpu.h"
...
class RNNTKernel final : public user_op::OpKernel {
 public:
  RNNTKernel() = default;
...
user_op::InferTmpSizeFn GenFwInferTmpSizeFn() {
  return [](user_op::InferContext* ctx) {
    ...
    return GetCudaAlignedSize(sizeof(T) * minibatch_size * maxT * maxU * 4);
  };
}


#define REGISTER_RNNT_KERNEL(device, dtype)                                          \
