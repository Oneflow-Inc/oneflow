...
inline hipblasOperation_t GetHipblasOp(char op) {
  switch (op) {
    case 'n':
    case 'N': {
      return HIPBLAS_OP_N;
...
template<typename T>
struct HipDataTypeTrait;

template<>
struct HipDataTypeTrait<float> {
  const static hipblasDatatype_t value = HIPBLAS_R_32F;
};

template<>
struct HipDataTypeTrait<half> {
  const static hipblasDatatype_t value = HIPBLAS_R_16F;
};

template<typename T>
void HipblasBatchGemm(hipblasHandle_t handle, char transa, char transb, int64_t m, int64_t n,
                     ...
                     int64_t batch_size) {
  hipblasOperation_t opa = GetHipblasOp(transa);
  hipblasOperation_t opb = GetHipblasOp(transb);
//   if (CUDA_VERSION >= 9010 && GetCudaSmVersion() >= 500) {
// #if CUDA_VERSION >= 9010
    hipblasDatatype_t data_type = HipDataTypeTrait<T>::value;
...
//   using comp_t = float;
//   cublasOperation_t opa = GetHipblasOp(transa);
//   cublasOperation_t opb = GetHipblasOp(transb);
...
// #endif
//     cudaDataType_t data_type = HipDataTypeTrait<half>::value;
//     cudaDataType_t comp_type = HipDataTypeTrait<comp_t>::value;
...
void BatchedGemm(DeviceCtx* ctx, char opa, char opb, int64_t m, int64_t n, int64_t k, float alpha,
  ...
  HipblasBatchGemm<T>(ctx->hipblas_pmh_handle(), opb, opa, n, m, k, static_cast<T>(alpha), b, ldb,
