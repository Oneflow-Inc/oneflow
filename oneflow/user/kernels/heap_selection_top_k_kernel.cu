...
template <typename T> T PowOf2Floor(T val, int32_t max_power) {
...
template <typename T> T PowOf2Ceil(T val, int32_t max_power) {
  CHECK_GT(val, GetZeroVal<T>());
...
__device__ void BitonicSwap(T *data, const int32_t i, const int32_t j,
                            const bool dir, const Compare &comp) {
...
__device__ void BitonicSort(T *data, const int32_t elem_cnt,
  ...
  for (int32_t size = 2; size <= elem_cnt / 2; size *= 2) {
    ...
    for (int32_t stride = size / 2; stride > 0; stride /= 2) {
      for (int32_t swap_id = threadIdx.x; swap_id < elem_cnt / 2;
           swap_id += blockDim.x) {
...
template <typename T> class Entry final {
public:
  __device__ __forceinline__ Entry(int32_t index, T value)
...
__global__ void HeapTopKKernel(const T *in_ptr, const int32_t instance_num,
  ...
  // heap_size entries from
  // corresponding set
  const T *input = in_ptr + blockIdx.x * instance_size;
  auto heap = MinHeap<T>(shared_entries + threadIdx.x * heap_size, heap_size,
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    // We also need heap_size * num_heap to be pow-of-2 which is necessary for
    // bitonic sort
    const int32_t heap_size = PowOf2Ceil(k, 16);
    int32_t num_heap = PowOf2Floor(
        kCudaMaxSharedMemoryByteSize / (heap_size * sizeof(Entry<T>)), 16);
    // Limitation 2: # of threads in thread block
    num_heap = std::min(num_heap, kCudaThreadsNumPerBlock);

    HeapTopKKernel<
        T><<<instance_num, num_heap, num_heap * heap_size * sizeof(Entry<T>),
             ctx->device_ctx()->cuda_stream()>>>(
        in->dptr<T>(), instance_num, instance_size, k, heap_size,
        GetMaxVal<int32_t>(), GetMinVal<T>(), out->mut_dptr<int32_t>());
  }
...
#define REGISTER_GPU_HEAP_SELECTION_TOP_K_KERNEL(dtype)                        \
  REGISTER_USER_KERNEL("top_k")                                                \
      .SetCreateFn<GpuHeapSelectionTopKKernel<dtype>>()                        \
      .SetIsMatchedHob(                                                        \
