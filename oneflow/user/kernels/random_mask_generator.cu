...
__global__ void GenerateGpu(curandState *state, const int64_t n,
...
#pragma unroll
    for (int j = 0; j < sizeof(PackType); ++j) {
      pack.b_value[j] = GenMask(&localState, rate);
    }
  ...
  if (id < rem_cnt) {
    mask[id + rem_offset] = GenMask(&localState, rate);
  }
  state[id] = localState;
}
...
RandomMaskGenerator<DeviceType::kGPU>::RandomMaskGenerator(int64_t seed) {
  ...
  OF_CUDA_CHECK(cudaMalloc(&curand_states_,
                           block_num_ * thread_num_ * sizeof(curandState)));
...
void RandomMaskGenerator<DeviceType::kGPU>::Generate(DeviceCtx *device_ctx,
                                                     const int64_t n,
                                                     const float rate,
                                                     int8_t *mask) {
  const int32_t elem_cnt_per_block =
      thread_num_ * sizeof(PackType) * kMinPackPerThread;
  const int32_t block_num = std::min(
      static_cast<int32_t>((n + elem_cnt_per_block - 1) / elem_cnt_per_block),
      block_num_);
  GenerateGpu<<<block_num, thread_num_, 0, device_ctx->cuda_stream()>>>(
      curand_states_, n, rate, mask);
}
