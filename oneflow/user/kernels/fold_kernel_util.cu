...
__global__ void CudaFoldForward(FoldParams<INDEX_T, NDIM, SDIM> params, const T* input_ptr, T* output_ptr) {
...
struct FoldKernelUtil<DeviceType::kGPU, T, INDEX_T, NDIM, SDIM> {
  ...
  static void Forward(DeviceCtx* ctx, const void* raw_params, const T* input_ptr, T* output_ptr) {
    ...
    CudaFoldForward<T, INDEX_T, NDIM, SDIM>
        <<<GetNumBlocks(fold_params->in_elem_cnt), kBlockSize, 0, ctx->cuda_stream()>>>(*fold_params, input_ptr, output_ptr);
  }
};
