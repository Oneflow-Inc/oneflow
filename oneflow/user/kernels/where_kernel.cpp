...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    WhereKernelUtil<device_type, T, CondT>::Where(
        ctx->device_ctx(), out->shape().elem_cnt(), cond->dptr<CondT>(),
        x->dptr<T>(), y->dptr<T>(), out->mut_dptr<T>());
  }
...
#define REGISTER_WHERE_KERNEL(device_type_v, dtype_pair, ctype_pair)           \
  REGISTER_USER_KERNEL("where")                                                \
      ...
      .SetIsMatchedHob(                                                        \
          ...
          (user_op::HobDataType("condition", 0) ==                             \
           OF_PP_PAIR_SECOND(ctype_pair)) &                                    \
...
#ifdef WITH_CUDA
OF_PP_SEQ_PRODUCT_FOR_EACH_TUPLE(REGISTER_WHERE_KERNEL, (DeviceType::kGPU),
