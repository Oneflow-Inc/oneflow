...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    NdarrayUtil<device, T>::BroadcastDiv(
        ctx->device_ctx(), tmp,
        XpuVarNdarray<const T>(z_tensor->shape(), z_tensor->dptr<T>(),
                               num_axes),
    ...
    NdarrayUtil<device, T>::ReduceSum(
        ctx->device_ctx(), XpuVarNdarray<T>(dy_tensor->shape(),
                                            dy_tensor->mut_dptr<T>(), num_axes),
        const_tmp, tmp);
    NdarrayUtil<device, T>::InplaceNegative(
        ctx->device_ctx(),
        XpuVarNdarray<T>(dy_tensor->shape(), dy_tensor->mut_dptr<T>(),
                         num_axes));
  };
...
#define REGISTER_BROADCAST_DIV_GRAD_KERNEL(device, dtype_pair)                 \
  REGISTER_USER_KERNEL("broadcast_div_grad")                                   \
      .SetCreateFn<                                                            \
          BroadcastDivGradKernel<device, OF_PP_PAIR_FIRST(dtype_pair)>>()      \
      .SetIsMatchedHob(                                                        \
