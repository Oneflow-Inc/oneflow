...
class DynamicLossScaleScheduleCpuKernel final : public user_op::OpKernel {
 ...
 private:
  void Compute(user_op::KernelComputeContext* ctx) const override {
    const int64_t* count_not_finite =
        ctx->Tensor4ArgNameAndIndex("count_not_finite", 0)->dptr<int64_t>();
    float* loss_scale = ctx->Tensor4ArgNameAndIndex("loss_scale", 0)->mut_dptr<float>();
    int64_t* good_step_counter =
        ctx->Tensor4ArgNameAndIndex("good_step_counter", 0)->mut_dptr<int64_t>();
    const int64_t increment_period = ctx->Attr<int64_t>("increment_period");
    const float multiplier = ctx->Attr<float>("multiplier");
    if (*count_not_finite == 0) {
      int64_t cur_good_step_counter = *good_step_counter + 1;
      if (cur_good_step_counter >= increment_period) {
        const float old_loss_scale = *loss_scale;
        const float new_loss_scale = std::min(old_loss_scale * multiplier, kLossScaleMax);
      ...
      *good_step_counter = 0;
      const float old_loss_scale = *loss_scale;
      const float new_loss_scale = std::max(old_loss_scale / multiplier, kLossScaleMin);
      *loss_scale = new_loss_scale;
      LOG(INFO) << "There are Nan or Inf in model_diffs, so we decrease loss_scale from"
                << old_loss_scale << " to " << new_loss_scale;
    }
  }
