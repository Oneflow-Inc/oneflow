...
template<typename T, int pack_size>
__global__ void batch_reshape_for_qkv(const int n, const T* query, const T* key, const T* value, T* new_query, T* new_key, T* new_value){
 ...
 private:
  ...
  void Compute(user_op::KernelComputeContext* ctx) const override {
    ...
    if(n % 4 == 0){
      ...
      RUN_CUDA_KERNEL((batch_reshape_for_qkv<T, 1>), ctx->stream(), n, n,
                    query->dptr<T>(), key->dptr<T>(), value->dptr<T>(), new_query->mut_dptr<T>(),
                    new_key->mut_dptr<T>(), new_value->mut_dptr<T>());
    }
