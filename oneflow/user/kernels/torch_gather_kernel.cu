 ...
 private:
  void Compute(KernelComputeContext* ctx) const override {
    ...
    RUN_CUDA_KERNEL((DoCUDAGatherDim<IN_T, IDX_T>), 
                    ctx->device_ctx(), 
                    20, // TODO: how to choose thread num?
                    input_nd_helper,
                    index_nd_helper,
...
class ScatterDimAddGpuKernel final : public user_op::OpKernel {
 ...
 private:
  void Compute(KernelComputeContext* ctx) const override {
    ...
    RUN_CUDA_KERNEL((DoCUDAScatterDimAdd<IN_T, IDX_T>), 
                  ctx->device_ctx(), 
                  20, // TODO: how to choose thread num?
                  src_nd_helper,
                  output_nd_helper,
                  src_tensor->shape().elem_cnt(),
                  dim,
                  index,
                  src,
                  output);
...
#define REGISTER_SCATTERDIMADD_GPUKERNEL(device, in_type, indices_type)                              \
  REGISTER_USER_KERNEL("scatter_dim_add")                                                             \
      .SetCreateFn<                                                                                \
      ScatterDimAddGpuKernel<device, OF_PP_PAIR_FIRST(in_type), OF_PP_PAIR_FIRST(indices_type)>>() \
      .SetIsMatchedHob((user_op::HobDeviceTag() == device)                                         \
...
OF_PP_SEQ_PRODUCT_FOR_EACH_TUPLE(REGISTER_GATHERDIM_GPUKERNEL,
                                (DeviceType::kGPU), 
                                GATHER_DATA_TYPE_SEQ,
                                INDEX_DATA_TYPE_SEQ)

OF_PP_SEQ_PRODUCT_FOR_EACH_TUPLE(REGISTER_SCATTERDIMADD_GPUKERNEL,
                                (DeviceType::kGPU), 
                                GATHER_DATA_TYPE_SEQ,
                                INDEX_DATA_TYPE_SEQ)
