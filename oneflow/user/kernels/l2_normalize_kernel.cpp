...
template <typename T>
static void L2NormalizeForward(const int32_t n, const int32_t c,
                               const int32_t d, const T epsilon, const T *in,
                               T *square_x_sum, T *out) {
...
template <typename T>
static void L2NormalizeBackward(const int32_t n, const int32_t c,
                                const int32_t d, const T epsilon, const T *out,
                                const T *out_diff, const T *square_x_sum,
                                T *in_diff) {
  for (int32_t i = 0; i < n; i++) {
    const T norm = std::sqrt(std::max(square_x_sum[i], epsilon));
    const int32_t offset = (i / d) * d * c + (i % d);
    if (square_x_sum[i] >= epsilon) {
      ...
      for (int32_t j = 0; j < c; j++) {
        const int32_t index = offset + j * d;
        in_diff[index] =
            (1 / norm) * (out_diff[index] - y_dy_inner_prod * out[index]);
      }
    } else {
      for (int32_t j = 0; j < c; j++) {
        const int32_t index = offset + j * d;
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    user_op::Tensor *square_x_sum =
        ctx->Tensor4ArgNameAndIndex("square_x_sum", 0);
    ...
    size_t square_x_sum_byte_size =
        square_x_sum->shape().elem_cnt() * sizeof(T);
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    const user_op::Tensor *square_x_sum =
        ctx->Tensor4ArgNameAndIndex("square_x_sum", 0);
    ...
    L2NormalizeBackward<T>(n, c, d, static_cast<T>(epsilon), y->dptr<T>(),
                           dy->dptr<T>(), square_x_sum->dptr<T>(),
                           dx->mut_dptr<T>());
  }
  bool AlwaysComputeWhenAllOutputsEmpty() const override { return false; }
};
