...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    binary_func(ctx->device_ctx(),
                XpuVarNdarray<K>(tensor_z->shape(), dptr_z, num_axes),
...
#define REGISTER_MATH_BINARY_BROADCAST_LOGICAL_KERNEL(math_type_pair, device,  \
                                                      data_type_pair)          \
  REGISTER_USER_KERNEL(OF_PP_PAIR_FIRST(math_type_pair))                       \
      .SetCreateFn<MathBinaryBroadcastKernel<                                  \
