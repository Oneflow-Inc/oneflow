...
public:
  NormalizeAttr(user_op::KernelInitContext *ctx) {
    ...
    if (mean_vec.size() == 1) {
      for (int i = 0; i < 3; ++i) {
        mean_.val[i] = mean_vec.at(0);
      }
    } else if (mean_vec.size() == 3) {
      for (int i = 0; i < 3; ++i) {
        mean_.val[i] = mean_vec.at(i);
      }
    } else {
      UNIMPLEMENTED();
    }
    ...
    if (std_vec.size() == 1) {
      for (int i = 0; i < 3; ++i) {
        inv_std_.val[i] = 1.0f / std_vec.at(0);
      }
    } else if (std_vec.size() == 3) {
      for (int i = 0; i < 3; ++i) {
        inv_std_.val[i] = 1.0f / std_vec.at(i);
      }
    } else {
      UNIMPLEMENTED();
    }
...
OutIdx2InIdx(int32_t *out_idx, int32_t *in_idx, const int8_t *mirror_dptr,
             int32_t out_W, int32_t H_offset, int32_t W_offset);
template <>
__device__ __forceinline__ void
OutIdx2InIdx<TensorLayout::kNCHW>(int32_t *out_idx, int32_t *in_idx,
                                  const int8_t *mirror_dptr, int32_t out_W,
                                  int32_t H_offset, int32_t W_offset) {
  if (mirror_dptr && mirror_dptr[out_idx[0]]) {
    out_idx[3] = out_W - 1 - out_idx[3];
  }
...
template <>
__device__ __forceinline__ void
OutIdx2InIdx<TensorLayout::kNHWC>(int32_t *out_idx, int32_t *in_idx,
                                  const int8_t *mirror_dptr, int32_t out_W,
                                  int32_t H_offset, int32_t W_offset) {
  if (mirror_dptr && mirror_dptr[out_idx[0]]) {
    out_idx[2] = out_W - 1 - out_idx[2];
  }
  in_idx[0] = out_idx[0];            // N
...
template <TensorLayout layout>
__global__ void CropMirrorNormalizeGpuImpl(
    int32_t elem_cnt, const uint8_t *in_dptr, float *out_dptr,
    const int8_t *mirror_dptr, int32_t out_W,
    const NdIndexOffsetHelper<int32_t, 4> in_helper,
    const NdIndexOffsetHelper<int32_t, 4> out_helper, int32_t H_offset,
    int32_t W_offset, const NormalizeVal mean, const NormalizeVal inv_std) {
  CUDA_1D_KERNEL_LOOP(out_offset, elem_cnt) {
    int32_t in_idx[4];
    int32_t out_idx[4];
    out_helper.OffsetToNdIndex(out_offset, out_idx);
    OutIdx2InIdx<layout>(out_idx, in_idx, mirror_dptr, out_W, H_offset,
                         W_offset);
    float mean_val;
    float inv_std_val;
    const int32_t c = in_idx[3];
    // When the compiler can’t resolve array indices to constants it will put
    // private arrays into
    // GPU local memory. Using local memory is slower than keeping array
    ...
    out_dptr[out_offset] =
        (static_cast<float>(in_dptr[in_offset]) - mean_val) * inv_std_val;
  }
}
...
private:
  void Compute(user_op::KernelComputeContext *ctx,
    ...
    if (mirror_blob) {
      mirror_dptr = mirror_blob->dptr<int8_t>();
    }
    ...
    if (output_layout == "NCHW") {
      ...
      CropMirrorNormalizeGpuImpl<TensorLayout::kNCHW><<<
          ...
          elem_cnt, in_dptr, out_dptr, mirror_dptr, out_W, in_helper,
          out_helper, H_offset, W_offset, mean, inv_std);
