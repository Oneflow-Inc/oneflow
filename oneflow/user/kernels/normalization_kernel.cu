...
void InferDimSizeAndDataFormat(const ShapeView &x_shape, const int32_t axis,
...
void InferXYCudnnTensorDesc(const ShapeView &xy_shape,
                            const DataType &data_type, const int32_t axis,
                            cudnnTensorDescriptor_t xy_desc) {
  int32_t n, c, h, w;
  cudnnTensorFormat_t format;
  InferDimSizeAndDataFormat(xy_shape, axis, &n, &c, &h, &w, &format);
  OF_CUDNN_CHECK(cudnnSetTensor4dDescriptor(
      xy_desc, format, GetCudnnDataType(data_type), n, c, h, w));
}

void InferParamCudnnTensorDesc(const cudnnTensorDescriptor_t xy_desc,
...
public:
  ...
  CudnnTensorDescHelper(const ShapeView &xy_shape, const DataType &data_type,
                        const int32_t axis, cudnnBatchNormMode_t mode) {
    ...
    OF_CUDNN_CHECK(cudnnGetTensor4dDescriptor(param_desc_, &param_data_type_,
                                              &n, &c, &h, &w, &n_stride,
...
size_t InferTrainWorkspaceSize(const ShapeView &x_shape,
                               const DataType data_type, const int32_t axis) {
  ...
  OF_CUDNN_CHECK(cudnnGetBatchNormalizationForwardTrainingExWorkspaceSize(
      handle, CUDNN_BATCHNORM_SPATIAL_PERSISTENT, CUDNN_BATCHNORM_OPS_BN,
      desc_helper.xy_desc(), nullptr, desc_helper.xy_desc(),
...
size_t InferGradWorkspaceSize(const ShapeView &x_shape,
                              const DataType data_type, const int32_t axis) {
#if defined(BN_ENABLE_EX_API)
  const CudnnTensorDescHelper desc_helper(x_shape, data_type, axis,
                                          CUDNN_BATCHNORM_SPATIAL_PERSISTENT);
  ...
  OF_CUDNN_CHECK(cudnnGetBatchNormalizationBackwardExWorkspaceSize(
      handle, CUDNN_BATCHNORM_SPATIAL_PERSISTENT, CUDNN_BATCHNORM_OPS_BN,
      desc_helper.xy_desc(), nullptr, desc_helper.xy_desc(), nullptr,
      desc_helper.xy_desc(), desc_helper.param_desc(), nullptr,
      &size_in_bytes));
  OF_CUDNN_CHECK(cudnnDestroy(handle));
  return std::max(size_in_bytes, static_cast<size_t>(1));
#else
...
size_t InferGradTmpSize(user_op::InferContext *ctx) {
  ...
  if (ctx->user_op_conf().op_type_name() == "normalization_add_relu_grad" &&
    ...
    tmp_size += GetCudaAlignedSize(dy->shape().elem_cnt() *
                                   GetSizeOfDataType(dy->data_type()));
  }
  tmp_size += GetCudaAlignedSize(
      InferGradWorkspaceSize(dy->shape(), dy->data_type(), axis));
  return tmp_size;
}
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    if (ctx->user_op_conf().has_input("_add_to_output", 0)) {
      ...
      Memcpy<DeviceType::kGPU>(
          ...
          add_to_output->shape().elem_cnt() *
              GetSizeOfDataType(add_to_output->data_type()));
    ...
    OF_CUDNN_CHECK(cudnnBatchNormalizationForwardInference(
        ctx->device_ctx()->cudnn_handle(), CUDNN_BATCHNORM_SPATIAL, sp_alpha,
        sp_beta, desc_helper.xy_desc(), x->dptr(), desc_helper.xy_desc(),
        y->mut_dptr(), desc_helper.param_desc(), gamma->dptr(), beta->dptr(),
        moving_mean->dptr(), moving_variance->dptr(), epsilon));
  }
  ...
  REGISTER_USER_KERNEL("normalization")                                        \
      ...
      .SetInplaceProposalFn(                                                   \
          [](const user_op::InferContext &ctx,                                 \
            ...
            if (ctx.user_op_conf().has_input("_add_to_output", 0)) {           \
              OF_RETURN_IF_ERROR(                                              \
                  AddInplaceArgPairFn("y", 0, "_add_to_output", 0, true));     \
            }                                                                  \
...
template <>
__global__ void AddReluGpu<half>(int64_t n, const half *x, const half *addend,
                                 half *y) {
...
template <typename T>
void AddRelu(DeviceCtx *device_ctx, int64_t n, const T *x, const T *addend,
             T *y) {
  AddReluGpu<T><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock, 0,
                  device_ctx->cuda_stream()>>>(n, x, addend, y);
}

template <>
void AddRelu<float16>(DeviceCtx *device_ctx, int64_t n, const float16 *x,
                      const float16 *addend, float16 *y) {
  AddRelu<half>(device_ctx, n, reinterpret_cast<const half *>(x),
                reinterpret_cast<const half *>(addend),
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    OF_CUDNN_CHECK(cudnnGetBatchNormalizationForwardTrainingExWorkspaceSize(
        ...
        CUDNN_BATCHNORM_OPS_BN, desc_helper.xy_desc(), nullptr,
        desc_helper.xy_desc(), desc_helper.param_desc(), nullptr,
        &workspace_size));
    size_t reserve_space_size;
    OF_CUDNN_CHECK(cudnnGetBatchNormalizationTrainingExReserveSpaceSize(
        ctx->device_ctx()->cudnn_handle(), CUDNN_BATCHNORM_SPATIAL_PERSISTENT,
        CUDNN_BATCHNORM_OPS_BN, nullptr, desc_helper.xy_desc(),
        &reserve_space_size));
    ...
    if (reserve_space_size == 0 &&
      ...
      OF_CUDNN_CHECK(cudnnBatchNormalizationForwardTrainingEx(
          ctx->device_ctx()->cudnn_handle(), CUDNN_BATCHNORM_SPATIAL_PERSISTENT,
          CUDNN_BATCHNORM_OPS_BN, sp_alpha, sp_beta, desc_helper.xy_desc(),
          x->dptr(), nullptr, nullptr, desc_helper.xy_desc(), y->mut_dptr(),
          desc_helper.param_desc(), gamma->dptr(), beta->dptr(), 1.0 - momentum,
          moving_mean->mut_dptr(), moving_variance->mut_dptr(), epsilon,
          mean->mut_dptr(), inv_variance->mut_dptr(), nullptr,
          workspace->mut_dptr(), workspace->shape().elem_cnt(), nullptr, 0));
    } else {
      OF_CUDNN_CHECK(cudnnBatchNormalizationForwardTraining(
          ctx->device_ctx()->cudnn_handle(), CUDNN_BATCHNORM_SPATIAL_PERSISTENT,
          sp_alpha, sp_beta, desc_helper.xy_desc(), x->dptr(),
          desc_helper.xy_desc(), y->mut_dptr(), desc_helper.param_desc(),
          gamma->dptr(), beta->dptr(), 1.0 - momentum, moving_mean->mut_dptr(),
          moving_variance->mut_dptr(), epsilon, mean->mut_dptr(),
    ...
    if (ctx->user_op_conf().op_type_name() == "normalization_add_relu") {
      ...
      if (ctx->user_op_conf().has_input("addend", 0)) {
        ...
        AddRelu(ctx->device_ctx(), elem_cnt, y->dptr<T>(), addend->dptr<T>(),
                y->mut_dptr<T>());
      } else {
        NewKernelUtil<DeviceType::kGPU>::Relu(ctx->device_ctx(), elem_cnt,
                                              y->dptr<T>(), y->mut_dptr<T>());
      }
    }
  }
  ...
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    if (ctx->user_op_conf().op_type_name() == "normalization_grad") {
      ...
      if (ctx->user_op_conf().has_output("addend_diff", 0)) {
        ...
        const size_t relu_dx_size = GetCudaAlignedSize(
    ...
    if (reserve_space_size == 0 && workspace_size <= bn_workspace_size) {
      OF_CUDNN_CHECK(cudnnBatchNormalizationBackwardEx(
          ctx->device_ctx()->cudnn_handle(), CUDNN_BATCHNORM_SPATIAL_PERSISTENT,
          CUDNN_BATCHNORM_OPS_BN, CudnnSPOnePtr<T>(), CudnnSPZeroPtr<T>(),
          CudnnSPOnePtr<T>(), CudnnSPZeroPtr<T>(), desc_helper.xy_desc(),
          x->dptr(), nullptr, nullptr, desc_helper.xy_desc(), bn_dy_ptr,
          nullptr, nullptr, desc_helper.xy_desc(), dx->mut_dptr(),
          desc_helper.param_desc(), gamma->dptr(), nullptr,
          gamma_diff->mut_dptr(), beta_diff->mut_dptr(), epsilon, mean->dptr(),
          inv_variance->dptr(), nullptr, bn_workspace_ptr, bn_workspace_size,
          nullptr, 0));
    } else {
      OF_CUDNN_CHECK(cudnnBatchNormalizationBackward(
          ctx->device_ctx()->cudnn_handle(), CUDNN_BATCHNORM_SPATIAL_PERSISTENT,
          CudnnSPOnePtr<T>(), CudnnSPZeroPtr<T>(), CudnnSPOnePtr<T>(),
          CudnnSPZeroPtr<T>(), desc_helper.xy_desc(), x->dptr(),
          desc_helper.xy_desc(), bn_dy_ptr, desc_helper.xy_desc(),
          dx->mut_dptr(), desc_helper.param_desc(), gamma->dptr(),
          gamma_diff->mut_dptr(), beta_diff->mut_dptr(), epsilon, mean->dptr(),
          inv_variance->dptr()));
    }
#else
    OF_CUDNN_CHECK(cudnnBatchNormalizationBackward(
        ...
        CudnnSPOnePtr<T>(), CudnnSPZeroPtr<T>(), CudnnSPOnePtr<T>(),
        CudnnSPZeroPtr<T>(), desc_helper.xy_desc(), x->dptr(),
        desc_helper.xy_desc(), bn_dy_ptr, desc_helper.xy_desc(), dx->mut_dptr(),
        desc_helper.param_desc(), gamma->dptr(), gamma_diff->mut_dptr(),
...
size_t InferFusedNormalizationAddReluTmpSize(user_op::InferContext *ctx) {
  ...
  cudnnHandle_t handle;
  OF_CUDNN_CHECK(cudnnCreate(&handle));
  CudnnActivationDesc activation_desc(CUDNN_ACTIVATION_RELU,
  ...
  OF_CUDNN_CHECK(cudnnGetBatchNormalizationForwardTrainingExWorkspaceSize(
      handle, CUDNN_BATCHNORM_SPATIAL_PERSISTENT, ops, desc_helper.xy_desc(),
      z_desc, desc_helper.xy_desc(), desc_helper.param_desc(),
...
size_t InferFusedNormalizationAddReluGradTmpSize(user_op::InferContext *ctx) {
  ...
  OF_CUDNN_CHECK(cudnnGetBatchNormalizationBackwardExWorkspaceSize(
      handle, CUDNN_BATCHNORM_SPATIAL_PERSISTENT, ops, desc_helper.xy_desc(),
      desc_helper.xy_desc(), desc_helper.xy_desc(), z_desc,
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    OF_CUDNN_CHECK(cudnnGetBatchNormalizationForwardTrainingExWorkspaceSize(
        ctx->device_ctx()->cudnn_handle(), CUDNN_BATCHNORM_SPATIAL_PERSISTENT,
        ops, desc_helper.xy_desc(), z_desc, desc_helper.xy_desc(),
        desc_helper.param_desc(), activation_desc.Get(), &min_workspace_size));
    ...
    OF_CUDNN_CHECK(cudnnGetBatchNormalizationTrainingExReserveSpaceSize(
        ctx->device_ctx()->cudnn_handle(), CUDNN_BATCHNORM_SPATIAL_PERSISTENT,
        ops, activation_desc.Get(), desc_helper.xy_desc(),
    ...
    OF_CUDNN_CHECK(cudnnBatchNormalizationForwardTrainingEx(
        ctx->device_ctx()->cudnn_handle(), CUDNN_BATCHNORM_SPATIAL_PERSISTENT,
        ops, CudnnSPOnePtr<T>(), CudnnSPZeroPtr<T>(), desc_helper.xy_desc(),
        x->dptr(), z_desc, z_ptr, desc_helper.xy_desc(), y->mut_dptr(),
        desc_helper.param_desc(), gamma->dptr(), beta->dptr(), 1.0 - momentum,
        moving_mean->mut_dptr(), moving_variance->mut_dptr(), epsilon,
        mean->mut_dptr(), inv_variance->mut_dptr(), activation_desc.Get(),
        tmp_buffer->mut_dptr(), workspace_size, reserve_space->mut_dptr(),
        reserve_space_size));
  }
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    OF_CUDNN_CHECK(cudnnGetBatchNormalizationBackwardExWorkspaceSize(
        ctx->device_ctx()->cudnn_handle(), CUDNN_BATCHNORM_SPATIAL_PERSISTENT,
        ops, desc_helper.xy_desc(), desc_helper.xy_desc(),
        desc_helper.xy_desc(), dz_desc, desc_helper.xy_desc(),
        desc_helper.param_desc(), activation_desc.Get(), &min_workspace_size));
    const size_t workspace_size = tmp_buffer->shape().elem_cnt();
    CHECK_GE(workspace_size, min_workspace_size);
    size_t min_reserve_space_size;
    ...
    OF_CUDNN_CHECK(cudnnBatchNormalizationBackwardEx(
        ctx->device_ctx()->cudnn_handle(), CUDNN_BATCHNORM_SPATIAL_PERSISTENT,
        ops, CudnnSPOnePtr<T>(), CudnnSPZeroPtr<T>(), CudnnSPOnePtr<T>(),
        CudnnSPZeroPtr<T>(), desc_helper.xy_desc(), x->dptr(),
        desc_helper.xy_desc(), y->dptr(), desc_helper.xy_desc(), dy->dptr(),
        dz_desc, dz_ptr, desc_helper.xy_desc(), dx->mut_dptr(),
        desc_helper.param_desc(), gamma->dptr(), beta->dptr(),
        gamma_diff->mut_dptr(), beta_diff->mut_dptr(), epsilon, mean->dptr(),
        inv_variance->dptr(), activation_desc.Get(), tmp_buffer->mut_dptr(),
        workspace_size, const_cast<void *>(reserve_space->dptr()),
        reserve_space_size));
  }
