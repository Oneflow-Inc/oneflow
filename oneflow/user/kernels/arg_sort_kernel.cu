...
public:
  ...
  TmpBufferManager(int32_t capacity, void *ptr, const ShapeView &in_shape)
    ...
    const int32_t sorted_in_aligned_bytes =
        GetCudaAlignedSize(sorted_in_elem_cnt_ * sizeof(T));
    ...
    temp_storage_bytes_ =
        capacity_ - sorted_in_aligned_bytes - indices_aligned_bytes;
...
template <typename T> class GpuArgSortKernel final : public user_op::OpKernel {
public:
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    TmpBufferManager<T> buf_manager(
        static_cast<int32_t>(tmp_buffer->shape().elem_cnt()),
    ...
    InitializeIndices<<<BlocksNum4ThreadsNum(elem_cnt), kCudaThreadsNumPerBlock,
                        0, ctx->device_ctx()->cuda_stream()>>>(
        elem_cnt, buf_manager.IndicesPtr(), instance_size);
    if (direction == "ASCENDING") {
      SortPairsAscending(in->dptr<T>(), buf_manager.IndicesPtr(), instance_num,
                         instance_size, buf_manager.TempStoragePtr(),
                         buf_manager.TempStorageBytes(),
                         buf_manager.SortedInPtr(), out->mut_dptr<int32_t>(),
                         ctx->device_ctx()->cuda_stream());
    } else if (direction == "DESCENDING") {
      SortPairsDescending(in->dptr<T>(), buf_manager.IndicesPtr(), instance_num,
                          instance_size, buf_manager.TempStoragePtr(),
                          buf_manager.TempStorageBytes(),
                          buf_manager.SortedInPtr(), out->mut_dptr<int32_t>(),
                          ctx->device_ctx()->cuda_stream());
...
#define REGISTER_GPU_ARG_SORT_KERNEL(dtype)                                    \
  REGISTER_USER_KERNEL("arg_sort")                                             \
      ...
      .SetInferTmpSizeFn([](user_op::InferContext *ctx) {                      \
        ...
        /* Sorted In */                                                        \
        const int32_t sorted_in_aligned_bytes =                                \
            GetCudaAlignedSize(elem_cnt * sizeof(dtype));                      \
        /* Indices */                                                          \
        const int32_t indices_aligned_bytes =                                  \
            GetCudaAlignedSize(elem_cnt * sizeof(int32_t));                    \
        /* CUB Temp Storage */                                                 \
        int32_t temp_storage_bytes = -1;                                       \
        const std::string &direction = ctx->Attr<std::string>("direction");    \
        if (direction == "ASCENDING") {                                        \
          temp_storage_bytes =                                                 \
              InferTempStorageForSortPairsAscending<dtype, int32_t>(           \
