...
class BroadcastMinimumBackward final : public user_op::OpKernel {
 public:
  BroadcastMinimumBackward() = default;
  ~BroadcastMinimumBackward() = default;
 ...
 private:
  void Compute(user_op::KernelComputeContext* ctx) const override {
    ...
    MinimumBackwardFunctor<device_type, T>()(ctx->device_ctx(), tensor_dz->shape().elem_cnt(),
                                             dptr_dz, dptr_x, dptr_y, dptr_dx, dptr_dy);
  }
  bool AlwaysComputeWhenAllOutputsEmpty() const override { return false; }
};

#define REGISTER_BW_MINIMUM_KERNEL(device, dtype)             \
  REGISTER_USER_KERNEL("broadcast_minimum_backward")          \
      .SetCreateFn<BroadcastMinimumBackward<device, dtype>>() \
      .SetIsMatchedHob((user_op::HobDeviceTag() == device)    \
                       & (user_op::HobDataType("dz", 0) == GetDataType<dtype>::value));

REGISTER_BW_MINIMUM_KERNEL(DeviceType::kCPU, float);
REGISTER_BW_MINIMUM_KERNEL(DeviceType::kCPU, double);
REGISTER_BW_MINIMUM_KERNEL(DeviceType::kGPU, float);
REGISTER_BW_MINIMUM_KERNEL(DeviceType::kGPU, double);

REGISTER_USER_OP_GRAD("broadcast_minimum")
    .SetBackwardOpConfGenFn([](user_op::BackwardOpConfContext* ctx) {
      const auto grad_op_name = ctx->FwOp().op_name() + "_grad";
      const auto& grad_op_func = [&ctx](user_op::BackwardOpBuilder& builder) {
        return builder.OpTypeName("broadcast_minimum_backward")
