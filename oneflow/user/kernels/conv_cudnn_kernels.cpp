  ...
  if (!x->is_dynamic()) {
    CudnnConvArgs args(
        ...
        Global<ResourceDesc, ForEnv>::Get()
            ...
            .cudnn_conf()
            .cudnn_conv_use_deterministic_algo_only(),
        Global<ResourceDesc, ForEnv>::Get()->resource().cudnn_conf().cudnn_conv_enable_pseudo_half()
            || (user_op_conf.attr<std::string>("data_format") == "channels_last"
                && std::is_same<PerfT, cudnnConvolutionBwdFilterAlgoPerf_t>::value));
    PerfT algo_perf;
    if (has_forced_algo) {
  ...
  void Compute(user_op::KernelComputeContext* ctx, user_op::OpKernelState* state) const override {
    ...
    CudnnConvArgsAndAlgo<cudnnConvolutionFwdAlgoPerf_t> args_and_algo(
        ...
        Global<ResourceDesc, ForEnv>::Get()
            ->resource()
            .cudnn_conf()
            .has_cudnn_conv_force_fwd_algo(),
        Global<ResourceDesc, ForEnv>::Get()->resource().cudnn_conf().cudnn_conv_force_fwd_algo());
