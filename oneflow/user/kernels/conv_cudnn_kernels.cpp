...
size_t InferTmpSizeWithCudnn(const user_op::TensorDesc* x, const user_op::TensorDesc* w,
  ...
  if (!x->is_dynamic()) {
    CudnnConvArgs args(
        user_op_conf, x->data_type(), ShapeView(x->shape()), w->data_type(), ShapeView(w->shape()),
        y->data_type(), ShapeView(y->shape()), user_op_conf.attr<std::string>("data_format"),
        workspace_size,
        Global<ResourceDesc, ForEnv>::Get()->resource().cudnn_conv_heuristic_search_algo(),
        Global<ResourceDesc, ForEnv>::Get()->resource().cudnn_conv_use_deterministic_algo_only(),
        Global<ResourceDesc, ForEnv>::Get()->resource().cudnn_conv_enable_pseudo_half()
 ...
 private:
  void Compute(user_op::KernelComputeContext* ctx, user_op::OpKernelState* state) const override {
    ...
    CudnnConvArgsAndAlgo<cudnnConvolutionFwdAlgoPerf_t> args_and_algo(
        ...
        Global<ResourceDesc, ForEnv>::Get()->resource().has_cudnn_conv_force_fwd_algo(),
        Global<ResourceDesc, ForEnv>::Get()->resource().cudnn_conv_force_fwd_algo());
 ...
 private:
  void Compute(user_op::KernelComputeContext* ctx) const override {
    ...
    CudnnConvArgsAndAlgo<cudnnConvolutionBwdFilterAlgoPerf_t> args_and_algo(
        ...
        Global<ResourceDesc, ForEnv>::Get()->resource().has_cudnn_conv_force_bwd_filter_algo(),
        Global<ResourceDesc, ForEnv>::Get()->resource().cudnn_conv_force_bwd_filter_algo());
    const CudnnConvArgs& args = args_and_algo.args;
      ...
      .SetInferTmpSizeFn([](user_op::InferContext* ctx) -> size_t {                              \
        ...
        return InferTmpSizeWithCudnn<cudnnConvolutionBwdFilterAlgoPerf_t>(                       \
            ...
            Global<ResourceDesc, ForEnv>::Get()                                                  \
                ->resource()                                                                     \
                .has_cudnn_conv_force_bwd_filter_algo(),                                         \
            Global<ResourceDesc, ForEnv>::Get()->resource().cudnn_conv_force_bwd_filter_algo()); \
      })
