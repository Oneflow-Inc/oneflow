...
template <typename PerfT> struct CudnnConvArgsAndAlgo final {
  using AlgoT = decltype(std::declval<PerfT>().algo);

  CudnnConvArgs args;
  PerfT algo_perf;

  CudnnConvArgsAndAlgo(const user_op::Tensor *x, const user_op::Tensor *w,
      ...
      : args(user_op_conf, x->data_type(), x->shape(), w->data_type(),
             ...
             job_desc.job_conf().cudnn_conv_enable_pseudo_half() ||
                 (user_op_conf.attr<std::string>("data_format") ==
                      "channels_last" &&
                  std::is_same<PerfT,
                               cudnnConvolutionBwdFilterAlgoPerf_t>::value)) {
    ...
    OF_CUDNN_CHECK(
        cudnnSetConvolutionMathType(args.cdesc.Get(), algo_perf.mathType));
  }
...
CudnnTensorDesc *GetBiasCudnnTensorDesc(const std::string &data_format,
                                        int32_t filters, DataType data_type) {
...
// for 3d and Nd
template <>
CudnnTensorDesc *GetBiasCudnnTensorDesc<3>(const std::string &data_format,
                                           int32_t filters,
                                           DataType data_type) {
  constexpr int NDims = 3 + 2;
  CHECK_EQ("channels_first", data_format)
      << "CUDNN Nd API only support channels first";
  std::vector<int32_t> bias_dim(NDims, 1);
  std::vector<int32_t> stride_of_bias_tensor(NDims, 1);
  bias_dim[1] = filters;
  stride_of_bias_tensor[0] = filters;
  return new CudnnTensorDesc(data_type, NDims, bias_dim.data(),
...
public:
  ...
  CreateOpKernelState(user_op::KernelInitContext *ctx) const {
    ...
    const user_op::TensorDesc *bias =
        ctx->TensorDesc4ArgNameAndIndex("bias", 0);
    if (bias != nullptr) {
      state->bias_desc.reset(GetBiasCudnnTensorDesc<NDims>(
          data_format, filters, GetDataType<T>::value));
    }
...
private:
  void Compute(user_op::KernelComputeContext *ctx,
    ...
    OF_CUDNN_CHECK(cudnnConvolutionForward(
        ctx->device_ctx()->cudnn_handle(), CudnnSPOnePtr<T>(), args.xdesc.Get(),
        in->dptr(), args.wdesc.Get(), weight->dptr(), args.cdesc.Get(),
        algo_perf.algo, buf->mut_dptr(), args.params.max_ws_size,
        CudnnSPZeroPtr<T>(), args.ydesc.Get(), out->mut_dptr()));

    const user_op::Tensor *bias = ctx->Tensor4ArgNameAndIndex("bias", 0);
    if (bias != nullptr) {
      ConvCudnnOpKernelState *conv_state =
          dynamic_cast<ConvCudnnOpKernelState *>(state);
      CHECK_NOTNULL(conv_state);
      OF_CUDNN_CHECK(cudnnAddTensor(
          ctx->device_ctx()->cudnn_handle(), CudnnSPOnePtr<T>(),
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    OF_CUDNN_CHECK(cudnnConvolutionBackwardData(
        ctx->device_ctx()->cudnn_handle(), alpha, args.wdesc.Get(),
        filter->dptr(), args.ydesc.Get(), dy->dptr(), args.cdesc.Get(),
        algo_perf.algo, buf->mut_dptr(), args.params.max_ws_size, beta,
        args.xdesc.Get(), dx->mut_dptr()));
  }
};
  ...
  REGISTER_USER_KERNEL("conv_data_grad")                                       \
      ...
      .SetInplaceProposalFn(                                                   \
          [](const user_op::InferContext &ctx,                                 \
            ...
            if (ctx.user_op_conf().has_input("_add_to_output", 0)) {           \
              OF_RETURN_IF_ERROR(                                              \
                  AddInplaceArgPairFn("dx", 0, "_add_to_output", 0, true));    \
            }                                                                  \
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    CudnnConvArgsAndAlgo<cudnnConvolutionBwdFilterAlgoPerf_t> args_and_algo(
        x, filter_diff, dy, buf, job_desc, ctx->user_op_conf(),
        ctx->device_ctx(),
        job_desc.job_conf().has_cudnn_conv_force_bwd_filter_algo(),
        job_desc.job_conf().cudnn_conv_force_bwd_filter_algo());
    const CudnnConvArgs &args = args_and_algo.args;
    const cudnnConvolutionBwdFilterAlgoPerf_t &algo_perf =
        args_and_algo.algo_perf;

    OF_CUDNN_CHECK(cudnnConvolutionBackwardFilter(
        ctx->device_ctx()->cudnn_handle(), CudnnSPOnePtr<T>(), args.xdesc.Get(),
        x->dptr(), args.ydesc.Get(), dy->dptr(), args.cdesc.Get(),
        algo_perf.algo, buf->mut_dptr(), args.params.max_ws_size,
        CudnnSPZeroPtr<T>(), args.wdesc.Get(), filter_diff->mut_dptr()));
  }
};
...
public:
  ...
  CreateOpKernelState(user_op::KernelInitContext *ctx) const {
    ...
    if (data_format == "channels_first") {
      ...
      state->bias_diff_desc.reset(new CudnnTensorDesc(
          CUDNN_TENSOR_NCHW, bias_diff->data_type(), 1,
          static_cast<int32_t>(bias_diff->shape().At(0)), 1, 1));
    } else {
      CHECK(data_format == "channels_last") << "Illegal data_format: "
                                            << data_format;
      CHECK_EQ(dy->shape().At(dy->shape().NumAxes() - 1),
               bias_diff->shape().At(0));
      state->bias_diff_desc.reset(new CudnnTensorDesc(
          CUDNN_TENSOR_NHWC, bias_diff->data_type(), 1,
          static_cast<int32_t>(bias_diff->shape().At(0)), 1, 1));
    }
    return std::move(state);
  }
...
private:
  void Compute(user_op::KernelComputeContext *ctx,
    ...
    dy_desc.reset(
        new CudnnTensorDesc(dy->data_type(), dy->shape(), data_format));
    ...
    OF_CUDNN_CHECK(cudnnConvolutionBackwardBias(
        ctx->device_ctx()->cudnn_handle(), CudnnSPOnePtr<T>(), dy_desc->Get(),
        dy->dptr<T>(), CudnnSPZeroPtr<T>(),
        bias_grad_state->bias_diff_desc->Get(), bias_diff->mut_dptr<T>()));
  }
};
