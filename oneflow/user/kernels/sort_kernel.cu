...
template <typename T> class GpuSortKernel final : public user_op::OpKernel {
public:
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    Memcpy<DeviceType::kGPU>(ctx->device_ctx(), out->mut_dptr<T>(),
    ...
    if (direction == "ASCENDING") {
      SortKeysAscending(in->dptr<T>(), instance_num, instance_size,
                        tmp_buffer->mut_dptr<void>(),
                        tmp_buffer->shape().elem_cnt(), out->mut_dptr<T>(),
                        ctx->device_ctx()->cuda_stream());
    } else if (direction == "DESCENDING") {
      SortKeysDescending(in->dptr<T>(), instance_num, instance_size,
                         tmp_buffer->mut_dptr<void>(),
                         tmp_buffer->shape().elem_cnt(), out->mut_dptr<T>(),
                         ctx->device_ctx()->cuda_stream());
    } else {
...
#define REGISTER_GPU_SORT_KERNEL(dtype)                                        \
  REGISTER_USER_KERNEL("sort")                                                 \
      ...
      .SetInferTmpSizeFn([](user_op::InferContext *ctx) {                      \
        ...
        const std::string &direction = ctx->Attr<std::string>("direction");    \
        if (direction == "ASCENDING") {                                        \
          return InferTempStorageForSortKeysAscending<dtype>(instance_num,     \
                                                             instance_size);   \
        } else if (direction == "DESCENDING") {                                \
          return InferTempStorageForSortKeysDescending<dtype>(instance_num,    \
                                                              instance_size);  \
        } else {                                                               \
          UNIMPLEMENTED();                                                     \
        }                                                                      \
