...
struct DiagonalFunctor<DeviceType::kCPU, T> final {
  void operator()(DeviceCtx* ctx, T* out_buf, const T* in_buf, int32_t size, int32_t dim1, int32_t dim2) {
...
struct DiagonalGradFunctor<DeviceType::kCPU, T> final {
  void operator()(DeviceCtx* ctx, T* dx_buf, const T* dy_buf, int32_t size, int32_t dim1,int32_t dim2) {
    std::cout<<size<<' '<<dim2<<std::endl;
     FOR_RANGE(int32_t, i, 0, size){
      FOR_RANGE(int32_t, j, 0, dim2){
