...
__device__ __inline__ int32_t AtomicCAS(int32_t *address, int32_t compare,
                                        int32_t val) {
  return atomicCAS(address, compare, val);
}

__device__ __inline__ int64_t AtomicCAS(int64_t *address, int64_t compare,
                                        int64_t val) {
...
__device__ __inline__ int64_t AtomicAdd(int64_t *address, int64_t val) {
  static_assert(sizeof(int64_t) == sizeof(CuInt64T), "size error");
  return static_cast<int64_t>(atomicAdd(reinterpret_cast<CuInt64T *>(address),
                                        static_cast<CuInt64T>(val)));
}

template <typename K, typename V>
__device__ bool TryGetOrInsert(K *key, volatile V *value, V *size, const K hash,
                               V *out) {
...
template <typename T>
__device__ bool GetOrInsertOne(const size_t capacity, T *table, T *size,
                               const T hash, T *out) {
  if (hash == 0) {
    *out = 0;
    return true;
...
template <typename T>
__global__ void EncodeGpu(const size_t capacity, T *table, T *size,
                          const int64_t n, const T *hash, T *out) {
...
struct CategoricalOrdinalEncodeKernelUtil<DeviceType::kGPU, T> {
  static void Encode(DeviceCtx *ctx, int64_t capacity, T *table, T *size,
    ...
    EncodeGpu<T><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock, 0,
                   ctx->cuda_stream()>>>(capacity, table, size, n, hash, out);
  }
};

#define INSTANTIATE_CATEGORICAL_ORDINAL_ENCODE_KERNEL_UTIL_GPU(type_cpp,       \
                                                               type_proto)     \
  template struct CategoricalOrdinalEncodeKernelUtil<DeviceType::kGPU,         \
                                                     type_cpp>;
OF_PP_FOR_EACH_TUPLE(INSTANTIATE_CATEGORICAL_ORDINAL_ENCODE_KERNEL_UTIL_GPU,
                     INDEX_DATA_TYPE_SEQ);
