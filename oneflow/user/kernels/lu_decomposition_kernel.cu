...
#if (CUDA_VERSION >= 11000) || defined(WITH_ROCM)
...
void OFgetrf_bufferSize(ep::Stream* stream, int32_t m, int32_t n, float* dA_array, int32_t lda,
                        int32_t& lwork) {
  OF_CUSOLVER_CHECK(GPUSOLVER(DnSgetrf_bufferSize)(stream->As<ep::CudaStream>()->cusolver_dn_handle(),
                                                m, n, dA_array, m, &lwork));
}

void OFgetrf_bufferSize(ep::Stream* stream, int32_t m, int32_t n, double* dA_array, int32_t lda,
                        int32_t& lwork) {
  OF_CUSOLVER_CHECK(GPUSOLVER(DnDgetrf_bufferSize)(stream->As<ep::CudaStream>()->cusolver_dn_handle(),
                                                m, n, dA_array, m, &lwork));
}
...
void OFgetrf(ep::Stream* stream, int32_t m, int32_t n, float* dA_array, int32_t lda, float* d_work,
  ...
  OF_CUSOLVER_CHECK(GPUSOLVER(DnSgetrf)(stream->As<ep::CudaStream>()->cusolver_dn_handle(), m, m,
...
void OFgetrf(ep::Stream* stream, int32_t m, int32_t n, double* dA_array, int32_t lda,
  ...
  OF_CUSOLVER_CHECK(GPUSOLVER(DnDgetrf)(stream->As<ep::CudaStream>()->cusolver_dn_handle(), m, m,
                                     dA_array, lda, d_work, pivot_ptr, d_info));
}
}  // namespace
...
class LUDecompositionKernel final : public user_op::OpKernel {
 ...
 private:
  ...
  void Compute(user_op::KernelComputeContext* ctx) const override {
    ...
    OF_CUDA_CHECK(
        GPU(Malloc)(reinterpret_cast<void**>(&batched_d_info), batch_count * sizeof(int32_t)));

    for (size_t batch = 0; batch < batch_count; batch++) {
      OFgetrf_bufferSize(stream, m, m, LU_ptr, m, lwork);
      OF_CUDA_CHECK(GPU(Malloc)(reinterpret_cast<void**>(&d_work), sizeof(T) * lwork));
      OFgetrf(stream, m, m, LU_ptr + batch * matrix_stride, lda, d_work,
              pivot_ptr + batch * pivot_stride, batched_d_info + batch);
      OF_CUDA_CHECK(GPU(Free)(d_work));
    }

    OF_CUDA_CHECK(GPU(MemcpyAsync)(batched_info.data(), batched_d_info,
                                  batch_count * sizeof(int32_t), GPU(MemcpyDeviceToHost),
                                  stream->cuda_stream()));
    ...
    OF_CUDA_CHECK(GPU(Free)(batched_d_info));
  }
