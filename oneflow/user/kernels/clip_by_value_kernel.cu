...
template <typename T, typename F>
__global__ void CudaClipBackward(F clip_func, int64_t n, const T *x,
                                 const T *dy, T *dx) {
...
template <typename T> struct ClipKernelUtil<DeviceType::kGPU, T> {
  template <typename F>
...
#define INITIATE_CLIP_KERNEL_UTIL_GPU(dtype, dtype_v)                          \
  ...
  template void ClipKernelUtil<DeviceType::kGPU, dtype>::Forward(              \
      DeviceCtx *, ClipByMinFunctor<dtype>, const int64_t n, const dtype *,    \
      dtype *);                                                                \
  template void ClipKernelUtil<DeviceType::kGPU, dtype>::Forward(              \
      DeviceCtx *, ClipByMaxFunctor<dtype>, const int64_t n, const dtype *,    \
      dtype *);                                                                \
  template void ClipKernelUtil<DeviceType::kGPU, dtype>::Forward(              \
      DeviceCtx *, ClipByMinMaxFunctor<dtype>, const int64_t n, const dtype *, \
      dtype *);                                                                \
  template void ClipKernelUtil<DeviceType::kGPU, dtype>::Backward(             \
      DeviceCtx *, ClipByMinGradFunctor<dtype>, const int64_t n,               \
      const dtype *, const dtype *, dtype *);                                  \
  template void ClipKernelUtil<DeviceType::kGPU, dtype>::Backward(             \
      DeviceCtx *, ClipByMaxGradFunctor<dtype>, const int64_t n,               \
      const dtype *, const dtype *, dtype *);                                  \
  template void ClipKernelUtil<DeviceType::kGPU, dtype>::Backward(             \
      DeviceCtx *, ClipByMinMaxGradFunctor<dtype>, const int64_t n,            \
      const dtype *, const dtype *, dtype *);
