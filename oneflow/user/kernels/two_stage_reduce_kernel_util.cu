...
struct TwoStageReduceKernelUtil<DeviceType::kGPU, T, K> {
  static void Divide(DeviceCtx *ctx, const int64_t n, const T *x,
                     const K *count, T *y) {
    DivideGpu<T, K><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock, 0,
                      ctx->cuda_stream()>>>(n, x, count, y);
  }

  static void Mask(DeviceCtx *ctx, const int64_t n, const T *x, const K *mask,
                   T *y) {
    MaskGpu<T, K><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock, 0,
                    ctx->cuda_stream()>>>(n, x, mask, y);
  }

  static void Scale(DeviceCtx *ctx, const int64_t n, const T *x, const K *scale,
                    T *y) {
    ScaleGpu<T, K><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock, 0,
                     ctx->cuda_stream()>>>(n, x, scale, y);
  }
};

#define INSTANTIATE_TWO_STAGE_REDUCE_KERNEL_UTIL_GPU(data_type_pair,           \
                                                     index_type_pair)          \
  template struct TwoStageReduceKernelUtil<DeviceType::kGPU,                   \
