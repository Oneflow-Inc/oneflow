...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    CHECK_NOTNULL(ctag);
    std::string tag_str(reinterpret_cast<char *>(ctag),
                        tag->shape().elem_cnt());
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    EventWriterHelper<DeviceType::kCPU, T>::WriteHistogramToFile(
        static_cast<float>(istep[0]), *value, tag_str);
  }
  bool AlwaysComputeWhenAllOutputsEmpty() const override { return true; }
};
...
template <typename T> class SummaryWritePb final : public user_op::OpKernel {
public:
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    std::string value_str(reinterpret_cast<char *>(cvalue),
                          value->shape().elem_cnt());
...
REGISTER_USER_KERNEL("summary_write_pb")
    ...
    .SetIsMatchedHob((user_op::HobDeviceTag() == "cpu") &
                     (user_op::HobDataType("in", 0) ==
                      GetDataType<int8_t>::value));

template <typename T> class SummaryWriteImage final : public user_op::OpKernel {
public:
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    EventWriterHelper<DeviceType::kCPU, T>::WriteImageToFile(
        static_cast<int64_t>(istep[0]), *value, tag_str);
  }
  bool AlwaysComputeWhenAllOutputsEmpty() const override { return true; }
};
