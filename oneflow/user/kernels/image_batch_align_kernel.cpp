...
void CopyFromTensorBuffer(T *image_ptr, const TensorBuffer &image_buffer,
...
template <typename T> struct ImageCopier final {
#define MAKE_COPY_FROM_TENSOR_BUFFER_SWITCH_ENTRY(func_name, F) func_name<T, F>
  DEFINE_STATIC_SWITCH_FUNC(void, CopyFromTensorBuffer,
                            MAKE_COPY_FROM_TENSOR_BUFFER_SWITCH_ENTRY,
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    memset(out_tensor->mut_dptr(), 0,
           out_tensor->shape().elem_cnt() *
               GetSizeOfDataType(out_tensor->data_type()));
    MultiThreadLoop(num_images, [&](size_t i) {
      const TensorBuffer &image_buffer = in_tensor->dptr<TensorBuffer>()[i];
      T *out_ptr =
          out_tensor->mut_dptr<T>() + i * max_height * max_width * channels;
      ImageCopier<T>::SwitchCopyFromTensorBuffer(
          SwitchCase(image_buffer.data_type()), out_ptr, image_buffer,
