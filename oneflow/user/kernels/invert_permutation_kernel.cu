...
template<typename T>
__global__ void InvertPermutationGpu(const int64_t n, const T* x, 
                                     T* y) {
  CUDA_1D_KERNEL_LOOP(i, n) { y[x[i]] = i; }
}

template<typename T>
void InvertPermutation(DeviceCtx* ctx, const int64_t n, const T* x, 
                       T* y)
{
    InvertPermutationGpu<T><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock, 0, ctx->cuda_stream()>>>(
      n, x, y);
}
...
class GpuInvertPermutationKernel final : public user_op::OpKernel {
 ...
 private:
    void Compute(user_op::KernelComputeContext* ctx) const override {
        ...
        InvertPermutation<T>(ctx->device_ctx(), elem_cnt, x_ptr,
                             y_ptr);
    }
