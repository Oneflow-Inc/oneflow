...
GetTargetResizedSize4ImageBuffer(const TensorBuffer &image_buffer,
  ...
  if (min_size > 0) {
    CHECK_GE(target_size, min_size);
  }
  if (max_size > 0) {
    CHECK_LE(target_size, max_size);
  }
...
bool CheckMatSizeMatch(const cv::Mat &mat, const bool resize_longer,
                       const int32_t target_size, const int32_t min_size,
...
void ImageTargetResize(const TensorBuffer &image_buffer,
                       ...
                       const bool resize_longer, const int32_t target_size,
  ...
  int interp_flag = GetCvInterpolationFlag(interp_type, image_mat.cols,
                                           image_mat.rows, res_w, res_h);
  cv::resize(image_mat, res_image_mat, cv::Size(res_w, res_h), 0, 0,
             interp_flag);

  CHECK_EQ(res_image_mat.ptr<void>(), resized_image_buffer->data());
  CHECK(CheckMatSizeMatch(res_image_mat, resize_longer, target_size, min_size,
                          max_size));
}
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    MultiThreadLoop(batch_size, [&](size_t i) {
      ...
      int interp_flag =
          GetCvInterpolationFlag(ctx->Attr<std::string>("interpolation_type"),
      ...
      if (in_buffer.data_type() == dtype) {
        cv::resize(in_img_mat, out_img_mat, cv::Size(res_w, res_h), 0, 0,
                   interp_flag);
      } else {
        cv::Mat res_img_mat;
        cv::resize(in_img_mat, res_img_mat, cv::Size(res_w, res_h), 0, 0,
                   interp_flag);
        CvMatConvertToDataType(res_img_mat, &out_img_mat, dtype);
      }

      char *cur_out_dptr = out_tensor->mut_dptr<char>() +
                           i * elem_cnt_per_img * GetSizeOfDataType(dtype);
      ...
      if (scale_tensor) {
        ...
        scale_dptr[0] =
            static_cast<float>(res_w) / static_cast<float>(origin_width);
        scale_dptr[1] =
            static_cast<float>(res_h) / static_cast<float>(origin_height);
      }
    });
  }
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    const std::string &interp_type =
        ctx->Attr<std::string>("interpolation_type");

    MultiThreadLoop(num_images, [&](size_t i) {
      ImageTargetResize(in_img_buf[i], out_img_buf + i, resize_longer,
                        target_size, min_size, max_size, interp_type);
      ...
      scale_buf[i].mut_data<float>()[0] =
          static_cast<float>(res_w) / static_cast<float>(org_w);
      scale_buf[i].mut_data<float>()[1] =
          static_cast<float>(res_h) / static_cast<float>(org_h);
...
#define REGISTER_IMAGE_RESIZE_KERNEL(dtype)                                    \
  REGISTER_USER_KERNEL("image_resize_to_fixed")                                \
      ...
      .SetIsMatchedHob(                                                        \
          ...
          (user_op::HobAttr<DataType>("data_type") ==                          \
