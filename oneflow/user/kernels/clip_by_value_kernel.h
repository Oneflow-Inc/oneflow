...
template <typename T> OF_DEVICE_FUNC T DeviceMin(T a, T b) {
...
template <typename T> OF_DEVICE_FUNC T DeviceMax(T a, T b) {
#if defined(__CUDA_ARCH__)
  return max(a, b);
#else
  return std::max(a, b);
...
template <typename T> struct ClipByMinFunctor {
  ClipByMinFunctor(T min) : min_value(min) {}
  OF_DEVICE_FUNC T operator()(T value) { return DeviceMax(value, min_value); }
  T min_value;
};

template <typename T> struct ClipByMaxFunctor {
  ClipByMaxFunctor(T max) : max_value(max) {}
  OF_DEVICE_FUNC T operator()(T value) { return DeviceMin(value, max_value); }
  T max_value;
};

template <typename T> struct ClipByMinMaxFunctor {
  ClipByMinMaxFunctor(T min, T max) : min_value(min), max_value(max) {}
  OF_DEVICE_FUNC T operator()(T value) {
    return DeviceMin(DeviceMax(value, min_value), max_value);
  }
  T min_value;
  T max_value;
};

template <typename T> struct ClipByMinGradFunctor {
...
template <typename T> struct ClipByMaxGradFunctor {
...
template <typename T> struct ClipByMinMaxGradFunctor {
...
template <DeviceType device_type, typename T> struct ClipKernelUtil {
  template <typename F>
