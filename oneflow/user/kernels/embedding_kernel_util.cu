...
__global__ void indicesFreq(const index_T* indices_buf, const int32_t num_indices, int32_t * tmp_buf){
    CUDA_1D_KERNEL_LOOP(i, num_indices){
        cuda::atomic::Add(tmp_buf + indices_buf[i], 1);
    }
}

template<typename T, typename index_T>
__global__ void embeddingScale(T* dx_buf, const int32_t emb_size, const int32_t emb_dim, int32_t * tmp_buf){
    CUDA_1D_KERNEL_LOOP(i, emb_size*emb_dim){
        int32_t emb_size_index = i/emb_dim;
        if(tmp_buf[emb_size_index]>1){
...
__global__ void embNorm_kernel(const T* in_buf, T* out_buf, double * emb_norm, int32_t *indices_frep,
                                  const double max_norm, const int32_t emb_size, const int32_t emb_dim){
    CUDA_1D_KERNEL_LOOP(i, emb_size * emb_dim){
       int32_t emb_size_index = i / emb_dim;
       int32_t emb_dim_index = i - emb_size_index * emb_dim;
       if(indices_frep[emb_size_index] > 0){
...
struct EmbeddingRenormFunctor<DeviceType::kCUDA, T, index_T> final{
    void operator()(ep::Stream* stream, const T* in_buf, const index_T* indices_buf, T* out_buf,
                    const double max_norm, const double norm_type, const int32_t num_indices,  const int32_t emb_size, const int32_t emb_dim,
                    void * tmp_buf){
        size_t bytes_used = 0;
        ...
        indicesFreq<index_T><<<BlocksNum4ThreadsNum(num_indices), kCudaThreadsNumPerBlock, 0,
        ...
        embNorm<T, index_T><<<BlocksNum4ThreadsNum(emb_size * emb_dim), kCudaThreadsNumPerBlock, 0,
           stream->As<ep::CudaStream>()->cuda_stream()>>>(in_buf, indices_frep, emb_norm, norm_type, emb_size, emb_dim);

        embNorm_kernel<T, index_T><<<BlocksNum4ThreadsNum(emb_size*emb_dim), kCudaThreadsNumPerBlock, 0,
           stream->As<ep::CudaStream>()->cuda_stream()>>>(in_buf, out_buf, emb_norm, indices_frep, max_norm, emb_size, emb_dim);
    }
...
struct EmbeddingGradFunctor<DeviceType::kCUDA, T, index_T> final{
    void operator()(ep::Stream* stream, const T* dy_buf, const index_T* indices_buf, T* dx_buf,
                    const int32_t padding_idx, const bool scale_grad_by_freq,  const int32_t num_indices, 
        ...
        if(scale_grad_by_freq){
           ...
           embeddingScale<T, index_T><<<BlocksNum4ThreadsNum(emb_size * emb_dim), kCudaThreadsNumPerBlock, 0,
...
#define INITIATE_EMBEDDING_KERNEL_UTIL_CUDA_IMPL(in_type_pair, index_type_pair)                   \
      ...
      template struct EmbeddingGradFunctor<DeviceType::kCUDA, OF_PP_PAIR_FIRST(in_type_pair),     \
                                       OF_PP_PAIR_FIRST(index_type_pair)>;
OF_PP_SEQ_PRODUCT_FOR_EACH_TUPLE(INITIATE_EMBEDDING_KERNEL_UTIL_CUDA_IMPL, EMBEDDING_DATA_TYPE_SEQ_CUDA,
                                 INDEX_DATA_TYPE_SEQ);
