...
__launch_bounds__(kBlockSize) __global__
    void DoCUDAMaxPool1dForward(const NdIndexOffsetHelper<int64_t, 3> index_helper,
                                ...
                                int64_t* indice_ptr, int32_t padding_l, int64_t n_batch,
                                int64_t n_channel, int64_t x_length, int64_t y_length,
                                int32_t kernel_size_l, int32_t stride_l, int32_t dilation_l) {
  Maxpool1dForwardCompute<half>(index_helper, elem_num, reinterpret_cast<const half*>(src),
                                reinterpret_cast<half*>(dest), indice_ptr, padding_l, n_batch,
                                n_channel, x_length, y_length, kernel_size_l, stride_l, dilation_l);
};
...
__launch_bounds__(kBlockSize) __global__
    void DoCUDAMaxPool3dForward(const NdIndexOffsetHelper<int64_t, 5> index_helper,
                                int64_t elem_num, const float16* src, float16* dest,
                                int64_t* indice_ptr, int32_t padding_t, int32_t padding_h,
                                int32_t padding_w, int64_t n_batch, int64_t n_channel,
                                int64_t x_time, int64_t x_height, int64_t x_width, int64_t y_time,
                                int64_t y_height, int64_t y_width, int32_t kernel_size_t,
                                int32_t kernel_size_h, int32_t kernel_size_w, int32_t stride_t,
                                int32_t stride_h, int32_t stride_w, int32_t dilation_t,
                                int32_t dilation_h, int32_t dilation_w) {
  Maxpool3dForwardCompute<half>(index_helper, elem_num, reinterpret_cast<const half*>(src),
                                reinterpret_cast<half*>(dest), indice_ptr, padding_t, padding_h,
                                padding_w, n_batch, n_channel, x_time, x_height, x_width, y_time,
                                y_height, y_width, kernel_size_t, kernel_size_h, kernel_size_w,
                                stride_t, stride_h, stride_w, dilation_t, dilation_h, dilation_w);
};
...
__launch_bounds__(kBlockSize) __global__
    void DoCUDAMaxPool1dBackward(const NdIndexOffsetHelper<int64_t, 3> index_helper,
                                 ...
                                 const int64_t n_channel, const int64_t src_length,
                                 const int64_t dst_length) {
  Maxpool1dBackwardCompute<half>(index_helper, elem_num, reinterpret_cast<const half*>(src),
...
__launch_bounds__(kBlockSize) __global__
    void DoCUDAMaxPool3dBackward(const NdIndexOffsetHelper<int64_t, 5> index_helper,
                                 ...
                                 const int64_t dst_time, const int64_t dst_height,
                                 const int64_t dst_width) {
  Maxpool3dBackwardCompute<half>(index_helper, elem_num, reinterpret_cast<const half*>(src),
                                 reinterpret_cast<half*>(dest), indice_ptr, n_batch, n_channel,
                                 src_time, src_height, src_width, dst_time, dst_height, dst_width);
};
// ========================half implementation===============================
