...
__global__ void DoCUDAMaxPool2dForward(
    ...
    FarwardCompute<T>(
      index_helper, elem_num, maxval, src, dest, indice_ptr, padding_h, padding_w, n_batch, 
      n_channel, x_height, x_width, y_height, y_width,
      kernel_size_h, kernel_size_w, stride_h, stride_w, dilation_h, dilation_w, return_indices, ceil_mode
    );  
};
