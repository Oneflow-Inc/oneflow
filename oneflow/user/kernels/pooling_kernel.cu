...
__global__ void DoCUDAMaxPool2dForward(const NdIndexOffsetHelper<int64_t, 4> index_helper,
  ...
  Maxpool2dFarwardCompute<T>(index_helper, elem_num, maxval, src, dest, indice_ptr, padding_h, padding_w,
                    n_batch, n_channel, x_height, x_width, y_height, y_width, kernel_size_h,
                    kernel_size_w, stride_h, stride_w, dilation_h, dilation_w, return_indices,
                    ceil_mode);
};
...
__global__ void DoCUDAMaxPool3dForward(const NdIndexOffsetHelper<int64_t, 5> index_helper,
                                       int64_t elem_num, T maxval, const T* src, T* dest,
                                       int64_t* indice_ptr, int32_t padding_t, int32_t padding_h, int32_t padding_w,
                                       int64_t n_batch, int64_t n_channel, int64_t x_time, int64_t x_height,
                                       ...
                                       int32_t stride_t, int32_t stride_h, int32_t stride_w, int32_t dilation_t, int32_t dilation_h,
                                       int32_t dilation_w, bool return_indices, bool ceil_mode) {
  Maxpool3dFarwardCompute<T>(index_helper, elem_num, maxval, src, dest, indice_ptr, padding_t, padding_h, padding_w,
                    n_batch, n_channel, x_time, x_height, x_width, y_time, y_height, y_width, kernel_size_t, kernel_size_h,
                    kernel_size_w, stride_t, stride_h, stride_w, dilation_t, dilation_h, dilation_w, return_indices,
                    ceil_mode);
};
...
__global__ void DoCUDAMaxPool2dBackward(const NdIndexOffsetHelper<int64_t, 4> index_helper,
  ...
  Maxpool2dBackwardCompute<T>(index_helper, elem_num, src, dest, indice_ptr, n_batch, n_channel, src_height,
...
__global__ void DoCUDAMaxPool3dBackward(const NdIndexOffsetHelper<int64_t, 5> index_helper,
                                        const int64_t elem_num, const T* src, T* dest,
                                        const int64_t* indice_ptr, const int64_t n_batch,
                                        const int64_t n_channel, const int64_t src_time, const int64_t src_height,
                                        const int64_t src_width, const int64_t dst_time, const int64_t dst_height,
                                        const int64_t dst_width, const bool return_indices,
                                        const bool ceil_mode) {
  Maxpool3dBackwardCompute<T>(index_helper, elem_num, src, dest, indice_ptr, n_batch, n_channel, src_time, src_height,
...
struct PoolingKernelUtil<DeviceType::kGPU, T> {
  ...
  static void Maxpool3dForward(DeviceCtx* ctx, const NdIndexOffsetHelper<int64_t, 5> index_helper,
    ...
    DoCUDAMaxPool3dForward<T>
        <<<BlocksNum4ThreadsNum(elem_num), kCudaThreadsNumPerBlock, 0, ctx->cuda_stream()>>>(
            index_helper, elem_num, maxval, src, dest, indice_ptr, 
            padding_before[0], padding_before[1], padding_before[2], n_batch, n_channel, 
            x_time, x_height, x_width, y_time, y_height, y_width,
            kernel_size[0], kernel_size[1], kernel_size[2], 
            stride[0], stride[1], stride[2], dilation[0], dilation[1], dilation[2],
            return_indices, ceil_mode);
  }
