...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    for (int32_t i = 0; i < n; ++i) {
      z[i] = BinaryFunctor<T>::Forward(x[i], y[i]);
    }
  }
...
#define REGISTER_MATH_BINARY_ELEMENTWISE_CPU_KERNEL_AND_GRAD(math_type_pair,   \
                                                             data_type_pair)   \
  REGISTER_USER_KERNEL(OF_PP_PAIR_FIRST(math_type_pair))                       \
      .SetCreateFn<MathBinaryElementwiseCpuKernel<                             \
...
OF_PP_SEQ_PRODUCT_FOR_EACH_TUPLE(
    REGISTER_MATH_BINARY_ELEMENTWISE_CPU_KERNEL_AND_GRAD,
