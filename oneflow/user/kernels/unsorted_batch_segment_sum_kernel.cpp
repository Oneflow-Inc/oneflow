...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    const user_op::Tensor *segment_ids =
        ctx->Tensor4ArgNameAndIndex("segment_ids", 0);
    ...
    BatchGatherKernelUtilImpl<device_type, T, K>::Backward(
        ctx->device_ctx(), data->dptr<T>(), segment_ids->dptr<K>(),
        flat_data_shape, out->shape().At(axis), out->mut_dptr<T>());
  }
  bool AlwaysComputeWhenAllOutputsEmpty() const override { return true; }
};

#define REGISTER_UNSORTED_BATCH_SEGMENT_SUM_KERNEL(device, out_dtype,          \
                                                   segment_ids_dtype)          \
  REGISTER_USER_KERNEL("unsorted_batch_segment_sum")                           \
      .SetCreateFn<UnsortedBatchSegmentSumKernel<                              \
          device, OF_PP_PAIR_FIRST(out_dtype),                                 \
...
OF_PP_SEQ_PRODUCT_FOR_EACH_TUPLE(REGISTER_UNSORTED_BATCH_SEGMENT_SUM_KERNEL,
