...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    SortPairsDescending(
        in->dptr<T>(), buf_manager.IndicesPtr(), instance_num, instance_size,
        buf_manager.TempStoragePtr(), buf_manager.TempStorageBytes(),
        buf_manager.SortedInPtr(), buf_manager.SortedIndicesPtr(),
        ctx->device_ctx()->cuda_stream());
    cudaMemcpy2DAsync(out->mut_dptr<int32_t>(), k * sizeof(int32_t),
                      buf_manager.SortedIndicesPtr(),
                      instance_size * sizeof(int32_t), k * sizeof(int32_t),
                      instance_num, cudaMemcpyDeviceToDevice,
                      ctx->device_ctx()->cuda_stream());
  }
