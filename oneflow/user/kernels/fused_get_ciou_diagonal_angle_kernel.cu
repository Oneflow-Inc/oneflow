...
struct FusedCiouAngleForwardFunctor {
  __device__ T Compute(T w1, T h1, T w2, T h2, T eps) const {
    T angle = (atan(w2 / (h2 + eps)) - atan(w1 / (h1 + eps))) * (atan(w2 / (h2 + eps)) - atan(w1 / (h1 + eps)));
...
struct FusedCiouAngleForwardFunctor<half> {
  __device__ half Compute(half w1, half h1, half w2, half h2, float eps) const {
    ...
    float angle = (atan(w2f / (h2f + eps)) - atan(w1f / (h1f + eps))) * (atan(w2f / (h2f + eps)) - atan(w1f / (h1f + eps)));
    return static_cast<half>(4.0 / (M_PI * M_PI)) * __float2half(angle);
  }
};

template<typename FUNCTOR, typename T>
__global__ void FusedCiouAngleForward(FUNCTOR functor, const int n, 
                                      const T* w1, const T* h1,
                                      const T* w2, const T* h2, const T eps, T* v) {
  CUDA_1D_KERNEL_LOOP(i, n) {
    v[i] = functor.Compute(w1[i], h1[i], w2[i], h2[i], eps);
  }
}
...
struct FusedCiouAngleBackwardFunctor {
  __device__ T ComputeW1(T w1, T h1, T w2, T h2, T eps) const {
    ...
    return static_cast<T>(-8) * angle_delta / (static_cast<T>((M_PI * M_PI)) * (h1 + eps) * (static_cast<T>(1) + (w1 * w1 / ((h1 + eps) * (h1 + eps)))));
  }

  __device__ T ComputeW2(T w1, T h1, T w2, T h2, T eps) const {
    T angle_delta = atan(w2 / (h2 + eps)) - atan(w1 / (h1 + eps));
    return static_cast<T>(8) * angle_delta / (static_cast<T>((M_PI * M_PI)) * (h2 + eps) * (static_cast<T>(1) + (w2 * w2 / ((h2 + eps) * (h2 + eps)))));
  }

  __device__ T ComputeH1(T w1, T h1, T w2, T h2, T eps) const {
    T angle_delta = atan(w2 / (h2 + eps)) - atan(w1 / (h1 + eps));
    return static_cast<T>(8) * w1 * angle_delta / (static_cast<T>((M_PI * M_PI)) * (h1 + eps) * (h1 + eps) * (static_cast<T>(1) + (w1 * w1 / ((h1 + eps) * (h1 + eps)))));
  }

  __device__ T ComputeH2(T w1, T h1, T w2, T h2, T eps) const {
    T angle_delta = atan(w2 / (h2 + eps)) - atan(w1 / (h1 + eps));
    return static_cast<T>(-8) * w2 * angle_delta / (static_cast<T>(M_PI * M_PI) * (h2 + eps) * (h2 + eps) * (static_cast<T>(1) + (w2 * w2 / ((h2 + eps) * (h2 + eps)))));
  }
};
...
struct FusedCiouAngleBackwardFunctor<half> {
  __device__ half ComputeW1(half w1, half h1, half w2, half h2, float eps) const {
    ...
    return __float2half(static_cast<float>(-8) * angle_delta / (M_PI * M_PI * (h1f + eps) * (static_cast<float>(1) + (w1f * w1f / ((h1f + eps) * (h1f + eps))))));
  }
  ...
  __device__ half ComputeW2(half w1, half h1, half w2, half h2, float eps) const {
    ...
    float h2f = __half2float(h2);
    float angle_delta = atan(w2f / (h2f + eps)) - atan(w1f / (h1f + eps));
    return __float2half(static_cast<float>(8) * angle_delta / (M_PI * M_PI * (h2f + eps) * (static_cast<float>(1) + (w2f * w2f / ((h2f + eps) * (h2f + eps))))));
  }
  ...
  __device__ half ComputeH1(half w1, half h1, half w2, half h2, float eps) const {
    ...
    float h2f = __half2float(h2);
    float angle_delta = atan(w2f / (h2f + eps)) - atan(w1f / (h1f + eps));
    return __float2half(static_cast<float>(8) * w1f * angle_delta / (M_PI * M_PI * (h1f + eps) * (h1f + eps) * (static_cast<float>(1) + (w1f * w1f / ((h1f + eps) * (h1f + eps))))));
  }
  ...
  __device__ half ComputeH2(half w1, half h1, half w2, half h2, float eps) const {
    ...
    float h2f = __half2float(h2);
    float angle_delta = atan(w2f / (h2f + eps)) - atan(w1f / (h1f + eps));
    return __float2half(static_cast<float>(-8) * w2f * angle_delta / (M_PI * M_PI * (h2f + eps) * (h2f + eps) * (static_cast<float>(1) + (w2f * w2f / ((h2f + eps) * (h2f + eps))))));
  }
};

template<typename FUNCTOR, typename T>
__global__ void FusedCiouAngleBackward(FUNCTOR functor, const int n, 
                                      const T* w1, const T* h1,
...
class FusedGetCiouDiagonalAngleKernel final : public user_op::OpKernel {
 ...
 private:
  ...
  void Compute(user_op::KernelComputeContext* ctx) const override {
    ...
    RUN_CUDA_KERNEL((FusedCiouAngleForward<decltype(fused_get_ciou_diagonal_angle_functor), T>), ctx->stream(),
                    elem_cnt, fused_get_ciou_diagonal_angle_functor, elem_cnt,
                    w1->dptr<T>(), h1->dptr<T>(), w2->dptr<T>(), h2->dptr<T>(), static_cast<T>(eps), v->mut_dptr<T>());
  }
...
class FusedGetCiouDiagonalAngleGradKernel final : public user_op::OpKernel {
 ...
 private:
  ...
  void Compute(user_op::KernelComputeContext* ctx) const override {
    ...
    RUN_CUDA_KERNEL((FusedCiouAngleBackward<decltype(fused_get_ciou_diagonal_angle_grad_functor), T>), ctx->stream(),
                    elem_cnt, fused_get_ciou_diagonal_angle_grad_functor, elem_cnt,
                    w1->dptr<T>(), h1->dptr<T>(), w2->dptr<T>(), h2->dptr<T>(), v_diff->dptr<T>(), static_cast<T>(eps),
                    w1_diff->mut_dptr<T>(), h1_diff->mut_dptr<T>(),
                    w2_diff->mut_dptr<T>(), h2_diff->mut_dptr<T>());
  }
  bool AlwaysComputeWhenAllOutputsEmpty() const override { return false; }
};
