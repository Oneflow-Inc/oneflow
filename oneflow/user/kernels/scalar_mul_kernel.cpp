...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    NewKernelUtil<device_type>::MulByScalar(ctx->device_ctx(),
                                            out->shape().elem_cnt(), in_ptr,
...
#define REGISTER_KERNEL(kernel_device_type, dtype)                             \
  REGISTER_USER_KERNEL("scalar_mul")                                           \
      .SetCreateFn<                                                            \
          ScalarMulUserKernel<DeviceType::k##kernel_device_type, dtype>>()     \
      .SetIsMatchedHob(                                                        \
          (user_op::HobDeviceTag() == DeviceType::k##kernel_device_type) &     \
