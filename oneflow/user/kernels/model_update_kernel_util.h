...
struct AdadeltaUpdateFunctor {
  OF_DEVICE_FUNC void operator()(const G* model_diff, T* model, T* square_avgs, T* acc_deltas, T scale,
                                 float l1, float l2, float rho, float epsilon, bool maximize, float weight_decay, float learning_rate) {
    const T model_val = *model;
    T model_diff_val = *model_diff; 
    if(maximize){
      model_diff_val = -model_diff_val; 
    }
    T model_diff_t =
