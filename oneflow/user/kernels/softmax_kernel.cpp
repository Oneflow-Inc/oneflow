...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    const int64_t num_instances =
        in->shape().Count(0, in->shape().NumAxes() - 1);
...
user_op::InferTmpSizeFn GenFwInferTmpSizeFn() {
  return [](user_op::InferContext *ctx) {
    ...
    return SoftmaxKernelUtil<
        device_type, T>::GetComputeProbTempStorageSizeInBytes(num_instances,
                                                              num_classes);
  };
}
...
user_op::InferTmpSizeFn GenBwInferTmpSizeFn() {
  return [](user_op::InferContext *ctx) {
    ...
    return SoftmaxKernelUtil<
        device_type, T>::GetComputeDiffTempStorageSizeInBytes(num_instances,
                                                              num_classes);
  };
}
