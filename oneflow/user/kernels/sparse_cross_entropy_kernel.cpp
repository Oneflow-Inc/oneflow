...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    SparseCrossEntropyKernelUtil<device_type, T, K>::ComputeEntropy(
        ...
        prediction->dptr<T>(), label->dptr<K>(), out->mut_dptr<T>());
  }
...
#define REGISTER_SPARSE_CROSS_ENTROPY_KERNEL(                                  \
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    size_t prediction_diff_bytes_size =
        prediction_diff->shape().elem_cnt() *
        GetSizeOfDataType(prediction_diff->data_type());
    ...
    SparseCrossEntropyKernelUtil<device_type, T, K>::ComputeDiff(
        ctx->device_ctx(), num_instances, num_classes, depth, lower_bound,
        prediction->dptr<T>(), label->dptr<K>(), dy->dptr<T>(),
...
#define REGISTER_SPARSE_CROSS_ENTROPY_GRAD_KERNEL(                             \
    kernel_class, kernel_name, device_type_v, dtype_pair, ltype_pair)          \
  REGISTER_USER_KERNEL(kernel_name)                                            \
      .SetCreateFn<kernel_class<device_type_v, OF_PP_PAIR_FIRST(dtype_pair),   \
                                OF_PP_PAIR_FIRST(ltype_pair)>>()               \
