...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    NewKernelUtil<device_type>::Sigmoid(ctx->device_ctx(),
                                        x->shape().elem_cnt(), x->dptr<T>(),
                                        y->mut_dptr<T>());
  }
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    NewKernelUtil<device_type>::SigmoidBackward(
        ctx->device_ctx(), y_blob->shape().elem_cnt(), y_blob->dptr<T>(),
        y_blob->dptr<T>(), dy_blob->dptr<T>(), dx_blob->mut_dptr<T>());
  }
  bool AlwaysComputeWhenAllOutputsEmpty() const override { return false; }
};
