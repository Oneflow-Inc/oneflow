...
void GetDimVectorInBytes(const ShapeView &tensor_shape,
...
template <> float16 GetDtypeMatchedValue(double floating, int64_t integral) {
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    device_memory_copier->Copy(ctx->device_ctx(), y->mut_dptr<T>(),
                               x->dptr<T>(), reduced_memory_copy_nd_desc);
  }
...
#define REGISTER_PAD_KERNEL(dev, dtype)                                        \
  REGISTER_USER_KERNEL("pad")                                                  \
      .SetCreateFn<PadKernel<dev, dtype>>()                                    \
      .SetIsMatchedHob(                                                        \
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    const int64_t size_of_data_type =
        static_cast<int64_t>(GetSizeOfDataType(dy->data_type()));
    ...
    std::unique_ptr<MemoryCopier> device_memory_copier(
        NewDefaultMemoryCopier(device_type));
    device_memory_copier->Copy(ctx->device_ctx(), dx->mut_dptr<T>(),
                               dy->dptr<T>(), reduced_memory_copy_nd_desc);
  }
  bool AlwaysComputeWhenAllOutputsEmpty() const override { return false; }
};
