...
static inline __device__ void GPUAtomicAddNoReturn(double* address, double val) {
...
__device__ __forceinline__ void FastSpecializedAtomicAdd(scalar_t* tensor, index_t index,
                                                         const index_t numel, scalar_t value) {
  GPUAtomicAddNoReturn(tensor + index, value);
}

template<class scalar_t, class index_t>
__device__ __forceinline__ void FastAtomicAdd(scalar_t* tensor, index_t index, const index_t numel,
...
#if defined(__CUDACC__)
    FastAtomicAdd(data, NC_offset + d * sD + h * sH + w * sW, memory_span, delta, true);
#else
