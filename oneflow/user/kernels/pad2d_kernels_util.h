...
template <typename T> struct DeviceAdd {
...
OF_DEVICE_FUNC void
DoReflectionPad2d(const IN_T *src, IN_T *dest,
                  const NdIndexOffsetHelper<int64_t, 4> &index_helper,
  ...
  XPU_1D_KERNEL_LOOP(k, elem_num) {
    ...
    ip_y = ip_y - pad_top;
    int64_t dest_index =
        n * dest_num + c * y_width * y_height + i * y_width + j;
    int64_t src_index =
        n * src_num + c * x_width * x_height + ip_y * x_width + ip_x;
...
template <typename IN_T>
OF_DEVICE_FUNC void
DoReflectionPad2dGrad(const IN_T *src, IN_T *dest,
                      const NdIndexOffsetHelper<int64_t, 4> &index_helper,
                      int64_t elem_num, int64_t src_num, int64_t dest_num,
  ...
  XPU_1D_KERNEL_LOOP(k, elem_num) {
    ...
    int64_t src_index =
        n * src_num + c * dy_width * dy_height + i * dy_width + j;
    int64_t dest_index =
...
#define INSTANTIATE_REFLECTION_PAD2D_FUNCTOR(device_type_v, dtype_pair)        \
  template struct ReflectionPad2dFunctor<device_type_v,                        \
                                         OF_PP_PAIR_FIRST(dtype_pair)>;

#define INSTANTIATE_REFLECTION_PAD2D_GRAD_FUNCTOR(device_type_v, dtype_pair)   \
  template struct ReflectionPad2dGradFunctor<device_type_v,                    \
                                             OF_PP_PAIR_FIRST(dtype_pair)>;
...
template <typename IN_T>
OF_DEVICE_FUNC void
DoReplicationPad2d(const IN_T *src, IN_T *dest,
                   const NdIndexOffsetHelper<int64_t, 4> &index_helper,
                   int64_t elem_num, int64_t src_num, int64_t dest_num,
                   int64_t y_height, int64_t y_width, int64_t x_height,
                   int64_t x_width, int64_t pad_left, int64_t pad_top) {
...
template <typename IN_T>
OF_DEVICE_FUNC void
DoReplicationPad2dGrad(const IN_T *src, IN_T *dest,
                       const NdIndexOffsetHelper<int64_t, 4> &index_helper,
                       int64_t elem_num, int64_t src_num, int64_t dest_num,
                       int64_t dy_height, int64_t dy_width, int64_t dx_height,
                       int64_t dx_width, int64_t pad_left, int64_t pad_top) {
...
#define INSTANTIATE_REPLICATION_PAD2D_FUNCTOR(device_type_v, dtype_pair)       \
  template struct ReplicationPad2dFunctor<device_type_v,                       \
                                          OF_PP_PAIR_FIRST(dtype_pair)>;

#define INSTANTIATE_REPLICATION_PAD2D_GRAD_FUNCTOR(device_type_v, dtype_pair)  \
  template struct ReplicationPad2dGradFunctor<device_type_v,                   \
                                              OF_PP_PAIR_FIRST(dtype_pair)>;
