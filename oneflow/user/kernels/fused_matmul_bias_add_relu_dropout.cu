...
__global__ RETURN_VOID_IF_FLOAT FusedReluDropoutGpu(
    ...
    T* y, const T* tail_x, bool* tail_mask, T* tail_y) {
    ...
    for (int64_t linear_index = global_thread_id * pack_size, step=gridDim.x * blockDim.x * pack_size; linear_index < elem_cnt;
        linear_index += step) {
...
template<typename T, int pack_size, bool tail>
__global__ RETURN_VOID_IF_HALF FusedReluDropoutGpu(
    uint64_t seed, one::CUDAGeneratorState* cuda_gen_state, uint64_t inc_offset,
    const int64_t elem_cnt, float rate, float scale, int64_t n_tail, const T* x, bool* mask,
    T* y, const T* tail_x, bool* tail_mask, T* tail_y) {
    int32_t global_thread_id = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    for (int64_t linear_index = global_thread_id * pack_size, step = gridDim.x * blockDim.x * pack_size; 
    ...
    if (tail && global_thread_id < n_tail) {
        // relu
        T tail_x_val = tail_x[global_thread_id]; 
        tail_out = tail_x_val > zero_val ? tail_x_val : zero_val; 
        // dropout
        const float rand_uniform = curand_uniform(&state);
        const bool mask_val = rand_uniform > rate;
        tail_mask[global_thread_id] = mask_val;
...
template<typename T, int pack_size, bool tail>
__global__ RETURN_VOID_IF_DOUBLE FusedReluDropoutGpu(
    uint64_t seed, one::CUDAGeneratorState* cuda_gen_state, uint64_t inc_offset,
    const int64_t elem_cnt, float rate, float scale, int64_t n_tail, const T* x, bool* mask,
    T* y, const T* tail_x, bool* tail_mask, T* tail_y) {
    int32_t global_thread_id = blockIdx.x * blockDim.x + threadIdx.x;
...
template<typename T>
void LaunchFusedReluDropoutKernel(ep::CudaStream* stream, uint64_t seed, one::CUDAGeneratorState* cuda_gen_state,
  ...
  if (tail) {
    ...
    FusedReluDropoutGpu<T, pack_size, true>
    ...
    FusedReluDropoutGpu<T, pack_size, false>
        <<<grid_size, kBlockSize, 0, stream->cuda_stream()>>>(
            seed, cuda_gen_state, inc_offset, elem_cnt, rate, scale, n_tail, x, mask, y,
 ...
 private:
  ...
  void Compute(user_op::KernelComputeContext* ctx, user_op::OpKernelState* dropout_state,
    ...
    */
    ...
    const user_op::Tensor* tmp_buffer = ctx->Tensor4ArgNameAndIndex("tmp_buffer", 0);
    ...
    for (int idx = 0; idx < weight_size; idx++) {
      ...
      if (idx == weight_size - 1) {
        tmp_relu_dropout_out_buf = ctx->Tensor4ArgNameAndIndex("out", 0)->mut_dptr<T>();
      } else {
      ...
      OF_CUBLAS_CHECK(cublasLtMatmul(
          ...
          matmul_cache->cublas_a_desc, in_buf_ptr, matmul_cache->cublas_b_desc, &sp_beta, matmul_out_ptr,
          matmul_cache->cublas_c_desc, matmul_out_ptr, matmul_cache->cublas_c_desc, nullptr,
  ...
  REGISTER_USER_KERNEL("fused_matmul_bias_add_relu_dropout")                             \
      ...
      .SetInferTmpSizeFn([](user_op::InferContext* ctx){ \
        ...
        for(int i = 0; i < weight_size - 1; i++){               \
            ...
            size_t tmp_size += GetCudaAlignedSize(batchsize * out_feature * sizeof(cpp_type)); \
        }                                                       \
        return tmp_size; \
      });
