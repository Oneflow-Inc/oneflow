    ...
    for (int64_t i = 0; i < 4; i++) {
      coefficients[i] = cubic_interp1d<T>(
          upsample_get_value_bounded<T>(
              in_dptr, in_width, in_height, input_x - 1, input_y - 1 + i),
          upsample_get_value_bounded<T>(
              in_dptr, in_width, in_height, input_x + 0, input_y - 1 + i),
          upsample_get_value_bounded<T>(
              in_dptr, in_width, in_height, input_x + 1, input_y - 1 + i),
          upsample_get_value_bounded<T>(
              in_dptr, in_width, in_height, input_x + 2, input_y - 1 + i),
          t_x);
    }
    out_dptr[index] = cubic_interp1d<T>(coefficients[0], coefficients[1], coefficients[2], coefficients[3], t_y);
  }
}
