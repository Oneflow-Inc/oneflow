...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    const Shape flat_out_shape =
        Shape({out->shape().Count(0, axis), out->shape().At(axis),
               out->shape().Count(axis + 1)});
    BatchGatherKernelUtilImpl<device_type, T, K>::Forward(
        ctx->device_ctx(), in->dptr<T>(), indices->dptr<K>(), flat_out_shape,
  ...
  REGISTER_USER_KERNEL("batch_gather")                                         \
      ...
      .SetIsMatchedHob(                                                        \
          ...
          (user_op::HobDataType("indices", 0) ==                               \
           OF_PP_PAIR_SECOND(indices_dtype)));
