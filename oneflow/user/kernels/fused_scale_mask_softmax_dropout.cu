...
void LaunchBroadcastForwardKernel(cudaStream_t stream, const T* x, T* y, T* softmax_y, const MASK* mask,
  ...
  if (elem_cnt < GetMaxVal<int32_t>()) {
    ...
    params.fill = fill;
    params.scale = scale;
    BroadcastScaleMaskLoad<T, ComputeType, MASK, num_dims, int64_t> load(x, mask, params);
    // DropoutStore<ComputeType, T> store(y, softmax_y, dropout_mask, cols, dropout_scale);
    cuda::softmax::DirectStore<ComputeType, T> store(y, cols);
...
void LaunchElementwiseForwardKernel(cudaStream_t stream, const T* x, T* y, T* softmax_y, const MASK* mask,
                         const int8_t* dropout_mask, const int64_t rows,
                         const int64_t cols, const float fill, const float scale,
...
void LaunchBroadcastBackwardKernel(cudaStream_t stream, const T* softmax_y, const T* dy, T* dx,
  ...
  if (elem_cnt < GetMaxVal<int32_t>()) {
    ...
    BroadcastScaleMaskStore<ComputeType, T, MASK, num_dims, int32_t> store(dx, mask, params);
    ...
    cuda::softmax::DirectLoad<T, ComputeType> load_softmax_y(softmax_y, cols);
    DropoutLoad<T, ComputeType> load_dy(dy, dropout_mask, cols, dropout_scale);
    BroadcastScaleMaskStore<ComputeType, T, MASK, num_dims, int64_t> store(dx, mask, params);
    OF_CUDA_CHECK((cuda::softmax::DispatchSoftmaxGrad<decltype(load_softmax_y), decltype(load_dy),
                                                      decltype(store), ComputeType>(
        stream, load_softmax_y, load_dy, store, rows, cols)));
  }
...
void LaunchElementwiseBackwardKernel(cudaStream_t stream, const T* softmax_y, const T* dy, T* dx,
                          const MASK* mask, const int8_t* dropout_mask, 
                          const int64_t rows, const int64_t cols, const float fill,
 ...
 private:
  ...
  void Compute(user_op::KernelComputeContext* ctx) const override {
    ...
    if (simplified_num_dims == 1) {
      ...
      // // template<typename T, typename ComputeType, typename MASK>
      LaunchElementwiseForwardKernel<T, ComputeType, MASK>(ctx->stream()->As<ep::CudaStream>()->cuda_stream(), x->dptr<T>(), y->mut_dptr<T>(),
        softmax_y->mut_dptr<T>(), mask->dptr<MASK>(), dropout_mask->dptr<int8_t>(), 
        rows, cols, mask_fill_value, scale_value, dropout_scale_value); 
    }

    // LaunchBroadcastForwardKernel(cudaStream_t stream, const T* x, T* y, T* softmax_y, const MASK* mask,
    //   const int8_t* dropout_mask, const int64_t elem_cnt, const int64_t rows,
    //   const int64_t cols, const float fill, const float scale,
    //   const float dropout_scale, const int64_t* input_dims,
    //   const int64_t* mask_dims)
...
#define DEFINE_ONE_ELIF(dims)                                                                 \
    ...
    LaunchBroadcastForwardKernel<T, ComputeType, MASK, dims>(                                          \
        ctx->stream()->As<ep::CudaStream>()->cuda_stream(), x->dptr<T>(), y->mut_dptr<T>(),   \
        softmax_y->mut_dptr<T>(), mask->dptr<MASK>(), dropout_mask->dptr<int8_t>(), elem_cnt, \
        rows, cols, mask_fill_value, scale_value,     \
        dropout_scale_value, simplified_input_dims, simplified_mask_dims);                      \
  }
    DEFINE_ONE_ELIF(2)
    // DEFINE_ONE_ELIF(3)
    // DEFINE_ONE_ELIF(4)
