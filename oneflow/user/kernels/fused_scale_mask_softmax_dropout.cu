...
template<typename T, typename ComputeType, typename MASK, size_t num_dims>
void LaunchBackwardKernel(cudaStream_t stream, const T* softmax_y, const T* dy, T* dx, const MASK* mask, 
                          const int8_t* dropout_mask, const int64_t rows, const int64_t cols,
                          const float fill, const float scale, const float dropout_scale,
                          const int64_t* input_dims, const int64_t* mask_dims) {
  NdIndexOffsetHelper<int64_t, num_dims> input_index_helper(input_dims);
  ...
  void Compute(user_op::KernelComputeContext* ctx) const override {
    ...
    if (num_dims == 2) {
      LaunchForwardKernel<T, ComputeType, MASK, 2>(ctx->stream()->As<ep::CudaStream>()->cuda_stream(), 
          x->dptr<T>(), y->mut_dptr<T>(), softmax_y->mut_dptr<T>(), 
          mask->dptr<MASK>(), dropout_mask->dptr<int8_t>(), rows, cols, 
  ...
  void Compute(user_op::KernelComputeContext* ctx) const override {
    ...
    if (num_dims == 2) {
      LaunchBackwardKernel<T, ComputeType, MASK, 2>(ctx->stream()->As<ep::CudaStream>()->cuda_stream(),
          softmax_y->dptr<T>(), dy->dptr<T>(), dx->mut_dptr<T>(), 
          mask->dptr<MASK>(), dropout_mask->dptr<int8_t>(), rows, cols, 
