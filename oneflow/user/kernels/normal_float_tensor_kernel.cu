...
}  // namespace CudaNormalFloatTensorKernel
...
class CudaNormalFloatTensorKernel final : public user_op::OpKernel {
 ...
 private:
  void Compute(user_op::KernelComputeContext* ctx, user_op::OpKernelState* state,
    ...
    OF_CUDA_CHECK((cuda::elementwise::Binary(CudaNormalFloatTensorFunctor<T>(static_cast<T>(mean)), elem_cnt, out_dptr,
                                        out_dptr , std->dptr<T>(), ctx->stream()->As<ep::CudaStream>()->cuda_stream())));
