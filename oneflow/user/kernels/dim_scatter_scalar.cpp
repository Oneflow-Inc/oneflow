...
class CpuDimScatterScalarUpdateKernel final : public user_op::OpKernel { 
  ...
  ~CpuDimScatterScalarUpdateKernel() override = default;                                              
...
template<DeviceType device_type, typename IN_T, typename IDX_T>                                 \
class CpuDimScatterScalarAddKernel final : public user_op::OpKernel { 
  public:                                                                                        
  CpuDimScatterScalarAddKernel() = default;                                                        
  ~CpuDimScatterScalarAddKernel() override = default;                                              
  ...
  void Compute(KernelComputeContext* ctx) const override {
    ...
    ScatterScalarAddFunctor<IN_T, IDX_T>(idx_nd_helper, output_nd_helper, ndim,
           index_tensor->shape().elem_cnt(), dim, upper_bound, index, src_scalar, output);
    }
...
#define REGISTER_CPU_SCATTERSCALAR_KERNEL(device, dtype, itype)                                       \
  ...
  REGISTER_USER_KERNEL("dim_scatter_scalar_add")                                                        \
      .SetCreateFn<CpuDimScatterScalarAddKernel<device, dtype, itype>>()                                    \
      .SetIsMatchedHob((user_op::HobDeviceTag() == device)                               \
                       & (user_op::HobDataType("input", 0) == GetDataType<dtype>::value) \
                       & (user_op::HobDataType("index", 0) == GetDataType<itype>::value));
