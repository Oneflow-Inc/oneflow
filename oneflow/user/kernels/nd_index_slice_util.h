...
template <typename T, typename I> struct NdIndexSliceArgs {
...
inline NdIndexSliceArgs<T, I>
ConstructNdIndexSliceArgs(const user_op::Tensor &dense,
  ...
  FOR_RANGE(int64_t, i, 0, dense.shape().NumAxes()) {
    args.dense_shape[i] = dense.shape().At(i);
  }
  return args;
}
...
template <typename I>
OF_DEVICE_FUNC int64_t OffsetInSliceToOffsetInDense(int64_t slice_size,
                                                    int64_t index_ndims,
...
template <typename T, typename I>
OF_DEVICE_FUNC void DoGatherNd(int64_t elem_cnt, int64_t slice_size,
                               int64_t index_ndims, const int64_t *dense_shape,
                               const I *indices, const T *dense, T *slices) {
  XPU_1D_KERNEL_LOOP(i, elem_cnt) {
    int64_t offset = OffsetInSliceToOffsetInDense(slice_size, index_ndims,
                                                  dense_shape, indices, i);
    slices[i] = dense[offset];
  }
}

template <DeviceType device_type, typename T> struct DeviceAdd {
  OF_DEVICE_FUNC static void Invoke(const T *x, T *y) { *y += *x; }
};

template <DeviceType device_type, typename T, typename I>
OF_DEVICE_FUNC void DoScatterNdAdd(int64_t elem_cnt, int64_t slice_size,
                                   int64_t index_ndims,
...
template <typename T, typename I>
OF_DEVICE_FUNC void
DoZeroByNdIndex(int64_t elem_cnt, int64_t slice_size, int64_t index_ndims,
...
#define INSTANTIATE_SCATTER_ND_ADD_FUNCTOR(device_type_v, dtype_pair,          \
                                           itype_pair)                         \
  template struct ScatterNdAddFunctor<device_type_v,                           \
                                      OF_PP_PAIR_FIRST(dtype_pair),            \
                                      OF_PP_PAIR_FIRST(itype_pair)>;

#define INSTANTIATE_ZERO_BY_ND_INDEX_FUNCTOR(device_type_v, dtype_pair,        \
                                             itype_pair)                       \
  template struct ZeroByNdIndexFunctor<device_type_v,                          \
                                       OF_PP_PAIR_FIRST(dtype_pair),           \
                                       OF_PP_PAIR_FIRST(itype_pair)>;

#define INSTANTIATE_ND_INDEX_SLICE_FUNCTORS(device_type_v, dtype_pair,         \
                                            itype_pair)                        \
