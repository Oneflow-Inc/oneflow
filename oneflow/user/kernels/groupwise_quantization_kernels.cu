...
template<size_t pack_size>
struct LoadCast<uint8_t, half, pack_size, 4, typename std::enable_if<pack_size % 8 == 0, void>::type> {
...
#pragma unroll
    for (int i = 0; i < pack_size / 8; ++i) {
      ...
      const uint32_t lsb4_8   = src.elem[i] >> 4;
      const uint32_t lsb8_12  = src.elem[i] >> 8;
      const uint32_t lsb12_16 = src.elem[i] >> 12;
      ...
      asm volatile("sub.f16x2 %0, %1, %2;\n" : "=r"(u32_h2[1].u32) : "r"(u32_h2[1].u32), "r"(I4s_TO_F16s_MAGIC_NUM));
      asm volatile("sub.f16x2 %0, %1, %2;\n" : "=r"(u32_h2[2].u32) : "r"(u32_h2[2].u32), "r"(I4s_TO_F16s_MAGIC_NUM));
      asm volatile("sub.f16x2 %0, %1, %2;\n" : "=r"(u32_h2[3].u32) : "r"(u32_h2[3].u32), "r"(I4s_TO_F16s_MAGIC_NUM));
