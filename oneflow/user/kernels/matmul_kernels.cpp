...
std::tuple<int32_t, int32_t, int32_t> CalcMNK(const ShapeView &a_shape,
                                              const ShapeView &out_shape,
                                              CBLAS_TRANSPOSE transpose_a) {
  int32_t num_axes = a_shape.NumAxes();
  int m = out_shape.At(num_axes - 2);
  int n = out_shape.At(num_axes - 1);
  int k = transpose_a == CblasTrans ? a_shape.At(num_axes - 2)
                                    : a_shape.At(num_axes - 1);
...
REGISTER_FUNCTION_CONFIG_DEF()
    .Bool("enable_float_compute_for_half_gemm", false,
          "true means that the type of intermedia value is float when compute "
          "half gemm");
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    CBLAS_TRANSPOSE trans_a =
        ctx->Attr<bool>("transpose_a") ? CblasTrans : CblasNoTrans;
    CBLAS_TRANSPOSE trans_b =
        ctx->Attr<bool>("transpose_b") ? CblasTrans : CblasNoTrans;
    ...
    NewKernelUtil<device_type>::OFGemm(
        ctx->device_ctx(), trans_a, trans_b, m, n, k, GetOneVal<T>(),
        a->dptr<T>(), b->dptr<T>(), GetZeroVal<T>(), out->mut_dptr<T>());
  }
};
  ...
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    if (ctx->job_desc().Bool("enable_float_compute_for_half_gemm")) {
      NewKernelUtil<DeviceType::kGPU>::OFHGemmWithFloat(
          ctx->device_ctx(), trans_a, trans_b, m, n, k, GetOneVal<float>(),
          a->dptr<float16>(), b->dptr<float16>(), GetZeroVal<float>(),
          out->mut_dptr<float16>());
    } else {
      NewKernelUtil<DeviceType::kGPU>::OFGemm(
          ctx->device_ctx(), trans_a, trans_b, m, n, k, GetOneVal<float16>(),
          a->dptr<float16>(), b->dptr<float16>(), GetZeroVal<float16>(),
          out->mut_dptr<float16>());
    }
  }
};
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    NewKernelUtil<device_type>::OFBatchedGemm(
        ctx->device_ctx(), trans_a, trans_b, batch_size, m, n, k,
        GetOneVal<T>(), a->dptr<T>(), b->dptr<T>(), GetZeroVal<T>(),
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    size_t batch_size = a->shape().Count(0, num_axes - 2);
    float16 **buf_dptr =
        reinterpret_cast<float16 **>(tmp_buf->mut_dptr<void>());
    if (ctx->job_desc().Bool("enable_float_compute_for_half_gemm")) {
      NewKernelUtil<DeviceType::kGPU>::OFBatchedHGemmWithFloat(
          ctx->device_ctx(), trans_a, trans_b, batch_size, m, n, k,
          GetOneVal<float>(), a->dptr<float16>(), b->dptr<float16>(),
          GetZeroVal<float>(), out->mut_dptr<float16>(), buf_dptr);
    } else {
      NewKernelUtil<DeviceType::kGPU>::OFBatchedGemm(
          ctx->device_ctx(), trans_a, trans_b, batch_size, m, n, k,
          GetOneVal<float16>(), a->dptr<float16>(), b->dptr<float16>(),
          GetZeroVal<float16>(), out->mut_dptr<float16>(), buf_dptr);
    }
  }
};
