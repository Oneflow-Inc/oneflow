...
class BroadcastMatmulKernel final : public user_op::OpKernel, public user_op::CudaGraphSupport {
 ...
 private:
  void Compute(user_op::KernelComputeContext* ctx) const override {
    ...
    broadcast_matmul->Launch(ctx->stream(), alpha, a_num_axes, a->shape_view().ptr(),
                             a->dptr(), b_num_axes, b->shape_view().ptr(), b->dptr(), beta,
                             out_num_axes, out->shape_view().ptr(), out->mut_dptr());
  }
};
