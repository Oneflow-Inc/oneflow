...
template <typename T>
void ComputeTopOne(const T *in_ptr, const Range &range, int32_t instance_size,
                   int32_t *out_ptr) {
  FOR_RANGE(int32_t, i, range.begin(), range.end()) {
    const T *in_ptr_i = in_ptr + i * instance_size;
    out_ptr[i] = std::distance(
        in_ptr_i, std::max_element(in_ptr_i, in_ptr_i + instance_size));
  }
}

template <typename T>
void ComputeTopK(const T *in_ptr, int32_t *indices_ptr, const Range &range,
                 int32_t instance_size, int32_t k, bool sorted,
                 int32_t *out_ptr) {
  FOR_RANGE(int32_t, i, range.begin(), range.end()) {
    ...
    std::nth_element(indices_ptr_i, indices_ptr_i + k,
                     indices_ptr_i + instance_size, comp);
    if (sorted) {
      std::sort(indices_ptr_i, indices_ptr_i + k, comp);
    }
...
template <typename T>
void CpuTopK(DeviceCtx *ctx, const T *in_ptr, int32_t *indices_ptr,
             int32_t instance_num, int32_t instance_size, int32_t k,
             bool sorted, int32_t *out_ptr) {
  const int32_t num_thread =
      std::min(instance_num, Global<ThreadPool>::Get()->thread_num());
  ...
  FOR_RANGE(int32_t, thread_id, 0, num_thread) {
    ...
    Global<ThreadPool>::Get()->AddWork([=, &bc]() {
      if (k == 1) {
        ...
        ComputeTopK(in_ptr, indices_ptr, range, instance_size, k, sorted,
                    out_ptr);
      }
      bc.Decrease();
    });
...
template <typename T> class TopKCpuKernel final : public user_op::OpKernel {
public:
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    int32_t *indices_ptr =
        tmp_buffer ? tmp_buffer->mut_dptr<int32_t>() : nullptr;
    CpuTopK(ctx->device_ctx(), in->dptr<T>(), indices_ptr, instance_num,
            instance_size, k, ctx->Attr<bool>("sorted"),
...
#define REGISTER_CPU_TOP_K_KERNEL(dtype)                                       \
  REGISTER_USER_KERNEL("top_k")                                                \
      ...
      .SetInferTmpSizeFn([](user_op::InferContext *ctx) {                      \
        ...
        return ctx->Attr<int32_t>("k") > 1                                     \
                   ? in_shape->elem_cnt() * sizeof(int32_t)                    \
                   : 0;                                                        \
      });
