...
class ReduceDeviceStageKernel final : public OpKernel {
 ...
 private:
  void Compute(KernelComputeContext* ctx) const override {
    ...
    CHECK(bcast_eq);
    bcast_eq->Launch(ctx->stream(), in->shape_view().NumAxes(), in->shape_view().ptr(),
                     in->dptr(), out->shape_view().NumAxes(), out->shape_view().ptr(),
...
class ReduceGlobalStageKernel final : public OpKernel {
 ...
 private:
  void Compute(KernelComputeContext* ctx) const override {
    ...
    CHECK(bcast_eq);
    bcast_eq->Launch(ctx->stream(), in->shape_view().NumAxes(), in->shape_view().ptr(),
                     in->dptr(), reduced_shape.NumAxes(), reduced_shape.ptr(), out->dptr(),
                     mask->mut_dptr());
  }
  bool AlwaysComputeWhenAllOutputsEmpty() const override { return false; }
};
