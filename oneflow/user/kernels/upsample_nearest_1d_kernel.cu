 ...
 private:
  void Compute(user_op::KernelComputeContext* ctx) const override {
    ...
    NdIndexOffsetHelper<int64_t, 3> out_helper(y_blob->shape().At(0), y_blob->shape().At(1),
                                               y_blob->shape().At(2));
    RUN_CUDA_KERNEL((UpsampleNearest1DForward<T>), ctx->device_ctx(), elem_cnt, elem_cnt, x_blob->dptr<T>(), in_helper, out_helper,
 ...
 private:
  void Compute(user_op::KernelComputeContext* ctx) const override {
    ...
    NdIndexOffsetHelper<int64_t, 3> dx_helper(dx_blob->shape().At(0), dx_blob->shape().At(1),
                                              dx_blob->shape().At(2));
    RUN_CUDA_KERNEL((UpsampleNearest1DBackward<T>), ctx->device_ctx(), elem_cnt, elem_cnt, dy_blob->dptr<T>(), dy_helper, dx_helper,
