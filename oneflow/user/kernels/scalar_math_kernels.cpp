...
template<typename Context>
std::unique_ptr<ep::primitive::BroadcastElementwiseBinary> NewBroadcastElementwiseAttrBinaryPrimitive(
    Context* ctx, ep::primitive::BinaryOp op) {
  const user_op::TensorDesc* x = ctx->TensorDesc4ArgNameAndIndex("in", 0);
  const user_op::TensorDesc* y = ctx->TensorDesc4ArgNameAndIndex("out", 0);
  const int64_t ndims = y->shape().NumAxes();
  ...
  return ep::primitive::NewPrimitive<ep::primitive::BroadcastElementwiseBinaryFactory>(
      ctx->device_type(), op, x->data_type(), y->data_type(), ndims, value);
}
...
class ScalarPowGradKernel final : public user_op::OpKernel {
 ...
 private:
  void Compute(user_op::KernelComputeContext* ctx) const override {
    ...
    if (elem_cnt != 0) {
      std::unique_ptr<ep::primitive::BroadcastElementwiseBinary> primitive =
          NewBroadcastElementwiseAttrBinaryPrimitive(ctx, ep::primitive::BinaryOp::kPowGrad);
      CHECK(primitive);
      primitive->Launch(ctx->stream(), x_tensor->shape_view().NumAxes(), x_tensor->shape_view().ptr(),
                        x_tensor->dptr(), dy_tensor->shape_view().NumAxes(), dy_tensor->shape_view().ptr(),
...
REGISTER_BINARY_MATH_WITH_ATTR_ELEMWISE_USER_KERNEL("scalar_pow_grad", ep::primitive::BinaryOp::kPowGrad); 
