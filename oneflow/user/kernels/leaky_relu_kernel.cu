...
template <typename T>
__global__ void LeakyReluForwardGpu(const int n, const float alpha, const T *x,
                                    T *y) {
  CUDA_1D_KERNEL_LOOP(i, n) { y[i] = x[i] > 0 ? x[i] : x[i] * alpha; }
}

template <typename T>
__global__ void LeakyReluBackwardGpu(const int n, const float alpha, const T *x,
                                     const T *dy, T *dx) {
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    RUN_CUDA_KERNEL((LeakyReluForwardGpu<T>), ctx->device_ctx(), elem_cnt,
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    const int32_t elem_cnt = x->shape().elem_cnt();
    const float alpha = ctx->Attr<float>("alpha");
    RUN_CUDA_KERNEL((LeakyReluBackwardGpu<T>), ctx->device_ctx(), elem_cnt,
                    elem_cnt, alpha, x->dptr<T>(), dy->dptr<T>(),
                    dx->mut_dptr<T>());
  }
  bool AlwaysComputeWhenAllOutputsEmpty() const override { return false; }
};
