...
namespace {
  ...
  __global__ void RmsPropUpdateGpu(int64_t n, T scale, float l1, float l2, T* mean_square, T* mean_gradient,
  ...
  void LarsUpdateKernelUtil<DeviceType::kGPU, T, G>::Update(
      ...
      const T* scale_by_ptr, const G* model_diff, T* model, T*data_tmp, T* model_diff_tmp) {
        if (scale_by_ptr != nullptr) { scale *= *scale_by_ptr; }
        GetModelDiffTGpu<T, G><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock, 0, ctx->cuda_stream()>>>(
  ...
  struct LarsUpdateKernelUtil<DeviceType::kGPU, T, float16> {
    static void Update(DeviceCtx* ctx, int64_t n, T scale, float l1, float l2, float momentum_beta, float epsilon,
        float lars_coefficient, float weight_decay, const float* learning_rate, const int64_t* train_step, T* momentum,
        const T* scale_by_ptr, const float16* model_diff, T* model, T*data_tmp, T* model_diff_tmp);
  };
