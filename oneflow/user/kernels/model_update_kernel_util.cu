...
__global__ void LambGradGpu(int64_t n, T scale, float l1, float l2, float beta1, float beta2,
                            float epsilon, const T* scale_by_ptr, const int64_t* skip_if, const G* model_diff,
...
template<typename T>
__global__ void LambUpdateGpu(int64_t n, float weight_decay, float learning_rate_val, const float* learning_rate,
...
void LambUpdateKernelUtil<DeviceType::kGPU, T, G>::Update(
    ...
    float epsilon, float weight_decay, float learning_rate_val, const float* learning_rate, const T* scale_by_ptr,
  ...
  LambUpdateGpu<T><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock, 0, ctx->cuda_stream()>>>(
      n, weight_decay, learning_rate_val, learning_rate, skip_if, w_norm, g_norm, adam_diff, model);
}
...
void LambUpdateKernelUtil<DeviceType::kGPU, T, float16>::Update(
  ...
  LambUpdateKernelUtil<DeviceType::kGPU, T, half>::Update(
      ctx, n, scale, l1, l2, beta1, beta2, epsilon, weight_decay, learning_rate_val, learning_rate, scale_by_ptr,
