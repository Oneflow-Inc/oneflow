...
bool operator&(FlipCode lhs, FlipCode rhs) {
  return static_cast<bool>(
      static_cast<std::underlying_type<FlipCode>::type>(lhs) &
...
template <typename T>
void FlipBoxes(TensorBuffer *boxes_buffer, int32_t image_width,
               int32_t image_height, FlipCode flip_code) {
...
template <typename T>
void FlipPolygons(TensorBuffer *polygons_buffer, int32_t image_width,
                  int32_t image_height, FlipCode flip_code) {
...
template <typename T>
void ImageNormalizeByChannel(TensorBuffer *image_buffer,
  ...
  FOR_RANGE(int, i, 0, (h * w)) {
    ...
    FOR_RANGE(int, j, 0, c) {
      image_data[j] = (image_data[j] - mean_vec.at(j)) / std_vec.at(j);
    }
  }
}

#define MAKE_IMAGE_NORMALIZE_SWITCH_ENTRY(func_name, T) func_name<T>
DEFINE_STATIC_SWITCH_FUNC(void, ImageNormalizeByChannel,
                          MAKE_IMAGE_NORMALIZE_SWITCH_ENTRY,
                          MAKE_DATA_TYPE_CTRV_SEQ(FLOATING_DATA_TYPE_SEQ));

#undef MAKE_IMAGE_NORMALIZE_SWITCH_ENTRY

template <typename T, typename I>
void PolygonsToMask(const TensorBuffer &polys,
                    const TensorBuffer &polys_nd_index, TensorBuffer *masks,
  ...
  masks->Resize(Shape({static_cast<int64_t>(mask_mat_vec.size()),
                       static_cast<int64_t>(im_h), static_cast<int64_t>(im_w)}),
                DataType::kInt8);
  int mask_idx = 0;
  for (const auto &mask_mat : mask_mat_vec) {
    CHECK(mask_mat.isContinuous());
    CHECK_EQ(mask_mat.total(), im_h * im_w);
    memcpy(masks->mut_data<int8_t>() + mask_idx * im_h * im_w,
           mask_mat.ptr<int8_t>(), mask_mat.total() * sizeof(int8_t));
    mask_idx += 1;
  }
}

#define MAKE_POLYGONS_TO_MASK_SWITCH_ENTRY(func_name, T, I) func_name<T, I>
DEFINE_STATIC_SWITCH_FUNC(void, PolygonsToMask,
                          MAKE_POLYGONS_TO_MASK_SWITCH_ENTRY,
                          MAKE_DATA_TYPE_CTRV_SEQ(FLOATING_DATA_TYPE_SEQ),
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    const user_op::Tensor *flip_code_tensor =
        ctx->Tensor4ArgNameAndIndex("flip_code", 0);
    ...
    MultiThreadLoop(num_images, [&](size_t i) {
      ...
      FlipCode flip_code =
          static_cast<FlipCode>(flip_code_tensor->dptr<int8_t>()[i]);
      if (flip_code != FlipCode::kNonFlip) {
        FlipImage(out_buffer, flip_code);
      }
    });
  }
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    const user_op::Tensor *image_size_tensor =
    ...
    MultiThreadLoop(num_images, [&](size_t i) {
      ...
      FlipCode flip_code =
          static_cast<FlipCode>(flip_code_tensor->dptr<int8_t>()[i]);
      SwitchFlipBoxes(SwitchCase(out_bbox_buffer->data_type()), out_bbox_buffer,
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    const user_op::Tensor *bbox_tensor = ctx->Tensor4ArgNameAndIndex("bbox", 0);
    const user_op::Tensor *scale_tensor =
        ctx->Tensor4ArgNameAndIndex("scale", 0);
    user_op::Tensor *out_tensor = ctx->Tensor4ArgNameAndIndex("out", 0);
    ...
    MultiThreadLoop(num_images, [&](size_t i) {
      ...
      SwitchScaleBoxes(SwitchCase(out_bbox_buffer->data_type()),
                       out_bbox_buffer, scale_w, scale_h);
    });
  }
  bool AlwaysComputeWhenAllOutputsEmpty() const override { return false; }
};
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    const user_op::Tensor *polygon_tensor =
        ctx->Tensor4ArgNameAndIndex("poly", 0);
    const user_op::Tensor *image_size_tensor =
        ctx->Tensor4ArgNameAndIndex("image_size", 0);
    const user_op::Tensor *flip_code_tensor =
        ctx->Tensor4ArgNameAndIndex("flip_code", 0);
    ...
    MultiThreadLoop(num_images, [&](size_t i) {
      const TensorBuffer &polygons_buffer =
          polygon_tensor->dptr<TensorBuffer>()[i];
      CHECK_EQ(polygons_buffer.shape().NumAxes(), 2);
      CHECK_EQ(polygons_buffer.shape().At(1), 2);
      TensorBuffer *out_polygons_buffer =
          out_tensor->mut_dptr<TensorBuffer>() + i;
      out_polygons_buffer->CopyFrom(polygons_buffer);
      int32_t image_width = image_size_tensor->dptr<int32_t>()[i * 2 + 0];
      int32_t image_height = image_size_tensor->dptr<int32_t>()[i * 2 + 1];
      FlipCode flip_code =
          static_cast<FlipCode>(flip_code_tensor->dptr<int8_t>()[i]);
      SwitchFlipPolygons(SwitchCase(out_polygons_buffer->data_type()),
                         out_polygons_buffer, image_width, image_height,
                         flip_code);
    });
  }
  bool AlwaysComputeWhenAllOutputsEmpty() const override { return false; }
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    MultiThreadLoop(num_images, [&](size_t i) {
      ...
      float scale_w = scale_tensor->dptr<float>()[i * 2 + 0];
      float scale_h = scale_tensor->dptr<float>()[i * 2 + 1];
      SwitchScalePolygons(SwitchCase(out_poly_buffer->data_type()),
                          out_poly_buffer, scale_w, scale_h);
    });
  }
  bool AlwaysComputeWhenAllOutputsEmpty() const override { return false; }
};
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    ...
    MultiThreadLoop(num_images, [&](size_t i) {
      ...
      TensorBuffer *out_buffer = out_tensor->mut_dptr<TensorBuffer>() + i;
      out_buffer->CopyFrom(in_buffer);
      SwitchImageNormalizeByChannel(SwitchCase(out_buffer->data_type()),
                                    out_buffer, std_vec, mean_vec);
    });
  }
  bool AlwaysComputeWhenAllOutputsEmpty() const override { return false; }
};
...
private:
  void Compute(user_op::KernelComputeContext *ctx) const override {
    const user_op::Tensor *poly_tensor = ctx->Tensor4ArgNameAndIndex("poly", 0);
    const user_op::Tensor *poly_index_tensor =
        ctx->Tensor4ArgNameAndIndex("poly_index", 0);
    const user_op::Tensor *image_size_tensor =
        ctx->Tensor4ArgNameAndIndex("image_size", 0);
    ...
    MultiThreadLoop(num_images, [&](size_t i) {
      const TensorBuffer &poly_buffer = poly_tensor->dptr<TensorBuffer>()[i];
      const TensorBuffer &poly_index_buffer =
          poly_index_tensor->dptr<TensorBuffer>()[i];
      int32_t image_width = image_size_tensor->dptr<int32_t>()[i * 2 + 0];
      int32_t image_height = image_size_tensor->dptr<int32_t>()[i * 2 + 1];
      TensorBuffer *mask_buffer = mask_tensor->mut_dptr<TensorBuffer>() + i;
      SwitchPolygonsToMask(
          SwitchCase(poly_buffer.data_type(), poly_index_buffer.data_type()),
          poly_buffer, poly_index_buffer, mask_buffer, image_width,
          image_height);
    });
  }
  bool AlwaysComputeWhenAllOutputsEmpty() const override { return false; }
...
std::function<Maybe<void>(const user_op::InferContext &,
                          user_op::AddInplaceArgPair)>
MakeInplaceProposalFn(const std::string &input_arg_name) {
  return [input_arg_name](
