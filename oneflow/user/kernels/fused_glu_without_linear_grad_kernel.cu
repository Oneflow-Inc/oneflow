...
template<typename T, typename IndexType, ep::primitive::BinaryOp d_act_type, ep::primitive::UnaryOp act_type, int32_t pack_size>
__global__ void FusedGluWithoutLinearGradGpu(
    const IndexType m, const IndexType packed_n, 
    const IndexType pack_num, const IndexType packed_stride,
    ...
    for(IndexType packed_index = global_thread_id, step = gridDim.x * blockDim.x;
      packed_index < pack_num; packed_index += step){
        ...
        const LoadPack* dy_load = reinterpret_cast<const LoadPack*>(dy)
                                     + (packed_row * packed_n + packed_col);
        const LoadPack* matmul_wx_load = reinterpret_cast<const LoadPack*>(matmul_wx)
                                     + (packed_row * packed_stride + packed_col);
...
#pragma unroll
        for (int i = 0; i < pack_size; i++) {
          ...
          if constexpr (d_act_type != ep::primitive::BinaryOp::kAdd){
            d_matmul_vx_vec.elem[i] = dact(d_act_gate, gate); // d_gate
          } else {
            d_matmul_vx_vec.elem[i] = d_act_gate;
          }
        }
        *(reinterpret_cast<LoadPack*>(d_matmul_wx) + (packed_row * packed_stride + packed_col)) = d_matmul_wx_vec;
        *(reinterpret_cast<LoadPack*>(d_matmul_vx) + (packed_row * packed_stride + packed_col)) = d_matmul_vx_vec;
    }
}

template<typename T, typename IndexType, ep::primitive::UnaryOp act_type, ep::primitive::BinaryOp d_act_type, int32_t pack_size>
void LaunchFusedGluWithoutLinearGradGpu(ep::Stream* stream, const IndexType m, const IndexType packed_n, 
  const int64_t pack_num, const IndexType packed_stride,
  const T* dy, const T* matmul_wx, const T* matmul_vx, 
  T* d_matmul_wx, T* d_matmul_vx
){ 
  ...
  FusedGluWithoutLinearGradGpu<T, IndexType, d_act_type, act_type, pack_size>
    <<<grid_size, block_size, 0, stream->As<ep::CudaStream>()->cuda_stream()>>>(
      m, packed_n, pack_num, packed_stride, dact, act,
      dy, matmul_wx, matmul_vx, d_matmul_wx, d_matmul_vx
  );
}

template<typename T, ep::primitive::UnaryOp act_type, ep::primitive::BinaryOp d_act_type, int32_t pack_size>
void DispatchIndexType(ep::Stream* stream, const int64_t m, const int64_t packed_n, 
  ...
  if(pack_num < (1 << 30)){
    LaunchFusedGluWithoutLinearGradGpu<T, int32_t, act_type, d_act_type, pack_size>(stream, m, packed_n, pack_num, packed_stride, dy, matmul_wx, matmul_vx, d_matmul_wx, d_matmul_vx);
  } else {
    LaunchFusedGluWithoutLinearGradGpu<T, int64_t, act_type, d_act_type, pack_size>(stream, m, packed_n, pack_num, packed_stride, dy, matmul_wx, matmul_vx, d_matmul_wx, d_matmul_vx);
  }
}

template<typename T, ep::primitive::UnaryOp act_type, ep::primitive::BinaryOp d_act_type, int32_t alignment>
void DispatchPackSize(ep::Stream* stream, const int64_t m, const int64_t n, 
  const int64_t stride,
  const T* dy, const T* matmul_wx, const T* matmul_vx, 
    T* d_matmul_wx, T* d_matmul_vx
){
  if constexpr (alignment/sizeof(T) > 0){
    ...
    DispatchIndexType<T, act_type, d_act_type, alignment/sizeof(T)>(stream, m, packed_n, pack_num, packed_stride, dy, matmul_wx, matmul_vx, d_matmul_wx, d_matmul_vx);
  } else {
    DispatchIndexType<T, act_type, d_act_type, 1>(stream, m, n, m*n, stride, dy, matmul_wx, matmul_vx, d_matmul_wx, d_matmul_vx);
  }
}
...
void DispatchAlignment(ep::Stream* stream, const int64_t m, const int64_t n, 
  ...
  const T* dy, const T* matmul_wx, const T* matmul_vx, 
    ...
    // dispatch alignment
    if(IsAligned(16)){
      DispatchPackSize<T, act_type, d_act_type, 16>(stream, m, n, stride, dy, matmul_wx, matmul_vx, d_matmul_wx, d_matmul_vx);
    } else if(IsAligned(8)) {
      DispatchPackSize<T, act_type, d_act_type, 8>(stream, m, n, stride, dy, matmul_wx, matmul_vx, d_matmul_wx, d_matmul_vx);
    } else if(IsAligned(4)) {
      DispatchPackSize<T, act_type, d_act_type, 4>(stream, m, n, stride, dy, matmul_wx, matmul_vx, d_matmul_wx, d_matmul_vx);
    } else if(IsAligned(2)) {
      DispatchPackSize<T, act_type, d_act_type, 2>(stream, m, n, stride, dy, matmul_wx, matmul_vx, d_matmul_wx, d_matmul_vx);
    } else {
      DispatchPackSize<T, act_type, d_act_type, 1>(stream, m, n, stride, dy, matmul_wx, matmul_vx, d_matmul_wx, d_matmul_vx);
    }
}

template<typename T>
void DispatchActivationType(ep::Stream* stream, const int64_t m, const int64_t n, 
  const std::string& activation, const int64_t stride,
  const T* dy, const T* matmul_wx, const T* matmul_vx, 
  T* d_matmul_wx, T* d_matmul_vx
) {
  if (activation == "none") {
    DispatchAlignment<T, ep::primitive::UnaryOp::kIdentity, /*fake*/ep::primitive::BinaryOp::kAdd>(stream, m, n, stride, dy, matmul_wx, matmul_vx, d_matmul_wx, d_matmul_vx);
  } else if (activation == "sigmoid") {
    DispatchAlignment<T, ep::primitive::UnaryOp::kSigmoid, ep::primitive::BinaryOp::kSigmoidBackwardWithDyY>(stream, m, n, stride, dy, matmul_wx, matmul_vx, d_matmul_wx, d_matmul_vx);
  } else if (activation == "relu") {
    DispatchAlignment<T, ep::primitive::UnaryOp::kRelu, ep::primitive::BinaryOp::kReluBackwardWithDyY>(stream, m, n, stride, dy, matmul_wx, matmul_vx, d_matmul_wx, d_matmul_vx);
  } else if (activation == "gelu") {
    DispatchAlignment<T, ep::primitive::UnaryOp::kGelu, ep::primitive::BinaryOp::kGeluBackwardWithDyX>(stream, m, n, stride, dy, matmul_wx, matmul_vx, d_matmul_wx, d_matmul_vx);
  } else if (activation == "fast_gelu") {
    DispatchAlignment<T, ep::primitive::UnaryOp::kFastGelu, ep::primitive::BinaryOp::kFastGeluBackwardWithDyX>(stream, m, n, stride, dy, matmul_wx, matmul_vx, d_matmul_wx, d_matmul_vx);
  } else if (activation == "silu") {
    DispatchAlignment<T, ep::primitive::UnaryOp::kSilu, ep::primitive::BinaryOp::kSiluBackwardWithDyX>(stream, m, n, stride, dy, matmul_wx, matmul_vx, d_matmul_wx, d_matmul_vx);
  } else {
    UNIMPLEMENTED();
  }
}
