...
std::tuple<uint64_t, dim3, dim3> calc_execution_policy(int64_t total_elements, ep::CudaStream* stream) {
...
__global__ void distribution_elementwise_grid_stride_kernel_double(int32_t numel,
                                                                    uint64_t seed,
                                                                    uint64_t offset,
                                                                    double lambd,
                                                                    double epsilon,
  ...
  int rounded_size = ((numel - 1)/(blockDim.x * gridDim.x * unroll_factor)+1) * blockDim.x * gridDim.x * unroll_factor;
  for(int32_t linear_index = idx; linear_index < rounded_size; linear_index += blockDim.x * gridDim.x * unroll_factor) {
    ...
    for (int ii = 0; ii < unroll_factor; ii++) {
      ...
      if (li < numel) {
        ...
        double log = static_cast<double>((&rand.x)[ii]) >= static_cast<double>(1.) - epsilon / 2 ? -epsilon / 2 : log_rand;
...
OF_LAUNCH_BOUNDS_2(block_size_bound, grid_size_bound)
__global__ void distribution_elementwise_grid_stride_kernel_float(int32_t numel,
                                                                    uint64_t seed,
                                                                    uint64_t offset,
                                                                    float lambd,
                                                                    float epsilon,
  ...
  for(int32_t linear_index = idx; linear_index < rounded_size; linear_index += blockDim.x * gridDim.x * unroll_factor) {
    ...
    for (int ii = 0; ii < unroll_factor; ii++) {
      ...
      if (li < numel) {
        ...
        // we need log to be not 0, and not underflow when converted to half
        // fast __logf approximation can underflow, so set log to -epsilon/2 for 1 or close to 1 args
        float log = static_cast<float>((&rand.x)[ii]) >= static_cast<float>(1.) - epsilon / 2 ? -epsilon / 2 : log_rand;
...
void ExponentialDistribution<DeviceType::kCUDA, double>::operator()(
    ...
    distribution_elementwise_grid_stride_kernel_double<<<grid, block, 0, stream->As<ep::CudaStream>()->cuda_stream()>>>(
        elem_cnt, 
        seed, 
        offset, 
        lambd_, 
...
void ExponentialDistribution<DeviceType::kCUDA, float>::operator()(
    ...
    distribution_elementwise_grid_stride_kernel_float<<<grid, block, 0, stream->As<ep::CudaStream>()->cuda_stream()>>>(
        elem_cnt, 
        seed, 
        offset, 
        lambd_, 
