...
struct DistributionFunctor<DistributionOp::kNormal4> {
  ...
  __device__ float4 operator()(GPURAND(StatePhilox4_32_10_t)* state) const {
    return GPURAND(_normal4)(state);
  }
};
...
struct DistributionFunctor<DistributionOp::kNormal2Double> {
  ...
  __device__ double2 operator()(GPURAND(StatePhilox4_32_10_t)* state) const {
    return GPURAND(_normal2_double)(state);
  }
};
...
struct DistributionFunctor<DistributionOp::kUniform4> {
  ...
  __device__ float4 operator()(GPURAND(StatePhilox4_32_10_t)* state) const {
    return GPURAND(_uniform4)(state);
  }
};
...
struct DistributionFunctor<DistributionOp::kUniform2Double> {
  ...
  __device__ double2 operator()(GPURAND(StatePhilox4_32_10_t)* state) const {
    return GPURAND(_uniform2_double)(state);
  }
};
