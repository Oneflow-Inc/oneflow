...
__global__ void UpsampleTrilinear3DBackward(const int64_t elem_cnt, const T* dy_dptr,
  ...
  CUDA_1D_KERNEL_LOOP(index, elem_cnt) {
    ...
    cuda::atomic::FastAdd(pos1, 0, static_cast<int>(elem_cnt),
                          t0lambda * h0lambda * w0lambda * pos2[0]);
    cuda::atomic::FastAdd(pos1, static_cast<int>(w1p), static_cast<int>(elem_cnt),
                          t0lambda * h0lambda * w1lambda * pos2[0]);
    cuda::atomic::FastAdd(pos1, static_cast<int>(h1p * in_width), static_cast<int>(elem_cnt),
                          t0lambda * h1lambda * w0lambda * pos2[0]);
    cuda::atomic::FastAdd(pos1, static_cast<int>(h1p * in_width + w1p), static_cast<int>(elem_cnt),
                          t0lambda * h1lambda * w1lambda * pos2[0]);
    cuda::atomic::FastAdd(pos1, static_cast<int>(t1p * in_height * in_width),
                          static_cast<int>(elem_cnt), t1lambda * h0lambda * w0lambda * pos2[0]);
    cuda::atomic::FastAdd(pos1, static_cast<int>(t1p * in_height * in_width + w1p),
                          static_cast<int>(elem_cnt), t1lambda * h0lambda * w1lambda * pos2[0]);
    cuda::atomic::FastAdd(pos1, static_cast<int>(t1p * in_height * in_width + h1p * in_width),
                          static_cast<int>(elem_cnt), t1lambda * h1lambda * w0lambda * pos2[0]);
    cuda::atomic::FastAdd(pos1, static_cast<int>(t1p * in_height * in_width + h1p * in_width + w1p),
