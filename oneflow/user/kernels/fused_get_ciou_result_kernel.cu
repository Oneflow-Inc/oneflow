...
template<typename T>
__global__ void FusedGetCiouResultBackward(const int n, const T* dy, const T* v, const T* iou,
                                           const T* rho2, const T* c2, T* dv, T* diou, T* drho2,
                                           T* dc2, float eps) {
  CUDA_1D_KERNEL_LOOP(i, n) {
    const T v_i = v[i];
    const T c2_i = c2[i];
 ...
 private:
  ...
  void Compute(user_op::KernelComputeContext* ctx) const override {
    ...
    RUN_CUDA_KERNEL((FusedGetCiouResultForward<T>), ctx->stream(), elem_cnt, elem_cnt, v->dptr<T>(),
                    iou->dptr<T>(), rho2->dptr<T>(), c2->dptr<T>(), y->mut_dptr<T>(), eps);
  }
 ...
 private:
  ...
  void Compute(user_op::KernelComputeContext* ctx) const override {
    ...
    RUN_CUDA_KERNEL((FusedGetCiouResultBackward<T>), ctx->stream(), elem_cnt, elem_cnt,
                    dy->dptr<T>(), v->dptr<T>(), iou->dptr<T>(), rho2->dptr<T>(), c2->dptr<T>(),
