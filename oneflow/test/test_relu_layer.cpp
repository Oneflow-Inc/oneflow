...
class ReLULayerTest : public ::testing::Test{
 protected:
  ReLULayerTest() {
    ...
    relu_layer = std::make_shared<ReLULayer<Dtype>>(
  ...
  std::shared_ptr<ReLULayer<Dtype>> relu_layer;
  std::shared_ptr<ReLUData<Dtype>> relu_data;
  std::shared_ptr<ReLUModel<Dtype>> relu_model;
};

TYPED_TEST_CASE(ReLULayerTest, TestDtypes);

TYPED_TEST(ReLULayerTest, TestSetup) {
...
TYPED_TEST(ReLULayerTest, TestSimpleForward) {
  typedef typename TypeParam Dtype;
...
// have no sign change
TYPED_TEST(ReLULayerTest, TestGradient) {
  typedef typename TypeParam Dtype;
