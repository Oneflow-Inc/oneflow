...
protected:
  ...
  std::shared_ptr<PoolingLayer<Dtype>> pooling_layer;
  std::shared_ptr<PoolingData<Dtype>> pooling_data;
  ...
  void TestForwardSquare() {
    ...
    pooling_layer = std::make_shared<PoolingLayer<Dtype>>(
      layer_name, proto_param->DebugString());
    pooling_layer->InitParam();
    pooling_data = std::shared_ptr<PoolingData<Dtype>>(
      dynamic_cast<PoolingData<Dtype>*>(
    ...
    EXPECT_EQ(pooling_data->idx->shape().num(), kNum);
    EXPECT_EQ(pooling_data->idx->shape().channels(), kChannels);
    EXPECT_EQ(pooling_data->idx->shape().height(), 2);
    EXPECT_EQ(pooling_data->idx->shape().width(), 4);
  ...
  void TestForwardRectHigh() {
    ...
    EXPECT_EQ(pooling_data->idx->shape().num(), kNum);
    EXPECT_EQ(pooling_data->idx->shape().channels(), kChannels);
    EXPECT_EQ(pooling_data->idx->shape().height(), 4);
    EXPECT_EQ(pooling_data->idx->shape().width(), 5);
  ...
  void TestForwardRectWide() {
    ...
    EXPECT_EQ(pooling_data->idx->shape().num(), kNum);
    EXPECT_EQ(pooling_data->idx->shape().channels(), kChannels);
    EXPECT_EQ(pooling_data->idx->shape().height(), 5);
    EXPECT_EQ(pooling_data->idx->shape().width(), 4);
...
TYPED_TEST_CASE(PoolingLayerTest, TestDtypes);
