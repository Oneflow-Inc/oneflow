...
namespace caffe {
  ...
  protected:
    ...
    void InitParameters() {
      ...
      softmax_model = std::shared_ptr<CuDNNSoftmaxModel<Dtype>>(
        dynamic_cast<CuDNNSoftmaxModel<Dtype>*>(
        softmax_layer->CreateModelParam()));
      softmax_model->AllocateEmptyBlobs();
      softmax_model->AlignBlobShapes(
        *dynamic_cast<const CuDNNSoftmaxModel<Dtype>*>(
    ...
    std::shared_ptr<CuDNNSoftmaxLayer<Dtype>> softmax_layer;
    std::shared_ptr<SoftmaxData<Dtype>> softmax_data;
    std::shared_ptr<CuDNNSoftmaxModel<Dtype>> softmax_model;
  };

  TYPED_TEST_CASE(CuDNNSoftmaxLayerTest, TestDtypes);

  TYPED_TEST(CuDNNSoftmaxLayerTest, TestSetup) {
    typedef typename TypeParam Dtype;

    softmax_layer = std::make_shared<CuDNNSoftmaxLayer<Dtype>>(
  ...
  TYPED_TEST(CuDNNSoftmaxLayerTest, TestForward) {
    typedef typename TypeParam Dtype;
  ...
  TYPED_TEST(CuDNNSoftmaxLayerTest, TestGradient) {
    typedef typename TypeParam Dtype;
