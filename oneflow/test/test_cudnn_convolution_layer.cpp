...
protected:
  ...
  std::shared_ptr<CuDNNConvolutionLayer<Dtype>> convolution_layer;
  std::shared_ptr<CuDNNConvolutionData<Dtype>> convolution_data;
  std::shared_ptr<ConvolutionModel<Dtype>> convolution_model;
};

TYPED_TEST_CASE(CuDNNConvolutionLayerTest, TestDtypes);

TYPED_TEST(CuDNNConvolutionLayerTest, TestSetup) {
  ...
  convolution_layer = std::make_shared<CuDNNConvolutionLayer<Dtype>>(
    layer_name, convolution_proto->DebugString());
  convolution_layer->InitParam();
  convolution_data = std::shared_ptr<CuDNNConvolutionData<Dtype>>(
    dynamic_cast<CuDNNConvolutionData<Dtype>*>(
  ...
  EXPECT_EQ(convolution_data->out_diff->shape().channels(), 4);
...
TYPED_TEST(CuDNNConvolutionLayerTest, TestSimpleConvolution) {
  typedef typename TypeParam Dtype;
...
TYPED_TEST(CuDNNConvolutionLayerTest, Test1x1Convolution) {
  typedef typename TypeParam Dtype;
...
TYPED_TEST(CuDNNConvolutionLayerTest, TestSimpleConvolutionGruop) {
  typedef typename TypeParam Dtype;
...
TYPED_TEST(CuDNNConvolutionLayerTest, Test1x1Gradient) {
  typedef typename TypeParam Dtype;
...
TYPED_TEST(CuDNNConvolutionLayerTest, TestGradientGroup) {
  typedef typename TypeParam Dtype;
