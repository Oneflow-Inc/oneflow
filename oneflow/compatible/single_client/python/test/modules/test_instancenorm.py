...
def _test_instancenorm1d(test_case, device):
    ...
    output_arr = np.array(
        [
            [
                [-0.9262, 1.5395, 0.2337, -0.8470],
                [-1.5486, -0.0260, 1.2125, 0.3621],
                [-1.5807, 0.2287, 1.1933, 0.1587],
            ],
            [
                [-0.2215, -1.5212, 0.6285, 1.1143],
                [-0.5016, 1.5917, 0.0110, -1.1011],
    ...
    m = flow.nn.InstanceNorm1d(num_features=3, eps=1e-5, momentum=0.1).to(
...
def _test_instancenorm2d(test_case, device):
    ...
    output = np.array(
        [
            [
                [
                    [-0.9155, 0.3100, 0.8339, -0.2747],
                    [0.8991, -1.8781, -1.0048, 0.2183],
                    [0.3412, 1.9598, 0.3977, -0.8868],
                ],
                [
                    [0.5860, -0.3169, 0.5763, -0.9675],
            ...
            [
                [
                    [1.3740, 1.3515, -0.4466, 0.4017],
                    [-0.7215, 1.0177, 0.5765, -0.6405],
                    [-1.6636, -0.6630, 0.7669, -1.3530],
                ],
                [
                    [-1.1583, 1.1444, -1.4363, 0.7516],
                    [0.8586, 1.4328, 0.0090, -1.0057],
    ...
    m = flow.nn.InstanceNorm2d(num_features=2, eps=1e-5, momentum=0.1).to(
        device=flow.device(device)
    )
    x = flow.Tensor(input_arr, device=flow.device(device))
    y = m(x)
...
def _test_instancenorm3d(test_case, device):
    ...
    output_arr = np.array(
        [
            [
                [
                    [
                        [1.0670, 0.3324, 1.4075, 1.5856],
                        [-0.5976, -0.1184, 1.0669, 0.6381],
                        [-0.9888, -0.9877, 0.4276, -1.5349],
                    ],
                    [
                        [-1.2319, 1.0813, -0.1134, -1.0680],
                ...
                [
                    [
                        ...
                        [0.2444, 1.3833, -1.4615, -0.1740],
                    ],
                    [
                        [-0.3162, -0.8518, 0.9094, 0.7097],
                        [0.6424, 0.5115, 1.8380, -1.1641],
            ...
            [
                ...
                [
                    ...
                    [
                        [-0.7288, -0.0616, -1.4350, -0.7749],
                        [1.7471, -1.5170, -0.4012, -1.5485],
    ...
    m = flow.nn.InstanceNorm3d(num_features=2, eps=1e-5, momentum=0.1).to(
        device=flow.device(device)
    )
    x = flow.Tensor(input_arr, device=flow.device(device))
    y = m(x)
