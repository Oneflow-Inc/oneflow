...
__global__ void UpsampleBilinearForward(const int64_t nthreads, const T* in_dptr,
  ...
  CUDA_1D_KERNEL_LOOP(index, nthreads) {
    ...
    const int bottom_h_index = (in_h < height - 1) ? ceilf(in_h) : height - 1;
    ...
    const int right_w_index = (in_w < width - 1) ? ceilf(in_w) : width - 1;
    const float w_lerp = in_w - left_w_index;
    const float top_left(
        in_dptr[n * channel_area + (c * height + top_h_index) * width + left_w_index]);
    const float top_right(
        in_dptr[n * channel_area + (c * height + top_h_index) * width + right_w_index]);
    const float bottom_left(
        in_dptr[n * channel_area + (c * height + bottom_h_index) * width + left_w_index]);
    const float bottom_right(
        in_dptr[n * channel_area + (c * height + bottom_h_index) * width + right_w_index]);
...
__global__ void UpsampleBilinearBackward(const int64_t nthreads, const T* dy_dptr,
  ...
  CUDA_1D_KERNEL_LOOP(index, nthreads) {
    ...
    const int bottom_h_index = (original_h < height - 1) ? ceilf(original_h) : height - 1;
    ...
    const int right_w_index = (original_w < width - 1) ? ceilf(original_w) : width - 1;
    const float w_lerp = original_w - floorf(original_w);

    const float dtop = (1 - h_lerp) * dy_dptr[index];
    atomicAdd(dx_dptr + n * channel_area + (c * height + top_h_index) * width + left_w_index,
              static_cast<T>((1 - w_lerp) * dtop));
    atomicAdd(dx_dptr + n * channel_area + (c * height + top_h_index) * width + right_w_index,
              static_cast<T>(w_lerp * dtop));
    const float dbottom = h_lerp * dy_dptr[index];
    atomicAdd(dx_dptr + n * channel_area + (c * height + bottom_h_index) * width + left_w_index,
              static_cast<T>((1 - w_lerp) * dbottom));
    atomicAdd(dx_dptr + n * channel_area + (c * height + bottom_h_index) * width + right_w_index,
