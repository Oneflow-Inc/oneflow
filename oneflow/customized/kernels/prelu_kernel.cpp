 ...
 private:
  void Compute(user_op::KernelComputeContext* ctx) const override {
    ...
    NdarrayUtil<DeviceType::kCPU, T>::BroadcastTo(
        ctx->device_ctx(), XpuVarNdarray<T>(x->shape(), tmp_buffer_ptr),
        XpuVarNdarray<const T>(left_extended_shape, alpha->dptr<T>()));
    FOR_RANGE(int32_t, i, 0, elem_cnt) {
      y_ptr[i] = x_ptr[i] > 0 ? x_ptr[i] : x_ptr[i] * tmp_buffer_ptr[i];
    }
  }
 ...
 private:
  void Compute(user_op::KernelComputeContext* ctx) const override {
    ...
    NdarrayUtil<DeviceType::kCPU, T>::BroadcastTo(
        ...
        XpuVarNdarray<const T>(left_extended_shape, alpha->dptr<T>()));
    FOR_RANGE(int32_t, i, 0, elem_cnt) {
      dx_ptr[i] = x_ptr[i] > 0 ? dy_ptr[i] : dy_ptr[i] * tmp_buffer_ptr[i];
    }
  }
  bool AlwaysComputeWhenAllOutputsEmpty() const override { return false; }
};
...
class CpuPReluAlphaGradKernel final : public user_op::OpKernel {
 ...
 private:
  void Compute(user_op::KernelComputeContext* ctx) const override {
    ...
    FOR_RANGE(int32_t, i, 0, elem_cnt) {
      alpha_diff_tmp_buf[i] = x_ptr[i] > 0 ? 0 : dy_ptr[i] * x_ptr[i];
    }
    const Shape& left_extended_shape =
        CreateLeftExtendedShape(ShapeView(alpha->shape()), x->shape().NumAxes());
