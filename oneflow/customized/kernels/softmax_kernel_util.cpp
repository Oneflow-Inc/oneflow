...
void SoftmaxKernelUtil<device_type, T>::ComputeProb(DeviceCtx* ctx, const int64_t n, const int64_t w, const T* in, T* tmp,
...
#define INSTANTIATE_SOFTMAX_KERNEL_UTIL(device_type, data_type)          \
  template struct SoftmaxKernelUtil<OF_PP_PAIR_FIRST(device_type), OF_PP_PAIR_FIRST(data_type)>;

OF_PP_SEQ_PRODUCT_FOR_EACH_TUPLE(INSTANTIATE_SOFTMAX_KERNEL_UTIL,
