...
__device__ float FloorModCalInDiff4GpuFloat_x(float x, float y, float dz) { 
  return dz;
 }

 __device__ float FloorModCalInDiff4GpuFloat_y(float x, float y, float dz) { 
  return -dz * floor(x/y);
 }
...
#define MATH_BINARY_GPU(func_name, fw_func, bw_func_call_x_diff, bw_func_call_y_diff, dtype)                        \
  ...
  __global__ void func_name##XBackwardGpu(const int n, const dtype* x, const dtype* y, const dtype* dz,             \
                                         dtype* dx) {                                                               \
  ...
  __global__ void func_name##YBackwardGpu(const int n, const dtype* x, const dtype* y, const dtype* dz,             \
                                         dtype* dy) {                                                               \
  ...
  #define MATH_BINARY_GPU_FLOAT_SEQ                                      \
    OF_PP_MAKE_TUPLE_SEQ("FloorMod", FloorMod)                           \

MATH_BINARY_GPU(FloorMod, fmodf, FloorModCalInDiff4GpuFloat_x, FloorModCalInDiff4GpuFloat_y, float);
