...
void CropBBox(const TensorBuffer& origin_bbox, const CropWindow& crop,
  ...
  CHECK_EQ(keep_mask.shape().At(1), num_boxes);
  CHECK_EQ(keep_mask.data_type(), DataType::kInt8);
  int64_t num_keeps = 0;
  FOR_RANGE(int, i, 0, num_boxes) {
    if (static_cast<bool>(keep_mask.data<int8_t>())) { num_keeps += 1; }
  }
  bbox->Resize(Shape({num_keeps, 4}), origin_bbox.data_type());
  num_keeps = 0;
  FOR_RANGE(int, i, 0, num_boxes) {
    if (static_cast<bool>(keep_mask.data<int8_t>())) {
      const float* origin_bbox_ptr = origin_bbox.data<float>() + i * 4;
      float* bbox_ptr = bbox->mut_data<float>() + num_keeps * 4;
      bbox_ptr[0] = std::max(origin_bbox_ptr[0], crop.left());
      bbox_ptr[1] = std::max(origin_bbox_ptr[1], crop.top());
      bbox_ptr[2] = std::min(origin_bbox_ptr[2], crop.right());
      bbox_ptr[3] = std::min(origin_bbox_ptr[3], crop.bottom());
      num_keeps += 1;
    }
  }
...
void FilterLabels(const TensorBuffer& origin_label, const TensorBuffer& keep_mask,
  ...
  CHECK_EQ(keep_mask.shape().NumAxes(), 1);
  CHECK_EQ(keep_mask.shape().At(1), num_labels);
  CHECK_EQ(keep_mask.data_type(), DataType::kInt8);
  int64_t num_keeps = 0;
  FOR_RANGE(int, i, 0, num_labels) {
    if (static_cast<bool>(keep_mask.data<int8_t>())) { num_keeps += 1; }
  }
  label->Resize(Shape({num_keeps, 4}), origin_label.data_type());
  num_keeps = 0;
  FOR_RANGE(int, i, 0, num_labels) {
    if (static_cast<bool>(keep_mask.data<int8_t>())) {
