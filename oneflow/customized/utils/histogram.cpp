...
Histogram::Histogram(std::vector<double> custom_bucket_limits)
    : custom_bucket_limits_(custom_bucket_limits.begin(), custom_bucket_limits.end()),
...
bool Histogram::DecodeFromProto(const HistogramProto& proto) {
  if ((proto.bucket_size() != proto.bucket_limit_size()) || (proto.bucket_size() == 0)) {
    return false;
  }
  min_ = proto.min();
  max_ = proto.max();
  ...
  custom_bucket_limits_.insert(custom_bucket_limits_.end(), proto.bucket_limit().begin(),
...
void Histogram::Clear() {
  ...
  for (size_t i = 0; i < bucket_limits_.size(); i++) { buckets_[i] = 0; }
}

void Histogram::Add(double value) {
  int b = std::upper_bound(bucket_limits_.begin(), bucket_limits_.end(), value)
...
double Histogram::Remap(double x, double x0, double x1, double y0, double y1) const {
...
void Histogram::EncodeToProto(HistogramProto* proto, bool preserve_zero_buckets) const {
  proto->Clear();
  proto->set_min(min_);
  proto->set_max(max_);
  proto->set_num(num_);
