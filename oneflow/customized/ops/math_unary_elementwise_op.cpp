...
REGISTER_USER_OP("math_unary_elementwise")
    .Input("x")
    .Output("y")
    .Attr("math_type", UserOpAttrType::kAtString)
...
REGISTER_USER_OP("math_unary_elementwise_grad")
    .Input("x")
    .Input("dy")
    .Output("dx")
    .Attr("math_type", UserOpAttrType::kAtString)
...
REGISTER_USER_OP_GRAD("math_unary_elementwise")
    .SetGenBackwardOpConfFn([](const user_op::UserOpWrapper& op, user_op::AddOpFn AddOp) {
      if (op.NeedGenGradTensor4OpInput("x", 0)) {
        user_op::UserOpConfWrapperBuilder builder(op.op_name() + "_grad");
        user_op::UserOpConfWrapper unary_grad_op =
            builder.Op("math_unary_elementwise_grad")
                .Input("x", op.input("x", 0))
                .Input("dy", op.GetGradTensorWithOpOutput("y", 0))
                .Output("dx")
                .Attr<std::string>("math_type", op.attr<std::string>("math_type"))
                .Build();
