...
void CalcOutAndPadding4Deconv(int64_t input_size, int32_t filter_size, int32_t dilation_rate,
                              int32_t stride, int32_t output_padding,
  ...
  if (padding_type == "valid") {
    if (output_size) {
      *output_size = (input_size - 1) * stride + effective_filter_size + output_padding;
      CHECK_GE((*output_size), 0);
    }
    if (padding_before) { *padding_before = 0; }
    if (padding_after) { *padding_after = 0; }
  } else if (padding_type == "same") {
    int32_t tmp_output_size = (input_size - 1) * stride + 1 + output_padding;
    if (output_size) { *output_size = tmp_output_size; }
    const int32_t padding_needed = std::max(
        0,
