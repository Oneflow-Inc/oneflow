...
#         self._free_weak_ref = flow.Storage._free_weak_ref  # type: ignore[attr-defined]

#     def expired(self):
#         return flow.Storage._expired(self.cdata)  # type: ignore[attr-defined]
...
#     return flow.cuda.Event.from_ipc_handle(device, handle)
...
#     t = flow._utils._rebuild_tensor(storage, storage_offset, size, stride)
#     if cls == flow.nn.parameter.Parameter:
...
#         t = flow.nn.parameter.Parameter(t, requires_grad=requires_grad)
#     else:
...
#     t = flow._utils._rebuild_tensor(storage, tensor_offset, tensor_size, tensor_stride)

#     if tensor_cls == flow.nn.parameter.Parameter:
...
def reduce_tensor(tensor):
    ...
    # flow.utils.hooks.warn_if_has_hooks(tensor)
