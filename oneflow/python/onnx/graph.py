...
class Graph(object):
    ...
    def update_node_shape_dtype(self, node, override=False):
        ...
        for i, inp in enumerate(node.inputs):
            if inp is None:
                ...
                continue
            if inp.is_const():
                tensor = numpy_helper.from_array(inp.get_tensor_value(as_list=False), name=inp.output[0])
    ...
    def make_graph(self, doc, graph_name="oneflow.python.onnx"):
        ...
        for op in const_ops:
            ...
            tensor = numpy_helper.from_array(op.get_tensor_value(as_list=False), tensor_name)
            initializers.append(tensor)
