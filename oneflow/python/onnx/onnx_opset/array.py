...
def _MakeGatherNdInnerLoop(ctx, params, index, dtype):
    ...
    trip_node = ctx.MakeNode("Size", [index.output[0]])
    cond_const = ctx.MakeConst(id_util.UniqueStr("cond"), np.ones((), dtype=np.bool))
    ...
    # body graph creation
    g = ctx.CreateNewGraphWithSameConfig()
    g.AddGraphInput(trip_name, TensorProto.INT64, [1])
    g.AddGraphInput(cond_name, TensorProto.BOOL, [])
    g.AddGraphInput(cur_name, dtype, [])
    g.parent_graph = ctx

    index_i = g.MakeNode("Gather", [index.output[0], trip_name], attr={"axis": 0})
    gather = g.MakeNode("Gather", [cur_name, index_i.output[0]], attr={"axis": 0})
    g.MakeNode("Squeeze", [gather.output[0]], attr={"axes": [0]}, outputs=[result_name])
    g.MakeNode("Identity", [cond_name], outputs=[cond_out_name])

    g.AddGraphOutput(cond_out_name, TensorProto.BOOL, [])
    g.AddGraphOutput(result_name, dtype, [])

    inner_loop = ctx.MakeNode(
        "Loop",
...
def _MakeGatherNd(ctx, params, indices, output, scope_name, t_params, shapes, dtypes):
    """make GatherNd op."""
    ...
    indices_shape = ctx.MakeNode("Shape", [indices], dtypes=[TensorProto.INT64])
    indices_size = ctx.MakeNode("Size", [indices])
    attr = {"axes": [0], "ends": [sys.maxsize], "starts": [-1]}
    inputs_map = {"data": indices_shape.output[0], **attr}
    inner_shape = GraphBuilder(ctx).MakeSlice(inputs_map, dtypes=[TensorProto.INT64])
    outter_shape = ctx.MakeNode(
        "Div", [indices_size.output[0], inner_shape], dtypes=[TensorProto.INT64]
    )
    flatten_shape = ctx.MakeNode(
        "Concat",
        [outter_shape.output[0], inner_shape],
        attr={"axis": 0},
        dtypes=[TensorProto.INT64],
    )
    flatten_indices = ctx.MakeNode("Reshape", [indices, flatten_shape.output[0]])

    # outter loop for each index
    # for (int i=0; i<outter_shape; i++) inner_loop(params, flatten_indices[i])
    cond_const = ctx.MakeConst(id_util.UniqueStr("cond"), np.ones((), dtype=np.bool))
    dummy_const = ctx.MakeConst(id_util.UniqueStr("dummy"), np.ones((), dtype=np.int64))
    ...
    g.AddGraphInput(trip_name, TensorProto.INT64, [1])
    g.AddGraphInput(cond_name, TensorProto.BOOL, [])
    g.AddGraphInput(dummy_name, t_params, [])
    g.parent_graph = ctx

    index = g.MakeNode(
        "Gather", [flatten_indices.output[0], trip_name], attr={"axis": 0}
    )
    index_squeeze = g.MakeNode("Squeeze", [index.output[0]], attr={"axes": [0]})
    # inner loop to gather result
    inner_loop = _MakeGatherNdInnerLoop(g, params, index_squeeze, t_params)
    g.MakeNode("Identity", [cond_name], outputs=[cond_out_name])
    g.MakeNode("Identity", [dummy_name], outputs=[dummy_out_name])
    g.MakeNode("Identity", [inner_loop.output[0]], outputs=[result_name])

    g.AddGraphOutput(cond_out_name, TensorProto.BOOL, [])
    g.AddGraphOutput(dummy_out_name, t_params, [])
    g.AddGraphOutput(result_name, t_params, [])

    gathernd_loop = ctx.MakeNode(
        "Loop",
    ...
    inner_loop_shape = ctx.MakeNode(
        "Shape", [gathernd_loop.output[1]], dtypes=[TensorProto.INT64]
    )
    # workaround in case gathernd_loop is 1-dimensional
    one_const = ctx.MakeConst(id_util.UniqueStr("one"), np.array([1], dtype=np.int64))
    inner_loop_shape_ = ctx.MakeNode(
        "Concat",
    ...
    output_inner_shape = GraphBuilder(ctx).MakeSlice(
        inputs_map, dtypes=[TensorProto.INT64]
    )
    attr = {"axes": [0], "ends": [-1], "starts": [0]}
    inputs_map = {"data": indices_shape.output[0], **attr}
    indices_outter_shape = GraphBuilder(ctx).MakeSlice(
        inputs_map, dtypes=[TensorProto.INT64]
    )
    output_shape_ = ctx.MakeNode(
        "Concat",
    ...
    output_shape = GraphBuilder(ctx).MakeSlice(inputs_map, dtypes=[TensorProto.INT64])
    ctx.MakeNode(
        "Reshape",
...
class GatherND:
    ...
    def Version_1(cls, ctx, node, **kwargs):
        ...
        util.MakeSure(t_params, "Dtype of {} is None".format(indices))
        shapes = node.output_shapes
        dtypes = node.output_dtypes
        ctx.RemoveNode(node.name)
        _MakeGatherNd(ctx, params, indices, output, node.name, t_params, shapes, dtypes)
