...
def test_conv2d_k2d1_valid(test_case):
    ...
    def conv2d_k3s1_valid(x=flow.FixedTensorDef((2, 4, 3, 5))):
        return flow.layers.conv2d(x, 6, kernel_size=3, strides=1, padding="VALID", **initer_args)
...
def test_conv2d_s2_valid(test_case):
    ...
    def conv2d_s2_valid(x=flow.FixedTensorDef((2, 4, 3, 5))):
        return flow.layers.conv2d(x, 6, kernel_size=1, strides=2, padding="VALID", **initer_args)
...
def test_conv2d_s2_same(test_case):
    ...
    def conv2d_s2_same(x=flow.FixedTensorDef((2, 4, 3, 5))):
        return flow.layers.conv2d(x, 6, kernel_size=3, strides=2, padding="SAME", **initer_args)
...
def test_conv2d_k3s1_nhwc_valid(test_case):
    ...
    def conv2d_k3s1_nhwc_valid(x=flow.FixedTensorDef((2, 3, 5, 4))):
        return flow.layers.conv2d(x, 6, kernel_size=3, strides=1, padding="VALID", data_format='NHWC', **initer_args)
...
def test_conv2d_k3s1_nhwc_same_d2(test_case):
    ...
    def conv2d(x=flow.FixedTensorDef((2, 7, 5, 4))):
        return flow.layers.conv2d(x, 6, kernel_size=3, strides=1, dilation_rate=2, padding="SAME", data_format='NHWC', **initer_args)
...
def test_conv2d_k3s1_nchw_same_g2(test_case):
    ...
    def conv2d(x=flow.FixedTensorDef((2, 4, 5, 3))):
        return flow.layers.conv2d(x, 6, kernel_size=3, strides=1, groups=2, padding="SAME", data_format='NCHW', **initer_args)
...
def test_conv2d_k3s1_nchw_same_depthwise(test_case):
    @flow.function(func_config)
    def conv2d(x=flow.FixedTensorDef((2, 4, 5, 3))):
        return flow.layers.conv2d(x, 4, kernel_size=3, strides=1, groups=4, padding="SAME", data_format='NCHW', **initer_args)
