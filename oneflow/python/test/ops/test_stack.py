...
def _compare_stack_with_np(
    ...
    def oneflow_stack(
        ...
        with flow.scope.placement(device_type, "0:0"):
            ...
            ).minimize(of_stack_out)

        return of_stack_out

    of_out_stack = oneflow_stack(input_1, input_2)

    assert np.allclose(
        of_out_stack, np_out_stack
    )
...
class Teststack1n1d(flow.unittest.TestCase):
    ...
    def test_stack_gpu(test_case):
        arg_dict = _gen_arg_dict(
            shape=(3, 16, 32), axis=-4, device_type="gpu", machine_ids="0:0", device_counts=1
        )
        for arg in GenArgList(arg_dict):
            _compare_stack_with_np(*arg)
...
class Teststack1n2d(flow.unittest.TestCase):
    ...
    def test_stack_gpu_1n2d(test_case):
        arg_dict = _gen_arg_dict(
            shape=(3, 8, 8, 4), axis=3, device_type="gpu", machine_ids="0:0-1", device_counts=2
        )
        for arg in GenArgList(arg_dict):
            _compare_stack_with_np(*arg)
