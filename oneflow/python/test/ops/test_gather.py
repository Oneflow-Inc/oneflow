...
def _of_dynamic_params_gather(params, indices, static_params_shape, compare_fn):
    flow.clear_default_session()
    func_config = flow.FunctionConfig()
    func_config.default_data_type(flow.float)
    func_config.default_distribute_strategy(flow.distribute.mirrored_strategy())
    ...
    def gather_fn(
        ...
        with flow.device_prior_placement("gpu", "0:0"):
            ...
            y = flow.gather(params_var, indices_def)
            flow.losses.add_loss(y)
    ...
    check_point.init()
    return gather_fn([params], [indices]).get().ndarray_list()[0]
...
def _compare_gather_with_tf(test_case, device_type, params_shape, indices_shape, mirrored=False):
    ...
    gather_fn = _make_gather_nd_fn(params, indices, device_type, mirrored, compare_dy)
...
def _compare_dynamic_gather_with_tf(test_case, params_shape, static_params_shape, indices_shape):
    params, indices = _random_inputs(params_shape, indices_shape)
    ...
    of_y = _of_dynamic_params_gather(params, indices, static_params_shape, compare_dy)
    test_case.assertTrue(np.array_equal(y.numpy(), of_y))


def _of_gather_dynamic_indices(params, indices, indices_static_shape, device_type):
    flow.clear_default_session()
    func_config = flow.FunctionConfig()
    func_config.default_data_type(flow.float)
    func_config.default_distribute_strategy(flow.distribute.mirrored_strategy())
    ...
    def gather_fn(
        ...
        with flow.device_prior_placement(device_type, "0:0"):
            return flow.gather(params_def, indices_def)

    return gather_fn([params], [indices]).get().ndarray_list()[0]


def _compare_gather_dynamic_indices_with_tf(
    ...
    of_y = _of_gather_dynamic_indices(params, indices, indices_static_shape, device_type)
    test_case.assertTrue(np.array_equal(y.numpy(), of_y))
...
def test_dynamic_gather(test_case):
    arg_dict = OrderedDict()
    arg_dict["params_shape"] = [(30, 15)]
    arg_dict["static_params_shape"] = [(32, 16)]
    arg_dict["indices_shape"] = [(12, 1)]
    for arg in GenArgList(arg_dict):
        _compare_dynamic_gather_with_tf(test_case, *arg)


def test_gather_dynamic_indices(test_case):
    arg_dict = OrderedDict()
    ...
    arg_dict["device_type"] = ["gpu"]
    for arg in GenArgList(arg_dict):
        _compare_gather_dynamic_indices_with_tf(test_case, *arg)


def test_gather_empty_indices(test_case):
    arg_dict = OrderedDict()
