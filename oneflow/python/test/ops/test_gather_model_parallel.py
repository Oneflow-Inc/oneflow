...
def _gen_test_data(params_shape, indices_shape, axis):
    ...
    indices = np.random.randint(low=0, high=params_shape[axis], size=indices_shape).astype(np.int32)
...
def _test_gather_model_parallel_fw(test_case, device_type, params_shape, indices_shape, axis, split_axis):
    ...
    def gather_model_parallel_fw_job(params=flow.FixedTensorDef(params_shape, dtype=flow.float),
