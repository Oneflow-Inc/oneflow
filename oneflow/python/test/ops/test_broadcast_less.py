...
def _make_less_fn(x, y, device_type, mirrored):
...
def _compare_less_with_tf(test_case, device_type, x_shape,
        y_shape, mirrored=False):
    x, y = _random_inputs(x_shape, y_shape)
    less_fn = _make_less_fn(x, y, device_type, mirrored)

    if mirrored:
        of_y = less_fn([x], [y]).get().ndarray_list()[0]
    else:
        of_y = less_fn(x, y).get().ndarray()
    tf_y = tf.math.less(x, y)
...
def test_less(test_case):
    ...
    for arg in GenArgList(arg_dict):
        _compare_less_with_tf(test_case, *arg)


def test_less_case_1(test_case):
    arg_dict = OrderedDict()
    arg_dict["device_type"] = ["gpu"]
...
def test_less_case_2(test_case):
    arg_dict = OrderedDict()
...
def test_less_case_3(test_case):
    arg_dict = OrderedDict()
    arg_dict["device_type"] = ["gpu"]
