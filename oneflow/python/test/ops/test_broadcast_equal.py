...
def _make_equal_fn(x, y, device_type, mirrored):
    ...
    def do_equal(x_blob, i_blob):
        with flow.device_prior_placement(device_type, "0:0"):
            y = flow.math.equal(x_blob, i_blob)
        return y
...
def _compare_equal_with_tf(test_case, device_type, x_shape,
        y_shape, mirrored=False):
    x, y = _random_inputs(x_shape, y_shape)
    equal_fn = _make_equal_fn(x, y, device_type, mirrored)

    if mirrored:
        of_y = equal_fn([x], [y]).get().ndarray_list()[0]
    else:
        of_y = equal_fn(x, y).get().ndarray()
    tf_y = tf.math.equal(x, y)
...
def test_equal(test_case):
    ...
    for arg in GenArgList(arg_dict):
        _compare_equal_with_tf(test_case, *arg)


def test_equal_case_1(test_case):
    arg_dict = OrderedDict()
    arg_dict["device_type"] = ["gpu"]
...
def test_equal_case_2(test_case):
    arg_dict = OrderedDict()
...
def test_equal_case_3(test_case):
    arg_dict = OrderedDict()
    arg_dict["device_type"] = ["gpu"]
