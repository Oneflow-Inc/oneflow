...
def compare_with_tensorflow(device_type, x, y, mirrored):
    ...
    if mirrored:
        ...
        def LessJob(a=flow.MirroredTensorDef(x.shape),
                b=flow.MirroredTensorDef(y.shape)):
            z = flow.math.less(a, b)
            return z
        of_out = LessJob([x], [y]).get().ndarray_list()[0]
    else:
        func_config.default_distribute_strategy(flow.distribute.consistent_strategy())

        @flow.function(func_config)
        def LessJob(a=flow.FixedTensorDef(x.shape),
                b=flow.FixedTensorDef(y.shape)):
            z = flow.math.less(a, b)
            return z
        of_out  = LessJob(x, y).get().ndarray()

    # TensorFlow
    tf_out = tf.math.less(x, y)
