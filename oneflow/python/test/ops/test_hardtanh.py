...
def _compare_hardtanh_with_np(
    input_shape, min_val, max_val, device_type, machine_ids, device_counts
):
    input_1 = np.random.random(size=input_shape).astype(np.float32)

    # input_1 = np.array([[-2, -1, 0], [1, 2, 3]]).astype(np.float32)
    ...
    def np_hardtanh(input):
        return np.clip(input, min_val, max_val)

    np_out_hardtanh = np_hardtanh(input_1)

    def np_diff(input):
        elem_cnt = input.size
        diff_array = np.zeros(shape=(elem_cnt,))
        np_input = input.flatten()
        for i in range(elem_cnt):
            if np_input[i] > min_val and np_input[i] < max_val:
                diff_array[i] = 1

        return np.reshape(diff_array, input.shape)
    ...
    def oneflow_hardtanh(
        ...
        of_hardtanh_out = flow.math.hardtanh(x_var, min_val, max_val)
    ...
    assert np.allclose(of_out_hardtanh, np_out_hardtanh)


def _gen_arg_dict(shape, min_val, max_val, device_type, machine_ids, device_counts):
    # Generate a dict to pass parameter to test case
    arg_dict = OrderedDict()
    arg_dict["input_shape"] = [shape]
    arg_dict["min_val"] = [min_val]
    arg_dict["max_val"] = [max_val]
