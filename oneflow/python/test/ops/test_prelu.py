...
def _check(test_case, x, y, shared_axes):
    alpha_of = test_global_storage.getToNdarray("alpha")
    alpha = np.expand_dims(alpha_of, axis = 0)
    dy = test_global_storage.getToNdarray("loss_diff")
    ...
    test_case.assertTrue(np.allclose(np_prelu_x_diff, test_global_storage.getToNdarray("x_diff")))
    test_case.assertTrue(np.allclose(np_prelu_alpha_diff, test_global_storage.getToNdarray("alpha_diff")))
...
def _run_test(test_case, device_type, dtype, x_shape, shared_axes):
    ...
    def PreluJob(x=flow.FixedTensorDef(x_shape, dtype=type_name_to_flow_type[dtype])):
        with flow.fixed_placement(device_type, "0:0"):
            ...
            flow.watch(alpha, test_global_storage.set("alpha"))
            flow.watch_diff(alpha, test_global_storage.set("alpha_diff"))
