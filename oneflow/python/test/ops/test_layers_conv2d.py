...
def compare_with_tensorflow(device_type, x_shape, filters, kernel_size, groups):
    ...
    def ConvJob():
        with flow.device_prior_placement(device_type, "0:0"):
            ...
            flow.watch(weight, test_global_storage.set("weight"))
            flow.watch_diff(weight, test_global_storage.set("weight_diff"))
    ...
    with tf.GradientTape(persistent=True) as tape:
        x = tf.Variable(test_global_storage.getToNdarray("x").transpose(0, 2, 3, 1))
        assert groups > 0
        assert x_shape[1] % groups == 0
        assert filters % groups == 0
        if groups == 1:
            weight = tf.Variable(test_global_storage.getToNdarray("weight").transpose(2, 3, 1, 0))
    ...
    loss_diff = test_global_storage.getToNdarray("loss_diff").transpose(0, 2, 3, 1)
    ...
    of_x_diff_arr = test_global_storage.getToNdarray("x_diff").transpose(0, 2, 3, 1)
    ...
    assert np.allclose(
        test_global_storage.getToNdarray("weight_diff").transpose(2, 3, 1, 0), tf_weight_diff.numpy(), rtol=1e-5, atol=1e-5
    )
