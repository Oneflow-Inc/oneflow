...
def compare_with_tensorflow(device_type, x, y, mirrored):
    ...
    if mirrored:
        ...
        def NotEqualJob(a=flow.MirroredTensorDef(x.shape),
                b=flow.MirroredTensorDef(y.shape)):
            z = flow.math.not_equal(a, b)
            return z
        of_out = NotEqualJob([x], [y]).get().ndarray_list()[0]
    else:
        func_config.default_distribute_strategy(flow.distribute.consistent_strategy())

        @flow.function(func_config)
        def NotEqualJob(a=flow.FixedTensorDef(x.shape),
                b=flow.FixedTensorDef(y.shape)):
            z = flow.math.not_equal(a, b)
            return z
        of_out  = NotEqualJob(x, y).get().ndarray()

    # TensorFlow
    tf_out = tf.math.not_equal(x, y)
...
def test_not_equal_1(test_case):
    arg_dict = OrderedDict()
    arg_dict["device_type"] = ["gpu"]
