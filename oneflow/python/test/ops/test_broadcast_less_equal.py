...
def _make_less_equal_fn(x, y, device_type, mirrored):
    ...
    def do_less_equal(x_blob, i_blob):
    ...
    if mirrored:
        ...
        def less_equal_fn(
            ...
            return do_less_equal(x_def, y_def)
...
def _compare_less_equal_with_tf(test_case, device_type, x_shape,
        y_shape, mirrored=False):
    x, y = _random_inputs(x_shape, y_shape)
    less_equal_fn = _make_less_equal_fn(x, y, device_type, mirrored)

    if mirrored:
        of_y = less_equal_fn([x], [y]).get().ndarray_list()[0]
    else:
        of_y = less_equal_fn(x, y).get().ndarray()
    tf_y = tf.math.less_equal(x, y)
...
def test_less_equal(test_case):
    ...
    for arg in GenArgList(arg_dict):
        _compare_less_equal_with_tf(test_case, *arg)


def test_less_equal_case_1(test_case):
    arg_dict = OrderedDict()
    arg_dict["device_type"] = ["gpu"]
...
def test_less_equal_case_2(test_case):
    arg_dict = OrderedDict()
...
def test_less_equal_case_3(test_case):
    arg_dict = OrderedDict()
    arg_dict["device_type"] = ["gpu"]
