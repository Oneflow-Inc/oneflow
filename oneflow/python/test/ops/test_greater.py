...
def compare_with_tensorflow(device_type, x, y, mirrored):
    ...
    if mirrored:
        ...
        def GreaterJob(a=flow.MirroredTensorDef(x.shape),
                b=flow.MirroredTensorDef(y.shape)):
            z = flow.math.greater(a, b)
            return z
        of_out = GreaterJob([x], [y]).get().ndarray_list()[0]
    else:
        func_config.default_distribute_strategy(flow.distribute.consistent_strategy())

        @flow.function(func_config)
        def GreaterJob(a=flow.FixedTensorDef(x.shape),
                b=flow.FixedTensorDef(y.shape)):
            z = flow.math.greater(a, b)
            return z
        of_out  = GreaterJob(x, y).get().ndarray()

    # TensorFlow
    tf_out = tf.math.greater(x, y)
