...
def _random_inputs(
    ...
    for col in range(indices_cols):
        if (
            allow_duplicate_index is False
            and indices_rows <= params_shape[col]
        ):
            rand_indices = np.arange(params_shape[col], dtype=np.int32)
            np.random.shuffle(rand_indices)
            indices_col = rand_indices[:indices_rows].reshape(
                indices_shape[:-1]
            )
        else:
            indices_col = np.random.randint(
                low=0,
                high=params_shape[col],
                size=(indices_rows,),
                dtype=np.int32,
...
def _make_scatter_nd_fn(
    indices, updates, shape, device_type, mirrored, compare_fn
):
    ...
    if mirrored:
        ...
        def scatter_nd_fn(
            ...
            updates_def=flow.MirroredTensorDef(
                updates.shape, dtype=flow.float
            ),
        ):
...
def _compare_scatter_nd_with_tf(
    ...
    _, updates, indices = _random_inputs(
    ...
    if mirrored:
        ...
        def compare_dy(params_grad):
            test_case.assertTrue(
                np.array_equal(dy_dx.numpy(), params_grad.ndarray())
            )
...
def _compare_scatter_nd_update_with_tf(
    ...
    def compare_dz_dy(updates_grad):
        test_case.assertTrue(
            np.allclose(dz_dy.numpy(), updates_grad.ndarray())
        )
...
def _of_tensor_scatter_nd_add(
    ...
    def do_tensor_scatter_nd_add(params_blob, indices_blob, updates_blob):
        with flow.device_prior_placement(device_type, "0:0"):
            ...
            out = flow.tensor_scatter_nd_add(
                params_var, indices_blob, updates_var
            )
    ...
    if mirrored:
        ...
        def tensor_scatter_nd_add_fn(
            ...
            return do_tensor_scatter_nd_add(
                params_def, indices_def, updates_def
            )
        ...
        check_point.init()
        return (
            tensor_scatter_nd_add_fn(params, indices, updates).get().ndarray()
        )
...
def _compare_tensor_scatter_nd_add_with_tf(
    ...
    with tf.GradientTape() as t1:
        ...
        tf_out1 = tf.tensor_scatter_nd_add(
            params_var, indices_const, updates_const
        )
    tf_params_grad = t1.gradient(tf_out1, params_var)

    with tf.GradientTape() as t2:
        updates_var = tf.Variable(updates)
        tf_out2 = tf.tensor_scatter_nd_add(
            params_const, indices_const, updates_var
        )
    ...
    def compare_params_grad(of_params_grad):
        ...
        of_params_grad_np = (
            of_params_grad.ndarray_list()[0]
            if mirrored
            else of_params_grad.ndarray()
        )
    ...
    def compare_updates_grad(of_updates_grad):
        ...
        of_updates_grad_np = (
            of_updates_grad.ndarray_list()[0]
            if mirrored
            else of_updates_grad.ndarray()
        )
        test_case.assertTrue(
...
def _of_scatter_nd_dynamic_indices(
    ...
    def scatter_nd_fn(
        ...
        updates_def=flow.MirroredTensorDef(
            updates_static_shape, dtype=flow.float
        ),
    ):
...
def _of_tensor_scatter_nd_update_dynamic_indices(
    ...
    def tensor_scatter_nd_update_fn(
        ...
        with flow.device_prior_placement("gpu", "0:0"):
            return flow.tensor_scatter_nd_update(
                params_def, indices_def, updates_def
            )
...
def _of_tensor_scatter_nd_add_dynamic_indices(
    ...
    def tensor_scatter_nd_add_fn(
        ...
        with flow.device_prior_placement("gpu", "0:0"):
            return flow.tensor_scatter_nd_add(
                params_def, indices_def, updates_def
            )
...
def test_tensor_scatter_nd_update_dynamic_indices(test_case):
    ...
    for arg in GenArgList(arg_dict):
        _compare_tensor_scatter_nd_update_dynamic_indices_with_tf(
            test_case, *arg
        )
