...
def compare_with_tensorflow(device_type, input_shape, axis, keepdims, rtol=1e-5, atol=1e-5):
    ...
    def ReduceAnyJob(x=flow.FixedTensorDef(input_shape)):
        with flow.device_prior_placement(device_type, "0:0"):
            return flow.math.reduce_any(x, axis=axis, keepdims=keepdims)
    x = np.random.rand(*input_shape).astype(np.float32)
    #x = np.array([[[0,2,3,2],[0,2,3,1],[0,3,4,-1]],[[1,0,2,0],[2,1,2,0],[-2,1,1,0]]],  dtype=np.float32)
    # OneFlow
...
def test_reduce_any(test_case):
    arg_dict = OrderedDict()
    arg_dict["device_type"] = ["gpu"]
    arg_dict["input_shape"] = [(64,53,54)]
