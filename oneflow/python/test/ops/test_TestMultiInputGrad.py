...
def test_TestMultiInput_grad_mirrored_inplace(test_case):
    ...
    def TestMultiInputJob():
        with flow.device_prior_placement("gpu", "0:0"):
            ...
            flow.watch(x1, test_global_storage.set("x1"))
            flow.watch_diff(x1, test_global_storage.set("x1_diff"))
            flow.watch(x2, test_global_storage.set("x2"))
            flow.watch_diff(x2, test_global_storage.set("x2_diff"))
            return loss

    check_point = flow.train.CheckPoint()
    check_point.init()
    out = TestMultiInputJob().get()
    x1_diff = test_global_storage.getToNdarray("x1_diff")
    x2_diff = test_global_storage.getToNdarray("x2_diff")

    expect_out = test_global_storage.getToNdarray("x1")
