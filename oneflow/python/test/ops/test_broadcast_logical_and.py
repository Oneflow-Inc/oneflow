...
def _make_logical_and_fn(x, y, device_type, mirrored):
    ...
    def do_logical_and(x_blob, i_blob):
    ...
    if mirrored:
        ...
        def logical_and_fn(
            ...
            return do_logical_and(x_def, y_def)
    ...
    return logical_and_fn

def _compare_logical_and_with_tf(test_case, device_type, x_shape,
        y_shape, mirrored=False):
    x, y = _random_inputs(x_shape, y_shape)
    logical_and_fn = _make_logical_and_fn(x, y, device_type, mirrored)

    if mirrored:
        of_y = logical_and_fn([x], [y]).get().ndarray_list()[0]
    else:
        of_y = logical_and_fn(x, y).get().ndarray()
    tf_y = tf.math.logical_and(x, y)
...
def test_logical_and(test_case):
    ...
    for arg in GenArgList(arg_dict):
        _compare_logical_and_with_tf(test_case, *arg)


def test_logical_and_case_1(test_case):
    arg_dict = OrderedDict()
    arg_dict["device_type"] = ["gpu"]
...
def test_logical_and_case_2(test_case):
    arg_dict = OrderedDict()
...
def test_logical_and_case_3(test_case):
    arg_dict = OrderedDict()
    arg_dict["device_type"] = ["gpu"]
