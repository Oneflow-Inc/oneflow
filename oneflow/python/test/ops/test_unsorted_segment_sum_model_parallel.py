...
def _gen_test_data(out_shape, segment_ids_shape, axis):
    ...
    segment_ids = np.random.randint(low=0, high=out_shape[axis], size=segment_ids_shape).astype(np.int32)
    ...
    if axis != 0:
        ...
        data_perm = list(range(axis, axis + segment_ids.ndim)) + list(range(0, axis)) + list(
            range(axis + segment_ids.ndim, data.ndim))
...
def _test_unsorted_segment_sum_model_parallel_fw(test_case, device_type, out_shape, segment_ids_shape, axis,
                                                 split_axis):
    ...
    data_arr, segment_ids_arr, out_arr = _gen_test_data(out_shape, segment_ids_shape, axis)

    @flow.function(func_config)
    def unsorted_segment_sum_job(data=flow.FixedTensorDef(data_arr.shape, dtype=flow.float),
        ...
        with flow.fixed_placement(device_type, '0:0-3'):
            if split_axis < axis:
                ...
                data = data.with_distribute(flow.distribute.split(split_axis + len(segment_ids.shape) - 1))
            ...
            if split_axis == axis:
                ...
                out0 = flow.unsorted_segment_sum(data=data, segment_ids=segment_ids, num_segments=out_shape[axis],
                                                 axis=axis)
            out1 = flow.unsorted_segment_sum_like(data=data, segment_ids=segment_ids, like=like, axis=axis)
            return out0, out1
