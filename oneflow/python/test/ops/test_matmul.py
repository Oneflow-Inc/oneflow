...
def compare_with_tensorflow(device_type, a_shape, b_shape, transpose_a, transpose_b):
    ...
    def MatmulJob():
        with flow.device_prior_placement(device_type, "0:0"):
            ...
            flow.watch(a, test_global_storage.set("a"))
            flow.watch_diff(a, test_global_storage.set("a_diff"))
            flow.watch(b, test_global_storage.set("b"))
            flow.watch_diff(b, test_global_storage.set("b_diff"))
    ...
    with tf.GradientTape(persistent=True) as tape:
        a = tf.Variable(test_global_storage.getToNdarray("a"))
        b = tf.Variable(test_global_storage.getToNdarray("b"))
    ...
    assert np.allclose(test_global_storage.getToNdarray("a_diff"), tf_a_diff.numpy(), atol=1e-03)
    assert np.allclose(test_global_storage.getToNdarray("b_diff"), tf_b_diff.numpy(), atol=1e-03)
