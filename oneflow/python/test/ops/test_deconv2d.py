...
def compare_with_tensorflow(device_type, params_case, dilations, data_format):
    ...
    if data_format == "NCHW":
        ...
        assert np.allclose(
            of_out.numpy().transpose(0, 2, 3, 1), tf_out.numpy(), rtol=1e-03, atol=1e-03
        )
        assert np.allclose(
        ...
        assert np.allclose(of_out.numpy(), tf_out.numpy(), rtol=1e-03, atol=1e-03), (
            of_out.numpy() - tf_out.numpy()
        )
        assert np.allclose(
            test_global_storage.Get("x_diff"), tf_x_diff.numpy(), rtol=1e-03, atol=1e-03
        )
        assert np.allclose(
