...
def compare_with_tensorflow(device_type, x_shape, y_shape, x_is_zero):
    ...
    def XlogyJob():
        with flow.device_prior_placement(device_type, "0:0"):
            if x_is_zero:
                x_initializer = flow.zeros_initializer()
            else:
                x_initializer = flow.random_uniform_initializer(minval=1, maxval=5)

            x = flow.get_variable(
                "x",
                shape=x_shape,
                dtype=flow.float,
                initializer=x_initializer,
                trainable=True,
            )
            y = flow.get_variable(
                "y",
