...
def _test_stack_backward(test_case, device, shapes):
    for s in shapes:
        ...
        out_of.backward()
        test_case.assertTrue(np.allclose(x_tensor.grad.numpy(), np.ones(x_tensor.shape), 1e-5, 1e-5))
        test_case.assertTrue(np.allclose(y_tensor.grad.numpy(), np.ones(y_tensor.shape), 1e-5, 1e-5))
...
class TestStack(flow.unittest.TestCase):
    def test_stack(test_case):
        ...
        arg_dict["test_fun"] = [_test_stack,  _test_stack_backward, _test_stack_different_dim]
        arg_dict["device"] = ["cpu", "cuda"]
        # Generate random tuple from 3D to 10D with values ranging from 1 to 10
        arg_dict["shape"] = [tuple(random.randrange(1, 10) for _ in range(i)) for i in range(3, 10)]
