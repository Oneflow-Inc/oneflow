...
class TestGather(flow.unittest.TestCase):
    def test_gather(test_case):
        ...
        output = flow.tmp.gather(flow.Tensor(input), flow.Tensor(index, dtype=flow.int), dim=0)
    ...
    def test_gather_random_array(test_case):
        ...
        output = flow.tmp.gather(flow.Tensor(input), flow.Tensor(index, dtype=flow.int), dim=1)
        test_case.assertTrue(np.allclose(output.numpy(), np_out))

        np_out2 = gather_numpy(input, index, dim=2)
        output2 = flow.tmp.gather(flow.Tensor(input), flow.Tensor(index, dtype=flow.int), dim=2)
        test_case.assertTrue(np.allclose(output2.numpy(), np_out2))

        np_out3 = gather_numpy(input, index, dim=3)
        output3 = flow.tmp.gather(flow.Tensor(input), flow.Tensor(index, dtype=flow.int), dim=3)
