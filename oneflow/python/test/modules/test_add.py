...
class TestAddModule(flow.unittest.TestCase):
    ...
    def test_add_cuda(test_case):
        x = flow.Tensor(np.random.randn(2, 3), device=flow.device("cuda"))
        y = flow.Tensor(np.random.randn(2, 3), device=flow.device("cuda"))
        of_out = flow.add(x, y)
        np_out = np.add(x.numpy(), y.numpy())
        test_case.assertTrue(np.allclose(of_out.numpy(), np_out, 1e-4, 1e-4))
        ...
        x = flow.Tensor(np.random.randn(2, 3), device=flow.device("cuda"))
        y = flow.Tensor(np.array([5.0]), device=flow.device("cuda"))
        of_out = flow.add(x, y)
        np_out = np.add(x.numpy(), y.numpy())
        test_case.assertTrue(np.allclose(of_out.numpy(), np_out, 1e-4, 1e-4))

        x = flow.Tensor(np.random.randn(1, 1), device=flow.device("cuda"))
        y = flow.Tensor(np.random.randn(2, 3), device=flow.device("cuda"))
        of_out = flow.add(x, y)
        np_out = np.add(x.numpy(), y.numpy())
        test_case.assertTrue(np.allclose(of_out.numpy(), np_out, 1e-4, 1e-4))
...
class TestAddCudaBigTensorModule(flow.unittest.TestCase):
    def test_add_cuda(test_case):
        x = flow.Tensor(np.random.randn(10000, 10000), device=flow.device("cuda"))
        y = flow.Tensor(np.random.randn(10000, 10000), device=flow.device("cuda"))
        of_out = flow.add(x, y)
        np_out = np.add(x.numpy(), y.numpy())
        test_case.assertTrue(np.allclose(of_out.numpy(), np_out, 1e-4, 1e-4))
...
class TestAddBackwardModule(flow.unittest.TestCase):
    ...
    def test_backward_gpu(test_case):
        x = 5
        y = flow.Tensor(
            np.random.randn(2, 3), requires_grad=True, device=flow.device("cuda")
        )
        of_out = flow.add(x, y).sum()
        of_out.backward()
        test_case.assertTrue(np.allclose(y.grad.numpy(), np.ones((2, 3)), 1e-4, 1e-4))
