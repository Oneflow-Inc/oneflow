...
def _test_broadcast_matmul_backward(test_case, device):
    input1 = flow.Tensor(
        [[[0.5893293023109436, -0.0376124233007431, 0.7791574001312256], [1.1614371538162231, 0.009700910188257694, 0.7281601428985596]], [[-0.27213698625564575, 0.7058051824569702, -0.4643424451351166], [2.2279646396636963, 0.05870082601904869, -0.18335142731666565]]], dtype=flow.float32, device=flow.device(device), requires_grad=True
    )
    input2 = flow.Tensor(
        [[0.25825661420822144, -0.4875393807888031], [-0.040459781885147095, -0.3713535666465759], [-1.633512258529663, -2.0034799575805664]], dtype=flow.float32, device=flow.device(device), requires_grad=True
    )
    of_out = flow.matmul(input1, input2)
    of_out = of_out.sum()
    of_out.backward()
    np_grad = [[[-0.22928276658058167, -0.411813348531723, -3.6369922161102295], [-0.22928276658058167, -0.411813348531723, -3.6369922161102295]], [[-0.22928276658058167, -0.411813348531723, -3.6369922161102295], [-0.22928276658058167, -0.411813348531723, -3.6369922161102295]]]
    test_case.assertTrue(np.allclose(input1.grad.numpy(), np_grad, rtol=1e-05))
