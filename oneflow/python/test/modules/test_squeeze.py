...
def _test_squeeze(test_case, device):
    input = flow.Tensor(np.array([[[[1, 1, 1]]]]).astype(np.int32), device=flow.device(device))
...
def _test_squeeze_backward(test_case, device):
    input = flow.Tensor(np.array([[[[1, 1, 1]]]]).astype(np.int32), device=flow.device(device), requires_grad=True)
