...
def _test_addmm(test_case, shape, alpha, beta, device):
    ...
    mat1_tensor = flow.Tensor(
        mat1, dtype=flow.float32, device=flow.device(device)
    )
    mat2_tensor = flow.Tensor(
        mat2, dtype=flow.float32, device=flow.device(device)
    )
    input_tensor = flow.Tensor(
        input, dtype=flow.float32, device=flow.device(device)
    )
