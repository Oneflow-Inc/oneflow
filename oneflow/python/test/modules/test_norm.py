...
def _np_vector_norm_backward(x,ord = 2, dim = None):
    ...
    if isinstance(ord,int) and isinstance(dim, int):
        if ord == 0:
            ...
            temp = (np.sum(np.abs(x**ord),dim))**(1./ord-1)
        ...
        if ord == 0:
            ...
            temp = (np.sum(np.abs(x**ord)))**(1./ord-1)
            re = np.where(x**ord <0 , -temp, temp) * (x**(ord-1)) 
...
def _test_fro_order_norm_backward(test_case, device):
    input = flow.Tensor(
        np.random.randn(5,4), dtype=flow.float32, device=flow.device(device), requires_grad= True
    )
...
def _test_1d_inf_order_norm_backward(test_case, device):
    for ord in [float("inf"), -float("inf")]:
        input = flow.Tensor(
            np.random.randn(5,), dtype=flow.float32, device=flow.device(device), requires_grad= True
        )
...
def _test_2d_digits_order_norm_backward(test_case, device):
    for ord in [1,-1]:
        input = flow.Tensor(
            np.random.randn(4,5), dtype=flow.float32, device=flow.device(device), requires_grad= True
        )
        of_out = flow.linalg.norm(input,ord = ord)
        of_out.backward()
        np_out_grad = _np_matrix_norm_backward(input.numpy(),ord = ord)
