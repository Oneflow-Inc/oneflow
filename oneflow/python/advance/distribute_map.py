...
@oneflow_export("advance.distribute_map")
def distribute_map(xs, f, axis = 0):
    _AssertInputOrOutput(xs)
    if isinstance(xs, (list, tuple)) == False: xs = [xs]
    splitted_xs = [oneflow.advance.distribute_split(x, axis=axis) for x in xs]
    ...
    result = [oneflow.advance.distribute_concat(x, axis=axis) for x in zip(*results)]
