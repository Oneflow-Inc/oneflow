...
def distribute_map(xs, f, axis=0):
    ...
    results = [
        _UnderSingleDevicePlacementScope(f, *x) for x in zip(*splitted_xs)
    ]
    ...
    result = [
        oneflow.advanced.distribute_concat(x, axis=axis) for x in zip(*results)
    ]
...
def _AssertInputOrOutput(xs):
    ...
    if isinstance(xs, (list, tuple)):
        assert len(xs) > 0
        assert all(
            [isinstance(x, remote_blob_util.ConsistentBlob) for x in xs]
        )
...
def _EachMachineIdAndDeviceId(placement_scope):
    for (
        machine_id,
        device_id_list,
    ) in placement_scope.machine_id2device_id_list.items():
