...
Maybe<void> RegisterForeignCallbackOnlyOnce(ForeignCallback *callback) {
  CHECK_ISNULL_OR_RETURN(Global<ForeignCallback>::Get())
...
Maybe<void> RegisterWatcherOnlyOnce(ForeignWatcher *watcher) {
  CHECK_ISNULL_OR_RETURN(Global<ForeignWatcher>::Get())
...
Maybe<bool> IsOpTypeCaseCpuSupportOnly(int64_t op_type_case) {
  ...
  return static_cast<bool>(
      *std::unique_ptr<OnlyCpuSupport>(NewObj<OnlyCpuSupport>(op_type_case)));
}
...
Maybe<bool> IsOpTypeNameCpuSupportOnly(const std::string &op_type_name) {
  ...
  CHECK_OR_RETURN(val != nullptr) << "op_type_name " << op_type_name
                                  << " not register";
...
void FixCpuDeviceNum(ConfigProto *config_proto) {
  if (config_proto->resource().cpu_device_num() > 0) {
    return;
  }
  config_proto->mutable_resource()->set_cpu_device_num(
...
Maybe<void> InitGlobalSession(const std::string &config_proto_str) {
  ...
  Global<SessionGlobalObjectsScope>::SetAllocated(
      new SessionGlobalObjectsScope());
...
Maybe<void> StartGlobalSession() {
  CHECK_NOTNULL_OR_RETURN(Global<SessionGlobalObjectsScope>::Get())
      << "session not found";
  CHECK_OR_RETURN(Global<MachineCtx>::Get()->IsThisMachineMaster());
  ...
  if (job_set.job().empty()) {
    return Error::JobSetEmptyError() << "no function defined";
  }
  CHECK_ISNULL_OR_RETURN(Global<Oneflow>::Get());
  Global<CtrlClient>::Get()->PushKV("session_job_set", job_set);
  Global<const InterJobReuseMemStrategy>::New(
      job_set.inter_job_reuse_mem_strategy());
  Global<Oneflow>::New();
...
Maybe<std::string> GetSerializedInterUserJobInfo() {
  ...
  std::string ret;
  google::protobuf::TextFormat::PrintToString(*Global<InterUserJobInfo>::Get(),
                                              &ret);
  return ret;
}
...
Maybe<void> LaunchJob(const std::shared_ptr<oneflow::ForeignJobInstance> &cb) {
  ...
  Global<BufferMgr<int64_t>>::Get()
      ->Get(kBufferNameGlobalWaitJobId)
      ->Send(job_id);
  return Maybe<void>::Ok();
}
...
Maybe<std::string> CheckAndCompleteUserOpConf(const std::string &op_conf_str) {
  ...
  CHECK_OR_RETURN(TxtString2PbMessage(op_conf_str, &op_conf))
      << "operator conf parse failed";
...
Maybe<std::string> InferOpConf(const std::string &op_conf_str,
  ...
  OperatorConf op_conf;
  CHECK_OR_RETURN(TxtString2PbMessage(op_conf_str, &op_conf))
      << "OperatorConf parse failed";
  CHECK_OR_RETURN(op_conf.has_scope_symbol_id());
  OpNodeSignature upstream_signature;
  CHECK_OR_RETURN(
      TxtString2PbMessage(upstream_signature_str, &upstream_signature))
  ...
  const auto &op =
      JUST(ConstructAndInferOp(op_conf, upstream_signature, scope));
...
Maybe<void> RunLogicalInstruction(const std::string &instruction_list_str,
  ...
  return eager::RunLogicalInstruction(instruction_list_str,
                                      eager_symbol_list_str);
}

Maybe<void> RunPhysicalInstruction(const std::string &instruction_list_str,
                                   const std::string &eager_symbol_list_str) {
  return eager::RunPhysicalInstruction(instruction_list_str,
                                       eager_symbol_list_str);
}
...
Maybe<long long> NewPhysicalObjectId() {
  CHECK_NOTNULL_OR_RETURN(Global<MachineCtx>::Get());
  return vm::IdUtil::NewPhysicalObjectId(
      Global<MachineCtx>::Get()->this_machine_id());
}

Maybe<long long> NewPhysicalSymbolId() {
  CHECK_NOTNULL_OR_RETURN(Global<MachineCtx>::Get());
  return vm::IdUtil::NewPhysicalSymbolId(
      Global<MachineCtx>::Get()->this_machine_id());
}
