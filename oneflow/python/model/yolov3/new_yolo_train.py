...
import numpy as np
import os
from darknet53 import YoloTrainNet
...
def yolo_train_job(images=input_blob_def_dict["images"], gt_boxes=input_blob_def_dict["gt_boxes"], gt_labels=input_blob_def_dict["gt_labels"], gt_valid_num=input_blob_def_dict["gt_valid_num"]):
    ...
    images = flow.unpack(images, num_piece_in_batch)
    gt_boxes = flow.unpack(gt_boxes, num_piece_in_batch)
    gt_labels = flow.unpack(gt_labels, num_piece_in_batch)
    gt_valid_num = flow.unpack(gt_valid_num, num_piece_in_batch)
