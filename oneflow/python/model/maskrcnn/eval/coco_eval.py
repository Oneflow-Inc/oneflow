...
def prepare_for_coco_segmentation(predictions, dataset):
    ...
    for image_id, prediction in tqdm(enumerate(predictions)):
        ...
        if list(masks.shape[-2:]) != [image_height, image_width]:
            masks = masker(np.expand_dims(masks, axis=0), prediction)
            masks = masks[0]
...
def evaluate_box_proposals(
    ...
    for image_id, prediction in enumerate(predictions):
        ...
        gt_boxes = np.array(gt_boxes).reshape(-1, 4)  # guard against no boxes
        gt_boxes = BoxList(gt_boxes, (image_width, image_height), mode="xywh").convert(
            "xyxy"
        )
        gt_areas = np.array([obj["area"] for obj in anno if obj["iscrowd"] == 0])
        ...
        _gt_overlaps = np.zeros(len(gt_boxes))
    ...
    gt_overlaps, _ = np.sort(gt_overlaps)
