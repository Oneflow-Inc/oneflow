...
class MaskPostProcessor():
    ...
    def forward(self, x, boxes):
        ...
        mask_prob = np.split(mask_prob, boxes_per_image)[:-1]
...
def expand_masks(mask, padding):
    ...
    padded_mask = np.zeros((N, 1, M + pad2, M + pad2), dtype=mask.dtype)
...
class Masker(object):
    ...
    def forward_single_image(self, masks, boxes):
        ...
        if len(res) > 0:
            ...
            res = np.empty((0, 1, masks.shape[-2], masks.shape[-1]), dtype=masks.dtype)
        return res
