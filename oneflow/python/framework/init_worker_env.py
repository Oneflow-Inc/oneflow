...
def SendBinaryAndConfig2Worker(machine, oneflow_worker_path, config_proto_path):
...
def init_worker(config_proto):
    ...
    global _temp_run_dir
    _temp_run_dir = os.getenv("HOME") + "/" + str(uuid.uuid1())
    run_dir = _temp_run_dir
    ...
    for machine in resource.machine:
        if machine.id == 0:
            continue
        SendBinaryAndConfig2Worker(machine, oneflow_worker_path, config_file.name)
