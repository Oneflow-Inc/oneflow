...
def JobBuildAndInferCtx_MirroredBlobGetDataType(job_name, lbn):
    job_name = str(job_name)
    lbn = str(lbn)
    dtype = oneflow_api.JobBuildAndInferCtx_MirroredBlobGetDataType(job_name, lbn)
    return int(dtype)
...
def JobBuildAndInferCtx_MirroredBlobIsDynamic(job_name, lbn):
    job_name = str(job_name)
    lbn = str(lbn)
    return oneflow_api.JobBuildAndInferCtx_MirroredBlobIsDynamic(job_name, lbn)


def JobBuildAndInferCtx_MirroredBlobIsTensorList(job_name, lbn):
    job_name = str(job_name)
    lbn = str(lbn)
    return oneflow_api.JobBuildAndInferCtx_MirroredBlobIsTensorList(job_name, lbn)


def JobBuildAndInferCtx_MirroredBlobGetBatchAxis(job_name, lbn):
    job_name = str(job_name)
    lbn = str(lbn)
    batch_axis_str = oneflow_api.JobBuildAndInferCtx_MirroredBlobGetBatchAxis(
        job_name, lbn
    )
...
def JobBuildAndInferCtx_MirroredBlobGetSplitAxisFromProducerView(job_name, lbn):
    job_name = str(job_name)
    lbn = str(lbn)
    split_axis_str = oneflow_api.JobBuildAndInferCtx_MirroredBlobGetSplitAxisFromProducerView(
        job_name, lbn
    )
...
def JobBuildAndInferCtx_MirroredBlobGetParallelConfFromProducerView(job_name, lbn):
    ...
    parallel_conf_str = GetParallelConf(job_name, lbn)
