...
class Tensor:
    ...
    def data(self):
        if self._local_or_consistent_tensor is not None:
            return Tensor(self._local_or_consistent_tensor.data)
        else:
            return None
    ...
    def grad(self):
        if self._local_or_consistent_tensor is not None:
            if self._local_or_consistent_tensor.grad is not None:
                return Tensor(self._local_or_consistent_tensor.grad)
        else:
            return None
    ...
    def detach(self):
        if self._local_or_consistent_tensor is not None:
            return Tensor(self._local_or_consistent_tensor.detach())
        else:
            return None
...
def _input_args_is_tensor(*args):
    return len(args) == 1 and isinstance(args[0], Tensor)
