...
int RunSerializedJobSet(const oneflow::JobSet& job_set) { return Main(job_set); }
int InitRunTimeJobSet() { TODO(); }
...
#define DATA_TYPE_FUNC_NAME_PAIR(type_cpp, type_proto) {type_proto, "OfBlob_CopyToBuffer_" #type_cpp},
...
#undef DATA_TYPE_FUNC_NAME_PAIR
  ...
  std::string OfBlob_GetCopyFromBufferFuncName(uint64_t of_blob_ptr) {
    using namespace oneflow; 
    static const HashMap<int64_t, std::string> data_type2func_name{
#define DATA_TYPE_FUNC_NAME_PAIR(type_cpp, type_proto) {type_proto, "OfBlob_CopyFromBuffer_" #type_cpp},
        OF_PP_FOR_EACH_TUPLE(DATA_TYPE_FUNC_NAME_PAIR, POD_DATA_TYPE_SEQ)
#undef DATA_TYPE_FUNC_NAME_PAIR
    };
    auto* of_blob = reinterpret_cast<OfBlob*>(of_blob_ptr);
