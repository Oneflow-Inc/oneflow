...
void RegisterForeignCallbackOnlyOnce(oneflow::ForeignCallback *callback,
                                     std::string *error_str) {
  return oneflow::RegisterForeignCallbackOnlyOnce(callback)
      .GetDataAndSerializedErrorProto(error_str);
}

void RegisterWatcherOnlyOnce(oneflow::ForeignWatcher *watcher,
                             std::string *error_str) {
  return oneflow::RegisterWatcherOnlyOnce(watcher)
      .GetDataAndSerializedErrorProto(error_str);
}
...
bool IsOpTypeNameCpuSupportOnly(const std::string &op_type_name,
                                std::string *error_str) {
...
std::string CurrentResource(std::string *error_str) {
  return oneflow::CurrentResource().GetDataAndSerializedErrorProto(
...
void InitEnv(const std::string &env_proto_str, std::string *error_str) {
  return oneflow::InitEnv(env_proto_str)
      .GetDataAndSerializedErrorProto(error_str);
}
...
void InitGlobalSession(const std::string &config_proto_str,
                       std::string *error_str) {
  using namespace oneflow;
  return InitGlobalSession(config_proto_str)
      .GetDataAndSerializedErrorProto(error_str);
}

void DestroyGlobalSession(std::string *error_str) {
  return oneflow::DestroyGlobalSession().GetDataAndSerializedErrorProto(
      error_str);
}

void StartGlobalSession(std::string *error_str) {
  return oneflow::StartGlobalSession().GetDataAndSerializedErrorProto(
      error_str);
}
...
std::string GetSerializedInterUserJobInfo(std::string *error_str) {
  return oneflow::GetSerializedInterUserJobInfo()
      .GetDataAndSerializedErrorProto(error_str, std::string(""));
}

std::string GetSerializedJobSet(std::string *error_str) {
  return oneflow::GetSerializedJobSet().GetDataAndSerializedErrorProto(
      error_str, std::string(""));
}

std::string GetSerializedStructureGraph(std::string *error_str) {
  return oneflow::GetSerializedStructureGraph().GetDataAndSerializedErrorProto(
      error_str, std::string(""));
}

std::string GetFunctionConfigDef(std::string *error_str) {
  return oneflow::GetFunctionConfigDef().GetDataAndSerializedErrorProto(
      error_str, std::string(""));
}
...
std::string GetMachine2DeviceIdListOFRecordFromParallelConf(
    const std::string &parallel_conf, std::string *error_str) {
  return oneflow::GetSerializedMachineId2DeviceIdListOFRecord(parallel_conf)
      .GetDataAndSerializedErrorProto(error_str, std::string(""));
}

long GetUserOpAttrType(const std::string &op_type_name,
                       const std::string &attr_name, std::string *error_str) {
...
bool IsInterfaceOpTypeCase(int64_t op_type_case) {
  return oneflow::IsClassRegistered<oneflow::IsInterfaceOpConf4OpTypeCase>(
      op_type_case);
}

long GetOpParallelSymbolId(const std::string &serialized_op_conf,
                           std::string *error_str) {
...
void RunLogicalInstruction(const std::string &vm_instruction_list,
                           ...
                           std::string *error_str) {
  return oneflow::RunLogicalInstruction(vm_instruction_list,
...
void RunPhysicalInstruction(const std::string &vm_instruction_list,
                            const std::string &eager_symbol_list_str,
                            std::string *error_str) {
  return oneflow::RunPhysicalInstruction(vm_instruction_list,
                                         eager_symbol_list_str)
      .GetDataAndSerializedErrorProto(error_str);
}

long CurrentMachineId(std::string *error_str) {
  return oneflow::CurrentMachineId().GetDataAndSerializedErrorProto(error_str,
                                                                    0LL);
}

long NewLogicalObjectId(std::string *error_str) {
  return oneflow::NewLogicalObjectId().GetDataAndSerializedErrorProto(error_str,
                                                                      0LL);
}

long NewLogicalSymbolId(std::string *error_str) {
  return oneflow::NewLogicalSymbolId().GetDataAndSerializedErrorProto(error_str,
                                                                      0LL);
}

long NewPhysicalObjectId(std::string *error_str) {
  return oneflow::NewPhysicalObjectId().GetDataAndSerializedErrorProto(
      error_str, 0LL);
}

long NewPhysicalSymbolId(std::string *error_str) {
  return oneflow::NewPhysicalSymbolId().GetDataAndSerializedErrorProto(
      error_str, 0LL);
}
...
void OfBlob_CurMutTensorCopyShapeFrom(uint64_t of_blob_ptr, long *array,
                                      int size) {
  using namespace oneflow;
  auto *of_blob = reinterpret_cast<OfBlob *>(of_blob_ptr);
