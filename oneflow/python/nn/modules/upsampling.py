...
@oneflow_export("nn.interpolate")
@experimental_api
class Interpolate(Upsample):
    r"""Down/up samples the input to either the given size or the given scale_factor.
    ...
    The modes available for resizing are: nearest,  bilinear.
    ...
    .. code-block:: python
        ...
        >>> input = flow.Tensor(np.arange(1, 5).reshape((1, 1, 2, 2)), dtype=flow.float32)
        >>> input = input.to("cuda")
        >>> m = flow.nn.interpolate(scale_factor=2.0, mode="nearest")
        >>> output = m(input)
        >>> output #doctest: +ELLIPSIS
        tensor([[[[1., 1., 2., 2.],
                  ...
