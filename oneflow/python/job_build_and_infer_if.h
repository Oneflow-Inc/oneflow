...
void JobBuildAndInferCtx_Open(const std::string &job_name,
                              std::string *error_str) {
  return oneflow::JobBuildAndInferCtx_Open(job_name)
      .GetDataAndSerializedErrorProto(error_str);
}

std::string JobBuildAndInferCtx_GetCurrentJobName(std::string *error_str) {
  return oneflow::JobBuildAndInferCtx_GetCurrentJobName()
      .GetDataAndSerializedErrorProto(error_str, std::string(""));
}

void JobBuildAndInferCtx_Close(std::string *error_str) {
  return oneflow::JobBuildAndInferCtx_Close().GetDataAndSerializedErrorProto(
      error_str);
}

void CurJobBuildAndInferCtx_CheckJob(std::string *error_str) {
  return oneflow::CurJobBuildAndInferCtx_CheckJob()
      .GetDataAndSerializedErrorProto(error_str);
}
...
void CurJobBuildAndInferCtx_SetTrainConf(
    const std::string &serialized_train_conf, std::string *error_str) {
...
void CurJobBuildAndInferCtx_Complete(std::string *error_str) {
  return oneflow::CurJobBuildAndInferCtx_Complete()
      .GetDataAndSerializedErrorProto(error_str);
}

bool CurJobBuildAndInferCtx_HasJobConf(std::string *error_str) {
  return oneflow::CurJobBuildAndInferCtx_HasJobConf()
...
std::string CurJobBuildAndInferCtx_AddAndInferMirroredOp(
  ...
  return oneflow::CurJobBuildAndInferCtx_AddAndInferMirroredOp(
             serialized_op_conf)
      .GetDataAndSerializedErrorProto(error_str, std::string(""));
}
...
std::string CurJobBuildAndInferCtx_AddAndInferConsistentOp(
    const std::string &serialized_op_conf, std::string *error_str) {
  return oneflow::CurJobBuildAndInferCtx_AddAndInferConsistentOp(
             serialized_op_conf)
      .GetDataAndSerializedErrorProto(error_str, std::string(""));
}

void CurJobBuildAndInferCtx_AddLossLogicalBlobName(const std::string &lbn,
                                                   std::string *error_str) {
  return oneflow::CurJobBuildAndInferCtx_AddLossLogicalBlobName(lbn)
      .GetDataAndSerializedErrorProto(error_str);
}

void CurJobBuildAndInferCtx_AddLbiAndDiffWatcherUuidPair(
    const std::string &lbi_uuid_pair, std::string *error_str) {
  return oneflow::CurJobBuildAndInferCtx_AddLbiAndDiffWatcherUuidPair(
             lbi_uuid_pair)
      .GetDataAndSerializedErrorProto(error_str);
}

std::string JobBuildAndInferCtx_GetSerializedIdListAsStaticShape(
    const std::string &job_name, const std::string &lbn,
    std::string *error_str) {
  return oneflow::JobBuildAndInferCtx_GetSerializedIdListAsStaticShape(job_name,
                                                                       lbn)
      .GetDataAndSerializedErrorProto(error_str, std::string(""));
}

long long JobBuildAndInferCtx_GetDataType(const std::string &job_name,
...
bool JobBuildAndInferCtx_IsDynamic(const std::string &job_name,
                                   const std::string &lbn,
                                   std::string *error_str) {
...
bool JobBuildAndInferCtx_DisableBoxing(const std::string &job_name,
                                       const std::string &lbn,
                                       std::string *error_str) {
...
bool JobBuildAndInferCtx_IsTensorList(const std::string &job_name,
                                      const std::string &lbn,
                                      std::string *error_str) {
...
std::string JobBuildAndInferCtx_GetBatchAxis(const std::string &job_name,
                                             const std::string &lbn,
                                             std::string *error_str) {
...
JobBuildAndInferCtx_GetSplitAxisFromProducerView(const std::string &job_name,
                                                 const std::string &lbn,
                                                 std::string *error_str) {
  return oneflow::JobBuildAndInferCtx_GetSplitAxisFromProducerView(job_name,
                                                                   lbn)
      .GetDataAndSerializedErrorProto(error_str, std::string(""));
}
...
std::string JobBuildAndInferCtx_GetSerializedParallelConfFromProducerView(
    const std::string &job_name, const std::string &lbn,
    std::string *error_str) {
  return oneflow::JobBuildAndInferCtx_GetSerializedParallelConfFromProducerView(
             job_name, lbn)
      .GetDataAndSerializedErrorProto(error_str, std::string(""));
}

bool JobBuildAndInferCtx_IsMirroredBlob(const std::string &job_name,
                                        const std::string &lbn,
                                        std::string *error_str) {
...
std::string JobBuildAndInferCtx_MirroredBlobGetSerializedSubLbi(
    const std::string &job_name, const std::string &lbn, int index,
    std::string *error_str) {
  return oneflow::JobBuildAndInferCtx_MirroredBlobGetSubLbi(job_name, lbn,
                                                            index)
      .GetDataAndSerializedErrorProto(error_str, std::string(""));
}
...
std::string JobBuildAndInferCtx_MirroredBlobGetSerializedIdListAsStaticShape(
    ...
    std::string *error_str) {
  return oneflow::
      JobBuildAndInferCtx_MirroredBlobGetSerializedIdListAsStaticShape(job_name,
                                                                       lbn)
          .GetDataAndSerializedErrorProto(error_str, std::string(""));
}

long long
JobBuildAndInferCtx_MirroredBlobGetDataType(const std::string &job_name,
                                            const std::string &lbn,
                                            std::string *error_str) {
...
bool JobBuildAndInferCtx_MirroredBlobIsDynamic(const std::string &job_name,
                                               const std::string &lbn,
                                               std::string *error_str) {
...
std::string
JobBuildAndInferCtx_MirroredBlobGetBatchAxis(const std::string &job_name,
                                             const std::string &lbn,
                                             std::string *error_str) {
...
std::string JobBuildAndInferCtx_MirroredBlobGetSplitAxisFromProducerView(
    const std::string &job_name, const std::string &lbn,
    std::string *error_str) {
  return oneflow::JobBuildAndInferCtx_MirroredBlobGetSplitAxisFromProducerView(
             job_name, lbn)
      .GetDataAndSerializedErrorProto(error_str, std::string(""));
}
