...
GetJobBuildAndInferCtx(const std::string &job_name) {
...
Maybe<void> CurJobBuildAndInferCtx_CheckJob() {
  return JUST(GetCurInferCtx())->CheckJob();
}

Maybe<void>
CurJobBuildAndInferCtx_SetJobConf(const std::string &serialized_job_conf) {
  // parse
  JobConfigProto job_conf;
  CHECK_OR_RETURN(TxtString2PbMessage(serialized_job_conf, &job_conf))
      << "job conf parse failed";
  return JUST(GetCurInferCtx())->SetJobConf(job_conf);
}

Maybe<void>
CurJobBuildAndInferCtx_SetTrainConf(const std::string &train_conf_str) {
  TrainConf train_conf;
  CHECK_OR_RETURN(TxtString2PbMessage(train_conf_str, &train_conf))
      << "train conf parse failed";
  return JUST(GetCurInferCtx())->SetTrainConf(train_conf);
}

Maybe<void> CurJobBuildAndInferCtx_Complete() {
  return JUST(GetCurInferCtx())->Complete();
}

Maybe<bool> CurJobBuildAndInferCtx_HasJobConf() {
  return JUST(GetCurInferCtx())->HasJobConf();
}

Maybe<std::string>
CurJobBuildAndInferCtx_AddAndInferMirroredOp(const std::string &op_conf_str) {
...
Maybe<std::string>
CurJobBuildAndInferCtx_AddAndInferConsistentOp(const std::string &op_conf_str) {
  OperatorConf op_conf;
  CHECK_OR_RETURN(TxtString2PbMessage(op_conf_str, &op_conf))
      << "operator conf parse failed";
  auto *ctx = JUST(GetCurInferCtx());
...
Maybe<std::string> JobBuildAndInferCtx_GetSerializedIdListAsStaticShape(
...
Maybe<bool> JobBuildAndInferCtx_IsDynamic(const std::string &job_name,
                                          const std::string &lbn) {
  auto *ctx = JUST(GetJobBuildAndInferCtx(job_name));
  return ctx->IsDynamic(lbn);
}

Maybe<bool> JobBuildAndInferCtx_DisableBoxing(const std::string &job_name,
                                              const std::string &lbn) {
  auto *ctx = JUST(GetJobBuildAndInferCtx(job_name));
  return ctx->DisableBoxing(lbn);
}

Maybe<bool> JobBuildAndInferCtx_IsTensorList(const std::string &job_name,
                                             const std::string &lbn) {
  auto *ctx = JUST(GetJobBuildAndInferCtx(job_name));
...
JobBuildAndInferCtx_GetSerializedParallelConfFromProducerView(
  ...
  return PbMessage2TxtString(
      JUST(ctx->GetParallelDescFromProducerView(lbn))->parallel_conf());
}

Maybe<void>
CurJobBuildAndInferCtx_AddLossLogicalBlobName(const std::string &lbn) {
...
Maybe<int>
JobBuildAndInferCtx_MirroredBlobGetNumSubLbi(const std::string &job_name,
                                             const std::string &lbn) {
  auto *ctx = JUST(GetJobBuildAndInferCtx(job_name));
...
Maybe<std::string>
JobBuildAndInferCtx_MirroredBlobGetSubLbi(const std::string &job_name,
...
Maybe<bool>
JobBuildAndInferCtx_MirroredBlobIsDynamic(const std::string &job_name,
                                          const std::string &lbn) {
  auto *ctx = JUST(GetJobBuildAndInferCtx(job_name));
...
Maybe<bool>
JobBuildAndInferCtx_MirroredBlobIsTensorList(const std::string &job_name,
                                             const std::string &lbn) {
  auto *ctx = JUST(GetJobBuildAndInferCtx(job_name));
...
Maybe<std::string> JobBuildAndInferCtx_MirroredBlobGetSplitAxisFromProducerView(
  ...
  auto *ctx = JUST(GetJobBuildAndInferCtx(job_name));
  return PbMessage2TxtString(
      *JUST(ctx->MirroredBlobGetSplitAxisFromProducerView(lbn)));
}
...
JobBuildAndInferCtx_MirroredBlobGetSerializedParallelConfFromProducerView(
  ...
  auto *ctx = JUST(GetJobBuildAndInferCtx(job_name));
  return PbMessage2TxtString(
      JUST(ctx->MirroredBlobGetParallelDescFromProducerView(lbn))
          ->parallel_conf());
}
