...
def init_worker(scp_binary = True, use_uuid = True):
    ...
    for machine in env_proto.machine:
        if machine.id == 0:
            continue
        _SendBinaryAndConfig2Worker(machine, oneflow_worker_path, env_file.name, run_dir, scp_binary)
...
def _SendBinaryAndConfig2Worker(machine, oneflow_worker_path, env_proto_path, run_dir, scp_binary):
