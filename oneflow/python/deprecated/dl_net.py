...
class DLNet(object):
  ...
  def CreateOperator(self, op_type_name, op_name, input_lbn, output_bn, **kw):
    placement_ctx.CurPlacementGroupAddOpName(op_name)
    return _CreateOperator(self.dl_net_conf_.op.add(), op_type_name,
                           op_name, input_lbn, output_bn, kw);
  ...
  def CurScopeOpNamesExceptCpuOnly(self):
    ...
    for op in self.dl_net_conf_.op:
      if (op.name.startswith(prefix)):
        if compiler.IsOpConfOnlyCpuSupported(op):
          yield op.name;
