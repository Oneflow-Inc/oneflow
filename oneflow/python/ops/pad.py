...
def pad_grad(x, paddings, constant_value=0, name=None):
    op_conf = op_conf_util.OperatorConf()
    setattr(op_conf, "name", name if name is not None else id_util.UniqueStr('PadGrad_'))
    setattr(op_conf.pad_grad_conf, "in", x.logical_blob_name)
    setattr(op_conf.pad_grad_conf, "out", "out")
    if isinstance(paddings, (list, tuple)):
        assert len(paddings) == len(x.static_shape), ValueError(
            "paddings must be the same size of input dims"
        )
        ...
        for i in range(max(zero_padding_num - 1, 0), len(x.static_shape)):
            ...
            trim_paddings.append(paddings[i][0])
            trim_paddings.append(paddings[i][1])
        op_conf.pad_grad_conf.paddings.extend(trim_paddings)
    else:
        raise ValueError("paddings must be a tuple or a list.")
    op_conf.pad_grad_conf.constant_value = constant_value
    compile_context.CurJobAddOp(op_conf)
    lbi = logical_blob_id_util.LogicalBlobId()
    lbi.op_name = op_conf.name
    lbi.blob_name = "out"
