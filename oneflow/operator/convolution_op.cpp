...
void ConvolutionOp::InferShape4FwBlobs(
  ...
  auto pad = CheckDimPara4CnnAndPooling(conv_conf.pad(), 
                                        conv_conf.pad_h(), 
                                        conv_conf.pad_w());
  auto kernel = CheckDimPara4CnnAndPooling(conv_conf.kernel_size(), 
                                           conv_conf.kernel_h(),
                                           conv_conf.kernel_w());
  auto stride = CheckDimPara4CnnAndPooling(conv_conf.stride(), 
                                           conv_conf.stride_h(),
                                           conv_conf.stride_w());
