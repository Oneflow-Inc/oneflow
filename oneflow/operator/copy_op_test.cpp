...
TEST(CopyOp, copy_3_3x4_shape) {
  ...
  copy_op->InferShape4FwBlobs(fp, kDataParallel, 2, 10);

  for(size_t i = 0;i < copy_op->output_bns().size();++ i){
    Shape* input_shape_ptr = shape_manager.bn2ShapePtr(copy_op->input_bns().at(i));
    Shape* output_shape_ptr = shape_manager.bn2ShapePtr(copy_op->output_bns().at(i));
