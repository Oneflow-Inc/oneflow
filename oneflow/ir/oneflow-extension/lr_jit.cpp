...
class JIT_Engine final {
 ...
 public:
  ...
  explicit JIT_Engine(const PyASTNodeWrapper& ast);
  double Invoke(double base_lr, int64_t step);
};

JIT_Engine::JIT_Engine(const PyASTNodeWrapper& ast) {
...
void LR_JIT::Register(const string& function_id, const PyASTNodeWrapper& ast) {
