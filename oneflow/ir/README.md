...
- There is parallel signature as 0 for OneFlow Ops in MLIR. It is implemented as MLIR dialect attribute. Some examples:
    - 1D SBP
        ```mlir
        %100 = "oneflow.relu"(%99) {parallel_signature = #oneflow.parallel_signature<[#oneflow.s<0>], [#oneflow.s<0>]>, ...
        ```
    - multiple inputs and outputs 1D SBP
        ```mlir
        %102 = "oneflow.add_n2"(%101, %97) {parallel_signature = #oneflow.parallel_signature<[#oneflow.s<0>, #oneflow.s<0>], [#oneflow.s<0>]>, ...
        ```
    - 2D SBP `matmul`
        ```
        %120 = "oneflow.matmul"(%119, %output_105) {parallel_signature = #oneflow.parallel_signature<[[#oneflow.s<0>, #oneflow.p], #oneflow.s<0>], [#oneflow.s<0>]>, ...
        ```
