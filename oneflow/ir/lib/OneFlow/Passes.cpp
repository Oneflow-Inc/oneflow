...
>>>>>>> fcf3d3972fdf00eb134cf76a724be7e770c84d16
...
#include <string>
  ...
  LLVM::LLVMFuncOp DeclareKernelLaunchCInterface(::mlir::PatternRewriter& rewriter,
    ...
    if (!(func = module->lookupSymbol<LLVM::LLVMFuncOp>(c_api_callee))) {
      ...
      auto func_type =
          LLVM::LLVMFunctionType::get(void_type, {llvm_ptr_type, llvm_ptr_type}, false);
      func =
          rewriter.create<LLVM::LLVMFuncOp>(loc, c_api_callee, func_type, LLVM::Linkage::External);
  ...
  ::llvm::SmallVector<::mlir::Value, 4> FuseConv2DBatchNorm(::mlir::PatternRewriter& rewriter,
    ...
    if (auto conv_op = llvm::dyn_cast<oneflow::Conv2DOp>(conv_result.getDefiningOp())) {
      if (auto bn_op =
              llvm::dyn_cast<oneflow::NormalizationInferenceOp>(bn_result.getDefiningOp())) {
        ...
        auto sqrt_op = rewriter.create<oneflow::SqrtOp>(
            conv_op->getLoc(), conv_op->getResultTypes(), SmallVector<Value, 4>({add_op.out()}),
        ...
        reshape_op_attrs.set(
            "shape",
            ArrayAttr::get(
                ctx, llvm::to_vector<8>(llvm::map_range(
                         ArrayRef<int64_t>(bn_gamma_new_shape),
                         [&](int64_t v) -> Attribute { return rewriter.getI64IntegerAttr(v); }))));
  ...
  ::llvm::SmallVector<::mlir::Value, 4> CreateFusedBiasAddMaskScale(
      ::mlir::PatternRewriter& rewriter, OpResult dropout_result, OpResult bias_add_result,
      Operation* mask) {
