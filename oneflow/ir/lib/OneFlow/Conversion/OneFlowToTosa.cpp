...
namespace tosa_mode {
const std::string CODEGEN = "codegen";
...
LogicalResult NormalizationInferenceOpmatchAndRewrite(NormalizationInferenceOp op,
  ...
  auto epsilon = rewriter.create<tosa::ConstOp>(loc, epsilon_type,
                                                DenseElementsAttr::get(epsilon_type, op.epsilon()));
  auto mean = reshape_dim(out_type, op.moving_mean());
  auto variance = reshape_dim(out_type, op.moving_variance());
  auto gamma = reshape_dim(out_type, op.gamma());
  auto beta = reshape_dim(out_type, op.beta());
  auto output = op.y();
  auto x = op.x();
...
struct OneFlowLoweringToTosaPass : public LowerOneFlowToTosaPassBase<OneFlowLoweringToTosaPass> {
  OneFlowLoweringToTosaPass() = default;
  OneFlowLoweringToTosaPass(const OneFlowLoweringToTosaPass& other)
      : LowerOneFlowToTosaPassBase(other) {}
  void runOnOperation() override;

 private:
  void lower4Backend();
  void lower4Codegen();
  Option<std::string> mode_{*this, "mode", llvm::cl::desc("the mode of this lowering pass"),
                            llvm::cl::init(tosa_mode::BACKEND)};
};
...
void OneFlowLoweringToTosaPass::lower4Backend() {
...
void OneFlowLoweringToTosaPass::runOnOperation() {
  if (mode_.c_str() == tosa_mode::BACKEND) {
    lower4Backend();
  } else if (mode_.c_str() == tosa_mode::CODEGEN) {
    lower4Codegen();
  }
}
