...
template<class T>
std::vector<AllocOpInfo> getAllocInfoList(T func);

std::vector<AllocOpInfo> getAllocInfoList(gpu::LaunchOp launchOp) {
  std::vector<memref::AllocOp> list;
  // collect all memref.alloc ops and gpu.launch_func ops
  launchOp->walk([&](memref::AllocOp alloc) { list.push_back(alloc); });
...
class FoldAllocToSubviewPass : public FoldAllocToSubviewPassBase<FoldAllocToSubviewPass> {
  void runOnOperation() override {
    ...
    if (list.size() == 1 && isMemPool(list.front().val_)) { return; }
    ...
    replaceAllocwithSubview(op, builder, ret);
  }
};
