...
static Attribute GetNumHeadsFromTranpose(PatternRewriter& rewriter, Operation* transpose) {
  ...
  NamedAttrList GetUserOpCommonAttrs(MLIRContext * ctx, const std::string& op_name) {
    ...
    attrs.set(OpTrait::IsOpConfCompatible<void>::getDeviceNameAttr(),
              ArrayAttr::get(ctx, llvm::to_vector<8>(llvm::map_range(
                                      ArrayRef<StringRef>({"@0:0"}), [&](StringRef v) -> Attribute {
  ...
  NamedAttrList GetJitOpAttributes(PatternRewriter & rewriter, StringRef op_name,
  ...
  static LogicalResult IsPaddingCouldBeAssimilatedIntoConv(
      PatternRewriter & rewriter, Attribute padding_before, Attribute padding_after,
      Attribute data_format) {
    if (padding_before.cast<ArrayAttr>().size() == 4
        && padding_after.cast<ArrayAttr>().size() == 4) {
