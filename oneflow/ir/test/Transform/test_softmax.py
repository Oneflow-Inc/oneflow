...
class SoftMaxModule(flow.nn.Module):
    def __init__(self):
        super().__init__()

    def forward(self, x):
        return flow.softmax(x)


def do_softmax_graph(test_case, data, with_cuda):
    x = flow.tensor(data, dtype=flow.float32)
    if with_cuda:
        x = x.cuda()
    module_to_run = SoftMaxModule()
...
class TestFuseCastScale(oneflow.unittest.MLIRTestCase):
    ...
    def test_relu_graph(test_case):
        ...
        if oneflow.sysconfig.with_cuda():
            do_softmax_graph(test_case, np.random.random((16, 128, 128)), True)
