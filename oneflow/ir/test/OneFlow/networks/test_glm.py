...
class ColumnParallelLinear(flow.nn.Module):
    def __init__(
        ...
        self.weight = Parameter(
            flow.Tensor(self.output_size_per_partition, self.input_size)
        )
...
class RowParallelLinear(flow.nn.Module):
    def __init__(
        ...
        self.weight = Parameter(
            flow.Tensor(self.output_size, self.input_size_per_partition)
        )
        # self.weight.model_parallel = True
    ...
    def __init__(
        ...
        if relative_encoding:
            self.relative = ColumnParallelLinear(
                hidden_size, hidden_size, gather_output=False, init_method=init_method
            )
