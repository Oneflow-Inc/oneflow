...
module  {
  func @IdempotentJob() {
    ...
    %0 = "oneflow.relu"(%data_output) {device_name = ["0:0-0"], device_tag = "gpu", hierarchy = [1], op_name = "Relu_2", op_type_name = "relu", output_lbn_segment_sizes = [1 : i32], output_lbns = ["Relu_2/out_0"], scope_symbol_id = 4611686018427420670 : i64} : (tensor<96x96xf32>) -> tensor<96x96xf32>
    // CHECK: %0 = "oneflow.relu"(%data_output)
    %1 = "oneflow.relu"(%data_output) {device_name = ["0:0-0"], device_tag = "gpu", hierarchy = [1], op_name = "Relu_1", op_type_name = "relu", output_lbn_segment_sizes = [1 : i32], output_lbns = ["Relu_1/out_0"], scope_symbol_id = 4611686018427420670 : i64} : (tensor<96x96xf32>) -> tensor<96x96xf32>
