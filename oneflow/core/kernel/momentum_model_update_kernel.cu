...
template<typename T>
__global__ void UpdateModelGpu(int64_t n, T beta, T learning_rate, T l1, T l2, const T* model_diff,
                               T* model, T* momentum) {
...
class MomentumMdUpdateKernelUtil<DeviceType::kGPU, T> final {
 public:
  static void UpdateModel(DeviceCtx* ctx, int64_t n, T beta, T learning_rate, const T l1,
    ...
    UpdateModelGpu<T><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock, 0, ctx->cuda_stream()>>>(
        n, beta, learning_rate, l1, l2, model_diff, model, momentum);
  }
};
