...
void FullyConnectedKernel<device_type, T>::ForwardDataContent(
  ...
  // out = in * weight
  NewKernelUtil<device_type, T>::BlobGemm(ctx.device_ctx, CblasNoTrans, CblasTrans,
                                          OneVal<T>::value, ZeroVal<T>::value, in_blob, weight_blob,
                                          out_blob);
  ...
  if (this->op_conf().fully_connected_conf().use_bias()) {
    ...
    NewKernelUtil<device_type, T>::BlobGemm(ctx.device_ctx, CblasNoTrans, CblasNoTrans,
...
void FullyConnectedKernel<device_type, T>::BackwardDataContent(
  ...
  if (this->op_conf().trainable()) {
    ...
    NewKernelUtil<device_type, T>::BlobGemm(ctx.device_ctx, CblasTrans, CblasNoTrans,
                                            OneVal<T>::value, ZeroVal<T>::value, out_diff_blob,
                                            in_blob, weight_diff_blob);
...
void FullyConnectedKernel<device_type, T>::InitConstBufBlobs(
  ...
  NewKernelUtil<device_type, T>::InitializeWithConf(ctx, bias_multiplier_initializer_conf, 0,
...
void FullyConnectedKernel<device_type, T>::InitModelBlobsWithRandomSeed(
  ...
  NewKernelUtil<device_type, T>::InitializeWithProperConf(
      ctx, GetMsgPtrFromPbMessage(this->op_conf().fully_connected_conf(), "weight_initializer"),
      (*random_seed_gen)(), BnInOp2Blob("weight"));
  if (this->op_conf().fully_connected_conf().use_bias()) {
    NewKernelUtil<device_type, T>::InitializeWithProperConf(
        ctx, GetMsgPtrFromPbMessage(this->op_conf().fully_connected_conf(), "bias_initializer"),
...
void FullyConnectedKernel<device_type, T>::InitModelBlobsWithDir(
  ...
  NewKernelUtil<device_type, T>::InitializeWithDir(ctx, part_id, part_num, model_load_dir,
