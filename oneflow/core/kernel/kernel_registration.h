...
public:
  ...
  void Finalize(OperatorConf::OpTypeCase *op_type,
...
#define NEW_REGISTER_KERNEL(op_type, ...)                                      \
  static kernel_registration::KernelRegistrar OF_PP_CAT(g_registrar,           \
                                                        __COUNTER__) =         \
...
#define REGISTER_KERNEL_WITH_NOTHING(op_type, ...)                             \
  NEW_REGISTER_KERNEL(op_type, __VA_ARGS__)                                    \
      .SetIsMatchedPred([](const KernelConf &) -> bool { return true; });
