...
void FasterRcnnUtil<T>::BboxTransform(int64_t boxes_num, const T* bboxes, const T* deltas,
  ...
  for (int64_t i = 0; i < boxes_num; ++i) {
    (BBox<T>::MutCast(pred_bboxes) + i)
        ->Transform(BBox<T>::Cast(bboxes) + i, BBoxDelta<T>::Cast(deltas) + i);
  }
}
...
void FasterRcnnUtil<T>::BboxTransformInv(int64_t boxes_num, const T* bboxes, const T* target_bboxes,
                                         T* deltas) {
  for (int64_t i = 0; i < boxes_num; ++i) {
    (BBoxDelta<T>::MutCast(deltas) + i)
        ->TransformInverse(BBox<T>::Cast(bboxes) + i, BBox<T>::Cast(target_bboxes) + i);
  }
}
...
void FasterRcnnUtil<T>::ClipBoxes(int64_t boxes_num, const int64_t image_height,
  ...
  for (int64_t i = 0; i < boxes_num; ++i) {
    (BBox<T>::MutCast(bboxes) + i)->Clip(image_height, image_width);
  }
}
