...
void EmbeddingLookupKernel<device_type, T>::BackwardDataContent(
  ...
  Blob* ids_diff_blob = BnInOp2Blob(this->kernel_conf().model_diff_bns(1));
  Memcpy<device_type>(ctx.device_ctx, weight_diff_blob->mut_dptr<T>(),
                      out_diff_blob->dptr<T>(),
                      sizeof(T) * out_diff_blob->shape().elem_cnt());
  Memcpy<device_type>(ctx.device_ctx, ids_diff_blob->mut_dptr<T>(),
                      in_blob->dptr<T>(),
 ...
 public:
  ...
  static void Forward(DeviceCtx* ctx, const Blob* in_blob,
    ...
    FOR_RANGE(int32_t, n, 0, in_blob->shape().At(0)) {
      ...
      if (idx == -1) { continue; }
      Memcpy<DeviceType::kCPU>(ctx, out_dptr + n * units,
                               weight_dptr + idx * units, sizeof(T) * units);
    }
  }

  static void Backward(DeviceCtx* ctx, const Blob* in_blob,
                       const Blob* out_diff_blob, Blob* ids_diff_blob,
    ...
    Memcpy<DeviceType::kCPU>(ctx, weight_diff_dptr, out_diff_dptr,
                             sizeof(T) * out_diff_blob->shape().elem_cnt());
    Memcpy<DeviceType::kCPU>(ctx, ids_diff_dptr, in_dptr,
                             sizeof(T) * in_blob->shape().elem_cnt());
  }
};
