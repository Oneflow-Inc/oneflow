...
void RecordLoadKernel::VirtualKernelInit() {
  ...
  const RecordLoadKernelConf &record_load_kernel_conf =
      kernel_conf().record_load_conf();
  ...
  int32_t part_name_suffix_length =
      record_load_op_conf.part_name_suffix_length();
  ...
  FOR_RANGE(int32_t, part_id, range.begin(), range.end()) {
    ...
    int32_t zero_count = std::max(
        part_name_suffix_length - static_cast<int32_t>(num.length()), 0);
    data_paths.push_back(JoinPath(
  ...
  bool save_to_local =
      Global<const IOConf>::Get()->save_downloaded_file_to_local_fs();
  in_stream_.reset(
      new PersistentInStream(DataFS(), data_paths, true, save_to_local));
  if (record_load_op_conf.has_random_shuffle_conf()) {
    const int32_t shuffle_buffer_size =
        record_load_op_conf.random_shuffle_conf().buffer_size();
    ...
    record_reader_.reset(
        new NaiveOFRecordReader(in_stream_.get(), num_max_read));
  }
}
...
void RecordLoadKernel::ForwardDataContent(
  ...
  status->record_num =
      record_reader_->Read(static_cast<size_t>(piece_size_in_one_loader_),
                           BnInOp2Blob("out")->mut_dptr<OFRecord>());
  BnInOp2Blob("out")->set_record_num(status->record_num);
  if (status->record_num < piece_size_in_one_loader_) {
    status->is_eof = true;
  }
}
