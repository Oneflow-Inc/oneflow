...
int64_t GetBlobDynamicTotalSize(const Blob *blob) {
  // blob dynamic total byte size = STATIC header byte size + DYNAMIC body byte
  // size
...
void DynamicBinarySplitKernel::ForwardDataContent(
  ...
  int64_t offset = 0;
  CHECK_EQ(header_ptr + in_blob->blob_desc().ByteSizeOfBlobHeader(),
           in_blob->dptr<char>());
...
void DynamicBinaryConcatKernel::ForwardDataContent(
  ...
  for (const auto &ibn : this->op_attribute().input_bns()) {
    ...
    if (dynamic_in_size > 0) {
      memcpy(out_blob->mut_contiguous_header_ptr() + offset, in_blob->dptr(),
             dynamic_in_size);
    }
...
// REGISTER_KERNEL(OperatorConf::kDynamicBinarySplitConf,
// DynamicBinarySplitKernel);
// REGISTER_KERNEL(OperatorConf::kDynamicBinaryConcatConf,
// DynamicBinaryConcatKernel);
REGISTER_KERNEL_WITH_DEVICE(OperatorConf::kDynamicBinarySplitConf,
                            DeviceType::kCPU, DynamicBinarySplitKernel);
REGISTER_KERNEL_WITH_DEVICE(OperatorConf::kDynamicBinaryConcatConf,
                            DeviceType::kCPU, DynamicBinaryConcatKernel);
