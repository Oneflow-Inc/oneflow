...
void CopyHdTestCase(OpKernelTestCase* test_case, const std::string& job_type,
  ...
  CopyOpConf* copy_hd_conf = test_case->mut_op_conf()->mutable_copy_hd_conf();
  CopyOpConf::Type hd_type = (h2d == "h2d" ? CopyOpConf::H2D : CopyOpConf::D2H);
  copy_hd_conf->set_type(hd_type);
  test_case->SetBlobSpecializedDeviceType((hd_type == CopyOpConf::H2D ? "out" : "in"),
                                          DeviceType::kGPU);
