...
void UnsortedSegmentSumKernelUtil<DeviceType::kCPU, T, K>::UnsortedSegmentSum(
    DeviceCtx *ctx, const K *segment_ids, const T *data,
  ...
  FOR_RANGE(int64_t, outer_idx, 0, outer_dim_size) {
    FOR_RANGE(int64_t, i, 0, num_segment_ids) {
      ...
      T *to = out + outer_idx * num_segments * inner_dim_size +
              idx * inner_dim_size;
      if (idx >= 0 && idx < num_segments) {
        const T *from = data + outer_idx * num_segment_ids * inner_dim_size +
                        i * inner_dim_size;
...
#define INITIATE_UNSORTED_SEGMENT_SUM_KERNEL_UTIL_CPU(in_type_pair,            \
                                                      index_type_pair)         \
