...
void CloneTestCase(OpKernelTestCase* clone_test_case,
  ...
  CloneOpConf* clone_conf =
      clone_test_case->mut_op_conf()->mutable_clone_conf();
  ...
  for (size_t i = 0; i < copies; ++i) {
    clone_test_case->template ForwardCheckBlob<T>(
        "out_" + std::to_string(i), blob_desc, {1, 2, 3, 4, 5, 6});
    clone_test_case->template InitBlob<T>("out_" + std::to_string(i) + "_diff",
                                          blob_desc, {6, 5, 4, 3, 2, 1});
  }
  clone_test_case->template BackwardCheckBlob<T>(GenDiffBn("in"), blob_desc,
                                                 {18, 15, 12, 9, 6, 3});
}

TEST_CPU_AND_GPU_OPKERNEL(CloneTestCase, FLOATING_DATA_TYPE_SEQ,
