...
template<>
template<typename T>
Blob* OpKernelTestUtil<DeviceType::kGPU>::CreateBlobWithSpecifiedValPtr(const BlobDesc* blob_desc,
                                                                        T* val, Regst* regst) {
  Blob* ret = CreateBlob(blob_desc, regst);
  CudaCheck(
      cudaMemcpy(ret->mut_dptr(), val, ret->ByteSizeOfDataContentField(), cudaMemcpyHostToDevice));
  return ret;
}

template<>
template<typename T>
void OpKernelTestUtil<DeviceType::kGPU>::BlobCmp(const std::string& blob_name, const Blob* lhs,
  ...
  if (lhs_device_type == DeviceType::kGPU) {
    Blob* mut_cpu_lhs =
        OpKernelTestUtil<DeviceType::kCPU>::CreateBlob(lhs->blob_desc_ptr(), nullptr);
    CudaCheck(cudaMemcpy(mut_cpu_lhs->mut_dptr(), lhs->dptr(), lhs->ByteSizeOfDataContentField(),
  ...
  if (rhs_device_type == DeviceType::kGPU) {
    Blob* mut_cpu_rhs =
        OpKernelTestUtil<DeviceType::kCPU>::CreateBlob(rhs->blob_desc_ptr(), nullptr);
    CudaCheck(cudaMemcpy(mut_cpu_rhs->mut_dptr(), rhs->dptr(), rhs->ByteSizeOfDataContentField(),
                         cudaMemcpyDeviceToHost));
    cpu_rhs = mut_cpu_rhs;
  }
  OpKernelTestUtil<DeviceType::kCPU>::template BlobCmp<T>(blob_name, cpu_lhs, DeviceType::kCPU,
                                                          cpu_rhs, DeviceType::kCPU);
}
...
void OpKernelTestUtil<DeviceType::kGPU>::CheckInitializeResult(
  ...
  Blob* cpu_blob = OpKernelTestUtil<DeviceType::kCPU>::CreateBlob(blob->blob_desc_ptr(), nullptr);
  CudaCheck(cudaMemcpy(cpu_blob->mut_dptr(), blob->dptr(), blob->ByteSizeOfDataContentField(),
                       cudaMemcpyDeviceToHost));
  OpKernelTestUtil<DeviceType::kCPU>::template CheckInitializeResult<T>(cpu_blob, initializer_conf);
}

#define INSTANTIATE_METHODS(type_cpp, type_proto)                                              \
  template Blob* OpKernelTestUtil<DeviceType::kGPU>::CreateBlobWithSpecifiedValPtr(            \
  ...
  template void OpKernelTestUtil<DeviceType::kGPU>::CheckInitializeResult<type_cpp>(           \
      const Blob* blob, const InitializerConf& initializer_conf);
OF_PP_FOR_EACH_TUPLE(INSTANTIATE_METHODS, POD_DATA_TYPE_SEQ);
