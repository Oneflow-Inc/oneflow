...
struct IndexedSlicesLazyAdamMdUpdateKernelUtil<DeviceType::kCPU, T, K, IDX> {
  static void Update(DeviceCtx *ctx, T beta1, T beta2, T epsilon,
    ...
    for (int64_t i = 0; i < n; ++i) {
      ...
      if (instance_id >= lower_bound && instance_id < upper_bound) {
        ...
        model[model_idx] =
            model[model_idx] -
            *learning_rate * new_m / (std::sqrt(new_v) + epsilon);
      }
    }
  }
  static void ComputeLocalLearningRate(DeviceCtx *ctx, T beta1, T beta2,
...
#define INSTANTIATE_INDEXED_SLICES_LAZY_ADAM_MODEL_UPDATE_KERNEL_UTIL_CPU(     \
  ...
  template struct IndexedSlicesLazyAdamMdUpdateKernelUtil<                     \
      DeviceType::kCPU, OF_PP_PAIR_FIRST(val_type_pair),                       \
      OF_PP_PAIR_FIRST(key_type_pair), OF_PP_PAIR_FIRST(idx_type_pair)>;
OF_PP_SEQ_PRODUCT_FOR_EACH_TUPLE(
    INSTANTIATE_INDEXED_SLICES_LAZY_ADAM_MODEL_UPDATE_KERNEL_UTIL_CPU,
