...
void PieceSliceKernel<device_type>::ForwardDataContent(
  ...
  const size_t valid_ins_num = in_blob->dim0_valid_num(0);
  ...
  if (ins_idx <= valid_ins_num) {
    PieceSliceKernelUtil<device_type>::PieceSlice(ctx.device_ctx, ins_idx, valid_ins_num, in_blob,
                                                  BnInOp2Blob("out"));
  }
}
...
void PieceSliceKernel<device_type>::BackwardDataContent(
  ...
  const size_t valid_ins_num = in_diff_blob->dim0_valid_num(0);
  const size_t total_ins_num = actor_stat->second;
  CHECK_LE(out_diff_idx, valid_ins_num);
  CHECK_LE(valid_ins_num, total_ins_num);
  CHECK_EQ(total_ins_num, in_diff_blob->static_shape().At(0));
  if (out_diff_idx <= valid_ins_num) {
    PieceSliceKernelUtil<device_type>::InstanceStack(ctx.device_ctx, out_diff_idx, valid_ins_num,
...
void PieceSliceKernel<device_type>::ForwardDim0ValidNum(
  ...
  CHECK(in_blob->has_instance_shape_field() || in_blob->has_dim1_valid_num_field());
...
void PieceSliceKernel<device_type>::ForwardDim1ValidNum(
  ...
  CHECK(out_blob->has_dim1_valid_num_field());
  auto* actor_stat = static_cast<std::pair<size_t, size_t>*>(ctx.other);
  const size_t ins_idx = actor_stat->first;
  const size_t total_ins_num = actor_stat->second;
  const size_t valid_ins_num = in_blob->dim0_valid_num(0);
