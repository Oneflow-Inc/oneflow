...
private:
  ...
  void ForwardDataContent(
    ...
    FOR_RANGE(int64_t, i, 0, conf.out_size()) {
      ...
      const std::string key = GenLogicalBlobName(conf.variable_op_name(i),
                                                 original_variable_conf.out());
      if (reader.HasKey(key)) {
        reader.Read(key, out_i);
      } else {
        std::cout << "WARNING! CANNOT find variable path in : "
                  << JoinPath(path, key) << ". It will be initialized. \n";
