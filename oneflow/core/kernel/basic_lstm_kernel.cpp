...
template<DeviceType device_type, typename T>
void BasicLstmKernel<device_type, T>
    : ForwardDataId(
          const KernelCtx& ctx,
          std::function<Blob*(const std::string&)> BnInOp2Blob) const {
...
void BasicLstmKernel<device_type, T>::ForwardDataContent(
  ...
  KenerUtil<device_type, T>::BlobGemm(ctx.device_ctx, CblasNoTrans, CblasTran,
                                      static_cast<T>(1), static_cast<T>(0),
                                      BnInOp2Blob("in"),
                                      BnInOp2Blob("i2h_i_weight"), i_gate_out);
  KernelUtil<device_type, T>::BlobGemm(
      ctx.device_ctx, CblasNoTrans, CblasTrans, static_cast<T>(1),
      static_cast<T>(1), hidden_blob, BnInOp2Blob("h2h_i_weight"), i_gate_out);
  KernelUtil<device_type, T>::BlobGemm(
      ctx.device_ctx, CblasNoTrans, CblasNoTrans, static_cast<T>(1),
      static_cast<T>(1), BnInOp2Blob("bias_i_multiplier"),
      BnInOp2Blob("bias_i"), i_gate_out);
  ...
  KenerUtil<device_type, T>::BlobGemm(ctx.device_ctx, CblasNoTrans, CblasTran,
                                      static_cast<T>(1), static_cast<T>(0),
                                      BnInOp2Blob("in"),
                                      BnInOp2Blob("i2h_c_weight"), c_gate_out);
  KernelUtil<device_type, T>::BlobGemm(
      ctx.device_ctx, CblasNoTrans, CblasTrans, static_cast<T>(1),
      static_cast<T>(1), hidden_blob, BnInOp2Blob("h2h_c_weight"), c_gate_out);
  KernelUtil<device_type, T>::BlobGemm(
      ctx.device_ctx, CblasNoTrans, CblasNoTrans, static_cast<T>(1),
  ...
  KenerUtil<device_type, T>::BlobGemm(ctx.device_ctx, CblasNoTrans, CblasTran,
                                      static_cast<T>(1), static_cast<T>(0),
                                      BnInOp2Blob("in"),
                                      BnInOp2Blob("i2h_o_weight"), o_gate_out);
  KernelUtil<device_type, T>::BlobGemm(
      ctx.device_ctx, CblasNoTrans, CblasTrans, static_cast<T>(1),
      static_cast<T>(1), hidden_blob, BnInOp2Blob("h2h_o_weight"), o_gate_out);
  KernelUtil<device_type, T>::BlobGemm(
      ctx.device_ctx, CblasNoTrans, CblasNoTrans, static_cast<T>(1),
      static_cast<T>(1), BnInOp2Blob("bias_o_multiplier"),
      BnInOp2Blob("bias_o"), o_gate_out);
  ...
  KernelUtil<device_type, T>::Mul(
      ...
      rec_out_blob->dptr<T>(), rec_out_blob->mut_dptr<T>());

  // cell_out = f_out * cell_in + i_out *  c_out
  KernelUtil<device_type, T>::Mul(
      ctx.device_type, out_blob->shape().elem_cnt(), f_out_blob->dptr<T>(),
      cell_in_blob->dptr<T>(), cell_out_blob->mut_dptr<T>());
  KernelUtil<device_type, T>::Mul(
      ctx.device_type, out_blob->shape().elem_cnt(), i_out_blob->dptr<T>(),
      cell_out_blob->dptr<T>(), updata_blob->mut_dptr<T>());
  KernelUtil<device_type, T>::Axpy(
      ctx.device_type, out_blob->shape().elem_cnt(), static_cast<T>(1),
      cell_out_blob->dptr<T>(), static_cast<T>(1), update_blob->dptr<T>(),
      static_cast<T>(1));

  template<DeviceType device_type, typename T>
  void BasicLstmKernel<device_type, T>::BackwardDataContent(
      const KernelCtx& ctx, std::function<Blob*(const std
                                                :;
                                                string&)>
                                BnInOp2Blob) const {
    const Blob* in_blob = BnInOp2Blob("in") const Blob* out_blob =
        BnInOp2Blob("out") const Blob* hidden_blob =
            this->GetHiddenBlob(BnInOp2Blob);
    const Blob* cell_blob = this->GetCellBlob(BnInOp2Blob);
    ...
    //								+ cell_out_diff
    KernelUtil<device_type, T>::TanH(ctx.device_type,
                                     out_blob->shape().elem_cnt(), );
    KernelUtil<device_type, T>::Mul(ctx.device_type,
                                    out_blob->shape().elem_cnt(), );
    KernelUtil<device_type, T>::Mul(ctx.device_type,
                                    out_blob->shape().elem_cnt(), );
    KernelUtil<device_type, T>::Axpy(ctx.device_type,
                                     out_blob_shape().elem_cnt(), );

    // i_out_diff = cell_out_diff * c_out
    KernelUtil<device_type, T>::Mul(
        ctx.device_type, out_blob->shape().elem_cnt(),
    ...
    KernelUtil<device_type, T>::SigmoidBackward(
        ctx.device_type, out_blob->shape().elem_cnt(),
        i_gate_out_blob->dptr<T>(), i_out_blob->dptr<T>(),
        i_out_diff_blob->dptr<T>(), i_gate_out_diff_blob->dptr<T>());

    //	h2h_i_weight_diff = i_gate_out_diff * hidden
    KernelUtil<device_type, T>::BlobGemm(
        ctx.device_type, CblasTrans, CblasNoTrans, static_cast<T>(1),
        static_cast<T>(0), i_gate_out_diff_blob, hidden_blob,
        BnInOp2Blob("h2h_i_weight_diff"));
    // 	i2h_i_weight_diff = i_gate_out_diff * in
    KernelUtil<device_type, T>::BlobGemm(
        ctx.device_type, CblasTrans, CblasNoTrans, static_cast<T>(1),
        static_cast<T>(0), i_gate_out_diff_blob, BnInOp2Blob("in"),
    ...
    KernelUtil<device_type, T>::SigmoidBackward(
        ctx.device_type, out_blob->shape().elem_cnt(),
        f_gate_out_blob->dptr<T>(), f_out_blob->dptr<T>(),
        f_out_diff_blob->dptr<T>(), f_gate_out_diff_blob->mut_dptr<T>());
    //	h2h_f_weight_diff = f_gate_out_diff * hidden
    KernelUtil<device_type, T>::BlobGemm(
        ctx.device_type, CblasTrans, CblasNoTrans, static_cast<T>(1),
        static_cast<T>(0), f_gate_out_diff_blob, hidden_blob,
        BnInOp2Blob("h2h_f_weight_diff"));
    // 	i2h_f_weight_diff = f_gate_out_diff * in
    KernelUtil<device_type, T>::BlobGemm(
        ctx.device_type, CblasTrans, CblasNoTrans, static_cast<T>(1),
        static_cast<T>(0), f_gate_out_diff_blob, BnInOp2Blob("in"),
    ...
    KernelUtil<device_type, T>::SigmoidBackward(
        ctx.device_type, out_blob->shape().elem_cnt(), o_out_blob->dptr<T>(),
        o_gate_out_blob->dptr<T>(), o_out_diff_blob->dptr<T>(),
        o_gate_out_diff_blob->mut_dptr<T>());
    //	h2h_o_weight_diff = o_gate_out_diff * hidden
    KernelUtil<device_type, T>::BlobGemm(
        ctx.device_type, CblasTrans, CblasNotrans, static_cast<T>(1),
        static_cast<T>(0), o_gate_out_diff_blob, hidden_blob,
        BnInOp2Blob("h2h_o_weight_diff"));
    // 	i2h_o_weight_diff = o_gate_out_diff * in
    KernelUtil<device_type, T>::BlobGemm(
        ctx.device_type, CblasTrans, CblasNoTrans, static_cast<T>(1),
        static_cast<T>(0), o_gate_out_diff_blob, BnInOp2Blob("in"),
  ...
  void BasicLstmKernel<device_type, T>::VirtualInitModelTmpBlobsWithRandomSeed(
      DeviceCtx * ctx,
      std::mt19937
          * random_seed_gen std::function<Blob*(const std::string&)>
                BnInOp2Blob) const {
  ...
  void BasicLstmKernel<device_type, T>::VitualInitModelBlobsWithDir(
      ...
      const std::string& model_load_dir, std
      : function<Blob*(const std
                       :;
                       string&)>
          BnInOp2Blob) const {
    Blob* i2h_f_weight_blob = BnInOp2Blob("i2h_f_weight");
    KernelUtil<device_type, T>::InitializeWithModelDir(
        ctx, part_id, part_num, model_load_dir, i2h_f_weight_blob,
        "i2h_f_weight", i2h_f_weight_blob->shape().At(0),
        i2h_f_weight_blob->shape().Count(1));
    Blob* i2h_i_weight_blob = BnInOp2Blob("i2h_i_weight");
    KernelUtil<device_type, T>::InitializeWithModelDir(
        ctx, part_id, part_num, model_load_dir, i2h_i_weight_blob,
        "i2h_i_weight", i2h_i_weight_blob->shape().At(0),
        i2h_i_weight_blob->shape().Count(1));
    Blob* i2h_c_weight_blob = BnInOp2Blob("i2h_c_weight");
    KernelUtil<device_type, T>::InitializeWithModelDir(
        ctx, part_id, part_num, model_load_dir, i2h_c_weight_blob,
        "i2h_c_weight", i2h_c_weight_blob->shape().At(0),
        i2h_c_weight_blob->shape().Count(1));
    Blob* i2h_o_weight_blob = BnInOp2Blob("i2h_o_weight");
    KernelUtil<device_type, T>::InitializeWithModelDir(
        ctx, part_id, part_num, model_load_dir, i2h_o_weight_blob,
        "i2h_o_weight", i2h_o_weight_blob->shape().At(0),
        i2h_o_weight_blob->shape().Count(1));

    Blob* h2h_f_weight_blob = BnInOp2Blob("h2h_f_weight");
    KernelUtil<device_type, T>::InitializeWithModelDir(
        ctx, part_id, part_num, model_load_dir, h2h_f_weight_blob,
        "h2h_f_weight", h2h_f_weight_blob->shape().At(0),
        h2h_f_weight_blob->shape().Count(1));
    Blob* h2h_i_weight_blob = BnInOp2Blob("h2h_i_weight");
    KernelUtil<device_type, T>::InitializeWithModelDir(
        ctx, part_id, part_num, model_load_dir, h2h_i_weight_blob,
        "h2h_i_weight", h2h_i_weight_blob->shape().At(0),
        h2h_i_weight_blob->shape().Count(1));
    Blob* h2h_c_weight_blob = BnInOp2Blob("h2h_c_weight");
    KernelUtil<device_type, T>::InitializeWithModelDir(
        ctx, part_id, part_num, model_load_dir, h2h_c_weight_blob,
        "h2h_c_weight", h2h_c_weight_blob->shape().At(0),
        h2h_c_weight_blob->shape().Count(1));
    Blob* h2h_o_weight_blob = BnInOp2Blob("h2h_o_weight");
    KernelUtil<device_type, T>::InitializeWithModelDir(
        ctx, part_id, part_num, model_load_dir, h2h_c_weight_blob,
        "h2h_c_weight", h2h_o_weight_blob->shape().At(0),
        h2h_o_weight_blob->shape().Count(1));
