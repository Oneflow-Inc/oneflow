...
class DistributeCastToDisableBoxingKernel final : public KernelIf<device_type> {
 public:
  OF_DISALLOW_COPY_AND_MOVE(DistributeCastToDisableBoxingKernel);
  DistributeCastToDisableBoxingKernel() = default;
  ~DistributeCastToDisableBoxingKernel() = default;
...
template<DeviceType device_type>
void DistributeCastToDisableBoxingKernel<device_type>::ForwardDataContent(
    const KernelCtx &ctx, std::function<Blob *(const std::string &)> BnInOp2Blob) const {
  CheckSizeAndCopyBlob(ctx.device_ctx, BnInOp2Blob("out"), GetInBlob(BnInOp2Blob));
}

template<DeviceType device_type>
const Blob *DistributeCastToDisableBoxingKernel<device_type>::GetInBlob(
...
ADD_DEVICE_TYPE_KERNEL_CREATOR(OperatorConf::kDistributeCastToDisableBoxingConf,
                               DistributeCastToDisableBoxingKernel);
