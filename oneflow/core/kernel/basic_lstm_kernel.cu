...
template<typename T>
__global__ void ComputeTanHDiffGpu(const int64_t n, const T* out,
...
template<typename T>
__global__ void ComputeSigmoidDiffGpu(const int64_t n, const T* out,
																		  const T* out_diff, const T* rec_out_diff,
																			T* plus_out_diff) {
	CUDA_1D_KERNEL_LOOP(i, n) {
...
template<typename T>
struct BasicLstmKernelUtil<DeviceType::kGpu, T> {
