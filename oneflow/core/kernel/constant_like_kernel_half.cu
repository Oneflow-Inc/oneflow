...
__global__ void NaiveHalfFillGpu(const int64_t elem_cnt, const float16 x, float16* y) {
  CUDA_1D_KERNEL_LOOP(i, elem_cnt) { y[i] = x; }
}
...
class HalfConstantLikeKernel final : public KernelIf<DeviceType::kGPU> {
 ...
 private:
  ...
  void ForwardDataContent(const KernelCtx& ctx,
    ...
    /*NewKernelUtil<DeviceType::kGPU>::Fill(ctx.device_ctx, out_blob->static_shape().elem_cnt(),
       static_cast<float16>(value), out_blob->mut_dptr<float16>());*/
    is_init_ = true;
  }
};

#define REGISTER_HALF_CONSTANT_LIKE_KERNEL                                                 \
  REGISTER_KERNEL_WITH_DEVICE_AND_DTYPE(OperatorConf::kConstantLikeConf, DeviceType::kGPU, \
                                        float16, HalfConstantLikeKernel)
