...
void BroadcastToCompatibleWithKernel<device_type, T>::ForwardDataContent(
  ...
  FOR_RANGE(int64_t, i, 0, num_axes) {
    if (std::find(broadcast_axes.begin(), broadcast_axes.end(), i) ==
        broadcast_axes.end()) {
  ...
  NdarrayUtil<device_type, T>::BroadcastTo(ctx.device_ctx,
                                           XpuVarNdarray<T>(y, num_axes),
                                           XpuVarNdarray<const T>(x, num_axes));
}

#define REGISTTER_BROADCAST_TO_COMPATIBLE_WITH_KERNEL(device_type_v,           \
                                                      dtype_pair)              \
  REGISTER_KERNEL_WITH_DEVICE_AND_DTYPE(                                       \
      OperatorConf::kBroadcastToCompatibleWithConf, device_type_v,             \
      OF_PP_PAIR_FIRST(dtype_pair),                                            \
      BroadcastToCompatibleWithKernel<device_type_v,                           \
                                      OF_PP_PAIR_FIRST(dtype_pair)>)

OF_PP_SEQ_PRODUCT_FOR_EACH_TUPLE(REGISTTER_BROADCAST_TO_COMPATIBLE_WITH_KERNEL,
                                 DEVICE_TYPE_SEQ, ARITHMETIC_DATA_TYPE_SEQ)

#if defined(WITH_CUDA)
REGISTTER_BROADCAST_TO_COMPATIBLE_WITH_KERNEL(DeviceType::kGPU,
