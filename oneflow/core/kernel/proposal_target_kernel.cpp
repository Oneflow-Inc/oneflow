...
void ProposalTargetKernel<T>::ForwardDataContent(
  ...
  FOR_RANGE(int64_t, i, 0, im_num) {
    ...
    ScoredBBoxSlice<T> fg_slice = ForegroundChoice(conf, rois_slice, fg_sample_size);
    ScoredBBoxSlice<T> bg_slice = BackgroundChoice(conf, rois_slice, fg_sample_size);
...
template<typename T>
void ProposalTargetKernel<T>::RoisNearestGtAndMaxIou(const int64_t rois_num, const int64_t gt_num,
...
template<typename T>
ScoredBBoxSlice<T> ProposalTargetKernel<T>::ForegroundChoice(const ProposalTargetOpConf& conf,
                                                             ScoredBBoxSlice<T>& rois_slice,
                                                             int64_t& fg_sample_size) const {
  const float fg_thresh = conf.fg_thresh();
  int64_t fg_end_index = rois_slice.FindByThreshold(fg_thresh);
...
template<typename T>
ScoredBBoxSlice<T> ProposalTargetKernel<T>::BackgroundChoice(const ProposalTargetOpConf& conf,
                                                             ScoredBBoxSlice<T>& rois_slice,
