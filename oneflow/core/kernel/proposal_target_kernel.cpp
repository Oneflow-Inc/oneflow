...
void ProposalTargetKernel<T>::ForwardDataContent(
  ...
  FOR_RANGE(int64_t, i, 0, im_num) {
    ...
    ScoredBBoxSlice<T> fg_slice = ForegroundChoice(conf, rois_slice);
    ScoredBBoxSlice<T> bg_slice = BackgroundChoice(conf, rois_slice, fg_slice.available_len());
...
void ProposalTargetKernel<T>::RoisNearestGtAndMaxIou(const int64_t rois_num, const int64_t gt_num,
                                                     const T* rpn_rois_ptr, const Int32List16* gt_boxes_ptr,
                                                     int32_t* roi_nearest_gt_index_ptr,
...
ScoredBBoxSlice<T> ProposalTargetKernel<T>::ForegroundChoice(ScoredBBoxSlice<T>& rois_slice) const {
  ...
  const int64_t fg_sample_size = std::min(fg_end_index + 1, (int64_t)std::round(num_roi_per_image * fg_fraction));
...
template<typename T>
void ProposalTargetKernel<T>::CopyRoIs(const ScoredBBoxSlice<T>& slice,
                                T* rois_ptr) const {
