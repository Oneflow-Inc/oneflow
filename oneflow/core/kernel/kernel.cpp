...
void Kernel::Forward(const KernelCtx& ctx,
                     std::function<Blob*(const std::string&)> BnInOp2Blob) const {
  if (this->op_conf().name() == "yolo-layer1-start") {	
    ctx.device_ctx->AddCallBack([]() { printf("yolo-layer1-conv %.3f ms\n", GetCurTime() / 1e6); });	
  }
  ...
  if (this->op_conf().name() == "yolo_pos_result_end") {	
    ctx.device_ctx->AddCallBack([]() { printf("yolo_pos_result_end %.3f ms\n", GetCurTime() / 1e6); });	
  }	
  if (this->op_conf().name() == "yolo_prob_result_end") {	
    ctx.device_ctx->AddCallBack(	
        []() { printf("yolo_prob_result_end %.3f ms\n", GetCurTime() / 1e6); });  
  }
}
