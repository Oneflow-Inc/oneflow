...
template <typename T>
__global__ void
GetLocalLearningRateGpu(const float *learning_rate, T weight_decay, T epsilon,
...
template <typename T>
__global__ void UpdateModelGpu(int64_t n, T weight_decay, T momentum_beta,
...
template <typename T> class LARSMdUpdateKernelUtil<DeviceType::kGPU, T> final {
public:
  static void UpdateModel(DeviceCtx *ctx, int64_t n, const float *learning_rate,
                          ...
                          const T *model_diff, T *model, T *momentum,
                          T *data_tmp) {
    KernelUtil<DeviceType::kGPU, T>::Dot(ctx, n, model, 1, model, 1,
                                         &data_tmp[0]);
    KernelUtil<DeviceType::kGPU, T>::Dot(ctx, n, model_diff, 1, model_diff, 1,
                                         &data_tmp[1]);
    GetLocalLearningRateGpu<T><<<1, 1, 0, ctx->cuda_stream()>>>(
        learning_rate, weight_decay, epsilon, lars_coefficient, train_step,
        data_tmp);
