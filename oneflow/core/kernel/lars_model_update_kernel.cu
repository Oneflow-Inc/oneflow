...
template<typename T>
__global__ void GetLocalLearningRateGpu(T learning_rate, T l2, T epsilon, T lars_coefficient,
...
template<typename T>
__global__ void UpdateModelGpu(int64_t n, T l1, T l2, T momentum_beta, const T* model_diff,
 ...
 public:
  static void UpdateModel(DeviceCtx* ctx, int64_t n, T learning_rate, T l1, T l2, T momentum_beta,
    ...
    GetLocalLearningRateGpu<T><<<1, 1, 0, ctx->cuda_stream()>>>(
        learning_rate, l2, epsilon, lars_coefficient, next_model_vid, data_tmp);
    UpdateModelGpu<T><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock, 0, ctx->cuda_stream()>>>(
        n, l1, l2, momentum_beta, model_diff, model, momentum, data_tmp);
  }
};
