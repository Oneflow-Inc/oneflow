...
class DeconvKernel final : public KernelIf<device_type> {
 ...
 private:
  ...
  void ForwardDataContent(const KernelCtx& ctx,
    ...
    CudnnConvArgs args(this->op_conf().deconv_conf().conv_conf(), y_blob->data_type(),
                        ShapeView(y_blob->shape()), weight_blob->data_type(),
                        ShapeView(weight_blob->shape()), x_blob->data_type(),
    ...
    CHECK_LE(algo_perf.memory, cudnn_buf->ByteSizeOfBlobBody())
        << "op (" << this->op_conf().name() << ") find algorithm " << algo_perf.algo << ", need memory "
        << algo_perf.memory << ", but cudnn_buf_limit_byte is " << cudnn_buf->ByteSizeOfBlobBody();
    CudaCheck(cudnnConvolutionBackwardData(ctx.device_ctx->cudnn_handle(), CudnnSPOnePtr<T>(), args.wdesc.Get(),
                                           weight_blob->dptr(), args.ydesc.Get(), x_blob->dptr(),
                                           args.cdesc.Get(), algo_perf.algo, cudnn_buf->mut_dptr(), args.params.max_ws_size,
                                           CudnnSPZeroPtr<T>(), args.xdesc.Get(), y_blob->mut_dptr()));
  }
};

#define REGISTER_DECONV_KERNEL(dev, dtype)                                                              \
    REGISTER_KERNEL_WITH_DEVICE_AND_DTYPE(OperatorConf::kDeconvConf, dev, dtype, DeconvKernel<dev, dtype>)             
