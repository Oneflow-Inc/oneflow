...
void RandomFillBlob(DeviceCtx *ctx, DeviceType device_type,
...
#define RANDOM_FILL_ENTRY(type_dev, data_type_pair)                            \
  {GetHashKey(type_dev, OF_PP_PAIR_SECOND(data_type_pair)),                    \
   &KernelUtil<type_dev,                                                       \
               OF_PP_PAIR_FIRST(data_type_pair)>::InitializeWithConf},
          OF_PP_SEQ_PRODUCT_FOR_EACH_TUPLE(RANDOM_FILL_ENTRY, DEVICE_TYPE_SEQ,
                                           ARITHMETIC_DATA_TYPE_SEQ)};
  fill_funcs.at(GetHashKey(device_type, blob->data_type()))(
      ctx, initializer_conf, random_seed, blob);
}
...
void DecodeRandomKernel<device_type>::VirtualKernelInit() {
  gen_.reset(
      new std::mt19937(this->kernel_conf().decode_random_conf().random_seed()));
...
void DecodeRandomKernel<device_type>::ForwardDataContent(
  ...
  if (is_init_ == false) {
    RandomFillBlob(ctx.device_ctx, device_type, conf.data_initializer(),
                   this->GenNextRandomSeed(), BnInOp2Blob("out"));
    is_init_ = true;
  }
}

ADD_DEVICE_TYPE_KERNEL_CREATOR(OperatorConf::kDecodeRandomConf,
                               DecodeRandomKernel);
