...
template<typename T>
__global__ void DiffAveragingL1RegularizationGpu(int64_t n, T l1, int32_t batch_size, const T* x,
                                                 T* y) {
  T zero = ZeroVal<T>::value;
  CUDA_1D_KERNEL_LOOP(i, n) { y[i] = y[i] / batch_size + l1 * ((x[i] >= zero) - (x[i] <= zero)); }
}
  ...
  static void DiffAveragingAndL1Regularization(DeviceCtx* ctx, int64_t n, float l1, const T* model,
    ...
    DiffAveragingL1RegularizationGpu<T>
        <<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock, 0, ctx->cuda_stream()>>>(
            n, static_cast<T>(l1), batch_size, model, model_diff_acc);
  }
};
