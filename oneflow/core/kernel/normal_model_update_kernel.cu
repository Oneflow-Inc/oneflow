...
template<typename T>
__global__ void UpdateModelGpu(int64_t n, int64_t batch_size, T momentum_beta, T learning_rate,
                               T l1, T l2, const T* model_diff, const T* pre_model, T* momentum,
                               T* model) {
...
void NormalMdUpdateKernelUtil<DeviceType::kGPU, T>::UpdateModel(
    DeviceCtx* ctx, int64_t n, int64_t batch_size, T momentum_beta, T learning_rate, const T l1,
    const T l2, const T* model_diff, const T* pre_model, T* momentum, T* model) {
  UpdateModelGpu<T><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock, 0, ctx->cuda_stream()>>>(
      n, batch_size, momentum_beta, learning_rate, l1, l2, model_diff, pre_model, momentum, model);
}
