...
private:
  ...
  Blob *
  GetOutBlob(std::function<Blob *(const std::string &)> BnInOp2Blob) const;
};
...
void DistributeCloneKernel<device_type>::ForwardDataContent(
  ...
  CheckSizeAndCopyBlob(ctx.device_ctx, GetOutBlob(BnInOp2Blob),
                       BnInOp2Blob("in"));
}
...
ADD_DEVICE_TYPE_KERNEL_CREATOR(OperatorConf::kDistributeCloneConf,
