...
template <typename T>
void InitializeWithConf(const InitializerConf &conf, const uint32_t random_seed,
                        Blob *blob) {
  KernelUtil<DeviceType::kCPU, T>::InitializeWithConf(nullptr, conf,
                                                      random_seed, blob);
}

struct InitializeWithConfUtil final {
#define MAKE_INITIALIZE_SWITCH_ENTRY(func_name, T) func_name<T>
  DEFINE_STATIC_SWITCH_FUNC(void, InitializeWithConf,
                            MAKE_INITIALIZE_SWITCH_ENTRY,
...
TensorSliceView GetPartSlice(const KernelConf &kernel_conf) {
  return GetPartSlice(
      kernel_conf, kernel_conf.model_io_v2_conf().parallel_ctx().parallel_id());
}
...
public:
  ...
  explicit OnDemandHostBlob(const RtBlobDesc &blob_desc) {
    blob_desc_.reset(new RtBlobDesc(
        BlobDesc(blob_desc.body_shape(), blob_desc.data_type())));
    Init();
  }
...
private:
  void Init() {
    ...
    blob_.reset(
        new Blob(host_mem_case, blob_desc_.get(), header.data(), data.data()));
  }
...
template <>
void SyncCopyToHost<DeviceType::kCPU>(DeviceCtx *ctx, const void *src,
...
#ifdef WITH_CUDA
template <>
void SyncCopyToHost<DeviceType::kGPU>(DeviceCtx *ctx, const void *src,
                                      void *dst, size_t size) {
  OF_CUDA_CHECK(cudaStreamSynchronize(ctx->cuda_stream()));
  OF_CUDA_CHECK(cudaMemcpyAsync(dst, src, size, cudaMemcpyDeviceToHost,
                                ctx->cuda_stream()));
  OF_CUDA_CHECK(cudaStreamSynchronize(ctx->cuda_stream()));
}
#endif
...
template <>
void SyncCopyToDevice<DeviceType::kCPU>(DeviceCtx *ctx, const void *src,
                                        void *dst, size_t size) {
  std::memcpy(dst, src, size);
}
...
#ifdef WITH_CUDA
template <>
void SyncCopyToDevice<DeviceType::kGPU>(DeviceCtx *ctx, const void *src,
                                        void *dst, size_t size) {
  OF_CUDA_CHECK(cudaStreamSynchronize(ctx->cuda_stream()));
  OF_CUDA_CHECK(cudaMemcpyAsync(dst, src, size, cudaMemcpyHostToDevice,
                                ctx->cuda_stream()));
  OF_CUDA_CHECK(cudaStreamSynchronize(ctx->cuda_stream()));
}
#endif
...
std::string GetTmpPartKey(const std::string &base, const int64_t parallel_id,
  ...
  return "tmp-part-" + std::to_string(parallel_id) + "-" +
         std::to_string(parallel_num) + "-" + base;
}

std::string GetTmpPartKey(const std::string &base,
                          const ParallelContext &parallel_ctx) {
  return GetTmpPartKey(base, parallel_ctx.parallel_id(),
...
template <DeviceType device_type> class AutoSyncBlobAccessor final {
public:
  OF_DISALLOW_COPY_AND_MOVE(AutoSyncBlobAccessor);
  AutoSyncBlobAccessor(DeviceCtx *ctx, Blob *underlying, bool read_sync,
                       bool write_sync)
      : device_ctx_(ctx), underlying_(underlying), read_sync_(read_sync),
        write_sync_(write_sync), host_blob_(underlying) {
    if (read_sync_) {
      SyncCopyToHost<device_type>(device_ctx_, underlying_->dptr(),
                                  host_blob_.blob()->mut_dptr(),
                                  underlying_->ByteSizeOfBlobBody());
    }
  }
  ~AutoSyncBlobAccessor() {
    if (write_sync_) {
      SyncCopyToDevice<device_type>(device_ctx_, host_blob_.blob()->dptr(),
                                    underlying_->mut_dptr(),
                                    underlying_->ByteSizeOfBlobBody());
    }
  }
...
template <> class AutoSyncBlobAccessor<DeviceType::kCPU> final {
public:
  OF_DISALLOW_COPY_AND_MOVE(AutoSyncBlobAccessor);
  AutoSyncBlobAccessor(DeviceCtx *ctx, Blob *underlying, bool read_sync,
                       bool write_sync)
...
private:
  ...
  void ForwardDataContent(
      ...
      std::function<Blob *(const std::string &)> BnInOp2Blob) const override {
    const ParallelContext &parallel_ctx =
        this->kernel_conf().model_io_v2_conf().parallel_ctx();
    ...
    const VariableOpConf &original_variable_conf =
        conf.original_variable_conf();
    const Shape logical_blob_shape(original_variable_conf.shape());
    const DataType data_type = ref->data_type();
    const std::string &var_lbn = GenLogicalBlobName(
        conf.variable_op_name(), original_variable_conf.out());
    const TensorSliceView slice = GetPartSlice(this->kernel_conf());
    AutoSyncBlobAccessor<device_type> ref_accessor(ctx.device_ctx, ref, false,
                                                   true);
    ...
    if (original_variable_conf.has_initializer()) {
      const std::string blob_cache_key =
          ...
          std::to_string(Global<MachineCtx>::Get()->this_machine_id()) +
          "-Counter-" + std::to_string(*counter_);
      const std::string barrier_key = "ModelInitBarrier-" + var_lbn +
                                      "-Counter-" + std::to_string(*counter_);
      OfCallOnce(blob_cache_key, [&]() {
        ...
        InitializeWithConfUtil::SwitchInitializeWithConf(
            SwitchCase(data_type), original_variable_conf.initializer(),
            random_seed_gen(), on_demand_host_logical_blob->blob());
        {
      ...
      Global<CtrlClient>::Get()->Barrier(barrier_key,
                                         parallel_ctx.parallel_num());
      {
        std::lock_guard<std::mutex> lock(blob_cache_mutex_);
      ...
      const auto &snapshot_conf =
          original_variable_conf.initialize_with_snapshot();
      const std::string key =
          snapshot_conf.has_key() ? snapshot_conf.key() : var_lbn;
...
template <DeviceType device_type>
HashMap<std::string, std::shared_ptr<OnDemandHostBlob>>
    ModelInitV2Kernel<device_type>::blob_cache_;

ADD_DEVICE_TYPE_KERNEL_CREATOR(OperatorConf::kModelInitV2Conf,
                               ModelInitV2Kernel);
...
private:
  ...
  void ForwardDataContent(
    ...
    const std::string snapshot_path =
        SyncReadStringFromBlob<device_type>(ctx.device_ctx, path);
    SnapshotReader reader(snapshot_path);
    reader.Read(var_lbn, logical_blob_shape, slice, ref_accessor.host_blob());
  }
};

ADD_DEVICE_TYPE_KERNEL_CREATOR(OperatorConf::kModelLoadV2Conf,
                               ModelLoadV2Kernel);
...
private:
  ...
  void ForwardDataContent(
    ...
    *counter_ += 1;
    ...
    if (is_broadcast && parallel_ctx.parallel_id() != 0) {
      return;
    }
    const std::string snapshot_path =
        SyncReadStringFromBlob<device_type>(ctx.device_ctx, path_blob);
    AutoSyncBlobAccessor<device_type> in_accessor(ctx.device_ctx, in_blob, true,
                                                  false);
    SnapshotWriter writer(snapshot_path);
    const std::string var_lbn = GenLogicalBlobName(
        conf.variable_op_name(), original_variable_conf.out());
    const std::string key =
        is_broadcast ? var_lbn : GetTmpPartKey(var_lbn, parallel_ctx);
    writer.Write(key, in_accessor.host_blob());
    if (!is_broadcast) {
      const int64_t parallel_num = parallel_ctx.parallel_num();
      Global<CtrlClient>::Get()->Barrier(snapshot_path + "-" + var_lbn +
                                             "-Counter-" +
      ...
      FOR_RANGE(int64_t, i, 0, parallel_num) {
        ...
        HostSliceCopy(total_blob.blob(), total_slice, part_blob.blob(),
                      part_slice);
        SnapshotFS()->RecursivelyDeleteDir(
            Dirname(JoinPath(snapshot_path, part_key)));
      }
...
ADD_DEVICE_TYPE_KERNEL_CREATOR(OperatorConf::kModelSaveV2Conf,
                               ModelSaveV2Kernel);
