...
//  (*bn2blob_ptr)["weight"] = KTCommon::CreateBlobWithVector({3, 4},
//  weight_mat);
//  (*bn2blob_ptr)["out"] = KTCommon::CreateBlobWithRandomValue({2, 3});
//  (*bn2blob_ptr)["out_diff"] = (*bn2blob_ptr)["out"];
//  (*bn2blob_ptr)["in_diff"] = KTCommon::CreateBlobWithRandomValue({2, 1, 2,
//  2});
...
//  op_conf.mutable_innerproduct_conf(); inner_product_conf->set_in("ip_in");
...
//
//  auto inner_product_kernel =
//      BuildInnerProductKernel<device_type, FloatingPointType>(true,
//      fill_conf);
