...
void BlasMatrixMult<float>(
  ...
  return cblas_sgemm(Order, TransA, TransB, M, N, K, alpha, A->dptr(), lda,
                     B->dptr(), ldb, beta, C->mut_dptr(), ldc);
}
...
void BlasMatrixMult<double>(
  ...
  return cblas_dgemm(Order, TransA, TransB, M, N, K, alpha, A->dptr(), lda,
                     B->dptr(), ldb, beta, C->mut_dptr(), ldc);
}
...
void InnerProductKernel<DeviceType::kCPU, floating_point_type>::Forward(
  ...
  Blob* bias_multiplier = BnInOp2BlobPtr(op()->model_temp_bns().at(0));
