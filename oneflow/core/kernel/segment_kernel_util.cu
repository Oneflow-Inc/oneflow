...
__global__ void SegmentSumKernel(const int input_outer_dim_size,
                                 const int inner_dim_size,
                                 const int output_outer_dim_size,
...
template <typename T, typename K, int OuterDimTileSize>
__global__ void SegmentSumGradKernel(const int in_diff_outer_dim_size,
                                     const int inner_dim_size,
                                     const int total_stripe_count,
                                     const T* out_diff,
                                     const K* segment_ids,
  ...
  for(int stripe_idx = threadIdx.x + blockIdx.x * blockDim.x; stripe_idx < total_stripe_count;
    ...
    const int actual_stripe_height = min(OuterDimTileSize, 
                                         in_diff_outer_dim_size - in_diff_outer_dim_index_base);
...
template <typename T, typename K>
void SegmentKernelUtilImpl<DeviceType::kGPU, T, K>::SegmentSumBackward(DeviceCtx* ctx,
  ...
  SegmentSumGradKernel<T, K, 8><<<config.block_count, config.threads_per_block>>>(
                           in_diff_outer_dim_size, inner_dim_size, total_stripe_count, out_diff,
                           segment_ids, in_diff);                           
}
