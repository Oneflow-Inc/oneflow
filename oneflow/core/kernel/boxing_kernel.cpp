...
void CalcSumOfBlobs(DeviceCtx *ctx,
  ...
  FOR_RANGE(size_t, i, 1, src_bns.size()) {
    ...
    KernelUtil<DeviceType::kCPU, T>::Axpy(
        ctx, dst_blob->static_shape().elem_cnt(), 1.0, src_blob_i->dptr<T>(), 1,
...
public:
  ...
  size_t OneElemSize() const {
    return GetSizeOfDataType(BnInOp2Blob_(bns_->Get(0))->data_type());
  }

  int64_t TotalElemNum() const { return seg_num_ * elem_sum_.back(); }

  template <typename DptrT, DptrT *(*GetDptrT)(Blob *)>
  std::tuple<int64_t, DptrT *>
  CalcContinuousElemNumStartFrom(int64_t idx) const {
    std::tuple<int64_t, DptrT *> ret(0, nullptr);
    int64_t seg_idx = idx / elem_sum_.back();
    int64_t idx_in_seg = idx % elem_sum_.back();
    auto elem_sum_it =
        std::upper_bound(elem_sum_.begin(), elem_sum_.end(), idx_in_seg);
    ...
    if (bn_idx > 0) {
      idx_in_blob -= elem_sum_[bn_idx - 1];
    }
...
void ConcatSplitPartDataContent(DeviceCtx *ctx, const DataContentDesc &in_desc,
                                const DataContentDesc &out_desc,
                                int32_t part_id, int32_t part_num) {
  ...
  while (in_elem_num > 0 || out_elem_num > 0 || in_idx < range.end() ||
         out_idx < range.end()) {
    if (in_elem_num == 0) {
      std::tie(in_elem_num, in_ptr) =
          in_desc.CalcContinuousElemNumStartFrom<const char, GetConstDptr>(
              in_idx);
...
void ConcatSplitDataContent(
  ...
  int32_t part_num =
      in_desc.TotalElemNum() * in_desc.OneElemSize() / min_byte_one_part;
  ...
  if (part_num >= 2) {
    ...
    FOR_RANGE(int32_t, part_id, 0, part_num) {
      Global<ThreadPool>::Get()->AddWork([&ctx, &in_desc, &out_desc, part_id,
                                          &part_num, &bc]() {
...
template <typename T> void BoxingKernel<T>::VirtualKernelInit() {
  ...
  if (boxing_conf.in_box_case() == BoxingOpConf::kConcatBox) {
    if (boxing_conf.out_box_case() == BoxingOpConf::kSplitBox) {
      ConcatSplitDataContent(
          ctx.device_ctx, BnInOp2Blob, op_attribute().input_bns(),
    ...
    if (boxing_conf.out_box_case() == BoxingOpConf::kSplitBox) {
      CalcSumOfBlobs<T>(ctx.device_ctx, BnInOp2Blob, op_attribute().input_bns(),
                        "middle");
      ConcatSplitDataContent(
