...
//    FloatingPointType>::ConcatBoxForward; bw_func_ =
//    &BoxingKernel<device_type, FloatingPointType>::ConcatBoxBackward;
//  } else {
...
//      InferCopyRulesFromBns(BnInOp2Blob, op()->input_bns(),
//      op()->output_bns(),
//                            concat_axis, 0, &fw_copy_rules_);
//    } else {
//      UNEXPECTED_RUN();
...
//                                  src_concat_axis, dst_split_axis,
//                                  copy_rules);
//  }
//}
//
...
// FloatingPointType>::InferCopyRulesFromEqualAxis(
//    std::function<Blob*(const std::string&)> BnInOp2Blob,
//    const std::vector<std::string>& src_bns,
//    const std::vector<std::string>& dst_bns,
//    std::vector<CopyRule>* copy_rules) const {
...
//    const KernelCtx& ctx, std::function<Blob*(const std::string&)>
//    BnInOp2Blob, const std::vector<CopyRule>& copy_rules) const {
...
//    KernelUtil<device_type, FloatingPointType>::BlasAxpy(
//        ctx, out_0->shape().elem_cnt(), 1.0, in_i->dptr<FloatingPointType>(),
//        1, out_0->mut_dptr<FloatingPointType>(), 1);
//  }
//
