...
__global__ void UpdateModelGpu(int64_t n, const float *learning_rate,
  ...
  const float local_learning_rate =
      *learning_rate * sqrt(1 - (*beta2_t)) / (1 - (*beta1_t));
  CUDA_1D_KERNEL_LOOP(i, n) {
    T model_diff_val = model_diff[i];
    if (abs(model_diff_val) < 1e-12) {
      continue;
    }
...
template <typename T>
__global__ void UpdateBetaTGpu(const int64_t *train_step, const T beta1,
...
public:
  static void UpdateModel(DeviceCtx *ctx, int64_t n, const float *learning_rate,
    ...
    UpdateModelGpu<T><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock, 0,
                        ctx->cuda_stream()>>>(n, learning_rate, weight_decay,
                                              beta1, beta2, epsilon, beta1_t,
                                              beta2_t, model_diff, model, m, v);
    UpdateBetaTGpu<T><<<1, 1, 0, ctx->cuda_stream()>>>(train_step, beta1, beta2,
                                                       beta1_t, beta2_t);
  }
};
