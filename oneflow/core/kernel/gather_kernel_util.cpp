...
void GatherForward(DeviceCtx *ctx, const Blob *indices, const Blob *in,
                   int64_t axis, Blob *out, const int64_t offset) {
  const Shape &flat_in_shape = GetFlatShape(in->shape(), axis);
  GatherKernelUtilImpl<device_type, T, K>::Forward(
      ctx, indices->dptr<K>(), indices->shape().elem_cnt(), in->dptr<T>(),
      flat_in_shape, out->mut_dptr<T>(), offset);
}

template <DeviceType device_type, typename T> struct GatherSwitchUtil final {
...
void GatherKernelUtil<device_type, T>::Forward(DeviceCtx *ctx,
                                               const Blob *indices,
                                               const Blob *in,
...
void GatherKernelUtil<device_type, T>::Forward(DeviceCtx *ctx,
  ...
  GatherSwitchUtil<device_type, T>::SwitchGatherForward(
      SwitchCase(indices->data_type()), ctx, indices, in, axis, out, offset);
}
...
template <typename T, typename K>
void GatherKernelUtilImpl<DeviceType::kCPU, T, K>::Forward(
    DeviceCtx *ctx, const K *indices, int64_t num_indices, const T *in,
  ...
  FOR_RANGE(int64_t, outer_idx, 0, outer_dim_size) {
    FOR_RANGE(int64_t, i, 0, num_indices) {
      ...
      T *to =
          out + outer_idx * num_indices * inner_dim_size + i * inner_dim_size;
      if (idx >= 0 && idx < gather_dim_size) {
        const T *from = in + outer_idx * gather_dim_size * inner_dim_size +
                        idx * inner_dim_size;
...
#define INITIATE_GATHER_KERNEL_UTIL_CPU_IMPL(in_type_pair, index_type_pair)    \
  template struct GatherKernelUtilImpl<DeviceType::kCPU,                       \
