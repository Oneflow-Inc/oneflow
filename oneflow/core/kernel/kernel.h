...
public:
  ...
  /*
   * return true means all below must be guaranteed when `Launch` function
   * return:
  ...
  void SystemForwardHeader(
      const KernelCtx &ctx,
      std::function<Blob *(const std::string &)> BnInOp2Blob) const {
    ForwardHeader(ctx, BnInOp2Blob);
  }
  void SystemForwardDataContent(
      const KernelCtx &ctx,
      std::function<Blob *(const std::string &)> BnInOp2Blob) const {
    ForwardDataContent(ctx, BnInOp2Blob);
  }
  virtual void
  Forward(const KernelCtx &ctx,
          std::function<Blob *(const std::string &)> BnInOp2Blob) const;
...
protected:
  ...
  virtual void InitConstBufBlobs(
      DeviceCtx *ctx,
      std::function<Blob *(const std::string &)> BnInOp2Blob) const {}
  ...
  void ForEachObnAndIsHeaderInferedBeforeCompute(
    ...
    const auto &modifier_map = this->kernel_conf_.op_attribute()
  ...
  template <typename HandlerT>
  void ForEachObnAndIsMutableByConsumer(
      std::function<Blob *(const std::string &)> BnInOp2Blob,
      const HandlerT &Handler) const {
    const auto &modifier_map = this->kernel_conf_.op_attribute()
                                   .arg_modifier_signature()
  ...
  virtual void
  ForwardHeader(const KernelCtx &ctx,
                std::function<Blob *(const std::string &)> BnInOp2Blob) const;
  virtual void
  ForwardShape(const KernelCtx &ctx,
               std::function<Blob *(const std::string &)> BnInOp2Blob) const;
  void NaiveForwardShape(
      std::function<Blob *(const std::string &)> &BnInOp2Blob) const;
...
template <DeviceType device_type> class KernelIf : public Kernel {
public:
...
#define REGISTER_KERNEL_CREATOR(k, f)                                          \
  REGISTER_CLASS_CREATOR(k, Kernel, f, const KernelConf &)
  ...
  Kernel *OF_PP_CAT(CreateKernel, __LINE__)(const KernelConf &kernel_conf) {   \
    static const HashMap<std::string, std::function<Kernel *()>> creators = {  \
        OF_PP_SEQ_PRODUCT_FOR_EACH_TUPLE(MAKE_KERNEL_CREATOR_ENTRY,            \
                                         ...
                                         data_type_seq)};                      \
    DeviceType device_type = CHECK_JUST(DeviceType4DeviceTag(                  \
        kernel_conf.op_attribute().op_conf().device_tag()));                   \
...
#define ADD_DEFAULT_KERNEL_CREATOR_WITH_GPU_HALF(op_type_case, kernel_class,   \
                                                 data_type_seq)                \
  namespace {                                                                  \
                                                                               \
  Kernel *OF_PP_CAT(CreateKernel, __LINE__)(const KernelConf &kernel_conf) {   \
