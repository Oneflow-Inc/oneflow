...
void BasicRnnKernel<device_type, T>::ForwardDataContent(
  ...
  const Blob* rec_out_blob = this->GetHiddenBlob(BnInOp2Blob);
...
void BasicRnnKernel<device_type, T>::ForwardDataId(
    const KernelCtx& ctx,
    std::function<Blob*(const std::string&)> BnInOp2Blob) const {
  BnInOp2Blob("rec_out")->CopyDataIdFrom<device_type>(ctx.device_ctx,
                                                     BnInOp2Blob("in"));
}
...
void BasicRnnKernel<device_type, T>::BackwardDataContent(
  ...
  const Blob* rec_in_blob = this->GetHiddenBlob(BnInOp2Blob);
  ...
  if (this->NeedExternalH0() && BnInOp2Blob("rec_out_diff")->col_id() == 0) {
