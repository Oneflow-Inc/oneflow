...
void BasicGruKernel<device_type, T>::ForwardDataContent(
  ...
  KernelUtil<device_type, T>::TanH(ctx.device_ctx,
                                   candidate_hidden_data_blob->shape().elem_cnt(),
  ...
  KernelUtil<device_type, T>::Axpy(
      ctx.device_ctx, static_cast<T>(candidate_hidden_blob->shape().At(0)), static_cast<T>(1),
      candidate_hidden_blob, static_cast<T>(1), plus_op_out_blob,
      static_cast<T>(1));
