...
__device__ IDX GetOutOffset(const IDX data_offset, const K *segment_ids,
                            const IDX num_segment_ids, const IDX num_segments,
                            const IDX inner_dim_size,
  ...
  if (idx >= 0 && idx < num_segments) {
    return outer_idx * num_segments * inner_dim_size + idx * inner_dim_size +
           inner_idx;
  } else {
    return -1;
  }
...
__global__ void
UnsortedSegmentSumGpu(const IDX data_elem_cnt, const K *segment_ids,
  ...
  CUDA_1D_KERNEL_LOOP_T(IDX, i, data_elem_cnt) {
    ...
    if (val != static_cast<T>(0)) {
      const int64_t out_offset =
          GetOutOffset<K, IDX>(i, segment_ids, num_segment_ids, num_segments,
                               inner_dim_size, segment_id_offset);
      if (out_offset >= 0) {
        gpu_atomic_add(out + out_offset, val);
      }
    }
  }
}
...
bool IsSafeUseIndex32(const int64_t num_segment_ids, const int64_t num_segments,
  ...
  const int64_t data_elem_cnt =
      outer_dim_size * num_segment_ids * inner_dim_size;
...
struct UnsortedSegmentSumKernelUtil<DeviceType::kGPU, T, K> final {
  static void UnsortedSegmentSum(DeviceCtx *ctx, const K *segment_ids,
                                 const T *data, int64_t num_segment_ids,
                                 int64_t num_segments, int64_t outer_dim_size,
                                 int64_t inner_dim_size,
                                 int64_t segment_id_offset, T *out) {
    const int64_t data_elem_cnt =
        outer_dim_size * num_segment_ids * inner_dim_size;
    if (IsSafeUseIndex32(num_segment_ids, num_segments, outer_dim_size,
                         inner_dim_size)) {
      UnsortedSegmentSumGpu<
          T, K, int32_t><<<BlocksNum4ThreadsNum(data_elem_cnt),
                           kCudaThreadsNumPerBlock, 0, ctx->cuda_stream()>>>(
          data_elem_cnt, segment_ids, num_segment_ids, data, num_segments,
          inner_dim_size, out, segment_id_offset);
    } else {
      UnsortedSegmentSumGpu<
          T, K, int64_t><<<BlocksNum4ThreadsNum(data_elem_cnt),
                           kCudaThreadsNumPerBlock, 0, ctx->cuda_stream()>>>(
          data_elem_cnt, segment_ids, num_segment_ids, data, num_segments,
          inner_dim_size, out, segment_id_offset);
    }
...
struct UnsortedSegmentSumKernelUtil<DeviceType::kGPU, float16, K> final {
  static void UnsortedSegmentSum(DeviceCtx *ctx, const K *segment_ids,
    ...
    UnsortedSegmentSumKernelUtil<DeviceType::kGPU, half, K>::UnsortedSegmentSum(
        ctx, segment_ids, reinterpret_cast<const half *>(data), num_segment_ids,
        num_segments, outer_dim_size, inner_dim_size, segment_id_offset,
        reinterpret_cast<half *>(out));
  }
};
#endif

#define INITIATE_UNSORTED_SEGMENT_SUM_KERNEL_UTIL_GPU(in_type_pair,            \
                                                      index_type_pair)         \
  template struct UnsortedSegmentSumKernelUtil<                                \
      DeviceType::kGPU, OF_PP_PAIR_FIRST(in_type_pair),                        \
