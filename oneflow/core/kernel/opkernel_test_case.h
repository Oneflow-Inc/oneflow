...
#define TEST_CPU_OPKERNEL(func_name, data_type_seq, ...)                  \
  OF_PP_SEQ_PRODUCT_FOR_EACH_TUPLE(MAKE_OPKERNEL_TEST_ENTRY, (func_name), \
                                   ((cpu, DeviceType::kCPU)), data_type_seq, __VA_ARGS__)

#ifdef WITH_CUDA
#define TEST_GPU_OPKERNEL(func_name, data_type_seq, ...)                  \
  OF_PP_SEQ_PRODUCT_FOR_EACH_TUPLE(MAKE_OPKERNEL_TEST_ENTRY, (func_name), \
                                   ((gpu, DeviceType::kGPU)), data_type_seq, __VA_ARGS__)
#define TEST_DIFF_KERNEL_IMPL(func_name, ...) \
  OF_PP_SEQ_PRODUCT_FOR_EACH_TUPLE(MAKE_TEST_DIFF_KERNEL_IMPL, (func_name), ##__VA_ARGS__)
#else
#define TEST_GPU_OPKERNEL(func_name, data_type_seq, ...)
#define TEST_DIFF_KERNEL_IMPL(func_name, ...)
#endif

inline std::string ExpectedBlobName(const std::string& name) { return name + "_$expected$"; }
...
class OpKernelTestCase {
 public:
  ...
  void SetBlobSpecializedDeviceType(const std::string& blob_name, DeviceType dev_type) {
  ...
  DataType default_data_type() const { return Global<JobDesc>::Get()->DefaultDataType(); }
  const HashMap<std::string, Blob*>& bn_in_op2blob() const { return bn_in_op2blob_; }
  DeviceType GetBlobDeviceType(const std::string& blob_name) const {
    const auto& it = bn_in_op2device_type_.find(blob_name);
    return (it == bn_in_op2device_type_.end()) ? default_device_type() : it->second;
  }
  ...
  template<typename T>
  void ForwardCheckBlob(const std::string&, const BlobDesc* blob_desc, const std::vector<T>& val);
  template<typename T>
  void BackwardCheckBlob(const std::string&, const BlobDesc* blob_desc, const std::vector<T>& val);
  ...
  template<typename T>
  void ForwardCheckBlobWithAnother(const std::string&, const BlobDesc* blob_desc,
                                   const std::string& expected_existed_blob_name,
                                   bool need_random_init);
  template<typename T>
  void BackwardCheckBlobWithAnother(const std::string&, const BlobDesc* blob_desc,
                                    const std::string& expected_existed_blob_name,
                                    bool need_random_init);
 ...
 private:
  template<typename T>
  void CheckBlob(const std::string&, const BlobDesc* blob_desc, const std::vector<T>& val,
                 bool need_random_init);
...
class DiffKernelImplTestCase final : public OpKernelTestCase {
 public:
  ...
  DiffKernelImplTestCase(bool is_train, bool is_forward, DataType default_data_type);
  ~DiffKernelImplTestCase() = default;

  void SetBlobNames(const std::list<std::string>& input_or_weight_bn_in_op,
  ...
  void SetInputBlobDesc(const std::string& bns_in_op, const Shape& shape, DataType data_type);

  void set_initiate_kernel_ctx(
      const std::function<void(const std::function<Blob*(const std::string&)>&)>&
          initate_kernel_ctx) {
 ...
 private:
  ...
  std::function<void(const std::function<Blob*(const std::string&)>&)> initiate_kernel_ctx_;
};

template<DeviceType device_type>
struct OpKernelTestUtil final {
  template<typename T>
  static void BlobCmp(const std::string& blob_name, const Blob* lhs, DeviceType lhs_device_type,
                      const Blob* rhs, DeviceType rhs_device_type);
  template<typename T>
  static Blob* CreateBlobWithRandomVal(const BlobDesc* blob_desc, Regst* regst);
  template<typename T>
  static Blob* CreateBlobWithSpecifiedVal(const BlobDesc* blob_desc, std::vector<T> val,
                                          Regst* regst);
  template<typename T>
 ...
 private:
  template<typename T>
  static Blob* CreateBlobWithSpecifiedValPtr(const BlobDesc*, T* val, Regst* regst);
};

#define STRINGIZE_OPKERNEL_TEST_ARGS(...) \
  OF_PP_CAT(                              \
      OF_PP_CAT(STRINGIZE_OPKERNEL_TEST_ARGS_, OF_PP_VARIADIC_SIZE(__VA_ARGS__))(__VA_ARGS__), )
...
#define STRINGIZE_OPKERNEL_TEST_ARGS_5(a0, a1, a2, a3, a4) (#a0, #a1, #a2, #a3, #a4)
#define STRINGIZE_OPKERNEL_TEST_ARGS_6(a0, a1, a2, a3, a4, a5) (#a0, #a1, #a2, #a3, #a4, #a5)
...
#define MAKE_OPKERNEL_TEST_ENTRY(func_name, device_type_pair, data_type_pair, ...)              \
  TEST(func_name, OF_PP_JOIN(_, __COUNTER__, OF_PP_PAIR_FIRST(device_type_pair),                \
                             OF_PP_PAIR_FIRST(data_type_pair), ##__VA_ARGS__)) {                \
    OpKernelTestCase opkernel_test_case;                                                        \
    opkernel_test_case.set_default_device_type(OF_PP_PAIR_SECOND(device_type_pair));            \
    func_name<OF_PP_PAIR_SECOND(device_type_pair), OF_PP_PAIR_FIRST(data_type_pair)>            \
        OF_PP_TUPLE_PUSH_FRONT(STRINGIZE_OPKERNEL_TEST_ARGS(__VA_ARGS__), &opkernel_test_case); \
