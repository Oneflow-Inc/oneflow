...
void ReluTestCase(OpKernelTestCase* relu_test_case, const std::string& job_type,
  ...
  relu_test_case->template InitBlob<T>(GenDiffBn("out"), blob_desc, {-8, 7, -6, 5, -4, 3, -2, 1});
  relu_test_case->template ForwardCheckBlob<T>("out", blob_desc, {1, 0, 0, 2, 0, 5, 0, 100});
  relu_test_case->template BackwardCheckBlob<T>(GenDiffBn("in"), blob_desc,
                                                {-8, 0, 0, 5, 0, 3, 0, 1});
}
TEST_CPU_AND_GPU_OPKERNEL(ReluTestCase, FLOATING_DATA_TYPE_SEQ, (train)(predict),
                          (forward)(backward));

DiffKernelImplTestCase* DiffReluKernelImpl(const std::string& job_type, const std::string& fw_or_bw,
