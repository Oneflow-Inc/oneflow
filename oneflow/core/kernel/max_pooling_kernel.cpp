...
template<typename T>
void MaxPoolingKernel<DeviceType::kCPU, T>::NCDHWProcess(const T& lhs, T& rhs) const {
  if (lhs > rhs) { rhs = lhs; }
}

template<typename T>
void MaxPoolingKernel<DeviceType::kCPU, T>::NDHWCProcess(const int64_t in_col,
...
template<typename T>
void MaxPoolingKernel<DeviceType::kCPU, T>::NCDHWProcessGrad(const T& in, const T& out,
                                                             const T& out_diff, const int64_t size,
                                                             T& in_diff) const {
