...
void UniqueWithCountsKernel<device_type, T, K>::ForwardDataContent(
  ...
  UniqueKernelUtil<device_type, T, K>::UniqueWithCounts(
      ctx.device_ctx, x->shape().elem_cnt(), x->dptr<T>(),
      num_unique->mut_dptr<K>(), y->mut_dptr<T>(), idx->mut_dptr<K>(),
      count->mut_dptr<K>(), workspace_ptr, workspace_size_in_bytes);
}

#define MAKE_UNIQUE_WITH_COUNTS_KERNEL_ENTRY(device_type_v, data_type_pair,    \
                                             indices_type_pair)                \
  NEW_REGISTER_KERNEL(                                                         \
      OperatorConf::kUniqueWithCountsConf,                                     \
...
OF_PP_SEQ_PRODUCT_FOR_EACH_TUPLE(MAKE_UNIQUE_WITH_COUNTS_KERNEL_ENTRY,
