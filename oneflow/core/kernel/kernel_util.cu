...
template <typename T>
__global__ void RsqrtGpu(const int64_t n, const T *x, T *y,
                         const float epsilon) {
...
template <typename T>
__global__ void DivByConstParaPtrGpu(const int64_t n, T *x,
                                     const T *alpha_ptr) {
...
template <typename T>
__global__ void MulByScalarParaGpu(const int64_t n, const T *x, const T y,
                                   T *z) {
  CUDA_1D_KERNEL_LOOP(i, n) { z[i] = x[i] * y; }
}
...
template <typename T> __global__ void gpu_set(const T value, T *addr) {
  *addr = value;
}
...
template <int32_t NDIMS> struct Int32Array { int32_t val[NDIMS]; };

template <typename T>
__global__ void
CopyColsRegionGpu(const int64_t row_num, const int64_t col_num, const T *x,
...
__device__ int32_t GetXIndex(const int32_t *y_shape, const int32_t *x_strides,
                             int32_t y_idx) {
  int32_t x_idx = 0;
...
__global__ void TransposeGpu(const Int32Array<NDIMS> y_shape,
                             const Int32Array<NDIMS> x_strides,
  ...
  CUDA_1D_KERNEL_LOOP(y_idx, elem_cnt) {
    const int32_t x_idx =
        GetXIndex<NDIMS>(y_dims_shared, x_strides_shared, y_idx);
...
template <int32_t NDIMS, typename T>
void Transpose(DeviceCtx *ctx, const ShapeView &x_shape,
  ...
  for (int32_t i = 0; i < NDIMS; ++i) {
    x_strides.val[i] = buff[permutation[i]];
  }
  TransposeGpu<NDIMS, T><<<SMBlocksNum4ThreadsNum(elem_cnt),
                           kCudaThreadsNumPerBlock, 0, ctx->cuda_stream()>>>(
      y_shape_struct, x_strides, elem_cnt, x, y);
}

template <typename T> struct TransposeUtil final {
...
__device__ void
MatrixShrinkCols(const size_t row_num, const size_t thread_col_num, const T *x,
  ...
  CUDA_1D_KERNEL_LOOP(index, row_num * thread_col_num) {
    ...
    if (((index / thread_num) % total_shrink_scale) != thread_col / y_col_num) {
      continue;
    }
    ...
    T reduced = x[x_start];
    for (int32_t x_index = x_start + y_col_num; x_index < x_end;
         x_index += y_col_num) {
...
template <typename T, T (*reduce_core_func)(const T, const T),
          size_t shift_size = 2>
__global__ void MatrixRowReduceGpu(const size_t row_num, const size_t col_num,
                                   const T *x, T *y, T *temp_storage,
                                   size_t temp_col_num) {
  const size_t temp_lda = temp_col_num;
  MatrixShrinkCols<T, reduce_core_func>(row_num, temp_lda, x, col_num, col_num,
                                        temp_storage, temp_col_num, temp_lda);
  __syncthreads();
  while (temp_col_num > (1 << shift_size)) {
    size_t new_temp_col_num = temp_col_num >> shift_size;
    MatrixShrinkCols<T, reduce_core_func>(row_num, temp_lda, temp_storage,
                                          temp_col_num, temp_lda, temp_storage,
                                          new_temp_col_num, temp_lda);
    temp_col_num = new_temp_col_num;
    __syncthreads();
  }
  MatrixShrinkCols<T, reduce_core_func>(row_num, temp_lda, temp_storage,
                                        temp_col_num, temp_lda, y, 1, 1);
}

template <typename T, T (*reduce_core_func)(const T, const T),
          size_t shift_size = 2>
void MatrixRowReduce(DeviceCtx *ctx, const size_t row_num, const size_t col_num,
                     const T *x, T *y, void *temp_storage,
                     const size_t temp_storage_bytes) {
  CHECK_NOTNULL(temp_storage);
  CHECK_GT(temp_storage_bytes / sizeof(T), row_num);
  const size_t temp_col_num_shift = std::floor(
      std::log2(std::min(temp_storage_bytes / sizeof(T) / row_num, col_num)));
  const size_t temp_col_num =
      std::min(static_cast<size_t>(kCudaThreadsNumPerBlock),
               static_cast<size_t>(1 << temp_col_num_shift));
  MatrixRowReduceGpu<
      T, reduce_core_func><<<BlocksNum4ThreadsNum(row_num * temp_col_num),
                             kCudaThreadsNumPerBlock, 0, ctx->cuda_stream()>>>(
      row_num, col_num, x, y, static_cast<T *>(temp_storage), temp_col_num);
}

template <typename T>
__global__ void AssignStridedAddrGpu(T **dev_ptrs, T *start_ptr,
                                     int32_t stride_len, int32_t stride_num) {
  CUDA_1D_KERNEL_LOOP(i, stride_num) {
    dev_ptrs[i] = start_ptr + i * stride_len;
  }
}

template <typename T>
void AssignStridedAddr(DeviceCtx *ctx, T **dev_ptrs, T *start_ptr,
                       int stride_len, int stride_num) {
  AssignStridedAddrGpu<T><<<BlocksNum4ThreadsNum(stride_num),
                            kCudaThreadsNumPerBlock, 0, ctx->cuda_stream()>>>(
...
#define KU_IF_METHOD                                                           \
  template <typename T, typename Derived> void GpuKernelUtilIf<T, Derived>::

KU_IF_METHOD Max(DeviceCtx *ctx, const int64_t n, const T *x, T *max_ptr,
                 T *temp_storage, size_t temp_storage_bytes) {
  OF_CUDA_CHECK(cub::DeviceReduce::Max(temp_storage, temp_storage_bytes, x,
                                       max_ptr, n, ctx->cuda_stream()));
}
KU_IF_METHOD Sum(DeviceCtx *ctx, const int64_t n, const T *x, T *sum_ptr,
                 T *temp_storage, size_t temp_storage_bytes) {
  OF_CUDA_CHECK(cub::DeviceReduce::Sum(temp_storage, temp_storage_bytes, x,
                                       sum_ptr, n, ctx->cuda_stream()));
}
KU_IF_METHOD CopyColsRegion(DeviceCtx *ctx, const int64_t row_num,
                            ...
                            const int64_t y_lda) {
  CopyColsRegionGpu<T><<<BlocksNum4ThreadsNum(row_num * col_num),
                         kCudaThreadsNumPerBlock, 0, ctx->cuda_stream()>>>(
      row_num, col_num, x, x_col_offset, x_lda, y, y_col_offset, y_lda);
}
KU_IF_METHOD RowMax(DeviceCtx *ctx, const int64_t row_num,
                    const int64_t col_num, const T *x, T *y, void *temp_storage,
                    const size_t temp_storage_bytes) {
  MatrixRowReduce<T, ReduceCoreMax>(ctx, row_num, col_num, x, y, temp_storage,
                                    temp_storage_bytes);
}
KU_IF_METHOD RowSum(DeviceCtx *ctx, const int64_t row_num,
                    const int64_t col_num, const T *x, T *y, void *temp_storage,
                    const size_t temp_storage_bytes) {
  MatrixRowReduce<T, ReduceCoreAdd>(ctx, row_num, col_num, x, y, temp_storage,
                                    temp_storage_bytes);
}
...
KU_IF_METHOD Transpose(DeviceCtx *ctx, const int32_t num_axis,
  ...
  TransposeUtil<T>::SwitchTranspose(SwitchCase(num_axis), ctx, x_shape, y_shape,
...
KU_IF_METHOD InitializeWithConf(DeviceCtx *ctx,
  ...
  WithHostBlobAndStreamSynchronizeEnv(ctx, blob, [&](Blob *host_blob) {
    KernelUtil<DeviceType::kCPU, T>::InitializeWithConf(
        nullptr, initializer_conf, random_seed, host_blob);
  });
}
...
KU_IF_METHOD Replicate(DeviceCtx *ctx, const int64_t n, T *y, const T *x) {
  ReplicateGpu<T><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock, 0,
                    ctx->cuda_stream()>>>(n, y, x);
}
KU_IF_METHOD AddByScalar(DeviceCtx *ctx, const int64_t n, const T *x, const T y,
                         T *z) {
  AddByScalarGpu<T><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock, 0,
                      ctx->cuda_stream()>>>(n, x, y, z);
}
KU_IF_METHOD MulByScalarPara(DeviceCtx *ctx, const int64_t n, const T *x,
                             const T y, T *z) {
  MulByScalarParaGpu<T><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock, 0,
                          ctx->cuda_stream()>>>(n, x, y, z);
}
...
KU_FLOATING_METHOD BatchedGemm(DeviceCtx *ctx, const enum CBLAS_ORDER order,
  ...
  AssignStridedAddr<T>(ctx, dev_a_ptrs, const_cast<T *>(a), a_stride,
                       batch_size);
  AssignStridedAddr<T>(ctx, dev_b_ptrs, const_cast<T *>(b), b_stride,
                       batch_size);
  ...
  cublasGemmBatchedEx(
      ...
      reinterpret_cast<const void **>(const_cast<const T **>(dev_b_ptrs)),
      data_type, ldb,
      reinterpret_cast<const void **>(const_cast<const T **>(dev_a_ptrs)),
      data_type, lda, reinterpret_cast<const void *>(&beta),
      reinterpret_cast<void **>(dev_c_ptrs), data_type, ldc, batch_size,
      data_type, CUBLAS_GEMM_DEFAULT);
#else
  cublas_gemmBatched<T>(ctx->cublas_pmh_handle(), cublas_trans_b,
                        cublas_trans_a, n, m, k, &alpha,
                        const_cast<const T **>(dev_b_ptrs), ldb,
                        const_cast<const T **>(dev_a_ptrs), lda, &beta,
                        dev_c_ptrs, ldc, batch_size);
#endif
}
...
KU_FLOATING_METHOD Div(DeviceCtx *ctx, const int64_t n, T *x, const T *alpha) {
  DivByConstParaPtrGpu<T><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock, 0,
                            ctx->cuda_stream()>>>(n, x, alpha);
}
KU_FLOATING_METHOD Div(DeviceCtx *ctx, const int64_t n, T *x, const T alpha) {
  DivByConstParaGpu<T><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock, 0,
                         ctx->cuda_stream()>>>(n, x, alpha);
}
...
KU_FLOATING_METHOD MulByScalar(DeviceCtx *ctx, const int64_t n, const T *x,
                               const T *y, T *z) {
  MulByScalarGpu<T><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock, 0,
                      ctx->cuda_stream()>>>(n, x, y, z);
}
KU_FLOATING_METHOD Reciprocal(DeviceCtx *ctx, const int n, const T *x, T *y) {
  ReciprocalGpu<T><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock, 0,
                     ctx->cuda_stream()>>>(n, x, y);
}
KU_FLOATING_METHOD Square(DeviceCtx *ctx, const int64_t n, const T *x, T *y) {
  SquareGpu<T><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock, 0,
                 ctx->cuda_stream()>>>(n, x, y);
}
KU_FLOATING_METHOD Sqrt(DeviceCtx *ctx, const int64_t n, const T *x, T *y) {
  SqrtGpu<T><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock, 0,
               ctx->cuda_stream()>>>(n, x, y);
}
...
KU_FLOATING_METHOD Addition(DeviceCtx *ctx, const int64_t n, T *out,
  ...
  if (out == in_0) {
    gpu_assign_add<T><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock, 0,
                        ctx->cuda_stream()>>>(n, out, in_1, in_2);
  } else {
    gpu_add<T><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock, 0,
                 ctx->cuda_stream()>>>(n, out, in_0, in_1, in_2);
  }
}
...
KU_FLOATING_METHOD Addition(DeviceCtx *ctx, const int64_t n, T *out,
                            ...
                            const T *in_3) {
  gpu_add<T><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock, 0,
               ctx->cuda_stream()>>>(n, out, in_0, in_1, in_2, in_3);
}
...
KU_FLOATING_METHOD Addition(DeviceCtx *ctx, const int64_t n, T *out,
  ...
  gpu_add<T><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock, 0,
               ctx->cuda_stream()>>>(n, out, in_0, in_1, in_2, in_3, in_4);
}
...
KU_FLOATING_METHOD Addition(DeviceCtx *ctx, const int64_t n, T *out,
                            ...
                            const T *in_6) {
  gpu_add<T><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock, 0,
               ctx->cuda_stream()>>>(n, out, in_0, in_1, in_2, in_3, in_4, in_5,
                                     in_6);
}
...
template <> __device__ float gpu_atomic_add(float *address, float val) {
  return atomicAdd(address, val);
}

#if defined(__CUDA_ARCH__) && __CUDA_ARCH__ >= 700 && CUDA_VERSION >= 10000
template <> __device__ half gpu_atomic_add(half *address, half val) {
  return atomicAdd(address, val);
}
#endif
...
template <> __device__ float gpu_atomic_max(float *address, const float val) {
...
template <>
__global__ void CastOnGpu<float, half>(const float *in, half *out,
                                       int64_t elem_num) {
  CUDA_1D_KERNEL_LOOP(i, elem_num) { out[i] = __float2half(in[i]); }
}

template <>
__global__ void CastOnGpu<half, float>(const half *in, float *out,
                                       int64_t elem_num) {
...
void CopyElemOnGpu(DeviceCtx *ctx, const T *in_dptr, U *out_dptr,
  ...
  if (std::is_same<T, U>::value) {
    ...
    CastOnGpu<T, U><<<BlocksNum4ThreadsNum(elem_num), kCudaThreadsNumPerBlock,
                      0, ctx->cuda_stream()>>>(in_dptr, out_dptr, elem_num);
  }
}

template <>
void CopyElemOnGpu<float, float16>(DeviceCtx *ctx, const float *in_dptr,
                                   float16 *out_dptr, int64_t elem_num) {
  CastOnGpu<float, half><<<BlocksNum4ThreadsNum(elem_num),
                           kCudaThreadsNumPerBlock, 0, ctx->cuda_stream()>>>(
      in_dptr, reinterpret_cast<half *>(out_dptr), elem_num);
}

template <>
void CopyElemOnGpu<float16, float>(DeviceCtx *ctx, const float16 *in_dptr,
                                   float *out_dptr, int64_t elem_num) {
  CastOnGpu<half, float><<<BlocksNum4ThreadsNum(elem_num),
                           kCudaThreadsNumPerBlock, 0, ctx->cuda_stream()>>>(
      reinterpret_cast<const half *>(in_dptr), out_dptr, elem_num);
}

#define INSTANTIATE_COPY_ELEM_ON_GPU(T, U)                                     \
  template void CopyElemOnGpu(DeviceCtx *ctx, const T *in_dptr, U *out_dptr,   \
                              int64_t elem_num);

#define MAKE_COPY_ELEM_ON_GPU_ENTRY(TPair, UPair)                              \
  INSTANTIATE_COPY_ELEM_ON_GPU(OF_PP_PAIR_FIRST(TPair), OF_PP_PAIR_FIRST(UPair))

OF_PP_SEQ_PRODUCT_FOR_EACH_TUPLE(MAKE_COPY_ELEM_ON_GPU_ENTRY, POD_DATA_TYPE_SEQ,
                                 POD_DATA_TYPE_SEQ)
