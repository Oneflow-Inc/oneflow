#include "oneflow/core/kernel/pod_boxing_kernel.h"
...
template<typename T>
void PodBoxingKernel<T>::ForwardDataContent(
    const KernelCtx& ctx, std::function<Blob*(const std::string&)> BnInOp2Blob) const {
  if (boxing_conf().in_box_case() == BoxingOpConf::kConcatBox) {
    if (boxing_conf().out_box_case() == BoxingOpConf::kSplitBox) {
      ConcatSplitDataContent(ctx.device_ctx, BnInOp2Blob, this->op_attribute().input_bns(),
                             boxing_conf().concat_box().axis(), this->op_attribute().output_bns(),
                             boxing_conf().split_box().axis());
    } else if (boxing_conf().out_box_case() == BoxingOpConf::kCloneBox) {
      ConcatSplitDataContent(ctx.device_ctx, BnInOp2Blob, this->op_attribute().input_bns(),
                             boxing_conf().concat_box().axis(), this->obn_0(), 0);
      BoxingKernelUtil::CopyFromFirstToOtherBlobs(ctx.device_ctx, BnInOp2Blob,
    ...
    if (boxing_conf().out_box_case() == BoxingOpConf::kSplitBox) {
      CalcSumOfBlobs<T>(ctx.device_ctx, BnInOp2Blob, this->op_attribute().input_bns(), "middle");
      ConcatSplitDataContent(ctx.device_ctx, BnInOp2Blob,
                             BoxingKernelUtil::ConstructPbRpf("middle"), 0,
                             this->op_attribute().output_bns(), boxing_conf().split_box().axis());
    } else if (boxing_conf().out_box_case() == BoxingOpConf::kCloneBox) {
      CalcSumOfBlobs<T>(ctx.device_ctx, BnInOp2Blob, this->op_attribute().input_bns(),
                        this->obn_0().Get(0));
      BoxingKernelUtil::CopyFromFirstToOtherBlobs(ctx.device_ctx, BnInOp2Blob,
                                                  this->op_attribute().output_bns(),
                                                  DataContentIterator::GetCopyBlobFieldMthd());
...
ADD_CPU_DEFAULT_KERNEL_CREATOR(OperatorConf::kPodBoxingConf, PodBoxingKernel,
