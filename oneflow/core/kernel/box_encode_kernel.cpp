#include "oneflow/core/kernel/box_encode_kernel.h"

namespace oneflow {

template<DeviceType device_type, typename T>
void BoxEncodeKernel<device_type, T>::ForwardDataContent(
  ...
  const BBoxRegressionWeights reg_weights = this->op_conf().box_encode_conf().regression_weights();
  ...
  BoxEncodeUtil<device_type, T>::Forward(
...
template<DeviceType device_type, typename T>
void BoxEncodeKernel<device_type, T>::ForwardDim0ValidNum(
    const KernelCtx&, std::function<Blob*(const std::string&)> BnInOp2Blob) const {
  const Blob* ref_boxes_blob = BnInOp2Blob("ref_boxes");
  const Blob* boxes_blob = BnInOp2Blob("boxes");
  CHECK(ref_boxes_blob->has_dim0_valid_num_field());
  CHECK(boxes_blob->has_dim0_valid_num_field());
  const int32_t num_boxes = ref_boxes_blob->shape().At(0);
  CHECK_EQ(num_boxes, boxes_blob->shape().At(0));
  BnInOp2Blob("boxes_delta")->set_dim0_valid_num(0, num_boxes);
}
...
ADD_DEFAULT_KERNEL_CREATOR(OperatorConf::kBoxEncodeConf, BoxEncodeKernel, FLOATING_DATA_TYPE_SEQ);
