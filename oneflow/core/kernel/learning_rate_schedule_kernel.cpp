...
double LinearWarmupLearningRate(const LinearWarmupConf &conf, double lr,
                                int64_t train_step) {
  CHECK_GE(conf.warmup_batches(), 0);
  ...
  if (train_step < conf.warmup_batches()) {
    multiplier =
        start_multiplier +
        (1.0 - start_multiplier) * (train_step * 1.0 / conf.warmup_batches());
  }
  return lr * multiplier;
}

bool TriggerWarmup(const LearningRateScheduleOpConf &conf, double lr,
                   int64_t train_step) {
...
double GetWarmupLearningRate(const WarmupConf &conf, double lr,
                             int64_t train_step) {
...
double ExponentialDecayedLearningRate(const ExponentialDecayConf &conf,
                                      double lr, int64_t cur_batch_num) {
  CHECK_GT(conf.decay_batches(), 0);
  double p = static_cast<double>(cur_batch_num) /
...
double PolynomialDecayedLearningRate(const PolynomialDecayConf &conf, double lr,
  ...
  return (lr - conf.end_learning_rate()) *
             std::pow(1.0 - (cur_batch / decay_batches), conf.power()) +
...
double LinearCosineDecayedLearningRate(const LinearCosineDecayConf &conf,
  ...
  double cosine_decay = 0.5 * (1.0 + std::cos(PI * 2.0 * conf.num_periods() *
                                              cur_batch / decay_batches));
...
double GetDecayedLearningRate(const LearningRateDecayConf &conf, double lr,
  ...
  if (conf.has_exponential_conf()) {
    ...
    return PiecewiseScalingLearningRate(conf.piecewise_scaling_conf(), lr,
                                        cur_batch_num);
  } else {
    UNIMPLEMENTED();
  }
...
void LearningRateScheduleKernel::ForwardDataContent(
  ...
  const LearningRateScheduleOpConf &conf =
      this->op_conf().learning_rate_schedule_conf();
  ...
  if (TriggerWarmup(conf, learning_rate, train_step)) {
    learning_rate =
        GetWarmupLearningRate(conf.warmup_conf(), learning_rate, train_step);
...
REGISTER_KERNEL(OperatorConf::kLearningRateScheduleConf,
