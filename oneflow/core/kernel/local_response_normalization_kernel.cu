...
void LocalResponseNormalizationKernel<DeviceType::kGPU, T>::ForwardDataContent(
  ...
  CudaCheck(cudnnLRNCrossChannelForward(
      ...
      SPOnePtr<T>(), batch_desc_->Get(), BnInOp2Blob("in")->dptr(), SPZeroPtr<T>(),
...
void LocalResponseNormalizationKernel<DeviceType::kGPU, T>::BackwardDataContent(
  ...
  CudaCheck(cudnnLRNCrossChannelBackward(
      ctx.device_ctx->cudnn_handle(), normalize_desc_->Get(), CUDNN_LRN_CROSS_CHANNEL_DIM1,
      SPOnePtr<T>(), batch_desc_->Get(), BnInOp2Blob("out")->dptr(), batch_desc_->Get(),
      BnInOp2Blob("out_diff")->dptr(), batch_desc_->Get(), BnInOp2Blob("in")->dptr(),
      SPZeroPtr<T>(), batch_desc_->Get(), BnInOp2Blob("in_diff")->mut_dptr()));
}
