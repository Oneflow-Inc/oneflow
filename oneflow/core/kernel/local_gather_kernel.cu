...
__global__ void LocalGatherForwardGpu(int64_t elem_cnt, const K* indices, int64_t num_indices,
...
template<typename T, typename K>
__global__ void LocalGatherBackwardGpu(int64_t elem_cnt, const K* indices, int64_t num_indices,
...
template<typename T, typename K>
struct LocalGatherKernelUtil<DeviceType::kGPU, T, K> final {
...
template<typename T, typename K>
void LocalGatherKernelUtil<DeviceType::kGPU, T, K>::Forward(DeviceCtx* ctx, const K* indices,
                                                            int64_t num_indices, const T* in,
                                                            const Shape& flat_in_shape, T* out) {
  const int64_t elem_cnt = flat_in_shape.At(0) * num_indices * flat_in_shape.At(2);
  LocalGatherForwardGpu<T, K>
      <<<BlocksNum4ThreadsNum(elem_cnt), kCudaThreadsNumPerBlock, 0, ctx->cuda_stream()>>>(
          elem_cnt, indices, num_indices, in, flat_in_shape.At(1), flat_in_shape.At(2), out);
}

template<typename T, typename K>
void LocalGatherKernelUtil<DeviceType::kGPU, T, K>::Backward(DeviceCtx* ctx, const K* indices,
                                                             int64_t num_indices, const T* out_diff,
                                                             const Shape& flat_in_shape,
                                                             T* in_diff) {
  const int64_t elem_cnt = flat_in_shape.At(0) * num_indices * flat_in_shape.At(2);
  LocalGatherBackwardGpu<T, K>
      <<<BlocksNum4ThreadsNum(elem_cnt), kCudaThreadsNumPerBlock, 0, ctx->cuda_stream()>>>(
          elem_cnt, indices, num_indices, out_diff, flat_in_shape.At(1), flat_in_shape.At(2),
          in_diff);
}

#define MAKE_LOCAL_GATHER_KERNEL_UTIL_ENTRY(in_type_pair, index_type_pair)                \
  template struct LocalGatherKernelUtil<DeviceType::kGPU, OF_PP_PAIR_FIRST(in_type_pair), \
                                        OF_PP_PAIR_FIRST(index_type_pair)>;
OF_PP_SEQ_PRODUCT_FOR_EACH_TUPLE(MAKE_LOCAL_GATHER_KERNEL_UTIL_ENTRY, FLOATING_DATA_TYPE_SEQ,
                                 INT_DATA_TYPE_SEQ);
#undef MAKE_LOCAL_GATHER_KERNEL_UTIL_ENTRY
