...
__global__ void
SigmoidCrossEntropyLossForward(const int64_t n, const PredType *prediction,
                               const LabelType *label, PredType *loss) {
  CUDA_1D_KERNEL_LOOP(index, n) {
    loss[index] =
        -1.f * prediction[index] * (label[index] - (prediction[index] >= 0)) +
        logf(1 + expf(prediction[index] -
                      2 * prediction[index] * (prediction[index] >= 0)));
  }
}

template <typename PredType, typename LabelType>
__global__ void
SigmoidCrossEntropyLossBackward(const int64_t n, const PredType *prediction,
...
private:
  void ForwardDataContent(
    ...
    SigmoidCrossEntropyLossForward<
        PredType, LabelType><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock,
...
template <typename PredType, typename LabelType>
class SigmoidCrossEntropyGradGpuKernel final
...
private:
  void ForwardDataContent(
    ...
    SigmoidCrossEntropyLossBackward<
        PredType><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock, 0,
                    ctx.device_ctx->cuda_stream()>>>(
        n, prediction->dptr<PredType>(), label->dptr<LabelType>(),
        pred_diff->mut_dptr<PredType>());
    KernelUtil<DeviceType::kGPU, PredType>::Mul(
        ctx.device_ctx, n, pred_diff->dptr<PredType>(),
        loss_diff->dptr<PredType>(), pred_diff->mut_dptr<PredType>());
  }
};
      ...
      .SetIsMatchedPred([](const KernelConf &conf) -> bool {                   \
        return ((conf.op_attribute().op_conf().device_tag() == "gpu") &&       \
                ...
                (GetDataType<ltype>::value ==                                  \
                 conf.op_attribute()                                           \
                     .op_conf()                                                \
                     .sigmoid_cross_entropy_grad_conf()                        \
                     .label_type()));                                          \
      })
