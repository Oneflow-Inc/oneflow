...
void ConvKernel<DeviceType::kGPU, float16>::WeightBackward(
  ...
  if (this->op_conf().trainable()) {
    CudaCheck(cudnnConvolutionBackwardFilter(
        device_ctx->cudnn_handle(), OnePtr<float16>::value, this->in_desc_->Get(),
        in_blob->dptr<float16>(), this->out_desc_->Get(), out_diff_blob->dptr<float16>(),
        this->conv_desc_->Get(),
        static_cast<cudnnConvolutionBwdFilterAlgo_t>(
            this->GetConvKernelConf().cudnn_bwd_filter_algo()),
        bw_cudnn_buf_ptr, bw_cudnn_buf_size, ZeroPtr<float16>::value, this->filter_desc_->Get(),
        weight_diff_blob->mut_dptr<float16>()));
  }
  if (in_diff_blob != nullptr) {
    CudaCheck(cudnnConvolutionBackwardData(
        device_ctx->cudnn_handle(), OnePtr<float16>::value, this->filter_desc_->Get(),
        weight_blob->dptr<float16>(), this->out_desc_->Get(), out_diff_blob->dptr<float16>(),
        this->conv_desc_->Get(),
        static_cast<cudnnConvolutionBwdDataAlgo_t>(this->GetConvKernelConf().cudnn_bwd_data_algo()),
        bw_cudnn_buf_ptr, bw_cudnn_buf_size, ZeroPtr<float16>::value, this->in_desc_->Get(),
        in_diff_blob->mut_dptr<float16>()));
  }
}

void ConvKernel<DeviceType::kGPU, float16>::BiasBackward(
    DeviceCtx* device_ctx, const Blob* out_diff_blob, Blob* bias_diff_blob,
    std::function<Blob*(const std::string&)> BnInOp2Blob) const {
  CHECK(this->EnableCudnn());
  CudaCheck(cudnnConvolutionBackwardBias(device_ctx->cudnn_handle(), OnePtr<float16>::value,
                                         this->out_desc_->Get(), out_diff_blob->dptr<float16>(),
                                         ZeroPtr<float16>::value, this->bias_desc_->Get(),
                                         bias_diff_blob->mut_dptr<float16>()));
}
