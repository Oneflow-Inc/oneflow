...
void CudnnConvKernel<T>::VirtualKernelInit(const ParallelContext*) {
  ...
  std::vector<int> stride_of_in_tensor(kernel_dim_size, 1);
  std::vector<int> stride_of_out_tensor(kernel_dim_size, 1);
  ...
  std::vector<int> in_dim(in_shape.dim_vec().begin(),
                              in_shape.dim_vec().end());
  std::vector<int> out_dim(out_shape.dim_vec().begin(),
                               out_shape.dim_vec().end());

  this->in_desc_ =
      new CudnnTensorDesc(CudnnDataType<T>::val, in_dim, stride_of_in_tensor);
  this->out_desc_ =
      new CudnnTensorDesc(CudnnDataType<T>::val, out_dim, stride_of_out_tensor);
  this->filter_desc_ =
  ...
  if (this->GetBoolFromCustomizedOpConf("use_bias")) {
    ...
    this->bias_desc_ = new CudnnTensorDesc(CudnnDataType<T>::val, 1, &filters);
  }
}
