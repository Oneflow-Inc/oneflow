...
private:
  ...
  using MdUpdateUtilT =
      IndexedSlicesLazyAdamMdUpdateKernelUtil<device_type, T, K, int32_t>;
  void
...
void IndexedSlicesLazyAdamMdUpdateKernel<device_type, T, K>::ForwardDataContent(
  ...
  float *local_learning_rate_ptr =
      BnInOp2Blob("local_learning_rate")->mut_dptr<float>();
  ...
  MdUpdateUtilT::ComputeLocalLearningRate(ctx.device_ctx, beta1, beta2,
                                          train_step_ptr, learning_rate_ptr,
                                          local_learning_rate_ptr);
  MdUpdateUtilT::Update(
      ctx.device_ctx, beta1, beta2, epsilon, num_indices, feature_size,
...
#define MAKE_INDEXED_SLICES_LAZY_ADAM_MODEL_UPDATE_KERNEL_ENTRY(               \
    device_type_v, data_type_pair, indices_type_pair)                          \
  NEW_REGISTER_KERNEL(OperatorConf::kIndexedSlicesLazyAdamModelUpdateConf,     \
                      IndexedSlicesLazyAdamMdUpdateKernel<                     \
      ...
      .SetIsMatchedPred([](const KernelConf &kernel_conf) -> bool {            \
        return ((kernel_conf.op_attribute().op_conf().device_tag() ==          \
                ...
                (OF_PP_PAIR_SECOND(indices_type_pair) ==                       \
                 kernel_conf.indexed_slices_lazy_adam_model_update_conf()      \
                     .indices_data_type()));                                   \
      });

OF_PP_SEQ_PRODUCT_FOR_EACH_TUPLE(
    MAKE_INDEXED_SLICES_LAZY_ADAM_MODEL_UPDATE_KERNEL_ENTRY, DEVICE_TYPE_SEQ,
