...
template <typename T> struct SliceBoxingKernelUtil<DeviceType::kGPU, T> {
  static void Add(DeviceCtx *ctx, int64_t n, const T *a, const T *b, T *out);
};

template <typename T>
void SliceBoxingKernelUtil<DeviceType::kGPU, T>::Add(DeviceCtx *ctx, int64_t n,
                                                     const T *a, const T *b,
                                                     T *out) {
  AddGpu<T><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock, 0,
              ctx->cuda_stream()>>>(n, a, b, out);
}

template <>
void SliceBoxingKernelUtil<DeviceType::kGPU, float16>::Add(DeviceCtx *ctx,
                                                           int64_t n,
                                                           const float16 *a,
                                                           const float16 *b,
                                                           float16 *out) {
  SliceBoxingKernelUtil<DeviceType::kGPU, half>::Add(
      ctx, n, reinterpret_cast<const half *>(a),
      reinterpret_cast<const half *>(b), reinterpret_cast<half *>(out));
}
