...
void DropoutTestCase(OpKernelTestCase<device_type>* dropout_test_case,
  ...
  dropout_test_case->template InitBlob<T>(GenDiffBn("out"), blob_desc,
                                          output_diff_data);
  if (is_train) {
    dropout_test_case->template ForwardCheckBlob<float>(
        "random_mask", random_mask_blob_desc, random_mask);
  }
  dropout_test_case->template ForwardCheckBlob<T>(
      "out", blob_desc,
      Dropout<device_type, T>(input_data, random_mask,
                              (is_train ? rate : 0.f)));
  dropout_test_case->template BackwardCheckBlob<T>(
      GenDiffBn("in"), blob_desc,
      Dropout<device_type, T>(output_diff_data, random_mask,
                              (is_train ? rate : 0.f)));
}
