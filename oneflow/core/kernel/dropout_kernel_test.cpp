...
std::vector<T> Dropout(const std::vector<T>& input,
  ...
  FOR_RANGE(int, i, 0, output.size()) {
    output.at(i) *= (random_mask.at(i) > rate) / (1.f - rate);
  }
  return output;
}
...
OpKernelTestCase* DropoutTestCase(const std::string& job_type,
  ...
  BlobDesc* random_mask_blob_desc =
      new BlobDesc(Shape({1, 8}), GetDataType<float>::val, false, false, 1);
  ...
  if (is_train) {
    dropout_test_case->ForwardCheckBlob(
        "random_mask", device_type,
        KTCommon<device_type, float>::CreateBlobWithSpecifiedVal(
            random_mask_blob_desc, random_mask));
  }
  dropout_test_case->ForwardCheckBlob(
      "out", device_type,
      KTC::CreateBlobWithSpecifiedVal(
          blob_desc, Dropout<device_type, T>(input_data, random_mask,
                                             (is_train ? rate : 0.f))));
  dropout_test_case->BackwardCheckBlob(
      GenDiffBn("in"), device_type,
      KTC::CreateBlobWithSpecifiedVal(
          blob_desc, Dropout<device_type, T>(output_diff_data, random_mask,
                                             (is_train ? rate : 0.f))));
  return dropout_test_case;
}

TEST_CPU_AND_GPU_OPKERNEL(DropoutTestCase, ARITHMETIC_DATA_TYPE_SEQ,
