...
void UpsampleNearestKernel<device_type, T>::ForwardDataContent(
  ...
  ResizeNearestNeighborUtil<device_type, T>::Forward(ctx, 1.f / scale, 1.f / scale, false, in_blob,
                                                     out_blob);
}
...
void UpsampleNearestKernel<device_type, T>::BackwardDataContent(
  ...
  const int32_t scale = this->op_conf().upsample_nearest_conf().scale();
  ResizeNearestNeighborUtil<device_type, T>::Backward(ctx, 1.f / scale, 1.f / scale, false,
...
void UpsampleNearestKernel<device_type, T>::ForwardInstanceShape(
  ...
  BnInOp2Blob("out")->set_instance_shape(
      Shape({in_shape.At(1), scale * in_shape.At(2), scale * in_shape.At(3)}));
}
