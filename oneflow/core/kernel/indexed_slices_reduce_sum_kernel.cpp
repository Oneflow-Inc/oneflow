...
void IndexedSlicesReduceSumKernel<device_type, T, K>::ForwardDataContent(
  ...
  IndexedSlicesReduceSumKernelUtil<device_type, K, T, int64_t>::ReduceSum(
      ...
      num_unique->mut_dptr<int64_t>(), y_indices->mut_dptr<K>(),
      y_values->mut_dptr<T>(), workspace_ptr, workspace_size_in_bytes);
}

#define MAKE_INDEXED_SLICES_REDUCE_SUM_KERNEL_ENTRY(                           \
    device_type_v, data_type_pair, indices_type_pair)                          \
  NEW_REGISTER_KERNEL(                                                         \
      OperatorConf::kIndexedSlicesReduceSumConf,                               \
      IndexedSlicesReduceSumKernel<device_type_v,                              \
      ...
      .SetIsMatchedPred([](const KernelConf &kernel_conf) -> bool {            \
        return ((kernel_conf.op_attribute().op_conf().device_tag() ==          \
                ...
                (OF_PP_PAIR_SECOND(indices_type_pair) ==                       \
                 kernel_conf.indexed_slices_reduce_sum_conf()                  \
                     .indices_data_type()));                                   \
      });

OF_PP_SEQ_PRODUCT_FOR_EACH_TUPLE(MAKE_INDEXED_SLICES_REDUCE_SUM_KERNEL_ENTRY,
