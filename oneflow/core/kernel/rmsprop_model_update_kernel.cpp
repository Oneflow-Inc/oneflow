...
const RMSPropModelUpdateConf &
GetRMSPropModelUpdateConf(const OperatorConf &op_conf) {
...
const PbMessage &
RMSPropMdUpdateKernel<device_type, T>::GetCustomizedOpConf() const {
...
void RMSPropMdUpdateKernel<device_type, T>::UpdateModel(
  ...
  const RMSPropModelUpdateConf &conf =
      GetRMSPropModelUpdateConf(this->op_conf());

  Blob *mean_gradient_blob = nullptr;
  if (conf.centered()) {
    mean_gradient_blob = BnInOp2Blob("mean_gradient");
  }

  RMSPropMdUpdateKernelUtil<device_type, T>::UpdateModel(
      ctx, model_blob->shape().elem_cnt(), train_step, learning_rate,
      static_cast<T>(conf.decay_rate()), static_cast<T>(conf.epsilon()),
      conf.centered(), weight_decay, model_diff_blob->dptr<T>(),
...
public:
  static void UpdateModel(DeviceCtx *ctx, int64_t n, const int64_t *train_step,
                          ...
                          bool centered, T weight_decay, const T *model_diff,
                          T *model, T *mean_square, T *mean_gradient) {
    if (centered) {
      for (int64_t i = 0; i < n; ++i) {
        T model_diff_val = model_diff[i];
        T mean_square_val = mean_square[i];
        mean_square_val = (1 - decay_rate) * model_diff_val * model_diff_val +
                          decay_rate * mean_square_val;
        mean_square[i] = mean_square_val;
        T mean_gradient_val = mean_gradient[i];
        mean_gradient_val =
            (1 - decay_rate) * model_diff_val + decay_rate * mean_gradient_val;
        mean_gradient[i] = mean_gradient_val;
        T denom_t = mean_square_val - mean_gradient_val * mean_gradient_val;
        model[i] =
            model[i] -
            *learning_rate * model_diff_val / std::sqrt(denom_t + epsilon);
      }
    } else {
      for (int64_t i = 0; i < n; ++i) {
        T model_diff_val = model_diff[i];
        mean_square[i] = (1 - decay_rate) * model_diff_val * model_diff_val +
                         decay_rate * mean_square[i];
        model[i] = model[i] -
                   *learning_rate * model_diff_val /
...
ADD_DEFAULT_KERNEL_CREATOR(OperatorConf::kRmspropModelUpdateConf,
