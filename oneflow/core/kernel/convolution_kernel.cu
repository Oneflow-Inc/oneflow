...
void CudnnConvolutionKernel<T>::ForwardDataContent(
  ...
  cudnnConvolutionFwdAlgo_t cudnn_fwd_algo =
      static_cast<cudnnConvolutionFwdAlgo_t>(conv_conf.cudnn_fwd_algo());
  CudaCheck(cudnnConvolutionForward(
      ctx.device_ctx->cudnn_handle(), CudnnDataType<T>::one, this->in_desc_,
      in_blob->dptr<T>(), this->filter_desc_, weight_blob->dptr<T>(),
      this->conv_desc_, cudnn_fwd_algo, cudnn_workspace->mut_dptr<T>(),
      cudnn_workspace->shape().At(0), CudnnDataType<T>::zero, this->out_desc_,
      out_blob->mut_dptr<T>()));
...
template<typename T>
void CudnnConvolutionKernel<T>::ComputeWeightDiff(
    const KernelCtx& ctx,
    std::function<Blob*(const std::string&)> BnInOp2Blob) const {
  auto conv_conf = this->op_conf().convolution_conf();
  const Blob* in_blob = BnInOp2Blob("in");
  ...
  cudnnConvolutionBwdFilterAlgo_t cudnn_bwd_filter_algo =
      static_cast<cudnnConvolutionBwdFilterAlgo_t>(
          conv_conf.cudnn_bwd_filter_algo());

  CudaCheck(cudnnConvolutionBackwardFilter(
      ctx.device_ctx->cudnn_handle(), CudnnDataType<T>::one, this->in_desc_,
      in_blob->dptr<T>(), this->out_desc_, out_diff_blob->dptr<T>(),
      this->conv_desc_, cudnn_bwd_filter_algo, cudnn_workspace->mut_dptr<T>(),
      cudnn_workspace->shape().At(0), CudnnDataType<T>::one, this->filter_desc_,
      weight_diff_blob->mut_dptr<T>()));
}

template<typename T>
void CudnnConvolutionKernel<T>::ComputeBiasDiff(
    const KernelCtx& ctx,
    std::function<Blob*(const std::string&)> BnInOp2Blob) const {
  const Blob* out_diff_blob = BnInOp2Blob("out_diff");
...
template<typename T>
void CudnnConvolutionKernel<T>::ComputeInputDiff(
    const KernelCtx& ctx,
    std::function<Blob*(const std::string&)> BnInOp2Blob) const {
  auto conv_conf = this->op_conf().convolution_conf();
  const Blob* out_diff_blob = BnInOp2Blob("out_diff");
  ...
  CudaCheck(cudnnConvolutionBackwardData(
      ...
      this->conv_desc_, cudnn_bwd_data_algo, cudnn_workspace->mut_dptr<T>(),
      cudnn_workspace->shape().At(0), CudnnDataType<T>::zero, this->in_desc_,
