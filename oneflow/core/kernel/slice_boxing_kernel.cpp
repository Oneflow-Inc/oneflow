...
void SliceBoxingAddKernel::ForwardDataContent(KernelContext* ctx) const {
  ...
  std::unique_ptr<ep::primitive::Add> primitive =
      ep::primitive::NewPrimitive<ep::primitive::AddFactory>(ctx->stream()->device_type(),
                                                             out->data_type());
  CHECK(primitive);
  std::unique_ptr<ep::primitive::Memset> memset_primitive = ep::primitive::NewPrimitive<ep::primitive::MemsetFactory>(ctx->stream()->device_type());
  ...
  FOR_RANGE(int64_t, i, 0, this->op_attribute().input_bns().size()) {
    ...
    if (i == 0) {
      ...
      if (in_i->shape() == out->shape()) {
        ...
        memset_primitive->Launch(ctx->stream(), buf->mut_dptr(), 0, buf->shape().elem_cnt() * GetSizeOfDataType(buf->data_type()));
