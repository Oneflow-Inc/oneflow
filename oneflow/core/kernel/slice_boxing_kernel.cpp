...
void SliceBoxingKernel<device_type, T>::VirtualKernelInit() {
  ...
  for (const TensorSliceViewProto &in_slice_proto : conf.in_slice()) {
    ...
    tensor_slice_copier_vec_.emplace_back(new TensorSliceCopier(
        out_slice, in_slice, this->kernel_conf().data_type()));
  }
}
...
template <DeviceType device_type, typename T>
const SliceBoxingConf &
SliceBoxingCopyKernel<device_type, T>::GetCustomizedBoxingConf() const {
...
void SliceBoxingCopyKernel<device_type, T>::ForwardDataContent(
  ...
  FOR_RANGE(int64_t, i, 0, this->op_attribute().input_bns().size()) {
    ...
    this->tensor_slice_copier_vec().at(i)->Copy(
        ctx.device_ctx, *this->memory_copier(), out, in_i);
  }
}

template <DeviceType device_type, typename T>
const SliceBoxingConf &
SliceBoxingAddKernel<device_type, T>::GetCustomizedBoxingConf() const {
    ...
    if (i == 0) {
      ...
      bool can_direct_access =
          ...
          (device_type == DeviceType::kGPU &&
           in_i->mem_case().has_device_cuda_mem() &&
      ...
      if (in_i->shape() == out->shape() && can_direct_access) {
        SliceBoxingKernelUtil<device_type, T>::Add(
            ctx.device_ctx, out->shape().elem_cnt(), in_i->dptr<T>(),
            out->dptr<T>(), out->mut_dptr<T>());
      } else {
        Blob *buf = BnInOp2Blob("buf");
        this->tensor_slice_copier_vec().at(i)->Copy(
            ctx.device_ctx, *this->memory_copier(), buf, in_i);
        SliceBoxingKernelUtil<device_type, T>::Add(
            ctx.device_ctx, out->shape().elem_cnt(), buf->dptr<T>(),
            out->dptr<T>(), out->mut_dptr<T>());
      }
    }
  }
...
ADD_DEFAULT_KERNEL_CREATOR(OperatorConf::kSliceBoxingCopyConf,
                           SliceBoxingCopyKernel,
                           POD_DATA_TYPE_SEQ FLOAT16_DATA_TYPE_SEQ)
ADD_DEFAULT_KERNEL_CREATOR(OperatorConf::kSliceBoxingAddConf,
                           SliceBoxingAddKernel,
