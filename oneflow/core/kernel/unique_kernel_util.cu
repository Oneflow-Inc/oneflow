...
template <typename ValueType, typename UnderlyingT,
...
public:
  __host__ __device__ __forceinline__ NotEqualToPreviousAdjacentIterator(
      const UnderlyingT *underlying, OffsetT offset)
  ...
  __host__ __device__ __forceinline__ reference operator*() const {
    return offset == 0 ? 0
                       : (underlying[offset] == underlying[offset - 1] ? 0 : 1);
  }
...
template <typename T> struct Buffer final {
  T *ptr = nullptr;
  size_t size_in_bytes = 0;
};

template <typename T> int64_t GetTempBufferSize(int64_t n) {
...
int64_t GetCubScanTempStorageSize(int64_t n) {
  ...
  OF_CUDA_CHECK((cub::DeviceScan::InclusiveSum<
                 NotEqualToPreviousAdjacentIterator<IDX, KEY>,
                 PermutationIterator<IDX, IDX *, IDX *>>(
      nullptr, cub_scan_temp_store_size, unique_counting_iter, remapping_iter,
      n)));
...
int64_t GetCubRleTempStorageSize(int64_t n) {
  ...
  OF_CUDA_CHECK(
      (cub::DeviceRunLengthEncode::Encode<KEY *, KEY *, IDX *, int64_t *>(
          nullptr, cub_rle_temp_store_size, nullptr, nullptr, nullptr, nullptr,
          n)));
...
template <typename KEY, typename IDX> int64_t GetCubTempStorageSize(int64_t n) {
  int64_t cub_temp_storage_size = 0;
  cub_temp_storage_size =
      std::max(cub_temp_storage_size, GetCubSortTempStorageSize<KEY, IDX>(n));
  cub_temp_storage_size =
      std::max(cub_temp_storage_size, GetCubScanTempStorageSize<KEY, IDX>(n));
  cub_temp_storage_size =
      std::max(cub_temp_storage_size, GetCubRleTempStorageSize<KEY, IDX>(n));
...
void UniqueAliasWorkspace(DeviceCtx *ctx, int64_t n, void *workspace,
  ...
  AliasPtr(workspace, &offset, cub_temp_storage,
           GetCubTempStorageSize<KEY, IDX>(n));
  *workspace_size_in_bytes = offset;
}

template <typename IDX> __global__ void IotaKernel(int64_t n, IDX *out) {
...
struct UniqueKernelUtil<DeviceType::kGPU, KEY, IDX> {
  static void Unique(DeviceCtx *ctx, int64_t n, const KEY *in, IDX *num_unique,
                     KEY *unique_out, IDX *idx_out, void *workspace,
                     int64_t workspace_size_in_bytes);
  static void UniqueWithCounts(DeviceCtx *ctx, int64_t n, const KEY *in,
                               IDX *num_unique, KEY *unique_out, IDX *idx_out,
                               IDX *count, void *workspace,
                               int64_t workspace_size_in_bytes);
  static void GetUniqueWorkspaceSizeInBytes(DeviceCtx *ctx, int64_t n,
                                            int64_t *workspace_size_in_bytes);
  static void
  GetUniqueWithCountsWorkspaceSizeInBytes(DeviceCtx *ctx, int64_t n,
                                          int64_t *workspace_size_in_bytes);
};

template <typename KEY, typename IDX>
void UniqueKernelUtil<DeviceType::kGPU, KEY, IDX>::Unique(
    DeviceCtx *ctx, int64_t n, const KEY *in, IDX *num_unique, KEY *unique_out,
  ...
  UniqueKernelUtil<DeviceType::kGPU, KEY, IDX>::UniqueWithCounts(
      ctx, n, in, num_unique, unique_out, idx_out,
      reinterpret_cast<IDX *>(workspace),
...
void UniqueKernelUtil<DeviceType::kGPU, KEY, IDX>::UniqueWithCounts(
    DeviceCtx *ctx, int64_t n, const KEY *in, IDX *num_unique, KEY *unique_out,
    IDX *idx_out, IDX *count, void *workspace,
    int64_t workspace_size_in_bytes) {
  ...
  Buffer<IDX> cub_sort_values_out;
  Buffer<void> cub_temp_storage;
  UniqueAliasWorkspace<KEY, IDX>(ctx, n, workspace, &rt_workspace_size,
                                 &cub_sort_keys_out, &cub_sort_values_out,
                                 &cub_temp_storage);
  CHECK_LE(rt_workspace_size, workspace_size_in_bytes);
  IotaKernel<IDX><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock, 0,
                    ctx->cuda_stream()>>>(n, cub_sort_values_in_ptr);
  OF_CUDA_CHECK((cub::DeviceRadixSort::SortPairs<KEY, IDX>(
      cub_temp_storage.ptr, cub_temp_storage.size_in_bytes, in,
      cub_sort_keys_out.ptr, cub_sort_values_in_ptr, cub_sort_values_out.ptr, n,
      0, sizeof(KEY) * 8, ctx->cuda_stream())));
  OF_CUDA_CHECK((cub::DeviceRunLengthEncode::Encode<KEY *, KEY *, IDX *, IDX *>(
      cub_temp_storage.ptr, cub_temp_storage.size_in_bytes,
      cub_sort_keys_out.ptr, unique_out, count, num_unique, n,
      ctx->cuda_stream())));
  NotEqualToPreviousAdjacentIterator<IDX, KEY> unique_counting_iter(
      cub_sort_keys_out.ptr, 0);
  PermutationIterator<IDX, IDX *, IDX *> remapping_iter(
      idx_out, cub_sort_values_out.ptr);
  OF_CUDA_CHECK((cub::DeviceScan::InclusiveSum<
                 NotEqualToPreviousAdjacentIterator<IDX, KEY>,
                 PermutationIterator<IDX, IDX *, IDX *>>(
      cub_temp_storage.ptr, cub_temp_storage.size_in_bytes,
      unique_counting_iter, remapping_iter, n, ctx->cuda_stream())));
}

template <typename KEY, typename IDX>
void UniqueKernelUtil<DeviceType::kGPU, KEY, IDX>::
    GetUniqueWorkspaceSizeInBytes(DeviceCtx *ctx, int64_t n,
                                  int64_t *workspace_size_in_bytes) {
  UniqueKernelUtil<DeviceType::kGPU, KEY, IDX>::
      GetUniqueWithCountsWorkspaceSizeInBytes(ctx, n, workspace_size_in_bytes);
...
void UniqueKernelUtil<DeviceType::kGPU, KEY, IDX>::
    GetUniqueWithCountsWorkspaceSizeInBytes(DeviceCtx *ctx, int64_t n,
                                            int64_t *workspace_size_in_bytes) {
  UniqueAliasWorkspace<KEY, IDX>(ctx, n, nullptr, workspace_size_in_bytes,
                                 nullptr, nullptr, nullptr);
}

#define INSTANTIATE_UNIQUE_KERNEL_UTIL_GPU(key_type_pair, idx_type_pair)       \
  template struct UniqueKernelUtil<DeviceType::kGPU,                           \
                                   OF_PP_PAIR_FIRST(key_type_pair),            \
                                   OF_PP_PAIR_FIRST(idx_type_pair)>;
OF_PP_SEQ_PRODUCT_FOR_EACH_TUPLE(INSTANTIATE_UNIQUE_KERNEL_UTIL_GPU,
