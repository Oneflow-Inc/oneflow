...
template <DeviceType device_type> class RandomGenerator;

template <> class RandomGenerator<DeviceType::kCPU> final {
public:
  ...
  template <typename T> void Uniform(const int64_t elem_cnt, T *dptr);
  template <typename T>
...
template <> class RandomGenerator<DeviceType::kGPU> final {
public:
  OF_DISALLOW_COPY_AND_MOVE(RandomGenerator);
  RandomGenerator(int64_t seed, DeviceCtx *device_ctx);
  ~RandomGenerator();
