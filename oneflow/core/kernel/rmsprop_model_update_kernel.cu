...
__global__ void UpdateModelGpu(int64_t n, const int64_t *train_step,
                               const float *learning_rate, T decay_rate,
                               T epsilon, T weight_decay, const T *model_diff,
  ...
  CUDA_1D_KERNEL_LOOP(i, n) {
    ...
    model[i] =
        model[i] - *learning_rate * model_diff_val * rsqrt(denom_t + epsilon);
  }
}
...
public:
  static void UpdateModel(DeviceCtx *ctx, int64_t n, const int64_t *train_step,
    ...
    if (centered) {
      UpdateModelGpu<T,
                     true><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock,
                             0, ctx->cuda_stream()>>>(
          n, train_step, learning_rate, decay_rate, epsilon, weight_decay,
          model_diff, model, mean_square, mean_gradient);
    } else {
      UpdateModelGpu<T,
                     false><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock,
                              0, ctx->cuda_stream()>>>(
          n, train_step, learning_rate, decay_rate, epsilon, weight_decay,
          model_diff, model, mean_square, mean_gradient);
    }
