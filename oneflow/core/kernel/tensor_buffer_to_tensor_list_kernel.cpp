...
template <typename D> struct SwitchUtil final {
#define MAKE_COPY_ELEM_SWITCH_ENTRY(func_name, S) func_name<S, D>
  DEFINE_STATIC_SWITCH_FUNC(void, CopyTensorBufferTo,
                            MAKE_COPY_ELEM_SWITCH_ENTRY,
...
private:
  ...
  void ForwardHeader(
    ...
    if (this->kernel_conf().need_do_shape()) {
...
void TensorBufferToTensorListKernel<T>::ForwardDataContent(
  ...
  FOR_RANGE(int, i, 0, in_blob->shape().elem_cnt()) {
    ...
    SwitchUtil<T>::SwitchCopyTensorBufferTo(SwitchCase(in_buffer.data_type()),
                                            in_buffer,
