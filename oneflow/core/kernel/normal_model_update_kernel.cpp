...
void NormalMdUpdateKernel<device_type, T>::VirtualKernelInit() {
  ...
  weight_decay_ =
      static_cast<T>(GetValFromPbMessage<float>(op_conf, "weight_decay"));
  if (!IsWeightDecaySupported()) {
    CHECK_EQ(weight_decay_, static_cast<T>(0));
  }
}
...
void NormalMdUpdateKernel<device_type, T>::ForwardDataContent(
  ...
  UpdateModel(ctx.device_ctx, weight_decay_, train_step_ptr, learning_rate_ptr,
              BnInOp2Blob);
}

#define INSTANTIATE_KERNEL(device_type, data_type_pair)                        \
  template class NormalMdUpdateKernel<device_type,                             \
