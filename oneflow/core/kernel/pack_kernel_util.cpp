...
void PackKernelUtil<device_type>::Pack(DeviceCtx *ctx, size_t in_index,
                                       size_t total_pack_num,
...
template <DeviceType device_type>
void PackKernelUtil<device_type>::Unpack(DeviceCtx *ctx, size_t out_index,
                                         size_t total_unpack_num,
                                         const Blob *in_blob, Blob *out_blob) {
  size_t in_byte_size = in_blob->ByteSizeOfBlobBody();
  size_t out_byte_size = out_blob->ByteSizeOfBlobBody();
