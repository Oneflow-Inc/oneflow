...
struct ArgWhereKernelUtil {
  static void ArgWhere(DeviceCtx *ctx, const ShapeView &in_shape,
                       const T *in_ptr, void *tmp, size_t tmp_max_bytes,
                       I *out_ptr, I *out_size_ptr);
  static size_t GetArgWhereWorkspaceSizeInBytes(DeviceCtx *ctx, int64_t n);
};

#define INSTANTIATE_ARG_WHERE_KERNEL_UTIL_INTERNAL(device_type_v, dtype,       \
                                                   itype, ndims)               \
  template struct ArgWhereKernelUtil<device_type_v, dtype, itype, ndims>;

#define INSTANTIATE_ARG_WHERE_KERNEL_UTIL(device_type_v, dtype_pair,           \
                                          itype_pair)                          \
  INSTANTIATE_ARG_WHERE_KERNEL_UTIL_INTERNAL(device_type_v,                    \
