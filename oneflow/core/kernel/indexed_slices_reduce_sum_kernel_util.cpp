...
template <typename IDX> int64_t GetUniqueIdxSize(int64_t n) {
...
void IndexedSlicesReduceSumKernelUtil<device_type, K, T, IDX>::ReduceSum(
  ...
  void *unique_workspace_ptr =
      reinterpret_cast<unsigned char *>(workspace) + unique_idx_size;
  const int64_t unique_workspace_size =
      workspace_size_in_bytes - unique_idx_size;
  UniqueKernelUtil<device_type, K, IDX>::Unique(
      ctx, n, indices, num_unique_indices, indices_out, unique_idx_ptr,
  ...
  UnsortedSegmentSumKernelUtil<device_type, T, IDX>::UnsortedSegmentSum(
      ctx, unique_idx_ptr, values, n, n, 1, m, 0, values_out);
}

template <DeviceType device_type, typename K, typename T, typename IDX>
void IndexedSlicesReduceSumKernelUtil<device_type, K, T, IDX>::
    GetReduceSumWorkspaceSizeInBytes(DeviceCtx *ctx, int64_t n, int64_t m,
                                     int64_t *workspace_size_in_bytes) {
  int64_t unique_workspace_size;
  UniqueKernelUtil<device_type, K, int64_t>::GetUniqueWorkspaceSizeInBytes(
      ctx, n, &unique_workspace_size);
  *workspace_size_in_bytes = GetUniqueIdxSize<IDX>(n) + unique_workspace_size;
}

#define INSTANTIATE_INDEXED_SLICES_REDUCE_SUM_KERNEL_UTIL(                     \
    device_type, key_type_pair, val_type_pair, idx_type_pair)                  \
  template struct IndexedSlicesReduceSumKernelUtil<                            \
      device_type, OF_PP_PAIR_FIRST(key_type_pair),                            \
      OF_PP_PAIR_FIRST(val_type_pair), OF_PP_PAIR_FIRST(idx_type_pair)>;
OF_PP_SEQ_PRODUCT_FOR_EACH_TUPLE(
    INSTANTIATE_INDEXED_SLICES_REDUCE_SUM_KERNEL_UTIL, DEVICE_TYPE_SEQ,
