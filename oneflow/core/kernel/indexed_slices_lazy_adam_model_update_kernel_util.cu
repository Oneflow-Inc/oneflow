...
__global__ void UpdateGpu(T beta1, T beta2, T epsilon, int64_t feature_size,
  ...
  CUDA_1D_KERNEL_LOOP(i, n) {
    ...
    if (instance_id >= lower_bound && instance_id < upper_bound) {
      ...
      v[model_idx] = new_v;
      model[model_idx] =
          model[model_idx] - *learning_rate * new_m / (sqrt(new_v) + epsilon);
    }
  }
}

template <typename T>
__global__ void ComputeLocalLearningRateGpu(T beta1, T beta2,
...
template <typename T, typename K, typename IDX>
void IndexedSlicesLazyAdamMdUpdateKernelUtil<DeviceType::kGPU, T, K, IDX>::
    Update(DeviceCtx *ctx, T beta1, T beta2, T epsilon, int64_t num_instance,
  ...
  UpdateGpu<T, K><<<BlocksNum4ThreadsNum(num_instance * feature_size),
      ...
      beta1, beta2, epsilon, feature_size, lower_bound, upper_bound,
      num_unique_instance, learning_rate, indices, values, model, m, v);
}

template <typename T, typename K, typename IDX>
void IndexedSlicesLazyAdamMdUpdateKernelUtil<DeviceType::kGPU, T, K, IDX>::
    ComputeLocalLearningRate(DeviceCtx *ctx, T beta1, T beta2,
                             const int64_t *train_step,
                             const float *learning_rate,
                             float *local_learning_rate) {
...
OF_PP_SEQ_PRODUCT_FOR_EACH_TUPLE(
    INSTANTIATE_INDEXED_SLICES_LAZY_ADAM_MODEL_UPDATE_KERNEL_UTIL_GPU,
