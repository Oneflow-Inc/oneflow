...
void WaitAndSendIdsKernel<T>::ForwardDataContent(
  ...
  if (status->out_idx_ >= status->out_num_) {
    status->buffer_status_ = Global<BufferMgr<int64_t>>::Get()
                                 ->Get(conf.wait_buffer_name())
                                 ->Receive(&status->in_id_);
    if (status->buffer_status_ == kBufferStatusErrorClosed) {
      return;
    }
    status->out_idx_ = 0;
  ...
  *BnInOp2Blob("out")->mut_dptr<T>() =
      conf.id_list(status->in_id_).value(status->out_idx_);
  ++status->out_idx_;
}

ADD_CPU_DEFAULT_KERNEL_CREATOR(OperatorConf::kWaitAndSendIdsConf,
                               WaitAndSendIdsKernel, INT_DATA_TYPE_SEQ);
