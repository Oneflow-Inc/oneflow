...
void AccumulateKernelTestCase(OpKernelTestCase* test_case, const std::string& job_type) {
  ...
  test_case->ForwardCheckBlob<T>("acc", blob_desc, {6, 5, 5, 5, 12, 6, 8, 9}, false);
}

TEST_CPU_AND_GPU_OPKERNEL(AccumulateKernelTestCase, FLOATING_DATA_TYPE_SEQ, (train)(predict));
