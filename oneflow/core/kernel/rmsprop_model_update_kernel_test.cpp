...
void RmspropModelUpdateTestCase(OpKernelTestCase* mdupdt_test_case,
  ...
  mdupdt_test_case->template InitBlob<T>("in_1", blob_desc,
                                         std::vector<T>(shape.elem_cnt(), 1));
  mdupdt_test_case->template InitBlob<T>("mean_square", blob_desc,
                                         std::vector<T>(shape.elem_cnt(), 0));

  mdupdt_test_case->template ForwardCheckBlob<T>(
      "model", blob_desc, std::vector<T>(shape.elem_cnt(), 0));
  mdupdt_test_case->template ForwardCheckBlob<T>(
      "mean_square", blob_desc, std::vector<T>(shape.elem_cnt(), 2), false);
}

TEST_CPU_AND_GPU_OPKERNEL(RmspropModelUpdateTestCase, FLOATING_DATA_TYPE_SEQ,
                          (train), (forward));

DiffKernelImplTestCase* DiffRmspropModelUpdateKernelImpl(
  ...
  mdupdt_test_case->SetBlobNames({"in_0", "packed_blob"},
                                 {"model", "mean_square"}, {}, {});
  ...
  mdupdt_test_case->SetInputBlobDesc("model", Shape({1, 3, 2}),
                                     DataType4CppTypeString(cpp_type));
  mdupdt_test_case->SetInputBlobDesc("mean_square", Shape({1, 3, 2}),
                                     DataType4CppTypeString(cpp_type));
  return mdupdt_test_case;
}

TEST_DIFF_KERNEL_IMPL(DiffRmspropModelUpdateKernelImpl, (train), (forward),
