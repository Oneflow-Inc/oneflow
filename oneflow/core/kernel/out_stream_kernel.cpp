#include "oneflow/core/kernel/out_stream_kernel.h"
  ...
  FOR_RANGE(int64_t, record_id, 0, max_record_num) {
    ...
    if (has_data_id_field) {
      ...
      FOR_RANGE(int32_t, blob_id, 1, op_attribute().input_bns().size()) {
        CHECK_STREQ(data_id, GetBlob(blob_id)->data_id(record_id));
      }
      if (*data_id == '\0') { break; }
      OFRecordEncoderIf::EncodeOneDataId(ctx.device_ctx, data_id, record);
    }
    ...
    std::string msg_bin;
    record.SerializeToString(&msg_bin);
    int64_t msg_size = msg_bin.size();
    char buf[sizeof(int64_t)] = {};
    *(int64_t*)buf = msg_size;

    std::pair<int64_t, std::string>& result = *(std::pair<int64_t, std::string>*)ctx.other;
