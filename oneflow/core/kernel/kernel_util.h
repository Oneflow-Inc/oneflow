...
template <typename T> OF_DEVICE_FUNC T ReduceCoreAdd(const T x, const T y) {
  return x + y;
}

template <typename T> OF_DEVICE_FUNC T ReduceCoreMax(const T x, const T y) {
  return x > y ? x : y;
}
...
struct KernelUtilIf {
  ...
  static void OFBatchedGemm(DeviceCtx *ctx, enum CBLAS_TRANSPOSE trans_a,
                            ...
                            T *c, T **buf) {
    Derived::BatchedGemm(ctx, CblasRowMajor, trans_a, trans_b, batch_size, m, n,
                         k, alpha, a, b, beta, c, buf);
  }
...
template <typename T, typename Derived> struct CpuKernelUtilIf {
  ...
  static void CopyColsRegion(DeviceCtx *ctx, const int64_t row_num,
                             ...
                             const int64_t y_lda);
  static void RowMax(DeviceCtx *ctx, const int64_t row_num,
  ...
  static void RowSum(DeviceCtx *ctx, const int64_t row_num,
                     const int64_t col_num, const T *x, T *y);
  static void RowSum(DeviceCtx *ctx, const int64_t row_num,
                     const int64_t col_num, const T *x, T *y,
                     void *temp_storage, const size_t temp_storage_bytes) {
  ...
  static void AddByScalar(DeviceCtx *ctx, const int64_t n, const T *x,
                          const T y, T *z);
  static void MulByScalarPara(DeviceCtx *ctx, const int64_t n, const T *x,
                              const T y, T *z);
};
...
struct KernelUtil<DeviceType::kCPU, T,
  ...
  static void BatchedGemm(DeviceCtx *ctx, const enum CBLAS_ORDER order,
                          const enum CBLAS_TRANSPOSE trans_a,
                          const enum CBLAS_TRANSPOSE trans_b, int batch_size,
                          int m, int n, int k, const T alpha, const T *a,
  ...
  static void Mul(DeviceCtx *ctx, const int64_t n, const T *x, const T *y,
                  T *z);
  static void MulByScalar(DeviceCtx *ctx, const int64_t n, const T *x,
                          const T *y, T *z);
  ...
  static void Rsqrt(DeviceCtx *ctx, const int64_t n, const T *x, T *y,
                    const float epsilon);
  ...
  static void Addition(DeviceCtx *ctx, const int64_t n, T *out, const T *in_0,
                       const T *in_1, const T *in_2, const T *in_3,
                       const T *in_4, const T *in_5);
  static void Addition(DeviceCtx *ctx, const int64_t n, T *out, const T *in_0,
                       const T *in_1, const T *in_2, const T *in_3,
                       const T *in_4, const T *in_5, const T *in_6);
  static void Addition(DeviceCtx *ctx, const int64_t n, T *out, const T *in_0,
                       const T *in_1, const T *in_2, const T *in_3,
                       const T *in_4, const T *in_5, const T *in_6,
                       const T *in_7);
...
template <typename T, typename Derived> struct GpuKernelUtilIf {
  static void Max(DeviceCtx *ctx, const int64_t n, const T *x, T *max_ptr,
                  T *temp_storage, size_t temp_storage_bytes);
  static void Sum(DeviceCtx *ctx, const int64_t n, const T *x, T *sum_ptr,
                  T *temp_storage, size_t temp_storage_bytes);
...
void CopyFromIterToIter(DeviceCtx *ctx, Iter &src_it, Iter &dst_it) {
  ...
  while (true) {
    if (src_size == 0) {
      std::tie(src_ptr, src_size) = src_it.GetNext();
    }
    if (dst_size == 0) {
      std::tie(dst_ptr, dst_size) = dst_it.GetNext();
    }
    if (src_size == 0) {
...
class FieldIterator {
 public:
  ...
  FieldIterator(std::function<Blob*(const std::string&)> BnInOp2Blob, const
...
class DataIdIterator final : public FieldIterator {
 public:
  ...
  static CopyBlobFieldMthd GetCopyBlobFieldMthd() { return
&Blob::CopyDataIdFrom; }

 private:
  char* GetMutPtr(Blob* blob) override { return blob->mut_data_id(); }

  size_t GetSizeOfField(Blob* blob) const override { return
...
&Blob::CopyColNumFrom; }
 ...
 private:
  char* GetMutPtr(Blob* blob) override { return
reinterpret_cast<char*>(blob->mut_col_num()); }

  size_t GetSizeOfField(Blob* blob) const override { return
blob->ByteSizeOfColNumField(); }
};
...
&Blob::CopyDim1ValidNumFrom; }
 ...
 private:
  ...
  size_t GetSizeOfField(Blob* blob) const override { return
blob->ByteSizeOfDim1ValidNumField(); }
};
...
&Blob::CopyDim2ValidNumFrom; }
 ...
 private:
  ...
  size_t GetSizeOfField(Blob* blob) const override { return
blob->ByteSizeOfDim2ValidNumField(); }
};
*/

template <typename T, typename U>
typename std::enable_if<std::is_same<T, U>::value>::type
CopyElem(const T *in_dptr, U *out_dptr, int64_t elem_num) {
...
#ifdef WITH_CUDA
template <typename T, typename U>
void CopyElemOnGpu(DeviceCtx *ctx, const T *in_dptr, U *out_dptr,
                   int64_t elem_num);
#endif
