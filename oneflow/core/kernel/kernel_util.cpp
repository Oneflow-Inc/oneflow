...
template<typename T>
void SequenceInitializer(int64_t num_rows, int64_t num_cols, T start, T stride, T* out) {
  FOR_RANGE(int64_t, r, 0, num_rows) {
    FOR_RANGE(int64_t, c, 0, num_cols) { *(out + r * num_cols + c) = start + c * stride; }
  }
}

template<typename T, typename SequenceInitializerConfT>
void SequenceInitializer(const SequenceInitializerConfT& initializer_conf, uint32_t random_seed,
                         Blob* blob) {
  ...
  SequenceInitializer<T>(num_rows, num_cols, static_cast<T>(initializer_conf.start()),
                         static_cast<T>(initializer_conf.stride()), blob->mut_dptr<T>());
}

template<typename T>
void SequenceInitializer(const SequenceInitializerConf& initializer_conf, uint32_t random_seed,
                         Blob* blob) {
  SequenceInitializer<T, SequenceInitializerConf>(initializer_conf, random_seed, blob);
}

template<typename T>
void IntSequenceInitializer(const IntSequenceInitializerConf& initializer_conf,
                            uint32_t random_seed, Blob* blob) {
  SequenceInitializer<T, IntSequenceInitializerConf>(initializer_conf, random_seed, blob);
}
...
KU_FLOATING_METHOD InitializeWithConf(DeviceCtx* ctx, const InitializerConf& initializer_conf,
  ...
  } else if (initializer_conf.has_sequence_conf()) {
    SequenceInitializer<T>(initializer_conf.sequence_conf(), random_seed, blob);
  } else {
    UNIMPLEMENTED();
  }
}
...
KU_INTEGRAL_METHOD InitializeWithConf(DeviceCtx* ctx, const InitializerConf& initializer_conf,
  ...
  } else if (initializer_conf.has_int_sequence_conf()) {
    IntSequenceInitializer<T>(initializer_conf.int_sequence_conf(), random_seed, blob);
  } else {
    UNIMPLEMENTED();
  }
}
