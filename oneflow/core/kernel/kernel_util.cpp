...
void RngUniform(const int64_t elem_cnt, const T min, const T max,
  ...
  std::uniform_real_distribution<T> random_distribution(
      min, std::nextafter(max, GetMaxVal<T>()));
...
template <typename T>
void RngTruncatedNormal(const int64_t elem_cnt, const T mean, const T std,
                        uint32_t random_seed, T *dptr) {
  CHECK_GE(elem_cnt, 0);
  CHECK(dptr);
  CHECK_GT(std, 0.0);
...
template <typename T>
void TruncatedNormalInitializer(
    const TruncatedNormalInitializerConf &initializer_conf,
    uint32_t random_seed, Blob *blob) {
  CHECK(blob->shape().elem_cnt());
  RngTruncatedNormal<T>(
      blob->shape().elem_cnt(), static_cast<T>(initializer_conf.mean()),
      static_cast<T>(initializer_conf.std()), random_seed, blob->mut_dptr<T>());
}
...
void MsraInitializer(const MsraInitializerConf &initializer_conf,
  ...
  T std = std::sqrt(
      static_cast<T>(2) /
      GenInitialFan<T>(variance_norm, blob, initializer_conf.data_format()));
  RngNormal<T>(blob->shape().elem_cnt(), GetZeroVal<T>(), static_cast<T>(std),
               random_seed, blob->mut_dptr<T>());
}

template <typename T>
void VarianceScalingInitializer(
    const VarianceScalingInitializerConf &initializer_conf,
    uint32_t random_seed, Blob *blob) {
  CHECK(blob->shape().elem_cnt());
  VarianceNorm variance_norm =
      static_cast<VarianceNorm>(initializer_conf.variance_norm());
  T scale =
      static_cast<T>(initializer_conf.scale()) /
      GenInitialFan<T>(variance_norm, blob, initializer_conf.data_format());
  ...
  if (kTruncatedNormal == distribution) {
    ...
    RngTruncatedNormal<T>(blob->shape().elem_cnt(), GetZeroVal<T>(), stddev,
                          random_seed, blob->mut_dptr<T>());
    ...
    RngUniform<T>(blob->shape().elem_cnt(), -limit, limit, random_seed,
                  blob->mut_dptr<T>());
  }
}

template <typename T>
void RangeInitializer(int64_t outer_size, int64_t idx_dim_size,
                      int64_t inner_size, T start, T stride, T *out) {
  FOR_RANGE(int64_t, i, 0, outer_size) {
    FOR_RANGE(int64_t, j, 0, idx_dim_size) {
      FOR_RANGE(int64_t, k, 0, inner_size) {
        *(out + i * idx_dim_size * inner_size + j * inner_size + k) =
            start + j * stride;
      }
    }
  }
...
void RangeInitializer(const RangeInitializerConfT &initializer_conf,
                      uint32_t random_seed, Blob *blob) {
  ...
  RangeInitializer<T>(
      blob->shape().Count(0, axis), blob->shape().At(axis),
      blob->shape().Count(axis + 1), static_cast<T>(initializer_conf.start()),
      static_cast<T>(initializer_conf.stride()), blob->mut_dptr<T>());
}

template <typename T>
void RangeInitializer(const RangeInitializerConf &initializer_conf,
                      uint32_t random_seed, Blob *blob) {
  RangeInitializer<T, RangeInitializerConf>(initializer_conf, random_seed,
                                            blob);
}

template <typename T>
void IntSequenceInitializer(const IntRangeInitializerConf &initializer_conf,
                            uint32_t random_seed, Blob *blob) {
  RangeInitializer<T, IntRangeInitializerConf>(initializer_conf, random_seed,
                                               blob);
}
...
void MatrixRowReduce(const int64_t row_num, const int64_t col_num, const T *x,
                     T *y) {
  FOR_RANGE(int64_t, i, 0, row_num) {
    y[i] = x[i * col_num];
    FOR_RANGE(int64_t, j, 1, col_num) {
      y[i] = reduce_core_func(y[i], x[i * col_num + j]);
    }
  }
}
  ...
  if (src_mem_case.has_host_mem() && dst_mem_case.has_host_mem()) {
    ...
    if (src_mem_case.has_host_mem() && dst_mem_case.has_device_cuda_mem()) {
      kind = cudaMemcpyKind::cudaMemcpyHostToDevice;
               ...
               dst_mem_case.has_host_mem()) {
      kind = cudaMemcpyKind::cudaMemcpyDeviceToHost;
    } else if (src_mem_case.has_device_cuda_mem() &&
               dst_mem_case.has_device_cuda_mem()) {
...
#define KU_IF_METHOD                                                           \
  template <typename T, typename Derived> void CpuKernelUtilIf<T, Derived>::
...
KU_IF_METHOD CopyColsRegion(DeviceCtx *ctx, const int64_t row_num,
                            const int64_t col_num, const T *x,
                            const int64_t x_col_offset, const int64_t x_lda,
                            T *y, const int64_t y_col_offset,
                            const int64_t y_lda) {
...
KU_IF_METHOD RowMax(DeviceCtx *ctx, const int64_t row_num,
                    const int64_t col_num, const T *x, T *y) {
  MatrixRowReduce<T, ReduceCoreMax>(row_num, col_num, x, y);
}
KU_IF_METHOD RowSum(DeviceCtx *ctx, const int64_t row_num,
                    const int64_t col_num, const T *x, T *y) {
...
KU_IF_METHOD AddByScalar(DeviceCtx *ctx, const int64_t n, const T *x, const T y,
                         T *z) {
  for (int64_t i = 0; i < n; ++i) {
    z[i] = x[i] + y;
  }
}
KU_IF_METHOD MulByScalarPara(DeviceCtx *ctx, const int64_t n, const T *x,
                             const T y, T *z) {
  for (int64_t i = 0; i < n; ++i) {
    z[i] = x[i] * y;
  }
...
KU_FLOATING_METHOD Gemv(DeviceCtx *ctx, const enum CBLAS_TRANSPOSE trans, int m,
  ...
  cblas_gemv<T>(CBLAS_ORDER::CblasColMajor, trans, n, m, alpha, a, lda, x, incx,
                beta, y, incy);
}
...
KU_FLOATING_METHOD BatchedGemm(DeviceCtx *ctx, const enum CBLAS_ORDER order,
  ...
  FOR_RANGE(int32_t, i, 0, batch_size) {
    KernelUtil<DeviceType::kCPU, T>::OFGemm(
        ctx, trans_a, trans_b, m, n, k, alpha, a + i * a_stride,
...
KU_FLOATING_METHOD MulByScalar(DeviceCtx *ctx, const int64_t n, const T *x,
                               const T *y, T *z) {
  for (int64_t i = 0; i < n; ++i) {
    z[i] = x[i] * y[0];
  }
...
KU_FLOATING_METHOD Reciprocal(DeviceCtx *ctx, const int n, const T *x, T *y) {
  for (int64_t i = 0; i < n; ++i) {
    y[i] = static_cast<T>(1.0) / x[i];
  }
}
KU_FLOATING_METHOD Rsqrt(DeviceCtx *ctx, const int64_t n, T *x,
                         const float epsilon) {
  for (int64_t i = 0; i < n; ++i) {
    x[i] = 1.0 / std::sqrt(x[i] + epsilon);
  }
}
KU_FLOATING_METHOD Rsqrt(DeviceCtx *ctx, const int64_t n, const T *x, T *y,
                         const float epsilon) {
  for (int64_t i = 0; i < n; ++i) {
    y[i] = 1.0 / std::sqrt(x[i] + epsilon);
  }
}
...
KU_FLOATING_METHOD Addition(DeviceCtx *ctx, const int64_t n, T *out,
                            const T *in_0) {
  for (int64_t i = 0; i != n; ++i) {
    out[i] = in_0[i];
  }
...
KU_FLOATING_METHOD Addition(DeviceCtx *ctx, const int64_t n, T *out,
                            const T *in_0, const T *in_1) {
  for (int64_t i = 0; i != n; ++i) {
    out[i] = in_0[i] + in_1[i];
  }
}
KU_FLOATING_METHOD Addition(DeviceCtx *ctx, const int64_t n, T *out,
                            const T *in_0, const T *in_1, const T *in_2) {
  for (int64_t i = 0; i != n; ++i) {
...
KU_FLOATING_METHOD Addition(DeviceCtx *ctx, const int64_t n, T *out,
  ...
  for (int64_t i = 0; i != n; ++i) {
    out[i] = in_0[i] + in_1[i] + in_2[i] + in_3[i] + in_4[i];
  }
}
KU_FLOATING_METHOD Addition(DeviceCtx *ctx, const int64_t n, T *out,
                            const T *in_0, const T *in_1, const T *in_2,
...
KU_FLOATING_METHOD Addition(DeviceCtx *ctx, const int64_t n, T *out,
  ...
  for (int64_t i = 0; i != n; ++i) {
    out[i] =
        in_0[i] + in_1[i] + in_2[i] + in_3[i] + in_4[i] + in_5[i] + in_6[i];
  }
}
KU_FLOATING_METHOD Addition(DeviceCtx *ctx, const int64_t n, T *out,
                            const T *in_0, const T *in_1, const T *in_2,
...
KU_FLOATING_METHOD InitializeWithConf(DeviceCtx *ctx,
  ...
  if (initializer_conf.has_constant_conf()) {
    ...
    TruncatedNormalInitializer<T>(initializer_conf.truncated_normal_conf(),
                                  random_seed, blob);
    ...
    VarianceScalingInitializer<T>(initializer_conf.variance_scaling_conf(),
                                  random_seed, blob);
  } else {
    UNIMPLEMENTED();
  }
...
KU_INTEGRAL_METHOD InitializeWithConf(DeviceCtx *ctx,
  ...
  if (initializer_conf.has_constant_int_conf()) {
    ...
    IntSequenceInitializer<T>(initializer_conf.int_range_conf(), random_seed,
                              blob);
  } else {
    UNIMPLEMENTED();
  }
...
KU_INTEGRAL_METHOD Mul(DeviceCtx *ctx, const int64_t n, const T *x, const T *y,
                       T *z) {
  for (int64_t i = 0; i < n; ++i) {
    z[i] = x[i] * y[i];
  }
