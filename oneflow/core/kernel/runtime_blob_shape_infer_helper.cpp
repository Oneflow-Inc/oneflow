...
RuntimeBlobShapeInferHelper::RuntimeBlobShapeInferHelper(
    const OperatorConf &op_conf, const KernelConf &kernel_conf,
  ...
  op_infer_cache_key_.dtype_signature_sym =
      SymbolOf(kernel_conf.dtype_signature());
}

void RuntimeBlobShapeInferHelper::UpdateInputBlobDescs7OpInferCacheKey(
    std::function<Blob *(const std::string &)> BnInOp2Blob) {
  auto ResetBlobDescAndGetShapeSym =
      [&](const std::string &ibn) -> Symbol<Shape> {
  ...
  FOR_RANGE(int, i, 0, input_bns.size()) {
    op_infer_cache_key_.ibn_idx2shape_sym.at(i) =
        ResetBlobDescAndGetShapeSym(input_bns.Get(i));
  }
}

BlobDesc *
RuntimeBlobShapeInferHelper::BlobDesc4BnInOp(const std::string &bn_in_op,
  ...
  blob_desc =
      new BlobDesc(rt_blob_desc.body(), rt_blob_desc.is_tensor_list(),
...
void RuntimeBlobShapeInferHelper::InferShape(
  ...
  auto Infer = [&](
      const OpInferCacheKey &key) -> std::shared_ptr<const OpInferCacheValue> {
    auto CachedBlobDesc4BnInOp =
        WithResultCached([&](const std::string &bn_in_op) -> BlobDesc * {
    ...
    CHECK_JUST(op_->InferOutBlobDescsIf(CachedBlobDesc4BnInOp, &parallel_ctx_,
                                        &sbp_signature_, [](OpContext *) {}));
    ...
    FOR_RANGE(int, i, 0, op_->output_bns().size()) {
      ...
      CHECK_EQ(blob->blob_desc().is_body_disabled(),
               blob_desc->is_body_disabled());
    }
    return std::shared_ptr<const OpInferCacheValue>(ret);
  };
  size_t cache_size =
      Global<ResourceDesc, ForSession>::Get()->thread_local_cache_max_size();
  const auto &shape_infer_ret =
      ThreadLocalCachedCall(cache_size, Infer, op_infer_cache_key_);
