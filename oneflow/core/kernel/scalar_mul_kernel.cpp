...
void ScalarMulKernel<device_type, T>::ForwardDataContent(
  ...
  KernelUtil<device_type, T>::Scal(ctx.device_ctx, out_blob->shape().elem_cnt(), scalar_operand,
...
void ScalarMulKernel<device_type, T>::BackwardDataContent(
  ...
  KernelUtil<device_type, T>::Scal(ctx.device_ctx, in_diff_blob->shape().elem_cnt(), scalar_operand,
