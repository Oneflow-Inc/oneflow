...
const LazyAdamModelUpdateConf &
GetLazyAdamModelUpdateConf(const OperatorConf &op_conf) {
...
const PbMessage &
LazyAdamMdUpdateKernel<device_type, T>::GetCustomizedOpConf() const {
...
void LazyAdamMdUpdateKernel<device_type, T>::UpdateModel(
  ...
  LazyAdamMdUpdateKernelUtil<device_type, T>::UpdateModel(
      ...
      static_cast<T>(lazy_adam_conf.beta1()),
      static_cast<T>(lazy_adam_conf.beta2()),
...
public:
  static void UpdateModel(DeviceCtx *ctx, int64_t n, const float *learning_rate,
    ...
    const float local_learning_rate =
        *learning_rate * std::sqrt(1 - (*beta2_t)) / (1 - (*beta1_t));
    for (int64_t i = 0; i < n; ++i) {
      T model_diff_val = model_diff[i];
      if (std::abs(model_diff_val) < 1e-12) {
        continue;
      }
      m[i] = beta1 * m[i] + (1 - beta1) * model_diff_val;
      v[i] = beta2 * v[i] + (1 - beta2) * model_diff_val * model_diff_val;
      model[i] =
          model[i] - local_learning_rate * m[i] / (std::sqrt(v[i]) + epsilon);
    }
    *beta1_t *= beta1;
    *beta2_t *= beta2;
  }
...
ADD_DEFAULT_KERNEL_CREATOR(OperatorConf::kLazyAdamModelUpdateConf,
                           LazyAdamMdUpdateKernel, FLOATING_DATA_TYPE_SEQ);
