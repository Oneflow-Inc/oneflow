...
void EltwiseKernel<device_type, T>::ForwardDataContent(
  ...
  Memset<device_type>(ctx.device_ctx, out_blob->mut_dptr(), 0, out_blob->ByteSizeOfDataContentField());
  Blob* tmp_blob = BnInOp2Blob("tmp");
  Memset<device_type>(ctx.device_ctx, tmp_blob->mut_dptr(), 0, out_blob->ByteSizeOfDataContentField());
