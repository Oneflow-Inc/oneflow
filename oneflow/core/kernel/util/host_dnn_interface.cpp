...
template <typename T>
static void ReluBackwardImpl(DeviceCtx *ctx, const int64_t n, const T *x,
...
template <typename T>
static void SigmoidBackwardImpl(DeviceCtx *ctx, const int64_t n, const T *x,
                                const T *y, const T *dy, T *dx) {
  for (int64_t i = 0; i != n; ++i) {
    dx[i] = y[i] * (1 - y[i]) * dy[i];
  }
...
template <typename T>
static void TanHBackwardImpl(DeviceCtx *ctx, const int64_t n, const T *x,
                             const T *y, const T *dy, T *dx) {
  for (int64_t i = 0; i != n; ++i) {
    dx[i] = (1 - y[i] * y[i]) * dy[i];
  }
...
void DnnIf<DeviceType::kCPU>::Relu(DeviceCtx *ctx, const int64_t n,
...
void DnnIf<DeviceType::kCPU>::ReluBackward(DeviceCtx *ctx, const int64_t n,
...
void DnnIf<DeviceType::kCPU>::Sigmoid(DeviceCtx *ctx, int64_t n, const float *x,
                                      float *y) {
  SigmoidImpl<float>(ctx, n, x, y);
}

void DnnIf<DeviceType::kCPU>::Sigmoid(DeviceCtx *ctx, int64_t n,
                                      const double *x, double *y) {
  SigmoidImpl<double>(ctx, n, x, y);
}

void DnnIf<DeviceType::kCPU>::SigmoidBackward(DeviceCtx *ctx, const int64_t n,
                                              const float *x, const float *y,
                                              const float *dy, float *dx) {
...
void DnnIf<DeviceType::kCPU>::TanH(DeviceCtx *ctx, const int64_t n,
                                   const float *x, float *y) {
...
void DnnIf<DeviceType::kCPU>::TanHBackward(DeviceCtx *ctx, const int64_t n,
                                           const float *x, const float *y,
                                           const float *dy, float *dx) {
