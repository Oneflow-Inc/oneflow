...
template <int32_t NDIMS, typename T>
void TransposeImpl(DeviceCtx *ctx, const ShapeView &x_shape,
...
void ArithemeticIf<DeviceType::kGPU>::Transpose(
  ...
  TRANSPOSE_CHECK;
  TransposeUtil<float>::SwitchTransposeImpl(
      SwitchCase(num_axis), ctx, x_shape, y_shape, permutation, elem_cnt, x, y);
}
...
void ArithemeticIf<DeviceType::kGPU>::Transpose(
  ...
  TRANSPOSE_CHECK;
  TransposeUtil<double>::SwitchTransposeImpl(
      SwitchCase(num_axis), ctx, x_shape, y_shape, permutation, elem_cnt, x, y);
}
...
void ArithemeticIf<DeviceType::kGPU>::Transpose(
  ...
  TRANSPOSE_CHECK;
  TransposeUtil<half>::SwitchTransposeImpl(
      SwitchCase(num_axis), ctx, x_shape, y_shape, permutation, elem_cnt,
      reinterpret_cast<const half *>(x), reinterpret_cast<half *>(y));
}
...
void ArithemeticIf<DeviceType::kGPU>::Transpose(
  ...
  TRANSPOSE_CHECK;
  TransposeUtil<int8_t>::SwitchTransposeImpl(
      SwitchCase(num_axis), ctx, x_shape, y_shape, permutation, elem_cnt, x, y);
}
...
void ArithemeticIf<DeviceType::kGPU>::Transpose(
  ...
  TRANSPOSE_CHECK;
  TransposeUtil<int32_t>::SwitchTransposeImpl(
      SwitchCase(num_axis), ctx, x_shape, y_shape, permutation, elem_cnt, x, y);
}
...
void ArithemeticIf<DeviceType::kGPU>::Transpose(
  ...
  TRANSPOSE_CHECK;
  TransposeUtil<int64_t>::SwitchTransposeImpl(
      SwitchCase(num_axis), ctx, x_shape, y_shape, permutation, elem_cnt, x, y);
}
...
void ArithemeticIf<DeviceType::kGPU>::InitializeWithConstConf(
  ...
  WithHostBlobAndStreamSynchronizeEnv(ctx, blob, [&](Blob *host_blob) {
    ArithemeticIf<DeviceType::kCPU>::InitializeWithConstConf(
        nullptr, initializer_conf, host_blob);
  });
}
...
template <>
__global__ void MulByScalarGpu<half>(const int64_t n, const half *x,
                                     const half y, half *z) {
...
template <typename T>
__global__ void MulByScalarPtrGpu(const int64_t n, const T *x, const T *y,
                                  T *z) {
  const T y_value = y[0];
...
template <typename T>
__global__ void AddByScalarPtrGpu(const int64_t n, const T *x, const T *y,
                                  T *z) {
  const T y_value = y[0];
  CUDA_1D_KERNEL_LOOP(i, n) { z[i] = x[i] + y_value; }
}
...
template <typename T>
__global__ void SubByScalarPtrGpu(const int64_t n, const T *x, const T *y,
                                  T *z) {
  const T y_value = y[0];
  CUDA_1D_KERNEL_LOOP(i, n) { z[i] = x[i] - y_value; }
}
...
template <typename T>
__global__ void DivByScalarPtrGpu(const int64_t n, const T *x, const T *y,
                                  T *z) {
  const T y_value = y[0];
  CUDA_1D_KERNEL_LOOP(i, n) { z[i] = x[i] / y_value; }
}
...
#define MUL_BY_SCALAR(T)                                                       \
  void ArithemeticIf<DeviceType::kGPU>::MulByScalar(                           \
...
void ArithemeticIf<DeviceType::kGPU>::MulByScalar(DeviceCtx *ctx,
                                                  const int64_t n,
                                                  const float16 *x,
                                                  const float16 y, float16 *z) {
  MulByScalarGpu<half><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock, 0,
                         ctx->cuda_stream()>>>(
      n, reinterpret_cast<const half *>(x), float16_2half(y),
      reinterpret_cast<half *>(z));
}

#define MUL_BY_SCALAR_PTR(T)                                                   \
  void ArithemeticIf<DeviceType::kGPU>::MulByScalarPtr(                        \
      DeviceCtx *ctx, const int64_t n, const T *x, const T *y, T *z) {         \
    MulByScalarPtrGpu<T><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock,   \
                           0, ctx->cuda_stream()>>>(n, x, y, z);               \
  }
...
void ArithemeticIf<DeviceType::kGPU>::MulByScalarPtr(DeviceCtx *ctx,
                                                     const int64_t n,
                                                     const float16 *x,
                                                     const float16 *y,
                                                     float16 *z) {
  MulByScalarPtrGpu<half><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock, 0,
                            ctx->cuda_stream()>>>(
...
#define ADD_BY_SCALAR_PTR(T)                                                   \
  void ArithemeticIf<DeviceType::kGPU>::AddByScalarPtr(                        \
      DeviceCtx *ctx, const int64_t n, const T *x, const T *y, T *z) {         \
    AddByScalarPtrGpu<T><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock,   \
                           0, ctx->cuda_stream()>>>(n, x, y, z);               \
  }
...
void ArithemeticIf<DeviceType::kGPU>::AddByScalarPtr(DeviceCtx *ctx,
                                                     const int64_t n,
                                                     const float16 *x,
                                                     const float16 *y,
                                                     float16 *z) {
  AddByScalarPtrGpu<half><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock, 0,
                            ctx->cuda_stream()>>>(
      n, reinterpret_cast<const half *>(x), reinterpret_cast<const half *>(y),
      reinterpret_cast<half *>(z));
}
...
#define SUB_BY_SCALAR_PTR(T)                                                   \
  void ArithemeticIf<DeviceType::kGPU>::SubByScalarPtr(                        \
      DeviceCtx *ctx, const int64_t n, const T *x, const T *y, T *z) {         \
    SubByScalarPtrGpu<T><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock,   \
                           0, ctx->cuda_stream()>>>(n, x, y, z);               \
  }
...
void ArithemeticIf<DeviceType::kGPU>::SubByScalarPtr(DeviceCtx *ctx,
                                                     const int64_t n,
                                                     const float16 *x,
                                                     const float16 *y,
                                                     float16 *z) {
  SubByScalarPtrGpu<half><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock, 0,
                            ctx->cuda_stream()>>>(
      n, reinterpret_cast<const half *>(x), reinterpret_cast<const half *>(y),
      reinterpret_cast<half *>(z));
}
...
#define DIV_BY_SCALAR_PTR(T)                                                   \
  void ArithemeticIf<DeviceType::kGPU>::DivByScalarPtr(                        \
      DeviceCtx *ctx, const int64_t n, const T *x, const T *y, T *z) {         \
    DivByScalarPtrGpu<T><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock,   \
                           0, ctx->cuda_stream()>>>(n, x, y, z);               \
  }
...
void ArithemeticIf<DeviceType::kGPU>::DivByScalarPtr(DeviceCtx *ctx,
                                                     const int64_t n,
                                                     const float16 *x,
                                                     const float16 *y,
                                                     float16 *z) {
  DivByScalarPtrGpu<half><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock, 0,
                            ctx->cuda_stream()>>>(
      n, reinterpret_cast<const half *>(x), reinterpret_cast<const half *>(y),
      reinterpret_cast<half *>(z));
}
...
#define FILL(T)                                                                \
  void ArithemeticIf<DeviceType::kGPU>::Fill(DeviceCtx *ctx, const int64_t n,  \
                                             const T value, T *y) {            \
    FillGpu<T><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock, 0,          \
                 ctx->cuda_stream()>>>(n, value, y);                           \
  }
...
void ArithemeticIf<DeviceType::kGPU>::Fill(DeviceCtx *ctx, const int64_t n,
                                           const float16 value, float16 *y) {
  FillGpu<half><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock, 0,
                  ctx->cuda_stream()>>>(n, float16_2half(value),
...
void ArithemeticIf<DeviceType::kGPU>::CopyColsRegion(
  ...
  CopyColsRegionGpu<half><<<BlocksNum4ThreadsNum(row_num * col_num),
                            kCudaThreadsNumPerBlock, 0, ctx->cuda_stream()>>>(
