...
void IndexedSlicesMomentumMdUpdateKernel<device_type, T, K>::ForwardDataContent(
  ...
  const int64_t lower_bound =
      this->kernel_conf().indexed_slices_momentum_model_update_conf().lower_bound();
  const int64_t upper_bound =
      this->kernel_conf().indexed_slices_momentum_model_update_conf().upper_bound();
...
OF_PP_SEQ_PRODUCT_FOR_EACH_TUPLE(MAKE_INDEXED_SLICES_MOMENTUM_MODEL_UPDATE_KERNEL_ENTRY,
