...
class IndexedSlicesMomentumMdUpdateKernel final : public KernelIf<device_type> {
public:
  OF_DISALLOW_COPY_AND_MOVE(IndexedSlicesMomentumMdUpdateKernel);
  IndexedSlicesMomentumMdUpdateKernel() = default;
  ~IndexedSlicesMomentumMdUpdateKernel() override = default;

private:
  using ReduceSumUtilT =
      IndexedSlicesReduceSumKernelUtil<device_type, K, T, int32_t>;
  using MdUpdateUtilT =
      IndexedSlicesMomentumMdUpdateKernelUtil<device_type, T, K, int32_t>;
  void
...
template <DeviceType device_type, typename T, typename K>
void IndexedSlicesMomentumMdUpdateKernel<device_type, T, K>::ForwardDataContent(
    const KernelCtx &ctx,
    std::function<Blob *(const std::string &)> BnInOp2Blob) const {
  const IndexedSlicesMomentumModelUpdateOpConf &conf =
      this->op_conf().indexed_slices_momentum_model_update_conf();
  const T beta = static_cast<T>(conf.beta());
  ...
  const int64_t lower_bound = this->kernel_conf()
                                  .indexed_slices_momentum_model_update_conf()
                                  .lower_bound();
  const int64_t upper_bound = this->kernel_conf()
                                  .indexed_slices_momentum_model_update_conf()
                                  .upper_bound();
  ReduceSumUtilT::ReduceSum(
      ctx.device_ctx, num_indices, feature_size, diff_indices->dptr<K>(),
  ...
  MdUpdateUtilT::Update(
      ctx.device_ctx, beta, num_indices, feature_size, lower_bound, upper_bound,
...
#define MAKE_INDEXED_SLICES_MOMENTUM_MODEL_UPDATE_KERNEL_ENTRY(                \
    device_type_v, data_type_pair, indices_type_pair)                          \
  NEW_REGISTER_KERNEL(OperatorConf::kIndexedSlicesMomentumModelUpdateConf,     \
                      IndexedSlicesMomentumMdUpdateKernel<                     \
                          device_type_v, OF_PP_PAIR_FIRST(data_type_pair),     \
      ...
      .SetIsMatchedPred([](const KernelConf &kernel_conf) -> bool {            \
        return ((kernel_conf.op_attribute().op_conf().device_tag() ==          \
                ...
                (OF_PP_PAIR_SECOND(indices_type_pair) ==                       \
                 kernel_conf.indexed_slices_momentum_model_update_conf()       \
                     .indices_data_type()));                                   \
      });

OF_PP_SEQ_PRODUCT_FOR_EACH_TUPLE(
    MAKE_INDEXED_SLICES_MOMENTUM_MODEL_UPDATE_KERNEL_ENTRY, DEVICE_TYPE_SEQ,
    FLOATING_DATA_TYPE_SEQ, INDEX_DATA_TYPE_SEQ)
#undef MAKE_INDEXED_SLICES_MOMENTUM_MODEL_UPDATE_KERNEL_ENTRY
