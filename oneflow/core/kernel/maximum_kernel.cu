...
struct MaximumKernelUtil<DeviceType::kGPU, T> {
  static void CWiseMaxWithMask(DeviceCtx* ctx, const int64_t n, T* x,
    ...
    CWiseMaxWithMaskGpu<T><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock,
                             0, ctx->cuda_stream()>>>(n, x, y, y_idx, mask);
  }

  static void CWiseSetWithMask(DeviceCtx* ctx, const int64_t n, T* x,
                               const T* y, const int x_idx,
                               const int32_t* mask) {
    CWiseSetWithMaskGpu<T><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock,
                             0, ctx->cuda_stream()>>>(n, x, y, x_idx, mask);
  }
};
