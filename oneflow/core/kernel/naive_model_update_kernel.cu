...
template<typename T>
__global__ void UpdateModelGpu(const int64_t n, T learning_rate, T l1, T l2, const T* model_diff,
                               T* model) {
...
class NaiveMdUpdateKernelUtil<DeviceType::kGPU, T> final {
 public:
  static void UpdateModel(DeviceCtx* ctx, const int64_t n, T learning_rate, T l1, T l2,
