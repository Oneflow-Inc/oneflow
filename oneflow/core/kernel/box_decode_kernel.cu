...
template<typename T>
__global__ void DecodeGpu(const int32_t num_boxes, const T* ref_boxes_ptr, const T* boxes_delta_ptr,
...
struct BoxDecodeUtil<DeviceType::kGPU, T> {
  static void Decode(DeviceCtx* ctx, const int32_t num_boxes, const T* ref_boxes_ptr,
    ...
    DecodeGpu<<<BlocksNum4ThreadsNum(num_boxes), kCudaThreadsNumPerBlock, 0, ctx->cuda_stream()>>>(
