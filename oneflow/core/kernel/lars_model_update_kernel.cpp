...
void LARSMdUpdateKernel<device_type, T>::Forward(
  ...
  LARSMdUpdateKernelUtil<device_type, T>::UpdateModel(
      ...
      momentum_blob->mut_dptr<T>(), model_diffs_blob->dptr<T>());
}
 ...
 public:
  static void UpdateModel(const KernelCtx& ctx, const int64_t n,
    ...
    ctx.device_ctx->cpu_stream()->SendWork([=]() {
      T model_square_sum = 0;
      T model_diff_square_sum = 0;
      for (int64_t i = 0; i != n; ++i) {
        model_square_sum += model[i] * model[i];
        model_diff_square_sum += model_diff[i] * model_diff[i];
      }
      const T local_lr =
          learning_rate * lars_coefficient * model_square_sum
          / (model_diff_square_sum + weight_decay * model_square_sum);
      for (int64_t i = 0; i != n; ++i) {
