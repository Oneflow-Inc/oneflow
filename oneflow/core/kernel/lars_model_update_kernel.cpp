...
const PbMessage &
LARSMdUpdateKernel<device_type, T>::GetCustomizedOpConf() const {
...
void LARSMdUpdateKernel<device_type, T>::UpdateModel(
  ...
  const LARSModelUpdateConf &lars_conf =
      GetLARSModelUpdateConf(this->op_conf());
  Memset<device_type>(ctx, data_tmp_blob->mut_dptr<T>(), 0,
                      data_tmp_blob->ByteSizeOfBlobBody());
  LARSMdUpdateKernelUtil<device_type, T>::UpdateModel(
      ctx, model_blob->shape().elem_cnt(), learning_rate, weight_decay,
      static_cast<T>(lars_conf.momentum_beta()),
      static_cast<T>(lars_conf.epsilon()),
      static_cast<T>(lars_conf.lars_coefficient()), train_step,
      model_diff_blob->dptr<T>(), model_blob->mut_dptr<T>(),
      momentum_blob->mut_dptr<T>(), data_tmp_blob->mut_dptr<T>());
}

template <typename T> class LARSMdUpdateKernelUtil<DeviceType::kCPU, T> final {
public:
  static void UpdateModel(DeviceCtx *ctx, int64_t n, const float *learning_rate,
                          T weight_decay, T momentum_beta, T epsilon,
    ...
    if (*train_step == 0) {
      ...
      local_learning_rate =
          *learning_rate * lars_coefficient * model_norm /
          (epsilon + model_diff_norm + weight_decay * model_norm);
    }
    FOR_RANGE(int64_t, i, 0, n) {
      T reg_diff = model_diff[i] + weight_decay * model[i];
      momentum[i] =
          momentum_beta * momentum[i] - local_learning_rate * reg_diff;
...
ADD_DEFAULT_KERNEL_CREATOR(OperatorConf::kLarsModelUpdateConf,
                           LARSMdUpdateKernel, FLOATING_DATA_TYPE_SEQ);
