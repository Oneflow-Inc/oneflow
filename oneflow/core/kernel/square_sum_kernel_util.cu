...
template <typename T> struct SquareSumKernelUtil<DeviceType::kGPU, T> {
  static void SquareSum(DeviceCtx *ctx, int64_t n, const T *x, T *y) {
    ...
    if (num_blocks == 0) {
      ...
      SquareSumGpu<T,
                   true><<<1, kCudaThreadsNumPerBlock, 0, ctx->cuda_stream()>>>(
          n, x, y);
    } else {
      Memset<DeviceType::kGPU>(ctx, y, 0, sizeof(T));
      SquareSumGpu<T, false><<<num_blocks, kCudaThreadsNumPerBlock, 0,
...
OF_PP_FOR_EACH_TUPLE(INSTANTIATE_SQUARE_SUM_KERNEL_UTIL_GPU,
