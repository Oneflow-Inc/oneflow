...
void MomentumModelUpdateTestCase(OpKernelTestCase* mdupdt_test_case,
  ...
  normal_mdupdt_conf->set_in_num(2);
  NormalModelUpdateOpUserConf* user_conf =
      normal_mdupdt_conf->mutable_user_conf();
  user_conf->set_learning_rate(0.5f);
  MomentumModelUpdateConf* momentum_conf = user_conf->mutable_momentum_conf();
  momentum_conf->set_beta(0.5f);

  DataType data_type = GetDataType<T>::value;
  mdupdt_test_case->InitJobConf([data_type](JobConf* job_conf) {
  ...
  mdupdt_test_case->template InitBlob<T>("in_1", blob_desc,
                                         std::vector<T>(shape.elem_cnt(), 3));
  mdupdt_test_case->template InitBlob<T>("momentum", blob_desc,
                                         std::vector<T>(shape.elem_cnt(), 4));
  ...
  mdupdt_test_case->template ForwardCheckBlob<T>(
      "model", blob_desc, std::vector<T>(shape.elem_cnt(), 3));
  mdupdt_test_case->template ForwardCheckBlob<T>(
      "momentum", blob_desc, std::vector<T>(shape.elem_cnt(), 1), false);
}

TEST_CPU_AND_GPU_OPKERNEL(MomentumModelUpdateTestCase, FLOATING_DATA_TYPE_SEQ,
                          (train), (forward));

DiffKernelImplTestCase* DiffMomentumModelUpdateKernelImpl(
  ...
  MomentumModelUpdateConf* momentum_conf = user_conf->mutable_momentum_conf();
  momentum_conf->set_beta(0.8f);

  mdupdt_test_case->SetBlobNames({"in_0", "packed_blob"}, {"model", "momentum"},
                                 {}, {});
  ...
  mdupdt_test_case->SetInputBlobDesc("model", Shape({1, 3, 2}),
                                     DataType4CppTypeString(cpp_type));
  mdupdt_test_case->SetInputBlobDesc("momentum", Shape({1, 3, 2}),
                                     DataType4CppTypeString(cpp_type));
  return mdupdt_test_case;
}

TEST_DIFF_KERNEL_IMPL(DiffMomentumModelUpdateKernelImpl, (train), (forward),
