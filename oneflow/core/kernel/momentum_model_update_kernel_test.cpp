...
Kernel* BuildMomentumMdUpdateKernel(float learning_rate, float beta) {
  ...
  MomentumModelUpdateOpConf* momentum_md_update_conf = op_conf.mutable_momentum_mdupdt_conf();
...
std::function<Blob*(const std::string&)> BuildBnInOp2Blob() {
  ...
  BlobDesc* blob_desc = new BlobDesc(Shape({1, 3, 2}), GetDataType<T>::value, false);
...
void TestMomentumMdUpdateKernel() {
  ...
  auto momentum_md_update_kernel = BuildMomentumMdUpdateKernel<device_type, T>(learning_rate, beta);
