...
private:
  void
  ForwardDataContent(const KernelCtx &ctx,
...
void TensorListToTensorBufferKernel::ForwardDataContent(
  ...
  while (!in_blob->IsEndTensor(in_tensor_view)) {
    ...
    FOR_RANGE(int, i, 0, dim_vec.size()) {
      dim_vec.at(i) = in_tensor_view.shape().At(i + 1);
    }
...
void TensorListToTensorBufferKernel::ForwardHeader(
  ...
  BnInOp2Blob("out")->mut_shape_view()->Set(
      0, BnInOp2Blob("in")->total_num_of_tensors());
}

NEW_REGISTER_KERNEL(OperatorConf::kTensorListToTensorBufferConf,
                    TensorListToTensorBufferKernel)
