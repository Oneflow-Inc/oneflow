...
void ReentrantLockStatus::Init(const KernelConf &kernel_conf) {
  ...
  lock_id2queued_request_act_id_.resize(
      conf.lock_id2intersecting_lock_ids_size());
...
bool ReentrantLockStatus::TryAcquireLock(int64_t lock_id) {
  ...
  for (int64_t intersect_lock_id : lock_id2intersecting_lock_ids_.at(lock_id)) {
    if (lock_id2acquired_num_.at(intersect_lock_id) > 0 ||
        (lock_id2queued_request_act_id_.at(intersect_lock_id).empty() ==
             false &&
         lock_id2queued_request_act_id_.at(intersect_lock_id).front() <
             act_id)) {
      blocked = true;
      break;
    }
...
void ReentrantLockStatus::RequestLock(int64_t lock_id,
                                      std::queue<int64_t> *unlocked_ids) {
  lock_id2queued_request_act_id_.at(lock_id).push(cur_act_id());
  ++total_queued_request_lock_num_;
  if (TryAcquireLock(lock_id)) {
    unlocked_ids->push(lock_id);
  }
}

void ReentrantLockStatus::ReleaseLock(int64_t lock_id,
                                      std::queue<int64_t> *unlocked_ids) {
  ...
  do {
    ...
    auto ReleaseRelatedLockId = [&](int64_t related_lock_id) {
      if (lock_id2queued_request_act_id_.at(related_lock_id).empty()) {
        return;
      }
    ...
    for (int64_t id : lock_id2intersecting_lock_ids_.at(lock_id)) {
      ReleaseRelatedLockId(id);
    }
...
void ReentrantLockKernel<T>::ForwardDataContent(
  ...
  if (status->cur_ibn() == "start") {
    ...
    status->ReleaseLock(*BnInOp2Blob("end")->dptr<T>(),
                        status->mut_cur_unlocked_ids());
  } else {
...
ADD_CPU_DEFAULT_KERNEL_CREATOR(OperatorConf::kReentrantLockConf,
                               ReentrantLockKernel, INT_DATA_TYPE_SEQ)
