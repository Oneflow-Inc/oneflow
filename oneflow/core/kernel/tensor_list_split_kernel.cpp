...
private:
  void ForwardDataContent(
    ...
    CHECK_EQ(in_blob->total_num_of_tensors(),
             this->op_attribute().output_bns_size());
    ...
    for (const auto &obn : this->op_attribute().output_bns()) {
      ...
      Memcpy<device_type>(ctx.device_ctx, out_tensor_view->mut_dptr(),
                          in_tensor.dptr(), in_tensor.ByteSize());
...
#define REGISTER_TENSOR_LIST_SPLIT_KERNEL(device, dtype)                       \
  REGISTER_KERNEL_WITH_DEVICE_AND_DTYPE(OperatorConf::kTensorListSplitConf,    \
                                        device, dtype,                         \
                                        TensorListSplitKernel<device, dtype>)

#define REGISTER_TENSOR_LIST_SPLIT_KERNEL_WITH_DEVICE_AND_DTYPE_PAIR(          \
    device, dtype_pair)                                                        \
  REGISTER_TENSOR_LIST_SPLIT_KERNEL(device, OF_PP_PAIR_FIRST(dtype_pair))

OF_PP_SEQ_PRODUCT_FOR_EACH_TUPLE(
    REGISTER_TENSOR_LIST_SPLIT_KERNEL_WITH_DEVICE_AND_DTYPE_PAIR,
