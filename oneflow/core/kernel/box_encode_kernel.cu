...
template<typename T>
__global__ void GpuForward(const int32_t num_boxes, const T* ref_boxes_ptr, const T* boxes_ptr,
  ...
  CUDA_1D_KERNEL_LOOP(i, num_boxes) {
    ...
    boxes_delta_ptr[i * 4 + 1] = weight_y * (ref_box_ctr_y - box_ctr_y) / box_height;
...
struct BoxEncodeUtil<DeviceType::kGPU, T> {
  static void Forward(DeviceCtx* ctx, const int32_t num_boxes, const T* ref_boxes_ptr,
    ...
    GpuForward<<<std::min(num_boxes, kCudaMaxBlocksNum), kCudaThreadsNumPerBlock, 0,
                 ctx->cuda_stream()>>>(num_boxes, ref_boxes_ptr, boxes_ptr, weight_x, weight_y,
                                       weight_w, weight_h, boxes_delta_ptr);
  }
};
