...
template<typename T>
__global__ void GpuForward(const T* boxes1_ptr, const int32_t num_boxes1, const T* boxes2_ptr,
                           const int32_t num_boxes2, float* iou_matrix_ptr) {
...
struct CalcIoUMatrixUtil<DeviceType::kGPU, T> {
  static void Forward(DeviceCtx* ctx, const T* boxes1_ptr, const int32_t num_boxes1,
    ...
    GpuForward<<<std::min(num_boxes1, kCudaMaxBlocksNum),
