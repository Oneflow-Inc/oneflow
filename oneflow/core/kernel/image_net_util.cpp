#include "oneflow/core/kernel/image_net_util.h"
#include <libgen.h>
#include <chrono>
...
void ImageNetUtil::GetFilePaths(
  ...
  for (int i = 0; i < image_directories.size(); ++i) {
    ...
    for (const auto& file_name : LocalFS()->ListDir(dir)) {
      const auto& file_path = JoinPath(dir, file_name);
...
void ImageNetUtil::SaveLabels(
  ...
  for (int i = 0; i < image_directories.size(); ++i) {
    const auto& dir = image_directories[i];
    label_names[i] = std::string(basename(const_cast<char*>(dir.c_str())));
  }
  ...
  std::vector<uint32_t> item_label_idx(img_file_paths.size());
  for (int i = 0; i < img_file_paths.size(); ++i) {
    item_label_idx[i] = file_path2label_idx.at(img_file_paths.at(i));
  }
