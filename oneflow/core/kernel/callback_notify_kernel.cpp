...
void CallbackNotifyKernel<T>::ForwardDataContent(
  ...
  const auto &buffer_name =
      this->op_conf().callback_notify_conf().callback_buffer_name(job_id);
  ...
  if (buffer_status == kBufferStatusSuccess) {
    foreign_job_instance->Finish();
  }
}

ADD_CPU_DEFAULT_KERNEL_CREATOR(OperatorConf::kCallbackNotifyConf,
                               CallbackNotifyKernel, INT_DATA_TYPE_SEQ);
