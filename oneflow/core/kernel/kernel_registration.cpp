...
HashMap<OperatorConf::OpTypeCase, std::vector<KernelRegistryVal>> *
MutKernelRegistry() {
  static HashMap<OperatorConf::OpTypeCase, std::vector<KernelRegistryVal>>
      creators;
  return &creators;
}
...
Kernel *CreateKernel(const KernelConf &kernel_conf) {
  ...
  if (kernel_registry->find(op_type) == kernel_registry->end()) {
    return nullptr;
  }
  ...
  for (const KernelRegistryVal &val : registry_vals) {
    if (val.cons.IsMatched(kernel_conf)) {
      CHECK(!is_matched) << "There are more than one kernel constraints "
                            "satisfied by kernel conf of "
