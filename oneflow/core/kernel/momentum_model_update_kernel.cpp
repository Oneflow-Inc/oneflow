...
//  float learning_rate =
//  op()->op_conf().momentum_mdupdt_conf().learning_rate(); float beta =
//  op()->op_conf().momentum_mdupdt_conf().beta(); float alpha = learning_rate /
//  JobDesc::Singleton()->batch_size(); CHECK(std::isfinite(alpha));
//
...
//
// template<DeviceType device_type, typename FloatingPointType>
// void MomentumMdUpdateKernel<device_type,
// FloatingPointType>::InitDataTmpBlobs(
//    const KernelCtx& ctx,
