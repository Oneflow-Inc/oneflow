...
const MomentumModelUpdateConf &
GetMomentumModelUpdateConf(const OperatorConf &op_conf) {
...
const PbMessage &
MomentumMdUpdateKernel<device_type, T>::GetCustomizedOpConf() const {
...
void MomentumMdUpdateKernel<device_type, T>::UpdateModel(
  ...
  MomentumMdUpdateKernelUtil<device_type, T>::UpdateModel(
      ctx, model_blob->shape().elem_cnt(), static_cast<T>(beta), train_step,
      learning_rate, weight_decay, model_diff_blob->dptr<T>(),
...
public:
  static void UpdateModel(DeviceCtx *, int64_t n, T beta,
    ...
    for (int64_t i = 0; i != n; ++i) {
      ...
      model[i] =
          model[i] + next_momentum - *learning_rate * weight_decay * model[i];
    }
  }
};
...
ADD_DEFAULT_KERNEL_CREATOR(OperatorConf::kMomentumModelUpdateConf,
