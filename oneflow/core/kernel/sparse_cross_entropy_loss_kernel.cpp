...
void SparseCrossEntropyLossKernel<device_type, PredType, LabelType>::
  ...
  if (prediction_diff != nullptr) {
    ...
    SparseCrossEntropyLossKernelUtil<device_type, PredType, LabelType>::
...
template<DeviceType device_type, typename PredType, typename LabelType>
const LossKernelConf& SparseCrossEntropyLossKernel<
    device_type, PredType,
    LabelType>::GetLossKernelConf(const KernelConf& kernel_conf) const {
...
#define SPARSE_CROSS_ENTROPY_LOSS_KERNEL_ENTRY(device_type, pred_type_pair, \
                                               label_type_pair)             \
  {GetHashKey(device_type, OF_PP_PAIR_SECOND(pred_type_pair),               \
              OF_PP_PAIR_SECOND(label_type_pair)),                          \
   []() {                                                                   \
     return new SparseCrossEntropyLossKernel<                               \
         device_type, OF_PP_PAIR_FIRST(pred_type_pair),                     \
         OF_PP_PAIR_FIRST(label_type_pair)>();                              \
   }},
      OF_PP_SEQ_PRODUCT_FOR_EACH_TUPLE(SPARSE_CROSS_ENTROPY_LOSS_KERNEL_ENTRY,
                                       DEVICE_TYPE_SEQ, FLOATING_DATA_TYPE_SEQ,
                                       INT_DATA_TYPE_SEQ)};
  return creators.at(GetHashKey(
      kernel_conf.op_attribute().device_type(),
