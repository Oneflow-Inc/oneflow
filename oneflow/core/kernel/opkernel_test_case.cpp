...
DEFINE_STATIC_SWITCH_FUNC(DataType, GetDataTypeValue, MAKE_OPK_HELPER_SWITCH_ENTRY,
                          MAKE_STRINGIZED_DATA_TYPE_CTRV_SEQ(POD_DATA_TYPE_SEQ))
...
void OpKernelTestUtil<DeviceType::kGPU>::BuildKernelCtx(KernelCtx* ctx) {
  if (!Global<CudaStreamHandle>::Get()) { Global<CudaStreamHandle>::New(nullptr, 0); }
...
OF_PP_FOR_EACH_TUPLE(INSTANTIATE_CPU_OPKERNEL_TEST_UTIL_METHODS, POD_DATA_TYPE_SEQ);
...
OF_PP_FOR_EACH_TUPLE(INSTANTIATE_OPKERNEL_TEST_CASE_METHODS, POD_DATA_TYPE_SEQ);
