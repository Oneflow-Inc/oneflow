...
template <int32_t power> struct PowUtil;

template <> struct PowUtil<1> final {
  template <typename T> __device__ static T pow(const T x) { return x; }
};

template <> struct PowUtil<2> final {
  template <typename T> __device__ static T pow(const T x) { return x * x; }
};
...
template <typename T>
__device__ void UpdateModel(const float learning_rate, T weight_decay,
                            T epsilon, T *model, const T m, const T v) {
  T model_val = *model;
...
__global__ void UpdateModelGpu(int64_t n, const float *learning_rate,
                               T weight_decay, T beta1, T beta2, T epsilon,
                               T *beta1_t, T *beta2_t, const T *model_diff,
                               T *model, T *m, T *v) {
  float lr;
...
template <typename T>
__global__ void UpdateBetaTGpu(const T beta1, const T beta2, T *beta1_t,
                               T *beta2_t) {
  *beta1_t *= beta1;
  *beta2_t *= beta2;
}
...
template <typename T> class AdamMdUpdateKernelUtil<DeviceType::kGPU, T> final {
public:
    ...
    if (do_bias_correction) {
      UpdateModelGpu<true,
                     T><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock, 0,
                          ctx->cuda_stream()>>>(
          n, learning_rate, weight_decay, beta1, beta2, epsilon, beta1_t,
          beta2_t, model_diff, model, m, v);
      UpdateBetaTGpu<T><<<1, 1, 0, ctx->cuda_stream()>>>(beta1, beta2, beta1_t,
                                                         beta2_t);
    } else {
      UpdateModelGpu<false,
                     T><<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock, 0,
                          ctx->cuda_stream()>>>(
          n, learning_rate, weight_decay, beta1, beta2, epsilon, beta1_t,
          beta2_t, model_diff, model, m, v);
