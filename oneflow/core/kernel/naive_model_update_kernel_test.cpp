...
void NaiveModelUpdateTestCase(OpKernelTestCase* mdupdt_test_case, const std::string& job_type,
  ...
  NormalModelUpdateOpUserConf* user_conf = normal_mdupdt_conf->mutable_user_conf();
  ...
  mdupdt_test_case->template InitBlob<T>("in_0", blob_desc, std::vector<T>(shape.elem_cnt(), 1));
  mdupdt_test_case->template InitBlob<T>("packed_blob", blob_desc,
                                         std::vector<T>(shape.elem_cnt(), 2));
  mdupdt_test_case->template ForwardCheckBlob<T>("model", blob_desc,
...
TEST_CPU_AND_GPU_OPKERNEL(NaiveModelUpdateTestCase, FLOATING_DATA_TYPE_SEQ, (train), (forward));

DiffKernelImplTestCase* DiffNaiveModelUpdateKernelImpl(const std::string& job_type,
                                                       const std::string& fw_or_bw,
                                                       const std::string& cpp_type) {
  auto* mdupdt_test_case = new DiffKernelImplTestCase(job_type == "train", fw_or_bw == "forward",
  ...
  mdupdt_test_case->set_initiate_kernel_ctx(
      [mdupdt_test_case](const std::function<Blob*(const std::string&)>& Blob4BnInOp) {
        auto* other = new std::tuple<int64_t, const Blob*>(-1, Blob4BnInOp("packed_blob"));
        mdupdt_test_case->mut_kernel_ctx()->other = other;
      });
  NormalModelUpdateOpConf* normal_mdupdt_conf =
      mdupdt_test_case->mut_op_conf()->mutable_normal_mdupdt_conf();
  ...
  mdupdt_test_case->SetInputBlobDesc("in_0", Shape({1, 3, 2}), DataType4CppTypeString(cpp_type));
  mdupdt_test_case->SetInputBlobDesc("packed_blob", Shape({1, 3, 2}),
                                     DataType4CppTypeString(cpp_type));
  mdupdt_test_case->SetInputBlobDesc("model", Shape({1, 3, 2}), DataType4CppTypeString(cpp_type));
