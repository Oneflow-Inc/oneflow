...
#define SOFTMAX_LOSS_TEST_UTIL_ENTRY(func_name, T) \
  ...
  DEFINE_STATIC_SWITCH_FUNC(void, Test, SOFTMAX_LOSS_TEST_UTIL_ENTRY,
  ...
  static void Test(OpKernelTestCase* test_case, const std::string& job_type,
    ...
    test_case->InitBlob<PredType>("prediction", blob_desc24, {1, 2, 3, 4, 1, 1, 1, 1});
    test_case->InitBlob<LabelType>("label", label_blob_desc, {2, 0});
    test_case->RandomInitBlob<PredType>("tmp_1D", blob_desc2);
    test_case->ForwardCheckBlob<PredType>("prob", blob_desc24,
    ...
    test_case->BackwardCheckBlob<PredType>(
        ...
        {0.0320586078, 0.0871443302, -0.7631171942, 0.6439142824, -0.75, 0.25, 0.25, 0.25});
  }
};

template<DeviceType device_type, typename PredType>
void SoftmaxLossKernelTestCase(OpKernelTestCase* test_case, const std::string& label_type,
                               const std::string& job_type, const std::string& fw_or_bw) {
  SoftmaxLossTestUtil<device_type, PredType>::SwitchTest(SwitchCase(label_type), test_case,
                                                         job_type, fw_or_bw);
}
