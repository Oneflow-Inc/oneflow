...
void PoolingCtx::set_pooling_2d_desc(DataType type) {
  ...
  */
  ...
  if (data_format == "channels_first") {
    in_stride = {in_shape[1] * in_shape[2] * in_shape[3], in_shape[2] * in_shape[3], in_shape[3],
                 1};
    out_stride = {out_shape[1] * out_shape[2] * out_shape[3], out_shape[2] * out_shape[3],
                  out_shape[3], 1};
  } else if (data_format == "channels_last") {
    in_stride = {in_shape[1] * in_shape[2] * in_shape[3], 1, in_shape[2] * in_shape[3],
                 in_shape[3]};
    out_stride = {out_shape[1] * out_shape[2] * out_shape[3], 1, out_shape[2] * out_shape[3],
                  out_shape[3]};
  } else {
    UNIMPLEMENTED();
  }
  in_desc_.reset(new CudnnTensorDesc(type, 4, in_shape.data(), in_stride.data()));
  out_desc_.reset(new CudnnTensorDesc(type, 4, out_shape.data(), out_stride.data()));
}
