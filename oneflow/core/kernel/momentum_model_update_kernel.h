...
class MomentumMdUpdateKernel final : public NormalMdUpdateKernel<device_type, T> {
 ...
 private:
  void UpdateModel(DeviceCtx* ctx, const Blob* pre_model_blob, const Blob* model_diff_blob,
                   int64_t next_model_vid, int64_t batch_size, T learning_rate, T l1, T l2,
...
class MomentumMdUpdateKernelUtil final {
 public:
  static void UpdateModel(DeviceCtx*, const int64_t n, const T beta, const T learning_rate,
