...
void TransposeTestCase_2d(OpKernelTestCase* transpose_test_case, const std::string& job_type,
  ...
  BlobDesc* blob_desc = new BlobDesc(Shape({1, 2, 3}), GetDataType<T>::value, false, false, 1);
  BlobDesc* trans_blob_desc =
      new BlobDesc(Shape({1, 3, 2}), GetDataType<T>::value, false, false, 1);
  transpose_test_case->template InitBlob<T>("in", blob_desc, {1, 2, 3, 4, 5, 6});
  transpose_test_case->template InitBlob<T>(GenDiffBn("out"), trans_blob_desc,
                                            {-8, 7, -6, 5, -4, 3});
  transpose_test_case->template ForwardCheckBlob<T>("out", trans_blob_desc, {1, 4, 2, 5, 3, 6});
  transpose_test_case->template BackwardCheckBlob<T>(GenDiffBn("in"), blob_desc,
                                                     {-8, -6, -4, 7, 5, 3});
}
TEST_CPU_AND_GPU_OPKERNEL(TransposeTestCase_2d, FLOATING_DATA_TYPE_SEQ, (train)(predict),
                          (forward)(backward));

DiffKernelImplTestCase* DiffTransposeKernelImpl_cfirst_to_clast(const std::string& job_type,
  ...
  test_case->SetInputBlobDesc(GenDiffBn("out"), trans_shape, DataType4CppTypeString(cpp_type));
  return test_case;
}
TEST_DIFF_KERNEL_IMPL(DiffTransposeKernelImpl_cfirst_to_clast, (train)(predict),
                      (forward)(backward), OF_PP_SEQ_MAP(OF_PP_PAIR_FIRST, FLOATING_DATA_TYPE_SEQ));

DiffKernelImplTestCase* DiffTransposeKernelImpl_clast_to_cfirst(const std::string& job_type,
                                                                const std::string& fw_or_bw,
                                                                const std::string& cpp_type) {
  auto* test_case = new DiffKernelImplTestCase(job_type == "train", fw_or_bw == "forward",
                                               DataType4CppTypeString(cpp_type));
