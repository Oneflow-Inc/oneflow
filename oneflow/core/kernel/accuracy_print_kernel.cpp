...
void AccuracyPrintKernel<T>::Forward(const KernelCtx& kernel_ctx,
  ...
  const Blob* batch_instance_num_blob = BnInOp2Blob("batch_instance_num");
  T accurate_num = accuracy_acc_blob->dptr<T>()[0];
  int32_t batch_instance_num = static_cast<int32_t>(batch_instance_num_blob->dptr<T>()[0]);
  float accuracy = accurate_num / batch_instance_num;
