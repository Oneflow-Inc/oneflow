...
__global__ void UpdateGpu(T beta, int64_t feature_size, int64_t lower_bound,
  ...
  CUDA_1D_KERNEL_LOOP(i, n) {
    ...
    if (instance_id >= lower_bound && instance_id < upper_bound) {
      ...
      const K model_idx =
          (instance_id - lower_bound) * feature_size + i % feature_size;
...
struct IndexedSlicesMomentumMdUpdateKernelUtil<DeviceType::kGPU, T, K, IDX> {
  static void Update(DeviceCtx *ctx, T beta, int64_t num_instance,
...
template <typename T, typename K, typename IDX>
void IndexedSlicesMomentumMdUpdateKernelUtil<DeviceType::kGPU, T, K, IDX>::
    Update(DeviceCtx *ctx, T beta, int64_t num_instance, int64_t feature_size,
           int64_t lower_bound, int64_t upper_bound,
           const IDX *num_unique_instance, const int64_t *train_step,
           const float *learning_rate, const K *indices, const T *values,
           T *model, T *momentum) {
  UpdateGpu<T, K><<<BlocksNum4ThreadsNum(num_instance * feature_size),
                    kCudaThreadsNumPerBlock, 0, ctx->cuda_stream()>>>(
      beta, feature_size, lower_bound, upper_bound, num_unique_instance,
...
#define INSTANTIATE_INDEXED_SLICES_MOMENTUM_MODEL_UPDATE_KERNEL_UTIL_GPU(      \
  ...
  template struct IndexedSlicesMomentumMdUpdateKernelUtil<                     \
      DeviceType::kGPU, OF_PP_PAIR_FIRST(val_type_pair),                       \
      OF_PP_PAIR_FIRST(key_type_pair), OF_PP_PAIR_FIRST(idx_type_pair)>;
OF_PP_SEQ_PRODUCT_FOR_EACH_TUPLE(
    INSTANTIATE_INDEXED_SLICES_MOMENTUM_MODEL_UPDATE_KERNEL_UTIL_GPU,
