...
Blob* GetBlob(int64_t blob_id) {
  return BnInOp2Blob(kernel_conf().input_bns(blob_id));
}
...
void PrintKernel::VirtualKernelInit(const ParallelContext* parallel_ctx) {
  ...
  OF_CALL_ONCE(root_path, GlobalFS()->RecursivelyCreateDir(root_path));
    std::string file_path = JoinPath(root_path, this->op_conf().print_conf().part_name_prefix() + std::to_string(parallel_ctx->parallel_id()));
    out_stream_.emplace_back(new PersistentOutStream(GlobalFS(), file_path));
}
...
void PrintKernel::Forward(
  ...
  FOR_RANGE(int64_t, record_id, 0, max_record_num) {
    ...
    if (std::string(data_id_str) == "") {
      break;
    }
    OFRecordEncoderIf::EncodeDataIdToOneRecord(ctx.device_ctx, data_id_str, record);
    FOR_RANGE(int32_t, blob_id, 0, total_blob_num) {
      const Blob* cur_blob = GetBlob(blob_id);
      GetOFRecordEncoder(conf.encode_case(), cur_blob->data_type())->EncodeOneFieldToOneRecord(ctx.device_ctx, record_id, cur_blob, conf.in(blob_id).name(), record);
    }
