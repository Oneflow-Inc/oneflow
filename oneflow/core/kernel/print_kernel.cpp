...
void PrintKernel::Forward(const KernelCtx& ctx,
  ...
  const auto& conf = op_conf().print_conf();
  const int32_t blob_num = op_attribute().input_bns().size();
  const Blob* first_blob = GetBlob(0);
  const int64_t record_num = first_blob->shape().At(0);
  ...
  FOR_RANGE(int32_t, blob_id, 1, blob_num) {
  ...
  FOR_RANGE(int64_t, record_id, 0, record_num) {
    ...
    FOR_RANGE(int32_t, blob_id, 0, blob_num) {
      const Blob* cur_blob = GetBlob(blob_id);
      const PrintRecordConf& cur_print_conf = conf.in(blob_id);
      const std::string field_name =
          cur_print_conf.has_name() ? cur_print_conf.name() : cur_print_conf.lbn();
