...
#define OBJECT_MSG_DEFINE_CONDITION_LIST_HEAD(elem_type, elem_field_name,      \
                                              field_name)                      \
  ...
  _OBJECT_MSG_DEFINE_CONDITION_LIST_HEAD(                                      \
      STATIC_COUNTER(field_counter), elem_type, elem_field_name, field_name);

#define OBJECT_MSG_CONDITION_LIST(obj_msg_type, obj_msg_field)                 \
  ObjectMsgConditionList<                                                      \
...
#define _OBJECT_MSG_DEFINE_CONDITION_LIST_HEAD(field_counter, elem_type,       \
                                               elem_field_name, field_name)    \
  _OBJECT_MSG_DEFINE_CONDITION_LIST_HEAD_FIELD(elem_type, elem_field_name,     \
                                               field_name)                     \
  OBJECT_MSG_DEFINE_CONDITION_LIST_ELEM_STRUCT(field_counter, elem_type,       \
                                               elem_field_name, field_name);   \
  OBJECT_MSG_DEFINE_CONDITION_LIST_LINK_EDGES(field_counter, elem_type,        \
                                              elem_field_name, field_name);    \
  OBJECT_MSG_OVERLOAD_INIT(field_counter,                                      \
                           ObjectMsgEmbeddedConditionListHeadInit);            \
  OBJECT_MSG_OVERLOAD_DELETE(field_counter,                                    \
                             ObjectMsgEmbeddedConditionListHeadDelete);        \
  DSS_DEFINE_FIELD(field_counter, "object message",                            \
                   OF_PP_CAT(field_name, _ObjectMsgListType),                  \
                   OF_PP_CAT(field_name, _));

#define _OBJECT_MSG_DEFINE_CONDITION_LIST_HEAD_FIELD(                          \
    elem_type, elem_field_name, field_name)                                    \
public:                                                                        \
  using OF_PP_CAT(field_name, _ObjectMsgListType) =                            \
      TrivialObjectMsgConditionList<                                           \
...
#define OBJECT_MSG_DEFINE_CONDITION_LIST_ELEM_STRUCT(                          \
    field_counter, elem_type, elem_field_name, field_name)                     \
public:                                                                        \
...
#define OBJECT_MSG_DEFINE_CONDITION_LIST_LINK_EDGES(                           \
    field_counter, elem_type, elem_field_name, field_name)                     \
public:                                                                        \
...
template <typename LinkField> class TrivialObjectMsgConditionList {
public:
  ...
  ObjectMsgConditionListStatus EmplaceBack(ObjectMsgPtr<value_type> &&ptr) {
    std::unique_lock<std::mutex> lock(*mut_mutex());
    if (is_closed_) {
      return kObjectMsgConditionListStatusErrorClosed;
    }
  ...
  ObjectMsgConditionListStatus PopFront(ObjectMsgPtr<value_type> *ptr) {
    std::unique_lock<std::mutex> lock(*mut_mutex());
    mut_cond()->wait(lock,
                     [this]() { return (!list_head_.empty()) || is_closed_; });
    if (list_head_.empty()) {
      return kObjectMsgConditionListStatusErrorClosed;
    }
  ...
  ObjectMsgConditionListStatus
  MoveFrom(TrivialObjectMsgList<kDisableSelfLoopLink, LinkField> *src) {
    std::unique_lock<std::mutex> lock(*mut_mutex());
    if (is_closed_) {
      return kObjectMsgConditionListStatusErrorClosed;
    }
  ...
  ObjectMsgConditionListStatus
  MoveTo(TrivialObjectMsgList<kDisableSelfLoopLink, LinkField> *dst) {
    std::unique_lock<std::mutex> lock(*mut_mutex());
    mut_cond()->wait(lock,
                     [this]() { return (!list_head_.empty()) || is_closed_; });
    if (list_head_.empty()) {
  ...
  ObjectMsgConditionListStatus
  TryMoveTo(TrivialObjectMsgList<kDisableSelfLoopLink, LinkField> *dst) {
    std::unique_lock<std::mutex> lock(*mut_mutex());
    if (list_head_.empty()) {
      return kObjectMsgConditionListStatusSuccess;
    }
...
private:
  std::mutex *mut_mutex() {
    return reinterpret_cast<std::mutex *>(&mutex_buff_[0]);
  }
