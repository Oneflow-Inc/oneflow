...
TEST(StructField, mutable_struct_mutable_field) {
  ...
  auto *bar = &foo.bar;
  auto *struct_ptr =
      STRUCT_FIELD(OneflowTestNamespaceFoo, bar)::StructPtr4FieldPtr(bar);
  auto *field_ptr =
      STRUCT_FIELD(OneflowTestNamespaceFoo, bar)::FieldPtr4StructPtr(&foo);
...
TEST(StructField, mutable_struct_const_field) {
  ...
  auto *struct_ptr =
      STRUCT_FIELD(OneflowTestNamespaceFoo, const_bar)::StructPtr4FieldPtr(bar);
  auto *field_ptr = STRUCT_FIELD(OneflowTestNamespaceFoo,
                                 const_bar)::FieldPtr4StructPtr(&foo);
  ASSERT_EQ(struct_ptr, &foo);
  ASSERT_EQ(field_ptr, bar);
}
...
TEST(StructField, compose) {
  using BFieldInY =
      typename ComposeStructField<STRUCT_FIELD(Y, d), STRUCT_FIELD(X, b)>::type;
  Y y;
  int *field_b = &y.d.b;
