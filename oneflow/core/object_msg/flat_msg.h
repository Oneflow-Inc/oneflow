...
#define FLAT_MSG_END(struct_name)                                              \
  static_assert(__is_flat_message_type__,                                      \
                "this struct is not a flat message");                          \
  OF_PUBLIC static const int __NumberOfFields__ =                              \
      STATIC_COUNTER(field_counter);                                           \
...
#define FLAT_MSG_DEFINE_ONEOF(oneof_name, type_and_field_name_seq)             \
  _FLAT_MSG_DEFINE_ONEOF(_FLAT_MSG_DEFINE_NOTHING, oneof_name,                 \
                         type_and_field_name_seq);

#define FLAT_MSG_DEFINE_STRICT_ONEOF(oneof_name, type_and_field_name_seq)      \
  _FLAT_MSG_DEFINE_ONEOF(_FLAT_MSG_DEFINE_ONEOF_VALUE4TYPE, oneof_name,        \
                         type_and_field_name_seq);

#define _FLAT_MSG_DEFINE_ONEOF(define_field_value4field_type, oneof_name,      \
                               type_and_field_name_seq)                        \
  static_assert(__is_flat_message_type__,                                      \
                "this struct is not a flat message");                          \
  FLAT_MSG_DEFINE_ONEOF_ENUM_TYPE(oneof_name, type_and_field_name_seq);        \
  FLAT_MSG_DEFINE_ONEOF_UNION(define_field_value4field_type, oneof_name,       \
                              type_and_field_name_seq);                        \
...
#define FLAT_MSG_DEFINE_REPEATED(field_type, field_name, max_size)             \
  static_assert(__is_flat_message_type__,                                      \
                "this struct is not a flat message");                          \
  _FLAT_MSG_DEFINE_REPEATED_FIELD(FLAT_MSG_TYPE_CHECK(field_type), field_name, \
                                  max_size);                                   \
  OF_PUBLIC INCREASE_STATIC_COUNTER(field_counter);                            \
...
#define FLAT_MSG_DEFINE_COMPARE_OPERATORS_BY_MEMCMP()                          \
  _FLAT_MSG_DEFINE_COMPARE_OPERATORS_BY_MEMCMP()
...
#define FLAT_MSG_DSS_DEFINE_UION_FIELD(field_counter, oneof_name,              \
                                       type_and_field_name_seq)                \
  DSS_DEFINE_FIELD(field_counter, "flat message",                              \
                   OF_PP_CAT(oneof_name, _OneofType),                          \
                   OF_PP_CAT(oneof_name, _));                                  \
  DSS_DEFINE_UNION_FIELD_VISITOR(                                              \
      field_counter, case_,                                                    \
      OF_PP_FOR_EACH_TUPLE(FLAT_MSG_MAKE_UNION_TYPE7FIELD4CASE,                \
                           type_and_field_name_seq));

#define FLAT_MSG_MAKE_UNION_TYPE7FIELD4CASE(field_type, field_name)            \
  OF_PP_MAKE_TUPLE_SEQ(field_type, OF_PP_CAT(field_name, _),                   \
                       _FLAT_MSG_ONEOF_ENUM_VALUE(field_name))

template <typename T, typename Enabled = void> struct FlatMsgSelfType {
  static_assert(T::__is_flat_message_type__, "T is not a flat message type");
  using type = T;
};

template <typename T>
struct FlatMsgSelfType<T,
                       typename std::enable_if<std::is_arithmetic<T>::value ||
                                               std::is_enum<T>::value>::type> {
  using type = T;
};

template <typename T> struct FlatMsg final {
  using value_type = T;
  ...
  template <typename RhsT> bool operator==(const RhsT &rhs) const {
...
template <typename T> struct FlatMsgIsScalar final {
  static const bool value =
      std::is_arithmetic<T>::value || std::is_enum<T>::value;
};

template <bool is_scalar> struct FlatMsgGetDefault final {
  template <typename T> static const T &Call(const T *val) {
    return val->__Default__();
  }
};
template <> struct FlatMsgGetDefault<true> final {
  template <typename T> static const T &Call(const T *val) { return *val; }
};
...
#define _FLAT_MSG_ONEOF_NOT_SET_VALUE(oneof_name)                              \
  OF_PP_CAT(k_, OF_PP_CAT(oneof_name, _not_set))
...
public:                                                                        \
  ...
  void CopyFrom(const self_type &rhs) {                                        \
    std::memcpy(reinterpret_cast<void *>(this),                                \
                reinterpret_cast<const void *>(&rhs), sizeof(self_type));      \
  }
...
public:                                                                        \
  enum _FLAT_MSG_ONEOF_ENUM_TYPE(oneof_name) {                                 \
    ...
    OF_PP_FOR_EACH_TUPLE(MAKE_FLAT_MSG_ONEOF_ENUM_CASE,                        \
                         type_and_field_name_seq)                              \
  }
...
#define FLAT_MSG_DEFINE_ONEOF_ACCESSOR(oneof_name, type_and_field_name_seq)    \
  _FLAT_MSG_DEFINE_ONEOF_CASE_ACCESSOR(oneof_name,                             \
                                       _FLAT_MSG_ONEOF_ENUM_TYPE(oneof_name)); \
  OF_PP_SEQ_PRODUCT_FOR_EACH_TUPLE(MAKE_FLAT_MSG_ONEOF_ACCESSOR,               \
                                   (_FLAT_MSG_ONEOF_ENUM_VALUE), (oneof_name), \
                                   type_and_field_name_seq)
...
public:                                                                        \
  const OF_PP_PAIR_FIRST(pair) & OF_PP_PAIR_SECOND(pair)() const {             \
    ...
    return FlatMsgGetDefault<FlatMsgIsScalar<OF_PP_PAIR_FIRST(pair)>::value>:: \
        Call(&OF_PP_CAT(oneof_name, _).OF_PP_CAT(OF_PP_PAIR_SECOND(pair), _)); \
  }                                                                            \
  bool OF_PP_CAT(has_, OF_PP_PAIR_SECOND(pair))() const {                      \
    return _FLAT_MSG_ONEOF_CASE_NAME(oneof_name)() ==                          \
  ...
  void OF_PP_CAT(set_, OF_PP_PAIR_SECOND(pair))(const OF_PP_PAIR_FIRST(pair) & \
                                                val) {                         \
...
public:                                                                        \
  ...
  public:                                                                      \
    ...
    OF_PP_FOR_EACH_TUPLE(_MAKE_FLAT_MSG_ONEOF_TEMPLATE_ACCESSOR,               \
                         type_and_field_name_seq);                             \
    define_field_value4field_type(type_and_field_name_seq);                    \
    template <self_oneof_case_type oneof_case> bool Has() const {              \
    ...
    typename FieldType4FieldValueStruct<oneof_case>::type *Mutable() {         \
      return MutableStruct<oneof_case>::Call(this);                            \
                                                                               ...
                                                                               \
    union {                                                                    \
      OF_PP_FOR_EACH_TUPLE(MAKE_FLAT_MSG_ONEOF_UNION_FIELD,                    \
                           type_and_field_name_seq)                            \
    };                                                                         \
...
public:                                                                        \
  ...
  struct FieldType4FieldValueStruct<_FLAT_MSG_ONEOF_ENUM_VALUE(field_name),    \
                                    Enabled> {                                 \
...
public:                                                                        \
  ...
  OF_PP_FOR_EACH_TUPLE(_MAKE_FLAT_MSG_ONEOF_VALUE4TYPE,                        \
                       type_and_field_name_seq);                               \
  template <typename T> bool HasField() const {                                \
    return Has<FieldValue4FieldType<T>::value>();                              \
  }                                                                            \
  template <typename T> const T &GetField() const {                            \
    return Get<FieldValue4FieldType<T>::value>();                              \
  }                                                                            \
  template <typename T> T *MutableField() {                                    \
...
#define _MAKE_FLAT_MSG_ONEOF_VALUE4TYPE(field_type, field_name)                \
  ...
  struct FieldValue4FieldType<field_type, Enabled> {                           \
    static const self_oneof_case_type value =                                  \
        _FLAT_MSG_ONEOF_ENUM_VALUE(field_name);                                \
  };

#define MAKE_FLAT_MSG_ONEOF_UNION_FIELD(field_type, field_name)                \
...
public:                                                                        \
  T OF_PP_CAT(oneof_name, _case)() const {                                     \
    return OF_PP_CAT(oneof_name, _).case_;                                     \
  }                                                                            \
...
public:                                                                        \
  ...
  std::size_t OF_PP_CAT(field_name, _size)() const {                           \
    return OF_PP_CAT(field_name, _).size();                                    \
  }                                                                            \
  const OF_PP_CAT(field_name, _RepeatedField) & field_name() const {           \
    return OF_PP_CAT(field_name, _);                                           \
  }                                                                            \
  const T &field_name(int32_t i) const {                                       \
    return OF_PP_CAT(field_name, _).Get(i);                                    \
  }                                                                            \
  ...
  T *OF_PP_CAT(mut_, field_name)(int32_t i) {                                  \
    return OF_PP_CAT(field_name, _).Mutable(i);                                \
  }                                                                            \
  T *OF_PP_CAT(mutable_, field_name)(int32_t i) {                              \
    return OF_PP_CAT(field_name, _).Mutable(i);                                \
  }                                                                            \
...
public:                                                                        \
  bool operator<(const self_type &rhs) const {                                 \
    return std::memcmp(reinterpret_cast<const void *>(this),                   \
...
template <typename T, std::size_t N> class FlatMsgRepeatedField final {
public:
  using value_type = T;
