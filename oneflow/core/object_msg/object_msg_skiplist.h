...
#define OBJECT_MSG_DEFINE_SKIPLIST_KEY(max_level, T, field_name)               \
  static_assert(__is_object_message_type__,                                    \
                "this struct is not a object message");                        \
  static_assert(std::is_standard_layout<T>::value,                             \
                "this struct is not standard layout");                         \
  OF_PRIVATE INCREASE_STATIC_COUNTER(field_counter);                           \
  _OBJECT_MSG_DEFINE_SKIPLIST_KEY(STATIC_COUNTER(field_counter), max_level, T, \
                                  field_name);

#define OBJECT_MSG_DEFINE_SKIPLIST_HEAD(elem_type, elem_field_name,            \
                                        field_name)                            \
  static_assert(__is_object_message_type__,                                    \
                "this struct is not a object message");                        \
  static_assert(!std::is_same<self_type, elem_type>::value,                    \
                "self loop link is not supported");                            \
  OF_PRIVATE INCREASE_STATIC_COUNTER(field_counter);                           \
  _OBJECT_MSG_DEFINE_SKIPLIST_HEAD(STATIC_COUNTER(field_counter), elem_type,   \
                                   elem_field_name, field_name);

#define OBJECT_MSG_SKIPLIST(elem_type, elem_field_name)                        \
  ObjectMsgSkipList<OBJECT_MSG_SKIPLIST_ELEM_STRUCT_FIELD(elem_type,           \
                                                          elem_field_name)>

#define OBJECT_MSG_SKIPLIST_FOR_EACH(skiplist_ptr, elem)                       \
  _OBJECT_MSG_SKIPLIST_FOR_EACH(                                               \
      std::remove_pointer<decltype(skiplist_ptr)>::type, skiplist_ptr, elem)

#define OBJECT_MSG_SKIPLIST_FOR_EACH_PTR(skiplist_ptr, elem)                   \
  _OBJECT_MSG_SKIPLIST_FOR_EACH_PTR(                                           \
      std::remove_pointer<decltype(skiplist_ptr)>::type, skiplist_ptr, elem)

#define OBJECT_MSG_SKIPLIST_UNSAFE_FOR_EACH_PTR(skiplist_ptr, elem)            \
  _OBJECT_MSG_SKIPLIST_UNSAFE_FOR_EACH_PTR(                                    \
      std::remove_pointer<decltype(skiplist_ptr)>::type, skiplist_ptr, elem)
// details

#define _OBJECT_MSG_DEFINE_SKIPLIST_HEAD(field_counter, elem_type,             \
                                         elem_field_name, field_name)          \
  _OBJECT_MSG_DEFINE_SKIPLIST_HEAD_FIELD(elem_type, elem_field_name,           \
                                         field_name);                          \
  OBJECT_MSG_DEFINE_SKIPLIST_ELEM_STRUCT(field_counter, elem_type,             \
                                         elem_field_name, field_name);         \
  OBJECT_MSG_DEFINE_SKIPLIST_LINK_EDGES(field_counter, elem_type,              \
                                        elem_field_name, field_name);          \
  OBJECT_MSG_OVERLOAD_INIT(field_counter, ObjectMsgEmbeddedSkipListHeadInit);  \
  OBJECT_MSG_OVERLOAD_DELETE(field_counter,                                    \
                             ObjectMsgEmbeddedSkipListHeadDelete);             \
  DSS_DEFINE_FIELD(field_counter, "object message",                            \
                   OF_PP_CAT(field_name, _ObjectMsgSkipListType),              \
                   OF_PP_CAT(field_name, _));

#define _OBJECT_MSG_DEFINE_SKIPLIST_HEAD_FIELD(elem_type, elem_field_name,     \
                                               field_name)                     \
public:                                                                        \
  using OF_PP_CAT(field_name, _ObjectMsgSkipListType) =                        \
      TrivialObjectMsgSkipList<OBJECT_MSG_SKIPLIST_ELEM_STRUCT_FIELD(          \
          elem_type, elem_field_name)>;                                        \
  const OF_PP_CAT(field_name, _ObjectMsgSkipListType) & field_name() const {   \
    return OF_PP_CAT(field_name, _);                                           \
  }                                                                            \
  OF_PP_CAT(field_name, _ObjectMsgSkipListType) *                              \
      OF_PP_CAT(mut_, field_name)() {                                          \
    return &OF_PP_CAT(field_name, _);                                          \
  }                                                                            \
  OF_PP_CAT(field_name, _ObjectMsgSkipListType) *                              \
      OF_PP_CAT(mutable_, field_name)() {                                      \
    return &OF_PP_CAT(field_name, _);                                          \
  }                                                                            \
                                                                               \
private:                                                                       \
...
#define OBJECT_MSG_DEFINE_SKIPLIST_ELEM_STRUCT(field_counter, elem_type,       \
                                               elem_field_name, field_name)    \
public:                                                                        \
...
#define OBJECT_MSG_DEFINE_SKIPLIST_LINK_EDGES(field_counter, elem_type,        \
                                              elem_field_name, field_name)     \
public:                                                                        \
...
#define _OBJECT_MSG_DEFINE_SKIPLIST_KEY(field_counter, max_level, T,           \
                                        field_name)                            \
  _OBJECT_MSG_DEFINE_SKIPLIST_KEY_FIELD(max_level, T, field_name)              \
  OBJECT_MSG_OVERLOAD_INIT(field_counter,                                      \
                           ObjectMsgEmbeddedSkipListIteratorInit);             \
  OBJECT_MSG_OVERLOAD_DELETE(field_counter,                                    \
                             ObjectMsgEmbeddedSkipListIteratorDelete);         \
  DSS_DEFINE_FIELD(field_counter, "object message",                            \
                   OF_PP_CAT(field_name, _ObjectMsgSkipListKeyType),           \
                   OF_PP_CAT(field_name, _));
...
public:                                                                        \
  ...
  ConstType<key_type> &field_name() const {                                    \
    return OF_PP_CAT(field_name, _).key();                                     \
  }                                                                            \
  key_type *OF_PP_CAT(mut_, field_name)() {                                    \
    return OF_PP_CAT(field_name, _).mut_key();                                 \
  }                                                                            \
  key_type *OF_PP_CAT(mutable_, field_name)() {                                \
    return OF_PP_CAT(field_name, _).mut_key();                                 \
  }                                                                            \
  template <typename T> void OF_PP_CAT(set_, field_name)(const T &val) {       \
...
#define OBJECT_MSG_SKIPLIST_ELEM_STRUCT_FIELD(elem_type, elem_field_name)      \
  StructField<elem_type, typename elem_type::OF_PP_CAT(                        \
                             elem_field_name, _ObjectMsgSkipListKeyType),      \
              elem_type::OF_PP_CAT(elem_field_name, _kDssFieldOffset)>

#define _OBJECT_MSG_SKIPLIST_FOR_EACH(skiplist_type, skiplist_ptr, elem)       \
  for (ObjectMsgPtr<skiplist_type::value_type> elem,                           \
       *end_if_not_null = nullptr;                                             \
       end_if_not_null == nullptr;                                             \
       end_if_not_null = nullptr, ++end_if_not_null)                           \
  EMBEDDED_LIST_FOR_EACH_WITH_EXPR(                                            \
      (StructField<skiplist_type, EmbeddedListLink,                            \
                   skiplist_type::ContainerLevelZeroLinkOffset()>::            \
           FieldPtr4StructPtr(skiplist_ptr)),                                  \
      skiplist_type::elem_level0_link_struct_field, elem_ptr,                  \
      (elem.Reset(elem_ptr), true))
...
#define _OBJECT_MSG_SKIPLIST_UNSAFE_FOR_EACH_PTR(skiplist_type, skiplist_ptr,  \
                                                 elem)                         \
...
template <typename ElemKeyField> class TrivialObjectMsgSkipList {
public:
  ...
  using elem_level0_link_struct_field =
      typename ComposeStructField<ElemKeyField,
                                  elem_key_level0_link_struct_field>::type;
  ...
  const value_type *FindPtr(const key_type &key) const {
    return skiplist_head_.Find(key);
  }
  bool EqualsEnd(const ObjectMsgPtr<value_type> &ptr) { return !ptr; }
  void Erase(const key_type &key) {
    ObjectMsgPtrUtil::ReleaseRef(skiplist_head_.Erase(key));
  }
  ...
  std::pair<ObjectMsgPtr<value_type>, bool> Insert(value_type *elem_ptr) {
    ...
    ret.second = success;
    if (success) {
      ObjectMsgPtrUtil::Ref(elem_ptr);
    }
    return ret;
  }

  void Clear() {
    skiplist_head_.Clear(
        [](value_type *elem) { ObjectMsgPtrUtil::ReleaseRef(elem); });
  }
