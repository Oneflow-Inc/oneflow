...
#define FLAT_MSG_VIEW_END(struct_name)                                         \
  static_assert(__is_flat_message_view_type__,                                 \
                "this struct is not a flat message view");                     \
  OF_PUBLIC static const int __LastFieldIndex__ =                              \
...
#define FLAT_MSG_VIEW_DEFINE_PATTERN(flat_msg_field_type, field_name)          \
  static_assert(__is_flat_message_view_type__,                                 \
                "this struct is not a flat message view");                     \
  _FLAT_MSG_VIEW_DEFINE_PATTERN(FLAT_MSG_TYPE_CHECK(flat_msg_field_type),      \
                                field_name);                                   \
  OF_PUBLIC INCREASE_STATIC_COUNTER(field_counter);                            \
  FLAT_MSG_VIEW_SPECIALIZE_FIELD_TYPE(STATIC_COUNTER(field_counter),           \
                                      flat_msg_field_type);                    \
  FLAT_MSG_VIEW_CHECK_LAST_FIELD_TYPE(STATIC_COUNTER(field_counter),           \
                                      flat_msg_field_type);                    \
  DSS_DEFINE_FIELD(STATIC_COUNTER(field_counter), "flat message view",         \
                   flat_msg_field_type *, OF_PP_CAT(field_name, _));

#define FLAT_MSG_VIEW_DEFINE_REPEATED_PATTERN(flat_msg_field_type, field_name) \
  static_assert(__is_flat_message_view_type__,                                 \
                "this struct is not a flat message view");                     \
  _FLAT_MSG_VIEW_DEFINE_REPEATED_PATTERN(                                      \
      FLAT_MSG_TYPE_CHECK(flat_msg_field_type), field_name);                   \
  OF_PUBLIC INCREASE_STATIC_COUNTER(field_counter);                            \
  ...
  FLAT_MSG_VIEW_CHECK_LAST_FIELD_TYPE(STATIC_COUNTER(field_counter),           \
                                      flat_msg_field_type);                    \
  DSS_DEFINE_FIELD(STATIC_COUNTER(field_counter), "flat message view",         \
                   FlatMsgViewPatternVec<flat_msg_field_type>,                 \
...
public:                                                                        \
  const field_type &field_name(int i) const {                                  \
    return *OF_PP_CAT(field_name, _).at(i);                                    \
  }                                                                            \
  ...
  static void OF_PP_CAT(__CheckLastFieldType__, __LINE__)() {                  \
    static_assert(                                                             \
        !(IsRepeatedPattern<field_index - 1>::value &&                         \
          std::is_same<__FlatMsgViewFieldType__<field_index - 1>::type,        \
                       field_type>::value),                                    \
        "repeated pattern shouldn't be followed by the pattern with same "     \
        "type");                                                               \
  }

#define _SPECIALIZE_IS_REPEATED_PATTERN(field_index)                           \
  template <typename Enabled> struct IsRepeatedPattern<field_index, Enabled> { \
    static const bool value = true;                                            \
  }

template <typename T> struct FlatMsgViewPatternVec {
  using value_type = T;
...
private:
  ...
  Vec *mut_vec() {
    Vec *__attribute__((__may_alias__)) ptr =
        reinterpret_cast<Vec *>(&vec_buffer_);
    return ptr;
  }

  const Vec &vec() const {
    const Vec *__attribute__((__may_alias__)) ptr =
        reinterpret_cast<const Vec *>(&vec_buffer_);
    return *ptr;
  }
...
template <typename FlatMsgViewT, typename FlatMsgOneofField,
          typename OneofValueType>
class FlatMsgViewFieldCtx {
public:
  using flat_msg_view_type = FlatMsgViewT;
  static_assert(std::is_same<OneofValueType,
  ...
  const OneofValueType *GetFlatMsg() const {
    return repeated_flag_msg_ + token_index_;
  }
...
struct FlatMsgViewFieldMatcher {
  static const bool is_repeated_pattern =
      WalkCtxType::flat_msg_view_type::template IsRepeatedPattern<
          field_counter>::value;
  // return true if error occured
  static bool Call(WalkCtxType *ctx, FieldPtrT *field) {
    return _FlatMsgViewFieldMatcher<is_repeated_pattern, WalkCtxType,
                                    FieldPtrT>::Call(ctx, field);
  }
};
...
struct _FlatMsgViewFieldMatcher<false, WalkCtxType, FieldPtrT> {
  ...
  static bool Call(WalkCtxType *ctx, FieldPtrT *field) {
    ...
    if (!oneof->template HasField<FieldType>()) {
      return true;
    }
...
struct FlatMsgViewUtil {
  static_assert(
      std::is_same<ValueType, typename FlatMsgOneofField::struct_type>::value,
      "invalid view match");
  static bool Match(FlatMsgViewT *flat_msg_view, const ValueType *data_ptr,
                    std::size_t size) {
    FlatMsgViewFieldCtx<FlatMsgViewT, FlatMsgOneofField, ValueType> ctx(
        data_ptr, size);
    bool ret =
        !flat_msg_view->template __WalkFieldUntil__<FlatMsgViewFieldMatcher>(
            &ctx);
    if (ret) {
      if (FlatMsgViewT::template IsRepeatedPattern<
              FlatMsgViewT::__LastFieldIndex__>::value) {
...
template <typename FlatMsgViewT, typename ValueType, typename ContainerT,
          typename Enabled = void>
struct FlatMsgViewContainerUtil {
  using FlatMsgOneofField =
      StructField<ValueType, typename ValueType::__OneofType,
                  ValueType::__kDssFieldOffset>;
  static bool Match(FlatMsgViewT *self, const ContainerT &container) {
    return FlatMsgViewUtil<
        FlatMsgViewT, FlatMsgOneofField,
        typename ContainerT::value_type>::Match(self, container.data(),
                                                container.size());
  }
};

template <typename FlatMsgViewT, typename ValueType, typename Enabled>
struct FlatMsgViewContainerUtil<FlatMsgViewT, ValueType,
                                std::vector<FlatMsg<ValueType>>, Enabled> {
  using FlatMsgOneofField =
      StructField<ValueType, typename ValueType::__OneofType,
                  ValueType::__kDssFieldOffset>;
  static_assert(sizeof(ValueType) == sizeof(FlatMsg<ValueType>), "");
  static_assert(alignof(ValueType) == alignof(FlatMsg<ValueType>), "");
  static bool Match(FlatMsgViewT *self,
                    const std::vector<FlatMsg<ValueType>> &container) {
...
template <typename FieldPtrT> struct _FlatMsgViewFieldInit<false, FieldPtrT> {
  static void Call(FieldPtrT *field) {}
};

template <typename FieldPtrT> struct _FlatMsgViewFieldInit<true, FieldPtrT> {
...
template <typename FieldPtrT> struct _FlatMsgViewFieldDelete<false, FieldPtrT> {
  static void Call(FieldPtrT *field) {}
};

template <typename FieldPtrT> struct _FlatMsgViewFieldDelete<true, FieldPtrT> {
  static void Call(FieldPtrT *field) { field->__Delete__(); }
};

template <typename T> struct FlatMsgView final {
  FlatMsgView(const FlatMsgView &) = delete;
  FlatMsgView(FlatMsgView &&) = delete;
  static_assert(T::__is_flat_message_view_type__,
                "T is not a flat message view type");
  ...
  ~FlatMsgView() {
    view_.template __ReverseWalkField__<FlatMsgViewFieldDelete>(&view_);
  }
  ...
  bool Match(const RepeatedFlatMsgT &repeated_flat_msg) {
    ...
    return FlatMsgViewContainerUtil<T, OneofType, RepeatedFlatMsgT>::Match(
        &view_, repeated_flat_msg);
  }
