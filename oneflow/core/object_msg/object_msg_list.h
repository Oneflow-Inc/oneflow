...
#define OBJECT_MSG_DEFINE_LIST_HEAD(elem_type, elem_field_name, field_name)    \
  static_assert(__is_object_message_type__,                                    \
                "this struct is not a object message");                        \
  OF_PRIVATE INCREASE_STATIC_COUNTER(field_counter);                           \
  _OBJECT_MSG_DEFINE_LIST_HEAD(STATIC_COUNTER(field_counter), elem_type,       \
...
#define OBJECT_MSG_LIST_PTR(obj_msg_type, obj_msg_field)                       \
  TrivialObjectMsgList<kDisableSelfLoopLink,                                   \
                       _OBJECT_MSG_LIST_STRUCT_FIELD(obj_msg_type,             \
                                                     obj_msg_field)> *

#define OBJECT_MSG_LIST_FOR_EACH(list_ptr, elem)                               \
  _OBJECT_MSG_LIST_FOR_EACH(std::remove_pointer<decltype(list_ptr)>::type,     \
                            list_ptr, elem)

#define OBJECT_MSG_LIST_FOR_EACH_PTR(list_ptr, elem)                           \
  _OBJECT_MSG_LIST_FOR_EACH_PTR(std::remove_pointer<decltype(list_ptr)>::type, \
                                list_ptr, elem)

#define OBJECT_MSG_LIST_UNSAFE_FOR_EACH_PTR(list_ptr, elem)                    \
  _OBJECT_MSG_LIST_UNSAFE_FOR_EACH_PTR(                                        \
      std::remove_pointer<decltype(list_ptr)>::type, list_ptr, elem)
...
#define _OBJECT_MSG_DEFINE_LIST_HEAD(field_counter, elem_type,                 \
                                     elem_field_name, field_name)              \
  _OBJECT_MSG_DEFINE_LIST_HEAD_FIELD_TYPE(elem_type, elem_field_name,          \
                                          field_name)                          \
  ...
  OBJECT_MSG_DEFINE_LIST_ELEM_STRUCT(field_counter, elem_type,                 \
                                     elem_field_name, field_name);             \
  OBJECT_MSG_DEFINE_LIST_LINK_EDGES(field_counter, elem_type, elem_field_name, \
                                    field_name);                               \
  OBJECT_MSG_OVERLOAD_INIT_WITH_FIELD_INDEX(field_counter,                     \
                                            ObjectMsgEmbeddedListHeadInit);    \
  OBJECT_MSG_OVERLOAD_DELETE(field_counter, ObjectMsgEmbeddedListHeadDelete);

#define _OBJECT_MSG_DEFINE_LIST_HEAD_FIELD_TYPE(elem_type, elem_field_name,    \
                                                field_name)                    \
public:                                                                        \
...
#define _OBJECT_MSG_DEFINE_LIST_HEAD_FIELD(elem_type, elem_field_name,         \
                                           field_name)                         \
public:                                                                        \
...
#define OBJECT_MSG_DEFINE_LIST_ELEM_STRUCT(field_counter, elem_type,           \
                                           elem_field_name, field_name)        \
public:                                                                        \
...
#define OBJECT_MSG_DEFINE_LIST_LINK_EDGES(field_counter, elem_type,            \
                                          elem_field_name, field_name)         \
public:                                                                        \
...
#define _OBJECT_MSG_DEFINE_LIST_LINK(field_counter, field_name)                \
  ...
  DSS_DEFINE_FIELD(field_counter, "object message", EmbeddedListLink,          \
                   OF_PP_CAT(field_name, _));
...
#define _OBJECT_MSG_LIST_FOR_EACH(list_type, list_ptr, elem)                   \
  ...
  EMBEDDED_LIST_FOR_EACH_WITH_EXPR(                                            \
      (StructField<                                                            \
          list_type, EmbeddedListLink,                                         \
          list_type::ContainerLinkOffset()>::FieldPtr4StructPtr(list_ptr)),    \
      list_type::value_link_struct_field, elem_ptr,                            \
...
struct ObjectMsgEmbeddedListLinkInit {
  static void Call(WalkCtxType *ctx, EmbeddedListLink *field) {
    field->__Init__();
  }
};
...
template <bool enable_self_loop_link> struct GetObjectMsgLinkType {};
template <> struct GetObjectMsgLinkType<true> {
  static const ObjectMsgLinkType value = kEnableSelfLoopLink;
};
template <> struct GetObjectMsgLinkType<false> {
...
public:
  ...
  template <int field_index = 0> void __Init__() { list_head_.__Init__(); }
  ...
  void MoveToDstBack(TrivialObjectMsgList *list) {
    list_head_.MoveToDstBack(&list->list_head_);
  }

  void Clear() {
    while (!empty()) {
      ObjectMsgPtrUtil::ReleaseRef(list_head_.PopFront());
    }
  }
...
public:
  ...
  template <int field_index> void __Init__() {
    list_head_.__Init__();
    using ThisInContainer = StructField<
        value_type, TrivialObjectMsgList,
  ...
  void MoveToDstBack(TrivialObjectMsgList *list) {
    while (!empty()) {
      MoveToDstBack(list_head_.Begin(), list);
    }
  }
...
class ObjectMsgList
    : public TrivialObjectMsgList<kDisableSelfLoopLink, LinkField> {
public:
