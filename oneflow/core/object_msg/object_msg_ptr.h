...
public:                                                                        \
  ...
  bool OF_PP_CAT(has_, field_name)() const {                                   \
    return OF_PP_CAT(field_name, _) != nullptr;                                \
  }                                                                            \
  void OF_PP_CAT(set_, field_name)(field_type * val) {                         \
    OF_PP_CAT(field_name, _) = val;                                            \
  }                                                                            \
  void OF_PP_CAT(clear_, field_name)() {                                       \
    OF_PP_CAT(set_, field_name)(nullptr);                                      \
  }                                                                            \
...
struct ObjectMsgRawPtrInit {
  static void Call(WalkCtxType *ctx, PtrFieldType *field) {
    static_assert(std::is_pointer<PtrFieldType>::value,
                  "PtrFieldType is not a pointer type");
    *field = nullptr;
  }
};
