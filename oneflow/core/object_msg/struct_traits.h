...
// details
template <typename T, typename F, int offset> struct StructField {
  using struct_type = T;
  using field_type = F;
...
template <typename X, typename Y> struct ComposeStructField {
  static_assert(
      std::is_same<typename X::field_type, typename Y::struct_type>::value,
      "invalid type");
  using type = StructField<typename X::struct_type, typename Y::field_type,
                           X::offset_value + Y::offset_value>;
};

template <typename T> struct ConstStruct { using type = const T; };
template <typename T> struct ConstStruct<const T> { using type = const T; };

template <typename T> using ConstType = typename ConstStruct<T>::type;

template <typename T> struct ConstRefOrPtrStruct {
  using type = ConstType<T> &;
};

template <typename T> struct ConstRefOrPtrStruct<T *> {
  using type = ConstType<T> *;
};
