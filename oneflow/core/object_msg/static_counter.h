...
#define STATIC_COUNTER(counter_name)                                           \
  _STATIC_COUNTER_NAME(counter_name)<_AUTO_INCREMENT()>::value

#define DEFINE_STATIC_COUNTER(counter_name)                                    \
  _DEFINE_STATIC_COUNTER(_AUTO_INCREMENT(), counter_name)
...
#define _DEFINE_STATIC_COUNTER(auto_counter, counter_name)                     \
  ...
  struct _STATIC_COUNTER_NAME(counter_name) {                                  \
    static const int value =                                                   \
        _STATIC_COUNTER_NAME(counter_name)<tpl_counter - 1>::value;            \
  };                                                                           \
...
#define _INCREASE_STATIC_COUNTER(auto_counter, counter_name)                   \
  ...
  struct _STATIC_COUNTER_NAME(counter_name)<auto_counter, Enabled> {           \
    static const int value =                                                   \
        _STATIC_COUNTER_NAME(counter_name)<auto_counter - 1>::value + 1;       \
  };
} // namespace oneflow
