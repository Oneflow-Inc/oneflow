...
#define OBJECT_MSG_DEFINE_OPTIONAL(field_type, field_name)                     \
  static_assert(__is_object_message_type__,                                    \
                "this struct is not a object message");                        \
  OBJECT_MSG_DEFINE_ONEOF(                                                     \
      OF_PP_CAT(field_name, __object_message_optional_field__),                \
...
#define OBJECT_MSG_ONEOF_CASE(oneof_name)                                      \
  _OBJECT_MSG_ONEOF_ENUM_TYPE(oneof_name)
#define OBJECT_MSG_ONEOF_CASE_VALUE(field_name)                                \
  _OBJECT_MSG_ONEOF_ENUM_VALUE(field_name)
#define OBJECT_MSG_ONEOF_NOT_SET_VALUE(oneof_name)                             \
  _OBJECT_MSG_ONEOF_NOT_SET_VALUE(oneof_name)

// details

#define _OBJECT_MSG_DEFINE_ONEOF_FIELD(field_counter, oneof_name,              \
                                       type_and_field_name_seq)                \
  OF_PP_FOR_EACH_TUPLE(OBJECT_MSG_TYPEDEF_ONEOF_FIELD,                         \
                       type_and_field_name_seq);                               \
  ...
  OBJECT_MSG_DEFINE_ONEOF_ACCESSOR(field_counter, oneof_name,                  \
                                   type_and_field_name_seq);                   \
  OBJECT_MSG_DSS_DEFINE_UION_FIELD(field_counter, oneof_name,                  \
                                   type_and_field_name_seq);

#define OBJECT_MSG_TYPEDEF_ONEOF_FIELD(field_type, field_name)                 \
  using OF_PP_CAT(field_name, _OneofFieldType) =                               \
      typename OBJECT_MSG_STRUCT_MEMBER(field_type);

#define OBJECT_MSG_DSS_DEFINE_UION_FIELD(field_counter, oneof_name,            \
                                         type_and_field_name_seq)              \
  DSS_DEFINE_FIELD(field_counter, "object message",                            \
                   OF_PP_CAT(oneof_name, _UnionStructType),                    \
                   OF_PP_CAT(oneof_name, _));                                  \
  DSS_DEFINE_UNION_FIELD_VISITOR(                                              \
      field_counter, case_,                                                    \
      OF_PP_FOR_EACH_TUPLE(OBJECT_MSG_MAKE_UNION_TYPE7FIELD4CASE,              \
                           type_and_field_name_seq));

#define OBJECT_MSG_MAKE_UNION_TYPE7FIELD4CASE(field_type, field_name)          \
  OF_PP_MAKE_TUPLE_SEQ(OF_PP_CAT(field_name, _OneofFieldType),                 \
                       OF_PP_CAT(field_name, _),                               \
                       _OBJECT_MSG_ONEOF_ENUM_VALUE(field_name))

#define OBJECT_MSG_DEFINE_ONEOF_ACCESSOR(field_counter, oneof_name,            \
                                         type_and_field_name_seq)              \
  _OBJECT_MSG_DEFINE_ONEOF_CASE_ACCESSOR(                                      \
      oneof_name, _OBJECT_MSG_ONEOF_ENUM_TYPE(oneof_name));                    \
  OBJECT_MSG_MAKE_ONEOF_CLEARER(field_counter, oneof_name);                    \
  OF_PP_SEQ_PRODUCT_FOR_EACH_TUPLE(MAKE_OBJECT_MSG_ONEOF_ACCESSOR,             \
...
public:                                                                        \
  ...
  OF_PP_PAIR_FIRST(pair) * OF_PP_CAT(mutable_, OF_PP_PAIR_SECOND(pair))() {    \
    ...
    if (!OF_PP_CAT(has_, OF_PP_PAIR_SECOND(pair))()) {                         \
      ...
      ObjectMsgNaiveInit<ObjectMsgAllocator, FieldType>::Call(mut_allocator(), \
                                                              ptr);            \
...
public:                                                                        \
  ...
  void OF_PP_CAT(reset_, OF_PP_PAIR_SECOND(pair))(const T &rhs) {              \
    static_assert(std::is_same<T, OF_PP_PAIR_FIRST(pair)>::value,              \
                  "error type");                                               \
    static_assert(T::__is_object_message_type__,                               \
                  "object message supported only");                            \
...
public:                                                                        \
  void OF_PP_CAT(clear_, oneof_name)() {                                       \
    __DssVisitField__<field_counter, ObjectMsgField__Delete__, void,           \
                      OF_PP_CAT(oneof_name,                                    \
                                _UnionStructType)>::Call(nullptr,              \
                                                         &OF_PP_CAT(           \
                                                             oneof_name, _));  \
    OF_PP_CAT(set_, OF_PP_CAT(oneof_name, _case))                              \
    (_OBJECT_MSG_ONEOF_NOT_SET_VALUE(oneof_name));                             \
...
public:                                                                        \
  const OF_PP_PAIR_FIRST(pair) & OF_PP_PAIR_SECOND(pair)() const {             \
    if (OF_PP_CAT(has_, OF_PP_PAIR_SECOND(pair))()) {                          \
      return GetterTrait<                                                      \
          std::is_base_of<ObjectMsgStruct, OF_PP_PAIR_FIRST(pair)>::value>::   \
          Call(                                                                \
              OF_PP_CAT(oneof_name, _).OF_PP_CAT(OF_PP_PAIR_SECOND(pair), _)); \
    }                                                                          \
    return ObjectMsgGetDefault<                                                \
        std::is_base_of<ObjectMsgStruct, OF_PP_PAIR_FIRST(pair)>::value>::     \
...
private:                                                                       \
  struct OF_PP_CAT(oneof_name, _UnionStructType) {                             \
    union {                                                                    \
      OF_PP_FOR_EACH_TUPLE(MAKE_OBJECT_MSG_ONEOF_UNION_FIELD,                  \
                           type_and_field_name_seq)                            \
    };                                                                         \
...
public:                                                                        \
  enum _OBJECT_MSG_ONEOF_ENUM_TYPE(oneof_name) {                               \
    ...
    OF_PP_FOR_EACH_TUPLE(MAKE_OBJECT_MSG_ONEOF_ENUM_CASE,                      \
                         type_and_field_name_seq)                              \
  }
...
#define _OBJECT_MSG_ONEOF_ENUM_TYPE(oneof_name)                                \
  OF_PP_CAT(oneof_name, _OneofEnumType)
#define _OBJECT_MSG_ONEOF_ENUM_VALUE(field) OF_PP_CAT(field, _OneofEnumValue)
#define _OBJECT_MSG_ONEOF_NOT_SET_VALUE(oneof_name)                            \
  OF_PP_CAT(k_, OF_PP_CAT(oneof_name, _NotSet))

#define OBJECT_MSG_STRUCT_MEMBER(class_name)                                   \
  std::conditional<ObjectMsgIsScalar<class_name>::value, class_name,           \
                   class_name *>::type

#define OBJECT_MSG_DEFINE_DEFAULT(object_message_type_name)                    \
public:                                                                        \
  const object_message_type_name &__Default__() const {                        \
    static const ObjectMsgStructDefault<object_message_type_name>              \
        default_object_msg;                                                    \
...
public:                                                                        \
  ...
  ObjectMsgAllocator *mut_allocator() const {                                  \
    return __object_msg_base__.mut_allocator();                                \
  }                                                                            \
                                                                               \
private:                                                                       \
  ObjectMsgBase __object_msg_base__;                                           \
  OF_PRIVATE INCREASE_STATIC_COUNTER(field_counter);                           \
  DSS_DEFINE_FIELD(STATIC_COUNTER(field_counter), "object message",            \
                   ObjectMsgBase, __object_msg_base__);
...
public:                                                                        \
  template <typename WalkCtxType> void ObjectMsg__Init__(WalkCtxType *ctx) {   \
...
private:                                                                       \
  ...
  struct ObjectMsgField__Init__                                                \
      : public ObjectMsgNaiveInit<WalkCtxType, PtrFieldType> {};

#define OBJECT_MSG_OVERLOAD_LINK_EDGES_GETTER(field_counter,                   \
                                              link_edges_getter)               \
public:                                                                        \
  template <typename Enabled>                                                  \
  struct LinkEdgesGetter<field_counter, Enabled> final                         \
      : public link_edges_getter {};
...
#define OBJECT_MSG_OVERLOAD_INIT_WITH_FIELD_INDEX(field_counter,               \
                                                  init_template)               \
private:                                                                       \
  template <typename WalkCtxType, typename PtrFieldType>                       \
  struct ObjectMsgField__Init__<field_counter, WalkCtxType, PtrFieldType>      \
...
public:                                                                        \
  void ObjectMsg__Delete__() {                                                 \
    this->__Delete__();                                                        \
    this->template __ReverseWalkField__<ObjectMsgField__Delete__, void>(       \
        nullptr);                                                              \
  }                                                                            \
                                                                               \
private:                                                                       \
  template <int field_counter, typename WalkCtxType, typename PtrFieldType>    \
  struct ObjectMsgField__Delete__                                              \
      : public ObjectMsgNaiveDelete<WalkCtxType, PtrFieldType> {};
...
template <typename T, typename Enabled = void> struct ObjectMsgSelfType {
  static_assert(T::__is_object_message_type__,
                "T is not a object message type");
  using type = T;
};
...
template <typename T> class ObjectMsgPtr;
...
struct ObjectMsgPtrUtil final {
  ...
  template <typename T> static void InitRef(T *ptr) {
    ptr->__mut_object_msg_base__()->InitRefCount();
  }
  template <typename T> static void Ref(T *ptr) {
    ptr->__mut_object_msg_base__()->IncreaseRefCount();
  }
  template <typename T> static void ReleaseRef(T *ptr) {
    CHECK_NOTNULL(ptr);
...
template <typename T> struct ObjectMsgStructDefault final {
...
template <bool is_object_msg> struct ObjectMsgGetDefault final {
  template <typename T> static const T &Call(T *const *val) {
    return (*val)->__Default__();
  }
};
template <> struct ObjectMsgGetDefault<false> final {
  template <typename T> static const T &Call(T const *val) { return *val; }
};

template <bool is_pointer> struct _ObjectMsgNaiveInit {
  template <typename WalkCtxType, typename PtrFieldType>
...
struct ObjectMsgNaiveInit {
  static void Call(WalkCtxType *ctx, PtrFieldType *field) {
    ...
    _ObjectMsgNaiveInit<is_ptr>::template Call<WalkCtxType, PtrFieldType>(
        ctx, field);
  }
};

template <> struct _ObjectMsgNaiveInit<true> {
  template <typename WalkCtxType, typename PtrFieldType>
  static void Call(WalkCtxType *ctx, PtrFieldType *field_ptr) {
    static_assert(std::is_pointer<PtrFieldType>::value,
                  "invalid use of _ObjectMsgNaiveInit");
...
template <bool is_pointer> struct _ObjectMsgNaiveDelete {
  template <typename WalkCtxType, typename PtrFieldType>
  static void Call(WalkCtxType *ctx, PtrFieldType *field) {}
};
...
struct ObjectMsgNaiveDelete {
  static void Call(WalkCtxType *ctx, PtrFieldType *field) {
    static const bool is_ptr = std::is_pointer<PtrFieldType>::value;
    _ObjectMsgNaiveDelete<is_ptr>::template Call<WalkCtxType, PtrFieldType>(
        ctx, field);
  }
};

template <> struct _ObjectMsgNaiveDelete<true> {
  template <typename WalkCtxType, typename PtrFieldType>
  static void Call(WalkCtxType *ctx, PtrFieldType *field) {
    static_assert(std::is_pointer<PtrFieldType>::value,
                  "invalid use of _ObjectMsgNaiveDelete");
    using FieldType = typename std::remove_pointer<PtrFieldType>::type;
    static_assert(std::is_base_of<ObjectMsgStruct, FieldType>::value,
                  "FieldType is not a subclass of ObjectMsgStruct");
    auto *ptr = *field;
...
template <typename T> class ObjectMsgPtr final {
public:
  ...
  const value_type &operator*() const { return *ptr_; }
  bool operator==(const ObjectMsgPtr &rhs) const {
  ...
  template <typename... Args> static ObjectMsgPtr New(Args &&... args) {
  ...
  static ObjectMsgPtr NewFrom(ObjectMsgAllocator *allocator, Args &&... args) {
    ObjectMsgPtr ret;
    ObjectMsgNaiveInit<ObjectMsgAllocator, value_type *>::Call(allocator,
                                                               &ret.ptr_);
...
template <typename T> struct ObjectMsgIsScalar {
  const static bool value = std::is_arithmetic<T>::value ||
                            std::is_enum<T>::value ||
