...
#define OBJECT_MSG_DEFINE_FLAT_MSG(field_type, field_name)                     \
  ...
  _OBJECT_MSG_DEFINE_FLAT_MSG(STATIC_COUNTER(field_counter), field_type,       \
                              field_name);

// details

#define _OBJECT_MSG_DEFINE_FLAT_MSG(field_counter, field_type, field_name)     \
  _OBJECT_MSG_DEFINE_FLAT_MSG_FIELD(FLAT_MSG_TYPE_CHECK(field_type),           \
                                    field_name)                                \
  ...
  DSS_DEFINE_FIELD(field_counter, "object message", field_type,                \
...
public:                                                                        \
  ...
  bool OF_PP_CAT(has_, field_name)() {                                         \
    return ObjectMsgFlatMsgHas<std::is_enum<field_type>::value,                \
                               field_type>::Call(OF_PP_CAT(field_name, _));    \
  }                                                                            \
  ...
  field_type *OF_PP_CAT(mut_, field_name)() {                                  \
...
template <bool is_enum, typename T> struct ObjectMsgFlatMsgHas {
  static bool Call(const T &val) { return true; }
};

template <typename T> struct ObjectMsgFlatMsgHas<true, T> {
  static bool Call(const T &val) { return val == static_cast<T>(0); }
};

template <bool is_enum, typename T> struct ObjectMsgFlatMsgClear {
  static void Call(T *ptr) { ptr->clear(); }
};

template <typename T> struct ObjectMsgFlatMsgClear<true, T> {
