...
inline ObjectMsgUnionFieldList *
FindExistedUnionFieldList(ObjectMsgFieldList *obj_msg_field_list,
  ...
  if (!last->has_union_field_list()) {
    return nullptr;
  }
  if (last->union_field_list().union_name() != oneof_name) {
    return nullptr;
  }
...
struct StaticDumpObjectMsgFieldName {
  static void Call(ObjectMsgFieldList *obj_msg_field_list,
                   const char *field_name, const char *oneof_name) {
    std::string field_name_str(field_name);
    if (!is_oneof_field) {
      auto *struct_field = obj_msg_field_list->mutable_object_msg_field()
                               ->Add()
                               ->mutable_struct_field();
      struct_field->set_field_type(
          StructT::template __DssFieldTypeId__<field_counter,
                                               FieldType>::Call());
    ...
    if (union_field_list == nullptr) {
      union_field_list = obj_msg_field_list->mutable_object_msg_field()
                             ->Add()
                             ->mutable_union_field_list();
...
struct StaticRecursivelyDumpObjectMsgContainerElemFieldName final {
  static void Call(std::unordered_map<std::string, ObjectMsgFieldList>
...
struct StaticRecursivelyDumpObjectMsgFieldName {
  static void Call(std::unordered_map<std::string, ObjectMsgFieldList>
                   ...
                   const char *field_name, const char *oneof_name) {
    using elem_type =
        typename StructT::template ContainerElemStruct<field_counter>::type;
...
struct _StaticRecursivelyDumpObjectMsgFieldName<FieldType, true> {
  static void Call(std::unordered_map<std::string, ObjectMsgFieldList>
    ...
    if (map.find(typeid(ObjectMsgFieldType).name()) != map.end()) {
      return;
    }
...
template <typename StructT, int field_counter, typename WalkCtxType,
          typename FieldType>
struct StaticRecursivelyDumpObjectMsgFieldName<StructT, field_counter,
                                               WalkCtxType, FieldType, true> {
  static void Call(std::unordered_map<std::string, ObjectMsgFieldList>
                       *mangled_type_name2obj_msg_field_list,
                   const char *field_name, const char *oneof_name) {
    _StaticRecursivelyDumpObjectMsgFieldName<
        FieldType, std::is_pointer<FieldType>::value>::
        Call(mangled_type_name2obj_msg_field_list);
  }
};

template <typename T>
void ObjectMsgReflection<T>::ReflectObjectMsgFields(
    ObjectMsgFieldList *obj_msg_field_list) const {
  T::template __WalkStaticVerboseField__<StaticDumpObjectMsgFieldName>(
      obj_msg_field_list);
}
...
void ObjectMsgReflection<T>::RecursivelyReflectObjectMsgFields(
    std::unordered_map<std::string, ObjectMsgFieldList>
        *mangled_type_name2obj_msg_field_list) const {
  const auto &map = *mangled_type_name2obj_msg_field_list;
  if (map.find(typeid(T).name()) != map.end()) {
    return;
  }
  auto *obj_msg_field_list =
      &(*mangled_type_name2obj_msg_field_list)[typeid(T).name()];
  T::template __WalkStaticVerboseField__<StaticDumpObjectMsgFieldName>(
      obj_msg_field_list);
  T::template __WalkStaticVerboseField__<
      StaticRecursivelyDumpObjectMsgFieldName>(
      mangled_type_name2obj_msg_field_list);
}

template <typename StructT, int field_counter, typename WalkCtxType,
          typename FieldType, bool is_oneof>
struct StaticDumpObjectMsgLinkEdges {
  static void Call(std::set<ObjectMsgContainerLinkEdge> *edges, const char *,
                   const char *) {
...
struct StaticRecursivelyDumpObjectMsgLinkEdge {
  static void Call(std::set<ObjectMsgContainerLinkEdge> *edges,
    ...
    using elem_type =
        typename StructT::template ContainerElemStruct<field_counter>::type;
    StaticRecursivelyDumpObjectMsgContainerElemLinkEdges<elem_type>::Call(
        edges);
  }
};
...
struct _StaticRecursivelyDumpObjectMsgLinkEdge<FieldType, true> {
  static void Call(std::set<ObjectMsgContainerLinkEdge> *edges) {
    using ObjectMsgFieldType = typename std::remove_pointer<FieldType>::type;
    ObjectMsgReflection<ObjectMsgFieldType>().RecursivelyReflectLinkEdges(
        edges);
  }
};
...
template <typename StructT, int field_counter, typename WalkCtxType,
          typename FieldType>
struct StaticRecursivelyDumpObjectMsgLinkEdge<StructT, field_counter,
                                              WalkCtxType, FieldType, true> {
  static void Call(std::set<ObjectMsgContainerLinkEdge> *edges,
                   const char *field_name, const char *oneof_name) {
    _StaticRecursivelyDumpObjectMsgLinkEdge<
        FieldType, std::is_pointer<FieldType>::value>::Call(edges);
  }
};
template <typename StructT, int field_counter, typename WalkCtxType,
          typename FieldType, bool is_oneof>
...
template <typename T>
void ObjectMsgReflection<T>::ReflectLinkEdges(
    std::set<ObjectMsgContainerLinkEdge> *edges) const {
...
void ObjectMsgReflection<T>::RecursivelyReflectLinkEdges(
  ...
  T::template __WalkStaticVerboseField__<StaticDumpObjectMsgLinkEdges>(edges);
  T::template __WalkStaticVerboseField__<
      StaticRecursivelyDumpObjectMsgLinkEdge>(edges);
}
...
std::string
ObjectMsgListReflection<Args...>::ToDot(const std::string &digraph_name) const {
  using Type2FieldList = std::unordered_map<std::string, ObjectMsgFieldList>;
  Type2FieldList type_name2field_list;
  ObjectMsgListReflectionObjectMsgFields<Type2FieldList, Args...>(
      &type_name2field_list);
