...
#define OBJECT_MSG_DEFINE_STRUCT(field_type, field_name)                       \
  ...
  _OBJECT_MSG_DEFINE_STRUCT(STATIC_COUNTER(field_counter), field_type,         \
                            field_name);
...
#define _OBJECT_MSG_DEFINE_STRUCT(field_counter, field_type, field_name)       \
  ...
  DSS_DEFINE_FIELD(field_counter, "object message",                            \
                   OF_PP_CAT(field_name, _ObjectMsgFieldType),                 \
...
public:                                                                        \
  ConstType<field_type> &field_name() const {                                  \
    ConstType<field_type> *__attribute__((__may_alias__)) ptr =                \
        reinterpret_cast<ConstType<field_type> *>(                             \
            &OF_PP_CAT(field_name, _).data[0]);                                \
    return *ptr;                                                               \
  }                                                                            \
  field_type *OF_PP_CAT(mutable_, field_name)() {                              \
    return OF_PP_CAT(field_name, _).mut_data();                                \
  }                                                                            \
  field_type *OF_PP_CAT(mut_, field_name)() {                                  \
    return OF_PP_CAT(mutable_, field_name)();                                  \
  }                                                                            \
  template <typename T> void OF_PP_CAT(set_, field_name)(T val) {              \
    static_assert(std::is_scalar<T>::value,                                    \
                  "only scalar data type supported");                          \
...
private:                                                                       \
  ...
  public:                                                                      \
    ...
    data_type *mut_data() {                                                    \
      data_type *__attribute__((__may_alias__)) ptr =                          \
          reinterpret_cast<data_type *>(&data[0]);                             \
      return ptr;                                                              \
    }                                                                          \
...
template <typename WalkCtxType, typename FieldType> struct ObjectMsgStructInit {
