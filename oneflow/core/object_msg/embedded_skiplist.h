...
template <int max_level> struct EmbeddedSkipListLink final {
public:
  ...
  void InsertAfter(EmbeddedSkipListLink *prev_skiplist_link, int levels) {
    ...
    for (; i < levels; ++i, ++prev_link) {
      while (prev_link->nullptr_empty()) {
        prev_link = (prev_link - 1)->prev() + 1;
      }
  ...
  static EmbeddedSkipListLink *ThisPtr4LinkPtr(EmbeddedListLink *slist_ptr,
                                               int level) {
    auto *links_ptr =
        (std::array<EmbeddedListLink, max_level> *)(slist_ptr - level);
    return StructField<self_type, decltype(links_),
                       LinksOffset()>::StructPtr4FieldPtr(links_ptr);
  }
...
private:
  template <typename Enabled = void> static constexpr int LinksOffset() {
...
template <typename T, int N = 20> struct EmbeddedSkipListKey {
public:
  ...
  template <typename Enabled = void>
  static constexpr int LevelZeroLinkOffset() {
    return offsetof(EmbeddedSkipListKey, link_) +
           link_type::LevelZeroLinkOffset();
  }
  ...
  const T &key() const {
    const T *__attribute__((__may_alias__)) ptr =
        reinterpret_cast<const T *>(&key_buffer_[0]);
    return *ptr;
  }
  T *mut_key() {
    T *__attribute__((__may_alias__)) ptr =
        reinterpret_cast<T *>(&key_buffer_[0]);
    return ptr;
  }
  ...
  static self_type *Find(const key_type &key, link_type *head, int size_shift) {
    EmbeddedListLink *last_link_less_than_key =
        SearchLastBottomLinkLessThan(key, head, size_shift);
    if (last_link_less_than_key->next() == head->mutable_link(0)) {
      return nullptr;
    }
  ...
  static self_type *Erase(const key_type &key, link_type *head,
                          int size_shift) {
  ...
  static std::pair<self_type *, bool> Insert(self_type *elem, link_type *head,
                                             int size_shift) {
    EmbeddedListLink *prev_list_link =
        SearchLastBottomLinkLessThan(elem->key(), head, size_shift);
    ...
    // CHECK_EQ(Find(ret_elem->key(), head), ret_elem, GetMaxVal<int32_t>() /
    // 2);
    return std::make_pair(ret_elem, success);
  }
  static EmbeddedSkipListKey *ThisPtr4LinkPtr(EmbeddedListLink *list_link_ptr,
                                              int level) {
    auto *skip_list_ptr = link_type::ThisPtr4LinkPtr(list_link_ptr, level);
    using FieldUtil =
        StructField<self_type, link_type, SkipListIteratorOffset()>;
    return FieldUtil::StructPtr4FieldPtr(skip_list_ptr);
  }

private:
  template <bool is_scalar, typename Enabled = void> struct KeyInitializer {
    static void Call(T *key) {}
  };
  template <typename Enabled> struct KeyInitializer<false, Enabled> {
  ...
  static int32_t RandomNumLevels(int size_shift) {
    ...
    for (int i = 1; (rand() % 2 == 0) && i < max_num_levels; ++i) {
      ++num_levels;
    }
    return num_levels;
  }

  static EmbeddedListLink *SearchLastBottomLinkLessThan(const key_type &key,
                                                        link_type *head,
                                                        int size_shift) {
...
template <typename ValueLinkField> class EmbeddedSkipListHead {
public:
  ...
  using value_level0_link_struct_field =
      typename ComposeStructField<ValueLinkField,
                                  value_key_level0_link_struct_field>::type;
  ...
  value_type *Begin() {
    ...
    if (begin_list_link == head_level0) {
      return nullptr;
    }
  ...
  value_type *Find(const key_type &key) {
    auto *key_link_ptr =
        key_link_type::Find(key, &skiplist_head_, size_shift());
    if (key_link_ptr == nullptr) {
      return nullptr;
    }
  ...
  const value_type *Find(const key_type &key) const {
    auto *key_link_ptr = key_link_type::Find(
        key, const_cast<EmbeddedSkipListLink<max_level> *>(&skiplist_head_),
        size_shift());
    if (key_link_ptr == nullptr) {
      return nullptr;
    }
  ...
  value_type *Erase(const key_type &key) {
    key_link_type *erased =
        key_link_type::Erase(key, &skiplist_head_, size_shift());
    --size_;
  ...
  std::pair<value_type *, bool> Insert(value_type *elem) {
    ...
    return std::make_pair(ValueLinkField::StructPtr4FieldPtr(ret_key_link),
                          success);
  }

  template <typename Callback> void Clear(const Callback &cb) {
    using link_type = EmbeddedSkipListLink<max_level>;
    for (; size_ > 0; --size_) {
      EmbeddedListLink *begin_list_link =
          skiplist_head_.mutable_link(0)->next();
      auto *begin = link_type::ThisPtr4LinkPtr(begin_list_link, 0);
      if (begin == &skiplist_head_) {
        break;
      }
      begin->Erase();
