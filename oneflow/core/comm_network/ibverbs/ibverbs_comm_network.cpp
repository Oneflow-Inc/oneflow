...
void IBVForkInit() {
  if (ibv_fork_init() != 0) {
    LOG(ERROR) << "ibv_fork_init failed";
  }
}
...
void IBVerbsCommNet::RegisterMemoryDone() {
  ...
  for (int64_t peer_id : peer_machine_id()) {
    ...
    Global<CtrlClient>::Get()->PullKV(GenTokensMsgKey(peer_id),
                                      &peer_tokens_msg);
...
IBVerbsCommNet::IBVerbsCommNet(const Plan &plan)
  ...
  int64_t this_machine_id = Global<MachineCtx>::Get()->this_machine_id();
  qp_vec_.assign(Global<ResourceDesc, ForSession>::Get()->TotalMachineNum(),
                 nullptr);
  for (int64_t peer_id : peer_machine_id()) {
    ...
    Global<CtrlClient>::Get()->PushKV(GenConnInfoKey(this_machine_id, peer_id),
                                      conn_info);
  }
  for (int64_t peer_id : peer_machine_id()) {
    IBVerbsConnectionInfo conn_info;
    Global<CtrlClient>::Get()->PullKV(GenConnInfoKey(peer_id, this_machine_id),
                                      &conn_info);
  ...
  for (int64_t peer_id : peer_machine_id()) {
    ...
    Global<CtrlClient>::Get()->ClearKV(
        GenConnInfoKey(this_machine_id, peer_id));
  }
  OF_BARRIER();
