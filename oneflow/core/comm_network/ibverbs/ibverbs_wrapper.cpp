...
ibvResult_t wrap_ibv_symbols(void) {
  if (ibvState == ibvInitialized) return ibvSuccess;
  if (ibvState == ibvError) return ibvSystemError;
  ...
  if (__sync_bool_compare_and_swap(&ibvState, ibvUninitialized, ibvInitializing) == false) {
    ...
    return (ibvState == ibvInitialized) ? ibvSuccess : ibvSystemError;
  }
...
ibvResult_t wrap_ibv_fork_init() {
  IBV_INT_CHECK(ibv_internal_fork_init, ibv_internal_fork_init(), -1, "ibv_fork_init");
}

ibvResult_t wrap_ibv_get_device_list(struct ibv_device ***ret, int *num_devices) {
  *ret = ibv_internal_get_device_list(num_devices);
  if (*ret == NULL) *num_devices = 0;
  return ibvSuccess;
}

ibvResult_t wrap_ibv_free_device_list(struct ibv_device **list) {
...
ibvResult_t wrap_ibv_open_device(
...
ibvResult_t wrap_ibv_close_device(
...
ibvResult_t wrap_ibv_get_async_event(
...
ibvResult_t wrap_ibv_ack_async_event(struct ibv_async_event *event) {
  IBV_PASSTHRU(ibv_internal_ack_async_event, ibv_internal_ack_async_event(event));
}

ibvResult_t wrap_ibv_query_device(
    struct ibv_context *context,
...
ibvResult_t wrap_ibv_query_port(
...
ibvResult_t wrap_ibv_query_gid(struct ibv_context *context, uint8_t port_num, int index,
                               union ibv_gid *gid) {
...
ibvResult_t wrap_ibv_reg_mr(struct ibv_mr **ret, struct ibv_pd *pd, void *addr, size_t length,
                            int access) {
...
ibvResult_t wrap_ibv_create_cq(struct ibv_cq **ret, struct ibv_context *context, int cqe,
...
ibvResult_t wrap_ibv_event_type_str(char **ret, enum ibv_event_type event) {
