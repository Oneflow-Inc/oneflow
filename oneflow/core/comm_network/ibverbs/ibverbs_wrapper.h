...
ibvResult_t wrap_ibv_fork_init(void);
...
ibvResult_t wrap_ibv_open_device(struct ibv_context **ret, struct ibv_device *device);
ibvResult_t wrap_ibv_close_device(struct ibv_context *context);
ibvResult_t wrap_ibv_get_async_event(struct ibv_context *context, struct ibv_async_event *event);
ibvResult_t wrap_ibv_ack_async_event(struct ibv_async_event *event);
ibvResult_t wrap_ibv_query_device(struct ibv_context *context, struct ibv_device_attr *device_attr);
ibvResult_t wrap_ibv_query_port(struct ibv_context *context, uint8_t port_num,
...
ibvResult_t wrap_ibv_dealloc_pd(struct ibv_pd *pd);
ibvResult_t wrap_ibv_reg_mr(struct ibv_mr **ret, struct ibv_pd *pd, void *addr, size_t length,
                            int access);
struct ibv_mr *wrap_direct_ibv_reg_mr(struct ibv_pd *pd, void *addr, size_t length, int access);
ibvResult_t wrap_ibv_dereg_mr(struct ibv_mr *mr);
ibvResult_t wrap_ibv_create_comp_channel(struct ibv_comp_channel **ret,
                                         struct ibv_context *context);
ibvResult_t wrap_ibv_destroy_comp_channel(struct ibv_comp_channel *channel);
...
static inline ibvResult_t wrap_ibv_poll_cq(struct ibv_cq *cq, int num_entries, struct ibv_wc *wc,
                                           int *num_done) {
  int done = cq->context->ops.poll_cq(
      cq, num_entries,
      wc); /*returns the number of wcs or 0 on success, a negative number otherwise*/
  if (done < 0) {
    LOG(WARNING) << "Call to ibv_poll_cq() returned " << done;
...
ibvResult_t wrap_ibv_modify_qp(struct ibv_qp *qp, struct ibv_qp_attr *attr, int attr_mask);
...
static inline ibvResult_t wrap_ibv_post_send(struct ibv_qp *qp, struct ibv_send_wr *wr,
                                             struct ibv_send_wr **bad_wr) {
  int ret = qp->context->ops.post_send(qp, wr, bad_wr); /*returns 0 on success, or the value of
                                                           errno on failure (which indicates the
                                                           failure reason)*/
  if (ret != IBV_SUCCESS) {
    LOG(WARNING) << "ibv_post_send() failed with error " << strerror(ret);
    return ibvSystemError;
  }
  return ibvSuccess;
}

static inline ibvResult_t wrap_ibv_post_recv(struct ibv_qp *qp, struct ibv_recv_wr *wr,
                                             struct ibv_recv_wr **bad_wr) {
  int ret = qp->context->ops.post_recv(qp, wr, bad_wr); /*returns 0 on success, or the value of
                                                           errno on failure (which indicates the
                                                           failure reason)*/
  if (ret != IBV_SUCCESS) {
    LOG(WARNING) << "ibv_post_recv() failed with error " << strerror(ret);
    return ibvSystemError;
  }
  return ibvSuccess;
}
