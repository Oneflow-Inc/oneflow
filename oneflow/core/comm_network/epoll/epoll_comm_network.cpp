...
int SockListen(int listen_sockfd, uint16_t listen_port,
               int32_t total_machine_num) {
  sockaddr_in sa = GetSockAddr("0.0.0.0", listen_port);
  int bind_result =
      bind(listen_sockfd, reinterpret_cast<sockaddr *>(&sa), sizeof(sa));
  if (bind_result == 0) {
...
int64_t GetMachineId(const sockaddr_in &sa) {
  ...
  for (int64_t i = 0;
       i < Global<ResourceDesc, ForSession>::Get()->TotalMachineNum(); ++i) {
    if (Global<ResourceDesc, ForSession>::Get()->machine(i).addr() == addr) {
      return i;
    }
  }
  UNIMPLEMENTED();
...
EpollCommNet::~EpollCommNet() {
  ...
  OF_BARRIER();
  for (IOEventPoller *poller : pollers_) {
    delete poller;
  }
  for (auto &pair : sockfd2helper_) {
    delete pair.second;
  }
}
...
EpollCommNet::EpollCommNet(const Plan &plan) : CommNetIf(plan) {
  pollers_.resize(Global<ResourceDesc, ForSession>::Get()->CommNetWorkerNum(),
                  nullptr);
  for (size_t i = 0; i < pollers_.size(); ++i) {
...
void EpollCommNet::InitSockets() {
  ...
  auto this_machine =
      Global<ResourceDesc, ForSession>::Get()->machine(this_machine_id);
  int64_t total_machine_num =
  ...
  if (this_listen_port != -1) {
    ...
    PushPort(this_machine_id, ((this_machine.data_port_agent() != -1)
                                   ? (this_machine.data_port_agent())
                                   : (this_listen_port)));
  } else {
    for (this_listen_port = 1024; this_listen_port < GetMaxVal<uint16_t>();
         ++this_listen_port) {
