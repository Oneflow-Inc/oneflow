...
CommNet::~CommNet() {
  for (int64_t stream_id = 0; stream_id < peer_machine_id_.size(); ++stream_id) {
    CHECK(stream_id2stream.at(stream_id).empty());
  }
  ready_cbs_.Close();
...
void CommNet::ReadDone(int64_t stream_id) {
  auto& local_stream = stream_id2stream_.at(stream_id);
  {
    ...
    if (!local_stream.empty()) {
      auto item = local_strem.front();
      CHECK(item.is_read);
      IssueCalBack(item.callback);
    }
  }
}
...
CommNet::CommNet(const Plan& plan) {
  ...
  for (const auto& task : plan.task()) {
    ...
    stream_ids.emplace(Global<IDMgr>::Get()->LocalWorkStreamId4TaskId(task.task.id()));
  }
