...
#ifndef ONEFLOW_CORE_DISTRIBUTED_RUNTIME_RPC_GRPC_CALL_H_
#define ONEFLOW_CORE_DISTRIBUTED_RUNTIME_RPC_GRPC_CALL_H_
  ...
  class Tag {
   public:
    ...
    void OnCompleted(Service* service, bool ok) {
      switch (callback_) {
        case kRequestReceived: call_->RequestReceived(service, ok); break;
        case kResponseSent:
          // No special handling needed apart from the Unref below.
          break;
        case kCancelled: call_->RequestCancelled(service, ok); break;
      }
 ...
 public:
  ...
  static void EnqueueRequest(GrpcService* grpc_service,
    ...
    auto call = new Call<Service, GrpcService, RequestMessage, ResponseMessage>(
        handle_request_function);
    if (supports_cancel) { call->RegisterCancellationHandler(); }
...
#endif  // ONEFLOW_CORE_DISTRIBUTED_RUNTIME_RPC_GRPC_CALL_H_
