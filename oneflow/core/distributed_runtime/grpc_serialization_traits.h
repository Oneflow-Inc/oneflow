#ifndef ONEFLOW_CORE_DISTRIBUTED_RUNTIME_GRPC_SERIALIZATION_TRAITS_H_
#define ONEFLOW_CORE_DISTRIBUTED_RUNTIME_GRPC_SERIALIZATION_TRAITS_H_

#include "grpc++/impl/codegen/proto_utils.h"

namespace grpc {
namespace oneflow_helper {
...
class UnlimitedSizeProtoSerializationTraits {
 public:
    ...
    *own_buffer = true;
    ...
    if (byte_size < 0) {
      return Status(StatusCode::INTERNAL, "Message length was negative");
        ...
        oneflow_helper::kGrpcBufferWriterMaxBufferLength) {
      ...
      *bp = g_core_codegen_interface->grpc_raw_byte_buffer_create(&slice, 1);
      ...
      oneflow_helper::GrpcBufferWriter writer(
          bp, oneflow_helper::kGrpcBufferWriterMaxBufferLength);
  ...
  static Status Deserialize(grpc_byte_buffer* buffer, T* msg,
    ...
    {
      oneflow_helper::GrpcBufferReader reader(buffer);
...
#define OF_GRPC_ALLOW_UNLIMITED_MESSAGE_SIZE(MessageType)               \
  namespace grpc {                                                      \
    template <>                                                         \
...
#endif  // ONEFLOW_CORE_DISTRIBUTED_RUNTIME_GRPC_SERIALIZATION_TRAITS_H_
