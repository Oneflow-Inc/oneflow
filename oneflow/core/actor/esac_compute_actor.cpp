...
void EsacCompActor::VirtualCompActorInit(const TaskProto &task_proto) {
  ...
  const int32_t input_bns_size = task_proto.exec_sequence()
                                     .exec_node()
                                     .Get(0)
                                     .kernel_conf()
                                     .op_attribute()
  ...
  FOR_RANGE(int64_t, i, 0, input_bns_size) {
    const int64_t regst_desc_id =
        exec_kernel_vec().at(0).bn_in_op2regst_desc_id.at(
            GenRepeatedBn("in", i));
...
void EsacCompActor::NormalProcessCustomizedReadableRegstMsg(
    const ActorMsg &msg) {
  CHECK_EQ(0, consumed_rs_.TryPushBackRegst(msg.regst()));
}

bool EsacCompActor::IsCustomizedReadReady() const {
...
bool EsacCompActor::ProducedCtrlRegstValid(int64_t regst_desc_id) const {
  return true;
}
