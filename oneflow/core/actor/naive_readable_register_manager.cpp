...
void NaiveReadableRegstMgr::ForEachCurReadableRegst(std::function<void(Regst*)> func) {
  for (const auto& pair : readable_regst_) {
    if (pair.second.empty() == false) { func(pair.second.front()); }
  }
}

bool NaiveReadableRegstMgr::IsReadReady() { return readable_regst_.size() == readable_regst_cnt_; }
