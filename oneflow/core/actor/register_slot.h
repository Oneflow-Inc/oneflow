...
public:
  ...
  RegstSlot()
      : regst_desc_id2regsts_(), available_regst_desc_cnt_(0),
        is_inited_(false) {}
  ~RegstSlot() = default;
  ...
  bool IsCurSlotReady() const {
    return available_regst_desc_cnt() == total_regst_desc_cnt();
  }
  ...
  void ForChosenFrontRegst(std::function<bool(int64_t)>,
                           std::function<void(Regst *)>) const;
  void
  ForChosenRegstDeq(std::function<bool(int64_t)>,
