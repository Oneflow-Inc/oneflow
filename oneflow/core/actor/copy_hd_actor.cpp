...
void CopyHdActor::InitDeviceCtx(const ThreadCtx& thread_ctx) {
  ...
  mut_device_ctx().reset(
      new CudaDeviceCtx(GetReservedWorkStreamId(0), thread_ctx.copy_hd_cuda_stream));
}
...
int CopyHdActor::HandlerNormal(const ActorMsg& msg) {
  if (msg.msg_type() == ActorMsgType::kEordMsg) {
    ...
    if (TryUpdtStateAsProducedRegst(msg.regst()) != 0) { pending_in_regst_.push(msg.regst()); }
    ActUntilFail();
  } else {
    UNIMPLEMENTED();
  }
...
void CopyHdActor::Act() {
  ...
  AsyncLaunchKernel(GenDefaultKernelCtx(), [&](int64_t regst_desc_id) -> Regst* {
...
bool CopyHdActor::IsReadAlwaysUnReadyFromNow() { return is_in_eord_ && pending_in_regst_.empty(); }

void CopyHdActor::AsyncReturnAllReadableRegst() { CHECK(pending_in_regst_.empty()); }

void CopyHdActor::ForEachCurReadableRegst(std::function<void(const Regst*)> handler) {
