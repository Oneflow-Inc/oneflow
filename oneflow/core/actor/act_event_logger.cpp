...
std::string ActEventLogger::act_event_bin_filename() {
  return act_event_bin_filename_;
}

ActEventLogger::ActEventLogger(bool is_experiment)
    : bin_out_stream_(
          LocalFS(),
          JoinPath(FLAGS_log_dir, (is_experiment ? experiment_prefix_ : "") +
...
void ParseActEvents(const std::string &act_event_filepath,
  ...
  while (!in_stream.ReadFully(reinterpret_cast<char *>(&act_event_size),
