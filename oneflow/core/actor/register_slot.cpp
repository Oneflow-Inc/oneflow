...
bool RegstSlot::HasRegstDescId(int64_t regst_desc_id) const {
  CHECK(is_inited_);
  return regst_desc_id2regsts_.find(regst_desc_id) !=
         regst_desc_id2regsts_.end();
}

const std::deque<Regst *> &
RegstSlot::RegstDeq4RegstDescId(int64_t regst_desc_id) const {
  CHECK(is_inited_);
...
int RegstSlot::TryPushBackRegst(Regst *regst) {
  ...
  if (it == regst_desc_id2regsts_.end()) {
    return -1;
  }
  if (it->second.empty()) {
    available_regst_desc_cnt_ += 1;
  }
...
void RegstSlot::PopFrontRegsts(const std::vector<int64_t> &regst_desc_ids) {
  ...
  for (int64_t regst_desc_id : regst_desc_ids) {
    CHECK_EQ(0, TryPopFrontRegst(regst_desc_id));
  }
}

void RegstSlot::InsertRegstDescId(int64_t regst_desc_id) {
  CHECK(is_inited_ == false);
  CHECK(regst_desc_id2regsts_.emplace(regst_desc_id, std::deque<Regst *>())
            .second);
}
