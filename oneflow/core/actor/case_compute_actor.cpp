...
void CaseCompActor::VirtualCompActorInit(const TaskProto &task_proto) {
  ...
  const int32_t output_bns_size = task_proto.exec_sequence()
                                      .exec_node()
                                      .Get(0)
                                      .kernel_conf()
                                      .op_attribute()
...
void CaseCompActor::TakeOverConsumedRegst(
...
void CaseCompActor::TakeOverProducedRegst(
  ...
  ForEachProducedRegst([&](Regst *regst) {
    ...
    CHECK_EQ(
        0,
        regst_desc_id2produced_rs_.at(regst_desc_id).TryPushBackRegst(regst));
  });
}
...
void CaseCompActor::Act() {
  ...
  case_status_.cmd =
      (case_status_.cur_selected_id == -1 ? kCaseCmdHandleInput
                                          : kCaseCmdHandleOutput);
  kernel_ctx.other = &case_status_;
  AsyncLaunchKernel(kernel_ctx, [&](int64_t regst_desc_id) -> Regst * {
    if (consumed_regst_desc_id_ == regst_desc_id) {
      return consumed_regst;
    }
...
void CaseCompActor::UpdtStateAsCustomizedProducedRegst(Regst *regst) {
  const int64_t regst_desc_id = regst->regst_desc_id();
  CHECK_EQ(
      0, regst_desc_id2produced_rs_.at(regst_desc_id).TryPushBackRegst(regst));
}
...
bool CaseCompActor::IsCustomizedReadAlwaysUnReadyFromNow() const {
  return ReceiveEordMsg(consumed_regst_desc_id_) &&
         case_status_.select_id2request_cnt.size() == 0;
}
...
int64_t CaseCompActor::GetCurSelectId() const {
  for (const auto &pair : case_status_.select_id2request_cnt) {
    ...
    if (regst_desc_id2produced_rs_.at(regst_desc_id).IsCurSlotReady()) {
      return pair.first;
    }
  }
  return -1;
...
void CaseCompActor::AsyncSendCustomizedConsumedRegstMsgToProducer() {
  if (case_status_.cmd != kCaseCmdHandleInput) {
    return;
  }
...
void CaseCompActor::NormalProcessCustomizedReadableRegstMsg(
    const ActorMsg &msg) {
  CHECK_EQ(0, consumed_rs_.TryPushBackRegst(msg.regst()));
}

void CaseCompActor::AsyncSendCustomizedProducedRegstMsgToConsumer() {
  if (case_status_.cmd != kCaseCmdHandleOutput) {
    return;
  }
  const int64_t regst_desc_id =
      out_bn_id2regst_desc_id_.at(case_status_.cur_selected_id);
  Regst *const regst =
      regst_desc_id2produced_rs_.at(regst_desc_id).Front(regst_desc_id);
...
bool CaseCompActor::ProducedCtrlRegstValid(int64_t regst_desc_id) const {
  return true;
}

bool CaseCompActor::CheckOutputActId(int64_t regst_desc_id) const {
  return false;
}
