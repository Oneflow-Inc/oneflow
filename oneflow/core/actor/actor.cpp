...
void CheckInplaceRegstDescId(const TaskProto &task_proto) {
  ...
  for (const auto &pair : task_proto.consumed_regst_desc_id()) {
    for (int64_t id : pair.second.regst_desc_id()) {
      consumed_regst_desc_ids.insert(id);
    }
  }
  for (const auto &pair : task_proto.produced_regst_desc()) {
    if (pair.second.has_inplace_consumed_regst_desc_id() == false) {
      continue;
    }
    int64_t in_regst_desc_id = pair.second.inplace_consumed_regst_desc_id();
    CHECK(consumed_regst_desc_ids.find(in_regst_desc_id) !=
          consumed_regst_desc_ids.end());
  }
}
...
void Actor::Init(const JobDesc *job_desc, const TaskProto &task_proto,
  ...
  for (const ExecNodeProto &node : task_proto.exec_sequence().exec_node()) {
    ExecKernel ek;
    ek.kernel =
        ConstructKernel(job_desc_, node.kernel_conf(), device_ctx_.get());
  ...
  is_kernel_launch_synchronized_ =
      std::all_of(exec_kernel_vec_.cbegin(), exec_kernel_vec_.cend(),
                  [](const ExecKernel &ek) {
                    return ek.kernel->IsKernelLaunchSynchronized();
                  });
  if (!is_kernel_launch_synchronized_) {
    CHECK_EQ(exec_kernel_vec_.size(), 1);
  }
  ...
  for (const auto &pair : task_proto.consumed_regst_desc_id()) {
    ...
    if (pair.first == "in_ctrl") {
      consumed_ctrl_regst_desc_ids_.insert(regst_desc_id_vec.begin(),
                                           regst_desc_id_vec.end());
    }
  }
...
void Actor::TakeOverInplaceConsumedAndProduced(
  ...
  for (const auto &pair : produced_ids) {
    ...
    int64_t in_regst_desc_id = pair.second.inplace_consumed_regst_desc_id();
    inplace_regst_desc_id_in2out_.insert(
        std::make_pair(in_regst_desc_id, out_regst_desc_id));
    inplace_regst_desc_id_out2in_.insert(
        std::make_pair(out_regst_desc_id, in_regst_desc_id));
...
void Actor::TakeOverNaiveConsumed(
  ...
  for (const auto &pair : consumed_ids) {
    ...
    if (is_naive_names == find_the_name || pair.first == "in_ctrl") {
      for (int64_t regst_desc_id : pair.second.regst_desc_id()) {
        if (inplace_consumed_rs_.HasRegstDescId(regst_desc_id)) {
          continue;
        }
...
void Actor::TakeOverNaiveProduced(
    const PbMap<std::string, RegstDescProto> &produced_ids) {
  ...
  for (const auto &pair : produced_ids) {
    bool find_the_name = names.find(pair.first) != names.end();
    if (inplace_produced_rs_.HasRegstDescId(pair.second.regst_desc_id())) {
      continue;
    }
    if (is_naive_names == find_the_name ||
        pair.first.substr(0, 9) == "out_ctrl_") {
  ...
  for (const auto &pair : produced_regsts_) {
    if (naive_produced_rs_.HasRegstDescId(pair.first) == false) {
      continue;
    }
    for (const auto &regst : pair.second) {
...
void Actor::ForEachProducedRegst(
...
Actor::Name2RegstDescIds(const std::string &name) const {
...
int64_t Actor::GetPieceId4NaiveOrInplaceCurReadableDataRegst() const {
  ...
  if (pid == init_val) {
    inplace_consumed_rs_.ForChosenFrontRegst(FirstFoundOnly, Select);
  }
  CHECK_GE(pid, 0);
  return pid;
}
...
void Actor::SetReadableRegstInfo(const Regst *regst,
                                 ReadableRegstInfo *info) const {
  info->set_regst_desc_id(regst->regst_desc_id());
  info->set_act_id(regst->act_id());
}

void Actor::ForEachCurNaiveReadableDataRegst(
    std::function<void(const Regst *)> func) const {
  ...
  if (msg.msg_type() == ActorMsgType::kEordMsg) {
    ...
    if (msg.SrcMachineId() == Global<MachineCtx>::Get()->this_machine_id()) {
      ...
      if (naive_consumed_rs_.HasRegstDescId(regst->regst_desc_id())) {
        ...
        int64_t out_regst_desc_id =
            inplace_regst_desc_id_in2out_.at(regst->regst_desc_id());
        CHECK(regst->packed_blob()->dptr() ==
              inplace_produced_rs_.Front(out_regst_desc_id)
                  ->packed_blob()
                  ->dptr());
...
void Actor::TryLogActEvent(const std::function<void()> &DoAct) const {
  if (Global<RuntimeCtx>::Get()->is_experiment_phase() ||
    ...
    act_event->set_is_experiment_phase(
        Global<RuntimeCtx>::Get()->is_experiment_phase());
    ...
    device_ctx_->AddCallBack([act_event]() {
      ...
      // The stream poller thread is not allowed to perform blocking RPC call.
      // Hence, the
      // RPC call is forwarded to the thread pool and will be executed there.
      Global<ThreadPool>::Get()->AddWork([act_event]() {
...
void Actor::AsyncSendConsumedCtrlRegstMsgToProducer() {
  auto IsChosenRegstDescId = [this](int64_t regst_desc_id) {
    return IsConsumedCtrlRegstDescId(regst_desc_id) &&
           ConsumedCtrlRegstValid(regst_desc_id);
  };
  ...
  naive_consumed_rs_.ForChosenRegstDeq(
      IsChosenRegstDescId, [&](const std::deque<Regst *> &reg_deq) {
        ...
        int32_t returned_regst_num = regst->regst_desc()
                                         ->regst_desc_type()
                                         .ctrl_regst_desc()
                                         .returned_regst_num();
...
void Actor::AsyncSendProducedCtrlRegstMsgToConsumer() {
  auto IsChosenRegstDescId = [this](int64_t regst_desc_id) {
    return IsProducedCtrlRegstDescId(regst_desc_id) &&
           ProducedCtrlRegstValid(regst_desc_id);
  };

  tmp_regst_desc_id_vec_.clear();
  naive_produced_rs_.ForChosenFrontRegst(
      IsChosenRegstDescId, [&](Regst *regst) {
        CHECK(regst->regst_desc()->regst_desc_type().has_ctrl_regst_desc());
        int64_t real_consumer_cnt =
            HandleRegstToConsumer(regst, [](int64_t) { return true; });
...
int64_t
Actor::HandleRegstToConsumer(Regst *regst,
  ...
  for (int64_t consumer : regst->consumers_actor_id()) {
    if (!IsAllowedActor(consumer)) {
      continue;
    }
    EnqueueAsyncMsg(
        ActorMsg::BuildRegstMsgToConsumer(actor_id_, consumer, regst));
    real_consumer_cnt += 1;
  }
...
bool Actor::IsReadReady() const {
  return naive_consumed_rs_.IsCurSlotReady() &&
         inplace_consumed_rs_.IsCurSlotReady() && IsCustomizedReadReady();
}

bool Actor::IsWriteReady() const {
  return naive_produced_rs_.IsCurSlotReady() &&
         inplace_produced_rs_.IsCurSlotReady() && IsCustomizedWriteReady();
}
...
void Actor::AsyncLaunchKernel(
  ...
  for (const ExecKernel &ek : exec_kernel_vec_) {
    ek.kernel->Launch(kernel_ctx, [&](const std::string &bn_in_op) -> Blob * {
      ...
      if (regst == nullptr) {
        regst = GetNaiveOrInplaceCurReadable(regst_desc_id_it->second);
      }
      if (regst == nullptr) {
...
void Actor::HandleProducedNaiveDataRegstToConsumer(
  ...
  naive_produced_rs_.ForEachFrontRegst([&](Regst *regst) {
    if (regst->regst_desc()->regst_desc_type().has_data_regst_desc()) {
      if (RegstPreProcess(regst) == false) {
        return;
      }
...
void Actor::HandleProducedNaiveDataRegstToConsumer(
    std::function<bool(Regst *)> RegstPreProcess) {
  HandleProducedNaiveDataRegstToConsumer(RegstPreProcess,
                                         [](int64_t) { return true; });
}
...
void Actor::HandleProducedInplaceDataRegstToConsumer(
    std::function<bool(Regst *)> RegstPreProcess) {
  HandleProducedInplaceDataRegstToConsumer(RegstPreProcess,
                                           [](int64_t) { return true; });
}
...
void Actor::AsyncSendRegstMsgToConsumer(
    Regst *regst, std::function<bool(int64_t)> IsAllowedActor) {
  int64_t real_consumer_cnt = HandleRegstToConsumer(regst, IsAllowedActor);
  if (real_consumer_cnt > 0) {
    naive_produced_rs_.TryPopFrontRegst(regst->regst_desc_id());
  }
}

void Actor::HandleConsumedNaiveDataRegstToProducer(
    std::function<bool(Regst *)> IsAllowedRegst) {
  tmp_regst_desc_id_vec_.clear();
  naive_consumed_rs_.ForEachFrontRegst([&](Regst *regst) {
    if (regst->regst_desc()->regst_desc_type().has_data_regst_desc()) {
      if (IsAllowedRegst(regst) == false) {
        return;
      }
      // must access regst before sending it to producer
      tmp_regst_desc_id_vec_.push_back(regst->regst_desc_id());
...
void Actor::AsyncSendRegstMsgToProducer(Regst *regst, int64_t producer) {
  ...
  EnqueueAsyncMsg(
      ActorMsg::BuildRegstMsgToProducer(actor_id_, producer, regst));
...
int Actor::TryUpdtStateAsProducedRegst(Regst *regst) {
  ...
  CHECK(produced_regsts_.find(regst->regst_desc_id()) !=
        produced_regsts_.end());
  ...
  if (inplace_produced_rs_.TryPushBackRegst(regst) == 0) {
    int64_t in_regst_desc_id =
        inplace_regst_desc_id_out2in_.at(regst->regst_desc_id());
  ...
  int64_t &expected_act_id =
      produced_regst2expected_act_id_[regst->regst_desc_id()];
  if (expected_act_id >= 0 && CheckOutputActId(regst->regst_desc_id())) {
    CHECK_EQ(regst->act_id(), expected_act_id);
  }
  expected_act_id =
      regst->act_id() + ActNumForEachOutput(regst->regst_desc_id());
  return 0;
}
...
void Actor::EnqueueAsyncMsg(const ActorMsg &msg) {
  if (is_kernel_launch_synchronized_ &&
      GetGlobalWorkStreamId() ==
          Global<IDMgr>::Get()->GlobalWorkStreamId4ActorId(
              msg.dst_actor_id())) {
...
Regst *Actor::GetNaiveOrInplaceCurReadable(int64_t regst_desc_id) const {
  ...
  if (regst == nullptr) {
    regst = inplace_consumed_rs_.Front(regst_desc_id);
  }
  return regst;
}
...
Regst *Actor::GetNaiveOrInplaceCurWriteable(int64_t regst_desc_id) const {
  ...
  if (regst == nullptr) {
    regst = inplace_produced_rs_.Front(regst_desc_id);
  }
  return regst;
}
