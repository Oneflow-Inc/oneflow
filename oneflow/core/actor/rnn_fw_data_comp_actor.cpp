...
bool RnnFwDataCompActor::ModelSatisfySSP(Regst* in_regst, Regst* model_regst) {
  if (JobDesc::Singleton()->is_train()) {
    ...
    int64_t cur_iteration = in_regst->piece_status().piece_id() / num_of_pieces_in_batch;
...
void RnnFwDataCompActor::UpdtInAndModelStatesOfRnnCell() {
  const PieceStatus& piece_status = material4act_.regst_id2regst.at(in_regst_desc_id_)->piece_status();
  if (piece_status.IsLastCol()) { // only one word in this line
