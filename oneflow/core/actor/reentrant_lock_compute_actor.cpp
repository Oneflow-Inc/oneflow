...
void ReentrantLockCompActor::VirtualCompActorInit(const TaskProto &task_proto) {
  ...
  const auto &kernel_conf =
      task_proto.exec_sequence().exec_node().Get(0).kernel_conf();
  const auto &ibns = kernel_conf.op_attribute().input_bns();
  for (const auto &ibn : ibns) {
    int64_t regst_desc_id =
        exec_kernel_vec().at(0).bn_in_op2regst_desc_id.at(ibn);
    if (ibn == "start") {
      eord_regst_desc_id_ = regst_desc_id;
    }
...
void ReentrantLockCompActor::NormalProcessCustomizedReadableRegstMsg(
    const ActorMsg &msg) {
...
const std::string &ReentrantLockCompActor::Ibn4RegstDescId(int64_t id) const {
  ...
  if (iter == regst_desc_id2ibn_.end()) {
    return ReentrantLockStatus::kEmptyIbn;
  }
...
void ReentrantLockCompActor::Act() {
  ...
  reentrant_lock_status_.set_cur_ibn(
      Ibn4RegstDescId(cur_processed_regst_desc_id_));
...
void ReentrantLockCompActor::VirtualAsyncSendNaiveProducedRegstMsgToConsumer() {
  if (reentrant_lock_status_.acquired_lock_to_be_sent() == false) {
    return;
  }
