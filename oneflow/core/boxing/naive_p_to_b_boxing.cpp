...
Maybe<one::Tensor> NaivePToB(const std::shared_ptr<one::Tensor>& tensor, Symbol<PlacedNdSbp> in,
  ...
  std::shared_ptr<one::Tensor> processed_in_tensor = JUST(PreprocessInputTensor4SliceBoxing(
      tensor, /* log_prefix */ "\t\tInternal boxing of naive-p-to-b, "));
...

  {
    ...
    if (in_parallel_id->has_value() || out_parallel_id->has_value()) {
      local_tensor =
          JUST(one::functional::EagerPToB(local_tensor, JUST(processed_in_tensor->parallel_desc()),
                                          new_out_placement, *tensor->shape()));
    }
  }
