...
Maybe<void> RawCheckCclPToS(Symbol<PlacedNdSbp> in, Symbol<PlacedNdSbp> out) {
...
static constexpr auto* CheckCclPToS = DECORATE(&RawCheckCclPToS, ThreadLocal);

}  // namespace

Maybe<one::Tensor> CclPToS(const std::shared_ptr<one::Tensor>& tensor, Symbol<PlacedNdSbp> in,
  ...
  std::shared_ptr<one::Tensor> local_tensor = JUST(tensor->cur_rank_phy_tensor());
  {
    ...
    if (in_parallel_id->has_value() || out_parallel_id->has_value()) {
      local_tensor = JUST(one::functional::EagerPToS(local_tensor, tensor_placement,
...
COMMAND(RegisterBoxingFunction("naive-p-to-s", CheckCclPToS, &CclPToS));
