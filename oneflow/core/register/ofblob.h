...
public:
  ...
  OfBlob(DeviceCtx *device_ctx, Blob *blob)
      : device_ctx_(device_ctx), blob_(blob),
        tensor_back_inserter_(new TensorBackInserter(blob)) {
  ...
  bool CurTensorIteratorEqEnd() const {
    return blob_->IsEndTensor(*cur_tensor_);
  }
  void CurTensorCopyShapeTo(int64_t *ptr, int64_t num_axis) const;
  template <typename T> void CurTensorAutoMemCopyTo(T *ptr, int64_t len) const;

  void ClearTensorLists();
  void AddTensor();
  bool CurMutTensorAvailable() const {
    return tensor_back_inserter_->IsCurMutTensorAvailable();
  }
...
inline int64_t OfBlob::TotalNumOfTensors() const {
  return blob_->total_num_of_tensors();
}

inline int64_t OfBlob::NumOfTensorListSlices() const {
  return blob_->num_of_tensor_list_slices();
}
...
inline void OfBlob::AddTensorListSlice() const {
  tensor_back_inserter_->add_tensor_list_slice();
}
...
inline void OfBlob::ClearTensorLists() {
  tensor_back_inserter_->ClearTensorLists();
}
...
inline void OfBlob::IncTensorIterator() {
  blob_->MoveToNextTensor(cur_tensor_.get());
}
...
inline void OfBlob::CurMutTensorCopyShapeFrom(const int64_t *ptr,
                                              int64_t num_axis) const {
  CHECK_EQ(num_axis, NumAxes());
...
void OfBlob::CurTensorAutoMemCopyTo(T *ptr, int64_t len) const {
  ...
  SyncAutoMemcpy(device_ctx_, ptr, cur_tensor_->dptr(), len * sizeof(T),
...
void OfBlob::CurMutTensorAutoMemCopyFrom(const T *ptr, int64_t len) const {
  ...
  CHECK(tensor_back_inserter_->cur_mut_tensor()->data_type() ==
        GetDataType<T>::value);
  SyncAutoMemcpy(device_ctx_,
                 tensor_back_inserter_->cur_mut_tensor()->mut_dptr(), ptr,
...
void OfBlob::StaticTensorAutoMemCopyFrom(const T *ptr, int64_t len) const {
  ...
  SyncAutoMemcpy(device_ctx_, blob_->sole_mut_tensor().mut_dptr(), ptr,
                 len * sizeof(T), blob_->mem_case(), mem_case_);
}
