...
std::unique_ptr<PodDesc> NewPodDesc(const PodProto &pod) {
  if (pod.has_tensor_pod()) {
    return std::make_unique<TensorPodDesc>(pod.tensor_pod());
  }
  if (pod.has_struct_pod()) {
    return std::make_unique<StructPodDesc>(pod.struct_pod());
  }
  // ignore field pod
  UNIMPLEMENTED();
...
bool TensorPodDesc::operator==(const PodDesc &rhs) const {
  ...
  if (tensor_rhs == nullptr) {
    return false;
  }
  return shape() == tensor_rhs->shape() &&
         data_type() == tensor_rhs->data_type();
}
...
bool StructPodDesc::operator==(const PodDesc &rhs) const {
  ...
  if (struct_rhs == nullptr) {
    return false;
  }
  if (field_id2field_idx_ != struct_rhs->field_id2field_idx_) {
    return false;
  }
  for (int i = 0; i < field_id2field_idx_.size(); ++i) {
    if (*fields_.at(i) != *struct_rhs->fields_.at(i)) {
      return false;
    }
  }
  return true;
...
void StructPodDesc::ToProto(StructPodProto *struct_pod_proto) const {
  ...
  for (const auto &field : fields_) {
    field->ToProto(struct_pod_proto->add_field());
  }
}
...
StructPodDesc *StructPodDesc::MutStructField(const FieldId &field_id,
  ...
  if (!HasField(field_id)) {
    AddField(field_id, std::make_unique<StructPodDesc>(), alignment);
  }
...
void StructPodDesc::AddField(const FieldId &field_id,
                             std::unique_ptr<PodDesc> &&field,
                             size_t alignment) {
