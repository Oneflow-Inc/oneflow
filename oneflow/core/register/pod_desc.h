...
template <> struct hash<oneflow::FieldId> {
  size_t operator()(const oneflow::FieldId &field_id) const {
    if (field_id.has_key()) {
      return std::hash<int>()(field_id.key());
    }
    if (field_id.has_lbi()) {
      return std::hash<oneflow::LogicalBlobId>()(field_id.lbi());
    }
    UNIMPLEMENTED();
  }
};
...
public:
  ...
  std::unique_ptr<PodDesc> Clone() const override {
    return std::make_unique<TensorPodDesc>(*this);
  }
...
public:
  ...
  StructPodDesc *MutStructField(const FieldId &field_id,
                                int32_t default_alignment);
  const PodDesc &Field(FieldKey field_key) const {
    return Field(NewFieldId(field_key));
  }
  ...
  void AddField(const FieldId &field_id, const PodDesc &pod_desc,
                size_t alignment);
  bool HasField(FieldKey field_key) const {
    return HasField(NewFieldId(field_key));
  }
  bool HasField(const FieldId &field_id) const;
  PodDesc *MutExistedField(FieldKey field_key) {
    return MutExistedField(NewFieldId(field_key));
  }

  std::unique_ptr<PodDesc> Clone() const override {
    return std::make_unique<StructPodDesc>(*this);
  }
  void InitFromProto(const StructPodProto &struct_pod);
  void ToProto(PodProto *pod_proto) const override {
    ToProto(pod_proto->mutable_struct_pod());
  }
...
private:
  ...
  FieldPodDesc(const FieldId &field_id, std::unique_ptr<PodDesc> &&pod,
               size_t alignment)
      : PodDesc(), field_id_(field_id), pod_(std::move(pod)),
...
template <typename T> const T &PodDesc::Cast() const {
  static_assert(std::is_same<T, TensorPodDesc>::value ||
                    std::is_same<T, StructPodDesc>::value,
                "only TensorPodDesc and StructPodDesc supported");
  return *dynamic_cast<const T *>(this);
}

template <typename T> T *PodDesc::MutCast() {
  static_assert(std::is_same<T, TensorPodDesc>::value ||
                    std::is_same<T, StructPodDesc>::value,
                "only TensorPodDesc and StructPodDesc supported");
