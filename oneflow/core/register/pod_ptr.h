...
class PodPtr final {
public:
  PodPtr(const PodDesc &pod_desc, char *ptr)
      : pod_desc_(&pod_desc), ptr_(ptr) {}
  PodPtr(const PodPtr &) = default;
  ~PodPtr() = default;

  template <typename T> const T *TensorPtr() const;
  template <typename T> const T *TensorPtr(FieldKey field_key) const {
    return TensorPtr<T>(field_key, nullptr);
  }
  template <typename T>
  const T *TensorPtr(FieldKey field_key, const T *default_ptr) const;

  template <typename T> T *MutTensorPtr();
  template <typename T> T *MutTensorPtr(FieldKey field_key) {
    return MutTensorPtr<T>(field_key, nullptr);
  }
  template <typename T> T *MutTensorPtr(FieldKey field_key, T *default_ptr);
  ...
  const PodPtr Field(FieldKey field_key) const {
    return Field(NewFieldId(field_key));
  }
  PodPtr MutField(FieldKey field_key) {
    return MutField(NewFieldId(field_key));
  }
...
template <typename T> const T *PodPtr::TensorPtr() const {
  CheckDataType<T>();
  return reinterpret_cast<const T *>(ptr_);
}

template <typename T> T *PodPtr::MutTensorPtr() {
  CheckDataType<T>();
