...
void CheckBlobInRegstNotDisabled(const RegstDescProto &regst_desc) {
  ...
  CHECK(regst_desc.regst_desc_type()
            .data_regst_desc()
            .packed_blob_desc()
...
RegstMgr::RegstMgr(const Plan &plan) {
  ...
  for (const ChunkProto &chunk : plan.block_chunk_list().chunk()) {
    if (chunk.machine_id() != this_machine_id) {
      continue;
    }
    if (chunk.mem_size() == 0) {
      continue;
    }
    char *chunk_ptr = Global<MemoryAllocator>::Get()->Allocate(
        chunk.mem_case(), chunk.mem_size());
  ...
  for (const MemBlockProto &mem_block : plan.block_chunk_list().mem_block()) {
    if (mem_block.machine_id() != this_machine_id) {
      continue;
    }
    ...
    if (mem_block.has_chunk_id()) {
      ...
      mem_block_ptr =
          chunk_id2ptr.at(mem_block.chunk_id()) + mem_block.chunk_offset();
    } else {
      mem_block_ptr = Global<MemoryAllocator>::Get()->Allocate(
          mem_block.mem_case(), mem_block.mem_size());
    }
    CHECK(mem_block_id2ptr_.emplace(mem_block.mem_block_id(), mem_block_ptr)
              .second);
  }
    ...
    for (const auto &pair : task.produced_regst_desc()) {
      ...
      CHECK(regst_desc_id2rt_regst_desc_
                .emplace(regst_desc_id,
                         std::make_unique<const RtRegstDesc>(regst_desc))
                .second);
      CHECK(regst_desc_id2parallel_ctx_
                .emplace(regst_desc_id, task.parallel_ctx())
                .second);
    }
  }
}
...
void RegstMgr::NewRegsts(const RegstDescProto &regst_desc_proto,
  ...
  const RegstDescTypeProto &regst_desc_type =
      regst_desc_proto.regst_desc_type();
  const RtRegstDesc *rt_regst_desc =
      regst_desc_id2rt_regst_desc_.at(regst_desc_id).get();
  ...
  if (mem_block_id != -1 &&
      mem_block_id2ptr_.find(mem_block_id) != mem_block_id2ptr_.end()) {
    main_mem_ptr = mem_block_id2ptr_.at(mem_block_id) +
                   regst_desc_proto.mem_block_offset();
  }
  if (header_block_id != -1 &&
      mem_block_id2ptr_.find(header_block_id) != mem_block_id2ptr_.end()) {
  ...
  if (regst_desc_type.has_data_regst_desc()) {
    for (const LbiBlobDescPair &pair :
         regst_desc_type.data_regst_desc().lbi2blob_desc()) {
  ...
  for (int64_t i = 0; i < rt_regst_desc->register_num(); ++i) {
    ...
    if (regst_desc_type.has_data_regst_desc()) {
      ...
      if (separated_header_mem_ptr != nullptr) {
        separated_header_mem_ptr +=
            rt_regst_desc->SeparatedHeaderByteSize4OneRegst();
      }
...
void RegstMgr::NewBlobsInOneRegst(const std::vector<LbiBlobDescPair> &lbis,
                                  Regst *regst,
                                  const RtRegstDesc *rt_regst_desc,
                                  char *main_mem_ptr,
                                  char *separated_header_mem_ptr) {
  size_t separated_header_mem_size =
      rt_regst_desc->SeparatedHeaderByteSize4OneRegst();
  ...
  if (separated_header_mem_size > 0) {
    ...
    if (separated_header_mem_ptr == nullptr) {
      separated_header_mem_ptr = Global<MemoryAllocator>::Get()->Allocate(
          host_mem_case, separated_header_mem_size);
    }
    regst->packed_blob_.reset(new Blob(regst->regst_desc()->mem_case(),
                                       packed_blob_desc,
      ...
      lbis, [&](const LbiBlobDescPair &lbi, int64_t body_offset,
        ...
        if (cur_body_pointer == nullptr) {
          ...
          blob_ptr = std::move(std::make_unique<Blob>(
              regst->regst_desc()->mem_case(), blob_desc,
              cur_header_pointer + header_offset, nullptr));
        } else {
          CHECK(rt_regst_desc->is_body_disabled() == false);
          blob_ptr = std::move(
              std::make_unique<Blob>(regst->regst_desc()->mem_case(), blob_desc,
                                     cur_header_pointer + header_offset,
                                     cur_body_pointer + body_offset));
          InitNonPODTypeBlobIfNeed(Global<MemoryAllocator>::Get(),
                                   blob_ptr.get());
        }
        ...
        const auto &parallel_ctx =
            regst_desc_id2parallel_ctx_.at(regst_desc_id);
        if (parallel_ctx.has_parallel_id()) {
          const int64_t parallel_id = parallel_ctx.parallel_id();
          {
            std::lock_guard<std::mutex> lock(mutex_);
            lbi2parallel_id2blob_[lbi.lbi()][parallel_id] =
                regst->GetBlobByLbi(lbi.lbi());
          }
        }
      });
}
...
const RtRegstDesc &
RegstMgr::RegstDesc4RegstDescId(int64_t regst_desc_id) const {
  const auto &it = regst_desc_id2rt_regst_desc_.find(regst_desc_id);
  CHECK(it != regst_desc_id2rt_regst_desc_.end());
  return *it->second;
}

Blob *RegstMgr::Blob4LbiAndParallelId(const LogicalBlobId &lbi,
