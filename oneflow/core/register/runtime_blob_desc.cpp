...
const Shape& RtBlobDesc::shape(const std::string& field_name) const {
  return header_pod_desc().ShapedField(field_name).shape();
}

DataType RtBlobDesc::data_type(const std::string& field_name) const {
  return header_pod_desc().ShapedField(field_name).data_type();
}

bool RtBlobDesc::has_data_id_field() const { return header_pod_desc().HasField("data_id"); }

bool RtBlobDesc::has_col_num_field() const { return header_pod_desc().HasField("col_num"); }
...
size_t RtBlobDesc::ByteSizeOfDataIdField() const {
  if (!header_pod_desc().HasField("data_id")) { return 0; }
  return header_pod_desc().ShapedField("data_id").ByteSize();
}

size_t RtBlobDesc::ByteSizeOfColNumField() const {
  if (!header_pod_desc().HasField("col_num")) { return 0; }
  return header_pod_desc().ShapedField("col_num").ByteSize();
}
