...
public:
  void CheckHeaderMutable() const override {
    CHECK(is_header_mutable) << "header mutable check not passed, blob's shape "
                                "is not mutable at this moment!";
  }

  void CheckBodyMutable() const override {
    CHECK(is_body_mutable) << "body mutable check not passed, blob's data is "
                              "not mutable at this moment!";
  }
};
...
public:
  ...
  Blob(const MemoryCase &mem_case, const RtBlobDesc *blob_desc,
       char *header_ptr);
  Blob(const MemoryCase &mem_case, const RtBlobDesc *blob_desc,
  ...
  template <typename T = void> T *ForceMutDptr() {
    CheckDataType<T>(data_type());
    return static_cast<T *>(dptr_);
  }
  ...
  const BlobAccessChecker *blob_access_checker() {
    return this->blob_access_checker_;
  }

private:
  void Init(const MemoryCase &mem_case, const RtBlobDesc *blob_desc,
            char *header_ptr, char *body_ptr);
  template <FieldKey key> const int64_t *header_field() const {
    return header_fields_[key];
  }
  template <FieldKey key> int64_t *mut_header_field() {
    return header_fields_[key];
  }
  template <FieldKey key> size_t header_field_capacity() const {
...
template <typename RecordType> class RecordBlob final {
public:
  ...
  void ForEachRecord(std::function<void(const RecordType &)> Handler) {
    FOR_RANGE(int32_t, i, 0, record_num_) {
      Handler(*(records_->mut_dptr<RecordType>() + i));
    }
  }
...
#define INIT_GLOBAL_BLOB_MUTABLE_CHECKER(is_header_mutable, is_body_mutable)   \
  COMMAND(                                                                     \
      Global<BlobAccessCheckerIf<is_header_mutable, is_body_mutable>>::        \
          SetAllocated(                                                        \
