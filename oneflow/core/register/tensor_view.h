...
template <typename ShapeViewType, typename ByteType> class TensorViewBase {
public:
...
protected:
  ...
  TensorViewBase(const Blob *blob, typename ShapeViewType::DimType *shape_ptr,
                 ByteType *dptr)
...
public:
  template <typename T = void> T *mut_dptr() const {
...
protected:
  ...
  MutTensorView(const Blob *blob, typename ShapeViewType::DimType *shape_ptr,
                char *dptr)
