...
Blob::Blob(Regst* regst, const BlobDesc* blob_desc, char* mem_ptr, const void* comm_net_token) {
  mem_ptr_ = mem_ptr;
  ...
  if (blob_desc->has_col_num_field()) {
    col_num_ptr_ = reinterpret_cast<int32_t*>(mem_ptr + blob_desc->ByteSizeOfDataIdField());
  } else {
    col_num_ptr_ = nullptr;
  }
  dptr_ = mem_ptr + blob_desc->ByteSizeOfDataIdField() + blob_desc->ByteSizeOfColNumField();
