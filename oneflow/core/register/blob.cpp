...
void TensorBackInserter::ReserveOneEmptyTensorList() {
  ...
  if (IsCurMutTensorAvailable()) {
    cur_mut_tensor_ = blob_->EndFullyMutTensor();
  }
}
...
FullyMutTensorView *TensorBackInserter::cur_mut_tensor() {
  return &cur_mut_tensor_;
}

void TensorBackInserter::add_tensor_list_slice() {
  return blob_->add_tensor_list_slice();
}

const MemoryCase &Blob::mem_case() const { return mem_case_; }

Blob::Blob(const MemoryCase &mem_case, const RtBlobDesc *blob_desc,
           char *header_ptr) {
  Init(mem_case, blob_desc, header_ptr,
       header_ptr + blob_desc->ByteSizeOfBlobHeader());
}

Blob::Blob(const MemoryCase &mem_case, const RtBlobDesc *blob_desc,
           char *header_ptr, char *body_ptr) {
  Init(mem_case, blob_desc, header_ptr, body_ptr);
}

void Blob::Init(const MemoryCase &mem_case, const RtBlobDesc *blob_desc,
                char *header_ptr, char *body_ptr) {
  mem_case_ = mem_case;
  ...
  FOR_RANGE(int32_t, i, 0, FieldKey::kFieldKeySize) {
    ...
    if (header_fields_[i] == nullptr) {
      ...
      header_field_capacities_[i] = blob_desc->header_pod_desc()
                                        .Field(key)
                                        .Cast<TensorPodDesc>()
                                        .shape()
                                        .elem_cnt();
  ...
  if (!blob_desc_->header_is_opaque()) {
    ...
    *mut_header_field<FieldKey::kLastTensorDataOffset>() = 0;
    ...
    if (blob_desc->is_dynamic()) {
      mut_shape_view_.reset(
          new MutShapeView(shape_ptr, static_shape().NumAxes()));
    }
...
void Blob::ResetTensorView() {
  begin_tensor_.reset(new TensorView(
      this, header_field<FieldKey::kTensorShapeList>(), dptr<char>()));
...
bool Blob::IsEndFullyMutTensor(const FullyMutTensorView &tensor) const {
  size_t shape_list_capacity =
      header_field_capacity<FieldKey::kTensorShapeList>();
  const int64_t *end_shape_ptr =
      header_field<FieldKey::kTensorShapeList>() + shape_list_capacity;
  const char *end_tensor_dptr = dptr<char>() + blob_desc().ByteSizeOfBlobBody();
  return tensor.shape().ptr() == end_shape_ptr ||
         tensor.dptr<char>() == end_tensor_dptr;
}
...
void Blob::add_tensor_list_slice() {
  size_t slice_buff_byte_size = blob_desc()
                                    .header_pod_desc()
                                    .Field(FieldKey::kTensorListSlices)
                                    .ByteSize();
...
FullyMutTensorView Blob::EndFullyMutTensor() {
  size_t shape_list_capacity =
      header_field_capacity<FieldKey::kTensorShapeList>();
  int64_t *end_shape_ptr =
      mut_header_field<FieldKey::kTensorShapeList>() + shape_list_capacity;
  char *end_tensor_dptr =
      ForceMutDptr<char>() + blob_desc().ByteSizeOfBlobBody();
...
size_t Blob::GetEndTensorDataOffset() const {
  ...
  size_t elem_cnt = 1;
  FOR_RANGE(int32_t, i, 0, static_shape().NumAxes()) {
    elem_cnt *= shape_ptr[i];
  }
  size_t last_tensor_byte_size = elem_cnt * GetSizeOfDataType(data_type());
  return *header_field<FieldKey::kLastTensorDataOffset>() +
         last_tensor_byte_size;
}

size_t Blob::ByteSizeOfBlobBody() const {
  if (blob_desc().header_is_opaque()) {
    return blob_desc().ByteSizeOfBlobBody();
  }
...
void Blob::CopyDataContentFrom(DeviceCtx *device_ctx, const Blob *rhs) {
  ...
  AutoMemcpy(device_ctx, mut_dptr(), rhs->dptr(), ByteSizeOfBlobBody(),
             mem_case(), rhs->mem_case());
}
...
void Blob::CopyValidDataContentFrom(DeviceCtx *device_ctx, const Blob *rhs) {
  ...
  AutoMemcpy(device_ctx, mut_dptr(), rhs->dptr(), body_byte_size, mem_case(),
             rhs->mem_case());
}

void Blob::CopyHeaderFrom(DeviceCtx *device_ctx, const Blob *rhs) {
  if (this == rhs || blob_desc().ByteSizeOfBlobHeader() == 0) {
    return;
  }
  CHECK_EQ(blob_desc().ByteSizeOfBlobHeader(),
           rhs->blob_desc().ByteSizeOfBlobHeader());
  if (blob_desc().header_is_opaque()) {
  ...
  {
    const int64_t shape_list_len =
        *rhs->header_field<FieldKey::kTensorListLength>();
    ...
    *mut_header_field<FieldKey::kLastTensorDataOffset>() =
    ...
    Memcpy<DeviceType::kCPU>(device_ctx,
                             mut_header_field<FieldKey::kTensorShapeList>(),
                             rhs->header_field<FieldKey::kTensorShapeList>(),
                             shape_list_len * num_axes * sizeof(int64_t));
  }
  {
    const int64_t seg_length =
        *rhs->header_field<FieldKey::kTensorListSlicesLength>();
    *mut_header_field<FieldKey::kTensorListSlicesLength>() = seg_length;
    Memcpy<DeviceType::kCPU>(device_ctx,
                             mut_header_field<FieldKey::kTensorListSlices>(),
