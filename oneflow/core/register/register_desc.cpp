...
BlobDesc *RegstDesc::MutSoleBlobDesc() {
  return const_cast<BlobDesc *>(SoleBlobDesc());
}

void RegstDesc::ForEachLbi(
    std::function<void(const LogicalBlobId &)> func) const {
  for (const auto &p : lbi2blob_desc_) {
    func(p.first);
  }
}
...
void RegstDesc::ToProto(RegstDescProto *ret) const {
  ...
  *(ret->mutable_regst_desc_type()) = regst_desc_type_;
  if (regst_desc_type_.has_data_regst_desc()) {
    ...
    packed_blob_desc_->ToProto(
        data_regst_desc_proto->mutable_packed_blob_desc());
    for (const auto &pair : lbi2blob_desc_) {
      LbiBlobDescPair *pb_pair =
          data_regst_desc_proto->mutable_lbi2blob_desc()->Add();
      *(pb_pair->mutable_lbi()) = pair.first;
      pair.second->ToProto(pb_pair->mutable_blob_desc());
    }
    CHECK(data_regst_time_shape_);
    data_regst_time_shape_->ToProto(
        data_regst_desc_proto->mutable_time_shape());
  ...
  *(ret->mutable_mem_case()) = mem_case_;
  ...
  if (hint_inplace_consumed_regst_desc_id_ != -1) {
    ret->set_hint_inplace_consumed_regst_desc_id(
        hint_inplace_consumed_regst_desc_id_);
  }
}
...
bool RegstDesc::HasSameBlobDescs(const RegstDesc *rhs) {
  if (rhs->lbi2blob_desc_.size() != lbi2blob_desc_.size()) {
    return false;
  }
  for (const auto &pair : rhs->lbi2blob_desc_) {
    auto iter = lbi2blob_desc_.find(pair.first);
    if (iter == lbi2blob_desc_.end()) {
      return false;
    }
    if (!(*(pair.second.get()) == *(iter->second.get()))) {
      return false;
    }
  }
  return true;
...
int64_t
RegstDesc::ByteOffsetInPackedBlobDescBody(const LogicalBlobId &lbi) const {
  ...
  std::sort(lbi_blob_desc_pairs.begin(), lbi_blob_desc_pairs.end(),
            CompareLbiBlobDescPair);

  bool found = false;
  int64_t offset = 0;
  rt_regst_desc.ForEachBlobDescOffsetInOnRegst(
      lbi_blob_desc_pairs, [&](const LbiBlobDescPair &lbi_blob_desc_pair,
