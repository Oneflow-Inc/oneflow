...
TensorSliceView::TensorSliceView(const std::initializer_list<Range> &ranges)
    : range_vec_(ranges) {
  UpdateShape();
}

TensorSliceView::TensorSliceView(const std::vector<Range> &ranges)
    : range_vec_(ranges) {
  UpdateShape();
}
...
TensorSliceView::TensorSliceView(const Shape &shape) {
  ...
  std::transform(shape.dim_vec().cbegin(), shape.dim_vec().cend(),
                 range_vec_.begin(),
...
TensorSliceView TensorSliceView::Intersect(const TensorSliceView &other) const {
  ...
  FOR_RANGE(int64_t, i, 0, range_vec_.size()) {
    const Range intersection =
        FindIntersectant(range_vec_.at(i), other.range_vec_.at(i));
...
const Range &TensorSliceView::At(int64_t index) const {
  return range_vec_.at(index);
}

const Shape &TensorSliceView::shape() const { return shape_; }

const std::vector<Range> &TensorSliceView::range_vec() const {
  return range_vec_;
}
...
NdIndex TensorSliceView::OffsetTo(const TensorSliceView &other) const {
  ...
  std::transform(range_vec_.cbegin(), range_vec_.cend(),
                 other.range_vec_.cbegin(), indices_vec.begin(),
  ...
  FOR_RANGE(int64_t, i, 0, num_axes) {
    if (i == axis) {
      ...
      CHECK(std::all_of(slices.cbegin() + 1, slices.cbegin(),
                        [&](const TensorSliceView &view) {
                          return view.At(i) == dim_range;
                        }));
    }
  }
