...
PodPtr PodPtrField(const PodDesc *pod_desc, const FieldId &field_id,
                   char *ptr) {
  const auto *struct_pod = dynamic_cast<const StructPodDesc *>(pod_desc);
  CHECK_NOTNULL(struct_pod);
  return PodPtr(struct_pod->Field(field_id),
                ptr + struct_pod->ByteOffset4Field(field_id));
}
...
PodPtr PodPtr::MutField(const FieldId &field_id) {
  return PodPtrField(pod_desc_, field_id, ptr_);
}
