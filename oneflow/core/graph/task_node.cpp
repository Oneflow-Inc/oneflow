...
void ForEachDataEdge(const std::unordered_set<TaskEdge *> &edges,
  ...
  for (TaskEdge *edge : edges) {
    ...
    int32_t data_regst_size =
        std::count_if(regsts.begin(), regsts.end(),
...
TaskNode::TaskNode()
    : machine_id_(-1), thrd_id_(-1), task_id_(-1), area_id_(0), chain_id_(-1),
      order_in_graph_(-1) {}
...
TaskNode::GetConsumedRegst(const std::string &name) {
  return consumed_regsts_.at(name);
}

std::shared_ptr<RegstDesc>
TaskNode::GetSoleConsumedRegst(const std::string &name) {
  auto it = consumed_regsts_.find(name);
  if (it == consumed_regsts_.end()) {
    return nullptr;
  }
...
void TaskNode::NaiveInferProducedDataRegstTimeShape() {
  ...
  ForEachConsumedDataRegst(
      [&time_shape](const std::string &name, const RegstDesc *regst) {
        if (time_shape) {
...
std::shared_ptr<Shape> TaskNode::GetFastestInputOutputTimeShape() const {
  ...
  auto UpdateRetShape = [&](TaskEdge *edge) {
    for (const auto &regst : edge->GetRegsts()) {
      if (!shape ||
          shape->elem_cnt() < regst->data_regst_time_shape()->elem_cnt()) {
...
void TaskNode::ForEachConsumedDataRegst(
    const std::function<void(const std::string &, const RegstDesc *)> &Handler)
    const {
...
void TaskNode::EraseZeroSizeConsumedRegst() {
  for (auto &pair : consumed_regsts_) {
    for (auto it = pair.second.begin(); it != pair.second.end();) {
      auto regst_ptr = *it;
      CHECK(regst_ptr);
      if (regst_ptr->regst_desc_type().has_data_regst_desc() &&
          regst_ptr->NumOfLbi() == 0) {
  ...
  EraseIf<std::string, std::list<std::shared_ptr<RegstDesc>>>(
      &consumed_regsts_,
      [](HashMap<std::string, std::list<std::shared_ptr<RegstDesc>>>::iterator
             it) { return it->second.empty(); });
}
...
void TaskNode::UnbindBnWithEmptyRegst() {
  exec_gph_.ForEachNode(
      [&](ExecNode *exec_node) { exec_node->UnbindBnWithEmptyRegst(); });
}
...
void TaskNode::BuildCtrlRegstDescIfNeed(TaskNode *dst_node) {
  if (IsMeaningLess() || dst_node->IsMeaningLess()) {
    return;
  }
  for (const TaskEdge *in_edge : dst_node->in_edges()) {
    if (in_edge->src_node() == this) {
      return;
    }
  }
...
std::shared_ptr<RegstDesc> TaskNode::ProduceRegst(const std::string &name,
  ...
  return ProduceRegst(name, enable_reuse_mem, min_register_num,
                      max_register_num, regst_desc_type);
}

std::shared_ptr<RegstDesc>
TaskNode::ProduceRegst(const std::string &name, bool enable_reuse_mem,
                       int32_t min_register_num, int32_t max_register_num,
                       const RegstDescTypeProto &regst_desc_type) {
  auto regst = NewProducedRegst(enable_reuse_mem, min_register_num,
                                max_register_num, regst_desc_type);
  CHECK(produced_regsts_.emplace(name, regst).second);
  return regst;
}
...
std::shared_ptr<RegstDesc>
TaskNode::NewProducedRegst(bool enable_reuse_mem, int32_t min_register_num,
                           int32_t max_register_num,
                           const RegstDescTypeProto &regst_desc_type) {
  ...
  *(regst->mut_regst_desc_type()) = regst_desc_type;
  ...
  regst->set_enable_reuse_mem(GlobalJobDesc().enable_reuse_mem() &&
                              enable_reuse_mem);
...
void TaskNode::FixRegisterNumRange() {
  for (auto &pair : produced_regsts_) {
    ...
    if (in_same_stream == false &&
        area_id_ != static_cast<int64_t>(kMdUpdtArea) &&
        GetTaskType() == TaskType::kCopyHd) { // TODO: delete this hack
      if (produced_regst->max_register_num() >= 2) {
        produced_regst->UpdtMinRegstNumIfNeed(2);
      }
    }
  }
}
...
void TaskNode::EraseConsumedRegstsByName(const std::string &name) {
  if (consumed_regsts_.find(name) != consumed_regsts_.end()) {
    for (auto &regst : consumed_regsts_[name]) {
...
std::shared_ptr<RegstDesc>
TaskEdge::GetRegst(const std::string &name_in_producer) const {
...
std::vector<std::shared_ptr<RegstDesc>> TaskEdge::GetRegsts() const {
  ...
  for (auto &pair : name_in_producer2regst_) {
    regst_descs.emplace_back(pair.second);
  }
  return regst_descs;
}
...
RegstDescProto *
FindOrCreateProducedCtrlRegstDesc(TaskProto *task_proto,
                                  const std::string &regst_desc_name) {
  auto *produced_regst_desc = task_proto->mutable_produced_regst_desc();
  if (produced_regst_desc->find(regst_desc_name) ==
      produced_regst_desc->end()) {
    RegstDescProto ctrl_regst_desc;
    InitCtrlRegstDesc(task_proto->task_id(), &ctrl_regst_desc);
    CHECK(
        produced_regst_desc->insert({regst_desc_name, ctrl_regst_desc}).second);
  }
  return &produced_regst_desc->at(regst_desc_name);
}

RegstDescIdSet *
FindOrCreateConsumedCtrlRegstDescIdSet(TaskProto *task_proto,
                                       const std::string &regst_desc_name) {
  auto *consumed_regst_desc_id_sets =
      task_proto->mutable_consumed_regst_desc_id();
  if (consumed_regst_desc_id_sets->find(regst_desc_name) ==
      consumed_regst_desc_id_sets->end()) {
    CHECK(
        consumed_regst_desc_id_sets->insert({regst_desc_name, RegstDescIdSet()})
            .second);
  }
  return &consumed_regst_desc_id_sets->at(regst_desc_name);
}

void TaskNode::ForEachInDataEdge(
    const std::function<void(TaskEdge *)> &Handler) const {
  ForEachDataEdge(in_edges(), Handler);
}

void TaskNode::ForEachOutDataEdge(
    const std::function<void(TaskEdge *)> &Handler) const {
  ForEachDataEdge(out_edges(), Handler);
}

void TaskNode::ForEachNodeOnInDataEdge(
    const std::function<void(TaskNode *)> &Handler) const {
  ForEachInDataEdge([&](TaskEdge *in_edge) { Handler(in_edge->src_node()); });
}

void TaskNode::ForEachNodeOnOutDataEdge(
    const std::function<void(TaskNode *)> &Handler) const {
  ForEachOutDataEdge(
      [&](TaskEdge *out_edge) { Handler(out_edge->dst_node()); });
}

void TaskNode::ForEachNodeOnInOutDataEdge(
    const std::function<void(TaskNode *)> &Handler) const {
  ForEachNodeOnInDataEdge(Handler);
  ForEachNodeOnOutDataEdge(Handler);
}

TaskEdge *TaskNode::GetSoleEdge(void (TaskNode::*ForEachEdge)(
    const std::function<void(TaskEdge *)> &) const) const {
...
size_t TaskNode::GetEdgesSize(void (TaskNode::*ForEachEdge)(
    const std::function<void(TaskEdge *)> &) const) const {
  size_t size = 0;
...
TaskEdge *TaskNode::SoleInDataEdge() const {
  return GetSoleEdge(&TaskNode::ForEachInDataEdge);
}

TaskEdge *TaskNode::SoleOutDataEdge() const {
  return GetSoleEdge(&TaskNode::ForEachOutDataEdge);
}

size_t TaskNode::in_data_edges_size() const {
  return GetEdgesSize(&TaskNode::ForEachInDataEdge);
}

size_t TaskNode::out_data_edges_size() const {
  return GetEdgesSize(&TaskNode::ForEachOutDataEdge);
}
