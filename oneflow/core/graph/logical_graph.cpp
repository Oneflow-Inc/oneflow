...
void LogicalGraph::ForEachLogicalNode(
  ...
  ForEachNode([&](LogicalNode *logical_node) {
    ...
    if (valid_node != nullptr) {
      valid_nodes.push_back(valid_node);
    }
  });
  for (LogicalNodeType *valid_node : valid_nodes) {
    func(valid_node);
  }
}
...
void LogicalGraph::NaiveBuildFwStruct(
  ...
  for (const PlacementGroup &p_group : placement.placement_group()) {
    for (const std::string &op_name : p_group.op_set().op_name()) {
      CHECK(name2parallel_desc
                .emplace(op_name, std::make_shared<ParallelDesc>(
                                      p_group.parallel_conf()))
                .second);
    }
  }
  ...
  for (OperatorConf cur_op_conf : dlnet_conf.op()) {
    ...
    const std::shared_ptr<ParallelDesc> &parallel_desc_ptr =
        parallel_desc_ptr_it->second;
    cur_op_conf.set_device_tag(
        CHECK_JUST(DeviceTag4DeviceType(parallel_desc_ptr->device_type())));
    std::shared_ptr<Operator> cur_op =
        ConstructOp(cur_op_conf, &GlobalJobDesc());
    ...
    cur_node->mut_parallel_desc() = parallel_desc_ptr;
    {
      ...
      if (op_time_shape_it != name2shape.end()) {
        ...
        if (op_time_shape.has_out_blob_time_shape()) {
          cur_node->reset_out_blob_time_shape(
              new Shape(op_time_shape.out_blob_time_shape()));
        }
...
void LogicalGraph::ForEachNecessaryCtrlEdge(
    const std::function<void(const LogicalNode *, const LogicalNode *, int64_t)>
        &Handler) const {
      ...
      for (const auto &ctrl_in_op_name : op->op_conf().ctrl_in_op_name()) {
        ...
        if (!IsReachable(src, dst)) {
          ...
          int64_t regst_desc_num =
              src_time_shape->elem_cnt() / dst_time_shape->elem_cnt();
...
void LogicalGraph::UpdateEdge2Ibn(const LogicalEdge *edge,
...
void LogicalGraph::UpdateEdge2Obn(const LogicalEdge *edge,
