...
std::function<bool(const LogicalNode*)> MakePredicatorHasActualOutDiff(const LogicalGraph* graph) {
  ...
  //     if (HasBwConnection(in_node, node)) {
  //       Handler(in_node);
  //       LOG(INFO) << "";
  //     }
  //   });
  ...
  //     if (HasBwConnection(node, out_node)) {
  //       Handler(out_node);
  //       LOG(INFO) << "";
  //     }
  //   });
  // };
  auto ForEachReachableToLoss = [&](LogicalNode* node,
                                    const std::function<void(LogicalNode*)>& Handler) {
    node->ForEachNodeOnInEdge([&](LogicalNode* in_node) {
      if (HasBwConnection(in_node, node)) {
        Handler(in_node);
        LOG(INFO) << "";
  ...
  graph->BfsForEachNode(loss_nodes, ForEachReachableToLoss,
