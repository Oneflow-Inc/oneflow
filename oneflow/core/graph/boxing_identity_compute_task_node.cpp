...
void BoxingIdentityCompTaskNode::Init(const CompTaskNode *src_node,
...
void BoxingIdentityCompTaskNode::ProduceAllRegstsAndBindEdges() {
  ...
  this->ForEachOutDataEdge(
      [&](TaskEdge *out_dege) { out_dege->AddRegst("out", out_regst); });
}

void BoxingIdentityCompTaskNode::ConsumeAllRegsts() {
  this->ForEachInDataEdge([&](TaskEdge *in_edge) {
