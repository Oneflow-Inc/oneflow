...
BldBoxingOpConfMthd GetBldBoxingOpConfMethodByFwParallelPolicy(
    const LogicalNode* in_logical, const LogicalNode* out_logical) {
...
HashMap<std::string, FuncForFindBldSubTskGphMthd>*
...
using FuncForFindBldBoxingOpConfMthd = std::function<BldBoxingOpConfMthd(
    const LogicalNode* src, const LogicalNode* dst)>;
HashMap<std::string, FuncForFindBldBoxingOpConfMthd>*
...
void AddFuncForFindBldBoxingOpConfMthd(const std::string& k,
                                       FuncForFindBldBoxingOpConfMthd v) {
  CHECK(GetFuncForFindBldBoxingOpConfMthd()->emplace(k, v).second);
}
void AddFuncForFindBldBoxingOpConfMthd(const std::string& k,
                                       std::function<BldBoxingOpConfMthd()> v) {
  AddFuncForFindBldBoxingOpConfMthd(
      k, [v](const LogicalNode*, const LogicalNode*) { return v(); });
}
void AddFuncForFindBldBoxingOpConfMthd(const std::string& k,
                                       BldBoxingOpConfMthd v) {
  AddFuncForFindBldBoxingOpConfMthd(
      k, [v](const LogicalNode*, const LogicalNode*) { return v; });
}
#define REGISTER_BLD_BOXING_OP_CONF_MTHD(k, v) \
  COMMAND(AddFuncForFindBldBoxingOpConfMthd(k, v))

BldSubTskGphMthd BldSubTskGphToMdSave(const LogicalNode*,
                                      const LogicalNode* save) {
...
const std::vector<LogicalBlobId>& LogicalNode::GetLbisTo(
    const LogicalNode* dst) const {
...
BldSubTskGphMthd GetMthdForBldSubTskGph(const LogicalNode* src_node,
  ...
  if (src_pd->parallel_num() == dst_pd->parallel_num()) {
    if (src_pd->policy() == kDataParallel
        && dst_pd->policy() == kDataParallel) {
      return &TaskGraph::BldSubTskGphByOneToOne;
    }
  }
...
BldBoxingOpConfMthd GetMthdForBldBoxingOpConf(const LogicalNode* src,
  ...
  if (it != GetFuncForFindBldBoxingOpConfMthd()->end()) {
...
REGISTER_BLD_BOXING_OP_CONF_MTHD("Loss"
                                 "NormalBackward",
                                 &GetBldBoxingOpConfMethodByBwParallelPolicy);
REGISTER_BLD_BOXING_OP_CONF_MTHD(
    "LossAcc"
    "LossPrint",
