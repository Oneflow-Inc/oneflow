...
void OpNode::InitLbi2NdSbp() {
  const auto Update = [&](const PbRpf<std::string>& bns) {
    for (const auto& bn : bns) {
      ...
      const cfg::NdSbp& parallel_distribution = NdSbp4BnInOp(bn);
      auto it = lbi2parallel_distribution_.find(lbi);
...
void OpGraph::InferOpNodeNdSbpSignature(
    OpNode* op_node,
    const cfg::NdSbpSignature& parallel_distribution_sig_conf) const {
  ...
  for (const std::string& ibn : op_node->op().input_bns()) {
    ...
    ibn2parallel_distribution_infer_hint.emplace(
        ibn,
        NdSbpInferHint(parallel_desc, logical_blob_desc, parallel_distribution));
  }
  ...
  CHECK_JUST(op_node->mut_op()->InferNdSbpSignatureIf(
      parallel_distribution_sig_conf, op_node->parallel_desc(), NdSbpInferHint4Ibn));
  ...
  JUST(TopoForEachNodeWithErrorCaptured([&](OpNode* op_node) -> Maybe<void> {
    ...
    cfg::NdSbpSignature parallel_distribution_sig_conf;
    {
      ...
      if (iter != op_name2parallel_distribution_sig_conf.end()) {
        parallel_distribution_sig_conf = cfg::NdSbpSignature(iter->second);
    ...
    InferOpNodeNdSbpSignature(op_node, parallel_distribution_sig_conf);
