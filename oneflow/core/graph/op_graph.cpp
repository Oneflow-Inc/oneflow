...
void UpdateJobParallelViewConf(
  ...
  auto *op_name2sbp_signature =
      job_parallel_view_conf->mutable_op_name2sbp_signature_conf();
  auto Update = [&](const std::string &bn) {
    const auto &sbp_parallel =
        op_node.sbp_signature().bn_in_op2sbp_parallel().at(bn);
    const OpBlobArg &oba = GenOpBlobArg(op_node.op().op_name(), bn);
    auto iter = oba2sbp_identical_obas.find(oba);
    if (iter == oba2sbp_identical_obas.end()) {
      return;
    }
    for (const auto &identical_obas : iter->second) {
      auto *sbp_signature = &(*op_name2sbp_signature)[identical_obas.op_name()];
      auto iter = sbp_signature->mutable_bn_in_op2sbp_parallel()->find(
...
bool OpEdge::CalcIsStrict121Connected() const {
  OpNode *src = src_node();
  OpNode *dst = dst_node();
  if (!src->parallel_desc().Equals(dst->parallel_desc())) {
    return false;
  }
  if (src->IsTimeShapeIdentity() == false) {
    return false;
  }
  if (dst->IsTimeShapeIdentity() == false) {
    return false;
  }
  if (*src->GetInputOutputFastestTimeShape() !=
      *dst->GetInputOutputFastestTimeShape()) {
    return false;
  }
...
const SbpParallel &
OpNode::SbpParallel4BnInOp(const std::string &bn_in_op) const {
...
std::string OpNode::VisualStr() const {
  std::string str = op().op_name();
  {
    for (int64_t machine_id : parallel_desc().sorted_machine_ids()) {
      ...
      std::string parallel_desc_str =
          std::to_string(machine_id) + ":" + dev_type + ":";
  ...
  if (in_edges().empty() == false) {
    str += "\\n" + GetTimeShapeStr(*GetInputBlobFastestTimeShape(),
                                   "in_blob_time_shape");
  }
...
BlobDesc *OpNode::MutLogicalBlobDesc4Lbi(const LogicalBlobId &lbi) {
  ...
  if (lbi2logical_blob_desc_.find(lbi) == lbi2logical_blob_desc_.end()) {
    lbi2logical_blob_desc_[lbi].reset(
        new BlobDesc(GlobalJobDesc().DefaultDataType()));
  }
...
Shape *OpNode::mut_out_blob_time_shape() {
  if (!out_blob_time_shape_) {
    out_blob_time_shape_.reset(new Shape());
  }
...
void OpNode::ForEachSplitOrBroadcastBlobDesc(
  ...
  if (sbp_parallel.has_split_parallel()) {
    ...
    BalancedSplitter bs(blob_desc.shape().At(axis),
    ...
    CHECK(sbp_parallel.has_broadcast_parallel() ||
...
void OpNode::ConcatBlobDesc(
    const ParallelDesc &blob_parallel_desc,
  ...
  if (sbp_parallel.has_split_parallel()) {
    ...
    BalancedSplitter bs(logical_blob_axis_dim,
                        blob_parallel_desc.parallel_num());
    ...
    FOR_RANGE(int64_t, axis_parallel_id, 0, blob_parallel_desc.parallel_num()) {
      CHECK_EQ(bs.At(axis_parallel_id).size(),
               blob_descs.at(axis_parallel_id)->shape().At(axis));
      same_blob_descs.at(axis_parallel_id)
          .reset(new BlobDesc(*blob_descs.at(axis_parallel_id)));
      same_blob_descs.at(axis_parallel_id)
          ->mut_shape()
          .Set(axis, logical_blob_axis_dim);
    }
    ...
    FOR_RANGE(int64_t, i, 1, blob_descs.size()) {
      CHECK(*blob_descs.at(i) == *blob_descs.at(0));
    }
...
int64_t OpNode::GetAxisParallelNum(
    const std::function<void(bool *, int32_t *, int64_t *)>
        &GetAxisParallelInfo) const {
  bool is_split = false;
  int32_t axis = -1;
...
void OpNode::SplitLogicalInputBlobDesc() {
  for (const std::string &bn : op().input_bns()) {
    ...
    const BlobDesc &logical_blob_desc =
        SrcNode4Ibn(bn).LogicalBlobDesc4Lbi(lbi);
    ...
    ForEachSplitOrBroadcastBlobDesc(
        logical_blob_desc, sbp_parallel, [&](const BlobDesc &blob_desc) {
          ...
          CHECK_NE(bn2parallel_id2blob_desc_[bn].back()->data_type(),
                   DataType::kInvalidDataType);
        });
    CHECK_EQ(bn2parallel_id2blob_desc_.at(bn).size(),
             parallel_desc().parallel_num());
  }
}
...
void OpNode::ConcatLogicalOutputBlobDesc() {
  for (const std::string &obn : op().output_bns()) {
    ...
    for (const auto &blob_desc : bn2parallel_id2blob_desc_.at(obn)) {
      if (blob_desc) {
        paralleled_blob_descs.push_back(blob_desc);
      }
    }
...
void OpNode::CheckBlobDescs(
    const std::function<BlobDesc *(const std::string &)> &GetBlobDesc4BnInOp,
    const ParallelContext *parallel_ctx) const {
  return;
  int64_t parallel_id = parallel_ctx->parallel_id();
  auto Check = [&](const std::string &bn) {
    if (bn2parallel_id2blob_desc_.find(bn) == bn2parallel_id2blob_desc_.end()) {
      return;
    }
    CHECK_EQ(parallel_ctx->parallel_num(),
             bn2parallel_id2blob_desc_.at(bn).size());
    const BlobDesc &blob_desc_from_exec_graph = *GetBlobDesc4BnInOp(bn);
    const BlobDesc &blob_desc_from_op_graph =
        *bn2parallel_id2blob_desc_.at(bn).at(parallel_id);
    CHECK_EQ(blob_desc_from_exec_graph.shape(),
             blob_desc_from_op_graph.shape());
    CHECK_EQ(blob_desc_from_exec_graph.data_type(),
             blob_desc_from_op_graph.data_type());
  };
  for (const std::string &bn : op().input_bns()) {
    Check(bn);
  }
  ...
  for (const std::string &bn : op().tmp_bns()) {
    Check(bn);
  }
  for (const std::string &bn : op().const_buf_bns()) {
    Check(bn);
  }
}
...
void OpNode::InferBlobParallelDesc() {
  ...
  CHECK_JUST(op().InferOutParallelDescIf(ParallelDesc4Obn, LogicalBlobDesc4Ibn,
...
void OpNode::InitInputBlobFastestTimeShape() {
  ...
  for (OpEdge *edge : in_edges()) {
    ...
    if (fastest == nullptr || shape->elem_cnt() > fastest->elem_cnt()) {
      fastest = shape;
    }
  }
  for (OpEdge *edge : in_edges()) {
    CHECK_EQ(fastest->elem_cnt() %
                 edge->src_node()->out_blob_time_shape()->elem_cnt(),
             0);
  }
  if (fastest != nullptr) {
    input_blob_fastest_time_shape_.reset(new Shape(fastest->dim_vec()));
  }
}
...
void OpGraph::InitNodes(const Job &job) {
  ...
  for (const auto &op_conf : job.net().op()) {
    ...
    OpNode *node =
        new OpNode(ParallelDesc(*ParallelConf4OpName(op_conf.name())), op_conf);
...
void OpGraph::InitEdges() {
  ...
  ForEachNode([&](OpNode *op_node) {
    for (const auto &obn : op_node->op().output_bns()) {
      ...
      if (!lbi2obn) {
        lbi2obn.reset(new HashMap<LogicalBlobId, std::string>());
      }
  ...
  ForEachNode([&](OpNode *op_node) {
    ...
    std::shared_ptr<HashMap<LogicalBlobId, std::vector<std::string>>>
        consumer_lbi2ibns(new HashMap<LogicalBlobId, std::vector<std::string>>);
    for (const auto &ibn : op_node->op().input_bns()) {
    ...
    for (const auto &pair : producer_op_name2lbis) {
      std::shared_ptr<std::vector<LogicalBlobId>> lbis(
          new std::vector<LogicalBlobId>(
              {pair.second.begin(), pair.second.end()}));
      const auto it = producer_op_name2lbi2obn.find(pair.first);
      CHECK(it != producer_op_name2lbi2obn.end()) << "producer_op_name: "
                                                  << pair.first;
...
void OpGraph::InitProducerOpName2CtrlConsumerOpNames(const Job &job) {
  for (const auto &op_conf : job.net().op()) {
    for (const auto &ctrl_in_op_name : op_conf.ctrl_in_op_name()) {
      auto *consumer_op_names =
          &producer_op_name2ctrl_consumer_op_names_[ctrl_in_op_name];
...
void OpGraph::InferBlobLastUsed() const {
  ...
  for (auto iter = op_names_.rbegin(); iter != op_names_.rend(); iter++) {
    ...
    auto *map =
        op->mut_blob_last_used_signature()->mutable_bn_in_op2blob_last_used();
...
void OpGraph::InferTimeShape() const {
  TopoForEachNode([&](OpNode *op_node) {
    ...
    CHECK_JUST(op_node->op().InferOutputBlobTimeShapeIf(
        GetInputBlobTimeShape, &parallel_ctx,
        op_node->mut_out_blob_time_shape()));
  });
}

void OpGraph::InferOpNodeSbpSignature(OpNode *op_node,
  ...
  for (const std::string &ibn : op_node->op().input_bns()) {
    ...
    ibn2sbp_infer_hint.emplace(
        ibn, SbpInferHint(parallel_desc, logical_blob_desc, sbp, batch_axis));
  }
  const auto &BatchAxis4BnInOp =
      [&](const std::string &bn_in_op) -> Maybe<const OptInt64 *> {
    return op_node->op().BatchAxis4BnInOp(bn_in_op);
  };
  CHECK_JUST(InferOpSbpSignature(op_node->mut_op(), sbp_sig_conf,
                                 op_node->parallel_desc(), ibn2sbp_infer_hint,
                                 BatchAxis4BnInOp));
  op_node->InitLbi2SbpParallel();
}

Maybe<void> OpGraph::InferOpNodeMirroredSignature(OpNode *op_node,
                                                  bool is_mirrored_conf) const {
  ...
  for (const std::string &ibn : op_node->op().input_bns()) {
    ...
    MirroredSigInferHint infer_ctx(
        parallel_desc, opt_mirrored_parallel.has_mirrored_parallel());
  ...
  JUST(op_node->mut_op()->InferMirroredSignatureIf(
      MirroredSigInferHint4Ibn, is_mirrored_conf, op_node->parallel_desc()));
  return Maybe<void>::Ok();
}
  ...
  FOR_RANGE(int64_t, parallel_id, 0, parallel_num) {
    auto BlobDesc4BnInOp = [&](const std::string &bn) -> BlobDesc * {
      if (std::find(input_bns.begin(), input_bns.end(), bn) !=
          input_bns.end()) {
        CHECK(bn2parallel_id2blob_desc->find(bn) !=
              bn2parallel_id2blob_desc->end());
        CHECK_EQ(bn2parallel_id2blob_desc->at(bn).size(), parallel_num);
      } else if (bn2parallel_id2blob_desc->find(bn) ==
                 bn2parallel_id2blob_desc->end()) {
      ...
      if (!*blob_desc) {
        blob_desc->reset(new BlobDesc(GlobalJobDesc().DefaultDataType()));
      }
...
const OpNode *OpGraph::OpNode4OpName(const std::string &op_name) const {
  ...
  if (op_node_it == op_name2op_node_.end()) {
    return nullptr;
  }
...
Maybe<void> OpGraph::InferLogicalBlobDesc(const Job &job) const {
  ...
  JUST(TopoForEachNodeWithErrorCaptured([&](OpNode *op_node) -> Maybe<void> {
    ...
    const auto &BatchAxis4Ibn =
      ...
      return producer->op().BatchAxis4BnInOp(
          *JUST(producer->op().obn4lbi(lbi)));
    };
    const auto &LogicalBlobDesc4Ibn =
        [&](const std::string &ibn) -> const BlobDesc & {
    ...
    JUST(op_node->mut_op()->InferBatchAxisIf(LogicalBlobDesc4Ibn,
                                             BatchAxis4Ibn));
    // Infer mirrored_signature
    bool is_mirrored_conf = false;
    {
      const auto &op_name2is_mirrored =
          job_parallel_view_conf.op_name2is_mirrored_parallel_view();
      const auto &iter = op_name2is_mirrored.find(op_node->op().op_name());
      if (iter != op_name2is_mirrored.end()) {
        is_mirrored_conf = iter->second;
      }
    }
    ...
    {
      const auto &op_name2sbp_sig_conf =
          job_parallel_view_conf.op_name2sbp_signature_conf();
      const auto &iter = op_name2sbp_sig_conf.find(op_node->op().op_name());
      if (iter != op_name2sbp_sig_conf.end()) {
        sbp_sig_conf = iter->second;
      }
    }
    InferOpNodeSbpSignature(op_node, sbp_sig_conf);
    op_node->InferBlobParallelDesc();
    UpdateJobParallelViewConf(*op_node, oba2sbp_identical_obas,
    ...
    JUST(op_node->mut_op()->FillLogicalBlobDescSignature(
        [&](const std::string &bn_in_op) -> Maybe<const BlobDesc &> {
          return op_node->LogicalBlobDesc4Lbi(
              op_node->op().BnInOp2Lbi(bn_in_op));
        }));
    return Maybe<void>::Ok();
  }));
  return Maybe<void>::Ok();
...
int32_t OpGraph::GetModelSplitAxis(const std::string &op_name,
                                   const LogicalBlobId &lbi) const {
  const SbpParallel &sbp_parallel = GetSbpParallel(op_name, lbi);
  CHECK(sbp_parallel.has_split_parallel());
...
int64_t OpGraph::GetSplitNum(const std::string &op_name,
                             const LogicalBlobId &lbi) const {
  OpNode *op_node = op_name2op_node_.at(GetOpNameKey(op_name, lbi));
  ...
  CHECK(sbp_parallel.has_split_parallel());
  return op_node->LogicalBlobDesc4Lbi(lbi_key).shape().At(
...
bool OpGraph::IsBatchAxisBlob(const std::string &op_name,
                              const LogicalBlobId &lbi) const {
  const auto &op = *op_name2op_node_.at(GetOpNameKey(op_name, lbi));
  const auto &opt_int64 =
      *CHECK_JUST(op.BatchAxis4Lbi(GetLogicalBlobIdKey(op_name, lbi)));
  return opt_int64.has_value();
}

void OpGraph::CheckBlobDescs(
  ...
  op_name2op_node_.at(op_name)->CheckBlobDescs(GetBlobDesc4BnInOp,
                                               parallel_ctx);
}
...
void OpGraph::ForEachChainFamily(
  ...
  ForEachConnectedComponent(ForEachConnectedWithSameSbp7ParallelDesc7TimeShape,
                            Handler);
}

std::string OpGraph::GetOpNameKey(const std::string &op_name,
                                  const LogicalBlobId &lbi) const {
...
std::function<const BlobDesc &(const LogicalBlobId &)>
OpGraph::MakeGetterBlobDesc4ModelLbi() const {
  ...
  TopoForEachNode([&](OpNode *op_node) {
    ...
    for (const auto &obn : op_node->op().output_bns()) {
      (*sbp_signature.mutable_bn_in_op2sbp_parallel())[obn]
          .mutable_split_parallel()
          ->set_axis(0);
    }
    auto MutUnparalleledBlobDesc4BnInOp =
        [&](const std::string &bn) -> BlobDesc * {
    ...
    CHECK_JUST(op_node->op().InferOutBlobDescsIf(MutUnparalleledBlobDesc4BnInOp,
                                                 &parallel_ctx, &sbp_signature,
                                                 [](OpContext *) {}));
  });
  auto model_lbi2blob_desc =
      std::make_shared<HashMap<LogicalBlobId, std::unique_ptr<BlobDesc>>>();
  ForEachNode([&](OpNode *op_node) {
    for (const std::string &tmp_bn : op_node->op().tmp_bns()) {
      ...
      if (iter == lbi2unparalleled_blob_desc.end()) {
        continue;
      }
      CHECK(model_lbi2blob_desc->emplace(lbi, std::move(iter->second)).second);
    }
  });
  return [model_lbi2blob_desc](
             const LogicalBlobId &model_lbi) -> const BlobDesc & {
    return *model_lbi2blob_desc->at(model_lbi);
  };
}

void OpGraph::ForEachDataAndCtrlInNode(
    OpNode *node, const std::function<void(OpNode *)> &Handler) const {
...
void OpGraph::ForEachDataAndCtrlOutNode(
    OpNode *node, const std::function<void(OpNode *)> &Handler) const {
  node->ForEachNodeOnOutEdge(Handler);
  const auto &op_name_it =
      producer_op_name2ctrl_consumer_op_names_.find(node->op().op_name());
  if (op_name_it == producer_op_name2ctrl_consumer_op_names_.end()) {
    return;
  }
...
OpGraph::MakePredicatorIsOpNameDataOrCtrlReachable() const {
  ...
  return [IsDataOrCtrlReachable, this](const std::string &lhs,
                                       const std::string &rhs) {
    const auto &src_node_it = op_name2op_node_.find(lhs);
    if (src_node_it == op_name2op_node_.end()) {
      return false;
    }
    const auto &dst_node_it = op_name2op_node_.find(rhs);
    if (dst_node_it == op_name2op_node_.end()) {
      return false;
    }
...
std::function<bool(const OpNode *, const OpNode *)>
OpGraph::MakePredicatorIsDataOrCtrlReachable() const {
  auto _1 = std::placeholders::_1;
  auto _2 = std::placeholders::_2;
  return MakePredicatorIsReachable(
      DataOrCtrlSourceNodes(),
...
void OpGraph::DumpOpTimeShape(Job *job) const {
  ForEachNode([&](OpNode *op_node) {
    auto *op_time_shape =
        &(*job->mutable_helper()
               ->mutable_op_name2op_time_shape())[op_node->op().op_name()];
    if (op_node->out_blob_time_shape() != nullptr) {
      op_node->out_blob_time_shape()->ToProto(
          op_time_shape->mutable_out_blob_time_shape());
    }
    const auto *in_blob_fastest_time_shape =
        op_node->GetInputBlobFastestTimeShape();
    if (in_blob_fastest_time_shape != nullptr) {
      in_blob_fastest_time_shape->ToProto(
          op_time_shape->mutable_in_blob_fastest_time_shape());
    }
  });
}
...
Maybe<void> OpGraph::ForEachOpNode(
    const std::function<Maybe<void>(const OpNode &)> &DoEach) const {
