...
void ForEachInplacedRegstDescs(
    ...
    const std::function<void(const HashSet<const RegstDescProto *> &)>
        &Handler) {
  InplaceRegstGraph inplace_gph(regst_desc);
  inplace_gph.ForEachConnectedComponent(
      [&](const HashSet<const InplaceRegstNode *> &nodes) {
        if (nodes.size() == 1) {
          return;
        }
        HashSet<const RegstDescProto *> regst_descs;
        for (const auto *node : nodes) {
          CHECK(regst_descs.emplace(node->regst_desc()).second);
        }
        Handler(regst_descs);
      });
}
...
SharableMemBlockNode::SharableMemBlockNode(
    int64_t chain_id, const HashSet<const RegstDescProto *> &regst_descs)
    : chain_id_(chain_id),
...
SharableMemBlockGraph::SharableMemBlockGraph(
  ...
  for (const TaskProto &task : plan_task_gph.plan().task()) {
    for (const auto &pair : task.produced_regst_desc()) {
      if (IsConsumersAndProducerInSameChain(pair.second, plan_task_gph) &&
        ...
        CHECK(chain_id2regst_descs[task.task_set_info().chain_id()]
                  .emplace(&pair.second)
                  .second);
      }
    }
  ...
  for (const auto &pair : chain_id2regst_descs) {
    ...
    ForEachInplacedRegstDescs(
        pair.second, [&](const HashSet<const RegstDescProto *> &regst_descs) {
...
void SharableMemBlockGraph::ForEachSourceNodeGroup(
    ...
    const std::function<void(const std::vector<const SharableMemBlockNode *> &)>
        &Handler) const {
  HashMap<int64_t, std::vector<const SharableMemBlockNode *>>
