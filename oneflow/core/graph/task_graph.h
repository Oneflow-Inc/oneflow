...
private:
  void AcyclicTopoForEachNode(
      std::function<bool(TaskNode *node)> IsAllowedStartNode,
      const std::function<void(TaskNode *node)> &Handler) const;

  void
  BuildTaskPath(CompTaskNode *src, CompTaskNode *dst,
  ...
  TaskNode *BuildTaskStep(
      ...
      const std::function<TaskNode *(int64_t machine_id, int32_t mem_zone_id)>
          &GetBufTask,
      const std::function<TaskNode *(int64_t machine_id, int32_t mem_zone_id,
                                     TaskNode *)> &SetBufTask,
      bool use_buf_task_node);
  ...
  void SetAreaIdForNewNodes(const LogicalNode *src_logical,
  ...
  void GetInplaceOpBlobArgList(
      ...
      const std::function<const TaskNode *(const std::string &)>
          &TaskNode4OpName) const;
  ...
  void ForEachGpuDeviceNodes(
      const std::function<void(const HashSet<TaskNode *> &dev_nodes)> &Handler)
      const;
