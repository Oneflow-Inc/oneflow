...
bool IsSourceNode(const Operator &op) {
  ...
  if (op_conf.has_distribute_clone_conf() &&
      op_conf.distribute_clone_conf().is_variable_ref()) {
    return true;
  }
  if (op_conf.has_distribute_split_conf() &&
...
void CheckSubGraph(const HashSet<const InplaceLbiNode *> &nodes) {
  ...
  for (const auto *node : nodes) {
    ...
    if (dynamic_cast<const UpdateInplaceLbiNode *>(node) != nullptr) {
      ...
      CHECK(dynamic_cast<const SourceOpInplaceLbiNode *>(
                node->SoleInEdge()->src_node()) != nullptr)
          << "UpdateInplaceLbiNode-lbi: " << PbMessage2TxtString(node->lbi())
          << ", src_node.in_edges_size: "
          << node->SoleInEdge()->src_node()->in_edges().size()
...
const InplaceLbiNode *
GetRoot(const HashSet<const InplaceLbiNode *> &nodes,
...
const InplaceLbiNode *
FindSoleIsMutableIbnConsumer(const SourceOpInplaceLbiNode *node) {
  const InplaceLbiNode *ret = nullptr;
  for (const InplaceLbiEdge *edge : node->out_edges()) {
    if (dynamic_cast<const UpdateInplaceLbiNode *>(edge->dst_node()) !=
        nullptr) {
      CHECK_ISNULL(ret);
      ret = edge->dst_node();
    }
...
void GetUnconnectedNodes(
  ...
  for (const InplaceLbiNode *node : nodes) {
    size_t cnt = 0;
    for (const InplaceLbiEdge *edge : node->in_edges()) {
      cnt += IsValidEdge(edge);
    }
    for (const InplaceLbiEdge *edge : node->out_edges()) {
      cnt += IsValidEdge(edge);
    }
    if (cnt == 0) {
      CHECK(cur_disabled_nodes->emplace(node).second);
    }
  }
}

const InplaceLbiNode *
GetFirstDiffNode(const std::vector<const InplaceLbiNode *> &lhs,
...
std::function<void(const InplaceLbiNode *,
                   const std::function<void(const InplaceLbiNode *)> &)>
GetForEachValidOutNode(
    const HashSet<const InplaceLbiNode *> *nodes,
    std::function<bool(const InplaceLbiEdge *)> IsValidEdge) {
  return [nodes, IsValidEdge](
      const InplaceLbiNode *node,
      const std::function<void(const InplaceLbiNode *)> &Handler) {
    node->ForEachNodeOnValidOutEdge(
        IsValidEdge, [&](const InplaceLbiNode *out_node) {
...
bool IsOtherIbnBoundToOneOfLbis(const HashSet<LogicalBlobId> &lbis,
                                const InplaceLbiEdge *edge) {
  const Operator &op = edge->op();
  for (const std::string &ibn : op.input_bns()) {
    if (ibn != edge->ibn() && lbis.find(op.BnInOp2Lbi(ibn)) != lbis.end()) {
      return true;
    }
  }
  return false;
...
void RemoveUnconnectedNodes(
    HashSet<const InplaceLbiNode *> *nodes,
    const std::function<bool(const InplaceLbiEdge *)> &IsValidEdge) {
  HashSet<const InplaceLbiNode *> cur_disabled_nodes;
  GetUnconnectedNodes(*nodes, IsValidEdge, &cur_disabled_nodes);
  for (const auto *node : cur_disabled_nodes) {
    nodes->erase(node);
  }
}
...
const InplaceLbiEdge *InplaceLbiNode::GetValidInEdge(
  ...
  if (!in_edges().empty() && IsValidEdge(SoleInEdge())) {
    return SoleInEdge();
  }
  return nullptr;
}
...
std::function<InplaceLbiNode *(const LogicalBlobId &)>
InplaceLbiGraph::MakeMutFindOrCreateNode(
    std::function<const Operator *(const std::string &)> Op4OpName) {
  auto lbi2node = std::make_shared<HashMap<LogicalBlobId, InplaceLbiNode *>>();
  return [this, lbi2node,
          Op4OpName](const LogicalBlobId &lbi) -> InplaceLbiNode * {
...
void InplaceLbiGraph::Init(
    const InplaceObasInfo &obas_info,
    const std::function<const Operator *(const std::string &)> &Op4OpName) {
  auto FindOrCreateNode = MakeMutFindOrCreateNode(Op4OpName);
  auto AddEdge = [&](const Operator &op, const LogicalBlobId &lbi,
                     const std::string &ibn, const std::string &obn,
                     bool is_mut) {
    auto *edge = new InplaceLbiEdge(&op, ibn, obn, is_mut);
    AddAllocatedEdge(edge);
    Connect<InplaceLbiNode, InplaceLbiEdge>(
        FindOrCreateNode(op.BnInOp2Lbi(ibn)), edge, FindOrCreateNode(lbi));
  };

  auto BuildNodeAndEdge4InplacePairs = [&](const OpBlobArgPairs &pairs,
                                           bool is_mut) {
    for (const auto &pair : pairs.pair()) {
      ...
      CHECK(std::find(op.input_bns().begin(), op.input_bns().end(), ibn) !=
            op.input_bns().end());
  ...
  ForEachNode(
      [](const InplaceLbiNode *node) { CHECK_LE(node->in_edges().size(), 1); });
...
void InplaceLbiGraph::ComputeSafeInplaceObns(
  ...
  ComputeSafeInplaceEdges(IsReachableFromLbiToOpName, [&](const InplaceLbiEdge
                                                              *edge) {
...
void InplaceLbiGraph::ForEachSafeInplaceEdgeInSourceOpSubTree(
  ...
  const InplaceLbiNode *root =
      GetRoot(nodes, [](const InplaceLbiEdge *) { return true; });
  const auto *source_op_root =
      dynamic_cast<const SourceOpInplaceLbiNode *>(root);
  if (source_op_root != nullptr) {
    const InplaceLbiNode *updt_node =
        FindSoleIsMutableIbnConsumer(source_op_root);
    if (updt_node != nullptr) {
      ...
      FixConstRefOrMutRefConflictsToUpdtNode(nodes, IsReachableFromLbiToOpName,
                                             &cur_disabled_edges);
      disabled_edges->insert(cur_disabled_edges.begin(),
                             cur_disabled_edges.end());
    }
    {
      HashSet<const InplaceLbiEdge *> cur_disabled_edges;
      FixMutRefConflictsFromSourceOpNode(source_op_root, IsValidEdge,
                                         &cur_disabled_edges);
      disabled_edges->insert(cur_disabled_edges.begin(),
                             cur_disabled_edges.end());
    }
    {
      ...
      BfsForEachNode(
          ...
          [&](const InplaceLbiNode *node) {
            const InplaceLbiEdge *in_edge = node->GetValidInEdge(IsValidEdge);
            if (in_edge != nullptr) {
              CHECK(cur_disabled_edges.emplace(in_edge).second);
            }
            if (dynamic_cast<const NormalInplaceLbiNode *>(node) != nullptr) {
              CHECK_NOTNULL(in_edge);
              if (node->IsConstRef(IsValidEdge)) {
                Handler(in_edge);
              }
            }
          });
...
void InplaceLbiGraph::ComputeSafeInplaceEdges(
  ...
  {
    // compute safe inplace edges in the subtree containning
    // SourceOpInplaceLbiNode as root
    HashSet<const InplaceLbiEdge *> cur_disabled_edges;
    ForEachSafeInplaceEdgeInSourceOpSubTree(remainder_nodes,
                                            IsReachableFromLbiToOpName, Handler,
                                            &cur_disabled_edges);
  ...
  while (!remainder_nodes.empty()) {
    ForEachTree(remainder_nodes, IsValidEdge,
                [&](const HashSet<const InplaceLbiNode *> &nodes) {
                  const InplaceLbiEdge *cur_disabled_edge =
                      FindFirstInterOpRefConflictMutRefEdge(
                          nodes, IsValidEdge, IsReachableFromLbiToOpName);
                  if (cur_disabled_edge != nullptr) {
                    disabled_edges.insert(cur_disabled_edge);
                  }
                });
    ForEachTree(remainder_nodes, IsValidEdge,
                [&](const HashSet<const InplaceLbiNode *> &nodes) {
                  const InplaceLbiEdge *cur_disabled_edge =
                      FindFirstConstRefConflictMutRefEdge(
                          nodes, IsValidEdge, IsReachableFromLbiToOpName);
                  if (cur_disabled_edge != nullptr) {
                    disabled_edges.insert(cur_disabled_edge);
                  }
                ...
                });
    {
      ...
      GetSafeInplaceObnEdges(remainder_nodes, IsValidEdge,
                             IsReachableFromLbiToOpName,
                             &cur_safe_inplace_obn_edges);
      for (const auto *edge : cur_safe_inplace_obn_edges) {
        Handler(edge);
      }
      disabled_edges.insert(cur_safe_inplace_obn_edges.begin(),
                            cur_safe_inplace_obn_edges.end());
    }
    RemoveUnconnectedNodes(&remainder_nodes, IsValidEdge);
...
void InplaceLbiGraph::FindAllEdges(
    const HashSet<const InplaceLbiNode *> &nodes,
    const std::function<bool(const InplaceLbiEdge *)> &IsValidEdge,
  ...
  for (const auto *node : nodes) {
    node->ForEachNodeOnValidOutEdge(
        IsValidEdge, [&](const InplaceLbiNode *out_node) {
          CHECK(cur_disabled_edges
                    ->emplace(out_node->GetSoleValidInEdge(IsValidEdge))
                    .second);
        });
  }
}
...
const InplaceLbiEdge *InplaceLbiGraph::FindFirstIntraOpRefConflictMutRefEdge(
  ...
  for (const auto *node : nodes) {
    CHECK(lbis.insert(node->lbi()).second);
  }
  ...
  TopoForEachNode({root}, ForEachInNode, ForEachOutNode,
                  [&](const InplaceLbiNode *node) {
                    if (ret != nullptr) {
                      return;
                    }
                    if (node->IsMutRef(IsValidEdge) &&
                        IsOtherIbnBoundToOneOfLbis(lbis, node->SoleInEdge())) {
                      ret = node->SoleInEdge();
                    }
                  });
  return ret;
...
bool InplaceLbiGraph::IsConstRefConflictMutRefNode(
    const InplaceLbiNode *mut_ref_node,
    const HashSet<const InplaceLbiNode *> &nodes,
    const std::function<bool(const InplaceLbiEdge *)> &IsValidEdge,
    const std::function<bool(const LogicalBlobId &, const std::string &)>
  ...
  auto ForEachNext =
      [&](const InplaceLbiNode *node,
          const std::function<void(const InplaceLbiNode *)> &Handler) {
        node->ForEachNodeOnValidOutEdge(IsValidEdge,
                                        [&](const InplaceLbiNode *out_node) {
                                          if (out_node != mut_ref_node) {
                                            Handler(out_node);
                                          }
                                        });
      };
  ...
  BfsForEachNode({GetRoot(nodes, IsValidEdge)}, ForEachNext,
                 [&](const InplaceLbiNode *node) {
                   conflict =
                       conflict ||
                       !IsLbiAllConsumerReachableToOpName(node->lbi(), op_name);
                 });
  return conflict;
}
...
const InplaceLbiEdge *InplaceLbiGraph::FindFirstConstRefConflictMutRefEdge(
  ...
  const InplaceLbiEdge *ret = nullptr;
  TopoForEachNode(
      {root}, ForEachInNode, ForEachOutNode,
      [&](const InplaceLbiNode *node) {
        if (ret != nullptr) {
          return;
        }
...
const InplaceLbiEdge *InplaceLbiGraph::FindFirstInterOpRefConflictMutRefEdge(
  ...
  HashMap<const InplaceLbiNode *, std::vector<const InplaceLbiNode *>>
      node2mut_ref_ancestors;
  {
    const InplaceLbiNode *root = GetRoot(nodes, IsValidEdge);
    auto ForEachInNode = GetForEachValidInNode(&nodes, IsValidEdge);
    auto ForEachOutNode = GetForEachValidOutNode(&nodes, IsValidEdge);
    TopoForEachNode({root}, ForEachInNode, ForEachOutNode,
                    [&](const InplaceLbiNode *node) {
                      if (node->IsMutRef(IsValidEdge)) {
                        mut_ref_nodes.insert(node);
                      }
                      size_t in_edges_size_check = 0;
                      ForEachInNode(node, [&](const InplaceLbiNode *in_node) {
                        node2mut_ref_ancestors[node] =
                            node2mut_ref_ancestors[in_node];
                        if (in_node->IsMutRef(IsValidEdge)) {
                          node2mut_ref_ancestors[node].push_back(in_node);
                        }
  ...
  std::vector<const InplaceLbiNode *> last_mut_ref_nodes;
  {
    ...
    for (const InplaceLbiNode *descendent : mut_ref_nodes) {
      for (const InplaceLbiNode *ancestor :
           node2mut_ref_ancestors.at(descendent)) {
        ++mut_ref_node2descendents_size[ancestor];
      }
    }
    for (const InplaceLbiNode *node : mut_ref_nodes) {
      if (mut_ref_node2descendents_size[node] == 0) {
        last_mut_ref_nodes.push_back(node);
      }
    }
  }
  if (last_mut_ref_nodes.size() <= 1) {
    return nullptr;
  }
  const InplaceLbiNode *first_diff_node = nullptr;
  {
    ...
    if (first_diff_node == nullptr) {
      first_diff_node =
          last_mut_ref_nodes.at(first.size() < second.size() ? 0 : 1);
    }
  }
  ...
  ForEachTree(nodes, IsValidEdge,
              [&](const HashSet<const InplaceLbiNode *> &nodes) {
                ...
                const InplaceLbiEdge *intra_op_conflict_ref_edge =
                    FindFirstIntraOpRefConflictMutRefEdge(nodes, IsValidEdge);
                if (const_ref_conflict_ref_edge == nullptr &&
                    intra_op_conflict_ref_edge == nullptr &&
...
void InplaceLbiGraph::ForEachTree(
    const HashSet<const InplaceLbiNode *> &nodes,
    const std::function<bool(const InplaceLbiEdge *)> &IsValidEdge,
    const std::function<void(const HashSet<const InplaceLbiNode *> &)> &Handler)
    const {
  auto ForEachNode =
      [&](const std::function<void(const InplaceLbiNode *)> &Handler) {
        for (const auto *node : nodes) {
...
void InplaceLbiGraph::FixConstRefOrMutRefConflictsToUpdtNode(
  ...
  CHECK_NOTNULL(root);
  {
    ...
    auto ForEachNext =
        [&](const InplaceLbiNode *node,
          ...
          node->ForEachNodeOnValidOutEdge(
              IsValidEdge, [&](const InplaceLbiNode *out_node) {
                if (dynamic_cast<const NormalInplaceLbiNode *>(out_node) ==
                    nullptr) {
                  return;
                }
                if (out_node->IsMutRef(IsValidEdge)) {
                  return;
                }
                if (!IsLbiAllConsumerReachableToOpName(
                        out_node->lbi(), updt_node->lbi().op_name())) {
                  return;
                }
                Handler(out_node);
              });
        };
    BfsForEachNode({root}, ForEachNext,
                   [&](const InplaceLbiNode *node) {
                     if (node == root) {
                       return;
                     }
  ...
  for (const auto *node : safe_const_ref_nodes) {
    node->ForEachNodeOnValidOutEdge(
        IsValidEdge, [&](const InplaceLbiNode *out_node) {
          if (safe_const_ref_nodes.find(out_node) ==
                  safe_const_ref_nodes.end() &&
              out_node != updt_node) {
  ...
  // remove mutable inplace edges from root which are not end with model update
  // node
  root->ForEachNodeOnValidOutEdge(
      IsValidEdge, [&](const InplaceLbiNode *out_node) {
        const auto *node = dynamic_cast<const NormalInplaceLbiNode *>(out_node);
        if (node != nullptr && node->IsMutRef(IsValidEdge)) {
          CHECK(nodes.find(out_node) != nodes.end());
          CHECK(
              cur_disabled_edges->emplace(node->GetSoleValidInEdge(IsValidEdge))
                  .second);
        }
      });
}
...
void InplaceLbiGraph::FixMutRefConflictsFromSourceOpNode(
  ...
  HashSet<const InplaceLbiNode *> safe_const_ref_nodes;
  {
    ...
    BfsForEachNode({root}, ForEachNext,
                   [&](const InplaceLbiNode *node) {
                     if (dynamic_cast<const NormalInplaceLbiNode *>(node) !=
                         nullptr) {
                       CHECK(safe_const_ref_nodes.emplace(node).second);
                     }
                   });
