...
void GroupParallelIdByMachine(
  ...
  FOR_RANGE(int64_t, parallel_id, 0, pd.parallel_num()) {
    (*machine_id2parallel_ids)[pd.MachineIdForParallelId(parallel_id)]
        .push_back(parallel_id);
  }
}
...
bool IsCopyContiguous(const TensorSliceView &src, const TensorSliceView &dst) {
  ...
  FOR_RANGE(int64_t, i, 1, src.range_vec().size()) {
    if (src.range_vec().at(i) != dst.range_vec().at(i)) {
      return false;
    }
  }
  return true;
...
Maybe<SubTskGphBuilderStatus> SliceBoxingSubTskGphBuilder::Build(
  ...
  if (SubTskGphBuilderUtil::HasEmptySliceIfSplit(
          src_parallel_desc.parallel_num(), src_sbp_parallel,
          logical_blob_desc)) {
    return Error::BoxingNotSupportedError();
  }
  if (SubTskGphBuilderUtil::HasEmptySliceIfSplit(
          dst_parallel_desc.parallel_num(), dst_sbp_parallel,
          logical_blob_desc)) {
    return Error::BoxingNotSupportedError();
  }
  if (!(SubTskGphBuilderUtil::IsBoxingS2B(src_sbp_parallel, dst_sbp_parallel) ||
        ...
        SubTskGphBuilderUtil::IsBoxingB2S(src_sbp_parallel,
                                          dst_sbp_parallel))) {
    return Error::BoxingNotSupportedError();
  }
  const auto GetBoxingGpuThrdId = [](const int64_t dev_id,
  ...
  const auto NewEdge = [&ctx]() -> TaskEdge * {
    return ctx->task_graph()->NewEdge();
  };
  ...
  const auto CreateBoxingNodeToHost =
      [&ctx, &lbi, &GetBoxingGpuThrdId,
       &NewEdge](TaskNode *src_node, const TensorSliceView &src_slice,
                 const TensorSliceView &dst_slice) -> SliceBoxingTaskNode * {
    SliceBoxingTaskNode *dst_node =
        ctx->task_graph()->NewNode<SliceBoxingTaskNode>();
    int64_t thrd_id = -1;
    if (src_node->device_type() == DeviceType::kCPU) {
      thrd_id =
          Global<IDMgr>::Get()->PickCpuThrdIdEvenly(src_node->machine_id());
    } else if (src_node->device_type() == DeviceType::kGPU) {
#ifdef WITH_CUDA
      thrd_id =
          GetBoxingGpuThrdId(src_node->GpuPhyId(), CudaWorkType::kCopyD2H);
#else
      UNIMPLEMENTED();
#endif
    } else {
    ...
    dst_node->Init(lbi, dst_slice, kSliceBoxingTaskModeCopy,
                   src_node->machine_id(), thrd_id,
  ...
  const auto BuildSubTaskGphS2B = [&ctx, &CreateBoxingNode121, &NewEdge](
      ...
      const BlobDesc &blob_desc, const std::vector<TaskNode *> &in_nodes,
      std::vector<TaskNode *> *out_nodes) {
    CHECK(SubTskGphBuilderUtil::IsBoxingS2B(in_sbp, out_sbp));
    const std::vector<TensorSliceView> in_slices =
        SubTskGphBuilderUtil::GetTensorSliceView(in_pd.parallel_num(), in_sbp,
                                                 blob_desc);
    ...
    FOR_RANGE(int64_t, out_id, 0, out_pd.parallel_num()) {
      SliceBoxingTaskNode *out_node = CreateBoxingNode121(
          out_pd, out_id, out_slice, kSliceBoxingTaskModeCopy);
      FOR_RANGE(int64_t, in_id, 0, in_pd.parallel_num()) {
        ...
        if (SubTskGphBuilderUtil::IsOnSameGPU(in_node, out_node)) {
          ...
          TaskNode *proxy_node = ctx->GetProxyNode(
              in_node, in_node->MemZoneId121(), out_node->machine_id(),
              Global<IDMgr>::Get()->CpuMemZoneId());
  ...
  const auto BuildSubTaskGphS2S = [&ctx, &lbi, &CreateBoxingNode121,
    ...
    const std::vector<TensorSliceView> out_slices =
        SubTskGphBuilderUtil::GetTensorSliceView(out_pd.parallel_num(), out_sbp,
                                                 blob_desc);
        ...
        if (out_node->machine_id() == in_machine_id) {
          for (const int64_t in_id : in_parallel_ids) {
            ...
            if (in_pd.device_type() == DeviceType::kGPU &&
              ...
              out_node->ConnectToSrcNodeWithSlice(copy_to_host, NewEdge(),
                                                  intersection);
              ...
              if (IsSameDevice(in_pd, out_pd, in_id, out_id) ||
                  (in_contiguous && out_contiguous)) {
                out_node->ConnectToSrcNodeWithSlice(in_node, NewEdge(),
                                                    in_slice);
              } else if (in_contiguous && !out_contiguous) {
                SliceBoxingTaskNode *copy_to_out_continuous =
                    CreateBoxingNode121(out_pd, out_id, intersection,
                                        kSliceBoxingTaskModeCopy);
                copy_to_out_continuous->ConnectToSrcNodeWithSlice(
                    in_node, NewEdge(), in_slice);
                out_node->ConnectToSrcNodeWithSlice(copy_to_out_continuous,
                                                    NewEdge(), intersection);
              } else if (!in_contiguous && out_contiguous) {
                SliceBoxingTaskNode *in_copy_to_continuous =
                    CreateBoxingNode121(in_pd, in_id, intersection,
                                        kSliceBoxingTaskModeCopy);
                in_copy_to_continuous->ConnectToSrcNodeWithSlice(
                    in_node, NewEdge(), in_slice);
                out_node->ConnectToSrcNodeWithSlice(in_copy_to_continuous,
                                                    NewEdge(), intersection);
              } else {
                SliceBoxingTaskNode *in_copy_to_continuous =
                    CreateBoxingNode121(in_pd, in_id, intersection,
                ...
                SliceBoxingTaskNode *copy_to_out_continuous =
                    CreateBoxingNode121(out_pd, out_id, intersection,
                                        kSliceBoxingTaskModeCopy);
                copy_to_out_continuous->ConnectToSrcNodeWithSlice(
                    in_copy_to_continuous, NewEdge(), intersection);
                out_node->ConnectToSrcNodeWithSlice(copy_to_out_continuous,
                                                    NewEdge(), intersection);
              }
            }
          ...
          const TensorSliceView concat_slice = TensorSliceView::Concatenate(
              intersections, in_sbp.split_parallel().axis());
          ...
          if (in_pd.device_type() == DeviceType::kCPU) {
            ...
            local_concat_thrd_id = GetBoxingGpuThrdId(
                in_nodes
                    .at(in_parallel_ids.at(out_id % in_parallel_ids.size()))
                    ->GpuPhyId(),
          ...
          local_concat_node->Init(lbi, concat_slice, kSliceBoxingTaskModeCopy,
                                  in_machine_id, local_concat_thrd_id,
                                  Global<IDMgr>::Get()->CpuMemZoneId());
          for (const int64_t in_id : in_parallel_ids) {
            local_concat_node->ConnectToSrcNodeWithSlice(
                in_nodes.at(in_id), NewEdge(), in_slices.at(in_id));
          }
          TaskNode *local_add_proxy_node = ctx->GetProxyNode(
              local_concat_node, Global<IDMgr>::Get()->CpuMemZoneId(),
              out_node->machine_id(), Global<IDMgr>::Get()->CpuMemZoneId());
          out_node->ConnectToSrcNodeWithSlice(local_add_proxy_node, NewEdge(),
                                              concat_slice);
        }
      }
      out_nodes->push_back(out_node);
  ...
  const auto BuildSubTaskGphP2S = [&ctx, &lbi, &CreateBoxingNode121,
                                   &CreateBoxingNodeToHost, &GetBoxingGpuThrdId,
                                   &NewEdge](
      const ParallelDesc &in_pd, const ParallelDesc &out_pd,
      const SbpParallel &in_sbp, const SbpParallel &out_sbp,
    ...
    FOR_RANGE(int64_t, out_id, 0, out_pd.parallel_num()) {
      const TensorSliceView &out_slice = out_slices.at(out_id);
      SliceBoxingTaskNode *out_node = CreateBoxingNode121(
          out_pd, out_id, out_slice, kSliceBoxingTaskModeAdd);
      for (const auto &pair : machine_id2in_parallel_ids) {
        const int64_t in_machine_id = pair.first;
        const std::vector<int64_t> &in_parallel_ids = pair.second;
        if (out_node->machine_id() == in_machine_id) {
          ...
          local_add_node->Init(lbi, out_slice, kSliceBoxingTaskModeAdd,
                               in_machine_id, local_add_thrd_id,
                               Global<IDMgr>::Get()->CpuMemZoneId());
          for (const int64_t in_id : in_parallel_ids) {
          ...
          TaskNode *local_add_proxy_node = ctx->GetProxyNode(
              local_add_node, Global<IDMgr>::Get()->CpuMemZoneId(),
              out_node->machine_id(), Global<IDMgr>::Get()->CpuMemZoneId());
          out_node->ConnectToSrcNodeWithSlice(local_add_proxy_node, NewEdge(),
                                              out_slice);
        }
  ...
  const auto BuildSubTaskGphP2B = [&ctx, &lbi, &GetBoxingGpuThrdId, &NewEdge](
    ...
    for (const auto &machine_id7in_parallel_ids : machine_id2in_parallel_ids) {
      ...
      const std::vector<int64_t> &in_ids_on_machine =
          machine_id7in_parallel_ids.second;
...
#ifdef WITH_CUDA
          local_add_thrd_id = GetBoxingGpuThrdId(
              in_nodes.at(in_ids_on_machine.front())->GpuPhyId(),
              CudaWorkType::kCopyH2D);
#else
          UNIMPLEMENTED();
#endif
        }
        local_add_node->Init(lbi, slice, kSliceBoxingTaskModeAdd, in_machine_id,
                             local_add_thrd_id);
        FOR_RANGE(int64_t, i, 0, in_ids_on_machine.size()) {
          local_add_node->ConnectToSrcNodeWithSlice(
              in_nodes.at(in_ids_on_machine.at(i)), NewEdge(), slice);
        }
        out_box_nodes.push_back(local_add_node);
      }
    }
    out_nodes->resize(out_pd.parallel_num());
    for (const auto &machine_id7out_parallel_ids :
         machine_id2out_parallel_ids) {
      const int64_t out_machine_id = machine_id7out_parallel_ids.first;
      TaskNode *in_box_node = nullptr;
      if (out_box_nodes.size() == 1) {
        in_box_node = ctx->GetProxyNode(out_box_nodes.front(),
                                        out_box_nodes.front()->MemZoneId121(),
                                        machine_id7out_parallel_ids.first,
                                        Global<IDMgr>::Get()->CpuMemZoneId());
      } else {
        auto *add_node = ctx->task_graph()->NewNode<SliceBoxingTaskNode>();
        add_node->Init(lbi, slice, kSliceBoxingTaskModeAdd,
                       machine_id7out_parallel_ids.first,
                       Global<IDMgr>::Get()->PickCpuThrdIdEvenly(
                           machine_id7out_parallel_ids.first),
                       Global<IDMgr>::Get()->CpuMemZoneId());
        for (TaskNode *out_box_node : out_box_nodes) {
          TaskNode *out_boxing_node_proxy = ctx->GetProxyNode(
              out_box_node, out_box_node->MemZoneId121(), out_machine_id,
              Global<IDMgr>::Get()->CpuMemZoneId());
          add_node->ConnectToSrcNodeWithSlice(out_boxing_node_proxy, NewEdge(),
                                              slice);
        }
      ...
      for (const int64_t out_id : machine_id7out_parallel_ids.second) {
        ...
        if (out_pd.device_type() == DeviceType::kCPU) {
          ...
          mem_zone_id = Global<IDMgr>::Get()->GpuMemZoneId(
              out_pd.DeviceIdForParallelId(out_id));
        } else {
          UNIMPLEMENTED();
        }
        (*out_nodes)[out_id] =
            ctx->GetProxyNode(in_box_node, Global<IDMgr>::Get()->CpuMemZoneId(),
                              out_machine_id, mem_zone_id);
      }
    }
  };

  const auto BuildSubTaskGphB2S = [&ctx, &lbi, &CreateBoxingNode121,
                                   &CreateBoxingNodeToHost, &GetBoxingGpuThrdId,
                                   &NewEdge, &sorted_src_comp_tasks,
    ...
    FOR_RANGE(int64_t, out_id, 0, out_pd.parallel_num()) {
      ...
      const int64_t nearest_idx = SubTskGphBuilderUtil::FindNearestNodeIndex(
          sorted_src_comp_tasks, dst_node);
      CompTaskNode *src_node = sorted_src_comp_tasks.at(nearest_idx);
      SliceBoxingTaskNode *slice_node = CreateBoxingNode121(
          in_pd, nearest_idx, out_slice, kSliceBoxingTaskModeCopy);
      slice_node->ConnectToSrcNodeWithSlice(src_node, NewEdge(), in_slice);
      TaskNode *out_node =
          ctx->GetProxyNode(slice_node, slice_node->MemZoneId121(),
  ...
  std::string comment;
  if (SubTskGphBuilderUtil::IsBoxingS2B(src_sbp_parallel, dst_sbp_parallel)) {
    BuildSubTaskGphS2B(src_parallel_desc, dst_parallel_desc, src_sbp_parallel,
                       dst_sbp_parallel, logical_blob_desc, in_nodes,
                       &out_nodes);
    comment = "BuildSubTaskGphS2B";
  } else if (SubTskGphBuilderUtil::IsBoxingS2S(src_sbp_parallel,
                                               dst_sbp_parallel)) {
    BuildSubTaskGphS2S(src_parallel_desc, dst_parallel_desc, src_sbp_parallel,
                       dst_sbp_parallel, logical_blob_desc, in_nodes,
                       &out_nodes);
    comment = "BuildSubTaskGphS2S";
  } else if (SubTskGphBuilderUtil::IsBoxingP2S(src_sbp_parallel,
                                               dst_sbp_parallel)) {
    BuildSubTaskGphP2S(src_parallel_desc, dst_parallel_desc, src_sbp_parallel,
                       dst_sbp_parallel, logical_blob_desc, in_nodes,
                       &out_nodes);
    comment = "BuildSubTaskGphP2S";
  } else if (SubTskGphBuilderUtil::IsBoxingP2B(src_sbp_parallel,
                                               dst_sbp_parallel)) {
    if (logical_blob_desc.shape().elem_cnt() <
        dst_parallel_desc.parallel_num()) {
      BuildSubTaskGphP2B(src_parallel_desc, dst_parallel_desc, src_sbp_parallel,
                         dst_sbp_parallel, logical_blob_desc, in_nodes,
                         &out_nodes);
      comment = "BuildSubTaskGphP2B";
    } else {
      BlobDesc flat_blob_desc(logical_blob_desc.data_type());
      flat_blob_desc.mut_shape() =
          Shape({logical_blob_desc.shape().elem_cnt()});
      std::vector<TaskNode *> middle_nodes;
      SbpParallel middle_sbp;
      middle_sbp.mutable_split_parallel()->set_axis(0);
      BuildSubTaskGphP2S(src_parallel_desc, dst_parallel_desc, src_sbp_parallel,
                         middle_sbp, flat_blob_desc, in_nodes, &middle_nodes);
      BuildSubTaskGphS2B(dst_parallel_desc, dst_parallel_desc, middle_sbp,
                         dst_sbp_parallel, flat_blob_desc, middle_nodes,
                         &out_nodes);
  ...
  } else if (SubTskGphBuilderUtil::IsBoxingB2S(src_sbp_parallel,
                                               dst_sbp_parallel)) {
    BuildSubTaskGphB2S(src_parallel_desc, dst_parallel_desc, src_sbp_parallel,
                       dst_sbp_parallel, logical_blob_desc, in_nodes,
                       &out_nodes);
  ...
  return TRY(BuildSubTskGphBuilderStatus(
      ...
      lbi, logical_blob_desc, "SliceBoxingSubTskGphBuilder", comment));
}
