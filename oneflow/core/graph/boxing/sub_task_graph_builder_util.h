...
struct SubTskGphBuilderUtil {
  ...
  static bool IsOnSameGPU(const int64_t src_machine_id, const int64_t src_dev_phy_id, const DeviceType& src_device_type, const int64_t dst_machine_id,
  ...
  static int64_t GetDistance(const int64_t src_machine_id, const int64_t src_dev_phy_id, const DeviceType& src_device_type, const int64_t dst_machine_id,
                             const int64_t dst_dev_phy_id, const DeviceType& dst_device_type);
  static int64_t GetMemZoneId(const int64_t machine_id,
                             const int64_t dev_phy_id, const DeviceType& device_type);
  ...
  static int64_t FindNearestParallelId(const ParallelDesc& from_pd,
                                   const int64_t to_machine_id, const int64_t to_dev_phy_id,
    ...
    for (int64_t i = 0; i < from_pd.parallel_num(); ++i) {
      ...
      int64_t distance = SubTskGphBuilderUtil::GetDistance(from_machine_id, from_dev_phy_id, from_device_type, to_machine_id, to_dev_phy_id, to_device_type);
      if (distance < nearest_distance) {
