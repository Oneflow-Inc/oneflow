...
std::string SbpParallelToString(const SbpParallel &sbp_parallel) {
  ...
  if (sbp_parallel.has_broadcast_parallel()) {
    ...
    serialized_sbp_parallel =
        "S(" + std::to_string(sbp_parallel.split_parallel().axis()) + ")";
  } else {
    UNIMPLEMENTED();
  }
...
std::string ParallelDescToString(const ParallelDesc &parallel_desc) {
  ...
  for (int64_t i = 0; i < sorted_machine_ids.size(); ++i) {
    ...
    serialized_parallel_desc +=
        std::to_string(machine_id) + ":" + device_type + ":";
...
std::string
SubTskGphBuilderStatusToCsvLine(const SubTskGphBuilderStatus &status) {
  ...
  serialized_status +=
      DataType_Name(status.logical_blob_desc().data_type()) + ",";
