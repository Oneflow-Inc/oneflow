...
Maybe<SubTskGphBuilderStatus> NaiveB2PSubTskGphBuilder::Build(
  ...
  if ((src_parallel_desc.parallel_num() == 1 || src_sbp_parallel.has_broadcast_parallel())
    ...
    FOR_RANGE(int64_t, out_id, 0, dst_parallel_desc.parallel_num()) {
      ...
      const int64_t nearest_src_machine_id =
          CHECK_JUST(src_parallel_desc.MachineId4ParallelId(nearest_src_parallel_id));
      const int64_t nearest_src_dev_phy_id =
          CHECK_JUST(src_parallel_desc.DeviceId4ParallelId(nearest_src_parallel_id));
      dst_id2nearest_src_id.emplace(out_id, nearest_src_parallel_id);
      const int64_t distance = SubTskGphBuilderUtil::GetDistance(
          nearest_src_machine_id, nearest_src_dev_phy_id, src_parallel_desc.device_type(),
    ...
    FOR_RANGE(int64_t, out_id, 0, dst_parallel_desc.parallel_num()) {
      ...
      if (out_id == nearest_dst_node_idx) {
        ...
        zeros_node->Init(dst_machine_id, Global<IDMgr>::Get()->GetGpuComputeThrdId(dst_dev_phy_id),
                         NewAreaId(), lbi, logical_blob_desc.shape(), logical_blob_desc.data_type(),
                         time_shape);
