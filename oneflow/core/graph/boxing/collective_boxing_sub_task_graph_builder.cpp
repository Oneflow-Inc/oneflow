...
void NcclInitCollectiveNode(CollectiveBoxingGenericTaskNode *node,
                            ...
                            int64_t parallel_id, const std::string &name,
  ...
  CollectiveBoxingGenericOpConf *conf =
      op_conf.mutable_collective_boxing_generic_conf();
...
int64_t FindRootParallelId(const ParallelDesc &multi_device,
                           const ParallelDesc &sole_device) {
...
class NcclCollectiveBoxingAllReduceSubTskGphBuilder final
    : public SubTskGphBuilder {
public:
  ...
  Build(SubTskGphBuilderCtx *ctx,
    ...
    if (dst_parallel_desc.Equals(src_parallel_desc) &&
      ...
      const std::string op_name =
          "System-Boxing-NcclCollectiveBoxingAllReduce-" + NewUniqueId();
      FOR_RANGE(int64_t, i, 0, src_parallel_desc.parallel_num()) {
        ...
        auto *collective_node =
            ctx->task_graph()->NewNode<CollectiveBoxingGenericTaskNode>();
        NcclInitCollectiveNode(collective_node, src_parallel_desc, i, op_name,
                               lbi, logical_blob_desc, OpType::kOpTypeAllReduce,
                               -1);
        Connect<TaskNode>(src_node, ctx->task_graph()->NewEdge(),
                          collective_node);
        Connect<TaskNode>(collective_node, ctx->task_graph()->NewEdge(),
                          dst_node);
      }
...
class NcclCollectiveBoxingReduceScatterSubTskGphBuilder final
    : public SubTskGphBuilder {
public:
  ...
  Build(SubTskGphBuilderCtx *ctx,
    ...
    if (dst_parallel_desc.Equals(src_parallel_desc) &&
        ...
        dst_parallel_desc.device_type() == DeviceType::kGPU &&
        dst_parallel_desc.parallel_num() > 1 &&
        logical_blob_desc.shape().At(0) % dst_parallel_desc.parallel_num() ==
            0 &&
...
class NcclCollectiveBoxingAllGatherSubTskGphBuilder final
    : public SubTskGphBuilder {
public:
  ...
  Build(SubTskGphBuilderCtx *ctx,
    ...
    if (dst_parallel_desc.EqualsIgnoringDeviceType(src_parallel_desc) &&
      ...
      const std::string op_name =
          "System-Boxing-NcclCollectiveBoxingAllGather-" + NewUniqueId();
      FOR_RANGE(int64_t, i, 0, src_parallel_desc.parallel_num()) {
        CompTaskNode *src_comp_task = sorted_src_comp_tasks.at(i);
        CompTaskNode *dst_node = sorted_dst_comp_tasks.at(i);
        TaskNode *src_node =
            ctx->GetProxyNode(src_comp_task, src_comp_task->MemZoneId121(),
                              dst_node->machine_id(), dst_node->MemZoneId121());
        auto *collective_node =
            ctx->task_graph()->NewNode<CollectiveBoxingGenericTaskNode>();
        NcclInitCollectiveNode(collective_node, dst_parallel_desc, i, op_name,
                               lbi, logical_blob_desc, OpType::kOpTypeAllGather,
                               -1);
        Connect<TaskNode>(src_node, ctx->task_graph()->NewEdge(),
                          collective_node);
        Connect<TaskNode>(collective_node, ctx->task_graph()->NewEdge(),
...
class NcclCollectiveBoxingReduceSubTskGphBuilder final
    : public SubTskGphBuilder {
public:
  ...
  Build(SubTskGphBuilderCtx *ctx,
    ...
    if (src_parallel_desc.parallel_num() > 1 &&
      ...
      const int64_t root_parallel_id =
          FindRootParallelId(src_parallel_desc, dst_parallel_desc);
      if (root_parallel_id == -1) {
        return Error::BoxingNotSupportedError();
      }

      const std::string op_name =
          "System-Boxing-NcclCollectiveBoxingReduce-" + NewUniqueId();
      FOR_RANGE(int64_t, i, 0, src_parallel_desc.parallel_num()) {
        CompTaskNode *src_node = sorted_src_comp_tasks.at(i);
        auto *collective_node =
            ctx->task_graph()->NewNode<CollectiveBoxingGenericTaskNode>();
        NcclInitCollectiveNode(collective_node, src_parallel_desc, i, op_name,
                               lbi, logical_blob_desc, OpType::kOpTypeReduce,
                               root_parallel_id);
        Connect<TaskNode>(src_node, ctx->task_graph()->NewEdge(),
                          collective_node);
...
class CollectiveBoxingScatterThenNcclAllGatherSubTskGphBuilder final
    : public SubTskGphBuilder {
public:
  ...
  ~CollectiveBoxingScatterThenNcclAllGatherSubTskGphBuilder() override =
      default;
  ...
  Build(SubTskGphBuilderCtx *ctx,
    ...
    if (src_parallel_desc.parallel_num() == 1 &&
        ...
        // a potential optimization: flat the blob and then relax this
        // requirement
        &&
        logical_blob_desc.shape().At(0) % dst_parallel_desc.parallel_num() ==
            0) {
      ...
      std::vector<TensorSliceView> out_slices =
          SubTskGphBuilderUtil::GetTensorSliceView(
              dst_parallel_desc.parallel_num(), split_sbp_parallel,
              logical_blob_desc);
      const std::string op_name =
          "System-Boxing-NcclCollectiveBoxingAllGather-" + NewUniqueId();
      FOR_RANGE(int64_t, out_id, 0, dst_parallel_desc.parallel_num()) {
        ...
        CompTaskNode *src_node = SubTskGphBuilderUtil::FindNearestNode(
        ...
        auto *collective_node =
            ctx->task_graph()->NewNode<CollectiveBoxingGenericTaskNode>();
        NcclInitCollectiveNode(collective_node, dst_parallel_desc, out_id,
                               op_name, lbi, logical_blob_desc,
                               OpType::kOpTypeAllGather, -1);
        Connect<TaskNode>(slice_node_proxy, ctx->task_graph()->NewEdge(),
                          collective_node);
        Connect<TaskNode>(collective_node, ctx->task_graph()->NewEdge(),
                          dst_node);
      }
...
class NcclCollectiveBoxingBroadcastSubTskGphBuilder final
    : public SubTskGphBuilder {
public:
  ...
  Build(SubTskGphBuilderCtx *ctx,
    ...
    if (src_parallel_desc.parallel_num() == 1 &&
      ...
      if (src_parallel_desc.device_type() == DeviceType::kCPU) {
        ...
        root_parallel_id = SubTskGphBuilderUtil::FindNearestNodeIndex(
            sorted_dst_comp_tasks, cpu_src_node);
        auto *nearest_dst_node = sorted_dst_comp_tasks.at(root_parallel_id);
        gpu_src_node = ctx->GetProxyNode(
            cpu_src_node, cpu_src_node->MemZoneId121(),
            nearest_dst_node->machine_id(), nearest_dst_node->MemZoneId121());
      } else if (src_parallel_desc.device_type() == DeviceType::kGPU) {
        root_parallel_id =
            FindRootParallelId(dst_parallel_desc, src_parallel_desc);
      ...
      const std::string op_name =
          "System-Boxing-NcclCollectiveBoxingBroadcast-" + NewUniqueId();
      FOR_RANGE(int64_t, i, 0, dst_parallel_desc.parallel_num()) {
        CompTaskNode *dst_node = sorted_dst_comp_tasks.at(i);
        auto *collective_node =
            ctx->task_graph()->NewNode<CollectiveBoxingGenericTaskNode>();
        NcclInitCollectiveNode(collective_node, dst_parallel_desc, i, op_name,
                               lbi, logical_blob_desc, OpType::kOpTypeBroadcast,
                               root_parallel_id);
        if (i == root_parallel_id) {
          Connect<TaskNode>(gpu_src_node, ctx->task_graph()->NewEdge(),
                            collective_node);
        } else {
...
CollectiveBoxingSubTskGphBuilder::CollectiveBoxingSubTskGphBuilder() {
  ...
  builders.emplace_back(
      new NcclCollectiveBoxingReduceScatterSubTskGphBuilder());
  ...
  builders.emplace_back(
      new CollectiveBoxingScatterThenNcclAllGatherSubTskGphBuilder());
...
Maybe<SubTskGphBuilderStatus> CollectiveBoxingSubTskGphBuilder::Build(
  ...
  if (!IsSourceTimeShape(*sorted_src_comp_tasks.front()
                              ->logical_node()
                              ->out_blob_time_shape())) {
    return Error::BoxingNotSupportedError();
  }
  return chain_builder_->Build(ctx, sorted_src_comp_tasks,
                               sorted_dst_comp_tasks, src_parallel_desc,
                               dst_parallel_desc, lbi, logical_blob_desc,
