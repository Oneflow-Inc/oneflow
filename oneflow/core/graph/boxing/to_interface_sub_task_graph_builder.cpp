...
Maybe<SubTskGphBuilderStatus> ToInterfaceSubTskGphBuilder::Build(
  ...
  const LogicalNode *dst_logical_node =
      sorted_dst_comp_tasks.front()->logical_node();
  if (dst_logical_node->op_vec().size() != 1) {
  ...
  if ((src_parallel_desc.parallel_num() == 1 ||
    ...
    for (CompTaskNode *dst_node : sorted_dst_comp_tasks) {
      ...
      if (SubTskGphBuilderUtil::IsOnSameGPU(nearest_src_node, dst_node)) {
        Connect<TaskNode>(nearest_src_node, ctx->task_graph()->NewEdge(),
                          dst_node);
      } else {
        TaskNode *proxy = ctx->GetProxyNode(
            nearest_src_node, nearest_src_node->MemZoneId121(),
    ...
    return TRY(BuildSubTskGphBuilderStatus(
        ...
        dst_sbp_parallel, lbi, logical_blob_desc, "ToInterfaceSubTskGphBuilder",
        "BuildSubTaskGphB2B"));
  } else if ((src_parallel_desc.parallel_num() == 1 ||
              src_sbp_parallel.has_broadcast_parallel()) &&
             (dst_parallel_desc.parallel_num() > 1 ||
              dst_sbp_parallel.has_split_parallel())) {
    ...
    const std::vector<TensorSliceView> out_slices =
        SubTskGphBuilderUtil::GetTensorSliceView(
    ...
    FOR_RANGE(int64_t, out_id, 0, dst_parallel_desc.parallel_num()) {
      ...
      if (src_parallel_desc.device_type() == DeviceType::kCPU) {
        slice_node->Init(
            lbi, out_slice, kSliceBoxingTaskModeCopy, src_machine_id,
      ...
      slice_node->ConnectToSrcNodeWithSlice(
          src_node, ctx->task_graph()->NewEdge(), in_slice);
      TaskNode *proxy = ctx->GetProxyNode(
          slice_node, slice_node->MemZoneId121(), dst_node->machine_id(),
          Global<IDMgr>::Get()->CpuMemZoneId());
      Connect<TaskNode>(proxy, ctx->task_graph()->NewEdge(), dst_node);
    }
    return TRY(BuildSubTskGphBuilderStatus(
