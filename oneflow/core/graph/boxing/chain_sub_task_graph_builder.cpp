...
Maybe<SubTskGphBuilderStatus> ChainSubTskGphBuilder::Build(
  ...
  for (const auto& builder : builders_) {
    Maybe<SubTskGphBuilderStatus> boxing_log_line = TRY(builder->Build(
    ...
    if (!boxing_log_line.IsOk()
        && SubTskGphBuilderUtil::IsErrorBoxingNotSupported(*boxing_log_line.error())) {
      continue;
    } else {
