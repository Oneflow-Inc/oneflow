...
void ChainActGraph::ForEachRegstDescConsumerPathIIScale(
  ...
  ForEachNode([&](ChainActNode* node) {
    for (const RegstAct* regst_act : node->ProducedRegstActs()) {
...
void ChainActGraph::ForEachRegstActConsumerPathDuration(
  ...
  TopoForEachChainActNode([&](const ChainActNode* cur_node) {
    ...
    for (const RegstAct* regst_act : cur_node->ProducedRegstActs()) {
    ...
    for (const RegstAct* regst_act : cur_node->LastConsumedRegstActs()) {
...
void ChainActGraph::InitNodes(
  ...
  HashMap<std::string, std::list<ActEvent*>> chain_with_act_id_str2act_events;
  ...
  for (const auto& pair : chain_with_act_id_str2act_events) {
...
void ChainActGraph::InitEdges(
  ...
  ForEachNode([&](ChainActNode* node) {
    ...
    for (const ActEvent* consumer_act_event : node->act_events()) {
...
void ChainActGraph::InitRegstActs7NodeProducedAndLastConsumedRegstActs(
    const HashMap<std::pair<int64_t, int64_t>, const ActEvent*>& regst_uid2producer_act_event,
    const HashMap<std::pair<int64_t, int64_t>, std::list<const ActEvent*>>&
        regst_uid2consumer_act_events) {
  HashMap<const ChainActNode*, int64_t> node2topo_order_value;
  ...
  for (const auto& pair : regst_uid2producer_act_event) {
    ...
    producer->AddProducedRegstActs(regst_act);
    int64_t max_consumer_topo_order_value = -1;
    const ActEvent* last_consumer_act_event = nullptr;
    for (const ActEvent* consumer_act_event : regst_act->consumer_act_events) {
      if (node2topo_order_value.at(Node4ActEvent(consumer_act_event))
          > max_consumer_topo_order_value) {
        max_consumer_topo_order_value = node2topo_order_value.at(Node4ActEvent(consumer_act_event));
...
ChainActGraph::ChainActGraph(const Plan& plan, std::unique_ptr<std::list<ActEvent>>&& act_events)
  ...
  InitNodes(&regst_uid2producer_act_event);
  InitEdges(regst_uid2producer_act_event, &regst_uid2consumer_act_events);
  InitRegstActs7NodeProducedAndLastConsumedRegstActs(regst_uid2producer_act_event,
                                                     regst_uid2consumer_act_events);
}
