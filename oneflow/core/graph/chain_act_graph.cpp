...
inline double Duration4RegstActConsumerPath(
  const RegstActCtx& regst_act_ctx,
  const ActEvent* consumer_act_event, const ChainActNode* consumer_node) {
...
ChainActNode::ChainActNode(std::list<std::unique_ptr<ActEvent>>&& act_events) 
 : act_events_(std::move(act_events)) {
  act_events_.sort(
      [](const std::unique_ptr<ActEvent>& lhs, const std::unique_ptr<ActEvent>& rhs) {
...
void ChainActNode::ForEachLastConsumedRegstAct(
    const std::function<void(const RegstAct*)>& Handler) const {
  for (const auto& last_consumed_regst_act : last_consumed_regst_acts_) { Handler(last_consumed_regst_act); }
}
...
void ChainActGraph::ForEachRegstDescConsumerPathMeanDuration(
  ...
  ForEachRegstActConsumerPathDuration([&](int64_t regst_desc_id, int64_t consumer_actor_id, double duration){
  ...
  ForEachNode([&](const ChainActNode* node) {
    node->ForEachProducedRegstAct([&](const RegstAct* regst_act) {
      ...
      for (const ActEvent* act_event : regst_act->consumer_act_events) {
        std::pair<int64_t, int64_t> consumed_regst_desc_id(regst_act->regst_desc_id, act_event->actor_id());
  ...
  TopoForEachChainActNode([&](const ChainActNode* cur_node){
    ...
    cur_node->ForEachLastConsumedRegstAct([&](const RegstAct* regst_act) {
      for (const ActEvent* consumer_act_event : regst_act->consumer_act_events) {
        double duration = Duration4RegstActConsumerPath(*(regst_act2regst_act_ctx.at(regst_act)), consumer_act_event, Node4ActEvent(consumer_act_event));
...
void ChainActGraph::CalcRegstActNodePathDuration(
  RegstActCtx* regst_act_ctx, const ChainActNode* node) const {
  double duration = 0;
    node->ForEachInEdge([&](const ChainActEdge* in_edge) {
      const ChainActNode* in_node = in_edge->src_node();
      if (regst_act_ctx->node2duration_to_producer.find(in_node) != regst_act_ctx->node2duration_to_producer.end()) {
        duration = std::max(duration, in_edge->duration() + regst_act_ctx->node2duration_to_producer.at(in_node));
      }
    });
...
void ChainActGraph::InitNodes(
  ...
  HashMap<std::pair<int64_t, int64_t>, std::list<std::unique_ptr<ActEvent>>> chain_id_with_act_id2act_events;
...
void ChainActGraph::InitNodeProducedRegstActs(
    const HashMap<std::pair<int64_t, int64_t>, const ActEvent*>& regst_uid2producer_act_event,
    const HashMap<std::pair<int64_t, int64_t>, std::list<const ActEvent*>>& regst_uid2consumer_act_events) {
  for (const auto& pair : regst_uid2producer_act_event) {
    ...
    auto regst_act = std::make_unique<RegstAct>(pair.first.first, pair.second, consumers_act_event_it->second);
...
void ChainActGraph::InitNodeLastConsumedRegstActs() {
  ...
  ForEachNode([&](const ChainActNode* node) {
    node->ForEachProducedRegstAct([&](const RegstAct* regst_act) {
      ...
      for (const ActEvent* consumer_act_event : regst_act->consumer_act_events) {
        int64_t cur_consumer_topo_order_value = TopoOrderValue4Node(Node4ActEvent(consumer_act_event));
...
std::function<int64_t(const ChainActNode*)> ChainActGraph::MakeGetterTopoOrderValue4Node()
    const {
