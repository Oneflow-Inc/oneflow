...
inline double Duration4RegstActConsumerPath(const ActEvent *producer_act_event,
  ...
  return path_duration + consumer_act_event->stop_time() -
...
std::string ChainActNode::VisualStr() const {
  ...
  ss << "act_id:" << act_id() << "\\n";
  ForEachActEvent(
      [&](const ActEvent *act_event) { ss << act_event->actor_id() << "\\n"; });
  return ss.str();
}

ChainActNode::ChainActNode(std::pair<int64_t, int64_t> chain_act_id_pair,
                           std::list<std::unique_ptr<ActEvent>> &&act_events)
    : chain_act_id_pair_(chain_act_id_pair),
      act_events_(std::move(act_events)) {
  act_events_.sort([](const std::unique_ptr<ActEvent> &lhs,
                      const std::unique_ptr<ActEvent> &rhs) {
...
void ChainActNode::ForEachInEdge(
    const std::function<void(const ChainActEdge *)> &Handler) const {
  for (const ChainActEdge *in_edge : in_edges()) {
    Handler(in_edge);
  }
}

void ChainActNode::ForEachOutEdge(
    const std::function<void(const ChainActEdge *)> &Handler) const {
  for (const ChainActEdge *out_edge : out_edges()) {
    Handler(out_edge);
  }
}

void ChainActNode::ForEachActEvent(
    const std::function<void(const ActEvent *)> &Handler) const {
  for (const auto &act_event : act_events_) {
    Handler(act_event.get());
  }
}

void ChainActNode::ForEachProducedRegstActGroup(
    const std::function<void(const std::list<const RegstAct *> &)> &Handler)
    const {
  for (const auto &pair : fake_outs2produced_regst_act_group_) {
    Handler(pair.second);
  }
}
...
void ChainActNode::ForEachLastConsumedRegstAct(
  ...
  for (const auto &last_consumed_regst_act_group :
       last_consumed_regst_act_groups_) {
    for (const RegstAct *regst_act : last_consumed_regst_act_group) {
      Handler(regst_act);
    }
  }
}
...
void ChainActNode::AddProducedRegstAct(std::unique_ptr<RegstAct> &&regst_act) {
  fake_outs2produced_regst_act_group_[regst_act->fake_producer_outs].push_back(
      regst_act.get());
...
void ChainActSubGraph::ForEachRegstActConsumerPathDuration(
  ...
  TopoForEachChainActNode([&](const ChainActNode *cur_node) {
    for (auto &ctx : ctx_window) {
      const auto &fake_producer_outs =
          ctx->regst_act_group.front()->fake_producer_outs;
      if (fake_producer_outs.find(cur_node) != fake_producer_outs.end()) {
        continue;
      }
      CalcRegstActNodePathDuration(ctx.get(), cur_node);
    }
    cur_node->ForEachProducedRegstActGroup(
        [&](const std::list<const RegstAct *> &regst_act_group) {
          std::shared_ptr<RegstActGroupCtx> ctx(
              new RegstActGroupCtx(regst_act_group, cur_node));
    ...
    cur_node->ForEachLastConsumedRegstAct([&](const RegstAct *regst_act) {
      ...
      for (const ActEvent *consumer : regst_act->consumer_act_events) {
        double path_duration =
            regst_act2ctx.at(regst_act)->node2duration_to_producer.at(
                Node4ActEvent(consumer));
        Handler(
            regst_act->regst_desc_id, consumer->actor_id(),
...
void ChainActSubGraph::CalcRegstActNodePathDuration(
    RegstActGroupCtx *regst_act_group_ctx, const ChainActNode *node) const {
  double duration = 0;
  ...
  if (duration > 0) {
    regst_act_group_ctx->node2duration_to_producer[node] = duration;
  }
}

void ChainActSubGraph::InitNodes(
    std::list<std::unique_ptr<ActEvent>> &&act_events,
    HashMap<std::pair<int64_t, int64_t>, const ActEvent *>
        *regst_uid2producer_act_event) {
  ...
  for (auto &act_event : act_events) {
    ...
    if (task_proto.task_set_info().area_id() == kMdUpdtArea) {
      continue;
    }
    chain_id_with_act_id2act_events[chain_act_id_pair].push_back(
        std::move(act_event));
  }
  for (auto &pair : chain_id_with_act_id2act_events) {
    ChainActNode *chain_act_node =
        new ChainActNode(pair.first, std::move(pair.second));
    ...
    chain_act_node->ForEachActEvent([&](const ActEvent *act_event) {
      ...
      for (const auto &produced_regst_desc : task_proto.produced_regst_desc()) {
        ...
        CHECK(
            regst_uid2producer_act_event->emplace(regst_uid, act_event).second);
      }
    });
  }
}
...
void ChainActSubGraph::InitEdges(
    const HashMap<std::pair<int64_t, int64_t>, const ActEvent *>
        &regst_uid2producer_act_event,
  ...
  ForEachNode([&](const ChainActNode *node) {
    ...
    node->ForEachActEvent([&](const ActEvent *consumer_act_event) {
      ...
      for (const auto &readable : consumer_act_event->readable_regst_infos()) {
        std::pair<int64_t, int64_t> regst_uid(readable.regst_desc_id(),
                                              readable.act_id());
        const auto &producer_act_event_it =
            regst_uid2producer_act_event.find(regst_uid);
        if (producer_act_event_it == regst_uid2producer_act_event.end()) {
          continue;
        }
        (*regst_uid2consumer_act_events)[regst_uid].push_back(
            consumer_act_event);
        act_event_with_consumer_.emplace(producer_act_event_it->second);
        ChainActNode *producer =
            act_event2chain_node_.at(producer_act_event_it->second);
        if (producer == node) {
          continue;
        }
        double &max_stop_time = producer2max_stop_time[producer];
        max_stop_time =
            std::max(max_stop_time, producer_act_event_it->second->stop_time());
      }
    });
...
void ChainActSubGraph::InitNodeProducedRegstAct(
  ...
  for (const auto &pair : regst_uid2producer_act_event) {
    const auto &consumers_act_event_it =
        regst_uid2consumer_act_events.find(pair.first);
    if (consumers_act_event_it == regst_uid2consumer_act_events.end()) {
      continue;
    }
    ChainActNode *producer = act_event2chain_node_.at(pair.second);
    auto regst_act = std::make_unique<RegstAct>(pair.first.first, pair.second,
                                                consumers_act_event_it->second);
    producer->ForEachOutEdge([&](const ChainActEdge *out_edge) {
      if (out_edge->duration() < regst_act->producer_act_event->stop_time()) {
        CHECK(
            regst_act->fake_producer_outs.emplace(out_edge->dst_node()).second);
      }
    });
...
void ChainActSubGraph::InitNodeLastConsumedRegstActGroup() const {
  ...
  auto ForEachConsumer =
      [](const std::list<const RegstAct *> &regst_act_group,
        ...
        for (const RegstAct *regst_act : regst_act_group) {
          for (const ActEvent *cur_consumer : regst_act->consumer_act_events) {
            Handler(cur_consumer);
          }
        }
      };
  ForEachNode([&](const ChainActNode *node) {
    node->ForEachProducedRegstActGroup(
        [&](const std::list<const RegstAct *> &regst_act_group) {
          ...
          ForEachConsumer(regst_act_group, [&](const ActEvent *cur_consumer) {
            int64_t cur_topo_order_value =
                TopoOrderValue4Node(Node4ActEvent(cur_consumer));
            if (cur_topo_order_value > max_topo_order_value) {
              max_topo_order_value = cur_topo_order_value;
              last_consumer = cur_consumer;
            }
          });
          act_event2chain_node_.at(last_consumer)
              ->AddLastConsumedRegstActGroup(regst_act_group);
        });
  });
}

std::function<int64_t(const ChainActNode *)>
ChainActSubGraph::MakeGetterTopoOrderValue4Node() const {
  auto node2topo_order_value =
      std::make_shared<HashMap<const ChainActNode *, int64_t>>();
  ...
  return [node2topo_order_value](const ChainActNode *node) {
    return node2topo_order_value->at(node);
  };
}

const ChainActNode *
ChainActSubGraph::Node4ActEvent(const ActEvent *act_event) const {
...
void ChainActSubGraph::ForEachActEvent(
    const std::function<void(const ActEvent *)> &Handler) const {
  ForEachNode(
      [&](const ChainActNode *node) { node->ForEachActEvent(Handler); });
}

bool ChainActSubGraph::IsActEventWithConsumer(const ActEvent *act_event) const {
  return act_event_with_consumer_.find(act_event) !=
         act_event_with_consumer_.end();
}

ChainActSubGraph::ChainActSubGraph(
    const HashMap<int64_t, const TaskProto &> &task_id2task_proto,
    std::list<std::unique_ptr<ActEvent>> &&act_events)
  ...
  InitNodeProducedRegstAct(regst_uid2producer_act_event,
                           regst_uid2consumer_act_events);
  ...
  ForEachChainActSubGraph([&](const ChainActSubGraph *sub_graph) {
    sub_graph->ForEachNode([&](const ChainActNode *node) {
      node->ForEachLastConsumedRegstAct([&](const RegstAct *regst_act) {
        int64_t produced_cnt =
            ++regst_desc_id2produced_cnt[regst_act->regst_desc_id];
        if (max_cnt < produced_cnt) {
          max_cnt = produced_cnt;
        }
        for (const ActEvent *act_event : regst_act->consumer_act_events) {
          std::pair<int64_t, int64_t> regst_desc_id_consumed(
              regst_act->regst_desc_id, act_event->actor_id());
          int64_t used_cnt =
              ++regst_desc_id_consumed2used_cnt[regst_desc_id_consumed];
          if (max_cnt < used_cnt) {
            max_cnt = used_cnt;
          }
        }
...
double ChainActGraph::CalcBaseII() const {
  ...
  ForEachChainActSubGraph([&](const ChainActSubGraph *sub_graph) {
    sub_graph->ForEachActEvent([&](const ActEvent *act_event) {
      ...
      if (sub_graph->IsActEventWithConsumer(act_event)) {
        ...
        max_act_cnt =
            std::max(max_act_cnt, actor_id2outputed_act_cnt[actor_id]);
      }
    });
  });
  ...
  ForEachChainActSubGraph([&](const ChainActSubGraph *sub_graph) {
    sub_graph->ForEachActEvent([&](const ActEvent *act_event) {
      ...
      if (frequence_it == actor_id2outputed_act_cnt.end()) {
        return;
      }
...
void ChainActGraph::GroupActEventByActId(
    std::list<std::unique_ptr<ActEvent>> &&act_events,
    HashMap<int64_t, std::list<std::unique_ptr<ActEvent>>>
        *act_id2act_event_group) const {
  // not considering RNN yet
  for (auto &act_event : act_events) {
    (*act_id2act_event_group)[act_event->act_id()].push_back(
        std::move(act_event));
  }
}
...
void ChainActGraph::MultiThreadBuildChainActSubGraph(
  ...
  for (auto &pair : *act_id2act_event_group) {
    thread_pool.AddWork([&]() {
      auto sub_graph = std::make_unique<ChainActSubGraph>(
          task_id2task_proto_, std::move(pair.second));
      {
...
ChainActGraph::ChainActGraph(const Plan &plan,
                             std::list<std::unique_ptr<ActEvent>> &&act_events)
    : plan_(&plan) {
