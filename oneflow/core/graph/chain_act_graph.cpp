...
void ChainActGraph::ForEachRegstActDuration(
  ...
  auto CalDuration = [&](const ChainActNode* node, RegstAct* regst_act) {
    ...
    ForEachInEdge(node, [&](const ChainActEdge* in_edge) {
      ...
      if (regst_act->node2duration.find(in_node) != regst_act->node2duration.end()) {
        duration = std::max(duration, in_edge->duration() + regst_act->node2duration.at(in_node));
      }
    });
    if (duration > 0) { regst_act->node2duration[node] = duration; }
  };
  for (int64_t i = 0; i < topo_nodes_.size(); ++i) {
    const ChainActNode* cur_node = topo_nodes_.at(i);
    for (RegstAct* regst_act : regst_act_window) {
      if (regst_act->fake_outs.find(cur_node) != regst_act->fake_outs.end()) { continue; }
