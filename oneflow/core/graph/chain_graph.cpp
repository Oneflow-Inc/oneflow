...
private:
  ...
  void CarefullySetBitset(std::vector<std::bitset<BITSET_SIZE>> *bitset_vec,
                          int64_t pos);
...
void ChainMerger::InitChains() {
  chain_list_.clear();
  int64_t bitset_num =
      std::ceil(static_cast<double>(task_node2uid_.size()) / BITSET_SIZE);
  for (const auto &task_node : task_nodes_) {
    chain_list_.emplace_back();
    Chain &cur_chain = chain_list_.back();
    cur_chain.nodes = {task_node};
    cur_chain.stream_area_id = std::make_pair(task_node->AreaId4ChainMerge(),
...
bool ChainMerger::DoMerge(std::list<ChainIt> &chains, ChainIt rhs) {
  ...
  if (rhs->nodes.front()->AreaId4ChainMerge() == kMdUpdtArea) {
    return false;
  }
  for (auto chains_it = chains.rbegin(); chains_it != chains.rend();
       ++chains_it) {
...
void ChainMerger::MergeChains() {
  ...
  for (auto cur_chain_it = chain_list_.begin();
       cur_chain_it != chain_list_.end();) {
...
void ChainMerger::CarefullySetBitset(
    std::vector<std::bitset<BITSET_SIZE>> *bitset_vec, int64_t pos) {
...
bool ChainMerger::IsSubset(const ChainIt &lhs, const ChainIt &rhs) const {
  ...
  for (int64_t i = 0; i < bitset_num; ++i) {
    if (lhs->ancestors_and_this.at(i) !=
        (lhs->ancestors_and_this.at(i) | rhs->ancestors.at(i))) {
      return false;
    }
  }
  return true;
...
bool NoOutRegstConsumedByBwNode(TaskNode *node) {
  ...
  for (TaskEdge *edge : fw_node->out_edges()) {
    auto *fw_consumer =
        dynamic_cast<NormalForwardCompTaskNode *>(edge->dst_node());
    if (fw_consumer == nullptr) {
      return false;
    }
    if (fw_consumer->HasBackwardCompTaskNode()) {
      return false;
    }
  }
  return true;
...
bool IsNonSoleKeepHeaderOnlyEdge(TaskNode *src_task, TaskNode *dst_task) {
  if (dst_task->in_edges().size() <= 1) {
    return false;
  }
  const auto *src_comp_task = dynamic_cast<CompTaskNode *>(src_task);
  if (src_comp_task == nullptr) {
    return false;
  }
  if (src_comp_task->logical_node()->op_vec().size() != 1) {
    return false;
  }
...
void CollectIgnoreTaskEdgesInFirstMergedChains(
    const std::vector<std::vector<TaskNode *>> &chains,
    HashSet<TaskEdge *> *ignore_edges) {
  auto HasGpuVariableOpInChain =
      [&](const std::vector<TaskNode *> &chain) -> bool {
    for (TaskNode *node : chain) {
      auto *fw_node = dynamic_cast<NormalForwardCompTaskNode *>(node);
      if (fw_node == nullptr) {
        continue;
      }
      if (fw_node->logical_node()->op_vec().size() != 1) {
        continue;
      }
      const auto &src_op = *fw_node->logical_node()->SoleOp();
      DeviceType device_type =
          CHECK_JUST(DeviceType4DeviceTag(src_op.op_conf().device_tag()));
    ...
    if (HasGpuVariableOpInChain(chain)) {
      ...
      for (TaskNode *node : chain) {
        for (TaskEdge *out_edge : node->out_edges()) {
          if (nodes_in_chain.find(out_edge->dst_node()) ==
              nodes_in_chain.end()) {
...
ChainGraph::ChainGraph(const TaskGraph &task_gph) : task_gph_(task_gph) {
  ...
  for (auto &task_nodes : chains) {
    PrioritizeUntrainableTaskNode(&task_nodes);
  }
  ...
  if (Global<ResourceDesc, ForSession>::Get()->enable_debug_mode()) {
    ToDotWithFilePath(
        JoinPath("dot", TypeName(), GlobalJobDesc().job_name() + ".dot"));
  }
}
...
void ChainGraph::CheckNoCycle() const {
  ...
  if (scc) {
    ...
    auto *ptr = scc.get();
    auto OnCycle = [ptr](ChainNode *chain_node) {
      return ptr->find(chain_node) != ptr->end();
    };
    const auto &chain_graph_filename =
        "job" + job_id + "_cycle_chain_graph.dot";
    ToDotWithFilePath(OnCycle, [](ChainEdge *) { return true; },
    ...
    const std::function<std::string(TaskNode *)> ColorNode = [&](TaskNode *t) {
      ...
      if (color_it != task_id2color.end()) {
        return ", style=filled, colorscheme=set312, fillcolor=" +
               std::to_string(color_it->second);
      } else {
        return std::string("");
      }
    };
    const std::function<std::string(TaskEdge *)> ColorEdge = [&](TaskEdge *te) {
      ...
      if (src_color_it != task_id2color.end() &&
          dst_color_it != task_id2color.end()) {
        return ", style=filled, colorscheme=set312, color=" +
               std::to_string(task_id2color.size());
      } else {
        return std::string("");
      }
    };
    const std::string colored_task_graph_filename =
        "optimized_dlnet_" + job_id +
        "_highlighted_cycle_task_nodes_in_chain_graph.dot";
    task_gph_.ToDotWithFilePath(ColorNode, ColorEdge,
...
void ChainGraph::CollectTaskNodeAncestors(
    const TaskGraph &task_gph,
    HashMap<TaskNode *, HashSet<TaskNode *>> *node2ancestors,
  ...
  task_gph.AcyclicTopoForEachNode([&](TaskNode *node) {
    ...
    if (node->GetTaskType() == TaskType::kTick) {
      return;
    }
    for (TaskEdge *in_edge : node->in_edges()) {
      if (ignore_edges && ignore_edges->find(in_edge) != ignore_edges->end()) {
        continue;
      }
      TaskNode *in_node = in_edge->src_node();
      if (in_node->GetTaskType() == TaskType::kTick) {
        continue;
      }
      if (IsNonSoleKeepHeaderOnlyEdge(in_node, node)) {
        continue;
      }
...
void ChainGraph::MergeTaskNodes(
    const HashMap<int64_t, std::vector<TaskNode *>> &machine2tasks,
    const HashMap<TaskNode *, HashSet<TaskNode *>> &node2ancestors,
  ...
  for (auto &pair : machine2tasks) {
    thread_pool.AddWork([&]() {
      ...
      auto &cur_chain_list = merger.GetChains();
      {
        ...
        for (const auto &chain : cur_chain_list) {
          chains->emplace_back(chain.nodes);
        }
      }
      counter.Decrease();
    });
...
void ChainGraph::PrioritizeUntrainableTaskNode(
    std::vector<TaskNode *> *task_nodes) const {
  ...
  auto ForEachInNode = [&](TaskNode *node,
                           const std::function<void(TaskNode *)> &Handler) {
    node->ForEachNodeOnInEdge([&](TaskNode *node_on_in_edge) {
      if (IsInSubset(node_on_in_edge)) {
        Handler(node_on_in_edge);
      }
    });
  };
  auto ForEachOutNode = [&](TaskNode *node,
                            const std::function<void(TaskNode *)> &Handler) {
    node->ForEachNodeOnOutEdge([&](TaskNode *node_on_out_edge) {
      if (IsInSubset(node_on_out_edge)) {
        Handler(node_on_out_edge);
      }
    });
  };
  ...
  PartialPriorTopoForEachNode(
      starts, ForEachInNode, ForEachOutNode, IsPrior,
      [&](TaskNode *node) { task_nodes->push_back(node); });
  HashSet<TaskNode *> task_nodes_set_check(task_nodes->begin(),
                                           task_nodes->end());
...
void ChainGraph::PartialPriorTopoForEachNode(
  ...
  task_gph_.TopoForEachNode(starts, ForEachInNode, ForEachOutNode,
                            [&](TaskNode *node) {
                              if (IsTaskNodePrior(node)) {
                                CHECK(prior_nodes.emplace(node).second);
                              }
                              nodes.push_back(node);
                            });
  // travel prior nodes;
  auto ForEachPriorInNode =
      [&](TaskNode *node, const std::function<void(TaskNode *)> &Handler) {
        ForEachInNode(node, [&](TaskNode *in_node) {
          if (prior_nodes.find(in_node) != prior_nodes.end()) {
            Handler(in_node);
          }
        });
      };
  auto ForEachPriorOutNode =
      [&](TaskNode *node, const std::function<void(TaskNode *)> &Handler) {
        ForEachOutNode(node, [&](TaskNode *out_node) {
          if (prior_nodes.find(out_node) != prior_nodes.end()) {
            Handler(out_node);
          }
        });
      };
  ...
  for (TaskNode *start : starts) {
    if (IsTaskNodePrior(start)) {
      prior_starts.push_back(start);
    }
  }
  task_gph_.DfsTopoForEachNodeSortByDistanceToSink(
      prior_starts, ForEachPriorInNode, ForEachPriorOutNode, Handler);
  // travel other nodes ;
  task_gph_.DfsTopoForEachNodeSortByDistanceToSink(
      starts, ForEachInNode, ForEachOutNode, [&](TaskNode *node) {
        if (prior_nodes.find(node) == prior_nodes.end()) {
          Handler(node);
        }
      });
}

void ChainGraph::InitChainNode(
    const std::vector<std::vector<TaskNode *>> &chains) {
  for (auto &chain : chains) {
  ...
  for (auto &chain : chains) {
    for (auto &cur_task_node : chain) {
      ...
      for (auto &task_in_edge : cur_task_node->in_edges()) {
        ...
        if (cur_chain_node == src_chain_node) {
          continue;
        }
        if (HasChainEdge(src_chain_node, cur_chain_node)) {
          continue;
        }
