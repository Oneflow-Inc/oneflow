...
ChainGraph::ChainGraph(const TaskGraph& task_gph) : task_gph_(task_gph) {
  ...
  for (auto& cur_task_node : ordered_task_nodes) {
    ...
    for (auto& in_task_edge : cur_task_node->in_edges()) {
      auto src_task_node = in_task_edge->src_node();
      if (cur_task_node->ancestors().find(src_task_node) == cur_task_node->ancestors().end()) {
        continue;  // removing kMdUpdt
      }
