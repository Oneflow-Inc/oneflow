...
class OpNode final : public Node<OpNode, OpEdge> {
 public:
  ...
  const cfg::NdSbpSignature& parallel_distribution_signature() const {
 ...
 private:
  ...
  HashMap<LogicalBlobId, cfg::NdSbp> lbi2parallel_distribution_;
...
class OpGraph final : public Graph<OpNode, OpEdge> {
 public:
  ...
  const cfg::NdSbp& GetNdSbp(const std::string& op_name,
                                                           const LogicalBlobId& lbi) const;
 ...
 private:
  ...
  void InferOpNodeNdSbpSignature(
      OpNode* op_node,
