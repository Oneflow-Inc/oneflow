...
public:
  ...
  explicit OpNode(const ParallelDesc &parallel_desc,
  ...
  const SbpSignature &sbp_signature() const {
    return *CHECK_JUST(op().sbp_signature());
  }
...
private:
  ...
  HashMap<std::string, std::vector<std::shared_ptr<BlobDesc>>> *
  mut_bn2parallel_id2blob_desc() {
...
public:
  ...
  explicit OpEdge(
      ...
      std::shared_ptr<HashMap<LogicalBlobId, std::vector<std::string>>>
          lbi2ibns)
      : lbis_(std::move(lbis)), lbi2obn_(std::move(lbi2obn)),
        lbi2ibns_(std::move(lbi2ibns)) {}
  ...
  const HashMap<LogicalBlobId, std::string> &lbi2obn() const {
    return *lbi2obn_;
  }
  const HashMap<LogicalBlobId, std::vector<std::string>> &lbi2ibns() const {
    return *lbi2ibns_;
  }
  std::string VisualStr() const override;
...
public:
  ...
  Maybe<void>
  ForEachOpNode(const std::function<Maybe<void>(const OpNode &)> &DoEach) const;

  const OpNode *OpNode4OpName(const std::string &name) const;

  std::function<const BlobDesc &(const LogicalBlobId &)>
  MakeGetterBlobDesc4ModelLbi() const;

  int32_t GetModelSplitAxis(const std::string &op_name,
                            const LogicalBlobId &lbi) const;
  BalancedSplitter GetBalancedSplitter(const std::string &op_name,
                                       const LogicalBlobId &lbi) const;
  int64_t GetParallelNum(const std::string &op_name) const;
  int64_t GetSplitNum(const std::string &op_name,
                      const LogicalBlobId &lbi) const;
  const SbpParallel &GetSbpParallel(const std::string &op_name,
                                    const LogicalBlobId &lbi) const;
  ...
  // a set of nodes is called a chain family if they can divided into several
  // connected chains
  void ForEachChainFamily(
  ...
  void
  ForEachDataAndCtrlInNode(OpNode *node,
                           const std::function<void(OpNode *)> &Handler) const;
  void
  ForEachDataAndCtrlOutNode(OpNode *node,
                            const std::function<void(OpNode *)> &Handler) const;
...
private:
  ...
  void InferOpNodeSbpSignature(OpNode *op_node,
                               const SbpSignature &sbp_sig_conf) const;
  Maybe<void> InferOpNodeMirroredSignature(OpNode *op_node,
  ...
  bool IsBatchAxisBlob(const std::string &op_name,
                       const LogicalBlobId &lbi) const;
  std::string GetOpNameKey(const std::string &op_name,
                           const LogicalBlobId &lbi) const;
  LogicalBlobId GetLogicalBlobIdKey(const std::string &op_name,
                                    const LogicalBlobId &lbi) const;

  std::function<bool(const OpNode *, const OpNode *)>
  MakePredicatorIsDataOrCtrlReachable() const;
  ...
  HashMap<std::string, HashSet<std::string>>
      producer_op_name2ctrl_consumer_op_names_;
};
