...
public:
  ...
  explicit PlanTaskNode(const TaskProto &task_proto)
  ...
  int64_t order_in_graph() const {
    return task_proto_->task_set_info().order_in_graph();
  }
...
public:
  ...
  void ComputeLifetimeSameChainActorIds(
...
private:
  ...
  bool IsReachableToAncestor(const PlanTaskNode *node,
  ...
  HashMap<int64_t, std::vector<const PlanTaskNode *>>
      chain_id2sorted_plan_task_nodes_;
};
