...
std::function<const RegstDescProto *(int64_t)> MakeGetterRegstDesc4RegstDescId(
  ...
  auto regst_desc_id2regst_desc =
      std::make_shared<HashMap<int64_t, const RegstDescProto *>>();
  for (const auto *regst_desc : regst_descs) {
    CHECK(regst_desc_id2regst_desc
              ->emplace(regst_desc->regst_desc_id(), regst_desc)
              .second);
  }
  return [regst_desc_id2regst_desc](
             int64_t regst_desc_id) -> const RegstDescProto * {
...
InplaceRegstGraph::InplaceRegstGraph(
    const HashSet<const RegstDescProto *> &regst_descs) {
  ...
  for (const RegstDescProto *regst_desc : regst_descs) {
    if (regst_desc->has_hint_inplace_consumed_regst_desc_id()) {
      const RegstDescProto *in_regst_desc = RegstDesc4RegstDescId(
          regst_desc->hint_inplace_consumed_regst_desc_id());
      if (in_regst_desc != nullptr) {
        auto *edge = new InplaceRegstEdge();
        AddAllocatedEdge(edge);
        Connect<InplaceRegstNode, InplaceRegstEdge>(
            FindOrCreate(in_regst_desc), edge, FindOrCreate(regst_desc));
      }
    }
  }
}
...
InplaceRegstGraph::MakeMutFindOrCreateNode() {
  auto regst_desc2node =
      std::make_shared<HashMap<const RegstDescProto *, InplaceRegstNode *>>();
  return [regst_desc2node,
          this](const RegstDescProto *regst_desc) -> InplaceRegstNode * {
