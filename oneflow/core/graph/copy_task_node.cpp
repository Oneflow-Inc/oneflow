...
void CopyHdTaskNode::Init(CopyHdOpConf::Type copy_type, int64_t machine_id, DeviceType dev_type,
  ...
  DeviceId device_id{process_id, dev_type, static_cast<uint32_t>(dev_phy_id)};
  // auto* stream_index_generator =
  //     Global<IDMgr>::Get()->GetStreamIndexGeneratorManager()->GetGenerator(device_id);
  uint32_t stream_index = 0;
  if (copy_type == CopyHdOpConf::H2D) {
    stream_index = 1;  // stream_index_generator->GenerateH2DStreamIndex();
  } else if (copy_type == CopyHdOpConf::D2H) {
    stream_index = 2;  // stream_index_generator->GenerateD2HStreamIndex();
  } else {
    UNIMPLEMENTED();
  }
