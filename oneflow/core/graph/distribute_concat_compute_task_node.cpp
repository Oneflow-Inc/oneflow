...
void DistributeConcatCompTaskNode::BuildExecGphStructAndBindInRegst() {
  ...
  mut_exec_gph().ForEachNode([&](ExecNode *cur_node) {
    const auto &ibn =
        cur_node->op()->input_bns().Get(parallel_ctx()->parallel_id());
...
void DistributeConcatCompTaskNode::BuildOutRegst() {
  ...
  mut_exec_gph().ForEachNode([&](ExecNode *cur_node) {
    ...
    for (ExecEdge *out_edge : cur_node->out_edges()) {
      found_lbis.insert(out_edge->lbi());
    }
  ...
  if (in_regst->NumOfLbi() == 1) {
    out_regst->set_hint_inplace_consumed_regst_desc_id(
        in_regst->regst_desc_id());
  }
}
