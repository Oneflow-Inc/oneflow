...
public:
  ...
  explicit NormalInplaceLbiNode(const LogicalBlobId &lbi)
      : InplaceLbiNode(lbi) {}
  ~NormalInplaceLbiNode() override = default;

  bool IsMutRef(const std::function<bool(const InplaceLbiEdge *)> &IsValidEdge)
      const override;
};
...
public:
  ...
  explicit SourceOpInplaceLbiNode(const LogicalBlobId &lbi)
      : InplaceLbiNode(lbi) {}
...
public:
  ...
  explicit UpdateInplaceLbiNode(const LogicalBlobId &lbi)
      : InplaceLbiNode(lbi) {}
...
public:
  ...
  InplaceLbiEdge(const Operator *op, const std::string &ibn,
                 const std::string &obn, bool is_mut_ref)
...
class InplaceLbiGraph final
    : public Graph<const InplaceLbiNode, const InplaceLbiEdge> {
public:
...
private:
  ...
  void
  ForEachTree(const HashSet<const InplaceLbiNode *> &nodes,
              const std::function<bool(const InplaceLbiEdge *)> &IsValidEdge,
              const std::function<void(const HashSet<const InplaceLbiNode *> &)>
                  &Handler) const;
  void
  FindAllEdges(const HashSet<const InplaceLbiNode *> &nodes,
               const std::function<bool(const InplaceLbiEdge *)> &IsValidEdge,
               HashSet<const InplaceLbiEdge *> *cur_disabled_edges) const;
};
