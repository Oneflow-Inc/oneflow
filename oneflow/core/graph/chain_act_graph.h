...
struct RegstAct {
  RegstAct(int64_t input_regst_desc_id,
           const ActEvent *input_producer_act_event,
...
public:
  ...
  // ForEach
  void
  ForEachInEdge(const std::function<void(const ChainActEdge *)> &Handler) const;
  void ForEachOutEdge(
      const std::function<void(const ChainActEdge *)> &Handler) const;
  void
  ForEachActEvent(const std::function<void(const ActEvent *)> &Handler) const;
  void ForEachLastConsumedRegstAct(
  ...
  void AddLastConsumedRegstActGroup(
      const std::list<const RegstAct *> &regst_act_group) {
...
class ChainActSubGraph final
    : public Graph<const ChainActNode, const ChainActEdge> {
public:
...
private:
  ...
  MakeGetterTopoOrderValue4Node() const;
  ...
  void
  InitEdges(const HashMap<std::pair<int64_t, int64_t>, const ActEvent *>
                &regst_uid2producer_act_event,
...
private:
  ...
  void ForEachChainActSubGraph(
      const std::function<void(const ChainActSubGraph *)> &Handler) const;
  void
  GroupActEventByActId(std::list<std::unique_ptr<ActEvent>> &&act_events,
