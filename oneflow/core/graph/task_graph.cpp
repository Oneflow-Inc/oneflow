...
bool IsInterfaceTask(const TaskNode *node) {
  ...
  if (comp_task_node->logical_node()->op_vec().size() != 1) {
    return false;
  }
  auto op_type_case =
      comp_task_node->logical_node()->SoleOp()->op_conf().op_type_case();
...
bool IsConnectToTickOp(const TaskNode *node) {
  ...
  if (dynamic_cast<const VariableOp *>(op) != nullptr) {
    return true;
  }
  return false;
}

std::function<TaskNode *(const std::string &)>
MakeGetterTaskNode4SoleOpName(const HashSet<TaskNode *> &task_nodes) {
  auto op_name2task_nodes =
      std::make_shared<HashMap<std::string, HashSet<TaskNode *>>>();
  for (TaskNode *task_node : task_nodes) {
    if (task_node->exec_gph().node_num() == 1) {
      ExecNode *exec_node = task_node->exec_gph().SoleNode();
      CHECK((*op_name2task_nodes)[exec_node->op()->op_name()]
                .emplace(task_node)
                .second);
    }
  }
  return [op_name2task_nodes](const std::string &op_name) -> TaskNode * {
    ...
    if (iter == op_name2task_nodes->end()) {
      return nullptr;
    }
...
MakePredicatorIsLbiAllConsumersReachable(
  ...
  auto IsDataOrCtrlReachable = [IsOpNameDataOrCtrlReachable](
      const TaskNode *src_node, const TaskNode *dst_node) -> bool {
    if (src_node->chain_id() == dst_node->chain_id() &&
        src_node->order_in_graph() <= dst_node->order_in_graph()) {
      return true;
    }
    const CompTaskNode *comp_src_node =
        dynamic_cast<const CompTaskNode *>(src_node);
    ...
    const CompTaskNode *comp_dst_node =
        dynamic_cast<const CompTaskNode *>(dst_node);
    ...
    return IsOpNameDataOrCtrlReachable(
        comp_src_node->logical_node()->SoleOp()->op_name(),
        comp_dst_node->logical_node()->SoleOp()->op_name());
  };
  return [TaskNode4SoleOpName, IsDataOrCtrlReachable](
             const LogicalBlobId &lbi, const std::string &op_name) -> bool {
    ...
    for (TaskEdge *out_edge : src_task_node->out_edges()) {
      if (IsLbiOnTaskEdge(out_edge, lbi)) {
        ...
        reachable_out_edges_size +=
            IsDataOrCtrlReachable(out_edge->dst_node(), dst_task_node);
      }
    }
...
bool IsInplaceAllowed(TaskNode *task_node, const std::vector<std::string> &bns,
                      const std::function<const TaskNode *(const std::string &)>
                          &TaskNode4SoleOpName) {
  if (task_node->exec_gph().node_num() != 1) {
    return false;
  ...
  for (const auto &bn : bns) {
    // TaskNode for bn is not nullptr if it's on the same device with
    // `task_node`
    if (TaskNode4SoleOpName(exec_node.op()->BnInOp2Lbi(bn).op_name()) ==
        nullptr) {
      return false;
    }
    const RegstDesc &regst_desc = *exec_node.RegstDesc4BnInOp(bn);
    if (regst_desc.NumOfLbi() != 1) {
      return false;
    }
  }
...
std::unique_ptr<BoxingLogger> CreateBoxingLogger() {
  if (Global<ResourceDesc, ForSession>::Get()->enable_debug_mode()) {
    return std::unique_ptr<BoxingLogger>(new CsvBoxingLogger(
        StrCat("boxing/log/", GlobalJobDesc().job_id()) + ".csv"));
  } else {
...
TaskGraph::TaskGraph(std::unique_ptr<const LogicalGraph> &&logical_gph) {
  ...
  HashMap<const LogicalNode *, std::vector<CompTaskNode *>>
  ...
  auto MutBufTask = [&](CompTaskNode *task_node, int64_t machine_id,
                        int32_t mem_zone_id) {
    auto &buf_vec = buf_task[task_node][machine_id];
    if (buf_vec.empty()) {
      buf_vec.assign(Global<ResourceDesc, ForSession>::Get()->MemZoneNum(),
                     nullptr);
    }
  ...
  auto AllocateCpuThrdIdEvenly = [&](const TaskNode *task_node) {
    ...
    offset =
        (offset + 1) % Global<ResourceDesc, ForSession>::Get()->CpuDeviceNum();
    return ret;
  };
  ...
  logical_gph_->ForEachNode([&](const LogicalNode *logical_node) {
    logical_node->GenSortedCompTaskNodes(
        AllocateCpuThrdIdEvenly, &machine_persistence_task_vec,
        [&](CompTaskNode *comp_task_node) {
  ...
  logical_gph_->ForEachEdge([&](const LogicalEdge *logical_edge) {
    BldSubTskGphMthd method = GetMthdForBldSubTskGph(logical_edge->src_node(),
                                                     logical_edge->dst_node());
...
void TaskGraph::ConnectCtrlEdges(
    const std::vector<CompTaskNode *> &src_task_nodes,
  ...
  FOR_RANGE(int32_t, i, 0, src_task_nodes.size()) {
    ...
    RegstDesc *ctrl_regst_desc = src_task_nodes.at(i)->BuildCtrlRegstDesc(
        dst_task_nodes.at(i), &regst_desc_name);
    ctrl_regst_desc->UpdtMinRegstNumIfNeed(ctrl_regst_num);
    ctrl_regst_desc->UpdtMaxRegstNumIfNeed(ctrl_regst_num);
    ctrl_regst_desc->mut_regst_desc_type()
        ->mutable_ctrl_regst_desc()
        ->set_returned_regst_num(ctrl_regst_num);
...
void TaskGraph::GenerateIndependentThrdId(
  ...
  for (auto pair : persistence_nodes) {
    machine_task_type_vec.emplace_back(
        std::make_pair(pair.first, pair.second->GetTaskType()));
  }

  ThrdIdGenerator generator(machine_task_type_vec,
                            Global<IDMgr>::Get()->BaseIndependentThrdId());
  for (const auto &pair : persistence_nodes) {
    int64_t thrd_id =
        generator.GenerateThrdId(pair.first, pair.second->GetTaskType());
    pair.second->set_thrd_id(thrd_id);
  }
}

void TaskGraph::AcyclicTopoForEachNode(
  ...
  auto ForEachInNode = [&](TaskNode *node,
    ...
    node->ForEachNodeOnInEdge([&](TaskNode *node_on_in_edge) {
      if (IsBackEdge(node_on_in_edge, node)) {
        return;
      }
  ...
  auto ForEachOutNode = [&](TaskNode *node,
    ...
    node->ForEachNodeOnOutEdge([&](TaskNode *node_on_out_edge) {
      if (IsBackEdge(node, node_on_out_edge)) {
        return;
      }
  ...
  ForEachNode([&](TaskNode *node) {
    if (IsSourceNode(node) && IsAllowedStartNode(node)) {
...
void TaskGraph::GetInplaceOpBlobArgList(
    ...
    const std::function<const TaskNode *(const std::string &)> &TaskNode4OpName)
    const {
  ...
  auto AddConstInplaceArgPair = [&](TaskNode *node, const std::string &ibn,
                                    const std::string &obn,
                                    const std::string &op_name) {
    if (IsInplaceAllowed(node, {ibn, obn}, TaskNode4OpName)) {
  ...
  for (TaskNode *task_node : dev_nodes) {
    ...
    for (const std::string &ibn : op.input_bns()) {
      if (op.InputBlobModifier4Ibn(ibn).is_mutable()) {
        ...
        *obas_info->mut_in_obas.mutable_oba()->Add() =
            GenOpBlobArg(op.op_name(), ibn);
      }
    }
    for (const std::string &obn : op.output_bns()) {
      const auto &obn_modifier = op.OutputBlobModifier4Obn(obn);
      if (obn_modifier.has_mutable_inplace_ibn()) {
        AddMutableInplaceArgPair(task_node, obn_modifier.mutable_inplace_ibn(),
                                 obn, op.op_name());
      } else if (obn_modifier.has_const_inplace_ibn()) {
        AddConstInplaceArgPair(task_node, obn_modifier.const_inplace_ibn(), obn,
                               op.op_name());
      }
    }
    ...
    if (op.op_conf().has_user_conf()) {
      ...
      for (const auto &pair : user_op_ctx->mut_inplace_obn2ibn) {
        AddMutableInplaceArgPair(task_node, pair.second, pair.first,
                                 op.op_name());
      }
      for (const auto &pair : user_op_ctx->con_inplace_obn2ibn) {
        AddConstInplaceArgPair(task_node, pair.second, pair.first,
                               op.op_name());
      }
    }
  }
...
void TaskGraph::GetSafeInplaceOpBlobArgList(
  ...
  auto IsLbiAllConsumersReachable = MakePredicatorIsLbiAllConsumersReachable(
      TaskNode4SoleOpName, IsOpNameDataOrCtrlReachable);
  InplaceLbiGraph origin_graph(obas_info, Op4OpName);
  InplaceLbiGraph safe_graph(*safe_obas_info, Op4OpName);
  origin_graph.ComputeSafeInplaceObns(safe_obas_info,
                                      IsLbiAllConsumersReachable);
  if (Global<ResourceDesc, ForSession>::Get()->enable_debug_mode()) {
    origin_graph.ToDotWithFilePath(JoinPath(
        "dot", "InplaceLbiGraph", GlobalJobDesc().job_name() + "_origin.dot"));
    safe_graph.ToDotWithFilePath(JoinPath(
        "dot", "InplaceLbiGraph", GlobalJobDesc().job_name() + "_safe.dot"));
  }
}

void TaskGraph::SetTaskRegstInplaceInfo(
  ...
  inplace_gph.ForEachConnectedComponent([&](
      const HashSet<const InplaceLbiNode *> &inplace_nodes) {
    for (const auto *inplace_node : inplace_nodes) {
      if (inplace_node->in_edges().empty()) {
        continue;
      }
      const auto *inplace_edge = inplace_node->SoleInEdge();
      auto *exec_node = TaskNode4SoleOpName(inplace_edge->op().op_name())
                            ->exec_gph()
                            .SoleNode();
...
void TaskGraph::ForEachGpuDeviceNodes(
    ...
    const {
  HashMap<std::pair<int64_t, int64_t>, HashSet<TaskNode *>>
      global_dev_phy_id2nodes;
  ForEachNode([&](TaskNode *task_node) {
    if (task_node->device_type() != DeviceType::kGPU) {
      return;
    }
    int64_t dev_phy_id =
        Global<IDMgr>::Get()->GetGpuPhyIdFromThrdId(task_node->thrd_id());
    global_dev_phy_id2nodes[{task_node->machine_id(), dev_phy_id}].emplace(
        task_node);
  });
  for (const auto &pair : global_dev_phy_id2nodes) {
    Handler(pair.second);
  }
}
...
void TaskGraph::EnableInplaceMemSharing(
  ...
  ForEachGpuDeviceNodes([&](const HashSet<TaskNode *> &dev_nodes) {
    ...
    GetSafeInplaceOpBlobArgList(&safe_inplace_obas_info, dev_nodes,
                                IsOpNameDataOrCtrlReachable);
...
DEFINE_BLD_SUB_TASK_GRAPH_METHOD(BldSubTskGphByBoxing) {
  ...
  for (const LogicalBlobId &lbi : lbis) {
    ...
    const SbpParallel &src_sbp_parallel =
        Global<OpGraph>::Get()->GetSbpParallel(src_logical->SoleOp()->op_name(),
                                               lbi);
    const SbpParallel &dst_sbp_parallel =
        Global<OpGraph>::Get()->GetSbpParallel(dst_logical->SoleOp()->op_name(),
                                               lbi);
    const std::shared_ptr<const ParallelDesc> &src_parallel_desc =
        src_logical->parallel_desc();
    const std::shared_ptr<const ParallelDesc> &dst_parallel_desc =
        dst_logical->parallel_desc();
    const BlobDesc &blob_desc = Global<OpGraph>::Get()->GetLogicalBlobDesc(lbi);
    auto status = CHECK_JUST(sub_tsk_gph_builder_->Build(
        sub_tsk_gph_builder_ctx_.get(), src_nodes, sorted_dst_comp_tasks,
...
DEFINE_BLD_SUB_TASK_GRAPH_METHOD(BldSubTskGphByPartialInLbiConnect) {
  ...
  CHECK_EQ(dst_logical->SoleOp()->input_bns().size(),
           sorted_dst_comp_tasks.size());
  FOR_RANGE(int, i, 0, sorted_dst_comp_tasks.size()) {
    const auto &lbi = dst_logical->SoleOp()->BnInOp2Lbi(
        dst_logical->SoleOp()->input_bns().Get(i));
    if (lbis.find(lbi) != lbis.end()) {
      BuildTaskPath(sorted_src_comp_tasks.at(0), sorted_dst_comp_tasks.at(i),
                    MutBufTask, true);
    }
  }
}
...
DEFINE_BLD_SUB_TASK_GRAPH_METHOD(BldSubTskGphByPartialOutLbiConnect) {
  ...
  CHECK_EQ(src_logical->SoleOp()->output_bns().size(),
           sorted_src_comp_tasks.size());
  FOR_RANGE(int, i, 0, sorted_src_comp_tasks.size()) {
    const auto &lbi = src_logical->SoleOp()->BnInOp2Lbi(
        src_logical->SoleOp()->output_bns().Get(i));
    if (lbis.find(lbi) != lbis.end()) {
      BuildTaskPath(sorted_src_comp_tasks.at(i), sorted_dst_comp_tasks.at(0),
                    MutBufTask, true);
    }
  }
}
...
void TaskGraph::BuildTaskPath(
  ...
  auto SetBufTask = [&](int64_t machine_id, int32_t mem_zone_id,
                        TaskNode *new_val) {
  ...
  while (cur_node->machine_id() != dst->machine_id() ||
    ...
    cur_node =
        BuildTaskStep(cur_node, dst, GetBufTask, SetBufTask, use_buf_task_node);
  }
  if (cur_node != dst) {
...
TaskNode *TaskGraph::BuildTaskStep(
  ...
  if (cur_node->MemZoneId121() != cpu_mem_zone_id) {
    ...
    next_mem_zone_id = cpu_mem_zone_id;
    if (!use_buf_task_node ||
        !(next_node = GetBufTask(dst->machine_id(), next_mem_zone_id))) {
...
TaskNode *TaskGraph::TryAddCopyH2DTaskTo(TaskNode *task) {
  if (IsInterfaceTask(task)) {
    return nullptr;
  }
  if (IsClassRegistered<TickTockTaskType>(task->GetTaskType())) {
    return nullptr;
  }
