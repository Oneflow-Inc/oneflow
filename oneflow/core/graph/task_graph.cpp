...
TaskGraph::TaskGraph(std::unique_ptr<const LogicalGraph>&& logical_gph) {
  ...
  auto Mut121BufTask = [&](CompTaskNode* task_node, int64_t machine_id,
                           int32_t mem_zone_id) {
  ...
  logical_gph_->ForEachEdge([&](const LogicalEdge* logical_edge) {
    ...
    (this->*method)(logical_edge->src_node(), logical_edge->dst_node(),
                    ...
                    Mut121BufTask, AllocateCpuThrdId);
  });
...
DEFINE_BLD_SUB_TASK_GRAPH_METHOD(BldSubTskGphByBoxing) {
  ...
  if (logical2sorted_out_box->find(src_logical)
      == logical2sorted_out_box->end()) {
    BuildOutBoxing(src_logical, sorted_src_comp_tasks,
                   &((*logical2sorted_out_box)[src_logical]),
                   AllocateCpuThrdId);
  }
  ...
  if (logical2sorted_in_box->find(dst_logical)
      == logical2sorted_in_box->end()) {
    BuildInBoxing(dst_logical, sorted_dst_comp_tasks,
                  &((*logical2sorted_in_box)[dst_logical]), AllocateCpuThrdId);
  }
...
DEFINE_BLD_SUB_TASK_GRAPH_METHOD(BldSubTskGphByOneToOne) {
  ...
  FOR_RANGE(size_t, i, 0, sorted_src_comp_tasks.size()) {
    ...
    Connect<TaskNode>(
        Build121BufTo(
            src, dst->machine_id(), dst->MemZoneId(),
...
DEFINE_BLD_SUB_TASK_GRAPH_METHOD(BldSubTskGphBySelectOneSourceToSoleSink) {
  ...
  BldSubTskGphByOneToOne(nullptr, nullptr, {selected_src_comp_task},
                         sorted_dst_comp_tasks, nullptr, nullptr, Mut121BufTask,
                         AllocateCpuThrdId);
}
...
TaskNode* TaskGraph::Build121BufTo(
    ...
    std::function<TaskNode*(int64_t machine_id, int32_t mem_zone_id)>
        Get121BufTask,
    std::function<TaskNode*(int64_t machine_id, int32_t mem_zone_id, TaskNode*)>
        Set121BufTask) {
  {
  ...
  if (src->machine_id() != dst_machine_id) {
    if (dst_mem_zone_id == cpu_mem_zone_id) {
      TaskNode* src_cpu = Build121BufTo(src, src->machine_id(), cpu_mem_zone_id,
                                        Get121BufTask, Set121BufTask);
      ...
      TaskNode* dst_cpu = Build121BufTo(src, dst_machine_id, cpu_mem_zone_id,
                                        Get121BufTask, Set121BufTask);
      return Build121BufTo(dst_cpu, dst_machine_id, dst_mem_zone_id,
                           Get121BufTask, Set121BufTask);
    }
  } else {
    if (src->MemZoneId() == dst_mem_zone_id) {
      ...
      if (dst_mem_zone_id == cpu_mem_zone_id) {
        return Set121BufTask(dst_machine_id, dst_mem_zone_id,
                             AddCopyD2HTaskIfNotCpu(src));
      } else {
        TaskNode* src_cpu = Build121BufTo(src, dst_machine_id, cpu_mem_zone_id,
                                          Get121BufTask, Set121BufTask);
