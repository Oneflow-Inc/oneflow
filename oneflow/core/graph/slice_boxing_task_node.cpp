...
void SliceBoxingTaskNode::Init(const LogicalBlobId &lbi,
                               const TensorSliceView &out_slice,
                               const SliceBoxingTaskMode mode,
...
void SliceBoxingTaskNode::Init(const LogicalBlobId &lbi,
  ...
  if (device_type == DeviceType::kCPU) {
    ...
    mem_zone_id = global_id_mgr->GpuMemZoneId(
...
void SliceBoxingTaskNode::ProduceAllRegstsAndBindEdges() {
  ...
  this->ForEachOutDataEdge(
      [&](TaskEdge *edge) { edge->AddRegst("out", out_regst_desc); });
...
void SliceBoxingTaskNode::ConsumeAllRegsts() {
  ...
  ForEachInDataEdge([&](TaskEdge *edge) {
    ...
    ConsumeRegst("in_" + std::to_string(order_it->second),
                 edge->GetSoleRegst());
    in_data_edge_cnt += 1;
  });
...
void SliceBoxingTaskNode::BuildExecGphAndRegst() {
  ...
  std::shared_ptr<Operator> op =
      ConstructOp(GetBoxingOpConf(), &GlobalJobDesc());
  node->mut_op() = op;
...
void SliceBoxingTaskNode::SetInDataEdgeSlice(const TaskEdge *edge,
                                             const TensorSliceView &slice) {
  CHECK(in_data_edge2slice_.emplace(edge, slice).second);
  ordered_in_data_edges_.push_back(edge);
}

void SliceBoxingTaskNode::ConnectToSrcNodeWithSlice(
    TaskNode *src, TaskEdge *edge, const TensorSliceView &slice) {
  Connect<TaskNode>(src, edge, this);
  SetInDataEdgeSlice(edge, slice);
}

void SliceBoxingTaskNode::SetOutShape(const Shape &shape) {
  out_shape_ = shape;
}
