...
class SharableMemBlockNode final
    : public Node<SharableMemBlockNode, SharableMemBlockEdge> {
public:
  OF_DISALLOW_COPY_AND_MOVE(SharableMemBlockNode);
  SharableMemBlockNode(int64_t chain_id,
                       const HashSet<const RegstDescProto *> &regst_descs);

  ~SharableMemBlockNode() = default;

  int64_t chain_id() const { return chain_id_; }
  const std::vector<const RegstDescProto *> &regst_descs() const {
    return regst_descs_;
  }
...
class SharableMemBlockEdge final
    : public Edge<SharableMemBlockNode, SharableMemBlockEdge> {
public:
...
public:
  ...
  void ForEachSourceNodeGroup(
      ...
      const std::function<void(
          const std::vector<const SharableMemBlockNode *> &)> &Handler) const;
};
