...
void OfCollectiveBoxingGenericTaskNode::Init(int64_t machine_id, int64_t thrd_id,
...
void OfCollectiveBoxingGenericTaskNode::ProduceAllRegstsAndBindEdges() {
  if (boxing::of_collective::GenericOpHasOutput(
          op_conf_.of_collective_boxing_generic_conf().rank_desc())) {
...
void OfCollectiveBoxingGenericTaskNode::ConsumeAllRegsts() {
  this->ForEachInDataEdge(
      [&](TaskEdge* in_edge) { ConsumeRegst("in", SoleInDataEdge()->GetSoleRegst()); });
}

void OfCollectiveBoxingGenericTaskNode::BuildExecGphAndRegst() {
...
void OfCollectiveBoxingGenericTaskNode::InferProducedDataRegstTimeShape() {
