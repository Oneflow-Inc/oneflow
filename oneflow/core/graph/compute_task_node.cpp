...
const LogicalNode *LogicalNodeOnEdge(
  ...
  if (target_node) {
    return target_node->logical_node();
  }
  return nullptr;
}
  ...
  while (!nodes.empty()) {
    ...
    if (comp_task_node) {
      ...
      (node->*ForEachDataEdge)([&](TaskEdge *task_edge) {
        if (visited_nodes.find((task_edge->*GetNode)()) ==
            visited_nodes.end()) {
...
std::string CompTaskNode::VisualStr() const {
  std::stringstream ss;
  for (const auto &op : logical_node()->op_vec()) {
    ss << op->op_name() << "\\n";
  }
  return ss.str();
}
...
const LogicalNode *CompTaskNode::GetOneSuccLogicalNodeOnEdge(TaskEdge *edge) {
  return LogicalNodeOnEdge(edge, &TaskEdge::dst_node,
                           &TaskNode::ForEachOutDataEdge);
}

const LogicalNode *CompTaskNode::GetOnePredLogicalNodeOnEdge(TaskEdge *edge) {
  return LogicalNodeOnEdge(edge, &TaskEdge::src_node,
                           &TaskNode::ForEachInDataEdge);
}

std::vector<CompTaskNode *>
CompTaskNode::GetSuccCompTaskNodesOnEdge(TaskEdge *edge) const {
  return GetCompTaskNodesOnEdge(edge, &TaskEdge::dst_node,
                                &TaskNode::ForEachOutDataEdge);
}

std::vector<CompTaskNode *>
CompTaskNode::GetPredCompTaskNodesOnEdge(TaskEdge *edge) const {
  return GetCompTaskNodesOnEdge(edge, &TaskEdge::src_node,
                                &TaskNode::ForEachInDataEdge);
}
