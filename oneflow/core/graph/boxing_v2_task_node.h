...
class BoxingV2TaskNode final : public TaskNode {
 public:
  ...
  void Init(const LogicalBlobId& lbi, const TensorSliceView& out_view, BoxingV2TaskMode mode);
  void ProduceAllRegstsAndBindEdges() override;
  void ConsumeAllRegsts() override;
  TaskType GetTaskType() const override { return TaskType::kBoxingV2; }
  void SetInDataEdgeView(const TaskEdge* edge, const TensorSliceView& view);
 ...
 private:
  ...
  HashMap<const TaskEdge*, TensorSliceView> in_data_edge2view_;
  std::vector<const TaskEdge*> ordered_in_data_edges_;
  LogicalBlobId lbi_;
  TensorSliceView out_view_;
