...
public:
  ...
  int64_t GpuPhyId() const {
    return Global<IDMgr>::Get()->GetGpuPhyIdFromThrdId(thrd_id_);
  }
  ...
  void
  ForEachNodeOnInDataEdge(const std::function<void(TaskNode *)> &Handler) const;
  void ForEachNodeOnOutDataEdge(
      const std::function<void(TaskNode *)> &Handler) const;
  void ForEachNodeOnInOutDataEdge(
      const std::function<void(TaskNode *)> &Handler) const;
...
protected:
  ...
  std::shared_ptr<RegstDesc> NewProducedRegst(bool enable_reuse_mem,
                                              int32_t min_register_num,
                                              int32_t max_register_num,
                                              const RegstDescTypeProto &);
  ...
  void ConsumeRegst(const std::string &name,
                    const std::shared_ptr<RegstDesc> &);
  ...
  TaskEdge *GetSoleEdge(void (TaskNode::*ForEachEdge)(
      const std::function<void(TaskEdge *)> &) const) const;
  size_t GetEdgesSize(void (TaskNode::*ForEachEdge)(
      const std::function<void(TaskEdge *)> &) const) const;
...
public:
  ...
  GetRegst(const std::string &name_in_producer) const;
...
#define REGISTER_INDEPENDENT_THREAD_NUM(task_type, ...)                        \
  REGISTER_CLASS_CREATOR(                                                      \
      task_type, IndependentThreadNum4TaskType,                                \
...
#define REGISTER_TICK_TOCK_TASK_TYPE(task_type)                                \
  REGISTER_CLASS_CREATOR(task_type, TickTockTaskType,                          \
