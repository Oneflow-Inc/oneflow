...
bool NormalForwardCompTaskNode::CanProduceSeperatedRegstsForEachOutBlob()
    const {
  return logical_node()->SoleOp()->output_bns().size() > 1 &&
         IsAllOutNodeNormalForward();
}
...
void NormalForwardCompTaskNode::ProduceOutRegstByNameAndBlockNum(
    const std::string &name, size_t mem_block_num) {
  if (mem_block_num != -1) {
...
void NormalForwardCompTaskNode::ProduceAllRegstsAndBindEdges() {
  ...
  if (op.op_conf().has_user_conf()) {
    ...
    CHECK(op_reg_result != nullptr) << "op_type_name " << op_type_name
                                    << " not register";
    if (op_reg_result->same_output_regst_num > 0) {
      mem_block_num = op_reg_result->same_output_regst_num;
    }
  }
  // when output blob num > 1 and task node on out edge is all
  ...
  if (CanProduceSeperatedRegstsForEachOutBlob()) {
    ...
    ForEachOutDataEdge([&](TaskEdge *edge) {
      ...
      CHECK(dst_node != nullptr)
          << "1regst1blob ONLY support normal fw comp task node 121";
      ...
      CHECK(is_found) << "Cannot find comsumed blob in dst op: "
                      << dst_op.op_name();
    });
  } else {
...
void NormalForwardCompTaskNode::BuildTmp7BufRegsts() {
  mut_exec_gph().ForEachNode([&](ExecNode *node) {
    ...
    node->AddBnToRegstAndBindIt(&Operator::const_buf_bns,
                                GetProducedRegst("const_buf"));
  });
}
...
void NormalForwardCompTaskNode::InferProducedDataRegstTimeShape() {
  ...
  const std::shared_ptr<Shape> &in_time_shape =
      in_regsts.front()->data_regst_time_shape();
  for (const auto &regst : in_regsts) {
    CHECK(*in_time_shape == *(regst->data_regst_time_shape()));
  }

  ForEachProducedDataRegst(
      [in_time_shape](const std::string &name, RegstDesc *regst) {
        if (name == "const_buf") {
