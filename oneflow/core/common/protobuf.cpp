...
bool TryGetFieldNameAndIndex4StrVal(const std::string &fd_name_with_idx,
                                    std::string *field_name, int32_t *index) {
...
const PbFd *GetPbFdFromPbMessage(const PbMessage &msg,
...
PbMessage *MutableRepeatedMessageInPbMessage(PbMessage *msg,
                                             const std::string &field_name,
                                             int index) {
  PROTOBUF_REFLECTION((*msg), field_name);
...
const PbMessage &GetMessageInPbMessage(const PbMessage &msg, int field_index) {
  const auto *d =
      const_cast<google::protobuf::Descriptor *>(msg.GetDescriptor());
  const auto *fd = const_cast<PbFd *>(d->FindFieldByNumber(field_index));
  CHECK_NOTNULL(fd);
  const auto *r =
      const_cast<google::protobuf::Reflection *>(msg.GetReflection());
  return r->GetMessage(msg, fd);
}
...
GetFieldNameAndIndex4StrVal(const std::string &fd_name_with_idx) {
  const size_t underline_pos = fd_name_with_idx.rfind('_');
  ...
  const int32_t idx =
      oneflow_cast<int32_t>(fd_name_with_idx.substr(underline_pos + 1));
  CHECK_GE(idx, 0);
  return std::make_pair(field_name, idx);
}

std::string GetStrValInPbFdOrPbRpf(const PbMessage &msg,
  ...
  if (fd) {
    ...
    return GetPbRpfFromPbMessage<std::string>(msg, prefix_idx.first)
        .Get(prefix_idx.second);
  }
}

bool HasStrFieldInPbFdOrPbRpf(const PbMessage &msg,
                              const std::string &fd_name_may_have_idx) {
  const PbFd *fd = msg.GetDescriptor()->FindFieldByName(fd_name_may_have_idx);
  if (fd != nullptr) {
    return true;
  ...
  int32_t index = 0;
  return TryGetFieldNameAndIndex4StrVal(fd_name_may_have_idx, &field_name,
                                        &index);
}

void ReplaceStrValInPbFdOrPbRpf(PbMessage *msg,
                                const std::string &fd_name_may_have_idx,
                                const std::string &old_val,
                                const std::string &new_val) {
  const PbFd *fd = msg->GetDescriptor()->FindFieldByName(fd_name_may_have_idx);
  if (fd) {
    CHECK_EQ(GetValFromPbMessage<std::string>(*msg, fd_name_may_have_idx),
             old_val);
    SetValInPbMessage<std::string>(msg, fd_name_may_have_idx, new_val);
  } else {
    const std::pair<std::string, int32_t> prefix_idx =
        GetFieldNameAndIndex4StrVal(fd_name_may_have_idx);
    CHECK_EQ(GetPbRpfFromPbMessage<std::string>(*msg, prefix_idx.first)
                 .Get(prefix_idx.second),
             old_val);
    PbRpf<std::string> *rpf =
        MutPbRpfFromPbMessage<std::string>(msg, prefix_idx.first);
