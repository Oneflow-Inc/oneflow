...
void ObjMsgSizedMemPool::__Init__(int64_t fixed_mem_size,
...
char *ObjMsgSizedMemPool::Allocate() {
  if (free_chunk_list().empty()) {
    Prefetch();
  }
  auto chunk =
      mut_free_chunk_list()->MoveFrontToDstBack(mut_occupied_chunk_list());
...
void ObjMsgSizedMemPool::Deallocate(char *ptr) {
  auto *chunk =
      StructField<ObjMsgMemBlock, char,
                  ObjMsgMemBlock::MemPtrOffset()>::StructPtr4FieldPtr(ptr)
          ->mut_chunk();
...
void ObjMsgSizedMemPool::Prefetch(ObjMsgChunkList *free_list) {
  for (int64_t i = 0; i < prefetch_cnt(); ++i) {
    auto chunk = ObjectMsgPtr<ObjMsgChunk>::NewFrom(mut_allocator(), this,
                                                    mem_block_size());
...
int CachedObjectMsgAllocatorBase::RoundUpShift(std::size_t size) const {
  return std::max<int>(kMemSizeShiftMin,
                       (static_cast<int>(std::log2(size)) + 1));
}

char *CachedObjectMsgAllocatorBase::RoundUpAllocate(std::mutex *mutex,
                                                    std::size_t size) {
  return allocators_.at(RoundUpShift(size) - kMemSizeShiftMin)->Allocate(mutex);
}

void CachedObjectMsgAllocatorBase::RoundUpDeallocate(std::mutex *mutex,
                                                     char *ptr,
                                                     std::size_t size) {
  auto *chunk =
      StructField<ObjMsgMemBlock, char,
...
void CachedObjectMsgAllocatorBase::Prefetch() {
  ...
  for (int i = kMemSizeShiftMin; i <= mem_size_shift_max_; ++i) {
    auto mem_pool = ObjectMsgPtr<ObjMsgSizedMemPool>::NewFrom(
        backend_allocator_, 1 << i, prefetch_cnt_);
