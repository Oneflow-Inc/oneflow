...
Error::operator std::string() const {
  return PbMessage2TxtString(*error_proto_);
}
...
Error Error::MultipleOpKernelsMatchedError(
  ...
  auto *multiple_op_kernels_matched_error =
      error->mutable_multiple_op_kernels_matched_error();
  for (const auto &msg : error_msgs) {
...
Error Error::MemoryZoneOutOfMemoryError(int64_t machine_id, int64_t mem_zone_id,
                                        uint64_t calc, uint64_t available,
                                        const std::string &device_tag) {
  auto error = std::make_shared<ErrorProto>();
  auto *memory_zone_out_of_memory_error =
      error->mutable_memory_zone_out_of_memory_error();
  ...
  memory_zone_out_of_memory_error->add_available(std::to_string(available) +
                                                 " bytes");
  memory_zone_out_of_memory_error->add_required(std::to_string(calc) +
                                                " bytes");
  return error;
}
