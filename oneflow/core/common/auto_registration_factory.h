...
template <typename Base, typename... Args> struct AutoRegistrationFactory {
public:
  template <typename Derived> struct RawRegisterType {
  ...
  bool IsClassRegistered(int32_t k, Args &&... args) {
    return creators_.find(k) != creators_.end();
  }
...
#define REGISTER_CLASS_WITH_ARGS(k, Base, Derived, ...)                        \
  static AutoRegistrationFactory<Base, __VA_ARGS__>::RawRegisterType<Derived>  \
      REGISTER_VAR_NAME(k)
#define REGISTER_CLASS_CREATOR(k, Base, f, ...)                                \
  static AutoRegistrationFactory<Base, ##__VA_ARGS__>::CreatorRegisterType     \
      REGISTER_VAR_NAME(k, f)

template <typename Base, typename... Args>
inline Base *NewObj(int32_t k, Args &&... args) {
  return AutoRegistrationFactory<Base, Args...>::Get().New(
