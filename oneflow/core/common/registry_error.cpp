...
namespace {
std::shared_ptr<ErrorFrame>* MutRegistryError() {
  static std::shared_ptr<ErrorFrame> registry_error;
...
Maybe<void> CheckAndClearRegistryFlag() {
  ...
  std::shared_ptr<ErrorFrame> registry_error_old = *MutRegistryError();
...
void CatchRegistryError(const std::function<Maybe<void>()>& handler) {
  ...
  if (!maybe_error.IsOk()) {
    if (!*MutRegistryError()) { *MutRegistryError() = maybe_error.error_frame(); }
  }
}
