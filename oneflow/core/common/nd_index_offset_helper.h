...
template<typename T, int N>
class NdIndexStridedOffsetHelper {
 public:
  NdIndexStridedOffsetHelper() {}
  template<class... Ts>
  OF_DEVICE_FUNC explicit NdIndexStridedOffsetHelper(T d0, Ts... dims) {
    constexpr int n = 1 + sizeof...(dims);
    static_assert(n <= N, "");
    T dims_arr[n] = {d0, static_cast<T>(dims)...};
    InitStrides(dims_arr, n);
  ...
  OF_DEVICE_FUNC explicit NdIndexStridedOffsetHelper(const T* dims) { InitStrides(dims, N); }

  template<typename U>
  OF_DEVICE_FUNC explicit NdIndexStridedOffsetHelper(const U* dims) {
    T dims_arr[N];
    for (int i = 0; i < N; ++i) { dims_arr[i] = dims[i]; }
    InitStrides(dims_arr, N);
  }

  OF_DEVICE_FUNC explicit NdIndexStridedOffsetHelper(const T* dims, int n) { InitStrides(dims, n); }

  ~NdIndexStridedOffsetHelper() = default;
...
#pragma unroll
 ...
 private:
  OF_DEVICE_FUNC void InitStrides(const T* dims, const int n) {
    for (int i = 0; i < n; ++i) { stride_[i] = dims[i]; }
