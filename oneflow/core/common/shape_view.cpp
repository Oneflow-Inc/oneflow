...
ShapeView::ShapeView(const ShapeProto &shape_proto)
    : ShapeViewBase<const int64_t>(shape_proto.dim().data(),
                                   shape_proto.dim_size()) {}
ShapeView::ShapeView(const Shape &shape)
    : ShapeViewBase<const int64_t>(shape.dim_vec().data(),
                                   shape.dim_vec().size()) {}

template <typename DimT> int64_t ShapeViewBase<DimT>::At(int64_t index) const {
  CHECK_GE(index, 0);
...
int64_t ShapeViewBase<DimT>::Count(int64_t begin_axis, int64_t end_axis) const {
  CHECK(0 <= begin_axis && begin_axis <= end_axis &&
        end_axis <= this->NumAxes())
      << begin_axis << " " << end_axis;
  int64_t cnt = 1;
...
template <typename DimT> int64_t ShapeViewBase<DimT>::elem_cnt() const {
  return this->Count(0);
}

template <typename DimT> std::string ShapeViewBase<DimT>::ToString() const {
  std::stringstream ss;
  ss << "(";
  FOR_RANGE(int, i, 0, this->NumAxes()) {
    int64_t dim = this->At(i);
    ss << dim;
    if (i != this->NumAxes() - 1 || this->NumAxes() == 1) {
      ss << ",";
    }
  }
  ss << ")";
...
template <typename DimT> void ShapeViewBase<DimT>::ToShape(Shape *shape) const {
  DimVector dim_vec;
