...
#define POD_PROTO_DEFINE_ONEOF(oneof_name, type_and_field_name_seq)      \
  ...
  POD_PROTO_DEFINE_ONEOF_ACCESSOR(oneof_name, type_and_field_name_seq)   \
...
#define MAKE_POD_PROTO_ONEOF_ENUM_CASE(type, field_name) POD_PROTO_ONEOF_ENUM_TYPE(field_name),

#define POD_PROTO_DEFINE_ONEOF_ACCESSOR(oneof_name, type_and_field_name_seq)    \
  OF_PP_SEQ_PRODUCT_FOR_EACH_TUPLE(MAKE_POD_PROTO_ONEOF_ACCESSOR, (oneof_name), \
                                   type_and_field_name_seq)

#define MAKE_POD_PROTO_ONEOF_ACCESSOR(oneof_name, pair)                     \
  const OF_PP_PAIR_FIRST(pair) & OF_PP_PAIR_SECOND(pair)() const {          \
    CHECK(POD_PROTO_ONEOF_CASE(oneof_name)()                                \
          == POD_PROTO_ONEOF_ENUM_VALUE(OF_PP_PAIR_SECOND(pair)));          \
