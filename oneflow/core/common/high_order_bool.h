...
template <typename T> class BoolFunctor {
public:
  virtual ~BoolFunctor() {}
  virtual bool operator()(const T &ctx) const = 0;
  virtual std::string DebugStr(const T &ctx,
                               bool display_result = true) const = 0;

protected:
  BoolFunctor() = default;
};

template <typename T> class BoolFunctorPtr final {
public:
  BoolFunctorPtr() = default;
  BoolFunctorPtr(const BoolFunctorPtr &) = default;
  BoolFunctorPtr(BoolFunctorPtr &&) = default;
  ~BoolFunctorPtr(){};
  BoolFunctorPtr(const std::shared_ptr<const BoolFunctor<T>> &ptr)
      : ptr_(ptr) {}
...
template <typename T> class AndBoolFunctor final : public BoolFunctor<T> {
public:
  AndBoolFunctor() = delete;
  AndBoolFunctor(const BoolFunctorPtr<T> lhs, const BoolFunctorPtr<T> rhs)
      : lhs_(lhs), rhs_(rhs) {}
  ~AndBoolFunctor() {}

  bool operator()(const T &ctx) const override {
    return lhs_(ctx) && rhs_(ctx);
  }
...
template <typename T> class OrBoolFunctor final : public BoolFunctor<T> {
public:
  OrBoolFunctor() = delete;
  OrBoolFunctor(const BoolFunctorPtr<T> lhs, const BoolFunctorPtr<T> rhs)
      : lhs_(lhs), rhs_(rhs) {}
  ~OrBoolFunctor() {}
...
template <typename T> class NotBoolFunctor final : public BoolFunctor<T> {
public:
...
template <typename T>
BoolFunctorPtr<T> BoolFunctorPtr<T>::
operator&(const BoolFunctorPtr &ptr) const {
...
template <typename T> BoolFunctorPtr<T> BoolFunctorPtr<T>::operator~() const {
...
public:
  ...
  HighOrderBoolFunctor(const std::string &debug_str,
                       const std::function<bool(const T &)> &bool_fn)
...
template <typename ContextT, typename T> class HobContextGetter final {
public:
  HobContextGetter(const T &const_value)
      : debug_str_(ToString(const_value)),
        context_getter_(
            [const_value](const ContextT &) { return const_value; }) {}
