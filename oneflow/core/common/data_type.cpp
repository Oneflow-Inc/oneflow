...
#define CHECK_MAX_VAL(T, limit_value)                                          \
  CHECK_EQ(GetMaxVal<T>(), std::numeric_limits<T>::max());
  OF_PP_FOR_EACH_TUPLE(CHECK_MAX_VAL, MAX_VAL_SEQ);
#undef CHECK_MAX_VAL

#define CHECK_MIN_VAL(T, limit_value)                                          \
  CHECK_EQ(GetMinVal<T>(), std::numeric_limits<T>::lowest());
