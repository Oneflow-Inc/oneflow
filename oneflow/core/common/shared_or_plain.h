...
template <typename S, typename P> class SharedOrPlain final {
public:
  SharedOrPlain(P data) : shared_ptr_() { SetData(data); }
  SharedOrPlain(const SharedOrPlain &rhs);
  SharedOrPlain(const std::shared_ptr<S> &shared_ptr)
...
template <typename S, typename P>
const typename SharedOrPlain<S, P>::PlainStruct *
SharedOrPlain<S, P>::CastToPlainStruct() const {
...
template <typename S, typename P>
typename SharedOrPlain<S, P>::PlainStruct *
SharedOrPlain<S, P>::MutCastToPlainStruct() {
  PlainStruct *__attribute__((__may_alias__)) ptr =
      reinterpret_cast<PlainStruct *>(&shared_ptr_);
  return ptr;
}

template <typename S, typename P> void SharedOrPlain<S, P>::SetData(P data) {
...
template <typename S, typename P> P SharedOrPlain<S, P>::plain_data() const {
  const PlainStruct *const ptr = CastToPlainStruct();
  CHECK(ptr->is_plain_data);
  return ptr->data;
}

template <typename S, typename P> bool SharedOrPlain<S, P>::IsPlain() const {
  const PlainStruct *const ptr = CastToPlainStruct();
  return ptr->is_plain_data;
}

template <typename S, typename P> SharedOrPlain<S, P>::~SharedOrPlain() {
  if (IsPlain()) {
