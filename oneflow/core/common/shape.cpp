...
Shape CreateReducedShape(const ShapeView &shape, const AxisVector &axis_vec) {
  ...
  for (int64_t axis : axis_vec) {
    dim_vec.at(ShiftNegativeAxis(axis, shape.NumAxes())) = 1;
  }
  return Shape(std::move(dim_vec));
}

Shape CreateLeftExtendedShape(const ShapeView &shape,
                              int ndims_left_extend_to) {
  ...
  for (; i < ndims_left_extend_to; ++i) {
    dim_vec.at(i) = shape.At(i - left_ones_num);
  }
  return Shape(std::move(dim_vec));
}

Shape CreateReducedShapeOrOnesShape(const ShapeView &shape,
                                    const AxisVector &axis_vec) {
  if (axis_vec.empty()) {
    return Shape::Ones(shape.NumAxes());
  }
...
std::string Shape::ToString() const {
  ...
  for (int64_t dim : dim_vec_) {
    ss << dim;
    if (++idx != dim_vec_.size() || dim_vec_.size() == 1) {
      ss << ",";
    }
  }
  ss << ")";
...
Shape Shape::RemoveOnes(const AxisVector &axis_vec) const {
  ...
  for (int64_t i = 0; i < this->dim_vec().size(); i++) {
    if (std::find(axis_vec_shifted.begin(), axis_vec_shifted.end(), i) ==
        axis_vec_shifted.end()) {
...
AxisVector Shape::Axes4BroadcastTo(const Shape &broadcast_shape) const {
  ...
  for (int64_t i = 0; i < NumAxes(); i++) {
    if (this->dim_vec().at(i) != broadcast_shape.dim_vec().at(i) &&
        this->dim_vec().at(i) == 1) {
...
bool Shape::Containing(const Shape &small_shape) const {
  if (this->NumAxes() < small_shape.NumAxes()) {
    return false;
  }
  FOR_RANGE(int, i, 0, small_shape.NumAxes()) {
    if (this->At(i) != small_shape.At(i)) {
      return false;
    }
  }
  return true;
