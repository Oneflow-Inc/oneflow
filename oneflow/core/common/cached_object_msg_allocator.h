...
public:
  static ObjMsgMemBlock *PlacementNew(char *mem_ptr,
                                      ObjMsgChunk *obj_msg_chunk);

  ObjMsgChunk *mut_chunk() { return chunk_; }

  char *mem_ptr() { return &mem_ptr_[0]; }

  template <typename Enabled = void> static constexpr int MemPtrOffset() {
...
public:
  ...
  CachedObjectMsgAllocatorBase(ObjectMsgAllocator *backend_allocator,
                               int64_t mem_size_shift_max, int64_t prefetch_cnt)
      : backend_allocator_(backend_allocator),
        mem_size_shift_max_(mem_size_shift_max), prefetch_cnt_(prefetch_cnt) {
    Prefetch();
  }
...
public:
  ...
  CachedObjectMsgAllocator(int mem_size_shift_max, int64_t prefetch_cnt)
      : CachedObjectMsgAllocatorBase(
            ObjectMsgDefaultAllocator::GlobalObjectMsgAllocator(),
            mem_size_shift_max, prefetch_cnt) {}
  CachedObjectMsgAllocator(ObjectMsgAllocator *backend_allocator,
                           int64_t mem_size_shift_max, int64_t prefetch_cnt)
      : CachedObjectMsgAllocatorBase(backend_allocator, mem_size_shift_max,
                                     prefetch_cnt) {}

  ~CachedObjectMsgAllocator() override = default;

  char *Allocate(std::size_t size) override {
    return RoundUpAllocate(&mutex_, size);
  }
...
public:
  ThreadUnsafeObjectMsgAllocator(const ThreadUnsafeObjectMsgAllocator &) =
      delete;
  ...
  ThreadUnsafeObjectMsgAllocator(ObjectMsgAllocator *backend_allocator,
                                 int64_t mem_size_shift_max,
                                 int64_t prefetch_cnt)
      : CachedObjectMsgAllocatorBase(backend_allocator, mem_size_shift_max,
                                     prefetch_cnt),
        thread_id_(std::this_thread::get_id()) {}

  ~ThreadUnsafeObjectMsgAllocator() override {
