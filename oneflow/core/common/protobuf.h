...
template <typename K, typename V> using PbMap = google::protobuf::Map<K, V>;
...
const PbFd *GetPbFdFromPbMessage(const PbMessage &,
...
PbMessage *MutableMessageInPbMessage(PbMessage *,
                                     const std::string &field_name);
...
template <typename T> inline std::vector<T> PbRf2StdVec(const PbRf<T> &rf) {
  return std::vector<T>(rf.begin(), rf.end());
}

template <typename T> inline PbRf<T> StdVec2PbRf(const std::vector<T> &vec) {
  return PbRf<T>(vec.begin(), vec.end());
}

// PbRpf <-> std::vector
template <typename T> inline std::vector<T> PbRpf2StdVec(const PbRpf<T> &rpf) {
  return std::vector<T>(rpf.begin(), rpf.end());
}

template <typename T> inline PbRpf<T> StdVec2PbRpf(const std::vector<T> &vec) {
...
template <typename T = PbMessage>
const T *TryGetMsgPtrFromPbMessage(const PbMessage &msg,
                                   const std::string &field_name) {
  PROTOBUF_GET_FIELDDESC(msg, field_name);
  if (fd == nullptr) {
    return nullptr;
...
bool IsInRepeatedField(const PbRf<T> &repeated_field, const T &value) {
  return std::find(repeated_field.cbegin(), repeated_field.cend(), value) !=
         repeated_field.cend();
}
...
inline bool operator<(const LogicalBlobId &lhs, const LogicalBlobId &rhs) {
  ...
  if (lhs.blob_name() != rhs.blob_name()) {
    return lhs.blob_name() < rhs.blob_name();
  }
  if (lhs.is_packed_id() != rhs.is_packed_id()) {
    return lhs.is_packed_id() < rhs.is_packed_id();
  }
  return false;
}
...
template <> struct hash<oneflow::LogicalBlobId> {
...
template <> struct hash<oneflow::OpBlobArg> {
...
template <> struct hash<oneflow::SbpParallel> {
