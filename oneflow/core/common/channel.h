...
template <typename T> class Channel final {
public:
...
template <typename T> ChannelStatus Channel<T>::Send(const T &item) {
  std::unique_lock<std::mutex> lock(mutex_);
  if (is_closed_) {
    return kChannelStatusErrorClosed;
  }
  queue_.push(item);
  cond_.notify_one();
...
template <typename T> ChannelStatus Channel<T>::Receive(T *item) {
  std::unique_lock<std::mutex> lock(mutex_);
