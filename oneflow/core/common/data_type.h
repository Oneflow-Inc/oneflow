...
template <typename T> struct IsFloat16 : std::integral_constant<bool, false> {};
...
#define SPECIALIZE_GET_DATA_TYPE(type_cpp, type_proto)                         \
  ...
  inline type_cpp GetTypeByDataType(                                           \
      std::integral_constant<DataType, type_proto>) {                          \
    return {};                                                                 \
  }
OF_PP_FOR_EACH_TUPLE(SPECIALIZE_GET_DATA_TYPE,
                     ALL_DATA_TYPE_SEQ FLOAT16_DATA_TYPE_SEQ);
#undef SPECIALIZE_GET_DATA_TYPE

template <DataType type>
using DataTypeToType =
    decltype(GetTypeByDataType(std::integral_constant<DataType, type>{}));
...
template <typename T> OF_DEVICE_FUNC T GetZeroVal() {
  return static_cast<T>(0);
}

template <typename T> OF_DEVICE_FUNC T GetOneVal() { return static_cast<T>(1); }

template <typename T> OF_DEVICE_FUNC T GetMinVal();

template <typename T> OF_DEVICE_FUNC T GetMaxVal();
...
template <DeviceType, typename T> struct DevDType { typedef T type; };

#if defined(WITH_CUDA)
template <> struct DevDType<DeviceType::kGPU, float16> {
  static_assert(sizeof(float16) == sizeof(half),
...
template <typename T> void CheckDataType(DataType data_type) {
  LOG_IF(FATAL,
         (std::is_same<T, void>::value == false &&
