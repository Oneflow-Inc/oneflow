...
bool GetCudnnVersion(libraryPropertyType type, int *version) {
  ...
  if (status == CUDNN_STATUS_SUCCESS) {
    ...
    LOG(ERROR) << "Failed to get cuDNN version: "
...
bool GetCudnnVersionString(std::string *version) {
  ...
  int version_minor;
  int version_patch;
  if (!GetCudnnVersion(libraryPropertyType::MAJOR_VERSION, &version_major)) {
    return false;
  }
  if (!GetCudnnVersion(libraryPropertyType::MINOR_VERSION, &version_minor)) {
    return false;
  }
  if (!GetCudnnVersion(libraryPropertyType::PATCH_LEVEL, &version_patch)) {
    return false;
  }
...
void DumpVersionInfo() {
...
#ifdef WITH_CUDA
  {
    ...
    if (err == cudaSuccess) {
      LOG(INFO) << "CUDA runtime version: "
                << GetCudaVersionString(cuda_runtime_version);
    } else {
      LOG(ERROR) << "Failed to get cuda runtime version: "
...

  {
    ...
    if (result == ncclSuccess) {
      ...
      LOG(INFO) << "NCCL version: " << nccl_version_major << "."
                << nccl_version_minor << "." << nccl_version_patch;
    } else {
      LOG(ERROR) << "Failed to get NCCL version: "
