...
std::vector<std::vector<std::string>> GenNodeRank(const Plan &plan) {
  ...
  for (const TaskProto &task_proto : plan.task()) {
    if (task_proto.machine_id() != 0) {
      continue;
    }
    if (Global<IDMgr>::Get()->GetDeviceTypeFromThrdId(task_proto.thrd_id()) ==
        DeviceType::kGPU) {
      continue;
    }
    ...
    std::string op_name = task_proto.exec_sequence()
                              .exec_node(0)
                              .kernel_conf()
                              .op_attribute()
                              .op_conf()
...
RegstDescProto *PlanUtil::GetSoleProducedDataRegst(TaskProto *task_proto) {
  ...
  for (auto &pair : *task_proto->mutable_produced_regst_desc()) {
    ...
    if (regst_desc->regst_desc_type().has_data_regst_desc()) {
      CHECK_ISNULL(ret);
      CHECK_EQ(
          regst_desc->regst_desc_type().data_regst_desc().lbi2blob_desc_size(),
          1);
      ret = regst_desc;
    }
  }
...
std::function<const TaskProto *(int64_t)>
PlanUtil::MakeGetterTaskProto4TaskId(const Plan &plan) {
  auto task_id2task_proto =
      std::make_shared<HashMap<int64_t, const TaskProto *>>();
  for (const TaskProto &task_proto : plan.task()) {
    task_id2task_proto->emplace(task_proto.task_id(), &task_proto);
  }
  return [task_id2task_proto](int64_t task_id) {
    return task_id2task_proto->at(task_id);
  };
}
...
void PlanUtil::CleanUselessMemBlockAndCheckValid(Plan *plan) {
  ...
  for (const auto &mem_block : plan->block_chunk_list().mem_block()) {
    CHECK(mem_block_id2mem_block.emplace(mem_block.mem_block_id(), mem_block)
              .second);
  }
  ...
  for (const TaskProto &task : plan->task()) {
    for (const auto &pair : task.produced_regst_desc()) {
      ...
      CHECK(mem_block_id2mem_block.find(regst.mem_block_id()) !=
            mem_block_id2mem_block.end());
      const MemBlockProto &mem_block =
          mem_block_id2mem_block.at(regst.mem_block_id());
      ...
      if (separated_header_mem_size > 0) {
        ...
        CHECK(header_mem_block.mem_case() ==
              MemoryCaseUtil::GetHostPinnedMemoryCaseForRegstSeparatedHeader(
                  regst.mem_case()));
        CHECK(header_mem_block.enable_reuse_mem() == false);
        const auto &header_block_job_ids =
            mem_block_id2job_ids[header_block_id];
        CHECK(header_block_job_ids.find(task.job_id()) !=
              header_block_job_ids.end());
  ...
  for (const auto &pair : mem_block_id2mem_block) {
    ...
    if (mem_block.has_chunk_id()) {
      ...
      CHECK_GE(chunk.mem_size(),
               mem_block.chunk_offset() + mem_block.mem_size());
      CHECK_EQ(mem_block.job_id_size(), 1);
      CHECK_GE(chunk.job_id_size(), 1);
      const HashSet<int64_t> &chunk_job_ids =
          chunk_id2job_ids.at(chunk.chunk_id());
...
void PlanUtil::ToDotFile(const Plan &plan, const std::string &filepath) {
  ...
  std::vector<std::vector<std::vector<std::string>>>
      machine_id2device_id2node_list(machine_num);
  ...
  HashMap<int64_t, HashMap<int64_t, std::string>>
      task_id2consumer_regst_id2name;
  HashMap<int64_t, std::string> task_id2op_name;

  auto InsertNodeDefByTaskProto = [&](const TaskProto &task_proto,
                                      const std::string &node_def) {
    if (Global<IDMgr>::Get()->GetDeviceTypeFromThrdId(task_proto.thrd_id()) ==
        DeviceType::kGPU) {
      int64_t device_id =
          Global<IDMgr>::Get()->GetGpuPhyIdFromThrdId(task_proto.thrd_id());
      machine_id2device_id2node_list[task_proto.machine_id()][device_id]
          .push_back(node_def);
    } else {
  ...
  auto GenEdgeColorStr = [](const RegstDescTypeProto &type) {
    if (type.has_ctrl_regst_desc()) {
      return "fontcolor=\"gray65\",color=\"gray65\"";
    }
    return "fontcolor=\"gray15\",color=\"gray15\"";
  };

  auto IsEsac2ReentrantLockEdge = [](const std::string &src_name,
  ...
  auto IsEsacNode = [](const std::string &name) {
    if (name.find("Esac") != std::string::npos) {
      return true;
    }
    return false;
  };
  ...
  for (const TaskProto &task_proto : plan.task()) {
    std::string node_def =
        "task" + std::to_string(task_proto.task_id()) + "[label=\"{{";
    // node_def += "<task_node_" + std::to_string(task_proto.task_id()) + ">";
    std::string op_name = "";
    for (const ExecNodeProto &exec_node :
         task_proto.exec_sequence().exec_node()) {
    ...
    for (const auto &pair : task_proto.produced_regst_desc()) {
      ...
      node_def += (pair.first + ":" + regst_id + ":" +
                   std::to_string(pair.second.register_num()));
      ++index;
    }
    node_def += "}}";
    node_def += ("\",tooltip=\"" + TaskType_Name(task_proto.task_type()) +
                 ...
                 ",colorscheme=set312, fillcolor=" +
                 std::to_string((task_proto.job_id() % 12) + 1));
    if (IsEsacNode(op_name)) {
      node_def += ",width=5,height=1.5";
    }
    node_def += "];\n";
    InsertNodeDefByTaskProto(task_proto, node_def);
    for (const auto &pair : task_proto.consumed_regst_desc_id()) {
      for (int64_t regst_desc_id : pair.second.regst_desc_id()) {
        task_id2consumer_regst_id2name[task_proto.task_id()][regst_desc_id] =
            pair.first;
      }
    }
  }
    ...
    for (const auto &pair : task_proto.produced_regst_desc()) {
      ...
      for (int64_t consumer_task_id : regst.consumer_task_id()) {
        ...
        std::string consumer_regst_name =
            task_id2consumer_regst_id2name[consumer_task_id]
                                          [regst.regst_desc_id()];
        ...
        if (IsEsac2ReentrantLockEdge(producer_op_name, consumer_op_name)) {
          log_stream << dst_node << "->" << src_node
                     << "[arrowhead=\"invempty\",fontcolor=\"red\",color="
                        "\"red\",taillabel=\""
                     << consumer_regst_name << "\",tailtooltip=\"" << tooltip;
        } else {
          log_stream << src_node << "->" << dst_node << "["
                     << GenEdgeColorStr(regst.regst_desc_type())
                     << ",headlabel=\"" << consumer_regst_name
                     << "\",headtooltip=\"" << tooltip;
        }
        log_stream << "\",tooltip=\"" << tooltip
                   << "\",arrowsize=0.5,labeldistance=1.5,penwidth=2"
                   << "];\n";
      }
    }
  }
  ...
  for (const auto &current_list : rank_list) {
    log_stream << "{ rank = same;";
    for (const auto &node_name : current_list) {
      log_stream << node_name << "; ";
    }
    log_stream << "}\n";
  }
  log_stream << "}\n";
