...
class NcclCollectiveBoxingExecutorBackend
    : public CollectiveBoxingExecutorBackend {
public:
...
private:
  ...
  std::vector<std::map<int64_t, std::unique_ptr<NcclDeviceCtx>>>
      stream_id2device_id2device_ctx_;
...
NcclCollectiveBoxingExecutorBackend::NcclCollectiveBoxingExecutorBackend()
  ...
  fusion_threshold_ =
      collective_boxing_conf_.nccl_fusion_threshold_mb() * 1024 * 1024;
    ...
    while (true) {
      {
        ...
        if (local_event_list.empty()) {
          event_list_cond_.wait(
              lock, [this]() { return (!event_list_.empty()) || shutdown_; });
        }
        local_event_list.splice(local_event_list.end(), event_list_);
      }
      if (local_event_list.empty() && shutdown_) {
        break;
      }
      for (auto it = local_event_list.begin(); it != local_event_list.end();) {
        ...
        if (err == cudaErrorNotReady) {
          ...
          auto callback_ptr =
              std::make_shared<std::function<void(Maybe<void>)>>(
...
NcclCollectiveBoxingExecutorBackend::~NcclCollectiveBoxingExecutorBackend() {
  ...
  for (auto &device_set7stream_id2device_id2comm :
       device_set2stream_id2device_id2comm_) {
...
void NcclCollectiveBoxingExecutorBackend::GroupRequests(
  ...
  auto CanFuse = [&](const RequestDesc *lhs, const RequestDesc *rhs) -> bool {
    if (lhs->device_set() != rhs->device_set()) {
      return false;
    }
    if (!IsOpFusionEnabled(lhs) || !IsOpFusionEnabled(rhs)) {
      return false;
    }
    if (lhs->op_desc().op_type() != rhs->op_desc().op_type()) {
      return false;
    }
    ...
    if (op_type == OpType::kOpTypeAllReduce) {
      if (collective_boxing_conf_.nccl_fusion_all_reduce_use_buffer()) {
        ...
        return lhs->op_desc().reduce_method() ==
                   rhs->op_desc().reduce_method() &&
  ...
  for (const RequestDesc *request : requests) {
    ...
    if (group.empty() || !CanFuse(group.back(), request) ||
...
void NcclCollectiveBoxingExecutorBackend::ExecuteGroup(
  ...
  std::map<int64_t, std::vector<std::function<void(Maybe<void>)>>>
      device_id2callbacks;
  ...
  auto &device_id2comm =
      device_set2stream_id2device_id2comm_.at(group.front()->device_set())
          .at(stream_id);
  ...
  if (group.front()->op_desc().op_type() == OpType::kOpTypeAllReduce &&
    ...
    for (int64_t i = 0; i < group.size(); ++i) {
      ...
      if (i != 0) {
        CHECK_EQ(request_desc->op_desc().reduce_method(),
                 group.front()->op_desc().reduce_method());
        CHECK_EQ(request_desc->op_desc().data_type(),
                 group.front()->op_desc().data_type());
      }
      const std::map<int64_t, RuntimeRequestInfo> &rank2request_info =
          ranks.at(i);
      ...
      for (const auto &rank7request_info : rank2request_info) {
        ...
        const DeviceDesc &device_desc =
            request_desc->device_set().device().Get(rank);
    ...
    const int64_t size_of_data_type =
        GetSizeOfDataType(group.front()->op_desc().data_type());
    ...
    for (auto &device_id7comm : device_id2comm) {
      ...
      OF_NCCL_CHECK(ncclAllReduce(
          device_ctx->fusion_buffer, device_ctx->fusion_buffer, elem_cnt,
    ...
    for (int64_t i = 0; i < group.size(); ++i) {
      ...
      for (const auto &rank7request_info : rank2request_info) {
        ...
        device_id2callbacks[device_id].push_back(request_info.callback);
        if (op_type == OpType::kOpTypeAllReduce) {
          OF_NCCL_CHECK(ncclAllReduce(send_buff, recv_buff, elem_cnt,
                                      nccl_data_type,
                                      GetNcclReduceOp(op_desc.reduce_method()),
                                      comm, device_ctx->stream));
        } else if (op_type == OpType::kOpTypeAllGather) {
          CHECK_EQ(elem_cnt % num_ranks, 0);
          OF_NCCL_CHECK(ncclAllGather(send_buff, recv_buff,
                                      elem_cnt / num_ranks, nccl_data_type,
                                      comm, device_ctx->stream));
        } else if (op_type == OpType::kOpTypeReduceScatter) {
          CHECK_EQ(elem_cnt % num_ranks, 0);
          ...
          OF_NCCL_CHECK(ncclReduce(send_buff, recv_buff, elem_cnt,
                                   nccl_data_type,
                                   GetNcclReduceOp(op_desc.reduce_method()),
                                   op_desc.root(), comm, device_ctx->stream));
        } else if (op_type == OpType::kOpTypeBroadcast) {
          OF_NCCL_CHECK(ncclBroadcast(send_buff, recv_buff, elem_cnt,
                                      nccl_data_type, op_desc.root(), comm,
                                      device_ctx->stream));
        } else {
          UNIMPLEMENTED();
        }
  ...
  for (auto &device_id7callbacks : device_id2callbacks) {
    ...
    OF_CUDA_CHECK(
        cudaEventRecord(event, device_id2device_ctx.at(device_id)->stream));
    {
      std::unique_lock<std::mutex> event_list_lock(event_list_mutex_);
      event_list_.emplace_back(
          Event{device_id, event, [=](const Maybe<void> &status) {
...
void NcclCollectiveBoxingExecutorBackend::Init(
  ...
  for (const auto &job_id7request_set :
    ...
    for (const RequestDesc *request : requests) {
      ...
      if (device_set2stream_id2device_id2comm_.count(device_set) > 0) {
        continue;
      }
      auto &stream_id2device_id2comm =
          device_set2stream_id2device_id2comm_[device_set];
      ...
      for (int64_t stream_id = 0; stream_id < num_streams_; ++stream_id) {
        ...
        if (local_ranks.count(0) > 0) {
          ...
          if (local_ranks.size() != device_set.device_size()) {
            const std::string rpc_key =
                GetNcclUniqueIdRpcKey(request->op_desc().name(), stream_id);
            Global<CtrlClient>::Get()->PushKV(
                rpc_key, NcclUniqueIdToString(nccl_unique_id));
          }
        } else {
          const std::string rpc_key =
              GetNcclUniqueIdRpcKey(request->op_desc().name(), stream_id);
          Global<CtrlClient>::Get()->PullKV(
              rpc_key, [&nccl_unique_id](const std::string &val) {
        ...
        for (const int64_t rank : local_ranks) {
          const int64_t device_id = device_set.device(rank).device_id();
          OF_CUDA_CHECK(cudaSetDevice(device_id));
          OF_NCCL_CHECK(ncclCommInitRank(&device_id2comm.at(device_id),
                                         device_set.device_size(),
                                         nccl_unique_id, rank));
        }
        OF_NCCL_CHECK(ncclGroupEnd());
      }
  ...
  OF_CUDA_CHECK(cudaDeviceGetStreamPriorityRange(
      nullptr, &cuda_stream_greatest_priority));
  ...
  for (int64_t stream_id = 0; stream_id < num_streams_; ++stream_id) {
    ...
    for (const int64_t device_id : local_device_ids) {
      device_id2device_ctx_.emplace(device_id,
                                    std::make_unique<NcclDeviceCtx>());
    }
    for (const int64_t device_id : local_device_ids) {
      auto &device_ctx = device_id2device_ctx_.at(device_id);
      OF_CUDA_CHECK(cudaSetDevice(device_id));
      OF_CUDA_CHECK(cudaStreamCreateWithPriority(
          &device_ctx->stream, cudaStreamNonBlocking,
...
void CollectiveBoxingExecutor::Init() {
  for (const auto &job_id7request_set :
       collective_boxing_plan_.job_id2request_set()) {
    ...
    for (const auto *request : requests) {
      if ((!collective_boxing_conf.enable_fusion()) || rough_groups.empty() ||
          request->dependency_depth() !=
              rough_groups.back().front()->dependency_depth() ||
    ...
    for (const auto &rough_group : rough_groups) {
      ...
      for (const auto &group : groups) {
        ...
        for (const auto *request : group) {
          std::set<int64_t> local_ranks;
          for (int64_t rank = 0; rank < request->device_set().device_size();
               ++rank) {
          ...
          CHECK(name2request_id_.emplace(request->op_desc().name(), request_id)
                    .second);
...
void CollectiveBoxingExecutor::DumpSummary() const {
  ...
  for (int64_t group_id = 0; group_id < group_id2group_state_.size();
       ++group_id) {
...
void CollectiveBoxingExecutor::Enqueue(const RankDesc &rank_desc,
  ...
  std::unique_lock<std::mutex> lock(mutex_);
  {
    ...
    if (request_state.IsReady()) {
      group_id2group_state_.at(request_state.group_id)
          .AddReadyRequest(request_id);
    }
  }
  ...
  while (true) {
    ...
    if (group_state.IsReady()) {
      ...
      current_group_idx_in_job_ =
          (current_group_idx_in_job_ + 1) % group_ids.size();
...
void CollectiveBoxingExecutor::RequestState::AddReadyRank(
    const RankDesc &rank_desc, const RuntimeRequestInfo &request_info) {
