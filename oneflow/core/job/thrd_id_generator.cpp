...
ThrdIdGenerator::ThrdIdGenerator(
  ...
  for (const auto &machine_task_type : machine_task_type_vec) {
    if (IsClassRegistered<TickTockTaskType>(machine_task_type.second)) {
      continue;
    }
    ...
    machine_task_type2thrd_num_[machine_task_type]++;
    machine2task_types[machine_task_type.first].emplace(
        machine_task_type.second);
  }
...
int64_t
ThrdIdGenerator::GetModThrdId(std::pair<int64_t, int64_t> machine_task_type) {
...
bool ThrdIdGenerator::TaskTypeThrdNumEqMax(int64_t task_type,
                                           int32_t thrd_num) {
