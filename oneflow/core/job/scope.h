...
#define DEFINE_SCOPE_CONFIG_GETTER(T, func_name)               \
  T func_name(const std::string& field_name) const {           \
    return CHECK_JUST(attrs_accessor_->func_name(field_name)); \
  }
  DEFINE_SCOPE_CONFIG_GETTER(bool, Bool);
  DEFINE_SCOPE_CONFIG_GETTER(int64_t, Int64);
  DEFINE_SCOPE_CONFIG_GETTER(double, Double);
  DEFINE_SCOPE_CONFIG_GETTER(const std::string&, String);
 ...
 private:
  ...
  std::unique_ptr<const DefaultedAttrsAccessor> attrs_accessor_;
};
