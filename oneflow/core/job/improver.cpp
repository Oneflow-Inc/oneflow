...
bool IsSharableRegstWithoutConsumer(const RegstDescProto &regst_desc) {
  return regst_desc.mem_block_id() == -1 &&
         regst_desc.consumer_task_id_size() == 0 &&
         regst_desc.enable_reuse_mem();
}

bool IsConsumersAndProducerInSameChain(
  ...
  for (int64_t consumer_task_id : regst_desc.consumer_task_id()) {
    if (ChainId4TaskId(consumer_task_id) != producer_chain_id) {
      return false;
    }
  }
  return true;
...
void ForEachSharableStreamRegstDescsWithoutConsumer(
    ...
    const std::function<void(const std::vector<const RegstDescProto *> &)>
        &Handler) {
  HashMap<int64_t, std::vector<const RegstDescProto *>>
  ...
  for (const auto &task : plan.task()) {
    ...
    for (const auto &pair : task.produced_regst_desc()) {
      if (IsSharableRegstWithoutConsumer(pair.second)) {
        global_work_stream_id2regst_descs[global_work_stream_id].push_back(
            &pair.second);
      }
    }
  }
...
void ForEachSameColoredStreamRegstDescWithoutConsumer(
    ...
    const std::function<void(const std::vector<const RegstDescProto *> &)>
        &Handler) {
  auto GetProducerTaskId = [](const RegstDescProto *regst_desc,
...
void ForEachSameColoredChainRegstDescs(
    ...
    const std::function<void(const std::vector<const RegstDescProto *> &)>
        &Handler) {
  std::vector<std::vector<const SharableMemBlockNode *>>
      sharable_mem_blocks_vec;
  sharable_mem_block_gph.ForEachSourceNodeGroup(
      &SharableMemBlockNode::chain_id,
      [&](const std::vector<const SharableMemBlockNode *>
              &sharable_mem_blocks) {
        sharable_mem_blocks_vec.push_back(sharable_mem_blocks);
      });
  std::vector<std::vector<std::vector<const RegstDescProto *>>>
      same_colored_regst_descs_vec(sharable_mem_blocks_vec.size());
  int64_t cpu_num = std::thread::hardware_concurrency();
  int64_t thread_pool_size =
      std::min<int64_t>(sharable_mem_blocks_vec.size(), cpu_num);
  ...
  FOR_RANGE(int64_t, i, 0, sharable_mem_blocks_vec.size()) {
    thread_pool.AddWork([i, &GetRegstDescs, &ComputeLifetimeSameChainActorIds,
                         &sharable_mem_blocks_vec,
                         &same_colored_regst_descs_vec, &counter]() {
      const auto &sharable_mem_blocks = sharable_mem_blocks_vec.at(i);
      RegstLifetimeGraph(GetRegstDescs(sharable_mem_blocks),
                         ComputeLifetimeSameChainActorIds)
  ...
  for (const auto &regst_descs_vec : same_colored_regst_descs_vec) {
    for (const auto &regst_descs : regst_descs_vec) {
      Handler(regst_descs);
    }
  }
}
...
void ForEachSameColoredChainRegstDescWithConsumer(
  ...
  auto ChainId4TaskId = [&](int64_t task_id) {
    return plan_task_graph.TaskProto4TaskId(task_id)
        ->task_set_info()
        .chain_id();
  };
  ...
  SharableMemBlockGraph sharable_mem_block_gph(plan_task_graph,
                                               IsSharableRegstWithConsumer);
  ...
  HashMap<const RegstDescProto *, std::vector<const RegstDescProto *>>
      header2members;
  for (const SharableMemBlockNode *sharable_mem_block :
       sharable_mem_block_gph.source_nodes()) {
    ...
    std::sort(regst_descs.begin(), regst_descs.end(),
              [&](const RegstDescProto *lhs, const RegstDescProto *rhs) {
                return regst_desc2mem_size.at(lhs) >
                       regst_desc2mem_size.at(rhs);
              });
  ...
  auto GetRegstDescs = [&](
    ...
    for (const SharableMemBlockNode *sharable_mem_block : sharable_mem_blocks) {
      for (const RegstDescProto *regst_desc :
           sharable_mem_block->regst_descs()) {
...
void ForEachInferredMemBlockId(
  ...
  ForEachSameColoredChainRegstDescWithConsumer(plan_task_graph,
                                               HandleMemBlockId);
}
...
uint64_t
CalcRegstNum(const RegstDescProto &regst_desc,
...
uint64_t CalcMemoryConsumed(
  ...
  for (const RegstDescProto *regst_desc : regst_descs) {
    ...
    uint64_t total_byte_size =
        RtRegstDesc(*regst_desc).TotalMainByteSize4AllRegst();
...
std::shared_ptr<HashMap<int64_t, RegstDescProto *>>
MakeRegstDescId2RegstDesc(Plan *plan) {
  auto regst_desc_id2regst_desc =
      std::make_shared<HashMap<int64_t, RegstDescProto *>>();
...
MakeGetterPathDurations4RegstDescId(const ChainActGraph &graph) {
...
std::function<const HashMap<int64_t, double> &(int64_t)>
MakeGetterPathIIScales4RegstDescId(const ChainActGraph &graph) {
...
void TryConnectWithMemSafeGuardCtrlRegstDesc(TaskProto *src_task_proto,
  ...
  RegstDescProto *ctrl_regst_desc = FindOrCreateProducedCtrlRegstDesc(
      src_task_proto, "out_ctrl_shared_mem_safe_guard");
  ...
  if (!IsInRepeatedField(ctrl_regst_desc->consumer_task_id(), dst_task_id)) {
    ...
    CHECK(!IsInRepeatedField(consumed_ctrl_regst_desc_ids->regst_desc_id(),
                             ctrl_regst_desc_id));
    consumed_ctrl_regst_desc_ids->add_regst_desc_id(ctrl_regst_desc_id);
  }
}

void CollectTailRegstConsumerTaskIds(
    const std::vector<const RegstDescProto *> &shared_mem_regsts,
    HashSet<int64_t> *task_ids) {
  for (const RegstDescProto *regst_proto : shared_mem_regsts) {
    if (regst_proto == shared_mem_regsts.front()) {
      continue;
    }
    for (int64_t consumer_id : regst_proto->consumer_task_id()) {
      task_ids->insert(consumer_id);
    }
  }
}

void CollectSinkTaskIds(
    const HashSet<int64_t> &task_ids,
  ...
  auto IsReachableToAnyOtherTask = [&](int64_t src_task_id) -> bool {
    for (int64_t dst_task_id : task_ids) {
      ...
      if (IsReachable(src_task_id, dst_task_id)) {
        return true;
      }
    }
    return false;
  ...
  for (int64_t src_task_id : task_ids) {
    if (!IsReachableToAnyOtherTask(src_task_id)) {
      sink_task_ids->push_back(src_task_id);
    }
  }
}

std::function<void(const std::vector<const RegstDescProto *> &)>
MakeSetterAddCtrlRegst(
  ...
  for (int i = 0; i < plan->task_size(); i++) {
    ...
    CHECK(
        task_id2task_proto->emplace(task_proto->task_id(), task_proto).second);
  }
  return [task_id2task_proto, IsReachable](
      const std::vector<const RegstDescProto *> &shared_mem_regsts) {
    if (shared_mem_regsts.size() == 1) {
      return;
    }
    ...
    CollectTailRegstConsumerTaskIds(shared_mem_regsts,
                                    &tail_regsts_consumer_task_ids);
    std::list<int64_t> sink_task_ids;
    CollectSinkTaskIds(tail_regsts_consumer_task_ids, IsReachable,
                       &sink_task_ids);
    for (int64_t sink_task_id : sink_task_ids) {
      TaskProto *sink_task_proto = task_id2task_proto->at(sink_task_id);
      TryConnectWithMemSafeGuardCtrlRegstDesc(header_task_proto,
                                              sink_task_proto);
    }
  };
}

void FixReliantCtrlRegstNum(
    const Plan &plan, const std::function<uint64_t(int64_t)> &GetRegstNum,
  ...
  for (const auto &task_proto : plan.task()) {
    for (const auto &pair : task_proto.produced_regst_desc()) {
      ...
      if (regst_type.has_ctrl_regst_desc() &&
        ...
        uint64_t regst_num =
            GetRegstNum(regst_type.ctrl_regst_desc().reliant_regst_desc_id()) +
    ...
    for (auto &pair : *task->mutable_produced_regst_desc()) {
      ...
      if (regst_desc->has_hint_inplace_consumed_regst_desc_id()) {
        ...
        if (in_regst_desc->mem_block_id() != -1 &&
            ...
            in_regst_desc->mem_block_offset() ==
...
void GenMemBlockAndChunk4Plan(Plan *plan) {
  ...
  auto GenMemBlock4RegstIfNeed = [&](RegstDescProto *regst_desc, int64_t job_id,
    ...
    int64_t regst_separated_size =
        rt_regst_desc.TotalSeparatedHeaderByteSize4AllRegst();

    if (mem_block_id2mem_block.find(mem_block_id) ==
  ...
  auto GenChunk4ReusedMemBlockIfNeed = [&](MemBlockProto *mem_block) {
    int64_t mzuid = MemoryCaseUtil::GenMemZoneUniqueId(mem_block->machine_id(),
                                                       mem_block->mem_case());
  ...
  for (auto &pair : mem_block_id2mem_block) {
    ...
    if (mem_block->enable_reuse_mem()) {
      GenChunk4ReusedMemBlockIfNeed(mem_block);
    }
  }
...
Maybe<void> Improver::CheckAllZoneNotOOM(
  ...
  FOR_RANGE(int64_t, machine_id, 0, mz_regst_descs.size()) {
    FOR_RANGE(int64_t, mem_zone_id, 0, mz_regst_descs[machine_id].size()) {
      ...
      const uint64_t calc = CalcMemoryConsumed(
          regst_descs, PathDurations4RegstDescId, PathIIScales4RegstDescId, ii);
      ...
      if (calc >= available) {
        ...
        const char *device_tag = JUST(DeviceTag4DeviceType(
            id_mgr->IsGpuMemZone(mem_zone_id) ? DeviceType::kGPU
                                              : DeviceType::kCPU));
        return Error::MemoryZoneOutOfMemoryError(machine_id, mem_zone_id, calc,
                                                 available, device_tag)
...
Maybe<double> Improver::BinarySearchII(
  ...
  JUST(CheckAllZoneNotOOM(mz_regst_descs, PathDurations4RegstDescId,
  ...
  while ((r - l) > ii_search_threshold) {
    mid = (l + r) / 2;
    const auto &oom_status =
        TRY(CheckAllZoneNotOOM(mz_regst_descs, PathDurations4RegstDescId,
...
Maybe<void> Improver::ForEachImprovedRegstNum(
  ...
  if (is_memory_limited) {
    ...
    ii = JUST(BinarySearchII(ii, PathDurations4RegstDescId,
                             PathIIScales4RegstDescId, mz_regst_descs));
  }
  LOG(INFO) << "memory " << (is_memory_limited ? "limited" : "unlimited")
            << " ii: " << ii;
  for (const auto &task_proto : plan.task()) {
    for (const auto &pair : task_proto.produced_regst_desc()) {
      uint64_t regst_num = 0;
      if (pair.second.has_inplace_consumed_regst_desc_id()) {
        ...
        regst_num = CalcRegstNum(pair.second, PathDurations4RegstDescId, ii,
                                 PathIIScales4RegstDescId);
      }
...
void Improver::ForEachInferredMemBlockCriticalSection(
    const Plan &plan,
    const std::function<int64_t(int64_t)> &OrderInGraph4TaskId,
    const std::function<void(const std::vector<const RegstDescProto *> &)>
        &Handler) const {
  HashMap<int32_t, std::vector<const RegstDescProto *>>
      mem_block_id2regst_descs;
  for (const auto &task : plan.task()) {
    for (const auto &pair : task.produced_regst_desc()) {
      int32_t mem_block_id = pair.second.mem_block_id();
      if (mem_block_id > start_mem_block_id_ &&
          pair.second.consumer_task_id_size() > 0) {
  ...
  for (auto &pair : mem_block_id2regst_descs) {
    std::sort(pair.second.begin(), pair.second.end(),
              [&](const RegstDescProto *lhs, const RegstDescProto *rhs) {
                int64_t lhs_order_in_graph =
                    OrderInGraph4TaskId(lhs->producer_task_id());
                int64_t rhs_order_in_graph =
                    OrderInGraph4TaskId(rhs->producer_task_id());
...
Maybe<Plan> Improver::GenAndInferMemBlockIdOnly(const AvailableMemDesc &amd,
  ...
  auto Zero2One = [&](int64_t) -> const HashMap<int64_t, double> & {
    return zero2one;
  };
...
Maybe<Plan> Improver::Improve(const AvailableMemDesc &amd,
                              const Plan &naive_plan,
  ...
  auto PathDurations4RegstDescId =
      MakeGetterPathDurations4RegstDescId(chain_act_graph);
  auto PathIIScales4RegstDescId =
      MakeGetterPathIIScales4RegstDescId(chain_act_graph);
  ...
  JUST(ForEachImprovedRegstNum(
      complete_plan, true, base_ii, PathDurations4RegstDescId,
      PathIIScales4RegstDescId, MakeSetterSetPlanRegstNum(&plan)));
  FixReliantCtrlRegstNum(plan, MakeGetterGetPlanRegstNum(&plan),
...
Plan Improver::GenAndInferMemBlockId(const Plan &naive_plan) const {
  ...
  PlanTaskGraph plan_task_graph(naive_plan);
  {
    ...
    if (GlobalJobDesc().use_memory_allocation_algorithm_v2()) {
      ...
      ForEachInferredMemBlockId(plan_task_graph, [&](int64_t regst_desc_id,
                                                     int64_t mem_block_id) {
        regst_desc_id2regst_desc->at(regst_desc_id)
            ->set_mem_block_id(mem_block_id);
  ...
  }
  {
    ...
    ForEachInferredMemBlockCriticalSection(
        plan, OrderInGraph4TaskId, MakeSetterAddCtrlRegst(&plan, IsReachable));
  }
  return plan;
}
