...
public:
  ...
  GetParallelDescFromProducerView(const std::string &lbn) const;

  bool IsMirroredBlob(const std::string &lbn) const;
  Maybe<int> MirroredBlobGetNumSubLbi(const std::string &lbn) const;
  Maybe<const LogicalBlobId *> MirroredBlobGetSubLbi(const std::string &lbn,
                                                     int index) const;

  Maybe<Shape>
  MirroredBlobGetStaticShape(const std::string &lbn_with_hint) const;
  Maybe<DataType>
  MirroredBlobGetDataType(const std::string &lbn_with_hint) const;
  ...
  Maybe<OptInt64>
  MirroredBlobGetBatchAxis(const std::string &lbn_with_hint) const;
  Maybe<OptInt64> MirroredBlobGetSplitAxisFromProducerView(
...
protected:
  virtual Maybe<void>
  CheckAllInputsWithSameParallelNum(const Operator &op,
                                    int32_t parallel_num) const = 0;
  virtual std::string GetMirroredOpName(const std::string &op_name,
  ...
  virtual Maybe<LogicalBlobId>
  FindOrCreateMirroredLbiFromCompatibleConsistentBlob(
      const LogicalBlobId &lbn) = 0;

  Job *mut_job() const { return job_; }
  int64_t job_id() const { return job_id_; }
  const HashMap<LogicalBlobId, std::vector<LogicalBlobId>> &
  mirrored_lbi2sub_lbis() const {
    return mirrored_lbi2sub_lbis_;
  }
  HashMap<LogicalBlobId, std::vector<LogicalBlobId>> *
  mut_mirrored_lbi2sub_lbis() {
    return &mirrored_lbi2sub_lbis_;
  }
  ...
  Maybe<OpAttribute> AddAndInferOp(const OperatorConf &op_conf,
...
private:
  ...
  Maybe<void>
  AddOpNameParallelConf2Placement(const std::string &op_name,
                                  const ParallelConf &parallel_conf);
  void InitIbn2DisableBoxing(const Operator &op,
                             HashMap<std::string, bool> *ibn2disable_boxing);
  void
  UpdateLbi2DisableBoxing(const Operator &op,
  ...
  Maybe<void> InferMirroredSignature(Operator *,
                                     bool is_mirrored_parallel_view_conf,
                                     const ParallelDesc &);
  Maybe<void> InferOpOutSbpParallel(Operator *, const SbpSignature &,
                                    const ParallelDesc &);
  ...
  HashMap<ParallelDesc, BlobPlacementGroup *>
      parallel_desc2blob_placement_group_;
...
public:
  ...
  LazyJobBuildAndInferCtx(Job *job, int64_t job_id)
      : JobBuildAndInferCtx(job, job_id) {}
...
private:
  ...
  std::string GetMirroredOpName(const std::string &op_name,
                                int64_t parallel_id) const override;
  int64_t SizeOfSubConsistentOpList(int64_t parallel_num) const override {
    return parallel_num;
  }
  ParallelConf GetMirroredOpParallelConf(const ParallelDesc &,
                                         int64_t parallel_id) const override;
...
public:
  ...
  EagerJobBuildAndInferCtx(Job *job, int64_t job_id)
      : JobBuildAndInferCtx(job, job_id) {}
