...
bool CompareVariableOpConf(const VariableOpConf &lhs,
                           const VariableOpConf &rhs) {
...
void SetModelIoDefaultJobConf(JobConfigProto *job_conf,
...
OperatorConf GenInputOpConf(const std::string &op_name, const std::string &out,
  ...
  InterfaceOpUtil::InitBlobConf(input_conf->mutable_blob_conf(),
                                parallel_blob_conf);
  ...
  FOR_RANGE(int64_t, job_id, 0, jobs.size()) {
    for (const OperatorConf &op_conf : jobs.at(job_id)->net().op()) {
      if (op_conf.has_variable_conf()) {
        if (var_op_name2op_conf->find(op_conf.name()) ==
            var_op_name2op_conf->end()) {
          CHECK(var_op_name2op_conf->emplace(op_conf.name(), op_conf).second);
        } else {
          CHECK(CompareVariableOpConf(
              var_op_name2op_conf->at(op_conf.name()).variable_conf(),
...
void MakeModelInitJob(
  ...
  job_builder.AddOps(master_parallel_conf,
                     {foreign_input_op_conf, tick_op_conf});

  if (var_op_name2op_conf.empty()) {
    return;
  }
  OperatorConf model_init_op_conf{};
  model_init_op_conf.set_name("System-ModelInit-ModelInit");
  ModelInitOpConf *model_init_conf =
      model_init_op_conf.mutable_model_init_conf();
  model_init_conf->set_tick(
      GenLogicalBlobName(foreign_input_op_conf.name(),
  ...
  for (const auto &pair : var_op_name2op_conf) {
    ...
    const auto &variable_op_parallel_blob_conf =
        var_op_name2parallel_blob_conf.at(var_op_name);
    const ParallelConf &variable_op_parallel_conf =
        variable_op_parallel_blob_conf.parallel_conf();
    CHECK_NE(variable_op_conf.variable_conf().data_type(),
    ...
    *model_init_conf->mutable_original_variable_conf()->Add() =
        variable_op_conf.variable_conf();
    *model_init_conf->mutable_out()->Add() = obn;
    const OperatorConf output_op_conf = GenOutputOpConf(
        var_op_name, GenLogicalBlobName(model_init_op_conf.name(), obn), "out",
...
void MakeModelLoadJob(
  ...
  const OperatorConf foreign_input_op_conf =
      GenForeignInputOpConf(job_name, 65536);
  job_builder.AddOps(master_parallel_conf,
                     {foreign_input_op_conf, tick_op_conf});
  ...
  ModelLoadOpConf *model_load_conf =
      model_load_op_conf.mutable_model_load_conf();
  ...
  for (const auto &pair : var_op_name2op_conf) {
    ...
    *model_load_conf->mutable_original_variable_conf()->Add() =
        variable_op_conf.variable_conf();
    *model_load_conf->mutable_out()->Add() = obn;
    const OperatorConf output_op_conf = GenOutputOpConf(
        var_op_name, GenLogicalBlobName(model_load_op_conf.name(), obn), "out",
        variable_op_parallel_blob_conf);
    job_builder.AddOps(variable_op_parallel_conf, {output_op_conf});
  }
...
void MakeModelSaveJob(
  ...
  job_builder.AddOps(master_parallel_conf,
                     {foreign_input_op_conf, tick_op_conf});
  if (var_op_name2parallel_blob_conf.empty()) {
    return;
  }
  OperatorConf model_save_op_conf{};
  model_save_op_conf.set_name("System-ModelSave-ModelSave");
  ModelSaveOpConf *model_save_conf =
      model_save_op_conf.mutable_model_save_conf();
  ...
  for (const auto &pair : var_op_name2parallel_blob_conf) {
    ...
    const OperatorConf input_op_conf =
        GenInputOpConf(var_op_name, "out", parallel_blob_conf);
    job_builder.AddOps(parallel_blob_conf.parallel_conf(), {input_op_conf});
    const std::string lbn = GenLogicalBlobName(
        input_op_conf.name(), input_op_conf.input_conf().out());
