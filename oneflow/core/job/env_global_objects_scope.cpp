...
Maybe<void> EnvGlobalObjectsScope::Init(const EnvProto& env_proto) {
  ...
  ep::CpuDeviceManager* cpu_devict_manager = dynamic_cast<ep::CpuDeviceManager*>(Global<ep::DeviceManagerRegistry>::Get()->GetDeviceManager(DeviceType::kCPU));
  ...
  cpu_devict_manager->SetDeviceNumThreads(num_threads);
