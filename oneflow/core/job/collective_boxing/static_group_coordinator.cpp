...
void StaticGroupCoordinator::AddRequest(void* coordinator_token) {
  ...
  LOG(INFO) << "Collective boxing StaticGroupCoordinator get request of job " << current_job_id_
            << " group index " << current_group_idx_in_job_;
...
#ifdef OF_DEBUG_LAZY_RUNTIME
      LOG(INFO) << "Collective boxing StaticGroupCoordinator execute kernel of job "
                << current_job_id_ << " group index " << current_group_idx_in_job_;
#endif  // OF_DEBUG_LAZY_RUNTIME
      group_state.Reset();
