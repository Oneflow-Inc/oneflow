...
int64_t JobDesc::all_reduce_group_min_byte() const {
  int64_t ret = job_.other().all_reduce_group_min_mbyte() * 1024 * 1024;
  CHECK_GT(ret, 0);
  return ret;
}

float JobDesc::all_reduce_group_size_warmup() const {
  float ret = job_.other().all_reduce_group_size_warmup();
  CHECK_GT(ret, 1);
  return ret;
}

int64_t JobDesc::all_reduce_group_num() const {
  int64_t ret = job_.other().all_reduce_group_num();
  CHECK_GT(ret, 0);
  return ret;
}

float JobDesc::all_reduce_lazy_ratio() const {
  float ratio = job_.other().all_reduce_lazy_ratio();
  CHECK_GE(ratio, 0.0);
  CHECK_LE(ratio, 1.0);
  return ratio;
}
...
int64_t JobDesc::piece_num_of_experiment_phase() const {
  return job_.other().exp_run_conf().piece_num_of_experiment_phase();
}

bool JobDesc::enable_experiment_run() const {
  return job_.other().exp_run_conf().enable_experiment_run();
}

size_t JobDesc::persistence_buf_byte() const {
  return job_.other().persistence_buf_mbyte() * 1024 * 1024;
}

size_t JobDesc::reserved_host_mem_byte() const {
  return job_.other().reserved_host_mem_mbyte() * 1024 * 1024;
}

size_t JobDesc::reserved_device_mem_byte() const {
  return job_.other().reserved_device_mem_mbyte() * 1024 * 1024;
}

bool JobDesc::save_downloaded_file_to_local_fs() const {
  return job_.other().save_downloaded_file_to_local_fs();
}

size_t JobDesc::rdma_mem_block_byte() const {
  return job_.other().rdma_mem_block_mbyte() * 1024 * 1024;
}

size_t JobDesc::rdma_recv_msg_buf_byte() const {
  return job_.other().rdma_recv_msg_buf_mbyte() * 1024 * 1024;
}

const std::string& JobDesc::MdSaveSnapshotsPath() const {
  return job_.other().train_conf().model_save_snapshots_path();
}

const TrainConf& GetTrainConf(const Job& job) {
  if (job.other().has_predict_conf()
      && job.other().predict_conf().has_tmp_split_fw_bw_train_conf()) {
    return job.other().predict_conf().tmp_split_fw_bw_train_conf();
  }
  CHECK(job.other().has_train_conf());
  return job.other().train_conf();
}

int32_t JobDesc::NumOfBatchesInSnapshot() const {
  return GetTrainConf(job_).num_of_batches_in_snapshot();
}
int64_t JobDesc::TotalBatchNum() const { return job_.other().total_batch_num(); }
...
int32_t JobDesc::PieceNumOfPrintLoss() const {
  return job_.other().train_conf().piece_num_of_print_loss();
}
int32_t JobDesc::PieceNumOfPrintAccuracy() const {
  return job_.other().train_conf().piece_num_of_print_accuracy();
}
...
int32_t JobDesc::DataPartNum() const { return job_.other().data_part_num(); }

const FileSystemConf& JobDesc::data_fs_conf() const { return job_.other().data_fs_conf(); }
const FileSystemConf& JobDesc::snapshot_fs_conf() const { return job_.other().snapshot_fs_conf(); }
...
JobDesc::JobDesc(const JobConf& job_conf) { Init(job_conf); }

JobDesc::JobDesc(const Job& job) { Init(job); }

void JobDesc::Init(const JobConf& job_conf) {
  *job_.mutable_net() = job_conf.net();
  *job_.mutable_resource() = job_conf.resource();
  *job_.mutable_placement() = job_conf.placement();
  *job_.mutable_other() = job_conf.other();
  Init();
}

void JobDesc::Init(const Job& job) {
  job_ = job;
  Init();
}
...
void JobDesc::Init() {
  ...
  AddRecordLoadOps();
#ifndef WITH_RDMA
  CHECK_EQ(job_.other().use_rdma(), false) << "Please compile ONEFLOW with RDMA";
#endif
#ifndef WITH_CUDA
  CHECK_EQ(job_.other().enable_nccl(), false) << "Please compile ONEFLOW with NCCL";
#endif  // WITH_CUDA
  int64_t piece_exp = job_.other().exp_run_conf().piece_num_of_experiment_phase();
  if (job_.other().has_train_conf()) {
    TrainConf* train_conf = job_.mutable_other()->mutable_train_conf();
    ...
    piece_exp = std::min(piece_exp, job_.other().total_batch_num() * NumOfPiecesInBatch());
  } else {
    if (piece_exp == -1) { piece_exp = 19; }
  }
  LOG(INFO) << "Set piece_num_of_experiment_phase " << piece_exp;
  job_.mutable_other()->mutable_exp_run_conf()->set_piece_num_of_experiment_phase(piece_exp);
#ifndef WITH_CUDA
  CHECK_EQ(job_.resource().gpu_device_num(), 0);
#endif
}

void JobDesc::SanityCheck() {
  int64_t machine_num = job_.resource().machine_size();
  FOR_RANGE(int64_t, i, 0, machine_num) { CHECK_EQ(job_.resource().machine(i).id(), i); }
}

int64_t JobDesc::GetMachineId(const std::string& addr) const {
  int64_t machine_id = -1;
  auto resource_conf = job_.resource();
...
void JobDesc::SplitDecodeOps() {
  ...
  for (OperatorConf& op_conf : *(job_.mutable_net()->mutable_op())) {
  ...
  for (OperatorConf& gen_op_conf : gen_op_confs) { *(job_.mutable_net()->add_op()) = gen_op_conf; }
}
...
void JobDesc::AddRecordLoadOps() {
  ...
  size_t op_num = job_.net().op_size();
  ...
  for (const PlacementGroup& p_group : job_.placement().placement_group()) {
  ...
  for (const auto& pair : data_info2decode_ops) {
    ...
    for (const ParallelConf* parallel_conf : parallel_confs) {
      ...
      OperatorConf* op = job_.mutable_net()->add_op();
      ...
      PlacementGroup* p_group = job_.mutable_placement()->add_placement_group();
...
void JobDesc::ConvertPseudoChainToChain() {
  ...
  auto MutOperatorConf4OpName = MakeMutableOperatorConf4OpName(&job_);
  auto ParallelConf4OpName = MakeGetterParallelConf4OpName(job_.placement());
  OpGraph(this).ForEachPseudoChain([&](const HashSet<OpNode*>& chain_nodes) {
    ...
    for (const auto& pair : has_diff2source_edges) {
      ...
      if (src_nodes.size() > 1 && dst_nodes.size() > 1) {
        AddIdentityOpAndReconnect("pseudo_chain_header_", &job_, pair.second,
  ...
  OpGraph(this).TopoForEachNode([&](OpNode* op_node) {
    ...
    for (const auto& pair : consumer_op_pr2edges) {
      AddIdentityOpAndReconnect(
          "all_reduce_overlapping_untrainable_", &job_, pair.second, MutOperatorConf4OpName,
...
void JobDesc::FixTickOpIfExists() {
  auto MutParallelConf4OpName = MakeGetterMutParallelConf4OpName(job_.mutable_placement());
  OperatorConf* tick_op_conf = nullptr;
  FOR_RANGE(int, idx, 0, job_.mutable_net()->op_size()) {
    OperatorConf* op_conf = job_.mutable_net()->mutable_op(idx);
  ...
  *MutParallelConf4OpName(tick_op_conf->name()) = *source_parallel_conf;
  ...
  OperatorConf* tick_log_counter = job_.mutable_net()->add_op();
