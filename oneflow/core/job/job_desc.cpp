...
void CheckFunctionConfig(const JobConfigProto &job_conf) {
  const auto &flag_name2flag_def =
      GlobalFunctionConfigDef().flag_name2flag_def();
...
void JobDesc::Init() {
  ...
  if (job_conf_.has_train_conf()) {
    if (piece_exp == -1) {
      piece_exp = 19 * NumOfPiecesInBatch();
    }
    piece_exp = std::max(piece_exp, NumOfPiecesInBatch());
    piece_exp =
        std::min(piece_exp, job_conf_.total_batch_num() * NumOfPiecesInBatch());
  } else {
    if (piece_exp == -1) {
      piece_exp = 19;
    }
  ...
  job_conf_.mutable_exp_run_conf()->set_piece_num_of_experiment_phase(
      piece_exp);
#ifndef WITH_CUDA
...
const UserOpAttrVal &
JobDesc::GetFunctionFlagVal(const std::string &field_name) const {
  const auto &iter = job_conf_.flag_name2flag_value().find(field_name);
  if (iter != job_conf_.flag_name2flag_value().end()) {
    return iter->second;
  }
  const auto &flag_name2flag_def =
      GlobalFunctionConfigDef().flag_name2flag_def();
...
bool IsInterfaceOpConf(const OperatorConf &op_conf) {
  return IsClassRegistered<IsInterfaceOpConf4OpTypeCase>(
      op_conf.op_type_case());
}

GlobalJobDescScope::GlobalJobDescScope(const JobConfigProto &job_conf,
                                       int64_t job_id) {
...
bool IsPullJob(const std::string &job_name,
               const InterUserJobInfo &inter_user_job_info) {
  for (const auto &pair :
       inter_user_job_info.output_or_var_op_name2pull_job_name()) {
    if (pair.second == job_name) {
      return true;
    }
  }
  return false;
...
bool IsPushJob(const std::string &job_name,
               const InterUserJobInfo &inter_user_job_info) {
  for (const auto &pair :
       inter_user_job_info.input_or_var_op_name2push_job_name()) {
    if (pair.second == job_name) {
      return true;
    }
  }
  if (job_name == inter_user_job_info.global_model_init_job_name()) {
    return true;
  }
  if (job_name == inter_user_job_info.global_model_load_job_name()) {
    return true;
  }
  if (job_name == inter_user_job_info.global_model_save_job_name()) {
    return true;
  }
  return false;
}
