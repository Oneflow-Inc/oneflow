...
std::string GetNcclUniqueIdRpcKey(
    const std::vector<std::pair<int64_t, int64_t>> &sorted_devices) {
  std::ostringstream oss;
  oss << "eager_nccl_unique_id_rpc_key";
  for (const auto &pair : sorted_devices) {
...
ncclComm_t EagerNcclCommMgr::GetCommForDevice(
  ...
  OF_CUDA_CHECK(cudaGetDevice(&dev));
  {
    ...
    if (it != device_set2device_id2comm_.end()) {
      return it->second.at(dev);
    }
  }
  std::vector<std::pair<int64_t, int64_t>> device_vec(device_set.cbegin(),
                                                      device_set.cend());
  ...
  int64_t machine = Global<MachineCtx>::Get()->this_machine_id();
  ...
  ncclComm_t comm;
  OF_NCCL_CHECK(
      ncclCommInitRank(&comm, device_vec.size(), nccl_unique_id, rank));
  {
    std::lock_guard<std::mutex> lock(mutex_);
