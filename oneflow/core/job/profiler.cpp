...
void Profiler::Profile(const Plan &plan,
  ...
  for (const auto &act_event : act_events) {
    ...
    ActTimeInfo act_time_info({act_event->ready_time(), act_event->start_time(),
                               act_event->stop_time()});
  ...
  for (auto &pair : actor_id2act_time_info) {
    ...
    profile_info_pair.second.set_avg_act_interval(
        act_num > 1 ? acc_act_interval / (act_num - 1) : 0);
