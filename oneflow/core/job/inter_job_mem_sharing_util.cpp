...
void GetOpName2JobId2TaskProtos(
    ...
    HashMap<std::string, HashMap<int64_t, std::vector<TaskProto *>>>
        *op_name2job_id2task_protos) {
...
HashMap<std::string, HashSet<int64_t>>
GetInterfaceOpName2JobIds(const std::vector<std::shared_ptr<Job>> &jobs) {
  ...
  FOR_RANGE(int64_t, i, 0, jobs.size()) {
    ...
    for (const auto &op : job.net().op()) {
      if (IsInterfaceOpConf(op)) {
        ...
        CHECK(unique_op_name_check.find(op.name()) ==
              unique_op_name_check.end());
      }
    }
  }
...
std::vector<HashSet<int64_t>>
InitJobId2MutualExclusionJobIds(const std::vector<std::shared_ptr<Job>> &jobs) {
  ...
  const InterJobReuseMemStrategy *strategy =
      Global<const InterJobReuseMemStrategy>::Get();
  if (strategy->has_custom_parallelism()) {
    auto *job_name2job_id = Global<JobName2JobId>::Get();
    for (const auto &group :
         strategy->custom_parallelism().nonparallel_group()) {
...
std::vector<HashSet<int64_t>>
GetMutualExclusionJobGroups(const std::vector<std::shared_ptr<Job>> &jobs) {
  int64_t job_size = jobs.size();
  ...
  std::vector<HashSet<int64_t>> job_id2mutual_exclusion_ids =
      InitJobId2MutualExclusionJobIds(jobs);
  ...
  FOR_RANGE(int64_t, i, 0, job_size) {
    FOR_RANGE(int64_t, j, 0, job_size) {
      if (job_id2mutual_exclusion_ids[i].find(j) ==
          job_id2mutual_exclusion_ids[i].end()) {
  ...
  FOR_RANGE(int64_t, this_job_id, 0, job_size) {
    ...
    for (int64_t enable_parallel_job_id :
         job_id2enable_parallel_ids[this_job_id]) {
      int64_t group_id = job_id2mem_share_group_id[enable_parallel_job_id];
      if (group_id != -1) {
        mem_share_group_id_used.emplace(group_id);
      }
    }
    FOR_RANGE(int64_t, this_group_id, 0, mem_share_group_num) {
      if (mem_share_group_id_used.find(this_group_id) ==
          mem_share_group_id_used.end()) {
  ...
  }
  {
    ...
    for (auto &job_group : job_groups) {
      for (int64_t job_id : job_group) {
        CHECK(job_id_unique_check.emplace(job_id).second);
      }
    }
  }
  return job_groups;
...
void MergeReusedChunk(
  ...
  for (const auto &pair : *chunk_id2chunk) {
    ...
    int64_t mzuid =
        MemoryCaseUtil::GenMemZoneUniqueId(chunk.machine_id(), mem_case);
    CHECK_EQ(chunk.job_id_size(), 1);
    CHECK(job_id2mzuid2chunk_id[chunk.job_id(0)]
              .emplace(mzuid, chunk.chunk_id())
              .second);
  }
  ...
  for (const HashSet<int64_t> &job_group : reuse_mem_job_groups) {
    ...
    HashMap<int64_t, HashSet<int64_t>> mzuid2job_ids =
        InitMzuid2JobIdsInJobGroup(job_group);
...
void MergeSharedMemBlockR2L(
  ...
  auto CheckValidAndGetMemBlock = [&](int64_t mem_block_id, int64_t mem_size,
    ...
    CHECK(mem_block_id2mem_block->find(mem_block_id) !=
  ...
  auto MergeAndEraseMemBlock = [&](MemBlockProto *merged_block,
                                   MemBlockProto *erased_block) {
  ...
  MemBlockProto *merged_mem_block = CheckValidAndGetMemBlock(
      merged_mem_block_id, left_rt_regst.TotalMainByteSize4AllRegst(),
      lhs->mem_case());
  MemBlockProto *erased_mem_block = CheckValidAndGetMemBlock(
      erased_mem_block_id, right_rt_regst.TotalMainByteSize4AllRegst(),
      rhs->mem_case());
  ...
  int64_t separated_header_mem_size =
      left_rt_regst.TotalSeparatedHeaderByteSize4AllRegst();
  if (separated_header_mem_size > 0) {
    CHECK_EQ(separated_header_mem_size,
             right_rt_regst.TotalSeparatedHeaderByteSize4AllRegst());
    ...
    MemoryCase header_mem_case =
        MemoryCaseUtil::GetHostPinnedMemoryCaseForRegstSeparatedHeader(
            lhs->mem_case());
    MemBlockProto *merged_header_block = CheckValidAndGetMemBlock(
        merged_header_id, separated_header_mem_size, header_mem_case);
    MemBlockProto *erased_header_block = CheckValidAndGetMemBlock(
        erased_header_id, separated_header_mem_size, header_mem_case);
    MergeAndEraseMemBlock(merged_header_block, erased_header_block);
    rhs->set_separated_header_mem_block_id(merged_header_id);
  }
}

void MergeSharedInterfaceMemBlock(
  ...
  for (const auto &pair : interface_op_name2job_ids) {
    interface_op_names.insert(pair.first);
  }
  HashMap<std::string, HashMap<int64_t, std::vector<TaskProto *>>>
      op_name2job_id2task_protos;
  ...
  for (const auto &op_job_pair : interface_op_name2job_ids) {
    if (op_job_pair.second.size() <= 1) {
      continue;
    }
    const HashMap<int64_t, std::vector<TaskProto *>>
        &job_id2same_op_name_sorted_task_protos =
            op_name2job_id2task_protos.at(op_job_pair.first);
    const auto &first_vec =
        job_id2same_op_name_sorted_task_protos.begin()->second;
    std::vector<MemoryCase> common_mem_case_vec(first_vec.size());
    std::transform(first_vec.cbegin(), first_vec.cend(),
                   common_mem_case_vec.begin(), [](TaskProto *tp) {
                     return PlanUtil::GetSoleProducedDataRegst(tp)->mem_case();
                   });
    for (const auto &pair : job_id2same_op_name_sorted_task_protos) {
      ...
      FOR_RANGE(int64_t, i, 0, first_vec.size()) {
        CHECK_EQ(task_protos.at(i)->machine_id(),
                 first_vec.at(i)->machine_id());
        RegstDescProto *first_regst_desc =
            PlanUtil::GetSoleProducedDataRegst(first_vec.at(i));
        RegstDescProto *regst_desc =
            PlanUtil::GetSoleProducedDataRegst(task_protos.at(i));

        MergeSharedMemBlockR2L(first_regst_desc, regst_desc,
                               mem_block_id2mem_block);

        MemoryCase common_mem_case;
        CHECK(MemoryCaseUtil::GetCommonMemoryCase(common_mem_case_vec.at(i),
                                                  regst_desc->mem_case(),
                                                  &common_mem_case));
      ...
      FOR_RANGE(int64_t, i, 0, task_protos.size()) {
        ...
        *(regst_desc->mutable_mem_case()) = common_mem_case_vec.at(i);
        CHECK(mem_block_id2mem_block->find(regst_desc->mem_block_id()) !=
              mem_block_id2mem_block->end());
        *(mem_block_id2mem_block->at(regst_desc->mem_block_id())
              .mutable_mem_case()) = common_mem_case_vec.at(i);
      }
    }
  }
}
...
void InterJobMemSharingUtil::MergeMemReusedChunkBetweenUserJobs(
  ...
  std::vector<HashSet<int64_t>> reuse_mem_job_groups =
      GetMutualExclusionJobGroups(user_jobs);
  ...
  for (MemBlockProto &mem_block :
       *plan->mutable_block_chunk_list()->mutable_mem_block()) {
    CHECK(mem_block_id2mem_block.emplace(mem_block.mem_block_id(), &mem_block)
              .second);
  }

  MergeReusedChunk(&chunk_id2chunk, &mem_block_id2mem_block,
