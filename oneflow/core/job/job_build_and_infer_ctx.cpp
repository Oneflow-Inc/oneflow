...
void ConvertProtobufMsg2Json(nlohmann::json& json_value, const PbMessage& pb_msg);

void ConvertRepeatedField2Json(nlohmann::json& json_value, const PbMessage& pb_msg,
                               const PbFd* pb_field,
...
void oneflow::JobBuildAndInferCtx::GetJobStructureGraphJson(nlohmann::json& json_pair) const {
  ...
  for (const auto& pair : op_name2op_) {
    ...
    ConvertProtobufMsg2Json(json_conf, op->op_conf());
