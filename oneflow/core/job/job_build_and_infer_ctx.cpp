...
void ResetOpConfIbn(OperatorConf *op_conf, const std::string &ibn,
                    const std::string lbn) {
  PbMessage *op_type_conf =
      MutableMessageInPbMessage(op_conf, op_conf->op_type_case());
  std::string lbn_may_with_hint =
      GetInputLbnInOpCustomizedConf(*op_type_conf, ibn);
  ...
  if (user_conf) {
    for (const auto &pair : user_conf->output()) {
      for (const std::string &old_lbn : pair.second.s()) {
        ...
        std::string new_lbn =
            GenLogicalBlobName(new_op_name, old_lbi.blob_name());
        (*(user_conf->mutable_output()))[pair.first].set_s(
            blob_name_id_pair.second, new_lbn);
      }
    }
  }
}
...
Maybe<void> EagerRunOps(const Job &job, HashSet<std::string> *op_names,
                        void (ForeignCallback::*interpret)(const std::string &,
                                                           const std::string &)
                            const) {
  ...
  JUST(op_graph->ForEachOpNode([&](const OpNode &op_node) -> Maybe<void> {
    if (!op_names->insert(op_node.op().op_name()).second) {
      return Maybe<void>::Ok();
    }
...
Maybe<void> JobBuildAndInferCtx::AddOpNameParallelConf2Placement(
  ...
  if (parallel_desc2placement_group_.find(parallel_desc) ==
      parallel_desc2placement_group_.end()) {
  ...
  for (const auto &obn : op->output_bns()) {
    ...
    if (iter == parallel_desc2blob_placement_group_.end()) {
      ...
      *blob_pg->mutable_parallel_conf() = parallel_desc.parallel_conf();
      iter = parallel_desc2blob_placement_group_.emplace(parallel_desc, blob_pg)
                 .first;
    }
    const auto &lbi = op->BnInOp2Lbi(obn);
    CHECK_OR_RETURN(std::find(iter->second->lbi().begin(),
                              iter->second->lbi().end(),
                              lbi) == iter->second->lbi().end());
...
Maybe<OperatorConf> JobBuildAndInferCtx::DecodeLbiHintAndReturnNewOpConf(
  ...
  auto op_conf_without_split_hint =
      std::make_shared<OperatorConf>(op.op_conf());
  PbMessage *op_type_conf =
      MutableMessageInPbMessage(op_conf_without_split_hint.get(),
  ...
  for (const std::string &ibn : op.input_bns()) {
    ...
    bool has_sbp_hint =
        JUST(GetSbpParallelInLbnOrNothing(lbn_may_with_hint, &sbp_parallel));
    bool has_disable_boxing_hint = JUST(
        ParseDisableBoxingFlag(lbn_may_with_hint, &(*ibn2disable_boxing)[ibn]));
    if (has_sbp_hint || has_disable_boxing_hint) {
      ...
      ReplaceInputLbnInOpCustomizedConf(op_type_conf, ibn, lbn_may_with_hint,
                                        lbn);
    }
  }
...
void JobBuildAndInferCtx::AddOpAndUpdateJobParallelViewConf(
    const OperatorConf &operator_conf, const SbpSignature &sbp_signature,
    bool is_mirrored_parallel_view) const {
  auto *op_name2sbp_sig = job_->mutable_job_parallel_view_conf()
  ...
  auto *op_name2is_mirrored_parallel_view =
      job_->mutable_job_parallel_view_conf()
          ->mutable_op_name2is_mirrored_parallel_view();
...
Maybe<void>
JobBuildAndInferCtx::InferMirroredSignature(Operator *op,
                                            bool is_mirrored_parallel_view_conf,
                                            const ParallelDesc &parallel_desc) {
  HashMap<std::string, MirroredSigInferHint> ibn2mirrored_sig_infer_hint;
  for (const std::string &ibn : op->input_bns()) {
    const LogicalBlobId &lbi = op->BnInOp2Lbi(ibn);
    CHECK_OR_RETURN(lbi2logical_blob_desc_.find(lbi) !=
                    lbi2logical_blob_desc_.end())
  ...
  JUST(op->InferMirroredSignatureIf(
      MirroredSigInferHint4Ibn, is_mirrored_parallel_view_conf, parallel_desc));
  return Maybe<void>::Ok();
}
...
JobBuildAndInferCtx::InferOpOutSbpParallel(Operator *op,
                                           ...
                                           const ParallelDesc &parallel_desc) {
  const auto &BatchAxis4BnInOp =
      [&](const std::string &bn_in_op) -> Maybe<const OptInt64 *> {
  ...
  for (const std::string &ibn : op->input_bns()) {
    ...
    CHECK_OR_RETURN(lbi2sbp_parallel_from_producer_view_.find(lbi) !=
        ...
        << Error::LogicalBlobNameNotExistError()
        << "when infer op_name: " << op->op_name()
        << " consumed op_name: " << lbi.op_name()
        << " blob_name: " << lbi.blob_name() << " not infer split axis";
    const SbpParallel *sbp_parallel =
        &lbi2sbp_parallel_from_producer_view_.at(lbi);
    const OptInt64 *batch_axis = JUST(BatchAxis4BnInOp(ibn));
    ibn2sbp_infer_hint.emplace(
        ibn, SbpInferHint(pd, logical_blob_desc, sbp_parallel, batch_axis));
  }

  JUST(InferOpSbpSignature(op, sbp_sig_conf, parallel_desc, ibn2sbp_infer_hint,
                           BatchAxis4BnInOp));

  const auto &bn2sbp_parallel =
      JUST(op->sbp_signature())->bn_in_op2sbp_parallel();
  for (const auto &obn : op->output_bns()) {
    ...
    CHECK_OR_RETURN(bn2sbp_parallel.find(obn) != bn2sbp_parallel.end())
        ...
        << " blob_name: " << lbi.blob_name() << " not infer split axis";
    CHECK_OR_RETURN(lbi2sbp_parallel_from_producer_view_
                        .emplace(lbi, bn2sbp_parallel.at(obn))
                        .second)
        << Error::BlobSplitAxisInferError() << "op_name: " << lbi.op_name()
        << " blob_name: " << lbi.blob_name() << " infer split axis repeated";
    CHECK_OR_RETURN(
        lbi2parallel_desc_from_producer_view_.emplace(lbi, parallel_desc)
            .second)
        << Error::BlobSplitAxisInferError() << "op_name: " << lbi.op_name()
...
Maybe<void>
JobBuildAndInferCtx::GenOpProducedEmptyLogicalBlobDesc(Operator *op) {
  // check consumed blob
  ...
  produced_bns.insert(produced_bns.end(), op->output_bns().begin(),
                      op->output_bns().end());
  produced_bns.insert(produced_bns.end(), op->tmp_bns().begin(),
                      op->tmp_bns().end());
  produced_bns.insert(produced_bns.end(), op->const_buf_bns().begin(),
                      op->const_buf_bns().end());
  for (const std::string &produced_bn : produced_bns) {
    ...
    lbi2logical_blob_desc_.emplace(
        lbi, std::make_unique<BlobDesc>(DataType::kInvalidDataType));
  }
  return Maybe<void>::Ok();
}

Maybe<void> JobBuildAndInferCtx::CheckOpBlobSplitability(Operator *op,
                                                         int64_t parallel_num) {
  HashSet<std::string> obns(op->output_bns().begin(), op->output_bns().end());
  auto GetParallelNum = [&](const std::string &bn_in_op) {
    if (obns.find(bn_in_op) == obns.end()) {
      return parallel_num;
    }
    return lbi2parallel_desc_from_producer_view_.at(op->BnInOp2Lbi(bn_in_op))
        .parallel_num();
  };
  for (const auto &pair : JUST(op->sbp_signature())->bn_in_op2sbp_parallel()) {
    ...
    if (JUST(op->OptMirroredParallel4BnInOp(pair.first))
            ->has_mirrored_parallel()) {
      continue;
    }
    ...
    CHECK_LT_OR_RETURN(axis, num_axes)
        << "op: " << op->op_name() << ", blob: " << pair.first
        << ", axis: " << axis << ", shape: " << logical_blob_desc.shape();
    CHECK_GE_OR_RETURN(logical_blob_desc.shape().At(axis), blob_parallel_num)
        ...
        << std::to_string(blob_parallel_num);
  }
  return Maybe<void>::Ok();
}
...
Maybe<ParallelConf> JobBuildAndInferCtx::InferOpParallelConf(
  ...
  for (const auto &ibn : op.input_bns()) {
    if (ibn2disable_boxing.at(ibn) == false) {
      continue;
    }
    const auto &lbi = op.BnInOp2Lbi(ibn);
    const auto &ibn_parallel_desc =
        lbi2parallel_desc_from_producer_view_.at(lbi);
    if (parallel_desc == nullptr) {
      parallel_desc = &ibn_parallel_desc;
    } else {
      CHECK_EQ_OR_RETURN(parallel_desc->parallel_num(),
                         ibn_parallel_desc.parallel_num());
    }
  }
  if (parallel_desc == nullptr) {
    return std::make_shared<ParallelConf>(origin_parallel_conf);
  }
  return std::make_shared<ParallelConf>(parallel_desc->parallel_conf());
}

void JobBuildAndInferCtx::InitIbn2DisableBoxing(
    const Operator &op, HashMap<std::string, bool> *ibn2disable_boxing) {
  for (const auto &ibn : op.input_bns()) {
...
bool JobBuildAndInferCtx::HasAnyMirroredBlobInput(const Operator &op) const {
  for (const auto &ibn : op.input_bns()) {
    const auto &lbi = op.BnInOp2Lbi(ibn);
    if (mirrored_lbi2sub_lbis_.find(lbi) != mirrored_lbi2sub_lbis_.end()) {
      return true;
    }
  }
  return false;
...
Maybe<const SbpParallel *>
JobBuildAndInferCtx::SbpParallel4Lbi(const LogicalBlobId &lbi) const {
...
JobBuildAndInferCtx::ParallelDesc4Lbi(const LogicalBlobId &lbi) const {
...
Maybe<bool>
JobBuildAndInferCtx::AllInputsBroadcastParallel(const Operator &op) const {
  for (const auto &ibn : op.input_bns()) {
    const LogicalBlobId &lbi = op.BnInOp2Lbi(ibn);
    const auto &iter = mirrored_lbi2sbp_parallel_.find(lbi);
    if (iter != mirrored_lbi2sbp_parallel_.end()) {
      if (!iter->second.has_broadcast_parallel()) {
        return false;
      }
    } else {
      if (!JUST(SbpParallel4Lbi(lbi))->has_broadcast_parallel()) {
        return false;
      }
    }
  }
...
Maybe<void> JobBuildAndInferCtx::CheckAllInputsConvertableToMirroredBlob(
  ...
  for (const auto &ibn : op.input_bns()) {
    ...
    if (sbp.has_split_parallel() && sbp.split_parallel().axis() == 0) {
      continue;
    }
    const std::string &lbn = GenLogicalBlobName(lbi);
    return Error::CheckFailedError() << "input lbn: " << lbn
                                     << " is not convertable to mirrored blob";
  }
  return Maybe<void>::Ok();
}

Maybe<void> LazyJobBuildAndInferCtx::CheckAllInputsWithSameParallelNum(
...
Maybe<OpAttribute>
JobBuildAndInferCtx::AddAndInferMirroredOp(const OperatorConf &op_conf) {
  CHECK_OR_RETURN(op_conf.has_scope_symbol_id());
  const auto &scope =
      Global<vm::SymbolStorage<Scope>>::Get()->Get(op_conf.scope_symbol_id());
  ...
  auto GetSubOpName = [&](int index) {
    return GetMirroredOpName(op_conf.name(), index);
  };
  ...
  FOR_RANGE(int32_t, i, 0, sub_op_list_size) {
    ...
    const ParallelConf &parallel_conf =
        GetMirroredOpParallelConf(parallel_desc, i);
    bool is_mirrored_parallel_view = GetIsMirroredParallelView();
    last_op_attribute = JUST(AddAndInferOp(sub_op_conf, parallel_conf, job_desc,
  ...
  for (const auto &obn : op->output_bns()) {
    ...
    FOR_RANGE(int32_t, i, 0, sub_op_list_size) {
      sub_lbis->at(i).set_op_name(GetSubOpName(i));
    }
...
Maybe<const LogicalBlobId *>
JobBuildAndInferCtx::GetSubLbi(const LogicalBlobId &lbi, int32_t index) {
  auto lbi_vec_iter = mirrored_lbi2sub_lbis_.find(lbi);
  if (lbi_vec_iter == mirrored_lbi2sub_lbis_.end()) {
    const auto &new_lbi =
        JUST(FindOrCreateMirroredLbiFromCompatibleConsistentBlob(lbi));
...
Maybe<OpAttribute>
JobBuildAndInferCtx::AddAndInferConsistentOp(const OperatorConf &op_conf) {
  CHECK_OR_RETURN(op_conf.has_scope_symbol_id());
  const auto &scope =
      Global<vm::SymbolStorage<Scope>>::Get()->Get(op_conf.scope_symbol_id());
  const auto &parallel_desc = *JUST(scope.GetParallelDesc(op_conf));
...
Maybe<OpAttribute> JobBuildAndInferCtx::AddAndInferOp(
  ...
  CHECK_NE_OR_RETURN(op_conf.device_tag(), "invalid_device")
      << Error::OpConfDeviceTagNoSetError() << "op_name: " << op_name
      << " not set device tag";
  ...
  auto new_op_conf = JUST(
      DecodeLbiHintAndReturnNewOpConf(*op, &sbp_sig_conf, &ibn2disable_boxing));
  AddOpAndUpdateJobParallelViewConf(*new_op_conf, sbp_sig_conf,
                                    is_mirrored_parallel_view);
  auto parallel_conf =
      JUST(InferOpParallelConf(*op, origin_parallel_conf, ibn2disable_boxing));
  JUST(AddOpNameParallelConf2Placement(op_name, *parallel_conf));
  UpdateLbi2DisableBoxing(*op, ibn2disable_boxing);
  // infer batch_axis
  const auto &BatchAxis4Ibn =
      [&](const std::string &ibn) -> Maybe<const OptInt64 *> {
    const LogicalBlobId &lbi = op->BnInOp2Lbi(ibn);
    const auto &op = *JUST(Op4OpName(lbi.op_name()));
    return op.BatchAxis4BnInOp(*JUST(op.obn4lbi(lbi)));
  };
  const auto &GetConstBlobDescBnInOp =
  ...
  JUST(op->InferLogicalOutBlobDescsIf(GetBlobDesc4BnInOp, BatchAxis4Ibn,
                                      parallel_desc));
  // Fill logical blob_desc signature.
  JUST(op->FillLogicalBlobDescSignature(
  ...
  auto ParallelDesc4Obn = [&](const std::string &obn) -> ParallelDesc * {
    ...
    if (iter == lbi2parallel_desc_from_producer_view_.end()) {
      iter = lbi2parallel_desc_from_producer_view_.emplace(lbi, parallel_desc)
                 .first;
    }
    return &iter->second;
  };
  JUST(op->InferOutParallelDescIf(ParallelDesc4Obn, GetBlobDesc4BnInOp,
                                  parallel_desc, JUST(op->sbp_signature())));
  // TODO(lixinqi): replace lbi2parallel_desc_from_producer_view_  with
  // ParallelSignature
...
Maybe<void>
JobBuildAndInferCtx::AddLossLogicalBlobName(const std::string &lbn) {
  if (IsMirroredBlob(lbn)) {
    return AddLossMirroredBlobName(lbn);
  }
  return AddLossConsistentBlobName(lbn);
}

Maybe<void>
JobBuildAndInferCtx::AddLossConsistentBlobName(const std::string &lbn) {
...
Maybe<OptInt64>
JobBuildAndInferCtx::GetBatchAxis(const std::string &lbn) const {
  JUST(CheckLbnValidAndExist(lbn));
...
Maybe<Operator *>
JobBuildAndInferCtx::Op4OpName(const std::string &op_name) const {
...
Maybe<OptInt64> JobBuildAndInferCtx::GetSplitAxisFromProducerView(
    const std::string &lbn) const {
  JUST(CheckLbnValidAndExist(lbn));
  OptInt64 ret;
  const auto &sbp =
      lbi2sbp_parallel_from_producer_view_.at(GenLogicalBlobId(lbn));
  if (sbp.has_split_parallel()) {
    ret.set_value(sbp.split_parallel().axis());
  }
  return ret;
}

Maybe<const ParallelDesc *>
JobBuildAndInferCtx::GetParallelDescFromProducerView(
    const std::string &lbn) const {
  JUST(CheckLbnValidAndExist(lbn));
...
Maybe<void>
JobBuildAndInferCtx::AddLossMirroredBlobName(const std::string &lbn) {
  const auto &mirrored_lbi = JUST(GetMirroredLbi(lbn));
  CHECK_OR_RETURN(job_->job_conf().has_train_conf())
      << Error::UnknownJobBuildAndInferError()
      << "job has no TrainConf when adding loss logical blob name";
  for (const auto &lbi : mirrored_lbi2sub_lbis_.at(*mirrored_lbi)) {
    job_->mutable_job_conf()->mutable_train_conf()->add_loss_lbn(
        GenLogicalBlobName(lbi));
  }
  return Maybe<void>::Ok();
}

Maybe<LogicalBlobId>
JobBuildAndInferCtx::GetMirroredLbi(const std::string &lbn_with_hint) const {
  const LogicalBlobId &lbi = GenLogicalBlobId(lbn_with_hint);
  if (mirrored_lbi2sub_lbis_.find(lbi) != mirrored_lbi2sub_lbis_.end()) {
    return lbi;
  }
  return Error::CheckFailedError() << lbn_with_hint
                                   << " is not a mirrored blob name";
}

Maybe<int> JobBuildAndInferCtx::MirroredBlobGetNumSubLbi(
...
Maybe<const LogicalBlobId *>
JobBuildAndInferCtx::MirroredBlobGetSubLbi(const std::string &lbn_with_hint,
                                           int index) const {
  const auto &mirrored_lbi = JUST(GetMirroredLbi(lbn_with_hint));
...
bool JobBuildAndInferCtx::IsMirroredBlob(const std::string &lbn) const {
  ...
  if (is_mirrored_blob) {
    return is_mirrored_blob;
  }
  const LogicalBlobId &lbi = GenLogicalBlobId(lbn);
  CHECK(lbi2logical_blob_desc_.find(lbi) != lbi2logical_blob_desc_.end())
      << "lbn: " << lbn;
  return false;
}
...
Maybe<bool> JobBuildAndInferCtx::MirroredBlobIsDynamic(
    const std::string &lbn_with_hint) const {
  const auto &lbi = *JUST(MirroredBlobGetSubLbi(lbn_with_hint, 0));
  return lbi2logical_blob_desc_.at(lbi)->is_dynamic();
}

Maybe<bool> JobBuildAndInferCtx::MirroredBlobIsTensorList(
    const std::string &lbn_with_hint) const {
  const auto &lbi = *JUST(MirroredBlobGetSubLbi(lbn_with_hint, 0));
...
Maybe<const ParallelDesc *>
JobBuildAndInferCtx::MirroredBlobGetParallelDescFromProducerView(
    const std::string &lbn_with_hint) const {
...
Maybe<void> JobBuildAndInferCtx::CheckPlacement() const {
  ...
  for (const PlacementGroup &placement_group :
       job_->placement().placement_group()) {
  ...
  for (const std::string &op_name : op_names_in_net) {
    CHECK_OR_RETURN(op_names_in_placement.find(op_name) !=
                    op_names_in_placement.end())
        << Error::PlacementError() << "job: " << job_->job_conf().job_name()
        << " op_name: " << op_name
...
Maybe<void> JobBuildAndInferCtx::CheckJobConf() const {
  if (job_->job_conf().job_type_case() == JobConfigProto::JOB_TYPE_NOT_SET) {
    return Error::JobTypeNotSetError()
           << "job_type not set, please set predict_conf or train_conf";
  }
  return Maybe<void>::Ok();
}

Maybe<void>
JobBuildAndInferCtx::CheckLbnValidAndExist(const std::string &lbn) const {
...
std::string
LazyJobBuildAndInferCtx::GetMirroredOpName(const std::string &op_name,
                                           int64_t parallel_id) const {
  return op_name + "_" + std::to_string(parallel_id);
}

std::string
EagerJobBuildAndInferCtx::GetMirroredOpName(const std::string &op_name,
                                            int64_t parallel_id) const {
  return op_name;
}

ParallelConf LazyJobBuildAndInferCtx::GetMirroredOpParallelConf(
    const ParallelDesc &parallel_desc, int64_t parallel_id) const {
  return parallel_desc.GetParallelIdOnlyParallelConf(parallel_id);
}

ParallelConf EagerJobBuildAndInferCtx::GetMirroredOpParallelConf(
    const ParallelDesc &parallel_desc, int64_t parallel_id) const {
  return parallel_desc.parallel_conf();
}

Maybe<LogicalBlobId>
LazyJobBuildAndInferCtx::FindOrCreateMirroredLbiFromCompatibleConsistentBlob(
    const LogicalBlobId &lbi) {
  const std::string &lbn = GenLogicalBlobName(lbi);
  const auto &sbn_it = mut_consistent_lbi2mirrored_lbi()->find(lbi);
  if (sbn_it != mut_consistent_lbi2mirrored_lbi()->end()) {
    return sbn_it->second;
  }
  ...
  auto PushBackSubLbi = [&](const std::string &op_name,
  ...
  if (sbp.has_broadcast_parallel()) {
    op_conf.set_name(kAutoMirroredBlobNamePrefix + "-DistributeClone-" +
                     NewUniqueId());
    ...
    op_conf.set_name(kAutoMirroredBlobNamePrefix + "-DistributeSplit-" +
                     NewUniqueId());
  ...
  }
  {
    ...
    const auto &scope =
        Global<vm::SymbolStorage<Scope>>::Get()->Get(scope_symbol_id);
    const auto *job_desc = JUST(scope.job_desc());
    JUST(
        AddAndInferOp(op_conf, parallel_desc.parallel_conf(), job_desc, false));
  }
  return mirrored_lbi;
}

Maybe<LogicalBlobId>
EagerJobBuildAndInferCtx::FindOrCreateMirroredLbiFromCompatibleConsistentBlob(
    const LogicalBlobId &lbi) {
  const std::string &lbn = GenLogicalBlobName(lbi);
  const auto &sbn_it = mut_consistent_lbi2mirrored_lbi()->find(lbi);
  if (sbn_it != mut_consistent_lbi2mirrored_lbi()->end()) {
  ...
  op_conf.set_device_tag(
      JUST(DeviceTag4DeviceType(parallel_desc.device_type())));
  op_conf.set_name(kAutoMirroredBlobNamePrefix + "-CastToMirrored-" +
                   NewUniqueId());
  ...
  *cast_to_mirrored_conf->mutable_sbp_parallel() = sbp;
  ...
  const std::string &op_attribute_str = PbMessage2TxtString(*op_attribute);
  const std::string &parallel_conf_str = PbMessage2TxtString(parallel_conf);
  JUST(GlobalMaybe<ForeignCallback>())
      ->EagerMirroredCast(op_attribute_str, parallel_conf_str);
  return mirrored_lbi;
}
...
Maybe<void> LazyJobBuildAndInferCtx::Complete() {
  ...
  auto scope =
      std::make_unique<GlobalJobDescScope>(mut_job()->job_conf(), job_id());
...
Maybe<void> EagerJobBuildAndInferCtx::Complete() {
  ...
  JUST(DoPass("GenerateBackwardAndOptimizerOpConfs"));
  JUST(DoPass("AddLbiDiffWatcherOpConfs"));
  JUST(EagerRunOps(job(), &executed_op_names_,
                   &ForeignCallback::EagerInterpretCompletedOp));
  return Maybe<void>::Ok();
}
...
void JobBuildAndInferCtx::InferBlobBackwardSignature(Operator *op) {
  ...
  auto *map = op->mut_blob_backward_used_signature()
  ...
  for (const auto &ibn : op->input_bns()) {
    SetIsBlobBackwardUsed(ibn);
  }
  for (const auto &obn : op->output_bns()) {
    SetIsBlobBackwardUsed(obn);
  }
}
...
void JobBuildAndInferCtx::InferBlobBackwardSignature(
  ...
  const auto &Op4Name = [&](const std::string &op_name) {
    return CHECK_JUST(Op4OpName(op_name));
  };
  UpdateOpName2AncestorsNeedNoGrad(op, Op4Name,
                                   &op_name2ancestors_need_no_grad_);
  ...
  const auto &DiffLbi4BnInOp = [&](const std::string &bn) -> LogicalBlobId * {
    ...
    if (std::find(input_bns.begin(), input_bns.end(), bn) != input_bns.end()) {
      ...
      if (op_name2ancestors_need_no_grad_.at(lbi.op_name())) {
        return nullptr;
      }
      if (op.InputBlobModifier4Ibn(bn).requires_grad() == false) {
        return nullptr;
      }
      return &in_diff2lbi[bn];
    } else if (std::find(output_bns.begin(), output_bns.end(), bn) !=
               output_bns.end()) {
      return &fake_diff_lbi;
    } else {
      LOG(FATAL) << "diff lbi for bn in op not found, bn: " << op.op_name()
                 << "/" << bn;
    }
    return nullptr;
  };
  const auto &FwLogicalBlobDescPtr4Lbi =
      [&](const LogicalBlobId &lbi) -> const BlobDesc * {
    const auto &iter = lbi2logical_blob_desc_.find(lbi);
    if (iter != lbi2logical_blob_desc_.end()) {
      return iter->second.get();
    }
    return nullptr;
  };
  const auto &LogicalBlobDesc4BnInOp =
      [&](const std::string &bn) -> const BlobDesc & {
    const LogicalBlobId &lbi = op.BnInOp2Lbi(bn);
  ...
  const auto &maybe_ok = TRY(GenerateBackwardOpConfIf(
      op, &bw_op_confs, DiffLbi4BnInOp, LogicalBlobDesc4BnInOp));
  CHECK(maybe_ok.IsOk() ||
  ...
  for (const auto &bw_op_conf : bw_op_confs) {
    const auto &bw_op =
        ConstructOp(bw_op_conf, op.device_type(), Global<JobDesc>::Get());
    for (const auto &ibn : bw_op->input_bns()) {
      const auto &lbi = bw_op->BnInOp2Lbi(ibn);
      if (FwLogicalBlobDescPtr4Lbi(lbi) != nullptr) {
        backward_used_lbis->insert(lbi);
      }
    }
  }
...
void FormateUserConf(nlohmann::json &json_conf) {
  ...
  std::string nomarl_array[] = {"at_int32", "at_int64",    "at_bool",
                                "at_float", "at_double",   "at_string",
                                "at_shape", "at_data_type"};
  std::string list_array[] = {"at_list_int32", "at_list_int64",
                              "at_list_float", "at_list_data_type",
                              "at_list_shape", "at_list_string"};
...
void FormateVariableConf(nlohmann::json &json_conf) {
  ...
  for (nlohmann::json::iterator it = variable_conf.begin();
       it != variable_conf.end(); ++it) {
...
std::string oneflow::JobBuildAndInferCtx::GetJobStructureGraphJson(
  ...
  for (const auto &pair : op_name2op_) {
    ...
    for (const auto &in_node_name : inbound_nodes) {
      inbound_nodes_vec.emplace_back(in_node_name);
    }
