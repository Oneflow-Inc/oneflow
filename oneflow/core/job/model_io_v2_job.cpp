...
std::string GetVariableLbn(const OperatorConf &variable_op_conf) {
  return GenLogicalBlobName(variable_op_conf.name(),
                            variable_op_conf.variable_conf().out());
}
...
void MakeModelInitJob(
  ...
  std::string prev_post_model_init_tick_lbn =
  ...
  for (const auto &pair : var_op_name2op_conf) {
    ...
    ModelInitV2OpConf *model_init_conf =
        model_init_op_conf.mutable_model_init_v2_conf();
    model_init_conf->set_ref(GetVariableLbn(variable_op_conf));
    *model_init_conf->mutable_original_variable_conf() =
    ...
    *model_init_conf->mutable_out() = "out";
    ...
    job_builder.AddOps(parallel_blob_conf.parallel_conf(),
                       {new_var_op_conf, model_init_op_conf});
  }
}
...
void MakeModelLoadJob(
  ...
  std::string prev_post_model_load_tick_lbn =
      GenLogicalBlobName(foreign_input_op_conf.name(),
                         foreign_input_op_conf.foreign_input_conf().out());
  for (const auto &pair : var_op_name2op_conf) {
    const auto &var_op_name = pair.first;
    ...
    const VariableOpConf &origin_variable_conf =
        variable_op_conf.variable_conf();
    ...
    ModelLoadV2OpConf *model_load_conf =
        model_load_op_conf.mutable_model_load_v2_conf();
    ...
    *model_load_conf->mutable_tick() = prev_post_model_load_tick_lbn;
    ...
    job_builder.AddOps(variable_op_parallel_conf,
                       {new_var_op_conf, model_load_op_conf});
  }
}
...
void MakeModelSaveJob(
  ...
  std::string prev_post_model_save_tick_lbn =
      GenLogicalBlobName(foreign_input_op_conf.name(),
                         foreign_input_op_conf.foreign_input_conf().out());
  for (const auto &pair : var_op_name2op_conf) {
    const auto &var_op_name = pair.first;
    ...
    ModelSaveV2OpConf *model_save_conf =
        model_save_op_conf.mutable_model_save_v2_conf();
    ...
    *model_save_conf->mutable_original_variable_conf() = variable_conf;
    ...
    job_builder.AddOps(parallel_blob_conf.parallel_conf(),
                       {new_var_op_conf, model_save_op_conf});
  }
}
