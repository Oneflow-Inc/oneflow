...
template <> struct hash<oneflow::ParallelBlobConf> {
...
bool operator==(const ParallelBlobConf &lhs, const ParallelBlobConf &rhs) {
  return BlobDesc(lhs.logical_blob_desc_conf()) ==
             BlobDesc(rhs.logical_blob_desc_conf()) &&
         lhs.parallel_conf() == rhs.parallel_conf() &&
         lhs.sbp_conf() == rhs.sbp_conf();
}
...
std::string net_topo_key(const std::string &plan_name) {
  return plan_name + "_net_topo";
}

std::string job_id2job_conf(const std::string &plan_name) {
  return plan_name + "_job_id2job_conf";
}
...
std::string sub_plan_key(const std::string &plan_name, int64_t machine_id,
                         int64_t thrd_id) {
  return plan_name + "_" + std::to_string(machine_id) + "_" +
...
void PushPlan(const std::string &plan_name, const Plan &plan) {
  ...
  HashMap<std::pair<int64_t, int64_t>, std::vector<TaskProto>>
  ...
  for (const auto &task : plan.task()) {
    ...
    mchn_thrd_id2task_protos[std::make_pair(task.machine_id(), task.thrd_id())]
        .emplace_back(task);
  }
  ...
  for (const auto &pair : machine_id2thrd_id_set) {
    ...
    *(machine_id2thrd_ids.at(pair.first).mutable_thrd_id()) =
        StdVec2PbRf(thrd_id_vec);
  }

  ClusterThrdIds cluster_thrd_ids;
  *(cluster_thrd_ids.mutable_machine_id2thrd_ids()) =
      HashMap2PbMap(machine_id2thrd_ids);
  Global<CtrlClient>::Get()->PushKV(cluster_thrd_ids_key(plan_name),
                                    cluster_thrd_ids);
  ...
  for (const auto &pair : machine_id2block7chunk) {
    Global<CtrlClient>::Get()->PushKV(block7chunk_key(plan_name, pair.first),
                                      pair.second);
  }

  Global<CtrlClient>::Get()->PushKV(net_topo_key(plan_name), plan.net_topo());
  Global<CtrlClient>::Get()->PushKV(job_id2job_conf(plan_name),
                                    plan.job_confs());
...
void PullPlan(const std::string &plan_name, Plan *plan) {
  ClusterThrdIds cluster_thrd_ids;
  Global<CtrlClient>::Get()->PullKV(cluster_thrd_ids_key(plan_name),
                                    &cluster_thrd_ids);
  PrintProtoToTextFile(
      cluster_thrd_ids,
      JoinPath(FLAGS_log_dir, cluster_thrd_ids_key(plan_name)));
  HashMap<int64_t, ThrdIds> machine_id2thrd_ids;
  ...
  for (auto thrd_id : thrd_id_vec) {
    ...
    Global<CtrlClient>::Get()->PullKV(
        sub_plan_key(plan_name, machine_id, thrd_id), &sub_plan);
  ...
  *(plan->mutable_job_confs()) = job_confs;
  ...
  Global<CtrlClient>::Get()->PullKV(block7chunk_key(plan_name, machine_id),
                                    &block7chunk);
...
const boxing::collective::RankDesc &GetRankDesc(const TaskProto &task_proto) {
  ...
  return GetRankDesc(task_proto.exec_sequence()
                         .exec_node(0)
                         .kernel_conf()
                         .op_attribute()
                         .op_conf());
...
void GetDeviceDesc(const TaskProto *task_proto,
                   boxing::collective::DeviceDesc *device_desc) {
  device_desc->set_machine_id(task_proto->machine_id());
  const int64_t thrd_id =
      Global<IDMgr>::Get()->ThrdId4ActorId(task_proto->task_id());
  device_desc->set_device_type(
      Global<IDMgr>::Get()->GetDeviceTypeFromThrdId(thrd_id));
  if (device_desc->device_type() == DeviceType::kGPU) {
    device_desc->set_device_id(
...
void GenCollectiveBoxingPlan(Job *job, Plan *plan) {
  ...
  while (true) {
    ...
    plan_task_graph.ForEachNode([&](const PlanTaskNode *node) {
      if (all_visited.count(node) != 0) {
        return;
      }
      int64_t in_cnt = 0;
      node->ForEachNodeOnInEdge([&](const PlanTaskNode *node_on_in_edge) {
        if (all_visited.count(node_on_in_edge) != 0) {
          return;
        }
        in_cnt += 1;
      });
    ...
    plan_task_graph.TopoForEachNode(src_nodes, ForEachNodeOnInEdge,
    ...
    if (collective_boxing_nodes.empty()) {
      break;
    }
    ...
    for (const PlanTaskNode *node : collective_boxing_nodes) {
      ...
      if (it == name2request_info.end()) {
        ...
        name2request_info.emplace(
            std::make_pair(name, std::move(request_info)));
        order += 1;
      } else {
        CHECK(it->second.op_desc == rank_desc.op_desc());
        CHECK(
            it->second.rank2node.emplace(std::make_pair(rank_desc.rank(), node))
                .second);
      }
    }
    int64_t collected = 0;
    for (const auto &name7request_info : name2request_info) {
      ...
      if (info.rank2node.size() == info.op_desc.num_ranks()) {
        collected += 1;
        boxing::collective::RequestDesc *request_desc =
            request_set->mutable_request()->Add();
        *request_desc->mutable_op_desc() = info.op_desc;
        for (int64_t i = 0; i < info.op_desc.num_ranks(); ++i) {
          GetDeviceDesc(
              info.rank2node.at(i)->task_proto(),
        ...
        for (const auto &pair : info.rank2node) {
...
Maybe<void> CompileCurJobOnMaster(Job *job, Plan *improved_plan,
                                  bool need_job_complete) {
  ...
  if (Global<MachineCtx>::Get()->IsThisMachineMaster()) {
    ...
    complete_plan = *JUST(Improver().GenAndInferMemBlockIdOnly(
  ...
  if (job_desc.enable_experiment_run()) {
    ...
    // Experiment Runtime
    {
      Runtime experiment_run(complete_plan,
                             job_desc.piece_num_of_experiment_phase(), true);
    }
    // Improve
    if (Global<MachineCtx>::Get()->IsThisMachineMaster()) {
      ...
      *improved_plan = *JUST(Improver().Improve(
          *Global<AvailableMemDesc>::Get(), naive_plan,
          JoinPath(FLAGS_log_dir,
                   ActEventLogger::experiment_act_event_bin_filename())));
      OF_BARRIER();
...
void MergeSubPlanWithoutGenNetTopo(Plan *plan,
                                   const std::vector<Plan> &sub_plans) {
...
// return:
//         op_A --> op_identity_tick --> op_C --> op_D --> op_E --> op_sink_tick
//         --> op_B
//                                        /
//                        op_src_tick ---/
//
// note: after this function called, op_src_tick is illegal and need to be
// deleted from plan
void LinkTickTaskProto(TaskProto *identity_tick, TaskProto *src_tick,
                       TaskProto *sink_tick) {
  ...
  *sink_tick_sole_regst->mutable_consumer_task_id() =
      id_tick_sole_regst->consumer_task_id();
  ...
  *id_tick_sole_regst->mutable_consumer_task_id() =
      src_tick_sole_regst->consumer_task_id();
  UpdateSoleObnRegstDescId(identity_tick);
}

void FixRegstHostMemCase(
  ...
  for (auto &pair : *task_proto->mutable_produced_regst_desc()) {
    ...
    for (int64_t consumer_task_id : regst->consumer_task_id()) {
      ...
      used_by_network = used_by_network || (task_proto->machine_id() !=
                                            consumer_task_proto->machine_id());
    }
    regst->mutable_mem_case()->mutable_host_mem()->set_used_by_network(
        used_by_network);
  }
}
...
void LinkMainPlan(Plan *plan, const Plan &main_plan,
  ...
  std::function<bool(const TaskProto *)> IsInterfaceTickTockTask;
  {
    ...
    for (const auto &task : main_plan.task()) {
      if (task.task_type() == TaskType::kTick) {
        CHECK(task_ids->emplace(task.task_id()).second);
      }
    }
    IsInterfaceTickTockTask = [task_ids](const TaskProto *task) {
      if (task_ids->find(task->task_id()) != task_ids->end()) {
        return true;
      }
      ...
      OperatorConf::OpTypeCase op_type_case =
          kernel_conf.op_attribute().op_conf().op_type_case();
  ...
  FOR_RANGE(int64_t, i, 0, plan->task_size()) {
    ...
    if (IsInterfaceTickTockTask(task) == false) {
      continue;
    }
  ...
  FOR_RANGE(int32_t, i, 0,
            Global<CriticalSectionDesc>::Get()->CriticalSectionNum()) {
    const CriticalSection &critical_section =
        Global<CriticalSectionDesc>::Get()->GetCriticalSection(i);
    TaskProto *identity_tick =
        sole_tick_op_name2sole_task.at(identity_tick_op_names.at(i));
    LinkTickTaskProto(
        identity_tick,
  ...
  }
  {
    ...
    FOR_RANGE(int32_t, i, 0,
      ...
      const CriticalSection &critical_section =
          Global<CriticalSectionDesc>::Get()->GetCriticalSection(i);
      CHECK(source_tick_op_names.emplace(critical_section.source_tick_op_name())
                .second);
    }
    Erase<PbRpf<TaskProto>>(*plan->mutable_task(), [&](const TaskProto &task) {
      if (task.task_type() == TaskType::kSourceTick) {
        ...
        CHECK(source_tick_op_names.find(op_conf.name()) !=
              source_tick_op_names.end());
        return true;
      } else {
        return false;
      }
...
void GetMemSharingOpBlobInfo(const JobBuilder &job_builder,
  ...
  *blob_conf->mutable_parallel_conf() =
      job_builder.ParallelConf4OpName(op_name);
  *blob_conf->mutable_logical_blob_desc_conf() =
      job.helper().lbn2logical_blob_desc().at(lbn);
...
void FilterOpName2ParallelBlobConf(
    const HashSet<OperatorConf::OpTypeCase> &match,
  ...
  FOR_RANGE(int64_t, job_id, 0, jobs.size()) {
    ...
    for (const OperatorConf &op_conf : jobs.at(job_id)->net().op()) {
      if (match.find(op_conf.op_type_case()) == match.end()) {
        continue;
      }
      const auto &iter = op_name2parallel_blob_conf->find(op_conf.name());
      if (iter == op_name2parallel_blob_conf->end()) {
        auto *first_op_parallel_blob_conf =
            &(*op_name2parallel_blob_conf)[op_conf.name()];
        GetMemSharingOpBlobInfo(job_builder, op_conf.name(),
...
void CheckNonDistributeOptimizerAvailable(
    const std::vector<std::shared_ptr<Job>> &jobs) {
  ...
  if (!has_job_enable_non_distributed_optimizer) {
    return;
  }
  ...
  FOR_RANGE(int64_t, job_id, 0, jobs.size()) {
    ...
    for (const OperatorConf &op_conf : jobs.at(job_id)->net().op()) {
      if (op_conf.op_type_case() == OperatorConf::kVariableConf) {
        continue;
      }
      if (var_names.find(op_conf.name()) == var_names.end()) {
        var_names.emplace(op_conf.name());
      } else {
        LOG(FATAL) << "Only support non_distribute_optimizer when jobs not "
...
void MakeMainJob(Job *main_job,
  ...
  {
    wait_and_send_ids_op_conf.set_name(
        std::string("System-Main-WaitAndSendIds_") + NewUniqueId());
    auto *wait_and_send_ids_conf =
        wait_and_send_ids_op_conf.mutable_wait_and_send_ids_conf();
    ...
    FOR_RANGE(int32_t, i, 0, Global<JobName2JobId>::Get()->size()) {
      id_list->Add();
    }
    ...
    for (const auto &pair : *Global<JobName2JobId>::Get()) {
      ...
      const auto &cs_idx =
          Global<CriticalSectionDesc>::Get()->CriticalSectionIds4JobId(job_id);
      *id_list->Mutable(job_id)->mutable_value() = {cs_idx.begin(),
                                                    cs_idx.end()};
    }
  }
  ...
  {
    reentrant_lock_op_conf.set_name(std::string("System-Main-ReentrantLock_") +
                                    NewUniqueId());
    auto *reentrant_lock_conf =
        reentrant_lock_op_conf.mutable_reentrant_lock_conf();
  ...
  FOR_RANGE(int64_t, i, 0,
    ...
    identity_tick_conf->add_tick(cs_case_op_conf.name() + "/" +
                                 GenRepeatedBn("out", i));
  ...
  {
    callback_notify_esac_op_conf.set_name(std::string("System-Main-Esac_") +
                                          NewUniqueId());
    auto *callback_notify_esac_conf =
        callback_notify_esac_op_conf.mutable_esac_conf();
    for (int64_t total_job_cs_id :
         Global<CriticalSectionDesc>::Get()
             ->job_id2total_job_critical_section_id()) {
      callback_notify_esac_conf->add_in(
          identity_tick_op_names->at(total_job_cs_id) + "/out");
    }
  ...
  {
    callback_notify_op_conf.set_name(
        std::string("System-Main-CallbackNotify_") + NewUniqueId());
    auto *callback_notify_conf =
        callback_notify_op_conf.mutable_callback_notify_conf();
    ...
    FOR_RANGE(int64_t, i, 0, Global<JobName2JobId>::Get()->size()) {
      buffer_names->Add();
    }
    for (const auto &pair : *Global<JobName2JobId>::Get()) {
      int64_t job_id = pair.second;
...
void ConnectCriticalSectionEndToReentrantLockEnd(
    Plan *main_plan, const LogicalBlobId &critical_section_sink_lbi) {
  ...
  FOR_RANGE(int64_t, i, 0, main_plan->task_size()) {
    ...
    if (task->task_type() == TaskType::kReentrantLock) {
      ...
      const auto &kernel_conf =
          task->exec_sequence().exec_node(0).kernel_conf();
      if (critical_section_sink_lbi.op_name() ==
          kernel_conf.op_attribute().op_conf().name()) {
  ...
  RegstDescProto *cs_end_regst =
      PlanUtil::GetSoleProducedDataRegst(cs_sink_task);
  cs_end_regst->add_consumer_task_id(reentrant_lock_task->task_id());
  reentrant_lock_task->mutable_consumed_regst_desc_id()
      ->at("in")
      .add_regst_desc_id(cs_end_regst->regst_desc_id());

  auto *reentrant_exec_node =
      reentrant_lock_task->mutable_exec_sequence()->mutable_exec_node(0);
  (*reentrant_exec_node->mutable_bn_in_op2regst_desc_id())["end"] =
      cs_end_regst->regst_desc_id();

  auto *op_attribute =
      reentrant_exec_node->mutable_kernel_conf()->mutable_op_attribute();
  op_attribute->add_input_bns("end");
  (*op_attribute->mutable_arg_signature()->mutable_bn_in_op2lbi())["end"] =
      critical_section_sink_lbi;

  auto *reentrant_lock_conf =
      op_attribute->mutable_op_conf()->mutable_reentrant_lock_conf();
...
Maybe<void> CompileMainJob(Job *main_job,
  ...
  {
    auto scope =
        std::make_unique<GlobalJobDescScope>(main_job->job_conf(), job_id);
    JUST(CompileCurJobOnMaster(main_job, main_plan, false));
  }
  ConnectCriticalSectionEndToReentrantLockEnd(main_plan,
                                              critical_section_sink_lbi);
  return Maybe<void>::Ok();
}
...
bool NeedAllocateMemory(const RegstDescTypeProto &regst_desc_type) {
  return regst_desc_type.has_data_regst_desc() &&
         regst_desc_type.data_regst_desc()
                 .packed_blob_desc()
                 .is_body_disabled() == false;
}

void FinishGlobalCriticalSectionDesc(const Plan &plan, int64_t job_size) {
  std::vector<HashMap<std::string, HashSet<int64_t>>>
      job_id2sole_op_name2mem_block_ids(job_size);
  ...
  FOR_RANGE(int64_t, i, 0, critical_section_desc->CriticalSectionNum()) {
    ...
    auto *input_output_mem_block_ids =
        &job_id2input_output_mem_block_ids[job_id];
    if (critical_section->has_input_output_critical_section()) {
      HashSet<int64_t> mem_block_ids;
      for (const auto &op_name :
           critical_section->input_output_critical_section()
               .lbi_producer_op_name()) {
        const auto &cur_mem_block_ids =
            job_id2sole_op_name2mem_block_ids.at(job_id).at(op_name);
        mem_block_ids.insert(cur_mem_block_ids.begin(),
                             cur_mem_block_ids.end());
      }
      *critical_section->mutable_mem_block_id() = {mem_block_ids.begin(),
                                                   mem_block_ids.end()};
      input_output_mem_block_ids->insert(mem_block_ids.begin(),
                                         mem_block_ids.end());
    } else {
  ...
  FOR_RANGE(int64_t, i, 0, critical_section_desc->CriticalSectionNum()) {
    auto *critical_section = critical_section_desc->MutCriticalSection(i);
    int64_t job_id = critical_section->job_id();
    const auto &input_output_mem_block_ids =
        job_id2input_output_mem_block_ids.at(job_id);
    if (critical_section->has_total_job_critical_section()) {
      ...
      auto *mem_block_ids = &job_id2mem_block_ids.at(job_id);
      {
        ...
        while (it != mem_block_ids->end()) {
          if (input_output_mem_block_ids.find(*it) ==
              input_output_mem_block_ids.end()) {
            ++it;
          } else {
      ...
      *critical_section->mutable_mem_block_id() = {mem_block_ids->begin(),
                                                   mem_block_ids->end()};
      *critical_section->mutable_chunk_id() = {
          job_id2chunk_ids.at(job_id).begin(),
...
void MakePullJob(const std::string &job_name, const std::string &op_name,
  ...
  auto *flag_name2flag_value =
      job->mutable_job_conf()->mutable_flag_name2flag_value();
  (*flag_name2flag_value)["__is_user_function__"].set_at_bool(false);
  auto *op_name2job_name = Global<InterUserJobInfo>::Get()
  ...
  {
    foreign_output_op_conf.set_name(std::string("System-Pull-ForeignOutput_") +
                                    NewUniqueId());
    auto *foreign_output_conf =
        foreign_output_op_conf.mutable_foreign_output_conf();
    foreign_output_conf->set_in(input_op_conf.name() + "/out");
    foreign_output_conf->set_ofblob_buffer_name(
        GetForeignOutputBufferName(job_name));
    ParallelConf parallel_conf;
    parallel_conf.set_device_tag("cpu");
    parallel_conf.add_device_name("0:0");
...
void MakePushJob(const std::string &job_name, const std::string &op_name,
  ...
  {
    foreign_input_op_conf.set_name(std::string("System-Push-ForeignInput_") +
                                   NewUniqueId());
    auto *foreign_input_conf =
        foreign_input_op_conf.mutable_foreign_input_conf();
    foreign_input_conf->set_out("out");
    foreign_input_conf->set_ofblob_buffer_name(
        GetForeignInputBufferName(job_name));
  ...
  OperatorConf output_op_conf;
  {
    ...
    InterfaceOpUtil::InitBlobConf(output_conf->mutable_blob_conf(),
                                  parallel_blob_conf);
...
REGISTER_FUNCTION_CONFIG_DEF()
    .Bool("__is_user_function__", true, "is user defined function");
...
Maybe<void> CompileAndMergePlanOnMaster(const PbRpf<Job> &conf_jobs,
  ...
  FOR_RANGE(int, i, 0, jobs.size()) {
    jobs.at(i).reset(new Job(conf_jobs.Get(i)));
  }
  if (jobs.size() > 1) {
    CheckNonDistributeOptimizerAvailable(jobs);
  }
  if (Global<MachineCtx>::Get()->IsThisMachineMaster()) {
  ...
  FOR_RANGE(int, i, 0, jobs.size()) {
    ...
    if (job_desc.Bool("__is_user_function__")) {
      function_jobs.push_back(jobs.at(i));
    }
  }
  if (Global<MachineCtx>::Get()->IsThisMachineMaster()) {
    ...
    for (const auto &pair : push_op_name2parallel_blob_conf) {
      ...
      MakePushJob(std::string("System-Push-") + pair.first, pair.first,
                  pair.second, push_job.get());
      jobs.emplace_back(push_job);
    }
    for (const auto &pair : pull_op_name2parallel_blob_conf) {
      auto pull_job = std::make_shared<Job>();
      MakePullJob(std::string("System-Pull-") + pair.first, pair.first,
                  pair.second, pull_job.get());
  ...
  FOR_RANGE(int64_t, i, 0, jobs.size()) {
    ...
    auto scope =
        std::make_unique<GlobalJobDescScope>(jobs.at(i)->job_conf(), i);
    JUST(CompileCurJobOnMaster(jobs.at(i).get(), &sub_plans.at(i), true));
  }
  if (Global<MachineCtx>::Get()->IsThisMachineMaster()) {
    MergeSubPlanWithoutGenNetTopo(plan, sub_plans);
    InterJobMemSharingUtil::MergeMemReusedChunkBetweenUserJobs(function_jobs,
                                                               plan);
    InterJobMemSharingUtil::MergeMemSharedInterfaceMemBlockBetweenJobs(jobs,
                                                                       plan);
    ...
    std::vector<std::string> identity_tick_op_names;
    {
      Job main_job;
      LogicalBlobId critical_section_sink_lbi;
      MakeMainJob(&main_job, &identity_tick_op_names,
                  &critical_section_sink_lbi);
      AddJobName2JobId(main_job.job_conf().job_name(), jobs.size());
      JUST(CompileMainJob(&main_job, critical_section_sink_lbi,
                          sub_plans.size(), &main_plan));
    }
...
Oneflow::~Oneflow() {
  ...
  if (Global<Profiler>::Get() != nullptr) {
    Global<Profiler>::Get()->Profile(
        plan_,
        JoinPath(FLAGS_log_dir, ActEventLogger::act_event_bin_filename()));
  }
}
