...
void UpdateCtrlRegstHandler4MemSharedPlan(Plan* plan) {
  for (TaskProto& task : *(plan->mutable_task())) {
    RegstHandlerProto ctrl_handler_proto = CreateRegstHandlerProto("Ctrl");
    ...
    if (IsRegstHandlerProtoEmpty(ctrl_handler_proto)) { continue; }
    bool update_existed = false;
    for (RegstHandlerProto& handler_proto : *(task.mutable_regst_handlers())) {
...
Oneflow::Oneflow(const std::string& job_conf_filepath) {
  ...
  if (machine_ctx->IsThisMachineMaster()) {
    ...
    UpdateCtrlRegstHandler4MemSharedPlan(&mem_shared_plan);
