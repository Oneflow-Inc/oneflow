...
void PullPlan(const std::string& plan_name, Plan* plan) {
  ...
  int64_t machine_id = GlobalProcessRankInfo::ThisMachineId();
...
void AddJobName2JobId(const std::string& job_name, int64_t job_id) {
  if (!GlobalProcessRankInfo::IsThisMachineMaster()) { return; }
...
Oneflow::~Oneflow() {
  if (GlobalProcessRankInfo::IsThisMachineMaster()) { runtime_buffers_scope_.reset(); }
  runtime_.reset();
