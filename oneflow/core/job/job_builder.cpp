...
std::function<const ParallelConf *(const std::string &)>
MakeGetterParallelConf4OpName(const Placement &placement) {
  auto op_name2parallel_conf =
      std::make_shared<HashMap<std::string, const ParallelConf *>>();
...
JobBuilder::JobBuilder(Job *job) : job_(job) {
  FOR_RANGE(int32_t, i, 0, job->net().op_size()) {
    CHECK(
        op_name2op_conf_
            .emplace(job->net().op(i).name(), job->mutable_net()->mutable_op(i))
            .second);
  }
  FOR_RANGE(int32_t, i, 0, job->placement().placement_group_size()) {
    ...
    for (const auto &op_name : placemnt_group->op_set().op_name()) {
      CHECK(op_name2parallel_conf_
                .emplace(op_name, placemnt_group->mutable_parallel_conf())
                .second);
    }
  }
  ...
  FOR_RANGE(int32_t, i, 0, job->placement().blob_placement_group_size()) {
    ...
    for (const auto &lbi : blob_pg->lbi()) {
      CHECK(
          lbi2blob_parallel_conf_.emplace(lbi, blob_pg->mutable_parallel_conf())
              .second);
    }
  }
}
...
const OperatorConf &
JobBuilder::OpConf4OpName(const std::string &op_name) const {
  return *op_name2op_conf_.at(op_name);
}

const ParallelConf &
JobBuilder::ParallelConf4Lbi(const LogicalBlobId &lbi) const {
  const auto &iter = lbi2blob_parallel_conf_.find(lbi);
  if (iter != lbi2blob_parallel_conf_.end()) {
    return *iter->second;
  }
  ...
  *placemnt_group->mutable_parallel_conf() = parallel_conf;
  for (const auto &op_conf : op_confs) {
    ...
    *mut_op_conf = op_conf;
    ...
    CHECK(op_name2parallel_conf_
              .emplace(op_conf.name(), placemnt_group->mutable_parallel_conf())
              .second);
  }
}

PlacementGroup *
JobBuilder::FindPlacementGroup(const std::string &op_name) const {
  FOR_RANGE(int32_t, i, 0, job_->mutable_placement()->placement_group_size()) {
    PlacementGroup *const cur_pg =
        job_->mutable_placement()->mutable_placement_group(i);
    const auto &op_names = cur_pg->op_set().op_name();
    if (std::find(op_names.begin(), op_names.end(), op_name) !=
        op_names.end()) {
      return cur_pg;
    }
  }
...
void JobBuilder::MutParallelConfOnlyOnce(const std::string &op_name,
  ...
  PlacementGroup *placement_group = FindPlacementGroup(op_name);
  {
    ...
    Erase<PbRpf<std::string>>(
        *op_names, [&](const std::string &x) { return x == op_name; });
    Placement *const placement = job_->mutable_placement();
    if (op_names->size() > 0) {
      placement_group = placement->mutable_placement_group()->Add();
    }
  }
...
void JobBuilder::RemoveOpByName(
    const std::unordered_set<std::string> &removing_names) {
  // Update net
  DLNetConf net = job_->net();
  job_->mutable_net()->clear_op();
  for (const OperatorConf &op_conf : net.op()) {
    if (removing_names.count(op_conf.name()) == 0) {
  ...
  for (const PlacementGroup &place : placement_group) {
    ...
    for (const std::string &name : place.op_set().op_name()) {
      if (removing_names.count(name) == 0) {
  ...
  auto *job_parallel_view_conf = job_->mutable_job_parallel_view_conf()
                                     ->mutable_op_name2sbp_signature_conf();
  auto *time_shape_conf =
      job_->mutable_helper()->mutable_op_name2op_time_shape();
  for (const std::string &op_name : removing_names) {
    // Update Sbp
    if (job_parallel_view_conf->count(op_name) > 0) {
      job_parallel_view_conf->erase(op_name);
    }
    // Update time shape
  ...
  if (job_->helper().has_identical_sbp_oba_pairs()) {
    auto identical_sbp_oba_pairs =
        job_->helper().identical_sbp_oba_pairs().pair();
    ...
    for (const auto &pair : identical_sbp_oba_pairs) {
      if (removing_names.count(pair.first().op_name()) == 0 &&
        ...
        *(job_->mutable_helper()
              ->mutable_identical_sbp_oba_pairs()
              ->mutable_pair()
              ->Add()) = pair;
      }
    }
...
void JobBuilder::DelOps(const std::vector<OperatorConf> &op_confs) {
  std::unordered_set<std::string> removing_names;
  for (const auto &op_conf : op_confs) {
    removing_names.insert(op_conf.name());
  }
  RemoveOpByName(removing_names);
}
...
void JobBuilder::AddOrMutOpsOnlyOnce(
    const ParallelConf &parallel_conf,
    const std::vector<OperatorConf> &op_confs) {
...
void JobBuilder::ForEachOperator(
    const std::function<void(const Operator &)> &Handler) const {
  for (const auto &pair : op_name2op_conf_) {
    ...
    std::shared_ptr<Operator> op =
        ConstructOp(*pair.second, device_type, &GlobalJobDesc());
    Handler(*op);
  }
}

const ParallelConf &
JobBuilder::ParallelConf4OpName(const std::string &op_name) const {
...
void JobBuilder::BindIdenticalSbpOpBlobArgPair(const OpBlobArg &first,
                                               const OpBlobArg &second) {
  auto *pair = job_->mutable_helper()
                   ->mutable_identical_sbp_oba_pairs()
                   ->mutable_pair()
                   ->Add();
...
const SbpSignature &
JobBuilder::SbpSignature4OpName(const std::string &op_name) const {
...
void JobBuilder::AddSbpSignature4OpName(const std::string &op_name,
  ...
  auto *op_name2sbp_signature_conf = job_->mutable_job_parallel_view_conf()
                                         ->mutable_op_name2sbp_signature_conf();
...
const OpTimeShape &
JobBuilder::TimeShape4OpName(const std::string &op_name) const {
  const auto &it = op_name2time_shapes_.find(op_name);
  CHECK(it != op_name2time_shapes_.end());
  return *(it->second);
}

void JobBuilder::AddTimeShape4OpName(const std::string &op_name,
...
void JobBuilder::AddBatchAxis4Lbn(const std::string &lbn,
                                  const OptInt64 &axis) {
