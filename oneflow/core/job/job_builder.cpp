...
void SetParallelDistributionSignature4Oba(Job* job,
  ...
  nd_sbp.ToProto(
      &(*nd_sbp_sig->mutable_bn_in_op2nd_sbp())[oba.bn_in_op()]);
  auto* op_name2nd_sbp_signature_conf =
      job->mutable_job_parallel_view_conf()->mutable_op_name2nd_sbp_signature_conf();
  (*op_name2nd_sbp_signature_map)[oba.op_name()] =
      &(*op_name2nd_sbp_signature_conf)[oba.op_name()];
}

void SetSbpSignature4Oba(Job* job, const OpBlobArg& oba, const cfg::SbpParallel& sbp_parallel) {
  auto* sbp_sig = &(
      *job->mutable_job_parallel_view_conf()->mutable_op_name2sbp_signature_conf())[oba.op_name()];
  sbp_parallel.ToProto(&(*sbp_sig->mutable_bn_in_op2sbp_parallel())[oba.bn_in_op()]);
}
...
void AddOrSetParallelDistributionSignature4OpName(
  ...
  if (it != op_name2nd_sbp_signature_map->end()) {
    nd_sbp_signature.ToProto(it->second);
  } else {
    auto* op_name2nd_sbp_signature_conf =
        job->mutable_job_parallel_view_conf()
            ->mutable_op_name2nd_sbp_signature_conf();
    nd_sbp_signature.ToProto(
        &(*op_name2nd_sbp_signature_conf)[op_name]);
    op_name2nd_sbp_signature_map->emplace(
        op_name, &(*op_name2nd_sbp_signature_conf)[op_name]);
  }
}
...
void AddOrSetSbpSignature4OpName(Job* job, const std::string& op_name,
  ...
  sbp_signature.ToProto(&(*op_name2sbp_signature_conf)[op_name]);
}
...
JobBuilder::JobBuilder(Job* job) : job_(job) {
  ...
  for (auto& pair :
       *(job_parallel_view_conf->mutable_op_name2nd_sbp_signature_conf())) {
...
void JobBuilder::RemoveOpByName(const std::unordered_set<std::string>& removing_names) {
  ...
  op_name2nd_sbp_signature_conf_.swap(
      builder.op_name2nd_sbp_signature_conf_);
...
void JobBuilder::SetSbpParallel4Oba(const OpBlobArg& oba, const cfg::SbpParallel& sbp_parallel) {
  ...
  *nd_sbp.add_sbp_parallel() = sbp_parallel;
  SetParallelDistributionSignature4Oba(job_, &op_name2nd_sbp_signature_conf_, oba,
                                       nd_sbp);
}

void JobBuilder::SetParallelDistribution4Oba(
    const OpBlobArg& oba, const cfg::ParallelDistribution& nd_sbp) {
  SetParallelDistributionSignature4Oba(job_, &op_name2nd_sbp_signature_conf_, oba,
                                       nd_sbp);
...
const cfg::SbpSignature JobBuilder::SbpSignature4OpName(const std::string& op_name) const {
...
void JobBuilder::AddSbpSignature4OpName(const std::string& op_name,
  ...
  AddOrSetParallelDistributionSignature4OpName(job_, &op_name2nd_sbp_signature_conf_,
                                               op_name, nd_sbp_signature);
  CHECK_EQ(GetParallelHierarchyNumAxes(op_name2parallel_conf_, op_name), 1);
