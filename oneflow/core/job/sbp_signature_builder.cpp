...
void SplitSbpSignatureListBuilder::CheckTemplate() {
  ...
  const auto &first =
      sbp_signature_template_.bn_in_op2sbp_parallel().begin()->second;
...
SbpSignatureBuilder &&SbpSignatureBuilder::Split(const std::string &bn_in_op,
                                                 int64_t axis) {
  (*sbp_signature_.mutable_bn_in_op2sbp_parallel())[bn_in_op]
...
SbpSignatureBuilder &&SbpSignatureBuilder::Split(const PbRpf<std::string> &bns,
                                                 int64_t axis) {
  for (const auto &bn_in_op : bns) {
    Split(bn_in_op, axis);
  }
  return std::move(*this);
}
