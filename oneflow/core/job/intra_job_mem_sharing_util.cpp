...
template <> struct hash<::oneflow::MemAllocAlgoType> {
...
void GenRegstDescId2RegstDesc(
    Plan *plan, HashMap<int64_t, RegstDescProto *> *regst_desc_id2regst_desc) {
...
void InitMemoryChains(
  ...
  for (int64_t i = 0; i < plan->task_size(); ++i) {
    ...
    DeviceType device_type =
        Global<IDMgr>::Get()->GetDeviceTypeFromThrdId(task->thrd_id());
    if (device_type != DeviceType::kGPU) {
      continue;
    }
    int64_t device_id =
        Global<IDMgr>::Get()->GetGpuPhyIdFromThrdId(task->thrd_id());
    int64_t device_unique_id = GenDeviceUniqueId(machine_id, device_id);
    MemoryChain *mem_chain =
        &((*device2chain2mem_chain)[device_unique_id]
                                   [task->task_set_info().chain_id()]);
    ...
    for (auto &pair : *(task->mutable_produced_regst_desc())) {
      ...
      if (regst_desc->mem_case().has_device_cuda_mem() &&
        ...
        mem_chain->total_mem_reused_size +=
  ...
  for (auto &device_pair : *device2chain2mem_chain) {
    ...
    for (auto &pair : *chain2mem_chain) {
      if (pair.second.mem_reused_regsts.empty()) {
        useless_chain_ids.insert(pair.first);
      }
    }
    for (int64_t chain_id : useless_chain_ids) {
      ...
      std::sort(mem_chain->sorted_tasks.begin(), mem_chain->sorted_tasks.end(),
                [&](const TaskProto *lhs, const TaskProto *rhs) {
                  int64_t lhs_order_in_graph =
                      lhs->task_set_info().order_in_graph();
                  int64_t rhs_order_in_graph =
                      rhs->task_set_info().order_in_graph();
...
bool TryMergeMemChain2MergedChains(
    ...
    const std::function<bool(const MemoryChain *, const MemoryChain *)>
        &IsStrictOrderL2R) {
  std::sort(merged_chains->begin(), merged_chains->end(),
            [&](MemoryChain *lhs, MemoryChain *rhs) {
  ...
  for (MemoryChain *merged_chain : *merged_chains) {
    if (IsStrictOrderL2R(merged_chain, mem_chain)) {
      ...
      merged_chain->mem_reused_regsts.insert(
          mem_chain->mem_reused_regsts.begin(),
...
void GenMemChainTasksAndRegsts(
  ...
  auto IsStrictOrderL2R = [&](const MemoryChain *lhs,
                              const MemoryChain *rhs) -> bool {
  ...
  for (auto &device_chain_pair : device2chain2mem_chain) {
    if (device_chain_pair.second.empty()) {
      continue;
    }
    // sort
    std::vector<MemoryChain *> mem_chains;
    std::vector<MemoryChain *> merged_chains;
    for (auto &pair : device_chain_pair.second) {
      mem_chains.push_back(&pair.second);
    }
    std::sort(mem_chains.begin(), mem_chains.end(),
              [&](MemoryChain *lhs, MemoryChain *rhs) {
                int64_t lhs_order_in_graph =
                    lhs->sorted_tasks.front()->task_set_info().order_in_graph();
                int64_t rhs_order_in_graph =
                    rhs->sorted_tasks.front()->task_set_info().order_in_graph();
                CHECK_NE(lhs_order_in_graph, rhs_order_in_graph);
                return lhs_order_in_graph < rhs_order_in_graph;
              });
    // merge
    for (MemoryChain *mem_chain : mem_chains) {
      if (!TryMergeMemChain2MergedChains(&merged_chains, mem_chain,
                                         IsStrictOrderL2R)) {
    ...
    for (MemoryChain *merged_chain : merged_chains) {
      std::vector<TaskProto *> *sorted_tasks =
          &((*mem_chain2sorted_tasks)[mem_chain_id]);
      CHECK(sorted_tasks->empty());
      sorted_tasks->insert(sorted_tasks->end(),
                           merged_chain->sorted_tasks.begin(),
...
void GenRegstAllocFreeTimeLineAndRegstMutualExclusions(
    ...
    HashMap<RegstDescProto *, HashSet<RegstDescProto *>>
  ...
  auto FindLastFreeIndexInSortedTasks =
      [&](RegstDescProto *regst_desc) -> int64_t {
  ...
  auto TryFindFirstInplacedRegstDesc =
      [&](RegstDescProto *consumer_regst) -> RegstDescProto * {
  ...
  for (RegstDescProto *regst_desc : mem_reused_regsts) {
    RegstDescProto *inplaced_regst_desc =
        TryFindFirstInplacedRegstDesc(regst_desc);
    if (inplaced_regst_desc != nullptr) {
      CHECK(consumer2inplaced_regst->emplace(regst_desc, inplaced_regst_desc)
                .second);
      continue;
    }

    CHECK(alloc_regsts_timeline
              ->at(task_id2sorted_id.at(regst_desc->producer_task_id()))
              .insert(regst_desc)
              .second);
    CHECK(regst_desc_id2free_index
              .emplace(regst_desc->regst_desc_id(),
                       FindLastFreeIndexInSortedTasks(regst_desc))
              .second);
  }
  ...
  for (auto pair : *consumer2inplaced_regst) {
    ...
    CHECK(regst_desc_id2free_index.find(inplaced_regst_desc_id) !=
          regst_desc_id2free_index.end());
  ...
  for (int64_t i = 0; i < sorted_tasks.size(); ++i) {
    for (RegstDescProto *alloc_regst : alloc_regsts_timeline->at(i)) {
      CHECK(regst2mutual_exclusion_regsts
                ->emplace(alloc_regst, HashSet<RegstDescProto *>())
                .second);
      for (RegstDescProto *remain_regst : remain_regsts) {
        CHECK(regst2mutual_exclusion_regsts->at(alloc_regst)
                  .insert(remain_regst)
                  .second);
        CHECK(regst2mutual_exclusion_regsts->at(remain_regst)
                  .insert(alloc_regst)
                  .second);
      }
...
public:
  ...
  void FindFreeOffsetAndNewBufferSize(int64_t size, int64_t *offset,
...
private:
  void CheckValid() {
    ...
    for (auto it = std::next(piece_list_.begin()); it != piece_list_.end();
         ++it) {
...
void MemBlockBuffer::FindFreeOffsetAndNewBufferSize(int64_t size,
                                                    int64_t *offset,
                                                    size_t *new_buffer_size) {
  CheckValid();
  for (auto it = piece_list_.begin(); it != piece_list_.end(); ++it) {
...
void MemReusedAlgorithm_AllocateByOrderAndMutualExclusion(
    ...
    const HashMap<RegstDescProto *, HashSet<RegstDescProto *>>
        &regst2mutual_exclusion_regsts,
    MemBlockResultInfo *result) {
  HashMap<RegstDescProto *, int64_t> *regst_desc2offset =
      &(result->regst_desc2offset);
  size_t buffer_size = 1;
  for (RegstDescProto *regst_desc : order) {
    MemBlockBuffer buffer(buffer_size);
    for (RegstDescProto *mutual_regst :
         regst2mutual_exclusion_regsts.at(regst_desc)) {
    ...
    int64_t offset = -1;
    buffer.FindFreeOffsetAndNewBufferSize(regst_desc2size.at(regst_desc),
                                          &offset, &buffer_size);
...
void MemReusedAlgorithm_MemSizeFirstAlgo(
  ...
  for (const auto &pair : regst2mutual_exclusion_regsts) {
    ...
    CHECK(regst_desc2size
              .emplace(pair.first,
                       RtRegstDesc(*pair.first).TotalMainByteSize4AllRegst())
              .second);
  }
  std::sort(order.begin(), order.end(),
            [&](RegstDescProto *lhs, RegstDescProto *rhs) {
              return regst_desc2size.at(lhs) > regst_desc2size.at(rhs);
            });
  MemReusedAlgorithm_AllocateByOrderAndMutualExclusion(
      order, regst_desc2size, regst2mutual_exclusion_regsts, result);
}
...
int64_t BfcAllocator::AllocateRaw(int64_t size) {
  ...
  if (candidate_piece == piece_list_.end()) {
    ...
    if (last_it->is_free) {
      ...
      CHECK(offset2occupied_piece_.emplace(offset, std::prev(piece_list_.end()))
                .second);
    }
  } else {
...
void SelectAlgorithmGenMemBlockOffset4Regsts(
  ...
  case kMutualExclusionFirstAlgo:
    MemReusedAlgorithm_MutualExclusionFirstAlgo(regst2mutual_exclusion_regsts,
                                                result);
    break;
  case kTimeLineAlgo:
...
void InitAlgo2Result(
    HashMap<MemAllocAlgoType, MemBlockResultInfo> *algo2result) {
  ...
  if (mem_alloc_algo_conf.use_mutual_exclusion_first_algo()) {
    CHECK(algo2result->emplace(kMutualExclusionFirstAlgo, MemBlockResultInfo())
              .second);
  }
  if (mem_alloc_algo_conf.use_time_line_algo()) {
...
void IntraJobMemSharingUtil::InferMemBlockId4MemReusedRegst(
  ...
  for (const auto &pair : mem_chain2mem_reused_regsts) {
    GenRegstAllocFreeTimeLineAndRegstMutualExclusions(
        ...
        regst_desc_id2regst_desc, &mem_chain2task2alloc_regsts[pair.first],
  ...
  HashMap<int64_t, HashMap<MemAllocAlgoType, MemBlockResultInfo>>
      mem_chain2algo2result;
  {
    int64_t work_size =
        mem_chain2mem_reused_regsts.size() * CountMemAllocAlgoNum();
    int64_t thread_pool_size =
        std::min<int64_t>(work_size, std::thread::hardware_concurrency());
    ...
    for (int64_t mem_chain_id : mem_chains) {
      ...
      for (auto &pair : mem_chain2algo2result.at(mem_chain_id)) {
        ...
        thread_pool.AddWork(
            [algo_id, mem_chain_id, &mem_chain2task2alloc_regsts,
             &mem_chain2task2free_regsts,
             &mem_chain2regst2mutual_exclusion_regsts, result, &counter]() {
              SelectAlgorithmGenMemBlockOffset4Regsts(
                  ...
                  mem_chain2regst2mutual_exclusion_regsts.at(mem_chain_id),
                  result);
              counter.Decrease();
            });
      }
  ...
  // step 3: choose best one for each mem chain and set offset for inplace
  // consumer regst
  for (const auto &pair : mem_chain2algo2result) {
    ...
    for (const auto &algo_result_pair : pair.second) {
      if (!best_result ||
          algo_result_pair.second.mem_block_size <
              best_result->mem_block_size) {
    ...
    // set inplace
    for (auto &consumer_inplace_pair :
         mem_chain2consumer2inplaced_regst.at(pair.first)) {
      ...
      consumer_regst_desc->set_mem_block_id(
          inplaced_regst_desc->mem_block_id());
      consumer_regst_desc->set_mem_block_offset(
          inplaced_regst_desc->mem_block_offset());
    }
  }
}
