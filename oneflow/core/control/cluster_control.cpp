...
void ClusterControl::MasterSendSessionStart() {
  ...
  Global<CtrlClient>::Get()->PushKV(GetHaltOrSessionStartCtrlKey(),
                                    cluster_control);
}
...
bool ClusterControl::WorkerReceiveHalt() {
  BarrierClear();
  ClusterControlProto cluster_control;
  Global<CtrlClient>::Get()->PullKV(GetHaltOrSessionStartCtrlKey(),
                                    &cluster_control);
  if (cluster_control.cmd() == kClusterCtrlCmdHalt) {
    return true;
  }
