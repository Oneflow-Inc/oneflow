...
namespace {
template <size_t... Idx>
static std::tuple<std::function<void(CtrlCall<(CtrlMethod)Idx> *)>...>
    GetHandlerTuple(std::index_sequence<Idx...>) {
  return {};
}
...
private:
  ...
  void EnqueueRequests() {
    for_each_i(handlers_, helper{this},
               std::make_index_sequence<kCtrlMethodNum>{});
  }

  template <CtrlMethod kMethod> void EnqueueRequest() {
    ...
    grpc_service_->RequestAsyncUnary(I, call->mut_server_ctx(),
                                     call->mut_request(), call->mut_responder(),
                                     cq_.get(), cq_.get(), call);
  }

  template <typename F> void Add(F f) {
    using args_type = typename function_traits<F>::args_type;
    using arg_type = typename std::remove_pointer<
        typename std::tuple_element<0, args_type>::type>::type;
  ...
  struct helper {
    ...
    template <typename T, typename V> void operator()(const T &t, V) {
  ...
  using HandlerTuple =
      decltype(GetHandlerTuple(std::make_index_sequence<kCtrlMethodNum>{}));
  ...
  HashMap<std::string, std::list<CtrlCall<CtrlMethod::kPullKV> *>>
      pending_kv_calls_;
