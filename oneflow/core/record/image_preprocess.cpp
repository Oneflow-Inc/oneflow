...
float GetRandomFloatValue(float min, float max,
                          std::function<int32_t(void)> NextRandomInt) {
  float ratio = static_cast<float>(NextRandomInt()) /
                static_cast<float>(GetMaxVal<int32_t>());
...
void ImagePreprocessImpl<PreprocessCase::kCropWithRandomSize>::DoPreprocess(
  ...
  while (max_attempts--) {
    float area_size =
        GetRandomFloatValue(area_min, area_max, NextRandomInt) * image->total();
    float aspect_ratio =
        GetRandomFloatValue(ratio_min, ratio_max, NextRandomInt);
