...
void ImagePreprocessImpl<PreprocessCase::kExpendTo>::DoPreprocess(
    cv::Mat* image, const ImagePreprocess& preprocess_conf,
    std::function<int32_t(void)> NextRandomInt) const {
  CHECK(preprocess_conf.has_expend_to());
  const int32_t width = preprocess_conf.expend_to().width();
  const int32_t height = preprocess_conf.expend_to().height();
