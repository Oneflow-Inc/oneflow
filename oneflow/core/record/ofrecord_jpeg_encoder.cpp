...
template <typename T>
void OFRecordEncoderImpl<EncodeCase::kJpeg, T>::EncodeOneCol(
  ...
  cv::Mat img =
      cv::Mat(shape.At(1), shape.At(2), type, const_cast<T *>(in_dptr)).clone();
...
OF_PP_FOR_EACH_TUPLE(INSTANTIATE_OFRECORD_JPEG_ENCODER,
