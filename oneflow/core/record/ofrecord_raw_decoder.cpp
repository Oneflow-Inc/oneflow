...
template <typename T>
void OFRecordDecoderImpl<EncodeCase::kRaw, T>::SetDim1ValidNum(
    const Feature &feature, Blob *out_blob, int64_t dim0_idx) const {
  UNIMPLEMENTED();
}
...
#define DEFINE_ONE_ELIF(PbT, CppT)                                             \
    ...
    if (padding_elem_num > 0) {                                                \
      std::memset(out_dptr + one_col_elem_num, 0,                              \
                  padding_elem_num * sizeof(T));                               \
    }                                                                          \
  }
...
OF_PP_FOR_EACH_TUPLE(INSTANTIATE_OFRECORD_RAW_DECODER,
                     ARITHMETIC_DATA_TYPE_SEQ CHAR_DATA_TYPE_SEQ)
