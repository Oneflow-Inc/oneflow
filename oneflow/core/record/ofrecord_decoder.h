...
public:
  ...
  virtual int32_t
  DecodeOneCol(DeviceCtx *, Blob *in_blob, const BlobConf &, int32_t cur_col_id,
               Blob *out_blob,
...
protected:
  ...
  virtual void ReadOneCol(DeviceCtx *, const Feature &, const BlobConf &,
                          ...
                          std::function<int32_t(void)> NextRandomInt) const = 0;
  virtual void SetDim1ValidNum(const Feature &feature, Blob *out_blob,
                               int64_t dim0_idx) const {
    UNIMPLEMENTED();
  }
  virtual void SetDim2ValidNum(const Feature &feature, Blob *out_blob,
                               int64_t dim0_idx) const {
    UNIMPLEMENTED();
  }
...
private:
  // return: max_col_num
  int32_t ReadColNum(DeviceCtx *, Blob *, const std::string &name,
                     Blob *out_blob) const;
  void ReadDataId(DeviceCtx *, Blob *in_blob, Blob *out_blob) const;
  void ReadDataContent(DeviceCtx *, Blob *in_blob, const BlobConf &,
                       int32_t col_id, Blob *out_blob,
                       std::function<int32_t(void)> NextRandomInt) const;
  void ReadPartDataContent(DeviceCtx *, Blob *in_blob, const BlobConf &,
                           int32_t col_id, Blob *out_blob, int32_t part_id,
                           int32_t part_num, int64_t one_col_elem_num,
...
template <EncodeCase encode_case, typename T> class OFRecordDecoderImpl;
