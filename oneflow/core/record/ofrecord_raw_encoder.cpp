...
template <typename T>
void EncodeDataToFeature(DeviceCtx *ctx, Feature *feature, const T *in_dptr,
                         size_t elem_num) {
  DataType data_type = GetDataType<T>();
  if (data_type == DataType::kInt8) {
    feature->mutable_bytes_list()->add_value(
        reinterpret_cast<const char *>(in_dptr), elem_num);
  } else if (data_type == DataType::kFloat16) {
    feature->mutable_bytes_list()->add_value(
        reinterpret_cast<const char *>(in_dptr), elem_num * 2);
  }
...
template <typename T>
void OFRecordEncoderImpl<EncodeCase::kRaw, T>::EncodeBlob(
    DeviceCtx *ctx, const Blob *in_blob, Feature *feature) const {
  EncodeDataToFeature(ctx, feature, in_blob->dptr<T>(),
