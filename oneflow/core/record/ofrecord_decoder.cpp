...
int32_t OFRecordDecoder<encode_case, T>::DecodeOneCol(
    DeviceCtx *ctx, Blob *in_blob, const BlobConf &blob_conf, int32_t col_id,
...
void OFRecordDecoder<encode_case, T>::ReadDataContent(
  ...
  if (part_num >= 2) {
    ...
    FOR_RANGE(int32_t, part_id, 0, part_num) {
      thread_pool->AddWork([&ctx, &in_blob, &blob_conf, &col_id, &out_blob, &bc,
                            part_id, &part_num, &one_col_elem_num, &random_seed,
                            this]() {
        ReadPartDataContent(ctx, in_blob, blob_conf, col_id, out_blob, part_id,
                            part_num, one_col_elem_num, random_seed);
        bc.Decrease();
      });
    }
    bc.WaitUntilCntEqualZero();
  } else {
    ReadPartDataContent(ctx, in_blob, blob_conf, col_id, out_blob, 0, 1,
  ...
  if (left_row_num > 0) {
    Memset<DeviceType::kCPU>(ctx,
                             out_blob->mut_dptr<T>() +
                                 record_blob.record_num() * one_col_elem_num,
