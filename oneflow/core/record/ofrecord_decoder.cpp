...
void OFRecordDecoder<encode_type, T>::ReadDataContent(
  ...
  int64_t one_row_elem_num = out_blob->shape().Count(1);
  int32_t i = 0;
  record_blob->ForEachRecord([&](const OFRecord& record) {
    const Feature& feature = record.feature().at(name);
    T* out_dptr = out_blob->dptr<T>() + i * one_row_elem_num;
    if (col_id < out_blob->col_num(i)) {
      ReadOneRow(ctx, feature, col_id, out_dptr);
    } else {
      Memset<DeviceType::kCPU>(ctx, out_dptr, 0, one_row_elem_num * sizeof(T));
    }
    i += 1;
  });
  int64_t left_row_num = out_blob->shape().At(0) - i;
  if (left_row_num > 0) {
    Memset<DeviceType::kCPU>(ctx, out_blob->dptr<T>() + i * one_row_elem_num, 0,
                             left_row_num * one_row_elem_num * sizeof(T));
  }
}

OFRecordDecoderIf* GetOFRecordDecoder(EncodeType, DataType) { return nullptr; }
