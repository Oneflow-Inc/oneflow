...
void OFRecordEncoder<encode_case, T>::EncodeOneFieldToOneRecord(DeviceCtx* ctx, int64_t record_id, const Blob* blob, const std::string& field_name, OFRecord& record) const {
    ...
    const Feature& feature = record.feature().at(field_name);
    T* in_dptr = blob->mut_dptr<T>() + record_id * one_col_elem_num;
