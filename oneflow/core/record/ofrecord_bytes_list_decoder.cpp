...
template <typename T>
void OFRecordDecoderImpl<EncodeCase::kBytesList, T>::SetDim1ValidNum(
...
template <typename T>
void OFRecordDecoderImpl<EncodeCase::kBytesList, T>::SetDim2ValidNum(
    const Feature &feature, Blob *out_blob, int64_t dim0_idx) const {
  static_assert(sizeof(T) == 1, "only char and int8_t supported");
  UNIMPLEMENTED();
}
...
void OFRecordDecoderImpl<EncodeCase::kBytesList, T>::ReadOneCol(
  ...
  int64_t shape_count2 =
      Shape(blob_conf.shape()).Count(1); // yes, Count(1) is right
