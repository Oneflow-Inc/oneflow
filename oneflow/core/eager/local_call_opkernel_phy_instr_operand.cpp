...
Maybe<void> LocalCallOpKernelPhyInstrOperand::Init() {
  mut_opkernel()->set_op_schema_for_main_thread(op_schema());
...
void LocalCallOpKernelPhyInstrOperand::ForEachMutMirroredObject(
  ...
  if (opt_transport_dep_object.has_value()) {
    DoEach(CHECK_JUST(opt_transport_dep_object)->mut_mirrored_object());
  }
  ...
  if (device->type() == "async_launched_nccl") {
    ...
    DoEach(device_schedule_dep_object->mut_mirrored_object());
  } else {
  ...
  for (int64_t index : opkernel().output_tuple_indexes4mut_obns()) {
    ...
    DoEach(CHECK_JUST(output->compute_local_dep_object())->mut_mirrored_object());
  }
}
