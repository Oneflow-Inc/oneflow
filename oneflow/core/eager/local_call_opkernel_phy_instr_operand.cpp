...
template <typename T>
void LocalCallOpKernelPhyInstrOperand<T>::ForEachConstMirroredObject(
...
template <typename T>
void LocalCallOpKernelPhyInstrOperand<T>::ForEachMutMirroredObject(
    const std::function<void(vm::MirroredObject* infer, vm::MirroredObject* compute)>& DoEach)
    const {
...
template <typename T>
void LocalCallOpKernelPhyInstrOperand<T>::ForEachMut2MirroredObject(
    const std::function<void(vm::MirroredObject* infer, vm::MirroredObject* compute)>& DoEach)
    const {
  const auto& output_list = outputs();
...
template class LocalCallOpKernelPhyInstrOperand<one::EagerBlobObjectListPtr>;
template class LocalCallOpKernelPhyInstrOperand<one::DTREagerBlobObjectListPtr>;
