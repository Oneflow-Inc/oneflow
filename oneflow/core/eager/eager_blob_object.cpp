...
EagerBlobObject::EagerBlobObject(
    ...
    const std::shared_ptr<const one::MutLocalTensorMeta>& mut_local_tensor_meta, DataType data_type,
    ...
    : is_dynamic_(false),
      ...
      blob_desc_(static_cast<bool>(mut_local_tensor_meta)
                     ...
                     : std::const_pointer_cast<Shape>(local_tensor_meta->shape_ptr()),
                 static_cast<bool>(mut_local_tensor_meta)
                     ? std::const_pointer_cast<Stride>(mut_local_tensor_meta->stride_ptr())
                     : std::const_pointer_cast<Stride>(local_tensor_meta->stride_ptr()),
                 data_type),
      local_tensor_meta_(local_tensor_meta),
      mut_local_tensor_meta_(mut_local_tensor_meta) {
  CHECK(static_cast<bool>(tensor_storage));
}

// user_op::TensorDesc overrides
const Shape& EagerBlobObject::shape() const {
  if (mut_local_tensor_meta_) {
    return mut_local_tensor_meta_->shape();
  } else {
    return local_tensor_meta_->shape();
  }
}
Shape* EagerBlobObject::mut_shape() {
  CHECK(mut_local_tensor_meta_);
  return std::const_pointer_cast<one::MutLocalTensorMeta>(mut_local_tensor_meta_)->mut_shape();
}
const Stride& EagerBlobObject::stride() const {
  if (mut_local_tensor_meta_) {
    return mut_local_tensor_meta_->stride();
  } else {
    return local_tensor_meta_->stride();
  }
}
Stride* EagerBlobObject::mut_stride() {
  CHECK(mut_local_tensor_meta_);
  return std::const_pointer_cast<one::MutLocalTensorMeta>(mut_local_tensor_meta_)->mut_stride();
}

std::shared_ptr<const Shape> EagerBlobObject::shape_ptr() const {
  if (mut_local_tensor_meta_) {
    return mut_local_tensor_meta_->shape_ptr();
  } else {
    return local_tensor_meta_->shape_ptr();
  }
}
std::shared_ptr<const Stride> EagerBlobObject::stride_ptr() const {
  if (mut_local_tensor_meta_) {
    return mut_local_tensor_meta_->stride_ptr();
  } else {
    return local_tensor_meta_->stride_ptr();
  }
}
