...
void StorageAdd(const EagerSymbol &symbol) {
  ...
  if (symbol.has_string_symbol()) {
    Global<vm::SymbolStorage<std::string>>::Get()->Add(symbol_id,
                                                       symbol.string_symbol());
  } else if (symbol.has_scope_symbol()) {
    Global<vm::SymbolStorage<Scope>>::Get()->Add(symbol_id,
                                                 symbol.scope_symbol());
  } else if (symbol.has_job_conf_symbol()) {
    Global<vm::SymbolStorage<JobDesc>>::Get()->Add(symbol_id,
                                                   symbol.job_conf_symbol());
  } else if (symbol.has_parallel_conf_symbol()) {
    Global<vm::SymbolStorage<ParallelDesc>>::Get()->Add(
        symbol_id, symbol.parallel_conf_symbol());
  } else if (symbol.has_op_conf_symbol()) {
    Global<vm::SymbolStorage<OperatorConf>>::Get()->Add(
        symbol_id, symbol.op_conf_symbol());
...
Maybe<void>
RunLogicalInstruction(const vm::InstructionListProto &instruction_list_proto,
                      const EagerSymbolList &eager_symbol_list) {
  for (const auto &eager_symbol : eager_symbol_list.eager_symbol()) {
    StorageAdd(eager_symbol);
  }
  return vm::Run(instruction_list_proto);
}

Maybe<void>
RunPhysicalInstruction(const vm::InstructionListProto &instruction_list_proto,
                       const EagerSymbolList &eager_symbol_list) {
  for (const auto &eager_symbol : eager_symbol_list.eager_symbol()) {
    StorageAdd(eager_symbol);
  }
...
RunPhysicalInstruction(const std::string &instruction_list_proto_str,
  ...
  CHECK_OR_RETURN(
      TxtString2PbMessage(instruction_list_proto_str, &instruction_list_proto))
      << "InstructionListProto parse failed";
  EagerSymbolList eager_symbol_list;
  CHECK_OR_RETURN(
      TxtString2PbMessage(eager_symbol_list_str, &eager_symbol_list))
