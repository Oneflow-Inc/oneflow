...
public:
  ...
  void Compute(vm::Instruction *instruction) const override {
    ...
    auto *blob = instruction->mut_operand_type(args->blob())
                     ->Mut<BlobObject>()
                     ->mut_blob();
    ...
    cudaError_t cuda_error =
        cudaHostRegister(dptr, size, cudaHostRegisterDefault);
...
COMMAND(vm::RegisterInstructionType<CudaHostRegisterBlobInstructionType>(
...
COMMAND(vm::RegisterInstructionType<CudaHostUnregisterBlobInstructionType>(
    "CudaHostUnregisterBlob"));
#endif

Maybe<void>
LazyReferenceInstructionType::Run(vm::Instruction *instruction) const {
  FlatMsgView<LazyReferenceInstruction> args(
      instruction->instr_msg().operand());
  vm::RwMutexedObject *eager_blob_rw =
      instruction->mut_operand_type(args->eager_blob());
  ...
  Blob *blob = Global<RegstMgr>::Get()->Blob4LbiAndParallelId(
      GenLogicalBlobId(lbn), parallel_ctx.parallel_id());
