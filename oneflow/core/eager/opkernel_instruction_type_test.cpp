...
int64_t NewJobDescSymbol(InstructionMsgList *list,
  ...
  list->EmplaceBack(vm::NewInstruction("InitJobDescSymbol")
                        ->add_init_symbol_operand(job_desc_id));
  return job_desc_id;
}

int64_t NewOpConfSymbol(InstructionMsgList *list,
  ...
  list->EmplaceBack(vm::NewInstruction("InitOperatorConfSymbol")
                        ->add_init_symbol_operand(op_conf_id));
  return op_conf_id;
}

int64_t NewOpNodeSignature(
    InstructionMsgList *list, const std::vector<std::string> &ibns,
  ...
  auto *bn_in_op2parallel_desc_symbol_id =
      op_node_signature.mutable_parallel_signature()
          ->mutable_bn_in_op2parallel_desc_symbol_id();
  auto *map = op_node_signature.mutable_sbp_signature()
                  ->mutable_bn_in_op2sbp_parallel();
  for (int i = 0; i < ibns.size(); ++i) {
    (*map)[ibns[i]].mutable_broadcast_parallel();
    (*bn_in_op2parallel_desc_symbol_id)[ibns[i]] =
        parallel_desc_symbol_ids_4_ibns[i];
    SetFakeLogicalBlobDesc(ibns[i]);
  }
  for (int i = 0; i < obns.size(); ++i) {
    (*map)[obns[i]].mutable_broadcast_parallel();
    (*bn_in_op2parallel_desc_symbol_id)[obns[i]] =
        parallel_desc_symbol_ids_4_obns[i];
    SetFakeLogicalBlobDesc(obns[i]);
  }
  int64_t op_node_signature_id = vm::TestUtil::NewSymbol(list);
  Global<vm::SymbolStorage<OpNodeSignatureDesc>>::Get()->Add(
      op_node_signature_id, op_node_signature);
...
int64_t InitOpKernelObject(InstructionMsgList *list,
  ...
  int64_t opkernel_id =
      vm::TestUtil::NewObject(list, "gpu", "0:0", &parallel_desc_id);
...
TEST(OpkernelInstructionType, new_opkernel) {
  ...
  auto vm_desc =
      ObjectMsgPtr<vm::VmDesc>::New(vm::TestUtil::NewVmResourceDesc().Get());
  vm::TestUtil::AddStreamDescByInstrNames(
      vm_desc.Mutable(), {"NewObject", "InitJobDescSymbol",
                          "InitOperatorConfSymbol", "InitOpKernelObject"});
  auto vm = ObjectMsgPtr<vm::VirtualMachine>::New(vm_desc.Get());
  vm->Receive(&list);
  while (!vm->Empty()) {
    vm->Schedule();
    OBJECT_MSG_LIST_FOR_EACH_PTR(vm->mut_thread_ctx_list(), t) {
      t->TryReceiveAndRun();
    }
  }
}
...
TEST(OpkernelInstructionType, delete_opkernel) {
  ...
  list.EmplaceBack(
      vm::NewInstruction("DeleteOpKernelObject")->add_mut_operand(opkernel_id));
  auto vm_desc =
      ObjectMsgPtr<vm::VmDesc>::New(vm::TestUtil::NewVmResourceDesc().Get());
  vm::TestUtil::AddStreamDescByInstrNames(
      vm_desc.Mutable(), {"NewObject", "InitJobDescSymbol",
...
TEST(OpkernelInstructionType, call_opkernel) {
  ...
  int64_t parallel_desc_id = 0;
  int64_t output_blob_id =
      vm::TestUtil::NewObject(&list, "gpu", "0:0", &parallel_desc_id);
  int64_t op_node_signature_id =
      NewOpNodeSignature(&list, {}, {}, {"out_0"}, {parallel_desc_id});
...
TEST(OpkernelInstructionType, consecutive_opkernel_calls) {
  ...
  int64_t y = 0;
  {
    ...
    int64_t tmp_buffer =
        vm::TestUtil::NewObject(&list, "gpu", "0:0", &y_parallel_desc_id);
    int64_t op_node_signature_id = NewOpNodeSignature(
        &list, {"in_0"}, {x_parallel_desc_id}, {"out_0", "tmp_buffer_0"},
...
TEST(OpkernelInstructionType, stateless_call_opkernel) {
  ...
  InstructionMsgList list;
  int64_t job_desc_id =
      NewJobDescSymbol(&list, std::make_shared<JobConfigProto>());
  int64_t parallel_desc_id = 0;
  int64_t opkernel_id =
      vm::TestUtil::NewObject(&list, "gpu", "0:0", &parallel_desc_id);
...
TEST(OpkernelInstructionType, consecutive_stateless_call_opkernel) {
  ...
  int64_t tmp_buffer =
      vm::TestUtil::NewObject(&list, "gpu", "0:0", &y_parallel_desc_id);
  op_node_signature_id = NewOpNodeSignature(
      &list, {"in_0"}, {parallel_desc_id}, {"out_0", "tmp_buffer_0"},
      {y_parallel_desc_id, y_parallel_desc_id});
  list.EmplaceBack(vm::NewInstruction("gpu.compute.UserStatelessCallOpKernel")
                       ->add_parallel_desc(y_parallel_desc_id)
                       ->add_symbol_operand(job_desc_id)
