...
public:
  ...
  void Infer(vm::Instruction *instruction) const override {
    ...
    const auto &job_desc_object =
        *CHECK_JUST(operand_job_desc->Get<vm::ObjectWrapper<JobDesc>>());
    for (int i = 0; i < view->op_size(); ++i) {
      ...
      vm::RwMutexedObject *rw_mutexed_object =
          instruction->mut_operand_type(view->op(i));
      ...
      rw_mutexed_object->Init<OpKernelObject>(
          op_conf_object.Get(), job_desc_object.GetPtr(), device_type);
    }
  }
...
COMMAND(vm::RegisterInstructionType<InitOpKernelObjectInstructionType>(
    "InitOpKernelObject"));
...
COMMAND(vm::RegisterInstructionType<DeleteOpKernelObjectInstructionType>(
    "DeleteOpKernelObject"));

namespace {

std::shared_ptr<MemoryCase> MakeMemCase(const DeviceType device_type,
...
Maybe<void> ForEachIbnAndBlobObject(vm::Instruction *instruction, const T &args,
  ...
  FOR_RANGE(int, i, 0, args.ibn_size()) {
    ...
    const std::string &bn_in_op =
        JUST(operand_ibn->template Get<vm::StringObject>())->str();
    const auto *operand_input_blob =
        instruction->operand_type(args.input_blob(i));
    CHECK_NOTNULL_OR_RETURN(operand_input_blob)
        << "bn_in_op: " << bn_in_op
        << ", object_id: " << args.input_blob(i).logical_object_id();
    const auto &blob_object =
        *JUST(operand_input_blob->template Get<BlobObject>());
...
Maybe<void> ForEachObnAndBlobObject(vm::Instruction *instruction, const T &args,
  ...
  FOR_RANGE(int, i, 0, args.obn_size()) {
    ...
    const std::string &bn_in_op =
        JUST(operand_obn->template Get<vm::StringObject>())->str();
    auto *operand_output_blob =
        instruction->mut_operand_type(args.output_blob(i));
  ...
  FOR_RANGE(int, i, 0, args.mut2_obn_size()) {
    ...
    const std::string &bn_in_op =
        JUST(operand_obn->template Get<vm::StringObject>())->str();
    auto *operand_output_blob =
        instruction->mut_operand_type(args.mut2_output_blob(i));
    CHECK_NOTNULL_OR_RETURN(operand_output_blob) << "obn: " << bn_in_op;
    auto *blob_object = operand_output_blob->template Mut<BlobObject>();
    JUST(Callback(bn_in_op, blob_object));
  }
...
template <typename T>
Maybe<void> MakeBlobDesc4BnInOp(
    vm::Instruction *instruction, const T &args,
    std::function<BlobDesc *(const std::string &)> *BlobDesc4BnInOp) {
  const auto &obn2blob_desc =
      std::make_shared<HashMap<std::string, BlobDesc *>>();
  {
    HashSet<const BlobDesc *> out_blob_descs;
    JUST(ForEachObnAndBlobObject(
        instruction, args, [&](const std::string &bn_in_op,
                               BlobObject *blob_object) -> Maybe<void> {
  ...
  const auto &ibn2blob_desc =
      std::make_shared<HashMap<std::string, const BlobDesc *>>();
  JUST(ForEachIbnAndBlobObject(
      instruction, args, [&](const std::string &bn_in_op,
                             const BlobObject &blob_object) -> Maybe<void> {
        CHECK_OR_RETURN(
            ibn2blob_desc->emplace(bn_in_op, &blob_object.blob_desc()).second);
        return Maybe<void>::Ok();
      }));
  *BlobDesc4BnInOp = [obn2blob_desc, ibn2blob_desc](
      const std::string &bn_in_op) -> BlobDesc * {
    const auto &output_iter = obn2blob_desc->find(bn_in_op);
    if (output_iter != obn2blob_desc->end()) {
      return output_iter->second;
    }
    const auto &input_iter = ibn2blob_desc->find(bn_in_op);
    if (input_iter != ibn2blob_desc->end()) {
      return const_cast<BlobDesc *>(input_iter->second);
    }
    return nullptr;
  };
  return Maybe<void>::Ok();
...
template <typename T>
Maybe<void>
MakeBlob4BnInOp(vm::Instruction *instruction, const T &args,
                std::function<Blob *(const std::string &)> *Blob4BnInOp,
                const std::function<bool(const std::string &,
                                         const BlobObject &)> &FilterOutBlob) {
  const auto &obn2blob = std::make_shared<HashMap<std::string, Blob *>>();
  JUST(ForEachObnAndBlobObject(
      instruction, args,
      [&](const std::string &bn_in_op, BlobObject *blob_object) -> Maybe<void> {
        if (!FilterOutBlob(bn_in_op, *blob_object)) {
          return Maybe<void>::Ok();
        }
        CHECK_OR_RETURN(
            obn2blob->emplace(bn_in_op, blob_object->mut_blob()).second);
        return Maybe<void>::Ok();
      }));
...
MakeBlob4BnInOp(vm::Instruction *instruction, const T &args,
                std::function<Blob *(const std::string &)> *Blob4BnInOp) {
  return MakeBlob4BnInOp(
      instruction, args, Blob4BnInOp,
...
void InitOutputBlobObjects(vm::Instruction *instruction, const T &args,
                           ...
                           DataType data_type) {
  const auto &InitRwMutexedObject =
      [&](vm::RwMutexedObject *rw_mutexed_object) {
        const auto &parallel_desc = instruction->parallel_desc();
        CHECK(static_cast<bool>(parallel_desc));
...
Maybe<void> CheckBlobParallel(vm::Instruction *instruction, const T &args,
  ...
  const auto &bn_in_op2parallel_desc_symbol_id =
      op_node_signature->parallel_signature()
          .bn_in_op2parallel_desc_symbol_id();

  const auto &ParallelDesc4BnInOp =
      [&](const std::string &bn_in_op) -> Maybe<const ParallelDesc *> {
    const auto &iter = bn_in_op2parallel_desc_symbol_id.find(bn_in_op);
    // TODO(Liang Depeng): should not tolerate nullptr.
    if (iter == bn_in_op2parallel_desc_symbol_id.end()) {
      return nullptr;
    }
  ...
  JUST(ForEachObnAndBlobObject(
      ...
      [&](const std::string &bn_in_op, BlobObject *blob_object) -> Maybe<void> {
        ...
        JUST(blob_object->CheckMemCase(*parallel_desc,
                                       instruction->stream().machine_id()));
        return Maybe<void>::Ok();
      }));
...
template <typename T>
Maybe<void> OpKernelInfer(OpKernelObject *opkernel_obj,
                          vm::Instruction *instruction, const T &args,
                          const std::shared_ptr<MemoryCase> &mem_case) {
  {
  ...
  JUST(instruction->parallel_desc()->GetParallelContext(
      &parallel_ctx, instruction->stream().machine_id(),
      instruction->stream().device_id()));
  JUST(opkernel_obj->ResetOpAndKernel(*op_node_signature, &parallel_ctx,
                                      BlobDesc4BnInOp,
  ...
  const auto &FilterOutBlob = [&](const std::string &bn_in_op,
                                  const BlobObject &blob_object) {
    return !(bn_in_op == "tmp_buffer_0" &&
             blob_object.blob_desc().shape() == empty_shape);
  };
...
Maybe<void> OpKernelInfer(SystemOpKernelObject *opkernel_obj,
  ...
  JUST(instruction->parallel_desc()->GetParallelContext(
      &parallel_ctx, instruction->stream().machine_id(),
      instruction->stream().device_id()));
  JUST(opkernel_obj->ResetKernel(*op_node_signature, &parallel_ctx,
                                 BlobDesc4BnInOp,
                                 instruction->parallel_desc().get()));
  JUST(CheckBlobParallel(instruction, args, op_node_signature));
  JUST(ForEachObnAndBlobObject(
...
template <typename T>
Maybe<void> OpKernelCompute(OpKernelObject *opkernel_obj,
                            vm::Instruction *instruction, const T &args) {
...
Maybe<void> OpKernelCompute(SystemOpKernelObject *opkernel_obj,
                            vm::Instruction *instruction,
                            const StatelessCallOpKernelInstrOperand &args) {
  DeviceCtx *device_ctx = instruction->stream().device_ctx().get();
  JUST(ForEachObnAndBlobObject(
...
template <typename T>
Maybe<T *> GetSharedOpKernel(vm::Instruction *instruction,
  ...
  const auto &job_desc_ptr =
      JUST(operand_job_desc->Get<vm::ObjectWrapper<JobDesc>>())->GetPtr();
  const auto *operand_op_conf = instruction->mut_operand_type(args.op_conf());
  const auto &op_conf =
      JUST(operand_op_conf->Get<vm::ObjectWrapper<OperatorConf>>())->Get();
  vm::RwMutexedObject *rw_mutexed_object =
      instruction->mut_operand_type(args.shared_opkernel());
  CHECK_OR_RETURN(!rw_mutexed_object->has_object() ||
                  rw_mutexed_object->Has<OpKernelObject>() ||
...
Maybe<void> CallOpKernelInstructionType::MaybeInfer(
    vm::Instruction *instruction, const CallOpKernelInstrOperand &args) const {
  auto *opkernel_obj =
      instruction->mut_operand_type(args.opkernel())->Mut<OpKernelObject>();
...
void CallOpKernelInstructionType::Infer(vm::Instruction *instruction) const {
  FlatMsgView<CallOpKernelInstrOperand> args(
...
Maybe<void> CallOpKernelInstructionType::MaybeCompute(
    vm::Instruction *instruction, const CallOpKernelInstrOperand &args) const {
  auto *opkernel_obj =
      instruction->mut_operand_type(args.opkernel())->Mut<OpKernelObject>();
...
Maybe<const OperatorConf &>
GetOpConf(vm::Instruction *instruction,
          const StatelessCallOpKernelInstrOperand &args) {
...
Maybe<void> UserStatelessCallOpKernelInstructionType::Infer(
  ...
  int64_t device_id = instruction->stream().device_id();
  auto *opkernel =
      JUST(GetSharedOpKernel<OpKernelObject>(instruction, device_type, args));
  const auto &mem_case = MakeMemCase(device_type, device_id);
  JUST(OpKernelInfer(opkernel, instruction, args, mem_case));
  return Maybe<void>::Ok();
}

void UserStatelessCallOpKernelInstructionType::Infer(
    vm::Instruction *instruction) const {
  FlatMsgView<StatelessCallOpKernelInstrOperand> args(
      instruction->instr_msg().operand());
...
Maybe<void> UserStatelessCallOpKernelInstructionType::Compute(
    vm::Instruction *instruction,
    const StatelessCallOpKernelInstrOperand &args) const {
  auto *opkernel_obj = instruction->mut_operand_type(args.shared_opkernel())
                           ->Mut<OpKernelObject>();
  JUST(OpKernelCompute(opkernel_obj, instruction, args));
  return Maybe<void>::Ok();
}

void UserStatelessCallOpKernelInstructionType::Compute(
    vm::Instruction *instruction) const {
  FlatMsgView<StatelessCallOpKernelInstrOperand> args(
      instruction->instr_msg().operand());
...
SystemStatelessCallOpKernelInstructionType::GetOutBlobMemCase(
...
Maybe<void> SystemStatelessCallOpKernelInstructionType::Infer(
  ...
  int64_t device_id = instruction->stream().device_id();
  auto *opkernel = JUST(
      GetSharedOpKernel<SystemOpKernelObject>(instruction, device_type, args));
  const auto &mem_case = GetOutBlobMemCase(device_type, device_id);
  JUST(OpKernelInfer(opkernel, instruction, args, mem_case));
  return Maybe<void>::Ok();
}

void SystemStatelessCallOpKernelInstructionType::Infer(
    vm::Instruction *instruction) const {
  FlatMsgView<StatelessCallOpKernelInstrOperand> args(
      instruction->instr_msg().operand());
  CHECK_OK(Infer(instruction, args.Get()))
...
Maybe<void> SystemStatelessCallOpKernelInstructionType::Compute(
    ...
    const StatelessCallOpKernelInstrOperand &args) const {
  auto *opkernel_obj = instruction->mut_operand_type(args.shared_opkernel())
                           ->Mut<SystemOpKernelObject>();
  JUST(OpKernelCompute(opkernel_obj, instruction, args));
  return Maybe<void>::Ok();
}

void SystemStatelessCallOpKernelInstructionType::Compute(
    vm::Instruction *instruction) const {
  FlatMsgView<StatelessCallOpKernelInstrOperand> args(
      instruction->instr_msg().operand());
  CHECK_OK(Compute(instruction, args.Get()))
...
template <typename T> void FeedOrFetchBlob(vm::Instruction *instruction) {
  ...
  Global<ForeignCallback>::Get()->OfBlobCall(args->unique_callback_id(),
                                             of_blob_ptr);
}
