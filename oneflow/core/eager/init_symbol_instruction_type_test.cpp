...
void TestInitSymbolInstructionType(const std::string &instr_type_name) {
  ...
  list.EmplaceBack(
      vm::NewInstruction("NewSymbol")->add_int64_operand(symbol_id));
  list.EmplaceBack(
      vm::NewInstruction(instr_type_name)->add_init_symbol_operand(symbol_id));
  vm->Receive(&list);
  while (!vm->Empty()) {
    vm->Schedule();
  ...
  auto *logical_object =
      vm->mut_id2logical_object()->FindPtr(vm::IdUtil::GetTypeId(symbol_id));
...
TEST(InitSymbolInstructionType, operator_conf) {
  vm::TestResourceDescScope resource_scope(1, 1);
  TestInitSymbolInstructionType<OperatorConf, OperatorConf>(
