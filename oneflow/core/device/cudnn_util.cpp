...
CudnnTensorDesc::~CudnnTensorDesc() {
  OF_CUDNN_CHECK(cudnnDestroyTensorDescriptor(val_));
}
...
CudnnTensorDesc::CudnnTensorDesc(DataType data_type, int dims, const int *dim,
                                 const int *stride) {
  OF_CUDNN_CHECK(cudnnCreateTensorDescriptor(&val_));
  OF_CUDNN_CHECK(cudnnSetTensorNdDescriptor(val_, GetCudnnDataType(data_type),
                                            dims, dim, stride));
}
CudnnTensorDesc::CudnnTensorDesc(DataType data_type, const ShapeView &shape,
  ...
  if (shape.NumAxes() == 3) {
    ...
    int kernel_w = 1;
    OF_CUDNN_CHECK(cudnnSetTensor4dDescriptor(
        val_, cudnn_data_format, GetCudnnDataType(data_type), data_num,
    ...
    OF_CUDNN_CHECK(cudnnSetTensorNdDescriptor(
        val_, GetCudnnDataType(data_type), shape.NumAxes(), tensor_dim.data(),
        stride_of_tensor.data()));
  }
}

CudnnFilterDesc::~CudnnFilterDesc() {
  OF_CUDNN_CHECK(cudnnDestroyFilterDescriptor(val_));
}
...
CudnnFilterDesc::CudnnFilterDesc(DataType data_type, const ShapeView &shape,
  ...
  if (shape.NumAxes() == 3) {
    ...
    int kernel_h = data_format == "channels_first"
                       ...
                       : static_cast<int>(shape.At(1));
    int kernel_w = 1;
    OF_CUDNN_CHECK(cudnnSetFilter4dDescriptor(val_, GetCudnnDataType(data_type),
    ...
    OF_CUDNN_CHECK(cudnnSetFilterNdDescriptor(val_, GetCudnnDataType(data_type),
...
template <typename T> const void *CudnnSPOnePtr() {
  ...
  const void *ret = std::is_same<T, double>::value
                        ? static_cast<const void *>(&dval)
