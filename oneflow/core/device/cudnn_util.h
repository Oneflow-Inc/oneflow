...
template <typename T> struct CudnnDataType;
...
public:
  ...
  CudnnTensorDesc(DataType data_type, int dims, const int *dim,
                  const int *stride);
  CudnnTensorDesc(DataType data_type, const ShapeView &shape,
...
template <typename T> const void *CudnnSPOnePtr();

template <typename T> const void *CudnnSPZeroPtr();
