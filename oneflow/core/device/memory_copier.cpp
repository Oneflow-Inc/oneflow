...
int64_t MemoryCopyNdDescGetNumAxes(const MemoryCopyNdDesc &desc) {
  return desc.extent.NumAxes();
}

void CheckPosExtent(const int64_t num_axes, const Shape &shape,
                    const NdIndex &pos, const Shape &extent) {
...
void CopyNDCpuImpl(DeviceCtx *ctx, void *dst, const void *src,
  ...
  FOR_RANGE(int64_t, i, 0, desc.extent.elem_cnt()) {
    ...
    unsigned char *dst_ptr =
        reinterpret_cast<unsigned char *>(dst) + dst_offset;
    const unsigned char *src_ptr =
        reinterpret_cast<const unsigned char *>(src) + src_offset;
    *dst_ptr = *src_ptr;
  }
}
...
void MemoryCopier::Copy(DeviceCtx *ctx, void *dst, const void *src,
  ...
  if (num_axes == 1) {
    ...
    void *dst_2d = (unsigned char *)dst + desc.dst_pos.At(0) * dst_pitch +
                   desc.dst_pos.At(1);
    const void *src_2d = (const unsigned char *)src +
                         desc.src_pos.At(0) * src_pitch + desc.src_pos.At(1);
...
void MemoryCopier::Copy2D(DeviceCtx *ctx, void *dst, size_t dst_pitch,
...
void MemoryCopier::Copy3D(DeviceCtx *ctx, void *dst, const void *src,
  ...
  FOR_RANGE(size_t, i, 0, depth) {
    void *dst_2d = (unsigned char *)dst +
                   (desc.dst_pos.At(0) + i) * dst_inner_area +
                   desc.dst_pos.At(1) * dst_pitch + desc.dst_pos.At(2);
    const void *src_2d = (unsigned char *)src +
                         (desc.src_pos.At(0) + i) * src_inner_area +
...
void HostMemoryCopier::Copy1D(DeviceCtx *ctx, void *dst, const void *src,
                              size_t count) const {
...
void CudaAsyncMemoryCopier::Copy1D(DeviceCtx *ctx, void *dst, const void *src,
                                   size_t count) const {
  OF_CUDA_CHECK(
      cudaMemcpyAsync(dst, src, count, cudaMemcpyDefault, ctx->cuda_stream()));
}

void CudaAsyncMemoryCopier::Copy2D(DeviceCtx *ctx, void *dst, size_t dst_pitch,
                                   const void *src, size_t src_pitch,
                                   size_t width, size_t height) const {
  OF_CUDA_CHECK(cudaMemcpy2DAsync(dst, dst_pitch, src, src_pitch, width, height,
...
void CudaAsyncMemoryCopier::Copy3D(DeviceCtx *ctx, void *dst, const void *src,
  ...
  params.srcPos =
      make_cudaPos(desc.src_pos.At(2), desc.src_pos.At(1), desc.src_pos.At(0));
  params.srcPtr =
      make_cudaPitchedPtr(const_cast<void *>(src), desc.src_shape.At(2),
                          desc.src_shape.At(2), desc.src_shape.At(1));
  params.dstPos =
      make_cudaPos(desc.dst_pos.At(2), desc.dst_pos.At(1), desc.dst_pos.At(0));
  params.dstPtr = make_cudaPitchedPtr(
      dst, desc.dst_shape.At(2), desc.dst_shape.At(2), desc.dst_shape.At(1));
  params.extent =
      make_cudaExtent(desc.extent.At(2), desc.extent.At(1), desc.extent.At(0));
...
REGISTER_DEFAULT_MEMORY_COPIER(DeviceType::kCPU,
                               []() { return new HostMemoryCopier(); });
#ifdef WITH_CUDA
REGISTER_DEFAULT_MEMORY_COPIER(DeviceType::kGPU,
                               []() { return new CudaAsyncMemoryCopier(); });
#endif
...
#define SPECIALIZE_COPY_ELEM(dtype)                                            \
  template void MemoryCopier::CopyElem<dtype>(                                 \
      DeviceCtx * ctx, void *dst, const void *src,                             \
      const MemoryCopyNdDesc &desc) const;
...
#define SPECIALIZE_COPY_ND_CPU_IMPL(NDIMS)                                     \
  template void CopyNDCpuImpl<NDIMS>(DeviceCtx * ctx, void *dst,               \
                                     const void *src,                          \
                                     const MemoryCopyNdDesc &desc);
