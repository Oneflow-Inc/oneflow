...
void InitGlobalCudaDeviceProp() {
  CHECK(Global<cudaDeviceProp>::Get() == nullptr)
      << "initialized Global<cudaDeviceProp> twice";
  Global<cudaDeviceProp>::New();
  cudaGetDeviceProperties(Global<cudaDeviceProp>::Get(), 0);
  if (IsCuda9OnTuringDevice()) {
    LOG(WARNING) << "CUDA 9 running on Turing device has known issues, "
                    "consider upgrading to CUDA 10";
  }
}

int32_t GetSMCudaMaxBlocksNum() {
  const auto &global_device_prop = *Global<cudaDeviceProp>::Get();
  int32_t n = global_device_prop.multiProcessorCount *
              global_device_prop.maxThreadsPerMultiProcessor;
  return (n + kCudaThreadsNumPerBlock - 1) / kCudaThreadsNumPerBlock;
}

bool IsCuda9OnTuringDevice() {
  const auto &global_device_prop = *Global<cudaDeviceProp>::Get();
  return CUDA_VERSION >= 9000 && CUDA_VERSION < 9020 &&
         global_device_prop.major == 7 && global_device_prop.minor == 5;
}

template <> void CudaCheck(cudaError_t error) {
  CHECK_EQ(error, cudaSuccess) << cudaGetErrorString(error);
}

template <> void CudaCheck(cudnnStatus_t error) {
  CHECK_EQ(error, CUDNN_STATUS_SUCCESS) << cudnnGetErrorString(error);
}

template <> void CudaCheck(cublasStatus_t error) {
  CHECK_EQ(error, CUBLAS_STATUS_SUCCESS) << CublasGetErrorString(error);
}

template <> void CudaCheck(curandStatus_t error) {
...
void ParseCpuMask(const std::string &cpu_mask, cpu_set_t *cpu_set) {
  ...
  for (int64_t i = masks.size() - 1; i >= 0; i--) {
    for (uint64_t b = 0; b < 32; b++) {
      if ((masks.at(i) & (1UL << b)) != 0) {
        CPU_SET_S(cpu, sizeof(cpu_set_t), cpu_set);
      }
      cpu += 1;
    }
  }
...
std::string CudaDeviceGetCpuMask(int32_t dev_id) {
  ...
  OF_CUDA_CHECK(cudaDeviceGetPCIBusId(
      pci_bus_id_buf.data(), static_cast<int>(pci_bus_id_buf.size()), dev_id));
  for (int32_t i = 0; i < pci_bus_id_buf.size(); ++i) {
    pci_bus_id_buf[i] = std::tolower(pci_bus_id_buf[i]);
  }
  const std::string pci_bus_id(pci_bus_id_buf.data(),
                               pci_bus_id_buf.size() - 1);
  const std::string pci_bus_id_short =
      pci_bus_id.substr(0, sizeof("0000:00") - 1);
  const std::string local_cpus_file = "/sys/class/pci_bus/" + pci_bus_id_short +
                                      "/device/" + pci_bus_id + "/local_cpus";
...
#define MAKE_ENTRY(type_cpp, type_cuda)                                        \
  if (val == GetDataType<type_cpp>::value) {                                   \
    return type_cuda;                                                          \
  }
...
CudaCurrentDeviceGuard::~CudaCurrentDeviceGuard() {
  OF_CUDA_CHECK(cudaSetDevice(saved_dev_id_));
}
