...
template <int32_t NDIMS, typename I> struct SOA { I val[NDIMS]; };
...
__global__ void CopyNDGpu(const int n, T *dst, const T *src,
                          ...
                          NdIndexOffsetHelper<I, NDIMS> copy_helper,
                          SOA<NDIMS, I> dst_pos, SOA<NDIMS, I> src_pos) {
...
size_t GetPackSize(const MemoryCopyNdDesc &desc, const void *dst,
                   const void *src) {
  const int64_t mask =
      desc.src_shape.dim_vec().back() | desc.dst_shape.dim_vec().back() |
...
void CopyNDByPackByIndexTypeGpu(DeviceCtx *ctx, void *dst, const void *src,
  ...
  CopyNDGpu<NDIMS, P, I><<<BlocksNum4ThreadsNum(elem_cnt),
                           kCudaThreadsNumPerBlock, 0, ctx->cuda_stream()>>>(
      elem_cnt, reinterpret_cast<P *>(dst), reinterpret_cast<const P *>(src),
      dst_helper, src_helper, copy_helper, dst_pos, src_pos);
}

template <int32_t NDIMS, typename P>
void CopyNDByPackGpu(DeviceCtx *ctx, void *dst, const void *src,
                     const MemoryCopyNdDesc &desc) {
...
void CopyNDGpuImpl(DeviceCtx *ctx, void *dst, const void *src,
                   const MemoryCopyNdDesc &desc) {
...
#define SPECIALIZE_COPY_ND_GPU_IMPL(NDIMS)                                     \
  template void CopyNDGpuImpl<NDIMS>(DeviceCtx * ctx, void *dst,               \
                                     const void *src,                          \
                                     const MemoryCopyNdDesc &desc);
