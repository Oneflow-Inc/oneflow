...
class HipDeviceDescriptorClass : public DeviceDescriptorClass {
 public:
  HipDeviceDescriptorClass() = default;
  ~HipDeviceDescriptorClass() override = default;
  ...
  std::shared_ptr<const DeviceDescriptorList> QueryDeviceDescriptorList() const override {
    ...
    for (int dev = 0; dev < n_dev; ++dev) { devices.at(dev) = HipDeviceDescriptor::Query(dev); }
    return std::make_shared<const BasicDeviceDescriptorList>(devices);
  }

  std::string Name() const override { return kHipDeviceDescriptorClassName; }
  ...
  std::shared_ptr<const DeviceDescriptorList> DeserializeDeviceDescriptorList(
    ...
    for (int i = 0; i < serialized_devices.size(); ++i) {
      devices.at(i) = HipDeviceDescriptor::Deserialize(serialized_devices.at(i));
    }
    return std::make_shared<const BasicDeviceDescriptorList>(devices);
  }
...
COMMAND(DeviceDescriptorClass::RegisterClass(std::make_shared<HipDeviceDescriptorClass>()));
