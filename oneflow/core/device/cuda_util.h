...
#define OF_CUDA_CHECK(condition)                                               \
  for (cudaError_t _of_cuda_check_status = (condition);                        \
       _of_cuda_check_status != cudaSuccess;)                                  \
  LOG(FATAL) << "Check failed: " #condition " : "                              \
             << cudaGetErrorString(_of_cuda_check_status) << " ("              \
...
#define OF_CUDNN_CHECK(condition)                                              \
  ...
  LOG(FATAL) << "Check failed: " #condition " : "                              \
             << cudnnGetErrorString(_of_cudnn_check_status) << " ("            \
...
#define OF_CUBLAS_CHECK(condition)                                             \
  ...
  LOG(FATAL) << "Check failed: " #condition " : "                              \
             << CublasGetErrorString(_of_cublas_check_status) << " ("          \
...
#define OF_CURAND_CHECK(condition)                                             \
  ...
  LOG(FATAL) << "Check failed: " #condition " : "                              \
             << CurandGetErrorString(_of_curand_check_status) << " ("          \
             << _of_curand_check_status << ") "

#define OF_NCCL_CHECK(condition)                                               \
  for (ncclResult_t _of_nccl_check_status = (condition);                       \
       _of_nccl_check_status != ncclSuccess;)                                  \
  LOG(FATAL) << "Check failed: " #condition " : "                              \
             << ncclGetErrorString(_of_nccl_check_status) << " ("              \
             << _of_nccl_check_status << ") "

template <typename T> void CudaCheck(T error);
...
inline size_t GetCudaWorkTypeSize() {
  return OF_PP_SEQ_SIZE(CUDA_WORK_TYPE_SEQ);
}
...
#define SPECIALIZE_CUDA_DATA_TYPE(type_cpp, type_cuda)                         \
  template <>                                                                  \
  struct CudaDataType<type_cpp>                                                \
      : std::integral_constant<cudaDataType_t, type_cuda> {};
