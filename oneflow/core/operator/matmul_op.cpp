...
void MatmulOp::GenerateBackwardOpConf(
  ...
  const MatmulOpConf& conf = op_conf().matmul_conf();
  {
    ...
    if (conf.transpose_a()) {
      matmul_a_op_conf->set_a(GenLogicalBlobName(BnInOp2Lbi("b")));
  ...
  }
  {
    ...
    if (conf.transpose_b()) {
      ...
      matmul_b_op_conf->set_b(GenLogicalBlobName(BnInOp2Lbi("a")));
      matmul_b_op_conf->set_transpose_a(true);
      matmul_b_op_conf->set_transpose_b(conf.transpose_a());
    } else {
      matmul_b_op_conf->set_a(GenLogicalBlobName(BnInOp2Lbi("a")));
