...
void GetwindowedOutputSizeWithPaddingNeeded(int64_t input_size, int32_t filter_size,
                                            int32_t dilation_rate, int32_t stride,
                                            const int32_t padding_needed, int64_t* output_size,
                                            int32_t* padding_before, int32_t* padding_after) {
  CHECK_GT(stride, 0);
  CHECK_GE(dilation_rate, 1);
...
void GetConvOutAndPad(const ShapeView& in_blob_shape, const PbMessage& conv_conf, DimVector* out,
  ...
  if (TryGetMsgPtrFromPbMessage(conv_conf, "torch_style_padding_conf")) {
    const PbMessage& torch_style_padding_conf =
        GetMessageInPbMessage(conv_conf, "torch_style_padding_conf");
    const PbRf<int32_t>& padding_needed =
        GetPbRfFromPbMessage<int32_t>(torch_style_padding_conf, "padding_needed");
    FOR_RANGE(int32_t, i, 0, opkernel_dim) {
      GetwindowedOutputSizeWithPaddingNeeded(
          in_blob_shape.At(DhwOffset(data_format) + i), kernel_size.Get(i), dilation_rate.Get(i),
          strides.Get(i), padding_needed.Get(i), out ? &(out->at(i)) : nullptr,
