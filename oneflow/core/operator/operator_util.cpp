    ...
    if (output_size) {
      *output_size =
          (input_size + *padding_before + *padding_after -
           effective_filter_size + stride + (ceil_mode ? stride - 1 : 0)) /
          stride;
...
void Get3DOutputSize(const DimVector &in, const std::vector<int32_t> &pool_size,
  ...
  FOR_RANGE(size_t, i, 0, 3) {
    ...
    if (dilation_rate) {
      GetWindowedOutputSize(in.at(i), pool_size.at(i), dilation_rate->at(i),
                            strides.at(i), padding_type, ceil_mode, out_ptr,
                            &(padding_before->at(i)), &(padding_after->at(i)));
    } else {
      GetWindowedOutputSize(in.at(i), pool_size.at(i), 1, strides.at(i),
                            padding_type, ceil_mode, out_ptr,
                            &(padding_before->at(i)), &(padding_after->at(i)));
    }
  }
}
...
void GetConvOutAndPad(const ShapeView &in_blob_shape,
  ...
  if (out) {
    out->assign(opkernel_dim, 0);
  }
  if (pad_small_side) {
    pad_small_side->assign(opkernel_dim, 0);
  }
  if (pad_large_side) {
    pad_large_side->assign(opkernel_dim, 0);
  }
...
void GetConvOutAndPad(const ShapeView &in_blob_shape,
  ...
  FOR_RANGE(int32_t, i, 0, opkernel_dim) {
    GetWindowedOutputSize(in_blob_shape.At(DhwOffset(data_format) + i),
                          kernel_size.at(i), dilation_rate.at(i), strides.at(i),
                          padding, out ? &(out->at(i)) : nullptr,
                          pad_small_side ? &(pad_small_side->at(i)) : nullptr,
                          pad_large_side ? &(pad_large_side->at(i)) : nullptr);
  }
}
