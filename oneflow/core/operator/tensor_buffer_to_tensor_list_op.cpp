...
public:
  ...
  Maybe<void> InferBlobDescs(
    ...
    const ShapeProto &shape =
        op_conf().tensor_buffer_to_tensor_list_conf().shape();
    ...
    out_desc->set_data_type(
        op_conf().tensor_buffer_to_tensor_list_conf().data_type());
...
REGISTER_CPU_OP(OperatorConf::kTensorBufferToTensorListConf,
                TensorBufferToTensorListOp);
