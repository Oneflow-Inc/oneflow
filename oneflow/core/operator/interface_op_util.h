...
struct InterfaceOpUtil final {
  static Maybe<void> InferOutBlobDesc(const InterfaceBlobConf &blob_conf,
                                      BlobDesc *out_blob_desc,
                                      const ParallelContext *parallel_ctx);
  static Maybe<void> InferBatchAxis(const InterfaceBlobConf &blob_conf,
                                    OptInt64 *batch_axis);
  static Maybe<void> GetInputLikeOpSbpSignature(
  ...
  static Maybe<void> GetOutputLikeOpSbpSignature(
      const InterfaceBlobConf &blob_conf, const PbRpf<std::string> &input_bns,
      const PbRpf<std::string> &output_bns, SbpSignature *sbp_signature);
