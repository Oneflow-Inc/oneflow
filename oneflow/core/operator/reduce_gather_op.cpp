...
void ReduceGatherOp::InferBlobDescs(std::function<BlobDesc*(const std::string&)> GetBlobDesc4BnInOp,
  ...
  *out_blob = *first_in_blob;
  int64_t out_blob_elem_cnt = first_in_blob->body_desc().shape().elem_cnt();
  for (int32_t i = 1; i < in_num; ++i) {
    out_blob_elem_cnt += GetBlobDesc4BnInOp(input_bns().Get(i))->body_desc().shape().elem_cnt();
  }
  out_blob->mut_body_desc().mut_shape() = Shape({out_blob_elem_cnt});
}
