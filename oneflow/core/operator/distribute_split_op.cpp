...
public:
  ...
  LogicalNode *NewProperLogicalNode() const override {
    return new DistributeSplitLogicalNode;
  }
...
private:
  ...
  Maybe<void> InferOutParallelDesc(
      ...
      const ParallelDesc &, const SbpSignature *) const override;
...
Maybe<void> DistributeSplitOp::InferBlobDescs(
  ...
  if (parallel_ctx->parallel_num() > 1) {
    ...
    auto *out_blob_desc =
        GetBlobDesc4BnInOp(output_bns().Get(parallel_ctx->parallel_id()));
  ...
  FOR_RANGE(int, i, 0, output_bns().size()) {
    ...
    if (blob_desc != nullptr) {
      out_blob_descs.push_back(blob_desc);
    }
  }
  BalancedSplitter bs(in_blob_desc.shape().At(split_axis),
                      out_blob_descs.size());
...
Maybe<void> DistributeSplitOp::InferOutParallelDesc(
  ...
  FOR_RANGE(int, i, 0, output_bns().size()) {
    ...
    if (op_parallel_desc.parallel_num() > 1) {
      ...
      *ParallelDesc4Obn(obn) =
          ParallelDesc(op_parallel_desc.GetParallelIdOnlyParallelConf(i));
    } else {
...
int32_t DistributeSplitOp::FixAxis(const int32_t axis,
