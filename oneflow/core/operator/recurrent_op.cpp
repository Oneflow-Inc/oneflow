...
Maybe<void> RecurrentOp::InferBlobDescs(
  ...
  if (!GetValFromCustomizedConf<std::string>("init_hidden").empty()) {
    ...
    CHECK_EQ_OR_RETURN(h0_blob_desc->data_type(), data_type);
    CHECK_EQ_OR_RETURN(h0_blob_desc->shape(), h0_shape);
    CHECK_EQ_OR_RETURN(h0_blob_desc->has_data_id_field(), in_blob_desc->has_data_id_field());
    CHECK_EQ_OR_RETURN(h0_blob_desc->max_col_num(), 1);
  } else {
