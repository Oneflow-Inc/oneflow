...
void RecurrentOp::InferBlobDescs(std::function<BlobDesc*(const std::string&)> GetBlobDesc4BnInOp,
  ...
  CHECK_EQ(in_blob_desc->body().data_type(), data_type);
  CHECK_EQ(in_blob_desc->body().shape().NumAxes(), 2);
  CHECK_EQ(in_blob_desc->header().has_col_num_field(), true);
  int64_t data_num = in_blob_desc->body().shape().At(0);
  ...
  if (!GetValFromCustomizedConf<std::string>("init_hidden").empty()) {
    ...
    CHECK_EQ(h0_blob_desc->body().data_type(), data_type);
    CHECK_EQ(h0_blob_desc->body().shape(), h0_shape);
    CHECK_EQ(h0_blob_desc->header().has_data_id_field(),
             in_blob_desc->header().has_data_id_field());
    CHECK_EQ(h0_blob_desc->header().max_col_num(), 1);
  } else {
  ...
  *out_blob_desc = *in_blob_desc;
  out_blob_desc->mut_body().mut_shape() = Shape({data_num, hidden_size});
  // recurrent_out
  BlobDesc* rec_out_blob_desc = GetBlobDesc4BnInOp("rec_out");
  *rec_out_blob_desc = *out_blob_desc;
  if (parallel_ctx->policy() == kDataParallel) {
    rec_out_blob_desc->mut_header().set_max_col_num(1);
  }
