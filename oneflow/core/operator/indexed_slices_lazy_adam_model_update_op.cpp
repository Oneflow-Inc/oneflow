...
private:
  ...
  void VirtualGenKernelConf(
      ...
      std::function<const BlobDesc &(const std::string &)>
          LogicalBlobDesc4BnInOp) const override;
};
...
Maybe<void> IndexedSlicesLazyAdamMdUpdateOp::InferBlobDescs(
  ...
  *GetBlobDesc4BnInOp("unique_diff_values") = *values;
  BlobDesc *num_unique_diff_indices =
      GetBlobDesc4BnInOp("num_unique_diff_indices");
  ...
  IndexedSlicesReduceSumOpUtil::GetReduceSumWorkspaceSizeInBytes(
      device_type(), values->data_type(), indices->data_type(),
      indices->shape().elem_cnt(), values->shape().Count(num_indices_axes),
...
void IndexedSlicesLazyAdamMdUpdateOp::VirtualGenKernelConf(
  ...
  IndexedSlicesLazyAdamModelUpdateKernelConf
      *indexed_slices_lazy_adam_model_update_conf =
          kernel_conf->mutable_indexed_slices_lazy_adam_model_update_conf();
  indexed_slices_lazy_adam_model_update_conf->set_indices_data_type(
      indices_blob->data_type());
...
REGISTER_OP(OperatorConf::kIndexedSlicesLazyAdamModelUpdateConf,
            IndexedSlicesLazyAdamMdUpdateOp);
