...
Maybe<void> IndexedSlicesNaiveMdUpdateOp::InferBlobDescs(
  ...
  FOR_RANGE(int64_t, i, 1, num_model_axes) {
    CHECK_EQ_OR_RETURN(model->shape().At(i),
                       values->shape().At(num_indices_axes + i - 1));
  }
...
void IndexedSlicesNaiveMdUpdateOp::VirtualGenKernelConf(
  ...
  IndexedSlicesNaiveModelUpdateKernelConf
      *indexed_slices_naive_model_update_conf =
          kernel_conf->mutable_indexed_slices_naive_model_update_conf();
  indexed_slices_naive_model_update_conf->set_indices_data_type(
...
REGISTER_OP(OperatorConf::kIndexedSlicesNaiveModelUpdateConf,
            IndexedSlicesNaiveMdUpdateOp);
