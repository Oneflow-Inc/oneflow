...
void AffineChannelOp::InitFromOpConf() {
  ...
  EnrollConstBufBn("moving_mean");
  EnrollConstBufBn("moving_variance");
  if (conf.center()) {
    EnrollModelBn("beta");
  } else {
...
NormalizationOpCtx* AffineChannelOp::NewNormalizationOpCtx(const Shape& in_shape) const {
  NormalizationOpCtx* op_ctx = new NormalizationOpCtx();
  op_ctx->axis = op_conf().affine_channel_conf().axis();
