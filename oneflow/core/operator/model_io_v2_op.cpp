...
void GenModelIoV2KernelConf(const VariableOpConf &variable_conf,
  ...
  if (variable_conf.split_axis().has_value()) {
    sbp_parallel.mutable_split_parallel()->set_axis(
        variable_conf.split_axis().value());
  } else {
  ...
  const std::vector<TensorSliceView> slices =
      SubTskGphBuilderUtil::GetTensorSliceView(parallel_ctx.parallel_num(),
                                               sbp_parallel, blob_desc);
  for (const auto &slice : slices) {
    slice.ToProto(
        kernel_conf->mutable_model_io_v2_conf()->mutable_slice_view()->Add());
  }
  *kernel_conf->mutable_model_io_v2_conf()->mutable_parallel_ctx() =
      parallel_ctx;
}
...
public:
  ...
  const PbMessage &GetCustomizedConf() const override {
    return op_conf().model_init_v2_conf();
  }
...
private:
  ...
  Maybe<void> InferSbpSignature(
    ...
    (*sbp_signature->mutable_bn_in_op2sbp_parallel())["out"]
        ...
        ->set_axis(0);
    (*sbp_signature->mutable_bn_in_op2sbp_parallel())["tick"]
  ...
  void VirtualGenKernelConf(
    ...
    GenModelIoV2KernelConf(
        op_conf().model_init_v2_conf().original_variable_conf(), *parallel_ctx,
        kernel_conf);
  }
};
...
public:
  ...
  const PbMessage &GetCustomizedConf() const override {
    return op_conf().model_load_v2_conf();
  }
...
private:
  ...
  Maybe<void> InferSbpSignature(
    ...
    (*sbp_signature->mutable_bn_in_op2sbp_parallel())["tick"]
        .mutable_broadcast_parallel();
    (*sbp_signature->mutable_bn_in_op2sbp_parallel())["path"]
        .mutable_broadcast_parallel();
    (*sbp_signature->mutable_bn_in_op2sbp_parallel())["out"]
        .mutable_split_parallel()
        ->set_axis(0);
  ...
  void VirtualGenKernelConf(
      ...
      KernelConf *kernel_conf) const override {
    GenModelIoV2KernelConf(
        op_conf().model_load_v2_conf().original_variable_conf(), *parallel_ctx,
        kernel_conf);
  }
};
...
public:
  ...
  const PbMessage &GetCustomizedConf() const override {
    return op_conf().model_save_v2_conf();
  }
...
private:
  ...
  void VirtualGenKernelConf(
      ...
      KernelConf *kernel_conf) const override {
    GenModelIoV2KernelConf(
        op_conf().model_save_v2_conf().original_variable_conf(), *parallel_ctx,
        kernel_conf);
  }
};
