...
void GetReduceSumWorkspaceSizeInBytes(int64_t n, int64_t m,
                                      int64_t *workspace_size_in_bytes) {
  IndexedSlicesReduceSumKernelUtil<device_type, K, T, int64_t>::
      GetReduceSumWorkspaceSizeInBytes(nullptr, n, m, workspace_size_in_bytes);
}

struct SwitchUtil final {
#define SWITCH_ENTRY(func_name, device_type, T, K) func_name<device_type, T, K>
  DEFINE_STATIC_SWITCH_FUNC(void, GetReduceSumWorkspaceSizeInBytes,
                            SWITCH_ENTRY,
...
void IndexedSlicesReduceSumOpUtil::GetReduceSumWorkspaceSizeInBytes(
    ...
    int64_t m, int64_t *workspace_size_in_bytes) {
  SwitchUtil::SwitchGetReduceSumWorkspaceSizeInBytes(
      SwitchCase(device_type, value_type, index_type), n, m,
