...
void DecodeInStreamOp::InferBlobDescs(
  ...
  const auto& decode_conf = op_conf().decode_in_stream_conf();
  const auto& conf = decode_conf.blob(0);
  ...
  bool r = decode_conf.has_dim0_inner_shape();
  out_blob_desc->set_has_dim0_valid_num_field(r);
  if (decode_conf.has_dim0_inner_shape()) {
    out_blob_desc->mut_dim0_inner_shape() = Shape(decode_conf.dim0_inner_shape());
  }
  if (decode_conf.has_dim0_valid_num()) { CHECK(decode_conf.has_dim0_inner_shape()); }
}
