...
public:
  ...
  LogicalNode *NewProperLogicalNode() const override {
    return new DistributeConcatLogicalNode;
  }

private:
  Maybe<void> InferParallelSignature() override;
  Maybe<void>
  InferBatchAxis(const std::function<const BlobDesc &(const std::string &)>
                     &LogicalBlobDesc4Ibn,
                 std::function<OptInt64 *(const std::string &)>
...
Maybe<void> DistributeConcatOp::InferBlobDescs(
  ...
  if (parallel_ctx->parallel_num() > 1) {
    const auto *in_blob_desc =
        GetBlobDesc4BnInOp(input_bns().Get(parallel_ctx->parallel_id()));
  ...
  for (size_t i = 0; i < input_bns().size(); ++i) {
    ...
    if (in_i_blob_desc == nullptr) {
      continue;
    }
    if (first_blob_desc_idx == i) {
      continue;
    }
    ...
    CHECK_EQ_OR_RETURN(in_i_blob_desc->data_type(),
                       first_blob_desc->data_type());
  }
...
Maybe<void> DistributeConcatOp::InferParallelSignature() {
  ...
  int64_t op_parallel_desc_symbol_id =
      JUST(scope.GetParallelDescSymbolId(op_conf()));
  mut_parallel_signature()->set_op_parallel_desc_symbol_id(
      op_parallel_desc_symbol_id);
  auto *map =
      mut_parallel_signature()->mutable_bn_in_op2parallel_desc_symbol_id();
  ...
  FOR_RANGE(int, i, 0, input_bns().size()) {
    const auto &in_parallel_conf =
        op_parallel_desc.GetParallelIdOnlyParallelConf(i);
    (*map)[input_bns().Get(i)] =
        Global<ForeignCallback>::Get()->MakeParallelDescSymbol(
...
Maybe<void> DistributeConcatOp::InferBatchAxis(
  ...
  FOR_RANGE(int32_t, i, 0, input_bns().size()) {
    CHECK_OR_RETURN(*BatchAxis4BnInOp(input_bns().Get(i)) ==
                    *BatchAxis4BnInOp(input_bns().Get(0)));
  }
...
Maybe<void> DistributeConcatOp::InferSbpSignature(
  ...
  auto LogicalBlobDesc4Ibn =
      [&](const std::string &ibn) -> Maybe<const BlobDesc &> {
  ...
  };
  {
    ...
    int64_t dim = 0;
    FOR_RANGE(int, i, 0, input_bns().size()) {
      const auto &in_parallel_desc =
          JUST(SbpInferHint4Ibn(input_bns().Get(i)))->parallel_desc();
    ...
    FOR_RANGE(int, i, 0, input_bns().size()) {
      CHECK_EQ_OR_RETURN(
          JUST(LogicalBlobDesc4Ibn(input_bns().Get(i))).shape().At(axis),
          bs.At(i).size());
    }
  }
...
int32_t DistributeConcatOp::FixAxis(const int32_t axis,
                                    const int64_t num_axes) const {
  int32_t ret = axis;
  if (axis < 0) {
    ret += num_axes;
  }
