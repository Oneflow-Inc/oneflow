...
void ContiguousInstanceStackOp::VirtualGenKernelConf(
    std::function<const BlobDesc*(const std::string&)> GetBlobDesc4BnInOp,
    const ParallelContext* parallel_ctx, KernelConf* kernel_conf) const {
  const BlobDesc* in_0 = GetBlobDesc4BnInOp(input_bns().Get(0));
  FOR_RANGE(int32_t, i, 1, op_conf().contiguous_instance_stack_conf().in_size()) {
    const BlobDesc* in_i = GetBlobDesc4BnInOp(input_bns().Get(i));
    CHECK_EQ(in_0->has_dim0_valid_num_field(), in_i->has_dim0_valid_num_field());
  }
  kernel_conf->set_need_do_dim0_valid_num(in_0->has_dim0_valid_num_field());
}
