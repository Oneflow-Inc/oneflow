...
void DecodeOFRecordOp::InferBlobDescs(
  ...
  FOR_RANGE(size_t, i, 0, output_bns().size()) {
    ...
    out_blob_desc->SetHasField<FieldKey::kColNum>(blob_conf.max_sequence_size() > 1);
    ...
    out_blob_desc->SetHasField<FieldKey::kDim1ValidNum>(decoder_if->HasDim1ValidNumField(encode));
    out_blob_desc->SetHasField<FieldKey::kDim2ValidNum>(decoder_if->HasDim2ValidNumField(encode));
  }
}
