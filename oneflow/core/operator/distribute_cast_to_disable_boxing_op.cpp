...
class DistributeCastToDisableBoxingOp final : public Operator {
 public:
  OF_DISALLOW_COPY_AND_MOVE(DistributeCastToDisableBoxingOp);
  DistributeCastToDisableBoxingOp() = default;
  ~DistributeCastToDisableBoxingOp() = default;
...
void DistributeCastToDisableBoxingOp::InitFromOpConf() {
  CHECK(op_conf().has_distribute_cast_to_disable_boxing_conf());

  EnrollRepeatedInputBn("in");
  EnrollOutputBn("out");
}

Maybe<void> DistributeCastToDisableBoxingOp::InferBlobDescs(
...
Maybe<void> DistributeCastToDisableBoxingOp::InferParallelSignature() {
...
Maybe<void> DistributeCastToDisableBoxingOp::InferBatchAxis(
    const std::function<const BlobDesc&(const std::string&)>& LogicalBlobDesc4Ibn,
...
Maybe<void> DistributeCastToDisableBoxingOp::InferSbpSignature(
    SbpSignature* sbp_signature, const SbpSignature& sbp_sig_conf,
    const std::function<int32_t(const SbpSignature&)>& CalcOrderValue4SbpSig,
    std::function<Maybe<const SbpInferHint*>(const std::string&)> SbpInferHint4Ibn,
...
Maybe<void> DistributeCastToDisableBoxingOp::GetSbpSignatures(
    const std::function<Maybe<const BlobDesc&>(const std::string&)>& LogicalBlobDesc4Ibn,
...
REGISTER_OP(OperatorConf::kDistributeCastToDisableBoxingConf, DistributeCastToDisableBoxingOp);
REGISTER_DISABLE_INPUT_BOXING_GROUP(OperatorConf::kDistributeCastToDisableBoxingConf);
