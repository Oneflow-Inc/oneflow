      ...
      FOR_RANGE(size_t, i, 0, in_blob_shape_vec.size()) {
        if (i == concat_axis) {
          ...
          if (i == 0 && in_blob_instance_inner_shape_vec) {
            (*instance_inner_shape_vec)[0] += (*in_blob_instance_inner_shape_vec)[0];
          }
        } else {
          CHECK_EQ(data_tmp_blob_shape_vec[i], in_blob_shape_vec[i]);
          if (i == 0 && in_blob_instance_inner_shape_vec) {
            CHECK(*instance_inner_shape_vec == *in_blob_instance_inner_shape_vec);
          }
        }
      }
    }
...
void BoxingOp::InferOutBlobDescs(
    ...
    const std::vector<int64_t>& data_tmp_instance_inner_shape_vec) const {
  const BoxingOpConf& conf = boxing_conf();
  ...
  std::vector<int64_t> instance_inner_shape_vec(data_tmp_instance_inner_shape_vec);
  if (conf.out_box_case() == BoxingOpConf::kSplitBox) {
    ...
    FOR_RANGE(size_t, i, 0, OutputBns().size()) {
      ...
      *out_blob_desc = *first_in_blob;
      ...
      if (split_conf.axis() == 0 && instance_inner_shape_vec.size() > 0) {
        size_t inner_shape_count_1 = Shape(instance_inner_shape_vec).Count(1);
        CHECK_EQ(split_conf.part_num(i) % inner_shape_count_1, 0);
        instance_inner_shape_vec[0] = split_conf.part_num(i) * scale / inner_shape_count_1;
        out_blob_desc->mut_instance_inner_shape() = Shape(instance_inner_shape_vec);
      }
    }
