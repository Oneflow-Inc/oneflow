...
void BoxingOp::InferBlobDescs(std::function<BlobDesc*(const std::string&)> GetBlobDesc4BnInOp,
  ...
  std::vector<int64_t> data_tmp_blob_instance_inner_shape_vec;
  if (GetBlobDesc4BnInOp(input_bns().Get(0))->has_instance_inner_shape()) {
    data_tmp_blob_instance_inner_shape_vec =
        GetBlobDesc4BnInOp(input_bns().Get(0))->instance_inner_shape().dim_vec();
  }
  ...
  if (conf.out_box_case() == BoxingOpConf::kSplitBox) {
    ...
    FOR_RANGE(size_t, i, 0, output_bns().size()) {
      ...
      *out_blob_desc = *first_in_blob;
      ...
      if (split_conf.axis() == 0 && data_tmp_blob_instance_inner_shape_vec.size() > 0) {
        size_t inner_shape_count_1 = Shape(data_tmp_blob_instance_inner_shape_vec).Count(1);
        CHECK_EQ(split_conf.part_num(i) % inner_shape_count_1, 0);
        data_tmp_blob_instance_inner_shape_vec[0] = split_conf.part_num(i) / inner_shape_count_1;
        out_blob_desc->mut_instance_inner_shape() = Shape(data_tmp_blob_instance_inner_shape_vec);
      }
    }
    ...
    for (const std::string& obn : output_bns()) {
      ...
      *out_blob_desc = *first_in_blob;
      out_blob_desc->mut_shape() = Shape(data_tmp_blob_shape_vec);
      if (data_tmp_blob_instance_inner_shape_vec.size() > 0) {
        out_blob_desc->mut_instance_inner_shape() = Shape(data_tmp_blob_instance_inner_shape_vec);
      }
    }
  } else {
...
void BoxingOp::InferDataTmpBlobDesc(
    ...
    std::vector<int64_t>* data_tmp_instance_inner_shape_vec_ptr) const {
  const BoxingOpConf& conf = op_conf().boxing_conf();
  if (conf.in_box_case() == BoxingOpConf::kConcatBox) {
    ...
    FOR_RANGE(size_t, ib_idx, 1, input_bns().size()) {
      ...
      const std::vector<int64_t>* in_blob_instance_inner_shape_vec = nullptr;
      if (in_blob_desc->has_instance_inner_shape()) {
        in_blob_instance_inner_shape_vec = &in_blob_desc->instance_inner_shape().dim_vec();
      }
      ...
      FOR_RANGE(size_t, i, 0, in_blob_shape_vec.size()) {
        if (i == concat_axis) {
          ...
          if (i == 0 && in_blob_instance_inner_shape_vec) {
            (*data_tmp_instance_inner_shape_vec_ptr)[0] += (*in_blob_instance_inner_shape_vec)[0];
          }
        } else {
          CHECK_EQ((*data_tmp_vec_ptr)[i], in_blob_shape_vec[i]);
          if (i == 0 && in_blob_instance_inner_shape_vec) {
            CHECK(*data_tmp_instance_inner_shape_vec_ptr == *in_blob_instance_inner_shape_vec);
          }
        }
      }
    }
  ...
  if (conf.in_box_case() == BoxingOpConf::kAddBox
    ...
    if (data_tmp_instance_inner_shape_vec_ptr->size() > 0) {
      data_tmp_blob_desc->mut_instance_inner_shape() =
          Shape(*data_tmp_instance_inner_shape_vec_ptr);
    }
  }
}
