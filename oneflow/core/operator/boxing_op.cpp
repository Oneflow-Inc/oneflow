...
void BoxingOp::InferBlobDescs(std::function<BlobDesc*(const std::string&)> GetBlobDesc4BnInOp,
  ...
  if (conf.in_box_case() == BoxingOpConf::kAddBox) {
    const Shape& first_in_blob_shape = first_in_blob->body().shape();
    for (const std::string& ibn : input_bns()) {
      CHECK_EQ(first_in_blob_shape, GetBlobDesc4BnInOp(ibn)->body().shape());
    }
  }

  std::vector<int64_t> data_tmp_blob_shape_vec =
      GetBlobDesc4BnInOp(input_bns().Get(0))->body().shape().dim_vec();
  ...
  if (conf.out_box_case() == BoxingOpConf::kSplitBox) {
    ...
    FOR_RANGE(size_t, i, 0, output_bns().size()) {
      ...
      *out_blob_desc = *first_in_blob;
      ...
      out_blob_desc->mut_body().mut_shape() = Shape(data_tmp_blob_shape_vec);
    }
...
void BoxingOp::InferDataTmpBlobDesc(std::function<BlobDesc*(const std::string&)> GetBlobDesc4BnInOp,
  ...
  if (conf.in_box_case() == BoxingOpConf::kConcatBox) {
    ...
    FOR_RANGE(size_t, ib_idx, 1, input_bns().size()) {
      ...
      const std::vector<int64_t>& in_blob_shape_vec = in_blob_desc->body().shape().dim_vec();
  ...
  if (conf.in_box_case() == BoxingOpConf::kAddBox
    ...
    data_tmp_blob_desc->mut_body().mut_shape() = Shape(*data_tmp_vec_ptr);
    data_tmp_blob_desc->mut_body().set_data_type(
        GetBlobDesc4BnInOp(input_bns().Get(0))->body().data_type());
  }
}
