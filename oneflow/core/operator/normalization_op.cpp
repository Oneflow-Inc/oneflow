...
void NormalizationOp::InferBlobDescs(
  ...
  const DataType in_data_type = in_blob_desc->body_desc().data_type();
  CHECK_EQ(in_data_type, Global<JobDesc>::Get()->DefaultDataType());
  *GetBlobDesc4BnInOp("out") = *in_blob_desc;
#ifdef WITH_CUDA
  int32_t in_dims = in_blob_desc->body_desc().shape().NumAxes();
...
#endif
  NormalizationOpCtx* op_ctx = NewNormalizationOpCtx(in_blob_desc->body_desc().shape());
  EnrollOpCtx(op_ctx);
  ...
  if (op_ctx->need_transpose) {
    ...
    transpose_blob_desc->mut_body_desc().mut_shape() = in_blob_desc->body_desc().shape();
    transpose_blob_desc->mut_body_desc().mut_shape().Set(op_ctx->axis,
                                                         in_blob_desc->body_desc().shape().At(0));
    transpose_blob_desc->mut_body_desc().mut_shape().Set(0, op_ctx->transpose_cols);
    transpose_blob_desc->mut_body_desc().set_data_type(in_data_type);
  ...
  tmp_blob_desc->mut_body_desc().set_data_type(in_data_type);
  int64_t tmp_elem_cnt =
      static_cast<int64_t>(tmp_storage_size / GetSizeOfDataType(in_data_type)) + 1;
  if (tmp_elem_cnt > 1) { tmp_blob_desc->mut_body_desc().mut_shape() = Shape({tmp_elem_cnt}); }
}
...
void NormalizationOp::InferBlobDescsForCudnn(
  ...
  InferParamBlobDescs(GetBlobDesc4BnInOp, conf, in_blob_desc->body_desc().shape().At(conf.axis()),
                      in_data_type, true);
}
...
void NormalizationOp::VirtualGenKernelConfForCudnn(
  ...
  GetBlobDesc4BnInOp("in")->body_desc().shape().ToProto(conf->mutable_in());
