...
void FullyConnectedOp::InferBlobDescs(
  ...
  *out_blob_desc = *in_blob_desc;
  ...
  // weight
  GetBlobDesc4BnInOp("weight")->mut_body().mut_shape() =
      Shape({units, in_blob_desc->body().shape().Count(1)});

  if (conf.use_bias()) {
    // bias
    GetBlobDesc4BnInOp("bias")->mut_body().mut_shape() = Shape({1, units});

    // bias_multiplier
    GetBlobDesc4BnInOp("bias_multiplier")->mut_body().mut_shape() =
        Shape({in_blob_desc->body().shape().At(0), 1});
  }
}
