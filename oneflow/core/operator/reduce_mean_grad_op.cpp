...
void ReduceMeanGradOp::InferBlobDescs(
  ...
  int64_t x_num_axes = std::max(x_blob_desc->shape().NumAxes(), dy_blob_desc->shape().NumAxes());
  const auto& x_shape = op_conf().reduce_mean_grad_conf().has_kept_dims_shape()
                            ? Shape(op_conf().reduce_mean_grad_conf().kept_dims_shape())
                            : dy_blob_desc->shape().CreateLeftExtendedShape(x_num_axes);
  const auto& like_shape = x_blob_desc->shape().CreateLeftExtendedShape(x_num_axes);
