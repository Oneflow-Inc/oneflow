...
Maybe<void> GetSbpSignature(const InterfaceBlobConf &blob_conf,
                            const PbRpf<std::string> &input_bns,
                            const PbRpf<std::string> &output_bns,
                            SbpSignature *sbp_signature, bool is_for_input_op) {
...
Maybe<void>
InterfaceOpUtil::InferOutBlobDesc(const InterfaceBlobConf &blob_conf,
  ...
  if (blob_conf.split_axis().has_value()) {
    int64_t split_axis = blob_conf.split_axis().value();
    BalancedSplitter bs(out_blob_desc->shape().At(split_axis),
...
Maybe<void> InterfaceOpUtil::GetInputLikeOpSbpSignature(
    const InterfaceBlobConf &blob_conf, const PbRpf<std::string> &input_bns,
...
Maybe<void> InterfaceOpUtil::GetOutputLikeOpSbpSignature(
    const InterfaceBlobConf &blob_conf, const PbRpf<std::string> &input_bns,
    const PbRpf<std::string> &output_bns, SbpSignature *sbp_signature) {
...
Maybe<void>
InterfaceOpUtil::InitBlobConf(InterfaceBlobConf *blob_conf,
