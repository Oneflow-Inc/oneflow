...
Maybe<void> AccuracyOp::InferBlobDescs(
  ...
  if (op_conf().accuracy_conf().has_weight()) {
    ...
    CHECK_EQ_OR_RETURN(weight->has_dim0_valid_num_field(),
                       label_blob_desc->has_dim0_valid_num_field());
    CHECK_EQ_OR_RETURN(weight->has_dim0_inner_shape(), label_blob_desc->has_dim0_inner_shape());
    if (label_blob_desc->has_dim0_inner_shape()) {
      CHECK_EQ_OR_RETURN(weight->dim0_inner_shape(), label_blob_desc->dim0_inner_shape());
    }
