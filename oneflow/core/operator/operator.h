...
public:
  ...
  template <typename T>
  const PbRf<T> &
  GetPbRfFromCustomizedConf(const std::string &field_name) const {
    return GetPbRfFromPbMessage<T>(GetCustomizedConf(), field_name);
  }
  template <typename T>
  const PbRpf<T> &
  GetPbRpfFromCustomizedConf(const std::string &field_name) const {
...
#define DEFINE_BLOB_NAMES_GETTER(getter_name)                                  \
  const PbRpf<std::string> &getter_name() const {                              \
    return op_attribute_.getter_name();                                        \
  }                                                                            \
  PbRpf<std::string> *mut_##getter_name() {                                    \
    return op_attribute_.mutable_##getter_name();                              \
  }
...
#undef DEFINE_BLOB_NAMES_GETTER
  ...
  Maybe<void> InferLogicalOutBlobDescsIf(
    ...
    return InferLogicalOutBlobDescs(BlobDesc4BnInOp, BatchAxis4Ibn,
                                    parallel_desc);
  }
  ...
  Maybe<void> InferOutBlobDescsIf(
      std::function<BlobDesc *(const std::string &)> GetBlobDesc4BnInOp,
      const ParallelContext *, const SbpSignature *sbp_signature,
      std::function<void(OpContext *)> EnrollOpCtx) const;
  ...
  Maybe<void> InferOutParallelDescIf(
      ...
      const ParallelDesc &, const SbpSignature *) const;
  ...
  Maybe<void>
  InferBatchAxisIf(const std::function<const BlobDesc &(const std::string &)>
                       &LogicalBlobDesc4Ibn,
  ...
  OutputBlobModifier4Obn(const std::string &obn) const;
  Maybe<const SbpParallel *>
  SbpParallel4BnInOp(const std::string &bn_in_op) const;
  Maybe<const OptInt64 *> BatchAxis4BnInOp(const std::string &bn_in_op) const;
  Maybe<const OptMirroredParallel *>
  OptMirroredParallel4BnInOp(const std::string &bn_in_op) const;
  ...
  ParallelSignature *mut_parallel_signature() {
    return op_attribute_.mutable_parallel_signature();
  }

  Maybe<const SbpSignature *> sbp_signature() const;
  SbpSignature *mut_sbp_signature() {
    return op_attribute_.mutable_sbp_signature();
  }
...
protected:
  ...
  virtual Maybe<void> GetSbpSignatures(SbpSignatureList *sbp_sig_list) const {
    UNIMPLEMENTED() << " GetSbpSignatures unimplemented, op name: "
                    << op_name();
  ...
  InputBlobModifier *EnrollInputBn(const std::string &ibn) {
    return EnrollInputBn(ibn, true);
  }
  ...
  OutputBlobModifier *EnrollOutputBn(const std::string &obn) {
    return EnrollOutputBn(obn, true);
  }
...
std::shared_ptr<Operator> ConstructOp(const OperatorConf &op_conf,
                                      const JobDesc *);
std::shared_ptr<Operator> ConstructOp(const OperatorConf &op_conf,
                                      DeviceType device_type, const JobDesc *);
...
inline OpBlobArg GenOpBlobArg(const std::string &op_name,
...
inline std::string GenLogicalBlobName(const std::string &op_name,
...
template <> struct hash<oneflow::OperatorConf> final {
