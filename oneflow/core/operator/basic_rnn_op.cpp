...
void BasicRnnOp::VirtualInferBlobDescs(
  ...
  int32_t hidden_size = GetBlobDesc4BnInOp("out")->body_desc().shape().At(1);
  const BlobDesc* in_blob_desc = GetBlobDesc4BnInOp("in");
  int64_t embedding_size = in_blob_desc->body_desc().shape().Count(1);
  ...
  *GetBlobDesc4BnInOp("plus_op_out") =
      BlobDesc(Shape({data_num, hidden_size}), Global<JobDesc>::Get()->DefaultDataType(), false,
               true, in_blob_desc->header_desc().max_col_num());
