...
class OfCollectiveBoxingGenericOp : public Operator {
 public:
  OF_DISALLOW_COPY_AND_MOVE(OfCollectiveBoxingGenericOp);
  OfCollectiveBoxingGenericOp() = default;
  ~OfCollectiveBoxingGenericOp() override = default;

 private:
  Maybe<void> InitFromOpConf() override {
    CHECK(op_conf().has_of_collective_boxing_generic_conf());
    const RankDesc& rank_desc = op_conf().of_collective_boxing_generic_conf().rank_desc();
  ...
  LogicalBlobId lbi4ibn(const std::string& input_bn) const override {
    return this->op_conf().of_collective_boxing_generic_conf().lbi();
  }
...
REGISTER_OP(OperatorConf::kOfCollectiveBoxingGenericConf, OfCollectiveBoxingGenericOp);
