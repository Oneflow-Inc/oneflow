...
void CloneOp::VirtualGenKernelConf(
  ...
  if (!kernel_conf->is_forward()) {
    ...
    if (in_diff_blob_desc->HasField<FieldKey::kDataId>()) {
      kernel_conf->set_need_do_col_num(true);
    }
    if (in_diff_blob_desc->HasField<FieldKey::kColNum>()) {
      kernel_conf->set_need_do_col_num(true);
    }
    if (in_diff_blob_desc->HasField<FieldKey::kDim0ValidNum>()) {
      kernel_conf->set_need_do_dim0_valid_num(true);
      kernel_conf->set_can_naive_do_dim0_valid_num(true);
    }
    if (in_diff_blob_desc->HasField<FieldKey::kDim1ValidNum>()) {
      kernel_conf->set_need_do_dim1_valid_num(true);
    }
    if (in_diff_blob_desc->HasField<FieldKey::kDim2ValidNum>()) {
      kernel_conf->set_need_do_dim2_valid_num(true);
    }
    if (in_diff_blob_desc->HasField<FieldKey::kRecordIdInDevicePiece>()) {
