#include "oneflow/core/operator/one_hot.h"

namespace oneflow {

void OneHot::InitFromOpConf() {
  CHECK(op_conf().has_one_hot_conf());
  EnrollInputBn("indices", false);
  EnrollOutputBn("out", false);
}

const PbMessage &OneHot::GetCustomizedConf() const { return op_conf().one_hot_conf(); }

void OneHot::InferBlobDescs(std::function<BlobDesc *(const std::string &)> GetBlobDesc4BnInOp,
...
REGISTER_OP(OperatorConf::kOneHotConf, OneHot);
