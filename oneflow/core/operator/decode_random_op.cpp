...
void DecodeRandomOp::InferBlobDescs(std::function<BlobDesc*(const std::string&)> GetBlobDesc4BnInOp,
  ...
  out_blob_desc->mut_body().set_data_type(conf.data_type());
  out_blob_desc->mut_header().set_has_data_id_field(Global<JobDesc>::Get()->SizeOfOneDataId() > 0);
  out_blob_desc->mut_header().set_has_col_num_field(conf.max_sequence_size() > 1);
  out_blob_desc->mut_header().set_max_col_num(conf.max_sequence_size());
}
