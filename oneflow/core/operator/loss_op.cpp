...
void LossOp::InferBlobDescs(std::function<BlobDesc*(const std::string&)> GetBlobDesc4BnInOp,
  ...
  loss_blob_desc->mut_body_desc().mut_shape() = Shape({pred_blob_desc->body_desc().shape().At(0)});
  loss_blob_desc->mut_body_desc().set_data_type(pred_blob_desc->body_desc().data_type());
  loss_blob_desc->mut_header_desc().set_has_data_id_field(
  ...
  if (!GetValFromCustomizedConf<std::string>("weight").empty()) {
    ...
    reduction_blob_desc->mut_body_desc().mut_shape() = Shape({1});
    reduction_blob_desc->mut_body_desc().set_data_type(pred_blob_desc->body_desc().data_type());
    reduction_blob_desc->mut_header_desc().set_has_data_id_field(
        pred_blob_desc->header_desc().has_data_id_field());
  }
