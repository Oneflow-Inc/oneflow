...
void SoftmaxOp::InferBlobDescs(std::function<BlobDesc*(const std::string&)> GetBlobDesc4BnInOp,
  ...
  *GetBlobDesc4BnInOp("out") = *in_blob_desc;
  SoftmaxOpCtx* op_ctx = NewSoftmaxOpCtx(in_blob_desc->body_desc().shape());
  EnrollOpCtx(op_ctx);

  // 1D blob store tmp calculate result
  BlobDesc* tmp_blob_desc = GetBlobDesc4BnInOp("softmax_num");
  tmp_blob_desc->mut_body_desc().mut_shape() = Shape({op_ctx->transpose_rows});
  tmp_blob_desc->mut_body_desc().set_data_type(in_blob_desc->body_desc().data_type());
  if (op_ctx->need_transpose) {
    ...
    transpose_blob_desc->mut_body_desc().mut_shape().Set(
        op_ctx->axis, in_blob_desc->body_desc().shape().At(op_ctx->dims - 1));
    transpose_blob_desc->mut_body_desc().mut_shape().Set(op_ctx->dims - 1, op_ctx->transpose_cols);
    transpose_blob_desc->mut_body_desc().set_data_type(in_blob_desc->body_desc().data_type());
