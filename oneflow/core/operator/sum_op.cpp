...
void SumOp::InitFromOpConf() {
  ...
  EnrollOutputBn("out");
  // For Parallel
  if (op_conf().sum_conf().has_axis() == false) { EnrollDataTmpBn("tmp"); }
}
...
void SumOp::VirtualGenKernelConf(
  ...
  if (op_conf().sum_conf().has_axis()) { return; }
