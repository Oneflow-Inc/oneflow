...
void LocalResponseNormalizationOp::InferBlobDescs(
  ...
  CHECK_EQ(in_blob_desc->body_desc().shape().NumAxes(), 4);
  ...
  *GetBlobDesc4BnInOp("out") = *in_blob_desc;
  ...
  if (device_type() == DeviceType::kCPU) {
    if (conf.data_format() == "channels_last") {
      GetBlobDesc4BnInOp("padded_square")->mut_body_desc().mut_shape() =
          Shape({in_blob_desc->body_desc().shape().At(3) + 2 * conf.depth_radius()});
    } else if (conf.data_format() == "channels_first") {
      GetBlobDesc4BnInOp("padded_square")->mut_body_desc().mut_shape() =
          Shape({1, in_blob_desc->body_desc().shape().At(1) + 2 * conf.depth_radius(),
                 in_blob_desc->body_desc().shape().At(2), in_blob_desc->body_desc().shape().At(3)});
    } else {
      UNIMPLEMENTED();
    }
    GetBlobDesc4BnInOp("normalize_coef")->mut_body_desc().mut_shape() =
        in_blob_desc->body_desc().shape();
...
void LocalResponseNormalizationOp::VirtualGenKernelConf(
  ...
  const Shape& in_shape = GetBlobDesc4BnInOp("in")->body_desc().shape();
