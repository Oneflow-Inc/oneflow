...
void LocalResponseNormalizationOp::InferBlobDescs(
  ...
  CHECK_EQ(in_blob_desc->body().shape().NumAxes(), 4);
  CHECK_EQ(in_blob_desc->body().data_type(), Global<JobDesc>::Get()->DefaultDataType());
  *GetBlobDesc4BnInOp("out") = *in_blob_desc;

  if (device_type() == DeviceType::kCPU) {
    if (conf.data_format() == "channels_last") {
      GetBlobDesc4BnInOp("padded_square")->mut_body().mut_shape() =
          Shape({in_blob_desc->body().shape().At(3) + 2 * conf.depth_radius()});
    } else if (conf.data_format() == "channels_first") {
      GetBlobDesc4BnInOp("padded_square")->mut_body().mut_shape() =
          Shape({1, in_blob_desc->body().shape().At(1) + 2 * conf.depth_radius(),
                 in_blob_desc->body().shape().At(2), in_blob_desc->body().shape().At(3)});
    } else {
      UNIMPLEMENTED();
    }
