...
Maybe<void> CaseOp::InferBlobDescs(
...
const PbMessage &CaseOp::GetCustomizedConf() const {
  return op_conf().case_conf();
}

Maybe<void> CaseOp::InferBatchAxis(
    std::function<OptInt64 *(const std::string &)> BatchAxis4BnInOp) const {
  for (const std::string &obn : output_bns()) {
    *BatchAxis4BnInOp(obn) = *BatchAxis4BnInOp("in");
  }
  return Maybe<void>::Ok();
}
...
LogicalNode *CaseOp::NewProperLogicalNode() const {
  return new CaseLogicalNode();
}
