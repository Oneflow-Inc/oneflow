...
void PoolingOp::InferBlobDescs(std::function<BlobDesc*(const std::string&)> GetBlobDesc4BnInOp,
  ...
  const Shape& in_shape = in_blob_desc->body_desc().shape();
  CHECK_GE(in_blob_desc->body_desc().shape().NumAxes(), 3);
  CHECK_LE(in_blob_desc->body_desc().shape().NumAxes(), 5);
  ...
  *out_blob_desc = *in_blob_desc;
  ...
  out_blob_desc->mut_body_desc().mut_shape() = GetOutShape(in_shape.At(0), in_c, out);
}
