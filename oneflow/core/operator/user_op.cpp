...
public:
  ...
  explicit UserOpKernelRegContext(
      const UserOp *user_op,
      std::function<BlobDesc *(const std::string &)> GetBlobDesc4BnInOp,
      const ParallelContext *parallel_ctx)
      : user_op::KernelRegContext(
            user_op::UserOpConfWrapper(user_op->op_conf())) {
...
#define INSERT_TO_ARG2TENSOR_DESC(prefix)                                      \
  for (const auto &bn : user_op->prefix##_bns()) {                             \
    ...
    arg2tensor_desc_.emplace(GenUnRepeatedBn(bn),                              \
                             GenTensorDescFromBlobDesc(blob_desc));            \
  }
...
private:
  ...
  HashMap<std::pair<std::string, int32_t>, user_op::TensorDesc>
      arg2tensor_desc_;
};
...
public:
  ...
  UserOpInferContext(
      const OperatorConf &op_conf, const ParallelContext *parallel_ctx,
      const SbpSignature *sbp_signature, const JobDesc &job_desc,
      std::function<BlobDesc *(const std::string &)> GetBlobDesc4BnInOp)
      : user_op::InferContext(user_op::UserOpConfWrapper(op_conf)),
        parallel_ctx_(parallel_ctx), sbp_signature_(sbp_signature),
        job_desc_(job_desc) {
    auto InitInOrOut =
        [&](const PbMap<std::string, UserOpConf::ListString> &arg_map,
            ArgVec *arg_vec) {
  ...
  Shape *Shape4ArgNameAndIndex(const std::string &arg_name,
                               int32_t index) override {
    auto it = arg2tensor_desc_.find(std::make_pair(arg_name, index));
    if (it == arg2tensor_desc_.end()) {
      return nullptr;
  ...
  DataType *Dtype4ArgNameAndIndex(const std::string &arg_name,
                                  int32_t index) override {
    auto it = arg2tensor_desc_.find(std::make_pair(arg_name, index));
    if (it == arg2tensor_desc_.end()) {
      return nullptr;
  ...
  bool *IsDynamic4ArgNameAndIndex(const std::string &arg_name,
                                  int32_t index) override {
    auto it = arg2tensor_desc_.find(std::make_pair(arg_name, index));
    if (it == arg2tensor_desc_.end()) {
      return nullptr;
  ...
  bool *IsTensorList4ArgNameAndIndex(const std::string &arg_name,
                                     int32_t index) override {
    auto it = arg2tensor_desc_.find(std::make_pair(arg_name, index));
    if (it == arg2tensor_desc_.end()) {
      return nullptr;
...
public:
  ...
  UserOpSbpContext(const OperatorConf &op_conf, SbpSignatureList *sbp_sig_list,
                   ...
                   std::function<Maybe<const BlobDesc &>(const std::string &)>
                       LogicalBlobDesc4Ibn)
      : user_op::SbpContext(user_op::UserOpConfWrapper(op_conf), sbp_sig_list,
                            device_type, parallel_num) {
    const auto &user_op_conf = op_conf.user_conf();
    for (auto it = user_op_conf.input().begin();
         it != user_op_conf.input().end(); ++it) {
      const std::string &arg_name = it->first;
      for (int32_t i = 0; i < it->second.s_size(); ++i) {
        const BlobDesc *blob =
            &CHECK_JUST(LogicalBlobDesc4Ibn(GenRepeatedBn(arg_name, i)));
        arg2tensor_desc_.emplace(std::make_pair(arg_name, i),
                                 GenTensorDescFromBlobDesc(blob));
        inputs_.emplace_back(std::make_pair(arg_name, i));
      }
    }
    for (auto it = user_op_conf.output().begin();
         it != user_op_conf.output().end(); ++it) {
      const std::string &arg_name = it->first;
      for (int32_t i = 0; i < it->second.s_size(); ++i) {
...
public:
  ...
  OptInt64 *BatchAxis4ArgNameAndIndex(const std::string &arg_name,
                                      int32_t index) override {
...
void UserOp::InitFromOpConf() {
  ...
  val_ = user_op::UserOpRegistryMgr::Get().GetOpRegistryResult(
  ...
  if (val_ != nullptr) {
    ...
    user_op::GetInputArgModifier GetInputArgModifierFn =
      ...
      if (std::find(input_bns().begin(), input_bns().end(), ibn) !=
          input_bns().end()) {
    ...
    user_op::GetOutputArgModifier GetOutputArgModifierFn =
      ...
      if (std::find(output_bns().begin(), output_bns().end(), obn) !=
          output_bns().end()) {
...
Maybe<void> UserOp::InferBlobDescs(
    std::function<BlobDesc *(const std::string &)> GetBlobDesc4BnInOp,
  ...
  CHECK_OR_RETURN(kernel_reg_val != nullptr)
      << "cannot find op_type: " << op_conf().user_conf().op_type_name()
      << " in kernel registry !";

  size_t tmp_size = kernel_reg_val->infer_tmp_size_fn(&infer_ctx);
  if (tmp_size > 0) {
    BlobDesc *tmp_buffer_blob =
        GetBlobDesc4BnInOp(GenRepeatedBn("tmp_buffer", 0));
  ...
  user_op::AddInplaceArgPair AddInplaceArgPairFn =
    ...
    CHECK_OR_RETURN(std::find(input_bns().begin(), input_bns().end(), ibn) !=
                    input_bns().end())
        << "Cannot find input_arg_name : " << in_arg_name
        << " input_arg_index : " << in_arg_index
        << " in op_name: " << op_conf().name();
    CHECK_OR_RETURN(std::find(output_bns().begin(), output_bns().end(), obn) !=
                    output_bns().end())
        << "Cannot find output_arg_name : " << out_arg_name
        << " output_arg_index : " << out_arg_index
        << " in op_name: " << op_conf().name();

    std::string repeated_ibn_err_msg =
        "Cannot repeated set inplace proposal for same intput arg : " +
        in_arg_name + " index : " + std::to_string(in_arg_index) +
        " in op_name: " + op_conf().name();
    std::string repeated_obn_err_msg =
        "Cannot repeated set inplace proposal for same output arg : " +
        out_arg_name + " index : " + std::to_string(out_arg_index) +
        " in op_name: " + op_conf().name();
    CHECK_OR_RETURN(bn_in_op_unique_check.insert(ibn).second)
        << repeated_ibn_err_msg;
    CHECK_OR_RETURN(bn_in_op_unique_check.insert(obn).second)
        << repeated_obn_err_msg;
...
Maybe<void> UserOp::InferOutBlobDescs(
  ...
  BlobDesc *first_in_blob_desc =
      FindValidBlobDescOfBnsInOp(GetBlobDesc4BnInOp, input_bns());
  ...
  for (const auto &pair : infer_ctx.outputs()) {
    ...
    out_blob_desc->set_data_type(
        *(infer_ctx.Dtype4ArgNameAndIndex(pair.first, pair.second)));
    out_blob_desc->mut_shape() =
        *(infer_ctx.Shape4ArgNameAndIndex(pair.first, pair.second));
    out_blob_desc->set_is_dynamic(
        *infer_ctx.IsDynamic4ArgNameAndIndex(pair.first, pair.second));
...
LogicalBlobId UserOp::lbi4ibn(const std::string &input_bn) const {
  ...
  return GenLogicalBlobId(
      op_conf().user_conf().input().at(pair.first).s(pair.second));
}
...
LogicalBlobId UserOp::lbi4obn(const std::string &output_bn) const {
  ...
  auto ret = GenLogicalBlobId(
      op_conf().user_conf().output().at(pair.first).s(pair.second));
...
Maybe<void> UserOp::InferBatchAxis(
  ...
  CHECK_OR_RETURN(val_ != nullptr)
      << "cannot find op_type: " << op_conf().user_conf().op_type_name()
      << " in op registry!";
  UserOpBatchAxisContext batch_axis_ctx(op_conf(), BatchAxis4BnInOp,
                                        LogicalBlobDesc4Ibn);
...
Maybe<void> UserOp::GetSbpSignatures(
  ...
  std::string tick_bn =
      GenRepeatedBn(user_op::kUserSourceOpTickInputArgName, 0);
  if (val_->op_def.input_size() == 0 && input_bns().size() == 1) {
    CHECK_OR_RETURN(input_bns().Get(0) == tick_bn)
        ...
        << " set ERROR input arg name : " << input_bns().Get(0)
        << " because NO input in op def";
  ...
  for (const auto &sbp_sig : sbp_sig_list->sbp_signature()) {
    ...
    for (const auto &ibn : input_bns()) {
      ...
      CHECK_OR_RETURN(bn2sbp.find(ibn) != bn2sbp.end())
          << "In op_name: " << op_conf().name()
          << " op_type_name: " << op_conf().user_conf().op_type_name()
          << ", input_arg_name : " << pair.first
          << " input_arg_index : " << pair.second
    ...
    for (const auto &obn : output_bns()) {
      ...
      CHECK_OR_RETURN(bn2sbp.find(obn) != bn2sbp.end())
          << "In op_name: " << op_conf().name()
          << " op_type_name: " << op_conf().user_conf().op_type_name()
          << ", output_arg_name : " << pair.first
          << " output_arg_index : " << pair.second
          << " have NOT set sbp signature";
    }
  }
  return Maybe<void>::Ok();
...
Symbol<OperatorConf> UserOp::GetOpConfWithoutOpNameAndLbn() const {
  ...
  for (auto &pair : *user_op_conf->mutable_input()) {
    for (auto &str : *pair.second.mutable_s()) {
      str = "undefined-op-name/undefined-ibn";
    }
  }
...
#define BLOB_DESCS_TO_PROTO(prefix, is_arg)                                    \
  for (const auto &bn : prefix##_bns()) {                                      \
    const BlobDesc *blob_desc = GetBlobDesc4BnInOp(bn);                        \
    if (blob_desc) {                                                           \
      blob_desc->ToProto(&(*user_conf->mutable_bn_in_op2blob_desc())[bn]);     \
    }                                                                          \
    if (is_arg) {                                                              \
      LogicalBlobDesc4BnInOp(bn).ToProto(                                      \
          &(*user_conf->mutable_bn_in_op2logical_blob_desc())[bn]);            \
    }                                                                          \
  }
