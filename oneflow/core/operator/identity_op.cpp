...
template <typename T> class IdentityOpTpl final : public Operator {
public:
  ...
  const PbMessage &GetCustomizedConf() const override {
    return T::GetCustomizedConf(op_conf());
  }
...
private:
  ...
  Maybe<void> GetSbpSignatures(const std::function<Maybe<const BlobDesc &>(
    ...
    SbpSignatureBuilder().PartialSum(bns).Build(
...
public:
  ...
  const PbMessage &GetCustomizedConf() const override {
    return op_conf().cast_to_mirrored_conf();
  }

private:
  Maybe<void> InferLogicalOutBlobDescs(
      const std::function<BlobDesc *(const std::string &)> &BlobDesc4BnInOp,
      const std::function<Maybe<const OptInt64 *>(const std::string &)>
          &BatchAxis4Ibn,
    ...
    *out = *BlobDesc4BnInOp("in");
    if (batch_axis.has_value()) {
      ...
      out->mut_shape().Set(batch_axis.value(),
                           dim / parallel_desc.parallel_num());
    }
    return Maybe<void>::Ok();
  }
  ...
  Maybe<void> InferSbpSignature(
      ...
      const ParallelDesc &parallel_desc) const override {
    CHECK_NE_OR_RETURN(
        op_conf().cast_to_mirrored_conf().sbp_parallel().parallel_type_case(),
        SbpParallel::PARALLEL_TYPE_NOT_SET)
        << "attribute sbp_parallel not set.";
    const auto &ibn_hint = *JUST(SbpInferHint4Ibn("in"));
    CHECK_EQ_OR_RETURN(ibn_hint.parallel_desc().parallel_num(),
  ...
  Maybe<void> InferMirroredSignature(
      ...
      const ParallelDesc &parallel_desc) override {
    ...
    CHECK_EQ_OR_RETURN(in_infer_hint.parallel_desc().parallel_num(),
                       parallel_desc.parallel_num());
...
private:
  ...
  Maybe<void> InferSbpSignature(
      ...
      const ParallelDesc &parallel_desc) const override {
    CHECK_NE_OR_RETURN(
        op_conf().cast_from_mirrored_conf().sbp_parallel().parallel_type_case(),
        SbpParallel::PARALLEL_TYPE_NOT_SET)
        << "attribute sbp_parallel not set.";
    const auto &ibn_hint = *JUST(SbpInferHint4Ibn("in"));
    CHECK_EQ_OR_RETURN(ibn_hint.parallel_desc().parallel_num(),
