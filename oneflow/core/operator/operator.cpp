...
std::shared_ptr<Operator> CheckAndConstructOp(const OperatorConf &op_conf,
  ...
  DeviceType device_type =
      CHECK_JUST(DeviceType4DeviceTag(op_conf.device_tag()));
...
void Operator::Init(const OperatorConf &op_conf, const JobDesc *conf_job_desc) {
  ...
  *this_op_conf = op_conf;
  if (has_job_desc() && job_desc().IsPredict()) {
    this_op_conf->set_trainable(false);
  }
  InitFromOpConf();
}

LogicalNode *Operator::NewProperLogicalNode() const {
...
LogicalBlobId *Operator::MutBnInOp2Lbi(const std::string &bn_in_op) {
  auto it = op_attribute_.mutable_arg_signature()->mutable_bn_in_op2lbi()->find(
      bn_in_op);
  if (it ==
      op_attribute_.mutable_arg_signature()->mutable_bn_in_op2lbi()->end()) {
    return nullptr;
  } else {
    return &(it->second);
  }
...
DeviceType Operator::device_type() const {
  DeviceType device_type =
      CHECK_JUST(DeviceType4DeviceTag(op_attribute_.op_conf().device_tag()));
  return device_type;
}
...
Maybe<const std::string *> Operator::obn4lbi(const LogicalBlobId &lbi) const {
  ...
  CHECK_OR_RETURN(iter != lbi2obn_.end())
      << "no logical blob id found. lbn: " << lbi.op_name() << "/"
      << lbi.blob_name();
  return &iter->second;
}
...
Maybe<void> Operator::InferParallelSignature() {
  ...
  for (const auto &ibn : input_bns()) {
    (*map)[ibn] = parallel_desc_symbol_id;
  }
  for (const auto &obn : output_bns()) {
    (*map)[obn] = parallel_desc_symbol_id;
  }
  for (const auto &tbn : tmp_bns()) {
    (*map)[tbn] = parallel_desc_symbol_id;
  }
  return Maybe<void>::Ok();
}
...
Maybe<void> Operator::InferLogicalOutBlobDescs(
  ...
  for (const auto &ibn : input_bns()) {
    (*map)[ibn].mutable_split_parallel()->set_axis(0);
  }
  for (const auto &obn : output_bns()) {
    (*map)[obn].mutable_split_parallel()->set_axis(0);
  }
...
Maybe<void> Operator::FillLogicalBlobDescSignature(
    const std::function<Maybe<const BlobDesc &>(const std::string &)>
        &BlobDesc4BnInOp) {
  auto *map = op_attribute_.mutable_logical_blob_desc_signature()
                  ->mutable_bn_in_op2blob_desc();
  for (const auto &ibn : input_bns()) {
    JUST(BlobDesc4BnInOp(ibn)).ToProto(&(*map)[ibn]);
  }
  for (const auto &obn : output_bns()) {
    JUST(BlobDesc4BnInOp(obn)).ToProto(&(*map)[obn]);
  }
  return Maybe<void>::Ok();
}
...
Maybe<void> Operator::InferOutParallelDescIf(
    ...
    const ParallelDesc &op_parallel_desc,
    const SbpSignature *sbp_signature) const {
  return InferOutParallelDesc(ParallelDesc4Obn, LogicalBlobDesc4Ibn,
                              op_parallel_desc, sbp_signature);
}
...
Maybe<void> Operator::InferOutputBlobTimeShapeIf(
  ...
  return InferOutputBlobTimeShape(GetTimeShape4BnInOp, parallel_ctx,
                                  time_shape);
}
...
Maybe<void> Operator::InferOutputBlobTimeShape(
  ...
  if (input_bns().empty() == false) {
    ...
    *time_shape =
        Shape({job_desc().TotalBatchNum(), job_desc().NumOfPiecesInBatch()});
  }
  return Maybe<void>::Ok();
}
...
void Operator::ForEachBnInOp(
    std::function<void(const std::string &)> Handler) const {
  for (const std::string &bn_in_op : input_bns()) {
    Handler(bn_in_op);
  }
  for (const std::string &bn_in_op : output_bns()) {
    Handler(bn_in_op);
  }
  for (const std::string &bn_in_op : const_buf_bns()) {
    Handler(bn_in_op);
  }
  for (const std::string &bn_in_op : tmp_bns()) {
    Handler(bn_in_op);
  }
}
...
Maybe<void> Operator::InferSbpSignatureIf(
  ...
  if (parallel_desc.parallel_num() == 1) {
    ...
    return InferSbpSignature(mut_sbp_signature(), sbp_sig_conf,
...
Maybe<void> Operator::InferSbpSignature(
  ...
  FilterSbpSignatureList(sbp_sig_list, sbp_sig_conf,
                         &filtered_sbp_sigs_by_conf);
  ...
  for (const auto &ibn : input_bns()) {
    ibn2producer_sbp_parallel[ibn] =
        &(JUST(SbpInferHint4Ibn(ibn))->sbp_parallel());
  }
  std::vector<const SbpSignature *> sorted_sbp_signatures;
  SortSbpSignatureListByCopyCost(filtered_sbp_sigs_by_conf, input_bns(),
                                 SbpInferHint4Ibn, CalcOrderValue4SbpSig,
...
Maybe<void> Operator::InferMirroredSignatureIf(
  ...
  return InferMirroredSignature(MirroredSigInferHint4Ibn,
                                is_mirrored_parallel_view_conf, parallel_desc);
}
...
Maybe<void> Operator::InferMirroredSignature(
  ...
  for (const auto &ibn : input_bns()) {
    ...
    is_mirrored_parallel_view_values.insert(
        infer_hint.is_mirrored_parallel_view());
  }
  ...
  if (is_mirrored_parallel_view_conf) {
    for (const auto &ibn : input_bns()) {
      const auto &infer_hint = *JUST(MirroredSigInferHint4Ibn(ibn));
      CHECK_EQ_OR_RETURN(infer_hint.parallel_desc().parallel_num(),
  ...
  for (const auto &obn : output_bns()) {
    SetIsMirroredParallel(obn);
  }
  return Maybe<void>::Ok();
}

Maybe<const SbpSignature *> Operator::sbp_signature() const {
  CHECK_OR_RETURN(op_attribute_.has_sbp_signature())
      << "sbp signature not infered";
  return &op_attribute_.sbp_signature();
}

Maybe<const SbpParallel *>
Operator::SbpParallel4BnInOp(const std::string &bn_in_op) const {
  CHECK_OR_RETURN(op_attribute_.has_sbp_signature())
      << "sbp signature not infered";
  const auto &map = op_attribute_.sbp_signature().bn_in_op2sbp_parallel();
  const auto &iter = map.find(bn_in_op);
  CHECK_OR_RETURN(iter != map.end()) << "blob_name " << bn_in_op
                                     << " not found in sbp signature";
  return &iter->second;
}

Maybe<const OptInt64 *>
Operator::BatchAxis4BnInOp(const std::string &bn_in_op) const {
  CHECK_OR_RETURN(op_attribute_.has_batch_axis_signature())
      << "batch axis signature not infered";
  const auto &map = op_attribute_.batch_axis_signature().bn_in_op2batch_axis();
  const auto &iter = map.find(bn_in_op);
  CHECK_OR_RETURN(iter != map.end()) << "blob_name " << bn_in_op
                                     << " not found in batch axis signature";
  return &iter->second;
}

Maybe<const OptMirroredParallel *>
Operator::OptMirroredParallel4BnInOp(const std::string &bn_in_op) const {
  CHECK_OR_RETURN(op_attribute_.has_mirrored_signature())
      << "mirrored signature not infered";
  const auto &map =
      op_attribute_.mirrored_signature().bn_in_op2opt_mirrored_parallel();
  const auto &iter = map.find(bn_in_op);
  CHECK_OR_RETURN(iter != map.end()) << "blob_name " << bn_in_op
                                     << " not found in mirrored signature";
  return &iter->second;
}

OptMirroredParallel *
Operator::MutOptMirroredParallel(const std::string &bn_in_op) {
  auto *map = op_attribute_.mutable_mirrored_signature()
...
bool HasBlobDescWithField(
  ...
  for (const std::string &bn_in_op : bn_in_ops) {
    const BlobDesc *blob_desc = GetBlobDesc4BnInOp(bn_in_op);
    if (blob_desc && Predicator4BlobDesc(blob_desc)) {
      return true;
    }
  }
  return false;
...
InputBlobModifier *Operator::EnrollInputBn(const std::string &ibn,
                                           bool has_diff) {
  LogicalBlobId lbi = lbi4ibn(ibn);
  auto *map = op_attribute_.mutable_arg_modifier_signature()
...
const InputBlobModifier &
Operator::InputBlobModifier4Ibn(const std::string &ibn) const {
  return op_attribute_.arg_modifier_signature().ibn2input_blob_modifier().at(
      ibn);
}

const OutputBlobModifier &
Operator::OutputBlobModifier4Obn(const std::string &obn) const {
  return op_attribute_.arg_modifier_signature().obn2output_blob_modifier().at(
      obn);
}
...
OutputBlobModifier *
Operator::MutOutputBlobModifier4Obn(const std::string &obn) {
  auto *map = op_attribute_.mutable_arg_modifier_signature()
...
void Operator::EnrollRepeatedInputBn(const std::string &ibn_prefix, int32_t num,
  ...
  FOR_RANGE(int32_t, i, 0, num) {
    EnrollInputBn(GenRepeatedBn(ibn_prefix, i), has_diff);
  }
}
...
void Operator::EmplaceLbi2Obn(const LogicalBlobId &lbi,
                              const std::string &obn) {
  CHECK(lbi2obn_.emplace(lbi, obn).second);
}

OutputBlobModifier *Operator::EnrollOutputBn(const std::string &obn,
                                             bool has_diff) {
...
void Operator::EnrollRepeatedOutputBnWithSetter(
    ...
    const std::function<void(OutputBlobModifier *)> &ModifierSetter) {
  EnrollRepeatedOutputBnWithSetter(
      obn_prefix, GetPbRpfFromCustomizedConf<std::string>(obn_prefix).size(),
...
void Operator::EnrollRepeatedOutputBn(const std::string &obn_prefix,
                                      int32_t num, bool has_diff) {
  FOR_RANGE(int32_t, i, 0, num) {
    EnrollOutputBn(GenRepeatedBn(obn_prefix, i), has_diff);
  }
}
...
std::shared_ptr<Operator> ConstructOp(const OperatorConf &op_conf,
                                      const JobDesc *job_desc) {
  if (IsCpuOnly(op_conf)) {
    return ConstructOp(op_conf, DeviceType::kCPU, job_desc);
  }
...
Maybe<void> Operator::InferBatchAxisIf(
  ...
  auto *map = op_attribute_.mutable_batch_axis_signature()
                  ->mutable_bn_in_op2batch_axis();
  for (const auto &ibn : input_bns()) {
    (*map)[ibn] = *JUST(BatchAxis4Ibn(ibn));
  }
  const auto &BatchAxis4BnInOp = [&](const std::string &bn_in_op) {
    return &(*map)[bn_in_op];
  };
...
Symbol<OperatorConf> Operator::GetOpConfWithoutOpNameAndLbn() const {
  ...
  for (const auto &ibn : input_bns()) {
    if (!HasStrFieldInPbFdOrPbRpf(*op_type_conf, ibn)) {
      continue;
    }
    ...
    ReplaceInputLbnInOpCustomizedConf(op_type_conf, ibn, lbn,
                                      "undefined-op-name/undefined-ibn");
  }
  return SymbolOf(op_conf);
}

std::shared_ptr<OpAttribute>
Operator::GetOpAttributeWithoutOpNameAndLbn() const {
...
Maybe<bool> GetSbpParallelInLbnOrNothing(const std::string &lbn,
                                         SbpParallel *sbp) {
...
Maybe<bool> ParseDisableBoxingFlag(const std::string &lbn_with_hint,
                                   bool *disable_boxing) {
...
Maybe<void> InferOpSbpSignature(
    ...
    std::function<Maybe<const OptInt64 *>(const std::string &)>
        BatchAxis4BnInOp) {
  auto SbpInferHint4Ibn =
      [&](const std::string &ibn) -> Maybe<const SbpInferHint *> {
    auto it = ibn2sbp_infer_hint.find(ibn);
    if (it == ibn2sbp_infer_hint.end()) {
      return Error::CheckFailedError()
             << "cannot find corresponding SbpInferHint for input_blob_name : "
             << ibn;
    }
    return &(it->second);
  };
  std::function<int32_t(const SbpSignature &)> CalcOrderValue4SbpSig;
  auto OrderValue4HasBatchAxis =
      [&](const std::string &bn, const SbpParallel &sbp_parallel) -> int32_t {
    const auto &batch_axis = *CHECK_JUST(BatchAxis4BnInOp(bn));
    return -1 * (batch_axis.has_value() && sbp_parallel.has_split_parallel() &&
                 sbp_parallel.split_parallel().axis() == batch_axis.value());
  };
  auto OrderValue4HasNoBatchAxis =
      [&](const std::string &ibn, const SbpParallel &sbp_parallel) -> int32_t {
    const auto &batch_axis = *CHECK_JUST(BatchAxis4BnInOp(ibn));
    return -2 * (batch_axis.has_value() == false &&
                 CHECK_JUST(SbpInferHint4Ibn(ibn))
                         ->sbp_parallel()
                         .has_split_parallel() == false &&
                 sbp_parallel.has_split_parallel() == false);
  };
  auto OrderValue4SbpHint = [&](const std::string &ibn,
                                const SbpParallel &sbp_parallel) -> int32_t {
    return -3 *
           (CHECK_JUST(SbpInferHint4Ibn(ibn))->sbp_parallel() == sbp_parallel);
  };
  if (sbp_sig_conf.bn_in_op2sbp_parallel().empty()) {
    CalcOrderValue4SbpSig = [&](const SbpSignature &sbp_signature) -> int32_t {
      ...
      for (const auto &ibn : op->input_bns()) {
        const auto &sbp_parallel_it =
            sbp_signature.bn_in_op2sbp_parallel().find(ibn);
      ...
      for (const auto &obn : op->output_bns()) {
        const auto &sbp_parallel_it =
            sbp_signature.bn_in_op2sbp_parallel().find(obn);
        CHECK(sbp_parallel_it != sbp_signature.bn_in_op2sbp_parallel().end());
  ...
  JUST(op->InferSbpSignatureIf(sbp_sig_conf, CalcOrderValue4SbpSig,
                               SbpInferHint4Ibn, parallel_desc));
  return Maybe<void>::Ok();
}

std::string
GetInputLbnInOpCustomizedConf(const PbMessage &msg,
  ...
  if (user_conf) {
    ...
    if (user_conf->input().find(pair.first) != user_conf->input().end()) {
      ...
      LOG(WARNING) << "cannot find input arg val in user op conf. (arg_name = "
                   << pair.first << ", id = " << std::to_string(pair.second)
                   << ")";
      return "";
    }
  } else {
...
void ReplaceInputLbnInOpCustomizedConf(PbMessage *msg,
                                       const std::string &fd_name_may_have_idx,
  ...
  if (user_conf) {
    ...
    CHECK(user_conf->input().find(pair.first) != user_conf->input().end())
        << "cannot find input arg val in user op conf. (arg_name = "
        << pair.first << ", id = " << std::to_string(pair.second) << ")\n"
...
Maybe<void>
InferOpOutSbpParallel(Operator *op, const OpNodeSignature &upstream_signature,
                      const std::function<const BlobDesc &(const std::string &)>
                          &ConstBlobDesc4Ibn,
                      const SbpSignature &sbp_sig_conf,
                      const ParallelDesc &parallel_desc) {
  ...
  const auto &SbpParallel4Ibn =
      [&](const std::string &ibn) -> const SbpParallel * {
    const auto &map =
        upstream_signature.sbp_signature().bn_in_op2sbp_parallel();
    return &map.at(ibn);
  };
...
Maybe<void> InferMirroredSignature(Operator *op,
                                   const OpNodeSignature &upstream_signature,
                                   bool is_mirrored,
                                   const ParallelDesc &parallel_desc) {
  HashMap<std::string, MirroredSigInferHint> ibn2mirrored_sig_infer_hint;
  for (const std::string &ibn : op->input_bns()) {
    const auto &map = upstream_signature.mirrored_signature()
                          .bn_in_op2opt_mirrored_parallel();
    const auto &opt_mirrored_parallel = map.at(ibn);
    ibn2mirrored_sig_infer_hint.emplace(
        ibn,
        MirroredSigInferHint(&parallel_desc,
                             opt_mirrored_parallel.has_mirrored_parallel()));
  }
  ...
  JUST(op->InferMirroredSignatureIf(MirroredSigInferHint4Ibn, is_mirrored,
                                    parallel_desc));
  return Maybe<void>::Ok();
}

Maybe<void> CheckOpInputSignature(const Operator &op,
                                  const OpNodeSignature &upstream_signature) {
  for (const auto &ibn : op.input_bns()) {
    {
      CHECK_OR_RETURN(upstream_signature.has_logical_blob_desc_signature());
      const auto &map =
          upstream_signature.logical_blob_desc_signature().bn_in_op2blob_desc();
    ...
    }
    {
      ...
      const auto &map =
          upstream_signature.batch_axis_signature().bn_in_op2batch_axis();
      CHECK_OR_RETURN(map.find(ibn) != map.end());
    }
  }
  return Maybe<void>::Ok();
...
Maybe<Operator> ConstructAndInferOp(const OperatorConf &op_conf,
  ...
  // infer is_mirrored
  JUST(InferMirroredSignature(op.get(), upstream_signature, is_mirrored,
                              parallel_desc));
  ...
  const auto &BlobDesc4BnInOp = [&](const std::string &bn_in_op) -> BlobDesc * {
    if (!bn_in_op2blob_desc[bn_in_op]) {
      bn_in_op2blob_desc[bn_in_op].reset(
          new BlobDesc(DataType::kInvalidDataType));
    }
    return bn_in_op2blob_desc[bn_in_op].get();
  };
  // infer logical blob_desc
  JUST(op->InferLogicalOutBlobDescsIf(BlobDesc4BnInOp, BatchAxis4Ibn,
                                      parallel_desc));
  JUST(op->FillLogicalBlobDescSignature(
      [&](const std::string &bn_in_op) -> Maybe<const BlobDesc &> {
        CHECK_OR_RETURN(bn_in_op2blob_desc.find(bn_in_op) !=
                        bn_in_op2blob_desc.end());
