    ...
    for (const auto& bn : output_bns()) {
      ...
      *desc = *JUST(GetLogicalBlobDesc4Obn(bn));
      const auto& nd_sbp =
          nd_sbp_signature->bn_in_op2nd_sbp().at(bn);
      desc->mut_shape() = *JUST(
          GetPhysicalShape(desc->shape(), nd_sbp, *parallel_desc, *parallel_ctx));
    }
  }
  return Maybe<void>::Ok();
}
...
Maybe<void> Operator::FillNdSbpSignature(
...
Maybe<void> Operator::InferNdSbpSignatureIf(
    ...
    std::function<Maybe<const NdSbpInferHint*>(const std::string&)>
        NdSbpInferHint4Ibn) {
  cfg::NdSbpSignature nd_sbp_signature;
  JUST(InferNdSbpSignature(&nd_sbp_signature,
                                          nd_sbp_constraints, parallel_desc,
                                          NdSbpInferHint4Ibn));
  JUST(FillNdSbpSignature(nd_sbp_signature));
  return Maybe<void>::Ok();
}
...
Maybe<void> Operator::InferNdSbpSignature(
  ...
  const auto IsBroadcast = [](const cfg::NdSbp& nd_sbp,
  ...
  if (parallel_hierarchy->NumAxes() == 1) {
    ...
    NdSbpSignatureToSbpSignature(nd_sbp_constraints,
                                                &sbp_constraints);
    cfg::SbpSignature sbp_signature;
    ...
    for (const auto& ibn : input_bns()) {
      ...
      if (nd_sbp_constraints.bn_in_op2nd_sbp_size() != 0) {
        const auto nd_sbp_constraints_it =
            nd_sbp_constraints.bn_in_op2nd_sbp().find(ibn);
        if (nd_sbp_constraints_it
            != nd_sbp_constraints.bn_in_op2nd_sbp().end()) {
...
Maybe<const cfg::NdSbp*> Operator::NdSbp4BnInOp(
...
Maybe<void> Operator::ToOpAttribute(OpAttribute* op_attribute) const {
  ...
  *op_attribute->mutable_arg_modifier_signature() = arg_modifier_signature_;
  ...
  if (nd_sbp_signature_) {
    nd_sbp_signature_->ToProto(
        op_attribute->mutable_nd_sbp_signature());
  } else {
...
Maybe<Shape> GetPhysicalShape(const Shape& logical_shape,
  ...
  if (parallel_desc.hierarchy()->NumAxes() == 1) {
    ...
    return GetNdHierarchyPhysicalShape(logical_shape, nd_sbp,
