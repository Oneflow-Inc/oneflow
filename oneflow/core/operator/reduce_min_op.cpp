#include "oneflow/core/operator/reduce_min_op.h"
#include "oneflow/core/operator/reduce_sbp_util.h"
#include "oneflow/core/kernel/kernel_util.h"

namespace oneflow {

void ReduceMinOp::InitFromOpConf() {
  CHECK(op_conf().has_reduce_min_conf());
  EnrollInputBn("in");
  EnrollOutputBn("out");
  EnrollTmpBn("fw_tmp");
}

const PbMessage& ReduceMinOp::GetCustomizedConf() const { return op_conf().reduce_min_conf(); }

Maybe<void> ReduceMinOp::InferBlobDescs(
    std::function<BlobDesc*(const std::string&)> GetBlobDesc4BnInOp,
    const ParallelContext* parallel_ctx) const {
  const ReduceMinOpConf& conf = op_conf().reduce_min_conf();
...
Maybe<void> ReduceMinOp::InferBatchAxis(
    std::function<OptInt64*(const std::string&)> BatchAxis4BnInOp) const {
  const auto& reduced_axes = op_conf().reduce_min_conf().axis();
  const bool keep_dims = op_conf().reduce_min_conf().keep_dims();
...
Maybe<void> ReduceMinOp::GetSbpSignatures(
    const std::function<Maybe<const BlobDesc*>(const std::string&)>& LogicalBlobDesc4Ibn,
    SbpSignatureList* sbp_sig_list) const {
  int32_t num_axes = JUST(LogicalBlobDesc4Ibn("in"))->shape().NumAxes();
  auto IsReducedAxis =
      ReduceSbpUtil::MakePredicatorIsReducedAxis(op_conf().reduce_min_conf().axis(), num_axes);
...
REGISTER_OP(OperatorConf::kReduceMinConf, ReduceMinOp);
