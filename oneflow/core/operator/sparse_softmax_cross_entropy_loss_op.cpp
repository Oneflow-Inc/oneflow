...
void SparseSoftmaxCrossEntropyLossOp::VirtualInferBlobDescs(
  ...
  CHECK_EQ(pred_blob_desc->body_desc().shape().NumAxes(), 2);
  // prob
  BlobDesc* prob_blob_desc = GetBlobDesc4BnInOp("prob");
  prob_blob_desc->mut_body_desc().mut_shape() = Shape(pred_blob_desc->body_desc().shape());
  prob_blob_desc->mut_body_desc().set_data_type(pred_blob_desc->body_desc().data_type());
