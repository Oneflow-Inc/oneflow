...
Maybe<void> SegmentSumGradOp::InferBlobDescs(
...
Maybe<void> SegmentSumGradOp::GetSbpSignatures(
    const std::function<Maybe<const BlobDesc*>(const std::string&)>& LogicalBlobDesc4Ibn,
    SbpSignatureList* sbp_sig_list) const {
  SbpSignatureBuilder()
      .Split(input_bns(), 0)
      .Split(output_bns(), 0)
      .MakeSplitSignatureListBuilder(JUST(LogicalBlobDesc4Ibn("out_diff"))->shape().NumAxes())
      .Build(sbp_sig_list);
  return Maybe<void>::Ok();
}
