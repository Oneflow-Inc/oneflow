...
void BasicRnnCellOp::InitFromOpConf() {
  ...
  if(op_conf().basic_rnn_cell_conf().rnn_type_case() == BasicRnnCellOpConf::kRnnCell) {
 ...
 if(GetBollFromSpecialConf("has_bias_term")) {
    EnrollModelBn("bias");
    EnrollModelTmpBn("bias_multiplier");
  }
  } else if (op_conf().basic_rnn_cell_conf().rnn_type_case() == BasicRnnCellOpConf::kLstmCell) {
    TODO();
  } else {
    UNEXPECTED_RUN();
  }
...
void BasicRnnCellOp::InferBlobDescs(
 ...
 if (parallel_ctx_policy() == kModelParallel) {
