...
void RMSPropModelUpdateOp::MdUpdtVirtualInitFromOpConf() {
  const auto &rmsprop_conf =
      op_conf().rmsprop_model_update_conf().user_conf().rmsprop_conf();
  EnrollInputBn("mean_square", false)->set_is_mutable(true);
  if (rmsprop_conf.centered()) {
    EnrollInputBn("mean_gradient", false)->set_is_mutable(true);
  }
}
...
RMSPropModelUpdateOp::AlwaysBroadcastParallelBns() const {
  return HashSet<std::string>{};
}

REGISTER_CLASS(NormalModelUpdateOpUserConf::kRmspropConf, NormalModelUpdtOp,
