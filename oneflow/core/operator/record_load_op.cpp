...
void RecordLoadOp::InitFromOpConf() {
  ...
  if (op_conf().record_load_conf().has_tick()) {
    EnrollInputBn("tick", false);
  }
  EnrollOutputBn("out", false);
}

const PbMessage &RecordLoadOp::GetCustomizedConf() const {
  return op_conf().record_load_conf();
}
...
Maybe<void> RecordLoadOp::InferBlobDescs(
  ...
  out_blob_desc->mut_shape() =
      Shape({batch_size / parallel_ctx->parallel_num()});
...
void RecordLoadOp::VirtualGenKernelConf(
  ...
  kernel_conf->mutable_record_load_conf()->set_device_piece_size(
      device_piece_size);
  kernel_conf->mutable_record_load_conf()->set_parallel_id(
      parallel_ctx->parallel_id());
  kernel_conf->mutable_record_load_conf()->set_parallel_num(
...
Maybe<void>
RecordLoadOp::GetSbpSignatures(SbpSignatureList *sbp_sig_list) const {
  SbpSignatureBuilder()
      .Broadcast(input_bns())
      .Split(output_bns(), 0)
