...
const PbMessage &OutputOp::GetCustomizedConf() const {
  return op_conf().output_conf();
}

Maybe<void> OutputOp::InferBatchAxis(
    std::function<OptInt64 *(const std::string &)> BatchAxis4BnInOp) const {
  OptInt64 *out_batch_axis = BatchAxis4BnInOp("out");
  InterfaceOpUtil::InferBatchAxis(op_conf().output_conf().blob_conf(),
                                  out_batch_axis);
...
Maybe<void> OutputOp::InferSbpSignature(
  ...
  InterfaceOpUtil::GetOutputLikeOpSbpSignature(
      op_conf().output_conf().blob_conf(), input_bns(), output_bns(),
      sbp_signature);
  return Maybe<void>::Ok();
}
