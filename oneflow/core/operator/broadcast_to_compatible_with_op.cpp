...
Maybe<void> GetBroadcastShape(const Shape &a_shape, const Shape &b_shape,
                              Shape *broadcast_shape) {
  Shape max_shape = Shape::Ones(std::max(a_shape.NumAxes(), b_shape.NumAxes()));
  Shape a_extend_shape =
      CreateLeftExtendedShape(ShapeView(a_shape), max_shape.NumAxes());
  Shape b_extend_shape =
      CreateLeftExtendedShape(ShapeView(b_shape), max_shape.NumAxes());
...
public:
  ...
  void InitFromOpConf() {
    ...
    FOR_RANGE(int, i, 0,
              op_conf().broadcast_to_compatible_with_conf().compatible_size()) {
      InputBlobModifier *modifer =
          MutInputBlobModifier4Ibn(GenRepeatedBn("compatible", i));
  ...
  Maybe<void> InferBlobDescs(
    ...
    int64_t num_compatibles =
        op_conf().broadcast_to_compatible_with_conf().compatible_size();
    const BlobDesc *x_desc = GetBlobDesc4BnInOp("x");
    Shape broadcasted_shape(x_desc->shape());
    FOR_RANGE(int64_t, i, 0, num_compatibles) {
      const BlobDesc *compatible_i =
          GetBlobDesc4BnInOp(GenRepeatedBn("compatible", i));
      GetBroadcastShape(broadcasted_shape, compatible_i->shape(),
                        &broadcasted_shape);
    }
...
private:
  void VirtualGenKernelConf(
    ...
    Shape x_extend_shape = CreateLeftExtendedShape(ShapeView(x_desc->shape()),
                                                   y_desc->shape().NumAxes());
  ...
  Maybe<void> GetSbpSignatures(const std::function<Maybe<const BlobDesc &>(
    ...
    for (const std::string ibn : input_bns()) {
      ...
      CHECK_OR_RETURN(
          ibn2extend_shape
              .emplace(ibn, CreateLeftExtendedShape(ShapeView(input_shape),
                                                    broadcast_num_axes))
              .second);
    }

    FOR_RANGE(int64_t, i, 0, broadcast_num_axes) {
      if (broadcasted_shape.At(i) == 1) {
        continue;
      }
      SbpSignature sbp_sig;
      for (const auto &pair : ibn2extend_shape) {
        if (pair.second.At(i) == 1) {
          (*sbp_sig.mutable_bn_in_op2sbp_parallel())[pair.first]
              .mutable_broadcast_parallel();
        } else {
          (*sbp_sig.mutable_bn_in_op2sbp_parallel())[pair.first]
              .mutable_split_parallel()
              ->set_axis(i - (broadcast_num_axes - pair.second.NumAxes()));
        }
      }
      (*sbp_sig.mutable_bn_in_op2sbp_parallel())["y"]
          .mutable_split_parallel()
          ->set_axis(i);
...
REGISTER_OP(OperatorConf::kBroadcastToCompatibleWithConf,
