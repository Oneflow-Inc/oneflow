...
public:
  ...
  void InitFromOpConf() override {
    ...
    EnrollRepeatedOutputBnWithSetter(
        "out", false, [](OutputBlobModifier *ob_modifier) {
          ob_modifier->set_header_infered_before_compute(false);
        });
  }

  const PbMessage &GetCustomizedConf() const override {
    return op_conf().tensor_list_split_conf();
  }
  ...
  Maybe<void> InferBlobDescs(
    ...
    DimVector dim_vec{in_desc->shape().dim_vec().begin() + 1,
                      in_desc->shape().dim_vec().end()};
    FOR_RANGE(int, i, 0, N) {
...
private:
  ...
  Maybe<void> InferBatchAxis(std::function<OptInt64 *(const std::string &)>
    ...
    for (const auto &obn : output_bns()) {
      *BatchAxis4BnInOp(obn) = *BatchAxis4BnInOp(SoleIbn());
    }
    return Maybe<void>::Ok();
  }
};
