...
struct NdarrayApplyBroadcastBinaryCoreWrapper<DeviceType::kCPU, T, NDIMS,
                                              binary_func>
    final {
  static void Apply(
      DeviceCtx *ctx,
...
template <typename T, int NDIMS, template <typename> class binary_func>
struct NdarrayApplyBroadcastInplaceBinaryCoreWrapper<DeviceType::kCPU, T, NDIMS,
                                                     binary_func>
    final {
...
OF_PP_SEQ_PRODUCT_FOR_EACH_TUPLE(INSTANTIATE_BROADCAST_BINARY_FUNC,
                                 ...
                                 DIM_SEQ, BINARY_FUNC_SEQ);

#define INSTANTIATE_BROADCAST_INPLACE_BINARY_FUNC(dtype_pair, NDIMS,           \
                                                  binary_func)                 \
