...
public:
  ...
  CpuSliceVarNdarray<CpuSliceVarNdarray<XT>>
  operator()(Slice &&slice0, Slice &&slice1, Slice &&slice2) {
    static_assert(XT::ndims == 3, "NDIMS error");
    return CpuSliceVarNdarray<CpuSliceVarNdarray<XT>>(std::move(*this),
                                                      {slice0, slice1, slice2});
  }
  CpuSliceVarNdarray<CpuSliceVarNdarray<XT>>
  operator()(Slice &&slice0, Slice &&slice1, Slice &&slice2, Slice &&slice3) {
    static_assert(XT::ndims == 4, "NDIMS error");
    return CpuSliceVarNdarray<CpuSliceVarNdarray<XT>>(
        std::move(*this), {slice0, slice1, slice2, slice3});
  }
  CpuSliceVarNdarray<CpuSliceVarNdarray<XT>>
  operator()(Slice &&slice0, Slice &&slice1, Slice &&slice2, Slice &&slice3,
             Slice &&slice4) {
    ...
    return CpuSliceVarNdarray<CpuSliceVarNdarray<XT>>(
        std::move(*this), {slice0, slice1, slice2, slice3, slice4});
  }

  template <typename AT> void CopyFrom(const AT &ndarray) {
    CpuNdarrayCopy(this, ndarray);
  }

  ALWAYS_INLINE void GetMutPtrAndContiguousSize(int64_t offset,
                                                typename XT::dtype **ptr,
                                                size_t *size) const {
    int64_t dim[XT::ndims] = {0};
    this->xpu_shape().template Offset2Coordinate<XT::ndims>(offset, dim);
    for (int i = 0; i < XT::ndims; ++i) {
      dim[i] = this->slice(i).Get(dim[i]);
    }
    size_t x_offset =
        this->x().xpu_shape().template Coordinate2Offset<XT::ndims>(dim);
...
protected:
  ...
  ALWAYS_INLINE void GetMutPtrAndMinContiguousSize(int64_t offset,
                                                   int64_t x_offset,
                                                   typename XT::dtype **ptr,
                                                   size_t *size) const {
...
private:
  static std::array<Slice, XT::ndims> &&
  BoundSlices(const XT &x, std::array<Slice, XT::ndims> &&slices) {
    FOR_RANGE(int32_t, i, 0, XT::ndims) {
      slices[i].Bound(x.xpu_shape().At(i));
    }
    return std::move(slices);
  }
  static Shape
  BoundedSlices2Shape(const std::array<Slice, XT::ndims> &bounded_slices) {
    DimVector dim_vec;
  ...
  void SetContiguousLength(const std::array<Slice, XT::ndims> &bounded_slices) {
    ...
    for (int i = XT::ndims - 1; i >= 0; --i) {
      if (bounded_slices[i].IsContiguous()) {
        contiguous_len_ *= bounded_slices[i].Size();
      }
      if (!(bounded_slices[i].IsContiguous() &&
            bounded_slices[i].IsCoveringAll())) {
        break;
      }
    }
  }
