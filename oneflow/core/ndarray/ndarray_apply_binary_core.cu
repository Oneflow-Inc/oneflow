...
__global__ void NdarrayApplyBinaryApplyGpu(
    size_t n, typename BinaryFuncTrait<binary_func, T>::return_type *y,
    const T *a, const T *b) {
...
struct NdarrayApplyBinaryCoreWrapper<DeviceType::kGPU, T, binary_func> final {
  static void Apply(
    ...
    RUN_CUDA_KERNEL((NdarrayApplyBinaryApplyGpu<T, binary_func>), ctx, n, n,
                    y.host_ptr(), a.host_ptr(), b.host_ptr());
  }
  static void InplaceApply(DeviceCtx *ctx, const XpuVarNdarray<T> &y,
                           const XpuVarNdarray<const T> &x) {
    size_t n = y.host_shape().HostElemNum();
    RUN_CUDA_KERNEL((NdarrayApplyBinaryInplaceApplyGpu<T, binary_func>), ctx, n,
                    n, y.host_ptr(), x.host_ptr());
  }
};

#define INSTANTIATE_NDARRAY_APPLY_BINARY_CORE(dtype_pair, binary_func)         \
  template struct NdarrayApplyBinaryCoreWrapper<                               \
      DeviceType::kGPU, OF_PP_PAIR_FIRST(dtype_pair), binary_func>;
