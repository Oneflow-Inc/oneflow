...
template <DeviceType device_type, typename T,
          template <typename> class unary_func, typename Enable = void>
...
#define DEFINE_NDARRAY_BROADCAST_UNARY(func_name, NDIMS)                       \
  NdarrayApplyBroadcastUnaryCoreWrapper<device_type, T, NDIMS,                 \
                                        unary_func>::func_name
  DEFINE_STATIC_SWITCH_FUNC(void, Apply, DEFINE_NDARRAY_BROADCAST_UNARY,
                            MAKE_NDIM_CTRV_SEQ(DIM_SEQ));
#undef DEFINE_NDARRAY_BROADCAST_UNARY
  static void CheckBroadcastable(const XpuVarNdarray<T> &y,
                                 const XpuVarNdarray<const T> &x) {
