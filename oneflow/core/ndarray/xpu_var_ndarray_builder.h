...
template <typename T> class XpuVarNdarrayBuilder final {
public:
  ...
  typename std::enable_if<!std::is_same<DT, const DT>::value,
                          XpuVarNdarray<DT>>::type
  operator()(Blob *blob, int ndims_extend_to) const {
    return XpuVarNdarray<DT>(blob, ndims_extend_to);
  }
  template <typename DT = T>
  typename std::enable_if<!std::is_same<DT, const DT>::value,
                          XpuVarNdarray<const DT>>::type
