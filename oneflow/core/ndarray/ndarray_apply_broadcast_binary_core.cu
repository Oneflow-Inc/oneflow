...
template <typename Index> struct XY2XFunctor final {
...
template <typename Index> struct XY2YFunctor final {
...
template <typename Index> struct XYZ2XZFunctor final {
...
template <typename Index> struct XYZ2YFunctor final {
  __host__ __device__ XYZ2YFunctor(Index dim_y, Index dim_z)
      : dim_yz_(dim_y * dim_z), dim_z_(dim_z) {}

  __host__ __device__ Index operator()(Index idx) const {
    return (idx % dim_yz_) / dim_z_;
  }
...
template <typename T, typename K, template <typename> class binary_func,
          typename OffsetFunctor>
__global__ void
PartialBroadcastGpu(K n,
                    typename BinaryFuncTrait<binary_func, T>::return_type *y,
                    const T *a, const T *b, OffsetFunctor offset_functor) {
  CUDA_1D_KERNEL_LOOP_T(K, i, n) {
    y[i] = binary_func<T>::Invoke(a[i], b[offset_functor(i)]);
  }
}
...
template <typename T, int NDIMS, template <typename> class binary_func>
struct NdarrayApplyBroadcastBinaryCoreWrapper<DeviceType::kGPU, T, NDIMS,
                                              binary_func>
    final {
  static void Apply(
      DeviceCtx *ctx,
    ...
    if (IsKernelSafeInt32(n) && PartialBroadcast<int32_t>(ctx, y, a, b)) {
      return;
    }
    if (!IsKernelSafeInt32(n) && PartialBroadcast<int64_t>(ctx, y, a, b)) {
      return;
    }
    RUN_CUDA_KERNEL((GpuBroadcastBinaryFunc<T, NDIMS, binary_func>), ctx, n, y,
                    a, b);
  }
    ...
    if (y.host_shape() == a.host_shape()) {
      if (y.host_shape().NumAxes() == 2) {
        ...
        if (b_dim0 == y_dim0 && b_dim1 == 1) {
          ...
          RUN_CUDA_KERNEL(
              (PartialBroadcastGpu<T, K, binary_func, XY2XFunctor<K>>), ctx, n,
              n, y.host_ptr(), a.host_ptr(), b.host_ptr(), xy2x);
          return true;
        }
        if (b_dim0 == 1 && b_dim1 == y_dim1) {
          XY2YFunctor<K> xy2y(y_dim1);
          RUN_CUDA_KERNEL(
              (PartialBroadcastGpu<T, K, binary_func, XY2YFunctor<K>>), ctx, n,
              n, y.host_ptr(), a.host_ptr(), b.host_ptr(), xy2y);
          return true;
        }
      }
      if (y.host_shape().NumAxes() == 3) {
        ...
        if (b_dim0 == y_dim0 && b_dim1 == 1 && b_dim2 == y_dim2) {
          ...
          RUN_CUDA_KERNEL(
              (PartialBroadcastGpu<T, K, binary_func, XYZ2XZFunctor<K>>), ctx,
              n, n, y.host_ptr(), a.host_ptr(), b.host_ptr(), xyz2xz);
          return true;
        }
        if (b_dim0 == 1 && b_dim1 == y_dim1 && b_dim2 == 1) {
          XYZ2YFunctor<K> xyz2y(y_dim1, y_dim2);
          RUN_CUDA_KERNEL(
              (PartialBroadcastGpu<T, K, binary_func, XYZ2YFunctor<K>>), ctx, n,
              n, y.host_ptr(), a.host_ptr(), b.host_ptr(), xyz2y);
          return true;
        }
      }
    }
...
template <typename T, int NDIMS, template <typename> class binary_func>
struct NdarrayApplyBroadcastInplaceBinaryCoreWrapper<DeviceType::kGPU, T, NDIMS,
                                                     binary_func>
    final {
  static void InplaceApply(DeviceCtx *ctx, const XpuVarNdarray<T> &y,
    ...
    using NBB = NdarrayApplyBroadcastBinaryCoreWrapper<DeviceType::kGPU, T,
                                                       NDIMS, binary_func>;
    if (IsKernelSafeInt32(n) &&
        NBB::template PartialBroadcast<int32_t>(ctx, y, a, x)) {
      return;
    }
    if (!IsKernelSafeInt32(n) &&
        NBB::template PartialBroadcast<int64_t>(ctx, y, a, x)) {
      return;
    }
    RUN_CUDA_KERNEL((GpuInplaceBroadcastBinaryFunc<T, NDIMS, binary_func>), ctx,
                    n, y, x);
  }
};
