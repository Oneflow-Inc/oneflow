...
#define SPECIALIZE_CPU_NDARRAY_REDUCE_IMPL(struct_name)                        \
  ...
  struct struct_name<DeviceType::kCPU, T, binary_func> final {                 \
    static bool Matched(const XpuVarNdarray<T> &y,                             \
...
#define INSTANTIATE_NDARRAY_REDUCE_IMPL(dtype, binary_func)                    \
  template struct NdarrayScalarReduce<DeviceType::kCPU,                        \
                                      OF_PP_PAIR_FIRST(dtype), binary_func>;   \
  template struct NdarrayMatrixRowReduce<                                      \
      DeviceType::kCPU, OF_PP_PAIR_FIRST(dtype), binary_func>;                 \
  template struct NdarrayMatrixColReduce<                                      \
      DeviceType::kCPU, OF_PP_PAIR_FIRST(dtype), binary_func>;                 \
  template struct NdarrayXYZCubeYReduce<DeviceType::kCPU,                      \
                                        OF_PP_PAIR_FIRST(dtype), binary_func>; \
  template struct NdarrayXYZCubeXZReduce<                                      \
      DeviceType::kCPU, OF_PP_PAIR_FIRST(dtype), binary_func>;
...
struct NdarrayReduceCoreWrapper<DeviceType::kCPU, T, NDIMS, binary_func> final {
  static void ReduceAxis(DeviceCtx *ctx,
                         const XpuReducedNdarray<T, NDIMS> &dst_reduced,
                         const XpuReducedNdarray<T, NDIMS> &x, int axis) {
    NdarrayReduceCore<T, NDIMS, binary_func>::ReduceAxis(dst_reduced, x, axis);
  }
};

#define INSTANTIATE_NDARRAY_REDUCE_CORE_WRAPPER(dtype_pair, NDIMS,             \
                                                binary_func)                   \
  template struct NdarrayReduceCoreWrapper<                                    \
