...
__global__ void MatrixColReduceBy1ThreadPerColumn(K num_elems, K num_cols,
                                                  const T *in, T *out) {
...
template <typename T> struct WithAlign2 {
  union {
    T value;
    int32_t padding;
  };
...
template <template <typename> class R, typename T, typename K>
__global__ void MatrixColReduceByWarpBlock(K num_elems, K num_cols, const T *in,
                                           T *out) {
  ...
  const K num_valid_threads =
      thread_dim_row * num_cols; // ASSERT: always <= num_elems
  ...
  if (col < num_cols) {
    ...
    T val = in[index];
    for (index += num_valid_threads; index < num_elems;
         index += num_valid_threads) {
...
template <template <typename> class R, typename T, typename K>
void MatrixColReduceBy1BlockLayer(DeviceCtx *ctx, K num_elems, K num_cols,
                                  const T *in, T *out) {
  CHECK_LE(num_cols, kCudaMaxBlocksNum * kCudaWarpSize);
  const K num_rows = num_elems / num_cols;
  CHECK_GT(num_rows, 0);
  if (num_rows < kCudaWarpSize) {
    RUN_CUDA_KERNEL((MatrixColReduceBy1ThreadPerColumn<R, T, K>), ctx, num_cols,
    ...
    Reduce<<<num_blocks, num_threads, 0, ctx->cuda_stream()>>>(
        num_elems, num_cols, in, out);
  }
}

const static int32_t kNumRows4OneBlockLayer = kCudaWarpSize * kCudaWarpSize;
const static int32_t kNumCols4OneBlockLayer =
    kCudaMaxBlocksNum * kCudaWarpSize / 2;

template <template <typename> class R, typename T, typename K>
void MatrixColReduceK(DeviceCtx *ctx, K num_rows, K num_cols, const T *in,
                      T *out, T *tmp) {
  ...
  if (num_rows < kNumRows4OneBlockLayer || num_cols > kNumCols4OneBlockLayer) {
    ...
    for (; true; ++scale_shift) {
      if ((num_rows >> scale_shift) < kNumRows4OneBlockLayer) {
        break;
      }
      if ((num_cols << scale_shift) > kNumCols4OneBlockLayer) {
        break;
      }
    }
    MatrixColReduceBy1BlockLayer<R, T, K>(ctx, num_elems,
                                          (num_cols << scale_shift), in, tmp);
...
void MatrixColReduce(DeviceCtx *ctx, int64_t num_rows, int64_t num_cols,
                     const T *in, T *out, T *tmp) {
  if (IsKernelSafeInt32(num_rows * num_cols)) {
    return MatrixColReduceK<R, T, int32_t>(ctx, num_rows, num_cols, in, out,
                                           tmp);
  } else {
    return MatrixColReduceK<R, T, int64_t>(ctx, num_rows, num_cols, in, out,
                                           tmp);
  }
}
...
OF_PP_FOR_EACH_ATOMIC(SPECIALIZE_CUB_FUNCTOR_4_BINARY_FUNC,
                      REDUCE_BINARY_FUNC_NAME_SEQ);
#undef SPECIALIZE_CUB_FUNCTOR_4_BINARY_FUNC

struct RowOffsetFunctor final {
  OF_DEVICE_FUNC explicit RowOffsetFunctor(int32_t num_cols)
      : num_cols_(num_cols) {}
  OF_DEVICE_FUNC int32_t operator()(const int32_t &x) const {
    return x * num_cols_;
  }
  int32_t num_cols_;
};
...
struct NdarrayMatrixRowReduce<DeviceType::kGPU, T, binary_func> final {
  static bool Matched(const XpuVarNdarray<T> &y,
                      const XpuVarNdarray<const T> &x) {
    if (y.shape().ElemNum() > GetMaxVal<int32_t>()) {
      return false;
    }
    if (x.shape().NumAxes() != 2) {
      return false;
    }
    if (y.shape().NumAxes() != 2) {
      return false;
    }
  ...
  static void Reduce(DeviceCtx *ctx, const XpuVarNdarray<T> &y,
    ...
    cub::TransformInputIterator<int32_t, RowOffsetFunctor,
    ...
    auto DoReduce = [&](T *tmp_storage_ptr) {
      int retcode = cub::DeviceSegmentedReduce::Reduce(
          tmp_storage_ptr, tmp_storage_bytes, x.ptr(), y.ptr(), num_rows,
          transform_input_iter, transform_input_iter + 1,
          typename CubFunctor4BianryFunc<T, binary_func>::type(),
          UnitOfBinaryFunc<T, binary_func>::Val(), ctx->cuda_stream());
      CHECK_EQ(retcode, 0) << "cub::DeviceSegmentedReduce::Reduce error";
    };
...
struct NdarrayMatrixColReduce<DeviceType::kGPU, T, binary_func> final {
  ...
  struct XY2YXFunctor final {
    __host__ __device__ XY2YXFunctor(int32_t dim_x, int32_t dim_y)
  ...
  static void Reduce(DeviceCtx *ctx, const XpuVarNdarray<T> &y,
    ...
    if (num_cols < kNumCols4OneBlockLayer) {
      return MatrixColReduce<binary_func, T>(ctx, num_rows, num_cols,
                                             x.host_ptr(), y.host_ptr(),
    ...
    using XY2YxIndexIter =
        cub::TransformInputIterator<int32_t, XY2YXFunctor,
                                    cub::CountingInputIterator<int32_t>>;
    XY2YxIndexIter xy2yx_iter(counting_intput_it, xy2yx);
    PermutationIterator<const T, const T *, XY2YxIndexIter> x_iter(x.ptr(),
                                                                   xy2yx_iter);
    size_t tmp_storage_bytes = 0;
    auto DoReduce = [&](T *tmp_storage_ptr) {
      int retcode = cub::DeviceSegmentedReduce::Reduce(
          tmp_storage_ptr, tmp_storage_bytes, x_iter, y.ptr(), num_cols,
          transform_input_iter, transform_input_iter + 1,
          typename CubFunctor4BianryFunc<T, binary_func>::type(),
          UnitOfBinaryFunc<T, binary_func>::Val(), ctx->cuda_stream());
      CHECK_EQ(retcode, 0) << "cub::DeviceSegmentedReduce::Reduce error";
...
struct NdarrayXYZCubeXZReduce<DeviceType::kGPU, T, binary_func> final {
  ...
  struct XYZ2YxzFunctor final {
    __host__ __device__ XYZ2YxzFunctor(int32_t dim_x, int32_t dim_y,
                                       int32_t dim_z)
  ...
  static void Reduce(DeviceCtx *ctx, const XpuVarNdarray<T> &y,
    ...
    using XYZ2YxzIndexIter =
        cub::TransformInputIterator<int32_t, XYZ2YxzFunctor,
                                    cub::CountingInputIterator<int32_t>>;
    XYZ2YxzIndexIter xyz2yxz_iter(counting_intput_it, xyz2yxz);
    PermutationIterator<const T, const T *, XYZ2YxzIndexIter> x_iter(
        x.ptr(), xyz2yxz_iter);
    size_t tmp_storage_bytes = 0;
    auto DoReduce = [&](T *tmp_storage_ptr) {
      int retcode = cub::DeviceSegmentedReduce::Reduce(
          tmp_storage_ptr, tmp_storage_bytes, x_iter, y.ptr(), num_rows,
          transform_input_iter, transform_input_iter + 1,
          typename CubFunctor4BianryFunc<T, binary_func>::type(),
          UnitOfBinaryFunc<T, binary_func>::Val(), ctx->cuda_stream());
      CHECK_EQ(retcode, 0) << "cub::DeviceSegmentedReduce::Reduce error";
...
__global__ void
NdarrayReduceGpuInplaceReduceAxis(const XpuReducedNdarray<T, NDIMS> dst_reduced,
                                  const XpuReducedNdarray<T, NDIMS> x,
                                  int axis) {
...
struct NdarrayReduceCoreWrapper<DeviceType::kGPU, T, NDIMS, binary_func> final {
  static void ReduceAxis(DeviceCtx *ctx,
    ...
    RUN_CUDA_KERNEL((NdarrayReduceGpuInplaceReduceAxis<T, NDIMS, binary_func>),
                    ctx, n, dst_reduced, x, axis);
  }
};

#define INSTANTIATE_NDARRAY_REDUCE_IMPL(dtype, binary_func)                    \
  template struct NdarrayScalarReduce<DeviceType::kGPU,                        \
                                      OF_PP_PAIR_FIRST(dtype), binary_func>;   \
  template struct NdarrayMatrixRowReduce<                                      \
      DeviceType::kGPU, OF_PP_PAIR_FIRST(dtype), binary_func>;                 \
  template struct NdarrayMatrixColReduce<                                      \
      DeviceType::kGPU, OF_PP_PAIR_FIRST(dtype), binary_func>;                 \
  template struct NdarrayXYZCubeYReduce<DeviceType::kGPU,                      \
                                        OF_PP_PAIR_FIRST(dtype), binary_func>; \
