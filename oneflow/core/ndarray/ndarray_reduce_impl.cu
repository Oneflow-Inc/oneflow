...
template<template<typename> class R, typename T, typename K, typename RetT>
void MatrixColReduceBy1BlockLayer(DeviceCtx* ctx, K num_elems, K num_cols, const T* in, RetT* out) {
  CHECK_LE(num_cols, kCudaMaxBlocksNum * kCudaWarpSize);
  const K num_rows = num_elems / num_cols;
  CHECK_GT(num_rows, 0);
  if (num_rows < kCudaWarpSize) {
    RUN_CUDA_KERNEL((MatrixColReduceBy1ThreadPerColumn<R, T, K, RetT>), ctx, num_cols, num_elems,
                    num_cols, in, out);
  } else {
...
void MatrixColReduceK(DeviceCtx* ctx, K num_rows, K num_cols, const T* in,
  ...
  if (num_rows < kNumRows4OneBlockLayer || num_cols > kNumCols4OneBlockLayer) {
    ...
    MatrixColReduceBy1BlockLayer<R, T, K, T>(ctx, num_elems, (num_cols << scale_shift), in, tmp);
...
void MatrixColReduce(DeviceCtx* ctx, int64_t num_rows, int64_t num_cols, const T* in,
                     typename BinaryFuncTrait<R, T>::return_type* out, T* tmp) {
...
struct NdarrayScalarReduce<DeviceType::kGPU, T, binary_func> final {
  ...
  static void Reduce(DeviceCtx* ctx, const XpuVarNdarray<RetT>& y, const XpuVarNdarray<const T>& x,
