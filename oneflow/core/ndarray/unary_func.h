...
struct UnaryFuncTrait final {
  typedef typename std::remove_const<decltype(
      UnaryFunc<T>::Invoke(*(const T *)nullptr))>::type return_type;
};

#define SPECIALIZE_CONST_TYPE_UNARY_FUNC(func_struct)                          \
  template <typename T> struct func_struct<const T> final {                    \
    static OF_DEVICE_FUNC const T Invoke(const T x) {                          \
      return func_struct<T>::Invoke(x);                                        \
    }                                                                          \
  }

template <typename T> struct UnaryFuncIdentity final {
  static OF_DEVICE_FUNC const T Invoke(const T x) { return x; }
};

template <typename T> struct UnaryFuncNegative final {
  static OF_DEVICE_FUNC const T Invoke(const T x) { return -x; }
};
SPECIALIZE_CONST_TYPE_UNARY_FUNC(UnaryFuncNegative);

template <typename T> struct UnaryFuncExp final {
  static OF_DEVICE_FUNC const T Invoke(const T x) { return std::exp(x); }
};
SPECIALIZE_CONST_TYPE_UNARY_FUNC(UnaryFuncExp);

template <> struct UnaryFuncExp<float16> final {
...
#if defined(__CUDACC__)
template <> struct UnaryFuncNegative<half> final {
...
template <> struct UnaryFuncExp<half> final {
  static __device__ __forceinline__ const half Invoke(const half x) {
