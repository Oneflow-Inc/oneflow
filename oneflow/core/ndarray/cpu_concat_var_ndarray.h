...
public:
  ...
  static_assert(CONCAT_AXES >= 0 && CONCAT_AXES < NDIMS,
                "CONCAT_AXES should be a valid dim");
  CpuConcatVarNdarray(const std::vector<CpuVarNdarray<T, NDIMS>> &var_ndarrays)
      : CpuNdarray<T, NDIMS>(CalcConcatenatedShape(var_ndarrays)),
        var_ndarrays_(var_ndarrays), dim_ranges_(CalcDimRanges(var_ndarrays)),
        contiguous_lens_(CalcContiguousLens(var_ndarrays)) {}
  ~CpuConcatVarNdarray() = default;

  template <typename XT> void CopyFrom(const XT &ndarray) {
  ...
  void GetMutPtrAndContiguousSize(int64_t offset, T **ptr, size_t *size) const {
    ...
    int32_t var_index = 0;
    this->GetVarNdarrayIndexAndInputDim(dim[CONCAT_AXES], &var_index,
                                        &dim[CONCAT_AXES]);
    int64_t input_offset = this->var_ndarray(var_index)
                               .xpu_shape()
                               .template Coordinate2Offset<NDIMS>(dim);
    this->GetMutPtrAndMinContiguousSize(var_index, input_offset, ptr, size);
  }

protected:
  ALWAYS_INLINE void GetVarNdarrayIndexAndInputDim(int64_t output_dim,
                                                   int32_t *var_index,
                                                   int64_t *input_dim) const {
    *var_index = CpuVarNdarrayIndex4OutputDim(output_dim);
    *input_dim = output_dim - dim_ranges_[*var_index].begin();
  }
  ALWAYS_INLINE const CpuVarNdarray<T, NDIMS>
  var_ndarray(int32_t var_index) const {
    return var_ndarrays_[var_index];
  }
  ALWAYS_INLINE void GetMutPtrAndMinContiguousSize(int32_t var_index,
                                                   int64_t var_offset, T **ptr,
                                                   size_t *size) const {
    size_t var_contiguous_size = 0;
    var_ndarray(var_index).GetMutPtrAndContiguousSize(var_offset, ptr,
                                                      &var_contiguous_size);
    *size = std::min(var_contiguous_size,
                     contiguous_lens_[var_index] -
...
private:
  ALWAYS_INLINE int32_t CpuVarNdarrayIndex4OutputDim(int64_t output_dim) const {
    ...
    FOR_RANGE(int32_t, i, 0, dim_ranges_.size()) {
      if (output_dim >= dim_ranges_[i].begin() &&
          output_dim < dim_ranges_[i].end()) {
        return i;
      }
    }
    UNIMPLEMENTED();
  }
  XpuShape CalcConcatenatedShape(
  ...
  void CheckInputShape(
    ...
    FOR_RANGE(int32_t, i, 1, var_ndarrays.size()) {
      FOR_RANGE(int32_t, j, 0, NDIMS) {
        ...
        CHECK_EQ(var_ndarrays[0].xpu_shape().At(j),
                 var_ndarrays[i].xpu_shape().At(j));
      }
    }
  }
  std::vector<Range> CalcDimRanges(
      const std::vector<CpuVarNdarray<T, NDIMS>> &var_ndarrays) const {
    int64_t axes_dim_num = 0;
