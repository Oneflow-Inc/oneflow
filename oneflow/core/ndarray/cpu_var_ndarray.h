...
class Slice;
template <typename XT> class CpuSliceVarNdarray;
...
public:
  ...
  CpuVarNdarray(const ShapeView &shape_view, T *ptr)
      : CpuNdarray<T, NDIMS>(XpuShape(shape_view)), ptr_(ptr),
        len_(shape_view.elem_cnt()) {
    CHECK_GT(len_, 0);
  }
  ...
  CpuSliceVarNdarray<CpuVarNdarray<T, NDIMS>>
  operator()(Slice &&slice0, Slice &&slice1, Slice &&slice2) {
    static_assert(NDIMS == 3, "NDIMS error");
    return CpuSliceVarNdarray<CpuVarNdarray<T, NDIMS>>(
        std::move(*this), {slice0, slice1, slice2});
  }
  CpuSliceVarNdarray<CpuVarNdarray<T, NDIMS>>
  ...
  ALWAYS_INLINE void GetMutPtrAndContiguousSize(int64_t offset, T **ptr,
                                                size_t *size) const {
    *ptr = ptr_ + offset;
    *size = len_ - offset;
  }
