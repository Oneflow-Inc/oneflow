...
struct NdarrayDefaultReduce final {
  static void Reduce(DeviceCtx *ctx, const XpuVarNdarray<T> &y,
                     const XpuVarNdarray<const T> &x,
                     const XpuVarNdarray<T> &tmp_storage) {
    return SwitchReduce(SwitchCase(y.shape().NumAxes()), ctx, y, x,
                        tmp_storage);
  }

private:
#define DEFINE_NDARRAY_REDUCE(func_name, NDIMS) func_name<NDIMS>
  DEFINE_STATIC_SWITCH_FUNC(void, Reduce, DEFINE_NDARRAY_REDUCE,
...
#undef DEFINE_NDARRAY_REDUCE
  ...
  template <int NDIMS>
  static void InplaceReduceAxis(DeviceCtx *ctx, int axis,
                                const XpuVarNdarray<T> &implace,
                                XpuShape *cur_shape) {
    ...
    while (cur_shape->At(axis) > 1) {
      ...
      NdarrayReduceCoreWrapper<device_type, T, NDIMS, binary_func>::ReduceAxis(
          ctx, to, from, axis);
    }
  }
};
...
struct NdarrayReduceCore final {
  ...
  ReduceAxis(const XpuReducedNdarray<T, NDIMS> &dst_reduced, const X &x,
             int axis) {
