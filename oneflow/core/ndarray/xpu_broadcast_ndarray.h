...
template <typename T, int NDIMS> struct XpuBroadcastNdarrayUtil;

template <typename T>
class XpuBroadcastNdarray final
    : public XpuNdarrayBase<XpuBroadcastNdarray<T>, T> {
public:
  OF_DEVICE_FUNC XpuBroadcastNdarray(const XpuShape &shape,
                                     const XpuVarNdarray<T> &var)
...
#define SPECIALIZE_XPU_BROADCAST_NDARRAY_UTIL(n)                               \
  template <typename T> struct XpuBroadcastNdarrayUtil<T, n + 1> final {       \
    OF_DEVICE_FUNC static void SrcCoordinate(const XpuShape &src_shape,        \
                                             int64_t coord[n + 1]) {           \
