...
struct NdarrayApplyBroadcastBinary<
  ...
  static void Apply(DeviceCtx *ctx, const XpuVarNdarray<RetT> &y,
                    const XpuVarNdarray<const T> &a,
                    const XpuVarNdarray<const T> &b) {
    if (a.shape() == b.shape()) {
      return NdarrayApplyBinary<device_type, T, binary_func>::Apply(ctx, y, a,
                                                                    b);
    }
    if (TryInplaceApply<std::is_same<RetT, T>::value>(ctx, y, a, b)) {
      return;
    }
    ...
    SimplifyBroadcastShapes(y.shape(), a.shape(), b.shape(), &simplified_y_dim,
                            &simplified_a_dim, &simplified_b_dim);
    return SwitchApply(
        SwitchCase(simplified_y_dim.size()), ctx,
  ...
  static typename std::enable_if<enabled, bool>::type
  TryInplaceApply(DeviceCtx *ctx, const XpuVarNdarray<RetT> &y,
                  const XpuVarNdarray<const T> &a,
                  const XpuVarNdarray<const T> &b) {
    bool is_inplace = (y.shape() == a.shape() && y.ptr() == a.ptr());
    if (is_inplace) {
      InplaceApply(ctx, y, b);
    }
    return is_inplace;
  }
  ...
  static void InplaceApply(DeviceCtx *ctx, const XpuVarNdarray<T> &y,
    ...
    if (y.shape() == x.shape()) {
      return NdarrayApplyBinary<device_type, T, binary_func>::InplaceApply(
          ctx, y, x);
    }
    CheckBroadcastable(y, reinterpret_cast<const XpuVarNdarray<const T> &>(y),
                       x);
    DimVector simplified_y_dim;
    DimVector simplified_x_dim;
    SimplifyBroadcastShapes(y.shape(), x.shape(), &simplified_y_dim,
                            &simplified_x_dim);
    return SwitchInplaceApply(
        SwitchCase(simplified_y_dim.size()), ctx,
...
#define MAKE_NDARRAY_BROADCAST_BINARY(func_name, NDIMS)                        \
  NdarrayApplyBroadcastBinaryCoreWrapper<device_type, T, NDIMS,                \
...
#define MAKE_NDARRAY_INPLACE_BROADCAST_BINARY(func_name, NDIMS)                \
  NdarrayApplyBroadcastInplaceBinaryCoreWrapper<device_type, T, NDIMS,         \
                                                binary_func>::func_name
  DEFINE_STATIC_SWITCH_FUNC(void, InplaceApply,
                            MAKE_NDARRAY_INPLACE_BROADCAST_BINARY,
                            MAKE_NDIM_CTRV_SEQ(DIM_SEQ));
#undef MAKE_NDARRAY_INPLACE_BROADCAST_BINARY

  static void CheckBroadcastable(
      const XpuVarNdarray<typename BinaryFuncTrait<binary_func, T>::return_type>
          &y,
...
struct NdarrayApplyBroadcastBinary<
  ...
  static void InplaceApply(DeviceCtx *ctx, const XpuVarNdarray<T> &y,
                           const XpuVarNdarray<const T> &x) {
    return NdarrayApplyBroadcastBinary<device_type, NewT, binary_func>::
        InplaceApply(ctx, reinterpret_cast<const XpuVarNdarray<NewT> &>(y),
