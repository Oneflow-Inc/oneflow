...
TEST(CpuSliceVarNdarray, 1d_slice_assign) {
  ...
  CpuNdarrayBuilder<int32_t, 1> ndarray;
  auto &&data_ndarray =
      ndarray.Var({static_cast<int64_t>(data.size())}, data.data());
  auto &&buffer_ndarray = ndarray.Var({10LL}, buffer.data());
...
TEST(CpuSliceVarNdarray, 1d_slice) {
  ...
  auto &&data_ndarray =
      ndarray.Var({static_cast<int64_t>(data.size())}, data.data());
  auto &&buffer_ndarray =
      ndarray.Var({static_cast<int64_t>(buffer.size())}, buffer.data());
  buffer_ndarray({}).CopyFrom(data_ndarray({1, -1}));
  ASSERT_EQ(
      memcmp(expected.data(), buffer.data(), sizeof(int32_t) * buffer.size()),
      0);
}
