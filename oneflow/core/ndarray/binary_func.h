...
#define BINARY_FUNC_NAME_SEQ                                                   \
  ARITHMETIC_BINARY_FUNC_NAME_SEQ LOGICAL_BINARY_FUNC_NAME_SEQ
...
#define SPECIALIZE_CONST_TYPE_BINARY_FUNC(func_struct)                         \
  template <typename T> struct func_struct<const T> final {                    \
    static OF_DEVICE_FUNC const typename BinaryFuncTrait<func_struct,          \
                                                         T>::return_type       \
    Invoke(const T x, const T y) {                                             \
      return func_struct<T>::Invoke(x, y);                                     \
    }                                                                          \
  }

template <typename T> struct BinaryFuncAdd final {
  static OF_DEVICE_FUNC const T Invoke(const T x, const T y) { return x + y; }
};
template <typename T> struct BinaryFuncSum final {
...
template <typename T> struct BinaryFuncSub final {
  static OF_DEVICE_FUNC const T Invoke(const T x, const T y) { return x - y; }
};
SPECIALIZE_CONST_TYPE_BINARY_FUNC(BinaryFuncSub);

template <typename T> struct BinaryFuncMul final {
  static OF_DEVICE_FUNC const T Invoke(const T x, const T y) { return x * y; }
};
template <typename T> struct BinaryFuncProd final {
  static OF_DEVICE_FUNC const T Invoke(const T x, const T y) {
...
template <typename T> struct BinaryFuncDiv final {
  static OF_DEVICE_FUNC const T Invoke(const T x, const T y) { return x / y; }
};
SPECIALIZE_CONST_TYPE_BINARY_FUNC(BinaryFuncDiv);

template <typename T> struct BinaryFuncFloorMod final {
  static OF_DEVICE_FUNC const T Invoke(const T x, const T y) {
#if defined(__CUDACC__)
    T trunc_mod = x % y;
    return (trunc_mod != T(0)) && ((y < T(0)) != (trunc_mod < T(0)))
               ? trunc_mod + y
               : trunc_mod;
#else
    T trunc_mod = x % y;
...
template <typename T> struct BinaryFuncMax final {
  static OF_DEVICE_FUNC const T Invoke(const T x, const T y) {
    return x > y ? x : y;
  }
};
...
template <typename T> struct BinaryFuncMin final {
  static OF_DEVICE_FUNC const T Invoke(const T x, const T y) {
    return x < y ? x : y;
  }
};
...
template <typename T> struct BinaryFuncEQ final {
...
template <typename T> struct BinaryFuncNE final {
  static OF_DEVICE_FUNC const int8_t Invoke(const T x, const T y) {
    return x != y;
  }
};
...
template <typename T> struct BinaryFuncGT final {
  static OF_DEVICE_FUNC const int8_t Invoke(const T x, const T y) {
    return x > y;
  }
};
...
template <typename T> struct BinaryFuncGE final {
  static OF_DEVICE_FUNC const int8_t Invoke(const T x, const T y) {
    return x >= y;
  }
};
...
template <typename T> struct BinaryFuncLT final {
  static OF_DEVICE_FUNC const int8_t Invoke(const T x, const T y) {
    return x < y;
  }
};
...
template <typename T> struct BinaryFuncLE final {
  static OF_DEVICE_FUNC const int8_t Invoke(const T x, const T y) {
    return x <= y;
  }
};
...
template <typename T> struct BinaryFuncAND final {
  static OF_DEVICE_FUNC const int8_t Invoke(const T x, const T y) {
    return x && y;
  }
};
template <typename T> struct BinaryFuncAll final {
  static OF_DEVICE_FUNC const int8_t Invoke(const T x, const T y) {
...
template <typename T> struct BinaryFuncAny final {
  static OF_DEVICE_FUNC const int8_t Invoke(const T x, const T y) {
    return x || y;
  }
};
...
template <> struct BinaryFuncAdd<half> final {
...
template <> struct BinaryFuncSub<half> final {
  static __device__ __forceinline__ const half Invoke(const half x,
                                                      const half y) {
...
template <> struct BinaryFuncMul<half> final {
  static __device__ __forceinline__ const half Invoke(const half x,
                                                      const half y) {
#if defined(__CUDA_ARCH__) && __CUDA_ARCH__ >= 530
    return __hmul(x, y);
...
template <> struct BinaryFuncDiv<half> final {
  static __device__ __forceinline__ const half Invoke(const half x,
                                                      const half y) {
...
template <> struct BinaryFuncMax<half> final {
  static __device__ __forceinline__ const half Invoke(const half x,
                                                      const half y) {
#if defined(__CUDA_ARCH__) && __CUDA_ARCH__ >= 530
...
template <> struct BinaryFuncMin<half> final {
  static __device__ __forceinline__ const half Invoke(const half x,
                                                      const half y) {
#if defined(__CUDA_ARCH__) && __CUDA_ARCH__ >= 530
...
template <> struct BinaryFuncFloorMod<float> final {
  static __device__ __forceinline__ const float Invoke(const float x,
                                                       const float y) {
...
template <> struct BinaryFuncFloorMod<double> final {
  static __device__ __forceinline__ const double Invoke(const double x,
                                                        const double y) {
...
template <> struct BinaryFuncFloorMod<half> final {
  static __device__ __forceinline__ const half Invoke(const half x,
                                                      const half y) {
#if __CUDA_ARCH__ >= 530
    const half trunc_mod =
        __float2half(fmodf(__half2float(x), __half2float(y)));
    return __hne(trunc_mod, GetZeroVal<half>()) &&
                   __hne(__hlt(y, GetZeroVal<half>()),
                         __hlt(trunc_mod, half(0)))
               ? trunc_mod + y
               : trunc_mod;
#else
    NO_HALF_UTIL_FOUND;
...
template <> struct BinaryFuncFloorMod<float16> final {
...
#define SPECIALIZE_UNIT_OF_BINARY_FUNC(binary_func, get_val)                   \
  template <typename T> struct UnitOfBinaryFunc<T, binary_func> final {        \
