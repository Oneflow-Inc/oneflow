...
public:
  ...
  template <int NDIMS> OF_DEVICE_FUNC T Get(int64_t offset) const {
    return ptr_[offset];
  }
  template <int NDIMS> OF_DEVICE_FUNC T Get(int64_t coord[NDIMS]) const {
    return ptr_[shape().template Coordinate2Offset<NDIMS>(coord)];
  }

  template <int NDIMS> OF_DEVICE_FUNC T *Mut(int64_t offset) const {
    return ptr_ + offset;
  }

  template <int NDIMS> OF_DEVICE_FUNC T *Mut(int64_t coord[NDIMS]) const {
    return ptr_ + shape().template Coordinate2Offset<NDIMS>(coord);
  }
