...
struct NdarrayApplyBinaryCoreWrapper<DeviceType::kCPU, T, binary_func> final {
  static void Apply(
    ...
    NdarrayApplyBinaryCore<T, binary_func>::Apply(y.shape().ElemNum(), y.ptr(),
                                                  a.ptr(), b.ptr());
  }
  static void InplaceApply(DeviceCtx *ctx, const XpuVarNdarray<T> &y,
                           const XpuVarNdarray<const T> &x) {
    NdarrayApplyBinaryCore<T, binary_func>::InplaceApply(y.shape().ElemNum(),
                                                         y.ptr(), x.ptr());
  }
};
...
#define INSTANTIATE_NDARRAY_APPLY_BINARY_CORE(dtype_pair, binary_func)         \
  template struct NdarrayApplyBinaryCoreWrapper<                               \
      DeviceType::kCPU, OF_PP_PAIR_FIRST(dtype_pair), binary_func>;
