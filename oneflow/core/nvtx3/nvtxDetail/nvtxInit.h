...
#define NVTX_ATOMIC_CAS_32(old, address, exchange, comparand) \
  __sync_synchronize();                                       \
  old = __sync_val_compare_and_swap(address, exchange, comparand)
#else
...
#pragma warning(pop)
    ...
    if (!injectionLibraryPath) {
       ...
       * the colon. */
      pos = 0;
      while (pos < bytesRead && pkgName[pos] != ':' && pkgName[pos] != '\0') { ++pos; }
      pkgName[pos] = 0;

      count = snprintf(injectionLibraryPathBuf, NVTX_BUFSIZE,
                       "/data/data/%s/files/libNvtxInjection%s.so", pkgName, bits);
    ...
    if (injectionLibraryPath) {
      ...
      if (!injectionLibraryHandle) {
        ...
        if (!init_fnptr) {
          ...
          NVTX_ERR("Failed to get address of function InitializeInjectionNvtx2 from injection "
                   "library\n");
...
NVTX_LINKONCE_DEFINE_FUNCTION void NVTX_VERSIONED_IDENTIFIER(nvtxInitOnce)(void) {
  ...
  NVTX_ATOMIC_CAS_32(old, &NVTX_VERSIONED_IDENTIFIER(nvtxGlobals).initState,
                     NVTX_INIT_STATE_STARTED, NVTX_INIT_STATE_FRESH);
