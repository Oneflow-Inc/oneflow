...
void PosixFileSystem::DelFile(const std::string &fname) {
  PCHECK(unlink(TranslateName(fname).c_str()) == 0) << "Fail to delete file "
                                                    << fname;
}
...
void PosixFileSystem::DeleteDir(const std::string &dirname) {
  PCHECK(rmdir(TranslateName(dirname).c_str()) == 0) << "Fail to delete dir "
                                                     << dirname;
}
...
void PosixFileSystem::RenameFile(const std::string &old_name,
  ...
  PCHECK(rename(TranslateName(old_name).c_str(),
                TranslateName(new_name).c_str()) == 0)
