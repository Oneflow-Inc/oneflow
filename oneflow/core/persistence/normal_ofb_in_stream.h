#ifndef ONEFLOW_CORE_PERSISTENCE_NORMAL_DATA_SET_IN_STREAM_H_
#define ONEFLOW_CORE_PERSISTENCE_NORMAL_DATA_SET_IN_STREAM_H_
#include "oneflow/core/persistence/ofb_in_stream.h"
namespace oneflow {

class NormalOfbInStream : public OfbInStream {
 public:
  OF_DISALLOW_COPY_AND_MOVE(NormalOfbInStream);
  NormalOfbInStream() = delete;
  NormalOfbInStream(fs::FileSystem* fs, const std::string& file_path)
      : OfbInStream(fs, file_path) {}
};

}  // namespace oneflow
#endif  // ONEFLOW_CORE_PERSISTENCE_NORMAL_DATA_SET_IN_STREAM_H_
