...
SSIZE_T pread(HANDLE hfile, char* src, size_t num_bytes, uint64_t offset) {
  ...
  BOOL read_result =
      ::ReadFile(hfile, src, static_cast<DWORD>(num_bytes), &bytes_read, &overlapped);
  if (TRUE == read_result) {
    result = bytes_read;
  } else if ((FALSE == read_result) && ((last_error = GetLastError()) != ERROR_IO_PENDING)) {
    result = (last_error == ERROR_HANDLE_EOF) ? 0 : -1;
  } else {
    if (ERROR_IO_PENDING == last_error) {  // Otherwise bytes_read already has the result.
      BOOL overlapped_result = ::GetOverlappedResult(hfile, &overlapped, &bytes_read, TRUE);
...
class WindowsRandomAccessFile : public RandomAccessFile {
 ...
 public:
  ...
  ~WindowsRandomAccessFile() override {
    if (hfile_ != NULL && hfile_ != INVALID_HANDLE_VALUE) { ::CloseHandle(hfile_); }
  }
  ...
  void Read(uint64_t offset, size_t n, char* result) const override {
    ...
    while (n > 0) {
      ...
      if (r > 0) {
        ...
        PLOG(FATAL) << "Read file warning for : " + filename_ + " Read fewer bytes than requested";
        return;
...
class WindowsWritableFile : public WritableFile {
 ...
 public:
  WindowsWritableFile(const std::string& fname, HANDLE hFile) : filename_(fname), hfile_(hFile) {}

  ~WindowsWritableFile() override {
    if (hfile_ != NULL && hfile_ != INVALID_HANDLE_VALUE) { WindowsWritableFile::Close(); }
  }

  void Append(const char* data, size_t n) override {
    DWORD bytes_written = 0;
    DWORD data_size = static_cast<DWORD>(n);
    BOOL write_result = ::WriteFile(hfile_, data, data_size, &bytes_written, NULL);
  ...
  void Close() override {
    ...
    Flush();
    PCHECK(FALSE != ::CloseHandle(hfile_)) << "CloseHandle failed for:" + filename_;
    hfile_ = INVALID_HANDLE_VALUE;
  }

  void Flush() override {
    PCHECK(FALSE != ::FlushFileBuffers(hfile_)) << "FlushFileBuffers failed for: " + filename_;
  }
};

}  // namespace

void WindowsFileSystem::NewRandomAccessFile(const std::string& fname,
  ...
  HANDLE hfile = ::CreateFileW(ws_translated_fname.c_str(), GENERIC_READ, share_mode, NULL,
                               OPEN_EXISTING, file_flags, NULL);
  PCHECK(INVALID_HANDLE_VALUE != hfile) << "NewRandomAccessFile failed to Create/Open: " + fname;
...
void WindowsFileSystem::NewWritableFile(const std::string& fname,
  ...
  DWORD share_mode = FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE;
  HANDLE hfile = ::CreateFileW(ws_translated_fname.c_str(), GENERIC_WRITE, share_mode, NULL,
                               CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
  PCHECK(INVALID_HANDLE_VALUE != hfile) << "Failed to create a NewWriteableFile: " + fname;
  result->reset(new WindowsWritableFile(translated_fname, hfile));
}

void WindowsFileSystem::NewAppendableFile(const std::string& fname,
                                          std::unique_ptr<WritableFile>* result) {
  std::string translated_fname = TranslateName(fname);
  std::wstring ws_translated_fname = Utf8ToWideChar(translated_fname);
  result->reset();
  ...
  PCHECK(INVALID_HANDLE_VALUE != hfile) << "Failed to create a NewAppendableFile: " + fname;
  UniqueCloseHandlePtr file_guard(hfile, CloseHandleFunc);
  DWORD file_ptr = ::SetFilePointer(hfile, NULL, NULL, FILE_END);
  PCHECK(INVALID_SET_FILE_POINTER != file_ptr) << "Failed to create a NewAppendableFile: " + fname;
  result->reset(new WindowsWritableFile(translated_fname, hfile));
  file_guard.release();
}
...
std::vector<std::string> WindowsFileSystem::ListDir(const std::string& dir) {
  ...
  PCHECK(find_handle != INVALID_HANDLE_VALUE) << "FindFirstFile failed for: " + translated_dir;
  do {
...
void WindowsFileSystem::DelFile(const std::string& fname) {
  ...
  PCHECK(_wunlink(file_name.c_str()) == 0) << "Failed to delete a file: " + fname;
}

void WindowsFileSystem::CreateDir(const std::string& name) {
  std::wstring ws_name = Utf8ToWideChar(name);
  PCHECK(_wmkdir(ws_name.c_str()) == 0) << "Failed to create a directory: " + name;
}

void WindowsFileSystem::DeleteDir(const std::string& name) {
  std::wstring ws_name = Utf8ToWideChar(name);
  PCHECK(_wrmdir(ws_name.c_str()) == 0) << "Failed to remove a directory: " + name;
}
...
uint64_t WindowsFileSystem::GetFileSize(const std::string& fname) {
  ...
  PCHECK(TRUE == ::GetFileAttributesExW(ws_translated_dir.c_str(), GetFileExInfoStandard, &attrs))
...
void WindowsFileSystem::RenameFile(const std::string& old_name, const std::string& new_name) {
