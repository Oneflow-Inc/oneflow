...
//
//  HMODULE hModule =
//      LoadLibraryExW(ws_file_name.c_str(), NULL,
...
//      // concurrent readers.
//      std::unique_lock<std::mutex> lock(mu_);
//      tSize r = hdfs_->hdfsPread(fs_, file_, static_cast<tOffset>(offset),
//      dst,
//                                 static_cast<tSize>(n));
//      if (r > 0) {
//        dst += r;
...
//        // Always reopen the file upon reaching EOF to see if there's more
//        data.
...
//        }
//        file_ =
//            hdfs_->hdfsOpenFile(fs_, hdfs_filename_.c_str(), O_RDONLY, 0, 0,
//            0);
//        if (file_ == nullptr) {
//          PLOG(WARNING) << filename_;
//          return ErrnoToStatus(errno);
...
//
//  hdfsFile file =
//      hdfs_->hdfsOpenFile(fs, TranslateName(fname).c_str(), O_RDONLY, 0, 0,
//      0);
//  if (file == nullptr) {
//    PLOG(WARNING) << fname;
//    return ErrnoToStatus(errno);
...
//
//  hdfsFile file =
//      hdfs_->hdfsOpenFile(fs, TranslateName(fname).c_str(), O_WRONLY, 0, 0,
//      0);
//  if (file == nullptr) {
//    PLOG(WARNING) << fname;
//    return ErrnoToStatus(errno);
...
//  FS_RETURN_IF_ERR(Connect(&fs));
//
//  // hdfsListDirectory returns nullptr if the directory is empty. Do a
//  separate
...
//  // Due to HDFS bug HDFS-8407, we can't distinguish between an error and
//  empty
...
//}
//
// Status HadoopFileSystem::GetFileSize(const std::string& fname, uint64_t*
// size) {
//  hdfsFS fs = nullptr;
//  FS_RETURN_IF_ERR(Connect(&fs));
//
//  hdfsFileInfo* info = hdfs_->hdfsGetPathInfo(fs,
//  TranslateName(fname).c_str()); if (info == nullptr) {
//    PLOG(WARNING) << fname;
//    return ErrnoToStatus(errno);
//  }
...
//
//  hdfsFileInfo* info = hdfs_->hdfsGetPathInfo(fs,
//  TranslateName(fname).c_str()); if (info == nullptr || info->mKind !=
