...
void SnapshotReader::Read(const std::string &key,
                          const Shape &logical_blob_shape, DataType data_type,
  ...
  const std::string path = GenDataFilePath(root_path_, key);
  const int64_t logical_blob_size =
      logical_blob_shape.elem_cnt() * GetSizeOfDataType(data_type);
  ...
  if (slice.shape().Count(1) == logical_blob_shape.Count(1)) {
    PersistentInStream in_stream(SnapshotFS(), path,
                                 slice.At(0).begin() * slice.shape().Count(1) *
                                     GetSizeOfDataType(data_type));
    in_stream.ReadFully(dst, slice.shape().elem_cnt() *
                                 GetSizeOfDataType(data_type));
  } else {
    ...
    std::unique_ptr<MemoryCopier> host_memory_copier(
        NewDefaultMemoryCopier(DeviceType::kCPU));
...
void SnapshotReader::Read(const std::string &key,
  ...
  Read(key, logical_blob_shape, blob->data_type(), slice,
       blob->mut_dptr<char>());
}
  ...
  OfCallOnce("SnapshotWriteCheckRootPath-" + snapshot_root_path, [&]() {
    if (SnapshotFS()->FileExists(snapshot_root_path)) {
      CHECK(SnapshotFS()->IsDirectory(snapshot_root_path))
          << "root directory of model snapshot not found, path: "
          << snapshot_root_path;
      CHECK(SnapshotFS()->IsDirEmpty(snapshot_root_path))
          << "root directory of model snapshot not empty, path: "
          << snapshot_root_path;
    } else {
...
void SnapshotWriter::Write(const std::string &key, const char *data,
                           size_t size) {
  const std::string path = GenDataFilePath(root_path_, key);
...
void SnapshotWriter::Close() {
  PersistentOutStream out_stream(SnapshotFS(),
                                 JoinPath(root_path_, "snapshot_done"));
}
