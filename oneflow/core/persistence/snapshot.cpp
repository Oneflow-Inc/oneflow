...
Snapshot::Snapshot(const std::string& snapshot_root_path) {
  persistence_fs_ = GetFS(Global<JobDesc>::Get()->persistence_path_conf());
  CHECK(persistence_fs_->IsDirectory(snapshot_root_path));
...
std::unique_ptr<PersistentOutStream> Snapshot::GetOutStream(const LogicalBlobId& lbi,
  ...
  OfCallOnce(op_name_dir, persistence_fs_, &fs::FileSystem::CreateDir);
  // bn_in_op_tmp_dir
  std::string bn_in_op_tmp_dir = JoinPath(op_name_dir, lbi.blob_name() + "_tmp4a58");
  OfCallOnce(bn_in_op_tmp_dir, persistence_fs_, &fs::FileSystem::CreateDir);
  // part_file
  std::string part_file = JoinPath(bn_in_op_tmp_dir, "part_" + std::to_string(part_id));
  return std::make_unique<PersistentOutStream>(persistence_fs_, part_file);
}
...
void Snapshot::ConcatLbnFile(const LogicalBlobId& lbi, int32_t part_num,
  ...
  {
    PersistentOutStream out_stream(persistence_fs_, concat_file);
    for (int32_t i = 0; i < part_num; ++i) {
      ...
      persistence_fs_->NewRandomAccessFile(part_file_path, &part_file);
      uint64_t part_file_size = persistence_fs_->GetFileSize(part_file_path);
      uint64_t offset = 0;
      ...
      persistence_fs_->DelFile(part_file_path);
    }
  }
  persistence_fs_->DeleteDir(part_dir);
  ...
  if (snapshot_done_cnt == Global<SnapshotMgr>::Get()->total_mbn_num()) {
    ...
    PersistentOutStream out_stream(persistence_fs_, snapshot_done_path);
  }
}
