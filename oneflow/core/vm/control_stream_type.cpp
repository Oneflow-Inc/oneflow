...
template <> class InferStreamType<ControlStreamType> final : public StreamType {
public:
  ...
  bool QueryInstructionStatusDone(
    ...
    return ControlStreamType().QueryInstructionStatusDone(stream,
                                                          status_buffer);
  }
  ...
  void Infer(VirtualMachine *vm,
             InstructionMsg *instruction_msg) const override {
    ControlStreamType().Infer(vm, instruction_msg);
  }
  void Compute(Instruction *instruction) const override {
    LOG(FATAL) << "UNIMPLEMENTED";
  }
  void Compute(VirtualMachine *, InstructionMsg *) const override {
    LOG(FATAL) << "UNIMPLEMENTED";
  }
  ...
  MakeStreamDesc(const Resource &resource,
    ...
    auto stream_desc =
        ControlStreamType().MakeStreamDesc(resource, this_machine_id);
  ...
  void Run(VirtualMachine *vm, InstructionMsg *instr_msg) const {
    ...
    FOR_RANGE(int, i, 0, view->symbol_id_size()) {
      ...
      auto logical_object = ObjectMsgPtr<LogicalObject>::NewFrom(
          vm->mut_vm_thread_only_allocator(), symbol_id);
      CHECK(
...
void ControlStreamType::Infer(VirtualMachine *vm,
                              InstructionMsg *instr_msg) const {
...
void ControlStreamType::Compute(VirtualMachine *vm,
                                InstructionMsg *instr_msg) const {
  const auto &instr_type_id = instr_msg->instr_type_id();
  CHECK_EQ(instr_type_id.stream_type_id().interpret_type(),
...
void ControlStreamType::InitInstructionStatus(
...
void ControlStreamType::DeleteInstructionStatus(
    const Stream &stream, InstructionStatusBuffer *status_buffer) const {
  // do nothing
}
...
ObjectMsgPtr<StreamDesc>
ControlStreamType::MakeStreamDesc(const Resource &resource,
                                  int64_t this_machine_id) const {
  auto ret = ObjectMsgPtr<StreamDesc>::New();
  ret->mutable_stream_type_id()->__Init__(
      LookupStreamType4TypeIndex<ControlStreamType>());
