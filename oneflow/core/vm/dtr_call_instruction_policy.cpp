...
#include "oneflow/core/vm/dtr_call_instruction_policy.h"
...
struct DtrOpCallInstructionUtil final {
  static inline Maybe<void> Prepare(DtrOpCallInstructionPolicy* op_call_instruction_policy,
  ...
  static inline void Compute(DtrOpCallInstructionPolicy* op_call_instruction_policy,
                             Instruction* instruction) {
    ep::Stream* stream = instruction->mut_stream()->mut_stream_policy()->stream();
    for (const auto& input : op_call_instruction_policy->mut_call_ctx()->inputs()) {
      if (input->tensor_storage()->blob_dptr() == nullptr) {
        // recompute input
      }
      // pin input
    }
    for (const auto& output : op_call_instruction_policy->mut_call_ctx()->outputs()) {
      // allocate output (see new PR) (fbip) and record op in new output.storage
      // pin output
    }
 ...
 private:
  static inline void InferTempStorageSize(DtrOpCallInstructionPolicy* op_call_instruction_policy) {
  ...
  static inline void TryInitOpKernelStateAndCache(
      DtrOpCallInstructionPolicy* op_call_instruction_policy, ep::Stream* stream,
  ...
  static inline Maybe<void> AllocateOutputBlobsMemory(
      DtrOpCallInstructionPolicy* op_call_instruction_policy, Allocator* allocator) {
  ...
  static inline void OpKernelCompute(DtrOpCallInstructionPolicy* op_call_instruction_policy,
...
DtrOpCallInstructionPolicy::DtrOpCallInstructionPolicy(
...
Maybe<void> DtrOpCallInstructionPolicy::Init() {
  return mut_opkernel()->ChooseOpKernel(&call_ctx_, &user_opkernel_, &need_temp_storage_);
}

template<typename DoEachT>
void DtrOpCallInstructionPolicy::ForEachConstDependence(const DoEachT& DoEach) const {
...
void DtrOpCallInstructionPolicy::InitStreamSequentialDependence() {
...
template<typename DoEachT>
void DtrOpCallInstructionPolicy::ForEachMutDependence(const DoEachT& DoEach) const {
...
template<typename DoEachT>
void DtrOpCallInstructionPolicy::ForEachMut2Dependence(const DoEachT& DoEach) const {
  const auto& output_list = outputs();
...
Maybe<void> DtrOpCallInstructionPolicy::Prepare(vm::Instruction* instruction) {
  return DtrOpCallInstructionUtil::Prepare(this, instruction);
}

void DtrOpCallInstructionPolicy::Compute(vm::Instruction* instruction) {
  DtrOpCallInstructionUtil::Compute(this, instruction);
}

std::string DtrOpCallInstructionPolicy::DebugName(const vm::Instruction& instruction) const {
