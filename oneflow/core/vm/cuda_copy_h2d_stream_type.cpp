...
void CudaCopyH2DStreamType::InitDeviceCtx(
    std::unique_ptr<DeviceCtx> *device_ctx, Stream *stream) const {
  device_ctx->reset(new CudaStreamHandleDeviceCtx(stream->mut_callback_list(),
                                                  stream->device_id()));
}

void CudaCopyH2DStreamType::InitInstructionStatus(
...
void CudaCopyH2DStreamType::DeleteInstructionStatus(
    const Stream &stream, InstructionStatusBuffer *status_buffer) const {
  // do nothing
}
...
void CudaCopyH2DStreamType::Compute(Instruction *instruction) const {
  ...
  CudaInstrStatusQuerier::MutCast(data_ptr)->SetLaunched(
      stream->device_ctx().get());
}

ObjectMsgPtr<StreamDesc>
CudaCopyH2DStreamType::MakeStreamDesc(const Resource &resource,
  ...
  ret->mutable_stream_type_id()->__Init__(
      LookupStreamType4TypeIndex<CudaCopyH2DStreamType>());
