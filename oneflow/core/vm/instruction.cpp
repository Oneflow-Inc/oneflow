...
template <InterpretType interpret_type> int64_t GetObjectId(int64_t);

template <> int64_t GetObjectId<kCompute>(int64_t val) {
  return IdUtil::GetValueId(val);
}

template <> int64_t GetObjectId<kInfer>(int64_t val) {
...
ObjectMsgPtr<InstructionMsg>
InstructionMsg::add_parallel_desc(int64_t symbol_id) {
  set_parallel_desc_symbol_id(symbol_id);
  return this;
}

ObjectMsgPtr<InstructionMsg>
InstructionMsg::add_double_operand(double double_operand) {
  add_instr_operand()->set_double_operand(double_operand);
  return this;
}

ObjectMsgPtr<InstructionMsg>
InstructionMsg::add_int64_operand(int64_t int64_operand) {
  add_instr_operand()->set_int64_operand(int64_operand);
  return this;
}

ObjectMsgPtr<InstructionMsg>
InstructionMsg::add_uint64_operand(uint64_t uint64_operand) {
  add_instr_operand()->set_uint64_operand(uint64_operand);
  return this;
}

ObjectMsgPtr<InstructionMsg>
InstructionMsg::add_bool_operand(bool bool_operand) {
...
ObjectMsgPtr<InstructionMsg>
InstructionMsg::add_const_operand(ObjectId logical_object_id) {
  CHECK(IdUtil::IsObjectId(logical_object_id));
  add_instr_operand()->mutable_const_operand()->mutable_operand()->__Init__(
      logical_object_id);
  return this;
}
...
ObjectMsgPtr<InstructionMsg> InstructionMsg::add_const_operand(
  ...
  CHECK(IdUtil::IsObjectId(logical_object_id));
  add_instr_operand()->mutable_const_operand()->mutable_operand()->__Init__(
      logical_object_id, sole_mirrored_object);
  return this;
}
...
ObjectMsgPtr<InstructionMsg> InstructionMsg::add_const_operand(
  ...
  CHECK(IdUtil::IsObjectId(logical_object_id));
  add_instr_operand()->mutable_const_operand()->mutable_operand()->__Init__(
      logical_object_id, all_mirrored_object);
  return this;
}

ObjectMsgPtr<InstructionMsg>
InstructionMsg::add_symbol_operand(ObjectId logical_object_id) {
  CHECK(IdUtil::IsSymbolId(logical_object_id));
  add_instr_operand()->mutable_symbol_operand()->mutable_operand()->__Init__(
      logical_object_id, SoleMirroredObject());
  return this;
}

ObjectMsgPtr<InstructionMsg>
InstructionMsg::add_mut_operand(ObjectId logical_object_id) {
  CHECK(IdUtil::IsObjectId(logical_object_id));
  add_instr_operand()->mutable_mut_operand()->mutable_operand()->__Init__(
      logical_object_id);
  return this;
}
...
ObjectMsgPtr<InstructionMsg>
InstructionMsg::add_init_symbol_operand(ObjectId logical_object_id) {
  CHECK(IdUtil::IsSymbolId(logical_object_id));
  add_instr_operand()
      ->mutable_init_symbol_operand()
      ->mutable_operand()
      ->__Init__(logical_object_id, SoleMirroredObject());
  return this;
}

ObjectMsgPtr<InstructionMsg>
InstructionMsg::add_mut2_operand(ObjectId logical_object_id) {
  CHECK(IdUtil::IsObjectId(logical_object_id));
  add_instr_operand()->mutable_mut2_operand()->mutable_operand()->__Init__(
      logical_object_id);
  return this;
}
...
ObjectMsgPtr<InstructionMsg> InstructionMsg::MakeInferInstrMsg() const {
  auto infer_instr_msg =
      ObjectMsgPtr<InstructionMsg>::NewFrom(mut_allocator(), *this);
  auto *stream_type_id =
      infer_instr_msg->mut_instr_type_id()->mut_stream_type_id();
...
template <> void CheckOperand<kDeviceMemZoneModifier>(const Operand &operand) {
  CHECK(IdUtil::IsObjectId(operand.logical_object_id()));
}

const RwMutexedObject *
Instruction::operand_type(const Operand &operand,
...
const RwMutexedObject *
Instruction::operand_value(const Operand &operand,
                           int64_t default_global_device_id) const {
  CHECK(IdUtil::IsValueId(operand.logical_object_id()));
  CHECK_EQ(instr_msg().instr_type_id().stream_type_id().interpret_type(),
...
RwMutexedObject *
Instruction::mut_operand_type(const Operand &operand,
                              int64_t default_global_device_id) {
  CHECK(IdUtil::IsValueId(operand.logical_object_id()));
  return MutMirroredObject<&IdUtil::GetTypeId>(operand,
                                               default_global_device_id)
      ->mut_rw_mutexed_object();
}

RwMutexedObject *
Instruction::mut_operand_value(const Operand &operand,
                               int64_t default_global_device_id) {
  CHECK(IdUtil::IsValueId(operand.logical_object_id()));
  CHECK_EQ(instr_msg().instr_type_id().stream_type_id().interpret_type(),
           InterpretType::kCompute);
  return MutMirroredObject<&IdUtil::GetValueId>(operand,
                                                default_global_device_id)
      ->mut_rw_mutexed_object();
}
...
const MirroredObject *Instruction::MirroredObjectUtil<interpret_type>::Get(
  ...
  return instruction.GetMirroredObject<&GetObjectId<interpret_type>>(
      operand, default_device_id);
}
...
MirroredObject *Instruction::MirroredObjectUtil<interpret_type>::Mut(
  ...
  CHECK(IdUtil::IsValueId(operand.logical_object_id()));
  int64_t default_device_id = instruction->GetOperandDefaultGlobalDeviceId();
  return instruction->MutMirroredObject<&GetObjectId<interpret_type>>(
      operand, default_device_id);
}

MirroredObject *
Instruction::mut_type_mirrored_object(const MutOperand &mut_operand) {
  return MutMirroredObject<kInfer>(mut_operand);
}
MirroredObject *
Instruction::mut_value_mirrored_object(const MutOperand &mut_operand) {
  return MutMirroredObject<kCompute>(mut_operand);
}

template <int64_t (*TransformLogicalObjectId)(int64_t)>
MirroredObject *
Instruction::MutMirroredObject(const Operand &operand,
                               int64_t default_global_device_id) {
  FlatMsg<MirroredObjectId> mirrored_object_id;
  mirrored_object_id->__Init__<TransformLogicalObjectId>(
      operand, default_global_device_id);
  auto *access =
      mut_mirrored_object_id2access()->FindPtr(mirrored_object_id.Get());
  if (access == nullptr) {
    return nullptr;
  }
...
Instruction::GetMirroredObject(const Operand &operand,
                               int64_t default_global_device_id) const {
  FlatMsg<MirroredObjectId> mirrored_object_id;
  mirrored_object_id->__Init__<TransformLogicalObjectId>(
      operand, default_global_device_id);
  const auto *access =
      mirrored_object_id2access().FindPtr(mirrored_object_id.Get());
  if (access == nullptr) {
    return nullptr;
  }
...
int64_t Instruction::GetOperandDefaultGlobalDeviceId() const {
  return stream().global_device_id();
}
...
const StreamType &Instruction::stream_type() const {
