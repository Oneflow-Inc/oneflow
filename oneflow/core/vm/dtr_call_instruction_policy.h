...
*/
#ifndef ONEFLOW_CORE_VM_DTR_OP_CALL_INSTRUCTION_POLICY_H_
#define ONEFLOW_CORE_VM_DTR_OP_CALL_INSTRUCTION_POLICY_H_
...
class DtrOpCallInstructionPolicy final : public InstructionPolicy {
 public:
   OF_DISALLOW_COPY_AND_MOVE(DtrOpCallInstructionPolicy);

  ~DtrOpCallInstructionPolicy() override = default;

  template<typename... Args>
  static Maybe<DtrOpCallInstructionPolicy> New(Args&&... args) {
    auto* ptr = new DtrOpCallInstructionPolicy(std::forward<Args>(args)...);
    JUST(ptr->Init());
    return std::shared_ptr<DtrOpCallInstructionPolicy>(ptr);
  }
...
#endif  // ONEFLOW_CORE_VM_DTR_OP_CALL_INSTRUCTION_POLICY_H_
