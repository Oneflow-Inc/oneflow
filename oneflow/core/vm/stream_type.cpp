...
void StreamType::Run(Instruction *instruction) const {
  const auto &stream_type_id =
      instruction->stream().stream_id().stream_type_id();
...
void StreamType::Run(VirtualMachine *vm, InstructionMsg *instr_msg) const {
  InterpretType interpret_type =
      instr_msg->instr_type_id().stream_type_id().interpret_type();
  if (interpret_type == InterpretType::kCompute) {
...
void StreamType::Run(VirtualMachine *vm, Instruction *instruction) const {
  auto interpret_type =
      instruction->stream().stream_id().stream_type_id().interpret_type();
  if (interpret_type == InterpretType::kCompute) {
...
void TryRegisterInferStreamTypeId(const StreamType *infer_stream_type,
  ...
  InferStreamTypeId4ComputeStreamTypeId()->emplace(compute_stream_type_id,
                                                   infer_stream_type_id);
}
