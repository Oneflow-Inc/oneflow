...
CudaAllocator::~CudaAllocator() {
  ...
  for (auto &pair : mem_ptr2block_) {
    OF_CUDA_CHECK(cudaFree(pair.first));
  }
}
...
bool CudaAllocator::AllocateBlockToExtendTotalMem(size_t aligned_size) {
  ...
  const size_t available_bytes =
      free_bytes - remain_bytes; // remain at least 50MiB memory

  // growth double total memory bytes if could
  if (total_memory_bytes_ > 0) {
    allocate_bytes = std::max(allocate_bytes,
                              std::min(total_memory_bytes_, available_bytes));
  }
...
bool CudaAllocator::DeallocateFreeBlockForGarbageCollection() {
  ...
  if (total_free_bytes > 0) {
    LOG(WARNING)
        << "CudaAllocator try deallocate free block for garbage collection. "
...
void CudaAllocator::Allocate(char **mem_ptr, std::size_t size) {
  ...
  CHECK(piece != nullptr) << "Error! : Out of memory when allocate size : "
                          << size;
  CHECK_NOTNULL(piece->ptr);
...
void CudaAllocator::Deallocate(char *mem_ptr, std::size_t size) {
  ...
  CHECK(it != ptr2piece_.end())
      << "Error! : Try deallocate mem_ptr non-existent. mem ptr = " << mem_ptr
      << " size = " << size;
