...
*/
#ifndef ONEFLOW_CORE_VM_DTR_NAIVE_ALLOCATOR_H_
#define ONEFLOW_CORE_VM_DTR_NAIVE_ALLOCATOR_H_
...
class DtrNaiveCudaAllocator final : public Allocator {
 public:
  OF_DISALLOW_COPY_AND_MOVE(DtrNaiveCudaAllocator);
  explicit DtrNaiveCudaAllocator(int64_t device_id);
  ~DtrNaiveCudaAllocator() override;
 ...
 private:
  ...
  // Piece is the basic memory unit of DtrNaiveCudaAllocator.
  ...
  //
  // DtrNaiveCudaAllocator has a set of Bin structures according to the binary multiple increasing
  // relation, which is used to quickly index and find the free Piece of appropriate size when
  // Allocate()
  //
  // The size of the smallest bin is 512 (512 is the smallest unit Allocated by
  // DtrNaiveCudaAllocator, and the memory size of all Allocated will be multiples of 512, 512 is
  // kCudaMemAllocAlignSize). The size of each Bin is twice the size of the previous Bin, like
...
#endif  // ONEFLOW_CORE_VM_DTR_NAIVE_ALLOCATOR_H_
