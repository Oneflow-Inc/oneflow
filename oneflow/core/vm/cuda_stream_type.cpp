...
void CudaStreamType::InitDeviceCtx(std::unique_ptr<DeviceCtx> *device_ctx,
                                   Stream *stream) const {
  device_ctx->reset(new CudaStreamHandleDeviceCtx(stream->mut_callback_list(),
                                                  stream->device_id()));
}

void CudaStreamType::InitInstructionStatus(
...
void CudaStreamType::DeleteInstructionStatus(
    const Stream &stream, InstructionStatusBuffer *status_buffer) const {
  // do nothing
}
...
CudaStreamType::MakeStreamDesc(const Resource &resource,
  ...
  ret->mutable_stream_type_id()->__Init__(
      LookupStreamType4TypeIndex<CudaStreamType>());
