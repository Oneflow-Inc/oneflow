...
bool IsSourceInstruction(const InstructionMsg &instr_msg) {
  for (const auto &instr_operand : instr_msg.operand()) {
    if (instr_operand->has_const_operand()) {
      return false;
    }
    if (instr_operand->has_mut_operand()) {
      return false;
    }
    if (instr_operand->has_mut2_operand()) {
      return false;
    }
    if (instr_operand->has_symbol_operand()) {
      return false;
    }
    if (instr_operand->has_init_symbol_operand()) {
      return false;
    }
    CHECK(instr_operand->has_separator() ||
          instr_operand->has_double_operand() ||
          instr_operand->has_double_operand() ||
          instr_operand->has_int64_operand() ||
          instr_operand->has_uint64_operand() ||
          instr_operand->has_bool_operand());
  }
  return true;
}
...
void VirtualMachine::ReleaseInstruction(
    Instruction *instruction,
    /*out*/ ReadyInstructionList *ready_instruction_list) {
  auto *rw_mutexed_object_accesses =
      instruction->mut_mirrored_object_id2access();
  ...
  TryMoveWaitingToReady(instruction, ready_instruction_list,
...
void VirtualMachine::TryReleaseFinishedInstructions(
  ...
  while (true) {
    ...
    if (instruction_ptr == nullptr || !instruction_ptr->Done()) {
      break;
    }
...
void VirtualMachine::FilterAndRunSourceInstructions(
    TmpPendingInstrMsgList *instr_msg_list) {
  OBJECT_MSG_LIST_FOR_EACH_PTR(instr_msg_list, instr_msg) {
    const auto &instr_type_id = instr_msg->instr_type_id();
    const StreamType &stream_type =
        instr_type_id.stream_type_id().stream_type();
...
void VirtualMachine::MakeInstructions(
    TmpPendingInstrMsgList *instr_msg_list,
    /*out*/ NewInstructionList *new_instruction_list) {
  auto IsStreamInParallelDesc = [](const ParallelDesc *parallel_desc,
                                   const Stream &stream) {
  ...
  OBJECT_MSG_LIST_FOR_EACH_PTR(instr_msg_list, instr_msg) {
    const StreamTypeId &stream_type_id =
        instr_msg->instr_type_id().stream_type_id();
    auto *stream_rt_desc =
        mut_stream_type_id2stream_rt_desc()->FindPtr(stream_type_id);
    CHECK_NOTNULL(stream_rt_desc);
    const auto &parallel_desc = GetInstructionParallelDesc(*instr_msg);
    OBJECT_MSG_SKIPLIST_UNSAFE_FOR_EACH_PTR(
        stream_rt_desc->mut_stream_id2stream(), stream) {
      if (!IsStreamInParallelDesc(parallel_desc.get(), *stream)) {
        continue;
      }
      new_instruction_list->EmplaceBack(
          stream->NewInstruction(instr_msg, parallel_desc));
    }
    instr_msg_list->Erase(instr_msg);
  }
}
...
VirtualMachine::GetInstructionParallelDesc(const InstructionMsg &instr_msg) {
  ...
  CHECK_EQ(map->size(), 1);
  return CHECK_JUST(map->Begin()
                        ->rw_mutexed_object()
                        .Get<ObjectWrapper<ParallelDesc>>())
      ->GetPtr();
}
...
MirroredObject *VirtualMachine::MutMirroredObject(int64_t logical_object_id,
  ...
  if (logical_object == nullptr) {
    return nullptr;
  }
  return logical_object->mut_global_device_id2mirrored_object()->FindPtr(
      global_device_id);
}

const MirroredObject *
VirtualMachine::GetMirroredObject(int64_t logical_object_id,
                                  int64_t global_device_id) {
...
void VirtualMachine::ForEachMirroredObject(Id2LogicalObject *id2logical_object,
  ...
  if (operand.has_all_mirrored_object()) {
    OBJECT_MSG_MAP_FOR_EACH_PTR(map, mirrored_object) {
      DoEach(mirrored_object);
    }
  } else {
    auto *mirrored_object =
        map->FindPtr(operand.GetGlobalDeviceId(global_device_id));
...
void VirtualMachine::ForEachConstMirroredObject(
  ...
  if (interpret_type == InterpretType::kCompute) {
    ForEachMirroredObject<&IdUtil::GetTypeId>(id2logical_object, operand,
                                              global_device_id, DoEach);
    ForEachMirroredObject<&IdUtil::GetValueId>(id2logical_object, operand,
                                               global_device_id, DoEach);
...
void VirtualMachine::ForEachConstMirroredObject(
    ...
    const ModifiedOperand<kDataMutableModifier, mem_zone_modifier>
        &mutable_operand,
    int64_t global_device_id, const DoEachT &DoEach) {
...
void VirtualMachine::ForEachMutMirroredObject(
    const InterpretType interpret_type, Id2LogicalObject *id2logical_object,
    const ModifiedOperand<kTypeAndDataMutableModifier, mem_zone_modifier>
        &mut2_operand,
    int64_t global_device_id, const DoEachT &DoEach) {
...
void VirtualMachine::ConsumeMirroredObject(OperandAccessType access_type,
  ...
  auto rw_mutexed_object_access = ObjectMsgPtr<RwMutexedObjectAccess>::NewFrom(
      instruction->mut_allocator(), instruction, mirrored_object,
      is_const_operand);
...
void VirtualMachine::ConnectInstruction(Instruction *src_instruction,
  ...
  auto edge = ObjectMsgPtr<InstructionEdge>::NewFrom(
      mut_vm_thread_only_allocator(), src_instruction, dst_instruction);
  bool src_inserted =
      src_instruction->mut_out_edges()->Insert(edge.Mutable()).second;
  bool dst_inserted =
      dst_instruction->mut_in_edges()->Insert(edge.Mutable()).second;
  CHECK_EQ(src_inserted, dst_inserted);
}

void VirtualMachine::ConsumeMirroredObjects(
  ...
  OBJECT_MSG_LIST_FOR_EACH_PTR(new_instruction_list, instruction) {
    ...
    InterpretType interpret_type =
        instruction->stream().stream_type_id().interpret_type();
    ...
    for (const auto &operand : operands) {
      if (operand->has_mut_operand()) {
        ForEachMutMirroredObject<kDeviceMemZoneModifier>(
            interpret_type, id2logical_object, operand->mut_operand(),
            global_device_id, ConsumeMutMirroredObject);
      } else if (operand->has_mut2_operand()) {
        ForEachMutMirroredObject<kDeviceMemZoneModifier>(
            interpret_type, id2logical_object, operand->mut2_operand(),
            global_device_id, ConsumeMutMirroredObject);
        ...
        ForEachMutMirroredObject<kHostConstMemZoneModifier>(
            interpret_type, id2logical_object, operand->init_symbol_operand(),
            0, ConsumeMutMirroredObject);
      } else {
        // do nothing
      }
    }
    for (const auto &operand : operands) {
      if (operand->has_const_operand()) {
        ForEachConstMirroredObject<kDeviceMemZoneModifier>(
            interpret_type, id2logical_object, operand->const_operand(),
            global_device_id, ConsumeConstMirroredObject);
        ...
        ForEachConstMirroredObject<kHostConstMemZoneModifier>(
            interpret_type, id2logical_object, operand->symbol_operand(), 0,
    ...
    auto *rw_mutexed_object_accesses =
        instruction->mut_mirrored_object_id2access();
    OBJECT_MSG_SKIPLIST_UNSAFE_FOR_EACH_PTR(rw_mutexed_object_accesses,
                                            rw_mutexed_object_access) {
      auto *mirrored_object = rw_mutexed_object_access->mut_mirrored_object();
      if (mirrored_object->rw_mutexed_object().access_list().size() == 1) {
        continue;
      }
      if (rw_mutexed_object_access->is_const_operand()) {
        auto *first = mirrored_object->mut_rw_mutexed_object()
                          ->mut_access_list()
                          ->Begin();
        if (!first->is_const_operand() &&
            first->mut_instruction() != instruction) {
          ConnectInstruction(first->mut_instruction(), instruction);
        }
      } else {
        auto *access_list =
            mirrored_object->mut_rw_mutexed_object()->mut_access_list();
        OBJECT_MSG_LIST_FOR_EACH_PTR(access_list, access) {
          if (access == rw_mutexed_object_access) {
            break;
          }
...
void VirtualMachine::FilterReadyInstructions(
    NewInstructionList *new_instruction_list,
    /*out*/ ReadyInstructionList *ready_instruction_list) {
  OBJECT_MSG_LIST_FOR_EACH_PTR(new_instruction_list, instruction) {
...
void VirtualMachine::DispatchAndPrescheduleInstructions(
  ...
  OBJECT_MSG_LIST_FOR_EACH_PTR(ready_instruction_list, instruction) {
    ...
    ready_instruction_list->MoveToDstBack(
        instruction, stream->mut_running_instruction_list());
    if (stream->is_active_stream_link_empty()) {
      active_stream_list->PushBack(stream);
    }
    const auto &stream_type = stream->stream_type();
    if (stream_type.SharingVirtualMachineThread()) {
      stream_type.Run(this, instruction);
    } else {
      stream->mut_thread_ctx()->mut_pending_instruction_list()->PushBack(
          instruction);
    }
    TryMoveWaitingToReady(
        instruction, &prescheduled,
...
void VirtualMachine::TryMoveWaitingToReady(Instruction *instruction,
  ...
  OBJECT_MSG_SKIPLIST_FOR_EACH_PTR(out_edges, out_edge) {
    ...
    if (!IsEdgeReady(out_instruction)) {
      continue;
    }
...
void VirtualMachine::__Init__(const VmDesc &vm_desc,
  ...
  *mutable_machine_id_range() = vm_desc.machine_id_range();
  ...
  OBJECT_MSG_SKIPLIST_UNSAFE_FOR_EACH_PTR(&vm_desc.stream_type_id2desc(),
                                          stream_desc) {
    if (stream_desc->num_threads() == 0) {
      continue;
    }
    auto stream_rt_desc =
        ObjectMsgPtr<StreamRtDesc>::NewFrom(allocator, stream_desc);
    mut_stream_type_id2stream_rt_desc()->Insert(stream_rt_desc.Mutable());
    BalancedSplitter bs(stream_desc->parallel_num(),
                        stream_desc->num_threads());
    for (int64_t i = 0, rel_global_device_id = 0;
         i < stream_desc->num_threads(); ++i) {
      auto thread_ctx =
          ObjectMsgPtr<ThreadCtx>::NewFrom(allocator, stream_rt_desc.Get());
      mut_thread_ctx_list()->PushBack(thread_ctx.Mutable());
      for (int j = bs.At(i).begin(); j < bs.At(i).end();
           ++j, ++rel_global_device_id) {
        StreamId stream_id;
        stream_id.__Init__(stream_desc->stream_type_id(),
                           stream_desc->start_global_device_id() +
                               rel_global_device_id);
        auto stream = ObjectMsgPtr<Stream>::NewFrom(
            mut_allocator(), thread_ctx.Mutable(), stream_id);
        CHECK(stream_rt_desc->mut_stream_id2stream()
                  ->Insert(stream.Mutable())
                  .second);
...
void VirtualMachine::Receive(InstructionMsgList *compute_instr_msg_list) {
  ...
  OBJECT_MSG_LIST_FOR_EACH_PTR(compute_instr_msg_list, compute_instr_msg) {
    ...
    compute_instr_msg_list->MoveToDstBack(compute_instr_msg,
                                          &new_instr_msg_list);
  }
...
void VirtualMachine::Schedule() {
  ...
  OBJECT_MSG_LIST_FOR_EACH_PTR(active_stream_list, stream) {
    ...
    if (stream->running_instruction_list().empty()) {
      active_stream_list->Erase(stream);
    }
  }
  ...
  if (pending_msg_list().size() > 0) {
    ...
    FilterReadyInstructions(&new_instruction_list,
                            /*out*/ ready_instruction_list);
