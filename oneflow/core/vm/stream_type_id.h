...
public:
  ...
  StreamTypeId(const StreamTypeId &rhs) {
    __Init__(rhs.stream_type_, rhs.interpret_type_);
  }
  ...
  void __Init__(const StreamType *stream_type) {
    __Init__(stream_type, InterpretType::kCompute);
  }
...
template <> struct hash<oneflow::vm::StreamTypeId> {
  size_t operator()(const oneflow::vm::StreamTypeId &stream_type_id) const {
    return std::hash<const oneflow::vm::StreamType *>()(
