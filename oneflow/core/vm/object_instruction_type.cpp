...
void ForEachMachineIdAndDeviceIdInRange(const ParallelDesc &parallel_desc,
  ...
  if (machine_id_range.size() < parallel_desc.sorted_machine_ids().size()) {
    FOR_RANGE(int64_t, machine_id, machine_id_range.begin(),
              machine_id_range.end()) {
    ...
    for (int64_t machine_id : parallel_desc.sorted_machine_ids()) {
      if (machine_id >= machine_id_range.begin() &&
          machine_id < machine_id_range.end()) {
        for (int64_t device_id : parallel_desc.sorted_dev_phy_ids(machine_id)) {
          DoEach(machine_id, device_id);
        }
      }
...
private:
  ...
  void Run(VirtualMachine *vm, InstructionMsg *instr_msg) const {
    ...
    std::shared_ptr<ParallelDesc> parallel_desc =
        vm->GetInstructionParallelDesc(*instr_msg);
    ...
    FOR_RANGE(int, i, 0, view->logical_object_id_size()) {
      ...
      auto logical_object = ObjectMsgPtr<LogicalObject>::NewFrom(
          vm->mut_vm_thread_only_allocator(), logical_object_id, parallel_desc);
      CHECK(
      ...
      ForEachMachineIdAndDeviceIdInRange(
          *parallel_desc, vm->machine_id_range(),
          [&](int64_t machine_id, int64_t device_id) {
            int64_t global_device_id = vm->vm_resource_desc().GetGlobalDeviceId(
                machine_id, device_tag, device_id);
            auto mirrored_object = ObjectMsgPtr<MirroredObject>::NewFrom(
                vm->mut_allocator(), logical_object.Mutable(),
                global_device_id);
            CHECK(global_device_id2mirrored_object
                      ->Insert(mirrored_object.Mutable())
                      .second);
          });
    }
  }
...
private:
  ...
  void Run(VirtualMachine *vm, InstructionMsg *instr_msg) const {
    ...
    auto logical_object = ObjectMsgPtr<LogicalObject>::NewFrom(
        vm->mut_vm_thread_only_allocator(), new_object, parallel_desc);
...
COMMAND(RegisterInstructionType<BroadcastObjectReferenceInstructionType>(
  ...
  void Run(VirtualMachine *vm, InstructionMsg *instr_msg) const {
    ...
    auto DoEachRhsObject = [&](MirroredObject *lhs, int64_t global_device_id) {
      ...
      FOR_RANGE(int, i, 0, args->rhs_object_id_size()) {
        ...
        const auto *rhs =
            vm->GetMirroredObject(rhs_object_id, global_device_id);
        if (rhs != nullptr) {
        ...
        *parallel_desc, vm->machine_id_range(),
        [&](int64_t machine_id, int64_t device_id) {
          FOR_RANGE(int, i, 0, args->lhs_object_id_size()) {
            ...
            if (lhs != nullptr) {
              DoEachRhsObject(lhs, global_device_id);
            }
          }
        });
  }
...
COMMAND(
    RegisterInstructionType<ReplaceMirroredInstructionType>("ReplaceMirrored"));
...
private:
  ...
  void Run(VirtualMachine *vm, InstructionMsg *instr_msg) const {
    ...
    FOR_RANGE(int, i, 0, view->object_size()) {
      ...
      auto *logical_object =
          vm->mut_id2logical_object()->FindPtr(logical_object_id);
      CHECK_NOTNULL(logical_object);
      auto *global_device_id2mirrored_object =
          logical_object->mut_global_device_id2mirrored_object();
      OBJECT_MSG_MAP_FOR_EACH_PTR(global_device_id2mirrored_object,
                                  mirrored_object) {
