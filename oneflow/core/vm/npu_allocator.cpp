...
inline size_t NpuMemAlignedBytes(size_t bytes) { return RoundUp(bytes, kNpuMemAllocAlignSize); }

inline bool IsAlignedSize(size_t size) { return size % kNpuMemAllocAlignSize == 0; }

static const size_t kPieceSplitThreshold = 128 << 20;  // 128MiB

}  // namespace

NpuAllocator::NpuAllocator(int64_t device_id)
  ...
  for (int i = 0; i < kBinNumSize; ++i) {
    ...
    CHECK_EQ(BinNum4BinSize(bin_size + kNpuMemAllocAlignSize - 1), i);
...
NpuAllocator::~NpuAllocator() {
  ...
  for (auto& pair : mem_ptr2block_) { OF_NPU_CHECK(aclrtFree(pair.first)); }
}

void NpuAllocator::InsertPiece2Bin(Piece* piece) {
...
void NpuAllocator::RemovePieceFromBin(Piece* piece) {
  CHECK(piece->is_free);
...
NpuAllocator::Piece* NpuAllocator::AllocatePiece() {
...
void NpuAllocator::DeallocatePiece(Piece* piece) {
  piece->ptr = nullptr;
  piece->size = 0;
  piece->bin_num = kInvalidBinNum;
  piece->is_free = true;
...
void NpuAllocator::MarkPiece(Piece* piece) {
  CHECK_NOTNULL(piece->ptr);
  CHECK(ptr2piece_.emplace(piece->ptr, piece).second);
}
void NpuAllocator::UnMarkPiece(Piece* piece) {
  CHECK_NOTNULL(piece->ptr);
...
NpuAllocator::Piece* NpuAllocator::FindPiece(size_t aligned_size) {
...
void NpuAllocator::MergeNeighbourFreePiece(Piece* lhs, Piece* rhs) {
  CHECK(lhs->is_free);
  CHECK(rhs->is_free);
  CHECK(lhs->next == rhs);
  CHECK(lhs == rhs->prev);
...
bool NpuAllocator::AllocateBlockToExtendTotalMem(size_t aligned_size) {
  ...
  OF_NPU_CHECK(aclrtGetMemInfo(ACL_DDR_MEM,&free_bytes, &total_bytes));//dck_caution_here
  ...
  const size_t final_allocate_bytes = NpuMemAlignedBytes(allocate_bytes);
  ...
  CHECK(mem_ptr2block_.emplace((char*)mem_ptr, Block(piece)).second);

  return true;
}

bool NpuAllocator::DeallocateFreeBlockForGarbageCollection() {
  std::cout<<"NpuAllocator::~DeallocateFreeBlockForGarbageCollection()"<<std::endl;
  ...
  if (total_free_bytes > 0) {
    VLOG(3) << "NpuAllocator try deallocate free block for garbage collection. "
...
void NpuAllocator::Allocate(char** mem_ptr, std::size_t size) {
  ...
  if (piece == nullptr) {
    // NOTE(chengcheng): In some corner case on ubuntu, npu memory not released even if OOM.
    //   So there need release all npu memory allocated by this process before core dump.
    LOG(WARNING) << "OOM error is detected, process will exit. And it will start to reset NPU "
                 << "device for releasing device memory.";
    OF_NPU_CHECK(aclrtResetDevice(0));//dck_caution_here
    LOG(FATAL) << "Error! : Out of memory when allocate size : " << size
               << ".\n The total_memory_bytes allocated by this NpuAllocator is : "
               << total_memory_bytes_;
  }
  CHECK_NOTNULL(piece->ptr);
