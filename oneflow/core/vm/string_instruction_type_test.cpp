...
TEST(StringStreamType, init_string_object) {
  ...
  list.EmplaceBack(
      NewInstruction("InitStringSymbol")->add_init_symbol_operand(symbol_id));
  vm->Receive(&list);
  while (!vm->Empty()) {
    vm->Schedule();
  ...
  auto *logical_object =
      vm->mut_id2logical_object()->FindPtr(IdUtil::GetTypeId(symbol_id));
  ASSERT_NE(logical_object, nullptr);
  auto *mirrored_object =
      logical_object->mut_global_device_id2mirrored_object()->FindPtr(0);
  ASSERT_NE(mirrored_object, nullptr);
  const auto *string_obj =
      CHECK_JUST(mirrored_object->rw_mutexed_object().Get<StringObject>());
