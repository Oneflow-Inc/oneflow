...
Transporter *GetTransporter(Instruction *instruction) {
  auto *device_ctx = dynamic_cast<TransporterDeviceCtx *>(
      instruction->mut_stream()->device_ctx().get());
...
void MakeSendRequests(Instruction *instruction,
  ...
  size_t data_size = 0;
  {
    ...
    data_token->mutable_mirrored_token()->set_logical_token(
        view->logical_token());
    data_token->mutable_mirrored_token()->set_global_device_id(
        stream.global_device_id());
    data_size = view->size();
    const auto &src_buffer_type = *CHECK_JUST(
    ...
    const auto &src_buffer_value = *CHECK_JUST(
  ...
  {
    char *buffer_ptr =
        instruction->mut_status_buffer()->mut_buffer()->mut_data();
    incomplete_cnt = reinterpret_cast<std::atomic<int64_t> *>(buffer_ptr);
  }
  GetTransporter(instruction)
      ->MakeSendTransportRequest(data_token.Get(), data_ptr, data_size,
...
void L2RSenderStreamType::InitInstructionStatus(
  ...
  static_assert(sizeof(std::atomic<int64_t>) < kInstructionStatusBufferBytes,
                "");
  new (status_buffer->mut_buffer()->mut_data())
      std::atomic<int64_t>(kRefCntInitVal);
}

void L2RSenderStreamType::DeleteInstructionStatus(
    const Stream &stream, InstructionStatusBuffer *status_buffer) const {
  // do nothing
}
...
bool L2RSenderStreamType::QueryInstructionStatusDone(
  ...
  return *reinterpret_cast<const std::atomic<int64_t> *>(
             status_buffer.buffer().data()) == 0;
}
...
ObjectMsgPtr<StreamDesc>
L2RSenderStreamType::MakeStreamDesc(const Resource &resource,
  ...
  ret->mutable_stream_type_id()->__Init__(
      LookupStreamType4TypeIndex<L2RSenderStreamType>());
