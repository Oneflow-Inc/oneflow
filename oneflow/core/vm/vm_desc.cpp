...
void SetMachineIdRange(Range *range, int64_t machine_num,
...
ObjectMsgPtr<VmDesc> MakeVmDesc(const Resource &resource,
  ...
  auto vm_desc = ObjectMsgPtr<VmDesc>::New(
      ObjectMsgPtr<VmResourceDesc>::New(resource).Get());
  SetMachineIdRange(vm_desc->mutable_machine_id_range(), resource.machine_num(),
                    this_machine_id);
  int cnt = 0;
  for (const auto &stream_type_id : stream_type_ids) {
    ...
    if (stream_desc) {
      ++cnt;
      CHECK(vm_desc->mut_stream_type_id2desc()
                ->Insert(stream_desc.Mutable())
                .second);
    }
  }
