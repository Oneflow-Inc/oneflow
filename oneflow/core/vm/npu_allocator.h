...
*/
#ifndef ONEFLOW_CORE_VM_NPU_ALLOCATOR_H_
#define ONEFLOW_CORE_VM_NPU_ALLOCATOR_H_
...
class NpuAllocator final : public Allocator {
 public:
  explicit NpuAllocator(int64_t device_id);
 ...
 private:
  ...
  // Piece is the basic memory unit of NpuAllocator.
  ...
  //
  // NpuAllocator has a set of Bin structures according to the binary multiple increasing relation,
  // which is used to quickly index and find the free Piece of appropriate size when Allocate()
  //
  // The size of the smallest bin is 512 (512 is the smallest unit Allocated by NpuAllocator,
  // and the memory size of all Allocated will be multiples of 512, 512 is kNpuMemAllocAlignSize).
  ...
  size_t BinSize4BinNum(int32_t bin_num) { return kNpuMemAllocAlignSize << bin_num; }

  int32_t BinNum4BinSize(size_t size) {
    uint64_t value = std::max(size, kNpuMemAllocAlignSize) >> 9;
...
#endif  // ONEFLOW_CORE_VM_NPU_ALLOCATOR_H_
