...
void MakeTransportRequest(
    const TransportDataToken &data_token,
...
void LocalhostTransporter::MakeSendTransportRequest(
    const TransportDataToken &data_token, const char *data_ptr,
    size_t data_size, std::atomic<int64_t> *incomplete_cnt,
    TransportKey2SendRequest *transport_key2send_request) const {
  MakeTransportRequest<kSendTransportRequestType>(data_token, data_ptr,
                                                  data_size, incomplete_cnt,
...
void LocalhostTransporter::MakeReceiveTransportRequest(
  ...
  MakeTransportRequest<kReceiveTransportRequestType>(
      data_token, data_ptr, data_size, incomplete_cnt,
      transport_key2send_request);
}
...
void LocalhostTransporter::Transport(
  ...
  OBJECT_MSG_MAP_FOR_EACH(transport_key2send_request, send_request) {
    ...
    ObjectMsgPtr<ReceiveTransportRequest> receive_request;
    {
      ...
      receive_request = hub->mut_transport_key2receive_request()->Find(
          send_request->transport_key());
      if (receive_request) {
        hub->mut_transport_key2receive_request()->Erase(
            receive_request.Mutable());
      } else {
        CHECK(hub->mut_transport_key2send_request()
                  ->Insert(send_request.Mutable())
                  .second);
      }
    }
    if (receive_request) {
      CopyAndDecreaseIncompleteCnt(receive_request.Mutable(),
                                   send_request.Mutable());
    }
  }
}
...
void LocalhostTransporter::Transport(
  ...
  OBJECT_MSG_MAP_FOR_EACH(transport_key2receive_request, receive_request) {
    ...
    {
      std::unique_lock<std::mutex> lock(*hub->mut_mutex());
      send_request = hub->mut_transport_key2send_request()->Find(
          receive_request->transport_key());
      if (send_request) {
        hub->mut_transport_key2send_request()->Erase(send_request.Mutable());
      } else {
        CHECK(hub->mut_transport_key2receive_request()
                  ->Insert(receive_request.Mutable())
                  .second);
      }
    }
    if (send_request) {
