...
class AccessBlobArgCbInstructionPolicy final : public InstructionPolicy {
 public:
  ...
  void Compute(Instruction* instruction) override {
    ...
    if (!eager_blob_object()->tensor_storage()->is_in_memory()) {
      OpCallInstructionPolicy tmp_op = eager_blob_object()->tensor_storage()->compute_op();
      CHECK_JUST(
          Recompute(&tmp_op, instruction->mut_stream()));
    }
    callback_(stream_policy->stream(), eager_blob_object());
    if (modifier_ == "mut" || modifier_ == "mut2") {
      eager_blob_object()->tensor_storage()->set_eviction_disabled(true);
    }
  }
