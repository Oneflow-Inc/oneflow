...
void CpuStreamType::InitDeviceCtx(std::unique_ptr<DeviceCtx> *device_ctx,
                                  Stream *stream) const {
  device_ctx->reset(new CpuDeviceCtx());
}

void CpuStreamType::InitInstructionStatus(
...
void CpuStreamType::DeleteInstructionStatus(
    const Stream &stream, InstructionStatusBuffer *status_buffer) const {
  // do nothing
}

bool CpuStreamType::QueryInstructionStatusDone(
...
CpuStreamType::MakeStreamDesc(const Resource &resource,
  ...
  ret->mutable_stream_type_id()->__Init__(
      LookupStreamType4TypeIndex<CpuStreamType>());
