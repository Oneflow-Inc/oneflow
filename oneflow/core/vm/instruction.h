...
INTRUSIVE_BEGIN(Instruction);
 public:
  ...
  bool is_lively_instruction_hook_empty() const { return lively_instruction_hook_.empty(); }
  bool is_pending_instruction_hook_empty() const { return pending_instruction_hook_.empty(); }
  bool is_barrier_instruction_hook_empty() const { return barrier_instruction_hook_.empty(); }
 ...
 private:
  ...
  Instruction() : intrusive_ref_(), status_buffer_(), instr_msg_(), parallel_desc_(), stream_(), instruction_hook_(), dispatched_instruction_hook_(), lively_instruction_hook_(), pending_instruction_hook_(), barrier_instruction_hook_(), mirrored_object_id2access_(), access_list_(), in_edges_(), out_edges_() {}
  INTRUSIVE_DEFINE_FIELD(intrusive::Ref, intrusive_ref_);
  // fields
  ...
  INTRUSIVE_DEFINE_FIELD(intrusive::ListHook, lively_instruction_hook_);
  // pending to ThreadCtx
  INTRUSIVE_DEFINE_FIELD(intrusive::ListHook, pending_instruction_hook_);
  INTRUSIVE_DEFINE_FIELD(intrusive::ListHook, barrier_instruction_hook_);
  // maps
