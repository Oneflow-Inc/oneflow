...
int64_t Stream::machine_id() const {
  return global_device_id() /
         thread_ctx().stream_rt_desc().stream_desc().num_streams_per_machine();
}
...
ObjectMsgPtr<Instruction>
Stream::NewInstruction(InstructionMsg *instr_msg,
  ...
  ObjectMsgPtr<Instruction> instruction =
      mut_free_instruction_list()->PopFront();
