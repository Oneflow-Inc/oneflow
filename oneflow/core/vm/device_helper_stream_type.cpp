...
public:
  ...
  void Infer(Instruction *instruction) const override {
    ...
    mem_buffer_object_type->mut_mem_case()
        ->mutable_device_cuda_mem()
...
public:
  ...
  void Compute(Instruction *instruction) const override {
    RwMutexedObject *value_rw_mutexed_object = nullptr;
    {
      FlatMsgView<FreeInstruction> view;
      CHECK(view.Match(instruction->instr_msg().operand()));
      value_rw_mutexed_object =
          instruction->mut_operand_value(view->mem_buffer());
    }
    const auto &stream = instruction->stream();
    cudaSetDevice(stream.device_id());
    OF_CUDA_CHECK(cudaFree(
...
void DeviceHelperStreamType::InitInstructionStatus(
...
void DeviceHelperStreamType::DeleteInstructionStatus(
    const Stream &stream, InstructionStatusBuffer *status_buffer) const {
  // do nothing
}
...
DeviceHelperStreamType::MakeStreamDesc(const Resource &resource,
  ...
  ret->mutable_stream_type_id()->__Init__(
      LookupStreamType4TypeIndex<DeviceHelperStreamType>());
