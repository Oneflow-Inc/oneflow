 ...
 private:
  ...
  void Run(VirtualMachine* vm, InstructionMsg* instr_msg) const {
    ...
    FOR_RANGE(int, i, 0, view->logical_object_id_size()) {
      ...
      auto logical_object = intrusive::MakeShared<LogicalObject>(logical_object_id);
      ...
      auto mirrored_object = intrusive::MakeShared<MirroredObject>(logical_object.Mutable(), 0);
      {
