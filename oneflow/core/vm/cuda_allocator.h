...
private:
  ...
  // If the Piece is_free = true, the pointer to the piece will be stored in the
  // Bin structure of
  // the corresponding BinSize. Pieces are stored in a linked list. The Piece's
  // prev and next are
  ...
  // Bin is a structure that stores a set of pieces which is free and has
  // similar size, and
  // these Pieces are arger than the size of bin
  //
  // CudaAllocator has a set of Bin structures according to the binary multiple
  // increasing relation,
  // which is used to quickly index and find the free Piece of appropriate size
  // when Allocate()
  //
  // The size of the smallest bin is 512 (512 is the smallest unit Allocated by
  // CudaAllocator,
  // and the memory size of all Allocated will be multiples of 512, 512 is
  ...
  // There maybe many consecutive disjoint Pieces distributed on the Block
  // memory
  struct Block {
    size_t size = 0;
    char *ptr = nullptr;
  ...
  int32_t BinNum4BinSize(size_t size) {
    ...
    return std::min(kBinNumSize - 1,
                    static_cast<int32_t>(63 ^ __builtin_clzll(value)));
  }
  ...
  // Insert the free Piece to the appropriate Bin which bin size is smaller than
  // piece
  ...
  // Insert a {piece->ptr, piece} pair into the ptr2piece_ map for search Piece
  // when call
  // Deallocate()
  void MarkPiece(Piece *piece);
  // Erase the {piece->ptr, piece} pair from ptr2piece_ because the ptr is
  // useless
