...
TestResourceDescScope::TestResourceDescScope(int64_t gpu_device_num,
                                             int64_t cpu_device_num) {
  Resource resource;
...
ObjectMsgPtr<VmResourceDesc> TestUtil::NewVmResourceDesc(int64_t device_num,
                                                         int64_t machine_num) {
  HashMap<std::string, int64_t> map{{"cpu", device_num}, {"gpu", device_num}};
  return ObjectMsgPtr<VmResourceDesc>::New(machine_num, map);
}

int64_t TestUtil::NewObject(InstructionMsgList *instr_msg_list,
  ...
  *parallel_desc_symbol_id = IdUtil::NewLogicalSymbolId();
  Global<SymbolStorage<ParallelDesc>>::Get()->Add(*parallel_desc_symbol_id,
                                                  parallel_conf);
  instr_msg_list->EmplaceBack(
      NewInstruction("NewParallelDescSymbol")
          ->add_int64_operand(*parallel_desc_symbol_id));
...
int64_t TestUtil::NewSymbol(InstructionMsgList *instr_msg_list) {
  ...
  instr_msg_list->EmplaceBack(
      NewInstruction("NewSymbol")->add_int64_operand(symbol_id));
  return symbol_id;
}

int64_t TestUtil::NewStringSymbol(InstructionMsgList *instr_msg_list,
                                  const std::string &str) {
  int64_t str_id = NewSymbol(instr_msg_list);
  Global<SymbolStorage<std::string>>::Get()->Add(str_id, str);
  instr_msg_list->EmplaceBack(
      NewInstruction("InitStringSymbol")->add_init_symbol_operand(str_id));
  return str_id;
}

void TestUtil::AddStreamDescByInstrNames(
    VmDesc *vm_desc, const std::vector<std::string> &instr_names) {
  TestUtil::AddStreamDescByInstrNames(vm_desc, 1, instr_names);
}

void TestUtil::AddStreamDescByInstrNames(
    VmDesc *vm_desc, int64_t parallel_num,
  ...
  auto Insert = [&](const std::string &instr_name) {
    ...
    auto instr_type =
        ObjectMsgPtr<StreamDesc>::New(stream_type_id, 1, parallel_num, 1);
