...
void MakeReceiveRequests(
  ...
  size_t data_size = 0;
  {
    ...
    data_size = view->size();
    const auto &dst_buffer_type = *CHECK_JUST(
    ...
    auto *dst_buffer_value = instruction->mut_operand_value(view->dst())
                                 ->Mut<MemBufferObjectValue>();
  ...
  GetTransporter(instruction)
      ->MakeReceiveTransportRequest(data_token.Get(), data_ptr, data_size,
                                    incomplete_cnt,
...
void L2RReceiverStreamType::InitDeviceCtx(
...
void L2RReceiverStreamType::InitInstructionStatus(
...
void L2RReceiverStreamType::DeleteInstructionStatus(
    const Stream &stream, InstructionStatusBuffer *status_buffer) const {
  // do nothing
}
...
ObjectMsgPtr<StreamDesc>
L2RReceiverStreamType::MakeStreamDesc(const Resource &resource,
  ...
  ret->mutable_stream_type_id()->__Init__(
      LookupStreamType4TypeIndex<L2RReceiverStreamType>());
