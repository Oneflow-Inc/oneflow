...
struct TransportRequestDataPointer {};

template<>
struct TransportRequestDataPointer<kReadTransportRequestType> {
  using type = const char*;
};

template<>
struct TransportRequestDataPointer<kWriteTransportRequestType> {
  using type = char*;
};
...
OBJECT_MSG_BEGIN(TransportRequest);
  // fields  
  OBJECT_MSG_DEFINE_RAW(typename TransportRequestDataPointer<request_type>::type, data_ptr);
  OBJECT_MSG_DEFINE_RAW(volatile std::atomic<int64_t>*, incomplete_cnt);
