...
template <typename T> void TryRegisterInstructionType() {
...
template <typename T> const InstructionType *LookupInstructionType() {
...
void RegisterInstrTypeId(const std::string &instr_type_name,
  ...
  RegisterInstrTypeId(instr_type_name, stream_type, LookupInstructionType<T>(),
                      interpret_type);
}
...
void RegisterInstructionType(const std::string &instr_type_name) {
  ...
  TryRegisterInferStreamTypeId<InferStreamType<typename T::stream_type>,
                               typename T::stream_type>();
  RegisterInstrTypeId<T>(instr_type_name,
                         LookupStreamType4TypeIndex<typename T::stream_type>(),
                         InterpretType::kCompute);
  RegisterInstrTypeId<T>(
      std::string("Infer-") + instr_type_name,
