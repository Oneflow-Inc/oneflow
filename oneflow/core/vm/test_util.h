...
struct TestUtil {
  ...
  static ObjectMsgPtr<VmResourceDesc> NewVmResourceDesc() {
    return NewVmResourceDesc(1);
  }
  ...
  static int64_t NewObject(InstructionMsgList *list,
  ...
  static void
  AddStreamDescByInstrNames(VmDesc *vm_desc,
                            const std::vector<std::string> &instr_names);
  static void
  AddStreamDescByInstrNames(VmDesc *vm_desc, int64_t parallel_num,
                            const std::vector<std::string> &instr_names);
};
