...
/*static*/ Maybe<uint32_t> GetRpcTokenCmdMajor(RpcTokenCmdLocalMajor cmd_local_major) {
	CHECK_LT_OR_RETURN(cmd_major, kRpcTokenCmdLocalMajorSize);
	...
	CHECK_LT_OR_RETURN(ret, RpcToken::kStartTokenMajor4Placement);
	return ret;
}
...
/*static*/ Maybe<void> WaitUntilDoneOrTimeout(const AsyncRpcCtx& ctx, int64_t seconds) {
	...
	while (*cond_cnt > 0) {
		...
		CHECK_LT_OR_RETURN(elapsed_seconds, seconds)
...
template<Maybe<void> (*SendOrRecv)(const RpcToken&, int64_t, void*, std::size_t, const std::function<void()>&)>
Maybe<void> AccessToAllOtherRanks(Symbol<SortedRankRanges> rank_ranges, const RpcToken& token, AsyncRpcCtx* ctx) {
...
template<
		Maybe<int64_t> (SortedRankRanges::*GetPrevOrNext)() const,
		Maybe<void> (*SendOrRecv)(const RpcToken&, int64_t, void*, std::size_t, const std::function<void()>&)>
Maybe<void> AccessToNearbyRank(Symbol<SortedRankRanges> rank_ranges, const RpcToken& token, AsyncRpcCtx* ctx) {
	if (rank_ranges->size() == 1) { return Maybe<void>::Ok(); }
	int64_t rank = JUST((rank_ranges->*GetPrevOrNext)());
...
Maybe<void> Send(const RpcToken& token, int64_t rank, void* buffer, std::size_t size, const std::function<void()>& Callback) {
	auto* transport = JUST(GlobalMaybe<Transport>());
	transport->Send(static_cast<uint64_t>(token), rank, buffer, size, Callback);
	return Maybe<void>::Ok();
}

Maybe<void> Recv(const RpcToken& token, int64_t rank, void* buffer, std::size_t size, const std::function<void()>& Callback) {
	auto* transport = JUST(GlobalMaybe<Transport>());
...
/*static*/ Maybe<void> RpcUtil::BroadcastToAllOtherRanks(Symbol<SortedRankRanges> rank_ranges, const RpcToken& token, AsyncRpcCtx* ctx) {
	JUST(AccessToAllOtherRanks<&Send>(rank_ranges, token, ctx));
	return Maybe<void>::Ok();
}

/*static*/ Maybe<void> RpcUtil::CollectFromAllOtherRanks(Symbol<SortedRankRanges> rank_ranges, const RpcToken& token, AsyncRpcCtx* ctx) {
