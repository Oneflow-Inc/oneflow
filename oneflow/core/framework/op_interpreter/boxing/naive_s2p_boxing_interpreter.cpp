...
Maybe<one::Tensor> NcclS2PBoxingInterpreter::Interpret(
  ...
  cfg::ParallelDistribution mid_parallel_distribution;
  mid_parallel_distribution.mutable_sbp_parallel()->Add()->mutable_broadcast_parallel();
  const auto& s2b_interpreter = std::make_shared<NcclCollectiveAllGatherBoxingInterpreter>();
  const auto& b2p_interpreter = std::make_shared<NaiveB2PBoxingInterpreter>();
  const auto& mid_tesnor = JUST(s2b_interpreter->Interpret(input, in_parallel_distribution,
                                                           SymbolOf(mid_parallel_distribution),
                                                           in_parallel_desc, out_parallel_desc));
  return JUST(b2p_interpreter->Interpret(mid_tesnor, SymbolOf(mid_parallel_distribution),
