...
static const kLimitParallelConfString = 1024 * 8;
struct FlatParallelConf {

	size_t available_size() const { return sizeof(FlatParallelConf) - kLimitParallelConfString + this-buffer_size; }
...
Maybe<void> SyncSymbolParallelDesc(uint64_t symbol_id, Symbol<ParallelDesc> parallel_desc) {
	...
	const auto& rpc_token =
			JUST(RpcToken::NewCmdRpcToken(kRankGroupRpcCmdSyncSymbolParallelDesc));
