...
TEST(ParallelConfUtil, MakeParallelConfSuccess) {
  ...
  auto parallel_conf = CHECK_JUST(MakeParallelConf(device_tag, machine_device_ids, nullptr, false));
...
TEST(ParallelConfUtil, MakeParallelConfError) {
  ...
  auto parallel_conf = TRY(MakeParallelConf(device_tag, machine_device_ids, nullptr, false));
  ASSERT_EQ(parallel_conf.error()->has_check_failed_error(), true);
}

TEST(ParallelConfUtil, ParseParallelConf) {
  {
    ...
    auto [device_tag, machine_device_ids, hierarchy, rematable] = ParseParallelConf(parallel_conf);
  ...
  }
  {
    ...
    parallel_conf.add_device_name("0:0-1");
    parallel_conf.mutable_hierarchy()->add_dim(2);
    parallel_conf.set_rematable(true);
    auto [device_tag, machine_device_ids, hierarchy, rematable] = ParseParallelConf(parallel_conf);
    ASSERT_EQ(device_tag, "cpu");
    ASSERT_NE(std::count(machine_device_ids.begin(), machine_device_ids.end(), "0:0-1"), 0);
    ASSERT_EQ(hierarchy->dim(0), 2);
    ASSERT_EQ(rematable, true);
  }
}
