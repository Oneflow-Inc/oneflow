...
Maybe<Tensor> Expand(
  ...
  const std::vector<int32_t>& in_shape_vec, 
  const std::vector<int32_t>& expand_shape_vec
){
  const int64_t storage_offset = 0;
  std::vector<int32_t> out_shape_vec;
  std::vector<int32_t> out_strides_vec;

  CHECK_JUST(getOutShapeAndStrideForFp(in_shape_vec, expand_shape_vec, out_shape_vec, out_strides_vec));
  DimVector dim_vec(out_shape_vec.begin(), out_shape_vec.end());
  StrideVector stride_vec(out_shape_vec.begin(), out_shape_vec.end());
  auto output = JUST(BasicView(input, Shape(dim_vec), Stride(stride_vec), storage_offset));
    ...
    auto backward_fn =
        std::make_shared<std::function<Maybe<void>(const TensorTuple&, TensorTuple*, bool)>>(
            [=](const TensorTuple& out_grads, TensorTuple* in_grads,
              ...
              in_grads->resize(1);
              in_grads->at(0) =
                  JUST(functional::ExpandGrad(out_grads.at(0), in_shape_vec, expand_shape_vec));
              return Maybe<void>::Ok();
            });
    TensorTuple outputs{output};
