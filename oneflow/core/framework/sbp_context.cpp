...
inline void SplitImpl(SbpSignature *sbp_sign, const std::string &bn,
                      int64_t axis) {
  (*sbp_sign->mutable_bn_in_op2sbp_parallel())[bn]
...
UserOpSbpSignatureBuilder &UserOpSbpSignatureBuilder::Split(const OpArg &op_arg,
                                                            int64_t axis) {
...
Maybe<void> GetSbpFnUtil::DefaultBroadcastToBroadcast(SbpContext *ctx) {
...
Maybe<void> GetSbpFnUtil::SplitForEachAxis(SbpContext *ctx) {
  ...
  int64_t num_axes = ctx->LogicalTensorDesc4InputArgNameAndIndex(
                            inputs.at(0).first, inputs.at(0).second)
                         .shape()
                         .NumAxes();
  for (const auto &pair : inputs) {
    CHECK_EQ(num_axes, ctx->LogicalTensorDesc4InputArgNameAndIndex(pair.first,
                                                                   pair.second)
                           .shape()
                           .NumAxes());
  }
