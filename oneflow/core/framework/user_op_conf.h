...
class OpArg final {
public:
  OpArg(std::string &&name, int32_t index)
...
template <typename T> class TypedAttrVal final : public AttrVal {
public:
...
public:
  ...
  template <typename T> const T &attr(const std::string &attr_name) const;
...
public:
  ...
  int32_t input_size(const std::string &arg_name) const {
    return conf_.input_size(arg_name);
  }
  const std::string &input(const std::string &arg_name, int32_t index) const {
  ...
  int32_t output_size(const std::string &arg_name) const {
    return conf_.output_size(arg_name);
  }
  const std::string &output(const std::string &arg_name, int32_t index) const {
    return conf_.output(arg_name, index);
  }
  std::string output_grad(const std::string &output_arg_name,
                          int32_t index) const;
  std::string GetGradTensorWithOpOutput(const std::string &output_arg_name,
                                        int32_t index) const;
  ...
  const TensorDesc &arg_tensor_desc(const std::string &arg_name,
                                    int32_t index) const;
  const TensorDesc &TensorDesc4ArgNameAndIndex(const std::string &arg_name,
                                               int32_t index) const;

public:
  void InputGradBind(const user_op::OpArg &input,
  ...
  void BindGradTensorWithOpInput(const std::string &logical_grad_blob_name,
                                 ...
                                 int32_t index) const;
  bool NeedGenGradTensor4OpInput(const std::string &input_arg_name,
                                 int32_t index) const;
...
public:
  ...
  UserOpConfWrapperBuilder &Op(const std::string &op_type_name) {
    return OpTypeName(op_type_name);
  }
...
using BackwardOpBuilderFn =
    std::function<UserOpConfWrapper(BackwardOpBuilder &)>;
class BackwardOpConfContext final {
public:
  BackwardOpConfContext(const UserOpWrapper &fw_op_wp,
