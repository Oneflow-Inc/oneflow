...
#define CASE_ENTRY(field, cpp_type, attr_type)                                 \
  /* UserOpAttrVal::ValueCase has the same order and naming convention as      \
   * UserOpAttrType */                                                         \
  case (static_cast<UserOpAttrVal::ValueCase>(attr_type)):                     \
    CHECK(attrs_                                                               \
              .emplace(kv.first,                                               \
                       std::make_shared<TypedAttrVal<cpp_type>>(               \
...
#undef CASE_ENTRY
    default:
      LOG(FATAL) << "Wrong attr value type: "
                 << static_cast<int32_t>(value_case);
    };
  }
}
...
const UserOpConf &UserOpConfWrapper::user_op_conf() const {
  return op_conf_.user_conf();
}

const std::string &UserOpConfWrapper::op_name() const {
  return op_conf_.name();
}
...
const std::string &UserOpConfWrapper::input(const std::string &arg_name,
                                            int32_t index) const {
  auto it = op_conf_.user_conf().input().find(arg_name);
  CHECK(it != op_conf_.user_conf().input().end()) << "arg_name: " << arg_name
                                                  << ", index: " << index;
  CHECK(index >= 0 && index < it->second.s_size());
  return it->second.s(index);
}

const std::string &UserOpConfWrapper::output(const std::string &arg_name,
                                             int32_t index) const {
  auto it = op_conf_.user_conf().output().find(arg_name);
  CHECK(it != op_conf_.user_conf().output().end()) << "arg_name: " << arg_name
                                                   << ", index: " << index;
  CHECK(index >= 0 && index < it->second.s_size());
  return it->second.s(index);
}

bool UserOpConfWrapper::has_input(const std::string &arg_name,
                                  int32_t index) const {
  return input_size(arg_name) > index;
}

bool UserOpConfWrapper::has_output(const std::string &arg_name,
                                   int32_t index) const {
...
int32_t UserOpConfWrapper::input_size(const std::string &arg_name) const {
  auto it = op_conf_.user_conf().input().find(arg_name);
  if (it == op_conf_.user_conf().input().end()) {
    return 0;
  }
...
int32_t UserOpConfWrapper::output_size(const std::string &arg_name) const {
  auto it = op_conf_.user_conf().output().find(arg_name);
  if (it == op_conf_.user_conf().output().end()) {
    return 0;
  }
  return it->second.s_size();
}
...
#define OP_WRAPPER_ATTR_MEMBER_FUNC(field, cpp_type, attr_type)                \
  template <>                                                                  \
  const cpp_type &UserOpConfWrapper::attr<cpp_type>(                           \
      const std::string &attr_name) const {                                    \
    auto it = attrs_.find(attr_name);                                          \
    if (it != attrs_.end()) {                                                  \
      return std::dynamic_pointer_cast<TypedAttrVal<cpp_type>>(it->second)     \
          ->val();                                                             \
    } else {                                                                   \
      LOG(FATAL) << "Cannot find the attr: " << attr_name                      \
                 << " with UserOpAttrType: "                                   \
                 << static_cast<int32_t>(attr_type);                           \
    }                                                                          \
  }                                                                            \
                                                                               \
  template <>                                                                  \
  UserOpConfWrapperBuilder &UserOpConfWrapperBuilder::Attr<cpp_type>(          \
      const std::string &attr_name, const cpp_type &val) {                     \
...
UserOpWrapper::UserOpWrapper(
  ...
  auto InitTensorDescFromOpArgs =
      [&](const PbMap<std::string, UserOpConf_ListString> &args) {
...
std::string UserOpWrapper::output_grad(const std::string &output_arg_name,
                                       int32_t index) const {
...
std::string
UserOpWrapper::GetGradTensorWithOpOutput(const std::string &output_arg_name,
                                         int32_t index) const {
  return output_grad(output_arg_name, index);
}

void UserOpWrapper::BindGradTensorWithOpInput(
    const std::string &logical_grad_blob_name,
    const std::string &input_arg_name, int32_t index) const {
  CHECK(NeedGenGradTensor4OpInput(input_arg_name, index));
  *diff_fn_(GenRepeatedBn(input_arg_name, index)) =
      GenLogicalBlobId(logical_grad_blob_name);
}

const TensorDesc &UserOpWrapper::arg_tensor_desc(const std::string &arg_name,
                                                 int32_t index) const {
...
const TensorDesc &
UserOpWrapper::TensorDesc4ArgNameAndIndex(const std::string &arg_name,
                                          int32_t index) const {
...
UserOpConfWrapper UserOpConfWrapperBuilder::Build() {
  ...
  if (scope_symbol_id_.has_value()) {
    op_conf.set_scope_symbol_id(scope_symbol_id_.value());
  }
  ...
  auto GenArgs = [&](const HashMap<std::string, std::vector<std::string>> &src,
    ...
    for (const auto &pair : src) {
      *(*arg_name2lbns)[pair.first].mutable_s() =
          StdVec2PbRpf<std::string>(pair.second);
    }
  };
  GenArgs(input_, user_conf->mutable_input());
  GenArgs(output_, user_conf->mutable_output());
  for (const auto &pair : attr_) {
    (*user_conf->mutable_attr())[pair.first] = pair.second;
  }
  wrapper_ =
      UserOpConfWrapper(*CHECK_JUST(CheckAndCompleteUserOpConfImpl(op_conf)));
  return wrapper_;
}

void BackwardOpConfContext::DefineOp(const std::string &op_name,
                                     const BackwardOpBuilderFn &fn) {
  auto it = op_builder_fns_.find(op_name);
  CHECK(it == op_builder_fns_.end()) << " op_name " << op_name
                                     << " has been defined.";
...
UserOpConfWrapper &BackwardOpConfContext::GetOp(const std::string &op_name) {
  ...
  if (it != op_builder_results_.end()) {
    ...
    CHECK(fn_it != op_builder_fns_.end()) << " op_name " << op_name
                                          << " has no builder function.";
    CHECK(fn_it->second != nullptr) << " op_name " << op_name
                                    << " builder function is null.";
    UserOpConfWrapperBuilder builder(op_name);
    auto ret = op_builder_results_.emplace(
        std::make_pair(op_name, std::move(fn_it->second(builder))));
    CHECK(ret.second == true) << " op_name " << op_name
                              << " build result insert failed.";
    ...
    if (arg_blob_num != arg.num()) {
      if (arg_blob_num == 0) {
        CHECK_OR_RETURN(arg.is_optional())
            ...
            << " arg name: " << arg.name()
            << " in OpDef must have blob in op_conf";
      } else {
...
Maybe<void> AddAttrDefaultValueAndCheckValid(const UserOpDef &op_def,
                                             OperatorConf *op_conf) {
  UserOpConf *user_conf = op_conf->mutable_user_conf();
  ...
  for (const auto &pair : user_conf->attr()) {
    CHECK_OR_RETURN(op_def_attr_names.find(pair.first) !=
                    op_def_attr_names.end())
        << " op_name: " << op_conf->name()
        << " op_type_name: " << user_conf->op_type_name()
  ...
  for (const auto &attr : op_def.attr()) {
    CHECK_OR_RETURN(
        ...
        << " op_type_name: " << user_conf->op_type_name()
        << " attr_name: " << attr.name()
        << " has different attr type in OpDef and OpConf, it should be with "
           "type: "
...
Maybe<void> AddUserOpConfOutputDefaultArg(const UserOpDef &op_def,
                                          OperatorConf *op_conf) {
  UserOpConf *user_conf = op_conf->mutable_user_conf();
  // add default output arg and lbn
  for (const auto &output_arg : op_def.output()) {
    if (user_conf->output().find(output_arg.name()) ==
            user_conf->output().end() &&
        (!output_arg.is_optional()) && (!output_arg.num_as_min())) {
      for (int32_t i = 0; i < output_arg.num(); ++i) {
        std::string lbn = GenLogicalBlobName(
            op_conf->name(), GenRepeatedBn(output_arg.name(), i));
...
Maybe<long long> GetUserOpAttrTypeImpl(const std::string &op_type_name,
  ...
  for (int32_t i = 0; i < op_def.attr_size(); ++i) {
    if (op_def.attr(i).name() == attr_name) {
      return op_def.attr(i).type();
    }
  }
  CHECK_OR_RETURN(false) << " Cannot find attr " << attr_name << " in op "
                         << op_type_name;
}

Maybe<OperatorConf>
CheckAndCompleteUserOpConfImpl(const OperatorConf &op_conf) {
  CHECK_OR_RETURN(op_conf.has_user_conf())
      << " Add default value only for user op";
  ...
  CHECK_OR_RETURN(val) << " Cannot find op_type_name: "
                       << user_conf->op_type_name();
  const UserOpDef &op_def = val->op_def;
  ...
  JUST(CheckArgDefIsValidInUserOpConf(op_conf, user_conf->input(),
                                      op_def.input()));
  JUST(CheckArgDefIsValidInUserOpConf(op_conf, user_conf->output(),
                                      op_def.output()));
  // check attr valid by user
  JUST(val->check_fn(user_op::UserOpDefWrapper(op_def),
                     user_op::UserOpConfWrapper(ret)));
  return ret;
}
