...
FLAT_MSG_BEGIN(FlatParallelDistribution);
  OF_PUBLIC Maybe<void> Init(uint64_t symbol_id, Symbol<cfg::ParallelDistribution> parallel_distribution) {
  ...
  OF_PUBLIC Maybe<void> Check(uint64_t symbol_id, Symbol<cfg::ParallelDistribution> parallel_distribution) const {
...
class SendFlatParallelDistributionAsyncRpcCtx : public AsyncRpcCtx {
 public:
  SendFlatParallelDistributionAsyncRpcCtx(
				uint64_t symbol_id, Symbol<cfg::ParallelDistribution> parallel_distribution)
...
class RecvFlatParallelDistributionAsyncRpcCtx : public AsyncRpcCtx {
 public:
  RecvFlatParallelDistributionAsyncRpcCtx(
				uint64_t symbol_id, Symbol<cfg::ParallelDistribution> parallel_distribution)
		: symbol_id_(symbol_id), parallel_distribution_(parallel_distribution) {}
  ...
  Maybe<void> MakeDataBufferAndCallback(int64_t rank, void** buffer, std::size_t* size,
    ...
    *Callback = [flat_parallel_distribution]() {};
	...
	Maybe<void> Check() const {
		...
		JUST(flat_parallel_distribution_->Check(symbol_id, parallel_distribution_));
		return Maybe<void>::Ok();
	}
...
Maybe<void> SyncSymbolParallelDistribution(uint64_t symbol_id, Symbol<cfg::ParallelDistribution> symbol) {
