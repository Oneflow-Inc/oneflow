...
using CheckAttrFn = std::function<Maybe<void>(const UserOpDefWrapper &,
...
using GetInputArgModifier = std::function<InputArgModifier *(
    const std::string &in_arg_name, int32_t in_arg_index)>;
using InputArgModifyFn =
    std::function<void(GetInputArgModifier, const UserOpConfWrapper &)>;
using OutputArgModifier = OutputBlobModifier;
using GetOutputArgModifier = std::function<OutputArgModifier *(
    const std::string &out_arg_name, int32_t out_arg_index)>;
using OutputArgModifyFn =
    std::function<void(GetOutputArgModifier, const UserOpConfWrapper &)>;
...
struct OpRegistryResult {
  ...
  GetSbpFn get_sbp_fn;
  // TODO(niuchong): move input_arg_modify_fn out of OpRegistryResult since it
  // is more about
...
public:
  ...
  OpRegistry &OptionalInputWithMinimum(const std::string &name,
                                       int32_t min_num);
  ...
  OpRegistry &OptionalOutputWithMinimum(const std::string &name,
                                        int32_t min_num);
...
static const std::string kUserSourceOpTickInputArgName =
