...
OpRegistry
UserOpRegistryMgr::CheckAndGetOpRegistry(const std::string &op_type_name) {
...
const OpRegistryResult *
UserOpRegistryMgr::GetOpRegistryResult(const std::string &op_type_name) {
  auto it = op_reg_result_.find(op_type_name);
  if (it != op_reg_result_.end()) {
    return &(it->second);
  }
  return nullptr;
}
...
OpGradRegistry
UserOpRegistryMgr::CheckAndGetOpGradRegistry(const std::string &op_type_name) {
  CHECK(!op_type_name.empty());
...
const OpGradRegistryResult *
UserOpRegistryMgr::GetOpGradRegistryResult(const std::string &op_type_name) {
  auto it = op_grad_reg_result_.find(op_type_name);
  if (it != op_grad_reg_result_.end()) {
    return &(it->second);
  }
  return nullptr;
}

OpKernelRegistry UserOpRegistryMgr::CheckAndGetOpKernelRegistry(
...
std::string GetErrorMsgOfSearchedOp(const KernelRegContext &ctx) {
  ...
  for (const auto &pair : ctx.inputs()) {
    ss << "\n DataType_Name of " << pair.first << "_" << pair.second << ": "
       << DataType_Name(ctx.TensorDesc4ArgNameAndIndex(pair.first, pair.second)
                            ->data_type());
  }
...
Maybe<const OpKernelRegistryResult *>
UserOpRegistryMgr::GetOpKernelRegistryResult(const std::string &op_type_name,
  ...
  for (const auto &reg_val : it->second) {
    if (reg_val.is_matched_hob(ctx)) {
      if (ret != nullptr) {
        ...
        return Error::MultipleOpKernelsMatchedError("There are more than one "
                                                    "kernels matching Current "
                                                    "OperatorConf. ",
                                                    debug_msgs)
               << GetErrorMsgOfSearchedOp(ctx);
      }
