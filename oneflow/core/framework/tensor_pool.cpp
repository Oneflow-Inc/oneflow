...
Maybe<vm::DTREagerBlobObject*> DTRTensorPool::find_best_tensor() {
    ...
    for (const auto object : candidates_) {
        if (auto shared_object = object.lock()) {
            ...
            // }
            if (static_cast<bool>(shared_object->compute_op()) && !shared_object->is_pinned() && (shared_object->is_evictable()) && shared_object->is_in_memory()) {
...
Maybe<void> DTRTensorPool::insert(std::shared_ptr<vm::DTREagerBlobObject> blob_object, size_t thres) {
