...
public:
  ...
  GetOpGradRegistryResult(const std::string &op_type_name);
  ...
  GetOpKernelRegistryResult(const std::string &op_type_name,
...
private:
  ...
  HashMap<std::string, std::vector<OpKernelRegistryResult>>
      op_kernel_reg_result_;
};

template <typename RegistryT> struct UserOpRegisterTrigger final {
...
#define REGISTER_USER_OP(name)                                                 \
  static ::oneflow::user_op::UserOpRegisterTrigger<                            \
      ::oneflow::user_op::OpRegistry>                                          \
      OF_PP_CAT(g_register_trigger, __COUNTER__) =                             \
          ::oneflow::user_op::UserOpRegistryMgr::Get().CheckAndGetOpRegistry(  \
              name)
...
#define REGISTER_USER_OP_GRAD(name)                                            \
  static ::oneflow::user_op::UserOpRegisterTrigger<                            \
      ::oneflow::user_op::OpGradRegistry>                                      \
      OF_PP_CAT(g_register_trigger, __COUNTER__) =                             \
          ::oneflow::user_op::UserOpRegistryMgr::Get()                         \
              .CheckAndGetOpGradRegistry(name)

#define REGISTER_USER_KERNEL(name)                                             \
  static ::oneflow::user_op::UserOpRegisterTrigger<                            \
      ::oneflow::user_op::OpKernelRegistry>                                    \
      OF_PP_CAT(g_register_trigger, __COUNTER__) =                             \
          ::oneflow::user_op::UserOpRegistryMgr::Get()                         \
              .CheckAndGetOpKernelRegistry(name)
