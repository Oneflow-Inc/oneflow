...
#define LIST_BASIC_ATTR_SEQ                                                    \
  OF_PP_MAKE_TUPLE_SEQ(at_list_int32, std::vector<int32_t>,                    \
                       UserOpAttrType::kAtListInt32)                           \
  OF_PP_MAKE_TUPLE_SEQ(at_list_int64, std::vector<int64_t>,                    \
                       UserOpAttrType::kAtListInt64)                           \
  OF_PP_MAKE_TUPLE_SEQ(at_list_float, std::vector<float>,                      \
                       UserOpAttrType::kAtListFloat)

#define LIST_ENUM_ATTR_SEQ                                                     \
  OF_PP_MAKE_TUPLE_SEQ(at_list_data_type, std::vector<DataType>,               \
                       UserOpAttrType::kAtListDataType)

#define LIST_MESSAGE_ATTR_SEQ                                                  \
  OF_PP_MAKE_TUPLE_SEQ(at_list_shape, std::vector<Shape>,                      \
                       UserOpAttrType::kAtListShape)

#define LIST_STRING_ATTR_SEQ                                                   \
  OF_PP_MAKE_TUPLE_SEQ(at_list_string, std::vector<std::string>,               \
                       UserOpAttrType::kAtListString)
...
template <typename T> struct GetAttrType;

template <UserOpAttrType AttrT> struct GetCppType;

#define SPECIALIZE_GET_ATTR_TYPE(field, type_cpp, type_proto)                  \
  template <>                                                                  \
  struct GetAttrType<type_cpp>                                                 \
      : std::integral_constant<UserOpAttrType, type_proto> {};                 \
  template <> struct GetCppType<type_proto> { typedef type_cpp type; };
