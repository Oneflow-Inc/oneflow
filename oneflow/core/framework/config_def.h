...
template <ConfigDefType config_def_type> struct ConfigDefBuidler final {
  ...
  const ConfigDefBuidler &Double(const std::string &name, double default_val,
                                 const std::string &description) const;
  const ConfigDefBuidler &String(const std::string &name,
...
#define REGISTER_CONFIG_DEF(config_def_type)                                   \
  static ConfigDefBuidler<config_def_type> OF_PP_CAT(                          \
      g_##config_def_type##_def_, __COUNTER__) =                               \
