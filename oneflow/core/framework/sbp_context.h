...
public:
  UserOpSbpSignatureBuilder(SbpSignatureList *sbp_sig_list)
      : sbp_sig_list_(sbp_sig_list) {}

  UserOpSbpSignatureBuilder &Split(const OpArg &op_arg, int64_t axis);
  UserOpSbpSignatureBuilder &Split(const std::vector<OpArg> &op_args,
                                   int64_t axis);
  UserOpSbpSignatureBuilder &
  Split(const std::vector<std::pair<std::string, int32_t>> &op_args,
        int64_t axis);

  UserOpSbpSignatureBuilder &Broadcast(const OpArg &op_arg);
  UserOpSbpSignatureBuilder &Broadcast(const std::vector<OpArg> &op_args);
  UserOpSbpSignatureBuilder &
  Broadcast(const std::vector<std::pair<std::string, int32_t>> &op_args);
  ...
  UserOpSbpSignatureBuilder &
  PartialSum(const std::vector<std::pair<std::string, int32_t>> &op_args);

  void Build() {
    *(sbp_sig_list_->mutable_sbp_signature()->Add()) = sbp_sig_tmp_;
  }
...
public:
  ...
  LogicalTensorDesc4InputArgNameAndIndex(const std::string &input_arg_name,
  ...
  UserOpSbpSignatureBuilder NewBuilder() {
    return UserOpSbpSignatureBuilder(sbp_sig_list_);
  }
...
protected:
  SbpContext(UserOpConfWrapper &&conf, SbpSignatureList *sbp_sig_list,
      ...
      : user_op_conf_(conf), sbp_sig_list_(sbp_sig_list),
        device_type_(device_type), parallel_num_(parallel_num) {}
