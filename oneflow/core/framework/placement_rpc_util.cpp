...
#include "oneflow/core/rpc/transport/transport.h"

namespace oneflow {

Maybe<void> NaiveTokenCheckAsyncRpcCtx::MakeDataBufferAndCallback(
...
Maybe<uint32_t> AutoIncrementalMajorValue4Placement() {
	static auto* major_value = new std::atomic<int64_t>(RpcToken::kStartTokenMajor4Placement);
...
class BroadcastCandidateMajorAsyncRpcCtx final : AsyncRpcCtx {
 public:
 ...
 private:
  std::shared_ptr<FlatParallelInfo> flat_parallel_info;
};

Maybe<void> BroadcastMyCandidateMajorToAllOtherRanks(Symbol<ParallelDesc> parallel_desc, uint32_t major) {
...
class CollectCandidateMajorAsyncRpcCtx final : AsyncRpcCtx {
 public:
	...
	Maybe<void> ForEachCandidateMajor(const std::function<void(uint32_t)>& DoEach) const {
		for (const auto& parallel_info : collected_parallel_info_) {
			CHECK_OR_RETURN(std::memcmp(
					parallel_info->buffer, parallel_conf_.data(), parallel_conf_.size()));
			JUST(DoEach(parallel_info->candidate_major));
		}
		return Maybe<void>::Ok();
	};
...
Maybe<void> ForEachReceivedCandidateMajor(
		Symbol<ParallelDesc> parallel_desc, const std::function<void(uint32_t)>& DoEach) {
	const auto& rank_ranges =
			JUST(SortedRankRanges::New4SoleDevicePerRankParallelDesc(parallel_desc));
	const auto& token = JUST(RpcToken4InitializingPlacementMajor());
	CollectCandidateMajorAsyncRpcCtx ctx{};
...
Maybe<void> BroadcastParallelConfSizeToOthers(
		Symbol<SortedRankRanges> rank_ranges, size_t parallel_conf_size) {
...
std::unique_ptr<RpcToken>* MutThreadLocalRpcToken(Symbol<ParallelDesc> parallel_desc) {
  static thread_local HashMap<Symbol<ParallelDesc>, std::unique_ptr<RpcToken>> parallel_desc2sync_token;
...
Maybe<RpcToken> GetAutoIncrementalRpcToken(Symbol<ParallelDesc> parallel_desc) {
  ...
  const auto& token = *MutThreadLocalRpcToken(parallel_desc);
	if (!token) { JUST(InitCurrentRpcToken); }
