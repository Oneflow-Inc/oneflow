...
std::shared_ptr<ConsistentTensor> ConsistentTensor::MakeTensor(
  ...
  if (is_lazy) {
    impl = std::make_shared<LazyConsistentTensorImpl>(shape, dtype, sbp_descriptor, parallel_desc,
                                                      requires_grad, is_leaf, retain_grad);
  } else {
    impl = std::make_shared<EagerConsistentTensorImpl>(shape, dtype, sbp_descriptor, parallel_desc,
                                                       requires_grad, is_leaf, retain_grad);
  }
