...
class NaiveAsyncRpcCtx final : public AsyncRpcCtx {
 public:
  ...
  NaiveAsyncRpcCtx(
      const RpcToken& rpc_token,
      const std::function<Maybe<void>(void**, std::size_t*, std::function<void()>*)>& PrepareSend,
      const std::function<Maybe<void>(int64_t, void**, std::size_t*, std::function<void()>*)>& PrepareRecvWithRank)
      : AsyncRpcCtx(rpc_token), prepare_send_(PrepareSend), prepare_recv_with_rank_(PrepareRecvWithRank) {}

  NaiveAsyncRpcCtx(
      const RpcToken& rpc_token,
      const std::function<Maybe<void>(int64_t, void**, std::size_t*, std::function<void()>*)>& PrepareSendWithRank,
      const std::function<Maybe<void>(void**, std::size_t*, std::function<void()>*)>& PrepareRecv)
      : AsyncRpcCtx(rpc_token), prepare_send_with_rank_(PrepareSendWithRank), prepare_recv_(PrepareRecv) {}

  NaiveAsyncRpcCtx(
      const RpcToken& rpc_token,
      const std::function<Maybe<void>(int64_t, void**, std::size_t*, std::function<void()>*)>& PrepareSendWithRank,
      const std::function<Maybe<void>(int64_t, void**, std::size_t*, std::function<void()>*)>& PrepareRecvWithRank)
      : AsyncRpcCtx(rpc_token), prepare_send_with_rank_(PrepareSendWithRank), prepare_recv_with_rank_(PrepareRecvWithRank) {}
 ...
 private:
  ...
  std::function<Maybe<void>(int64_t, void**, std::size_t*, std::function<void()>*)> prepare_send_with_rank_;
  std::function<Maybe<void>(void**, std::size_t*, std::function<void()>*)> prepare_recv_;
  std::function<Maybe<void>(int64_t, void**, std::size_t*, std::function<void()>*)> prepare_recv_with_rank_;
};
