...
Maybe<void>
BatchAxisInferFnUtil::DefaultAsFirstHasValueInput(BatchAxisContext *ctx) {
...
Maybe<void> BatchAxisInferFnUtil::NaiveInferBatchAxis(BatchAxisContext *ctx) {
  ...
  for (const auto &in_arg_pair : ctx->inputs()) {
    ...
    if (cur_in_batch_axis->has_value() == false) {
      continue;
    }
    if (batch_axis) {
  ...
  *ctx->BatchAxis4ArgNameAndIndex(sole_out_arg_pair.first,
                                  sole_out_arg_pair.second) = *batch_axis;
  return Maybe<void>::Ok();
}
