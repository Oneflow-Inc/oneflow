...
OpRegistry& OpRegistry::SetDeviceAndStreamInferFn(DeviceAndStreamInferFn device_and_stream_infer_fn) {
...
Maybe<OpRegistry&> OpRegistry::Finish() {
  ...
  if (result_.cpu_only_supported && result_.device_and_stream_infer_fn == nullptr) {
    result_.device_and_stream_infer_fn = [](DeviceAndStreamInferContext* ctx) -> Maybe<Symbol<Stream>> {
      for (const auto& pair : ctx->inputs()) {
