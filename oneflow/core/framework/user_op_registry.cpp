...
bool InsertIfNotExists(const std::string &name,
                       HashSet<std::string> *unique_names) {
...
OpRegistry &OpRegistry::ArgImpl(bool is_input, const std::string &name,
...
#define OP_REG_ARG_MEMBER_FUNC(name_prefix, is_input, is_optional)             \
  ...
  OpRegistry &OpRegistry::name_prefix##WithMinimum(const std::string &name,    \
                                                   int32_t min_num) {          \
...
void AddAttrWithDefault(OpRegistryResult *result, const std::string &name,
                        UserOpAttrType type,
...
#define ATTR_MEMBER_FUNC(field, cpp_type, attr_type)                           \
  template <>                                                                  \
  OpRegistry &OpRegistry::Attr<cpp_type>(                                      \
      const std::string &name, UserOpAttrType type, cpp_type &&default_val) {  \
    CHECK(InsertIfNotExists(name, &unique_names_));                            \
    CHECK_EQ(type, attr_type);                                                 \
    AddAttrWithDefault(&result_, name, type,                                   \
                       [default_val](UserOpDef::AttrDef *attr_def) {           \
                         AttrValAccessor<cpp_type>::Attr(                      \
...
OpRegistry &
OpRegistry::SetTensorDescInferFn(TensorDescInferFn tensor_desc_infer_fn) {
  result_.tensor_desc_infer_fn = std::move(tensor_desc_infer_fn);
  return *this;
}

OpRegistry &
OpRegistry::SetBatchAxisInferFn(BatchAxisInferFn batch_axis_infer_fn) {
...
OpRegistry &
OpRegistry::SetInputArgModifyFn(InputArgModifyFn input_arg_modify_fn) {
  result_.input_arg_modify_fn = std::move(input_arg_modify_fn);
  return *this;
}

OpRegistry &
OpRegistry::SetOutputArgModifyFn(OutputArgModifyFn output_arg_modify_fn) {
...
OpRegistry &OpRegistry::Finish() {
  ...
  if (result_.batch_axis_infer_fn == nullptr) {
    result_.batch_axis_infer_fn =
        BatchAxisInferFnUtil::DefaultAsFirstHasValueInput;
  }
  ...
  if (result_.input_arg_modify_fn == nullptr) {
    result_.input_arg_modify_fn = [](GetInputArgModifier,
                                     const UserOpConfWrapper &) {};
  }
  if (result_.output_arg_modify_fn == nullptr) {
    result_.output_arg_modify_fn = [](GetOutputArgModifier,
                                      const UserOpConfWrapper &) {};
  }
  return *this;
}
