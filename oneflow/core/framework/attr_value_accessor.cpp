...
#define BASIC_AND_ENUM_ATTR_SEQ_ENTRY(field, cpp_type, attr_type)              \
  ...
  template <>                                                                  \
  void AttrValAccessor<cpp_type>::Attr(const cpp_type &cpp_val,                \
...
template <> Shape AttrValAccessor<Shape>::Attr(const UserOpAttrVal &val) {
  return Shape(val.at_shape());
}
template <>
void AttrValAccessor<Shape>::Attr(const Shape &cpp_val,
                                  UserOpAttrVal *attr_val) {
...
#define LIST_BASIC_ATTR_SEQ_ENTRY(field, cpp_type, attr_type)                  \
  ...
  void AttrValAccessor<cpp_type>::Attr(const cpp_type &cpp_val,                \
                                       UserOpAttrVal *attr_val) {              \
    *(attr_val->mutable_##field()->mutable_val()) =                            \
        StdVec2PbRf<cpp_type::value_type>(cpp_val);                            \
  }
...
#define LIST_ENUM_ATTR_SEQ_ENTRY(field, cpp_type, attr_type)                   \
  ...
  void AttrValAccessor<cpp_type>::Attr(const cpp_type &cpp_val,                \
                                       UserOpAttrVal *attr_val) {              \
    using proto_type = std::remove_reference_t<decltype(                       \
        attr_val->field().val())>::value_type;                                 \
    std::vector<proto_type> vec;                                               \
    vec.reserve(cpp_val.size());                                               \
    for (const auto &value : cpp_val) {                                        \
      vec.emplace_back(static_cast<proto_type>(value));                        \
    }                                                                          \
    *(attr_val->mutable_##field()->mutable_val()) =                            \
        StdVec2PbRf<proto_type>(vec);                                          \
  }
...
template <>
std::vector<Shape>
AttrValAccessor<std::vector<Shape>>::Attr(const UserOpAttrVal &val) {
  std::vector<Shape> ret;
  ret.reserve(val.at_list_shape().val_size());
  for (const auto &value : val.at_list_shape().val()) {
    ret.emplace_back(value);
  }
  return ret;
}
template <>
void AttrValAccessor<std::vector<Shape>>::Attr(
    const std::vector<Shape> &cpp_val, UserOpAttrVal *attr_val) {
  if (attr_val->at_list_shape().val_size() > 0) {
...
AttrValAccessor<std::vector<std::string>>::Attr(const UserOpAttrVal &val) {
  return PbRpf2StdVec<std::string>(val.at_list_string().val());
}
template <>
void AttrValAccessor<std::vector<std::string>>::Attr(
    const std::vector<std::string> &cpp_val, UserOpAttrVal *attr_val) {
  *(attr_val->mutable_at_list_string()->mutable_val()) =
      StdVec2PbRpf<std::string>(cpp_val);
}
