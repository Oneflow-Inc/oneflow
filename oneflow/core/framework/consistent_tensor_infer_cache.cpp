...
size_t SrcOpConsistentTensorMetaInferArgs::hash_value() const {
  ...
  hash_value ^= std::hash<Symbol<cfg::NdSbp>>()(parallel_distribution_);
  return hash_value;
}
...
/* static */ Maybe<const ConsistentTensorInferResult> ConsistentTensorInferCache::Infer(
  ...
  JUST(op->FillOpParallelDesc(parallel_desc.shared_from_symbol()));
  {
    ...
    cfg::NdSbpSignature parallel_distribution_constraints;
    JUST(infer_args.MakeNdSbpConstraints(user_op_expr,
    ...
    JUST(op->InferNdSbpSignatureIf(parallel_distribution_constraints, *parallel_desc,
                                                  NdSbpInferHint4Ibn));
  }
  auto* result =
  ...
  for (int32_t i = 0; i < user_op_expr.input_size(); ++i) {
    ...
    const auto& parallel_distribution = SymbolOf(*JUST(op->NdSbp4BnInOp(ibn)));
  ...
  for (int32_t i = 0; i < user_op_expr.output_size(); ++i) {
    ...
    const auto& parallel_distribution = SymbolOf(*JUST(op->NdSbp4BnInOp(obn)));
