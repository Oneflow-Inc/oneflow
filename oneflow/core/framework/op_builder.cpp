...
Maybe<UserOpExpr> OpBuilder::Build() {
  return UserOpExpr::New(op_name_, /**is_support_stride=*/false, std::move(proto_), indexed_ibns_, indexed_obns_);
}

Maybe<UserOpExpr> OpBuilder::Build(const bool is_support_stride) {
  return UserOpExpr::New(op_name_, is_support_stride, std::move(proto_), indexed_ibns_, indexed_obns_);
}
