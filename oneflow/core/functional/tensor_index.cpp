...
Maybe<Tensor> ApplyAdvancedIndexing(const std::shared_ptr<Tensor>& input,
  ...
  if (transposed_input->is_consistent()) {
    ...
    std::vector<Symbol<cfg::SbpParallel>> grad_sbp_tuple(n, broadcast_sbp);
    packed_indices = JUST(ToConsistent(packed_indices, placement, grad_sbp_tuple, grad_sbp_tuple));
  } else {
