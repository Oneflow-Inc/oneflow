 ...
 public:
  ...
  Maybe<TensorTuple> operator()(const std::shared_ptr<one::Tensor>& input,
    ...
    for (int32_t i = 0; i < num_indices; i++) {
      ...
      output[i] = JUST(LogicalSlice(input, start, stop, step, /*enable_view_slice=*/false));
      start[pos_dim] = end_idx;
    }
    stop[pos_dim] = input->shape()->At(ndim - 1);
    output[num_indices] = JUST(LogicalSlice(input, start, stop, step, /*enable_view_slice=*/false));
