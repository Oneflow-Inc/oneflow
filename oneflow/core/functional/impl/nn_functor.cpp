...
class PoolingNDFunctor {
 public:
  Maybe<TensorTuple> operator()(const std::shared_ptr<one::Tensor>& x,
    ...
    ctx->set_kernel_size(kernel_size);
    if (stride.has_value()) {
      ctx->set_stride(*JUST(stride));
    } else {
      ctx->set_stride(
          kernel_size);  // If stride is None, we set it as kernel_size to align Pytorch.
    }
 ...
 public:
  ...
  Maybe<Tensor> operator()(const std::shared_ptr<one::Tensor>& x,
    ...
    if (!is_training) {
      ...
      ctx->set_axis(axis);
      // convert torch momentum to tensorflow momentum
      ctx->set_momentum(1.0f - momentum);
      ctx->set_training(false);
 ...
 public:
  ...
  Maybe<Tensor> operator()(const std::shared_ptr<one::Tensor>& input, const int64_t& num_classes,
    ...
    if (is_on_value_double || is_off_value_double) {
      ...
      ctx->set_integer_off_value(0);
    } else {
      ctx->set_dtype(kInt64);
      ctx->set_floating_on_value(0);
      ctx->set_floating_off_value(0);
      ctx->set_integer_on_value(JUST(on_value.As<int64_t>()));
      ctx->set_integer_off_value(JUST(off_value.As<int64_t>()));
    }
 ...
 public:
  ...
  Maybe<Tensor> operator()(const std::shared_ptr<one::Tensor>& x, const int64_t& diagonal,
    ...
    if (is_fill_value_double) {
      ctx->set_floating_fill_value(JUST(fill_value.As<double>()));
      ctx->set_integer_fill_value(0);
      ctx->set_is_floating_fill_value(true);
    } else {
      ctx->set_floating_fill_value(0);
      ctx->set_integer_fill_value(JUST(fill_value.As<int64_t>()));
      ctx->set_is_floating_fill_value(false);
    }

    if (is_scale_double) {
      ctx->set_floating_scale_value(JUST(scale.As<double>()));
      ctx->set_integer_scale_value(0);
      ctx->set_is_floating_scale_value(true);
    } else {
      ctx->set_floating_scale_value(0);
      ctx->set_integer_scale_value(JUST(scale.As<int64_t>()));
      ctx->set_is_floating_scale_value(false);
    }
    return OpInterpUtil::Dispatch<Tensor>(*op_, {x}, ctx);
  }
