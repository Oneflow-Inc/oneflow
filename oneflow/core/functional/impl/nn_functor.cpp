...
class BinaryCrossEntropyLossFunctor : public LossFunctorBase {
 public:
  ...
  Maybe<Tensor> operator()(const std::shared_ptr<one::Tensor>& input,
                           ...
                           const std::string& reduction) const {
    MutableAttrMap attrs;
    auto out = weight ? OpInterpUtil::Dispatch<Tensor>(
                   *op_weight_, {input, target, JUST(ToContiguous(JUST(weight)))}, attrs)
    ...
    if (weight) {
      if (pos_weight) {
        out = JUST(OpInterpUtil::Dispatch<Tensor>(
            *op_weight_pos_,
            {input, target, JUST(ToContiguous(JUST(weight))), JUST(ToContiguous(JUST(pos_weight)))},
            attrs));
      } else {
        out = JUST(OpInterpUtil::Dispatch<Tensor>(
            *op_weight_, {input, target, JUST(ToContiguous(JUST(weight)))}, attrs));
      ...
      if (pos_weight) {
        out = JUST(OpInterpUtil::Dispatch<Tensor>(
            *op_pos_, {input, target, JUST(ToContiguous(JUST(pos_weight)))}, attrs));
      } else {
  ...
  Maybe<Tensor> operator()(const std::shared_ptr<one::Tensor>& input,
    ...
    if (weight) {
      kernel_result = JUST(OpInterpUtil::Dispatch<TensorTuple>(
          *op_weight_, {input_, target_, JUST(ToContiguous(JUST(weight)))}, attrs));
    } else {
...
class CrossEntropyFunctor {
 public:
  ...
  Maybe<Tensor> operator()(const std::shared_ptr<one::Tensor>& input,
    ...
    if (weight) {
      kernel_result = JUST(OpInterpUtil::Dispatch<TensorTuple>(
          *op_nll_weight_, {input_, target_, JUST(ToContiguous(JUST(weight)))}, attrs));
    } else {
