    ...
    const auto out =
        JUST(sequence_function(functional::Sub)
                 ...
                 .then([&margin](const std::shared_ptr<one::Tensor>& x) {
                   return functional::ScalarAdd(x, Scalar(margin), /*alpha=*/1.0, /*inplace=*/true);
                 })
    ...
    if (swap) {
      auto distance_swap =
          JUST(VectorNorm(JUST(ScalarAdd(eps, JUST(Sub(positive, negative)), /*alpha=*/1.0)), p,
                          dim, /*keepdim=*/false, positive->dtype()));
      da_n = JUST(Minimum(distance_swap, da_n));
    }
    auto triplet_loss =
        JUST(Clamp(JUST(ScalarAdd(JUST(Sub(da_p, da_n)), margin, /*alpha=*/1.0, /*inplace=*/false)),
                   /*min=*/0.0, NullOpt));
