  ...
  Maybe<Tensor> operator()(const std::shared_ptr<one::Tensor>& x,
    ...
    if (bias) {
      ...
      return OpInterpUtil::Dispatch<Tensor>(
          *bias_op_, {conv_out, JUST(bias)}, bias_attrs);
    } else {
      return conv_out;
    }
  }
 ...
 public:
  ...
  Maybe<Tensor> operator()(const std::shared_ptr<one::Tensor>& x,
    ...
    deconv_out = JUST(OpInterpUtil::Dispatch<Tensor>(
        *deconv_op_, {x, weight}, deconv_attrs));
    if (bias) {
      MutableAttrMap bias_attrs;
      JUST(bias_attrs.SetAttr<int32_t>("axis", 1));
      return OpInterpUtil::Dispatch<Tensor>(*bias_op_, {deconv_out, JUST(bias)},
                                            bias_attrs);
    } else {
      return deconv_out;
    }
  ...
  Maybe<Tensor> operator()(const std::shared_ptr<one::Tensor>& input,
    ...
    auto out =
        weight ? OpInterpUtil::Dispatch<Tensor>(
            *op_weight_,
            {input, target, JUST(ToContiguous(JUST(weight)))}, attrs)
    ...
    if (weight) {
      if (pos_weight) {
        out = JUST(OpInterpUtil::Dispatch<Tensor>(
            *op_weight_pos_,
            {input, target, JUST(ToContiguous(JUST(weight))),
      ...
      if (pos_weight) {
        out = JUST(OpInterpUtil::Dispatch<Tensor>(
            *op_pos_,
            {input, target, JUST(ToContiguous(JUST(pos_weight)))},
            attrs));
      } else {
        out = JUST(OpInterpUtil::Dispatch<Tensor>(*op_, {input, target},
                                                  attrs));
      }
    }
    return apply_reduction(out, reduction);
  ...
  Maybe<Tensor> operator()(const std::shared_ptr<one::Tensor>& input,
    ...
    if (weight) {
      kernel_result = JUST(OpInterpUtil::Dispatch<TensorTuple>(
          *op_weight_,
          {input_, target_, JUST(ToContiguous(JUST(weight)))}, attrs));
    } else {
      kernel_result = JUST(OpInterpUtil::Dispatch<TensorTuple>(
          *op_, {input_, target_}, attrs));
    }
...
class SparseCrossEntropyFunctor {
 public:
  ...
  Maybe<Tensor> operator()(const std::shared_ptr<one::Tensor>& prediction,
    ...
    return OpInterpUtil::Dispatch<Tensor>(*op_, {prediction, label},
                                          attrs);
  }
 ...
 public:
  ...
  Maybe<Tensor> SparseSoftmaxCrossEntropyOperator(const std::shared_ptr<one::Tensor>& logits,
    ...
    const auto& result = JUST(OpInterpUtil::Dispatch<TensorTuple>(
        *op_sparse_softmax_cross_entropy_, {logits, label}, attrs));
    return result->at(1);
  }
  ...
  Maybe<Tensor> EagerSparseSoftmaxCrossEntropyMsOperator(
    ...
    const auto& max_device_stage = JUST(OpInterpUtil::Dispatch<TensorTuple>(
        *op_reduce_max_device_stage_, {logits}, attrs));
    ...
    const auto& max_global_stage = JUST(OpInterpUtil::Dispatch<TensorTuple>(
        *op_reduce_max_global_stage_,
        {max_global_stage_input0, max_global_stage_input1}, attrs));
    ...
    const auto& output_exp = JUST(OpInterpUtil::Dispatch<TensorTuple>(
        *op_exp_, {(*output_broadcast_sub)[0]}, attrs));
    // op_reduce_sum_
    JUST(attrs.SetAttr<std::vector<int32_t>>("axis", {axis}));
    JUST(attrs.SetAttr<bool>("keepdims", true));
    const auto& output_reduce_sum = JUST(OpInterpUtil::Dispatch<TensorTuple>(
        *op_reduce_sum_, {(*output_exp)[0]}, attrs));
    ...
    const auto& output = JUST(OpInterpUtil::Dispatch<Tensor>(
        *op_sparse_cross_entropy_ms_, {(*predictions)[0], label},
        attrs));
    return output;
  }
  ...
  Maybe<Tensor> operator()(const std::shared_ptr<one::Tensor>& x,
    ...
    if (!training) {
      ...
      return OpInterpUtil::Dispatch<one::Tensor>(
          *norm_eval_op_,
          {x, JUST(moving_mean), JUST(moving_variance),
           gamma_val, beta_val},
          attrs);
    }
    if (moving_mean) {
    ...
    if (!is_training) {
      ...
      if (addend) {
        const auto& add_result = JUST(OpInterpUtil::Dispatch<one::Tensor>(
            *add_op_, {normalize_result, JUST(addend)}));
      ...
      if (addend) {
        return OpInterpUtil::Dispatch<one::Tensor>(
            *fused_addend_norm_training_stats_op_,
            {x, JUST(addend), JUST(moving_mean),
             JUST(moving_variance), gamma, beta},
            attrs);
      } else {
        return OpInterpUtil::Dispatch<one::Tensor>(
...
class DropoutFunctor {
 public:
  ...
  Maybe<Tensor> operator()(const std::shared_ptr<one::Tensor>& x, const float& p,
    ...
    if (addend) {
      if ((!training) || p == 0.0) {
        JUST(OpInterpUtil::Dispatch(*add_op_, {x, JUST(addend)},
                                    outputs.get()));
      } else {
        outputs->resize(2);
        JUST(OpInterpUtil::Dispatch(*dropout_addend_op_,
                                    {x, JUST(addend)}, outputs.get(),
...
class DropoutGradFunctor {
 public:
  ...
  Maybe<Tensor> operator()(const std::shared_ptr<one::Tensor>& dy,
    ...
    return OpInterpUtil::Dispatch<Tensor>(*dropout_grad_op_, {dy, mask},
                                          dropout_grad_attrs);
  }
...
class FusedSelfAttentionGradFunctor {
 public:
  ...
  Maybe<Tensor> operator()(const std::shared_ptr<one::Tensor>& query_mul_key_grad,
    ...
    return OpInterpUtil::Dispatch<Tensor>(
        *op_,
        {query_mul_key_grad, value_grad, hidden_states},
        attrs);
  }
...
class FusedScaleTrilSoftmaxMaskScaleFunctor {
 public:
  ...
  Maybe<TensorTuple> operator()(const std::shared_ptr<one::Tensor>& x, const float p,
    ...
    return OpInterpUtil::Dispatch<TensorTuple>(*fused_op_, {x, mask},
                                               fused_attrs);
  }
  ...
  Maybe<Tensor> operator()(const std::shared_ptr<one::Tensor>& a,
    ...
    if (p > 0.0) {
      return SequenceFunction<Maybe<Tensor>()>([&]() -> Maybe<Tensor> {
          ...
          .then([&](const std::shared_ptr<one::Tensor>& x) {
            return OpInterpUtil::Dispatch<Tensor>(
                *fused_bias_add_mask_scale_op_, {a, b, x},
...
class CtcGreedyDecoderFunctor {
 public:
  ...
  Maybe<TensorTuple> operator()(const std::shared_ptr<one::Tensor>& log_probs,
    ...
    return OpInterpUtil::Dispatch<TensorTuple>(
        *op_, {log_probs, input_lengths}, attrs);
  }
...
class PariticalFCSampleDisableBoxing {
 public:
  ...
  Maybe<TensorTuple> operator()(const std::shared_ptr<one::Tensor>& sampled_weight_diff,
    ...
    return OpInterpUtil::Dispatch<TensorTuple>(
        *op_, {sampled_weight_diff, sampled_label});
  }
