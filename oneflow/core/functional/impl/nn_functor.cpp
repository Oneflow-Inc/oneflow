 ...
 public:
  ...
  Maybe<Tensor> operator()( const std::shared_ptr<one::Tensor>& dy,
    ...
    CHECK_EQ_OR_RETURN(dy_num_axes, matmul_wx_num_axes)
      << "number of axes of \'matmul_wx\' (" << matmul_wx_num_axes << ") should equal to the one of \'dy\' (" << dy_num_axes << ")";
    ...
    if(is_split_mode){
      CHECK_EQ_OR_RETURN(dy_shape.At(dy_num_axes-1), matmul_wx_shape.At(matmul_wx_num_axes-1))
        << "last dimension of \'dy\'(" << dy_shape.At(dy_num_axes-1)
    ...
    if(is_split_mode){
      ...
      << "number of axes of \'matmul_vx\' (" << matmul_vx_num_axes << ") should equal to the one of \'dy\' (" << dy_num_axes << ")";
      ...
      CHECK_EQ_OR_RETURN(dy_shape.At(dy_num_axes-1), matmul_vx_shape.At(matmul_vx_num_axes-1))
        << "last dimension of \'dy\'(" << dy_shape.At(dy_num_axes-1)
        << ") and \'matmul_vx\'(" << matmul_vx_shape.At(matmul_vx_num_axes-1) 
        << ") is not consistent";
    }
    ...
    if (is_split_mode) {
      return OpInterpUtil::Dispatch<one::Tensor>(*split_op_, {dy, matmul_wx, JUST(matmul_vx)}, attrs);
    } else {
