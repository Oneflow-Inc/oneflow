...
class CompilationResourceStore {
 public:
  explicit CompilationResourceStore(const std::string &builder_name,
    ...
    builder_.reset(new xla::XlaBuilder(absl::StrCat("XlaBuilder_",
                                                    builder_name)));
...
void XlaLaunchKernel<device_type, T>::BuildLocalExecutable(
    const CompilationResourceStore &resource,
  ...
  mola::XlaCompiler compiler(resource.client(), resource.builder(),
                             this->op_conf().xla_launch_conf(), device_type,
                             resource.parallel_ctx(), entry_blobs,
...
void XlaLaunchKernel<device_type, T>::AsyncRunExecutable(
    const CompilationResourceStore &resource, xla::LocalExecutable *executable,
    const std::vector<Blob *> &entry_blobs,
...
void XlaLaunchKernel<device_type, T>::ForwardDataContent(
  ...
  CompilationResourceStore resource(this->op_conf().name(), device_type,
                                    1 /*intra_op_num_threads*/);
  mola::CompilationResult compile_result;
  BuildLocalExecutable(resource, entry_blobs, entry_blob_names,
                       &compile_result);

  xla::LocalExecutable *executable = compile_result.executable.get();
  CHECK(executable) << "Executable build failed.";
  AsyncRunExecutable(resource, executable, entry_blobs,
