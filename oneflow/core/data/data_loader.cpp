...
DataLoader::DataLoader(std::shared_ptr<Dataset> dataset, size_t batch_size, size_t max_count,
                       size_t qsize, size_t num_replicas, size_t rank)
    : batch_size_(batch_size),
      load_count_(max_count),
      dataset_(dataset),
...
size_t DataLoader::LoadBatch() {
  ...
  load_count_ -= batch_idx_seq.size();
  return load_count_;
}
