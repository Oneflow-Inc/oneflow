...
DataField* DataInstance::AddField(const std::string& field_name, 
                                  const DataFieldProto& field_proto) {
  CHECK(fields_.emplace(field_name, CreateDataField(feature, data_field_proto)).second);
...
std::unique_ptr<DataField> CreateDataField(const DataFieldProto& proto) {
...
std::unique_ptr<DataField> CreateDataFieldFromFeature(const Feature& feature,
  ...
  auto data_field_ptr = CreateDataField(proto);
  data_field_ptr->FromFeature(feature, proto);
  return data_field_ptr;
}
