...
void GenerateMs1BackwardOpConf(
    const Operator& op, std::vector<OperatorConf>* op_confs,
    const std::function<LogicalBlobId*(const std::string&)>& DiffLbi4BnInOp) {
  CHECK(op.op_conf().has_sparse_cross_entropy_ms1_conf());
  if (DiffLbi4BnInOp("prediction") != nullptr) {
    OperatorConf sparse_cross_entropy_ms1_grad_op;
    sparse_cross_entropy_ms1_grad_op.set_name(op.op_name() + "_grad");
    SparseCrossEntropyMs1GradOpConf* sparse_cross_entropy_ms1_grad_op_conf =
        sparse_cross_entropy_ms1_grad_op.mutable_sparse_cross_entropy_ms1_grad_conf();
    sparse_cross_entropy_ms1_grad_op_conf->set_dy(GenLogicalBlobName(*DiffLbi4BnInOp("out")));
    sparse_cross_entropy_ms1_grad_op_conf->set_label(GenLogicalBlobName(op.BnInOp2Lbi("label")));
    sparse_cross_entropy_ms1_grad_op_conf->set_prediction(
        GenLogicalBlobName(op.BnInOp2Lbi("prediction")));
    sparse_cross_entropy_ms1_grad_op_conf->set_prediction_diff("prediction_diff");
    sparse_cross_entropy_ms1_grad_op_conf->set_depth(
        op.op_conf().sparse_cross_entropy_ms1_conf().depth());
    op_confs->push_back(sparse_cross_entropy_ms1_grad_op);
    DiffLbi4BnInOp("prediction")->set_op_name(sparse_cross_entropy_ms1_grad_op.name());
    DiffLbi4BnInOp("prediction")->set_blob_name("prediction_diff");
  }
}
...
REGISTER_OP_GRAD(OperatorConf::kSparseCrossEntropyMs1Conf, &GenerateMs1BackwardOpConf);
