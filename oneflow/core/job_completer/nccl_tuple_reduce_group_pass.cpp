...
void NcclTupleReduceGroupPass::Apply(const OpGraph& op_graph, Job* job) {
  ...
  op_graph.DfsTopoForEachNodeSortByDistanceToSink(
      ...
      [&](const OpNode* node) {
        if (!node->op().op_conf().has_nccl_tuple_reduce_conf()) { return; }
        pd2reduce_nodes[node->parallel_desc()].push_back(node);
        ...
        reduce_node2out_size[node] = out_size;
      });
  for (auto& pd7broadcast_nodes : pd2reduce_nodes) {
    std::reverse(pd7broadcast_nodes.second.begin(), pd7broadcast_nodes.second.end());
  }
  for (const auto& pd7reduce_nodes : pd2reduce_nodes) {
    OperatorConf group_op_conf{};
    group_op_conf.set_name("System-Boxing-NcclTupleReduce-" + NewUniqueId());
    NcclTupleReduceOpConf* conf = group_op_conf.mutable_nccl_tuple_reduce_conf();
    int64_t new_out_id = 0;
    for (const OpNode* node : pd7reduce_nodes.second) {
      HashMap<LogicalBlobId, std::string> old_lbi2new_lbn;
      const NcclTupleReduceOpConf& old_conf = node->op().op_conf().nccl_tuple_reduce_conf();
