...
void GenEagerInplaceIdentityBackwardOpConf(
    const Operator& op, std::vector<OperatorConf>* op_confs,
    const std::function<LogicalBlobId*(const std::string&)>& DiffLbi4BnInOp) {
  if (DiffLbi4BnInOp("in") != nullptr) {
    OperatorConf grad_op{};
    grad_op.set_name("System-AutoGrad-" + op.op_name());
    auto* eager_inplace_identity_conf = grad_op.mutable_eager_inplace_identity_conf();
    eager_inplace_identity_conf->set_in(GenLogicalBlobName(*DiffLbi4BnInOp("out")));
    eager_inplace_identity_conf->set_out("out");
    op_confs->push_back(grad_op);
    DiffLbi4BnInOp("in")->set_op_name(grad_op.name());
    DiffLbi4BnInOp("in")->set_blob_name(eager_inplace_identity_conf->out());
  }
}

REGISTER_OP_GRAD(OperatorConf::kEagerInplaceIdentityConf, &GenEagerInplaceIdentityBackwardOpConf);
