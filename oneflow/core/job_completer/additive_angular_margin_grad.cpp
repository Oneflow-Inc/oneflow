...
void GenerateBackwardOpConf(
  ...
  if (DiffLbi4BnInOp("in") != nullptr) {
    ...
    conf->set_dx("dx");
    op_confs->push_back(additive_angular_margin_grad_op);
    DiffLbi4BnInOp("in")->set_op_name(additive_angular_margin_grad_op.name());
...
REGISTER_OP_GRAD(OperatorConf::kAdditiveAngularMarginConf, &GenerateBackwardOpConf);
REGISTER_OP_GRAD(OperatorConf::kAdditiveAngularMarginMs1Conf, &GenerateMs1BackwardOpConf);
