...
void GenerateBackwardOpConf(
  ...
  if (DiffLbi4BnInOp("in") != nullptr) {
    ...
    conf->set_out("out");
    conf->set_constant_value(op.op_conf().pad_conf().constant_value());
    *conf->mutable_paddings() = GetPbRfFromPbMessage<int32_t>(op.op_conf().pad_conf(), "paddings");
...
void GenerateGradBackwardOpConf(
    const Operator& op, std::vector<OperatorConf>* op_confs,
    const std::function<LogicalBlobId*(const std::string&)>& DiffLbi4BnInOp) {
  ...
  if (DiffLbi4BnInOp("in") != nullptr) {
    ...
    *conf->mutable_paddings() =
        GetPbRfFromPbMessage<int32_t>(op.op_conf().pad_grad_conf(), "paddings");
    op_confs->push_back(op_conf);
    DiffLbi4BnInOp("in")->set_op_name(op_conf.name());
    DiffLbi4BnInOp("in")->set_blob_name(conf->out());
  }
...
REGISTER_OP_GRAD(OperatorConf::kPadGradConf, &GenerateGradBackwardOpConf);
