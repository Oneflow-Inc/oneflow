...
void GenerateInputVarOpConf(
  ...
  if (!conf.has_weight()) {
    OperatorConf weight_var_op = GenerateVariableOpConf(LogicalBlobDesc4BnInOp("weight"),
  ...
  if (conf.use_bias()) {
    if (!conf.has_bias()) {
      OperatorConf bias_var_op =
          GenerateVariableOpConf(LogicalBlobDesc4BnInOp("bias"), op.op_name() + "-bias", "bias");
