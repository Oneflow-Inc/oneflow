...
void GenerateInputVarOpConf(
    ...
    const std::function<const BlobDesc&(const std::string&)>& UnparalleledBlobDesc4BnInOp) {
  ...
  if (!conf.has_weight()) {
    OperatorConf weight_var_op = GenerateVariableOpConf(UnparalleledBlobDesc4BnInOp("weight"),
  ...
  if (conf.use_bias()) {
    if (!conf.has_bias()) {
      OperatorConf bias_var_op = GenerateVariableOpConf(UnparalleledBlobDesc4BnInOp("bias"),
