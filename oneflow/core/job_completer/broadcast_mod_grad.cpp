...
void GenerateBackwardOpConf(
  ...
  CHECK(op.op_conf().has_broadcast_mod_conf());
  if (DiffLbi4BnInOp("a") != nullptr) {
    OperatorConf broadcast_mod_a;
    broadcast_mod_a.set_name(op.op_name() + "_grad_a_mod");
    BroadcastModOpConf* broadcast_mod_a_op_conf = broadcast_mod_a.mutable_broadcast_mod_conf();
    broadcast_mod_a_op_conf->set_a(GenLogicalBlobName(*DiffLbi4BnInOp("out")));
    broadcast_mod_a_op_conf->set_b(GenLogicalBlobName(op.BnInOp2Lbi("b")));
    broadcast_mod_a_op_conf->set_out("out");
    op_confs->push_back(broadcast_mod_a);
    ...
    if (LogicalBlobDesc4BnInOp("out").shape() == LogicalBlobDesc4BnInOp("a").shape()) {
      DiffLbi4BnInOp("a")->set_op_name(broadcast_mod_a.name());
      ...
      reshape_like_a_op_conf->set_x(broadcast_mod_a.name() + "/out");
      ...
      reduce_sum_like_a_op_conf->set_x(broadcast_mod_a.name() + "/out");
  ...
  if (DiffLbi4BnInOp("b") != nullptr) {
    OperatorConf broadcast_mod_grad_op;
    broadcast_mod_grad_op.set_name(op.op_name() + "_grad");
    BroadcastModGradOpConf* broadcast_mod_grad_op_conf =
        broadcast_mod_grad_op.mutable_broadcast_mod_grad_conf();
    broadcast_mod_grad_op_conf->set_b(GenLogicalBlobName(op.BnInOp2Lbi("b")));
    broadcast_mod_grad_op_conf->set_y(GenLogicalBlobName(op.BnInOp2Lbi("out")));
    broadcast_mod_grad_op_conf->set_dy(GenLogicalBlobName(*DiffLbi4BnInOp("out")));
    broadcast_mod_grad_op_conf->set_db("db");
    op_confs->push_back(broadcast_mod_grad_op);
    DiffLbi4BnInOp("b")->set_op_name(broadcast_mod_grad_op.name());
...
REGISTER_OP_GRAD(OperatorConf::kBroadcastModConf, &GenerateBackwardOpConf);
