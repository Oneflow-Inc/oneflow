...
void GenerateBackwardOpConf(
  ...
  const user_op::GradRegistrationVal* val = LookUpInGradRegistry(user_conf.op_type_name());
  CHECK(val != nullptr) << " Cannot find op_type: " << user_conf.op_type_name()
    << " 's grad func in GradRegistration!";
  UserOpWrapper user_op(op.op_conf(), LogicalBlobDesc4BnInOp, DiffLbi4BnInOp);
  auto AddOp = [&] (const UserOpConfWrapper& wrapper) {
  ...
  for(const std::string& ibn : op.input_bns()) {
    ...
    if (lbi != nullptr) {
      CHECK(lbi.has_op_name() && lbi.has_blob_name()) << " user_op: " << op.op_name() 
