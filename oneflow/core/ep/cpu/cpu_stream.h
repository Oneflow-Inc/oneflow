...
class CpuStream : public Stream {
 public:
  ...
  void SetParallelNumberThreads(size_t number_threads) {
  #if OF_CPU_THREADING_RUNTIME == OF_RUNTIME_OMP
    // Affects omp_get_max_threads() Get the logical core book
    omp_set_num_threads(number_threads);
  #elif OF_CPU_THREADING_RUNTIME == OF_RUNTIME_TBB
    tbb::global_control global_thread_limit(tbb::global_control::max_allowed_parallelism, number_threads);
  #endif
  }

  template<typename F>
  void Parallel(int64_t begin, int64_t end, const F& func, size_t grain_size, size_t number_threads) {
    if (begin >= end) { return; }
  ...
  #elif OF_CPU_THREADING_RUNTIME == OF_RUNTIME_TBB
    SetParallelNumberThreads(number_threads);
    size_t nthr_chunk_size = divup((end - begin), number_threads);
