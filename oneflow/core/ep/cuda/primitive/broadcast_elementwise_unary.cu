...
<<<<<<< HEAD
  ...
  for (IndexType idx = blockIdx.x * blockDim.x + threadIdx.x; idx < params.count;
...
#pragma unroll
      ...
      void DispatchIndexType(CudaStream * stream, size_t num_dims, const int64_t* src_dims,
        ...
        if (count < GetMaxVal<int32_t>() / 2) {
          LaunchKernel<op, Src, Dst, max_dims, pack_size, int32_t>(
              stream, num_dims, src_dims, src_strides, src, dst_dims, dst_strides, dst,
              continuous_output, attr0, attr1, count);
        } else {
      ...
      void DispatchPackSize(CudaStream * stream, size_t pack_size, size_t num_dims,
        ...
        void (*func)(CudaStream* /*stream*/, size_t /*num_dims*/, const int64_t* /*src_dims*/,
                     const int64_t* /*src_strides*/, const Src* /*src*/,
                     const int64_t* /*dst_dims*/, const int64_t* /*dst_strides*/, Dst* /*dst*/,
        ...
        func(stream, num_dims, src_dims, src_strides, src, dst_dims, dst_strides, dst,
             continuous_output, attr0, attr1);
      }

      template<UnaryOp op, typename Src, typename Dst>
      void DispatchNumDims(CudaStream * stream, size_t pack_size, size_t num_dims,
                           const int64_t* src_dims, const int64_t* src_strides, const Src* src,
                           const int64_t* dst_dims, const int64_t* dst_strides, Dst* dst,
                           bool continuous_output, Scalar attr0, Scalar attr1) {
        void (*func)(CudaStream* /*stream*/, size_t /*pack_size*/, size_t /*num_dims*/,
                     const int64_t* /*src_dims*/, const int64_t* /*src_strides*/,
                     const Src* /*src*/, const int64_t* /*dst_dims*/,
                     const int64_t* /*dst_strides*/, Dst* /*dst*/, bool /*continuous_output*/,
      ...
      template<UnaryOp op, typename Src, typename Dst>
      void LaunchWithSimplified(
          CudaStream * stream, size_t simplified_num_dims, int64_t * simplified_src_dims,
          int64_t * simplified_src_strides, const Src* src, int64_t* simplified_dst_dims,
      ...
      __global__ void LaunchFillKernel(UnaryFunctor<DeviceType::kCUDA, op, Src, Dst> functor,
                                       Dst * dst, const Src* src, size_t pack_count, size_t count,
                                       size_t tail_count, Dst* tail_dst) {
...
#pragma unroll
      ...
      typename std::enable_if<(pack != 0), void>::type LaunchPackFill(
          CudaStream * stream, Dst * dst, const Src* src, size_t count, Scalar attr0,
          Scalar attr1) {
        ...
        if (tail_count > 0) {
          LaunchFillKernel<op, Src, Dst, pack, true>
              <<<BlocksNum4ThreadsNum(pack_count), kCudaThreadsNumPerBlock, 0,
                 stream->cuda_stream()>>>(functor, dst, src, pack_count, count, tail_count,
                                          dst + tail_offset);
        } else {
      ...
      class BroadcastElementwiseUnaryImpl : public BroadcastElementwiseUnary {
       public:
        ...
        void Launch(Stream* stream, size_t num_src_dims, const int64_t* src_dims, const void* src,
          ...
          Launch(stream, num_src_dims, src_dims, src_strides, src, num_dst_dims, dst_dims,
                 dst_strides, dst);
        }
        ...
        void Launch(Stream* stream, size_t num_src_dims, const int64_t* src_dims,
          ...
          SimplifyBroadcastDims<kMaxNumDims>(num_src_dims, src_dims, src_strides, num_dst_dims,
                                             dst_dims, dst_strides, &simplified_num_dims,
                                             simplified_src_dims, simplified_src_strides,
          ...
          CheckInplace(simplified_num_dims, simplified_src_strides, src, simplified_dst_strides,
                       dst);
      ...
      class BroadcastElementwiseUnaryFactoryImpl : public BroadcastElementwiseUnaryFactory {
       public:
        ...
        std::unique_ptr<BroadcastElementwiseUnary> New(UnaryOp op, DataType src_type,
                                                       DataType dst_type,
