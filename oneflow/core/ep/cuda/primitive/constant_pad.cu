...
void DispatchIndexType(Stream* stream, void* dst, const int64_t* dst_dims, const void* src,
  ...
  if (elem_cnt < GetMaxVal<uint32_t>()) {
    LaunchKernel<num_dims, uint32_t, StorageType>(stream, dst, dst_dims, src, src_dims,
                                                  padding_before, padding_after, packed_pad_val,
                                                  elem_cnt);
  } else {
    LaunchKernel<num_dims, uint64_t, StorageType>(stream, dst, dst_dims, src, src_dims,
                                                  padding_before, padding_after, packed_pad_val,
                                                  elem_cnt);
  }
}
