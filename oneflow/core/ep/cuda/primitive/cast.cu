...
struct CastFunctor<
    To, half,
    typename std::enable_if<!(std::is_same<To, half>::value || std::is_same<To, cuComplex>::value
...
struct CastFunctor<
    To, nv_bfloat16,
    typename std::enable_if<!(std::is_same<To, nv_bfloat16>::value || std::is_same<To, half>::value
