...
#ifndef ONEFLOW_CORE_PRIMITIVE_COMMON_COPY_ND_WITH_STRIDE_H_
#define ONEFLOW_CORE_PRIMITIVE_COMMON_COPY_ND_WITH_STRIDE_H_
...
struct CopyNdWithStrideKernelParams {
  NdIndexOffsetWithStrideHelper<IndexType, num_dims> src_index_helper;
  NdIndexOffsetWithStrideHelper<IndexType, num_dims> dst_index_helper;
  NdIndexOffsetWithStrideHelper<IndexType, num_dims> copy_index_helper;
  IndexType dst_pos[num_dims];
  IndexType src_pos[num_dims];
  IndexType src_strides[num_dims];
  IndexType target_strides[num_dims];
  IndexType count{};
  const void* src{};
  void* dst{};
};
...
void LaunchKernel(Stream* stream, void* dst, const int64_t* dst_dims, const int64_t* dst_pos,
                  ...
                  const int64_t* extent, const int64_t* stride, size_t count) {

  CopyNdWithStrideKernelParams<num_dims, IndexType> params;
  IndexType element_count = 1;
  for(IndexType i=num_dims-1; i!=-1; --i){
    params.dst_pos[i] = dst_pos[i];
    params.src_pos[i] = src_pos[i];
    params.src_strides[i] = stride[i];
    params.target_strides[i] = element_count;
    element_count *= src_dims[i];
  }
  params.dst_index_helper = NdIndexOffsetWithStrideHelper<IndexType, num_dims>(params.target_strides);
  params.src_index_helper = NdIndexOffsetWithStrideHelper<IndexType, num_dims>(params.src_strides);
  params.copy_index_helper = NdIndexOffsetWithStrideHelper<IndexType, num_dims>(extent);
...
void DispatchMovementSize(Stream* stream, size_t movement_size, void* dst, const int64_t* dst_dims,
                          const int64_t* dst_pos, const void* src, const int64_t* src_dims,
                          const int64_t* src_pos, const int64_t* extent, const int64_t* stride) {
  void (*func)(Stream* /*stream*/, void* /*dst*/, const int64_t* /*dst_dims*/,
               const int64_t* /*dst_pos*/, const void* /*src*/, const int64_t* /*src_dims*/,
               const int64_t* /*src_pos*/, const int64_t* /*extent*/, const int64_t* /*stride*/) = nullptr;
  if (movement_size == 1) {
  ...
  func(stream, dst, dst_dims, dst_pos, src, src_dims, src_pos, extent, stride);
}
...
void LaunchWithSimplified(Stream* stream, size_t movement_size, size_t num_dims, void* dst,
                          ...
                          const int64_t* src_dims, const int64_t* src_pos, const int64_t* extent, const int64_t* stride) {
  ...
  func(stream, movement_size, dst, dst_dims, dst_pos, src, src_dims, src_pos, extent, stride);
}
