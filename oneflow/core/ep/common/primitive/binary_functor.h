...
struct BinaryFunctor<device, BinaryOp::kErfBackwardWithDyX, Src, Dst> {
  ...
  OF_DEVICE_FUNC Dst operator()(Src dy, Src x) const {
    return dy * static_cast<Src>(2) * rsqrt_functor(M_PI) * expf(-x * x);
  }
