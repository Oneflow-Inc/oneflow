...
struct UnaryFunctor<device, UnaryOp::kElu, Dst, Src> {
  ...
  OF_DEVICE_FUNC Dst operator()(Src src) const {
    return (src > static_cast<Src>(0.0))
               ? static_cast<Dst>(src)
               : static_cast<Dst>(alpha * (exp(src) - static_cast<Src>(1)));
  }
  const Src alpha;
};
...
struct UnaryFunctor<device, UnaryOp::kCelu, Dst, Src> {
  ...
  OF_DEVICE_FUNC Dst operator()(Src src) const {
    return (src > static_cast<Src>(0.0))
               ? static_cast<Dst>(src)
               : static_cast<Dst>(alpha * (exp(src * inv_alpha) - static_cast<Src>(1)));
  }
  const Src alpha;
  const Src inv_alpha;
};
...
struct UnaryFunctor<device, UnaryOp::kSilu, Dst, Src> {
  ...
  OF_DEVICE_FUNC Dst operator()(Src src) const { return (src / (static_cast<Src>(1) + exp(-src))); }
};
...
struct UnaryFunctor<device, UnaryOp::kSelu, Dst, Src> {
  ...
  OF_DEVICE_FUNC Dst operator()(Src src) const {
    return (src > static_cast<Src>(0.0))
               ? static_cast<Dst>(src * scale)
               : static_cast<Dst>(scale * alpha * (exp(src) - static_cast<Src>(1)));
  }
...
struct UnaryFunctor<device, UnaryOp::kSoftSign, Dst, Src> {
  ...
  OF_DEVICE_FUNC Dst operator()(Src src) const { return src / (static_cast<Src>(1) + abs(src)); }
};
...
struct UnaryFunctor<device, UnaryOp::kSoftPlus, Dst, Src> {
  ...
  OF_DEVICE_FUNC Dst operator()(Src src) const {
    return (src * beta) > threshold ? src : log(static_cast<Src>(1.0) + exp(src * beta)) / beta;
  }
...
struct UnaryFunctor<device, UnaryOp::kThreshold, Dst, Src> {
  ...
  OF_DEVICE_FUNC Dst operator()(Src src) const { return (src > threshold) ? src : value; }
  const Src threshold;
  const Src value;
};
