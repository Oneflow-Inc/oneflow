...
Maybe<void> Expand::Apply(const ExpandCaptureState* ctx, const TensorTuple& out_grads,
  ...
  attrs.reset();
  attrs.SetAttr<std::vector<int32_t>>("logical_out_shape", ctx->logical_out_shape);
  attrs.SetAttr<std::vector<int32_t>>("logical_expand_shape", ctx->logical_expand_shape);
  in_grads->at(0) = JUST(
