...
class ConsistentToConsistentGradFunction : public OpExprGradFunction<ConsistentToConsistentState> {
 public:
  ...
  Maybe<void> Apply(const ConsistentToConsistentState* state, const TensorTuple& out_grads,
    ...
    const auto& grad_grad_sbp_list = JUST(GetSbpList(state->nd_sbp));
    in_grads->at(0) = JUST(one::functional::ToConsistent(out_grad, state->parallel_desc,
