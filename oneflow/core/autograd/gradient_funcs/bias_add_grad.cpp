...
class BiasAdd : public OpExprGradFunction {
 public:
  Maybe<void> Init(const OpExpr& op) override {
    ...
    backward_input_op_ = JUST(OpBuilder("identity", op_name_ + "_input" + FakeGradientOpSuffix)
                                  .Input("in")
                                  .Output("out")
                                  .Build());
  ...
  Maybe<void> Apply(const OpExprInterpState* ctx, const TensorTuple& out_grads,
    ...
    if (reduce_axes_vec_.size() != num_axes - 1 || !backward_bias_op_.get()) {
      reduce_axes_vec_.clear();
      for (int i = 0; i < num_axes; ++i) {
        if (i != bias_add_axis_) { reduce_axes_vec_.push_back(i); }
      }
      backward_bias_op_ = JUST(OpBuilder("reduce_sum", op_name_ + "_bias" + FakeGradientOpSuffix)
                                   .Input("input_tensor")
                                   .Output("output_tensor")
                                   .Attr("axis", reduce_axes_vec_)
                                   .Attr("keepdims", false)
                                   .Build());
    }
