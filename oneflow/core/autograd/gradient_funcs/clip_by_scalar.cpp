...
class ClipByScalar : public OpExprGradFunction<ClipByScalarCaptureState> {
 public:
  ...
  Maybe<void> Capture(ClipByScalarCaptureState* state, const TensorTuple& inputs,
    ...
    auto* interp_ctx = dynamic_cast<const ClipByScalarOpInterpCtx*>(ctx);
  ...
  Maybe<void> Apply(const ClipByScalarCaptureState* state, const TensorTuple& out_grads,
                    TensorTuple* in_grads) const override {
    CHECK_EQ_OR_RETURN(out_grads.size(), 1);
    in_grads->resize(1);
    if (state->requires_grad) {
      const auto& x = state->SavedTensors().at(0);
      in_grads->at(0) = JUST(functional::ClampGrad(out_grads.at(0), x, state->min, state->max));
    }
    return Maybe<void>::Ok();
  }
};
