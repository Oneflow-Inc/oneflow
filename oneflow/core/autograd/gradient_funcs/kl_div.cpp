...
struct KLDivLossCaptureState : public AutoGradCaptureState {
  bool requires_input_grad = false;
  bool requires_target_grad = false;
...
Maybe<void> KLDivLoss::Capture(KLDivLossCaptureState* ctx, const TensorTuple& inputs,
  ...
  ctx->requires_input_grad = inputs.at(0)->requires_grad();
  ctx->requires_target_grad = inputs.at(1)->requires_grad();
  if (!(ctx->requires_input_grad || ctx->requires_target_grad)) { return Maybe<void>::Ok(); }
...
Maybe<void> KLDivLoss::Apply(const KLDivLossCaptureState* ctx, const TensorTuple& out_grads,
  ...
  in_grads->resize(2);
  if (ctx->requires_input_grad) {
    in_grads->at(0) = JUST(functional::KLDivLossInputGrad(dy, input, target, ctx->log_target));
  }
  if (ctx->requires_target_grad) {
    in_grads->at(1) = JUST(functional::KLDivLossTargetGrad(dy, input, target, ctx->log_target));
  }
