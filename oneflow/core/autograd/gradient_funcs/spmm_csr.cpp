...
struct SpmmCsrCaptureState : public AutoGradCaptureState {
...
class SpmmCsr : public OpExprGradFunction<SpmmCsrCaptureState> {
 public:
  Maybe<void> Init(const OpExpr& op) override;
  Maybe<void> Capture(SpmmCsrCaptureState* ctx, const TensorTuple& inputs,
                      const TensorTuple& outputs, const AttrMap& attrs) const override;
  Maybe<void> Apply(const SpmmCsrCaptureState* ctx, const TensorTuple& out_grads,
...
Maybe<void> SpmmCsr::Init(const OpExpr& op) {
...
Maybe<void> SpmmCsr::Capture(SpmmCsrCaptureState* ctx, const TensorTuple& inputs,
...
Maybe<void> SpmmCsr::Apply(const SpmmCsrCaptureState* ctx, const TensorTuple& out_grads,
  ...
  if (ctx->requires_grad_b) {
    const auto& a_csr_row = JUST(VectorAt(ctx->SavedTensors(), ctx->row_index));
    const auto& a_csr_col = JUST(VectorAt(ctx->SavedTensors(), ctx->col_index));
    const auto& a_csr_val = JUST(VectorAt(ctx->SavedTensors(), ctx->val_index));
...
REGISTER_OP_EXPR_GRAD_FUNCTION("spmm_csr", SpmmCsr);
}  // namespace one
}  // namespace oneflow
