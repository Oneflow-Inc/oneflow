...
class AddN : public OpExprGradFunction<AddNCaptureState> {
 public:
  ...
  Maybe<void> Capture(AddNCaptureState* state, const TensorTuple& inputs, const TensorTuple& outputs,
  ...
  Maybe<void> Apply(const AddNCaptureState* state, const TensorTuple& out_grads,
                    TensorTuple* in_grads) const override {
    CHECK_EQ_OR_RETURN(out_grads.size(), 1);
    in_grads->resize(state->input_num);
    for (int i = 0; i < state->input_num; ++i) {
      if (state->requires_grad.at(i)) { in_grads->at(i) = out_grads.at(0); }
    }
    return Maybe<void>::Ok();
  }
};
