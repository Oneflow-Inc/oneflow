...
class BinaryMathOp : public OpExprGradFunction<BinaryMathCaptureState> {
  ...
  Maybe<void> Capture(BinaryMathCaptureState* state, const TensorTuple& inputs,
  ...
  Maybe<void> Apply(const BinaryMathCaptureState* state, const TensorTuple& out_grads,
    ...
    if (state->x_requires_grad) { in_grads->at(0) = JUST(BwXFunc(x, y, out_grads.at(0))); }
    if (state->y_requires_grad) { in_grads->at(1) = JUST(BwYFunc(x, y, out_grads.at(0))); }
    return Maybe<void>::Ok();
  }
};
