...
Maybe<void> SpmmCoo::Capture(SpmmCooCaptureState* ctx,
...
Maybe<void> SpmmCoo::Apply(const SpmmCooCaptureState* ctx,
  ...
  if (ctx->requires_grad_b) {
    ...
    JUST(VectorAt(*in_grads, 3)) =
        JUST(functional::SpmmCoo(a_coo_row, a_coo_col, a_coo_val,
                                ctx->a_num_rows, ctx->a_num_cols,
