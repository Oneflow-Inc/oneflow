...
Maybe<void> Where::Capture(WhereCaptureState* state, const TensorTuple& inputs,
  ...
  state->SaveTensorForBackward(inputs.at(0));  // condition
  state->SaveTensorForBackward(inputs.at(1));  // x
  state->SaveTensorForBackward(inputs.at(2));  // y
  return Maybe<void>::Ok();
}

Maybe<void> Where::Apply(const WhereCaptureState* state, const TensorTuple& out_grads,
                         TensorTuple* in_grads) const {
  if ((!state->requires_grad_x) && (!state->requires_grad_y)) { return Maybe<void>::Ok(); }
  CHECK_EQ_OR_RETURN(out_grads.size(), 1);
  const std::shared_ptr<oneflow::one::Tensor>& condition = state->SavedTensors().at(0);
  const std::shared_ptr<oneflow::one::Tensor>& x = state->SavedTensors().at(1);
  const std::shared_ptr<oneflow::one::Tensor>& y = state->SavedTensors().at(2);
...
class WhereScalarX : public WhereScalar {
 public:
  Maybe<void> Apply(const WhereScalarCaptureState* state, const TensorTuple& out_grads,
                    TensorTuple* in_grads) const override {
    if (!state->requires_grad) { return Maybe<void>::Ok(); }
    CHECK_EQ_OR_RETURN(out_grads.size(), 1);
    const std::shared_ptr<oneflow::one::Tensor>& condition = state->SavedTensors().at(0);
