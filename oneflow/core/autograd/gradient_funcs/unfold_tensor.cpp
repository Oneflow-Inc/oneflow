...
Maybe<void> UnfoldTensor::Init(const OpExpr& op) {
    ...
    int32_t size;
    int32_t step;
    grad_op_ = JUST(op_expr_helper::UnfoldTensorGradOp(dimension, size, step,
...
Maybe<void> UnfoldTensor::Apply(const UnfoldTensorCaptureState* ctx, const TensorTuple& out_grads,
    ...
    in_grads->at(0) = JUST(functional::UnfoldTensorGrad(out_grads.at(0), in, ctx->dimension, ctx->size, ctx->step));
    return Maybe<void>::Ok();                         
}
