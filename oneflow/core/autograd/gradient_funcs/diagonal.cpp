...
class Diagonal : public OpExprGradFunction<DiagonalInterpState> {
 public:
  ...
  Maybe<void> Capture(DiagonalInterpState* ctx, const TensorTuple& inputs, const TensorTuple& outputs,
...
Maybe<void> Diagonal::Apply(const DiagonalInterpState* ctx, const TensorTuple& out_grads,
  ...
  if (ctx->requires_grad) {
    ...
    in_grads->at(0) = JUST(functional::DiagGrad(out_grads.at(0), x, ctx->offset));
  }
  return Maybe<void>::Ok();
}
