...
void GenerateBackwardOpConf(
  ...
  if (DiffLbi4BnInOp("weight") != nullptr) {
    OperatorConf matmul_op;
    matmul_op.set_name(op.op_name() + "_weight_grad");
    MatmulOpConf* matmul_op_conf = matmul_op.mutable_matmul_conf();
    ...
    op_confs->push_back(matmul_op);
    DiffLbi4BnInOp("weight")->set_op_name(matmul_op.name());
