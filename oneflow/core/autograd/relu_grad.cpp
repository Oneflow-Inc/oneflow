...
void GenerateBackwardOpConf(const Operator& op, std::vector<OperatorConf>* op_confs,
  ...
  if (DiffLbi4BnInOp("x") != nullptr) {
    ...
    relu_grad_op_conf->set_y(GenLogicalBlobName(op.BnInOp2Lbi("y")));
    relu_grad_op_conf->set_dy(GenLogicalBlobName(*DiffLbi4BnInOp("y")));
    relu_grad_op_conf->set_dx("dx");
    op_confs->push_back(relu_grad_op);
    DiffLbi4BnInOp("x")->set_op_name(relu_grad_op.name());
