...
// #endif
        /*
        auto strm_indx_getter = StreamIndexGetterRegistryManager::Get().StreamIndexGetterFuncs()[{parallel_desc_->device_type(), comp_task_node->GetTaskType()}];
        comp_task_node->set_thrd_id(strm_indx_getter(dev_phy_id));
        */
