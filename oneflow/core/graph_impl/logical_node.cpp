  ...
  for (int64_t machine_id : parallel_desc_->sorted_machine_ids()) {
    for (int64_t dev_phy_id : parallel_desc_->sorted_dev_phy_ids(machine_id)) {
      ...
      if (parallel_desc_->device_type() == DeviceType::kGPU) {
        auto strm_indx_getter = StreamIndexGetterRegistryManager::Get().StreamIndexGetterFuncs();
        std::pair<DeviceType, TaskType> dev_task_type(parallel_desc_->device_type(),
                                                      comp_task_node->GetTaskType());
        if (strm_indx_getter.find(dev_task_type) == strm_indx_getter.end()) { UNIMPLEMENTED(); }
        comp_task_node->set_thrd_id(strm_indx_getter[dev_task_type](dev_phy_id));
