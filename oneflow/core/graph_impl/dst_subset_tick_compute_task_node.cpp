...
void DstSubsetTickCompTaskNode::InferProducedDataRegstTimeShape() {
...
REGISTER_COMPUTE_TASK_NODE_STREAM_INDEX_GETTER(DeviceType::kCPU, TaskType::kDstSubsetTick)            \
  .SetStreamIndexGetterFn([](DeviceId device_id) -> uint32_t {                    \
    ...
    return cpu_stream_index_generator->GenerateIndependentTaskStreamIndex(TaskType::kDstSubsetTick);  \
  });
