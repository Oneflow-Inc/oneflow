...
class SbpCollector {
 public:
  ...
  void CollectUniverse(cfg::SbpSignature& sbp_);
  // Collect all the possible Sbp Parallel from a SbpNode
  void CollectUniverse(SbpNode<cfg::SbpSignature>* sbp_node);
  // Collect all the possible Sbp Parallel from a SbpGraph
  void CollectUniverse(SbpGraph<cfg::SbpSignature>& sbp_graph);
  // Initialize sbp proxy with given parallel candidates of a blob
  SbpNode<cfg::SbpSignature>* InitializePorxy(
