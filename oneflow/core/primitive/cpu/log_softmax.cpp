...
template<typename T>
void LogSoftmaxCpu(size_t rows, size_t cols, const T* x, T* y) {
  for (size_t i = 0; i < rows; ++i) {
    ...
    for (size_t j = 0; j < cols; ++j) { row_y[j] -= std::log(row_sum); }
  }
}
...
class LogSoftmaxImpl : public LogSoftmax {
 public:
  ...
  void Launch(StreamContext* stream_ctx, size_t rows, size_t cols, const void* x,
              void* y) override {
    LogSoftmaxCpu(rows, cols, reinterpret_cast<const T*>(x), reinterpret_cast<T*>(y));
  }
};
