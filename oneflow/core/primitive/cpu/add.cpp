...
template<typename T, dnnl::memory::data_type onednn_type>
class AddImpl : public Add {
 public:
  ...
  void Launch(StreamContext* stream_ctx, const void* const* srcs, size_t arity, void* dst,
    ...
    dnnl::engine* onednn_engine = CHECK_NOTNULL(dynamic_cast<CpuStreamContext*>(stream_ctx))->onednn_engine();
    dnnl::stream* onednn_stream = CHECK_NOTNULL(dynamic_cast<CpuStreamContext*>(stream_ctx))->onednn_stream();
    ...
    {
      auto md = dnnl::memory::desc(src_dims, onednn_type, dnnl::memory::format_tag::x);
...
std::unique_ptr<Add> NewAdd() {
  return std::unique_ptr<Add>(new AddImpl<T, onednn_type>());
}
...
#define MAKE_NEW_ADD_ENTRY(type_cpp, type_proto, type_onednn) {type_proto, NewAdd<type_cpp, type_onednn>},
