...
class ElementwiseUnary : public Primitive {
 public:
  ...
  virtual void Launch(StreamContext* stream_ctx, const void* src, void* dst, size_t count) = 0;
};
...
class ElementwiseUnaryFactory : public Factory<ElementwiseUnary> {
 public:
  ...
  virtual std::unique_ptr<ElementwiseUnary> New(UnaryOp op, DataType in_dtype, DataType out_dtype) = 0;
};
...
#endif  // ONEFLOW_CORE_PRIMITIVE_ELEMENTWISE_UNARY_H_
