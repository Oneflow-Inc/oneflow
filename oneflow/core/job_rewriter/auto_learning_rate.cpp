...
Maybe<void> AutoLearningRate::Apply(const OpGraph &op_graph, Job *job) const {
  ...
  auto AddScheduleOp = [&](const std::string &op_name,
                           const float learning_rate) -> std::string {
    const class oneflow::OpNode *op_node = op_graph.OpNode4OpName(
        GenLogicalBlobId(train_conf.train_step_lbn()).op_name());
    CHECK_OR_RETURN(op_node != nullptr)
        << "op node not fould in op graph, op name: " << op_name;
    ...
    if (model_update_conf.has_warmup_conf() ||
      ...
      schedule_op_conf.set_scope_symbol_id(
  ...
  if (!train_conf.has_secondary_lr_lbn()) {
    if (train_conf.has_secondary_lr()) {
      const std::string lbn =
          AddScheduleOp("System-Train-SecondaryLearningRate-Scheduler",
                        train_conf.secondary_lr());
