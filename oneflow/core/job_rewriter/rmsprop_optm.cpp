...
OperatorConf GenerateRmspropHelperVariableOpConf(const VariableOp &op,
...
void GenerateOptimizerOpConf(const VariableOp &op,
  ...
  OperatorConf mean_square_var(
      GenerateRmspropHelperVariableOpConf(op, "mean_square", 0.f));
  ...
  *(mdupdt_op_conf->mutable_user_conf()) =
  ...
  if (GlobalJobDesc()
    ...
    OperatorConf mean_gradient_var(
        GenerateRmspropHelperVariableOpConf(op, "mean_gradient", 0.f));
    mdupdt_op_conf->set_mean_gradient(mean_gradient_var.name() + "/out");
    job_builder->AddOps(parallel_conf,
                        {mean_square_var, mean_gradient_var, mdupdt_op});
  } else {
...
REGISTER_OPTIMIZER(NormalModelUpdateOpUserConf::kRmspropConf,
