  ...
  Maybe<void> Apply(const OpGraph &op_graph,
    ...
    op_graph.ForEachNode([&](OpNode *op_node) {
      if (op_node->op().op_conf().has_variable_conf()) {
        ...
        VariableOpConf *variable_conf =
            variable_op_conf.mutable_variable_conf();
        if (!variable_conf->has_data_type()) {
          variable_conf->set_data_type(
              job_builder->job().job_conf().default_data_type());
        }
        if (!variable_conf->has_initializer() &&
        ...
        auto *var_op_name2random =
            Global<JobSetCompileCtx>::Get()->GetVarOpName2randomSeed();
        ...
        const auto &pair =
            var_op_name2random->insert({var_op_name, random_seed});
        if (variable_conf->has_random_seed()) {
          CHECK_EQ(variable_conf->random_seed(), pair.first->second);
        } else {
          variable_conf->set_random_seed(pair.first->second);
        }
        job_builder->AddOrMutOpsOnlyOnce(
            op_node->parallel_desc().parallel_conf(), {variable_op_conf});
      }
    });
