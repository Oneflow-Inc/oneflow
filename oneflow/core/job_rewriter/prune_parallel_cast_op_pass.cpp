...
public:
  ...
  bool IsEnabled() const override {
    return GlobalJobDesc().prune_parallel_cast_ops();
  }
...
Maybe<void> PruneParallelCastOpsPass::Apply(const OpGraph &op_graph,
  ...
  op_graph.ForEachNode([&](const OpNode *op_node) {
    ...
    if (!op_conf.has_parallel_cast_conf()) {
      return;
    }
    ...
    const LogicalBlobId &parallel_cast_out_lbi =
        op_node->op().BnInOp2Lbi("out");
    const SbpParallel &parallel_cast_sbp_parallel =
        op_node->SbpParallel4Lbi(parallel_cast_in_lbi);
    const SbpParallel &producer_sbp_parallel =
        producer->SbpParallel4Lbi(parallel_cast_in_lbi);
    if (op_node->parallel_desc() != producer->parallel_desc()) {
      return;
    }
    if (parallel_cast_sbp_parallel != producer_sbp_parallel &&
        op_node->out_edges().size() > 1) {
      return;
    }
    for (const OpEdge *out_edge : op_node->out_edges()) {
      const OpNode *consumer = out_edge->dst_node();
      if (consumer->op().op_conf().has_parallel_cast_conf()) {
        return;
      }
      if (consumer->parallel_desc() != op_node->parallel_desc()) {
        return;
      }
      if (consumer->SbpParallel4Lbi(parallel_cast_out_lbi) !=
          parallel_cast_sbp_parallel) {
        return;
      }
    }
      ...
      for (const std::string &ibn : consumer->op().input_bns()) {
        if (consumer->op().BnInOp2Lbi(ibn) == parallel_cast_out_lbi) {
          ReplaceInputLbnInOpCustomizedConf(
              conf, ibn, GenLogicalBlobName(parallel_cast_out_lbi),
