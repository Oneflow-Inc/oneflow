...
Maybe<void>
IndexedSlicesOptimizerRewritePass::Apply(const OpGraph &op_graph,
  ...
  op_graph.ForEachNode([&](const OpNode *src_node) {
    ...
    if (src_node->out_edges().size() != 1) {
      return;
    }
    std::string indices_lbn;
    std::string values_lbn;
    std::string model_op_name;
    std::function<void(OperatorConf * /*new_optimizer_op_conf*/,
                       const std::string & /*indices*/,
                       const std::string & /*values*/)>
        BuildOptimizer;
    if (!src_op_conf.has_user_conf()) {
      return;
    }
    const user_op::UserOpConfWrapper src_op(src_op_conf);
    if (src_op.op_type_name() == "unsorted_segment_sum" &&
    ...
    if (dst_op_conf.has_naive_model_update_conf()) {
      const NaiveModelUpdateOpConf &old_optimizer_conf =
          dst_op_conf.naive_model_update_conf();
      const LogicalBlobId &model_lbi = dst_node->op().BnInOp2Lbi("model");
      model_op_name = model_lbi.op_name();
      BuildOptimizer = [&](OperatorConf *new_optimizer_op_conf,
                           const std::string &indices,
                           const std::string &values) {
        IndexedSlicesNaiveModelUpdateOpConf *new_optimizer_conf =
            new_optimizer_op_conf
                ->mutable_indexed_slices_naive_model_update_conf();
        ...
        new_optimizer_conf->set_learning_rate(
            old_optimizer_conf.learning_rate());
      };
      ...
      BuildOptimizer = [&](OperatorConf *new_optimizer_op_conf,
        ...
        IndexedSlicesMomentumModelUpdateOpConf *new_optimizer_conf =
            new_optimizer_op_conf
                ->mutable_indexed_slices_momentum_model_update_conf();
        ...
        new_optimizer_conf->set_learning_rate(
            old_optimizer_conf.learning_rate());
        new_optimizer_conf->set_beta(
            old_optimizer_conf.user_conf().momentum_conf().beta());
      };
      ...
      BuildOptimizer = [&](OperatorConf *new_optimizer_op_conf,
        ...
        IndexedSlicesLazyAdamModelUpdateOpConf *new_optimizer_conf =
            new_optimizer_op_conf
                ->mutable_indexed_slices_lazy_adam_model_update_conf();
        ...
        new_optimizer_conf->set_learning_rate(
            old_optimizer_conf.learning_rate());
        new_optimizer_conf->set_beta1(
            old_optimizer_conf.user_conf().lazy_adam_conf().beta1());
        new_optimizer_conf->set_beta2(
            old_optimizer_conf.user_conf().lazy_adam_conf().beta2());
        new_optimizer_conf->set_epsilon(
            old_optimizer_conf.user_conf().lazy_adam_conf().epsilon());
      };
    } else {
      return;
    }
    ...
    if (include_op_name_set.find(model_op_name) == include_op_name_set.end()) {
      return;
    }
    for (const OpNode *node : op_nodes_to_remove) {
    ...
    for (const OpNode *node : op_nodes_apply_to_diff) {
      ...
      if (new_conf.has_user_conf() &&
          new_conf.user_conf().op_type_name() == "scalar_mul") {
        ReplaceInputLbnInOpCustomizedConf(
            new_conf.mutable_user_conf(), "in_0",
            GenLogicalBlobName(node->op().BnInOp2Lbi("in_0")), values_lbn);
    ...
    new_optimizer_op_conf.set_name("System-Optimizer-IndexedSlices-" +
                                   model_op_name);
...
REGISTER_FUNCTION_PASS("IndexedSlicesOptimizerRewritePass",
                       IndexedSlicesOptimizerRewritePass);
