...
public:
  ...
  bool IsEnabled() const override {
    return GlobalJobDesc().IsTrain() &&
           GlobalJobDesc().prune_cast_to_static_shape_ops();
  }
...
Maybe<void>
PruneCastToStaticShapeOpsPass::Apply(const OpGraph &op_graph,
  ...
  op_graph.ForEachNode([&](const OpNode *op_node) {
    ...
    if (!op_conf.has_cast_to_static_shape_conf()) {
      return;
    }
    ...
    const BlobDesc &cast_in_logical_blob_desc =
        producer->LogicalBlobDesc4Lbi(cast_in_lbi);
    if (cast_in_logical_blob_desc.is_dynamic()) {
      return;
    }
    for (const OpEdge *out_edge : op_node->out_edges()) {
      ...
      for (const std::string &ibn : consumer->op().input_bns()) {
        if (consumer->op().BnInOp2Lbi(ibn) == cast_out_lbi) {
          ReplaceInputLbnInOpCustomizedConf(conf, ibn,
                                            GenLogicalBlobName(cast_out_lbi),
...
REGISTER_FUNCTION_PASS("PruneCastToStaticShapeOpsPass",
