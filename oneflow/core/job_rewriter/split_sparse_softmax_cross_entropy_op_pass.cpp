...
void UpdateProbConsumerOpConf(const std::string &new_prob_lbn,
...
Maybe<void>
SplitSparseSoftmaxCrossEntropyOpPass::Apply(const OpGraph &op_graph,
  ...
  op_graph.ForEachNode([&](const OpNode *node) {
    ...
    if (op_conf.user_conf().op_type_name() !=
    ...
    const int32_t split_axis =
        node->LogicalBlobDesc4Lbi(node->op().BnInOp2Lbi("prediction_0"))
            .shape()
            .NumAxes() -
        1;
    SbpSignature reduce_max_device_stage_sbp_signature;
    (*reduce_max_device_stage_sbp_signature
          .mutable_bn_in_op2sbp_parallel())["in_0"]
        .mutable_split_parallel()
        ->set_axis(split_axis);
    (*reduce_max_device_stage_sbp_signature
          .mutable_bn_in_op2sbp_parallel())["out_0"]
        .mutable_split_parallel()
        ->set_axis(split_axis);
    (*job_builder->mutable_job_parallel_view_conf()
          ->mutable_op_name2sbp_signature_conf())[reduce_max_device_stage_op
                                                      .op_name()] =
        reduce_max_device_stage_sbp_signature;
    ...
    auto reduce_max_global_stage_op =
        user_op::UserOpConfWrapperBuilder(
            ...
            .Input("device_count",
                   reduce_max_device_stage_op.output("count", 0))
            .Output("out")
            .Output("mask")
            .Attr("axis", axis_vec)
            .Attr("keepdims", true)
    ...
    (*reduce_max_global_stage_sbp_signature
        ...
        .mutable_broadcast_parallel();
    (*job_builder->mutable_job_parallel_view_conf()
          ->mutable_op_name2sbp_signature_conf())[reduce_max_global_stage_op
                                                      .op_name()] =
        reduce_max_global_stage_sbp_signature;
    ...
    auto broadcast_sub_max_op =
        user_op::UserOpConfWrapperBuilder(op_name + "-split_softmax_sub_max")
            ...
            .Build();
    job_builder->AddOps(node->parallel_desc().parallel_conf(),
                        {broadcast_sub_max_op.op_conf()});

    auto exp_op =
        user_op::UserOpConfWrapperBuilder(op_name + "-split_softmax_exp")
            .Op("exp")
    ...
    auto reduce_sum_op =
        user_op::UserOpConfWrapperBuilder(op_name + "-split_softmax_reduce_sum")
            .Op("reduce_sum")
            ...
            .Build();
    job_builder->AddOps(node->parallel_desc().parallel_conf(),
                        {reduce_sum_op.op_conf()});

    auto broadcast_div_op =
        user_op::UserOpConfWrapperBuilder(op_name + "-split_softmax_div")
            .Op("broadcast_div")
            ...
            .Build();
    job_builder->AddOps(node->parallel_desc().parallel_conf(),
                        {broadcast_div_op.op_conf()});
    UpdateProbConsumerOpConf(broadcast_div_op.output("z", 0), node,
                             job_builder);

    auto sparse_cross_entropy_ms_op =
