...
Maybe<void> AutoTrainStep::Apply(const OpGraph &op_graph, Job *job) const {
  ...
  const std::string train_step_name =
      "System-Train-TrainStep-" + job->job_conf().job_name();
  ...
  *variable_conf->mutable_shape()->mutable_dim()->Add() = 1;
  ...
  variable_conf->mutable_initializer()->mutable_constant_int_conf()->set_value(
      0);
  ...
  identity_conf->set_in(
      GenLogicalBlobName(variable_op_conf.name(), variable_conf->out()));
  ...
  auto scalar_add_op =
      user_op::UserOpConfWrapperBuilder(train_step_name + "-ScalarAdd")
          .Op("scalar_add")
  ...
  auto assign_op =
      user_op::UserOpConfWrapperBuilder(train_step_name + "-Assign")
          .Op("assign")
          .Input("ref", GenLogicalBlobName(variable_op_conf.name(),
                                           variable_conf->out()))
  ...
  job_builder.AddOps(parallel_conf,
                     {variable_op_conf, identity_op_conf,
                      scalar_add_op.op_conf(), assign_op.op_conf()});
  job->mutable_job_conf()->mutable_train_conf()->set_train_step_lbn(
      train_step_lbn);
  return Maybe<void>::Ok();
}
