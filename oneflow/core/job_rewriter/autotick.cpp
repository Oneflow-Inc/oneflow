...
std::unique_ptr<MutOpConTickInputHelper>
NewMutOpConTickInputHelper(const OperatorConf &op_conf) {
...
void GroupTickByParallelDesc(const OpGraph &op_graph, JobBuilder *job_builder) {
  ...
  op_graph.ForEachNode([&](OpNode *op_node) {
    auto mut_tick_input_helper =
        NewMutOpConTickInputHelper(op_node->op().op_conf());
    if (!mut_tick_input_helper) {
      return;
    }
    if (mut_tick_input_helper->IsTickInputBound() == true) {
      return;
    }
  ...
  for (const auto &pair : parallel_desc2op_node) {
    ...
    device_tick_op.set_name("System-AutoTick-Prepend-DeviceTick_" +
                            NewUniqueId());
    ...
    for (const auto *op_node : pair.second) {
      ...
      job_builder->MutOpsOnlyOnce(
          {mut_tick_input_helper->NewTickInputBoundOpConf(
              device_tick_op.name() + "/out")});
    }
  }
}

void BuildSourceTickOpAndParallelConf(OperatorConf *src_tick_op,
...
void BuildSinkTickOpAndParallelConf(OperatorConf *sink_tick_op,
                                    JobBuilder *job_builder) {
...
void BuildPartialTickOp(OperatorConf *partial_tick_op,
...
void BuildPartialTickOp7SinkTickOp(
    ...
    const std::function<const ParallelConf *(const std::string &)>
        &ParallelConf4OpName,
...
void ConnectSourceTickAndOtherTick(JobBuilder *job_builder) {
  ...
  job_builder->ForEachOperator([&](const Operator &op) {
    if (op.op_name() != src_tick_op.name()) {
      CHECK(!op.op_conf().has_source_tick_conf());
    }
    auto mut_helper = NewMutOpConTickInputHelper(op.op_conf());
    if (!mut_helper) {
      return;
    }
    if (mut_helper->IsTickInputBound() == true) {
      return;
    }
...
const Shape &GetOpTimeShape(const OpNode *op_node) {
  ...
  if (output_shape == nullptr) {
    output_shape = op_node->GetInputBlobFastestTimeShape();
  }
  return *output_shape;
};
...
OperatorConf MakeTickOpConf(const std::string &tick_name) {
  ...
  tick_op_conf.set_name(std::string("System-AutoTick-" + tick_name + "Tick_") +
                        NewUniqueId());
...
OperatorConf MakeDeviceTickOpConf(const std::string &tick_name) {
  ...
  device_tick_op_conf.set_name(
      std::string("System-AutoTick-" + tick_name + "DeviceTick_") +
      NewUniqueId());
...
OperatorConf AppendTick(const std::string tick_name,
  ...
  for (const auto &op_name : op_names) {
    device_tick_op_conf.add_ctrl_in_op_name(op_name);
  }
...
OperatorConf AppendTick(const std::string tick_name,
                        const std::list<const OpNode *> &op_nodes,
                        JobBuilder *job_builder) {
  ...
  return AppendTick(tick_name, op_names,
                    op_nodes.front()->parallel_desc().parallel_conf(),
                    job_builder);
}

OperatorConf PrependTick(const HashSet<const OpNode *> &op_nodes,
                         JobBuilder *job_builder) {
...
OperatorConf AppendAccTick(const Shape &src_shape,
  ...
  const OperatorConf &tick_op_conf =
      AppendTick("AppendAcc", op_nodes, job_builder);
  OperatorConf acc_op_conf;
  {
    acc_op_conf.set_name(std::string("System-AutoTick-AccTick_") +
                         NewUniqueId());
    auto *acc_conf = acc_op_conf.mutable_acc_tick_conf();
    CHECK(tick_op_conf.has_device_tick_conf());
    acc_conf->set_one(tick_op_conf.name() + "/" +
                      tick_op_conf.device_tick_conf().out());
    acc_conf->set_acc("acc");
    acc_conf->set_max_acc_num(tick_shape.elem_cnt() / src_shape.elem_cnt());
  }
  OperatorConf last_device_tick_op_conf;
  {
    last_device_tick_op_conf.set_name(std::string("System-AutoTick-Tick_") +
                                      NewUniqueId());
    auto *device_tick_conf =
        last_device_tick_op_conf.mutable_device_tick_conf();
...
CriticalSection *
AddGlobalCriticalSection(const std::string &src_tick_op_name,
  ...
  Global<CriticalSectionDesc>::Get()->AddCriticalSection(
      std::move(critical_sec));
  return ret;
}
...
std::vector<std::string> GetOpNames(const HashSet<const OpNode *> &op_nodes) {
  ...
  for (const OpNode *op_node : op_nodes) {
    ret.push_back(op_node->op().op_name());
  }
  return ret;
};
...
void InitOpTypeCase2OpNodes(
    ...
    HashMap<OperatorConf::OpTypeCase, HashSet<const OpNode *>>
        *op_type_case2op_nodes) {
  op_graph.ForEachNode([&](OpNode *op_node) {
    const auto &op_conf = op_node->op().op_conf();
    if (IsInterfaceOpConf(op_conf)) {
      CHECK((*op_type_case2op_nodes)[op_conf.op_type_case()]
                .emplace(op_node)
                .second);
    }
  });
...
void ForEachInputCriticalSectionOpNodes(
    const OpGraph &op_graph,
    const std::function<void(const HashSet<const OpNode *> &,
  ...
  if (op_type_case2op_nodes[op_type_case].empty()) {
    return;
  }
  ...
  for (const OpNode *op_node : op_type_case2op_nodes[op_type_case]) {
    op_node->ForEachNodeOnOutEdge(
        [&](OpNode *out_node) { op_nodes.insert(out_node); });
  }
...
std::vector<OperatorConf>
AddTickForTimeShape(const Shape &src_time_shape,
                    const HashSet<const OpNode *> &op_nodes,
                    JobBuilder *job_builder) {
  HashMap<std::pair<ParallelDesc, std::pair<Shape, Shape>>,
          std::list<const OpNode *>>
      pd7ts2op_nodes;
  for (const OpNode *op_node : op_nodes) {
    auto ts = std::make_pair(*op_node->GetInputOutputFastestTimeShape(),
                             GetOpTimeShape(op_node));
...
void AddGlobalInputOutputCriticalSection(
    const HashSet<const OpNode *> &op_nodes,
    const std::vector<std::string> &lbi_producer_op_names,
    JobBuilder *job_builder) {
  auto time_shape = std::make_unique<Shape>(DimVector{
  ...
  for (const OpNode *op_node : op_nodes) {
    CHECK(parallel_desc2op_nodes[op_node->parallel_desc()]
              .insert(op_node)
              .second);
  }
  ...
  for (const auto &pair : parallel_desc2op_nodes) {
    ...
    for (const auto &sink_tick :
         AddTickForTimeShape(*time_shape, pair.second, job_builder)) {
      sink_ticks.push_back(sink_tick);
      CHECK(op_name2parallel_conf
                .emplace(sink_tick.name(), &pair.first.parallel_conf())
                .second);
    }
  }
  OperatorConf src_tick_op_conf;
  {
    ...
    for (auto &op_conf : source_ticks) {
      op_conf.mutable_tick_conf()->add_tick(
          src_tick_op_conf.name() + "/" +
  ...
  OperatorConf sink_tick_op_conf;
  BuildPartialTickOp7SinkTickOp(&sink_tick_op_conf, ParallelConf4OpName,
                                tick_lbis, job_builder);
  auto *io_cs = AddGlobalCriticalSection(src_tick_op_conf.name(),
                                         sink_tick_op_conf.name())
...
void AutoSourceTick(const OpGraph &op_graph, JobBuilder *job_builder) {
  ...
  op_graph.ForEachNode([&](OpNode *node) {
    CHECK(!node->op().op_conf().has_source_tick_conf());
  });
  ConnectSourceTickAndOtherTick(job_builder);
}

void AddTickForTimeShape(const OpGraph &op_graph, JobBuilder *job_builder) {
  const auto &src_time_shape =
      *GetSrcTickOpNode(op_graph)->out_blob_time_shape();
...
void AutoSinkTick(const OpGraph &op_graph, JobBuilder *job_builder) {
  op_graph.ForEachNode(
      [&](OpNode *node) { CHECK(!node->op().op_conf().has_sink_tick_conf()); });
  const auto &src_time_shape =
      *GetSrcTickOpNode(op_graph)->out_blob_time_shape();
  HashSet<LogicalBlobId> tick_lbis;
  op_graph.ForEachNode([&](OpNode *op_node) {
    ...
    CHECK(tick_lbis.emplace(op_node->op().BnInOp2Lbi(op_node->op().SoleObn()))
              .second);
  });
  auto ParallelConf4OpName = [&](const std::string &op_name) {
...
void AddGlobalInputCriticalSections(const OpGraph &op_graph,
                                    JobBuilder *job_builder) {
  ForEachInputCriticalSectionOpNodes(
      op_graph, [&](const HashSet<const OpNode *> &op_nodes,
                    const std::vector<std::string> &lbi_producer_op_names) {
        AddGlobalInputOutputCriticalSection(op_nodes, lbi_producer_op_names,
                                            job_builder);
      });
}

void AddGlobalOutputCriticalSections(const OpGraph &op_graph,
                                     JobBuilder *job_builder) {
