...
void GroupBoxingByDstParallel(const OpGraph &op_graph,
  ...
  op_graph.ForEachNode([&](const OpNode *node) {
    ...
    if (IsClassRegistered<DisableInputBoxingGroup>(op_type_case)) {
      return;
    }
    for (const std::string &ibn : node->op().input_bns()) {
      ...
      if (producer.parallel_desc() != node->parallel_desc() ||
          producer_sbp != consumer_sbp) {
        lbi2consumer_grouped_by_parallel_sbp[lbi][{node->parallel_desc(),
                                                   consumer_sbp}]
            .push_back({node, ibn});
  ...
  for (const auto &lbi7groups : lbi2consumer_grouped_by_parallel_sbp) {
    ...
    for (const auto &parallel7group : lbi7groups.second) {
      if (parallel7group.second.size() < 2) {
        continue;
      }
      ...
      job_builder->AddOps(dst_parallel_desc.parallel_conf(),
                          {identity_op_conf});
      SbpSignature identity_sbp_signature;
      (*identity_sbp_signature.mutable_bn_in_op2sbp_parallel())["in"] =
          dst_sbp_parallel;
      (*identity_sbp_signature.mutable_bn_in_op2sbp_parallel())["out"] =
          dst_sbp_parallel;
      ...
      for (const auto &consumer7ibn : parallel7group.second) {
        ...
        PbMessage *consumer_op_type_conf = MutableMessageInPbMessage(
            &consumer_op_conf, consumer_op_conf.op_type_case());
        ReplaceInputLbnInOpCustomizedConf(consumer_op_type_conf, ibn,
