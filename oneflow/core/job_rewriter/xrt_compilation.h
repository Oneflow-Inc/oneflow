...
inline void RebuildXrtCompiledJob(const OpGraph &op_graph, Job *job) {
  ...
  if (Global<ResourceDesc, ForSession>::Get()->enable_debug_mode()) {
    TeePersistentLogStream::Create("job_without_xrt_" +
                                   std::to_string(job_desc.job_id()))
        ->Write(*job);
  }
  // Run compilation time passes currently include `MarkClusterId`,
  // `BuildSubGraph`
  // and `RebuildCompiledJob`.
  xrt::RunCompilationTimeXrtPasses(op_graph, job, job_desc.IsTrain());

  if (Global<ResourceDesc, ForSession>::Get()->enable_debug_mode()) {
    TeePersistentLogStream::Create("job_with_xrt_" +
                                   std::to_string(job_desc.job_id()))
        ->Write(*job);
  }
#endif // OF_WITH_XRT
