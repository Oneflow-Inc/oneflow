...
public:
  using NaiveFunc = std::function<void(
      const Operator &, std::vector<OperatorConf> *,
      const std::function<LogicalBlobId *(const std::string &)> &)>;
  using MaybeFunc = std::function<Maybe<void>(
      const Operator &, std::vector<OperatorConf> *,
      const std::function<LogicalBlobId *(const std::string &)> &,
      const std::function<const BlobDesc &(const std::string &)> &)>;
  ...
  Maybe<void>
  Call(const Operator &, std::vector<OperatorConf> *,
       const std::function<LogicalBlobId *(const std::string &)> &,
...
#define REGISTER_OP_GRAD(op_type_case, gen_grad_func)                          \
  REGISTER_CLASS_CREATOR(                                                      \
      op_type_case, GenerateBackwardOpConfWrapperStruct,                       \
