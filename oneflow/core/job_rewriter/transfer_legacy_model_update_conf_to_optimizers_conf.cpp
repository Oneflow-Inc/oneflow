...
class TransferLegacyModelUpdateConfToOptimizersConf final : public JobPass {
 public:
  OF_DISALLOW_COPY_AND_MOVE(TransferLegacyModelUpdateConfToOptimizersConf);
  TransferLegacyModelUpdateConfToOptimizersConf() = default;
  ~TransferLegacyModelUpdateConfToOptimizersConf() override = default;
...
Maybe<void> TransferLegacyModelUpdateConfToOptimizersConf::Apply(const OpGraph& op_graph,
                                                                 Job* job) const {
  JobBuilder job_builder(job);
  const TrainConf& train_conf = job->job_conf().train_conf();
  if (train_conf.has_optimizers_conf()) { return Maybe<void>::Ok(); }
  const LegacyModelUpdateConf& legacy_model_update_conf = train_conf.legacy_model_update_conf();
  OptimizersConf* optimizers_conf =
      job->mutable_job_conf()->mutable_train_conf()->mutable_optimizers_conf();
  OptimizerConf* optimizer_conf = optimizers_conf->add_optimizer_conf();
  op_graph.ForEachNode([&](OpNode* op_node) {
    if (op_node->op().op_conf().has_variable_conf()) {
      optimizer_conf->add_variable_op_names(op_node->op().op_name());
    }
  });
  if (legacy_model_update_conf.has_learning_rate_decay()) {
    optimizer_conf->mutable_learning_rate_decay()->CopyFrom(
        legacy_model_update_conf.learning_rate_decay());
  }
  if (legacy_model_update_conf.has_warmup_conf()) {
    optimizer_conf->mutable_warmup_conf()->CopyFrom(legacy_model_update_conf.warmup_conf());
  }
  if (legacy_model_update_conf.has_clip_conf()) {
    optimizer_conf->mutable_clip_conf()->CopyFrom(legacy_model_update_conf.clip_conf());
  }
  if (legacy_model_update_conf.has_weight_decay_conf()) {
    optimizer_conf->mutable_weight_decay_conf()->CopyFrom(
        legacy_model_update_conf.weight_decay_conf());
  }
...
REGISTER_JOB_PASS("TransferLegacyModelUpdateConfToOptimizersConf",
                  TransferLegacyModelUpdateConfToOptimizersConf);
