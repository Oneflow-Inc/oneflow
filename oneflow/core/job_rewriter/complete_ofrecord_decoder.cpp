...
void SplitDecodeOps(Job *job) {
  ...
  for (OperatorConf &op_conf : *(job->mutable_net()->mutable_op())) {
    if (op_conf.has_decode_ofrecord_conf() == false) {
      continue;
    }
    if (op_conf.decode_ofrecord_conf().blob_size() == 1) {
      continue;
    }
  ...
  for (OperatorConf &gen_op_conf : gen_op_confs) {
    *(job->mutable_net()->add_op()) = gen_op_conf;
  }
}

void AddRecordLoadOps(Job *job) {
  HashMap<std::pair<std::string, std::string>, std::vector<OperatorConf *>>
      data_info2decode_ops;
  ...
  HashMap<std::pair<std::string, std::string>, const RandomShuffleConf *>
      data_info2shuffle_conf;
  ...
  FOR_RANGE(size_t, idx, 0, op_num) {
    ...
    const DecodeOFRecordOpConf &decode_conf = op_conf->decode_ofrecord_conf();
    if (decode_conf.has_in()) {
      continue;
    }
    if (decode_conf.blob_size() == 0) {
      continue;
    }
    std::pair<std::string, std::string> data_info = {
        decode_conf.data_dir(), decode_conf.part_name_prefix()};
    ...
    const RandomShuffleConf *shuffle_conf =
        decode_conf.has_random_shuffle_conf()
            ? &decode_conf.random_shuffle_conf()
            : nullptr;
    if (data_info2shuffle_conf.find(data_info) !=
        data_info2shuffle_conf.end()) {
      if (shuffle_conf == nullptr) {
        CHECK(data_info2shuffle_conf.at(data_info) == nullptr);
      } else {
        CHECK(data_info2shuffle_conf.at(data_info) != nullptr);
        CHECK_EQ(data_info2shuffle_conf.at(data_info)->buffer_size(),
                 shuffle_conf->buffer_size());
      }
    } else {
  ...
  for (const PlacementGroup &p_group : job->placement().placement_group()) {
    for (const std::string &op_name : p_group.op_set().op_name()) {
      CHECK(
          name2parallel_conf.emplace(op_name, &p_group.parallel_conf()).second);
    }
  }
  ...
  for (const auto &pair : data_info2decode_ops) {
    ...
    for (const auto &decode_op : pair.second) {
      batch_size2decode_ops[decode_op->decode_ofrecord_conf().batch_size()]
          .push_back(decode_op);
    }
    for (const auto &batch_size_n_decode_ops : batch_size2decode_ops) {
      ...
      for (const OperatorConf *op_conf : batch_size_n_decode_ops.second) {
        ...
        auto iter =
            std::find_if(parallel_confs.begin(), parallel_confs.end(),
                         [&](const ParallelConf *parallel_conf) {
                           PbMd message_diff;
                           return message_diff.Equivalent(
                               *parallel_conf, *(op_parallel_conf_it->second));
                         });
      ...
      LOG_IF(WARNING, parallel_confs.size() > 1)
          << "Operators sharing the same data information belong to different "
             "placement groups";
      for (const ParallelConf *parallel_conf : parallel_confs) {
        ...
        std::string record_load_lbi_name =
            record_load_op_name + "/" + record_load_out_name;
        ...
        record_load_op->set_part_name_suffix_length(
            data_info2suffix_length.at(pair.first));
        record_load_op->set_batch_size(batch_size_n_decode_ops.first);
        if (data_info2shuffle_conf.at(pair.first) != nullptr) {
          *record_load_op->mutable_random_shuffle_conf() =
              *data_info2shuffle_conf.at(pair.first);
        }
        PlacementGroup *p_group =
            job->mutable_placement()->add_placement_group();
        ...
        *(p_group->mutable_parallel_conf()) = *parallel_conf;
        for (OperatorConf *op : batch_size_n_decode_ops.second) {
          ...
          CHECK(op_parallel_conf_it != name2parallel_conf.end());
          PbMd message_diff;
          if (!message_diff.Equivalent(*parallel_conf,
                                       *(op_parallel_conf_it->second))) {
            continue;
          }
