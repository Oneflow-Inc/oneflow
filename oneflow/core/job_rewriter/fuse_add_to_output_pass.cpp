...
public:
  ...
  bool IsEnabled() const override {
    return GlobalJobDesc().job_conf().enable_fuse_add_to_output();
  }
  Maybe<void> Apply(const OpGraph &op_graph,
                    JobBuilder *job_builder) const override;
};

Maybe<void> FuseAddToOutputPass::Apply(const OpGraph &op_graph,
  ...
  auto IsAddToOutputSupported = [&](const OpNode *node,
                                    const LogicalBlobId &lbi) -> bool {
    const OperatorConf &op_conf = node->op().op_conf();
    if (!op_conf.has_user_conf()) {
      return false;
    }
    if (op_name2op_conf.find(op_conf.name()) != op_name2op_conf.end()) {
      return false;
    }
    auto it = supported_op_type_name2output_arg.find(
        op_conf.user_conf().op_type_name());
    if (it == supported_op_type_name2output_arg.end()) {
      return false;
    }
    const user_op::UserOpConfWrapper user_op_conf(op_conf);
    if (GenLogicalBlobId(user_op_conf.output(it->second.name(),
                                             it->second.index())) != lbi) {
      return false;
    }
    ...
    if (output_consumer_cnt != 1) {
      return false;
    }
    // already fused
    if (user_op_conf.has_input("_add_to_output", 0)) {
      return false;
    }
    return true;
  };
  ...
  op_graph.ForEachNode([&](const OpNode *op_node) {
    for (const std::string &ctrl_in_op_name :
         op_node->op().op_conf().ctrl_in_op_name()) {
  ...
  op_graph.ForEachNode([&](const OpNode *op_node) {
    ...
    if (op_conf.user_conf().op_type_name() != "add_n") {
      return;
    }
    if (op_name2op_conf.find(op_conf.name()) != op_name2op_conf.end()) {
      return;
    ...
    const user_op::UserOpConfWrapper user_op_conf(op_conf);
    if (user_op_conf.input_size("in") != 2) {
      return;
    }
    ...
    if ((!IsReachable(in_0.op_name(), in_1.op_name())) &&
        IsAddToOutputSupported(in_0_node, in_0)) {
    ...
    *(*(new_add_to_op_conf.mutable_user_conf()
            ->mutable_input()))["_add_to_output"]
         .mutable_s()
    ...
    for (const OpEdge *out_edge : op_node->out_edges()) {
      ...
      for (const std::string &ibn : consumer->op().input_bns()) {
        if (consumer->op().BnInOp2Lbi(ibn) == out) {
          ...
          PbMessage *conf = MutableMessageInPbMessage(
              &consumer_op_conf, consumer_op_conf.op_type_case());
