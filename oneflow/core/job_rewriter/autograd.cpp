...
bool AllSplitDistribution(const cfg::NdSbp& parallel_distribution) {
...
void BindFwBwObaPairs(const OpGraph& op_graph, const OpBlobArgPairs& fw_bw_oba_pairs,
  ...
  op_graph.ForEachNode([&](OpNode* op_node) {
    ...
    auto TryInserSplitParalleledObas = [&](const std::string& bn) {
      ...
      const auto& fw_parallel_distribution = op_node->NdSbp4Lbi(lbi);
...
bool IsBroadcast(const cfg::NdSbp& parallel_distribution,
