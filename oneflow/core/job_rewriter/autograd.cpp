...
void GenerateOriginDiffLbi(JobPassCtx* ctx, const OpGraph& op_graph, JobBuilder* job_builder,
  ...
  if (train_conf.has_dynamic_loss_scale_policy()) {
    ...
    if (data_type == DataType::kFloat) {
      ...
      job_builder->SetNdSbp4Oba(cast_in_op_blob_arg,
                                               broadcast_nd_sbp);
      ...
      job_builder->SetNdSbp4Oba(cast_out_op_blob_arg,
                                               broadcast_nd_sbp);
    ...
    }
    {
      ...
      if (time_shape_elem_cnt != 1) {
        ...
        job_builder->SetNdSbp4Oba(repeat_in_op_blob_arg,
                                                 broadcast_nd_sbp);
        ...
        job_builder->SetNdSbp4Oba(repeat_out_op_blob_arg,
                                                 broadcast_nd_sbp);
...
bool IsBroadcast(const cfg::NdSbp& nd_sbp,
...
void ClipGradientByGlobalNorm(const OpGraph& op_graph, JobBuilder* job_builder,
  ...
  ForEachAggregatedParamGroup(
      ...
      [&](const ParallelDesc& parallel_desc, const cfg::NdSbp& nd_sbp,
        ...
        is_broadcast_nd_sbp.push_back(
            IsBroadcast(nd_sbp, parallel_desc));
  ...
  const bool all_group_broadcast =
      std::all_of(is_broadcast_nd_sbp.begin(),
                  is_broadcast_nd_sbp.end(), [](bool i) { return i; });
