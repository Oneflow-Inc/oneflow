...
const TrainConf &GetTrainConf() {
  return GlobalJobDesc().job_conf().train_conf();
}
...
bool AnyLbiWithDiffLbi(const OpEdge *op_edge) {
  ...
  auto IsOutputBlobModifierRequiresGrad = [&](const LogicalBlobId &lbi) {
    return src_op.OutputBlobModifier4Obn(op_edge->lbi2obn().at(lbi))
        .requires_grad();
  };
  auto IsInputBlobModifierRequiresGrad = [&](const LogicalBlobId &lbi) {
    ...
    for (const std::string &ibn : ibns) {
      if (dst_op.InputBlobModifier4Ibn(ibn).requires_grad()) {
        return true;
      }
    }
  ...
  for (const LogicalBlobId &lbi : op_edge->lbis()) {
    if (IsOutputBlobModifierRequiresGrad(lbi) &&
        IsInputBlobModifierRequiresGrad(lbi)) {
      return true;
    }
  }
...
void CheckNotReachableAmongOpNodes(const OpGraph &op_graph,
                                   const std::list<OpNode *> &op_nodes) {
...
Maybe<void> GetLossOpNodes(const OpGraph &op_graph,
  ...
  if (loss_op_nodes->empty()) {
    return Error::LossBlobNotFoundError("Loss blob not found.");
  }
  return Maybe<void>::Ok();
}

Maybe<void> GetLossOpNodesAndAscendants(const OpGraph &op_graph,
                                        HashSet<OpNode *> *op_nodes) {
  std::list<OpNode *> starts;
  JUST(GetLossOpNodes(op_graph, &starts));
  auto ForEachNextNode = [&](OpNode *op_node,
                             const std::function<void(OpNode *)> &Handler) {
    for (OpEdge *edge : op_node->in_edges()) {
      if (AnyLbiWithDiffLbi(edge)) {
...
Maybe<void>
MakePredicatorNeedBackwardOp(const OpGraph &op_graph,
                             std::function<bool(OpNode *)> *NeedBackwardOp) {
  auto var_op_nodes_and_descendants = std::make_shared<HashSet<OpNode *>>();
  GetVariableOpNodesAndDescendants(op_graph,
                                   var_op_nodes_and_descendants.get());
  auto loss_op_nodes_and_ascendants = std::make_shared<HashSet<OpNode *>>();
  JUST(GetLossOpNodesAndAscendants(op_graph,
                                   loss_op_nodes_and_ascendants.get()));
  *NeedBackwardOp = [var_op_nodes_and_descendants,
                     loss_op_nodes_and_ascendants](OpNode *op_node) {
    if (var_op_nodes_and_descendants->find(op_node) ==
        var_op_nodes_and_descendants->end()) {
      return false;
    }
    if (loss_op_nodes_and_ascendants->find(op_node) ==
        loss_op_nodes_and_ascendants->end()) {
      return false;
    }
    for (const auto &ibn : op_node->op().input_bns()) {
      if (op_node->op().InputBlobModifier4Ibn(ibn).requires_grad()) {
        return true;
      }
    }
    for (const auto &obn : op_node->op().output_bns()) {
      if (op_node->op().OutputBlobModifier4Obn(obn).requires_grad()) {
        return true;
      }
    }
    return false;
...
MakePredicatorHasDiff4LbiOpName(
    const OpGraph &op_graph,
    const std::function<bool(OpNode *)> &NeedBackwardOp) {
  auto lbis2ops_with_in_diff =
      std::make_shared<HashMap<LogicalBlobId, HashSet<std::string>>>();
  op_graph.ForEachEdge([&](OpEdge *edge) {
    if (NeedBackwardOp(edge->src_node()) && NeedBackwardOp(edge->dst_node())) {
      for (const auto &lbi : edge->lbis()) {
        ...
        if (edge->src_node()
                ->op()
                .OutputBlobModifier4Obn(obn)
                .requires_grad()) {
          (*lbis2ops_with_in_diff)[lbi].emplace(
              edge->dst_node()->op().op_name());
        }
      }
    }
  });
  return [lbis2ops_with_in_diff](const LogicalBlobId &lbi,
                                 const std::string &op_name) {
    if (lbis2ops_with_in_diff->find(lbi) == lbis2ops_with_in_diff->end()) {
      return false;
    }
...
void GenerateOriginDiffLbi(const LogicalBlobId &lbi,
                           std::vector<OperatorConf> *op_confs,
                           LogicalBlobId *out_diff_lbi) {
  OperatorConf constant_like_op{};
  constant_like_op.set_name(lbi.op_name() + "_" + lbi.blob_name() +
                            "_grad_ConstantLike");
  ConstantLikeOpConf *constant_like_conf =
      constant_like_op.mutable_constant_like_conf();
...
const ParallelConf &ProducerParallelConf4Lbi(const OpGraph &op_graph,
...
void ScaleModelDiffByConstantLossInstanceNum(
  ...
  for (auto &pair : *lbi2diff_lbi) {
    ...
    auto scalar_mul_op = user_op::UserOpConfWrapperBuilder(
                             "System-ModelDiffScale-ScalarMul_" + NewUniqueId())
                             .Op("scalar_mul")
                             ...
                             .Build();
    job_builder->AddOps(ProducerParallelConf4Lbi(op_graph, lbi),
...
Maybe<void> TryMirroredCastTotalLossInstanceNum(
  ...
  auto IsMirrored4Lbi = [](const LogicalBlobId &lbi,
                           OpNode *op_node) -> Maybe<bool> {
    const auto &obn = *JUST(op_node->op().obn4lbi(lbi));
    const auto &opt_mirrored_parallel =
        *JUST(op_node->op().OptMirroredParallel4BnInOp(obn));
  ...
  if (is_mirrored) {
    ...
    op_conf.set_name("System-Cast-Mirrored-TotalLossInstanceNum" +
                     NewUniqueId());
    CastFromMirroredOpConf *cast_from_mirrored =
        op_conf.mutable_cast_from_mirrored_conf();
    cast_from_mirrored->set_in(
        GenLogicalBlobName(*total_loss_instance_num_lbi));
    ...
    const auto &parallel_conf =
        job_builder->ParallelConf4Lbi(*total_loss_instance_num_lbi);
    int64_t scope_symbol_id = Global<ForeignCallback>::Get()->MakeScopeSymbol(
        job_builder->job().job_conf().DebugString(),
        parallel_conf.DebugString(), true);
...
void ScaleModelDiffByDynamicLossInstanceNum(
    ...
    HashMap<LogicalBlobId, LogicalBlobId> *lbi2diff_lbi,
    const HashMap<LogicalBlobId, OpNode *> &loss_lbi2loss_node) {
  auto BuildInstanceNumOpConf4LossOpNode = [&](const LogicalBlobId &loss_lbi,
                                               const OpNode *op_node,
                                               LogicalBlobId *lbi) {
    OperatorConf instance_num_op;
    instance_num_op.set_name("System-Autograd-" + loss_lbi.op_name() + "-" +
                             loss_lbi.blob_name() + "-LossInstanceNum");
    ...
    instance_num_op_conf->set_data_type(
        op_node->LogicalBlobDesc4Lbi(loss_lbi).data_type());
    instance_num_op_conf->mutable_include_axis_conf()->add_axis(0);
    instance_num_op.set_scope_symbol_id(
        op_node->op().op_conf().scope_symbol_id());
  ...
  if (loss_lbi2loss_node.size() == 1) {
    ...
    for (const auto &pair : loss_lbi2loss_node) {
      ...
      BuildInstanceNumOpConf4LossOpNode(pair.first, pair.second,
                                        &loss_instance_num_lbi);
      total_loss_instance_num_conf->add_in(
          GenLogicalBlobName(loss_instance_num_lbi));
    }
    ...
    int64_t scope_symbol_id = Global<ForeignCallback>::Get()->MakeScopeSymbol(
        job_builder->job().job_conf().DebugString(),
        parallel_conf.DebugString(), false);
    op_conf.set_scope_symbol_id(scope_symbol_id);
    job_builder->AddOps(parallel_conf, {op_conf});
  ...
  CHECK_JUST(TryMirroredCastTotalLossInstanceNum(
      job_builder, loss_lbi2loss_node, &total_loss_instance_num_lbi));
  for (auto &pair : *lbi2diff_lbi) {
    const LogicalBlobId &lbi = pair.first;
    LogicalBlobId &diff_lbi = pair.second;
    auto scalar_div_op =
        user_op::UserOpConfWrapperBuilder("System-ModelDiffScale-ScalarDiv_" +
                                          NewUniqueId())
...
Maybe<void> MakeGetterLossOpNode4OpName(
  ...
  auto loss_op_name2op_node =
      std::make_shared<HashMap<std::string, OpNode *>>();
  for (OpNode *op_node : loss_nodes) {
    CHECK(
        loss_op_name2op_node->emplace(op_node->op().op_name(), op_node).second);
  }
  *LossOpNode4OpName =
      [loss_op_name2op_node](const std::string &op_name) -> OpNode * {
    return loss_op_name2op_node->at(op_name);
  };
  return Maybe<void>::Ok();
}

void BindFwBwObaPairs(const OpGraph &op_graph,
                      const OpBlobArgPairs &fw_bw_oba_pairs,
                      JobBuilder *job_builder) {
  ...
  op_graph.ForEachNode([&](OpNode *op_node) {
    ...
    for (const auto &ibn : op_node->op().input_bns()) {
      TryInserSplitParalleledObas(ibn);
    }
    for (const auto &obn : op_node->op().input_bns()) {
      TryInserSplitParalleledObas(obn);
    }
  });
  for (const auto &pair : fw_bw_oba_pairs.pair()) {
    CHECK(split_paralleled_obas.find(pair.first()) ==
          split_paralleled_obas.end());
    if (split_paralleled_obas.find(pair.second()) !=
        split_paralleled_obas.end()) {
...
void CalcFwBwObaPairs(
  ...
  op_graph.ReverseTopoForEachNode([&](OpNode *op_node) {
    ...
    for (const auto &ibn : op.input_bns()) {
      const auto &in_diff_lbi_it =
          in_oba2in_diff_lbi.find(GenOpBlobArg(op.op_name(), ibn));
      if (in_diff_lbi_it == in_oba2in_diff_lbi.end()) {
        continue;
      }
      if (in_diff_lbi2in_oba.find(in_diff_lbi_it->second) ==
          in_diff_lbi2in_oba.end()) {
  ...
  op_graph.TopoForEachNode([&](OpNode *op_node) {
    ...
    for (const auto &obn : op.output_bns()) {
      const auto &out_diff_lbi_it =
          out_oba2out_diff_lbi.find(GenOpBlobArg(op.op_name(), obn));
      if (out_diff_lbi_it == out_oba2out_diff_lbi.end()) {
        continue;
      }
      if (out_diff_lbi2out_oba.find(out_diff_lbi_it->second) ==
          out_diff_lbi2out_oba.end()) {
  ...
  job_builder.ForEachOperator([&](const Operator &op) {
    ...
    for (const auto &obn : op.output_bns()) {
      ...
      }
      {
        ...
        if (clone_out_oba_it == clone_bw_add_out_lbi2out_oba.end()) {
          continue;
        }
        auto *pair = fw_bw_oba_pairs->mutable_pair()->Add();
        *pair->mutable_first() = GenOpBlobArg(op.op_name(), obn);
...
void InitOutOba2OutDiffLbi(
  ...
  for (const std::string &loss_lbn : GetTrainConf().loss_lbn()) {
    ...
    const auto loss_node_it = std::find_if(
        loss_nodes.cbegin(), loss_nodes.cend(), [&](const OpNode *node) {
          return node->op().op_name() == loss_lbi.op_name();
        });
    CHECK(loss_node_it != loss_nodes.cend());
    const OpNode *loss_op_node = *loss_node_it;
    const auto bn_it = std::find_if(
        loss_op_node->op().output_bns().cbegin(),
        loss_op_node->op().output_bns().cend(), [&](const std::string &obn) {
          return loss_lbi == loss_op_node->op().BnInOp2Lbi(obn);
        });
    CHECK(bn_it != loss_op_node->op().output_bns().cend());
    LogicalBlobId *out_diff_lbi = &(*out_oba2out_diff_lbi)[GenOpBlobArg(
        loss_op_node->op().op_name(), *bn_it)];
    std::vector<OperatorConf> ops;
    GenerateOriginDiffLbi(loss_lbi, &ops, out_diff_lbi);
    int64_t scope_symbol_id = loss_op_node->op().op_conf().scope_symbol_id();
    for (auto &op : ops) {
      op.set_scope_symbol_id(scope_symbol_id);
    }
    job_builder->AddOps(loss_op_node->parallel_desc().parallel_conf(), ops);
  }
}

void CalcOutLbi2OutDiffLbi(
    const OpGraph &op_graph,
    const HashMap<OpBlobArg, LogicalBlobId> &out_oba2out_diff_lbi,
  ...
  op_graph.ForEachNode([&](OpNode *op_node) {
    for (const auto &obn : op_node->op().output_bns()) {
      ...
      if (out_diff_lbi_it != out_oba2out_diff_lbi.end()) {
        CHECK(
            out_lbi2out_diff_lbi->emplace(lbi, out_diff_lbi_it->second).second);
      }
    }
  });
}

void ClipGradientByGlobalNorm(
    const OpGraph &op_graph, JobBuilder *job_builder,
    HashMap<LogicalBlobId, LogicalBlobId> *lbi2diff_lbi,
    const ClipByGlobalNormConf &conf) {
  if (lbi2diff_lbi->empty()) {
    return;
  }
  ...
  ParallelConf global_norm_parallel_conf =
      all_same_parallel_desc ? parallel_desc->parallel_conf()
                             : GenParallelConfOfCpuZeroOnMaster();
  int64_t scope_symbol_id = Global<ForeignCallback>::Get()->MakeScopeSymbol(
      job_builder->job().job_conf().DebugString(),
      global_norm_parallel_conf.DebugString(), false);
  ...
  for (const auto &pair : *lbi2diff_lbi) {
    ...
    square_sum_op_conf.set_name("System-ClipGradient-GlobalNorm-SquareSum-" +
                                NewUniqueId());
    SquareSumOpConf *square_sum_conf =
        square_sum_op_conf.mutable_square_sum_conf();
    square_sum_conf->set_x(GenLogicalBlobName(diff_lbi));
    square_sum_conf->set_y("y");
    square_sum_op_conf.set_scope_symbol_id(
        ScopeSymbolId4Lbi(op_graph, pair.first));
    job_builder->AddOps(lbi2parallel_desc.at(pair.first)->parallel_conf(),
                        {square_sum_op_conf});
    lbns_to_add.push_back(
  ...
  while (lbns_to_add.size() != 1) {
    ...
    user_op::UserOpConfWrapperBuilder add_op_builder(
        "System-ClipGradient-GlobalNorm-Add-" + NewUniqueId());
  ...
  auto inv_global_norm_op =
      user_op::UserOpConfWrapperBuilder(
          ...
          .Build();
  job_builder->AddOps(global_norm_parallel_conf,
                      {inv_global_norm_op.op_conf()});
  OperatorConf inv_clip_norm_op_conf{};
  inv_clip_norm_op_conf.set_name("System-ClipGradient-GlobalNorm-InvClipNorm-" +
                                 NewUniqueId());
  ...
  auto minimum_op =
      user_op::UserOpConfWrapperBuilder(
          "System-ClipGradient-GlobalNorm-Minimum-" + NewUniqueId())
          .Op("broadcast_minimum")
          .Input("x", inv_global_norm_op.output("y", 0))
          .Input("y",
                 GenLogicalBlobName(inv_clip_norm_op_conf.name(),
  ...
  for (auto &pair : *lbi2diff_lbi) {
    ...
    auto scalar_mul_op =
        user_op::UserOpConfWrapperBuilder(
            ...
            .ScopeSymbolId(ScopeSymbolId4Lbi(op_graph, lbi))
            .Build();
    job_builder->AddOps(lbi2parallel_desc.at(lbi)->parallel_conf(),
                        {scalar_mul_op.op_conf()});
...
void GetVariableOpNodesAndDescendants(const OpGraph &op_graph,
                                      HashSet<OpNode *> *op_nodes) {
  std::list<OpNode *> starts;
  op_graph.ForEachNode([&](OpNode *op_node) {
...
Maybe<void> GenerateBackwardOpConfIf(
  ...
  if (!IsClassRegistered<GenerateBackwardOpConfWrapperStruct>(op_type_case)) {
    return Error::GradientFunctionNotFound()
           << PbMessage2TxtString(op.op_conf());
  }
...
Maybe<void>
AutoGrad(const OpGraph &op_graph, JobBuilder *job_builder,
         HashMap<LogicalBlobId, LogicalBlobId> *out_lbi2out_diff_lbi) {
  std::function<bool(OpNode *)> NeedBackwardOp;
  ...
  auto ForEachInNode = [&](OpNode *op_node,
                           const std::function<void(OpNode *)> &Handler) {
    op_node->ForEachNodeOnInEdge([&](OpNode *in_node) {
      if (NeedBackwardOp(in_node)) {
        Handler(in_node);
      }
    });
  };
  auto ForEachOutNode = [&](OpNode *op_node,
                            const std::function<void(OpNode *)> &Handler) {
    op_node->ForEachNodeOnOutEdge([&](OpNode *out_node) {
      if (NeedBackwardOp(out_node)) {
        Handler(out_node);
      }
    });
  };
  auto HasDiff4LbiOpName =
      MakePredicatorHasDiff4LbiOpName(op_graph, NeedBackwardOp);
  ...
  op_graph.TopoForEachNode(
      loss_nodes, ForEachOutNode, ForEachInNode,
  ...
  for (OpNode *op_node : topo_nodes) {
    ...
    auto DiffLbi4BnInOp = [&](const std::string &bn) -> LogicalBlobId * {
      ...
      if (std::find(input_bns.begin(), input_bns.end(), bn) !=
          input_bns.end()) {
        if (HasDiff4LbiOpName(op_node->op().BnInOp2Lbi(bn), op_name) == false) {
          return nullptr;
        }
        if (op_node->op().InputBlobModifier4Ibn(bn).requires_grad() == false) {
          return nullptr;
        }
        return &in_oba2in_diff_lbi[GenOpBlobArg(op_name, bn)];
      } else if (std::find(output_bns.begin(), output_bns.end(), bn) !=
                 output_bns.end()) {
        if (op_node->op().OutputBlobModifier4Obn(bn).requires_grad() == false) {
          return nullptr;
        }
        const auto &out_diff_lbi_it =
            out_oba2out_diff_lbi.find(GenOpBlobArg(op_name, bn));
        if (out_diff_lbi_it == out_oba2out_diff_lbi.end()) {
          return nullptr;
        }
        return &out_diff_lbi_it->second;
      } else {
        LOG(FATAL) << "diff lbi for bn in op not found, bn: " << op_name << "/"
                   << bn;
      }
      return nullptr;
    };
    ...
    GenerateCloneGradOpIfNeed(*op_node, job_builder, in_oba2in_diff_lbi,
                              &out_oba2out_diff_lbi,
                              &out_oba2clone_bw_add_out_lbi);
    std::vector<OperatorConf> ops;
    JUST(GenerateBackwardOpConfIf(op_node->op(), &ops, DiffLbi4BnInOp,
                                  LogicalBlobDesc4BnInOp));
  ...
  CalcFwBwObaPairs(op_graph, in_oba2in_diff_lbi, out_oba2out_diff_lbi,
                   out_oba2clone_bw_add_out_lbi, *job_builder,
                   &fw_bw_oba_pairs);
...
Maybe<void> ScaleModelDiffByLossInstanceNum(
    const OpGraph &op_graph, JobBuilder *job_builder,
    HashMap<LogicalBlobId, LogicalBlobId> *lbi2diff_lbi) {
  std::function<OpNode *(const std::string &)> LossOpNode4OpName;
  ...
  for (const auto &loss_lbn : train_conf.loss_lbn()) {
    ...
    CHECK(
        loss_lbi2op_node.emplace(lbi, LossOpNode4OpName(lbi.op_name())).second);
  }
  ...
  if (all_loss_time_shape_eq_src) {
    ...
    for (const auto &pair : loss_lbi2op_node) {
      const BlobDesc *cur_blob_desc =
          &pair.second->LogicalBlobDesc4Lbi(pair.first);
      if (blob_desc != nullptr) {
        CHECK(*blob_desc == *cur_blob_desc);
      }
      blob_desc = cur_blob_desc;
    }
    if (blob_desc->is_dynamic()) {
      ScaleModelDiffByDynamicLossInstanceNum(op_graph, job_builder,
                                             lbi2diff_lbi, loss_lbi2op_node);
    } else {
      ScaleModelDiffByConstantLossInstanceNum(
          op_graph, job_builder, lbi2diff_lbi, blob_desc->shape().elem_cnt());
    }
  } else {
    ...
    for (const auto &pair : loss_lbi2op_node) {
      ...
      const int64_t time_shape_elem_cnt =
          pair.second->out_blob_time_shape()->elem_cnt();
      // TODO: consider batch_axis or sbp
      const int64_t loss_elem_cnt = cur_blob_desc->shape().elem_cnt() *
                                    time_shape_elem_cnt /
...
void ScaleModelDiffByLossScale(
  ...
  for (auto &pair : *lbi2diff_lbi) {
    ...
    LogicalBlobId &diff_lbi = pair.second;
    auto scalar_mul_op = user_op::UserOpConfWrapperBuilder(
                             "System-ModelDiffScale-ScalarMul-" + NewUniqueId())
                             .Op("scalar_mul")
                             .Input("in", GenLogicalBlobName(diff_lbi))
                             .Output("out")
                             .Attr<bool>("has_float_operand", true)
...
void RegularizeGradient(const OpGraph &op_graph, JobBuilder *job_builder,
  ...
  for (auto &pair : *lbi2diff_lbi) {
    ...
    const VariableOpConf &variable_conf =
        model_op_node->op().op_conf().variable_conf();
    if (!variable_conf.has_regularizer()) {
      continue;
    }
...
void ClipGradient(const OpGraph &op_graph, JobBuilder *job_builder,
  ...
  if (clip_conf.has_clip_by_global_norm()) {
    ClipGradientByGlobalNorm(op_graph, job_builder, lbi2diff_lbi,
...
void AddDiffParallelCast(const OpGraph &op_graph, JobBuilder *job_builder,
  ...
  for (auto &pair : *lbi2diff_lbi) {
    ...
    LogicalBlobId &diff_lbi = pair.second;
    const OpNode *model_op_node = op_graph.OpNode4OpName(lbi.op_name());
    if (model_op_node->parallel_desc().parallel_num() <= 1) {
      continue;
    }
    int64_t scope_symbol_id = model_op_node->op().op_conf().scope_symbol_id();
    OperatorConf parallel_cast_op_conf{};
    parallel_cast_op_conf.set_name("System-AutoGrad-ParallelCast-" +
                                   NewUniqueId());
    ParallelCastOpConf *parallel_cast_conf =
        parallel_cast_op_conf.mutable_parallel_cast_conf();
...
void AddDiffStaticShapeCast(
  ...
  for (auto &pair : *lbi2diff_lbi) {
    ...
    cast_to_static_shape_op_conf.set_name("System-AutoGrad-StaticShapeCast-" +
                                          NewUniqueId());
