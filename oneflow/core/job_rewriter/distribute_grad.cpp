...
Maybe<void> GenerateBackwardOpConf4Concat(
  ...
  const DistributeConcatOpConf &distribute_concat_conf =
      op.op_conf().distribute_concat_conf();
  OperatorConf split_op;
  split_op.set_name(op.op_conf().name() + "_grad");
  DistributeSplitOpConf *split_op_conf =
      split_op.mutable_distribute_split_conf();
...
Maybe<void> GenerateBackwardOpConf4Split(
  ...
  const DistributeSplitOpConf &distribute_split_conf =
      op.op_conf().distribute_split_conf();
  OperatorConf concat_op;
  concat_op.set_name(op.op_conf().name() + "_grad");
  DistributeConcatOpConf *concat_op_conf =
      concat_op.mutable_distribute_concat_conf();
  ...
  FOR_RANGE(int32_t, i, 0, distribute_split_conf.out_size()) {
    ...
    const bool has_diff_i =
        DiffLbi4BnInOp(obn_of_distribute_split_op) != nullptr;
    CHECK_EQ(has_diff_i, has_diff_0);
    if (has_diff_i) {
      concat_op_conf->add_in(
          GenLogicalBlobName(*DiffLbi4BnInOp(obn_of_distribute_split_op)));
    }
  }
...
Maybe<void> GenerateBackwardOpConf4Clone(
  ...
  DistributeAddOpConf *partial_op_conf =
      partial_op.mutable_distribute_add_conf();
  const bool has_diff_0 = DiffLbi4BnInOp(op.output_bns().Get(0)) != nullptr;
  FOR_RANGE(int32_t, i, 0, conf.out_size()) {
    const std::string &obn_of_distribute_clone_op = op.output_bns().Get(i);
    const bool has_diff_i =
        DiffLbi4BnInOp(obn_of_distribute_clone_op) != nullptr;
    CHECK_EQ(has_diff_i, has_diff_0);
    if (has_diff_i) {
      partial_op_conf->add_in(
          GenLogicalBlobName(*DiffLbi4BnInOp(obn_of_distribute_clone_op)));
    }
  }
...
Maybe<void> GenerateBackwardOpConf4Add(
  ...
  DistributeCloneOpConf *broadcast_op_conf =
      broadcast_op.mutable_distribute_clone_conf();
  ...
  FOR_RANGE(int32_t, i, 0, distribute_add_conf.in_size()) {
    ...
    if (DiffLbi4BnInOp(ibn_of_distribute_add_op) != nullptr) {
      DiffLbi4BnInOp(ibn_of_distribute_add_op)
          ->set_op_name(broadcast_op.name());
...
REGISTER_OP_GRAD(OperatorConf::kDistributeConcatConf,
                 &GenerateBackwardOpConf4Concat);
REGISTER_OP_GRAD(OperatorConf::kDistributeSplitConf,
                 &GenerateBackwardOpConf4Split);
REGISTER_OP_GRAD(OperatorConf::kDistributeCloneConf,
                 &GenerateBackwardOpConf4Clone);
