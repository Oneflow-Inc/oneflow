...
int64_t GetSoleOutBlobSize(const OpNode *node) {
  ...
  return blob_desc.shape().elem_cnt() *
...
public:
  ...
  bool IsEnabled() const override {
    return GlobalJobDesc().IsTrain() &&
           GlobalJobDesc().enable_non_distributed_optimizer();
  }
  Maybe<void> Apply(const OpGraph &op_graph,
                    JobBuilder *job_builder) const override;
};

Maybe<void> NonDistributedOptimizerPass::Apply(const OpGraph &op_graph,
                                               JobBuilder *builder) const {
  HashMap<ParallelDesc, HashMap<const OpNode *, std::vector<const OpNode *>>>
  ...
  op_graph.ForEachNode([&](const OpNode *node) {
    ...
    while (cur_node != nullptr) {
      if (cur_node->parallel_desc().device_type() != DeviceType::kGPU) {
        break;
      }
      if (cur_node != node) {
        if (cur_node->in_edges().size() > 1) {
          break;
        }
        if (cur_node->op().input_bns().size() != 1) {
          break;
        }
        const std::string &sole_ibn = cur_node->op().input_bns().Get(0);
        if (!cur_node->SbpParallel4BnInOp(sole_ibn).has_broadcast_parallel()) {
          break;
        }
      }
      if (cur_node->op().output_bns().size() != 1) {
        break;
      }
      const std::string &sole_obn = cur_node->op().output_bns().Get(0);
      if (!cur_node->SbpParallel4BnInOp(sole_obn).has_broadcast_parallel()) {
        break;
      }
      const auto &lbi = cur_node->op().BnInOp2Lbi(sole_obn);
      if (CHECK_JUST(cur_node->BatchAxis4Lbi(lbi))->has_value()) {
        break;
      }
    ...
    if (op_seq_without_batch_dim.empty()) {
      return;
    }
    ...
    last_node->ForEachNodeOnOutEdge([&](const OpNode *dst) {
      ...
      min_consumer_topo_order =
          std::min(min_consumer_topo_order, op_node2topo_order.at(dst));
    });
  ...
  for (const auto &pair : pd2last_node2node_seqs) {
    ...
    for (const auto &last_node7node_seqs : last_node2node_seqs) {
      ...
      last_node_out_size_pairs.emplace_back(last_node,
                                            last_node2model_size.at(last_node));
    }
    std::sort(last_node_out_size_pairs.begin(), last_node_out_size_pairs.end(),
              [&](const std::pair<const OpNode *, int64_t> &lhs,
                  const std::pair<const OpNode *, int64_t> &rhs) {
                return lhs.second > rhs.second;
              });
    for (const auto &last_node7out_size : last_node_out_size_pairs) {
      const auto it =
          std::min_element(parallel_id2size.cbegin(), parallel_id2size.cend());
      const int64_t min_parallel_id =
          std::distance(parallel_id2size.cbegin(), it);
    ...
    for (const auto &last_node7node_seqs : last_node2node_seqs) {
      ...
      const ParallelConf parallel_conf =
          NonDistributedParallelConf4ParallelId(pd, parallel_id);
  ...
  for (auto &pair : pd2last_nodes) {
    ...
    std::sort(last_nodes->begin(), last_nodes->end(),
    ...
    FOR_RANGE(int64_t, i, 1, last_nodes->size()) {
      ...
      const OpNode *prev_var_node =
          last_node2var_node.at(last_nodes->at(i - 1));
...
REGISTER_FUNCTION_PASS("NonDistributedOptimizerPass",
