...
public:
  using Func = std::function<void(const VariableOp &, const ParallelConf &,
                                  JobBuilder *, const LogicalBlobId &)>;
  GenerateOptimizerOpConfWrapperStruct(const Func &f)
      : func_(std::make_unique<Func>(f)) {}
  void Call(const VariableOp &op, const ParallelConf &parallel_conf,
...
#define REGISTER_OPTIMIZER(model_update_case, gen_grad_func)                   \
  REGISTER_CLASS_CREATOR(                                                      \
      model_update_case, GenerateOptimizerOpConfWrapperStruct, ([] {           \
