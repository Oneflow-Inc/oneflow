...
void GenerateOptimizerOpConf(const VariableOp &op,
  ...
  if (has_snapshot_path && SnapshotFS()->FileExists(file_path)) {
    ...
    momentum_var.mutable_variable_conf()
        ->mutable_initialize_with_snapshot()
        ->set_path(JoinPath(job_builder->job()
                                .job_conf()
    ...
    if (has_snapshot_path) {
      LOG(INFO) << file_path << " not found, will be initialized";
    }
    InitializerConf constant_initializer;
    constant_initializer.mutable_constant_conf()->set_value(0.f);
    *(momentum_var.mutable_variable_conf()->mutable_initializer()) =
...
REGISTER_OPTIMIZER(NormalModelUpdateOpUserConf::kMomentumConf,
