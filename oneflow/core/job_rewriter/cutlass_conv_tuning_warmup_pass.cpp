...
Maybe<void> CutlassConvTuningWarmupPass::Apply(Job* job, JobPassCtx* ctx) const {
  ...
  op_graph.ForEachNode([&](const OpNode* node) {
    ...
    if (in_desc.data_type() != DataType::kFloat16 && in_desc.data_type() != DataType::kInt8) {
      return;
    }
    ...
    if (in_desc.data_type() == DataType::kFloat16) {
      ...
      operation = CutlassConvTuner::Get().FindConv2dOperation(
          stream->As<ep::CudaStream>(), key, configuraion, arguments, workspace, kMaxWorkspaceSize);
    } else if (in_desc.data_type() == DataType::kInt8) {
      cutlass::library::ConvFunctionalKey key(
          cutlass::library::Provider::kCUTLASS, cutlass::library::ConvKind::kFprop,
      ...
      UNIMPLEMENTED() << "Only support float16 and int8 conv2d";
    }
