...
OperatorConf GenerateAdamHelperVariableOpConf(const VariableOp &op,
...
void GenerateOptimizerOpConf(const VariableOp &op,
  ...
  *(mdupdt_op_conf->mutable_user_conf()) =
  ...
  const AdamModelUpdateConf &adam_conf =
      mdupdt_op_conf->user_conf().adam_conf();
  if (adam_conf.do_bias_correction()) {
    beta1_t_var =
        GenerateAdamHelperVariableOpConf(op, "beta1_t", adam_conf.beta1());
    SetScalarShapeAndSbpConf(&beta1_t_var);
    beta2_t_var =
        GenerateAdamHelperVariableOpConf(op, "beta2_t", adam_conf.beta2());
...
REGISTER_OPTIMIZER(NormalModelUpdateOpUserConf::kAdamConf,
