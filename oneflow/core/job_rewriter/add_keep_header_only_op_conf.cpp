  ...
  op_graph.TopoForEachNode([&](OpNode *node) {
    ...
    for (const auto &pair : src_node2ibns) {
      ...
      for (const std::string &ibn : cur_ibns) {
        ...
        op_conf.set_name("SrcOp-" + src_node->op().op_name() + "-SrcBlob-" +
                         cur_lbi.blob_name() + "-DstOp-" +
                         node->op().op_name() + "-DstBlob-" + ibn +
                         "-keep_header_only");
        KeepHeaderOnlyOpConf *kho_conf =
            op_conf.mutable_keep_header_only_conf();
        ...
        *(kho_conf->mutable_out()->Add()) = cur_lbi.blob_name();
        ...
        ReplaceInputLbnInOpCustomizedConf(dst_op_type_conf, ibn,
                                          GenLogicalBlobName(cur_lbi), lbn);
