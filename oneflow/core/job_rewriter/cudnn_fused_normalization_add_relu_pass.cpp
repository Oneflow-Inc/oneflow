...
public:
  ...
  bool IsEnabled() const override {
    if (GlobalJobDesc()
            .job_conf()
            .has_enable_cudnn_fused_normalization_add_relu()) {
      bool enabled = GlobalJobDesc()
                         .job_conf()
                         .enable_cudnn_fused_normalization_add_relu();
      CHECK(!enabled || IsFusedBnAddReluSupported())
          << "Option 'enable_cudnn_fused_normalization_add_relu' is only "
             "supported when cuDNN "
             "version >= 7.4.1";
      return enabled;
    } else {
...
Maybe<void>
CudnnFusedNormalizationAddReluPass::Apply(const OpGraph &op_graph,
  ...
  op_graph.ForEachNode([&](const OpNode *op_node) {
    ...
    if (op_type_name != "normalization_add_relu" &&
        op_type_name != "normalization_add_relu_grad") {
      return;
    }
    const user_op::UserOpConfWrapper user_op_conf(op_conf);
    const BlobDesc &x_desc = op_node->LogicalBlobDesc4Lbi(
        GenLogicalBlobId(user_op_conf.input("x", 0)));
    const int32_t axis = user_op_conf.attr<int32_t>("axis");
    if (x_desc.data_type() != DataType::kFloat16) {
      return;
    }
    ...
    if (x_shape.At(axis) % 4 != 0) {
      return;
    }
    OperatorConf new_op_conf = op_conf;
    new_op_conf.mutable_user_conf()->set_op_type_name("cudnn_fused_" +
                                                      op_type_name);
...
REGISTER_FUNCTION_PASS("CudnnFusedNormalizationAddReluPass",
