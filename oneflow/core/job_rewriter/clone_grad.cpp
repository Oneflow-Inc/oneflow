...
void GenerateCloneGradOpIfNeed(
  ...
  for (const auto &obn : op_node.op().output_bns()) {
    out_lbi2out_oba[op_node.op().BnInOp2Lbi(obn)] =
        GenOpBlobArg(op_node.op().op_name(), obn);
  }
  ...
  for (const auto &obn : op_node.op().output_bns()) {
    ...
    std::vector<LogicalBlobId> lbis_to_add =
        std::move(out_oba2in_diff_lbis[oba]);
    ...
    while (lbis_to_add.size() != 1) {
      ...
      user_op::UserOpConfWrapperBuilder add_op_builder(
          op_node.op().op_name() + "_clone_grad_" + NewUniqueId());
      ...
      const auto add_op =
          add_op_builder.Output("out")
              .ScopeSymbolId(
                  job_builder->OpConf4OpName(lbi.op_name()).scope_symbol_id())
              .Build();
      job_builder->AddOps(job_builder->ParallelConf4Lbi(lbi),
                          {add_op.op_conf()});
      lbis_to_add.push_back(GenLogicalBlobId(add_op.output("out", 0)));
    }
    if (need_add) {
      CHECK(out_oba2clone_bw_add_out_lbi->emplace(oba, lbis_to_add.front())
                .second);
    }
