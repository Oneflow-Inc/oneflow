...
std::function<OperatorConf *(const std::string &)>
MakeMutableOperatorConf4OpName(Job *job) {
  auto op_name2op_conf =
      std::make_shared<HashMap<std::string, OperatorConf *>>();
  ...
  return [op_name2op_conf](const std::string &op_name) {
    return op_name2op_conf->at(op_name);
  };
}

void AddIdentityOp(const std::string &op_name, Job *job,
  ...
  TupleIdentityOpConf *tuple_identity_op_conf =
      tuple_identity_op->mutable_tuple_identity_conf();
  int32_t idx = 0;
...
void AddIdentityOpAndReconnect(
    const std::string &op_name_prefix, Job *job,
    const std::vector<OpEdge *> &op_edges,
    const std::function<OperatorConf *(const std::string &)>
        &MutOperatorConf4OpName,
    const ParallelConf &parallel_conf) {
  // add identity op
  HashSet<LogicalBlobId> lbis;
  for (OpEdge *edge : op_edges) {
    lbis.insert(edge->lbis().begin(), edge->lbis().end());
  }
  HashMap<LogicalBlobId, LogicalBlobId> old_lbi2new_lbi;
  HashMap<LogicalBlobId, std::string> old_lbi2new_obn;
  const auto &identity_op_name = op_name_prefix + NewUniqueId();
  AddIdentityOp(identity_op_name, job, lbis, &old_lbi2new_lbi, &old_lbi2new_obn,
                parallel_conf);
  // reconnect to identity op
  HashMap<LogicalBlobId, SbpParallel> old_lbi2sbp_parallel;
  for (OpEdge *edge : op_edges) {
    OperatorConf *op_conf =
        MutOperatorConf4OpName(edge->dst_node()->op().op_name());
    ...
    for (const LogicalBlobId &lbi : edge->lbis()) {
      ...
      std::string identity_out_lbn =
          GenLogicalBlobName(old_lbi2new_lbi.at(lbi));
      for (const std::string &ibn : edge->lbi2ibns().at(lbi)) {
        ReplaceInputLbnInOpCustomizedConf(op_type_conf, ibn, lbn_check,
                                          identity_out_lbn);
  ...
  auto *sbp_sig_conf_map = job->mutable_job_parallel_view_conf()
                               ->mutable_op_name2sbp_signature_conf();
  auto *sbp_parallel_map =
      (*sbp_sig_conf_map)[identity_op_name].mutable_bn_in_op2sbp_parallel();
...
MakePredicatorIsReachableFromAnyVariableOps(const OpGraph &op_graph) {
...
REGISTER_FUNCTION_CONFIG_DEF()
    .Bool("enable_pseudo_chain_merge", false,
          "ties up chain headers unreachable from any variable ops");
class TieUpChainHeadersUnReachableFromAnyVariableOps final
    : public OpGraphPass {
  bool IsEnabled() const override {
    return GlobalJobDesc().IsTrain() &&
           GlobalJobDesc().Bool("enable_pseudo_chain_merge");
  }

  Maybe<void> Apply(const OpGraph &op_graph, Job *job) const override {
    auto IsReachableFromAnyVariableOps =
        MakePredicatorIsReachableFromAnyVariableOps(op_graph);
    ...
    op_graph.ForEachChainFamily([&](const HashSet<OpNode *> &chain_nodes) {
      ...
      // ignore small chain
      if (chain_nodes.size() - source_nodes.size() <= 2) {
        return;
      }
      AddIdentityOpAndReconnect(
          "pseudo_chain_header_", job, source_edges, MutOperatorConf4OpName,
