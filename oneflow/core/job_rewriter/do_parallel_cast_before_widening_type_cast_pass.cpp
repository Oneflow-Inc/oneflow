...
public:
  ...
  void Put(const OperatorConf &op_conf) {
    _op_confs_to_update[op_conf.name()] = op_conf;
  }
  std::vector<OperatorConf> op_confs() {
    std::vector<OperatorConf> ret;
    for (const auto &x : _op_confs_to_update) {
...
Maybe<void>
DoParallelCastBeforeWideningTypeCast::Apply(const OpGraph &op_graph,
  ...
  op_graph.ForEachNode([&op_conf_cache](OpNode *parallel_cast_node) {
    ...
    if (!parallel_cast_op_conf.has_parallel_cast_conf()) {
      return;
    }
    auto *cast_node = parallel_cast_node->SoleInEdge()->src_node();
    if (cast_node->out_edges().size() != 1) {
      return;
    }
    auto cast_op_conf = op_conf_cache.GetLatest(cast_node->op().op_conf());
    if (!(cast_op_conf.has_user_conf() &&
          cast_op_conf.user_conf().op_type_name() == "cast")) {
      return;
    }
    ...
    const auto cast_in_dtype =
        cast_node->LogicalBlobDesc4Lbi(cast_in_lbi).data_type();
    const auto cast_out_dtype = cast_conf_wrapper.attr<DataType>("dtype");
    if (!(cast_in_dtype == DataType::kFloat16 &&
          (cast_out_dtype == DataType::kFloat ||
           cast_out_dtype == DataType::kDouble))) {
      return;
    }
    ...
    // replace parallel_cast op input with cast op input
    {
      ...
      const std::string parallel_cast_input =
          parallel_cast_op_conf.parallel_cast_conf().in();
      ReplaceInputLbnInOpCustomizedConf(
          new_parallel_cast_op_conf.mutable_parallel_cast_conf(), "in",
          parallel_cast_input, cast_input);
    ...
    // replace cast op input with parallel_cast op output
    {
      ...
      const std::string parallel_cast_output =
          parallel_cast_op_conf.name() + "/" +
          parallel_cast_op_conf.parallel_cast_conf().out();
      const std::string cast_input = cast_conf_wrapper.input("in", 0);
      ReplaceInputLbnInOpCustomizedConf(new_cast_op_conf.mutable_user_conf(),
                                        "in_0", cast_input,
    ...
    for (OpEdge *edge : parallel_cast_node->out_edges()) {
      ...
      OperatorConf dst_op_conf =
          op_conf_cache.GetLatest(dst_node->op().op_conf());
