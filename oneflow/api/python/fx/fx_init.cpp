...
PyObject* replacement_method(PyObject* self, PyObject* args, PyObject* kwargs) {
  ...
  PyCFunctionObject* patch_method_c = ((PyCFunctionObject*)to_restore->original_fn);
  ...
  PyObject* args_ = Py_BuildValue("(OOO)", to_restore->original_fn, args, kwargs);
  if (!args_) { return nullptr; }
...
static PyObject* patch_function(PyObject* self, PyObject* args) {
  static PyMethodDef ReplacementMethod = {"replace", (PyCFunction)(void (*)())replacement_method,
                                          METH_VARARGS | METH_KEYWORDS,
                                          "Replaced method implementation."};

  ToRestore to_restore = {};
  if (!PyArg_ParseTuple(args, "OO", &to_restore.original_fn, &to_restore.patch_fn)) {
    return nullptr;
  }
  if (!PyCFunction_Check(to_restore.original_fn)) {
    ...
    if (PyUnicode_Check(obj_repr)) { err << PyUnicode_AS_DATA(obj_repr) << " "; }
...
void initFx(PyObject* module) {
  ...
  static struct PyModuleDef path = {PyModuleDef_HEAD_INIT, "oneflow._C._fx", /* name of module */
  ...
  if (!patch) { throw "pymodule create failed"; }
  if (PyModule_AddObject(module, "_fx", patch) != 0) { throw "add oneflow fx pymodule failed"; }
}
}  // namespace fx
}  // namespace oneflow
