...
#define EXPORT_COPY_DATA_API(T, type_proto)                                                       \
  m.def("OfBlob_CopyToBuffer_" OF_PP_STRINGIZE(T), [](uint64_t of_blob_ptr, py::object array) {   \
    oneflow::NumPyArrayPtr array_ptr(array.ptr());                                                \
    OfBlob_CopyToBuffer_##T(of_blob_ptr, array_ptr);                                              \
  });                                                                                             \
  m.def("OfBlob_CopyFromBuffer_" OF_PP_STRINGIZE(T), [](uint64_t of_blob_ptr, py::object array) { \
    oneflow::NumPyArrayPtr array_ptr(array.ptr());                                                \
