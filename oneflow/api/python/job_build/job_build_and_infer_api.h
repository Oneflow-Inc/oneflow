...
inline std::shared_ptr<oneflow::cfg::ErrorProto> JobBuildAndInferCtx_Open(
...
inline std::shared_ptr<oneflow::cfg::ErrorProto> JobBuildAndInferCtx_Close() {
  return oneflow::JobBuildAndInferCtx_Close().GetDataAndErrorProto();
}

inline std::shared_ptr<oneflow::cfg::ErrorProto> CurJobBuildAndInferCtx_CheckJob() {
  return oneflow::CurJobBuildAndInferCtx_CheckJob().GetDataAndErrorProto();
}

inline std::shared_ptr<oneflow::cfg::ErrorProto> CurJobBuildAndInferCtx_SetJobConf(
...
inline std::shared_ptr<oneflow::cfg::ErrorProto> CurJobBuildAndInferCtx_SetTrainConf(
...
inline std::shared_ptr<oneflow::cfg::ErrorProto> CurJobBuildAndInferCtx_Complete() {
  return oneflow::CurJobBuildAndInferCtx_Complete().GetDataAndErrorProto();
}

inline std::pair<bool, std::shared_ptr<oneflow::cfg::ErrorProto>>
CurJobBuildAndInferCtx_HasJobConf() {
...
inline std::shared_ptr<oneflow::cfg::ErrorProto>
CurJobBuildAndInferCtx_AddLbiAndDiffWatcherUuidPair(const std::string& lbi_uuid_pair) {
...
inline std::pair<long long, std::shared_ptr<oneflow::cfg::ErrorProto>>
JobBuildAndInferCtx_GetDataType(const std::string& job_name, const std::string& lbn) {
  return oneflow::JobBuildAndInferCtx_GetDataType(job_name, lbn).GetDataAndErrorProto(0LL);
}

inline std::pair<bool, std::shared_ptr<oneflow::cfg::ErrorProto>> JobBuildAndInferCtx_IsDynamic(
    const std::string& job_name, const std::string& lbn) {
  return oneflow::JobBuildAndInferCtx_IsDynamic(job_name, lbn).GetDataAndErrorProto(false);
}

inline std::pair<bool, std::shared_ptr<oneflow::cfg::ErrorProto>> JobBuildAndInferCtx_DisableBoxing(
    const std::string& job_name, const std::string& lbn) {
  return oneflow::JobBuildAndInferCtx_DisableBoxing(job_name, lbn).GetDataAndErrorProto(false);
}

inline std::pair<bool, std::shared_ptr<oneflow::cfg::ErrorProto>> JobBuildAndInferCtx_IsTensorList(
    const std::string& job_name, const std::string& lbn) {
  return oneflow::JobBuildAndInferCtx_IsTensorList(job_name, lbn).GetDataAndErrorProto(false);
}

inline std::pair<std::string, std::shared_ptr<oneflow::cfg::ErrorProto>>
JobBuildAndInferCtx_GetBatchAxis(const std::string& job_name, const std::string& lbn) {
...
inline std::shared_ptr<oneflow::cfg::ErrorProto> CurJobBuildAndInferCtx_AddLossLogicalBlobName(
    const std::string& lbn) {
  return oneflow::CurJobBuildAndInferCtx_AddLossLogicalBlobName(lbn).GetDataAndErrorProto();
}

inline std::pair<bool, std::shared_ptr<oneflow::cfg::ErrorProto>>
JobBuildAndInferCtx_IsMirroredBlob(const std::string& job_name, const std::string& lbn) {
  return oneflow::JobBuildAndInferCtx_IsMirroredBlob(job_name, lbn).GetDataAndErrorProto(false);
}

inline std::pair<int, std::shared_ptr<oneflow::cfg::ErrorProto>>
