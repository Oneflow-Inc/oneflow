...
ONEFLOW_FUNCTION_LIBRARY(m) {
  ...
  m.add_functor("DispatchImageDecoderRandomCropResize",
    ...
    auto& attrs = THREAD_CACHED_MUTABLE_ATTR_MAP({"target_width", "target_height", "seed", "num_workers", "max_num_pixels",
  ...
  m.add_functor("DispatchCOCOReader",
      ...
      auto& attrs = THREAD_CACHED_MUTABLE_ATTR_MAP({"session_id", "annotation_file", "image_dir", "batch_size",
        ...
        attrs.SetAttr("remove_images_without_annotations", remove_images_without_annotations);
  ...
  m.add_functor("DispatchCOCOReader",
        ...
        m.add_functor("DispatchImageBatchAlign",
                      [](const std::shared_ptr<OpExpr>& op, const std::shared_ptr<Tensor>& input,
                         int32_t alignment, const Shape& shape, const Symbol<DType>& data_type,
                         bool dynamic_out) -> Maybe<Tensor> {
                        auto& attrs = THREAD_CACHED_MUTABLE_ATTR_MAP("shape", "data_type",
                                                                     "alignment", "dynamic_out");
                        attrs.SetAttr("shape", shape);
                        attrs.SetAttr("data_type", data_type->data_type());
  ...
  m.add_functor(
      ...
      [](const std::shared_ptr<OpExpr>& op, const std::vector<std::string>& files,
         ...
         const std::vector<Symbol<SbpParallel>>& sbp_tuple) -> Maybe<Tensor> {
          auto& attrs = THREAD_CACHED_MUTABLE_ATTR_MAP({"files", "batch_size", "random_shuffle", "shuffle_mode", "shuffle_buffer_size",
  ...
  m.add_functor(
      ...
      [](const std::shared_ptr<OpExpr>& op, const std::string& data_file_prefix, int64_t seq_length,
         ...
         int64_t random_seed, const Optional<Symbol<Device>>& device) -> Maybe<Tensor> {
            auto& attrs = THREAD_CACHED_MUTABLE_ATTR_MAP({"data_file_prefix", "seq_length", "label_length", "num_samples", "batch_size", "dtype",
             ...
             "split_sizes", "split_index", "shuffle", "random_seed");
              ...
              m.add_functor("DispatchRmspropUpdate",
                            [](const std::shared_ptr<OpExpr>& op, const TensorTuple& inputs,
                              ...
                              auto& attrs = THREAD_CACHED_MUTABLE_ATTR_MAP(
                                  "learning_rate_val", "scale", "l1", "l2", "centered", "epsilon",
  ...
  m.add_functor(
      ...
      [](const std::shared_ptr<OpExpr>& op, const TensorTuple& inputs, float learning_rate,
         ...
         bool do_bias_correction) -> Maybe<void> {
                auto& attrs = THREAD_CACHED_MUTABLE_ATTR_MAP({"learning_rate_val", "bias_correction1_val", "bias_correction2_val", "scale", "l1",
             "l2", "beta1", "beta2", "epsilon", "weight_decay", "amsgrad", "do_bias_correction");
                ...
                m.add_functor(
                    ...
                    [](const std::shared_ptr<OpExpr>& op, const TensorTuple& inputs,
                      ...
                      auto& attrs = THREAD_CACHED_MUTABLE_ATTR_MAP(
                          "learning_rate_val", "scale", "l1", "l2", "lr_decay", "weight_decay",
                ...
                m.add_functor("DispatchMomentumUpdate",
                              [](const std::shared_ptr<OpExpr>& op, const TensorTuple& inputs,
                                 float learning_rate, double scale, float l1, float l2, float beta,
                                 float dampening, bool nesterov, bool maximize,
                                 float weight_decay) -> Maybe<void> {
                                auto& attrs = THREAD_CACHED_MUTABLE_ATTR_MAP(
                                    "learning_rate_val", "scale", "l1", "l2", "beta", "dampening",
                                    "nesterov", "maximize", "weight_decay");
                                attrs.SetAttr("learning_rate_val", learning_rate);
                                attrs.SetAttr("scale", scale);
                                attrs.SetAttr("l1", l1);
                                attrs.SetAttr("l2", l2);
                ...
                m.add_functor(
                    ...
                    [](const std::shared_ptr<OpExpr>& op, const TensorTuple& inputs,
                      ...
                      auto& attrs = THREAD_CACHED_MUTABLE_ATTR_MAP(
                          "learning_rate_val", "bias_correction1_val", "bias_correction2_val",
                          "scale", "l1", "l2", "beta1", "beta2", "epsilon", "weight_decay",
                          "do_bias_correction");
                      attrs.SetAttr("learning_rate_val", learning_rate);
                      attrs.SetAttr("bias_correction1_val", bias_correction1);
                      attrs.SetAttr("bias_correction2_val", bias_correction2);
                ...
                m.add_functor("DispatchFtrlUpdate",
                              [](const std::shared_ptr<OpExpr>& op, const TensorTuple& inputs,
                                 ...
                                 float weight_decay) -> Maybe<void> {
                                auto& attrs = THREAD_CACHED_MUTABLE_ATTR_MAP(
                                    "learning_rate_val", "scale", "l1", "l2", "lr_power", "lambda1",
                ...
                m.add_functor("DispatchAdadeltaUpdate",
                              [](const std::shared_ptr<OpExpr>& op, const TensorTuple& inputs,
                                 float learning_rate, double scale, float l1, float l2, float rho,
                                 float epsilon, bool maximize, float weight_decay) -> Maybe<void> {
                                auto& attrs = THREAD_CACHED_MUTABLE_ATTR_MAP(
                                    "learning_rate_val", "scale", "l1", "l2", "rho", "epsilon",
  ...
  m.add_functor("DispatchRawReader",
                [](const std::shared_ptr<OpExpr>& op, const std::vector<std::string>& files,
                   ...
                   const std::vector<Symbol<SbpParallel>>& sbp_tuple) -> Maybe<Tensor> {
                  auto& attrs = THREAD_CACHED_MUTABLE_ATTR_MAP({"files", "shape", "data_type", "batch_size", "random_shuffle",
                       "shuffle_block_size", "seed", "nd_sbp");
                    attrs.SetAttr<std::vector<std::string>>("files", files);
                    attrs.SetAttr<Shape>("shape", shape);
                    attrs.SetAttr<DataType>("data_type", data_type->data_type());
