...
ONEFLOW_FUNCTION_LIBRARY(m) {
  ...
  m.add_functor(
      "DispatchAdamaxUpdate",
      [](const std::shared_ptr<OpExpr>& op, const TensorTuple& inputs, float learning_rate,
         double scale, float l1, float l2, float weight_decay, float beta1, float beta2,
         float bias_correction1_val, float epsilon, bool do_bias_correction) -> Maybe<void> {
        auto& attrs = THREAD_CACHED_MUTABLE_ATTR_MAP(
            "learning_rate_val", "scale", "l1", "l2", "weight_decay", "beta1", "beta2",
            "bias_correction1_val", "epsilon", "do_bias_correction");
