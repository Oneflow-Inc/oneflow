...
ONEFLOW_FUNCTION_LIBRARY(m) {
  ...
  //   [](const std::shared_ptr<OpExpr>& op, const std::shared_ptr<Tensor> input, int64_t
  //   target_width, int64_t target_height,
  //      int64_t seed, int64_t num_workers, int64_t max_num_pixels, float random_area_min, float
  //      random_area_max, float random_aspect_ratio_min, float random_aspect_ratio_max, int64_t
  //      warmup_size, int64_t num_attempts) -> Maybe<Tensor> {
  //     auto ctx = std::make_shared<OfrecordImageDecoderRandomCropOpInterpCtx>();
  //     ctx->name = name;
  //     ctx->color_space = color_space;
  //     ctx->num_attempts = num_attempts;
