...
ONEFLOW_FUNCTION_LIBRARY(m) {
  ...
  m.add_functor(
    ...
    [](const std::shared_ptr<OpExpr>& op, const std::shared_ptr<Tensor>& input, const std::string name, 
       const Shape& shape, const Symbol<DType>& data_type, bool dim1_varying_length, bool truncate) -> Maybe<Tensor> {
  ...
  m.add_functor(
    "DispatchCoinFlip",
    [](const std::shared_ptr<OpExpr>& op, int64_t batch_size, Scalar probability, int64_t seed, 
       bool has_seed, const Symbol<ParallelDesc>& placement,
         const std::vector<Symbol<cfg::SbpParallel>>& sbp_tuple) -> Maybe<Tensor> {
         auto ctx = std::make_shared<CoinFlipOpInterpCtx>();
         ctx->probability = JUST(probability.As<float>());
         ctx->batch_size = batch_size;
  ...
  m.add_functor(
    ...
    [](const std::shared_ptr<OpExpr>& op, const TensorTuple& input, int64_t crop_h, 
       ...
       const std::vector<float>& std, const Symbol<DType>& output_dtype, const std::string output_layout,
       const std::string color_space) -> Maybe<Tensor> {
  ...
  m.add_functor(
    ...
    [](const std::shared_ptr<OpExpr>& op, const TensorTuple& input, int64_t crop_h, int64_t crop_w, float crop_pos_x,
       float crop_pos_y, const std::vector<float>& mean,
       const std::vector<float>& std, const Symbol<DType>& output_dtype, const std::string output_layout,
       const std::string color_space) -> Maybe<Tensor> {
  ...
  m.add_functor(
    "DispatchOfrecordImageDecoderRandimCrop",
    [](const std::shared_ptr<OpExpr>& op, const std::shared_ptr<Tensor>& input, const std::string name, 
       const std::string color_space, const std::vector<float>& random_area, 
       const std::vector<float>& random_aspect_ratio, int32_t num_attempts, 
       int64_t seed, bool has_seed) -> Maybe<Tensor> {
  ...
  m.add_functor(
    ...
    [](const std::shared_ptr<OpExpr>& op, const std::shared_ptr<Tensor> input, const std::vector<Shape>& out_shapes,
       const std::vector<Symbol<DType>>& out_dtypes, bool dynamic_out) -> Maybe<TensorTuple> {
  ...
  m.add_functor(
    ...
    [](const std::shared_ptr<OpExpr>& op, const std::shared_ptr<Tensor> input, int32_t target_size, int32_t min_size, 
       int32_t max_size, bool resize_longer, const std::string interpolation_type) -> Maybe<TensorTuple> {
  ...
  m.add_functor(
    ...
    [](const std::shared_ptr<OpExpr>& op, const std::shared_ptr<Tensor> input, int64_t target_width,
       int64_t target_height, int64_t channels, const Symbol<DType>& data_type,
  ...
  m.add_functor(
    ...
    [](const std::shared_ptr<OpExpr>& op, const std::shared_ptr<Tensor> input, const std::vector<float>& mean,
       const std::vector<float>& std) -> Maybe<Tensor> {
  ...
  m.add_functor(
    "DispatchCOCOReader",
    [](const std::shared_ptr<OpExpr>& op, const std::string image_dir, const std::string annotation_file,
       int64_t batch_size, bool shuffle_after_epoch, int64_t random_seed, bool group_by_ratio,
       bool remove_images_without_annotations, bool stride_partition, int64_t session_id,
