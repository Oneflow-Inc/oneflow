...
*/
#ifndef ONEFLOW_API_PYTHON_OP_OP_MGR_HELPER_H_
#define ONEFLOW_API_PYTHON_OP_OP_MGR_HELPER_H_
...
Maybe<bool> IsInterfaceOpTypeCase(int64_t op_type_case) {
  return oneflow::IsClassRegistered<int32_t, oneflow::IsInterfaceOpConf4OpTypeCase>(op_type_case);
}

Maybe<long long> GetOpParallelSymbolId(const std::string& op_conf_str) {
  OperatorConf op_conf;
  CHECK_OR_RETURN(TxtString2PbMessage(op_conf_str, &op_conf)) << "OperatorConf parse failed";
  CHECK_OR_RETURN(op_conf.has_scope_symbol_id());
...
#endif  // ONEFLOW_API_PYTHON_OP_OP_MGR_HELPER_H_
