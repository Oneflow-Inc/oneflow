...
N_BLOCKS = 9  # 生成器的 resnet block 数量
POOL_SIZE = 50
LR = 0.0001  # learning rate
BETA1 = 0.5  # Adam 的参数 beta1
BETA2 = 0.9  # Adam 的参数 beta2
LAMBDA_A = 1  # (A -> B -> A) 的损失权重
LAMBDA_B = 1  # (B -> A -> B) 的损失权重
LAMBDA_IDENTITY = 0.5

class CycleGANModel:
    def __init__(self):
        self.device = "cuda"
        self.netG_A = ResnetGenerator(n_blocks=N_BLOCKS).to(self.device)
        self.netG_B = ResnetGenerator(n_blocks=N_BLOCKS).to(self.device)

        self.netD_A = NLayerDiscriminator().to(self.device)
        self.netD_B = NLayerDiscriminator().to(self.device)
