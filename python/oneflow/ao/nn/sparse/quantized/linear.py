...
import oneflow
from oneflow.ao.nn.sparse.quantized.utils import LinearBlockSparsePattern
from oneflow.nn.quantized.modules.utils import _quantize_weight, hide_packed_params_repr

# TODO (zaf): Inherit from `quantized.LinearPackedParams` (T83294430)
class LinearPackedParams(oneflow.nn.Module):
    _version = 1

    def __init__(self, row_block_size=1, col_block_size=4, dtype=oneflow.qint8):
        super().__init__()
        self.prepack_op = oneflow.ops.sparse.qlinear_prepack
        self.unpack_op = oneflow.ops.sparse.qlinear_unpack

        if dtype != oneflow.qint8:
            raise NotImplementedError("Linear prepacking only supports QINT8")
        self.dtype = dtype
        wq = oneflow._empty_affine_quantized([1, 1], scale=1.0, zero_point=0, dtype=oneflow.qint8)
        self.set_weight_bias(wq, None, row_block_size, col_block_size)
        # Hack to make oneflow.jit.script/oneflow.jit.load work
        # Once we have self.unpack_op working we wont need this.
        self.__annotations__['bias'] = Optional[oneflow.Tensor]

    def _get_name(self):
        return "SparseQuantizedLinearPackedParams"

    @oneflow.jit.export
    def set_weight_bias(self, weight: oneflow.Tensor, bias: Optional[oneflow.Tensor],
...
class Linear(oneflow.nn.Module):
    r"""
    A quantized sparse linear module with quantized tensor as inputs and outputs.
    """
    _version = 1
    _FLOAT_MODULE = oneflow.nn.Linear

    def __init__(self, in_features, out_features, row_block_size, col_block_size, bias=True, dtype=oneflow.qint8):
        super().__init__()
        ...
        if bias:
            bias = oneflow.zeros(self.out_features, dtype=oneflow.float)
        else:
            bias = None

        qweight = oneflow._empty_affine_quantized([out_features, in_features],
                                                scale=1, zero_point=0, dtype=oneflow.qint8)
    ...
    def forward(self, x: oneflow.Tensor) -> oneflow.Tensor:
        return oneflow.ops.sparse.qlinear(x, self._packed_params._packed_params, self.scale, self.zero_point)

    def _save_to_state_dict(self, destination, prefix, keep_vars):
        super()._save_to_state_dict(destination, prefix, keep_vars)
        destination[prefix + 'scale'] = oneflow.tensor(self.scale)
        destination[prefix + 'zero_point'] = oneflow.tensor(self.zero_point)
    ...
    def set_weight_bias(self, w: oneflow.Tensor, b: Optional[oneflow.Tensor],
                        row_block_size: Optional[int], col_block_size: Optional[int]) -> None:
        assert row_block_size is not None and col_block_size is not None
    ...
    def from_float(cls, mod):
        ...
        assert dtype == oneflow.qint8, 'Weight observer must have dtype oneflow.qint8'
        w_sc, w_zp = weight_post_process.calculate_qparams()
        if isinstance(w_zp, oneflow.Tensor):
            assert not oneflow.any(w_zp.bool()), "All weight zero points must map to 0"
        else:
