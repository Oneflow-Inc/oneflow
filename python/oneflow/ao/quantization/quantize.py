...
import oneflow.nn.quantized as nnq
from oneflow.nn.intrinsic import _FusedModule

from oneflow.quantization.quantization_mappings import (
...
from oneflow.quantization.stubs import DeQuantStub, QuantWrapper
from oneflow.quantization.qconfig import (
...
def is_activation_post_process(module):
    return (isinstance(module, oneflow.quantization.ObserverBase) or
            isinstance(module, oneflow.quantization.FakeQuantizeBase))
...
def _propagate_qconfig_helper(module, qconfig_dict, allow_list=None,
    ...
    oneflow.quantization.qconfig.assert_valid_qconfig(module_qconfig, module)
...
def add_observer_(module, qconfig_propagation_list=None, non_leaf_module_list=None, device=None, custom_module_class_mapping=None):
    ...
    if len(module._modules) == 0 and not isinstance(module, oneflow.nn.Sequential) \
...
def prepare(model, inplace=False, allow_list=None,
    ...
    """
    oneflow._C._log_api_usage_once("quantization_api.quantize.prepare")
...
def quantize(model, run_fn, run_args, mapping=None, inplace=False):
    ...
    Return:
        Quantized model.
    """
    oneflow._C._log_api_usage_once("quantization_api.quantize.quantize")
    if mapping is None:
...
def quantize_dynamic(model, qconfig_spec=None, dtype=oneflow.qint8,
    ...
    """
    oneflow._C._log_api_usage_once("quantization_api.quantize.quantize_dynamic")
    if qconfig_spec is None:
        if dtype == oneflow.qint8:
            qconfig_spec = {
        ...
        if dtype is oneflow.qint8:
            default_qconfig = default_dynamic_qconfig
        elif dtype is oneflow.float16:
            default_qconfig = float16_dynamic_qconfig
        elif dtype is oneflow.quint8:
...
def prepare_qat(model, mapping=None, inplace=False):
    ...
    Args:
        ...
        inplace: carry out model transformations in-place, the original module
                 is mutated
    """
    oneflow._C._log_api_usage_once("quantization_api.quantize.prepare_qat")
    if mapping is None:
...
def quantize_qat(model, run_fn, run_args, inplace=False):
    ...
    Return:
        Quantized model.
    """
    oneflow._C._log_api_usage_once("quantization_api.quantize.quantize_qat")
    if not inplace:
        model = copy.deepcopy(model)
    model.train()
...
def convert(
    ...
    """
    oneflow._C._log_api_usage_once("quantization_api.quantize.convert")
    if not inplace:
