...
class MultiTableEmbedding(Module):
    ...
    def __init__(
        ...
        if isinstance(embedding_dim, (list, tuple)):
            self.column_dims = embedding_dim
        ...
        key_value_store_options["storage_dim"] = scale_factor * self.embedding_dim
        ...
        if tables is not None:
            ...
            for table in tables:
                if table.__contains__("columns"):
                    ...
                    assert len(columns) == len(self.column_dims)
        ...
        embedding_tables["column_dims"] = self.column_dims
