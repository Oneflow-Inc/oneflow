...
from oneflow.compatible.single_client.core.job.lbi_diff_watcher_info_pb2 import LbiAndDiffWatcherUuidPair
...
def Watch(blob_watched: oneflow._oneflow_internal.BlobDesc, handler_or_prompt: Optional[Union[Callable, str]]=None) -> None:
...
def WatchDiff(blob_watched: oneflow._oneflow_internal.BlobDesc, handler_or_prompt: Optional[Union[Callable, str]]=None) -> None:
...
def _MakeSubConsistentBlobHandlers(blob_watched, handler):
    ...
    return [_WrapperHandler4ParallelIdAndLocalBlob(i, handler4parallel_id_and_local_blob) for i in range(len(blob_watched.sub_consistent_blob_list))]
...
def _MakeHandler4ParallelIdAndLocalBlob(blob_watched, handler):
    ...
    def HandlerParallelIdAndLocalBlob(parallel_id, local_blob):
        ...
        if len(parallel_id2consistent_local_blob) != len_sub_remote_blobs:
            return
        local_blob_list = [parallel_id2consistent_local_blob[parallel_id] for i in range(len_sub_remote_blobs)]
