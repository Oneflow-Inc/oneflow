...
def CropMirrorNormalize(input_blob: oneflow._oneflow_internal.BlobDesc, mirror_blob: Optional[oneflow._oneflow_internal.BlobDesc]=None, color_space: str='BGR', output_layout: str='NCHW', crop_h: int=0, crop_w: int=0, crop_pos_y: float=0.5, crop_pos_x: float=0.5, mean: Sequence[float]=[0.0], std: Sequence[float]=[1.0], output_dtype: flow.dtype=flow.float, name: Optional[str]=None) -> oneflow._oneflow_internal.BlobDesc:
    ...
    if input_blob.dtype is flow.tensor_buffer:
        ...
        print('ERROR! oneflow.compatible.single_client.data.crop_mirror_normalize op', ' NOT support input data type : ', input_blob.dtype)
...
def image_batch_align(images: oneflow._oneflow_internal.BlobDesc, shape: Sequence[int], dtype: flow.dtype, alignment: int, name: Optional[str]=None) -> oneflow._oneflow_internal.BlobDesc:
    ...
    op = flow.user_op_builder(name).Op('image_batch_align').Input('in', [images]).Output('out').Attr('shape', shape).Attr('data_type', dtype).Attr('alignment', alignment).Build()
    return op.InferAndTryRun().SoleOutputBlob()
