...
def api_distribute_clone(x: oneflow._oneflow_internal.BlobDesc, name: Optional[str]=None) -> Tuple[oneflow._oneflow_internal.BlobDesc]:
...
def api_distribute_add(xs: Sequence[oneflow._oneflow_internal.BlobDesc], name: Optional[str]=None) -> oneflow._oneflow_internal.BlobDesc:
...
def api_distribute_split(x: oneflow._oneflow_internal.BlobDesc, axis: int=0, name: Optional[str]=None) -> Tuple[oneflow._oneflow_internal.BlobDesc]:
...
def api_distribute_concat(xs: Sequence[oneflow._oneflow_internal.BlobDesc], axis: int=0, name: Optional[str]=None) -> oneflow._oneflow_internal.BlobDesc:
...
def api_distribute_map(xs: Union[Sequence[oneflow._oneflow_internal.BlobDesc], oneflow._oneflow_internal.BlobDesc], f: Callable[[oneflow._oneflow_internal.BlobDesc, oneflow._oneflow_internal.BlobDesc], oneflow._oneflow_internal.BlobDesc], axis: int=0) -> Tuple[oneflow._oneflow_internal.BlobDesc]:
...
def distribute_map(xs, f, axis=0):
    ...
    output_is_not_container = all([isinstance(x, oneflow._oneflow_internal.ConsistentBlob) for x in results])
...
def cast_to_current_logical_view(x: oneflow._oneflow_internal.BlobDesc) -> oneflow._oneflow_internal.BlobDesc:
    if isinstance(x, oneflow._oneflow_internal.ConsistentBlob) and flow.scope.mirrored_view_enabled() or (isinstance(x, oneflow._oneflow_internal.MirroredBlob) and flow.scope.consistent_view_enabled()):
        x = flow.identity(x)
    return x
...
def _AssertInputOrOutput(xs):
    ...
    if isinstance(xs, (list, tuple)):
        assert len(xs) > 0
        assert all([isinstance(x, oneflow._oneflow_internal.ConsistentBlob) for x in xs])
...
def _EachMachineIdAndDeviceId(parallel_desc_symbol):
    for (machine_id, device_id_list) in parallel_desc_symbol.machine_id2device_id_list.items():
