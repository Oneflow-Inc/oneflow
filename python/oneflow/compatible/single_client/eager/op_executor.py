from oneflow.compatible.single_client.core.operator import op_node_signature_pb2 as op_node_signature_pb
...
from oneflow.compatible.single_client.python.eager import blob_register as blob_register_util
from oneflow.compatible.single_client.python.eager import symbol_storage as symbol_storage
...
from oneflow.compatible.single_client.python.framework import python_callback as python_callback
from oneflow.compatible.single_client.python.experimental import name_scope as name_scope
...
def OpKernelCall(opkernel_object, op_attribute, blob_register):
    ...
    def BuildInstruction(builder):
        with blob_register_util.BnInOp2BlobObjectScope(blob_register, op_attribute) as bn_in_op2blob_object:
            cfg_op_attribute = oneflow._oneflow_internal.deprecated.MakeOpAttributeByString(str(op_attribute))
            builder.StatefulCall(cfg_op_attribute, opkernel_object, bn_in_op2blob_object, boxing_util.BoxingTo)
...
def MirroredCast(op_attribute, blob_register):
    ...
    def BuildInstruction(builder):
        with blob_register_util.BnInOp2BlobObjectScope(blob_register, op_attribute) as bn_in_op2blob_object:
            ...
            op_arg_parallel_attr = oneflow._oneflow_internal.GetOpArgParallelAttribute(parallel_desc_symbol, str(op_attribute), 'out')
            out_blob_object = builder.MakeReferenceBlobObject(in_blob_object, op_arg_parallel_attr)
...
def DistributeSplitOrClone(op_attribute, parallel_conf, blob_register):
    ...
    def GetInBlobObject(builder, ibn, bn_in_op2blob_object):
        ...
        in_op_parallel_desc_sym = oneflow._oneflow_internal.GetPlacementSymbol(parallel_sig[ibn])
        in_op_arg_parallel_attr = oneflow._oneflow_internal.GetOpArgParallelAttribute(in_op_parallel_desc_sym, str(op_attribute), ibn)
        return boxing_util.BoxingTo(builder, origin_blob_object, in_op_arg_parallel_attr)

    def BuildInstruction(builder):
        with blob_register_util.BnInOp2BlobObjectScope(blob_register, op_attribute) as bn_in_op2blob_object:
            physical_out_blob_objects = builder.UnpackLogicalBlobToPhysicalBlobs(GetInBlobObject(builder, 'in', bn_in_op2blob_object))
...
def DistributeConcatOrAdd(op_attribute, parallel_conf, blob_register):
    op_parallel_desc_sym = oneflow._oneflow_internal.GetPlacementSymbol(op_attribute.parallel_signature.op_parallel_desc_symbol_id)
    parallel_size = len(op_attribute.input_bns)
    op_arg_parallel_attr = oneflow._oneflow_internal.GetOpArgParallelAttribute(op_parallel_desc_sym, str(op_attribute), 'out')
    op_arg_blob_attr = oneflow._oneflow_internal.GetOpArgBlobAttribute(str(op_attribute), 'out')
    parallel_sig = op_attribute.parallel_signature.bn_in_op2parallel_desc_symbol_id
    ...
    def BuildInstruction(builder):
        with blob_register_util.BnInOp2BlobObjectScope(blob_register, op_attribute) as bn_in_op2blob_object:
            ...
            bn_in_op2blob_object['out'] = builder.PackPhysicalBlobsToLogicalBlob(in_blob_objects, op_arg_parallel_attr, op_arg_blob_attr)
    oneflow._oneflow_internal.deprecated.LogicalRun(BuildInstruction)
...
def _FindOrCreateVarBlobObject(op_attribute, parallel_conf, blob_register):
    ...
    if var_blob is not None:
        blob_register.SetObject4BlobName(var_blob.logical_blob_name, var_blob.blob_object)
        return
...
def _Watch(op_attribute, parallel_conf, blob_register):
    ...
    if in_blob_object.op_arg_parallel_attr.is_mirrored():
        blob = oneflow._oneflow_internal.EagerMirroredBlob(lbi, in_blob_object, default_blob_register)
    else:
        blob = oneflow._oneflow_internal.EagerConsistentBlob(lbi, in_blob_object, default_blob_register)
...
def _MakeEagerLogicalBlob(op_attribute, obn, blob_register):
    ...
    if mirrored_sig_map[obn].HasField('mirrored_parallel'):
        return oneflow._oneflow_internal.EagerMirroredBlob(lbi, blob_object, default_blob_register)
    else:
        return oneflow._oneflow_internal.EagerConsistentBlob(lbi, blob_object, default_blob_register)
...
def _Assign(var_blob_object, value_blob_object):
    ...
    def BuildAssignInstruction(builder):
        new_parallel_desc_symbol = boxing_util.TryReplaceDeviceTag(builder, var_blob_object.parallel_desc_symbol, 'cpu')
        consumer_op_arg_parallel_attr = oneflow._oneflow_internal.OpArgParallelAttribute(new_parallel_desc_symbol, str(var_blob_object.op_arg_parallel_attr.sbp_parallel), str(var_blob_object.op_arg_parallel_attr.opt_mirrored_parallel))
        tmp_blob_object = boxing_util.BoxingTo(builder, value_blob_object, consumer_op_arg_parallel_attr)
...
def _MakeModelIOPathInputBuilds(op_conf, path, bn_in_op2blob_object):
    ...
    def BuildFeedPathInstruction(builder):
        ...
        builder.FeedBlob(blob_object, python_callback.GetIdForRegisteredCallback(FeedPath))
        builder.InsertRemoveForeignCallbackInstruction(blob_object.object_id, python_callback.GetIdForRegisteredCallback(FeedPath))
...
def _EagerRunModelLoad(var_op_conf, snapshot_path):
    ...
    (BuildModelIOPathInputInstruction, BuildFeedPathInstruction) = _MakeModelIOPathInputBuilds(path_input_op_conf, snapshot_path, path_input_blob_objects)
    ...
    def BuildModelLoadInstruction(builder):
        ...
        op_attribute = op_infer_util.Infer(model_load_op_conf, ibn2blob_object=model_load_blob_objects)
        parallel_conf = path_blob_object.parallel_desc_symbol.parallel_conf
        cfg_op_attribute = oneflow._oneflow_internal.deprecated.MakeOpAttributeByString(str(op_attribute))
        builder.StatelessCall(cfg_op_attribute, parallel_conf, model_load_blob_objects, boxing_util.BoxingTo)
    sess = session_ctx.GetDefaultSession()
    with scope_util.ScopeContext(scope_util.MakeScope(_BuildNotMirroredScope)):
        oneflow._oneflow_internal.deprecated.LogicalRun(BuildModelIOPathInputInstruction)
...
def _EagerRunModelSave(var_blobs, snapshot_path):
    ...
    def BuildModelSaveInstruction(builder):
        ...
        op_attribute = op_infer_util.Infer(model_save_op_conf, ibn2blob_object=model_save_blob_objects)
        parallel_conf = path_blob_object.parallel_desc_symbol.parallel_conf
        cfg_op_attribute = oneflow._oneflow_internal.deprecated.MakeOpAttributeByString(str(op_attribute))
        builder.StatelessCall(cfg_op_attribute, parallel_conf, model_save_blob_objects, boxing_util.BoxingTo)
    sess = session_ctx.GetDefaultSession()
    with scope_util.ScopeContext(scope_util.MakeScope(_BuildNotMirroredScope)):
        oneflow._oneflow_internal.deprecated.LogicalRun(BuildModelIOPathInputInstruction)
        oneflow._oneflow_internal.deprecated.LogicalRun(BuildFeedPathInstruction)
...
def _GenModelIOPathInputOpConfAndRetLbi():
    ...
    blob_conf.data_type = oneflow._oneflow_internal.deprecated.GetProtoDtype4OfDtype(flow.int8)
