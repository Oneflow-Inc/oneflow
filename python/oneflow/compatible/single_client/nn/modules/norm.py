...
class Norm(Module):
    ...
    def _vector_norm(self, x, ord, dim):
        if isinstance(ord, str) and ord in ['fro', 'nuc']:
            ...
            if ord == 0:
                ...
                return flow.experimental.pow(flow.experimental.sum(flow.experimental.pow(flow.experimental.abs(x), ord), dim=dim), 1.0 / ord)
        else:
    ...
    def _matrix_norm(self, x, ord, dim):
        if isinstance(ord, str) and ord in ['fro', 'nuc']:
            ...
            if ord == float('inf'):
                return flow.experimental.max(flow.experimental.sum(flow.experimental.abs(x), dim=1))
            else:
                return flow.experimental.min(flow.experimental.sum(flow.experimental.abs(x), dim=1))
        elif isinstance(ord, int):
            if ord == 1:
                return flow.experimental.max(flow.experimental.sum(flow.experimental.abs(x), dim=0))
            elif ord == -1:
                return flow.experimental.min(flow.experimental.sum(flow.experimental.abs(x), dim=0))
            elif ord == 2:
                raise NotImplementedError
            elif ord == -2:
                raise NotImplementedError
    ...
    def forward(self, x):
        ...
        if self.dim == None and self.ord == None:
            ...
            assert num_axes <= 2, 'input must be 1-D or 2-D when dim is None and ord is not None'
            res = self._vector_norm(x, self.ord, self.dim) if num_axes == 1 else self._matrix_norm(x, self.ord, self.dim)
            ...
            if isinstance(self.dim, int):
                ...
                res = self._vector_norm(x, ord=2 if self.ord == None else self.ord, dim=self.dim)
            else:
                ...
                self.dim = temp
                res = self._matrix_norm(x, ord='fro' if self.ord == None else self.ord, dim=self.dim)
        else:
