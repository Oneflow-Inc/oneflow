        ...
        if has_node_list():
            ...
            if _unittest_worker_initilized == False:
                ...
                if enable_init_by_host_list():
                    ...
                    bootstrap_conf_list = flow.env.init_bootstrap_confs(node_list(), int(master_port), config_world_size, config_rank_ctrl_port, config_node_size)
            ...
            bootstrap_conf_list = flow.env.init_bootstrap_confs(['127.0.0.1'], master_port, config_world_size)
            ...
            for rank in range(1, config_world_size):
                ...
                oneflow_cmd = 'python3 -m oneflow.compatible.single_client --start_worker' + ' --env_proto=' + run_dir + '/log_' + str(rank) + '/' + 'env_proto_' + str(rank) + '.proto'
                subprocess.Popen(oneflow_cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True)
                os.remove(env_file.name)
            atexit.register(flow.deprecated.delete_worker_of_multi_process, run_dir=run_dir)
