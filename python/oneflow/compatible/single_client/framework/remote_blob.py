...
from oneflow.compatible.single_client.python.eager import eager_blob_util as eager_blob_util
...
def with_distribute(self, distribute):
    new = type(self)(self.lbi, self.job_name, oneflow._oneflow_internal.distribute.auto())
...
def get_lazy_shape_log_warning(self):
    if flow.scope.mirrored_view_enabled():
        return '%s\n%s\n%s' % ('WARNING:', 'You access a consistent blob shape in mirrored view, there may be problems,', "you should add 'x = flow.cast_to_current_logical_view(x)'.")
    else:
        return ''

def get_mirror_shape_log_warning(self):
    if flow.scope.consistent_view_enabled():
        return '%s\n%s\n%s' % ('WARNING:', 'You access a mirrored blob shape in consistent view, there may be problems,', "you should add 'x = flow.cast_to_current_logical_view(x)'.")
    else:
        return ''
...
def RegisterMethod4LazyConsistentBlob():
    ...
    oneflow._oneflow_internal.LazyConsistentBlob.get_lazy_shape_log_warning = get_lazy_shape_log_warning

def RegisterMethod4LazyMirroredBlob():
    RegisterMethod4BlobDef(oneflow._oneflow_internal.LazyMirroredBlob)
    oneflow._oneflow_internal.LazyMirroredBlob.get_mirror_shape_log_warning = get_mirror_shape_log_warning
...
def BlobObjectNumpy(blob_object, tmp_name=None):
    ...
    def FetchBlobNumpy(blob_object):
        ...
        def BoxingToSingleDevice(builder):
            ...
            tmp_op_arg_parallel_attr = oneflow._oneflow_internal.OpArgParallelAttribute(tmp_parallel_desc_symbol, str(blob_object.op_arg_parallel_attr.sbp_parallel), str(blob_object.op_arg_parallel_attr.opt_mirrored_parallel))
            with flow.scope.placement(parallel_conf.device_tag(), list(parallel_conf.device_name())):
                tmp_blob_object = boxing_util.BoxingTo(builder, blob_object, tmp_op_arg_parallel_attr)
        ...
        return oneflow._oneflow_internal.EagerPhysicalBlob(consistent_blob_name, blob_register, eager_blob_util._GetPhysicalBlobHeaderCache).numpy()
...
def RegisterMethod4EagerBlobTrait():
    oneflow._oneflow_internal.EagerBlobTrait.sub_consistent_blob_list = sub_consistent_blob_list
...
def eager_with_distribute(self, distribute):
    new = type(self)(self.lbi, blob_object=self.blob_object, blob_register=blob_register, job_name=self.job_name, distribute=self.distribute)
    new.set_distribute(distribute)
    return new

def RegisterMethod4EagerConsistentBlob():
    oneflow._oneflow_internal.EagerConsistentBlob.dtype = dtype
    oneflow._oneflow_internal.EagerConsistentBlob.with_distribute = eager_with_distribute
    oneflow._oneflow_internal.EagerConsistentBlob.with_gradient_distribute = with_gradient_distribute