...
from oneflow.compatible.single_client.python.ops import initializer_util as initializer_util
...
from oneflow.compatible.single_client.core.framework import variable_meta_info_pb2 as variable_meta_info_pb
from oneflow.compatible.single_client.core.framework import user_op_attr_pb2 as attr_value_pb
from oneflow.compatible.single_client.python.experimental import interface_op_read_and_write
...
class FileBackendVariableBlob:
    ...
    def __init__(self, var_dir: str, dtype: Optional[flow.dtype]=None, shape: Optional[Sequence[int]]=None):
        ...
        if self.has_meta_info_:
            ...
            self.dtype_ = dtype_util.convert_proto_dtype_to_oneflow_dtype(meta_info.data_type)
    ...
    def numpy(self) -> np.ndarray:
        ...
        return np.fromfile(self.file_path, dtype=dtype_util.convert_oneflow_dtype_to_numpy_dtype(self.dtype)).reshape(self.shape)
ValueContainer = Union[EagerBlobTrait, FileBackendVariableBlob, np.ndarray, 'oneflow.compatible.single_client.Tensor']
...
def _GetCheckpoint(path: str) -> Union[Dict[str, FileBackendVariableBlob], FileBackendVariableBlob]:
...
@session_ctx.try_init_default_session
def GetCheckpoint(path: str) -> Union[Dict[str, FileBackendVariableBlob], FileBackendVariableBlob]:
    """
    Load variable(s) from file system.
    """
    return _GetCheckpoint(path)

def Load(path: str) -> Union[Dict[str, FileBackendVariableBlob], FileBackendVariableBlob]:
    return _GetCheckpoint(path)
...
def _ReadSlice(container: ValueContainer) -> Iterable[Tuple[Sequence[int], Sequence[int], np.ndarray]]:
    """
    ...
    if isinstance(container, flow.Tensor):
        ...
        def ReadFromTensor(tensor, start_nd_idx, stop_nd_idx):
            ...
            return tensor[tuple([slice(start_nd_idx[i], stop_nd_idx[i]) for i in range(len(start_nd_idx))])].numpy()
        ...
        def ReadFromEagerBlob(eager_blob, start_nd_idx, stop_nd_idx):
            ...
            return _LogicalSlice(eager_blob.blob_object, start_nd_idx, stop_nd_idx, scope_symbol_id)
        yield from _ForEachSlice(container, ReadFromEagerBlob)
    elif isinstance(container, FileBackendVariableBlob):
        np_dtype = np.dtype(dtype_util.convert_oneflow_dtype_to_numpy_dtype(container.dtype))
        with open(container.file_path, 'rb') as f:
            ...
            def ReadFromFile(_, start_nd_idx, stop_nd_idx):
                ...
                return np.frombuffer(slice, dtype=np_dtype).reshape(np.array(stop_nd_idx) - np.array(start_nd_idx))
...
def _SaveVarDict(path: str, var_dict: Optional[Dict[str, Union[FileBackendVariableBlob, EagerBlobTrait]]]=None) -> None:
    if var_dict is None:
    ...
    def IsFileOrNonEmptyDir(path):
        ...
        if os.path.isdir(path) and len(os.listdir(path)) != 0:
            return True
        return False
    assert not IsFileOrNonEmptyDir(path), '{} is a file or non-empty directory! Note that flow.save is different from torch.save. It saves each weight as a separated file so that a directory instead of a file should be given.'.format(path)
    os.makedirs(path, exist_ok=True)
    for (name, var) in var_dict.items():
        meta_info = variable_meta_info_pb.VariableMetaInfo()
        meta_info.shape.dim[:] = var.shape
        meta_info.data_type = oneflow._oneflow_internal.deprecated.GetProtoDtype4OfDtype(var.dtype)
...
@session_ctx.try_init_default_session
def SaveVarDict(path: str, var_dict: Optional[Dict[str, Union[FileBackendVariableBlob, EagerBlobTrait]]]=None) -> None:
    """
...
def _LogicalSlice(input_blob_object: oneflow._oneflow_internal.BlobObject, start: Sequence[int], stop: Sequence[int], scope_symbol_id: int) -> np.ndarray:
    """
    ...
    def AsyncSlice(Yield):
        ...
        def build(builder):
            ...
            builder.StatelessCall(cfg_op_attribute, parallel_conf, bn_in_op2blob_object, boxing_util.BoxingTo)
    ...
    blob = oneflow._oneflow_internal.EagerConsistentBlob(lbi, blob_object=blob_object, blob_register=blob_register, job_name=FAKE_JOB_NAME)
    return blob.numpy()

def _GetCpu0VariableBlobFromNumpy(np_array: np.ndarray, dtype: flow.dtype) -> oneflow._oneflow_internal.EagerConsistentBlob:
    """
    ...
    with flow.scope.placement('cpu', '0:0'):
        op_name = id_util.UniqueStr(OP_PREFIX)
        op_conf = get_variable.GenerateVariableOpConf(name=op_name, shape=np_array.shape, dtype=dtype, initializer=initializer_util.zeros_initializer(dtype=dtype), trainable=False)
        ...
        var_blob = get_variable.CreateEagerVariableBlob(op_attribute, job_name=FAKE_JOB_NAME)
        interface_op_read_and_write.FeedValueToInterfaceBlobObject(var_blob.blob_object, np_array)
        return var_blob

def _LogicalSliceAssign(ref_blob_object: oneflow._oneflow_internal.BlobObject, value_blob_object: oneflow._oneflow_internal.BlobObject, start: Sequence[int], stop: Sequence[int], scope_symbol_id: Optional[int]) -> None:
    """
...
def FeedValueToVariable(var_blob: Union[oneflow._oneflow_internal.EagerConsistentBlob, 'oneflow.compatible.single_client.Tensor'], value: ValueContainer, scope_symbol_id: Optional[int]) -> None:
    """
    Feed the value of `value` to the variable `var_blob`
    """
    assert isinstance(value, (EagerBlobTrait, FileBackendVariableBlob, np.ndarray, flow.Tensor)), 'Unknown value type: {}'.format(type(value).__name__)
    if isinstance(value, FileBackendVariableBlob):
        if not value.has_meta_info_:
            value = FileBackendVariableBlob(value.var_dir_, var_blob.dtype, var_blob.shape)
    ...
    assert var_blob.dtype == value_flow_dtype, '{} vs {}'.format(var_blob.dtype, value_flow_dtype)
    ...
    for (start, stop, slice) in _ReadSlice(value):
        ...
        _LogicalSliceAssign(var_blob_object, slice_value_blob.blob_object, start, stop, scope_symbol_id)
...
def _ForEachSlice(container: ValueContainer, f: Union[Callable[[EagerBlobTrait, Sequence[int], Sequence[int]], Any], Callable[[FileBackendVariableBlob, Sequence[int], Sequence[int]], Any], Callable[[np.ndarray, Sequence[int], Sequence[int]], Any]]):
    """
    Slice container into slices whose size < SLICE_BYTES. For every slice,
    yield start_nd_idx, stop_nd_idx and f(slice)
    """
    assert isinstance(container, (EagerBlobTrait, FileBackendVariableBlob, np.ndarray, flow.Tensor)), 'Unknown type: {}'.format(type(container).__name__)
...
def init_by_initializer_conf(var_blob: Union[EagerBlobTrait, 'oneflow.compatible.single_client.Tensor'], initializer_conf: initializer_conf_util.InitializerConf, sync_between_multi_machine: bool, scope_symbol_id: Optional[int], random_seed: int=0):
    ...
    def GenerateValueAndAssign(var_blob, start_nd_idx, stop_nd_idx):
        ...
        _LogicalSliceAssign(var_blob_object, slice_value_blob.blob_object, start_nd_idx, stop_nd_idx, scope_symbol_id)
...
def Init() -> None:
    ...
    for (op_name, var_blob) in GetAllVariables().items():
        ...
        if var_conf.HasField('initialize_with_snapshot'):
            ...
            var_dir = os.path.dirname(os.path.join(initialize_with_snapshot_conf.path, snapshot_key))
            LoadVariables({op_name: GetCheckpoint(var_dir)})
            continue
        scope_symbol_id = _GetScopeSymbolIdFromEagerBlob(var_blob)
        init_by_initializer_conf(var_blob, var_conf.initializer, False, scope_symbol_id, var_conf.random_seed)
    oneflow._oneflow_internal.eager.single_client.Sync()