...
from oneflow.compatible.single_client.python.experimental import enable_typing_check as enable_typing_check
...
def CheckGlobalFunctionAnnotation(signature):
    ...
    if all((p.annotation is not inspect._empty for (_, p) in parameters.items())):
        for (_, p) in parameters.items():
            assert p.kind == inspect._ParameterKind.POSITIONAL_OR_KEYWORD, 'no parameters like *args or **kwargs supported'
...
def CheckGlobalFunctionReturnAnnotation(cls):
    if cls is None:
        ...
        assert cls.__args__ is not None, 'T in oneflow.compatible.single_client.typing.Callback[T] cannot be omitted'
        assert len(cls.__args__) == 1
        _CheckGlobalFunctionReturnAnnotation(cls.__args__[0])
    elif oft.OriginFrom(cls, oft.Bundle):
        assert cls.__args__[0] in (oft.Numpy, oft.ListNumpy), 'T in oneflow.compatible.single_client.typing.Bundle[T] must be one of (oneflow.compatible.single_client.typing.Numpy, oneflow.compatible.single_client.typing.ListNumpy)'
        assert len(cls.__args__) == 1
        _CheckGlobalFunctionReturnAnnotation(cls.__args__[0])
    else:
...
def CheckReturnByAnnotation(function_name, ret, annotation):
    ...
    if annotation is None:
        error_str = '%s does not matched return annotation %s of global_function %s.' % (ret, annotation, function_name)
...
def _CheckReturnByAnnotation(function_name, ret, annotation):
    ...
    if oft.OriginFrom(annotation, typing.Tuple):
        ...
        assert len(ret) == len(annotation.__args__), '%s length compare: %s v.s. %s' % (error_str, len(ret), len(annotation.__args__))
        ...
        assert len(annotation.__args__) == 1, '%s element type in list must be unique' % error_str
        for ret_i in ret:
        ...
        for (key, val) in ret.items():
            assert type(key) is annotation.__args__[0], 'type of %s:%s and %s:%s do not matched return annotation (%s, %s) of global_function %s.' % (key, type(key), val, type(val), annotation.__args__[0], annotation.__args__[1], function_name)
            _CheckReturnByAnnotation(function_name, val, annotation.__args__[1])
    elif oft.OriginFrom(annotation, oft.Numpy):
        assert isinstance(ret, oneflow._oneflow_internal.BlobDesc), 'type(ret): %s' % type(ret)
        assert not ret.is_dynamic, 'only fixed shaped blob compatible to oneflow.compatible.single_client.typing.Numpy. you can change annotation to oneflow.compatible.single_client.typing.ListNumpy '
...
def TransformReturnedBundle(bundle_blob, annotation):
    ...
    if isinstance(bundle_blob, (local_blob_util.LocalBlob,)):
        ...
        return type(bundle_blob)((TransformReturnedBundle(elem, annotation) for elem in bundle_blob))
    elif type(bundle_blob) is dict:
        return {key: TransformReturnedBundle(val, annotation) for (key, val) in bundle_blob.items()}
    else:
        raise NotImplementedError('invalid return  %s : %s found' % (bundle_blob, type(bundle_blob)))
...
def TransformReturnedLocalBlob(local_blob, annotation):
    if oft.OriginFrom(annotation, typing.Tuple):
        ...
        return [TransformReturnedLocalBlob(elem, annotation.__args__[0]) for elem in local_blob]
    elif oft.OriginFrom(annotation, typing.Dict):
        ...
        raise NotImplementedError('invalid watch callback parameter annotation %s found' % annotation)
...
def CheckWatchCallbackParameterAnnotation(parameters):
    ...
    if not oft.OriginFrom(annotation, oft.PyStructCompatibleToBlob):
        raise NotImplementedError('invalid watch callback paremeter annotation %s found. ' % annotation + 'candidate annotations: oneflow.compatible.single_client.typing.Numpy, oneflow.compatible.single_client.typing.ListNumpy. ')

def CheckWatchedBlobByAnnotation(blob, annotation):
    if annotation is inspect._empty:
        return
    if oft.OriginFrom(annotation, oft.Numpy):
        assert not blob.is_dynamic, 'only fixed shaped blob compatible to oneflow.compatible.single_client.typing.Numpy. you can change annotation to oneflow.compatible.single_client.typing.ListNumpy '
    elif oft.OriginFrom(annotation, oft.ListNumpy):
        pass
    else:
        raise NotImplementedError('invalid watch callback parameter annotation %s found' % annotation)
