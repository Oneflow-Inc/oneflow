...
from oneflow.compatible.single_client.core.control import ctrl_bootstrap_pb2 as ctrl_bootstrap_pb
...
def api_grpc_use_no_signal(val: bool=True) -> None:
    ...
    """
    print('WARNING:', 'oneflow.compatible.single_client.env.grpc_use_no_signal is deprecated, users no longer need to set rpc use signal or not. \n', traceback.format_stack()[-2])
    return None
...
def CompleteEnvProto(env_proto, is_multi_client):
    ...
    if env_proto.HasField('ctrl_port') == False:
        if len(env_proto.machine) == 1:
            ...
            raise ValueError("a ctrl_port is required if running multi-node, set it with 'oneflow.compatible.single_client.env.ctrl_port([YOUR PORT])'")
...
def HasAllMultiClientEnvVars():
    return os.getenv('MASTER_ADDR') and os.getenv('MASTER_PORT') and os.getenv('WORLD_SIZE') and os.getenv('RANK') and os.getenv('LOCAL_RANK')
