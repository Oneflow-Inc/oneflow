...
from oneflow.compatible.single_client.python.framework.check_point_v2 import LoadVariables, SaveVarDict, GetCheckpoint
from oneflow.compatible.single_client.python.framework.function_util import api_oneflow_function
from oneflow.compatible.single_client.python.framework.function_util import FunctionConfig as ExecutionConfig
from oneflow.compatible.single_client.python.framework.local_blob import LocalBlob
from oneflow.compatible.single_client.python.framework.session_util import api_clear_default_session
...
from oneflow.compatible.single_client.python.nn.optimizer.optimizer import Optimizer as OOPOptimizer
...
class DataModule(Module):
    ...
    def infer_oneflow_data_placeholder(self, batch: Tuple[Any]=None, optimizer_idx: int=0):
        return None
...
class NumpyDataModule(DataModule):
    ...
    def infer_oneflow_data_placeholder(self, batch: Tuple[np.ndarray, ...]=None, optimizer_idx: int=0):
        assert isinstance(batch, tuple), 'model.NumpyDataModule must return a tuple.'
        data_placeholder_list = []
        for item in batch:
            assert isinstance(item, np.ndarray), 'model.NumpyDataModule must return a tuple of numpy.'
            of_dtype = dtype_util.convert_numpy_dtype_to_oneflow_dtype(item.dtype)
            numpy_placeholder = oneflow_typing.Numpy.Placeholder(shape=item.shape, dtype=of_dtype)
...
class Callback(ABC):
    ...
    def on_training_step_end(self, outputs: Optional[Union[LocalBlob, Tuple[LocalBlob, ...], Tensor, Tuple[Tensor, ...]]], step_idx: int=0, optimizer_idx: int=0):
        pass

    def on_validation_step_end(self, outputs: Optional[Union[LocalBlob, Tuple[LocalBlob, ...], Tensor, Tuple[Tensor, ...]]], step_idx: int=0):
        pass
...
class Model(ABC, Module):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__()
        self._is_deprecated_function_style = kwargs['is_deprecated_function_style'] if 'is_deprecated_function_style' in kwargs else False
    ...
    def fit(self, training_config: Optional[TrainingConfig]=None, validation_config: Optional[ValidationConfig]=None, checkpoint_config: Optional[CheckpointConfig]=None, callbacks: Optional[Union[Callback, List[Callback]]]=None, max_steps: int=100):
        """ Runs the full training and validation routine.
        """
        self._max_steps = max_steps
        api_clear_default_session()
        self._sub_models = self._get_and_check_sub_models(training_config, validation_config, checkpoint_config, callbacks)
        ...
        for step_idx in range(0, self._max_steps):
            for sub_model in self._sub_models:
                try:
                    ...
                    print('Model step_idx {} {} failed.'.format(step_idx, sub_model.name))
                    raise e

    def method_overrided(self, method_name: str=None) -> bool:
        return getattr(self.__class__, method_name) != getattr(Model, method_name)

    def _get_and_check_sub_models(self, training_config: Optional[TrainingConfig]=None, validation_config: Optional[ValidationConfig]=None, checkpoint_config: Optional[CheckpointConfig]=None, callbacks: Optional[Union[Callback, List[Callback]]]=None):
        sub_models = []
        self._train_model = TrainModel(training_config, self, callbacks) if self._is_deprecated_function_style else TrainModelOOPStyle(training_config, self, callbacks)
        if self._train_model.is_valid:
            sub_models.append(self._train_model)
        elif training_config is not None:
            print(self._train_model.error_msg, "{}'s fit() will not do training.".format(self.__class__.__name__))
        self._val_model = ValidateModel(validation_config, self, callbacks) if self._is_deprecated_function_style else ValidateModelOOPStyle(validation_config, self, callbacks)
        if self._val_model.is_valid:
            sub_models.append(self._val_model)
        elif validation_config is not None:
            print(self._val_model.error_msg, "{}'s fit() will not do validation.".format(self.__class__.__name__))
        if len(sub_models) == 0:
            print("{}'s fit() will do nothing because there has no valid configuration.".format(self.__class__.__name__))
            return sub_models
        self._checkpoint_model = CheckpointModel(checkpoint_config, self, callbacks) if self._is_deprecated_function_style else CheckpointModelOOPStyle(checkpoint_config, self, callbacks)
        if self._checkpoint_model.is_valid:
            sub_models.append(self._checkpoint_model)
        elif checkpoint_config is not None:
            print(self._checkpoint_model.error_msg, "{}'s fit() will not do checkpoint.".format(self.__class__.__name__))
        return sub_models
...
class SubModel(ABC):
    ...
    def _get_and_check_cbs(self):
        ...
        if isinstance(self._cbs, list):
            for cb in self._cbs:
                assert isinstance(cb, Callback), "model callbacks' type must be model.Callback or List[model.Callback]."
            return True
...
class TrainModel(SubModel):
    ...
    def __init__(self, cfg: TrainingConfig=None, model: Model=None, callbacks: Optional[Union[Callback, List[Callback]]]=None):
    ...
    def step(self, step_idx: int=0):
        ...
        for optimizer_idx in range(0, len(self._opts)):
            ...
            self._method_callback('on_training_step_end', outputs=outputs, step_idx=step_idx, optimizer_idx=optimizer_idx)
    ...
    def _get_and_check_opts(self):
        ...
        if isinstance(opt_conf, Optimizer):
            ...
            for opt in opt_conf:
                assert isinstance(opt, Optimizer), 'model.configure_optimizers() must return Optimizer                     or List[Optimizer, ...] or Tuple[Optimizer, ...]'
            self._opts = opt_conf
        else:
            assert False, 'model.configure_optimizers() must return Optimizer                 or List[Optimizer, ...] or Tuple[Optimizer, ...]'
        return True

    def _get_and_check_jobs(self):
        self._is_numpy_input = True if isinstance(self._cfg.data, NumpyDataModule) else False
        self._jobs = []
        if self._is_numpy_input:
            ...
            for optimizer_idx in range(0, len(self._opts)):
                ...
                self._jobs.insert(optimizer_idx, self._construct_numpy_job(batch, optimizer_idx))
        else:
            for optimizer_idx in range(0, len(self._opts)):
    ...
    def _construct_job(self, optimizer_idx: int=0):
        ...
        def job():
            batch = self._cfg.data(0, optimizer_idx)
            outputs = self._model.training_step(batch=batch, optimizer_idx=optimizer_idx)
            loss = None
            ...
            return outputs
        job.__name__ = self._model.__class__.__name__ + '_Model_train_job_' + str(optimizer_idx)
    ...
    def _construct_numpy_job(self, batch, optimizer_idx):
        ...
        def job(*input_batch):
            outputs = self._model.training_step(batch=input_batch, optimizer_idx=optimizer_idx)
            loss = None
            if isinstance(outputs, tuple) and len(outputs) > 0:
                loss = outputs[0]
            else:
        ...
        job.__name__ = self._model.__class__.__name__ + '_Model_train_numpy_job_' + str(optimizer_idx)
        deco = api_oneflow_function(type='train', function_config=self._cfg.exe_cfg)
        return deco(job)

class ValidateModel(SubModel):

    def __init__(self, cfg: ValidationConfig=None, model: Model=None, callbacks: Optional[Union[Callback, List[Callback]]]=None):
    ...
    def step(self, step_idx: int=0):
        ...
        if (step_idx + 1) % self._cfg.step_interval == 0:
            ...
            self._method_callback('on_validation_step_end', step_idx=step_idx, outputs=outputs)
...
class CheckpointModel(SubModel):
    ...
    def __init__(self, cfg: CheckpointConfig=None, model: Model=None, callbacks: Optional[Union[Callback, List[Callback]]]=None):
    ...
    def step(self, step_idx: int=0):
        ...
        if self._cfg.need_save:
            if (step_idx + 1) % self._cfg.save_step_interval == 0:
                self._save_checkpoint(dirpath=self._cfg.save_dirpath + '-' + str(step_idx))
...
class TrainModelOOPStyle(SubModel):
    ...
    def _get_and_check_opts(self):
        ...
        if isinstance(opt_conf, OOPOptimizer):
            ...
            for opt in opt_conf:
                assert isinstance(opt, OOPOptimizer), 'model.configure_optimizers() must return Optimizer                     or List[Optimizer, ...] or Tuple[Optimizer, ...]'
            self._opts = opt_conf
        else:
            assert False, 'model.configure_optimizers() must return Optimizer                 or List[Optimizer, ...] or Tuple[Optimizer, ...]'
        return True
...
def _infer_job_signature(data_module, batch, optimizer_idx, job):
    ...
    for (i, placeholder) in enumerate(placeholder_list):
        para_name = data_module.__class__.__name__ + '_opt_' + str(optimizer_idx) + '_para_' + str(i)
        para_list.append(inspect.Parameter(name=para_name, kind=inspect.Parameter.POSITIONAL_OR_KEYWORD, annotation=placeholder))
