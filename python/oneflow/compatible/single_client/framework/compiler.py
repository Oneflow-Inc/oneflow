...
from oneflow.compatible.single_client.python.framework import placement_util as placement_util
...
from oneflow.compatible.single_client.python.framework import runtime_mode as runtime_mode
...
from oneflow.compatible.single_client.python.lib.core import func_inspect_util as func_inspect_util
...
def Compile(session, function_desc, config_proto):
    with InterpretScope(session, function_desc, config_proto):
        ...
        session.StashJob(function_desc.job_func.__name__, function_desc.job_func.__name__ + '_after_complete')
...
def InterpretScope(session, function_desc, config_proto):
    ...
    if placement_scope is None:
        ...
        tag_and_dev_ids = (placement_scope.device_tag, placement_scope.machine_device_ids)
    ...
    is_mirrored = isinstance(distribute_strategy, distribute_util.DistributeMirroredStrategy)
    assert isinstance(hierarchy, (list, tuple)) or hierarchy is None
    if hierarchy is not None:
        hierarchy = oneflow._oneflow_internal.Size(tuple(hierarchy))
    scope = scope_util.MakeInitialScope(job_conf, *tag_and_dev_ids, hierarchy, is_mirrored)
...
def _CompileJob(session, function_desc):
    ...
    if len(parameters) == 0:
        ...
        func.__oneflow_input_blob_defs__ = _MakeInputBlobDefFromParameterSignature(parameters)
    else:
        raise NotImplementedError('All parameters of global function should be annotated')
    ...
    func.__oneflow_output_remote_blobs__ = _RecursiveMakeRetRemoteBlobs(ret, allow_cpu_return_op=function_desc.function_attribute.allow_cpu_return_op)
...
def _InterpretGlobalFunction(function_desc, args):
    ...
    return_annotation = func.__oneflow_function_signature__.return_annotation
    oft_util.CheckReturnByAnnotation(func.__name__, ret, return_annotation)
    return _RecursiveMakeRetRemoteBlobs(ret, allow_cpu_return_op=function_desc.function_attribute.allow_cpu_return_op)
...
def _CloneArgBlobDef(args):
    ...
    raise NotImplementedError('oneflow.compatible.single_client.global_function only accepts nested input blob defs')
...
def _RecursiveMakeInputBlobs(input_blob_def):
    ...
    raise NotImplementedError('oneflow.compatible.single_client.global_function accepts ' + 'ArgBlobDefs or list/tuple/dict nested ArgBlobDefs as argument')
...
def _RecursiveMakeRetRemoteBlobs(remote_blobs, **kwarg):
    ...
    raise NotImplementedError('oneflow.compatible.single_client.global_function returns ' + 'RemoteBlob or list/tuple/dict nested RemoteBlob only')