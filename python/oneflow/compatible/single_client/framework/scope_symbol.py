...
class ScopeSymbol(Symbol):
    ...
    def __init__(self, symbol_id, scope_proto, parent_scope_symbol=None):
        ...
        self.job_desc_symbol_ = oneflow._oneflow_internal.GetJobConfSymbol(scope_proto.job_desc_symbol_id())
        self.device_parallel_desc_symbol_ = oneflow._oneflow_internal.GetPlacementSymbol(scope_proto.device_parallel_desc_symbol_id())
        self.host_parallel_desc_symbol_ = oneflow._oneflow_internal.GetPlacementSymbol(scope_proto.host_parallel_desc_symbol_id())
    ...
    def BuildWithNewParallelDesc(self, instruction_builder, device_tag, machine_device_ids):
    ...
    def BuildWithNewParallelConf(self, instruction_builder, parallel_conf):
        (device_tag, machine_device_ids, hierarchy) = oneflow._oneflow_internal.GetDeviceTagAndMachineDeviceIdsAndHierarchy(parallel_conf)
        return self.BuildWithNewParallelDesc(instruction_builder, device_tag, machine_device_ids)
...
def BuildInitialScope(instruction_builder, session_id, job_conf, device_tag, machine_device_ids, is_mirrored):
    scope_proto = scope_cfg.ScopeProto()
...
def MakeParallelConf(device_tag, machine_device_ids):
    ...
    for machine_device_id in machine_device_ids:
        assert isinstance(machine_device_id, str), 'type of machine_device_id (%s) is not string' % type(machine_device_id)
        assert re.match('^\\d+:\\d+(-\\d+)?$', machine_device_id) is not None, 'machine_device_id: %s is not valid' % machine_device_id
