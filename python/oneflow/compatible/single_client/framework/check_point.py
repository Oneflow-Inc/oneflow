...
from oneflow.compatible.single_client.python.framework import job_instance as job_instance
...
def _MakeModelInitJobFunc():
    ...
    return job_instance.MakeJobInstance(str(sess.inter_user_job_info.global_model_init_job_name), push_cb=push_cb, finish_cb=finish_cb)
...
def _MakeModelLoadJobFunc(path):
    ...
    def finish_cb():
        pass
    sess = session_ctx.GetDefaultSession()
    return job_instance.MakeJobInstance(str(sess.inter_user_job_info.global_model_load_job_name), push_cb=push_cb, finish_cb=finish_cb)
...
def _MakeModelSaveJobFunc(path):
    ...
    def finish_cb():
        pass
    sess = session_ctx.GetDefaultSession()
    return job_instance.MakeJobInstance(str(sess.inter_user_job_info.global_model_save_job_name), push_cb=push_cb, finish_cb=finish_cb)
...
class SimpleCheckPointManager(object):
    ...
    def initialize_or_restore(self) -> None:
        ...
        if name:
            check_point_v2.LoadVariables(check_point_v2.GetCheckpoint(self._GetSnapshotPath(name)))
        else:
            self.save()
