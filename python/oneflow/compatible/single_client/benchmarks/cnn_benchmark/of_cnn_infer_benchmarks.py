...
parser.add_argument('--use_xla_jit', dest='use_xla_jit', action='store_true', default=False, required=False, help='inference with xla jit')
parser.add_argument('--precision', type=str, default='float32', required=False, help='inference with low precision')
parser.add_argument('--print_every_n_iter', type=int, default=1, required=False, help='print log every n iterations')
...
model_dict = {'resnet50': resnet_model.resnet50, 'inceptionv3': inceptionv3_model.inceptionv3, 'vgg16': vgg_model.vgg16, 'alexnet': alexnet_model.alexnet}
...
def InferenceNet():
    ...
    if args.data_dir:
        ...
        (labels, images) = data_loader.load_imagenet(args.data_dir, args.image_size, batch_size, args.data_part_num)
    else:
    ...
    def create_callback(step):
        ...
        def callback(predictions):
            if step % args.print_every_n_iter == 0:
                ...
                print('iter {}, speed: {:.3f}(sec/batch), {:.3f}(images/sec)'.format(step, duration, images_per_sec))
