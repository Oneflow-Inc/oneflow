...
def PreTrain(input_ids_blob, input_mask_blob, token_type_ids_blob, masked_lm_positions_blob, masked_lm_ids_blob, masked_lm_weights_blob, next_sentence_label_blob, vocab_size, seq_length=512, hidden_size=768, num_hidden_layers=12, num_attention_heads=12, intermediate_size=3072, hidden_act='gelu', hidden_dropout_prob=0.1, attention_probs_dropout_prob=0.1, max_position_embeddings=512, type_vocab_size=16, max_predictions_per_seq=20, initializer_range=0.02):
    ...
    (lm_loss, _, _) = _AddMaskedLanguageModelLoss(input_blob=backbone.sequence_output(), output_weights_blob=backbone.embedding_table(), positions_blob=masked_lm_positions_blob, label_id_blob=masked_lm_ids_blob, label_weight_blob=masked_lm_weights_blob, seq_length=seq_length, hidden_size=hidden_size, vocab_size=vocab_size, max_predictions_per_seq=max_predictions_per_seq, hidden_act=bert_util.GetActivation(hidden_act), initializer_range=initializer_range)
    pooled_output = PooledOutput(backbone.sequence_output(), hidden_size, initializer_range)
    (ns_loss, _, _) = _AddNextSentenceOutput(input_blob=pooled_output, label_blob=next_sentence_label_blob, hidden_size=hidden_size, initializer_range=initializer_range)
...
def PooledOutput(sequence_output, hidden_size, initializer_range):
    with flow.scope.namespace('bert-pooler'):
        ...
        pooled_output = bert_util._FullyConnected(first_token_tensor, input_size=hidden_size, units=hidden_size, weight_initializer=bert_util.CreateInitializer(initializer_range), name='dense')
        pooled_output = flow.math.tanh(pooled_output)
    return pooled_output

def _AddMaskedLanguageModelLoss(input_blob, output_weights_blob, positions_blob, label_id_blob, label_weight_blob, seq_length, hidden_size, vocab_size, max_predictions_per_seq, hidden_act, initializer_range):
    ...
    with flow.scope.namespace('cls-predictions'):
        ...
        with flow.scope.namespace('transform'):
            if callable(hidden_act):
                ...
                act_fn = hidden_act
            input_blob = bert_util._FullyConnected(input_blob, input_size=hidden_size, units=hidden_size, activation=act_fn, weight_initializer=bert_util.CreateInitializer(initializer_range), name='dense')
            if callable(hidden_act):
                input_blob = hidden_act(input_blob)
                input_blob = bert_util._LayerNorm(input_blob, hidden_size)
        output_bias = flow.get_variable(name='output_bias', shape=[vocab_size], dtype=input_blob.dtype, initializer=flow.constant_initializer(1.0))
        ...
        pre_example_loss = flow.nn.sparse_softmax_cross_entropy_with_logits(logits=logit_blob, labels=label_id_blob)
...
def _AddNextSentenceOutput(input_blob, label_blob, hidden_size, initializer_range):
    with flow.scope.namespace('cls-seq_relationship'):
        output_weight_blob = flow.get_variable(name='output_weights', shape=[2, hidden_size], dtype=input_blob.dtype, initializer=bert_util.CreateInitializer(initializer_range))
        output_bias_blob = flow.get_variable(name='output_bias', shape=[2], dtype=input_blob.dtype, initializer=flow.constant_initializer(0.0))
        ...
        pre_example_loss = flow.nn.sparse_softmax_cross_entropy_with_logits(logits=logit_blob, labels=label_blob)
