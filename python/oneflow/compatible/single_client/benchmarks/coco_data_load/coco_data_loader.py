...
def coco_data_load(cfg, machine_id, nrank):
    with flow.scope.placement('cpu', '{}:0-{}'.format(machine_id, nrank - 1)):
        (image, image_id, image_size, bbox, label, segm_poly, segm_poly_index) = flow.data.coco_reader(annotation_file=cfg.annotation_file, image_dir=cfg.image_dir, batch_size=cfg.batch_size, shuffle=cfg.shuffle_after_epoch, stride_partition=cfg.stride_partition, name='coco_reader')
        ...
        (image, new_size, scale) = flow.image.target_resize(image, target_size=aligned_target_size, max_size=aligned_max_size)
        ...
        image = flow.image.batch_align(image, shape=(aligned_target_size, aligned_max_size, 3), dtype=flow.float, alignment=cfg.image_align_size)
        gt_bbox = flow.tensor_buffer_to_list_of_tensors(bbox, (cfg.max_num_objs, 4), flow.float, True)
        gt_label = flow.tensor_buffer_to_list_of_tensors(label, (cfg.max_num_objs,), flow.int32, True)
        segm_mask = flow.detection.object_segmentation_polygon_to_mask(segm_poly, segm_poly_index, new_size)
        gt_mask = flow.tensor_buffer_to_list_of_tensors(segm_mask, (cfg.max_num_objs, aligned_target_size, aligned_max_size), flow.int8, True)
        return {'image': image, 'image_size': new_size, 'gt_bbox': list(gt_bbox), 'gt_label': list(gt_label), 'gt_mask': list(gt_mask)}
...
def _benchmark(iter_num, drop_first_iters, verbose=False):
    ...
    print('mean of time elapsed of {} iters (dropped {} first iters): {}'.format(iter_num, drop_first_iters, s[drop_first_iters:].mean()))
