...
def make_style_transfer(image_height, image_width, channels=3):
    ...
    def style_model_predict(image: flow.typing.Numpy.Placeholder((1, channels, image_height, image_width), dtype=flow.float32)) -> flow.typing.Numpy:
...
class TestSaveAndLoadModel(flow.unittest.TestCase):
    INPUT_IMAGE_FILE = '/dataset/model_zoo/fast_neural_style/images/content-images/amber.jpg'
    ...
    def test_style_model(self):
        ...
        if not os.path.exists(saved_model_version_dir):
            saved_model_builder = flow.saved_model.ModelBuilder(saved_model_path).ModelName('style_transfer').Version(model_version)
        ...
        for input_name in input_names:
            print('input "{}" info: {}'.format(input_name, sess.input_info(input_name, job_names[0])))
        output_names = sess.list_outputs()
        print('output names:', output_names)
        for output_name in output_names:
            print('input "{}" info: {}'.format(output_name, sess.output_info(output_name, job_names[0])))
