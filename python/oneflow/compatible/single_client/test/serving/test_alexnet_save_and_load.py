...
from oneflow.compatible.single_client.core.serving import saved_model_pb2 as saved_model_pb
...
def make_alexnet_infer_func(batch_size, image_size):
    ...
    def alexnet_inference(image: flow.typing.Numpy.Placeholder(image_shape, dtype=flow.float32), label: flow.typing.Numpy.Placeholder(label_shape, dtype=flow.int32)) -> flow.typing.Numpy:
...
class TestSaveAndLoadModel(flow.unittest.TestCase):
    ...
    def test_alexnet(test_case, batch_size=DEFAULT_BATCH_SIZE, num_batchs=6):
        init_env()
        (alexnet_infer, input_lbns, output_lbns) = make_alexnet_infer_func(batch_size, (DEFAULT_IMAGE_SIZE, DEFAULT_IMAGE_SIZE, 3))
        ...
        if os.path.exists(saved_model_path) and os.path.isdir(saved_model_path):
            print("WARNING: The model version path '{}' already exist, old version directory will be removed".format(model_version_path))
            shutil.rmtree(saved_model_path)
        saved_model_builder = flow.saved_model.ModelBuilder(saved_model_path)
        signature_builder = saved_model_builder.ModelName(model_name).Version(model_version).AddFunction(alexnet_infer).AddSignature('regress')
        ...
        dataset = ImageNetRecordDataset(batch_size=new_batch_size, image_resize_size=DEFAULT_IMAGE_SIZE, data_format='NHWC')
        ...
        flow.clear_default_session()
        model_meta_file_path = os.path.join(saved_model_path, str(model_version), 'saved_model.prototxt')
        saved_model_proto = load_saved_model(model_meta_file_path)
        sess = flow.serving.InferenceSession()
        checkpoint_path = os.path.join(saved_model_path, str(model_version), saved_model_proto.checkpoint_dir)
        ...
        for output_name in output_names:
            print('output "{}" info: {}'.format(output_name, sess.output_info(output_name, job_name)))
