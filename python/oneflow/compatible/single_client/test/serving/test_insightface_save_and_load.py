...
def make_insightface_resnet100_func(batch_size=1, image_height=112, image_width=112, channels=3):
    shape = (batch_size, channels, image_height, image_width)

    @flow.global_function(type='predict', function_config=get_predict_config())
    def insightface_resnet100_func(image: flow.typing.Numpy.Placeholder(shape)) -> flow.typing.Numpy:
...
class TestSaveAndLoadModel(flow.unittest.TestCase):
    ...
    def test_insightface(self):
        ...
        dataset = FaceEmoreRecordDataset(data_dir=self.DATA_DIR, num_data_parts=self.NUM_DATA_PARTS, batch_size=self.BATCH_SIZE, image_width=self.IMAGE_SIZE, image_height=self.IMAGE_SIZE, data_format='NCHW')
        (image_list, issame_list) = dataset.load_batchs(self.NUM_ITER)
        print('Define inference function for insightface')
        infer_fn = make_insightface_resnet100_func(self.BATCH_SIZE, self.IMAGE_SIZE, self.IMAGE_SIZE)
        ...
        saved_model_builder = flow.saved_model.ModelBuilder(saved_model_path).ModelName('insightface').Version(model_version)
