...
def make_resnet_infer_func(batch_size, image_size):
    ...
    def resnet_inference(image: flow.typing.Numpy.Placeholder(image_shape, dtype=flow.float32)) -> flow.typing.Numpy:
...
class TestSaveAndLoadModel(flow.unittest.TestCase):
    ...
    def test_resnet(test_case, batch_size=DEFAULT_BATCH_SIZE, num_batchs=6):
        ...
        (resnet_infer, input_lbns, output_lbns) = make_resnet_infer_func(batch_size, image_size)
        flow.load_variables(flow.checkpoint.get(DEFAULT_CHECKPOINT_DIR))
        dataset = ImageNetRecordDataset(batch_size=batch_size, image_resize_size=DEFAULT_IMAGE_SIZE, data_format='NCHW')
        ...
        signature_builder = saved_model_builder.ModelName('resnet50').Version(model_version).AddFunction(resnet_infer).AddSignature('regress')
