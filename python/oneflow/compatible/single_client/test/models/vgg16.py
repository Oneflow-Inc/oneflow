...
def _conv2d_layer(name, input, filters, kernel_size=3, strides=1, padding='VALID', data_format='NCHW', dilation_rate=1, activation=op_conf_util.kRelu, use_bias=True, weight_initializer=flow.random_uniform_initializer(), bias_initializer=flow.constant_initializer()):
...
def _conv_block(in_blob, index, filters, conv_times):
    ...
    for i in range(conv_times):
        conv_i = _conv2d_layer(name='conv{}'.format(index), input=conv_block[i], filters=filters, kernel_size=3, strides=1)
...
def vgg(images, labels, trainable=True):
    ...
    fc6 = flow.layers.dense(inputs=pool5, units=4096, activation=flow.math.relu, use_bias=True, kernel_initializer=_get_kernel_initializer(), bias_initializer=_get_bias_initializer(), trainable=trainable, name='fc1')
    fc7 = flow.layers.dense(inputs=fc6, units=4096, activation=flow.math.relu, use_bias=True, kernel_initializer=_get_kernel_initializer(), bias_initializer=_get_bias_initializer(), trainable=trainable, name='fc2')
    fc8 = flow.layers.dense(inputs=fc7, units=1001, use_bias=True, kernel_initializer=_get_kernel_initializer(), bias_initializer=_get_bias_initializer(), trainable=trainable, name='fc_final')
    loss = flow.nn.sparse_softmax_cross_entropy_with_logits(labels, fc8, name='softmax_loss')
