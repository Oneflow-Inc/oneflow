...
from cnns_tests import TestAlexNetMixin, TestResNet50Mixin, TestVgg16Mixin, TestInceptionV3Mixin
...
class TestEagerMixin(object):
    ...
    def setUp(self):
        ...
        flow.enable_eager_execution(True)
flow.unittest.register_test_cases(scope=globals(), directory=os.path.dirname(os.path.realpath(__file__)), filter_by_num_nodes=lambda x: x == 1, base_class=absltest.TestCase, test_case_mixin=TestEagerMixin)
