...
def _data_load(args, data_dir):
    ...
    rsz = flow.image.resize(image, resize_x=IMAGE_SIZE, resize_y=IMAGE_SIZE, color_space='RGB')
    normal = flow.image.crop_mirror_normalize(rsz, color_space='RGB', output_layout='NCHW', mean=rgb_mean, output_dtype=flow.float)
    return (label, normal)

def _conv2d(name, input, filters, kernel_size, strides=1, padding='SAME', data_format='NCHW', dilations=1, weight_initializer=flow.variance_scaling_initializer()):
    weight = flow.get_variable(name + '-weight', shape=(filters, input.shape[1], kernel_size, kernel_size), dtype=input.dtype, initializer=weight_initializer, trainable=g_trainable)
    return flow.nn.conv2d(input, weight, strides, padding, None, data_format, dilations, name=name)

def _batch_norm(inputs, name=None):
    return flow.layers.batch_normalization(inputs=inputs, axis=1, momentum=0.997, epsilon=1e-05, center=True, scale=True, trainable=g_trainable, name=name)

def conv2d_affine(input, name, filters, kernel_size, strides, activation=op_conf_util.kNone):
...
def bottleneck_transformation(input, block_name, filters, filters_inner, strides):
    a = conv2d_affine(input, block_name + '_branch2a', filters_inner, 1, 1, activation=op_conf_util.kRelu)
    b = conv2d_affine(a, block_name + '_branch2b', filters_inner, 1, strides, activation=op_conf_util.kRelu)
...
def resnet_stem(input):
    ...
    conv1_bn = conv1
    pool1 = flow.nn.avg_pool2d(conv1_bn, ksize=3, strides=2, padding='VALID', data_format='NCHW', name='pool1')
...
def resnet50(args, data_dir):
    ...
    with flow.scope.namespace('Resnet'):
        ...
        fc1001 = flow.layers.dense(flow.reshape(pool5, (pool5.shape[0], -1)), units=1001, use_bias=True, kernel_initializer=flow.xavier_uniform_initializer(), bias_initializer=flow.zeros_initializer(), trainable=g_trainable, name='fc1001')
        g_output_key.append('fc1001')
        loss = flow.nn.sparse_softmax_cross_entropy_with_logits(labels, fc1001, name='softmax_loss')
...
def main(args):
    ...
    def TrainNet():
        ...
        flow.optimizer.SGD(flow.optimizer.PiecewiseConstantScheduler([], [0.0032]), momentum=0).minimize(loss)
        return loss
