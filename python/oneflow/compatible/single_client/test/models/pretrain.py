...
def _AddNextSentenceOutput(input_blob, label_blob, hidden_size, initializer_range):
    with flow.scope.namespace('cls-seq_relationship'):
        output_weight_blob = flow.get_variable(name='output_weights', shape=[2, hidden_size], dtype=input_blob.dtype, model_name='weight', initializer=bert_util.CreateInitializer(initializer_range))
        output_bias_blob = flow.get_variable(name='output_bias', shape=[2], dtype=input_blob.dtype, model_name='bias', initializer=flow.constant_initializer(0.0))
