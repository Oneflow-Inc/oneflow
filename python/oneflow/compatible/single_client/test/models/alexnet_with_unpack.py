...
def _conv2d_layer(args, name, input, filters, kernel_size=3, strides=1, padding='SAME', data_format='NCHW', dilation_rate=1, activation=op_conf_util.kRelu, use_bias=False, weight_initializer=flow.random_uniform_initializer(), bias_initializer=flow.random_uniform_initializer()):
...
def _data_load_layer(args, data_dir):
    ...
    return (flow.unpack(label, args.num_piece_in_batch), flow.unpack(normal, args.num_piece_in_batch))

def _dense_layer(inputs, units, activation=None, use_bias=True, kernel_initializer=None, bias_initializer=None, trainable=True, name=None):
    ...
    weight = flow.get_variable(name='{}-weight'.format(name_prefix), shape=(units, inputs.shape[1]), dtype=inputs.dtype, initializer=kernel_initializer if kernel_initializer is not None else flow.constant_initializer(0), trainable=trainable, model_name='weight')
    weight = flow.identity(weight)
    weight = flow.repeat(weight, args.num_piece_in_batch)
    out = flow.matmul(a=inputs, b=weight, transpose_b=True, name='{}_matmul'.format(name_prefix))
    if use_bias:
        bias = flow.get_variable(name='{}-bias'.format(name_prefix), shape=(units,), dtype=inputs.dtype, initializer=bias_initializer if bias_initializer is not None else flow.constant_initializer(0), trainable=trainable, model_name='bias')
        bias = flow.identity(bias)
        bias = flow.repeat(bias, args.num_piece_in_batch)
        out = flow.nn.bias_add(out, bias, name='{}_bias_add'.format(name_prefix))
    out = activation(out, name='{}_activation'.format(name_prefix)) if activation is not None else out
    out = flow.reshape(out, in_shape[:-1] + (units,)) if in_num_axes > 2 else out
    return out

def alexnet(args, images, labels, trainable=True):
    conv1 = _conv2d_layer(args, 'conv1', images, filters=64, kernel_size=11, strides=4, padding='VALID')
    ...
    fc1 = _dense_layer(inputs=pool5, units=4096, activation=flow.math.relu, use_bias=False, kernel_initializer=_get_initializer(), bias_initializer=False, trainable=trainable, name='fc1')
    dropout1 = fc1
    fc2 = _dense_layer(inputs=dropout1, units=4096, activation=flow.math.relu, use_bias=False, kernel_initializer=_get_initializer(), bias_initializer=False, trainable=trainable, name='fc2')
    dropout2 = fc2
    fc3 = _dense_layer(inputs=dropout2, units=1001, activation=None, use_bias=False, kernel_initializer=_get_initializer(), bias_initializer=False, trainable=trainable, name='fc3')
    loss = flow.nn.sparse_softmax_cross_entropy_with_logits(labels, fc3, name='softmax_loss')
    return loss
...
def main(args):
    ...
    def alexnet_train_job():
        ...
        flow.optimizer.SGD(flow.optimizer.PiecewiseConstantScheduler([], [1e-05]), momentum=0).minimize(loss)
