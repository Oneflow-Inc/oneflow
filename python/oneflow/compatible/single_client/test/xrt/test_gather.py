...
class TestGather(unittest.TestCase):
    ...
    def make_job(self, input_shape, indices_shape, axis, dtype=flow.float32):
        ...
        def gather_job(x=flow.FixedTensorDef(input_shape, dtype=dtype), indices=flow.FixedTensorDef(indices_shape, dtype=flow.int32)):
    ...
    def make_xla_job(self, input_shape, indices_shape, axis, dtype=flow.float32):
        ...
        @flow.global_function(config)
        def xla_gather_job(x=flow.FixedTensorDef(input_shape, dtype=dtype), indices=flow.FixedTensorDef(indices_shape, dtype=flow.int32)):
            return flow.gather(x, indices, axis=axis)
...
class TestBatchGather(TestGather):
    ...
    def make_job(self, input_shape, indices_shape, axis, dtype=flow.float32):
        ...
        @flow.global_function(config)
        def batch_gather_job(x=flow.FixedTensorDef(input_shape, dtype=dtype), indices=flow.FixedTensorDef(indices_shape, dtype=flow.int32)):
    ...
    def make_xla_job(self, input_shape, indices_shape, axis, dtype=flow.float32):
        ...
        @flow.global_function(config)
        def xla_batch_gather_job(x=flow.FixedTensorDef(input_shape, dtype=dtype), indices=flow.FixedTensorDef(indices_shape, dtype=flow.int32)):
            return flow.gather(x, indices, batch_dims=axis)
