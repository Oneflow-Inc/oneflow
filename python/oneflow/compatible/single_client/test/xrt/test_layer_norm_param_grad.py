...
def make_job(shape, gamma_shape, params_axis, dtype=flow.float32):
    ...
    def layer_norm_param_grad_job(dy=flow.FixedTensorDef(shape, dtype=dtype), norm=flow.FixedTensorDef(shape, dtype=dtype), gamma=flow.FixedTensorDef(gamma_shape, dtype=dtype)):
        return flow.layers.layer_norm_param_grad(dy, norm, gamma, begin_params_axis=params_axis)
...
def make_xla_job(shape, gamma_shape, params_axis, dtype=flow.float32):
    ...
    @flow.global_function(config)
    def xla_layer_norm_param_grad_job(dy=flow.FixedTensorDef(shape, dtype=dtype), norm=flow.FixedTensorDef(shape, dtype=dtype), gamma=flow.FixedTensorDef(gamma_shape, dtype=dtype)):
        return flow.layers.layer_norm_param_grad(dy, norm, gamma, begin_params_axis=params_axis)
...
class TestLayerNormParamGrad(unittest.TestCase):
    ...
    def _test_body(self, dy, norm, gamma, params_axis, dtype=np.float32):
        ...
        self.assertTrue(np.allclose(d_norm1.numpy(), d_norm2.numpy(), rtol=0.001, atol=1e-05))
        self.assertTrue(np.allclose(d_beta1.numpy(), d_beta2.numpy(), rtol=0.001, atol=1e-05))
        self.assertTrue(np.allclose(d_gamma1.numpy(), d_gamma2.numpy(), rtol=0.001, atol=1e-05))
