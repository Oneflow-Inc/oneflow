...
def make_job(a_shape, b_shape, axis, dtype=flow.float32):
    ...
    def concat_job(x=flow.FixedTensorDef(a_shape, dtype=dtype), y=flow.FixedTensorDef(b_shape, dtype=dtype)):
...
def make_trt_job(a_shape, b_shape, axis, dtype=flow.float32):
    ...
    @flow.global_function(config)
    def trt_concat_job(x=flow.FixedTensorDef(a_shape, dtype=dtype), y=flow.FixedTensorDef(b_shape, dtype=dtype)):
        return flow.concat([x, y], axis=axis)
