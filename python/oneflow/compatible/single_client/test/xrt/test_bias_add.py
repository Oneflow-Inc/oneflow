...
def make_job(x_shape, b_shape, dtype=flow.float32):
    ...
    def bias_add_job(x=flow.FixedTensorDef(x_shape, dtype=dtype), bias=flow.FixedTensorDef(b_shape, dtype=dtype)):
...
def make_xla_job(x_shape, b_shape, dtype=flow.float32):
    ...
    @flow.global_function(config)
    def xla_bias_add_job(x=flow.FixedTensorDef(x_shape, dtype=dtype), bias=flow.FixedTensorDef(b_shape, dtype=dtype)):
        return flow.nn.bias_add(x, bias)
...
def make_trt_job(x_shape, b_shape, dtype=flow.float32):
    ...
    @flow.global_function(config)
    def trt_bias_add_job(x=flow.FixedTensorDef(x_shape, dtype=dtype), bias=flow.FixedTensorDef(b_shape, dtype=dtype)):
        return flow.nn.bias_add(x, bias)
