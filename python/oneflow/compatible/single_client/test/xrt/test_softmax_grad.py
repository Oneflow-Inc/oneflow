...
def make_job(shape, axis, dtype=flow.float32):
    ...
    def softmax_grad_job(y=flow.FixedTensorDef(shape, dtype=dtype), dy=flow.FixedTensorDef(shape, dtype=dtype)):
...
def make_xla_job(shape, axis, dtype=flow.float32):
    ...
    @flow.global_function(config)
    def xla_softmax_grad_job(y=flow.FixedTensorDef(shape, dtype=dtype), dy=flow.FixedTensorDef(shape, dtype=dtype)):
        return flow.nn.softmax_grad(y, dy, axis=axis)
