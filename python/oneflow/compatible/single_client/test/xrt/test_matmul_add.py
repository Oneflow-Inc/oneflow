...
def make_job(a_shape, b_shape, trans_a=False, trans_b=False, fuse_add_to_output=True, dtype=flow.float32):
    config.use_xla_jit(False)
    config.use_tensorrt(False)
    config.enable_fuse_add_to_output(fuse_add_to_output)

    @flow.global_function(config)
    def matmul_job(a=flow.FixedTensorDef(a_shape, dtype=dtype), b=flow.FixedTensorDef(b_shape, dtype=dtype)):
        out = flow.matmul(a, b, transpose_a=trans_a, transpose_b=trans_b)
        c = flow.get_variable('c', shape=out.shape, dtype=flow.float, initializer=flow.ones_initializer(), trainable=True)
        out = flow.math.add_n([out, c])
        return out
    return matmul_job

def make_xla_job(a_shape, b_shape, trans_a=False, trans_b=False, fuse_add_to_output=True, dtype=flow.float32):
    config.use_xla_jit(True)
    config.use_tensorrt(False)
    config.enable_fuse_add_to_output(fuse_add_to_output)

    @flow.global_function(config)
    def xla_matmul_job(a=flow.FixedTensorDef(a_shape, dtype=dtype), b=flow.FixedTensorDef(b_shape, dtype=dtype)):
        out = flow.matmul(a, b, transpose_a=trans_a, transpose_b=trans_b)
        c = flow.get_variable('c', shape=out.shape, dtype=flow.float, initializer=flow.ones_initializer(), trainable=True)
        out = flow.math.add_n([out, c])
        return out
...
def make_trt_job(a_shape, b_shape, trans_a=False, trans_b=False, fuse_add_to_output=True, dtype=flow.float32):
    config.use_xla_jit(False)
    config.use_tensorrt(True)
    config.enable_fuse_add_to_output(fuse_add_to_output)

    @flow.global_function(config)
    def trt_matmul_job(a=flow.FixedTensorDef(a_shape, dtype=dtype), b=flow.FixedTensorDef(b_shape, dtype=dtype)):
        out = flow.matmul(a, b, transpose_a=trans_a, transpose_b=trans_b)
        c = flow.get_variable('c', shape=out.shape, dtype=flow.float, initializer=flow.ones_initializer(), trainable=True)
        out = flow.math.add_n([out, c])
        return out
...
class TestMatmul(unittest.TestCase):
    ...
    def _test_ones_body(self, m, k, n, trans_a, trans_b, fuse_add_to_output, dtype=np.float32):
    ...
    def _test_random_body(self, m, k, n, trans_a, trans_b, fuse_add_to_output, dtype=np.float32):
        shape_a = self.make_shape(m, k, trans_a)
        shape_b = self.make_shape(k, n, trans_b)
