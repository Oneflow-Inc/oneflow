...
class Test1dSspVariableProxy(flow.unittest.TestCase):
    ...
    def test_1d_ring_buffer_Wm_assign_Wc_plus_1(test_case):
        ...
        def Foo() -> tp.Numpy:
            with flow.scope.placement('cpu', device_name):
                ...
                (ref, value) = flow.experimental.ssp_variable_proxy(w, buffer_size=buffer_size)
    ...
    def test_add_ssp_variable_proxy(test_case):
        ...
        def Foo() -> tp.Numpy:
            with flow.scope.placement('cpu', device_name), flow.experimental.scope.config(ssp_num_stages=buffer_size, ssp_stage_id=0):
                w = flow.get_variable('w', shape=(10,), dtype=flow.float, initializer=flow.constant_initializer(0))
                loss = w + flow.constant_like(w, value=0.0, dtype=flow.float)
                flow.optimizer.SGD(flow.optimizer.PiecewiseConstantScheduler([], [-10.0]), momentum=0).minimize(loss)
                return loss
        zeros = np.zeros((10,)).astype(np.float32)
        ones = np.ones((10,)).astype(np.float32)
        for i in range(buffer_size):
