...
def _check(test_case, data_type, data, segment_ids, out_shape, axis, out):
    ...
    if axis != 0:
        ...
        data_perm = list(range(axis, axis + segment_ids.ndim)) + list(range(0, axis)) + list(range(axis + segment_ids.ndim, data.ndim))
...
def _gen_segment_ids(out_shape, axis, segment_ids_shape):
    return np.random.randint(0, out_shape[axis], tuple(segment_ids_shape)).astype(np.int32)

def _gen_data(data_type, out_shape, axis, segment_ids_shape):
    data_shape = out_shape[0:axis] + segment_ids_shape + out_shape[axis + 1:]
    if data_type == 'float16':
        return np.random.uniform(-1.0, 1.0, size=data_shape).astype(np.float16).astype(np.float32)
    else:
...
def _run_test(test_case, device, data_type, out_shape, axis, segment_ids_shape):
    ...
    def unsorted_segment_sum_job(data: oft.Numpy.Placeholder(data.shape, dtype=dtype), segment_ids: oft.Numpy.Placeholder(segment_ids.shape, dtype=flow.int32)):
        with flow.scope.placement(device, '0:0'):
            if data_type == 'float16':
                data = flow.cast(data, dtype=flow.float16)
                return flow.cast(flow.math.unsorted_segment_sum(data=data, segment_ids=segment_ids, num_segments=out_shape[axis], axis=axis), dtype=dtype)
            else:
                return flow.math.unsorted_segment_sum(data=data, segment_ids=segment_ids, num_segments=out_shape[axis], axis=axis)

    @flow.global_function(function_config=func_config)
    def unsorted_segment_sum_like_job(data: oft.Numpy.Placeholder(data.shape, dtype=dtype), segment_ids: oft.Numpy.Placeholder(segment_ids.shape, dtype=flow.int32), like: oft.Numpy.Placeholder(out_shape, dtype=dtype)):
        with flow.scope.placement(device, '0:0'):
            if data_type == 'float16':
                data = flow.cast(data, dtype=flow.float16)
                like = flow.cast(like, dtype=flow.float16)
                return flow.cast(flow.math.unsorted_segment_sum_like(data=data, segment_ids=segment_ids, like=like, axis=axis), dtype=dtype)
            else:
                return flow.math.unsorted_segment_sum_like(data=data, segment_ids=segment_ids, like=like, axis=axis)
