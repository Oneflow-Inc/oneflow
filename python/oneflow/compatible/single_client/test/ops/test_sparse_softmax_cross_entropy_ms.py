...
def compare_with_tensorflow(device_type, data_type, label_type, num_classes, batch_size):
    ...
    def SparseSoftmaxCrossEntropyWithLogitsJob(labels: oft.Numpy.Placeholder((batch_size,), dtype=type_name_to_flow_type[label_type])):
        with flow.scope.placement(device_type, '0:0'):
            x = flow.get_variable('x', shape=(batch_size, num_classes), dtype=type_name_to_flow_type[data_type], initializer=flow.random_uniform_initializer(minval=-10, maxval=10), trainable=True)
        with flow.scope.placement(device_type, '0:0-3'):
            labels = flow.parallel_cast(labels, distribute=flow.distribute.broadcast())
            logits = flow.parallel_cast(x, distribute=flow.distribute.split(len(x.shape) - 1))
            loss = flow.nn.distributed_sparse_softmax_cross_entropy_with_logits(labels, logits)
