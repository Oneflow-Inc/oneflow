...
def compare_with_tensorflow(device_type, target_dtype, predictions_shape, k, with_finite=False):
    ...
    targets = np.random.randint(classes, size=instance_num).astype(type_name_to_np_type[target_dtype])
    predictions = np.random.rand(*predictions_shape).astype('float32')
    if with_finite:
        predictions[np.random.randint(instance_num)][np.random.randint(classes)] = float('inf')

    @flow.global_function(function_config=func_config)
    def IntopkJob(targets: tp.ListNumpy.Placeholder((instance_num + 10,), dtype=type_name_to_flow_type[target_dtype]), predictions: tp.ListNumpy.Placeholder(tuple([dim + 5 for dim in predictions_shape]), dtype=flow.float)):
