...
def gen_gather_test_sample(input_shape, index_shape, dim, is_float=True):
    ...
    def _np_dim_scatter_add(src, dim, index, outshape):
        ...
        for srcidx in range(0, src.size):
            ...
            output[np.unravel_index(output_offset, outshape)] += src[np.unravel_index(srcidx, src.shape)]
        return output
    if is_float:
...
def _make_dim_gather_fn(test_case, input, index, dim, grad, device_type, value_type, index_type, machine_ids, device_counts):
    ...
    if value_type == flow.float16:
        ...
        def gather_fn(params_def: oft.Numpy.Placeholder(input.shape, dtype=flow.float32), indices_def: oft.Numpy.Placeholder(index.shape, dtype=index_type)) -> oft.Numpy:
            with flow.scope.placement(device_type, '0:0'):
                x_var = flow.get_variable('input', shape=input.shape, dtype=flow.float32, initializer=flow.constant_initializer(0))
            ...
            with flow.scope.placement(device_type, '0:0'):
                flow.optimizer.SGD(flow.optimizer.PiecewiseConstantScheduler([], [0.001]), momentum=0).minimize(y_f32)
        ...
        @flow.global_function(type='train', function_config=func_config)
        def gather_fn(params_def: oft.Numpy.Placeholder(input.shape, dtype=value_type), indices_def: oft.Numpy.Placeholder(index.shape, dtype=index_type)) -> oft.Numpy:
            with flow.scope.placement(device_type, '0:0'):
                x_var = flow.get_variable('input', shape=input.shape, dtype=value_type, initializer=flow.constant_initializer(0))
                x_var = flow.cast_to_current_logical_view(x_var)
                x = x_var + params_def
            y = flow.dim_gather(x, dim, indices_def)
            with flow.scope.placement(device_type, '0:0'):
        ...
        def gather_fn(params_def: oft.Numpy.Placeholder(input.shape, dtype=flow.float32), indices_def: oft.Numpy.Placeholder(index.shape, dtype=index_type)) -> oft.Numpy:
            ...
            with flow.scope.placement(device_type, '0:0'):
                flow.optimizer.SGD(flow.optimizer.PiecewiseConstantScheduler([], [0.001]), momentum=0).minimize(y_fp32)
            flow.watch_diff(x, _compare_diff)
            return y_fp32
        return gather_fn

def _compare_dim_gather_with_samples(test_case, device_type, sample, value_type, index_type, machine_ids, device_count):
    gather_fn = _make_dim_gather_fn(test_case, sample['input'].astype(value_type[0]), sample['index'].astype(index_type[0]), sample['dim'], sample['grad'].astype(value_type[0]), device_type, value_type[1], index_type[1], machine_ids, device_count)
    y = gather_fn(sample['input'].astype(value_type[0]), sample['index'].astype(index_type[0]))
    y.astype(value_type[0])
    if value_type == flow.float16:
        test_case.assertTrue(np.allclose(y, sample['output'].astype(np.float32), 0.001, 0.001))
    else:
        test_case.assertTrue(np.allclose(y, sample['output'].astype(value_type[0])))

def _gen_arg_dict(device_type='gpu', value_type='float', machine_ids='0:0', device_count=1):
