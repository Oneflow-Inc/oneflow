...
def compare_broadcast_like_with_tf(device_type, input_shape, like_shape, broadcast_axes, rtol=1e-05, atol=1e-05):
    ...
    def broadcast_like_forward(x: tp.Numpy.Placeholder(shape=input_shape, dtype=flow.float), y: tp.Numpy.Placeholder(shape=like_shape, dtype=flow.float)):
...
class TestBroadcastLike(flow.unittest.TestCase):
    ...
    def test_broadcast_like_grad(test_case):
        ...
        def watch_matmul_diff_job(images: tp.Numpy.Placeholder((3, 3), dtype=flow.float)) -> None:
            weight_initializer = flow.constant_initializer(2)
            weight_shape = (3, 1)
            weight = flow.get_variable('three-weight', shape=weight_shape, initializer=weight_initializer)
            weight_broadcast = flow.broadcast_like(weight, like=images, broadcast_axes=(1,))
