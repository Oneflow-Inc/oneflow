...
def compare_with_not_fused(test_case, device_type, x_shape, data_type, data_format):
    ...
    def FlowJob(value: oft.Numpy.Placeholder(x_shape), bias: oft.Numpy.Placeholder(bias_shape)):
        with flow.scope.placement(device_type, '0:0'):
            ...
            if data_type == 'float16':
                y1 = flow.cast(flow.math.gelu(flow.nn.bias_add(flow.cast(x1, dtype=flow.float16), flow.cast(bias1, dtype=flow.float16), data_format=data_format)), dtype=flow.float)
                y2 = flow.cast(flow.nn.fused_bias_add_gelu(flow.cast(x2, dtype=flow.float16), flow.cast(bias2, dtype=flow.float16), data_format=data_format), dtype=flow.float)
            else:
                y1 = flow.math.gelu(flow.nn.bias_add(x1, bias1, data_format=data_format))
