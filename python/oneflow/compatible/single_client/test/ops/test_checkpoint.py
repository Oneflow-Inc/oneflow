...
def get_placement():
    ...
    machine_device_ids = ['{}:{}'.format(node_id, device_ids) for node_id in range(node_size)]
...
def get_simple_momentum_training_model(dtype):
    ...
    def model() -> tp.Numpy:
        with get_placement():
            x = flow.get_variable(name='x', shape=(4, 5), dtype=flow.float32, initializer=flow.random_normal_initializer(mean=10, stddev=1))
            w = flow.get_variable(name='w', shape=(5, 6), dtype=flow.float32, initializer=flow.random_normal_initializer(mean=10, stddev=1), distribute=flow.distribute.split(0))
            y = flow.matmul(x, w)
            flow.optimizer.SGD(flow.optimizer.PiecewiseConstantScheduler([], [0.01]), momentum=0.9).minimize(y)
            return y
    return model
...
def get_simple_model(dtype):
    ...
    def add() -> tp.Numpy:
        with get_placement():
            x = flow.get_variable(name='x', shape=(9, 3), dtype=dtype, initializer=flow.random_normal_initializer(mean=10, stddev=1), distribute=flow.distribute.split(0))
            y = flow.get_variable(name='y', shape=(9, 3), dtype=dtype, initializer=flow.constant_initializer(5, dtype=dtype))
            z = flow.get_variable(name='z', shape=(9, 3), dtype=dtype, initializer=flow.random_normal_initializer())
...
def get_large_model(dtype):
    ...
    def large() -> tp.Numpy:
        with get_placement():
            x = flow.get_variable(name='x', shape=(10, 2801, 820, 4), dtype=dtype, initializer=flow.random_normal_initializer(mean=10, stddev=1), distribute=flow.distribute.split(0))
...
def get_add_and_reduce_mean_model(dtype):
    ...
    def model() -> tp.Numpy:
        with get_placement():
            x = flow.get_variable(name='x', shape=(10, 801, 820, 4), dtype=dtype, initializer=flow.random_normal_initializer(mean=10, stddev=1), distribute=flow.distribute.split(0))
            y = flow.get_variable(name='y', shape=(10, 801, 820, 4), dtype=dtype, initializer=flow.random_normal_initializer(mean=10, stddev=1), distribute=flow.distribute.split(0))
...
def _TestPartiallyLoadNumpy(test_case, dtype):
    ...
    test_case.assertTrue(np.allclose(flow_res, np_res), {'flow_res': flow_res, 'np_res': np_res, 'diff': flow_res - np_res})
    test_case.assertTrue(np.array_equal(var_y_value_before_loading, var_y_value_after_loading))
...
def _TestMixedModel(test_case, dtype):
    with tempfile.TemporaryDirectory() as save_dir1, tempfile.TemporaryDirectory() as save_dir2:
        ...
        def get_variable(name):
            return flow.get_variable(name=name, shape=(10, 80, 40, 20), dtype=dtype, initializer=flow.random_normal_initializer(mean=10, stddev=1), distribute=flow.distribute.split(0))
        ...
        res = mixed_model()
        test_case.assertTrue(np.allclose(res, var_dict_from_model1['x'].numpy() + var_dict_from_model2['x'].numpy()))

def _TestResumeTraining(test_case):
    with tempfile.TemporaryDirectory() as save_dir:
        refresh_session()
        model = get_checkpoint_ready_model(get_simple_momentum_training_model, flow.float32)
        model()
        flow.checkpoint.save(save_dir)
        model()
...
class TestCheckpoint(flow.unittest.TestCase):
    ...
    @unittest.skipIf(flow.unittest.env.eager_execution_enabled(), "legacy model io doesn't work in eager mode")
    ...
    @flow.unittest.skip_unless_1n4d()
    @unittest.skipIf(not flow.unittest.env.eager_execution_enabled(), 'Save and load are covered by other tests in lazy mode')
