...
def compare_with_np(device_type, label_type, num_classes, num_sample, batch_size):
    ...
    def PartialFcJob(labels: oft.Numpy.Placeholder((batch_size,), dtype=type_name_to_flow_type[label_type])):
        with flow.scope.placement(device_type, '0:0'):
            x = flow.get_variable('x-weight', shape=(num_classes, 128), dtype=flow.float, initializer=flow.random_uniform_initializer(minval=-10, maxval=10), trainable=True)
        with flow.scope.placement(device_type, '0:0-3'):
            ...
            (maped_label, sampled_label, sampled_weight) = flow.distributed_partial_fc_sample(weight=x.with_distribute(weight_distribute), label=labels.with_distribute(lebels_distribute), num_sample=num_sample)
        with flow.scope.placement(device_type, '0:0'):
            ...
            flow.watch_diff(sampled_weight, test_global_storage.Setter('sampled_weight_diff'))
    ...
    for i in range(gpu_num):
        ...
        assert np.all((local_sample_labels >= lower) & (local_sample_labels < upper)) == True
        assert len(local_sample_labels) == len(np.unique(local_sample_labels))
        assert np.array_equal(local_label, global_sample_labels[0:len(local_label)]) == True
