...
class TestUnaryElementwiseOps(flow.unittest.TestCase):
    ...
    def test_log_sigmoid(test_case):
        ...
        test_case.assertTrue(np.allclose(y, -np.log(1 + np.exp(-x)), equal_nan=True, rtol=0.001, atol=1e-05))
    ...
    def test_softplus(test_case):
        ...
        test_case.assertTrue(np.allclose(y, np.log(np.exp(x) + 1), equal_nan=True, rtol=0.001, atol=1e-05))
