...
def compare_with_tensorflow_addons_lamb(test_case, device_type, x_shape, beta1, beta2, epsilon, weight_decay, learning_rate, train_iters):
    ...
    def testLAMB(random_mask: flow.typing.Numpy.Placeholder(x_shape, dtype=flow.float32)) -> flow.typing.Numpy:
        with flow.scope.placement(device_type, '0:0-0'):
            x = flow.get_variable(name='x', shape=x_shape, dtype=flow.float32, initializer=flow.random_uniform_initializer(minval=-10, maxval=10), trainable=True)
            loss = flow.math.reduce_mean(x + random_mask)
            flow.optimizer.LAMB(flow.optimizer.PiecewiseConstantScheduler([], [learning_rate]), beta1=beta1, beta2=beta2, epsilon=epsilon, weight_decay=weight_decay).minimize(loss)
            return x
    random_masks_seq = []
    ...
    opt = tfa.optimizers.LAMB(learning_rate=learning_rate, beta_1=beta1, beta_2=beta2, epsilon=epsilon, weight_decay_rate=weight_decay)
    var_list = []
    ...
    for (i, o, t) in zip(range(len(var_list)), x_list, var_list):
        diff = o - t
        test_case.assertTrue(np.allclose(x_list[i], var_list[i], rtol=0.001, atol=0.001), (i, case, diff))
    diff = x.flatten() - var.numpy().flatten()
    test_case.assertTrue(np.allclose(x.flatten(), var.numpy().flatten(), rtol=0.001, atol=0.001), (case, diff))
