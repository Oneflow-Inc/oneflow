...
def _of_image_decode(images):
    ...
    def image_decode_job(images_def: oft.ListListNumpy.Placeholder(shape=static_shape, dtype=flow.int8)):
        images_buffer = flow.tensor_list_to_tensor_buffer(images_def)
        decoded_images_buffer = flow.image_decode(images_buffer)
        return flow.tensor_buffer_to_tensor_list(decoded_images_buffer, shape=(640, 640, 3), dtype=flow.uint8)
...
class TestImageDecode(flow.unittest.TestCase):
    ...
    def test_image_decode(test_case):
        _compare_jpg_decode_with_pil(test_case, ['/dataset/mscoco_2017/val2017/000000000139.jpg', '/dataset/mscoco_2017/val2017/000000000632.jpg'])
