...
def _random_inputs(x_shape, x_dtype, index_shape, index_dtype):
    ...
    for col in range(index_cols):
        index_col = np.random.randint(low=0, high=x_shape[col], size=(index_rows,), dtype=index_dtype).reshape(index_shape[:-1])
        index.append(index_col)
    index = np.stack(index, axis=len(index_shape) - 1)
    return (x, index)

def _make_gather_nd_fn(x_shape, index_shape, x_dtype, index_type, device_type, device_num, dynamic, need_grad, comp_diff_fn):
    assert device_num >= 1
    ...
    def do_gather_nd(x, index):
        x_var = flow.get_variable('params', shape=(1,), dtype=x_dtype, initializer=flow.constant_initializer(0, x_dtype))
    ...
    if dynamic:
        ...
        def gather_nd_fn(x: flow.typing.ListNumpy.Placeholder(x_shape, dtype=x_dtype), index: flow.typing.ListNumpy.Placeholder(index_shape, dtype=index_type)) -> flow.typing.ListNumpy:
            return do_gather_nd(x, index)
    else:

        @flow.global_function(type=fn_type, function_config=func_config)
        def gather_nd_fn(x: flow.typing.Numpy.Placeholder(x_shape, dtype=x_dtype), index: flow.typing.Numpy.Placeholder(index_shape, dtype=index_type)) -> flow.typing.Numpy:
            return do_gather_nd(x, index)
    return gather_nd_fn
...
def _compare_with_np(test_case, shape, index_shape, dynamic_shape=None, dynamic_index_shape=None, dtype='float32', index_dtype='int32', device_type='gpu', device_num=1, dynamic=False):
    ...
    if dynamic:
        ...
        for _ in range(device_num):
            (x_, index_) = _random_inputs(dynamic_shape, x_dtype, dynamic_index_shape, index_dtype)
        ...
        (x, index) = _random_inputs(dynamic_shape, x_dtype, dynamic_index_shape, index_dtype)
    ...
    gather_nd_fn = _make_gather_nd_fn(shape, index_shape, x_of_dtype, index_of_dtype, device_type, device_num, dynamic, need_grad, comp_diff if device_num == 1 else None)
