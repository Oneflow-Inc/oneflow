...
def compare_with_tensorflow(device_type, a_shape, b_shape, transpose_a, transpose_b, data_type, fuse_add_to_output, enable_tf32, alpha):
    ...
    def MatmulJob():
        with flow.scope.placement(device_type, '0:0'):
            a = flow.get_variable('a', shape=a_shape, dtype=dtype, initializer=flow.random_uniform_initializer(minval=0, maxval=1), trainable=True)
            b = flow.get_variable('b', shape=b_shape, dtype=dtype, initializer=flow.random_uniform_initializer(minval=0, maxval=1), trainable=True)
            if data_type == 'float16':
                out = flow.matmul(flow.cast(a, dtype=flow.float16), flow.cast(b, dtype=flow.float16), transpose_a, transpose_b, alpha)
                c = flow.get_variable('c', shape=out.shape, dtype=dtype, initializer=flow.random_uniform_initializer(minval=-1, maxval=1), trainable=True)
                loss = flow.cast(out + flow.cast(c, dtype=flow.float16), dtype=flow.float)
            else:
    ...
    if data_type == 'float16':
        ...
        tolerance = 0.001
    assert np.allclose(of_out.numpy(), tf_out.numpy(), rtol=tolerance, atol=tolerance), np.max(np.abs(of_out.numpy() - tf_out.numpy()))
    assert np.allclose(test_global_storage.Get('a_diff'), tf_a_diff.numpy(), rtol=tolerance, atol=tolerance)
    assert np.allclose(test_global_storage.Get('b_diff'), tf_b_diff.numpy(), rtol=tolerance, atol=tolerance)
    assert np.allclose(test_global_storage.Get('c_diff'), tf_c_diff.numpy(), rtol=tolerance, atol=tolerance)
...
def filter_args_v2(arg_dict_list):
    ...
    for arg_dict in arg_dict_list:
        ...
        if a_shape[-1] != b_shape[-2]:
            continue
        if arg_dict['device_type'] == 'cpu' and (arg_dict['data_type'] == 'float16' or arg_dict['enable_tf32'] is True):
            continue
        if arg_dict['data_type'] != 'float32' and arg_dict['enable_tf32'] is True:
            continue
        if arg_dict['test_add_to_output'] is False and arg_dict['fuse_add_to_output'] is True:
            continue
        arg_dict['atol'] = 1e-05
        ret.append(arg_dict)
    return ret
...
def make_matmul_func(a_shape, b_shape, trans_a, trans_b, alpha, dtype, device_type, test_add_to_output, fuse_add_to_output, tf32):
    assert device_type in ['gpu', 'cpu']
    flow.clear_default_session()
    ...
    @flow.global_function(type='train', function_config=func_config)
    def matmul_job() -> typing.Tuple[flow.typing.Numpy, flow.typing.Numpy, flow.typing.Numpy, flow.typing.Numpy]:
        a_var = flow.get_variable('a', shape=a_shape, dtype=flow.float32, initializer=flow.random_uniform_initializer(minval=0, maxval=1), trainable=True)
        b_var = flow.get_variable('b', shape=b_shape, dtype=flow.float32, initializer=flow.random_uniform_initializer(minval=0, maxval=1), trainable=True)
        ...
        add_to = flow.get_variable('c', shape=c.shape, dtype=flow.float32, initializer=flow.random_uniform_initializer(minval=-1, maxval=1), trainable=True)
...
def compare_with_np(test_case, a_shape, b_shape, transpose_a, transpose_b, alpha=1.0, data_type='float32', device_type='gpu', test_add_to_output=False, fuse_add_to_output=False, enable_tf32=False, rtol=1e-05, atol=1e-08):

    def print_dbg_info(lhs=None, rhs=None):
        print(f'a_shape: {a_shape}, b_shape: {b_shape}, transpose_a: {transpose_a}, transpose_b: {transpose_b}, alpha: {alpha}, data_type: {data_type}, device_type: {device_type}, test_add_to_output: {test_add_to_output}, fuse_add_to_output: {fuse_add_to_output}, enable_tf32: {enable_tf32}')
        if lhs is not None:
    ...
    dtype = type_name_to_flow_type[data_type]
    matmul_fn = make_matmul_func(a_shape, b_shape, transpose_a, transpose_b, alpha, dtype, device_type, test_add_to_output, fuse_add_to_output, enable_tf32)
    ...
    if transpose_a:
        ...
        a_diff_ = np_matmul(c_diff, b, transpose_a, not transpose_b, bias=None, alpha=alpha)
    ...
    if transpose_b:
        b_diff_ = np_matmul(c_diff.reshape((-1, c_diff.shape[-1])), a.reshape((-1, a.shape[-1])), True, transpose_a, bias=None, alpha=alpha)
    else:
        b_diff_ = np_matmul(a.reshape((-1, a.shape[-1])), c_diff.reshape((-1, c_diff.shape[-1])), not transpose_a, False, bias=None, alpha=alpha)
