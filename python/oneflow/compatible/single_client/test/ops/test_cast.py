...
def cast_forward_compare_with_tensorflow(test_cast, device_type, input_shape, dtype):
    ...
    def cast_forward(input_def: oft.Numpy.Placeholder(shape=input_shape, dtype=type_name_to_flow_type[dtype])):
...
def compare_with_tensorflow(device_type, input_shape, dtype):
    ...
    def CastJob():
        with flow.scope.placement(device_type, '0:0'):
            x = flow.get_variable('in', shape=input_shape, dtype=type_name_to_flow_type[dtype], initializer=flow.random_uniform_initializer(), trainable=True)
