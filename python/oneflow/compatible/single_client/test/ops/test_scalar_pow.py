...
def _compare_scalar_pow_with_np(input_shape, exponent, device_type, value_type, machine_ids, device_counts):
    ...
    def oneflow_pow(of_input_1: tp.Numpy.Placeholder(shape=input_1.shape, dtype=value_type[1])) -> tp.Numpy:
        ...
        with flow.scope.placement(device_type, '0:0'):
            flow.optimizer.SGD(flow.optimizer.PiecewiseConstantScheduler([], [0.001]), momentum=0).minimize(of_pow_out)
        return of_pow_out
...
class TestScalarPow1n1d(flow.unittest.TestCase):
    ...
    def test_scalar_pow_cpu(test_case):
        arg_dict = _gen_arg_dict(shape=(3, 3), exponent=1.4, device_type='cpu', value_type='float', machine_ids='0:0', device_counts=1)
        for arg in GenArgList(arg_dict):
            _compare_scalar_pow_with_np(*arg)

    @unittest.skipIf(os.getenv('ONEFLOW_TEST_CPU_ONLY'), 'only test cpu cases')
    def test_scalar_pow_gpu(test_case):
        arg_dict = _gen_arg_dict(shape=(4, 4), exponent=2.3, device_type='gpu', value_type='float', machine_ids='0:0', device_counts=1)
        for arg in GenArgList(arg_dict):
            _compare_scalar_pow_with_np(*arg)
...
class TestScalarPow1n2d(flow.unittest.TestCase):
    ...
    def test_pow_gpu_1n2d(test_case):
        arg_dict = _gen_arg_dict(shape=(4, 8, 4), exponent=2.0, device_type='gpu', value_type='float', machine_ids='0:0-1', device_counts=2)
        for arg in GenArgList(arg_dict):
            _compare_scalar_pow_with_np(*arg)
