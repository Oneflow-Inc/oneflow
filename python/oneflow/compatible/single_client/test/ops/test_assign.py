...
import os
flow_to_np_dtype_dict = {flow.int32: np.int32, flow.float: np.single, flow.double: np.float}
...
def _of_assign_and_relu(value, dtype, device_type, assign=flow.assign):
    ...
    def assign_fn(value_def: oft.Numpy.Placeholder(value.shape, dtype=dtype)):
        var = flow.get_variable(name='var', shape=value.shape, dtype=dtype, initializer=flow.constant_initializer(0))
