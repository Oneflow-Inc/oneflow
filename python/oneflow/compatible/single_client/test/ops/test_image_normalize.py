...
def _of_image_normalize(images, image_shape, std, mean):
    ...
    def image_normalize_job(images_def: oft.ListListNumpy.Placeholder(shape=image_shape, dtype=flow.float)):
        images_buffer = flow.tensor_list_to_tensor_buffer(images_def)
        norm_images = flow.image_normalize(images_buffer, std, mean)
        return flow.tensor_buffer_to_tensor_list(norm_images, shape=image_shape[1:], dtype=flow.float)
