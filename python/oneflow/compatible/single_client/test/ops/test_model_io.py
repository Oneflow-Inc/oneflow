...
def _make_gen_var_func(shape, dtype, lr):
    ...
    def gen_var(x: tp.Numpy.Placeholder(shape=shape, dtype=dtype)) -> tp.Numpy:
        var = flow.get_variable(name='var', shape=shape, dtype=dtype, initializer=flow.random_uniform_initializer())
        y = var + x
        flow.optimizer.SGD(flow.optimizer.PiecewiseConstantScheduler([], [lr]), momentum=0).minimize(y)
        return var
    return gen_var
...
def _make_get_var_func(shape, dtype):
    ...
    def get_var() -> tp.Numpy:
        return flow.get_variable(name='var', shape=shape, dtype=dtype, initializer=flow.random_uniform_initializer(), reuse=True)
    return get_var

def _load_snapshot_manually(path, shape, dtype):
    var_path = os.path.join(path, 'var', 'out')
    return np.fromfile(var_path, dtype=flow.convert_oneflow_dtype_to_numpy_dtype(dtype)).reshape(*shape)
...
def _test_model_io(test_case, shape, dtype, lr, num_iters):
    ...
    snapshot_path = model_save_root_dir + 'snapshot-{}'.format(time.strftime('%Y%m%d-%H:%M:%S'))
    checkpoint = flow.train.CheckPoint()
    checkpoint.init()
    variables = []
    for i in range(num_iters):
        var = gen_var(np.random.rand(*shape).astype(flow.convert_oneflow_dtype_to_numpy_dtype(dtype)))
        if i > 0:
