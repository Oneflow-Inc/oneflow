...
def compare_with_tensorflow(device_type, x_shape, data_type, axes):
    ...
    def MomentsJob():
        with flow.scope.placement(device_type, '0:0'):
            x = flow.get_variable('x', shape=x_shape, dtype=type_name_to_flow_type[data_type], initializer=flow.random_uniform_initializer(minval=-10, maxval=10), trainable=True)
