...
def summary_demo():
    ...
    with tempfile.TemporaryDirectory() as logdir:
        ...
        @flow.global_function(function_config=func_config)
        def ScalarJob(value: flow.typing.ListNumpy.Placeholder((1,), dtype=flow.float), step: flow.typing.ListNumpy.Placeholder((1,), dtype=flow.int64), tag: flow.typing.ListNumpy.Placeholder((1000,), dtype=flow.int8)):
            flow.summary.scalar(value, step, tag)

        @flow.global_function(function_config=func_config)
        def HistogramJob(value: flow.typing.ListNumpy.Placeholder((200, 200, 200), dtype=flow.float), step: flow.typing.ListNumpy.Placeholder((1,), dtype=flow.int64), tag: flow.typing.ListNumpy.Placeholder((9,), dtype=flow.int8)):
            flow.summary.histogram(value, step, tag)

        @flow.global_function(function_config=func_config)
        def PbJob(value: flow.typing.ListNumpy.Placeholder((1500,), dtype=flow.int8), step: flow.typing.ListNumpy.Placeholder((1,), dtype=flow.int64)):
            flow.summary.pb(value, step=step)

        @flow.global_function(function_config=func_config)
        def ImageJob(value: flow.typing.ListNumpy.Placeholder(shape=(100, 2000, 2000, 4), dtype=flow.uint8), step: flow.typing.ListNumpy.Placeholder((1,), dtype=flow.int64), tag: flow.typing.ListNumpy.Placeholder((10,), dtype=flow.int8)):
        ...
        hparams = {flow.summary.HParam('learning_rate', flow.summary.RealRange(0.01, 0.1)): 0.02, flow.summary.HParam('dense_layers', flow.summary.IntegerRange(2, 7)): 5, flow.summary.HParam('optimizer', flow.summary.ValueSet(['adam', 'sgd'])): 'adam', flow.summary.HParam('accuracy', flow.summary.RealRange(0.01, 0.1)): 0.001, flow.summary.HParam('magic', flow.summary.ValueSet([False, True])): True, flow.summary.Metric('loss', float): 0.02, 'dropout': 0.6}
        for i in range(200):
        ...
        for i in range(20):
            projecotr.exception_projector(value=value, tag=tag_exception, step=step, sample_name=sample_name, sample_type=sample_type, x=x)
            projecotr.embedding_projector(value=value, label=label, tag=tag_embedding, step=step, sample_name=sample_name, sample_type=sample_type, x=x)
        images = [cv2.cvtColor(np.ones([512, 512], np.uint8), cv2.COLOR_BGR2RGB).astype(np.uint8), cv2.cvtColor(np.ones([512, 512], np.uint8), cv2.COLOR_BGR2RGB).astype(np.uint8)]
