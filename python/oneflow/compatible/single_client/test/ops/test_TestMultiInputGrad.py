...
def TestMultiInput(x1, x2):
    return flow.user_op_builder('my_test_multi_input').Op('TestMultiInput').Input('x1', [x1]).Input('x2', [x2]).Output('y').Build().InferAndTryRun().RemoteBlobList()[0]
    ...
    def test_TestMultiInput_grad_mirrored_inplace(test_case):
        ...
        def TestMultiInputJob():
            with flow.scope.placement('gpu', '0:0'):
                x1 = flow.get_variable('x1', shape=shape, dtype=flow.float, initializer=flow.random_uniform_initializer(minval=-10, maxval=10), trainable=True)
                x2 = flow.get_variable('x2', shape=shape, dtype=flow.float, initializer=flow.random_uniform_initializer(minval=-10, maxval=10), trainable=True)
