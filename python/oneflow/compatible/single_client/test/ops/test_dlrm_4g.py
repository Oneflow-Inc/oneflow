...
func_config.indexed_slices_optimizer_conf(
    dict(include_op_names=dict(op_name=["embedding"]))
)
...
def embedding_lookup(indices, dim, name, embedding_name, optimizer):
    return (
...
def embedding_lookup_var(indices, dim, name, embedding_name, optimizer):
    indices = flow.hierarchical_parallel_cast(indices, nd_sbp=["B"])
    embedding_table = flow.get_variable(
        name="embedding",
        shape=(vocab_size, embedding_size),
        initializer=flow.random_uniform_initializer(minval=-0.00016, maxval=0.00016),
        distribute=flow.distribute.split(1),
    )
    embedding = flow.gather(params=embedding_table, indices=indices)
    embedding = flow.hierarchical_parallel_cast(
        embedding, nd_sbp=["S(0)"], grad_mode="manual", grad_nd_sbp=["S(2)"],
    )
    return embedding
...
hidden_units2 = [1024, 1024, 512, 256]
...
def _model(sparse_indices, dense, batch_tril_indices):
    sparse_embedding1 = embedding_lookup_var(
...
def main():
    ...
    check_point.init()
    check_point.save("init_model")
    for i in range(max_iter):
        of_out, logits = DlrmTrain(batch_tril_indices).get()
        # print("of_out", logits.numpy())
