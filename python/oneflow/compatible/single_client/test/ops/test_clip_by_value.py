...
def _of_clip_by_value(values, min, max, device_type='gpu', dynamic=False, grad_cb=None):
    ...
    if callable(grad_cb):
        ...
        def clip(values_blob):
            with flow.scope.placement(device_type, '0:0'):
                x = flow.get_variable('values', shape=values.shape, dtype=data_type, initializer=flow.constant_initializer(0))
    ...
    if dynamic:
        ...
        def clip_fn(values_def: oft.ListNumpy.Placeholder(values.shape, dtype=data_type)):
...
def _compare_with_tf(test_case, values, min, max, device_type, dynamic):
    ...
    def compare_dy(dy_blob):
        test_case.assertTrue(np.array_equal(dy.numpy(), dy_blob.numpy_list()[0] if dynamic else dy_blob.numpy()))
    of_y = _of_clip_by_value(values=values, min=min, max=max, device_type=device_type, dynamic=dynamic, grad_cb=compare_dy)
