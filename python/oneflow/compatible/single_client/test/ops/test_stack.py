...
def _compare_stack_with_np(input_shape, axis, device_type, machine_ids, device_counts):
    ...
    def oneflow_stack(of_input_1: tp.Numpy.Placeholder(shape=input_1.shape), of_input_2: tp.Numpy.Placeholder(shape=input_2.shape), of_mul: tp.Numpy.Placeholder(shape=np_random_mul.shape)) -> tp.Numpy:
...
class Teststack1n1d(flow.unittest.TestCase):
    ...
    def test_stack_cpu(test_case):
        arg_dict = _gen_arg_dict(shape=(4, 6), axis=2, device_type='cpu', machine_ids='0:0', device_counts=1)
        for arg in GenArgList(arg_dict):
            _compare_stack_with_np(*arg)

    @unittest.skipIf(os.getenv('ONEFLOW_TEST_CPU_ONLY'), 'only test cpu cases')
    def test_stack_gpu(test_case):
        arg_dict = _gen_arg_dict(shape=(3, 16, 32), axis=-4, device_type='gpu', machine_ids='0:0', device_counts=1)
        for arg in GenArgList(arg_dict):
            _compare_stack_with_np(*arg)
...
class Teststack1n2d(flow.unittest.TestCase):
    ...
    def test_stack_gpu_1n2d(test_case):
        arg_dict = _gen_arg_dict(shape=(3, 8, 8, 4), axis=3, device_type='gpu', machine_ids='0:0-1', device_counts=2)
        for arg in GenArgList(arg_dict):
            _compare_stack_with_np(*arg)
