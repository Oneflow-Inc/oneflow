...
class TestShuffle(flow.unittest.TestCase):
    ...
    def test_shuffle(_):
        ...
        for (device_type, x_shape, data_type) in GenArgList(arg_dict):
            ...
            def TestJob(x: oft.Numpy.Placeholder(x_shape, dtype=type_name_to_flow_type[data_type])):
            ...
            @flow.global_function(function_config=flow.FunctionConfig())
            def TestJob1(x: oft.Numpy.Placeholder(x_shape, dtype=type_name_to_flow_type[data_type])):
                with flow.scope.placement(device_type, '0:0'):
