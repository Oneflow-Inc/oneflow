...
def _compare_pixel_shuffle_with_np(input_shape, upscale_factor, device_type, machine_ids, device_counts):
    ...
    def np_pixel_shuffle(input):
        ...
        assert _channel % upscale_factor ** 2 == 0, 'The channels of input tensor must be divisible by (upscale_factor * upscale_factor)'
        _new_c = int(_channel / upscale_factor ** 2)
        out = np.reshape(input, [_batch, _new_c, upscale_factor * upscale_factor, _height, _width])
        out = np.reshape(out, [_batch * _new_c, upscale_factor, upscale_factor, _height, _width])
        out = np.transpose(out, [0, 3, 1, 4, 2])
        out = np.reshape(out, [_batch, _new_c, _height * upscale_factor, _width * upscale_factor])
        return out
    ...
    def oneflow_pixel_shuffle(of_input_1: tp.Numpy.Placeholder(shape=input_1.shape), of_mul: tp.Numpy.Placeholder(shape=np_random_mul.shape)) -> tp.Numpy:
        with flow.scope.placement(device_type, '0:0'):
            v = flow.get_variable(shape=input_1.shape, dtype=flow.float32, initializer=flow.zeros_initializer(), name='x_var')
            x_var = of_input_1 + v
        flow.watch_diff(x_var, assert_prediction_grad)
        of_pixel_shuffle_out = flow.nn.PixelShuffle(x_var, upscale_factor, name='PixelShuffle')
...
class TestPixelShuffle1n1d(flow.unittest.TestCase):
    ...
    def test_pixel_shuffle_cpu(test_case):
        arg_dict = _gen_arg_dict(shape=(3, 4, 2, 2), upscale_factor=2, device_type='cpu', machine_ids='0:0', device_counts=1)
        for arg in GenArgList(arg_dict):
            _compare_pixel_shuffle_with_np(*arg)

    @unittest.skipIf(os.getenv('ONEFLOW_TEST_CPU_ONLY'), 'only test cpu cases')
    def test_pixel_shuffle_gpu(test_case):
        arg_dict = _gen_arg_dict(shape=(3, 16, 2, 2), upscale_factor=2, device_type='gpu', machine_ids='0:0', device_counts=1)
        for arg in GenArgList(arg_dict):
            _compare_pixel_shuffle_with_np(*arg)
...
class TestPixelShuffle1n2d(flow.unittest.TestCase):
    ...
    def test_pixel_shuffle_gpu_1n2d(test_case):
        arg_dict = _gen_arg_dict(shape=(4, 16, 2, 2), upscale_factor=2, device_type='gpu', machine_ids='0:0-1', device_counts=2)
        for arg in GenArgList(arg_dict):
            _compare_pixel_shuffle_with_np(*arg)
