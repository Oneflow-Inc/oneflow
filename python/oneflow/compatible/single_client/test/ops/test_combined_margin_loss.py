...
def margin_loss(loss_m1, loss_m2, loss_m3, s, inputs, labels):
    ...
    if loss_m1 != 1.0 or loss_m2 != 0.0 or loss_m3 != 0.0:
        if loss_m1 == 1.0 and loss_m2 == 0.0:
            s_m = s * loss_m3
            gt_one_hot = flow.one_hot(labels, depth=class_num, on_value=s_m, off_value=0.0, dtype=flow.float)
            ...
            new_zy = body * s
            diff = new_zy - zy
            gt_one_hot = flow.one_hot(labels, depth=class_num, on_value=1.0, off_value=0.0, dtype=flow.float)
            body = gt_one_hot * diff
            inputs = inputs + body
    return inputs

def test_combined_margin_loss(test_case, device_type, input_shape, label_shape, data_type, m1, m2, m3, s):
    ...
    def test_job(x: oft.Numpy.Placeholder(input_shape, dtype=flow.float32), labels: oft.Numpy.Placeholder(label_shape, dtype=flow.int32)):
        ...
        with flow.scope.placement(device_type, '0:0-3'):
            y1 = flow.combined_margin_loss(x1.with_distribute(flow.distribute.split(1)), labels.with_distribute(flow.distribute.broadcast()), m1, m2, m3) * s
