...
def grouped_convolution2D(inputs, filters, padding, num_groups, strides=None, dilation_rate=None):
    ...
    for (conv_idx, (input_tensor, filter_tensor)) in enumerate(zip(input_list, filter_list)):
        output_list.append(tf.nn.conv3d(input_tensor, filter_tensor, padding='VALID', strides=[1, 1, 1, 1, 1], data_format='NDHWC'))
...
def compare_with_tensorflow(test_case, device_type, x_shape, filters, kernel_size, groups):
    ...
    def ConvJob():
        with flow.scope.placement(device_type, '0:0'):
            ...
            loss = flow.layers.conv3d(x, filters, kernel_size=kernel_size, strides=1, padding='valid', data_format='NCDHW', dilation_rate=1, groups=groups, use_bias=False, kernel_initializer=flow.random_uniform_initializer(minval=0, maxval=100), weight_name='conv3d_weight')
            weight_shape = (filters, x.shape[1] // groups, kernel_size, kernel_size, kernel_size)
            weight = flow.get_variable(name='conv3d_weight', shape=weight_shape, dtype=flow.float, initializer=flow.random_uniform_initializer(minval=0, maxval=100))
    ...
    with tf.GradientTape(persistent=True) as tape:
        ...
        if groups == 1:
            weight = tf.Variable(test_global_storage.Get('weight').transpose(2, 3, 4, 1, 0))
            tf_out = tf.nn.conv3d(x, weight, strides=[1, 1, 1, 1, 1], padding='VALID', data_format='NDHWC')
        else:
            weight = tf.Variable(test_global_storage.Get('weight').transpose(2, 3, 4, 1, 0))
    ...
    test_case.assertTrue(np.allclose(test_global_storage.Get('weight_diff').transpose(2, 3, 4, 1, 0), tf_weight_diff.numpy(), rtol=1e-05, atol=1e-05))
