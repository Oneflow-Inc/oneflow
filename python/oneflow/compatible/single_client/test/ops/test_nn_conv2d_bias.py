...
def grouped_convolution2D(inputs, filters, padding, num_groups, strides=None, dilation_rate=None):
    ...
    for (conv_idx, (input_tensor, filter_tensor)) in enumerate(zip(input_list, filter_list)):
        output_list.append(tf.nn.conv2d(input_tensor, filter_tensor, padding='VALID', strides=[1, 1, 1, 1], data_format='NHWC'))
    outputs = tf.concat(output_list, axis=-1)
    return outputs

def compare_with_tensorflow(device_type, x_shape, filters, kernel_size, groups, data_format='NCHW', padding='VALID', stride=1):
    ...
    def RunConvBias():
        with flow.scope.placement(device_type, '0:0'):
            ...
            weight = flow.get_variable('conv-weight', shape=weight_shape, dtype=flow.float, initializer=flow.random_uniform_initializer(minval=0, maxval=100))
            bias = flow.get_variable('conv-bias', shape=(filters,), dtype=flow.float, initializer=flow.random_uniform_initializer(minval=0, maxval=100))
            loss = flow.nn.conv2d(x, weight, bias=bias, strides=[stride, stride], padding=padding, dilations=[1, 1], groups=groups, name='conv')
    ...
    with tf.GradientTape(persistent=True) as tape:
        ...
        if groups == 1:
            ...
            conv_out = tf.nn.conv2d(x, weight, strides=[1, stride, stride, 1], padding=padding, data_format='NHWC')
        else:
            weight = tf.Variable(test_global_storage.Get('weight').transpose(weight_data_transpose))
            conv_out = grouped_convolution2D(x, weight, padding=padding, num_groups=groups)
    ...
    max_diff = np.max(np.absolute(of_out.numpy().transpose(xy_data_transpose) - tf_out.numpy()))
    assert np.allclose(of_out.numpy().transpose(xy_data_transpose), tf_out.numpy(), rtol=0.005, atol=0.005), max_diff
    assert np.allclose(test_global_storage.Get('x_diff').transpose(xy_data_transpose), tf_x_diff.numpy(), rtol=0.005, atol=0.005)
    assert np.allclose(test_global_storage.Get('weight_diff').transpose(weight_data_transpose), tf_weight_diff.numpy(), rtol=0.005, atol=0.005)
    assert np.allclose(test_global_storage.Get('bias_diff'), tf_bias_diff.numpy(), rtol=0.005, atol=0.005)
