...
def fused_cast_scale(x, scale_by_tensor, scale, name):
    return flow.user_op_builder(name).Op('fused_cast_scale').Input('x', [x]).Input('scale_by_tensor', [scale_by_tensor]).Output('y').Attr('scale', float(scale)).Build().InferAndTryRun().RemoteBlobList()[0]

def compare_with_tensorflow(device_type, input_shape, in_dtype, out_dtype, test_fuse_cast_scale_pass, has_scalar_mul):
    ...
    def FusedCastScaleJob():
        with flow.scope.placement(device_type, '0:0'):
            x = flow.get_variable('in', shape=input_shape, dtype=flow.float, initializer=flow.random_uniform_initializer(), trainable=True)
            scale = flow.get_variable('scale', shape=(1,), dtype=flow.float, initializer=flow.random_uniform_initializer(), trainable=False)
            ...
            if test_fuse_cast_scale_pass:
                ...
                if has_scalar_mul:
                    ...
                    scale_val = 1.0
                loss = fused_cast_scale(loss, flow.cast(scale, dtype=type_name_to_flow_type[out_dtype]), scale=scale_val, name='fused_cast_scale')
    ...
    with tf.GradientTape(persistent=True) as tape:
        ...
        tf_out = tf.cast(tf_out, dtype=type_name_to_np_type[out_dtype]) * tf.cast(scale, dtype=type_name_to_np_type[out_dtype])
        if has_scalar_mul:
            tf_out = tf_out * 0.125
