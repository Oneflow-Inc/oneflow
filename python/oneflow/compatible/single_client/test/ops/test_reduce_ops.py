...
def compare_reduce_any_with_tensorflow(device_type, input_shape, axis, keepdims, rtol=1e-05, atol=1e-05):
...
def compare_reduce_prod_with_tensorflow(device_type, input_shape, axis, keepdims, rtol=1e-05, atol=1e-05):
    assert device_type in ['gpu', 'cpu']
    flow.clear_default_session()
    func_config = flow.FunctionConfig()
...
def compare_reduce_min_with_tensorflow(device_type, input_shape, axis, keepdims, rtol=1e-05, atol=1e-05):
    assert device_type in ['gpu', 'cpu']
    flow.clear_default_session()
    func_config = flow.FunctionConfig()
    func_config.default_data_type(flow.float32)
    ...
    def ReduceMinJob(x: oft.Numpy.Placeholder(input_shape, dtype=flow.float)):
        with flow.scope.placement(device_type, '0:0'):
            x += flow.get_variable(name='v1', shape=input_shape, dtype=flow.float, initializer=flow.zeros_initializer())
...
def compare_reduce_all_with_tensorflow(device_type, input_shape, axis, keepdims, rtol=1e-05, atol=1e-05):
    assert device_type in ['gpu', 'cpu']
    flow.clear_default_session()
    func_config = flow.FunctionConfig()
    func_config.default_data_type(flow.int8)
...
def compare_reduce_sum_with_tensorflow(test_case, device_type, input_shape, axis, keepdims):
    assert device_type in ['gpu', 'cpu']
    flow.clear_default_session()
    func_config = flow.FunctionConfig()
...
def compare_reduce_euclidean_norm_with_tensorflow(device_type, input_shape, axis, keepdims, rtol=0.001, atol=0.001):
    assert device_type in ['gpu', 'cpu']
    flow.clear_default_session()
    func_config = flow.FunctionConfig()
    func_config.default_data_type(flow.float32)
...
def compare_reduce_logsumexp_with_tensorflow(device_type, input_shape, axis, keepdims, rtol=1e-05, atol=1e-05):
    assert device_type in ['gpu', 'cpu']
    flow.clear_default_session()
    func_config = flow.FunctionConfig()
    func_config.default_data_type(flow.float32)
...
def compare_reduce_std_with_tensorflow(device_type, input_shape, axis, keepdims, rtol=1e-05, atol=1e-05):
    assert device_type in ['gpu', 'cpu']
    flow.clear_default_session()
    func_config = flow.FunctionConfig()
    func_config.default_data_type(flow.float32)
...
def compare_reduce_variance_with_tensorflow(device_type, input_shape, axis, keepdims, rtol=1e-05, atol=1e-05):
    assert device_type in ['gpu', 'cpu']
    flow.clear_default_session()
    func_config = flow.FunctionConfig()
    func_config.default_data_type(flow.float32)
...
def compare_reduce_max_with_tensorflow(device_type, input_shape, axis, keepdims, rtol=1e-05, atol=1e-05):
    assert device_type in ['gpu', 'cpu']
    flow.clear_default_session()
    func_config = flow.FunctionConfig()
