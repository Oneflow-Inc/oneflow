...
def compare_with_np(device_type, device_num, data_type, max_input_length, batch_size, num_classes, max_target_length, blank, reduction, zero_infinity):
    ...
    if reduction == 'mean':
        np_out = np.mean(np.divide(np_out, np.clip(target_lengths, 1, a_max=None).astype(np_data_type)))
    elif reduction == 'sum':
        np_out = np.sum(np_out)
    np_grad_out = np.ones_like(np_loss, dtype=np_data_type)
    if reduction == 'mean':
        np_grad_out = np.divide(np_grad_out, np.clip(target_lengths, 1, a_max=None).astype(np_data_type))
    ...
    def ctc_loss_job(log_probs: tp.Numpy.Placeholder(shape=(max_input_length, batch_size, num_classes), dtype=flow_data_type), targets: tp.Numpy.Placeholder(shape=(batch_size, max_target_length), dtype=flow.int32), input_lengths: tp.Numpy.Placeholder(shape=(batch_size,), dtype=flow.int32), target_lengths: tp.Numpy.Placeholder(shape=(batch_size,), dtype=flow.int32)) -> tp.Numpy:
        with flow.scope.placement(device_type, '0:0'):
            v = flow.get_variable(shape=log_probs.shape, dtype=flow_data_type, initializer=flow.zeros_initializer(), name='x_var')
            x_var = log_probs + v
        flow.watch_diff(x_var, assert_loss_grad)
        loss = flow.ctc_loss(x_var, targets, input_lengths, target_lengths, blank, reduction, zero_infinity)
        with flow.scope.placement(device_type, '0:0'):
            flow.optimizer.SGD(flow.optimizer.PiecewiseConstantScheduler([], [0.001]), momentum=0).minimize(loss)
        return loss
