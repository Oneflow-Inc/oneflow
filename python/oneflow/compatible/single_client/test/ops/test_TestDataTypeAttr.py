...
def TestDataTypeAttr(input, output_type):
    ...
    return flow.user_op_builder('TestDataTypeAttr').Op('TestDataTypeAttr').Input('in', [input]).Output('out').Attr('output_type', output_type).Build().InferAndTryRun().RemoteBlobList()[0]
