...
def _compare_triplet_margin_loss_with_np(anchor_shape, pos_shape, neg_shape, eps, margin, p, swap, device_type, machine_ids, device_counts):
    ...
    def np_triplet_margin_loss(np_anchor, np_pos, np_neg, eps, np_margin, np_p, swap):
        ...
        return {'np_triplet_margin_loss': np_triplet_margin_loss, 'np_triplet_margin_loss_mean': np_triplet_margin_loss_mean, 'np_triplet_margin_loss_sum': np_triplet_margin_loss_sum}
    ...
    def oneflow_marginloss(of_anchor: tp.Numpy.Placeholder(shape=anchor.shape), of_pos: tp.Numpy.Placeholder(shape=pos.shape), of_neg: tp.Numpy.Placeholder(shape=neg.shape)) -> Dict[str, tp.Numpy]:
        with flow.scope.placement(device_type, '0:0'):
            v = flow.get_variable(shape=anchor.shape, dtype=flow.float32, initializer=flow.constant_initializer(0), name='x_var')
            x_anchor = of_anchor + v
        flow.watch_diff(x_anchor, assert_prediction_grad)
        triplet_marginloss = flow.nn.TripletMarginLoss(x_anchor, of_pos, of_neg, margin=margin, p=p, swap=swap, reduction='none', name='of_tripletmarginloss')
        ...
        triplet_marginloss_sum = flow.nn.TripletMarginLoss(x_anchor, of_pos, of_neg, margin=margin, p=p, swap=swap, reduction='sum', name='of_tripletmarginloss_sum')
        with flow.scope.placement(device_type, '0:0'):
            flow.optimizer.SGD(flow.optimizer.PiecewiseConstantScheduler([], [0.001]), momentum=0).minimize(triplet_marginloss_mean)
        return {'of_triplet_margin_loss': triplet_marginloss, 'of_triplet_margin_loss_mean': triplet_marginloss_mean, 'of_triplet_margin_loss_sum': triplet_marginloss_sum}
    of_out_tripletloss_dict = oneflow_marginloss(anchor, pos, neg)
    assert np.allclose(of_out_tripletloss_dict['of_triplet_margin_loss'], np_out_tripletloss_dict['np_triplet_margin_loss'], atol=0.001)
    assert np.allclose(of_out_tripletloss_dict['of_triplet_margin_loss_mean'], np_out_tripletloss_dict['np_triplet_margin_loss_mean'], atol=0.001)
    assert np.allclose(of_out_tripletloss_dict['of_triplet_margin_loss_sum'], np_out_tripletloss_dict['np_triplet_margin_loss_sum'], atol=0.001)
...
class Test_triplet_loss_1n1d(flow.unittest.TestCase):
    ...
    def test_triplet_margin_loss_cpu(test_case):
        arg_dict = _gen_arg_dict(shape=(3, 3), eps=1e-06, margin=1, p=1.5, swap=False, device_type='cpu', machine_ids='0:0', device_counts=1)
        for arg in GenArgList(arg_dict):
            _compare_triplet_margin_loss_with_np(*arg)

    @unittest.skipIf(os.getenv('ONEFLOW_TEST_CPU_ONLY'), 'only test cpu cases')
    def test_margin_ranking_loss_gpu(test_case):
        arg_dict = _gen_arg_dict(shape=(3, 6), eps=1e-06, margin=1, p=2.0, swap=False, device_type='gpu', machine_ids='0:0', device_counts=1)
        for arg in GenArgList(arg_dict):
            _compare_triplet_margin_loss_with_np(*arg)
...
class Testmarginloss1n2d(flow.unittest.TestCase):
    ...
    def test_margin_ranking_loss_1n2d(test_case):
        arg_dict = _gen_arg_dict(shape=(6, 6), eps=1e-06, margin=1, p=2.0, swap=False, device_type='gpu', machine_ids='0:0-1', device_counts=2)
        for arg in GenArgList(arg_dict):
            _compare_triplet_margin_loss_with_np(*arg)
