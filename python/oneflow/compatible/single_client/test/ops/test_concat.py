...
def compare_with_tensorflow(device_type, x_shape, y_shape, dtype, axis):
    ...
    def ConcatJob():
        with flow.scope.placement(device_type, '0:0'):
            x = flow.get_variable('x', shape=x_shape, dtype=type_name_to_flow_type[dtype], initializer=flow.random_uniform_initializer(minval=-10, maxval=10), trainable=True)
            y = flow.get_variable('y', shape=y_shape, dtype=type_name_to_flow_type[dtype], initializer=flow.random_uniform_initializer(minval=-10, maxval=10), trainable=True)
...
def _of_dynamic_concat(inputs, input_static_shape, axis, device_type, watch_cb=None, make_watch_diff_cb=None):
    ...
    @flow.global_function(type='train', function_config=func_config)
    def dynamic_concat_job(input_0_def: oft.ListNumpy.Placeholder(shape=input_static_shape, dtype=flow.float), input_1_def: oft.ListNumpy.Placeholder(shape=input_static_shape, dtype=flow.float)):
        var_0 = flow.get_variable('Var0', shape=(1,), dtype=flow.float, initializer=flow.constant_initializer(value=1, dtype=flow.float), trainable=True)
        var_1 = flow.get_variable('Var1', shape=(1,), dtype=flow.float, initializer=flow.constant_initializer(value=1, dtype=flow.float), trainable=True)
        ...
        if callable(watch_cb):
            flow.watch(var_0, watch_cb)
            flow.watch(var_1, watch_cb)
        result = flow.concat([var_0, var_1], axis=axis, max_dim_size=input_static_shape[axis])
        flow.optimizer.SGD(flow.optimizer.PiecewiseConstantScheduler([], [0.0001]), momentum=0).minimize(result)
...
def _test_dynamic_concat(test_case, shape, axis, device_type, verbose=False):
    ...
    def make_watch_diff_cb(input_idx):
        ...
        def watch_diff_cb(blob):
            test_case.assertTrue(np.array_equal(blob.numpy(0), np.ones(shape=inputs[input_idx].shape, dtype=np.single)))
        return watch_diff_cb
    of_output = _of_dynamic_concat(inputs, tuple(shape), axis, device_type, print_blob if verbose else None, make_watch_diff_cb)
    if verbose:
...
def _test_static_concat(test_case, shape, axis):
    ...
    def compare_var_diff(var_blob):
        test_case.assertTrue(np.array_equal(var_blob.numpy(), np.ones(shape=shape, dtype=np.single)))

    @flow.global_function(type='train', function_config=func_config)
    def static_concat_job(input_0_def: oft.Numpy.Placeholder(shape=shape, dtype=flow.float), input_1_def: oft.Numpy.Placeholder(shape=shape, dtype=flow.float)):
        var = flow.get_variable('var', shape=shape, dtype=flow.float, initializer=flow.random_uniform_initializer(), trainable=True)
        concated = flow.concat([input_0_def, input_1_def, var], axis=axis)
        test_case.assertTrue(not concated.is_dynamic)
        flow.optimizer.SGD(flow.optimizer.PiecewiseConstantScheduler([], [0.001]), momentum=0).minimize(concated)
    ...
    test_case.assertTrue(np.array_equal(np.concatenate([inputs[0], inputs[1], var.numpy()], axis=axis), concated.numpy()))
...
def _test_hybrid_concat(test_case, static_shape, axis, max_dim_size=None, verbose=False):
    ...
    def compare_var_diff(var_blob):
        test_case.assertTrue(np.array_equal(var_blob.numpy(), np.ones(shape=static_shape, dtype=np.single)))
    ...
    @flow.global_function(type='train', function_config=func_config)
    def hybrid_concat_job(input_0_def: oft.ListNumpy.Placeholder(shape=static_shape, dtype=flow.float), input_1_def: oft.ListNumpy.Placeholder(shape=static_shape, dtype=flow.float)):
        var = flow.get_variable('var', shape=static_shape, dtype=flow.float, initializer=flow.random_uniform_initializer(), trainable=True)
        constant = flow.constant(1.0, dtype=flow.float, shape=rand_sub_shape)
        inputs = [flow.cast_to_current_logical_view(input) for input in [var, input_0_def, input_1_def, constant]]
    ...
    test_case.assertTrue(np.array_equal(np.concatenate([var.numpy(), output, np.ones(shape=rand_sub_shape, dtype=np.single)], axis=axis), concated.numpy(0)))
