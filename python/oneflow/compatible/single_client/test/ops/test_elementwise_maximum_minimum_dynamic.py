...
def _compare_dynamic_Xmum_with_np(input_shape, data_shape, compare_type, device_type, machine_ids, device_counts):
    ...
    def oneflow_Xmum(of_input_1: tp.ListNumpy.Placeholder(shape=data_shape), of_input_2: tp.ListNumpy.Placeholder(shape=data_shape)) -> tp.ListNumpy:
        with flow.scope.placement(device_type, '0:0'):
            v1 = flow.get_variable(shape=(1,), dtype=flow.float32, initializer=flow.zeros_initializer(), name='x1_var')
            v1 = flow.cast_to_current_logical_view(v1)
            x1_var = of_input_1 + v1
            v2 = flow.get_variable(shape=(1,), dtype=flow.float32, initializer=flow.zeros_initializer(), name='x2_var')
...
def _gen_arg_dict(shape, data_shape, compare_type, device_type, machine_ids, device_counts):
...
class TestXmum1n1d(flow.unittest.TestCase):
    ...
    def test_Xmum_cpu(test_case):
        arg_dict = _gen_arg_dict(shape=(4, 4), data_shape=(10, 10), compare_type=['maximum', 'minimum'], device_type='cpu', machine_ids='0:0', device_counts=1)
        for arg in GenArgList(arg_dict):
            _compare_dynamic_Xmum_with_np(*arg)

    @unittest.skipIf(os.getenv('ONEFLOW_TEST_CPU_ONLY'), 'only test cpu cases')
    def test_Xmum_gpu(test_case):
        arg_dict = _gen_arg_dict(shape=(3, 3), data_shape=(10, 10), compare_type=['maximum', 'minimum'], device_type='gpu', machine_ids='0:0', device_counts=1)
        for arg in GenArgList(arg_dict):
            _compare_dynamic_Xmum_with_np(*arg)
