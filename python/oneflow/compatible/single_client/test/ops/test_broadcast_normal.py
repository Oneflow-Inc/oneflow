...
from test_util import GenArgDict, GenArgList, type_name_to_flow_type, type_name_to_np_type
...
def RunOneflowOp(device_type, flow_op, x, y, data_type):
    ...
    def FlowJob(x: oft.Numpy.Placeholder(x.shape, dtype=flow_type), y: oft.Numpy.Placeholder(y.shape, dtype=flow_type)):
        with flow.scope.placement(device_type, '0:0'):
            x += flow.get_variable(name='x', shape=x.shape, dtype=flow_type, initializer=flow.zeros_initializer(), trainable=True)
            y += flow.get_variable(name='y', shape=y.shape, dtype=flow_type, initializer=flow.zeros_initializer(), trainable=True)
            loss = flow_op(x, y)
...
def compare_with_tensorflow_grad(device_type, flow_op, tf_op, x_shape, y_shape, data_type, input_minval=-10, input_maxval=10, out_rtol=1e-05, out_atol=1e-05, diff_rtol=0.0001, diff_atol=0.001):
    assert device_type in ['gpu', 'cpu']
    np_type = type_name_to_np_type[data_type]
    x = np.random.uniform(low=input_minval, high=input_maxval, size=x_shape).astype(np_type)
    y = np.random.uniform(low=input_minval, high=input_maxval, size=y_shape).astype(np_type)
...
def compare_with_tensorflow(device_type, flow_op, tf_op, x_shape, y_shape, data_type, input_minval=-10, input_maxval=10, out_rtol=1e-05, out_atol=1e-05):
    assert device_type in ['gpu', 'cpu']
    flow.clear_default_session()
    func_config = flow.FunctionConfig()
    func_config.default_data_type(flow.float)
    ...
    def FlowJob(x: oft.Numpy.Placeholder(x_shape, dtype=flow_type), y: oft.Numpy.Placeholder(y_shape, dtype=flow_type)):
        with flow.scope.placement(device_type, '0:0'):
            return flow_op(x, y)
    np_type = type_name_to_np_type[data_type]
    if np_type in (np.int8, np.int32, np.int64):
        x = np.random.randint(low=input_minval, high=input_maxval, size=x_shape).astype(np_type)
        y = np.random.randint(low=input_minval, high=input_maxval, size=y_shape).astype(np_type)
    else:
        x = np.random.uniform(low=input_minval, high=input_maxval, size=x_shape).astype(np_type)
        y = np.random.uniform(low=input_minval, high=input_maxval, size=y_shape).astype(np_type)
    if flow_op in (flow.math.divide, flow.math.mod):
