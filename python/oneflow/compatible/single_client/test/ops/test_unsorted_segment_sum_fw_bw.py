...
def _random_inputs(data_shape, segment_ids_shape, axis, num_segments):
    ...
    segment_ids = np.random.randint(low=0, high=num_segments, size=segment_ids_shape, dtype=np.int32)
...
def _make_unsorted_segment_sum_fn(data, segment_ids, axis, num_segments, device_type, mirrored, compare_fn):
    ...
    def do_unsorted_segment_sum(x_blob, i_blob):
        with flow.scope.placement(device_type, '0:0'):
            x = flow.get_variable('data', shape=data.shape, dtype=flow.float32, initializer=flow.constant_initializer(0))
            x = x + x_blob
            y = flow.math.unsorted_segment_sum(x, i_blob, axis=axis, num_segments=num_segments)
    ...
    if mirrored:
        ...
        def unsorted_segment_sum_fn(data_def: oft.ListNumpy.Placeholder(data.shape, dtype=flow.float), segment_ids_def: oft.ListNumpy.Placeholder(segment_ids.shape, dtype=flow.int32)):
            return do_unsorted_segment_sum(data_def, segment_ids_def)
    else:

        @flow.global_function(type='train', function_config=func_config)
        def unsorted_segment_sum_fn(data_def: oft.Numpy.Placeholder(data.shape, dtype=flow.float), segment_ids_def: oft.Numpy.Placeholder(segment_ids.shape, dtype=flow.int32)):
            return do_unsorted_segment_sum(data_def, segment_ids_def)
    return unsorted_segment_sum_fn

def _compare_unsorted_segment_sum_with_tf(test_case, device_type, data_shape, segment_ids_shape, axis, num_segments, mirrored=False):
    (data, segment_ids) = _random_inputs(data_shape, segment_ids_shape, axis, num_segments)
    ...
    unsorted_segment_sum_fn = _make_unsorted_segment_sum_fn(data, segment_ids, axis, num_segments, device_type, mirrored, compare_dy)
    if mirrored:
