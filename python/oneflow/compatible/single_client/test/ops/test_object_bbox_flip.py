...
def _of_object_bbox_flip(bbox_list, image_size, flip_code):
    ...
    def object_bbox_flip_job(bbox_def: oft.ListListNumpy.Placeholder(shape=tuple(bbox_shape), dtype=flow.float), image_size_def: oft.ListNumpy.Placeholder(shape=image_size.shape, dtype=flow.int32)):
...
def _compare_bbox_flip(test_case, anno_file, batch_size, flip_code, print_debug_info=False):
    ...
    while sample_cnt < batch_size:
        ...
        image_size_list.append([coco.imgs[rand_img_id]['width'], coco.imgs[rand_img_id]['height']])
        sample_cnt += 1
