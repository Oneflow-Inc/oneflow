...
def getInstanceNorm1DOutAndGrad(input, gout, eps):
    ...
    gmean += tmp
    gin_np = gout_reshape_to_1d * gamma * invar_np + gvar * scale * 2.0 * in_sub_mean + gmean * scale
    return (np.reshape(out_np, list(input.shape)), np.reshape(gin_np, list(input.shape)))

def _compare_instance_norm_nd_with_np(input_shape, device_type, machine_ids, device_counts, eps, affine):
    ...
    def instanceNormJob(of_input: tp.Numpy.Placeholder(shape=input.shape), multipler: tp.Numpy.Placeholder(shape=input.shape)) -> tp.Numpy:
