...
def split_like(input, like, name):
    return flow.user_op_builder(name).Op('split_like').Input('in', [input]).Input('like', like).Output('out', len(like)).Attr('axis', 0).Build().InferAndTryRun().RemoteBlobList()
...
def compare_with_np(device_type, x_shape, like0_shape, like1_shape, dtype):
    ...
    def SplitLikeJob(x: oft.Numpy.Placeholder(x_shape, dtype=flow.float)):
        v = flow.get_variable('x', shape=x_shape, dtype=flow.float, initializer=flow.constant_initializer(0), trainable=True)
        x += v
