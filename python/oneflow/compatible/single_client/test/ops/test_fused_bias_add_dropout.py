...
def compare_with_not_fused(test_case, device_type, x_shape, data_type, data_format, rate, seed, fuse_add_to_output):
    ...
    def FlowJob(value: oft.Numpy.Placeholder(x_shape), bias: oft.Numpy.Placeholder(bias_shape), addend: oft.Numpy.Placeholder(x_shape)):
        with flow.scope.placement(device_type, '0:0'):
            ...
            addend += flow.get_variable(name='v3', shape=(1,), dtype=flow.float, initializer=flow.zeros_initializer())
            ...
            if data_type == 'float16':
                out1 = flow.nn.dropout(flow.nn.bias_add(flow.cast(x1, dtype=flow.float16), flow.cast(bias1, dtype=flow.float16), data_format=data_format), rate=rate, seed=seed, name='dropout')
                y1 = flow.cast(out1 + flow.cast(addend1, dtype=flow.float16), dtype=flow.float)
                out2 = flow.nn.fused_bias_add_dropout(flow.cast(x2, dtype=flow.float16), flow.cast(bias2, dtype=flow.float16), data_format=data_format, rate=rate, seed=seed)
                y2 = flow.cast(out2 + flow.cast(addend2, dtype=flow.float16), dtype=flow.float)
            else:
                y1 = flow.nn.dropout(flow.nn.bias_add(x1, bias1, data_format=data_format), rate=rate, seed=seed, name='dropout') + addend1
                y2 = flow.nn.fused_bias_add_dropout(x2, bias2, data_format=data_format, rate=rate, seed=seed) + addend2
    ...
    test_case.assertTrue(np.allclose(bias1_diff, bias2_diff, rtol=tol, atol=tol, equal_nan=True))
    bias1_diff = test_global_storage.Get('bias1_diff')
    bias2_diff = test_global_storage.Get('bias2_diff')
    test_case.assertTrue(np.allclose(bias1_diff, bias2_diff, rtol=tol, atol=tol, equal_nan=True))
