...
def _make_slice_func(slice_args, input_shape, dtype=flow.float32, func_cfg=None):
    ...
    def slice_job(x: otp.Numpy.Placeholder(shape=input_shape, dtype=dtype)) -> tp.List[otp.Numpy]:
...
def _make_slice_with_fp16_func(slice_args, input_shape, func_cfg=None):
    ...
    @flow.global_function(type='predict', function_config=func_cfg)
    def slice_job(x: otp.Numpy.Placeholder(shape=input_shape, dtype=flow.float32)) -> tp.List[otp.Numpy]:
        x = flow.cast(x, flow.float16)
        y = _do_slice(x, slice_args)
        return [flow.cast(y_i, flow.float32) for y_i in y]
    return slice_job

def _make_slice_dynamic_func(slice_args, input_shape, dtype=flow.float32, func_cfg=None):
    if func_cfg is None:
        func_cfg = flow.FunctionConfig()
        func_cfg.default_logical_view(flow.scope.mirrored_view())

    @flow.global_function(type='predict', function_config=func_cfg)
    def slice_dynamic_job(x: otp.ListNumpy.Placeholder(shape=input_shape, dtype=dtype)) -> tp.List[otp.ListNumpy]:
        return _do_slice(x, slice_args, name='SliceDynamic')
    return slice_dynamic_job

def _make_slice_with_grad_func(slice_tup_list, input_shape, watch_diff_cb=None, dtype=flow.float32, func_cfg=None):

    @flow.global_function(type='train', function_config=func_cfg)
    def slice_with_grad_job(x: otp.Numpy.Placeholder(shape=input_shape, dtype=dtype)) -> otp.Numpy:
        var = flow.get_variable(shape=input_shape, dtype=dtype, initializer=flow.constant_initializer(0.0), name='variable')
        x = x + var
...
def _make_slice_update_func(slice_tup_list, input_shape, update_shape, dtype=flow.float32, func_cfg=None):

    @flow.global_function(type='predict', function_config=func_cfg)
    def slice_update_job(x: otp.Numpy.Placeholder(shape=input_shape, dtype=dtype), update: otp.Numpy.Placeholder(shape=update_shape, dtype=dtype)) -> otp.Numpy:
        return flow.slice_update(x, update, slice_tup_list)
    return slice_update_job

def _make_slice_update_grad_func(slice_tup_list, input_shape, update_shape, diff_watcher_maker=None, dtype=flow.float32, func_cfg=None):

    @flow.global_function(type='train', function_config=func_cfg)
    def slice_update_train_job(x: otp.Numpy.Placeholder(shape=input_shape, dtype=dtype), update: otp.Numpy.Placeholder(shape=update_shape, dtype=dtype)) -> otp.Numpy:
        x_var = flow.get_variable(shape=input_shape, dtype=dtype, initializer=flow.constant_initializer(0.0), name='x')
        update_var = flow.get_variable(shape=update_shape, dtype=dtype, initializer=flow.constant_initializer(0.0), name='update')
        x = x + x_var
...
def _test_slice(test_case, input, slice_args, outputs, dtype=flow.float32, device_tag=DEFAULT_DEVICE_TAG, verbose=False):
    input = input.astype(flow.convert_oneflow_dtype_to_numpy_dtype(dtype))
    outputs = [output.astype(flow.convert_oneflow_dtype_to_numpy_dtype(dtype)) for output in outputs]
...
def _test_slice_dynamic(test_case, input, slice_args, outputs, static_shape=None, dtype=flow.float32, device_tag=DEFAULT_DEVICE_TAG):
    input = input.astype(flow.convert_oneflow_dtype_to_numpy_dtype(dtype))
    outputs = [output.astype(flow.convert_oneflow_dtype_to_numpy_dtype(dtype)) for output in outputs]
...
def _test_slice_with_grad(test_case, input, slice_args, output, diff, dtype=flow.float32, device_tag=DEFAULT_DEVICE_TAG, verbose=False):
    input = input.astype(flow.convert_oneflow_dtype_to_numpy_dtype(dtype))
    ...
    func_cfg.default_data_type(dtype)
    func_cfg.default_placement_scope(flow.scope.placement(device_tag, '0:0'))
    slice_func = _make_slice_with_grad_func(slice_args, input.shape, WatchDiff, dtype, func_cfg)
    of_output = slice_func(input)
    if verbose:
        print('of_output: {}\n{}\n'.format(of_output.shape, of_output))
    test_case.assertTrue(np.array_equal(output, of_output))

def _test_slice_update(test_case, input, update, slice_args, output, dtype=flow.float32, device_tag=DEFAULT_DEVICE_TAG, verbose=False):
    input = input.astype(flow.convert_oneflow_dtype_to_numpy_dtype(dtype))
    ...
    func_cfg.default_data_type(dtype)
    func_cfg.default_placement_scope(flow.scope.placement(device_tag, '0:0'))
    slice_func = _make_slice_update_func(slice_args, input.shape, update.shape, dtype, func_cfg)
...
def _test_slice_update_grad(test_case, input, update, slice_args, output, input_diff, update_diff, dtype=flow.float32, device_tag=DEFAULT_DEVICE_TAG, verbose=False):
    input = input.astype(flow.convert_oneflow_dtype_to_numpy_dtype(dtype))
    update = update.astype(flow.convert_oneflow_dtype_to_numpy_dtype(dtype))
    output = output.astype(flow.convert_oneflow_dtype_to_numpy_dtype(dtype))
    ...
    func_cfg.default_data_type(dtype)
    func_cfg.default_placement_scope(flow.scope.placement(device_tag, '0:0'))
    slice_func = _make_slice_update_grad_func(slice_args, input.shape, update.shape, _make_diff_watcher, dtype, func_cfg)
...
class TestSliceV2(flow.unittest.TestCase):
    ...
    def test_slice_base(test_case):
        ...
        arg_dict['dtype'] = [flow.uint8, flow.int8, flow.int32, flow.int64, flow.float32, flow.float64]
    ...
    def test_slice_into_two_parts(test_case):
        ...
        slice_args = [[(None, None, None), (0, 2, None), (None, None, None)], [(None, None, None), (2, None, None), (None, None, None)]]
    ...
    def test_slice_with_collapse_dims(test_case):
        ...
        slice_args = [[(None, None, None), (0, 2, None), (None, None, None), (None, None, None), (1, None, None)]]
        outputs = [input[:, 0:2, :, :, 1:]]
        _test_slice(test_case, input, slice_args, outputs)
    ...
    def test_slice_with_float16(test_case):
        ...
        of_outputs = slice_func(input)
        test_case.assertTrue(np.allclose(outputs[0], of_outputs[0], rtol=0.001, atol=0.0001))
    ...
    def test_slice_dynamic_base(test_case):
        ...
        arg_dict['device_tag'] = ['cpu', 'gpu']
        for kwarg in test_util.GenArgDict(arg_dict):
            _test_slice_dynamic(test_case, input, slice_args, outputs, static_shape=(2, 5, 5), **kwarg)

    def test_slice_dynamic_at_two_dims(test_case):
        input = np.random.rand(2, 3, 2, 2)
        slice_args = [[(None, None, None), (2, None, None), (None, None, None), (1, None, None)]]
        outputs = [input[:, 2:, :, 1:]]
        _test_slice_dynamic(test_case, input, slice_args, outputs, static_shape=(2, 5, 3, 3))

    def test_slice_dynamic_at_first_dim_and_last_dim(test_case):
        input = np.random.rand(3, 6, 3, 3)
        slice_args = [[(1, None, None), (None, None, None), (None, None, None), (1, None, None)]]
        outputs = [input[1:, :, :, 1:]]
        _test_slice_dynamic(test_case, input, slice_args, outputs, static_shape=(4, 5, 5, 3))
    ...
    def test_slice_dynamic_empty_blob(test_case):
        ...
        outputs = [input[:, :, 2:3]]
        _test_slice_dynamic(test_case, input, slice_args, outputs, static_shape=(8, 2, 10))
    ...
    def test_slice_update_grad(test_case):
        ...
        arg_dict['verbose'] = [False]
        for kwarg in test_util.GenArgDict(arg_dict):
            _test_slice_update_grad(test_case, input, update, slice_tup_list, output, input_diff, update_diff, **kwarg)
