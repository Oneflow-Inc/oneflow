...
def compare_with_tensorflow(device_type, x_shape, filters, kernel_size, groups, of_padding='SAME', tf_padding='SAME', stride_h=1, stride_w=1, data_format='NCHW', dilation_h=1, dilation_w=1):
    ...
    def ConvJob():
        with flow.scope.placement(device_type, '0:0'):
            ...
            loss = flow.nn.conv2d(x, weight, strides=[stride_h, stride_w], padding=of_padding, data_format=data_format, dilations=[dilation_h, dilation_w], groups=groups)
    ...
    with tf.GradientTape(persistent=True) as tape:
        ...
        tf_out = tf.nn.conv2d(x, weight, strides=[1, stride_h, stride_w, 1], padding=tf_padding, data_format='NHWC', dilations=[1, dilation_h, dilation_w, 1])
    ...
    idx = np.where(np.abs(of_out.numpy().transpose(xy_data_transpose) - tf_out.numpy()) > 0.0005)
    assert np.allclose(of_out.numpy().transpose(xy_data_transpose), tf_out.numpy(), rtol=1e-05, atol=1e-05)
    assert np.allclose(test_global_storage.Get('x_diff').transpose(xy_data_transpose), tf_x_diff.numpy(), rtol=0.0001, atol=0.0001)
    assert np.allclose(test_global_storage.Get('weight_diff').transpose(weight_data_transpose), tf_weight_diff.numpy(), rtol=1e-05, atol=1e-05)
