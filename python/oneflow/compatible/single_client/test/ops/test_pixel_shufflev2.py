...
def _compare_pixel_shuffle_with_np(input_shape, h_factor, w_factor, device_type, machine_ids, device_counts):
    ...
    def oneflow_pixel_shuffle(of_input_1: tp.Numpy.Placeholder(shape=input_1.shape)) -> tp.Numpy:
        with flow.scope.placement(device_type, '0:0'):
            v = flow.get_variable(shape=input_1.shape, dtype=flow.float32, initializer=flow.zeros_initializer(), name='x_var')
            x_var = of_input_1 + v
        flow.watch_diff(x_var, assert_prediction_grad)
        of_pixel_shuffle_out = flow.nn.PixelShufflev2(x_var, h_factor, w_factor, name='PixelShufflev2')
        with flow.scope.placement(device_type, '0:0'):
            flow.optimizer.SGD(flow.optimizer.PiecewiseConstantScheduler([], [0.001]), momentum=0).minimize(of_pixel_shuffle_out)
...
class TestPixelShuffle1n1d(flow.unittest.TestCase):
    ...
    def test_pixel_shuffle_cpu(test_case):
        arg_dict = _gen_arg_dict(shape=(3, 16, 2, 4), h_factor=2, w_factor=4, device_type='cpu', machine_ids='0:0', device_counts=1)
        for arg in GenArgList(arg_dict):
            _compare_pixel_shuffle_with_np(*arg)

    @unittest.skipIf(os.getenv('ONEFLOW_TEST_CPU_ONLY'), 'only test cpu cases')
    def test_pixel_shuffle_gpu(test_case):
        arg_dict = _gen_arg_dict(shape=(2, 16, 2, 2), h_factor=2, w_factor=2, device_type='gpu', machine_ids='0:0', device_counts=1)
        for arg in GenArgList(arg_dict):
            _compare_pixel_shuffle_with_np(*arg)
...
class TestPixelShuffle1n2d(flow.unittest.TestCase):
    ...
    def test_pixel_shuffle_gpu_1n2d(test_case):
        arg_dict = _gen_arg_dict(shape=(4, 16, 2, 4), h_factor=2, w_factor=2, device_type='gpu', machine_ids='0:0-1', device_counts=2)
        for arg in GenArgList(arg_dict):
            _compare_pixel_shuffle_with_np(*arg)
