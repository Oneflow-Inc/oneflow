...
def _compare_relu6_with_np(input_shape, device_type, value_type, machine_ids, device_counts):
    ...
    if value_type[1] == flow.float16:
        ...
        def oneflow_relu6(of_input_1: tp.Numpy.Placeholder(shape=input_1.shape, dtype=flow.float32)) -> tp.Numpy:
            ...
            with flow.scope.placement(device_type, '0:0'):
                flow.optimizer.SGD(flow.optimizer.PiecewiseConstantScheduler([], [0.001]), momentum=0).minimize(of_relu6_out_f32)
            flow.watch_diff(x_var, assert_prediction_grad)
            return of_relu6_out_f32
    elif value_type[1] == flow.float32 or value_type[1] == flow.float64:

        @flow.global_function(type='train', function_config=func_config)
        def oneflow_relu6(of_input_1: tp.Numpy.Placeholder(shape=input_1.shape, dtype=value_type[1])) -> tp.Numpy:
            with flow.scope.placement(device_type, '0:0'):
            ...
            with flow.scope.placement(device_type, '0:0'):
                flow.optimizer.SGD(flow.optimizer.PiecewiseConstantScheduler([], [0.001]), momentum=0).minimize(of_relu6_out)
            return of_relu6_out
