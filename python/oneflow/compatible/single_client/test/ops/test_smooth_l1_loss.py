...
def gen_numpy_data(prediction, label, beta=1.0):
    ...
    return {'loss': loss.reshape(original_shape), 'prediction_grad': prediction_grad.reshape(original_shape)}
    ...
    def test_smooth_l1_loss(_):
        ...
        for case in GenArgList(arg_dict):
            ...
            def assert_prediction_grad(b):
                ...
                assert np.allclose(prediction_grad, b.numpy()), (case, prediction_grad, b.numpy())

            @flow.global_function(type='train', function_config=func_config)
            def TestJob(prediction: oft.Numpy.Placeholder(prediction_shape, dtype=type_name_to_flow_type[data_type]), label: oft.Numpy.Placeholder(prediction_shape, dtype=type_name_to_flow_type[data_type])):
                v = flow.get_variable('prediction', shape=prediction_shape, dtype=type_name_to_flow_type[data_type], initializer=flow.constant_initializer(0), trainable=True)
