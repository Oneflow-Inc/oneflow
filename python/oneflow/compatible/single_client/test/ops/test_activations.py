...
def compare_with_tensorflow(device_type, activation_type, shape, data_type):
    ...
    of_activation_map = {'relu': flow.nn.relu, 'sigmoid': flow.math.sigmoid, 'tanh': flow.math.tanh}
    tf_activation_map = {'relu': tf.nn.relu, 'sigmoid': tf.math.sigmoid, 'tanh': tf.math.tanh}

    @flow.global_function(type='train', function_config=func_config)
    def ActivationJob():
        with flow.scope.placement(device_type, '0:0'):
            x = flow.get_variable('x', shape=shape, dtype=data_type, initializer=flow.random_uniform_initializer(minval=-10, maxval=10), trainable=True)
