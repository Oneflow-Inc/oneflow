...
def make_self_attn_qk_v_func(batch_size, seq_len, num_heads, head_size, fused, fp16):
    ...
    def self_attn_qk_v_fw_bw(h: flow.typing.Numpy.Placeholder(shape=(seq_len, batch_size, hidden_size), dtype=flow.float32)) -> typing.Tuple[flow.typing.Numpy, flow.typing.Numpy]:
        var = flow.get_variable('var', shape=(1,), dtype=flow.float32, initializer=flow.constant_initializer(1.0, dtype=flow.float32), trainable=True)
        h = h * var
        if fused:
        ...
        if fused:
            (qmk, v) = flow.nn.fused_self_attention_query_mul_key_and_value(h, head_size=head_size, alpha=alpha)
        else:
            h = flow.reshape(h, (seq_len, batch_size, -1, 3 * head_size))
            (q, k, v) = (flow.transpose(flow.slice(h, begin=[None, None, None, head_size * i], size=[None, None, None, head_size]), perm=[1, 2, 0, 3]) for i in range(3))
...
def compare_fused_with_no_fused(test_case, batch_size, seq_len, num_heads, head_size, fp16, verbose=False):
    hidden_size = num_heads * 3 * head_size
    input = gen_random_input((seq_len, batch_size, hidden_size))
    func = make_self_attn_qk_v_func(batch_size, seq_len, num_heads, head_size, True, fp16)
    (qmk, v) = func(input)
    func_ = make_self_attn_qk_v_func(batch_size, seq_len, num_heads, head_size, False, fp16)
    (qmk_, v_) = func_(input)
    (_q, _k, _v) = np_qkv(input, head_size)
    _qmk = np_bgemm(_q.transpose(1, 2, 0, 3), _k.transpose(1, 2, 3, 0), get_alpha(head_size))
