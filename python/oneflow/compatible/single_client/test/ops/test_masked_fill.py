...
def _test_masked_fill_fw_bw(test_case, device, x_shape, mask_shape, type_name, value=0):
    ...
    def test_masked_fill_fw_bw_job(x: oft.Numpy.Placeholder(x_shape, dtype=flow_type), mask: oft.Numpy.Placeholder(mask_shape, dtype=flow_type)):
        with flow.scope.placement(device, '0:0'):
            y = flow.get_variable(name='vx', shape=(1,), dtype=flow.float, initializer=flow.zeros_initializer())
