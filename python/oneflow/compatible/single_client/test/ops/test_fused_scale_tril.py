...
def _test_fused_scale_tril_fw_bw(test_case, device, shape, type_name, diagonal, fill_value, scale):
    ...
    def test_fused_scale_tril_fw_bw_job(x: oft.Numpy.Placeholder(shape, dtype=flow_type)):
        with flow.scope.placement(device, '0:0'):
            ...
            if type_name == 'float16':
                out = flow.cast(flow.math.fused_scale_tril(flow.cast(x, flow.float16), diagonal, scale=scale), flow.float)
            else:
    ...
    np_out = np.where(np.tril(np.ones(shape), diagonal), test_global_storage.Get('x') * scale, np.full(shape, fill_value).astype(np_type))
