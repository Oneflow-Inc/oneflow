...
def _of_where(condition, x, y, device_type='gpu', machine_device_ids='0:0', dynamic=False, dz_dx_watcher=None, dz_dy_watcher=None):
    ...
    if callable(dz_dx_watcher) and callable(dz_dy_watcher):
        ...
        def do_where(condition, x, y):
            with flow.scope.placement(device_type, '0:0'):
                x_var = flow.get_variable('x', shape=x.shape, dtype=flow.float, initializer=flow.constant_initializer(0))
                x_var = flow.cast_to_current_logical_view(x_var)
                x_var = x_var + x
                y_var = flow.get_variable('y', shape=y.shape, dtype=flow.float, initializer=flow.constant_initializer(0))
    ...
    if dynamic:
        ...
        def where_fn(condition_def: oft.ListNumpy.Placeholder(condition.shape, dtype=flow.int32), x_def: oft.ListNumpy.Placeholder(x.shape, dtype=flow.float), y_def: oft.ListNumpy.Placeholder(y.shape, dtype=flow.float)):
            return do_where(condition_def, x_def, y_def)
        return where_fn([condition], [x], [y]).get().numpy_list()[0]
    else:
        func_config.default_placement_scope(flow.scope.placement(device_type, machine_device_ids))
        func_config.default_logical_view(flow.scope.consistent_view())

        @flow.global_function(type=func_config_type, function_config=func_config)
        def where_fn(condition_def: oft.Numpy.Placeholder(condition.shape, dtype=flow.int32), x_def: oft.Numpy.Placeholder(x.shape, dtype=flow.float), y_def: oft.Numpy.Placeholder(y.shape, dtype=flow.float)):
            return do_where(condition_def, x_def, y_def)
...
def _compare_with_tf(test_case, cond_shape, x_shape, y_shape, device_type='gpu', machine_device_ids='0:0', dynamic=False, verbose=False):
    (condition, x, y) = _random_input(cond_shape, x_shape, y_shape)
    ...
    def compare_dz_dx(dz_dx_blob):
        if verbose:
            ...
            print('of_dz_dx:', dz_dx_blob.numpy_list()[0] if dynamic else dz_dx_blob.numpy())
        test_case.assertTrue(np.array_equal(dz_dx.numpy(), dz_dx_blob.numpy_list()[0] if dynamic else dz_dx_blob.numpy()))

    def compare_dz_dy(dz_dy_blob):
        if verbose:
            print('condition:', condition)
            print('tf_dz_dy:', dz_dy.numpy())
            print('of_dz_dy:', dz_dy_blob.numpy_list()[0] if dynamic else dz_dy_blob.numpy())
        test_case.assertTrue(np.array_equal(dz_dy.numpy(), dz_dy_blob.numpy_list()[0] if dynamic else dz_dy_blob.numpy()))
    of_z = _of_where(condition, x, y, device_type, machine_device_ids, dynamic, compare_dz_dx, compare_dz_dy)
...
def _of_where_with_x_and_y_are_none(input, input_shape=None):
    ...
    if input_shape is None:
        ...
        @flow.global_function(function_config=func_config)
        def where_fn(input_def: oft.ListNumpy.Placeholder(input_shape, dtype=flow.float)):
            return flow.where(input_def)
