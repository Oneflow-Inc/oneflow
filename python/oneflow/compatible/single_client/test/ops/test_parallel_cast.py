...
def _test(test_case, device_num):
    ...
    def test_fn(a: flow.typing.Numpy.Placeholder(a_shape), b: flow.typing.Numpy.Placeholder(b_shape), c: flow.typing.Numpy.Placeholder(c_shape)) -> flow.typing.Numpy:
        var_a = flow.get_variable(name='var_a', shape=a_shape, dtype=flow.float32, initializer=flow.ones_initializer(), distribute=flow.distribute.split(1))
        a = flow.parallel_cast(a, distribute=flow.distribute.split(1))
        a = var_a * a
        out = flow.matmul(a, b)
        out = flow.parallel_cast(out, distribute=flow.distribute.broadcast(), gradient_distribute=flow.distribute.broadcast())
...
@unittest.skipIf(flow.unittest.env.eager_execution_enabled(), "Parallel cast SBP doesn't work in eager mode")
