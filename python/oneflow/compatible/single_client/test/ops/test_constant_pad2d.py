...
def _make_op_function(test_case, input, padding, constant_value, grad, device_type, value_type, machine_ids, device_counts):
    ...
    if value_type == flow.float32 or value_type == flow.float64:
        ...
        def op_function(x: tp.Numpy.Placeholder(input.shape, dtype=value_type)):
            with flow.scope.placement(device_type, '0:0'):
                x += flow.get_variable(name='input', shape=input.shape, dtype=value_type, initializer=flow.zeros_initializer())
                out = flow.constant_pad2d(x, padding, constant_value)
                flow.optimizer.SGD(flow.optimizer.PiecewiseConstantScheduler([], [0]), momentum=0).minimize(out)
        ...
        def op_function(x: tp.Numpy.Placeholder(input.shape, dtype=flow.float32)):
            with flow.scope.placement(device_type, '0:0'):
                x += flow.get_variable(name='input', shape=input.shape, dtype=flow.float32, initializer=flow.zeros_initializer())
                y_int32 = flow.constant_pad2d(x, padding, constant_value)
                y_fp32 = flow.cast(y_int32, dtype=flow.float32)
                flow.optimizer.SGD(flow.optimizer.PiecewiseConstantScheduler([], [0]), momentum=0).minimize(y_fp32)
            flow.watch_diff(x, _compare_diff)
            return y_fp32
        return op_function
        ...
        def op_function(x: tp.Numpy.Placeholder(input.shape, dtype=flow.float32)):
            with flow.scope.placement(device_type, '0:0'):
                x_var = flow.get_variable(name='input', shape=input.shape, dtype=flow.float32, initializer=flow.constant_initializer(0))
...
def gen_numpy_test_sample(input_shape, padding, constant_value, is_float=True):
    ...
    numpy_results = {'input': input, 'padding': padding, 'constant_value': constant_value, 'output': output, 'grad': grad}
    return numpy_results

def _compare_op_function_with_samples(test_case, device_type, sample, value_type, machine_ids, device_count):
    op_function = _make_op_function(test_case, sample['input'].astype(value_type[0]), sample['padding'], sample['constant_value'], sample['grad'].astype(value_type[0]), device_type, value_type[1], machine_ids, device_count)
