...
def _gen_segment_ids(out_shape, num_segments, segment_ids_shape):
    ...
    return np.random.randint(low=0, high=out_shape[axis], size=segment_ids_shape, dtype=np.int32)
...
def _make_unsoted_segment_sum_fn(device, data, segment_ids, num_segments):
    ...
    def unsorted_batch_segment_sum_job(data: oft.Numpy.Placeholder(data.shape, dtype=flow.float), segment_ids: oft.Numpy.Placeholder(segment_ids.shape, dtype=flow.int32)):
        with flow.scope.placement(device, '0:0'):
            x = flow.get_variable('data', shape=data.shape, dtype=flow.float32, initializer=flow.constant_initializer(0))
            data = x + data
            res = flow.math.unsorted_batch_segment_sum(data=data, segment_ids=segment_ids, num_segments=num_segments)
            flow.optimizer.SGD(flow.optimizer.PiecewiseConstantScheduler([], [0.001]), momentum=0).minimize(res)
...
def _run_test(test_case, device, out_shape, num_segments, segment_ids_shape):
    ...
    unsorted_batch_segment_sum_out = _make_unsoted_segment_sum_fn(device, data, segment_ids, num_segments).get()
