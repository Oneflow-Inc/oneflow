...
class Add(flow.deprecated.nn.Module):
    ...
    def forward(self, x, y):
        return self.module_builder_.OpName('Add_%s' % self.call_seq_no).Input('in', [x, y]).Build().InferAndTryRun().RemoteBlobList()[0]
...
def _make_global_func(test_case, x_shape, y_shape):
    ...
    def AddJob(x: oft.Numpy.Placeholder(shape=x_shape), y: oft.Numpy.Placeholder(shape=y_shape)) -> oft.Numpy:
        with flow.scope.namespace('AddJob'):
            add_op = flow.find_or_create_module('Add', Add)
            z = add_op(x, y)
            test_case.assertTrue(z.op_name == 'AddJob-Add_{}'.format(add_op.call_seq_no - 1))
            v = add_op(z, x)
            test_case.assertTrue(v.op_name == 'AddJob-Add_{}'.format(add_op.call_seq_no - 1))
        return z
    return AddJob
