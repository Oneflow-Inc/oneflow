...
def GenerateTest(test_case, a_shape, b_shape, dtype=flow.int32, device_type='gpu'):
    ...
    def MyTestJob(a: oft.Numpy.Placeholder(a_shape, dtype=dtype), b: oft.Numpy.Placeholder(b_shape, dtype=dtype)):
        with flow.scope.placement(device_type, '0:0'):
            ...
            return (equal_out, not_equal_out, greater_than_out, greater_equal_out, less_than_out, less_equal_out)
    a = np_array(dtype, a_shape)
    b = np_array(dtype, b_shape)
    (equal_out, not_equal_out, greater_than_out, greater_equal_out, less_than_out, less_equal_out) = MyTestJob(a, b).get()
    ...
    test_case.assertTrue(np.array_equal(greater_than_out.numpy(), func_greater_than(a, b)))
    test_case.assertTrue(np.array_equal(greater_equal_out.numpy(), func_greater_equal(a, b)))
