...
def np_ctc_greedy_decoder(log_probs, input_lengths, merge_repeated=True):
    ...
    decodes = np.zeros((log_probs.shape[1], log_probs.shape[0]), dtype=input_lengths.dtype)
    ...
    for b in range(input_lengths.size):
        ...
        for t in range(input_length):
            ...
            if max_indice != blank_label and (not (merge_repeated and max_indice == prev_indices)):
                decodes[b, t_dec] = max_indice
                t_dec += 1
            prev_indices = max_indice
    return (decodes, neg_sum_logits)

def compare_with_np(device_type, device_num, data_type, max_input_length, batch_size, num_classes, merge_repeated):
    ...
    func_config.default_placement_scope(flow.scope.placement(device_type, '0:0-{}'.format(device_num - 1)))
    log_probs = np.random.random(size=(max_input_length, batch_size, num_classes)).astype(np_data_type)
    log_probs = log_softmax(log_probs, axis=2)
    input_lengths = np.random.randint(max_input_length / 2, high=max_input_length, size=(batch_size,), dtype=np.int64)

    @flow.global_function(function_config=func_config)
    def ctc_greedy_decoder_job(log_probs: tp.Numpy.Placeholder(shape=(max_input_length, batch_size, num_classes), dtype=flow_data_type), input_lengths: tp.Numpy.Placeholder(shape=(batch_size,), dtype=flow.int64)) -> Tuple[tp.Numpy, tp.Numpy]:
        with flow.scope.placement(device_type, '0:0'):
            (decoded, neg_sum_logits) = flow.nn.ctc_greedy_decoder(log_probs, input_lengths, merge_repeated)
        return (decoded, neg_sum_logits)
    (of_decoded, of_neg_sum_logits) = ctc_greedy_decoder_job(log_probs, input_lengths)
    (np_decoded, np_neg_sum_logits) = np_ctc_greedy_decoder(log_probs, input_lengths, merge_repeated)
