...
def _compare_logsoftmax_with_np(input_shape, axis, device_type, machine_ids, device_counts):
    ...
    def oneflow_logsoftmax(of_input_1: tp.Numpy.Placeholder(shape=input_1.shape)) -> tp.Numpy:
        ...
        with flow.scope.placement(device_type, '0:0'):
            flow.optimizer.SGD(flow.optimizer.PiecewiseConstantScheduler([], [0.001]), momentum=0).minimize(of_logsoftmax_out)
...
class Testlogsoftmax1n1d(flow.unittest.TestCase):
    ...
    def test_logsoftmax_cpu(test_case):
        arg_dict = _gen_arg_dict(shape=(2, 64, 32), axis=(1, -1), device_type='cpu', machine_ids='0:0', device_counts=1)
        for arg in GenArgList(arg_dict):
            _compare_logsoftmax_with_np(*arg)

    @unittest.skipIf(os.getenv('ONEFLOW_TEST_CPU_ONLY'), 'only test cpu cases')
    def test_logsoftmax_gpu(test_case):
        arg_dict = _gen_arg_dict(shape=(2, 4, 6, 2), axis=(2, -2), device_type='gpu', machine_ids='0:0', device_counts=1)
        for arg in GenArgList(arg_dict):
            _compare_logsoftmax_with_np(*arg)
...
class Testlogsoftmax1n2d(flow.unittest.TestCase):
    ...
    def test_logsoftmax_gpu_1n2d(test_case):
        arg_dict = _gen_arg_dict(shape=(2, 8), axis=(-1, 1), device_type='gpu', machine_ids='0:0-1', device_counts=2)
        for arg in GenArgList(arg_dict):
            _compare_logsoftmax_with_np(*arg)
