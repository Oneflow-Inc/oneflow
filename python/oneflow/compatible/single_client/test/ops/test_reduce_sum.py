...
def compare_with_tensorflow(device_type, data_type, input_shape, axis, keepdims, rtol=1e-05, atol=1e-05):
    ...
    def ReduceSumJob(x: oft.Numpy.Placeholder(input_shape)):
        with flow.scope.placement(device_type, '0:0'):
            if data_type == 'float16':
                y = flow.cast(flow.math.reduce_sum(flow.cast(x, dtype=flow.float16), axis=axis, keepdims=keepdims), dtype=flow.float32)
            else:
    ...
    assert np.allclose(of_out.numpy(), tf_out.numpy(), rtol=rtol, atol=atol), (of_out.numpy(), tf_out.numpy())
