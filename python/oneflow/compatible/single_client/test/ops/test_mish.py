...
def _compare_mish_with_np(input_shape, device_type, machine_ids, device_counts):
    ...
    def np_diff(input):
        ...
        return np.tanh(u) + input * (1 - np.tanh(u) ** 2) * (np.exp(input) / (1 + np.exp(input)))
    ...
    def oneflow_mish(of_input_1: tp.Numpy.Placeholder(shape=input_1.shape)) -> tp.Numpy:
        ...
        with flow.scope.placement(device_type, '0:0'):
            flow.optimizer.SGD(flow.optimizer.PiecewiseConstantScheduler([], [0.001]), momentum=0).minimize(of_mish_out)
        return of_mish_out
...
class Testmish1n1d(flow.unittest.TestCase):
    ...
    def test_mish_gpu(test_case):
        arg_dict = _gen_arg_dict(shape=(3, 16, 32), device_type='gpu', machine_ids='0:0', device_counts=1)
        for arg in GenArgList(arg_dict):
            _compare_mish_with_np(*arg)
...
class Testmish1n2d(flow.unittest.TestCase):
    ...
    def test_mish_gpu_1n2d(test_case):
        arg_dict = _gen_arg_dict(shape=(3, 8, 8, 4), device_type='gpu', machine_ids='0:0-1', device_counts=2)
        for arg in GenArgList(arg_dict):
            _compare_mish_with_np(*arg)
