...
def _get_images_static_shape(images):
    ...
    assert isinstance(image_static_shape, np.ndarray), 'image_shapes: {}, image_static_shape: {}'.format(str(image_shapes), str(image_static_shape))
...
def _get_bbox_static_shape(bbox_list):
    ...
    assert isinstance(bbox_static_shape, np.ndarray), 'bbox_shapes: {}, bbox_static_shape: {}'.format(str(bbox_shapes), str(bbox_static_shape))
...
def _of_target_resize_bbox_scale(images, bbox_list, target_size, max_size):
    ...
    def target_resize_bbox_scale_job(image_def: oft.ListListNumpy.Placeholder(shape=tuple(image_shape), dtype=flow.float), bbox_def: oft.ListListNumpy.Placeholder(shape=tuple(bbox_shape), dtype=flow.float)):
        images_buffer = flow.tensor_list_to_tensor_buffer(image_def)
        (resized_images_buffer, new_size, scale) = flow.image_target_resize(images_buffer, target_size=target_size, max_size=max_size)
        bbox_buffer = flow.tensor_list_to_tensor_buffer(bbox_def)
        scaled_bbox = flow.object_bbox_scale(bbox_buffer, scale)
        scaled_bbox_list = flow.tensor_buffer_to_tensor_list(scaled_bbox, shape=bbox_shape[1:], dtype=flow.float)
    ...
    (output_bbox_list, output_image_size) = target_resize_bbox_scale_job([input_image_list], [input_bbox_list]).get()
    return (output_bbox_list.numpy_lists()[0], output_image_size.numpy_list()[0])

def _compare_bbox_scale(test_case, anno_file, image_dir, batch_size, target_size, max_size, print_debug_info=False):
    (images, bbox_list) = _random_sample_images(anno_file, image_dir, batch_size)
    (of_bbox_list, image_size_list) = _of_target_resize_bbox_scale(images, bbox_list, target_size, max_size)
...
class TestObjectBboxScale(flow.unittest.TestCase):
    ...
    def test_object_bbox_scale(test_case):
        _compare_bbox_scale(test_case, '/dataset/mscoco_2017/annotations/instances_val2017.json', '/dataset/mscoco_2017/val2017', 4, 800, 1333)
