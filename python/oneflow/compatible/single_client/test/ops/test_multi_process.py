...
@unittest.skipIf(os.getenv('ONEFLOW_TEST_GITHUB_HOSTED'), 'this will fail because github hosted VM has only two CPU cores')
    ...
    def test_multi_process(test_case):
        ...
        def Foo():
            with flow.scope.placement('gpu', '0:0-3'):
                x = flow.get_variable('x', shape=(2, 5), dtype=flow.float, initializer=flow.random_uniform_initializer(minval=0, maxval=1), trainable=False)
            return x
        of_ret = Foo().get()
    ...
    def test_worker_to_master_communication(test_case):
        ...
        def Foo():
            ...
            with flow.scope.placement('gpu', '0:3'):
                y = flow.get_variable('y', shape=(2, 5), dtype=flow.float, initializer=flow.constant_initializer(0), trainable=False)
                flow.assign(y, x)
            return y
        of_ret = Foo().get()
        test_case.assertEqual(of_ret.numpy().shape, (2, 5))
