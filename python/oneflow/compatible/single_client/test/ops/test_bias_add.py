...
def RunOneflowBiasAdd(data_type, device_type, value, bias, flow_args):
    ...
    def FlowJob(value: oft.Numpy.Placeholder(value.shape), bias: oft.Numpy.Placeholder(bias.shape)):
        with flow.scope.placement(device_type, '0:0'):
            value += flow.get_variable(name='v1', shape=(1,), dtype=flow.float, initializer=flow.zeros_initializer())
            bias += flow.get_variable(name='v2', shape=(1,), dtype=flow.float, initializer=flow.zeros_initializer())
...
def CompareBiasAddWithTensorFlow(data_type, device_type, input_shapes, op_args=None, input_minval=-10, input_maxval=10, y_rtol=1e-05, y_atol=1e-05, x_diff_rtol=1e-05, x_diff_atol=1e-05):
    assert device_type in ['gpu', 'cpu']
    if op_args is None:
        (flow_args, tf_args) = ([], [])
    else:
        (flow_args, tf_args) = (op_args.flow_args, op_args.tf_args)
    x = [np.random.uniform(low=input_minval, high=input_maxval, size=input_shape).astype(np.float32) for input_shape in input_shapes]
    (of_y, of_x_diff1, of_x_diff2) = RunOneflowBiasAdd(data_type, device_type, *x, flow_args)
