...
def _compare_diag_with_np(device_type, device_num, data_type, input_shape, diagonal):
    ...
    def diag_job(input: tp.Numpy.Placeholder(shape=input_shape, dtype=flow.float)) -> tp.Numpy:
        with flow.scope.placement(device_type, '0:0'):
            input_var = flow.get_variable('input', shape=input_shape, dtype=flow.float, initializer=flow.zeros_initializer(), trainable=True)
        ...
        with flow.scope.placement(device_type, '0:0'):
            flow.optimizer.Adam(flow.optimizer.PiecewiseConstantScheduler([], [0.0001])).minimize(output)
        return output
