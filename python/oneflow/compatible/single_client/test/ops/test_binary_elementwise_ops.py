...
def compare_with_tensorflow(test_case, device_type, flow_op, tf_op, x_shape, y_shape, data_type, x_minval=-10, x_maxval=10, y_minval=-10, y_maxval=10, compare_grad=True, out_rtol=1e-05, out_atol=1e-05, diff_rtol=1e-05, diff_atol=1e-05):
    ...
    (of_out, of_x_diff, of_y_diff) = RunOneflowBinaryOp(device_type, flow_op, x, y, data_type)
    (tf_out, tf_x_diff, tf_y_diff) = RunTensorFlowBinaryOp(tf_op, x, y)
    test_case.assertTrue(np.allclose(of_out, tf_out, rtol=out_rtol, atol=out_atol, equal_nan=True))
    if compare_grad:
        test_case.assertTrue(np.allclose(of_x_diff, tf_x_diff.numpy(), rtol=diff_rtol, atol=diff_atol, equal_nan=True))
        test_case.assertTrue(np.allclose(of_y_diff, tf_y_diff.numpy(), rtol=diff_rtol, atol=diff_atol, equal_nan=True))
    flow.clear_default_session()
