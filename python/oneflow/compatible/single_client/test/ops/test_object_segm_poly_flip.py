...
def _of_object_segm_poly_flip(poly_list, image_size, flip_code):
    ...
    def object_segm_poly_flip_job(poly_def: oft.ListListNumpy.Placeholder(shape=tuple(poly_shape), dtype=flow.float), image_size_def: oft.ListNumpy.Placeholder(shape=image_size.shape, dtype=flow.int32)):
...
def _get_segm_poly_static_shape(poly_list):
    ...
    assert isinstance(poly_static_shape, np.ndarray), 'poly_shapes: {}, poly_static_shape: {}'.format(str(poly_shapes), str(poly_static_shape))
...
class TestObjectSegmPolyFlip(flow.unittest.TestCase):
    ...
    def test_object_segm_poly_flip(test_case):
        _compare_segm_poly_flip(test_case, '/dataset/mscoco_2017/annotations/instances_val2017.json', 4, 1)
