...
def _test_unsorted_segment_sum_model_parallel_fw(test_case, device_type, out_shape, segment_ids_shape, axis, split_axis):
    ...
    def unsorted_segment_sum_job(data: oft.Numpy.Placeholder(data_arr.shape, dtype=flow.float), segment_ids: oft.Numpy.Placeholder(segment_ids_arr.shape, dtype=flow.int32), like: oft.Numpy.Placeholder(out_arr.shape, dtype=flow.float)):
        with flow.scope.placement(device_type, '0:0-3'):
            ...
            if split_axis == axis:
                ...
                out0 = flow.unsorted_segment_sum(data=data, segment_ids=segment_ids, num_segments=out_shape[axis], axis=axis)
