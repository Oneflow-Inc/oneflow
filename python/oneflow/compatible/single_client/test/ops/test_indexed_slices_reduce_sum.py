...
def _run_test(test_case, indices, values, indices_dtype, values_dtype, device):
    ...
    def TestJob(indices: oft.Numpy.Placeholder(indices.shape, dtype=indices_dtype), values: oft.Numpy.Placeholder(values.shape, dtype=values_dtype)):
    ...
    _check(test_case, indices, values, out_indices.numpy(), out_values.numpy(), num_unique.numpy())
