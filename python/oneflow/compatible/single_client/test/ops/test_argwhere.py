...
def _of_argwhere(x, index_dtype, device_type='gpu', device_num=1, dynamic=False):
    ...
    if dynamic is True:
        ...
        def argwhere_fn(x: flow.typing.ListNumpy.Placeholder(x.shape, dtype=data_type)) -> flow.typing.ListNumpy:
        ...
        @flow.global_function('predict', function_config=func_config)
        def argwhere_fn(x: flow.typing.Numpy.Placeholder(x.shape, dtype=data_type)) -> flow.typing.ListNumpy:
            return flow.argwhere(x, dtype=out_data_type)
        return argwhere_fn(x)[0]

def _compare_with_np(test_case, shape, value_dtype, index_dtype, device_type='gpu', device_num=1, dynamic=False, verbose=False):
    if verbose:
        print('shape:', shape)
    ...
    y = np.argwhere(x)
    of_y = _of_argwhere(x, index_dtype, device_type=device_type, device_num=device_num, dynamic=dynamic)
    if verbose is True:
        print('input:', x)
        print('np result:', y)
...
def _dynamic_multi_iter_compare(test_case, iter_num, shape, value_dtype, index_dtype, device_type='gpu', verbose=False):
    ...
    @flow.global_function('predict', function_config=func_config)
    def argwhere_fn(x: flow.typing.Numpy.Placeholder(tuple(shape), dtype=data_type)) -> flow.typing.ListNumpy:
        return flow.argwhere(x, dtype=out_data_type)
    results = []
    for x_ in x:
        y_ = argwhere_fn(x_)[0]
