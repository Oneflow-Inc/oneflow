...
def _test_gather(test_case, src, dst):
    ...
    def test_fn(x: flow.typing.Numpy.Placeholder((1024, 1024)), indices: flow.typing.Numpy.Placeholder(shape=(64,), dtype=flow.int32)) -> flow.typing.Numpy:
        with flow.scope.placement('gpu', '0:0-3', (2, 2)):
            if src[0] == 'S(0)':
                x = flow.hierarchical_parallel_cast(x, parallel_distribution=['B', 'B'])
                indices = flow.hierarchical_parallel_cast(indices, parallel_distribution=['S(0)', 'S(0)'])
                if src[1] == 'S(0)':
                    x = flow.hierarchical_parallel_cast(x, parallel_distribution=['B', 'B'])
                    indices = flow.hierarchical_parallel_cast(indices, parallel_distribution=['S(0)', 'S(0)'])
                elif src[1] == 'S(1)':
                    x = flow.hierarchical_parallel_cast(x, parallel_distribution=['B', 'S(1)'])
                    indices = flow.hierarchical_parallel_cast(indices, parallel_distribution=['S(0)', 'B'])
                elif src[1] == 'P':
                    x = flow.hierarchical_parallel_cast(x, parallel_distribution=['B', 'S(0)'])
                    indices = flow.hierarchical_parallel_cast(indices, parallel_distribution=['S(0)', 'B'])
                elif src[1] == 'B':
                    x = flow.hierarchical_parallel_cast(x, parallel_distribution=['B', 'B'])
                    indices = flow.hierarchical_parallel_cast(indices, parallel_distribution=['S(0)', 'B'])
            elif src[0] == 'P':
                x = flow.hierarchical_parallel_cast(x, parallel_distribution=['S(0)', 'S(0)'])
                indices = flow.hierarchical_parallel_cast(indices, parallel_distribution=['B', 'B'])
                if src[1] == 'S(0)':
                    x = flow.hierarchical_parallel_cast(x, parallel_distribution=['S(0)', 'B'])
                    indices = flow.hierarchical_parallel_cast(indices, parallel_distribution=['B', 'S(0)'])
                elif src[1] == 'S(1)':
                    x = flow.hierarchical_parallel_cast(x, parallel_distribution=['S(0)', 'S(1)'])
                    indices = flow.hierarchical_parallel_cast(indices, parallel_distribution=['B', 'B'])
                elif src[1] == 'P':
                    x = flow.hierarchical_parallel_cast(x, parallel_distribution=['S(0)', 'S(0)'])
                    indices = flow.hierarchical_parallel_cast(indices, parallel_distribution=['B', 'B'])
            ...
            x = flow.hierarchical_parallel_cast(x, parallel_distribution=dst, name='gather_cast')
            if dst[0] == 'S(0)':
                x = flow.hierarchical_parallel_cast(x, parallel_distribution=['S(0)', 'S(0)'])
            elif dst[0] == 'B':
                x = flow.hierarchical_parallel_cast(x, parallel_distribution=['B', 'B'])
            elif dst[0] == 'S(1)':
                x = flow.hierarchical_parallel_cast(x, parallel_distribution=['S(1)', 'S(1)'])
            else:
                raise NotImplementedError
...
def _test_train(test_case):
    ...
    @flow.global_function('train', function_config=func_config)
    def test_fn(x: flow.typing.Numpy.Placeholder((1024, 4)), indices: flow.typing.Numpy.Placeholder(shape=(12,), dtype=flow.int32)) -> flow.typing.Numpy:
        with flow.scope.placement('gpu', '0:0-3', (2, 2)):
            x = flow.hierarchical_parallel_cast(x, parallel_distribution=['S(0)', 'S(0)'])
            indices = flow.hierarchical_parallel_cast(indices, parallel_distribution=['B', 'B'])
            x = flow.hierarchical_parallel_cast(x, parallel_distribution=['S(0)', 'B'])
            v = flow.get_variable(name='v', shape=(1024, 4), parallel_distribution=['S(0)', 'B'], initializer=flow.zeros_initializer())
            x = x + v
            indices = flow.hierarchical_parallel_cast(indices, parallel_distribution=['B', 'S(0)'])
            x = flow.gather(x, indices)
            x = flow.hierarchical_parallel_cast(x, parallel_distribution=['B', 'S(0)'], grad_mode='manual', grad_parallel_distribution=['B', 'S(0)'])
            x = flow.math.relu(x)
            x = flow.hierarchical_parallel_cast(x, parallel_distribution=['B', 'B'])
        x = flow.hierarchical_parallel_cast(x, parallel_distribution=['B'])
        flow.optimizer.SGD(flow.optimizer.PiecewiseConstantScheduler([], [0.001]), momentum=0).minimize(x)
        return x
...
def _test_reshape(test_case):
    ...
    def FlowJob(x: flow.typing.Numpy.Placeholder((4, 6), dtype=flow.float)):
        with flow.scope.placement('gpu', '0:0-3', (2, 2)):
            v = flow.get_variable('x', shape=(4, 6), dtype=flow.float, initializer=flow.constant_initializer(0), trainable=True, parallel_distribution=['S(0)', 'S(1)'])
            x = flow.hierarchical_parallel_cast(x, parallel_distribution=['S(0)', 'S(1)'])
            x += v
...
def _test_reshape_like(test_case):
    ...
    def FlowJob(x: flow.typing.Numpy.Placeholder((4, 3, 2, 3), dtype=flow.float)):
        with flow.scope.placement('gpu', '0:0-3', (2, 2)):
            v1 = flow.get_variable('v1', shape=(4, 3, 2, 3), dtype=flow.float, initializer=flow.constant_initializer(0), trainable=True, parallel_distribution=['S(0)', 'S(2)'])
            v2 = flow.get_variable('v2', shape=(4, 3, 6), dtype=flow.float, initializer=flow.constant_initializer(0), trainable=True, parallel_distribution=['S(0)', 'S(2)'])
            x = flow.hierarchical_parallel_cast(x, parallel_distribution=['S(0)', 'S(2)'])
            x += v1
...
class TestHierarchicalParallelCast(flow.unittest.TestCase):
    ...
    def test_change_axis1(test_case):
        ...
        arg_dict['src'] = [['S(0)', 'S(0)'], ['S(0)', 'S(1)'], ['S(0)', 'P'], ['S(0)', 'B']]
