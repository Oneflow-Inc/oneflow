...
def _run_count_test(test_case, device_type, x_shape, dtype):
    ...
    def count_not_finite_job(x: oft.Numpy.Placeholder(x_shape, dtype=type_name_to_flow_type[dtype])):
...
def _run_multi_count_test(test_case, device_type, x1_shape, x2_shape, dtype, x1_count, x2_count):
    flow.clear_default_session()
    func_config = flow.FunctionConfig()
    func_config.default_data_type(flow.float)

    @flow.global_function(function_config=func_config)
    def multi_count_not_finite_job(x1: oft.Numpy.Placeholder(x1_shape, dtype=type_name_to_flow_type[dtype]), x2: oft.Numpy.Placeholder(x2_shape, dtype=type_name_to_flow_type[dtype])):
        x_list = []
