...
def _compare_swish_with_np(input_shape, beta, device_type, machine_ids, device_counts):
    ...
    def oneflow_swish(of_input_1: tp.Numpy.Placeholder(shape=input_1.shape)) -> tp.Numpy:
        ...
        with flow.scope.placement(device_type, '0:0'):
            flow.optimizer.SGD(flow.optimizer.PiecewiseConstantScheduler([], [0.001]), momentum=0).minimize(of_swish_out)
        return of_swish_out
...
class Testswish1n1d(flow.unittest.TestCase):
    ...
    def test_swish_cpu(test_case):
        arg_dict = _gen_arg_dict(shape=(4, 6), beta=1, device_type='cpu', machine_ids='0:0', device_counts=1)
        for arg in GenArgList(arg_dict):
            _compare_swish_with_np(*arg)

    @unittest.skipIf(os.getenv('ONEFLOW_TEST_CPU_ONLY'), 'only test cpu cases')
    def test_swish_gpu(test_case):
        arg_dict = _gen_arg_dict(shape=(3, 16, 32), beta=10, device_type='gpu', machine_ids='0:0', device_counts=1)
        for arg in GenArgList(arg_dict):
            _compare_swish_with_np(*arg)
...
class Teststack1n2d(flow.unittest.TestCase):
    ...
    def test_swish_gpu_1n2d(test_case):
        arg_dict = _gen_arg_dict(shape=(3, 8, 8, 4), beta=2, device_type='gpu', machine_ids='0:0-1', device_counts=2)
        for arg in GenArgList(arg_dict):
            _compare_swish_with_np(*arg)
