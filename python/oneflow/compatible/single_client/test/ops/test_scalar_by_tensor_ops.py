...
def compare_with_tensorflow(device_type, data_type, x_shape, case):
    ...
    def ScalarAddByTensorJob():
        with flow.scope.placement(device_type, '0:0'):
            ...
            y = flow.get_variable('y', shape=(1,), dtype=flow.float, initializer=flow.random_uniform_initializer(minval=0, maxval=100), trainable=True)
            if case == 'add':
    ...
    assert np.allclose(test_global_storage.Get('x_diff'), tf_x_diff.numpy(), rtol=1e-05, atol=1e-05)
    assert np.allclose(test_global_storage.Get('y_diff'), tf_y_diff.numpy(), rtol=1e-05, atol=1e-05)
