...
def _random_inputs(params_shape, indices_shape):
    ...
    indices = np.random.randint(low=0, high=params_shape[len(indices_shape) - 1], size=indices_shape, dtype=np.int32)
...
def _make_gather_fn(params, indices, axis, batch_dims, device_type, mirrored, compare_fn):
    ...
    if mirrored:
        ...
        def gather_fn(params_def: oft.ListNumpy.Placeholder(params.shape, dtype=flow.float32), indices_def: oft.ListNumpy.Placeholder(indices.shape, dtype=flow.int32)):
            return do_gather(params_def, indices_def)
    else:

        @flow.global_function(type='train', function_config=func_config)
        def gather_fn(params_def: oft.Numpy.Placeholder(params.shape, dtype=flow.float32), indices_def: oft.Numpy.Placeholder(indices.shape, dtype=flow.int32)):
            return do_gather(params_def, indices_def)
    return gather_fn

def _compare_gather_with_tf(test_case, device_type, params_shape, indices_shape, axis, batch_dims, mirrored=False):
    ...
    if mirrored:
        ...
        def compare_dy(params_grad):
            test_case.assertTrue(np.allclose(dy, params_grad.numpy_list()[0], atol=1e-05, rtol=1e-05))
    else:

        def compare_dy(params_grad):
            test_case.assertTrue(np.allclose(dy, params_grad.numpy(), atol=1e-05, rtol=1e-05))
