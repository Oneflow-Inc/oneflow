...
def compare_with_tensorflow(device_type, x_shape, filters, kernel_size, groups, of_padding='SAME', tf_padding='SAME', stride=1, data_format='NCHW'):
    ...
    def DynamicConvJob(x: oft.ListNumpy.Placeholder((10, 3, 100, 100))):
        with flow.scope.placement(device_type, '0:0'):
            x_var = flow.get_variable(name='v1', shape=(1,), dtype=flow.float, initializer=flow.zeros_initializer())
            ...
            loss = flow.nn.conv2d(x, weight, strides=[stride, stride], padding=of_padding, data_format=data_format, dilations=[1, 1], groups=groups)
    ...
    with tf.GradientTape(persistent=True) as tape:
        ...
        weight = tf.Variable(global_storage['weight'].numpy().transpose(weight_data_transpose))
        tf_out = tf.nn.conv2d(x, weight, strides=[1, stride, stride, 1], padding=tf_padding, data_format='NHWC')
    idx = np.where(np.abs(of_out.transpose(xy_data_transpose) - tf_out.numpy()) > 0.0005)
    assert np.allclose(of_out.transpose(xy_data_transpose), tf_out.numpy(), rtol=0.001, atol=0.001)
    ...
    if device_type == 'cpu':
        rtol *= 100
        atol *= 100
    assert np.allclose(global_storage['x_diff'].numpy_list()[0].transpose(xy_data_transpose), tf_x_diff.numpy(), rtol=rtol, atol=atol), global_storage['x_diff'].numpy_list()[0].transpose(xy_data_transpose) - tf_x_diff.numpy()
    assert np.allclose(global_storage['weight_diff'].numpy().transpose(weight_data_transpose), tf_weight_diff.numpy(), rtol=0.005, atol=0.005)
