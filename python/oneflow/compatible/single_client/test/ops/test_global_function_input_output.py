...
def _test_input_ndarray_not_contiguous(test_case, shape):
    ...
    rand_dim_slice_stop = random.randrange(rand_dim_slice_start + 1, input.shape[rand_axis])
    slice_list = []
    ...
    def foo_job(x_def: oft.Numpy.Placeholder(shape=slice_input.shape, dtype=flow.float)):
    ...
    @flow.global_function()
    def foo_job(x_def: oft.Numpy.Placeholder(shape=input.shape[::-1], dtype=flow.float)):
        y = x_def + flow.constant(1.0, shape=(1,), dtype=flow.float)
        return y
...
class TestGlobalFunctionInputOutput(flow.unittest.TestCase):
    ...
    def test_lazy_input_output(test_case):
        ...
        def foo_job(input_def: oft.Numpy.Placeholder(shape=(2, 5))):
            var = flow.get_variable(name='var', shape=(2, 5), dtype=flow.float, initializer=flow.ones_initializer())
    ...
    def test_eager_output(test_case):
        ...
        def foo_job():
            ...
            return x
        ret = foo_job().get()
        test_case.assertTrue(np.array_equal(np.ones(shape=(2, 5), dtype=np.single), ret.numpy_list()[0]))
    ...
    def test_eager_multi_output(test_case):
        ...
        def foo_job():
            x = flow.constant(1, shape=(2, 5), dtype=flow.float)
            y = flow.get_variable(name='var', shape=(64, 4), dtype=flow.float, initializer=flow.zeros_initializer())
            return (x, y)
        (x, y) = foo_job().get()
        test_case.assertTrue(np.array_equal(np.ones(shape=(2, 5), dtype=np.single), x.numpy_list()[0]))
        test_case.assertTrue(np.array_equal(np.zeros(shape=(64, 4), dtype=np.single), y.numpy()))
    ...
    def test_eager_multi_input(test_case):
        ...
        @flow.global_function(function_config=func_config)
        def foo_job(x_def: oft.ListNumpy.Placeholder(shape=(3, 4), dtype=flow.float), y_def: oft.ListNumpy.Placeholder(shape=(1,), dtype=flow.float)):
            y = x_def * y_def
            test_case.assertTrue(np.allclose(y.numpy(0), output))
