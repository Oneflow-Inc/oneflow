...
def _test_logical_slice(test_case, var_shape, slice_tuples, split_axis, device_tag, flow_dtype, device_num):
    ...
    def slice_fn():
        with flow.scope.placement(device_tag, '0:0-{}'.format(device_num - 1)):
            var = flow.get_variable(name='var', shape=var_shape, dtype=flow_dtype, initializer=flow.random_uniform_initializer(-10, 10, dtype=flow_dtype), distribute=flow.distribute.split(split_axis))
