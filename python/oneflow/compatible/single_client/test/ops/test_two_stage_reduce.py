...
def _test_two_stage_reduce(test_case, flow_func, np_func, device_type, axis, split_axis):
    ...
    def two_stage_reduce_job(x: oft.Numpy.Placeholder((4, 20, 20, 20))):
        ...
        with flow.scope.placement(device_type, '0:0-3'):
            loss = flow_func(x.with_distribute(flow.distribute.split(split_axis)), axis=axis, keepdims=True)
