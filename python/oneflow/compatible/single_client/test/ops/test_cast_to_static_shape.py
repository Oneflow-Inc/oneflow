...
def _make_cast_to_static_shape_fn(test_case, shape, data_type, device_type, device_num, compare_diff_fn):
    ...
    @flow.global_function(type='train' if require_grad else 'predict', function_config=func_config)
    def cast_to_static_shape_fn(x: flow.typing.ListNumpy.Placeholder(shape=shape, dtype=dtype)) -> flow.typing.ListNumpy:
        x_var = flow.get_variable(name='x_var', shape=(1,), dtype=flow.float32, initializer=flow.zeros_initializer())
...
def _check_cast_to_static_shape(test_case, shape, data_type, device_type, device_num):
    ...
    cast_to_static_shape_fn = _make_cast_to_static_shape_fn(test_case, shape, data_type, device_type, device_num, comp_diff)
