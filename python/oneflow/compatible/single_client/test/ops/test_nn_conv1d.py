...
def compare_with_tensorflow(device_type, x_shape, filters, kernel_size, groups, of_padding='SAME', tf_padding='SAME', stride=1, data_format='NCDHW', dilation=1):
    ...
    def ConvJob():
        with flow.scope.placement(device_type, '0:0'):
            ...
            loss = flow.nn.conv1d(x, weight, strides=[stride], padding=of_padding, data_format=data_format, dilations=dilation, groups=groups)
    ...
    with tf.GradientTape(persistent=True) as tape:
        ...
        tf_out = tf.nn.conv1d(x, weight, stride=[1, stride, 1], padding=tf_padding, data_format='NWC', dilations=[1, dilation, 1])
