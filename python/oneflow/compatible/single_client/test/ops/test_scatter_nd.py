...
def _random_inputs(params_shape, indices_shape, updates_shape, allow_duplicate_index=True):
    ...
    for col in range(indices_cols):
        if allow_duplicate_index is False and indices_rows <= params_shape[col]:
            ...
            indices_col = np.random.randint(low=0, high=params_shape[col], size=(indices_rows,), dtype=np.int32).reshape(indices_shape[:-1])
    ...
    if allow_duplicate_index is False:
        ...
        for nd_index in indices.reshape(-1, indices.shape[-1]):
            ...
            assert nd_index_str not in existing_nd_index_set, 'random generated duplicate nd index {}'.format(nd_index_str)
...
def _make_scatter_nd_fn(indices, updates, shape, device_type, mirrored, compare_fn):
    ...
    def do_scatter_nd(indices_blob, updates_blob):
        with flow.scope.placement(device_type, '0:0'):
            x = flow.get_variable('updates', shape=updates.shape, dtype=flow.float32, initializer=flow.constant_initializer(0))
    ...
    if mirrored:
        ...
        def scatter_nd_fn(indices_def: oft.ListNumpy.Placeholder(indices.shape, dtype=flow.int32), updates_def: oft.ListNumpy.Placeholder(updates.shape, dtype=flow.float)):
            return do_scatter_nd(indices_def, updates_def)
    else:

        @flow.global_function(type='train', function_config=func_config)
        def scatter_nd_fn(indices_def: oft.Numpy.Placeholder(indices.shape, dtype=flow.int32), updates_def: oft.Numpy.Placeholder(updates.shape, dtype=flow.float)):
            return do_scatter_nd(indices_def, updates_def)
    return scatter_nd_fn

def _compare_scatter_nd_with_tf(test_case, device_type, params_shape, indices_shape, updates_shape, mirrored=False, verbose=False):
    ...
    if mirrored:
        ...
        def compare_dy(params_grad):
            test_case.assertTrue(np.array_equal(dy_dx.numpy(), params_grad.numpy_list()[0]))
    else:

        def compare_dy(params_grad):
            test_case.assertTrue(np.array_equal(dy_dx.numpy(), params_grad.numpy()))
    scatter_nd_fn = _make_scatter_nd_fn(indices, updates, params_shape, device_type, mirrored, compare_dy)
    if mirrored:
...
def _compare_scatter_nd_update_with_tf(test_case, device_type, params_shape, indices_shape, updates_shape, allow_duplicate_index=False, verbose=False):
    (params, updates, indices) = _random_inputs(params_shape, indices_shape, updates_shape, allow_duplicate_index)
    ...
    @flow.global_function(type='train', function_config=func_config)
    def scatter_nd_update_grad_fn(x_def: oft.Numpy.Placeholder(params.shape, dtype=flow.float), indices_def: oft.Numpy.Placeholder(indices.shape, dtype=flow.int32), y_def: oft.Numpy.Placeholder(updates.shape, dtype=flow.float)):
        with flow.scope.placement(device_type, '0:0'):
            x = flow.get_variable('params', shape=params.shape, dtype=flow.float32, initializer=flow.constant_initializer(0))
            y = flow.get_variable('updates', shape=updates.shape, dtype=flow.float32, initializer=flow.constant_initializer(0))
            x = x + x_def
            y = y + y_def
            z = flow.tensor_scatter_nd_update(x, indices_def, y)
            flow.optimizer.SGD(flow.optimizer.PiecewiseConstantScheduler([], [0.001]), momentum=0).minimize(z)
...
def _of_tensor_scatter_nd_add(params, indices, updates, device_type, mirrored, params_grad_watcher, updates_grad_watcher):
    flow.clear_default_session()
    func_config = flow.FunctionConfig()
    func_config.default_data_type(flow.float)
    ...
    def do_tensor_scatter_nd_add(params_blob, indices_blob, updates_blob):
        with flow.scope.placement(device_type, '0:0'):
            params_var = flow.get_variable('params', shape=params_blob.shape, dtype=flow.float32, initializer=flow.constant_initializer(0))
            updates_var = flow.get_variable('updates', shape=updates_blob.shape, dtype=flow.float32, initializer=flow.constant_initializer(0))
            ...
            flow.optimizer.SGD(flow.optimizer.PiecewiseConstantScheduler([], [0.001]), momentum=0).minimize(out)
    ...
    if mirrored:
        ...
        @flow.global_function(type='train', function_config=func_config)
        def tensor_scatter_nd_add_fn(params_def: oft.ListNumpy.Placeholder(params.shape, dtype=flow.float), indices_def: oft.ListNumpy.Placeholder(indices.shape, dtype=flow.int32), updates_def: oft.ListNumpy.Placeholder(updates.shape, dtype=flow.float)):
            return do_tensor_scatter_nd_add(params_def, indices_def, updates_def)
        return tensor_scatter_nd_add_fn([params], [indices], [updates]).get().numpy_list()[0]
    else:
        func_config.default_logical_view(flow.scope.consistent_view())

        @flow.global_function(type='train', function_config=func_config)
        def tensor_scatter_nd_add_fn(params_def: oft.Numpy.Placeholder(params.shape, dtype=flow.float), indices_def: oft.Numpy.Placeholder(indices.shape, dtype=flow.int32), updates_def: oft.Numpy.Placeholder(updates.shape, dtype=flow.float)):
            return do_tensor_scatter_nd_add(params_def, indices_def, updates_def)
        return tensor_scatter_nd_add_fn(params, indices, updates).get().numpy()

def _compare_tensor_scatter_nd_add_with_tf(test_case, params_shape, indices_shape, updates_shape, device_type, mirrored):
    ...
    def compare_params_grad(of_params_grad):
        ...
        of_params_grad_np = of_params_grad.numpy_list()[0] if mirrored else of_params_grad.numpy()
    ...
    def compare_updates_grad(of_updates_grad):
        ...
        of_updates_grad_np = of_updates_grad.numpy_list()[0] if mirrored else of_updates_grad.numpy()
        test_case.assertTrue(np.allclose(tf_updates_grad_np, of_updates_grad_np))
    of_out = _of_tensor_scatter_nd_add(params, indices, updates, device_type, mirrored, compare_params_grad, compare_updates_grad)
    test_case.assertTrue(np.allclose(tf_out1.numpy(), of_out))

def _of_scatter_nd_dynamic_indices(indices, updates, indices_static_shape, updates_static_shape, params_shape):
    flow.clear_default_session()
    func_config = flow.FunctionConfig()
    func_config.default_data_type(flow.float)
    func_config.default_logical_view(flow.scope.mirrored_view())
    ...
    def scatter_nd_fn(indices_def: oft.ListNumpy.Placeholder(indices_static_shape, dtype=flow.int32), updates_def: oft.ListNumpy.Placeholder(updates_static_shape, dtype=flow.float)):
        with flow.scope.placement('gpu', '0:0'):
            return flow.scatter_nd(indices_def, updates_def, params_shape)
    return scatter_nd_fn([indices], [updates]).get().numpy_list()[0]

def _compare_scatter_nd_dynamic_indices_with_tf(test_case, indices_shape, updates_shape, indices_static_shape, updates_static_shape, params_shape):
    (_, updates, indices) = _random_inputs(params_shape, indices_shape, updates_shape)
    indices_const = tf.constant(indices)
    x = tf.Variable(updates)
    y = tf.scatter_nd(indices_const, x, params_shape)
    of_y = _of_scatter_nd_dynamic_indices(indices, updates, indices_static_shape, updates_static_shape, params_shape)
    test_case.assertTrue(np.allclose(y.numpy(), of_y))

def _of_tensor_scatter_nd_update_dynamic_indices(params, indices, updates, indices_static_shape, updates_static_shape):
    flow.clear_default_session()
    func_config = flow.FunctionConfig()
    func_config.default_data_type(flow.float)
    func_config.default_logical_view(flow.scope.mirrored_view())
    ...
    @flow.global_function(function_config=func_config)
    def tensor_scatter_nd_update_fn(params_def: oft.ListNumpy.Placeholder(params.shape, dtype=flow.float), indices_def: oft.ListNumpy.Placeholder(indices_static_shape, dtype=flow.int32), updates_def: oft.ListNumpy.Placeholder(updates_static_shape, dtype=flow.float)):
        with flow.scope.placement('gpu', '0:0'):
            return flow.tensor_scatter_nd_update(params_def, indices_def, updates_def)
    return tensor_scatter_nd_update_fn([params], [indices], [updates]).get().numpy_list()[0]

def _compare_tensor_scatter_nd_update_dynamic_indices_with_tf(test_case, params_shape, indices_shape, updates_shape, indices_static_shape, updates_static_shape):
    ...
    of_z = _of_tensor_scatter_nd_update_dynamic_indices(params, indices, updates, indices_static_shape, updates_static_shape)
    test_case.assertTrue(np.allclose(z.numpy(), of_z))

def _of_tensor_scatter_nd_add_dynamic_indices(params, indices, updates, indices_static_shape, updates_static_shape):
    flow.clear_default_session()
    func_config = flow.FunctionConfig()
    func_config.default_data_type(flow.float)
    func_config.default_logical_view(flow.scope.mirrored_view())
    ...
    @flow.global_function(function_config=func_config)
    def tensor_scatter_nd_add_fn(params_def: oft.ListNumpy.Placeholder(params.shape, dtype=flow.float), indices_def: oft.ListNumpy.Placeholder(indices_static_shape, dtype=flow.int32), updates_def: oft.ListNumpy.Placeholder(updates_static_shape, dtype=flow.float)):
        with flow.scope.placement('gpu', '0:0'):
            return flow.tensor_scatter_nd_add(params_def, indices_def, updates_def)
    return tensor_scatter_nd_add_fn([params], [indices], [updates]).get().numpy_list()[0]

def _compare_tensor_scatter_nd_add_dynamic_indices_with_tf(test_case, params_shape, indices_shape, updates_shape, indices_static_shape, updates_static_shape):
    (params, updates, indices) = _random_inputs(params_shape, indices_shape, updates_shape)
    i = tf.constant(indices)
    x = tf.Variable(params)
    y = tf.Variable(updates)
    z = tf.tensor_scatter_nd_add(x, i, y)
    of_z = _of_tensor_scatter_nd_add_dynamic_indices(params, indices, updates, indices_static_shape, updates_static_shape)
    test_case.assertTrue(np.allclose(z.numpy(), of_z))
