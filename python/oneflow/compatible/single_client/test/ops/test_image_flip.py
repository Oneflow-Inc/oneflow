...
def _of_image_flip(images, image_shape, flip_code):
    ...
    def image_flip_job(images_def: oft.ListListNumpy.Placeholder(shape=image_shape, dtype=flow.float)):
        images_buffer = flow.tensor_list_to_tensor_buffer(images_def)
        flip_images = flow.image_flip(images_buffer, flip_code)
        return flow.tensor_buffer_to_tensor_list(flip_images, shape=image_shape[1:], dtype=flow.float)
...
class TestImageFlip(flow.unittest.TestCase):
    ...
    def test_image_flip(test_case):
        _compare_image_flip_with_cv(test_case, ['/dataset/mscoco_2017/val2017/000000000139.jpg', '/dataset/mscoco_2017/val2017/000000000632.jpg'])
