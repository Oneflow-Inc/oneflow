...
from test_util import GenArgDict, test_global_storage, type_name_to_flow_type, type_name_to_np_type
...
def _test_tril_fw_bw(test_case, device, shape, type_name, diagonal, fill_value):
    ...
    def test_tril_fw_bw_job(x: oft.Numpy.Placeholder(shape, dtype=flow_type)):
        with flow.scope.placement(device, '0:0'):
            x_var = flow.get_variable(name='xv', shape=(1,), dtype=flow.float, initializer=flow.zeros_initializer())
            x += flow.cast(x_var, dtype=flow_type)
            if type_name == 'float16':
                out = flow.cast(flow.math.tril(flow.cast(x, flow.float16), diagonal), flow.float)
            else:
                out = flow.math.tril(x, diagonal)
            flow.optimizer.SGD(flow.optimizer.PiecewiseConstantScheduler([], [0.0001]), momentum=0).minimize(out)
    ...
    np_out = np.where(np.tril(np.ones(shape), diagonal), test_global_storage.Get('x'), np.full(shape, fill_value).astype(np_type))
    ...
    def test_tril_fw_bw(test_case):
        ...
        for arg in GenArgDict(arg_dict):
            if arg['device'] == 'cpu' and arg['type_name'] == 'float16':
                continue
            if isinstance(arg['fill_value'], float) and arg_dict['type_name'] not in ['float32', 'float16', 'double']:
                continue
