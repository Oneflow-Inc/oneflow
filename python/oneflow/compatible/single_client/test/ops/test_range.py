...
def compare_range_with_np_CPU(device_type, machine_ids, device_counts):
    ...
    def oneflow_range() -> List[tp.Numpy]:
        with flow.scope.placement(device_type, machine_ids):
            ...
            x_var = flow.get_variable('cpu_input', shape=(3,), dtype=flow.float32, initializer=flow.constant_initializer(0))
            x_out = out_2 + x_var
            flow.optimizer.SGD(flow.optimizer.PiecewiseConstantScheduler([], [0.001]), momentum=0).minimize(x_out)
...
def compare_range_with_np_GPU(device_type, machine_ids, device_counts):
    ...
    def oneflow_range_gpu() -> List[tp.Numpy]:
        with flow.scope.placement(device_type, machine_ids):
            ...
            x_var = flow.get_variable('gpu_input', shape=(3,), dtype=flow.float32, initializer=flow.constant_initializer(0.0))
            x_gpu_out = x_var + out_2
            flow.optimizer.SGD(flow.optimizer.PiecewiseConstantScheduler([], [0.001]), momentum=0).minimize(x_gpu_out)
