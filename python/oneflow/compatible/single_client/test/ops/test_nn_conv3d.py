...
def compare_with_tensorflow(device_type, x_shape, filters, kernel_size, groups, of_padding='SAME', tf_padding='SAME', stride_d=1, stride_h=1, stride_w=1, data_format='NCDHW', dilation_d=1, dilation_h=1, dilation_w=1):
    ...
    def ConvJob():
        with flow.scope.placement(device_type, '0:0'):
            ...
            if data_format == 'NCDHW':
                ...
                weight_shape = (filters, kernel_size, kernel_size, kernel_size, x.shape[4] // groups)
            ...
            loss = flow.nn.conv3d(x, weight, strides=[stride_d, stride_h, stride_w], padding=of_padding, data_format=data_format, dilations=[dilation_d, dilation_h, dilation_w], groups=groups)
    ...
    with tf.GradientTape(persistent=True) as tape:
        ...
        tf_out = tf.nn.conv3d(x, weight, strides=[1, stride_d, stride_h, stride_w, 1], padding=tf_padding, data_format='NDHWC', dilations=[1, dilation_d, dilation_h, dilation_w, 1])
    ...
    diff_idx = np.where(np.abs(test_global_storage.Get('x_diff').transpose(xy_data_transpose) - tf_x_diff.numpy()) > 0.0005)
