...
def _of_poly_to_mask_pipline(images, poly_list, poly_index_list, num_segms_list, target_size, max_size):
    ...
    def poly_to_mask_job(image_def: oft.ListListNumpy.Placeholder(shape=tuple(image_shape), dtype=flow.float), poly_def: oft.ListListNumpy.Placeholder(shape=tuple(poly_shape), dtype=flow.float), poly_index_def: oft.ListListNumpy.Placeholder(shape=tuple(poly_index_shape), dtype=flow.int32)):
        ...
        mask_buffer = flow.object_segmentation_polygon_to_mask(scaled_poly_buffer, poly_index_buffer, new_size)
        mask_list = flow.tensor_buffer_to_tensor_list(mask_buffer, shape=(max_num_segms, target_size, max_size), dtype=flow.int8)
        scaled_poly_list = flow.tensor_buffer_to_tensor_list(scaled_poly_buffer, shape=poly_shape[1:], dtype=flow.float)
    ...
    (output_mask_list, output_poly_list) = poly_to_mask_job([input_image_list], [input_poly_list], [input_poly_index_list]).get()
...
def _poly_to_mask_with_cv(img_segm_poly_list, image_size_list):
    ...
    for (segm_poly_list, size) in zip(img_segm_poly_list, image_size_list):
        ...
        for poly_list in segm_poly_list:
            ...
            poly_array_list = [np.int32(np.round(np.asarray(poly)).reshape(-1, 2)) for poly in poly_list]
...
def _poly_to_mask_with_of_and_cv(test_case, anno_file, image_dir, batch_size, target_size, max_size, img_ids=None, print_debug_info=False):
    ...
    (of_mask_list, of_scaled_poly_list) = _of_poly_to_mask_pipline(images, poly_list, poly_index_list, num_segms_list, target_size, max_size)
    of_mask_list = [mask_array.reshape(-1, mask_array.shape[-2], mask_array.shape[-1]) for mask_array in of_mask_list]
    if print_debug_info:
    ...
    if test_case is not None:
        for (of_scaled_poly, scaled_poly, poly_index, scaled_poly_index) in zip(of_scaled_poly_list, scaled_poly_list, poly_index_list, scaled_poly_index_list):
            if print_debug_info:
                print('compare scaled poly: shape {} vs {}\n\tmax_abs_diff: {}'.format(of_scaled_poly.shape, scaled_poly.shape, np.max(np.absolute(of_scaled_poly - scaled_poly))))
        ...
        for (of_mask, gt_mask) in zip(of_mask_list, img_segm_mask_list):
            if print_debug_info:
                print('compare segm mask: shape {} vs {}'.format(of_mask.shape, gt_mask.shape))
...
def _vis_img_segm_mask_cmp(mask_list, cmp_mask_list):
    ...
    def display_fig():
        ...
        assert mask_array.shape == cmp_mask_array.shape, '{} vs {}'.format(str(mask_array.shape), str(cmp_mask_array.shape))
...
class TestObjectSegmPolyToMask(flow.unittest.TestCase):
    ...
    def test_poly_to_mask(test_case):
        _poly_to_mask_with_of_and_cv(test_case, '/dataset/mscoco_2017/annotations/instances_val2017.json', '/dataset/mscoco_2017/val2017', 4, 800, 1333)
if __name__ == '__main__':
    unittest.main()