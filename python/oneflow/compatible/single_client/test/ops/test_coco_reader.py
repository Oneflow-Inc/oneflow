...
def _make_coco_data_load_fn(anno_file, image_dir, nthread, batch_size, stride_partition, shuffle_after_epoch, ret_image_id_only=False):
    ...
    def coco_load_fn():
        with flow.scope.placement('cpu', '0:0-{}'.format(nthread - 1)):
            (image, image_id, image_size, gt_bbox, gt_label, gt_segm, gt_segm_index) = flow.data.coco_reader(annotation_file=anno_file, image_dir=image_dir, batch_size=batch_size, shuffle=shuffle_after_epoch, stride_partition=stride_partition, name='COCOReader')
            if ret_image_id_only:
                return image_id
            decoded_image = flow.image_decode(image, dtype=flow.float)
            image_list = flow.tensor_buffer_to_tensor_list(decoded_image, shape=(800, 1333, 3), dtype=flow.float)
            ...
            segm_index_list = flow.tensor_buffer_to_tensor_list(gt_segm_index, shape=(1024, 3), dtype=flow.int32)
...
def _get_coco_sorted_imgs(anno_file):
    ...
    for (i, img_id) in enumerate(img_ids):
        ...
        if not _has_valid_annotation(anno):
            continue
        img_info_list.append(dict(index=i, image_id=img_id, group_id=group_id, anno_len=len(anno_ids)))
        ...
        for i in range(self._max_iter):
            ...
            for rank in range(self._shards):
                ...
                if len(group_buckets_cur_rank[0]) > 0 and len(group_buckets_cur_rank[1]) > 0:
                    if group_buckets_cur_rank[0][0]['index'] < group_buckets_cur_rank[1][0]['index']:
...
class TestCocoReader(flow.unittest.TestCase):
    ...
    def test_coco_reader(test_case, verbose=VERBOSE):
        ...
        for (i, sample) in enumerate(samples):
            if verbose:
                print('#{} of label:\n'.format(i), label[0][i].squeeze(), type(label[0][i].squeeze()), label[0][i].squeeze().shape)
                print('#{} coco label:\n'.format(i), sample['label'], type(sample['label']), sample['label'].shape)
    ...
    def test_coco_reader_distributed_stride(test_case, verbose=VERBOSE):
        ...
        if verbose:
            ...
            for (i, image_info) in enumerate(image_info_list[:20]):
                print('index: {}, image_id: {}, group_id: {}, anno len: {}'.format(i, image_info['image_id'], image_info['group_id'], image_info['anno_len']))
