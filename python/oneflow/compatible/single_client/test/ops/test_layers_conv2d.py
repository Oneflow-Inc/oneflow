...
def compare_with_tensorflow(test_case, device_type, x_shape, filters, kernel_size, groups):
    ...
    def ConvJob():
        with flow.scope.placement(device_type, '0:0'):
            ...
            loss = flow.layers.conv2d(x, filters, kernel_size=kernel_size, strides=[1, 1], padding='valid', data_format='NCHW', dilation_rate=1, groups=groups, use_bias=False, kernel_initializer=flow.random_uniform_initializer(minval=0, maxval=100), weight_name='conv2d_weight')
            weight_shape = (filters, x.shape[1] // groups, kernel_size, kernel_size)
            weight = flow.get_variable(name='conv2d_weight', shape=weight_shape, dtype=flow.float, initializer=flow.random_uniform_initializer(minval=0, maxval=100))
    ...
    test_case.assertTrue(np.allclose(test_global_storage.Get('weight_diff').transpose(2, 3, 1, 0), tf_weight_diff.numpy(), rtol=1e-05, atol=1e-05))
