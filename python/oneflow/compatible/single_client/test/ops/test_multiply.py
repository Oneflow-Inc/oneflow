...
from test_util import Args, CompareOpWithTensorFlow, GenArgDict, test_global_storage, type_name_to_flow_type, type_name_to_np_type
...
def _test_element_wise_mul_fw_bw(test_case, device, shape, type_name):
    ...
    def test_element_wise_mul_job(x: oft.Numpy.Placeholder(shape, dtype=flow.float), y: oft.Numpy.Placeholder(shape, dtype=flow.float)):
        with flow.scope.placement(device, '0:0'):
            x += flow.get_variable(name='vx', shape=(1,), dtype=flow.float, initializer=flow.zeros_initializer())
            y += flow.get_variable(name='vy', shape=(1,), dtype=flow.float, initializer=flow.zeros_initializer())
    ...
    test_case.assertTrue(np.allclose(test_global_storage.Get('x') * test_global_storage.Get('y'), test_global_storage.Get('out')))
    test_case.assertTrue(np.allclose(test_global_storage.Get('out_diff') * test_global_storage.Get('x'), test_global_storage.Get('y_diff')))
    test_case.assertTrue(np.allclose(test_global_storage.Get('out_diff') * test_global_storage.Get('y'), test_global_storage.Get('x_diff')))
