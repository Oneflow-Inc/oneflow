...
def gen_gather_test_sample(input_shape, index_shape, dim, is_float=True):
    ...
    ret = {'input': input.astype(np.float32), 'index': index.astype(np.int32), 'dim': dim, 'output': output.astype(np.float32), 'grad': grad.astype(np.float32)}
    return ret
...
def _make_dim_gather_fn(test_case, sample, datashape):
    ...
    def DynamicDimGatherJob(params_def: oft.ListNumpy.Placeholder(datashape, dtype=flow.float32), index_def: oft.ListNumpy.Placeholder(datashape, dtype=flow.int32)) -> oft.ListNumpy:
        x_var = flow.get_variable('input', shape=(1,), dtype=flow.float32, initializer=flow.constant_initializer(0))
...
def _compare_dim_gather_with_samples(test_case, inputshape, indexshape, dim, maxshape):
    ...
    test_case.assertTrue(np.allclose(out, sample['output'].astype(np.float32), 0.001, 0.001))
...
class TestDynamicDimGather(flow.unittest.TestCase):
    ...
    def test_dynamic_dim_gather(test_case):
        _compare_dim_gather_with_samples(test_case, inputshape=(2, 2), indexshape=(2, 2), dim=1, maxshape=(10, 10))
        _compare_dim_gather_with_samples(test_case, inputshape=(2, 2), indexshape=(2, 2), dim=0, maxshape=(10, 10))
        _compare_dim_gather_with_samples(test_case, inputshape=(4, 4, 3), indexshape=(4, 1, 3), dim=1, maxshape=(10, 10, 10))
