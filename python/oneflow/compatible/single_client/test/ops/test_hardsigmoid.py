...
def _compare_hardsigmoid_with_np(input_shape, device_type, value_type, machine_ids, device_counts):
    ...
    if value_type[1] == flow.float16:
        ...
        def oneflow_hardsigmoid(of_input_1: tp.Numpy.Placeholder(shape=input_1.shape, dtype=flow.float32)) -> tp.Numpy:
            ...
            with flow.scope.placement(device_type, '0:0'):
                flow.optimizer.SGD(flow.optimizer.PiecewiseConstantScheduler([], [0.001]), momentum=0).minimize(of_hardsigmoid_out_f32)
            flow.watch_diff(x_var, assert_prediction_grad)
            return of_hardsigmoid_out_f32
    else:

        @flow.global_function(type='train', function_config=func_config)
        def oneflow_hardsigmoid(of_input_1: tp.Numpy.Placeholder(shape=input_1.shape, dtype=value_type[1])) -> tp.Numpy:
            with flow.scope.placement(device_type, '0:0'):
                v = flow.get_variable(shape=input_1.shape, dtype=value_type[1], initializer=flow.zeros_initializer(), name='x_var')
                x_var = of_input_1 + v
            flow.watch_diff(x_var, assert_prediction_grad)
            of_hardsigmoid_out = flow.nn.hardsigmoid(x_var)
            with flow.scope.placement(device_type, '0:0'):
                flow.optimizer.SGD(flow.optimizer.PiecewiseConstantScheduler([], [0.001]), momentum=0).minimize(of_hardsigmoid_out)
...
class Testhardsigmoid1n1d(flow.unittest.TestCase):
    ...
    def test_hardsigmoid_cpu(test_case):
        arg_dict = _gen_arg_dict(shape=(3, 16), device_type='cpu', value_type='float', machine_ids='0:0', device_counts=1)
        for arg in GenArgList(arg_dict):
            _compare_hardsigmoid_with_np(*arg)

    @unittest.skipIf(os.getenv('ONEFLOW_TEST_CPU_ONLY'), 'only test cpu cases')
    def test_hardsigmoid_gpu(test_case):
        arg_dict = _gen_arg_dict(shape=(16, 16), device_type='gpu', value_type='float', machine_ids='0:0', device_counts=1)
        for arg in GenArgList(arg_dict):
            _compare_hardsigmoid_with_np(*arg)
...
class Testhardsigmoid1n2d(flow.unittest.TestCase):
    ...
    def test_hardsigmoid_gpu_1n2d(test_case):
        arg_dict = _gen_arg_dict(shape=(4, 8, 16), device_type='gpu', value_type='float', machine_ids='0:0-1', device_counts=2)
        for arg in GenArgList(arg_dict):
            _compare_hardsigmoid_with_np(*arg)
