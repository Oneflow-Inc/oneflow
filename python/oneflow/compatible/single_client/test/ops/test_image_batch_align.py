...
def _of_image_batch_align(images, input_shape, output_shape, alignment):
    ...
    def image_batch_align_job(images_def: oft.ListListNumpy.Placeholder(shape=input_shape, dtype=flow.float)):
        images_buffer = flow.tensor_list_to_tensor_buffer(images_def)
        image = flow.image_batch_align(images_buffer, shape=output_shape[1:], dtype=flow.float, alignment=alignment)
        return image
...
def _compare_image_batch_align(test_case, image_files, alignment, print_debug_info=False):
    ...
    aligned_image_shape = [image_shape[0], _roundup(image_shape[1], alignment), _roundup(image_shape[2], alignment), image_shape[3]]
    if print_debug_info:
        print('image_shape:', image_shape)
        print('aligned_image_shape:', aligned_image_shape)
    image_tensor = _of_image_batch_align(images, tuple(image_shape), tuple(aligned_image_shape), alignment)
...
class TestImageBatchAlign(flow.unittest.TestCase):
    ...
    def test_image_batch_align(test_case):
        _compare_image_batch_align(test_case, ['/dataset/mscoco_2017/val2017/000000000139.jpg', '/dataset/mscoco_2017/val2017/000000000632.jpg', '/dataset/mscoco_2017/val2017/000000000785.jpg', '/dataset/mscoco_2017/val2017/000000001000.jpg'], 16)
