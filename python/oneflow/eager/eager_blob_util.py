...
def FetchTensorBlobAsNumpyList(parallel_size, blob_object):
    ...
    def AsyncFetchBlobBody(Yield):
        ...
        def BuildFetchBlobBodyInstruction(builder):
            builder.FetchBlobBody(blob_object, python_callback.GetIdForRegisteredCallback(fetcher))
            builder.InsertRemoveForeignCallbackInstruction(blob_object.object_id, python_callback.GetIdForRegisteredCallback(fetcher))
...
def _FetchBlobHeader(blob_object):
    ...
    def AsyncFetchBlobHeader(Yield):
        ...
        def BuildFetchBlobHeaderInstruction(builder):
            builder.FetchBlobHeader(blob_object, python_callback.GetIdForRegisteredCallback(fetcher))
            builder.InsertRemoveForeignCallbackInstruction(blob_object.object_id, python_callback.GetIdForRegisteredCallback(fetcher))
        oneflow._oneflow_internal.deprecated.PhysicalRun(BuildFetchBlobHeaderInstruction)
...
def _MakeFetcherEagerPhysicalBlobHeaderFromOfBlob(Yield):
    ...
    def Callback(ofblob):
        Yield(oneflow._oneflow_internal.EagerPhysicalBlobHeader(ofblob.static_shape, ofblob.shape, oneflow._oneflow_internal.deprecated.GetProtoDtype4OfDtype(ofblob.dtype)))
    return Callback
