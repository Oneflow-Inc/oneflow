...
def get_device_property(db_manager):
    ...
    """
    sql = ("SELECT name,totalMemory,computeMajor,computeMinor,"
    ...
    db_manager.execute_sql(sql)
    (name, totalGlobalMem, computeMajor, computeMinor,
     maxThreadsPerBlock, maxBlocksPerSm, regsPerBlock,
     regsPerMultiprocessor, warpSize, sharedMemPerBlock,
     sharedMemPerMultiprocessor, numSms, sharedMemPerBlockOptin
...
def sqlite_to_google_trace_event(args):
    try:
        ...
        device_property = get_device_property(
            db_manager)  # properties of cuda device
        ...
        traceEvents_meta = [
            ...
            },
            {
                ...
                "tid": 0,
                "args": {
                    "labels": f"GPU 0(CUDA HW {bus_location} - {name})"
                }
            },
            {
                "name": "process_sort_index",
        ...
        for row in db_manager.cursor.fetchall():
            ...
            db_manager.execute_sql(
                f"SELECT value FROM StringIds WHERE id = {nameId};")
        ...
        for row in db_manager.cursor.fetchall():
            ...
            db_manager.execute_sql(
                f"SELECT value FROM StringIds WHERE id is {nameId};")
            name = db_manager.cursor.fetchone()[0]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--input',
        help='Input nvidia nsight system .sqlite file path')
    parser.add_argument(
        '--output',
        '-o',
