...
class ModelBuilder(object):
    ...
    def __init__(self, save_path: str):
        if not isinstance(save_path, str):
            raise ValueError("param 'save_path' must be str, but got {}".format(save_path))
        self.version_ = None
        self.checkpoint_dir_ = self.DEFAULT_CHECKPOINT_DIR
        self.saved_model_dir_ = save_path
        self.saved_model_pb_filename_ = '{}.pb'.format(self.DEFAULT_SAVED_MODEL_FILE_BASENAME)
        self.saved_model_pbtxt_filename_ = '{}.prototxt'.format(self.DEFAULT_SAVED_MODEL_FILE_BASENAME)
    ...
    def _check_input_output_name_conflict(self):
        ...
        def check_name_conflict(name, interface_def):
            ...
            if lbn in lbn_set:
                raise ValueError('input conflict, {} already bind to other input'.format(lbn))
            lbn_set.add(lbn)
    ...
    def Save(self, save_model_before_graph_complete: bool=True):
        ...
        for (graph_name, graph_def) in self.proto.graphs.items():
            job = sess.Job(graph_name if save_model_before_graph_complete else graph_name + '_after_complete')
        ...
        if os.path.exists(version_dir):
            raise ValueError('Directory of model "{}" version "{}" already exist.'.format(self.saved_model_dir_, self.version_))
        ...
        saved_model_pbtxt_path = os.path.join(version_dir, self.saved_model_pbtxt_filename_)
...
class GraphBuilder(object):
    ...
    def __init__(self, name: str, model_builder: typing.Optional[ModelBuilder]=None):
        ...
        if not isinstance(model_builder, ModelBuilder) and model_builder is not None:
            raise ValueError("param 'model_builder' must be a type of ModelBuilder or None")
        if model_builder is not None:
            if name in model_builder.proto.graphs:
                raise ValueError('graph function ({}) is already added to model ({})'.format(name, model_builder.proto.name))
    ...
    def Finish(self):
        ...
        for (_, signature_def) in self.proto.signatures.items():
            for (_, input_def) in signature_def.inputs.items():
                ...
                oneflow._oneflow_internal.JobBuildAndInferCtx_CheckLbnValidAndExist(self.name, input_lbn)
                GetInterfaceBlobConf(self.name, input_lbn, input_def.blob_conf)
            for (_, output_def) in signature_def.outputs.items():
                oneflow._oneflow_internal.JobBuildAndInferCtx_CheckLbnValidAndExist(self.name, Lbi2Lbn(output_def.lbi))
        self.finished_ = True
...
class SignatureBuilder(object):
    ...
    def __init__(self, name: str, graph_builder: typing.Optional[GraphBuilder]=None):
        ...
        if not isinstance(graph_builder, GraphBuilder) and graph_builder is not None:
            raise ValueError("param 'graph_builder' must be a type of GraphBuilder or None")
        if graph_builder is not None:
            if name in graph_builder.proto.signatures:
                raise ValueError('signature ({}) already exist in graph ({})'.format(name, graph_builder.name))
    ...
    def Input(self, input_name: str, lbn: str):
        ...
        if input_name in self.proto.inputs:
            raise ValueError('input_name ({}) already exist in signature ({}) of graph ({})'.format(input_name, self.name, self.graph_builder_.name))
    ...
    def Output(self, output_name: str, lbn: str):
        ...
        if output_name in self.proto.outputs:
            raise ValueError('output_name ({}) already exist in signature ({}) of graph ({})'.format(output_name, self.name, self.graph_builder_.name))
...
def GetInterfaceBlobConf(job_name, lbn, blob_conf=None):
    ...
    split_axis = c_api_util.JobBuildAndInferCtx_GetSplitAxisFromProducerView(job_name, lbn)
