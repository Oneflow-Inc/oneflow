...
def _signature_proto_to_cfg(signature_proto, mut_signature_cfg):
    ...
    for (input_name, input_def) in signature_proto.inputs.items():
        ...
        _inferface_blob_conf_proto_to_cfg(input_def.blob_conf, input_def_cfg.mutable_blob_conf())
...
def _inferface_blob_conf_proto_to_cfg(inferface_blob_conf_proto, mut_inferface_blob_conf_cfg):
    assert isinstance(inferface_blob_conf_proto, interface_blob_conf_proto.InterfaceBlobConf)
    assert isinstance(mut_inferface_blob_conf_cfg, interface_blob_conf_proto_cfg.InterfaceBlobConf)
    ...
    if inferface_blob_conf_proto.HasField('parallel_distribution'):
        ...
        if sbp_proto.HasField('split_parallel'):
            ...
            mut_inferface_blob_conf_cfg.mutable_parallel_distribution().mutable_sbp_parallel().Add().CopyFrom(sbp)
...
class InferenceSession(object):
    ...
    def _check_status(self, *status):
        ...
        if check_success is False:
            ...
            raise ValueError('The calling to {} is only allowed when status is {}, current status is {}'.format(caller_func_name, allowed_status, self.status_))
    ...
    def _make_config_proto(self):
        ...
        if self.option_.device_tag == 'gpu':
            ...
            raise NotImplementedError('not supported device tag {}'.format(self.option_.device_tag))
    ...
    def open(self, job_name, signature=None, batch_size=None):
        ...
        tag_and_dev_ids = placement_util.GetDefaultMachineDeviceIds(self.config_proto_.resource)
        scope = scope_util.MakeInitialScope(job_conf, *tag_and_dev_ids, None, self.is_mirrored_)
    ...
    def compile(self, op_list):
        ...
        for op_conf in op_list:
            if _need_check_device_tag(op_conf) and op_conf.device_tag != device_tag:
                print("WARNING: the device_tag of op {} is not equal to the device_tag of seesion's current scope ({} vs. {}), which may cause the op graph to be incompatible".format(op_conf.name, op_conf.device_tag, device_tag))
    ...
    def load_saved_model(self, saved_model_dir, model_version=ModelVersionPolicy.LATEST, saved_model_meta_file_basename='saved_model', graph_name=None, signature_name=None):
        ...
        if not os.path.isdir(saved_model_path):
            raise ValueError('version {} of saved model in dir {} do not exist'.format(model_version, saved_model_dir))
        subfiles = list(os.listdir(saved_model_path))
        saved_model_meta_pb_filename = saved_model_meta_file_basename + '.pb'
        saved_model_meta_prototxt_filename = saved_model_meta_file_basename + '.prototxt'
        saved_model_proto = saved_model_pb.SavedModel()
        if saved_model_meta_pb_filename in subfiles:
            saved_model_meta_file_path = os.path.join(saved_model_path, saved_model_meta_pb_filename)
            ...
            saved_model_meta_file_path = os.path.join(saved_model_path, saved_model_meta_prototxt_filename)
            with open(saved_model_meta_file_path, 'rt') as f:
                text_format.Merge(f.read(), saved_model_proto)
        else:
            raise ValueError('saved model meta file {} do not exist in {}'.format(saved_model_meta_file_basename, saved_model_path))
        self.set_checkpoint_path(os.path.join(saved_model_path, saved_model_proto.checkpoint_dir))
        signature = None
    ...
    def list_inputs(self):
        self._check_status(self.SessionStatus.RUNNING)
        input_names = []
        for (input_name, _) in self.inter_user_job_info_.input_or_var_op_name2push_job_name.items():
    ...
    def list_outputs(self):
        self._check_status(self.SessionStatus.RUNNING)
        output_names = []
        for (output_name, _) in self.inter_user_job_info_.output_or_var_op_name2pull_job_name.items():
    ...
    def _get_op_blob_info(self, job_name, op_name, blob_name):
        ...
        lbn = oneflow._oneflow_internal.JobBuildAndInferCtx_GetOpBlobLbn(job_name, op_name, blob_name)
    ...
    def _run_push_jobs(self, **kwargs):
        for (input_name, push_job_name) in self.inter_user_job_info_.input_or_var_op_name2push_job_name.items():
            ...
            push_job_inst = job_instance_util.MakePushJobInstance(push_job_name, input_name, push_fn)
            self._run_job(push_job_inst)

    def _run_pull_jobs(self, user_job_name):
        output_futures = {}
        for (output_name, pull_job_name) in self.inter_user_job_info_.output_or_var_op_name2pull_job_name.items():
            future = self.event_loop_.create_future()
            pull_fn = self._make_pull_job_cb(output_name, user_job_name, future)
            pull_job_inst = job_instance_util.MakePullJobInstance(pull_job_name, output_name, pull_fn)
    ...
    def _make_pull_job_cb(self, output_name, user_job_name, future):
        output_lbn = oneflow._oneflow_internal.JobBuildAndInferCtx_GetOpBlobLbn(user_job_name, output_name, 'out')
        split_axis = c_api_util.JobBuildAndInferCtx_GetSplitAxisFromProducerView(user_job_name, output_lbn)
    ...
    def _run_load_checkpoint_job(self):
        ...
        def copy_model_load_path(ofblob):
            ofblob.CopyFromNdarray(np.frombuffer(self.checkpoint_path_.encode('ascii'), dtype=np.int8))
        load_checkpoint_job_inst = job_instance_util.MakeJobInstance(self.inter_user_job_info_.global_model_load_job_name, push_cb=copy_model_load_path)
