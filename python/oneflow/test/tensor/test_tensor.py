...
class TestTensor(flow.unittest.TestCase):
    ...
    def test_local_tensor_init_methods(test_case):
        test_case._test_tensor_init_methods(lambda *args, **kwargs: flow.Tensor(*args, **kwargs), lambda x: x.numpy())

    @flow.unittest.skip_unless_1n2d()
    def test_consistent_tensor_init_methods(test_case):
        test_case._test_tensor_init_methods(lambda *args, **kwargs: flow.Tensor(*args, **kwargs, sbp=flow.sbp.broadcast, placement=flow.placement('cuda', {0: range(2)})), lambda x: x.to_consistent(sbp=flow.sbp.broadcast).to_local().numpy())
