...
class TestTensorComplex64(unittest.TestCase):
    ...
    def test_mul_cpu(self):
        ...
        for i, input_shape in enumerate(self.shape):
            ...
            # backward
            flow_ret.sum().backward()
            compare_result(flow_x.grad.numpy(), flow_y.numpy().conjugate(), self.rtol, self.atol)
            compare_result(flow_y.grad.numpy(), flow_x.numpy().conjugate(), self.rtol, self.atol)
    ...
    def test_mul_cuda(self):
        ...
        for i, input_shape in enumerate(self.shape):
            ...
            flow_ret.sum().backward()
            compare_result(
                flow_x.grad.cpu().detach().numpy(), flow_y.numpy().conjugate(), self.rtol, self.atol
            )
            compare_result(
                flow_y.grad.cpu().detach().numpy(), flow_x.numpy().conjugate(), self.rtol, self.atol
            )
