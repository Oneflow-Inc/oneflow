...
class TestDemo(flow.unittest.TestCase):
    ...
    @autotest(n=100, auto_backward=True, check_graph=True, rtol=1e-2, atol=1e-4, include_complex=True)
    def test_mm(self):
    ...
    @autotest(n=100, auto_backward=False, check_graph=True, rtol=1e-2, atol=1e-4, include_complex=True)
    ...
    def test_np_mm(self):
        shape_a = (2,6)
        shape_b = (6,1)
        np_a = np.random.randn(*shape_a) + 1.0j * np.random.randn(*shape_a)
        np_b = np.random.randn(*shape_b) + 1.0j * np.random.randn(*shape_b)
        np_a = np_a.astype(np.complex64)
        np_b = np_b.astype(np.complex64)
        flow_a = flow.tensor(np_a, dtype=flow.complex64).requires_grad_(True)
        flow_b = flow.tensor(np_b, dtype=flow.complex64).requires_grad_(True)
        torch_a = torch_original.tensor(np_a, dtype=torch_original.complex64).requires_grad_(True)
        torch_b = torch_original.tensor(np_b, dtype=torch_original.complex64).requires_grad_(True)
        paddle_a = paddle.to_tensor(np_a, dtype=paddle.complex64, stop_gradient=False)
        paddle_b = paddle.to_tensor(np_b, dtype=paddle.complex64, stop_gradient=False)
        
        # np backward pass
        # D = np.ones((np_a @ np_b).shape) + 1.0j * np.ones((np_a @ np_b).shape)
        D = np.ones((np_a @ np_b).shape)
        D = D.astype(np.complex64)
        dA = np.matmul(D, np.conj(np.transpose(np_b)))
        dB = np.matmul(np.conj(np.transpose(np_a)), D) 
        
        # flow backward pass
        ret = flow.matmul(flow_a, flow_b)
        ret.sum().backward()
        flow_dA = flow_a.grad.detach()
        flow_dB = flow_b.grad.detach()

        # torch backward pass
        ret = torch_original.matmul(torch_a, torch_b)
        ret.sum().backward()
        torch_dA = torch_a.grad.detach()
        torch_dB = torch_b.grad.detach()
        
        # paddle backward pass
        ret = paddle.matmul(paddle_a, paddle_b)
        ret.sum().backward()
        paddle_dA = paddle_a.grad.detach()
        paddle_dB = paddle_b.grad.detach()
        ...
        print("[flow] flow_dA and flow_dB:")
        print(f"flow_dA: {flow_dA}")
        print(f"flow_dB: {flow_dB}")

        print("[torch] torch_dA and torch_dB:")
        print(f"torch_dA: {torch_original.resolve_conj(torch_dA)}")
        print(f"torch_dB: {torch_original.resolve_conj(torch_dB)}")

        print("[paddle] torch_dA and torch_dB:")
        print(f"paddle_dA: {paddle_dA}")
        print(f"paddle_dB: {paddle_dB}")
    ...
    def test_np_mv(self):
        ...
        D = np.ones((2,1))
        D = D.astype(np.complex64)
        dA = np.matmul(D, np.conj(np.transpose(np.expand_dims(np_b, -1))))
        dB = np.matmul(np.conj(np.transpose(np_a)), D)
        
        # flow backward pass
        ret = flow.mv(flow_a, flow_b)
        ret.sum().backward()
        flow_dA = flow_a.grad.detach()
        flow_dB = flow_b.grad.detach()

        # torch backward pass
        ret = torch_original.mv(torch_a, torch_b)
        ret.sum().backward()
        torch_dA = torch_a.grad.detach()
        torch_dB = torch_b.grad.detach()
