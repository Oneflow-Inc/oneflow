...
def _test_linear_graph_save_load_global_broadcast(test_case, model_tensor_placement, model_file_placement):
...
def _test_graph_save_load_global_split_2(test_case, model_tensor_placement, model_file_placement):
    ...
    sbp_for_special_keys={"System-Train-TrainStep": BROADCAST,
    ...
    class PipelineGraph(flow.nn.Graph):
        def __init__(self, module_pipleine):
            ...
            self.add_optimizer(flow.optim.SGD(self.module_pipeline.parameters(), lr=0.001))
    ...
    def train_with_graph(call_cnt=0, state_dict_dir=None, last_state_dict=None):
        ...
        x = flow.tensor([[ 0.4286,  0.7402,  0.4161,  0.6103,  0.7394,  1.1330, -0.2311, -0.1013,  0.8537,
                           0.9757, -0.9842,  0.3839, -0.5551, -0.8832,  0.7820,  0.7421],
                         [-0.1581, -1.0319,  1.8430,  0.3576,  0.7288, -0.6912,  0.9966,  1.0840, -1.1760,
                           1.5683, -0.2098, -1.6439, -2.7049,  0.1949,  1.6377,  0.0745]],
                         dtype=oneflow.float32, placement=P0, sbp=BROADCAST)
        ...
        if call_cnt == 1:
            ...
            global_state_dict = flow.dict_to_global(local_state_dict, placement=model_file_placement, sbp=flow.sbp.split(0),
            ...
            if cur_rank == 0:
                test_case.assertTrue(
                    np.array_equal(
                        global_state_dict["module_pipeline"]["m_stage0.linear.weight"].to_local().numpy(),
                        module_pipleine.m_stage0.linear.weight.to_local().numpy()[:4]   # The first half of shape (8, 16)
                    )
                )
                test_case.assertTrue(
                    np.array_equal(
                        global_state_dict["module_pipeline"]["m_stage0.linear.bias"].to_local().numpy(),
                        module_pipleine.m_stage0.linear.bias.to_local().numpy()[:4]   # The first half of shape (8,)
                    )
                )
            if cur_rank == 1:
                test_case.assertTrue(
                    np.array_equal(
                        global_state_dict["module_pipeline"]["m_stage1.linear.weight"].to_local().numpy(),
                        module_pipleine.m_stage1.linear.weight.to_local().numpy()[:, 4:]    # The second half of shape (1, 8)
                    )
                )
                test_case.assertTrue(
                    np.array_equal(
                        global_state_dict["module_pipeline"]["m_stage1.linear.bias"].to_local().numpy(),
        ...
        if call_cnt == 1:
            ...
            test_case.assertTrue(
                np.array_equal(
                    iter0_state_dict["module_pipeline"]["m_stage0.linear.weight"].to_local().numpy(),
                    last_state_dict["module_pipeline"]["m_stage0.linear.weight"].to_local().numpy(),
                )
            )
            test_case.assertTrue(
                np.array_equal(
                    iter0_state_dict["module_pipeline"]["m_stage0.linear.bias"].to_local().numpy(),
                    last_state_dict["module_pipeline"]["m_stage0.linear.bias"].to_local().numpy(),
                )
            )
            test_case.assertTrue(
                np.array_equal(
                    iter0_state_dict["module_pipeline"]["m_stage1.linear.weight"].to_local().numpy(),
                    last_state_dict["module_pipeline"]["m_stage1.linear.weight"].to_local().numpy(),
                )
            )
            test_case.assertTrue(
                np.array_equal(
                    iter0_state_dict["module_pipeline"]["m_stage1.linear.bias"].to_local().numpy(),
                    last_state_dict["module_pipeline"]["m_stage1.linear.bias"].to_local().numpy(),
                )
            )
        ...
        if call_cnt == 0:
            model_file_state_dict = flow.dict_to_global(iter1_state_dict, placement=model_file_placement, sbp=flow.sbp.split(0),
                                                        sbp_for_special_keys=sbp_for_special_keys)
...
def _test_graph_save_load_global_split_4(test_case, model_tensor_placement, model_file_placement):
    ...
    def train_with_graph(call_cnt=0, state_dict_dir=None, last_state_dict=None):
        ...
        x = flow.tensor([[ 0.4286,  0.7402,  0.4161,  0.6103,  0.7394,  1.1330, -0.2311, -0.1013,  0.8537,
                         ...
                         [-0.1581, -1.0319,  1.8430,  0.3576,  0.7288, -0.6912,  0.9966,  1.0840, -1.1760,
                           1.5683, -0.2098, -1.6439, -2.7049,  0.1949,  1.6377,  0.0745]],
                         dtype=flow.float32, placement=P0, sbp=BROADCAST)
        ...
        if call_cnt == 1:
            ...
            if cur_rank == 1:
                test_case.assertTrue(
                    np.array_equal(
                        global_state_dict["module_pipeline"]["m_stage1.linear.weight"].to_local().numpy(),
                        module_pipleine.m_stage1.linear.weight.to_local().numpy()[2:, :]    # The second half of shape (4, 8)
                    )
                )
                test_case.assertTrue(
                    np.array_equal(
                        global_state_dict["module_pipeline"]["m_stage1.linear.bias"].to_local().numpy(),
                        module_pipleine.m_stage1.linear.bias.to_local().numpy()[2:]         # The second half if shape (4,)
                    )
                )
        ...
        if call_cnt == 1:
            ...
            test_case.assertTrue(
                np.array_equal(
                    iter0_state_dict["module_pipeline"]["m_stage2.linear.weight"].to_local().numpy(),
                    last_state_dict["module_pipeline"]["m_stage2.linear.weight"].to_local().numpy(),
                )
            )
            test_case.assertTrue(
                np.array_equal(
                    iter0_state_dict["module_pipeline"]["m_stage2.linear.bias"].to_local().numpy(),
                    last_state_dict["module_pipeline"]["m_stage2.linear.bias"].to_local().numpy(),
                )
            )

            test_case.assertTrue(
                np.array_equal(
                    iter0_state_dict["module_pipeline"]["m_stage3.linear.weight"].to_local().numpy(),
                    last_state_dict["module_pipeline"]["m_stage3.linear.weight"].to_local().numpy(),
                )
            )
            test_case.assertTrue(
                np.array_equal(
                    iter0_state_dict["module_pipeline"]["m_stage3.linear.bias"].to_local().numpy(),
                    last_state_dict["module_pipeline"]["m_stage3.linear.bias"].to_local().numpy(),
                )
            )
