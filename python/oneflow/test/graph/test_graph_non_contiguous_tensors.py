...
class ModuleTest(flow.nn.Module):
    def __init__(self, in_features, out_features, contiguous:bool):
        ...
        if contiguous:
            ...
            self.weight = flow.nn.Parameter(flow.ones(out_features, in_features).transpose(0,1))
...
def _test_graph_non_contiguous_tensors(test_case, device):
    ...
    test_case.assertFalse(graph_non_contiguous_tensors.model.weight.origin.is_contiguous())
    ...
    test_case.assertTrue(np.array_equal(contiguous_graph_output.numpy(), non_contiguous_graph_output.numpy()))
