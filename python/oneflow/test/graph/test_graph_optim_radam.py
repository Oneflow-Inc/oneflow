...
def compare_with_numpy_radam(
    ...
    class CustomRAdamGraph(flow.nn.Graph):
        def __init__(self):
            super().__init__()
            self.m = simp_module
            self.add_optimizer(radam)
    ...
    for i in range(train_iters):
        ...
        radam_x = radam_graph(mask_tensor)
    ...
    def train_by_numpy():
        ...
        def np_train_one_iter(step, grad):
            ...
            print(
                f"numpy: mt: {m}, vt: {v}, m_hat: {m_hat}, rho_t: {rho_t}, rho_inf: {rho_inf}, b1: {bias_correction1}, b2: {bias_correction2}, b2n: {bias_correction2_numerator}"
            )
            if rho_t > 5:
        ...
        for i in range(1, train_iters + 1):
            (x, mt, vt) = np_train_one_iter(i, random_grad_seq[i - 1])
