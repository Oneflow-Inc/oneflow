...
def compare_with_numpy_adamax(
    ...
    class CustomAdamaxGraph(flow.nn.Graph):
        def __init__(self):
            super().__init__()
            self.m = simp_module
            self.add_optimizer(adamax)
    ...
    of_res_list = []
    adamax_graph = CustomAdamaxGraph()
    ...
    for i in range(train_iters):
        ...
        adamax_x = adamax_graph(mask_tensor)
    ...
    def train_by_numpy():
        ...
        def np_train_one_iter(step, grad):
            ...
            param = x - ((learning_rate / bias_correction1) * m / norm)
            return (param, m, norm)

        for i in range(1, train_iters + 1):
            (x, mt, normt) = np_train_one_iter(i, random_grad_seq[i - 1])
