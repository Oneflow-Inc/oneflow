...
class TestFeedVariableTensor(unittest.TestCase):
    ...
    def test_feed_var_tensor(test_case):
        ...
        with oneflow._oneflow_internal.lazy_mode.gard(True):
            oneflow._oneflow_internal.JobBuildAndInferCtx_Open('cc_test_variable_op_expr_job')
            ...
            var_conf = oneflow._oneflow_internal.oneflow.core.operator.op_conf.FeedVariableOpConf()
            var_conf.set_in_0('EagerTensorInput')
            var_conf.set_out_0('out_0')
            var_op = oneflow._oneflow_internal.one.FeedVariableOpExpr(op_name, var_conf, ['in_0'], ['out_0'])
