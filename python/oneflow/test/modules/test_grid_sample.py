...
class TestGridSample(flow.unittest.TestCase):
    def test_grid_sample_4d(test_case):
        ...
        np_grid = np.array([[[-0.9, -4.1], [0, 0.2000], [1, -1], [-0.333, 1e-6], [0.5, 1.0]],
             [[-1.0, -0.5], [0, 0.3333], [1, -1], [-0.200, 1e-6], [1.5, 0.5]]]).reshape(1, 2, 5, 2)
        grid = flow.Tensor(np_grid)
        groundtruth = np.reshape(np.array([[0., 8., 5., 7., 9.], [1., 8., 5., 8., 0.]]), (1, 1, 2, 5))
    ...
    def test_flow_affine_grid_cudnn_with_random_data(test_case):
        ...
        grid = torch.nn.functional.affine_grid(theta, (N, C, out_H, out_W), align_corners=align_corners).to(device)
        input = random_pytorch_tensor(ndim=4, dim0=N, dim1=C, dim2=in_H, dim3=in_W).to(device)
    ...
    def test_flow_affine_grid_5d_with_random_data(test_case):
        ...
        grid = torch.nn.functional.affine_grid(theta, (N, C, out_D, out_H, out_W), align_corners=align_corners).to(device)
        input = random_pytorch_tensor(ndim=5, dim0=N, dim1=C, dim2=in_D, dim3=in_H, dim4=in_W).to(device)
        output = torch.nn.functional.grid_sample(input, grid, mode=mode, padding_mode=padding_mode,
                                                 align_corners=align_corners)
        return output
