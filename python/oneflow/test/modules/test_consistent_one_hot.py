...
def _test_onehot_broadcast(test_case, placement, sbp, device, num_classes, size, on_value, off_value):
    x = np.random.randint(9, size=size)
    input = flow.tensor(x, device=flow.device(device), dtype=flow.int64)
    input = input.to_consistent(
    ...
    if flow.env.get_rank() == 0:
        test_case.assertTrue(np.allclose(output.to_local().numpy(), np_out, 1e-06, 1e-06))


class OneHottConsistentCase(oneflow.unittest.TestCase):
    @consistent
    def test_one_hot_broadcast(test_case):
        arg_dict = OrderedDict()
        arg_dict["test_fun"] = [
            _test_onehot_broadcast,
        ]
...
if __name__ == "__main__":
    unittest.main()
#单卡两卡都有问题 : This is a oneflow bug,