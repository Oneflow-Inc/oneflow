...
def _test_rnn(test_case, device):
    ...
    rnn_torch = torch.nn.RNN(input_size = input_size, 
    ...
    for w in rnn_torch.parameters():
        weights_torch.append(w.cpu().data.numpy())
        
    rnn_flow = flow.nn.RNN(input_size = input_size, 
                hidden_size = hidden_size, 
                num_layers =num_layers, 
                nonlinearity =nonlinearity, 
                bias = bias, 
    ...
    z_flow = out_flow.sum()
    z_flow.backward()
    test_case.assertTrue(np.allclose(x_torch.cpu().data.numpy(), x_flow.cpu().data.numpy()))
...
def _test_lstm(test_case, device):
    ...
    lstm_torch = torch.nn.LSTM(input_size = input_size, 
                            hidden_size = hidden_size, 
                            num_layers =num_layers, 
                            bias = bias, 
                            batch_first = batch_first, 
    ...
    lstm_flow = flow.nn.LSTM(input_size = input_size, 
                hidden_size = hidden_size, 
                num_layers =num_layers, 
                bias = bias, 
                batch_first = batch_first, 
...
def _test_gru(test_case, device):
    ...
    gru_torch = torch.nn.GRU(input_size = input_size, 
                                hidden_size = hidden_size, 
                                num_layers =num_layers, 
                                bias = bias, 
                                batch_first = batch_first, 
    ...
    gru_flow = flow.nn.GRU(input_size = input_size, 
                    hidden_size = hidden_size, 
                    num_layers =num_layers, 
                    bias = bias, 
                    batch_first = batch_first, 
