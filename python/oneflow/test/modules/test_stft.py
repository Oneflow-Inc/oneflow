...
class Test(flow.unittest.TestCase):
    def test_stft(test_case):
        ...
        y_torch = torch.stft(
             x, n_fft=rand_fft, window=win,win_length=rand_fft, center=True, onesided=True, normalized=False,return_complex=False,
        )

        x_flow = flow.tensor(np_tensor).to("cuda")
        win_flow=flow.tensor(win_tensor).to("cuda")

        y_flow = flow.stft(x_flow, n_fft=rand_fft, window=win_flow,win_length=rand_fft, center=True,onesided=True, normalized=False,return_complex=False)

        test_case.assertTrue(
            np.allclose(
        y_flow.numpy(), y_torch.cpu().numpy(), rtol=1e-5, atol=1e-5
        )
        )
