...
def _run_test_quantize(
    ...
    input_tensor = input_tensor.to_consistent(placement=placement, sbp=sbp)
    ...
    if flow.env.get_rank() == 0:
        out = output_tensor.to_local().numpy()
        _check_quantize(
            test_case,
            input,
            out.flatten(),
        ...
        for arg in GenArgList(arg_dict):
            ...
            for placement in all_placement():
                for sbp in all_sbp(placement, max_dim=2):
                    _run_test_quantize(placement, sbp, *arg)
