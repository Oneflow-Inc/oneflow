...
def _np_vector_norm_backward(x, ord=2, dim=None):
    ...
    if isinstance(ord, int) and isinstance(dim, int):
        ...
        if ord == 0:
            ...
            re = np.where(x ** ord < 0, -temp, temp) * x ** (ord - 1)
    elif isinstance(ord, float) and isinstance(dim, int) and (ord in [float('inf'), float('-inf')]):
        if ord == float('inf'):
            max_ind = np.argmax(np.abs(x), dim)
            index = [(i, max_ind[i]) for i in range(len(max_ind))] if dim == 1 else [(max_ind[i], i) for i in range(len(max_ind))]
            print(index)
            for j in index:
            ...
            index = [(i, min_ind[i]) for i in range(len(min_ind))] if dim == 1 else [(min_ind[i], i) for i in range(len(min_ind))]
            for j in index:
                re[j] += 1 if x[j] != 0 else 0
            re = np.where(x < 0, -re, re)
    return re
...
def _test_norm_1d(test_case, device):
    input = flow.Tensor(np.random.randn(10), dtype=flow.float32, device=flow.device(device))
...
def _test_norm_2d(test_case, device):
    input = flow.Tensor(np.random.randn(5, 4), dtype=flow.float32, device=flow.device(device))
    of_out_1 = flow.linalg.norm(input)
...
def _test_norm_Nd(test_case, device):
    input1 = flow.Tensor(np.random.randn(3, 4, 3), dtype=flow.float32, device=flow.device(device))
    input2 = flow.Tensor(np.random.randn(3, 4, 3, 5), dtype=flow.float32, device=flow.device(device))
...
def _test_fro_order_norm_backward(test_case, device):
    input = flow.Tensor(np.random.randn(5, 4), dtype=flow.float32, device=flow.device(device), requires_grad=True)
...
def _test_1d_inf_order_norm_backward(test_case, device):
    for ord in [float('inf'), -float('inf')]:
        input = flow.Tensor(np.random.randn(5), dtype=flow.float32, device=flow.device(device), requires_grad=True)
...
def _test_2d_digits_order_norm_backward(test_case, device):
    for ord in [1, -1]:
        input = flow.Tensor(np.random.randn(4, 5), dtype=flow.float32, device=flow.device(device), requires_grad=True)
        of_out = flow.linalg.norm(input, ord=ord)
        of_out.backward()
        np_out_grad = _np_matrix_norm_backward(input.numpy(), ord=ord)
        test_case.assertTrue(np.allclose(input.grad.numpy(), np_out_grad, 1e-05, 1e-05))
...
class TestNormModule(flow.unittest.TestCase):
    ...
    def test_norm(test_case):
        ...
        arg_dict['fun'] = [_test_norm_1d, _test_norm_2d, _test_norm_Nd, _test_fro_order_norm_backward, _test_1d_inf_order_norm_backward, _test_2d_inf_order_norm_backward, _test_1d_digits_order_norm_backward, _test_2d_digits_order_norm_backward]
