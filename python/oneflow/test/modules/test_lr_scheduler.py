...
class TestLrScheduler(flow.unittest.TestCase):
    ...
    def test_cosine_annealing_lr(test_case):
        optimizer = flow.optim.SGD([{'params': [Parameter(flow.Tensor([1.0]))]}], lr=TestLrScheduler.base_lr)
        ...
        alpha = 0.5
        steps = 10
        cosine_annealing_lr = flow.optim.lr_scheduler.CosineAnnealingLR(optimizer, steps=steps, alpha=alpha)
        for i in range(1, 21):
            cosine_annealing_lr.step()
            new_lr = cosine_annealing_lr_step(TestLrScheduler.base_lr, i, steps, alpha)
            test_case.assertAlmostEqual(cosine_annealing_lr.get_last_lr()[0], new_lr, places=4)
    ...
    def test_lambda_lr(test_case):
        optimizer = flow.optim.SGD([{'params': [Parameter(flow.Tensor([1.0]))]}, {'params': [Parameter(flow.Tensor([1.0]))]}], lr=TestLrScheduler.base_lr)
        lambdas = [lambda step: step // 30, lambda step: 0.95 * step]

        def lambda_lr_step(base_lrs, current_step):
            return [base_lr * lmbda(current_step) for (base_lr, lmbda) in zip(base_lrs, lambdas)]
