...
class TestSumModule(flow.unittest.TestCase):
    ...
    @autotest(n=10, check_graph=False, auto_backward=True, include_complex=True, atol=1e-2, rtol=1e-5)
    def test_sum_complex_dtype(test_case):
        ...
        y = torch.sum(x, dim=np.random.randint(0, 3), keepdim=random_bool(),dtype=random_dtype(["complex"]))
        return y
