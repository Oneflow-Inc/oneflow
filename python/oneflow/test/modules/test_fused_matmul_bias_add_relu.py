...
def _test_fused_matmul_bias_add_relu(
    test_case, transpose_a, transpose_b
):
    ...
    origin_out = flow._C.relu(
                    flow._C.bias_add(
                        flow._C.matmul(origin_a_tensor, origin_b_tensor, 
                            transpose_a=transpose_a, transpose_b=transpose_b), origin_bias_tensor, axis=1)) # TODO: currently only support 2d fused matmul. 
