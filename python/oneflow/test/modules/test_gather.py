...
def _test_gather(test_case, device):
    ...
    output = flow.gather(flow.Tensor(input, device=flow.device(device)), flow.Tensor(index, dtype=flow.int, device=flow.device(device)), dim=0)
...
def _test_gather_random_array(test_case, device):
    ...
    np_out = np.take_along_axis(input, index, 1)
    output = flow.gather(flow.Tensor(input, device=flow.device(device)), flow.Tensor(index, dtype=flow.int, device=flow.device(device)), dim=1)
    test_case.assertTrue(np.allclose(output.numpy(), np_out))
    np_out2 = np.take_along_axis(input, index, 2)
    output2 = flow.gather(flow.Tensor(input, device=flow.device(device)), flow.Tensor(index, dtype=flow.int, device=flow.device(device)), dim=2)
    test_case.assertTrue(np.allclose(output2.numpy(), np_out2))
    np_out3 = np.take_along_axis(input, index, 3)
    output3 = flow.gather(flow.Tensor(input, device=flow.device(device)), flow.Tensor(index, dtype=flow.int, device=flow.device(device)), dim=3)
...
def _test_gather_backward(test_case, device):
    ...
    output = flow.gather(of_input, flow.Tensor(index, dtype=flow.int, device=flow.device(device)), dim=0)
    out_sum = output.sum()
    out_sum.backward()
    test_case.assertTrue(np.array_equal(output.numpy(), np_out))
...
class TestGather(flow.unittest.TestCase):
    ...
    def test_gather(test_case):
        ...
        arg_dict['test_fun'] = [_test_gather, _test_gather_tensor_function, _test_gather_random_array, _test_gather_backward]
