...
def _test_spectral_norm(test_case, device):
    ...
    for apply_dp in (True, False):
        ...
        for requires_grad in (True, False):
            ...
            m_flow = flow.nn.utils.remove_spectral_norm(m_flow)
            test_case.assertTrue(np.allclose(wrapped_m(input_tensor).numpy(), pre_remove_out.numpy()))
            ...
            test_case.assertTrue(np.allclose(wrapped_m(input_tensor).numpy(), eval_out0.numpy()))
            test_case.assertTrue(np.allclose(last_train_u.numpy(), m_flow.weight_u.numpy()))
            test_case.assertTrue(np.allclose(last_train_v.numpy(), m_flow.weight_v.numpy()))
