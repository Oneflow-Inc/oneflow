...
def _test_concat_with_0_input(test_case, device):
    input1 = flow.LongTensor().to(flow.float32).to(flow.device(device))
    input2 = flow.tensor(
        np.random.randn(2, 6, 5, 3), dtype=flow.float32, device=flow.device(device)
    )
    of_out1 = flow.cat([input1, input2], dim=0)
    of_out2 = flow.cat([input1, input2, input1, input1], dim=0)
    ...
    torch_out2 = torch.cat((torch_input1, torch_input2, torch_input1, torch_input1), 0)
