...
def _test_ones_like(test_case, device, shape):
    ...
    test_case.assertTrue(np.array_equal(np.ones_like(x.numpy()), flow.ones_like(x).numpy()))
    x2 = flow.Tensor(np.ones([2, 4], dtype=int))
    test_case.assertTrue(np.array_equal(np.ones_like(x2.numpy()), flow.ones_like(x2).numpy()))

def _test_zeros_like(test_case, device, shape):
    x = flow.Tensor(np.ones(shape, dtype=np.float64))
    test_case.assertTrue(np.array_equal(np.zeros_like(x.numpy()), flow.zeros_like(x).numpy()))
    x2 = flow.Tensor(np.ones(shape, dtype=int))
    test_case.assertTrue(np.array_equal(np.zeros_like(x2.numpy()), flow.zeros_like(x2).numpy()))
...
class TestConstantModule(flow.unittest.TestCase):
    ...
    def test_cast(test_case):
        ...
        arg_dict['test_fun'] = [_test_ones, _test_different_dtype, _test_zeros, _test_ones_backward, _test_zeros_backward, _test_ones_like, _test_zeros_like, _test_new_ones]
