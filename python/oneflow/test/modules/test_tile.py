...
def _test_tile_less_dim_b(test_case, device):
    input = flow.Tensor(np.random.randn(3, 2, 5), dtype=flow.float32, device=flow.device(device))
    sizes = (3, 4)
    np_out = np_tile(input.numpy(), sizes)
    of_out = input.tile(reps=sizes)
    test_case.assertTrue(np.array_equal(of_out.numpy(), np_out))
...
def _test_tile_less_dim_c(test_case, device):
    input = flow.Tensor(np.random.randn(4, 3, 2, 5, 3), dtype=flow.float32, device=flow.device(device))
    sizes = (2, 3, 4, 4)
    np_out = np_tile(input.numpy(), sizes)
    of_out = input.tile(reps=sizes)
    test_case.assertTrue(np.array_equal(of_out.numpy(), np_out))
...
def _test_tile_less_dim_b_backward(test_case, device):
    input = flow.Tensor(np.random.randn(3, 2, 5), dtype=flow.float32, device=flow.device(device), requires_grad=True)
    sizes = (3, 4)
    of_out = input.tile(reps=sizes)
    of_out = of_out.sum()
    of_out.backward()
...
def _test_tile_less_dim_c_backward(test_case, device):
    input = flow.Tensor(np.random.randn(4, 3, 2, 5, 3), dtype=flow.float32, device=flow.device(device), requires_grad=True)
    sizes = (2, 3, 4, 4)
    of_out = input.tile(reps=sizes)
    of_out = of_out.sum()
    of_out.backward()
...
class TestTile(flow.unittest.TestCase):
    ...
    def test_tile(test_case):
        ...
        arg_dict['test_fun'] = [_test_tile_less_dim_a, _test_tile_less_dim_b, _test_tile_less_dim_c, _test_tile_same_dim, _test_tile_same_dim_int, _test_tile_same_dim_int8, _test_tile_less_dim_a_backward, _test_tile_less_dim_b_backward, _test_tile_less_dim_c_backward, _test_tile_same_dim_backward]
