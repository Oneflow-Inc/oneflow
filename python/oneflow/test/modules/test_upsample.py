...
class TestUpsample2d(flow.unittest.TestCase):
    ...
    def test_upsample3d_nearest_output_size(test_case):
        device = random_device()
        x = random_tensor(ndim=5, dim0=1, dim1=2, dim2=12, dim3=937, dim4=32).to(
            device
        )
    ...
    def test_upsample1d_linear_output_size(test_case):
        device = random_device()
        x = random_tensor(ndim=3, dim0=1, dim1=2, dim2=937).to(device)
        m = torch.nn.Upsample(size=(30), mode="linear")
        y = m(x)
        return y
