...
def _test_stack_backward(test_case, device, shape):
    ...
    out_of.backward()
    test_case.assertTrue(np.allclose(x_tensor.grad.numpy(), np.ones(x_tensor.shape), 1e-05, 1e-05))
    test_case.assertTrue(np.allclose(y_tensor.grad.numpy(), np.ones(y_tensor.shape), 1e-05, 1e-05))
...
class TestStack(flow.unittest.TestCase):
    ...
    def test_stack(test_case):
        ...
        arg_dict['test_fun'] = [_test_stack, _test_stack_tuple_input, _test_stack_backward, _test_stack_different_dim, _test_stack_multi_input]
        arg_dict['device'] = ['cpu', 'cuda']
        arg_dict['shape'] = [tuple((random.randrange(1, 10) for _ in range(i))) for i in range(3, 6)]
