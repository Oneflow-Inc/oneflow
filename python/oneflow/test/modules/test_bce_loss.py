...
def _test_bceloss_impl(test_case, device, reduction):
    ...
    of_out.backward()
    if reduction == 'none':
        np_grad = np.array([[1.5370497, -0.90033215, 0.851115], [-0.6636245, 1.2913125, -1.7615942]]).astype(np.float32)
    elif reduction == 'sum':
        np_grad = np.array([[1.5370497, -0.90033215, 0.851115], [-0.6636245, 1.2913125, -1.7615942]]).astype(np.float32)
    else:
        np_grad = np.array([[0.25617492, -0.15005533, 0.14185251], [-0.11060409, 0.21521877, -0.29359904]]).astype(np.float32)
    test_case.assertTrue(np.allclose(input.grad.numpy(), np_grad, 1e-05, 1e-05))
    input_none = input = flow.Tensor(np.array([[1.2, 0.2, -0.3], [0.7, 0.6, -2]]).astype(np.float32), dtype=flow.float32, requires_grad=True, device=flow.device(device))
    ...
    if reduction == 'none':
        np_grad_none = np.array([[0.7685, -0.4502, 0.4256], [-0.3318, 0.6457, -0.8808]]).astype(np.float32)
    elif reduction == 'sum':
        np_grad_none = np.array([[0.7685, -0.4502, 0.4256], [-0.3318, 0.6457, -0.8808]]).astype(np.float32)
    else:
        np_grad_none = np.array([[0.1281, -0.075, 0.0709], [-0.0553, 0.1076, -0.1468]]).astype(np.float32)
    test_case.assertTrue(np.allclose(input_none.grad.numpy(), np_grad_none, 0.0001, 0.0001))
    input_none = input = flow.Tensor(np.array([[1.2, 0.2, -0.3], [0.7, 0.6, -2]]).astype(np.float32), dtype=flow.float32, requires_grad=True, device=flow.device(device))
    sigmoid_input_none = activation(input_none)
    loss_none = flow.nn.BCELoss(reduction=reduction)
    loss_none = loss_none.to(device)
