...
def _test_ones_like_float(test_case, placement, sbp, shape, device):
    ...
    if flow.env.get_rank() == 0:
        test_case.assertTrue(y.to_local().dtype is flow.float32)
        test_case.assertTrue(y.to_local().shape == x.to_local().shape)
        test_case.assertTrue(y.to_local().device == x.to_local().device)
        y_numpy = np.ones_like(x.to_local().numpy())
        test_case.assertTrue(np.array_equal(y.to_local().numpy(), y_numpy))
...
def _test_ones_like_int(test_case, placement, sbp, shape, device):
    ...
    y = flow.ones_like(x)
    if flow.env.get_rank() == 0:
        test_case.assertTrue(y.to_local().dtype is flow.int)
        test_case.assertTrue(y.to_local().shape == x.to_local().shape)
        test_case.assertTrue(y.to_local().device == x.to_local().device)
        y_numpy = np.ones_like(x.to_local().numpy())
        test_case.assertTrue(np.array_equal(y.to_local().numpy(), y_numpy))
