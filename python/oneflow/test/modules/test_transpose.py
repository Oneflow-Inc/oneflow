...
def _test_transpose_backward(test_case, device):
    x = flow.Tensor(np.random.randn(2, 6, 5, 3), dtype=flow.float32, device=flow.device(device), requires_grad=True)
    y = flow.transpose(x, 0, 1).sum()
    y.backward()
    test_case.assertTrue(np.allclose(x.grad.numpy(), np.ones((2, 6, 5, 3)), 1e-05, 1e-05))

def _test_transpose_backward_v2(test_case, device):
    x = flow.Tensor(np.random.randn(2, 3, 4, 5), dtype=flow.float32, device=flow.device(device), requires_grad=True)
    y = flow.transpose(x, 3, 1).sum()
    y.backward()
    test_case.assertTrue(np.allclose(x.grad.numpy(), np.ones((2, 3, 4, 5)), 1e-05, 1e-05))
...
class TestTranspose(flow.unittest.TestCase):
    ...
    def test_transpose(test_case):
        ...
        arg_dict['fun'] = [_test_transpose, _test_tensor_transpose, _test_tranpose_negative_dim, _test_transpose_backward, _test_transpose_backward_v2]
