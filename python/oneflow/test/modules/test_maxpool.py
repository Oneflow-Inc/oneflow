...
class TestMaxPoolingFunctional(flow.unittest.TestCase):
    ...
    def profile_maxpool2d(test_case):
        torch.nn.functional.max_pool2d(torch.ones(1, 32, 64, 64), kernel_size=3, padding=1)
        torch.nn.functional.max_pool2d(torch.ones(1, 32, 64, 64), kernel_size=3, stride=2, padding=1)
        torch.nn.functional.max_pool2d(torch.ones(16, 32, 64, 64), kernel_size=3, padding=1)
        torch.nn.functional.max_pool2d(torch.ones(16, 32, 64, 64), kernel_size=3, stride=2, padding=1)
        torch.nn.functional.max_pool2d(torch.ones(16, 32, 64, 64), kernel_size=3, stride=2, padding=1, ceil_mode=True)
