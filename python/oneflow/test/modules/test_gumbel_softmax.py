...
def _test_gumbel_softmax(test_case, tau, dim, device, dtype):
    ...
    y_hard = F.gumbel_softmax(x)
    test_case.assertEqual(shape, y_soft.shape)
    test_case.assertEqual(shape, y_hard.shape)
...
def _test_backward(test_case, tau, dim, device, dtype):
    ...
    test_case.assertTrue(np.testing.assert_allclose(
        x_hard.grad.numpy(),
        x_soft.grad.numpy(),
        rtol=1e-5,
        atol=1e-8
...
class TestGumbelSoftmaxModule(flow.unittest.TestCase):
    def test_gumbel_softmax(test_case):
        ...
        arg_dict["fun"] = [_test_gumbel_softmax, _test_backward]
