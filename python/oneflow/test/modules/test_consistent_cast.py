...
class TestConsistentCast_XToB(flow.unittest.TestCase):
    ...
    def test_consistent_to_consistent_stb_gpu_to_cpu(test_case):
        if flow.distributed.get_rank() == 0:
            np_arr = np.array(
                [[4, 6, 5, 20], [6, 8, 9, 0], [3, 7, 5, 0], [6, 8, 9, 0]],
                dtype=np.float32,
        ...
        placement = flow.placement("cuda", {0: range(3)})
        consistent_tensor = tensor.to_consistent(placement, flow.sbp.split(0))
        new_placement = flow.placement("cpu", {0: range(4)})
        broadcast_tensor = consistent_tensor.to_consistent(
            new_placement, flow.sbp.broadcast
        )
        test_case.assertTrue(broadcast_tensor.placement, new_placement)
    ...
    def test_consistent_to_consistent_ptb_gpu_to_cpu(test_case):
        if flow.distributed.get_rank() == 0:
            np_arr = np.array(
                [[4, 6, 5, 20], [6, 8, 9, 0], [3, 7, 5, 0], [6, 8, 9, 0]],
                dtype=np.float32,
    ...
    def test_consistent_to_consistent_stb_cpu_to_gpu(test_case):
        if flow.distributed.get_rank() == 0:
            np_arr = np.array(
                [[4, 6, 5, 20], [6, 8, 9, 0], [3, 7, 5, 0], [6, 8, 9, 0]],
                dtype=np.float32,
        ...
        tensor = flow.Tensor(np_arr, device=device, dtype=flow.float32)
        placement = flow.placement("cpu", {0: range(3)})
        consistent_tensor = tensor.to_consistent(placement, flow.sbp.split(0))
        new_placement = flow.placement("cuda", {0: range(4)})
        broadcast_tensor = consistent_tensor.to_consistent(
            new_placement, flow.sbp.broadcast
    ...
    def test_consistent_to_consistent_ptb_cpu_to_gpu(test_case):
        if flow.distributed.get_rank() == 0:
            np_arr = np.array(
                [[4, 6, 5, 20], [6, 8, 9, 0], [3, 7, 5, 0], [6, 8, 9, 0]],
                dtype=np.float32,
    ...
    def test_consistent_to_consistent_stb_cpu_to_cpu(test_case):
        if flow.distributed.get_rank() == 0:
            np_arr = np.array(
                [[4, 6, 5, 20], [6, 8, 9, 0], [3, 7, 5, 0], [6, 8, 9, 0]],
                dtype=np.float32,
    ...
    def test_consistent_to_consistent_ptb_cpu_to_cpu(test_case):
        if flow.distributed.get_rank() == 0:
            np_arr = np.array(
                [[4, 6, 5, 20], [6, 8, 9, 0], [3, 7, 5, 0], [6, 8, 9, 0]],
                dtype=np.float32,
