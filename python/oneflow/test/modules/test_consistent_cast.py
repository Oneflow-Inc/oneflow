...
class TestConsistentCast_XToB(flow.unittest.TestCase):
    def test_consistent_to_consistent_btb(test_case):
        if flow.distributed.get_rank() == 0:
            np_arr = np.array(
        ...
        test_case.assertTrue(broadcast_tensor.placement, new_placement)
        if flow.distributed.get_rank() == 0 or flow.distributed.get_rank() == 1:
            test_case.assertTrue(
                np.array_equal(
                    broadcast_tensor.to_local().numpy(),
                    np.array(
    ...
    def test_consistent_to_consistent_stb(test_case):
        if flow.distributed.get_rank() == 0:
            np_arr = np.array(
                [[4, 6, 5, 20], [6, 8, 9, 0], [3, 7, 5, 0], [6, 8, 9, 0]],
                dtype=np.float32,
    ...
    def test_consistent_to_consistent_ptb(test_case):
        if flow.distributed.get_rank() == 0:
            np_arr = np.array(
                [[4, 6, 5, 20], [6, 8, 9, 0], [3, 7, 5, 0], [6, 8, 9, 0]],
                dtype=np.float32,
