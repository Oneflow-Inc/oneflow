                ...
                for i in range(self.pad_out_depth):
                    for j in range(self.pad_out_height):
                        for k in range(self.pad_out_width):
                            ...
                            out[n, c, i, j, k] = np.max(pad_x[n, c, start_i:end_i, start_j:end_j, start_k:end_k])
                            self.arg_max[n, c, i, j, k] = np.argmax(pad_x[n, c, start_i:end_i, start_j:end_j, start_k:end_k])
                ...
                for i in range(self.pad_out_depth):
                    for j in range(self.pad_out_height):
                        for k in range(self.pad_out_width):
                            ...
                            end_j = start_j + self.w_height
                            end_k = start_k + self.w_width
                            index = np.unravel_index(self.arg_max[n, c, i, j, k], self.kernel_size)
                            dx[n, c, start_i:end_i, start_j:end_j, start_k:end_k][index] += d_loss[n, c, i, j, k]
        dx = dx[:, :, self.padding[0]:self.pad_shape[2] - self.padding[0], self.padding[1]:self.pad_shape[3] - self.padding[1], self.padding[2]:self.pad_shape[4] - self.padding[2]]
        dx = dx.reshape(self.x_shape)
        return dx

def _test_maxpool1d_impl(test_case, device):
    input_arr = np.array([[[-0.89042996, 2.33971243, -0.86660827, 0.80398747], [-1.46769364, -0.78125064, 1.50086563, -0.76278226], [1.31984534, 0.20741192, -0.86507054, -0.40776015], [-0.89910823, 0.44932938, 1.49148118, -0.22036761]], [[-0.5452334, -0.10255169, -1.42035108, 0.73922913], [-0.03192764, 0.69341935, 0.96263152, -1.52070843], [0.02058239, 1.504032, 1.84423001, -0.0130596], [2.20517719, 0.38449598, 0.85677771, 0.60425179]], [[-1.64366213, 0.51370298, -0.21754866, -0.05085382], [1.17065374, 1.13857674, -1.13070507, 0.44353707], [-1.30783846, -0.48031445, 0.41807536, -2.13778887], [0.08259005, 0.5798125, 0.03024696, 1.96100924]]])
    (kernel_size, stride, padding) = ((3,), (1,), (1,))
    output = np.array([[[2.33971243, 2.33971243, 2.33971243, 0.80398747], [-0.78125064, 1.50086563, 1.50086563, 1.50086563], [1.31984534, 1.31984534, 0.20741192, -0.40776015], [0.44932938, 1.49148118, 1.49148118, 1.49148118]], [[-0.10255169, -0.10255169, 0.73922913, 0.73922913], [0.69341935, 0.96263152, 0.96263152, 0.96263152], [1.504032, 1.84423001, 1.84423001, 1.84423001], [2.20517719, 2.20517719, 0.85677771, 0.85677771]], [[0.51370298, 0.51370298, 0.51370298, -0.05085382], [1.17065374, 1.17065374, 1.13857674, 0.44353707], [-0.48031445, 0.41807536, 0.41807536, 0.41807536], [0.5798125, 0.5798125, 1.96100924, 1.96100924]]])
    output_indice = np.array([[[1, 1, 1, 3], [1, 2, 2, 2], [0, 0, 1, 3], [1, 2, 2, 2]], [[1, 1, 3, 3], [1, 2, 2, 2], [1, 2, 2, 2], [0, 0, 2, 2]], [[1, 1, 1, 3], [0, 0, 1, 3], [1, 2, 2, 2], [1, 1, 3, 3]]])
    grad = np.array([[[0.0, 3.0, 0.0, 1.0], [0.0, 1.0, 3.0, 0.0], [2.0, 1.0, 0.0, 1.0], [0.0, 1.0, 3.0, 0.0]], [[0.0, 2.0, 0.0, 2.0], [0.0, 1.0, 3.0, 0.0], [0.0, 1.0, 3.0, 0.0], [2.0, 0.0, 2.0, 0.0]], [[0.0, 3.0, 0.0, 1.0], [2.0, 1.0, 0.0, 1.0], [0.0, 1.0, 3.0, 0.0], [0.0, 2.0, 0.0, 2.0]]])
    m = flow.nn.MaxPool1d(kernel_size=kernel_size, stride=stride, padding=padding, return_indices=True)
...
class TestPooling(flow.unittest.TestCase):
    ...
    def test_maxpool2d(test_case):
        arg_dict = OrderedDict()
        arg_dict['test_fun'] = [_test_maxpool2d, _test_maxpool2d_ceil_mode, _test_maxpool2d_special_kernel_size, _test_maxpool2d_diff_kernel_stride, _test_maxpool2d_negative_input, _test_maxpool2d_backward, _test_maxpool2d_special_kernel_size_backward, _test_maxpool2d_diff_kernel_stride_backward, _test_maxpool2d_negative_input_backward]
        arg_dict['device'] = ['cpu', 'cuda']
        for arg in GenArgList(arg_dict):
            arg[0](test_case, *arg[1:])
    ...
    def test_maxpool3d(test_case):
        arg_dict = OrderedDict()
        arg_dict['test_fun'] = [_test_maxpool3d, _test_maxpool3d_backward, _test_maxpool3d_special_kernel_size_backward, _test_maxpool3d_negative_input_backward, _test_maxpool3d_diff_kernel_stride_backward]
        arg_dict['device'] = ['cpu', 'cuda']
        for arg in GenArgList(arg_dict):
            arg[0](test_case, *arg[1:])
