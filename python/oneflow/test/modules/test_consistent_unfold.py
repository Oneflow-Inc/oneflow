...
def _test_unfold(test_case, ndim, placement, sbp):
    ...
    size = np.random.randint(dim[dimension_])
    step = np.random.randint(dim[dimension_])

    z = x.unfold(dimension_,2,1)

    return z


class TestUnfoldConsistent(flow.unittest.TestCase):
    @consistent
    def test_unfold(test_case):
        ndim = 1#np.random.randint(1, 6)
        for placement in all_placement():
            for sbp in all_sbp(placement, max_dim=ndim):
                _test_unfold(test_case, ndim, placement, sbp)
