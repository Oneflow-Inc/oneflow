...
def _test_meshgrid_forawd(test_case, device):
    input1 = flow.Tensor(np.array([1, 2, 3]), dtype=flow.float32, device=flow.device(device))
    input2 = flow.Tensor(np.array([4, 5, 6]), dtype=flow.float32, device=flow.device(device))
...
def _test_meshgrid_forawd_3tensor(test_case, device):
    input1 = flow.Tensor(np.array([1, 2, 3]), dtype=flow.float32, device=flow.device(device))
    input2 = flow.Tensor(np.array([4, 5, 6]), dtype=flow.float32, device=flow.device(device))
    input3 = flow.Tensor(np.array([7, 8, 9]), dtype=flow.float32, device=flow.device(device))
    (np_x, np_y, np_z) = np.meshgrid(input1.numpy(), input2.numpy(), input3.numpy(), indexing='ij')
...
class TestMeshGrid(flow.unittest.TestCase):
    ...
    def test_meshgrid(test_case):
        ...
        arg_dict['test_fun'] = [_test_meshgrid_forawd, _test_meshgrid_forawd_scalr, _test_meshgrid_forawd_3tensor]
