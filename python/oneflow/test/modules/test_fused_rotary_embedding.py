...
def _test_fused_rotary_embedding(
    ...
    if layout == "BHMK":
        M = dims[2]
        K = dims[3]
        x = np.random.uniform(low=-1, high=1, size=(*dims, ))
        ...
        fused_out = flow._C.fused_rotary_embedding(fused_x, fused_cos, fused_sin, "BHMK")

        test_case.assertTrue(
            np.allclose(naive_out, fused_out.numpy(), atol=5e-2, rtol=1e-3)
        )
    else:
        M = dims[1]
        K = dims[3]
        ...
        num_elements = 1
        stride = [1] * len(merged_dims)

        for i in range(len(merged_dims)):
            dim = merged_dims[len(merged_dims)-1-i]
            num_elements = num_elements * dim
            if i > 0:
                stride[len(merged_dims)-1-i] = stride[len(merged_dims)-i] * merged_dims[len(merged_dims)-i]
        
        for i in range(len(merged_dims)-1):
            stride[i] = stride[i] // 2
        num_elements = num_elements // 2
        
        for i in range(num_elements):
            index = [1] * len(merged_dims)
            offset = i
            for j in range(len(stride)):
                s = stride[j]
                index[j] = offset // s
        ...
        naive_out = x.reshape(dims) * cos.reshape([1, M, 1, K]) + y.reshape(dims) * sin.reshape([1, M, 1, K])

        fused_x = flow.tensor(x, dtype=dtype, device="cuda")
        fused_cos = flow.tensor(cos, dtype=dtype, device="cuda")
        fused_sin = flow.tensor(sin, dtype=dtype, device="cuda")
        
        fused_out = flow._C.fused_rotary_embedding(fused_x, fused_cos, fused_sin, "BME")
