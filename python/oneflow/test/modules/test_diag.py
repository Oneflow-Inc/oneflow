...
def _test_diag_forward(test_case, shape, diagonal, device):
    ...
    test_case.assertTrue(np.allclose(input.diag(diagonal=diagonal).numpy(), np_out, 1e-05, 1e-05, equal_nan=True))
...
def _test_diag_one_dim_backward(test_case, diagonal, device):
    ...
    test_case.assertTrue(np.allclose(input.grad.numpy(), np_grad, 1e-05, 1e-05, equal_nan=True))
    input = flow.Tensor(np.random.randn(3), device=flow.device(device), requires_grad=True)
...
def _test_diag_other_dim_non_square_backward(test_case, diagonal, device):
    input = flow.Tensor(np.random.randn(3, 4), device=flow.device(device), requires_grad=True)
    of_out = flow.diag(input, diagonal).sum()
    of_out.backward()
    if diagonal > 0:
        np_tmp = np.zeros([3, 1])
...
class TestDiag(flow.unittest.TestCase):
    ...
    def test_diag_backward(test_case):
        arg_dict = OrderedDict()
        arg_dict['test_fun'] = [_test_diag_one_dim_backward, _test_diag_other_dim_backward, _test_diag_other_dim_non_square_backward]
        arg_dict['diagonal'] = [1, 0, -1]
        arg_dict['device'] = ['cpu', 'cuda']
        for arg in GenArgList(arg_dict):
