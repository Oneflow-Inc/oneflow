...
def _np_smoothl1loss(np_input, np_target, beta=1.0):
    ...
    return {'none': loss.reshape(original_shape), 'mean': np.mean(loss), 'sum': np.sum(loss)}
...
def _np_smoothl1loss_grad(np_input, np_target, beta=1.0):
    ...
    return {'none': np_input_grad_sum, 'mean': np_input_grad_mean, 'sum': np_input_grad_sum}
...
def _test_smoothl1loss_impl(test_case, device, shape, data_type, reduction, beta):
    ...
    input = flow.Tensor(x, dtype=type_name_to_flow_type[data_type], requires_grad=True, device=flow.device(device))
