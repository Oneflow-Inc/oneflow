...
def compare_with_numpy_adam(test_case, device, x_shape, scale, learning_rate, train_iters, betas, weight_decay, eps):
    random_grad_seq = []
    ...
    def train_by_oneflow():
        ...
        adam = flow.optim.Adam([{'params': [x], 'lr': learning_rate, 'betas': betas, 'eps': eps, 'weight_decay': weight_decay, 'scale': scale}])
    ...
    test_case.assertTrue(np.allclose(oneflow_res.flatten(), numpy_res.flatten(), rtol=0.001, atol=0.001))
