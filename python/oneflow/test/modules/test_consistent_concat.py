...
def _test_cat_with_random_data(test_case, placement, sbp):
    x = random_pytorch_tensor(ndim=2, dim0=2, dim1=4).to_consistent(
...
def _test_concat_with_input_0_size_data(test_case, placement, sbp):
    x = random_pytorch_tensor(4, 2, 4, 2, 4).to_consistent(placement=placement, sbp=sbp)
    y = random_pytorch_tensor(4, 2, 4, random(0, 3), 4).to_consistent(
        placement=placement, sbp=sbp
    )
...
def _test_concat_with_output_0_size_data(test_case, placement, sbp):
    x = random_pytorch_tensor(4, 2, 0, 2, 4).to_consistent(placement=placement, sbp=sbp)
    y = random_pytorch_tensor(4, 2, 0, 2, 4).to_consistent(placement=placement, sbp=sbp)
...
def _test_cat_only_one_tensor(test_case, placement, sbp):
    x = random_pytorch_tensor(4, 2, 4, random(1, 3)).to_consistent(
        placement=placement, sbp=sbp
    )
