...
def _test_clamp_backward(test_case, shape, device):
    ...
    y.backward()
    test_case.assertTrue(np.allclose(x.grad.numpy(), _numpy_clamp_grad(x.numpy(), 0.1, 0.5), 1e-05, 1e-05))
...
class TestClampModule(flow.unittest.TestCase):
    ...
    def test_clamp(test_case):
        ...
        arg_dict['fun'] = [_test_clamp, _test_tensor_clamp, _test_clamp_scalar_min, _test_clamp_scalar_max, _test_clamp_integral, _test_clamp_backward]
