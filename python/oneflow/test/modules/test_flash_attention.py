...
def gen_bert_mask(k):
    ...
    start = se.min(dim=-1).values - 1
    end = se.max(dim=-1).values + 1
    padding_mask = repeat(flow.arange(s).cuda(), "s -> b s", b=b)
    padding_mask = (padding_mask >= start.unsqueeze(1)) + (
        padding_mask < end.unsqueeze(1)
    )
    return padding_mask.unsqueeze(1).unsqueeze(1)


def gen_random_data(
    batch_size, num_heads, seqlen_q, seqlen_kv, head_dim, dtype, unpad_kv="random"
):
    q = (
...
def _test_flash_attention(
    ...
    if add_mask == False and unpad_kv != False:
        return
    if dtype == flow.bfloat16 and unpad_kv != False:
        return
    q, k, v, mask, bias = gen_random_data(
        batch_size, num_heads, seqlen_q, seqlen_kv, head_dim, dtype, unpad_kv
    )
    q.retain_grad()
    k.retain_grad()
    v.retain_grad()
    bias.retain_grad()
    mask_fp = ((1 - mask) * -10000.0).to(dtype)  # float, 0/1 -> 0 ~ -inf

    if add_mask:
        if add_bias:
            out = flash_attention(q, k, v, mask=mask, bias=bias, unpad_kv=unpad_kv)
            ref_out = flow.matmul(
                flow.softmax(
                    flow.matmul(q, k.transpose(-1, -2)) / (head_dim ** 0.5)
                    + mask_fp
                    + bias,
                    dim=-1,
                ),
            ...
            out = flash_attention(q, k, v, mask=mask, unpad_kv=unpad_kv)
            ref_out = flow.matmul(
                flow.softmax(
                    flow.matmul(q, k.transpose(-1, -2)) / (head_dim ** 0.5) + mask_fp,
                    dim=-1,
                ),
                v,
            )
        ...
        if add_bias:
            out = flash_attention(q, k, v, bias=bias, unpad_kv=unpad_kv)
            ref_out = flow.matmul(
                flow.softmax(
                    flow.matmul(q, k.transpose(-1, -2)) / (head_dim ** 0.5) + bias,
                    dim=-1,
                ),
                v,
            )
        else:
            out = flash_attention(q, k, v, unpad_kv=unpad_kv)
            ref_out = flow.matmul(
                flow.softmax(
                    flow.matmul(q, k.transpose(-1, -2)) / (head_dim ** 0.5), dim=-1,
                ),
                v,
            )
    ...
    test_case.assertTrue(
        np.allclose(out.numpy(), ref_out.numpy(), atol=atol, rtol=rtol)
    )

    # sm75 && sm86 do not have enough shared memory for bwd when head_dim>64.
...
class TestFlashAttention(flow.unittest.TestCase):
    def test_flash_attention(test_case):
        ...
        arg_dict["unpad_kv"] = [True, False, "auto", "bert"]
