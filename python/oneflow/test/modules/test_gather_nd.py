...
def _test_gather_nd(test_case, device):
    ...
    output = flow.gather_nd(flow.Tensor(input, dtype=flow.float, device=flow.device(device)), flow.Tensor(indices, dtype=flow.int, device=flow.device(device)))
...
def _test_gather_nd_backward(test_case, device):
    ...
    output = flow.gather_nd(of_input, flow.Tensor(indices, dtype=flow.int, device=flow.device(device)))
    out_sum = output.sum()
    out_sum.backward()
    test_case.assertTrue(np.array_equal(output.numpy(), np_out))
...
class TestGather_nd(flow.unittest.TestCase):
    ...
    def test_gather_nd(test_case):
        ...
        arg_dict['test_fun'] = [_test_gather_nd, _test_gather_nd_t, _test_gather_nd_backward, _test_gather_nd_backward_t]
