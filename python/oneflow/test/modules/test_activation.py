...
def _test_relu6_impl(test_case, shape, device):
    ...
    of_out = of_out.sum()
    of_out.backward()
    test_case.assertTrue(np.allclose(of_input.grad.numpy(), np.where(np_input > 6, 0, np.where(np_input < 0, 0, 1)), 1e-05, 1e-05))
...
def _test_tanh_nn_impl(test_case, shape, device):
    ...
    of_out = of_out.sum()
    of_out.backward()
    test_case.assertTrue(np.allclose(of_input.grad.numpy(), 1.0 - np_out * np_out, 1e-05, 1e-05))
...
def _test_softmax_backward_normal(test_case, device):
    ...
    axis = 0
    m = flow.nn.Softmax(dim=axis)
    x = flow.Tensor(np.random.randn(2, 3, 4, 5), requires_grad=True, device=flow.device(device), dtype=flow.float64)
    y = m(x).sum()
    y.backward()
    test_case.assertTrue(np.allclose(x.grad.numpy(), x_grad, 1e-05, 1e-05))
...
class TestSoftmax(flow.unittest.TestCase):
    ...
    def test_softmax(test_case):
        arg_dict = OrderedDict()
        arg_dict['fun'] = [_test_softmax, _test_softmax_dim_1, _test_softmax_dim_2, _test_softmax_dim_3, _test_softmax_backward_normal, _test_softmax_backward_1_dim]
        arg_dict['device'] = ['cpu', 'cuda']
        for arg in GenArgList(arg_dict):
            arg[0](test_case, *arg[1:])
...
def _test_hardsigmoid_impl(test_case, shape, device):
    ...
    of_out = of_out.sum()
    of_out.backward()
    test_case.assertTrue(np.allclose(x.grad.numpy(), _np_hardsigmoid_grad(np_out), 1e-05, 1e-05))
...
def _test_logsoftmax_backward(test_case, device):
    ...
    input_arr = np.array([[[[2.0, 1.0, 9.0, 3.0, 4.0], [1.0, 6.0, 7.0, 1.0, 4.0], [4.0, 7.0, 5.0, 8.0, 1.0], [9.0, 5.0, 7.0, 8.0, 5.0]], [[1.0, 1.0, 5.0, 3.0, 5.0], [3.0, 6.0, 3.0, 7.0, 8.0], [8.0, 8.0, 1.0, 2.0, 6.0], [3.0, 5.0, 6.0, 1.0, 1.0]], [[8.0, 3.0, 6.0, 3.0, 7.0], [8.0, 5.0, 1.0, 2.0, 7.0], [3.0, 9.0, 4.0, 6.0, 5.0], [5.0, 1.0, 2.0, 3.0, 6.0]]], [[[3.0, 5.0, 3.0, 1.0, 7.0], [5.0, 2.0, 6.0, 3.0, 5.0], [5.0, 1.0, 8.0, 6.0, 9.0], [9.0, 8.0, 4.0, 5.0, 1.0]], [[7.0, 5.0, 7.0, 1.0, 6.0], [3.0, 3.0, 6.0, 6.0, 7.0], [9.0, 4.0, 1.0, 5.0, 7.0], [7.0, 6.0, 9.0, 8.0, 6.0]], [[6.0, 7.0, 5.0, 3.0, 9.0], [4.0, 1.0, 2.0, 3.0, 2.0], [4.0, 3.0, 8.0, 7.0, 8.0], [1.0, 3.0, 8.0, 6.0, 2.0]]]])
    x = flow.Tensor(input_arr, requires_grad=True, device=flow.device(device), dtype=flow.float64)
    x_grad = np.array([[[[0.46211716, 0.96402758, -0.99505475, -0.76159416, 0.90514825], [0.96402758, -0.96402758, -0.46211716, 0.76159416, 0.46211716], [0.46211716, -0.99505475, 0.90514825, -0.76159416, 0.9993293], [0.0, 0.90514825, -0.90514825, -0.90514825, -0.96402758]], [[0.99505475, 0.96402758, 0.76159416, -0.76159416, 0.46211716], [0.0, -0.90514825, 0.90514825, -0.46211716, -0.46211716], [0.46211716, -0.96402758, 0.0, 0.90514825, 0.46211716], [0.96402758, 0.46211716, 0.90514825, 0.9981779, 0.9866143]], [[-0.76159416, 0.96402758, -0.46211716, 0.0, 0.76159416], [-0.96402758, -0.96402758, 0.46211716, 0.46211716, -0.9866143], [0.46211716, -0.99505475, 0.96402758, 0.46211716, 0.90514825], [-0.96402758, 0.76159416, 0.99505475, 0.90514825, -0.96402758]]], [[[-0.46211716, -0.96402758, 0.99505475, 0.76159416, -0.90514825], [-0.96402758, 0.96402758, 0.46211716, -0.76159416, -0.46211716], [-0.46211716, 0.99505475, -0.90514825, 0.76159416, -0.9993293], [0.0, -0.90514825, 0.90514825, 0.90514825, 0.96402758]], [[-0.99505475, -0.96402758, -0.76159416, 0.76159416, -0.46211716], [0.0, 0.90514825, -0.90514825, 0.46211716, 0.46211716], [-0.46211716, 0.96402758, 0.0, -0.90514825, -0.46211716], [-0.96402758, -0.46211716, -0.90514825, -0.9981779, -0.9866143]], [[0.76159416, -0.96402758, 0.46211716, 0.0, -0.76159416], [0.96402758, 0.96402758, -0.46211716, -0.46211716, 0.9866143], [-0.46211716, 0.99505475, -0.96402758, -0.46211716, -0.90514825], [0.96402758, -0.76159416, -0.99505475, -0.90514825, 0.96402758]]]])
    y = m(x).sum()
    y.backward()
    test_case.assertTrue(np.allclose(x.grad.numpy(), x_grad, 1e-05, 1e-05))
...
class TestLogSoftmax(flow.unittest.TestCase):
    ...
    def test_log_softmax(test_case):
        arg_dict = OrderedDict()
        arg_dict['fun'] = [_test_logsoftmax, _test_logsoftmax_dim_2, _test_logsoftmax_dim_3, _test_logsoftmax_backward]
        arg_dict['device'] = ['cpu', 'cuda']
        for arg in GenArgList(arg_dict):
            arg[0](test_case, *arg[1:])
...
def _test_logsigmoid(test_case, device):
    ...
    of_out = of_out.sum()
    of_out.backward()
    np_grad = [0.2689414213699951, 0.11920292202211764, 0.04742587317756669, 3.716893710287265e-05, 0.0005002011070795276]
    test_case.assertTrue(np.allclose(x.grad.numpy(), np_grad, 1e-05, 1e-05))
...
class TestSoftplusModule(flow.unittest.TestCase):
    ...
    def test_softplus(test_case):
        arg_dict = OrderedDict()
        arg_dict['test_fun'] = [_test_softplus, _test_softplus_beta, _test_softplus_threshold, _test_softplus_backward]
        arg_dict['device'] = ['cpu']
        for arg in GenArgList(arg_dict):
            arg[0](test_case, *arg[1:])
...
def _test_hardtanh_impl(test_case, shape, device):
    ...
    of_out = of_out.sum()
    of_out.backward()
    test_case.assertTrue(np.allclose(x.grad.numpy(), _np_hardtanh_grad(np_out), 1e-05, 1e-05))
