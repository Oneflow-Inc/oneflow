...
class TestConstantPad2dModule(flow.unittest.TestCase):
    ...
    def test_with_random_data(test_case):
        for device in ['cpu', 'cuda']:
            ...
            test_module_against_pytorch(test_case, 'nn.ConstantPad2d', extra_annotations={'padding': int, 'value': float}, extra_generators={'input': random_tensor(ndim=4, dim2=spatial_size, dim3=spatial_size), 'padding': random(0, 3), 'value': random(0, 10)}, device=device)
...
class TestConstantPad3dModule(flow.unittest.TestCase):
    ...
    def test_with_random_data(test_case):
        for device in ['cpu', 'cuda']:
            spatial_size = np.random.randint(1, 6)
            test_module_against_pytorch(test_case, 'nn.ConstantPad3d', extra_annotations={'padding': int, 'value': float}, extra_generators={'input': random_tensor(ndim=5, dim2=spatial_size, dim3=spatial_size, dim4=spatial_size), 'padding': random(0, 3), 'value': random(0, 10)}, device=device)
