...
def _test_mul(test_case, placement, x_sbp, y_sbp):
    x = random_pytorch_tensor(ndim=3, dim0=16, dim1=8, dim2=24).to_consistent(
        placement, x_sbp
    )
    y = random_pytorch_tensor(ndim=2, dim0=8, dim1=24).to_consistent(placement, y_sbp)
    z = torch.mul(x, y)
    return z
        ...
        for placement in all_placement():
            for x_sbp in all_sbp(placement, max_dim=2):
                for y_sbp in all_sbp(placement, max_dim=2):
                    _test_mul(test_case, placement, x_sbp, y_sbp)
