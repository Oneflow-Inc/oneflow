...
def _test_var(test_case, ndim, placement, sbp):
    ...
    z = x.var(dimension_, True)
    return z


class TestVarConsistent(flow.unittest.TestCase):
    @consistent
    def test_var(test_case):
        ndim = np.random.randint(1, 6)
        for placement in all_placement():
            for sbp in all_sbp(placement, max_dim=ndim):
                _test_var(test_case, ndim, placement, sbp)
