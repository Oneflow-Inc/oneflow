...
def _np_bcewithlogitsloss(np_input, np_target, np_weight=None, np_pos_weight=None, reduction='none'):
    ...
    if np_pos_weight is not None:
        ...
        _loss = (1 - np_target) * np_input + _log_weight * (np.log(np.exp(_neg_max_val) + np.exp(_neg_input - _max_val)) + _max_val)
    else:
        _loss = (1 - np_target) * np_input + _max_val
        _loss += np.log(np.exp(_neg_max_val) + np.exp(_neg_input - _max_val))
    if np_weight is not None:
        assert np_weight.shape == np_input.shape, 'The weight shape must be the same as Input shape'
...
def _np_bcewithlogitsloss_grad(np_input, np_target, np_weight, np_pos_weight):
    ...
    return {'mean': np_bce_with_logits_grad_mean, 'sum': np_bce_with_logits_grad_sum, 'none': np_bce_with_logits_grad_sum}
...
class TestBCEWithLogitsLossModule(flow.unittest.TestCase):
    ...
    def test_bcewithlogitsloss(test_case):
        ...
        arg_dict['shape'] = [(3, 5), (10, 9, 21), (14, 22, 9, 21), (3, 2, 4, 16, 5), (1,)]
