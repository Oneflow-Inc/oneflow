...
def _test_linear_no_bias(test_case, device):
    ...
    input_arr = np.array([[-0.94630778, -0.83378579, -0.87060891], [2.0289922, -0.28708987, -2.18369248], [0.35217619, -0.67095644, -1.58943879], [0.08086036, -1.81075924, 1.20752494], [0.8901075, -0.49976737, -1.07153746], [-0.44872912, -1.07275683, 0.06256855], [-0.22556897, 0.74798368, 0.90416439], [0.48339456, -2.32742195, -0.59321527]], dtype=np.float32)
...
def _test_linear_backward_with_bias(test_case, device):
    linear = flow.nn.Linear(3, 8)
    linear = linear.to(device)
    x = flow.Tensor([[-0.94630778, -0.83378579, -0.87060891], [2.0289922, -0.28708987, -2.18369248], [0.35217619, -0.67095644, -1.58943879], [0.08086036, -1.81075924, 1.20752494], [0.8901075, -0.49976737, -1.07153746], [-0.44872912, -1.07275683, 0.06256855], [-0.22556897, 0.74798368, 0.90416439], [0.48339456, -2.32742195, -0.59321527]], device=flow.device(device), requires_grad=True)
    flow.nn.init.constant_(linear.weight, 2.068758)
    flow.nn.init.constant_(linear.bias, 0.23)
    of_out = linear(x)
    of_out = of_out.sum()
    of_out.backward()
    np_grad = np.array([[16.5501, 16.5501, 16.5501], [16.5501, 16.5501, 16.5501], [16.5501, 16.5501, 16.5501], [16.5501, 16.5501, 16.5501], [16.5501, 16.5501, 16.5501], [16.5501, 16.5501, 16.5501], [16.5501, 16.5501, 16.5501], [16.5501, 16.5501, 16.5501]])
...
class TestLinear(flow.unittest.TestCase):
    ...
    def test_linear_forward(test_case):
        ...
        arg_dict['test_fun'] = [_test_linear_no_bias, _test_linear_with_bias, _test_linear_3_dimension_input, _test_linear_4_dimension_input, _test_identity]
    ...
    def test_identity_with_random_data(test_case):
        m = torch.nn.Identity(x=random().to(int), unused_argument1=random().to(float), unused_argument2=random().to(float))
        m.train(random())
        device = random_device()
        m.to(device)
