...
def compare_with_numpy_rmsprop(test_case, device, x_shape, scale, learning_rate, momentum, train_iters, alpha, eps, weight_decay, centered):
    random_grad_seq = []
    ...
    def train_by_oneflow():
        ...
        param_list = list()
        param_list.append(x)
        rmsprop = flow.optim.RMSprop([{'params': param_list, 'lr': learning_rate, 'alpha': alpha, 'eps': eps, 'weight_decay': weight_decay, 'momentum': momentum, 'centered': centered, 'scale': scale}])

        def train_one_iter(grad):
            grad_tensor = flow.Tensor(grad, requires_grad=False, device=flow.device(device))
    ...
    test_case.assertTrue(np.allclose(oneflow_res.flatten(), numpy_res.flatten(), rtol=0.0001, atol=0.0001))
