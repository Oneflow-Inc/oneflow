...
def _test_marginrankingloss_none(test_case, shape, margin, device):
    input1 = flow.Tensor(np.random.randn(*shape), dtype=flow.float32, device=flow.device(device))
    input2 = flow.Tensor(np.random.randn(*shape), dtype=flow.float32, device=flow.device(device))
    target_pos = flow.Tensor(np.ones(shape), dtype=flow.float32, device=flow.device(device))
    target_neg = flow.Tensor(-1 * np.ones(shape), dtype=flow.float32, device=flow.device(device))
    ...
    np_out_pos = np_margin_ranking_loss(margin, input1.numpy(), input2.numpy(), target_pos.numpy(), reduction='none')
    ...
    np_out_neg = np_margin_ranking_loss(margin, input1.numpy(), input2.numpy(), target_neg.numpy(), reduction='none')
...
def _test_marginrankingloss_mean(test_case, shape, margin, device):
    ...
    margin_ranking_loss = margin_ranking_loss.to(device)
    of_out_pos = margin_ranking_loss(input1, input2, target_pos)
    np_out_pos = np_margin_ranking_loss(margin, input1.numpy(), input2.numpy(), target_pos.numpy(), reduction='mean')
    test_case.assertTrue(np.allclose(of_out_pos.numpy(), np_out_pos, 1e-05, 1e-05))
    of_out_neg = margin_ranking_loss(input1, input2, target_neg)
    np_out_neg = np_margin_ranking_loss(margin, input1.numpy(), input2.numpy(), target_neg.numpy(), reduction='mean')
    test_case.assertTrue(np.allclose(of_out_neg.numpy(), np_out_neg, 1e-05, 1e-05))
...
def _test_marginrankingloss_sum(test_case, shape, margin, device):
    ...
    margin_ranking_loss = margin_ranking_loss.to(device)
    of_out_pos = margin_ranking_loss(input1, input2, target_pos)
    np_out_pos = np_margin_ranking_loss(margin, input1.numpy(), input2.numpy(), target_pos.numpy(), reduction='sum')
    test_case.assertTrue(np.allclose(of_out_pos.numpy(), np_out_pos, 1e-05, 1e-05))
    of_out_neg = margin_ranking_loss(input1, input2, target_neg)
    np_out_neg = np_margin_ranking_loss(margin, input1.numpy(), input2.numpy(), target_neg.numpy(), reduction='sum')
    test_case.assertTrue(np.allclose(of_out_neg.numpy(), np_out_neg, 1e-05, 1e-05))

def _test_marginrankingloss_grad(test_case, shape, margin, device):
    input1 = flow.Tensor(np.random.randn(*shape), dtype=flow.float32, device=flow.device(device), requires_grad=True)
    input2 = flow.Tensor(np.random.randn(*shape), dtype=flow.float32, device=flow.device(device), requires_grad=True)
    ...
    of_out.backward()
    (np_out_grad1, np_out_grad2) = np_margin_ranking_loss_grad(margin, input1.numpy(), input2.numpy(), target.numpy())
...
class TestMarginRankingLossModule(flow.unittest.TestCase):
    ...
    def test_margin_ranking_loss(test_case):
        ...
        arg_dict['test_fun'] = [_test_marginrankingloss_none, _test_marginrankingloss_mean, _test_marginrankingloss_sum, _test_marginrankingloss_grad]
