...
def _test_tensor_buffer_convert(test_case, device):
    input = flow.Tensor(np.random.rand(16, 24, 32, 36), dtype=flow.float32, device=flow.device(device))
    tensor_buffer = flow.tensor_to_tensor_buffer(input, instance_dims=2)
    orig_tensor = flow.tensor_buffer_to_tensor(tensor_buffer, dtype=flow.float32, instance_shape=[32, 36])
