...
def _test_normal(test_case, mean, std, shape, device, dtype):
    ...
    def helper(self, device, dtype, ptype, t_transform, std_transform):
        ...
        self.assertTrue(np.allclose(t_transform(q).std().item(), std_transform(1), atol=0.2, rtol=0))

        q.normal_(2, 3)
        self.assertTrue(np.allclose(t_transform(q).mean().item(), 2, atol=0.3, rtol=0))
        self.assertTrue(np.allclose(t_transform(q).std().item(), std_transform(3), atol=0.3, rtol=0))

        q = flow.empty(100, 100, dtype=dtype, device=device)
        q_row1 = q[0:1].clone()
        q[99:100].normal_()
        self.assertTrue(np.allclose(t_transform(q[99:100]).mean().item(), 0, atol=0.3, rtol=0))
        self.assertTrue(np.allclose(t_transform(q[99:100]).std().item(), std_transform(1), atol=0.3, rtol=0))
        ...
        self.assertTrue(np.allclose(t_transform(r[:50]).mean().item(), 0, atol=0.2, rtol=0))
        self.assertTrue(np.allclose(t_transform(r[50:]).mean(), 1, atol=0.2, rtol=0))
        self.assertTrue(np.allclose(t_transform(r).std(), std_transform(1), atol=0.2, rtol=0))
        ...
        self.assertTrue(np.allclose(t_transform(r[50:]).mean(), 1, atol=0.2, rtol=0))
        self.assertTrue(np.allclose(t_transform(r).std(), std_transform(3), atol=0.2, rtol=0))
        ...
        self.assertEqual(r.dtype, dtype)
        self.assertEqual(str(r.device), str(device))
        self.assertTrue(np.allclose(t_transform(r[:50]).mean().numpy(), 0, atol=0.2, rtol=0))
        self.assertTrue(np.allclose(t_transform(r[50:]).mean().numpy(), 1, atol=0.2, rtol=0))
        self.assertTrue(np.allclose(t_transform(r[:, :50]).std().numpy(), std_transform(4), atol=0.3, rtol=0))
        self.assertTrue(np.allclose(t_transform(r[:, 50:]).std().numpy(), std_transform(1), atol=0.2, rtol=0))
        ...
        self.assertTrue(np.allclose(t_transform(r).std().numpy(), std_transform(3), atol=0.3, rtol=0))
        ...
        r.fill_(42)
        # flow.normal(2, 0, (10, 10), dtype=dtype, device=device, out=r)
