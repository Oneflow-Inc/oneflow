...
def _test_fftn(test_case, params: dict, dtype=np.complex64):
    ...
    # forward
    y_torch = torch.fft.fftn(x_torch, 
                            s=n, 
                            dim=dims, 
                            norm=norm)
...
def _test_ifftn(test_case, params: dict, dtype=np.complex64):
    ...
    # forward
    y_torch = torch.fft.ifftn(x_torch, 
                            s=n, 
                            dim=dims, 
                            norm=norm)
    y_torch_sum = y_torch.sum()
    ...
    def test_gather(test_case):
        ...
        for _ in range(10):
            ...
            if np.random.randint(2) == 1:
                ...
                dims = np.random.choice(total_dims_range, size=num_dims, replace=False).tolist()
            else:
                dims = None
            ...
            if np.random.randint(2) == 1 and dims is not None:
                ...
                for i in range(num_dims):
                    n_ = np.random.randint(low=1, high=shape[i]) if np.random.randint(2) == 1 else -1
                    n.append(n_)
            else:
                n = None
