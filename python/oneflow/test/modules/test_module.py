...
class TestModule(flow.unittest.TestCase):
    ...
    def test_load_map_location(test_case):
        ...
        with tempfile.TemporaryDirectory() as save_dir:
            ...
            loaded = flow.load(save_dir, global_src_rank=0, map_location=flow.placement('cuda', [0]))
        assert np.array_equal(loaded['x'].numpy(), x.numpy())
        ...
        with tempfile.TemporaryDirectory() as save_dir:
            flow.save({'x': x, 'y': y}, save_dir, global_dst_rank=0)
            loaded = flow.load(save_dir, global_src_rank=0, map_location=flow.placement('cpu', [0]))
        assert np.array_equal(loaded['x'].numpy(), x.numpy())
