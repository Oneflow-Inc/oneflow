...
def _test_randperm_with_generator(test_case, placement, sbp, N, device, dtype):
    ...
    y_1 = y_1.to_global(placement=placement, sbp=sbp)
    y_2 = y_2.to_global(placement=placement, sbp=sbp)
...
def _test_randperm_backward(test_case, placement, sbp, N, device, dtype):
    ...
    x = flow.randperm(N, device=device, dtype=dtype).to_global(placement=placement, sbp=sbp)
