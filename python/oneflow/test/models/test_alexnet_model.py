...
class AlexNet(flow.model.Model):
    ...
    def forward(self, images, trainable=False):
        conv1 = _conv2d_layer('conv1', images, filters=64, kernel_size=11, strides=4, padding='VALID')
        ...
        fc1 = flow.layers.dense(inputs=pool5, units=4096, activation=flow.math.relu, use_bias=False, kernel_initializer=_get_initializer(), bias_initializer=False, trainable=trainable, name='fc1')
        dropout1 = fc1
        fc2 = flow.layers.dense(inputs=dropout1, units=4096, activation=flow.math.relu, use_bias=False, kernel_initializer=_get_initializer(), bias_initializer=False, trainable=trainable, name='fc2')
        dropout2 = fc2
        fc3 = flow.layers.dense(inputs=dropout2, units=1001, activation=None, use_bias=False, kernel_initializer=_get_initializer(), bias_initializer=False, trainable=trainable, name='fc3')
        return fc3
    ...
    def configure_optimizers(self):
        return flow.optimizer.SGD(flow.optimizer.PiecewiseConstantScheduler([], [1e-05]), momentum=0)
...
def test_1n1c(test_case):
    ...
    alexnet_md.fit(training_config=train_config, validation_config=val_config, checkpoint_config=ck_config, callbacks=[loss_monitor_cb], max_steps=20)

def _conv2d_layer(name, input, filters, kernel_size=3, strides=1, padding='SAME', data_format='NCHW', dilation_rate=1, activation=op_conf_util.kRelu, use_bias=False, weight_initializer=flow.random_uniform_initializer(), bias_initializer=flow.random_uniform_initializer()):
...
def _data_load_layer(args, data_dir):
    ...
    (image, label) = flow.data.ofrecord_image_classification_reader(data_dir, batch_size=total_batch_size, data_part_num=args.data_part_num, image_feature_name='encoded', label_feature_name='class/label', color_space='RGB', name='decode')
