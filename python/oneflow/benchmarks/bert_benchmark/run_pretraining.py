...
parser.add_argument('--skip_iter_num', type=int, default=10, help='number of skipping iterations for benchmark purpose.')
...
parser.add_argument('--loss_print_every_n_iter', type=int, default=1, required=False, help='print loss every n iteration')
parser.add_argument('--model_save_every_n_iter', type=int, default=200, required=False, help='save model every n iteration')
parser.add_argument('--model_save_dir', type=str, default='./output/model_save-{}'.format(str(datetime.now().strftime('%Y-%m-%d-%H:%M:%S'))), required=False, help='model save directory')
parser.add_argument('--save_last_snapshot', type=bool, default=False, required=False, help='save model snapshot for last iteration')
...
parser.add_argument('--lr_decay_num_same_as_iter_num', default=False, type=lambda x: str(x).lower() == 'true')
...
def BertDecoder(data_dir, batch_size, data_part_num, seq_length, max_predictions_per_seq):
    ...
    ofrecord = flow.data.ofrecord_reader(data_dir, batch_size=batch_size, data_part_num=data_part_num, name='decode')
    ret = {}
    for (k, v) in config_ordered_dict.items():
        ret[k] = flow.data.ofrecord_raw_decoder(ofrecord, k, shape=(v,), dtype=flow.float if k == 'masked_lm_weights' else flow.int32)
    return ret

def BuildPreTrainNet(batch_size, data_part_num, seq_length=128, max_position_embeddings=512, num_hidden_layers=12, num_attention_heads=12, hidden_dropout_prob=0.1, attention_probs_dropout_prob=0.1, vocab_size=30522, type_vocab_size=2, max_predictions_per_seq=20):
    hidden_size = 64 * num_attention_heads
    intermediate_size = hidden_size * 4
    decoders = BertDecoder(args.data_dir, batch_size, data_part_num, seq_length, max_predictions_per_seq)
    ...
    return PreTrain(input_ids, input_mask, token_type_ids, masked_lm_positions, masked_lm_ids, masked_lm_weights, next_sentence_labels, vocab_size, seq_length=seq_length, hidden_size=hidden_size, num_hidden_layers=num_hidden_layers, num_attention_heads=num_attention_heads, intermediate_size=intermediate_size, hidden_act='gelu', hidden_dropout_prob=hidden_dropout_prob, attention_probs_dropout_prob=attention_probs_dropout_prob, max_position_embeddings=max_position_embeddings, type_vocab_size=type_vocab_size, max_predictions_per_seq=max_predictions_per_seq, initializer_range=0.02)
_BERT_MODEL_UPDATE_CONF = dict(learning_rate_decay=dict(polynomial_conf=dict(decay_batches=args.iter_num if args.lr_decay_num_same_as_iter_num else args.lr_decay_num, end_learning_rate=0.0)), warmup_conf=dict(linear_conf=dict(warmup_batches=args.warmup_batches, start_multiplier=0)), clip_conf=dict(clip_by_global_norm=dict(clip_norm=1.0)), adam_conf=dict(epsilon=1e-06), weight_decay_conf=dict(weight_decay_rate=args.weight_decay_rate, excludes=dict(pattern=['bias', 'LayerNorm', 'layer_norm'])))
...
def PretrainJob():
    ...
    (total_loss, mlm_loss, nsp_loss) = BuildPreTrainNet(batch_size, args.data_part_num, seq_length=args.seq_length, max_position_embeddings=args.max_position_embeddings, num_hidden_layers=args.num_hidden_layers, num_attention_heads=args.num_attention_heads, hidden_dropout_prob=args.hidden_dropout_prob, attention_probs_dropout_prob=args.attention_probs_dropout_prob, vocab_size=args.vocab_size, type_vocab_size=args.type_vocab_size, max_predictions_per_seq=args.max_predictions_per_seq)
    flow.losses.add_loss(total_loss)
    return (total_loss, mlm_loss, nsp_loss)

def main():
    print('='.ljust(66, '='))
    print('Running bert: num_gpu_per_node = {}, num_nodes = {}.'.format(args.gpu_num_per_node, args.node_num))
    print('='.ljust(66, '='))
    ...
    for step in range(args.skip_iter_num + args.iter_num):
        cb = speedometer.speedometer_cb(step, start_time, total_batch_size, args.skip_iter_num, args.iter_num, args.loss_print_every_n_iter)
