...
class FunctionConfig(object):
    ...
    def __getattr__(self, attr_name: str) -> Callable[[Optional[Union[bool, int, float, str]]], None]:
        name2default = session_ctx.GetDefaultSession().function_flag_name2default_val
        assert attr_name in name2default
        flag_name2flag_value = self.function_desc.job_config_proto.mutable_flag_name2flag_value()
        default_val = name2default[attr_name]

        def FunctionConfigSetter(attr_value: Optional[Union[bool, int, float, str]]=None) -> None:
            if default_val.HasField('at_bool'):
                ...
                raise NotImplementedError("config_flag `%s' with type %s is not supported" % (attr_name, type(attr_value)))
        return FunctionConfigSetter

def api_oneflow_function(type: str='predict', function_config: FunctionConfig=None) -> Callable[[Callable], Callable]:
    ...
    if isinstance(type, FunctionConfig):
        ...
        print('WARNING: flow.global_function(func_config) is deprecated. Please replace it with flow.global_function(type, func_config).\n            ')
...
def set_default_data_type(func_desc, value):
    ...
    """
    func_desc.job_config_proto.set_default_data_type(data_type_cfg.DataType(oneflow._oneflow_internal.deprecated.GetProtoDtype4OfDtype(value)))
...
def set_default_initializer_conf(func_desc, value):
    ...
    pb_util.PythonDict2CFG(value, func_desc.job_config_proto.mutable_default_initializer_conf())
...
def static_mem_alloc_policy_white_list_has_policy(func_desc, policy):
    ...
    Returns:
        [type]: [description]
    """
    return getattr(func_desc.job_config_proto.mutable_memory_allocation_algorithm_conf(), policy)()
...
def static_mem_alloc_policy_white_list_add_policy(func_desc, policy):
    ...
    Args:
        ...
        policy ([type]): [description]
    """
    getattr(func_desc.job_config_proto.mutable_memory_allocation_algorithm_conf(), 'set_' + policy)(True)
...
def static_mem_alloc_policy_white_list_remove_policy(func_desc, policy):
    ...
    Args:
        ...
        policy ([type]): [description]
    """
    getattr(func_desc.job_config_proto.mutable_memory_allocation_algorithm_conf(), 'set_' + policy)(False)
...
def set_enable_inplace_in_reduce_struct(func_desc, value=True):
    print("'enable_inplace_in_reduce_struct' has been deprecated, has no effect and will be removed in the future.")

@oneflow_function_config('enable_nccl')
def set_enable_nccl(func_desc, value=True):
    print("'enable_nccl' has been deprecated, has no effect and will be removed in the future.")

@oneflow_function_config('use_nccl_inter_node_communication')
def set_use_nccl_inter_node_communication(func_desc, value=True):
    print("'use_nccl_inter_node_communication' has been deprecated, has no effect and will be removed in the future.")

@oneflow_function_config('use_boxing_v2')
def set_use_boxing_v2(func_desc, value=True):
    print("'use_boxing_v2' has been deprecated, has no effect and will be removed in the future.")
...
def set_enable_all_reduce_group(func_desc, value=True):
    print("'enable_all_reduce_group' has been deprecated, has no effect and will be removed in the future.")

@oneflow_function_config('all_reduce_group_num')
def set_all_reduce_group_num(func_desc, value):
    print("'all_reduce_group_num' has been deprecated, has no effect and will be removed in the future.")

@oneflow_function_config('all_reduce_lazy_ratio')
def set_all_reduce_lazy_ratio(func_desc, value):
    print("'all_reduce_lazy_ratio' has been deprecated, has no effect and will be removed in the future.")

@oneflow_function_config('all_reduce_group_min_mbyte')
def set_all_reduce_group_min_mbyte(func_desc, value):
    print("'all_reduce_group_min_mbyte' has been deprecated, has no effect and will be removed in the future.")

@oneflow_function_config('all_reduce_group_size_warmup')
def set_all_reduce_group_size_warmup(func_desc, value):
    print("'all_reduce_group_size_warmup' has been deprecated, has no effect and will be removed in the future.")

@oneflow_function_config('all_reduce_fp16')
def set_all_reduce_fp16(func_desc, value=True):
    print("'all_reduce_fp16' has been deprecated, has no effect and will be removed in the future.")

@oneflow_function_config('optimizer_placement_optimization_mode', 'train.optimizer_placement_optimization_mode')
...
@oneflow_function_config('optimizer_placement_optimization_threshold', 'train.optimizer_placement_optimization_threshold')
...
def set_disable_all_reduce_sequence(func_desc, value=True):
    print("'disable_all_reduce_sequence' has been deprecated, has no effect and will be removed in the future.")
...
def set_non_distributed_optimizer_group_size_mbyte(func_desc, value):
    print("'non_distributed_optimizer_group_size_mbyte' has been deprecated, has no effect and will be removed in the future.")
...
def set_enable_float_compute_for_half_gemm(func_desc, value=True):
    ...
    Args:
        ...
        value (bool, optional): [description]. Defaults to True.
    """
    print('WARNING: enable_float_compute_for_half_gemm has been deprecated, because we always use float compute for half gemm. Please remove it.\n        ')
...
def set_qat_per_channel(func_desc, value=True):
    func_desc.job_config_proto.mutable_qat_config().set_per_channel_weight_quantization(value)
...
def set_qat_moving_min_max_momentum(func_desc, value: float):
    func_desc.job_config_proto.mutable_qat_config().set_moving_min_max_stop_update_after_iters(value)
...
def set_model_update_conf(func_desc, value):
    ...
    Args:
        ...
        value ([type]): [description]
    """
    print('WARNING: func_config.train.* has been deprecated. Please replace it by the new optimizer api.\n        ')
    print(traceback.format_stack()[-3])
    assert type(value) is dict
...
def set_tensorrt_use_fp16(func_desc, value=True):
    ...
    func_desc.job_config_proto.mutable_xrt_config().mutable_tensorrt_config().set_use_fp16(value)
...
def set_tensorrt_use_int8(func_desc, value=True):
    ...
    """
    set_use_tensorrt(func_desc, True)
    func_desc.job_config_proto.mutable_xrt_config().mutable_tensorrt_config().set_use_int8(value)
...
def set_tensorrt_int8_calibration(func_desc, value):
    ...
    func_desc.job_config_proto.mutable_xrt_config().mutable_tensorrt_config().set_int8_calibration(value)
...
def deprecated_set_default_distribute_strategy(*args, **kwargs):
    print('WARNING:', 'function_config.default_distribute_strategy', 'has been deprecated. Please use {} instead.'.format('function_config.default_logical_view'))
