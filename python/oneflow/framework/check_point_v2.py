...
class FileBackendVariableBlob:
    ...
    def __init__(self, var_dir: str, dtype: Optional[oneflow.dtype]=None, shape: Optional[Sequence[int]]=None):
...
def _GetCpu0VariableBlobFromNumpy(np_array: np.ndarray, dtype: oneflow.dtype) -> oneflow._oneflow_internal.EagerConsistentBlob:
    """
...
def FeedValueToVariable(var_blob: Union[oneflow._oneflow_internal.EagerConsistentBlob, 'oneflow.Tensor'], value: ValueContainer, scope_symbol_id: Optional[int]) -> None:
    """
    Feed the value of `value` to the variable `var_blob`
    """
    assert isinstance(value, (EagerBlobTrait, FileBackendVariableBlob, np.ndarray, oneflow.Tensor)), 'Unknown value type: {}'.format(type(value).__name__)
...
def _ForEachSlice(container: ValueContainer, f: Union[Callable[[EagerBlobTrait, Sequence[int], Sequence[int]], Any], Callable[[FileBackendVariableBlob, Sequence[int], Sequence[int]], Any], Callable[[np.ndarray, Sequence[int], Sequence[int]], Any]]):
    ...
    """
    assert isinstance(container, (EagerBlobTrait, FileBackendVariableBlob, np.ndarray, oneflow.Tensor)), 'Unknown type: {}'.format(type(container).__name__)
...
def init_by_initializer_conf(var_blob: Union[EagerBlobTrait, 'oneflow.Tensor'], initializer_conf: initializer_conf_util.InitializerConf, sync_between_multi_machine: bool, scope_symbol_id: Optional[int], random_seed: int=0):
