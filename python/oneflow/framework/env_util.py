...
def HasAllMultiClientEnvVars():
    ...
    if len(env_var_with_value) != 0 and len(env_var_without_value) != 0:
        warnings.warn(f"Among four environment variables required for distributed training, only {', '.join('`{0}`'.format(x) for x in env_var_with_value)} are set, but {', '.join('`{0}`'.format(x) for x in env_var_without_value)} are not set.")
