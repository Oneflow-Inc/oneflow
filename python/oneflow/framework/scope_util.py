...
def api_scope_config(**kwargs):
    ...
    def SetScopeProto(scope_proto):
        for (attr_name, py_value) in kwargs.items():
            ...
            attr_util.SetAttrValue(scope_proto.mutable_attr_name2attr_value()[attr_name], py_value, name2default[attr_name])
    sess = session_ctx.GetDefaultSession()
    scope = MakeScope(lambda old_scope, builder: builder.BuildScopeByProtoSetter(old_scope, SetScopeProto))
...
def deprecated_current_scope(*args, **kwargs):
    print('WARNING:', 'oneflow.scope.current_scope', 'will be removed in the future, use {} instead.'.format('oneflow.current_scope'))
...
def MakeInitialScope(job_conf, device_tag, machine_device_ids, hierarchy, is_mirrored):
    ...
    def BuildInitialScope(builder):
        ...
        scope = builder.BuildInitialScope(session_id, job_conf, device_tag, machine_device_ids, hierarchy, is_mirrored)
