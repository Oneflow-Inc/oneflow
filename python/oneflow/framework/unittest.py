...
def register_test_cases(scope: Dict[str, Any], directory: str, filter_by_num_nodes: Callable[[bool], int], base_class: unittest.TestCase=unittest.TestCase, test_case_mixin=_ClearDefaultSession) -> None:

    def FilterTestPyFile(f):
        return os.path.isfile(os.path.join(directory, f)) and f.endswith('.py') and f.startswith('test')

    def FilterMethodName(module, name):
        method = getattr(module, name)
        return name.startswith('test') and callable(method) and filter_by_num_nodes(_GetNumOfNodes(method))
    ...
    for f in onlytest_files:
        ...
        test_func_names = [name for name in dir(module) if FilterMethodName(module, name)]
...
def has_world_size():
    ...
    if os.getenv('ONEFLOW_TEST_WORLD_SIZE'):
        assert os.getenv('ONEFLOW_TEST_WORLD_SIZE').isdigit(), 'env var ONEFLOW_TEST_WORLD_SIZE must be num'
        return True
    else:
        return False
...
class TestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        if has_node_list():
                ...
                if enable_init_by_host_list():
                    ...
                    for machine in env_util.default_env_proto.machine:
                        if machine.id == 0:
                            ...
                            launch_worker_via_agent(host=machine.addr, env_proto=env_util.default_env_proto)
                else:
                    ...
                    bootstrap_conf_list = oneflow.env.init_bootstrap_confs(node_list(), int(master_port), config_world_size, config_rank_ctrl_port, config_node_size)
                    ...
                    for bootstrap_conf in bootstrap_conf_list:
                        ...
                        launch_worker_via_agent(host=bootstrap_conf.host, env_proto=worker_env_proto)
            ...
            bootstrap_conf_list = oneflow.env.init_bootstrap_confs(['127.0.0.1'], master_port, config_world_size)
            env_proto = env_util.default_env_proto
            assert len(env_proto.machine) == 1 and env_proto.HasField('ctrl_bootstrap_conf') == 1
            ...
            for rank in range(1, config_world_size):
                ...
                worker_env_proto.cpp_logging_conf.log_dir = run_dir + '/log_' + str(rank)
                ...
                os.system('cp ' + env_file.name + ' ' + run_dir + '/log_' + str(rank) + '/env_proto_' + str(rank) + '.proto')
                oneflow_cmd = 'python3 -m oneflow --start_worker' + ' --env_proto=' + run_dir + '/log_' + str(rank) + '/' + 'env_proto_' + str(rank) + '.proto'
                subprocess.Popen(oneflow_cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True)
                os.remove(env_file.name)
            atexit.register(oneflow.deprecated.delete_worker_of_multi_process, run_dir=run_dir)
...
def skip_unless(n, d):
    if node_size() == n and device_num() == d:
        ...
        return unittest.skip('only runs when node_size is {} and device_num is {}'.format(n, d))
