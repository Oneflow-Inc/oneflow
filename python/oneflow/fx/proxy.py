...
class TracerBase:
    ...
    def _find_user_frame(self):
        ...
        while frame:
            frame = frame.f_back
            if frame and all(
                not frame.f_code.co_filename.endswith(file) for file in fx_files
            ):
                break
...
class Proxy:
    """
    ``Proxy`` objects are ``Node`` wrappers that oneflow through the
    ...
    def __iter__(self) -> Iterable["Proxy"]:
        ...
        inst = list(dis.get_instructions(calling_frame.f_code))[
            calling_frame.f_lasti // 2
        ]
    ...
    def __flow_function__(self, orig_method, types, args=None, kwargs=None):
