...
class _MinimizerBase:
    ...
    def run_a(self, mod: oneflow.fx.GraphModule, inputs: Tensors) -> TensorOrTensors:
        """
    ...
    def run_b(self, mod: oneflow.fx.GraphModule, inputs: Tensors) -> TensorOrTensors:
        """
        Run `mod` with `inputs` and generate output. The output will be compared with
        output of run_a().
        """
    ...
    def _store_outputs(
        ...
        submodule: oneflow.fx.GraphModule,
    ):
        """
        ...
        # Only one output
        if isinstance(output_node.args[0], oneflow.fx.Node):
    ...
    def _get_submod_inputs(
        self, main_module: oneflow.fx.GraphModule, submod_path: str
        ...
        if set(placeholders) <= self.a_outputs.keys():
            ...
            def get_inputs(self: oneflow.nn.Module, inputs: Any):
                nonlocal a_input
                a_input = inputs
    ...
    def _build_submodule(self, nodes: NodeSet) -> Tuple[oneflow.fx.GraphModule, str]:
        """
        ...
        Returns:
            split_module (oneflow.fx.GraphModule): the module after split.
