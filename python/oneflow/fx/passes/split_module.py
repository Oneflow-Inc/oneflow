...
class Partition:
    def __init__(self, name: str):
        ...
        self.graph: oneflow.fx.graph.Graph = oneflow.fx.graph.Graph()
        self.environment: Dict[oneflow.fx.node.Node, oneflow.fx.node.Node] = {}
...
def split_module(
    ...
    split_callback: Callable[[oneflow.fx.node.Node], int],
):
    partitions: Dict[str, Partition] = {}
    orig_nodes: Dict[str, oneflow.fx.node.Node] = {}

    def record_cross_partition_use(
        def_node: oneflow.fx.node.Node, use_node: Optional[oneflow.fx.node.Node]
    ):  # noqa: B950
    ...
    base_mod_env: Dict[str, oneflow.fx.node.Node] = {}
    base_mod_graph: oneflow.fx.graph.Graph = oneflow.fx.graph.Graph()
    base_mod_attrs: Dict[str, oneflow.fx.graph_module.GraphModule] = {}
    for node in m.graph.nodes:
    ...
    for partition_name in sorted_partitions:
        ...
        base_mod_attrs[submod_name] = oneflow.fx.graph_module.GraphModule(
        ...
        output_val = base_mod_graph.call_module(
            submod_name, tuple(base_mod_env[name] for name in partition.inputs)
        )
        if len(partition.outputs) > 1:
            # Unpack multiple return values from submodule
            output_val_proxy = oneflow.fx.proxy.Proxy(output_val)
    ...
    for node in m.graph.nodes:
        if node.op == "output":
            base_mod_graph.output(
                oneflow.fx.graph.map_arg(node.args[0], lambda n: base_mod_env[n.name])
            )  # noqa: B950

    return oneflow.fx.graph_module.GraphModule(base_mod_attrs, base_mod_graph)
