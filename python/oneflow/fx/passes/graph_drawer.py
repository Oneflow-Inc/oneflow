...
from .tools_common import typename
...
class FxGraphDrawer:
    ...
    def _get_node_style(self, node: flow.fx.Node) -> Dict[str, str]:
        template = {
            "shape": "record",
            "fillcolor": "#CAFFE3",
            "style": '"filled,rounded"',
    ...
    def _get_leaf_node(
        self, module: flow.nn.Module, node: flow.fx.Node
    ) -> flow.nn.Module:
        py_obj = module
    ...
    def _typename(self, target: Any) -> str:
        if isinstance(target, flow.nn.Module):
    ...
    def _get_node_label(self, module: flow.fx.GraphModule, node: flow.fx.Node) -> str:
    ...
    def _to_dot(
        ...
        for node in graph_module.graph.nodes:
            ...
            if node.op == "call_module":
                ...
                if not isinstance(leaf_module, flow.fx.GraphModule):
