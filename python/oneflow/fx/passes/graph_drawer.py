...
class FxGraphDrawer:
    """
    Visualize a oneflow.fx.Graph with graphviz
    Basic usage:
    ...
    def __init__(
        self,
        graph_module: oneflow.fx.GraphModule,
        name: str,
        ...
        for node in graph_module.graph.nodes:
            ...
            if not isinstance(leaf_node, oneflow.fx.GraphModule):
                continue
    ...
    def _get_node_style(self, node: oneflow.fx.Node) -> Dict[str, str]:
        template = {
            "shape": "record",
            "fillcolor": "#CAFFE3",
            "style": '"filled,rounded"',
    ...
    def _get_leaf_node(
        self, module: oneflow.nn.Module, node: oneflow.fx.Node
    ) -> oneflow.nn.Module:
        py_obj = module
    ...
    def _typename(self, target: Any) -> str:
        if isinstance(target, oneflow.nn.Module):
            return oneflow.typename(target)
    ...
    def _to_dot(
        self, graph_module: oneflow.fx.GraphModule, name: str, ignore_getattr: bool
    ) -> pydot.Dot:
        """
        ...
        for node in graph_module.graph.nodes:
            ...
            if node.op == "call_module":
                ...
                if not isinstance(leaf_module, oneflow.fx.GraphModule):
