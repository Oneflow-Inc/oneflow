...
class OperatorSupport:
    ...
    def is_node_supported(
        self, submodules: Dict[str, oneflow.nn.Module], node: oneflow.fx.Node
    ) -> bool:
        """
        Args:
        ...
        for i, dtypes in enumerate(args_dtypes):
            ...
            if not isinstance(node.args[i], oneflow.fx.Node):
                continue
        ...
        for k, dtypes in kwargs_dtypes.items():
            ...
            # If arg is not a node then we don't check it
            if not isinstance(node.kwargs[k], oneflow.fx.Node):
                continue
