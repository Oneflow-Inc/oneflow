...
Tensors = Union[Tuple[oneflow.Tensor], List[oneflow.Tensor]]
TensorOrTensors = Union[oneflow.Tensor, Tensors]
NodeList = List[oneflow.fx.Node]
NodeSet = Set[oneflow.fx.Node]
Names = List[str]
...
def get_node_target(
    ...
    "oneflow". e.g. _VariableFunctionsClass.relu would become oneflow.relu.
    """
    ...
    if node.op == "call_module":
        ...
        return oneflow.typename(submodules[node.target])
...
class FxNetAccFusionsFinder:
    ...
    def __init__(self, module: oneflow.fx.GraphModule, acc_nodes: NodeSet):
        self.module = module
    ...
    def __call__(self) -> Dict[oneflow.fx.Node, NodeSet]:
        result: Dict[oneflow.fx.Node, NodeSet] = {}
