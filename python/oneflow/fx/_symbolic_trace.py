            ...
            if action == "c_call":
                if c_patch_enabled:
                    if arg in patched_fns:
                        oneflow._C._fx.patch_function(arg, patched_in)
...
class Tracer(TracerBase):
    ...
    def is_leaf_module(self, m: oneflow.nn.Module, module_qualified_name: str) -> bool:
        ...
        """
        return m.__module__.startswith("oneflow.nn") and not isinstance(
            m, oneflow.nn.Sequential
        )
    ...
    def create_args_for_root(self, root_fn, is_module, concrete_args=None):
        ...
        if any(not isinstance(i, pytree.LeafSpec) for i in in_spec.children_specs):
            ...
            def flatten_fn(*args):
                ...
                self.graph._pytree_info = self.graph._pytree_info._replace(
                    out_spec=out_spec
                )
                return out_args
...
def wrap(fn_or_name: Union[str, Callable]):
    ...
    traced through::
        ...
        oneflow.fx.wrap('my_custom_function')
