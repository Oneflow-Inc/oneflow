...
# def reduce_graph_module(body: Dict[Any, Any], import_block: str) -> oneflow.nn.Module:
...
#     forward, body: Dict[Any, Any], importer: Optional[PackageImporter]
# ) -> oneflow.nn.Module:
#     """
...
#     class CodeOnlyModule(oneflow.nn.Module):
...
#         def is_leaf_module(self, _: oneflow.nn.Module, __: str) -> bool:
#             return True
...
"""
        ...
        for param_name, param in self._parameters.items():
            model_str += f"{tab*2}self.{param_name} = oneflow.nn.Parameter(oneflow.empty({list(buffer.shape)}))\n"
...
# patched_cat.__module__ = 'oneflow'
# patched_cat.__name__ = 'cat'
# oneflow.cat = patched_cat
