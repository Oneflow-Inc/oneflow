...
import oneflow
from oneflow.fx.experimental.param_fetch import lift_lowering_attrs_to_nodes
from oneflow.fx.graph import Graph
from oneflow.fx.graph_module import GraphModule
from oneflow.fx.node import Node, Target, Argument, map_arg, map_aggregate
...
def serialize_shape(shape: oneflow.Size) -> str:
...
def serialize_tensor_quantization(
    tensor: oneflow.Tensor, weights: Dict, pcq_prefix: str
) -> Tuple[Dict, Dict]:
    """
    Args:
    ...
    if tensor.qscheme() in {oneflow.per_tensor_affine, oneflow.per_tensor_symmetric}:
...
def serialize_weight(tensor: oneflow.Tensor, weights: Dict, name: str) -> Dict:
...
def serialize_leaf_module(
    ...
    for p_name, p_value in node.attrs_for_lowering.items():  # type: ignore[attr-defined]
        if isinstance(p_value, oneflow.Tensor):
...
def serialize_module(fx_module: GraphModule, weights: Dict, name_prefix="") -> Dict:
    ...
    def add_weight_tensors(named_tensors):
        for name, p in named_tensors:
            if name.startswith("parent.") or not isinstance(p, oneflow.Tensor):
                continue
    ...
    for node in fx_module.graph.nodes:
        ...
        if node.op == "call_module":
            if isinstance(submodules[node.target], GraphModule):
                ...
                node_rep["parameters"] = serialize_leaf_module(
                    node, serialized_dict["weights"], weights, prefix + node.target,
                )
        ...
        if node.op == "get_attr":
            ...
            if node.target.startswith("parent."):
                ...
                submod: Optional[oneflow.nn.Module] = (
                ...
                if isinstance(target, oneflow.Tensor) and qualname not in weights:
        ...
        def get_arg_info(arg: Argument) -> Any:
            ...
            elif isinstance(arg, oneflow.dtype):
                return str(arg)
            else:
                return arg
        ...
        if node.op == "output":
            node_rep["args"] = map_arg(node.args, get_output_arg_info,)
