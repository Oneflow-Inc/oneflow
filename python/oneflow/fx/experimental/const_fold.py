...
class FoldedGraphModule(oneflow.fx.GraphModule):
    """
    ...
    def __init__(
        ...
        const_subgraph: Optional[oneflow.fx.Graph] = None,
        ...
        self.const_subgraph_module = (
            ...
            else oneflow.fx.GraphModule(root, const_subgraph)
        )
    ...
    def run_folding(self):
        ...
        for i in range(len(folded_attrs)):
            setattr(
                self, self.const_output_names[i], oneflow.nn.Parameter(folded_attrs[i])
            )


def split_const_subgraphs(module: oneflow.nn.Module,) -> FoldedGraphModule:
    """
    ...
    graph.
    """
    mod_traced = oneflow.fx.symbolic_trace(module)
    ...
    const_nodes: Set[oneflow.fx.Node] = set()
    ...
    def mod_partition(node: oneflow.fx.Node):
    ...
    for node in split.submod_1.graph.nodes:
        ...
        if is_folded_attr:
            ...
            setattr(
                mod_traced, const_output_name, oneflow.nn.Parameter(oneflow.randn(1)),
            )
