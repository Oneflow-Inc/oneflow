...
from oneflow.fx.node import Argument, Target
from oneflow.nn.utils.fusion import fuse_conv_bn_eval
from typing import Type, Dict, Any, Tuple, Iterable, Optional, List, cast
import oneflow
import oneflow.nn as nn
import oneflow.nn.functional as F
from oneflow.fx.passes.shape_prop import ShapeProp
import copy
from collections import defaultdict
import oneflow.utils.mkldnn as th_mkldnn
import operator
import time
import logging
...
def replace_node_module(
    node: fx.Node, modules: Dict[str, Any], new_module: oneflow.nn.Module
):
...
def fuse(model: oneflow.nn.Module, inplace=False) -> oneflow.nn.Module:
    """
...
def remove_dropout(model: nn.Module) -> nn.Module:
    ...
    class DropoutRemover(oneflow.fx.Transformer):
        def call_module(
    ...
    for node in nodes:
        if node.op == "call_module":
            ...
            if type(cur_module) in mkldnn_map:
                new_module = mkldnn_map[type(cur_module)](cur_module, oneflow.float)
...
def gen_mkl_autotuner(example_inputs, iters=10, warmup=1):
    ...
    def use_mkl_heuristic(graph: MklSubgraph) -> bool:
        ...
        sample_inputs = [oneflow.randn(node.shape) for node in input_nodes]  # type: ignore[attr-defined]
        ...
        if node.op == "call_module":
            ...
            if type(cur_module) in mkldnn_supported:
                ...
                if sample_parameter is not None:
                    assert (
                        sample_parameter.dtype == oneflow.float
                    ), "this pass is only for oneflow.float modules"
                    assert sample_parameter.device == oneflow.device(
                        "cpu"
