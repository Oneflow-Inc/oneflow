...
from oneflow.fx.graph import Graph
from oneflow.fx.graph_module import GraphModule
from oneflow.fx.node import Node
from oneflow.fx._symbolic_trace import symbolic_trace
...
def get_first_dim(t: oneflow.Tensor) -> int:
    """
    A free function primarily for use in the merge_matmul graph transformation below
    that returns the first dimension of a Tensor. This is necessary because oneflow.Tensor.shape
...
def may_depend_on(a: Node, b: Node, search_depth: int = 6):
    """
    Determine if one node depends on another in a oneflow.fx.Graph.
...
def merge_matmul(in_mod: oneflow.nn.Module):
    """
    ...
    for node in gm.graph.nodes:
        if node.op != "call_function" or node.target is not oneflow.matmul:
            continue
    ...
    for rhs, mms in rhs_users.items():
        ...
        merge_mm_cat = gm.graph.call_function(oneflow.cat, (lhs,), {})
        ...
        merge_mm = gm.graph.call_function(oneflow.matmul, (merge_mm_cat, rhs,), {})

        # Split the result of the merged matmul using the shapes of the LHS operands
        # to ascertain how large each chunk should be.
        merge_mm_sizes = [gm.graph.call_function(get_first_dim, (l,), {}) for l in lhs]
        merge_mm_split = gm.graph.call_function(
            oneflow.split, (merge_mm, merge_mm_sizes), {}
        )
        merge_mm_res = [
