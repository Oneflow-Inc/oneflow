...
def replace_pattern(gm: GraphModule, pattern: Callable, replacement: Callable) -> List[Match]:
    ...
    .. code-block:: python
        ...
        class M(flow.nn.Module):
            ...
            def forward(self, x, w1, w2):
                m1 = flow.cat([w1, w2]).sum()
                m2 = flow.cat([w1, w2]).sum()
                return x + flow.max(m1) + flow.max(m2)

        def pattern(w1, w2):
            return flow.cat([w1, w2]).sum()
    ...
    ``p = flow.cat([a, b])`` in ``pattern``, you could match
    ``m = flow.cat([a, b])`` in the original ``forward`` function,
    ...
    .. code-block:: python
        ...
        def forward(self, x, w1, w2):
            stack_1 = flow.stack([w1, w2])
            sum_1 = stack_1.sum()
            stack_2 = flow.stack([w1, w2])
            sum_2 = stack_2.sum()
            max_1 = flow.max(sum_1)
            add_1 = x + max_1
            max_2 = flow.max(sum_2)
            add_2 = add_1 + max_2
            return add_2
