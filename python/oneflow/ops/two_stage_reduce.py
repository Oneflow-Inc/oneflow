...
def api_two_stage_reduce_max(x: oneflow._oneflow_internal.BlobDesc, axis: Optional[Union[int, Sequence[int]]]=None, keepdims: bool=False, name: Optional[str]=None) -> oneflow._oneflow_internal.BlobDesc:
...
def api_two_stage_reduce_min(x: oneflow._oneflow_internal.BlobDesc, axis: Optional[Union[int, Sequence[int]]]=None, keepdims: bool=False, name: Optional[str]=None) -> oneflow._oneflow_internal.BlobDesc:
...
def two_stage_reduce(x, axis=None, keepdims=False, op_type_name=None, name=None):
    ...
    parallel_id = 0
    for (machine_id, device_ids) in parallel_desc_symbol.machine_id2device_id_list.items():
        for device_id in device_ids:
            with flow.scope.placement(device_tag, '@' + str(machine_id) + ':' + str(device_id)):
                (device_stage_out, device_stage_count) = reduce_device_stage(x_list[parallel_id], axis, op_type_name + '_device_stage', name + '_device_stage' + str(parallel_id))
...
def reduce_device_stage(x, axis, op_name, name):
    (out, mask, count) = flow.user_op_builder(name).Op(op_name).Input('in', [x]).Output('out').Output('mask').Output('count').Attr('axis', axis).Build().InferAndTryRun().RemoteBlobList()
    return (out, count)

def reduce_global_stage(x, device_count, axis, keepdims, op_name, name):
    (out, mask) = flow.user_op_builder(name).Op(op_name).Input('in', [x]).Input('device_count', [device_count]).Output('out').Output('mask').Attr('axis', axis).Attr('keepdims', keepdims).Build().InferAndTryRun().RemoteBlobList()
    return out
