...
class SGD(Optimizer):
    ...
    def __init__(self, parameters: Union[Iterator[Parameter], List[Dict]], lr: float=0.001, momentum: float=0.0, scale: float=1.0):
        super().__init__()
        ...
        self._momentum_sgd = flow.builtin_op('momentum_update').Input('model').Input('model_diff').Input('momentum').Attr('l1', 0.0).Attr('l2', 0.0).Attr('weight_decay', 0.0).Build()
        self._sgd = flow.builtin_op('sgd_update').Input('model').Input('model_diff').Attr('weight_decay', 0.0).Attr('l1', 0.0).Attr('l2', 0.0).Build()
            ...
            for param_group in self.param_groups:
                ...
                for param in param_group.parameters:
                    ...
                    if param_group['momentum'] == 0.0:
                        ...
                        self._momentum_sgd(param, param.grad, momentum_buf, learning_rate_val=lr, scale=scale, beta=beta)
    ...
    def add_to_graph_train_config(self, train_conf, var2var_op_name_dict):
        for param_group in self.param_groups:
            ...
            assert math.isclose(base_scale, 1, rel_tol=0.0001) or math.isclose(scale, base_scale, rel_tol=0.0001), 'nn.Graph only support one scale factor at the moment, base_scale {} vs scale {}'.format(base_scale, scale)
