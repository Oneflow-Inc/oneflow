    ...
    def _single_tensor_update(self, param_group):
        for param in param_group.parameters:
            ...
            flow._C.dispatch_radam_update(
                ...
                bias_correction2_numerator=param_group["bias_correction2_numerator"],
            )
        ...
        with flow.no_grad():
            ...
            for param_group in self.param_groups:
                if param_group["do_bias_correction"]:
                    ...
                    param_group["bias_correction2_numerator"] = (
                        2
                        * (self._state["step"] + 1)
                        * math.pow(param_group["betas"][1], self._state["step"] + 1)
                    )
                else:
                    param_group["bias_correction1"] = 1.0
                    param_group["bias_correction2"] = 1.0
                    param_group["bias_correction2_numerator"] = 1.0
