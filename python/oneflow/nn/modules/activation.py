...
from oneflow.nn.modules.linear import NonDynamicallyQuantizableLinear
...
# ref from https://github.com/pytorch/pytorch/blob/v1.10.1/torch/nn/modules/activation.py#L852-L1014
class MultiheadAttention(Module):
    ...
    def __init__(
        ...
        if bias:
            self.in_proj_bias = Parameter(flow.empty(3 * embed_dim, **factory_kwargs))
        else:
        ...
        if add_bias_kv:
            self.bias_k = Parameter(flow.empty((1, 1, embed_dim), **factory_kwargs))
            self.bias_v = Parameter(flow.empty((1, 1, embed_dim), **factory_kwargs))
        else:
    ...
    Outputs:
        ...
        if not self._qkv_same_embed_dim:
            (
                attn_output,
                attn_output_weights,
            ) = flow.nn.functional.multi_head_attention_forward(
                query,
                key,
                value,
                self.embed_dim,
