...
class MultiheadAttention(Module):
    ...
    The interface is consistent with PyTorch.
    The documentation is referenced from https://pytorch.org/docs/stable/generated/torch.nn.MultiheadAttention.html.
    ...
    def __init__(
        ...
        if self._qkv_same_embed_dim is False:
            self.q_proj_weight = flow.nn.Parameter(flow.Tensor(embed_dim, embed_dim))
            self.k_proj_weight = flow.nn.Parameter(flow.Tensor(embed_dim, self.kdim))
            self.v_proj_weight = flow.nn.Parameter(flow.Tensor(embed_dim, self.vdim))
            self.register_parameter("in_proj_weight", None)
        else:
            self.in_proj_weight = flow.nn.Parameter(
                flow.empty(3 * embed_dim, embed_dim)
            )
        ...
        if bias:
            self.in_proj_bias = flow.nn.Parameter(flow.empty(3 * embed_dim))
        else:
            self.register_parameter("in_proj_bias", None)
        self.out_proj = flow.nn.Linear(embed_dim, embed_dim, bias=bias)

        if add_bias_kv:
            self.bias_k = flow.nn.Parameter(flow.empty(1, 1, embed_dim))
            self.bias_v = flow.nn.Parameter(flow.empty(1, 1, embed_dim))
        else:
    ...
    Shapes for outputs:
        ...
        if not self._qkv_same_embed_dim:
            return flow.nn.functional.multi_head_attention_forward(
                query,
                key,
                value,
                self.embed_dim,
