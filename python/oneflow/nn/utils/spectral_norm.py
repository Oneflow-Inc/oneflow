...
class SpectralNorm:
    ...
    def reshape_weight_to_matrix(self, weight: flow.Tensor) -> flow.Tensor:
        weight_mat = weight
        if self.dim != 0:
    ...
    def compute_weight(self, module: Module, do_power_iteration: bool) -> flow.Tensor:
        ...
        if do_power_iteration:
            with flow.no_grad():
                for _ in range(self.n_power_iterations):
                    ...
                    v = normalize(
                        flow.mv(weight_mat.t(), u), dim=0, eps=self.eps, out=v
                    )
                    u = normalize(flow.mv(weight_mat, v), dim=0, eps=self.eps, out=u)
                if self.n_power_iterations > 0:
                    # See above on why we need to clone
                    u = u.clone().contiguous()
...
def spectral_norm(
    ...
    .. note::
        ...
        :func:`flow.nn.utils.parametrizations.spectral_norm` using the new
        parametrization functionality in
        :func:`flow.nn.utils.parametrize.register_parametrization`. Please use
