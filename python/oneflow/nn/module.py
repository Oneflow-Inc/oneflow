...
class Module(object):
    ...
    def register_buffer(self, name: str, tensor: Optional[Tensor], persistent: bool=True) -> None:
        if '_buffers' not in self.__dict__:
            ...
            raise TypeError("cannot assign '{}' object to buffer '{}' (Tensor or None required)".format(type(tensor), name))
        else:
    ...
    def register_parameter(self, name: str, param: Optional[Parameter]) -> None:
        ...
        if param is None:
            ...
            raise TypeError("cannot assign '{}' object to parameter '{}' (nn.Parameter or None required)".format(type(param), name))
        else:
    ...
    def __setattr__(self, name: str, value: Union[Tensor, 'Module']) -> None:
        ...
        if isinstance(value, Parameter):
            ...
            if value is not None:
                raise TypeError("cannot assign '{}' as parameter '{}' (nn.Parameter or None expected)".format(type(value), name))
            self.register_parameter(name, value)
        else:
            ...
            if isinstance(value, Module):
                ...
                if value is not None:
                    raise TypeError("cannot assign '{}' as child module '{}' (nn.Module or None expected)".format(type(value), name))
                modules[name] = value
            else:
                buffers = self.__dict__.get('_buffers')
                if buffers is not None and name in buffers:
                    if value is not None and (not isinstance(value, Tensor)):
                        raise TypeError("cannot assign '{}' as buffer '{}' (Tensor or None expected)".format(type(value), name))
                    buffers[name] = value
                else:
        ...
        for (name, param) in local_state.items():
            ...
            if key in state_dict:
                ...
                try:
                    ...
                    error_msgs.append('While copying the parameter named "{}", whose dimensions in the model are {} and whose dimensions in the checkpoint are {}, an exception occurred : {}.'.format(key, param.shape, input_param.shape, ex.args))
            elif strict:
